...
function UpdateSetOption($name, $value)
{
	...
	$fn = $_SERVER['DOCUMENT_ROOT']."/thurly/managed_cache/".strtoupper($DBType)."/e5/".md5("b_option").".php";
...
function UpdateGetHTTPPage($requestDataAdd, &$errorMessage)
{
	...
	if ($FP)
	{
		...
		if ($bUseProxy)
		{
			$requestString .= "POST http://".$serverIP."/thurly/updates/us_updater_actions.php HTTP/1.0\r\n";
			if (strlen($proxyUserName) > 0)
				$requestString .= "Proxy-Authorization: Basic ".base64_encode($proxyUserName.":".$proxyPassword)."\r\n";
		}
		else
			$requestString .= "POST /thurly/updates/us_updater_actions.php HTTP/1.0\r\n";


		$requestString .= "User-Agent: ThurlySMUpdater\r\n";
...
function UpdateActivateCoupon($coupon, &$errorMessage)
{
	...
	if (is_writable($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/admin/define.php"))
	{
		if ($fp = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/admin/define.php", 'w'))
		{
	...
	{
		if (is_writable($_SERVER['DOCUMENT_ROOT']."/thurly/license_key.php"))
		{
			if ($fp = fopen($_SERVER['DOCUMENT_ROOT']."/thurly/license_key.php", "wb"))
			{
...
<html>
	<head>
		...
		<link rel="stylesheet" type="text/css" href="/thurly/themes/.default/sysupdate.css">
		<link rel="stylesheet" type="text/css" href="/thurly/themes/.default/adminstyles.css">
	</head>
	<body>
	<?
	if (strlen($errorMessage) > 0)
	...
	?>
	<form method="POST" action="/thurly/coupon_activation.php">
