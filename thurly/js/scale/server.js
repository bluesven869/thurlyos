...
;(function(window) {
	...
	BX.Scale.Server = function (hostname, params)
	{
		...
		this.mustChangeThurlyUsrPass = params.LAST_PASSWORD_CHANGE && params.LAST_PASSWORD_CHANGE.search(/^password must be changed/) != -1;
		...
		if(params.roles !== undefined)
		{
			for(var i in BX.Scale.rolesList)
			{
				...
				rParams.noActions = this.bxEnvNeedUpdate && this.mustChangeThurlyUsrPass;
	...
	BX.Scale.Server.prototype.getDomObj = function()
	{
		if(!this.domObj)
		{
			...
			else if(BX.Scale.thurlyEnvType != 'crm')
			{
	...
	BX.Scale.Server.prototype.getAviableActionsList = function()
	{
		...
		if(!this.bxEnvNeedUpdate && !this.mustChangeThurlyUsrPass)
		{
		...
		else if(this.mustChangeThurlyUsrPass)
		{
			result["CHANGE_PASSWD_THURLY"] = true;
		}
