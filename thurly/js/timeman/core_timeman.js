...
var TMPoint = '/thurly/tools/timeman.php',
	intervals = {
		OPENED: 60000,
		CLOSED: 30000,
		EXPIRED: 30000,
...
BX.timeman_query = function(action, data, callback, bForce)
{
	...
	var query_data = {
		'method': 'POST',
		'dataType': 'json',
		'url': TMPoint + '?action=' + action + '&site_id=' + SITE_ID + '&sessid=' + BX.thurly_sessid(),
			...
			this.MAIN_ROW_CELL_BTN.appendChild(BX.create('span', {
				...
				events: {
					click: BX.proxy(function(){
						...
						{
							BX.loadScript(
								'/thurly/tools/oauth/socserv.ajax.php?action=getsettings&site_id=' + BX.message('SITE_ID') + '&sessid=' + BX.thurly_sessid()
							)
						}
						else
						{
...
BX.CTimeManClock.prototype.Show = function()
{
	if (!this.isReady)
	{
		...
		return BX.ajax.get(TMPoint, {action:'clock', start_time: this.params.start_time, clock_id: this.CLOCK_ID, sessid: BX.thurly_sessid()}, BX.delegate(this.Ready, this));
	}
...
BX.CTimeManTasksSelector.prototype.Show = function()
{
	if (!this.isReady)
	{
		...
		BX.timeman.showWait();
		return BX.ajax.get(TMPoint, {action:'tasks', suffix: suffix, sessid: BX.thurly_sessid()}, BX.delegate(this.Ready, this));
	}
...
BX.CTimeManTabEditorControl.prototype.InitLHE = function()
{
	...
	BX.ajax.get("/thurly/tools/timeman.php?action=editor&obname="+this.lhename+"_"+rand+"&sessid=" + BX.thurly_sessid(), function(data){
	...
	BX.addCustomEvent(window, 'LHE_OnInit', BX.proxy(function(data){
		if(data.id == this.lhename+"_"+rand)
		{
			if (!BX.CDialog)
			{
				arScripts = ["/thurly/js/main/core/core_window.js"];
				arCss = ["/thurly/js/main/core/css/core_window.css"];
				BX.loadCSS(arCss);
...
BX.CTimeManUploadForm.prototype.UploadFile = function()
{
	...
	var form = BX.create("form", {
		props : {
			method : "post",
			action : "/thurly/tools/timeman.php",
...
BX.CTimeManUploadForm.prototype.DeleteFile = function(e)
{
	...
	if (confirm(BX.message("JS_CORE_TM_CONFIRM_TO_DELETE"))) {
		if (!BX.hasClass(_this.parentNode, "saved"))
		{
			...
			var url = "/thurly/tools/timeman.php";
	...
	if (this.files && this.files.length>0)
	{
		...
		for(i=0;i<files.length;i++)
		{
			filelist.appendChild(
				...
				children : [
					BX.create("a", {
						props : { href : "/thurly/tools/timeman.php?action=get_attachment&fid="+files[i].fileID+"&report_id="+this.report_id+"&user_id="+this.user_id+"&sessid="+BX.thurly_sessid(), target : "_blank", className : "upload-file-name"},
						text : files[i].name
					}),
...
BX.CTimeManUploadForm.prototype.RefreshUpload = function(files,uniqueID)
{
	for(i = 0; i < files.length; i++)
	{
		...
		if (files[i].fileID)
		{
			...
			BX.adjust(elem.firstChild, {props : {href : "/thurly/tools/timeman.php?action=get_attachment&fid="+files[i].fileID+"&report_id="+this.report_id+"&user_id="+this.user_id+"&sessid="+BX.thurly_sessid()}});
