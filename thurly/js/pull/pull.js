...
;(function(window){
	...
	if (!window.BX)
	{
		if (typeof(console) == 'object') console.error('PULL notice: thurly core not loaded');
		return;
	}
	if (window.BX.PULL)
	{
	...
	_confirm = null,
	_beforeUnload = false,
	_pathToAjax = '/thurly/components/thurly/pull.request/ajax.php?',
	...
	BX.PULL.getChannelID = function(code, withoutCache, send)
	{
		...
		BX.ajax({
			...
			lsTimeout: 1,
			timeout: 30,
			data: {'PULL_GET_CHANNEL' : 'Y', 'SITE_ID': (BX.message.SITE_ID? BX.message('SITE_ID'): ''), 'MOBILE': _mobileMode? 'Y':'N', 'CACHE': withoutCache? 'N': 'Y', 'PULL_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				...
				else
				{
					...
					if (typeof(data) == 'object' && data.THURLY_SESSID)
					{
					...
					if (typeof(data) == 'object' && data.ERROR == 'SESSION_ERROR')
					{
						...
						BX.onCustomEvent(window, 'onPullError', [data.ERROR, data.THURLY_SESSID]);
					}
	...
	BX.PULL.connectPull = function(force)
	{
		...
		_updateStateTimeout = setTimeout(function(){
			...
			var _ajax = BX.ajax({
				...
				data: _pullMethod=='PULL'? {'PULL_UPDATE_STATE' : 'Y', 'CHANNEL_ID': _channelID, 'CHANNEL_LAST_ID': _channelLastID, 'SITE_ID': (BX.message.SITE_ID? BX.message('SITE_ID'): ''), 'PULL_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}: {},
		...
		_watchTimeout = setTimeout(function()
		{
			...
			if (arWatchTag.length > 0)
			{
				BX.ajax({
					...
					lsTimeout: 5,
					data: {'PULL_UPDATE_WATCH' : 'Y', 'WATCH' : arWatchTag, 'SITE_ID': (BX.message.SITE_ID? BX.message('SITE_ID'): ''), 'PULL_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
					onsuccess: BX.delegate(function(data)
					{
