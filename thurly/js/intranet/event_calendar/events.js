...
JCEC.prototype.BuildEventDiv = function(arAtr, oEvent)
{
	...
	{
		c = r.insertCell(-1);
		c.innerHTML = '<img class="bxec-iconkit" src="/thurly/images/1.gif">';
		c.className = 'bxec-event-ar-l';
	}
	else
	{
		var ddCell = r.insertCell(-1);
		ddCell.innerHTML = '<img class="bxec-iconkit" src="/thurly/images/1.gif">';
		ddCell.className = 'bxec-event-dd-dot';
	}

	 var
		bEnc = (oEvent.HOST || (oEvent.GUESTS && oEvent.GUESTS.length > 0)),
		encIcon = bEnc ? '<img class="bxec-iconkit bxec-enc-icon" src="/thurly/images/1.gif" align="top">' : '',
...
JCEC.prototype.BuildEventActions = function(P)
{
	...
	var pView = oDiv_.appendChild(BX.create('IMG', {props: {className : 'bxec-iconkit bxec-ev-view-icon', src: "/thurly/images/1.gif", title: EC_MESS.ViewEvent}}));
	pView.onclick = function(){_this.ShowViewEventDialog(oEvent);};

	if (!this.bReadOnly && !oEvent.bSuperposed)
	{
		var pEdit = oDiv_.appendChild(BX.create('IMG', {props: {className : 'bxec-iconkit bxec-ev-edit-icon', src: "/thurly/images/1.gif", title: EC_MESS.EditEvent}}));
		pEdit.onclick = function(){_this.ShowEditEventDialog({oEvent: oEvent});};
		count++;

		// Add del button
		var pDel = oDiv_.appendChild(BX.create('IMG', {props: {className : 'bxec-iconkit bxec-ev-del-icon', src: "/thurly/images/1.gif", title: oEvent.HOST ? EC_MESS.DelEncounter : EC_MESS.DelEvent}}));
