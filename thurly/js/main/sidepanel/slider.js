...
BX.SidePanel.Slider.prototype =
{
	...
	canChangeHistory: function()
	{
		return (
			...
			!this.getUrl().match(/^\/thurly\/(components|tools)\//i)
		);
	},
	...
	createLoader: function(loader)
	{
		...
		else if (matches = loader.match(/^([a-z0-9-_.]+):([a-z0-9-_.]+)$/i))
		{
			...
			var svg = "/thurly/images/" + moduleId + "/slider/" + svgName + ".svg";
	...
	firePageEvent: function(eventName)
	{
		...
		{
			BX.onCustomEvent("BX.ThurlyOS.PageSlider:" + eventName, [this]);
			BX.onCustomEvent("ThurlyOS.Slider:" + eventName, [this]);
		}
	...
	fireFrameEvent: function(eventName)
	{
		...
		if (frameWindow && frameWindow.BX)
		{
			...
			if (BX.util.in_array(eventName, ["onClose", "onOpen"]))
			{
				frameWindow.BX.onCustomEvent("BX.ThurlyOS.PageSlider:" + eventName, [this]);
				frameWindow.BX.onCustomEvent("ThurlyOS.Slider:" + eventName, [this]); // Compatibility
			}
		}
