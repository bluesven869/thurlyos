...
RatingLike.Vote = function(likeId, voteAction)
{
	BX.ajax({
		...
		method: 'POST',
		dataType: 'json',
		data: {'RATING_VOTE' : 'Y', 'RATING_VOTE_TYPE_ID' : BXRL[likeId].entityTypeId, 'RATING_VOTE_ENTITY_ID' : BXRL[likeId].entityId, 'RATING_VOTE_ACTION' : voteAction, 'sessid': BX.thurly_sessid()},
...
RatingLike.List = function(likeId, page)
{
	...
	BX.ajax({
		...
		method: 'POST',
		dataType: 'json',
		data: {'RATING_VOTE_LIST' : 'Y', 'RATING_VOTE_TYPE_ID' : BXRL[likeId].entityTypeId, 'RATING_VOTE_ENTITY_ID' : BXRL[likeId].entityId, 'RATING_VOTE_LIST_PAGE' : page, 'PATH_TO_USER_PROFILE' : BXRL[likeId].pathToUserProfile, 'sessid': BX.thurly_sessid()},
		onsuccess: function(data)
		{
			BXRL[likeId].countText.innerHTML = data.items_all;
			...
			for (var i = 0; i < data.items.length; i++)
			{
				...
				{
					avatarNode = BX.create("IMG", {
						attrs: {src: '/thurly/images/main/blank.gif'},
