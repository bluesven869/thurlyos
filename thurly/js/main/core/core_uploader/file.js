		...
		cnvConstr = (function(){
			...
			cnvConstructor.prototype = {
				...
				load : function(file, callback, id, callbackFail) {
					...
					onload event we got e.target.src of one element but source of image was from '/thurly/images/1.gif'. */
					// TODO check if chrome and other browsers cache local files for now. If it does not then delete next 2 strings
					if (!BX.browser.IsFirefox())
						image.src = '/thurly/images/1.gif';
					...
					this.onload = BX.delegate(function(e){
						if (e && e.target && e.target.src && e.target.src.substr(-20) == "/thurly/images/1.gif")
							return;
						if (!!callback)
						{
							try {
