	...
	var dateFormats = {
		...
		x : function() {
			...
			return BX.date.format([
				...
				["-", BX.date.convertThurlyFormat(BX.message("FORMAT_DATETIME")).replace(/:s/g, "")],
				["s", "sago"],
				["i", "iago"],
				["today", "today, "+ampm],
...
BX.date.convertThurlyFormat = function(format)
{
...
BX.calendar.InsertDaysBack = function(input, days)
{
	if (days != '')
	{
		...
		input.value = BX.date.format(BX.date.convertThurlyFormat(BX.message('FORMAT_DATE')), d, null);
	}
	else
	{
		input.value = '';
...
{
	return BX.date.format(
		BX.date.convertThurlyFormat(BX.message(bTime ? 'FORMAT_DATETIME' : 'FORMAT_DATE')),
		value,
		null,
		!!bUTC
	);
...
BX.calendar.ValueToStringFormat = function(value, thurlyFormat, bUTC)
{
	return BX.date.format(
		BX.date.convertThurlyFormat(thurlyFormat),
		value,
		null,
		!!bUTC
	);
...
BX.CClockSelector.prototype.Show = function()
{
	if (!this.isReady)
	{
		...
		return BX.ajax.get('/thurly/tools/clock_selector.php', {start_time: this.params.start_time, clock_id: this.CLOCK_ID, sessid: BX.thurly_sessid()}, BX.delegate(this.Ready, this));
	}
