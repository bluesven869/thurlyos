...
BX.Finder.OpenCompanyDepartment = function(provider, id, department)
{
	...
	if (!BX.Finder.loadPlace[id])
	{
		...
		if (BX.Finder.context == 'access')
		{
			ajaxSendUrl = '/thurly/tools/access_dialog.php';
		}
		else
		{
		...
		BX.ajax({
			...
			dataType: 'html',
			processData: true,
			data: {'mode': 'ajax', 'action' : 'structure-item', 'provider' : provider, 'item' : id, 'sessid': BX.thurly_sessid(), 'site_id': BX.message('SITE_ID')||''},
	...
	if (element.value != '')
	{
		BX.Finder.searchTimeout = setTimeout(function() {
			...
			BX.ajax({
				...
				dataType: 'html',
				processData: true,
				data: {'mode': 'ajax', 'action' : 'search', 'provider' : provider, 'search' : element.value, 'sessid': BX.thurly_sessid(), 'site_id': BX.message('SITE_ID')||''},
				onsuccess: function(data)	{
					if (data == '')
					{
