...
{
	(new BX.CAdminDialog({
		'content_url': '/thurly/components/thurly/desktop/admin_settings.php?lang='+BX.message('LANGUAGE_ID')+'&bxpublic=Y',
		'content_post': 'sessid='+BX.thurly_sessid()+'&type=desktop&desktop_page=0&action=new&desktop_backurl=/thurly/admin/',
		'draggable': true,
		'resizable': true,
...
BX.adminPanel.prototype.hideNotify = function(element)
{
	...
	if (BX.type.isDomNode(element) && element.getAttribute('data-ajax') == "Y")
	{
		...
		{
			BX.ajax({
				url: '/thurly/admin/admin_notify.php',
				method: 'POST',
				dataType: 'json',
				data: {'ID' : notifyId, 'sessid': BX.thurly_sessid()}
			});
		}
	}
...
BX.adminFav = {
	url: '/thurly/admin/favorite_act.php',
	lastId: null,
...
BX.adminTabControl.prototype.ShowWarnings = function(form_name, warnings)
{
	...
	for(var i in warnings)
	{
		...
		img.src = '/thurly/panel/main/images_old/icon_warn.gif';
...
BX.adminTabControl.prototype.SaveSettings =  function(el)
{
	...
	var options_url = '/thurly/admin/user_options.php?lang='+BX.message('LANGUAGE_ID')+'&sessid=' + BX.thurly_sessid();
			...
			for (var i = 0; i < this.node.rows[0].cells.length; i++)
			{
				this.mirror_thead.rows[0].cells[i].appendChild(
					BX.create('SPAN', {
						...
						html: '<img src="/thurly/images/1.gif" style="height: 0; width: ' + (this.node.rows[0].cells[i].offsetWidth-4) + 'px;">'
					}));
...
BX.AdminFilter = function(filter_id, aRows)
{
	...
	this.DeleteFromBase = function(id)
	{
		...
		BX.ajax.post('/thurly/admin/filter_act.php', data, callback);
	...
	this.SaveOpenTab = function(id)
	{
		...
		BX.ajax.post('/thurly/admin/filter_act.php', data);
	};
