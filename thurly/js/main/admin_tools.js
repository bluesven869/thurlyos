...
function JCAdminMenu(sOpenedSections)
{
	...
	this.ToggleDynSection = function(cell, module_id, div_id, level)
	{
		...
		if(div.innerHTML == '')
		{
			...
			this.request.Send('/thurly/admin/get_menu.php?lang='+phpVars.LANGUAGE_ID+'&admin_mnu_module_id='+module_id+'&admin_mnu_menu_id='+encodeURIComponent(div_id));
		}
...
function TabControl(name, unique_name, aTabs)
{
	...
	this.ShowWarnings = function(form_name, warnings)
	{
		...
		for(var i in warnings)
		{
			...
			img.src = '/thurly/themes/'+phpVars.ADMIN_THEME_ID+'/images/icon_warn.gif';
	...
	this.SaveSettings =  function()
	{
		...
		var options_url = '/thurly/admin/user_options.php?lang='+phpVars.LANGUAGE_ID+'&sessid='+phpVars.thurly_sessid;
	...
	this.DisableSettings = function()
	{
		...
		request.Send('/thurly/admin/user_options.php?lang=' + phpVars.LANGUAGE_ID + sParam + '&sessid='+phpVars.thurly_sessid);
	}
...
function JCUserOptions()
{
	...
	this.SaveOption = function(sCategory, sName, sValName, sVal, bCommon)
	{
		...
		if(sParam != '')
			document.cookie = phpVars.cookiePrefix+"_LAST_SETTINGS=" + sParam + "&sessid="+phpVars.thurly_sessid+"; expires=Thu, 31 Dec 2020 23:59:59 GMT; path=/;";
	...
	this.SendData = function(callback)
	{
		...
		if(sParam != '')
		{
			...
			_this.request.Send('/thurly/admin/user_options.php?'+sParam+'&sessid='+phpVars.thurly_sessid);
		}
	}
	...
	this.DeleteOption = function(sCategory, sName, bCommon, callback)
	{
		...
		_this.request.Action = callback;
		_this.request.Send('/thurly/admin/user_options.php?action=delete&c='+sCategory+'&n='+sName+(bCommon == true? '&common=Y':'')+'&sessid='+phpVars.thurly_sessid);
	}
}
...
function JCPanel()
{
	...
	this.ShowOn = function()
	{
		...
		if(bIE)
		{
			...
			{
				document.body.style.backgroundImage = "url(/thurly/images/1.gif)";
...
var WizardWindow = {
	...
	Open : function(wizardName, sessid)
	{
		...
		this.currentDialog.SetContent('<iframe class="content" style="background-color: transparent; height:400px;" allowtransparency="true" scrolling="no" id="wizard_iframe_' + iframeID + '" width="100%" src="/thurly/admin/wizard_install.php?lang='+phpVars.LANGUAGE_ID+'&wizardName='+wizardName+'&bxsender=admin_wizard_dialog&sessid='+sessid+'" frameborder="0"></iframe>');
		this.currentDialog.Show();
	...
	ShowWaitWindow : function()
	{
		if (this.currentDialog && BX.type.isDomNode(this.currentDialog.PARTS.CONTENT))
		{
			...
			waiter.style.background = "#fff url(/thurly/panel/main/images/waiter-white.gif) 3px center no-repeat";
...
function JCStartMenu()
{
	...
	this.ShowStartMenu = function(button, back_url)
	{
		...
		if(!menuStart || !menuStart.menuItems)
		{
			...
			request.Send('/thurly/admin/get_start_menu.php?lang='+phpVars.LANGUAGE_ID+(back_url? '&back_url_pub='+encodeURIComponent(back_url):'')+'&sessid='+phpVars.thurly_sessid);
		}
		else
		{
	...
	this.OpenDynMenu = function(menu, module_id, items_id, back_url)
	{
		...
		request.Send('/thurly/admin/get_start_menu.php?mode=dynamic&lang='+phpVars.LANGUAGE_ID+'&admin_mnu_module_id='+encodeURIComponent(module_id)+'&admin_mnu_menu_id='+encodeURIComponent(items_id)+(back_url? '&back_url_pub='+encodeURIComponent(back_url):'')+'&sessid='+phpVars.thurly_sessid);
	}
	...
	this.OpenURL = function(item, arguments, url, back_url)
	{
		...
		{
			request.Action = function(result){}
			request.Send('/thurly/admin/get_start_menu.php?mode=save_recent&url='+encodeURIComponent(url)+'&text='+encodeURIComponent(itemInfo['TEXT'])+'&title='+encodeURIComponent(itemInfo['TITLE'])+'&icon='+itemInfo['ICON']+'&sessid='+phpVars.thurly_sessid);
		}
		if(back_url)
