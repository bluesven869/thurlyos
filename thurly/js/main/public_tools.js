...
JCComponentUtils.prototype.ClearCache = function(params)
{
	...
	ShowWaitWindow();
	CHttpRequest.Send('/thurly/admin/clear_component_cache.php?' + params);
};
...
JCComponentUtils.prototype.EnableComponent = function(params)
{
	CHttpRequest.Action = function(result){window.location = window.location.href;};
	ShowWaitWindow();
	CHttpRequest.Send('/thurly/admin/enable_component.php?' + params);
};
...
var jsExtLoader = {
	...
	{
		if (/\/thurly\/js\/main\/public_tools.js$/i.test(script_src)) return; // sorry guys, i cannot execute myself :-)
		if (jsExtLoader.__isScriptLoaded(script_src)) return;

		jsExtLoader.arLoadedScripts.push(script_src);

		if (script_src.substring(0, 8) != '/thurly/')
			script_src = '/thurly/admin/' + script_src;
