...
if(!BX.Disk.pathToUser)
{
	BX.Disk = (function ()
	{
		...
		return	{
			...
			{
				response = response || {};
				var dontCloseCurrentPopupWindow = response.dontCloseCurrentPopupWindow || false;
				if (!response.message) {
				...
				if(currentPopup)
				{
					if(!dontCloseCurrentPopupWindow|| !currentPopup.isShown() || currentPopup.uniquePopupId === 'bx-disk-status-action')
					{
						currentPopup.destroy();
					}
					else
					{
						//we have another window and we can't close it
						return;
					}
				}
			...
			showActionModal: function (params)
			{
				...
				if(params.showLoaderIcon) {
					iconSrc = '/thurly/js/main/core/images/yell-waiter.gif';
				}
				else if(params.showSuccessIcon) {
					iconSrc = '/thurly/js/main/core/images/viewer-tick.png';
				}
				else if(!!params.icon)
				{
					iconSrc = params.icon;
			...
			{
				return (BX.browser.IsMac()? "http://dl.thurlyos.com/b24/thurlyos_desktop.dmg": "http://dl.thurlyos.com/b24/thurlyos_desktop.exe");
			},

			getUrlToShowObjectInGrid: function (objectId, cmd)
			{
				return '/thurly/tools/disk/focus.php?ncc=1&action=showObjectInGrid&objectId='
					+ encodeURIComponent(objectId)
					+ '&SITE_ID=' + encodeURIComponent(BX.message('SITE_ID'))
					+ (cmd? '&cmd=' + encodeURIComponent(cmd) : '')
				;
			},
			...
			appendSystemRight: function (params) {
				...
				var isThurlyOS = params.isThurlyOS || false;
				var entityId = params.item.id;
				var entityName = params.item.name;
				var entityAvatar = params.item.avatar;
				var type = params.type;
				...
				if(isThurlyOS && entityId && entityId != "G2" && entityId.search('G') == 0)
				{
					return;
				}
			...
			openWindowForSelectDocumentService: function (params) {
				...
				var imageSrc = '/thurly/images/disk/disk_description' + suffix + '_en.png';
				if(lang == 'kz')
					lang = 'ru';
				switch(lang)
				{
					...
					case 'sc':
					case 'tc':
						imageSrc = '/thurly/images/disk/disk_description' + suffix + '_' + lang + '.png';
						break;
				}
