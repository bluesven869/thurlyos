...
;(function (window)
{
	...
	Desktop.prototype.login = function (callback)
	{
		...
		if (typeof(callback)=='function')
		{
			params.success = BX.delegate(function(sessid) {
				...
				{
					BX.message({'thurly_sessid': sessid});
				}
				callback(sessid);
	...
	Desktop.prototype.logout = function (terminate, reason, skipCheck)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'IM_DESKTOP_LOGOUT' : 'Y', 'sessid': BX.thurly_sessid()},
	...
	Desktop.prototype.checkUpdate = function (openBrowser)
	{
		...
		if (!openBrowser && this.enableInVersion(16))
			...
			this.browse(BX.browser.IsMac()? "http://dl.thurlyos.com/b24/thurlyos_desktop.dmg": "http://dl.thurlyos.com/b24/thurlyos_desktop.exe", "desktopApp");
	...
	Desktop.prototype.diskAttachStatus = function()
	{
		...
		return ThurlyDisk? ThurlyDisk.enabled: false;
	};
