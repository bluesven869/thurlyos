;(function (window)
{
	...
	MessengerCommon.prototype.muteMessageChat = function(dialogId, mute, sendAjax)
	{
		...
		if (sendAjax)
		{
			BX.ajax({
				...
				dataType: 'json',
				timeout: 60,
				data: {'IM_CHAT_MUTE' : 'Y', 'CHAT_ID': chatId, 'MUTE': this.BXIM.messenger.userChatBlockStatus[chatId][this.BXIM.userId]? 'Y':'N', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
			});
		}
	};
	...
	MessengerCommon.prototype.getDateFormatType = function(type)
	{
		...
		if (type == 'MESSAGE_TITLE')
		{
			format = [
				...
				["today", "today"],
				["yesterday", "yesterday"],
				["", BX.date.convertThurlyFormat(BX.message("IM_M_MESSAGE_TITLE_FORMAT_DATE"))]
			];
		}
		...
		else if (type == 'RECENT_TITLE')
		{
			format = [
				...
				["today", "today"],
				["yesterday", "yesterday"],
				["", BX.date.convertThurlyFormat(BX.message("IM_CL_RESENT_FORMAT_DATE"))]
			]
		}
		...
		else
		{
			format = [
				...
				["", BX.date.convertThurlyFormat(BX.message("FORMAT_DATETIME"))]
			];
		}
		return format;
	}
	...
	{
		return "/thurly/js/im/images/default-avatar-"+type+".png";
	};
	...
	MessengerCommon.prototype.getUserParam = function(userId, reset)
	{
		...
		else
		{
			if (reset || !(this.BXIM.messenger.users[userId] && this.BXIM.messenger.users[userId].id))
			{
				...
				this.BXIM.messenger.hrphoto[userId] = '/thurly/js/im/images/hidef-avatar-v3.png';
				if (reset)
				{
	...
	MessengerCommon.prototype.getUserLastDate = function(userData)
	{
		...
		else if (userData.absent)
		{
			...
			text = BX.message('IM_STATUS_VACATION_TITLE').replace('#DATE#',
				BX.date.format(BX.date.convertThurlyFormat(BX.message("FORMAT_DATE")), userData.absent.getTime()/1000)
			);
	...
	MessengerCommon.prototype.isIntranet = function() {return this.BXIM.thurlyIntranet;};
	...
	MessengerCommon.prototype.setColor = function(color, chatId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'IM_SET_COLOR' : 'Y', 'COLOR' : color, 'CHAT_ID': chatId, 'sessid': BX.thurly_sessid()},
	...
	MessengerCommon.prototype.renameChat = function(chatId, title)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'IM_CHAT_RENAME' : 'Y', 'CHAT_ID' : chatId, 'CHAT_TITLE': title, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data){
				if (data.ERROR)
				{
	...
	MessengerCommon.prototype.contactListGetFromServer = function(onSuccess)
	{
		...
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 30,
			data: {'IM_CONTACT_LIST' : 'Y', 'IM_AJAX_CALL' : 'Y', 'DESKTOP' : (this.isDesktop()? 'Y': 'N'), 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
	...
	MessengerCommon.prototype.contactListRealSearch = function(text, callback)
	{
		...
		this.BXIM.messenger.contactListSearchTimeout = setTimeout(BX.delegate(function(){
			...
			BX.ajax({
				...
				dataType: 'json',
				timeout: 30,
				data: {'IM_CONTACT_LIST_SEARCH' : 'Y', 'SEARCH' : this.contactListRealSearchText, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data){
	...
	MessengerCommon.prototype.recentListHide = function(dialogId, sendAjax)
	{
		...
		if (sendAjax)
		{
			BX.ajax({
				...
				dataType: 'json',
				timeout: 60,
				data: {'IM_RECENT_HIDE' : 'Y', 'DIALOG_ID' : dialogId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
			});
		}
	...
	MessengerCommon.prototype.recentListGetFromServer = function()
	{
		...
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 30,
			data: {'IM_RECENT_LIST' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				if (data && data.THURLY_SESSID)
				{
	...
	MessengerCommon.prototype.chatListPrepare = function(params)
	{
		...
		var showThurlyOSSearch = activeSearch && searchText.length >= 3 && this.BXIM.messenger.realSearchAvailable && !this.BXIM.messenger.realSearch && !viewOnlyIntranet;
		...
		else if (activeSearch)
		{
			...
			category.push({'id': 'structure', 'name': this.BXIM.thurlyIntranet? BX.message('IM_CTL_CHAT_STRUCTURE'): BX.message('IM_CL_GROUP'), 'title': '', 'more': this.BXIM.thurlyIntranet? BX.message('IM_CL_MORE_STRUCTURE'): BX.message('IM_CL_MORE_GROUP'), skip: !showStructureBlock});
		...
		else
		{
			...
			category.push({'id': 'ol', 'name': BX.message('IM_CTL_CHAT_OL'), 'title': '', 'more': BX.message('IM_CTL_CHAT_OL'), skip: this.BXIM.userExtranet});
			category.push({'id': 'extranet', 'name': BX.message('IM_CTL_CHAT_EXTRANET'), 'title': BX.message('IM_CL_CREATE_PRIVATE'), 'more': BX.message('IM_CL_MORE_EXTRANET')});
			category.push({'id': 'structure', 'name': this.BXIM.thurlyIntranet? BX.message('IM_CTL_CHAT_STRUCTURE'): BX.message('IM_CTL_CHAT_GROUP'), 'title': '', 'more': this.BXIM.thurlyIntranet? BX.message('IM_CL_MORE_STRUCTURE'): BX.message('IM_CL_MORE_GROUP'), skip: !showStructureBlock});
			category.push({'id': 'blocked', 'name': BX.message('IM_CTL_CHAT_BLOCKED'), 'title': '', 'more': BX.message('IM_CL_MORE_EXTRANET')});
		}
		...
		for (var i = 0; i < category.length; i++)
		{
			...
			{
				if (!showThurlyOSSearch || category[i].id != 'extranet')
				{
					continue;
				}
			}

			if (groupElements[i].length <= 0 && !(category[i].id == 'private' || category[i].id == 'open' || category[i].id == 'chat' || showThurlyOSSearch && category[i].id == 'extranet'))
				continue;
			...
			if (groupElements[i].length <= 0)
			{
				if (showThurlyOSSearch && category[i].id == 'extranet')
				{
					chatList.appendChild(BX.create("div", {props : { className: "bx-messenger-chatlist-search-button-wrap"}, children : [
						BX.create("span", {props : { className: "bx-messenger-chatlist-search-button"}, html: this.BXIM.thurlyIntranet? BX.message('IM_SEARCH_B24'): BX.message('IM_SEARCH_SITE')})
					]}));
				}
	...
	MessengerCommon.prototype.joinParentChat = function(messageId, chatId)
	{
		...
		else
		{
			...
			BX.ajax({
				...
				skipAuthCheck: true,
				timeout: 60,
				data: {'IM_PARENT_CHAT_JOIN' : 'Y', 'CHAT_ID' : chatId, 'MESSAGE_ID' : messageId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
	...
	MessengerCommon.prototype.sendWriting = function(dialogId)
	{
		...
		if (!this.BXIM.messenger.writingSendList[dialogId])
		{
			...
			BX.ajax({
				...
				dataType: 'json',
				timeout: 30,
				data: {'IM_START_WRITING' : 'Y', 'DIALOG_ID' : dialogId, 'OL_SILENT' : olSilentMode, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data)
				{
					if (data && data.THURLY_SESSID)
					{
	...
	MessengerCommon.prototype.leaveFromChat = function(chatId, sendAjax)
	{
		...
		else
		{
			BX.ajax({
				...
				dataType: 'json',
				timeout: 60,
				data: {'IM_CHAT_LEAVE' : 'Y', 'CHAT_ID' : chatId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data){
					if (data.ERROR == '')
					{
	...
	MessengerCommon.prototype.unreadMessage = function(messageId) // TODO unreadMessage
	{
		...
		var _ajax = BX.ajax({
			...
			timeout: 60,
			skipAuthCheck: true,
			data: {'IM_UNREAD_MESSAGE' : 'Y', 'USER_ID' : dialogId, 'LAST_ID': lastId, 'TAB' : this.BXIM.messenger.currentTab, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
		})
	...
	MessengerCommon.prototype.readMessage = function(userId, send, sendAjax, skipCheck)
	{
		...
		if (sendAjax)
		{
			...
			//this.BXIM.messenger.readMessageTimeout[userId+'_'+this.BXIM.messenger.currentTab] = setTimeout(BX.delegate(function(){
				var sendData = {'IM_READ_MESSAGE' : 'Y', 'USER_ID' : userId, 'TAB' : this.BXIM.messenger.currentTab, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()};
	...
	MessengerCommon.prototype.loadHistory = function(userId, isHistoryDialog, loadFromButton)
	{
		...
		if (!this.BXIM.messenger.historyEndOfList[userId] || !this.BXIM.messenger.historyEndOfList[userId][isHistoryDialog])
		{
			...
			BX.ajax({
				...
				skipAuthCheck: true,
				timeout: 30,
				data: {'IM_HISTORY_LOAD_MORE' : 'Y', 'USER_ID' : userId, 'PAGE_ID' : this.BXIM.messenger.historyOpenPage[userId], 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data){
					if (tmpLoadMoreWait)
	...
	MessengerCommon.prototype.loadMessageByDate = function(chatId, lastLoadDate, firstMessageId)
	{
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 30,
			data: {'IM_LOAD_MESSAGE_BY_DATE' : 'Y', 'CHAT_ID' : chatId, 'LAST_LOAD' : lastLoadDate, 'FIRST_MESSAGE_ID' : firstMessageId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				if (data && data.THURLY_SESSID)
				{
	...
	MessengerCommon.prototype.loadUserData = function(userId)
	{
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 30,
			data: {'IM_USER_DATA_LOAD' : 'Y', 'USER_ID' : userId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				if (data.ERROR == '')
				{
	...
	MessengerCommon.prototype.loadChatData = function(chatId)
	{
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 30,
			data: {'IM_CHAT_DATA_LOAD' : 'Y', 'CHAT_ID' : chatId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				if (data.ERROR == '')
				{
	...
	MessengerCommon.prototype.joinToChat = function(chatId)
	{
		...
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 60,
			data: {'IM_CHAT_JOIN' : 'Y', 'CHAT_ID' : chatId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
	...
	MessengerCommon.prototype.messageUrlAttachDelete = function(messageId, attachId)
	{
		...
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 30,
			data: {'IM_URL_ATTACH_DELETE' : 'Y', 'ID': messageId, 'ATTACH_ID' : attachId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
		});
		...
		if (!onlyDraw)
		{
			...
			this.BXIM.messenger.popupMessengerLikeBlockTimeout[messageId] = setTimeout(BX.delegate(function(){
				...
				BX.ajax({
					...
					skipAuthCheck: true,
					timeout: 30,
					data: {'IM_LIKE_MESSAGE' : 'Y', 'ID': messageId, 'ACTION' : likeAction, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
					onsuccess: BX.delegate(function(data) {
						if (data.ERROR == '')
						{
	...
	MessengerCommon.prototype.editMessageAjax = function(id, text)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'IM_EDIT_MESSAGE' : 'Y', ID: id, MESSAGE: text, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data) {
	...
	MessengerCommon.prototype.deleteMessageAjax = function(id)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'IM_DELETE_MESSAGE' : 'Y', ID: id, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data) {
				if (data.ERROR)
					return false;
	...
	MessengerCommon.prototype.shareMessageAjax = function(id, type, date)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'IM_SHARE_MESSAGE' : 'Y', ID: id, TYPE: type, DATE: date? date: 0, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data) {
				if (data.ERROR)
					return false;
	...
	MessengerCommon.prototype.clickButtonKeyboard = function()
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'IM_BOT_COMMAND' : 'Y', 'BOT_ID': botId, 'COMMAND' : command, 'COMMAND_PARAMS' : commandParams, 'DIALOG_ID': dialogId, 'MESSAGE_ID': messageId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data){
				this.sendBotCommand = false;
			}, this),
			onfailure: BX.delegate(function(){
	...
	MessengerCommon.prototype.diskChatDialogFileRegister = function(chatId, text)
	{
		...
		this.BXIM.disk.timeout[chatId] = setTimeout(BX.delegate(function(){
			...
			BX.ajax({
				...
				skipAuthCheck: true,
				timeout: 30,
				data: {'IM_FILE_REGISTER' : 'Y', CHAT_ID: chatId, RECIPIENT_ID: recipientId, TEXT: text, MESSAGE_TMP_ID: tmpMessageId, FILES: JSON.stringify(this.BXIM.disk.filesRegister[chatId]), OL_SILENT: olSilentMode, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data) {
					if (data.ERROR != '')
					{
						this.BXIM.messenger.sendMessageFlag--;
	...
	MessengerCommon.prototype.diskChatDialogUploadError = function(stream, pIndex, data)
	{
		...
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 30,
			data: {'IM_FILE_UNREGISTER' : 'Y', CHAT_ID: stream.post.REG_CHAT_ID, FILES: stream.post.REG_PARAMS, MESSAGES: JSON.stringify(messages), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
		});
		window.onbeforeunload = this.BXIM.disk.OldBeforeUnload;
	...
	MessengerCommon.prototype.phoneCommand = function(command, params, async, successCallback)
	{
		...
		BX.ajax({
			...
			timeout: 30,
			async: async,
			data: {'IM_PHONE' : 'Y', 'COMMAND': command, 'PARAMS' : JSON.stringify(params), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
	...
	MessengerCommon.prototype.phoneCallStart = function()
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'IM_PHONE' : 'Y', 'COMMAND': 'init', 'NUMBER' : this.BXIM.webrtc.phoneNumber, 'NUMBER_USER' : BX.util.htmlspecialcharsback(this.BXIM.webrtc.phoneNumberUser), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data){
				if (data.ERROR == '')
				{
	...
	MessengerCommon.prototype.phoneAuthorize = function()
	{
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 30,
			data: {'IM_PHONE' : 'Y', 'COMMAND': 'authorize', 'UPDATE_INFO': this.BXIM.webrtc.phoneCheckBalance? 'Y': 'N', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				if (data && data.THURLY_SESSID)
				{
	...
	MessengerCommon.prototype.phoneOnAuthResult = function(e)
	{
		...
		else if (!this.isMobile() && e.code == 302)
		{
			BX.ajax({
				...
				dataType: 'json',
				timeout: 30,
				data: {'IM_PHONE' : 'Y', 'COMMAND': 'onetimekey', 'KEY': e.key, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data)
				{
					if (data.ERROR == '')
					{
	...
	MessengerCommon.prototype.getHrPhoto = function(userId, color)
	{
		...
		if (userId == 'phone')
		{
			hrphoto = '/thurly/js/im/images/hidef-phone-v3.png';
		}
		else if (this.BXIM.messenger.hrphoto[userId])
		{
			hrphoto = this.BXIM.messenger.hrphoto[userId];
			if (this.BXIM.messenger.hrphoto[userId] != '/thurly/js/im/images/hidef-avatar-v3.png')
			{
				color = '';
			}
		}
		...
		{
			hrphoto = '/thurly/js/im/images/hidef-avatar-v3.png'
		}
		else
		{
	...
	MessengerCommon.prototype.linesGetSessionHistory = function(sessionId)
	{
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'sessionGetHistory', 'SESSION_ID': sessionId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				if (data && data.THURLY_SESSID)
				{
	...
	MessengerCommon.prototype.linesJoinSession = function(chatId)
	{
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'joinSession', 'CHAT_ID': chatId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
			}, this),
			onfailure: BX.delegate(function(){
	...
	MessengerCommon.prototype.linesStartSession = function(chatId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'startSession', 'CHAT_ID': chatId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
			}, this),
			onfailure: BX.delegate(function(){
	...
	MessengerCommon.prototype.linesStartSessionByMessage = function(messageId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'startSessionByMessage', 'CHAT_ID' : chatId, 'MESSAGE_ID' : messageId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
			}, this),
			onfailure: BX.delegate(function(){
	...
	MessengerCommon.prototype.linesOpenSession = function(userCode, params)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'openSession', 'USER_CODE': userCode, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data){
				if (data.ERROR == '')
				{
					if (params.SLIDER == 'Y')
	...
	MessengerCommon.prototype.linesVoteSend = function(dialogId, messageId, rating)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'IM_LINES_VOTE_SEND': 'Y', 'DIALOG_ID': dialogId, 'MESSAGE_ID': messageId, 'RATING': rating, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
	...
	MessengerCommon.prototype.linesSaveToQuickAnswers = function(messageId, silentMode)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'saveToQuickAnswers', 'CHAT_ID' : chatId, 'MESSAGE_ID' : messageId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
	...
	MessengerCommon.prototype.linesVoteHeadSend = function(sessionId, rating)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'voteHead', 'SESSION_ID' : sessionId, 'RATING' : rating, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		});
	...
	MessengerCommon.prototype.linesAnswer = function(chatId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'answer', 'CHAT_ID' : chatId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
	...
	MessengerCommon.prototype.linesSkip = function(chatId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'skip', 'CHAT_ID' : chatId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
			}, this),
			onfailure: BX.delegate(function(){
	...
	MessengerCommon.prototype.linesActivateSilentMode = function(chatId, flag, force)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'silentMode', 'ACTIVATE': flag? 'Y': 'N', 'CHAT_ID' : chatId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
	...
	MessengerCommon.prototype.linesActivatePinMode = function(chatId, flag)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'pinMode', 'ACTIVATE': flag, 'CHAT_ID' : chatId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
	...
	MessengerCommon.prototype.linesCloseDialog = function(chatId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'closeDialog', 'CHAT_ID' : chatId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
	...
	MessengerCommon.prototype.linesMarkAsSpam = function(chatId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'markSpam', 'CHAT_ID' : chatId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
	...
	MessengerCommon.prototype.linesInterceptSession = function(chatId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'interceptSession', 'CHAT_ID' : chatId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
			}, this),
			onfailure: BX.delegate(function(){
	...
	MessengerCommon.prototype.linesCreateLead = function(chatId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'createLead', 'CHAT_ID' : chatId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
			}, this),
			onfailure: BX.delegate(function(){
	...
	MessengerCommon.prototype.linesChangeCrmEntity = function(messageId)
	{
		...
		else
		{
			BX.ajax({
				...
				method: 'POST',
				timeout: 30,
				data: {'IM_CRM_SELECTOR' : 'Y', 'sessid': BX.thurly_sessid()}
			});
	...
	MessengerCommon.prototype.linesChangeCrmEntityAjax = function(result)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'changeCrmEntity', 'CHAT_ID' : chatId, 'MESSAGE_ID': messageId, 'ENTITY_TYPE' : entityType, 'ENTITY_ID': entityId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
			}, this),
			onfailure: BX.delegate(function(){
	...
	MessengerCommon.prototype.linesCancelCrmExtend = function(messageId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'COMMAND': 'cancelCrmExtend', 'CHAT_ID' : chatId, 'MESSAGE_ID': messageId, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				this.BXIM.messenger.blockJoinChat[chatId] = false;
			}, this),
			onfailure: BX.delegate(function(){
	...
	MessengerCommon.prototype.openRenamePortal = function(button)
	{
		...
		else if (typeof(BX.ThurlyOS) != 'undefined')
		{
