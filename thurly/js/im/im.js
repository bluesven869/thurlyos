...
BX.IM = function(domNode, params)
{
	...
	this.revision = 116; // api revision - check include.php
	this.ieVersion = BX.browser.DetectIeVersion();
	this.errorMessage = '';
	this.animationSupport = true;
	this.context = params.context;
	this.design = params.design;
	this.thurlyNetwork = params.thurlyNetwork;
	this.thurlyNetwork2 = params.thurlyNetwork2;
	this.thurlyOpenLines = params.thurlyOpenLines;
	this.thurlyos = params.thurlyos;
	this.isAdmin = params.isAdmin;
	this.thurlyIntranet = params.thurlyIntranet;
	this.thurlyosnet = params.thurlyosnet;
	this.thurlyXmpp = params.thurlyXmpp;
	this.thurlyMobile = params.thurlyMobile;
	...
	this.pathToAjax = params.path.im? params.path.im: '/thurly/components/thurly/im.messenger/im.ajax.php';
	this.pathToCallAjax = params.path.call? params.path.call: '/thurly/components/thurly/im.messenger/call.ajax.php';
	this.pathToFileAjax = params.path.file? params.path.file: '/thurly/components/thurly/im.messenger/file.ajax.php';
	this.pathToBlankImage = '/thurly/js/im/images/blank.gif';
	...
	if (this.init)
	{
		...
		{
			this.windowTitle = this.thurlyIntranet? (!BX.browser.IsMac()? BX.message('IM_DESKTOP_B24_TITLE'): BX.message('IM_DESKTOP_B24_OSX_TITLE')): BX.message('IM_WM');
...
BX.IM.prototype.callTo = function(userId, video)
{
	...
	BX.desktopUtils.runningCheck(function(){
		BX.desktopUtils.goToBx("bx://callto/"+(video? 'video': 'audio')+"/"+userId+(BXIM.thurlyosnet? '/thurlyosnet/Y':''));
...
BX.IM.prototype.openSettings = function(params)
{
	...
	var linesTabEnable = BX.MessengerCommon.isPage() && this.thurlyOpenLines? true: false;
	...
	this.settingsTableConfig['notify'] = {
		...
		'headers' : [
			...
			this.thurlyXmpp? BX.message('IM_SETTINGS_NOTIFY_XMPP'): false,
			BX.message('IM_SETTINGS_NOTIFY_EMAIL'),
			this.thurlyMobile? BX.message('IM_SETTINGS_NOTIFY_PUSH'): false
		],
		'rows' : [],
	...
	this.settingsView.privacy = {
		...
		'condition': BX.delegate(function(){ return !this.thurlyIntranet}, this),
		'settings': [
			...
			this.thurlyosnet? {'title': BX.message('IM_SETTINGS_PRIVACY_PROFILE'), name: 'privacyProfile', 'type': 'select', items: [{title: BX.message('IM_SETTINGS_SELECT_1_3'), value: 'all'}, {title: BX.message('IM_SETTINGS_SELECT_2_3'), value: 'contact'}, {title: BX.message('IM_SETTINGS_SELECT_3_3'), value: 'nobody'}], 'value': this.settings.privacyProfile}: null
		]
	};
	...
	else if (config.type == 'notifyControl')
	{
		...
		*/
		items.push(BX.create("div", {attrs: {id: "bx-messenger-settings-notify-clients"}, style : {display: this.settings.notifyScheme == 'simple'? 'block':'none'}, props : { className: "bx-messenger-settings-notify-clients"}, children : [
			...
			this.thurlyXmpp? BX.create("div", {props: {className: 'bx-messenger-settings-notify-clients-item'}, children: [
			...
			this.thurlyMobile? BX.create("div", {props: {className: 'bx-messenger-settings-notify-clients-item'}, children: [
...
BX.IM.prototype.saveSettings = function(settings)
{
	...
	this.saveSettingsTimeout[timeoutKey] = setTimeout(BX.delegate(function(){
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'IM_SETTING_SAVE' : 'Y', 'IM_AJAX_CALL' : 'Y', SETTINGS: JSON.stringify(settings), 'sessid': BX.thurly_sessid()}
		});
...
BX.IM.prototype.saveFormSettings = function()
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_SETTINGS_SAVE' : 'Y', 'IM_AJAX_CALL' : 'Y', SETTINGS: JSON.stringify(values), 'sessid': BX.thurly_sessid()},
...
BX.IM.prototype.GetNotifySettings = function()
{
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_SETTINGS_NOTIFY_LOAD' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			...
			if (data.ERROR == "")
			{
				if (this.settings.notifyScheme == 'simple')
				{
					for (var configName in data.VALUES)
					{
						...
						else if (this.thurlyXmpp && !BX('notifySchemeSendXmpp').checked && configName.substr(0,5) == 'xmpp|')
							data.VALUES[configName] = false;
						else if (!BX('notifySchemeSendEmail').checked && configName.substr(0,6) == 'email|')
							data.VALUES[configName] = false;
						else if (this.thurlyMobile && !BX('notifySchemeSendPush').checked && configName.substr(0,5) == 'push|')
							data.VALUES[configName] = false;
				...
				if (data.NAMES['im'])
				{
					...
					for (var notifyId in data.NAMES['im']['NOTIFY'])
					{
						...
						rows.push([
							...
							this.thurlyXmpp? {'type': 'checkbox', name: 'notify|xmpp|im|'+notifyId}: false,
							...
							this.thurlyMobile? {'type': 'checkbox', name: 'notify|push|im|'+notifyId}: false
						]);
					}
				}
				...
				for (var moduleId in data.NAMES)
				{
					...
					for (var notifyId in data.NAMES[moduleId]['NOTIFY'])
					{
						...
						rows.push([
							...
							this.thurlyXmpp? {'type': 'checkbox', name: 'notify|xmpp|'+moduleId+'|'+notifyId}: false,
							...
							this.thurlyMobile? {'type': 'checkbox', name: 'notify|push|'+moduleId+'|'+notifyId}: false
						]);
					}
				}
...
BX.IM.prototype.GetSimpleNotifySettings = function()
{
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_SETTINGS_SIMPLE_NOTIFY_LOAD' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data) {
			if (data.ERROR == "")
			{
				var rows = [];
...
BX.Notify = function(BXIM, params)
{
	...
	if (this.panelEnabled)
	{
		...
		if (this.panelButtonNetwork)
		{
			...
			if (this.BXIM.thurlyNetwork)
			{
				this.panelButtonNetwork.href = "https://www.thurlyos.net/";
	...
	if (this.BXIM.init)
	{
		...
		this.panel.appendChild(this.BXIM.audio.reminder = BX.create("audio", { props : { className : "bx-notify-audio" }, children : [
			BX.create("source", { attrs : { src : "/thurly/js/im/audio/reminder.ogg", type : "audio/ogg; codecs=vorbis" }}),
			BX.create("source", { attrs : { src : "/thurly/js/im/audio/reminder.mp3", type : "audio/mpeg" }})
		]}));
...
BX.Notify.prototype.viewNotify = function(id, read, send)
{
	...
	if (send)
	{
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'IM_NOTIFY_VIEW' : 'Y', 'ID' : parseInt(id), 'READ': (read? 'Y':'N'), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
		});
	}
	...
	if (this.BXIM.settings.notifyAutoRead)
	{
		...
		if (send)
		{
			BX.ajax({
				...
				dataType: 'json',
				timeout: 60,
				data: {'IM_NOTIFY_READ' : 'Y', 'ID' : id, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
			});
		}
...
BX.Notify.prototype.confirmRequest = function(params, popup)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_NOTIFY_CONFIRM' : 'Y', 'NOTIFY_ID' : params.notifyId, 'NOTIFY_VALUE' : params.notifyValue, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data) {
...
BX.Notify.prototype.deleteNotify = function(notifyId)
{
	...
	if (sendRequest)
	{
		...
		var DATA = {};
		if (groupDelete)
			DATA = {'IM_NOTIFY_GROUP_REMOVE' : 'Y', 'NOTIFY_ID' : notifyId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()};
		else
			DATA = {'IM_NOTIFY_REMOVE' : 'Y', 'NOTIFY_ID' : notifyId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()};
...
BX.Notify.prototype.blockNotifyType = function(settingName)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_NOTIFY_BLOCK_TYPE' : 'Y', 'BLOCK_TYPE' : settingName, 'BLOCK_RESULT' : (blockResult? 'Y': 'N'), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
	});
...
BX.Notify.prototype.loadNotify = function(send)
{
	...
	BX.ajax({
		...
		lsTimeout: 5,
		timeout: 30,
		data: {'IM_NOTIFY_LOAD' : 'Y', 'IM_AUTO_READ' : (this.BXIM.settings.notifyAutoRead? 'Y': 'N'), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data) {
...
BX.Notify.prototype.notifyHistory = function(event)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_NOTIFY_HISTORY_LOAD_MORE' : 'Y', 'PAGE' : !this.BXIM.settings.loadLastNotify && this.notifyHistoryPage == 0? 1: this.notifyHistoryPage, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data)
		{
			if (data && data.THURLY_SESSID)
			{
				BX.message({'thurly_sessid': data.THURLY_SESSID});
			}
			if (data.ERROR == '')
			{
				this.notifyLoad = true;
			...
			else
			{
				if (data.ERROR == 'SESSION_ERROR' && this.sendAjaxTry < 2)
				{
					...
					BX.onCustomEvent(window, 'onImError', [data.ERROR, data.THURLY_SESSID]);
				}
...
BX.Notify.prototype.sendNotifyAnswer = function(notifyAnswer, popup)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_NOTIFY_ANSWER' : 'Y', 'NOTIFY_ID' : id, 'NOTIFY_ANSWER' : input.value, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data) {
...
BX.Messenger = function(BXIM, params)
{
	...
	this.realSearchAvailable = !this.BXIM.userExtranet || !this.BXIM.thurlyIntranet && !this.BXIM.thurlyosnet;
	...
	if (this.BXIM.init)
	{
		...
		this.notify.panel.appendChild(this.BXIM.audio.newMessage1 = BX.create("audio", { props : { className : "bx-messenger-audio" }, children : [
			BX.create("source", { attrs : { src : "/thurly/js/im/audio/new-message-1.ogg", type : "audio/ogg; codecs=vorbis" }}),
			BX.create("source", { attrs : { src : "/thurly/js/im/audio/new-message-1.mp3", type : "audio/mpeg" }})
		]}));
		this.notify.panel.appendChild(this.BXIM.audio.newMessage2 = BX.create("audio", { props : { className : "bx-messenger-audio" }, children : [
			BX.create("source", { attrs : { src : "/thurly/js/im/audio/new-message-2.ogg", type : "audio/ogg; codecs=vorbis" }}),
			BX.create("source", { attrs : { src : "/thurly/js/im/audio/new-message-2.mp3", type : "audio/mpeg" }})
		]}));
		this.notify.panel.appendChild(this.BXIM.audio.send = BX.create("audio", { props : { className : "bx-messenger-audio" }, children : [
			BX.create("source", { attrs : { src : "/thurly/js/im/audio/send.ogg", type : "audio/ogg; codecs=vorbis" }}),
			BX.create("source", { attrs : { src : "/thurly/js/im/audio/send.mp3", type : "audio/mpeg" }})
		]}));
		...
		for(var userId in this.users)
		{
			...
			{
				this.message[userId+'birthday'] = {'id' : userId+'birthday', 'senderId' : 0, 'recipientId' : userId, 'date' : BX.MessengerCommon.getNowDate(true), 'text' : BX.message('IM_M_BIRTHDAY_MESSAGE').replace('#USER_NAME#', '<img src="/thurly/js/im/images/blank.gif" class="bx-messenger-birthday-icon"><strong>'+this.users[userId].name+'</strong>') };
...
BX.Messenger.prototype.openMessenger = function(userId, params)
{
			...
			this.popupMessengerBodyDialog = BX.create("div", { props : { className : "bx-messenger-body-dialog bxu-file-input-over" }, children: [
				...
				this.popupMessengerTextareaPlace = BX.create("div", { props : { className : "bx-messenger-textarea-place"}, children : [
					...
					BX.create("div", { props : { className : "bx-messenger-textarea-open-invite" }, children : [
						BX.create("div", { props : { className : "bx-messenger-textarea-open-invite-text-box" }, children: [
							BX.create("div", { props : { className : "bx-messenger-textarea-open-invite-text-box-element" }, children: [
								this.popupMessengerTextareaOpenText = BX.create("div", { props : { className : "bx-messenger-textarea-open-invite-text" }, html: BX.message(this.BXIM.thurlyIntranet? 'IM_O_INVITE_TEXT': 'IM_O_INVITE_TEXT_SITE')})
							]})
						]}),
	...
	if (!BX.MessengerCommon.isDesktop() && this.BXIM.thurlyIntranet && this.BXIM.platformName != '' && this.BXIM.settings.bxdNotify)
	{
		...
		this.popupMessengerDesktopTimeout = setTimeout(BX.delegate(function(){
			var acceptButton = BX.delegate(function(){
				window.open(BX.browser.IsMac()? "http://dl.thurlyos.com/b24/thurlyos_desktop.dmg": "http://dl.thurlyos.com/b24/thurlyos_desktop.exe", "desktopApp");
...
BX.Messenger.prototype.openChatCreateForm = function(type)
{
	...
	if (type == 'chat')
	{
		...
		descriptionNodes = [
			...
			BX.create("div", { props : { className : "bx-messenger-box-create-text"}, html: BX.message(this.BXIM.thurlyIntranet? 'IM_C_ABOUT_CHAT': 'IM_C_ABOUT_CHAT_CHAT').split('#BR#').join("<br />").replace('#PROFILE_END#', '</a>').replace('#PROFILE_START#', '<a href="'+BXIM.path.profile+'edit/" target="_blank">')})
		];
	}
	else if (type == 'open' && (!this.BXIM.userExtranet || this.openChatEnable))
	{
		...
		descriptionNodes = [
			...
			BX.create("div", { props : { className : "bx-messenger-box-create-text"}, html: BX.message(this.BXIM.thurlyIntranet? 'IM_C_ABOUT_OPEN': 'IM_C_ABOUT_OPEN_SITE').split('#BR#').join("<br />").replace('#PROFILE_END#', '</a>').replace('#PROFILE_START#', '<a href="'+BXIM.path.profile+'edit/" target="_blank">').replace('#CHAT_END#', '</b>').replace('#CHAT_START#', '<b>')})
		];
	}
	else
	{
		...
		descriptionNodes = [
			...
			BX.create("div", { props : { className : "bx-messenger-box-create-text"}, html: BX.message(this.BXIM.thurlyIntranet? 'IM_C_ABOUT_PRIVATE': 'IM_C_ABOUT_PRIVATE_SITE').split('#BR#').join("<br />").replace('#PROFILE_END#', '</a>').replace('#PROFILE_START#', '<a href="'+BXIM.path.profile+'edit/" target="_blank">')})
		];
	}
...
BX.Messenger.prototype.kickFromChat = function(chatId, userId)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 60,
		data: {'IM_CHAT_LEAVE' : 'Y', 'CHAT_ID' : chatId, 'USER_ID' : userId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data){
			if (data.ERROR == '')
			{
	...
	if (params.userRedraw)
	{
		...
		if (this.userInChat[chatId])
		{
			this.userInChat[chatId].sort(BX.delegate(function(a, b) {
				...
				if (this.users[a].avatar != "/thurly/js/im/images/blank.gif") { i += 5 }
				if (a < b) { i += 1 }
				ii = 0;
				...
				if (this.users[b].avatar != "/thurly/js/im/images/blank.gif") { ii += 5 }
				if (b < a) { ii += 1 }
	...
	this.popupChatDialog = new BX.PopupWindow('bx-messenger-popup-newchat', bindElement, {
		...
		content : BX.create("div", { props : { className : "bx-messenger-popup-newchat-wrap bx-messenger-popup-newchat-wrap-style-"+params.type+(BX.browser.IsMac()? '': ' bx-messenger-custom-scroll') }, children: [
			...
			params.type == 'CHAT_CREATE'? null: BX.create("div", { props : { className : "bx-messenger-popup-newchat-box bx-messenger-popup-newchat-dest bx-messenger-popup-newchat-dest-even"+(params.type == 'MENTION'? ' bx-messenger-hide': '') }, children: [
				...
				this.popupChatDialogContactListSearch = BX.create("input", {props : { className : "bx-messenger-input" }, attrs: {type: "text", placeholder: BX.message(this.BXIM.thurlyIntranet? 'IM_M_SEARCH_PLACEHOLDER_CP': 'IM_M_SEARCH_PLACEHOLDER'), value: ''}})
			]}),
...
BX.Messenger.prototype.sendRequestChatDialog = function(params)
{
	...
	{
		data = {'IM_CHAT_ADD' : 'Y', 'TYPE' : params.type, 'TITLE' : params.title, 'MESSAGE' : params.message, 'USERS' : JSON.stringify(params.users), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()};
	}
	else if (params.action == 'CHAT_EXTEND')
	{
		data = {'IM_CHAT_EXTEND' : 'Y', 'CHAT_ID' : params.chatId, 'HISTORY': (this.popupChatDialogShowHistory && this.popupChatDialogShowHistory.checked? 'Y':'N'), 'USERS' : JSON.stringify(params.users), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()};
...
BX.Messenger.prototype.openPopupExternalData = function(bind, type, setAngle, params)
{
	...
	var ajaxData = { 'IM_GET_EXTERNAL_DATA' : 'Y', 'TYPE': type, 'TS': this.popupPopupMenuDateCreate, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()};
...
BX.Messenger.prototype.loadHistoryFiles = function(chatId, afterDelete)
{
	...
	if (!this.historyFilesEndOfList[chatId])
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'IM_HISTORY_FILES_LOAD' : 'Y', 'CHAT_ID' : chatId, 'PAGE_ID' : this.historyFilesOpenPage[chatId], 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data){
				if (tmpLoadMoreWait)
...
BX.Messenger.prototype.showContext = function(messageId)
{
	BX.ajax({
		...
		skipAuthCheck: true,
		timeout: 30,
		data: {'IM_LOAD_CONTEXT_MESSAGE' : 'Y', 'MESSAGE_ID' : messageId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data)
		{
			if (data && data.THURLY_SESSID)
			{
...
BX.Messenger.prototype.deleteAllHistory = function(userId)
{
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_HISTORY_REMOVE_ALL' : 'Y', 'USER_ID' : userId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
	});
...
BX.Messenger.prototype.drawHistory = function(userId, historyElements, loadFromServer, sort)
{
	...
	if (loadFromServer && (!this.showMessage[userId] || this.showMessage[userId] && this.showMessage[userId].length < 20))
	{
		...
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 30,
			data: {'IM_HISTORY_LOAD' : 'Y', 'USER_ID' : userId, 'USER_LOAD' : userIsChat? (this.chat[userId.toString().substr(4)] && this.chat[userId.toString().substr(4)].fake? 'Y': 'N'): (this.users[userId] && this.users[userId].fake? 'Y': 'N'), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				if (data && data.THURLY_SESSID)
				{
...
BX.Messenger.prototype.drawHistoryFiles = function(chatId, filesElements, loadFromServer)
{
	...
	if (loadFromServer && chatId > 0)
	{
		...
		BX.ajax({
			...
			skipAuthCheck: true,
			timeout: 30,
			data: {'IM_HISTORY_FILES_LOAD' : 'Y', 'CHAT_ID' : chatId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				if (data && data.THURLY_SESSID)
				{
	...
	if (this.popupHistorySearchInput.value != '')
	{
		this.historySearchTimeout = setTimeout(BX.delegate(function(){
			BX.ajax({
				...
				dataType: 'json',
				timeout: 30,
				data: {'IM_HISTORY_SEARCH' : 'Y', 'USER_ID' : this.historyUserId, 'SEARCH' : this.popupHistorySearchInput.value, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data){
					if (tmpLoadMoreWait)
						BX.remove(tmpLoadMoreWait);
					this.historySearchBegin = false;
	...
	if (this.historyDateSearch != '')
	{
		this.historySearchTimeout = setTimeout(BX.delegate(function(){
			BX.ajax({
				...
				dataType: 'json',
				timeout: 30,
				data: {'IM_HISTORY_DATE_SEARCH' : 'Y', 'USER_ID' : this.historyUserId, 'DATE' : this.historyDateSearch, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data){
					if (tmpLoadMoreWait)
						BX.remove(tmpLoadMoreWait);
					this.historySearchBegin = false;
	...
	if (this.popupHistoryFilesSearchInput.value != '')
	{
		this.historyFilesSearchTimeout = setTimeout(BX.delegate(function(){
			BX.ajax({
				...
				dataType: 'json',
				timeout: 30,
				data: {'IM_HISTORY_FILES_SEARCH' : 'Y', 'CHAT_ID' : this.historyChatId, 'SEARCH' : this.popupHistoryFilesSearchInput.value, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data){
					if (tmpLoadMoreWait)
						BX.remove(tmpLoadMoreWait);
					this.historyFilesSearchBegin = false;
	...
	this.updateStateTimeout = setTimeout(
		BX.delegate(function(){
			...
			var _ajax = BX.ajax({
				...
				lsTimeout: 1,
				timeout: 30,
				data: {'IM_UPDATE_STATE' : 'Y', 'OPEN_MESSENGER' : this.popupMessenger != null? 1: 0, 'TAB' : this.currentTab, 'FM' : JSON.stringify(this.flashMessage), 'FN' :  JSON.stringify(this.notify.flashNotify), 'SITE_ID': BX.message('SITE_ID'),'IM_AJAX_CALL' : 'Y', 'DESKTOP' : (BX.MessengerCommon.isDesktop()? 'Y': 'N'), 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data)
				{
					if (send)
	...
	this.updateStateTimeout = setTimeout(
		BX.delegate(function(){
			BX.ajax({
				...
				data: {'IM_UPDATE_STATE_LIGHT' : 'Y', 'SITE_ID': BX.message('SITE_ID'), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function(data)
				{
					if (send)
...
BX.Messenger.prototype.textareaIconDialogClick = function(id, messageId, params)
{
	...
	if (!icon && !params.___ajaxSkip)
	{
		BX.ajax({
			...
			dataType: 'json',
			timeout: 60,
			data: {'IM_GET_TEXTAREA_ICONS': 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data){
...
BX.Messenger.prototype.sendFrameToken = function(botId, dialogId)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 60,
		data: {'IM_OPEN_REST_TOKEN': 'Y', 'BOT_ID' : botId, 'DIALOG_ID' : dialogId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
	});
}
	...
	this.popupTransferDialog = new BX.PopupWindow('bx-messenger-popup-transfer', bindElement, {
		...
		content : BX.create("div", { props : { className : "bx-messenger-popup-newchat-wrap" }, children: [
			...
			BX.create("div", { props : { className : "bx-messenger-popup-newchat-box bx-messenger-popup-newchat-dest bx-messenger-popup-newchat-dest-even" }, children: [
				...
				this.popupTransferDialogContactListSearch = BX.create("input", {props : { className : "bx-messenger-input" }, attrs: {type: "text", placeholder: BX.message(this.BXIM.thurlyIntranet? 'IM_M_SEARCH_PLACEHOLDER_CP': 'IM_M_SEARCH_PLACEHOLDER'), value: ''}})
			]}),
...
BX.Messenger.prototype.linesSendTransfer = function(chatId)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 60,
		data: {'COMMAND': 'transfer', 'CHAT_ID' : chatId, 'TRANSFER_ID': this.linesTransferUser, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(){
		...
		if (BX.MessengerCommon.isDesktop())
		{
			...
			BX.desktop.addCustomEvent("bxCallAnswer", BX.delegate(function(chatId, userId, video, callToGroup) {
				...
				BX.ajax({
					...
					dataType: 'json',
					timeout: 30,
					data: {'IM_CALL' : 'Y', 'COMMAND': 'answer', 'CHAT_ID': chatId, 'CALL_TO_GROUP': callToGroup? 'Y': 'N', 'RECIPIENT_ID' : this.callUserId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
					onsuccess: BX.delegate(function(){
			...
			BX.desktop.addCustomEvent("bxSaveSettings", BX.delegate(function(settings) {
				...
				if (this.BXIM.messenger != null)
				{
					...
					if (BX.MessengerCommon.isPage() && this.BXIM.thurlyOpenLines)
					{
			...
			BX.desktop.addCustomEvent("BXProtocolUrl", BX.delegate(function(command, params) {
				...
				params = params? params: {}
				if (params.thurlyosnet && params.thurlyosnet == 'Y' && !this.BXIM.thurlyosnet)
					return false;
		...
		initJs = "<script type=\"text/javascript\">"+
			"BX.ready(function() {"+
				"BXIM = new BX.IM(null, {"+
					...
					"'thurlyNetwork': "+this.BXIM.thurlyNetwork+","+
					"'thurlyNetwork2': "+this.BXIM.thurlyNetwork2+","+
					"'thurlyOpenLines': "+this.BXIM.thurlyOpenLines+","+
					"'thurlyos': "+this.BXIM.thurlyos+","+
					"'thurlyIntranet': "+this.BXIM.thurlyIntranet+","+
					"'thurlyXmpp': "+this.BXIM.thurlyXmpp+","+
					"'thurlyMobile': "+this.BXIM.thurlyMobile+","+
...
BX.IM.Desktop.prototype.onAwayAction = function (away, manual)
{
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_IDLE' : 'Y', 'IM_AJAX_CALL' : 'Y', IDLE: away? 'Y': 'N', MANUAL: manual? 'Y': 'N', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data)
		{
			if (data && data.THURLY_SESSID)
			{
...
BX.IM.WebRTC = function(BXIM, params)
{
	...
	if (this.enabled)
	{
					...
					if (this.callInit || this.callActive)
					{
						setTimeout(BX.delegate(function(){
							BX.ajax({
								...
								dataType: 'json',
								timeout: 30,
								data: {'IM_CALL' : 'Y', 'COMMAND': 'busy', 'CHAT_ID': params.chatId, 'RECIPIENT_ID' : params.senderId, 'VIDEO': params.video? 'Y': 'N', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
							});
					...
					if (this.callInit || this.callActive)
					{
						...
						else if (this.initiator && this.callChatId == params.chatId)
						{
							...
							BX.ajax({
								...
								dataType: 'json',
								timeout: 30,
								data: {'IM_CALL' : 'Y', 'COMMAND': 'answer', 'CHAT_ID': this.callChatId, 'CALL_TO_GROUP': this.callToGroup? 'Y': 'N',  'RECIPIENT_ID' : this.callUserId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
							});
						}
					}
					else
		...
		{
			var domNode = BX.create("link", { attrs : { href : "/thurly/js/im/css/call.css", rel : "stylesheet", type : "text/css" }});
			document.head.insertBefore(domNode, document.head.firstChild);
			var domNode = BX.create("link", { attrs : { href : "/thurly/css/main/font-awesome.css", rel : "stylesheet", type : "text/css" }});
			document.head.insertBefore(domNode, document.head.firstChild);
			var domNode = BX.create("script", { attrs : { src : "/thurly/js/im/call.js", type : "text/javascript" }});
			document.head.insertBefore(domNode, document.head.firstChild);
			var domNode = BX.create("script", { attrs : { src : "/thurly/js/main/webrtc/adapter.js", type : "text/javascript" }});
			document.head.insertBefore(domNode, document.head.firstChild);
			domNode = BX.create("script", { attrs : { src : "/thurly/js/im/webrtc/janus.js", type : "text/javascript" }});
			document.head.insertBefore(domNode, document.head.firstChild);
		}
	}
	else
...
BX.IM.WebRTC.prototype.initAudio = function(onlyError)
{
	if (onlyError === true)
	{
		this.panel.appendChild(this.BXIM.audio.error = BX.create("audio", { props : { className : "bx-messenger-audio" }, children : [
			BX.create("source", { attrs : { src : "/thurly/js/im/audio/video-error.ogg", type : "audio/ogg; codecs=vorbis" }}),
			BX.create("source", { attrs : { src : "/thurly/js/im/audio/video-error.mp3", type : "audio/mpeg" }})
		]}));
	...
	this.panel.appendChild(this.BXIM.audio.dialtone = BX.create("audio", { props : { className : "bx-messenger-audio" }, children : [
		BX.create("source", { attrs : { src : "/thurly/js/im/audio/video-dialtone.ogg", type : "audio/ogg; codecs=vorbis" }}),
		BX.create("source", { attrs : { src : "/thurly/js/im/audio/video-dialtone.mp3", type : "audio/mpeg" }})
	]}));

	this.panel.appendChild(this.BXIM.audio.ringtone = BX.create("audio", { props : { className : "bx-messenger-audio" }, children : [
		BX.create("source", { attrs : { src : "/thurly/js/im/audio/video-ringtone.ogg", type : "audio/ogg; codecs=vorbis" }}),
		BX.create("source", { attrs : { src : "/thurly/js/im/audio/video-ringtone.mp3", type : "audio/mpeg" }})
	]}));

	this.panel.appendChild(this.BXIM.audio.start = BX.create("audio", { props : { className : "bx-messenger-audio" }, children : [
		BX.create("source", { attrs : { src : "/thurly/js/im/audio/video-start.ogg", type : "audio/ogg; codecs=vorbis" }}),
		BX.create("source", { attrs : { src : "/thurly/js/im/audio/video-start.mp3", type : "audio/mpeg" }})
	]}));

	this.panel.appendChild(this.BXIM.audio.stop = BX.create("audio", { props : { className : "bx-messenger-audio" }, children : [
		BX.create("source", { attrs : { src : "/thurly/js/im/audio/video-stop.ogg", type : "audio/ogg; codecs=vorbis" }}),
		BX.create("source", { attrs : { src : "/thurly/js/im/audio/video-stop.mp3", type : "audio/mpeg" }})
	]}));
...
BX.IM.WebRTC.prototype.setLocalAndSend = function(userId, desc)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_CALL' : 'Y', 'COMMAND': 'signaling', 'CHAT_ID': this.callChatId,  'RECIPIENT_ID' : userId, 'PEER': JSON.stringify( desc ), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
	});
...
BX.IM.WebRTC.prototype.onIceCandidate = function (userId, candidates)
{
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_CALL' : 'Y', 'COMMAND': 'signaling', 'CHAT_ID': this.callChatId,  'RECIPIENT_ID' : userId, 'PEER': JSON.stringify(candidates), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
	});
}
...
BX.IM.WebRTC.prototype.peerConnectionReconnect = function (userId)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_CALL' : 'Y', 'COMMAND': 'reconnect', 'CHAT_ID' : this.callChatId,  'RECIPIENT_ID' : userId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(){
...
BX.IM.WebRTC.prototype.callInvite = function(userId, video, screen)
{
		...
		if (!BX.MessengerCommon.isDesktop())
		{
			this.BXIM.openConfirm(BX.message('IM_CALL_NO_WEBRT'), [
				this.BXIM.platformName == ''? null: new BX.PopupWindowButton({
					text : BX.message('IM_M_CALL_BTN_DOWNLOAD'),
					className : "popup-window-button-accept",
					events : { click : BX.delegate(function() { window.open(BX.browser.IsMac()? "http://dl.thurlyos.com/b24/thurlyos_desktop.dmg": "http://dl.thurlyos.com/b24/thurlyos_desktop.exe", "desktopApp"); BX.proxy_context.popupWindow.close(); }, this) }
				}),
				new BX.PopupWindowButton({
					text : BX.message('IM_NOTIFY_CONFIRM_CLOSE'),
					className : "popup-window-button-decline",
	...
	if (!this.callActive && !this.callInit && userId > 0)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'IM_CALL' : 'Y', 'COMMAND': 'invite', 'CHAT_ID' : userId, 'CHAT': (callToChat? 'Y': 'N'), 'VIDEO' : video? 'Y': 'N', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				if (data.ERROR == '')
				{
...
BX.IM.WebRTC.prototype.callInviteExperimental = function(dialogId, video)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_CALL' : 'Y', 'COMMAND': 'inviteExperimental', 'CHAT_ID' : dialogId, 'CHAT': (callToChat? 'Y': 'N'), 'VIDEO' : video? 'Y': 'N', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
	});
...
BX.IM.WebRTC.prototype.callInviteUserToChat = function(users)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 60,
		data: {'IM_CALL' : 'Y', 'COMMAND': 'invite_user', 'USERS': JSON.stringify(users), 'CHAT_ID': this.callChatId, 'RECIPIENT_ID' : this.callUserId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data){
...
BX.IM.WebRTC.prototype.callCommand = function(chatId, command, params, async)
{
	...
	if (chatId > 0)
	{
		BX.ajax({
			...
			timeout: 30,
			async: async,
			data: {'IM_CALL' : 'Y', 'COMMAND': command, 'CHAT_ID': chatId, 'RECIPIENT_ID' : this.callUserId, 'PARAMS' : JSON.stringify(params), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(){
				if (this.callDialogAllow)
					this.callDialogAllow.close();
			}, this)
				...
				BX.create("div", { props : { className : 'bx-messenger-call-video-main'}, children: [
					BX.create("div", { props : { className : 'bx-messenger-call-video-main-wrap'}, children: [
						BX.create("div", { props : { className : 'bx-messenger-call-video-main-watermark'}, children: [
							BX.create("img", { props : { className : 'bx-messenger-call-video-main-watermark-img'},  attrs : {src : '/thurly/js/im/images/watermark_'+(this.BXIM.language == 'ru'? 'ru': 'en')+'.png'}})
						]}),
...
BX.IM.WebRTC.prototype.callOverlayUpdatePhoto = function()
{
	...
	for (var i in this.callOverlayPhotoUsers)
	{
		if (i == 'phone')
		{
			this.callOverlayPhotoUsers[i].src = '/thurly/js/im/images/hidef-phone-v3.png';
		}
		else if (this.messenger.hrphoto[i])
		{
			this.callOverlayPhotoUsers[i].src = this.messenger.hrphoto[i];
			if (this.messenger.hrphoto[i] != '/thurly/js/im/images/hidef-avatar-v3.png')
			{
		...
		{
			this.callOverlayPhotoUsers[i].src = '/thurly/js/im/images/hidef-avatar-v3.png';
		}
		else
		{
	...
	for (var i in this.callOverlayVideoPhotoUsers)
	{
		if (i == 'phone')
		{
			this.callOverlayVideoPhotoUsers[i].src = '/thurly/js/im/images/hidef-phone-v3.png';
		}
		else if (this.messenger.hrphoto[i])
		{
		...
		{
			this.callOverlayVideoPhotoUsers[i].src = '/thurly/js/im/images/hidef-avatar-v3.png';
		}
		else
		{
	...
	if (this.callOverlayPhotoCompanion)
	{
		...
		{
			this.callOverlayPhotoCompanion.src = '/thurly/js/im/images/hidef-phone-v3.png';
		}
		else if (this.messenger.hrphoto[companionUserId])
		{
			this.callOverlayPhotoCompanion.src  = this.messenger.hrphoto[companionUserId];
			if (this.messenger.hrphoto[companionUserId] != '/thurly/js/im/images/hidef-avatar-v3.png')
			{
		...
		{
			this.callOverlayPhotoCompanion.src  = '/thurly/js/im/images/hidef-avatar-v3.png';
		}
...
BX.IM.WebRTC.prototype.startCallExperimental = function(roomId)
{
	...
	this.callView = new CallView({
		...
		server: 'https://testphone.thurly.info:8089/janus',
...
BX.IM.ScreenSharing.prototype.setLocalAndSend = function(userId, desc)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_SHARING' : 'Y', 'COMMAND': 'signaling', 'USER_ID' : userId, 'PEER': JSON.stringify( desc ), 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()}
	});
...
BX.IM.ScreenSharing.prototype.onIceCandidate = function (userId, candidates)
{
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_SHARING' : 'Y', 'COMMAND': 'signaling', 'USER_ID' : userId, 'PEER': JSON.stringify(candidates), 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()}
	});
}
...
BX.IM.ScreenSharing.prototype.peerConnectionReconnect = function (userId)
{
	...
	BX.ajax({
		...
		dataType: 'json',
		timeout: 30,
		data: {'IM_SHARING' : 'Y', 'COMMAND': 'reconnect', 'USER_ID' : userId, 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(){
			this.initPeerConnection(userId, true);
		}, this)
	});
...
BX.IM.ScreenSharing.prototype.callCommand = function(command, async)
{
	...
	BX.ajax({
		...
		timeout: 30,
		async: async != false,
		data: {'IM_SHARING' : 'Y', 'COMMAND': command, 'USER_ID': this.callUserId, 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()}
	});
};
	...
	BX.garbage(function(){
		...
		if (chatId > 0)
		{
			BX.ajax({
				...
				timeout: 30,
				async: false,
				data: {'IM_FILE_UNREGISTER' : 'Y', CHAT_ID: chatId, FILES: JSON.stringify(this.filesProgress), MESSAGES: JSON.stringify(messages), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()}
			});
		}
	}, this);
};
							...
							var enableNode = BX.create('div', {children: [
								...
								BX.create('span', {props: {className: 'bx-messenger-ajax'}, html: BX.message('IM_D_SEND_PUBLIC_ADMIN_ON'), events: {'click': BX.delegate(function(){
									...
									BX.ajax({
										...
										dataType: 'json',
										timeout: 30,
										data: {'IM_DISK_ACTIVATE_PUBLIC_LINK' : 'Y', 'STATUS' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
										onsuccess: BX.delegate(function(data){
...
BX.IM.DiskManager.prototype.drawHistoryFiles = function(chatId, fileId, params)
{
	...
	for (var i = 0; i < fileIds.length; i++)
	{
		...
		var fileDate = BX.MessengerCommon.formatDate(file.date, [
			...
			["today", "today"],
			["yesterday", "yesterday"],
			["", BX.date.convertThurlyFormat(BX.message("FORMAT_DATE"))]
		]);
...
BX.IM.DiskManager.prototype.chatDialogInit = function()
{
	...
	if (BX.DiskFileDialog)
	{
		...
		BX.addCustomEvent(BX.DiskFileDialog, 'loadItems', BX.delegate(function(link, name)
		{
			...
			BX.DiskFileDialog.target[name] = link.replace('/thurly/tools/disk/uf.php', this.BXIM.pathToFileAjax);
...
BX.IM.DiskManager.prototype.saveToDisk = function(chatId, fileId, params)
{
	...
	BX.ajax({
		...
		skipAuthCheck: true,
		timeout: 30,
		data: {'IM_FILE_SAVE_TO_DISK' : 'Y', CHAT_ID: chatId, FILE_ID: fileId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data) {
...
BX.IM.DiskManager.prototype.deleteFile = function(chatId, fileId, params)
{
	...
	BX.ajax({
		...
		skipAuthCheck: true,
		timeout: 30,
		data: {'IM_FILE_DELETE' : 'Y', CHAT_ID: chatId, FILE_ID: fileId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data)
		{
...
BX.IM.DiskManager.prototype.uploadFromDisk = function(tab, path, selected, text)
{
	...
	BX.ajax({
		...
		skipAuthCheck: true,
		timeout: 30,
		data: {'IM_FILE_UPLOAD_FROM_DISK' : 'Y', CHAT_ID: chatId, RECIPIENT_ID: recipientId, MESSAGE: text, MESSAGE_TMP_ID: tmpMessageId, 'OL_SILENT': olSilentMode, FILES: JSON.stringify(paramsFileId), 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
		onsuccess: BX.delegate(function(data) {
			if (data.ERROR != '')
			{
				this.messenger.sendMessageFlag--;
