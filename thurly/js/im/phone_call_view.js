...
(function()
{
	...
	BX.PhoneCallView.prototype.setLineNumber = function(lineNumber)
	{
		...
		this.setOnSlave(desktopEvents.setCallId, [lineNumber]);
	};
	...
	BX.PhoneCallView.prototype.loadCrmCard = function(entityType, entityId, tryNumber, skipChangeEvent)
	{
		...
		BX.ajax({
			...
			onsuccess: function(HTML)
			{
				...
				if(possibleError.ERROR === 'SESSION_ERROR' && possibleError.THURLY_SESSID && tryNumber <= maxTryCount)
				{
					BX.message({'thurly_sessid': possibleError.THURLY_SESSID});
					setTimeout(function()
					{
	...
	BX.PhoneCallView.prototype.getUiStateButtons = function(uiState)
	{
		...
		switch (uiState)
		{
			...
			case BX.PhoneCallView.UiState.sipPhoneError:
				...
				break;
			case BX.PhoneCallView.UiState.connected:
				result.buttons = ['hold', 'mute', 'qualityMeter', 'fold'];
				if(!this.callListId)
	...
	BX.PhoneCallView.prototype.loadRestApp = function(params)
	{
		...
		BX.ajax({
			url: '/thurly/tools/voximplant/rest_app.php',
			method: 'POST',
			dataType: 'html',
			data: data,
			onsuccess: function(HTML)
	...
	Desktop.prototype.isFeatureSupported = function(featureName)
	{
		...
		return desktopFeatureMap[featureName] >= this.getVersion();
	};
	...
	{
		return (BX.MessengerCommon.isDesktop() ? '/desktop_app/call_list.ajax.php' : '/thurly/components/thurly/crm.activity.call_list/ajax.php')
	};
	...
	TransferPopup.prototype.render = function()
	{
		return BX.create("div", { props : { className : "bx-messenger-popup-newchat-wrap" }, children: [
			...
			BX.create("div", { props : { className : "bx-messenger-popup-newchat-box bx-messenger-popup-newchat-dest bx-messenger-popup-newchat-dest-even" }, children: [
				...
				this.elements.input = BX.create("input", {props : { className : "bx-messenger-input" }, attrs: {type: "text", placeholder: BX.message(this.BXIM.thurlyIntranet ? 'IM_M_SEARCH_PLACEHOLDER_CP': 'IM_M_SEARCH_PLACEHOLDER'), value: ''}})
			]}),
	...
	FormManager.prototype.load = function(params)
	{
		...
		window.ThurlyOSFormLoader.load(formData);
	...
	FormManager.prototype.unload = function()
	{
		if(this.currentForm)
		{
			window.ThurlyOSFormLoader.unload(this.currentForm);
			this.currentForm = null;
		}
	};
