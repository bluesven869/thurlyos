...
BXMedialibAdmin.prototype =
{
	...
	BuildCollection: function(oCol, ind)
	{
		...
		if (pCont)
		{
			var
				...
				img = titleDiv.appendChild(BX.create("IMG", {props:{src: '/thurly/images/1.gif', className: 'ml-col-icon ml-col-icon-closed'}})),
				arHideItems = {length: 0},
				ch = titleDiv.appendChild(BX.create("INPUT", {props:{type: 'checkbox', value: 'c_' + oCol.id}})),
				menuIc = titleDiv.appendChild(BX.create("IMG", {props:{src: '/thurly/images/1.gif', className: 'ml-col-menu', id: 'mlccm_' + oCol.id}})),
			...
			for (i = 0; i < 10; i++)
				html += "<img src='/thurly/images/1.gif' />";
	...
	DisplayItem: function(Params)
	{
		var
			...
			tmbImg = itemDiv.appendChild(BX.create("IMG", {props:{src: oItem.thumb_path || '/thurly/images/1.gif', className: 'ml-item-thumb'}})),
		...
		var view = butCont.appendChild(BX.create("IMG", {props: {src: '/thurly/images/1.gif', className: 'ml-item-view', title: ML_MESS.ViewItem}}));
		...
		if (Params.Access.edit || Params.Access.del)
		{
			...
			{
				var edit = butCont.appendChild(BX.create("IMG", {props: {src: '/thurly/images/1.gif', className: 'ml-item-edit', title: ML_MESS.EditItem}}));
			...
			{
				var del = butCont.appendChild(BX.create("IMG", {props:{src: '/thurly/images/1.gif', className: 'ml-item-del', title: ML_MESS.DelItem}}));
	...
	_AddCollToItem: function(id, checkAccess)
	{
		...
		var
			...
			pDel = pDiv.appendChild(BX.create("IMG", {props:{src: '/thurly/images/1.gif', className: 'ml-col-del', title: ML_MESS.DelColFromItem, id: 'mlsd_it_' + id}}, this.EditItemDialog.pFrameDoc));
	...
	{
		return '/thurly/admin/fileman_medialib.php?sessid=' + (sessid || this.sessid) + '&lang=' + this.oConfig.lang + (action ? '&action=' + action : '');
	},
	...
	InitContextMenu: function()
	{
		...
		[
			...
			{id: 'access', name: ML_MESS.Access,title: ML_MESS.AccessTitle,handler: function(pEl){window.location = "/thurly/admin/fileman_medialib_access.php?col_id=" + pEl.id.substr('mlccm_'.length);}},
	...
	TypeOnChange: function(Params)
	{
		if (this.Types[Params.typeInd].id != this.curType.id)
			window.location = "/thurly/admin/fileman_medialib_admin.php?lang=" + this.oConfig.lang + "&type=" + this.Types[Params.typeInd].id; // + '&sessid=' + this.sessid;
	},
