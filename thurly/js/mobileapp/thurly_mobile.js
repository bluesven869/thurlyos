...
(function ()
{
	 ...
	 * @param params.values - The array of values. For example - ["apple","google","thurly"]
	 * @param params.default_value - The selected item by default. For example - "thurly"
...
//#############################
	...
	app.passVar = function (variable, key)
	{
		...
		if (evalVar)
		{
			...
			if (platform.toUpperCase() == "ANDROID")
			{
				...
				key = key || false;
				if (key)
					ThurlyOSAndroid.receiveStringValue(JSON.stringify({variable: evalVar, key: key}));
				else
					ThurlyOSAndroid.receiveStringValue(evalVar);
			} else
			{
				return evalVar;
			}
	...
	app.BasicAuth = function (params)
	{
		...
		var authParams = {
			...
			success: function (data)
			{
				...
				{
					if (BX.message.thurly_sessid != data.sessid_md5)
					{
						BX.message.thurly_sessid = data.sessid_md5;
	...
	window.ThurlyAnimation = {
		...
		animate: function (options)
		{
			...
			return ThurlyAnimation.animateProgress(options);
		},

		animateProgress: function (options)
		{
			var start = new Date();
			var delta = options.transition || ThurlyAnimation.transitions.linear;
...
//Events' handlers
	...
	document.addEventListener("deviceready", function ()
	{
		...
		BX.addCustomEvent("onSessIdChanged", function (data)
			{
				BX.message.thurly_sessid = data.sessid;
			}
		);
