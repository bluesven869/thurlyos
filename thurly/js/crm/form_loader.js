var ThurlyOSFormLoader = {
	...
	init: function()
	{
		...
		if(!window.ThurlyOSFormObject || !window[window.ThurlyOSFormObject])
			return;
		if(!window[window.ThurlyOSFormObject].forms)
			return;

		window[window.ThurlyOSFormObject].forms.forEach(this.preLoad, this);
	},
		...
		switch (data.action)
		{
			...
			case 'analytics':
				data.value.forEach(function(item) {
					if (item.type == 'ga' && window.dataLayer)
					{
					...
					else if (item.type == 'ya' && !window['yaCounter' + item.yaId])
					{
						if (!this.yaId)
						{
							if (window['Ya'] && Ya.Metrika && Ya.Metrika.counters()[0])
							{
	...
	onFrameLoad: function(uniqueLoadId)
	{
		...
		if(typeof window.postMessage === 'function' && !ie)
		{
			...
			if (window.BX && window.BX.SiteGuest)
			{
				var pages = window.BX.SiteGuest.getPages();
				if (pages && pages.length > 0)
				{
					frameParameters.visitedPages = pages;
					this.addEventHandler(form, 'send', function (data) {
						//window.BX.SiteGuest.clearPages();
						window.BX.SiteGuest.send('link', data.gid);
					});
				}
			}
			...
			form.iframe.contentWindow.postMessage(
				JSON.stringify(frameParameters), this.domain
			);
		}
		else
		{
