...
class Manager
{
	...
	public static function resolveIblock($iblockId)
	{
		...
		{
			self::$catalog = \Thurly\Main\Loader::includeModule("catalog");
		}
	 ...
	 * @param integer $iblockId Information block identifier.
	 *
	 * @return \Thurly\Iblock\PropertyIndex\Indexer
	 * @throws \Thurly\Main\Db\SqlQueryException
	 */
	...
	{
		\Thurly\Iblock\IblockTable::update($iblockId, array(
			"PROPERTY_INDEX" => "I",
		));
		...
		{
			\Thurly\Iblock\IblockTable::update($productIblock, array(
				"PROPERTY_INDEX" => "I",
			));
		}
			...
			if (!$notifyList->fetch())
			{
				\CAdminNotify::add(array(
					"MESSAGE" => Loc::getMessage("IBLOCK_NOTIFY_PROPERTY_REINDEX", array(
						"#LINK#" => "/thurly/admin/iblock_reindex.php?lang=".\Thurly\Main\Application::getInstance()->getContext()->getLanguage(),
					)),
					"TAG" => "iblock_property_reindex",
					"MODULE_ID" => "iblock",
					"ENABLE_CLOSE" => "Y",
