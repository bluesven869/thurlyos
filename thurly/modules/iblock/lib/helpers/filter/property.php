<?php
namespace Thurly\Iblock\Helpers\Filter;
...
use Thurly\Main\UI\Filter\Options as FilterOptions;
 ...
 *
 * @package Thurly\Iblock\Helpers\Filter
 */
class Property
{
	/**
	...
	protected static function renderByECrm($filterId, $listProperty)
	{
		...
		Asset::getInstance()->addCss('/thurly/js/crm/css/crm.css');
		Asset::getInstance()->addJs('/thurly/js/crm/crm.js');
	...
	protected static function renderByEmployee($filterId, array $listProperty)
	{
		...
		if(!empty($listProperty))
		{
			...
			ob_start();
			foreach($listProperty as $property):
				$APPLICATION->includeComponent('thurly:intranet.user.selector.new', '',
					array(
						'MULTIPLE' => 'N',
	...
	protected static function addFilterByE($property, $controlSettings, &$filter, &$filtered)
	{
		...
		$filterOption = new \Thurly\Main\UI\Filter\Options($controlSettings["FILTER_ID"]);
		$filterData = $filterOption->getFilter();
