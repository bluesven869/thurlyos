...
class Helper
{
	 ...
	 *
	 * @param \Thurly\Iblock\InheritedProperty\BaseTemplate $ipropTemplates Templates to lookup.
	...
	public static function makeFileName(
		\Thurly\Iblock\InheritedProperty\BaseTemplate $ipropTemplates,
		$templateName,
		array $fields,
		array $file
	)
		...
		if (preg_match("/^(.+)(\\.[a-zA-Z0-9]+)\$/", $file["name"], $fileName))
		{
			...
			if ($TEMPLATE != "")
			{
				...
				if ($template != "")
				{
					...
					return \Thurly\Iblock\Template\Engine::process($entity, $TEMPLATE).$fileName[2];
				}
				elseif ($modifiers != "")
				{
