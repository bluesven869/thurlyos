...
namespace Thurly\Iblock\Component;
/**
 * Class Tools
 * Provides various useful methods.
 *
 * @package Thurly\Iblock\Component
 */
class Tools
{
	 ...
	 *
	 * @param string $message Message to show with thurly:system.show_message component.
	...
	public static function process404($message = "", $defineConstant = true, $setStatus = true, $showPage = false, $pageFile = "")
	{
		...
		if ($showPage)
		{
			if ($APPLICATION->RestartWorkarea())
			{
				...
				\Thurly\Main\Page\Frame::setEnable(false);
				if ($pageFile)
					require(\Thurly\Main\Application::getDocumentRoot().rel2abs("/", "/".$pageFile));
				else
					require(\Thurly\Main\Application::getDocumentRoot()."/404.php");
				die();
			}
		}
	}
