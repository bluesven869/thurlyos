...
class BaseTemplate
{
	/** @var \Thurly\Iblock\InheritedProperty\BaseValues|null */
	...
	{
		$templateList = \Thurly\Iblock\InheritedPropertyTable::getList(array(
		...
		while ($row = $templateList->fetch())
		{
			...
			if (array_key_exists($CODE, $templates))
			{
				...
				{
					if ($templates[$CODE] != "")
						\Thurly\Iblock\InheritedPropertyTable::update($row["ID"], array(
							"TEMPLATE" => $templates[$CODE],
						));
					else
						\Thurly\Iblock\InheritedPropertyTable::delete($row["ID"]);
		...
		if (!empty($templates))
		{
			foreach ($templates as $CODE => $TEMPLATE)
			{
				if ($TEMPLATE != "")
				{
					\Thurly\Iblock\InheritedPropertyTable::add(array(
						"IBLOCK_ID" => $this->entity->getIblockId(),
						"CODE" => $CODE,
						"ENTITY_TYPE" => $this->entity->getType(),
						"ENTITY_ID" => $this->entity->getId(),
