...
require_once($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/iblock/classes/general/subelement.php');
...
do{ //one iteration loop
	...
	if ('' != $strProductName)
	{
		...
		$arPostParams['sessid'] = thurly_sessid();
	}
	...
	)
	{
		...
		if (!(check_thurly_sessid() || $_SESSION['IBLOCK_CUSTOM_FORM']===true))
		{
		...
		else
		{
			...
			if (
				$arShowTabs['catalog']
				&& file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/admin/templates/subproduct_edit_validator.php")
			)
			{
				// errors'll be appended to $strWarning;
				include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/admin/templates/subproduct_edit_validator.php");
			}
			if ($arShowTabs['product_group'])
			{
			...
			if ($strWarning == '')
			{
				...
				if ($strWarning == '')
				{
					...
					if ($arShowTabs['catalog'])
					{
						include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/admin/templates/subproduct_edit_action.php");
					}
					if ($arShowTabs['product_group'])
					{
						CCatalogAdminProductSetEdit::setSkuFormParams(array('TYPE' => CCatalogProductSet::TYPE_GROUP));
	...
	if (!$changeTabs && !empty($dontsave) && check_thurly_sessid())
	{
		if($bWorkflow)
			CIBlockElement::WF_UnLock($ID);
...
if ($arTranslit["TRANSLITERATION"] == "Y")
{
	...
	<td style="white-space: nowrap;">
		<input type="text" size="50" name="SUB_NAME" id="SUB_NAME" maxlength="255" value="<?echo $str_NAME?>"><image id="sub_name_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/thurly/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()">
	</td>
</tr><?
	...
	<td><?echo $tabControl->GetCustomLabelHTML()?></td>
	<td style="white-space: nowrap;">
		<input type="text" size="50" name="SUB_CODE" id="SUB_CODE" maxlength="255" value="<?echo $str_CODE?>"><image id="sub_code_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/thurly/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()">
	</td>
</tr><?
...
if ($arShowTabs['catalog'])
{
	...
	include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/admin/templates/subproduct_edit.php");
...
if ($arShowTabs['bizproc']):
	...
	foreach ($arDocumentStates as $arDocumentState)
	{
		...
		<tr>
			...
			<td width="60%"><?if (strlen($arDocumentState["ID"]) > 0):?><a href="/thurly/admin/bizproc_log.php?ID=<?= $arDocumentState["ID"] ?>"><?endif;?><?= strlen($arDocumentState["STATE_TITLE"]) > 0 ? $arDocumentState["STATE_TITLE"] : $arDocumentState["STATE_NAME"] ?><?if (strlen($arDocumentState["ID"]) > 0):?></a><?endif;?></td>
		</tr>
		<?endif;?>
		<?
		if (strlen($arDocumentState["ID"]) <= 0)
	...
	if ($ID > 0):
		...
		if ($bStartWorkflowPermission):
			...
			<tr>
				<td colspan="2" align="center">
					<a href="/thurly/admin/<?=MODULE_ID?>_start_bizproc.php?document_id=<?= $ID ?>&document_type=<?= DOCUMENT_TYPE ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>"><?echo GetMessage("IBEL_BIZPROC_START")?></a>
				</td>
			</tr>
			<?
		endif;
...
if ($ID > 0 && !$bSubCopy && BX_SUB_SETTINGS)
{
	if (!$ajaxReload)
	{
		...
		if ($arShowTabs['product_group'])
		{
			...
			$reloadParams['sessid'] = thurly_sessid();
			$setBtn = "{
