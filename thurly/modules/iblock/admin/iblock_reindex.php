...
{
	$OfferIblocks = array();
	$offersIterator = \Thurly\Catalog\CatalogIblockTable::getList(array(
...
$iblockList = \Thurly\Iblock\IblockTable::getList(array(
...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["Reindex"]=="Y")
{
	...
	else
	{
		...
		$index = \Thurly\Iblock\PropertyIndex\Manager::createIndexer($IBLOCK_ID);
		...
		{
			$index->endIndex();
			\Thurly\Iblock\PropertyIndex\Manager::checkAdminNotification();
			CThurlyComponent::clearComponentCache("thurly:catalog.smart.filter");
...
elseif (empty($iblockDropDown))
{
	...
	$aMenu = array(
		array(
			...
			"LINK" => '/thurly/admin/iblock_reindex_admin.php?lang='.LANGUAGE_ID,
			"ICON" => "btn_list",
		)
	);
