...
if($arIBTYPE===false)
	LocalRedirect("/thurly/admin/iblock_type_admin.php?lang=".LANGUAGE_ID);
...
while($dbrs = $rsIBlocks->NavNext(true, "f_"))
{
	...
	)
	{
		...
		$row->AddViewField("WORKFLOW_TEMPLATES", '<a href="/thurly/admin/iblock_bizproc_workflow_admin.php?document_type=iblock_'.$f_ID.'&lang='.LANGUAGE_ID.'&back_url_list='.urlencode($APPLICATION->GetCurPageParam("", array())).'">'.$cnt.'</a>');
	}
	...
	)
	{
		$arActions[] = array(
			...
			"ACTION"=>"window.location='/thurly/admin/iblock_bizproc_workflow_admin.php?document_type=iblock_".$f_ID."&lang=".LANGUAGE_ID.'&back_url_list='.urlencode($APPLICATION->GetCurPageParam("", array()))."';"
		);
	}
