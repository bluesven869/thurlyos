	...
	if ($lAdmin->EditAction())
	{
		...
		if ($boolSubCatalog)
		{
			...
			if ($intSubPropValue > 0)
			{
				...
				$ipropValues = new \Thurly\Iblock\InheritedProperty\ElementValues($productIblock, $intSubPropValue);
				$ipropValues->clearValues();
				\Thurly\Iblock\PropertyIndex\Manager::updateElementIndex($productIblock, $intSubPropValue);
				unset($productIblock);
			}
		}
	}
...
if (!(false == B_ADMIN_SUBELEMENTS_LIST && $bCopy))
{
	...
	while ($arRes = $rsData->NavNext(true, "f_"))
	{
		...
		if ($f_LOCKED_USER_NAME)
			$row->AddViewField("LOCKED_USER_NAME", '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$f_WF_LOCKED_BY.'" title="'.GetMessage("IBEL_A_USERINFO").'">'.$f_LOCKED_USER_NAME.'</a>');
		if ($f_USER_NAME)
			$row->AddViewField("USER_NAME", '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$f_MODIFIED_BY.'" title="'.GetMessage("IBEL_A_USERINFO").'">'.$f_USER_NAME.'</a>');
		$row->AddViewField("CREATED_USER_NAME", '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$f_CREATED_BY.'" title="'.GetMessage("IBEL_A_USERINFO").'">'.$f_CREATED_USER_NAME.'</a>');
		...
		if ($boolSubBizproc)
		{
			...
			foreach ($arDocumentStates as $kk => $vv)
			{
				...
				$arStr[$vv["TEMPLATE_ID"]] .= "<a href=\"/thurly/admin/bizproc_log.php?ID=".$kk."\">".(strlen($vv["STATE_TITLE"]) > 0 ? $vv["STATE_TITLE"] : $vv["STATE_NAME"])."</a><br />";
...
function ShowNewOffer(id)
{
	...
	else
	{
		...
		PostParams.sessid = BX.thurly_sessid();
		(new BX.CAdminDialog({
...
?>
	...
	(new BX.CAdminDialog({
		'content_url': '/thurly/tools/catalog/iblock_subelement_generator.php?subIBlockId=<? echo $intSubIBlockID; ?>&subPropValue=<? echo $intSubPropValue; ?>&subTmpId=<? echo $strSubTMP_ID; ?>&iBlockId=<? echo $arSubCatalog['PRODUCT_IBLOCK_ID']; ?>',
		'content_post': PostParams,
		'draggable': true,
		'resizable': true,
