...
require_once($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/iblock/prolog.php');
...
Main\Page\Asset::getInstance()->addJs('/thurly/js/iblock/iblock_edit.js');
...
do{ //one iteration loop
	...
	if (!$errorTriger)
	{
		...
		if(
			is_array($PROP)
			&& !empty($arFileProps)
			&& !class_exists('\Thurly\Main\UI\FileInput', true)
		)
		{
		...
		)
		{
			if (CCatalogAdminTools::changeTabs($IBLOCK_ID, $ID, $arMainCatalog))
			{
				...
				LocalRedirect("/thurly/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, $arUrlParams, "&".$tabControl->ActiveTabParam()));
			}
		}
		...
		)
		{
			...
			if(!check_thurly_sessid() && !$bCustomForm)
			{
			...
			else
			{
				if ($bCatalog)
				{
					...
					if (
						$arShowTabs['catalog']
						&& file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/admin/templates/product_edit_validator.php")
					)
					{
					// errors'll be appended to $strWarning;
						...
						include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/admin/templates/product_edit_validator.php");
					}
					if ($arShowTabs['product_set'])
					{
				...
				if ($strWarning == '')
				{
					...
					{
						$ELEMENT_PREVIEW_PICTURE_FILE_NAME = \Thurly\Iblock\Template\Helper::convertArrayToModifiers($_POST["IPROPERTY_TEMPLATES"]["ELEMENT_PREVIEW_PICTURE_FILE_NAME"]);
						$ELEMENT_DETAIL_PICTURE_FILE_NAME = \Thurly\Iblock\Template\Helper::convertArrayToModifiers($_POST["IPROPERTY_TEMPLATES"]["ELEMENT_DETAIL_PICTURE_FILE_NAME"]);
					...
					{
						$ipropValues = new \Thurly\Iblock\InheritedProperty\ElementValues($IBLOCK_ID, $ID);
					...
					if ('' == $strWarning && $bCatalog)
					{
						...
						if ($arShowTabs['catalog'])
						{
							include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/admin/templates/product_edit_action.php");
						}
			...
			else
			{
				...
				if(strlen($apply) <= 0 && strlen($save_and_add) <= 0)
				{
					...
					{
						LocalRedirect("/thurly/admin/".CIBlock::GetAdminElementListLink($IBLOCK_ID, array('find_section_section'=> $find_section_section)));
					}
				}
				elseif(strlen($save_and_add) > 0)
				{
					...
					{
						LocalRedirect("/thurly/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, 0, $params, "&".$tabControl->ActiveTabParam()));
					}
				}
				else
				{
					LocalRedirect("/thurly/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array(
						"WF" => ($WF=="Y"? "Y": null),
						"find_section_section" => $find_section_section,
						"return_url" => (strlen($return_url) > 0? $return_url: null),
		...
		if(!empty($dontsave) && check_thurly_sessid())
		{
			if($bWorkflow)
				CIBlockElement::WF_UnLock($ID);
...
else
{
	...
	else
	{
		...
		if($arElement = $result->ExtractFields("str_"))
		{
			...
			$ipropTemlates = new \Thurly\Iblock\InheritedProperty\ElementTemplates($IBLOCK_ID, $WF_ID);
		}
		else
		{
			$WF_ID=0;
			...
			$ipropTemlates = new \Thurly\Iblock\InheritedProperty\ElementTemplates($IBLOCK_ID, 0);
			$ipropTemlates->getValuesEntity()->setParents($str_IBLOCK_ELEMENT_SECTION);
		}
		$str_IPROPERTY_TEMPLATES = $ipropTemlates->findTemplates();
		$str_IPROPERTY_TEMPLATES["ELEMENT_PREVIEW_PICTURE_FILE_NAME"] = \Thurly\Iblock\Template\Helper::convertModifiersToArray($str_IPROPERTY_TEMPLATES["ELEMENT_PREVIEW_PICTURE_FILE_NAME"]);
		$str_IPROPERTY_TEMPLATES["ELEMENT_DETAIL_PICTURE_FILE_NAME"] = \Thurly\Iblock\Template\Helper::convertModifiersToArray($str_IPROPERTY_TEMPLATES["ELEMENT_DETAIL_PICTURE_FILE_NAME"]);
	}
	...
	if ( !$bAutocomplete && !$bPropertyAjax )
	{
		...
		if (!$bCopy)
		{
			$aMenu[] = array(
				...
				"LINK"=>"/thurly/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array(
		...
		if($ID > 0 && !$bCopy)
		{
			...
			$arSubMenu[] = array(
				...
				"LINK" => "/thurly/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, 0, array(
					"IBLOCK_SECTION_ID" => $MENU_SECTION_ID,
					"find_section_section" => $find_section_section,
				)),
				'ICON' => 'edit',
			...
			if($bWorkflow && !defined("CATALOG_PRODUCT"))
			{
				$arSubMenu[] = array(
					...
					"LINK" => '/thurly/admin/iblock_history_list.php?ELEMENT_ID='.$ID.'&type='.urlencode($arIBlock["IBLOCK_TYPE_ID"]).'&lang='.LANGUAGE_ID.'&IBLOCK_ID='.$IBLOCK_ID.'&find_section_section='.$find_section_section,
				);
			}
...
if($customFormFile):
	...
	if($arTranslit["TRANSLITERATION"] == "Y")
	{
		...
		function set_linked()
		{
			...
			{
				if(linked)
					name_link.src='/thurly/themes/.default/icons/iblock/link.gif';
				else
					name_link.src='/thurly/themes/.default/icons/iblock/unlink.gif';
			}
			var code_link = document.getElementById('code_link');
			if(code_link)
			{
				if(linked)
					code_link.src='/thurly/themes/.default/icons/iblock/link.gif';
				else
					code_link.src='/thurly/themes/.default/icons/iblock/unlink.gif';
			}
	...
	<script type="text/javascript">
		var InheritedPropertiesTemplates = new JCInheritedPropertiesTemplates(
			...
			'/thurly/admin/iblock_templates.ajax.php?ENTITY_TYPE=E&IBLOCK_ID=<?echo intval($IBLOCK_ID)?>&ENTITY_ID=<?echo intval($ID)?>'
		);
		BX.ready(function(){
			setTimeout(function(){
...
$tabControl->Begin(array(
	"FORM_ACTION" => "/thurly/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, null, $arEditLinkParams)
));
...
if($arTranslit["TRANSLITERATION"] == "Y")
{
	...
	?>
		<tr id="tr_NAME">
			...
			<td style="white-space: nowrap;">
				<input type="text" size="50" name="NAME" id="NAME" maxlength="255" value="<?echo $str_NAME?>"><img id="name_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/thurly/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
	...
	?>
		<tr id="tr_CODE">
			<td><?echo $tabControl->GetCustomLabelHTML()?></td>
			<td style="white-space: nowrap;">
				<input type="text" size="50" name="CODE" id="CODE" maxlength="255" value="<?echo $str_CODE?>"><img id="code_link" title="<?echo GetMessage("IBEL_E_LINK_TIP")?>" class="linked" src="/thurly/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
	...
	if (!$bAutocomplete && ($ID > 0 && !$bCopy))
	{
		...
		if ($arLinkedProp)
		{
			...
			?><tr id="tr_LINKED_PROP<?echo $arLinkedProp["ID"]?>">
				<td colspan="2"><a title="<?=$linkedTitle; ?>" href="/thurly/admin/<?=htmlspecialcharsbx(CIBlock::GetAdminElementListLink($arLinkedProp["IBLOCK_ID"], array('set_filter'=>'Y', 'find_el_property_'.$arLinkedProp["ID"]=>$ID, 'find_section_section' => -1)))?>"><?=htmlspecialcharsbx(CIBlock::GetArrayByID($arLinkedProp["IBLOCK_ID"], "NAME").": ".$elements_name); ?></a></td>
			</tr><?
			}
...
?>
	<tr id="tr_PREVIEW_PICTURE" class="adm-detail-file-row">
		...
		<td width="60%">
			...
			<?else:?>
				<?
				if (class_exists('\Thurly\Main\UI\FileInput', true))
				{
					echo \Thurly\Main\UI\FileInput::createInstance(array(
...
$tabControl->EndCustomField("DETAIL_TEXT",
	...
	?>
		<tr id="tr_TAGS">
			...
			<td>
				<?if(Thurly\Main\Loader::includeModule('search')):
					$arLID = array();
	...
	<?else:?>
		...
		<td width="60%">
			...
			?>
				...
				<input type="button" value="<?echo GetMessage("IBLOCK_ELEMENT_EDIT_PROP_ADD")?>..." onClick="jsUtils.OpenWindow('/thurly/admin/iblock_section_search.php?lang=<?echo LANGUAGE_ID?>&amp;IBLOCK_ID=<?echo $IBLOCK_ID?>&amp;n=input_IBLOCK_SECTION&amp;m=y&amp;iblockfix=y&amp;tableId=iblocksection-<?=$IBLOCK_ID; ?>', 900, 700);">
		</td>
	<?endif;?>
	</tr>
...
if ($arShowTabs['catalog'])
{
	...
	include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/admin/templates/product_edit.php");
...
if ($arShowTabs['sku'])
{
	...
	$boolSubWorkFlow = Thurly\Main\Loader::includeModule("workflow") && $arSubIBlock["WORKFLOW"] != "N";
	$boolSubBizproc = Thurly\Main\Loader::includeModule("bizproc") && $arSubIBlock["BIZPROC"] != "N";

	$intSubPropValue = (0 == $ID || $bCopy ? '-'.$TMP_ID : $ID);
	$strSubTMP_ID = $TMP_ID;

	$strSubElementAjaxPath = '/thurly/admin/iblock_subelement_admin.php?WF=Y&IBLOCK_ID='.$intSubIBlockID.'&type='.urlencode($strSubIBlockType).'&lang='.LANGUAGE_ID.'&find_section_section=0&find_el_property_'.$arSubCatalog['SKU_PROPERTY_ID'].'='.((0 == $ID) || ($bCopy) ? '-'.$TMP_ID : $ID).'&TMP_ID='.urlencode($strSubTMP_ID);
	if ($boolIncludeOffers && file_exists($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/iblock/admin/templates/iblock_subelement_list.php'))
	{
		require($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/iblock/admin/templates/iblock_subelement_list.php');
	}
	else
	{
...
if ($arShowTabs['bizproc']):
	...
	foreach ($arDocumentStates as $arDocumentState)
	{
		...
		<tr class="heading">
			<td colspan="2">
				...
				<?if (strlen($arDocumentState["ID"]) > 0 && strlen($arDocumentState["WORKFLOW_STATUS"]) > 0):?>
					(<a href="<?echo htmlspecialcharsbx("/thurly/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array(
						"WF"=>$WF,
						"find_section_section" => $find_section_section,
						"stop_bizproc" => $arDocumentState["ID"],
					),  "&".thurly_sessid_get()))?>"><?echo GetMessage("IBEL_BIZPROC_STOP")?></a>)
				<?endif;?>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td width="60%"><?if (strlen($arDocumentState["ID"]) > 0):?><a href="/thurly/admin/bizproc_log.php?ID=<?= $arDocumentState["ID"] ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>"><?endif;?><?= strlen($arDocumentState["STATE_TITLE"]) > 0 ? $arDocumentState["STATE_TITLE"] : $arDocumentState["STATE_NAME"] ?><?if (strlen($arDocumentState["ID"]) > 0):?></a><?endif;?></td>
		</tr>
		<?endif;?>
		<?
		if (strlen($arDocumentState["ID"]) <= 0)
			...
			if (!empty($arTasks))
			{
				...
				<tr>
					...
					<td width="60%">
						...
						{
							?><a href="/thurly/admin/bizproc_task.php?id=<?= $arTask["ID"] ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>" title="<?= strip_tags($arTask["DESCRIPTION"]) ?>"><?= $arTask["NAME"] ?></a><br /><?
						}
						?>
					</td>
				</tr>
	...
	if ($ID > 0):
		...
		if ($bStartWorkflowPermission):
			...
			<tr>
				<td colspan="2" align="center">
					<a href="/thurly/admin/<?=MODULE_ID?>_start_bizproc.php?document_id=<?= $ID ?>&document_type=<?= DOCUMENT_TYPE ?>&back_url=<?= urlencode($APPLICATION->GetCurPageParam("", array('bxpublic'))) ?>"><?echo GetMessage("IBEL_BIZPROC_START")?></a>
				</td>
			</tr>
			<?
		endif;
...
elseif(!$bPropertyAjax && (!isset($_REQUEST['nobuttons']) || $_REQUEST['nobuttons'] !== "Y")):
	...
	$edit_in_panel = "{
		...
		className: 'adm-btn-add',
		action: function () {
			location.href = '/thurly/admin/".CIBlock::GetAdminElementEditLink(
			$IBLOCK_ID,
			$ID,
			$editInPanelParams
		)."';
...
)
{
	...
	echo
		...
		' <a href="/thurly/admin/iblock_edit.php?type='.htmlspecialcharsbx($type).'&amp;lang='.LANGUAGE_ID.'&amp;ID='.$IBLOCK_ID.'&amp;admin=Y&amp;return_url='.urlencode("/thurly/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ID, array("WF" => ($WF=="Y"? "Y": null), "find_section_section" => $find_section_section, "return_url" => (strlen($return_url)>0? $return_url: null)))).'">',
