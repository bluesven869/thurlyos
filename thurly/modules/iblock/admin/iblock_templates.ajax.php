...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/iblock/admin_tools.php");
...
if (check_thurly_sessid())
{
	...
	{
		$ipropTemplates = new \Thurly\Iblock\InheritedProperty\IblockTemplates($_REQUEST["ENTITY_ID"]);
		$arFields = array(
			"NAME" => $_POST["NAME"],
			"CODE" => $_POST["CODE"],
	...
	{
		$ipropTemplates = new \Thurly\Iblock\InheritedProperty\SectionTemplates($_REQUEST["IBLOCK_ID"], $_REQUEST["ENTITY_ID"]);
		$arFields = array(
	...
	{
		$ipropTemplates = new \Thurly\Iblock\InheritedProperty\ElementTemplates($_REQUEST["IBLOCK_ID"], $_REQUEST["ENTITY_ID"]);
	...
	if ($ipropTemplates)
	{
		...
		if (is_array($_POST["IPROPERTY_TEMPLATES"]))
		{
			foreach ($_POST["IPROPERTY_TEMPLATES"] as $TEMPLATE_NAME => $TEMPLATE_VALUE)
			{
				$templates[$TEMPLATE_NAME] = array(
					"TEMPLATE" => \Thurly\Iblock\Template\Helper::convertArrayToModifiers($TEMPLATE_VALUE),
				);
			}
		}
		...
		foreach ($templates as $TEMPLATE_NAME => $templateInfo)
		{
			$result[] = array(
				"id" => $TEMPLATE_NAME,
				"value" => \Thurly\Main\Text\HtmlFilter::encode(
					\Thurly\Iblock\Template\Engine::process($entity, $templateInfo["TEMPLATE"])
				),
			);
		}
