...
$dsc_cookie_name = COption::GetOptionString('main', 'cookie_name', 'THURLY_SM')."_DSC";
	...
	if($bCatalog)
	{
		if ($boolCatalogPrice && (isset($_POST["CATALOG_PRICE"]) || isset($_POST["CATALOG_CURRENCY"])))
		{
			...
			foreach($CATALOG_PRICE as $elID => $arPrice)
			{
				...
				$ipropValues = new \Thurly\Iblock\InheritedProperty\ElementValues($IBLOCK_ID, $elID);
				$ipropValues->clearValues();
				\Thurly\Iblock\PropertyIndex\Manager::updateElementIndex($IBLOCK_ID, $elID);
			}
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if ($bCatalog && 'E' == $f_TYPE)
	{
		...
		if (isset($arVisibleColumnsMap['CATALOG_TYPE']))
		{
			...
			if (
				$arRes['CATALOG_TYPE'] == \Thurly\Catalog\ProductTable::TYPE_SKU
				|| $arRes['CATALOG_TYPE'] == \Thurly\Catalog\ProductTable::TYPE_SET
			)
			{
			...
			foreach($arProperties[$aProp["ID"]] as $prop_id => $prop)
			{
				...
				elseif($prop['PROPERTY_TYPE']=='F' && ($last_property_id != $prop["ID"]))
				{
					if($prop['MULTIPLE'] == "Y")
					{
						...
						{
							$arEditHTML[] = \Thurly\Main\UI\FileInput::createInstance(array(
	...
	if ($bBizproc)
	{
		if ($f_TYPE == "E")
		{
			...
			foreach ($arDocumentStates as $kk => $vv)
			{
				...
				$arStr[$vv["TEMPLATE_ID"]] .= "<a href=\"/thurly/admin/bizproc_log.php?ID=".$kk.'&back_url='.urlencode($APPLICATION->GetCurPageParam("", array("mode", "table_id")))."\">".(strlen($vv["STATE_TITLE"]) > 0 ? $vv["STATE_TITLE"] : $vv["STATE_NAME"])."</a><br />";
	...
	<script>
		...
		function CreateDialogChPrice()
		{
			var paramsWindowChanger =
			{
				...
				content_url: "/thurly/tools/catalog/iblock_catalog_change_price.php?lang=" + "<?=LANGUAGE_ID?>" + "&bxpublic=Y",
				content_post: "<?=thurly_sessid_get()?>" + "&sTableID=<?=$sTableID?>",
				width: 800,
				height: 415,
				resizable: false,
				buttons: [
