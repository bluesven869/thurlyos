...
$APPLICATION->AddHeadScript('/thurly/js/main/admin_tools.js');
$APPLICATION->AddHeadScript('/thurly/js/iblock/iblock_edit.js');
...
)
{
	...
	{
		$SECTION_PICTURE_FILE_NAME = \Thurly\Iblock\Template\Helper::convertArrayToModifiers($_POST["IPROPERTY_TEMPLATES"]["SECTION_PICTURE_FILE_NAME"]);
		$SECTION_DETAIL_PICTURE_FILE_NAME = \Thurly\Iblock\Template\Helper::convertArrayToModifiers($_POST["IPROPERTY_TEMPLATES"]["SECTION_DETAIL_PICTURE_FILE_NAME"]);
	...
	else
	{
		...
		{
			$ipropValues = new \Thurly\Iblock\InheritedProperty\SectionValues($IBLOCK_ID, $ID);
		...
		if(strlen($apply) <= 0 && strlen($save_and_add) <= 0)
		{
			...
			{
				LocalRedirect("/thurly/admin/".CIBlock::GetAdminSectionListLink($IBLOCK_ID, array('find_section_section'=>intval($find_section_section))));
			}
		}
		elseif(strlen($save_and_add) > 0)
		{
			if (defined('BX_PUBLIC_MODE') && BX_PUBLIC_MODE == 1)
			{
				...
				?>
					<script type="text/javascript">
						top.BX.ajax.post(
							'/thurly/admin/<?echo $l = CUtil::JSEscape(CIBlock::GetAdminSectionEditLink($IBLOCK_ID, 0, array(
			...
			{
				LocalRedirect("/thurly/admin/".CIBlock::GetAdminSectionEditLink($IBLOCK_ID, 0, array(
					"find_section_section" => intval($find_section_section),
					"IBLOCK_SECTION_ID" => $IBLOCK_SECTION_ID,
					"return_url" => (strlen($return_url) > 0? $return_url: null),
				), "&".$tabControl->ActiveTabParam()));
		...
		{
			LocalRedirect("/thurly/admin/".CIBlock::GetAdminSectionEditLink($IBLOCK_ID, $ID, array(
				'find_section_section' => intval($find_section_section),
				'return_url' => strlen($return_url) > 0? $return_url: null,
			), "&".$tabControl->ActiveTabParam()));
		}
...
{
	$ID = 0;
	$ipropTemlates = new \Thurly\Iblock\InheritedProperty\SectionTemplates($IBLOCK_ID, 0);
}
else
{
	$ipropTemlates = new \Thurly\Iblock\InheritedProperty\SectionTemplates($IBLOCK_ID, $ID);
}
...
else
{
	...
	$str_IPROPERTY_TEMPLATES["SECTION_PICTURE_FILE_NAME"] = \Thurly\Iblock\Template\Helper::convertModifiersToArray($str_IPROPERTY_TEMPLATES["SECTION_PICTURE_FILE_NAME"]);
	$str_IPROPERTY_TEMPLATES["SECTION_DETAIL_PICTURE_FILE_NAME"] = \Thurly\Iblock\Template\Helper::convertModifiersToArray($str_IPROPERTY_TEMPLATES["SECTION_DETAIL_PICTURE_FILE_NAME"]);
...
<script type="text/javascript">
	var InheritedPropertiesTemplates = new JCInheritedPropertiesTemplates(
		...
		'/thurly/admin/iblock_templates.ajax.php?ENTITY_TYPE=S&IBLOCK_ID=<?echo intval($IBLOCK_ID)?>&ENTITY_ID=<?echo intval($ID)?>'
	);
	BX.ready(function(){
		setTimeout(function(){
...
$tabControl->Begin(array(
	"FORM_ACTION" => "/thurly/admin/".CIBlock::GetAdminSectionEditLink($IBLOCK_ID, null, $arEditLinkParams),
));
...
if($arTranslit["TRANSLITERATION"] == "Y")
{
	...
	?>
		<tr id="tr_NAME">
			<td><?echo $tabControl->GetCustomLabelHTML()?></td>
			<td nowrap>
				<input type="text" size="50" name="NAME" id="NAME" maxlength="255" value="<?echo $str_NAME?>"><image id="name_link" title="<?echo GetMessage("IBSEC_E_LINK_TIP")?>" class="linked" src="/thurly/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
	...
	?>
		<tr id="tr_CODE">
			...
			<td nowrap>
				...
				<input type="text" size="50" name="CODE" id="CODE" maxlength="255" value="<?echo $str_CODE?>"><image id="code_link" title="<?echo GetMessage("IBSEC_E_LINK_TIP")?>" class="linked" src="/thurly/themes/.default/icons/iblock/<?if($bLinked) echo 'link.gif'; else echo 'unlink.gif';?>" onclick="set_linked()" />
			</td>
		</tr>
	<?
...
$newUploader = class_exists('\Thurly\Main\UI\FileInput', true);
		...
		?>
			<tr>
				<td align="left" colspan="2">
					<a href="/thurly/admin/userfield_edit.php?lang=<?echo LANGUAGE_ID?>&amp;ENTITY_ID=<?echo urlencode($entity_id)?>&amp;back_url=<?echo urlencode($APPLICATION->GetCurPageParam('', array('bxpublic'))."&tabControl_active_tab=user_fields_tab")?>"><?echo $tabControl->GetCustomLabelHTML()?></a>
				</td>
			</tr>
		<?
		...
		<tr><td align="center" colspan="2">
			...
			function addSectionProperty(iblock_id, select_id, shadow_id, table_id)
			{
				...
				if(select && select.value == 0)
				{
					...
					(new BX.CAdminDialog({
						'content_url' : '/thurly/admin/iblock_edit_property.php?lang=<?echo LANGUAGE_ID?>&IBLOCK_ID='+iblock_id+'&ID=n0&bxpublic=Y&from_module=iblock&return_url=section_edit',
						'width' : 700,
						'height' : 400,
...
if(strlen($return_url)>0)
	...
	$bu = "/thurly/admin/".CIBlock::GetAdminSectionListLink($IBLOCK_ID, array('find_section_section'=>intval($find_section_section)));
