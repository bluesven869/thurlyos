...
Main\Page\Asset::getInstance()->addJs('/thurly/js/catalog/tbl_edit.js');

$arJSDescription = array(
	'js' => '/thurly/js/iblock/sub_generator.js',
	'css' => '/thurly/panel/iblock/sub-generator.css',
	'lang' => '/thurly/modules/iblock/lang/'.LANGUAGE_ID.'/admin/iblock_subelement_generator.php'
);
...
if(!$bReadOnly && check_thurly_sessid())
{
	...
	if(strlen($_POST['save']) > 0)
	{
		...
		$parentElement = new \Thurly\Iblock\Template\Entity\Element($parentElementId);
		...
		foreach($arCombinationResult as $arPropertySaveValues)
		{
			...
			$sku = new \Thurly\Iblock\Template\Entity\Element(0);
			$sku->setFields($arIBlockElementAdd);
			$arIBlockElementAdd["NAME"] = htmlspecialcharsback(\Thurly\Iblock\Template\Engine::process($sku, $title));
			...
			if($idNewElement)
			{
				...
				$element = new \Thurly\Iblock\InheritedProperty\ElementValues($subIBlockId, $idNewElement);
				$template = new \Thurly\Iblock\InheritedProperty\BaseTemplate($element);
				$template->set(array(
					"MY_TEMPLATE" => $title,
				));
			}
