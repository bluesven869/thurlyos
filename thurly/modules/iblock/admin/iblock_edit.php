...
function __AddPropCellType($intOFPropID,$strPrefix,$arPropInfo)
{
	...
	if ($arUserTypeList === null)
	{
		...
		\Thurly\Main\Type\Collection::sortByColumn($arUserTypeList, array('DESCRIPTION' => SORT_STRING));
	}
...
)
{
	...
	if(!$bVarsFromForm)
	{
		...
		else
		{
			...
			{
				$ipropValues = new \Thurly\Iblock\InheritedProperty\IblockValues($ID);
			...
			if(!$bVarsFromForm)
			{
				...
				if(strlen($apply)<=0)
				{
					if(strlen($_REQUEST["return_url"])>0)
						...
						LocalRedirect("/thurly/admin/iblock_admin.php?type=".$type."&lang=".LANG."&admin=".($_REQUEST["admin"]=="Y"? "Y": "N"));
				}
				LocalRedirect("/thurly/admin/iblock_edit.php?type=".$type."&tabControl_active_tab=".urlencode($tabControl_active_tab)."&lang=".LANG."&ID=".$ID."&admin=".($_REQUEST["admin"]=="Y"? "Y": "N").(strlen($_REQUEST["return_url"])>0? "&return_url=".urlencode($_REQUEST["return_url"]): ""));
			}
		}
	}
...
else
{
	...
	$ipropTemlates = new \Thurly\Iblock\InheritedProperty\IblockTemplates($ID);
...
if(CIBlockRights::UserHasRightTo($ID, $ID, "iblock_edit")):
	$aMenu = array(
		array(
			...
			"LINK"=>'/thurly/admin/iblock_admin.php?lang='.LANGUAGE_ID.'&type='.urlencode($type).'&admin='.($_REQUEST["admin"]=="Y"? "Y": "N"),
			"ICON"=>"btn_list",
		)
	);
...
<script>
	var InheritedPropertiesTemplates = new JCInheritedPropertiesTemplates(
		'frm',
		'/thurly/admin/iblock_templates.ajax.php?ENTITY_TYPE=B&ENTITY_ID=<?echo intval($ID)?>'
	);
	BX.ready(function(){
		setTimeout(function(){
...
<form method="POST" name="frm" id="frm" action="/thurly/admin/iblock_edit.php?type=<?echo htmlspecialcharsbx($type)?>&amp;lang=<?echo LANG?>&amp;admin=<?echo ($_REQUEST["admin"]=="Y"? "Y": "N")?>"  ENCTYPE="multipart/form-data">
<?=thurly_sessid_post()?>
...
?>
	...
	<tr class="adm-detail-required-field">
		...
		if ('O' == $str_CATALOG_TYPE)
		{
			...
			echo "<br>".str_replace('#LINK#','/thurly/admin/iblock_edit.php?type='.$str_PRODUCT_IBLOCK_TYPE_ID.'&lang='.LANGUAGE_ID.'&ID='.$str_PRODUCT_IBLOCK_ID.'&admin=Y',GetMessage('IB_E_OF_SITES'));
...
if ($bCatalog)
{
	...
	<tr>
		...
		<td width="60%"><a href="/thurly/admin/iblock_edit.php?type=<? echo $str_PRODUCT_IBLOCK_TYPE_ID; ?>&lang=<? echo LANGUAGE_ID; ?>&ID=<? echo $str_PRODUCT_IBLOCK_ID; ?>&admin=Y"><? echo htmlspecialcharsbx($str_PRODUCT_IBLOCK_NAME); ?></a>
...
?>
	...
	<?else:?>
		...
		<tr>
			<td nowrap width="40%"><?echo GetMessage("IB_E_EVERYONE")?> [<a class="tablebodylink" href="/thurly/admin/group_edit.php?ID=2&amp;lang=<?=LANGUAGE_ID?>">2</a>]:</td>
			<td width="60%">
		...
		while($r = $groups->GetNext()):
			...
			?>
		<tr>
			<td nowrap width="40%"><?echo $r["NAME"]?> [<a class="tablebodylink" href="/thurly/admin/group_edit.php?ID=<?=$r["ID"]?>&lang=<?=LANGUAGE_ID?>"><?=$r["ID"]?></a>]:</td>
			<td width="60%">
...
$tabControl->BeginNextTab();
	...
	<tr>
		<td colspan="2">
						...
						<tr>
							<td>
								<?if(IsModuleInstalled("bizprocdesigner")):?>
									<a href="/thurly/admin/iblock_bizproc_workflow_edit.php?document_type=iblock_<?= $ID ?>&lang=<?=LANGUAGE_ID?>&ID=<?=$arTemplate["ID"]?>&back_url_list=<?= urlencode($APPLICATION->GetCurPageParam("", array()))?>" target="_blank"><?= $arTemplate["NAME"] ?> [<?=$arTemplate["ID"]?>]</a>
								<?else:?>
									<?= $arTemplate["NAME"] ?>
								<?endif?>
			...
			<?endif;?>
			<?if(IsModuleInstalled("bizprocdesigner")):?>
			<a href="/thurly/admin/iblock_bizproc_workflow_admin.php?document_type=iblock_<?= $ID ?>&lang=<?=LANGUAGE_ID?>&back_url_list=<?= urlencode($APPLICATION->GetCurPageParam("", array())) ?>" target="_blank"><?echo GetMessage("IB_E_GOTO_BP")?></a>
			<?endif?>
		</td>
	</tr>
	<?
