...
class CIBlockPropertyXmlID
{
		...
		if(strlen($value["VALUE"])>0)
		{
			if(!array_key_exists($value["VALUE"], $cache))
			{
				...
				if($ar_res)
					...
					' [<a href="'.
					'/thurly/admin/iblock_element_edit.php?'.
	...
	public static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
	{
		...
		return  '<input name="'.htmlspecialcharsbx($strHTMLControlName["VALUE"]).'" id="'.htmlspecialcharsbx($strHTMLControlName["VALUE"]).'" value="'.htmlspecialcharsEx($value["VALUE"]).'" size="20" type="text">'.
			'<input type="button" value="..." onClick="jsUtils.OpenWindow(\''.CUtil::JSEscape('/thurly/admin/iblock_element_search.php?lang='.LANGUAGE_ID.'&n='.urlencode($strHTMLControlName["VALUE"]).'&get_xml_id=Y&a=b'.($fixIBlock ? '&iblockfix=y' : '').'&tableId='.$windowTableId).'\', 900, 700);">'.
