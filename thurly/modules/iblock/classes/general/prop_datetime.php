...
class CIBlockPropertyDateTime
{
	...
	public static function AddFilterFields($arProperty, $strHTMLControlName, &$arFilter, &$filtered)
	{
		...
		$existFilterOptions = class_exists('\Thurly\Main\UI\Filter\Options') && method_exists('\Thurly\Main\UI\Filter\Options', 'getFilter');
		...
		{
			$filterOption = new \Thurly\Main\UI\Filter\Options($strHTMLControlName["FILTER_ID"]);
	...
	public static function ConvertToDB($arProperty, $value)
	{
		if (strlen($value["VALUE"]) > 0)
		{
			try
			{
				$time = Thurly\Main\Type\DateTime::createFromUserTime($value['VALUE']);
	...
	public static function ConvertFromDB($arProperty, $value, $format = '')
	{
		if (strlen($value["VALUE"]) > 0)
		{
			try
			{
				$time = new Thurly\Main\Type\DateTime($value['VALUE'], "Y-m-d H:i:s");
				$time->toUserTime();
