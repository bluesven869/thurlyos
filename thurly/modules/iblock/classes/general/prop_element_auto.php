	...
	public static function GetPropertyFieldHtml($arProperty, $arValue, $strHTMLControlName)
	{
		...
		else
		{
			...
			elseif ($arSettings['VIEW'] == 'E')
			{
				...
				onclick="jsUtils.OpenWindow('/thurly/admin/iblock_element_search.php?lang=<? echo LANGUAGE_ID; ?>&IBLOCK_ID=<? echo $arProperty["LINK_IBLOCK_ID"]; ?>&n=&k=&lookup=<? echo 'jsMLI_'.$control_id; ?><?=($fixIBlock ? '&iblockfix=y' : '').'&tableId='.$windowTableId; ?>', 900, 700);"><?
			}
			if ($arProperty['USER_TYPE_SETTINGS']['SHOW_ADD'] == 'Y' && $fixIBlock)
			{
				...
				?><input type="button" style="margin-top: 5px;" value="<? echo htmlspecialcharsbx($strButtonCaption); ?>"
					...
					onclick="jsUtils.OpenWindow('<? echo '/thurly/admin/'.CIBlock::GetAdminElementEditLink(
		...
		else
		{
			...
			elseif ($arSettings['VIEW'] == 'E')
			{
				?><input style="float: left; margin-right: 10px; margin-top: 5px;" type="button" value="<? echo Loc::getMessage('BT_UT_EAUTOCOMPLETE_MESS_SEARCH_ELEMENT'); ?>"
					...
					onclick="jsUtils.OpenWindow('/thurly/admin/iblock_element_search.php?lang=<? echo LANGUAGE_ID; ?>&IBLOCK_ID=<? echo $arProperty["LINK_IBLOCK_ID"]; ?>&n=&k=&m=y&lookup=<? echo 'jsMLI_'.$control_id; ?><?=($fixIBlock ? '&iblockfix=y' : '').'&tableId='.$windowTableId; ?>', 900, 700);"><?
			}
			if ($arProperty['USER_TYPE_SETTINGS']['SHOW_ADD'] == 'Y' && $fixIBlock)
			{
				if ($arSettings['IBLOCK_MESS'] == 'Y')
