 ...
 */
	...
	function CAdminSubList($table_id, $sort = false, $list_url, $arHideHeaders = false)
	{
		...
		$this->requiredDialogParams['sessid'] = thurly_sessid();
		if (!$this->bPublicMode)
		{
		...
		$arJSDescr = array(
			'js' => '/thurly/js/iblock/subelement.js',
			'rel' => array('admin_interface')
		);
		if ($this->bPublicMode)
			$arJSDescr['css'] = '/thurly/panel/iblock/sub-public.css';
		else
			$arJSDescr['css'] = '/thurly/panel/iblock/sub-admin.css';
 ...
 */
	...
	function CAdminSubForm($name, $tabs, $bCanExpand = true, $bDenyAutosave = false, $list_url = '', $boolShowSettings = false)
	{
		...
		$arJSDescr = array(
			'js' => '/thurly/js/iblock/subelementdet.js',
			'css' => '/thurly/panel/iblock/sub-detail-public.css',
			'rel' => array('admin_interface')
		);
	...
	function ShowSettings()
	{
		...
		require($_SERVER['DOCUMENT_ROOT']."/thurly/modules/main/interface/settings_admin_form.php");
...
class CAdminSubResult extends CAdminResult
{
	...
	function GetNavPrint($title, $show_allways=true, $StyleText="", $template_path=false, $arDeleteParam=false)
	{
		if($template_path === false)
			$template_path = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/iblock/admin/templates/navigation.php";
