...
class CAllIBlockElement
{
	...
	function MkFilter($arFilter, &$arJoinProps, &$arAddWhereFields, $level = 0, $bPropertyLeftJoin = false)
	{
		...
		{
			$f = new \Thurly\Iblock\PropertyIndex\QueryBuilder($arFilter["IBLOCK_ID"]);
			if ($f->isValid())
			{
...
//							$arUserGroups[] = SONET_RELATIONS_TYPE_FRIENDS2;
	...
	function Add($arFields, $bWorkFlow=false, $bUpdateSearch=true, $bResizePictures=false)
	{
		...
		$ipropTemplates = new \Thurly\Iblock\InheritedProperty\ElementTemplates($arFields["IBLOCK_ID"], 0);
		if(is_set($arFields, "PREVIEW_PICTURE"))
		{
			if(is_array($arFields["PREVIEW_PICTURE"]))
			{
				...
				else
				{
					...
					$arFields["PREVIEW_PICTURE"]["name"] = \Thurly\Iblock\Template\Helper::makeFileName(
						$ipropTemplates
		...
		if(is_set($arFields, "DETAIL_PICTURE"))
		{
			if(is_array($arFields["DETAIL_PICTURE"]))
			{
				...
				else
				{
					...
					$arFields["DETAIL_PICTURE"]["name"] = \Thurly\Iblock\Template\Helper::makeFileName(
						$ipropTemplates
		...
		else
		{
			...
			{
				$ipropTemplates = new \Thurly\Iblock\InheritedProperty\ElementTemplates($arIBlock["ID"], $ID);
			...
			\Thurly\Iblock\PropertyIndex\Manager::updateElementIndex($arIBlock["ID"], $ID);
	...
	public static function Delete($ID)
	{
		...
		foreach($arSql as $strWhere)
		{
			...
			while ($zr = $z->Fetch())
			{
				...
				$piId = \Thurly\Iblock\PropertyIndex\Manager::resolveElement($zr["IBLOCK_ID"], $zr["ID"]);
				...
				$ipropTemplates = new \Thurly\Iblock\InheritedProperty\ElementTemplates($zr["IBLOCK_ID"], $zr["ID"]);
				...
				\Thurly\Iblock\PropertyIndex\Manager::deleteElementIndex($zr["IBLOCK_ID"], $piId);
