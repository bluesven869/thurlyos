		...
		)
		{
			...
			if ($arFieldType["Type"] == "E:EList")
			{
				...
				if ($fl)
				{
					...
					$GLOBALS["APPLICATION"]->AddHeadScript('/thurly/js/iblock/iblock_edit.js');
				}
				$fl = false;
			}
					...
					window.BXHtmlEditor.Show(
						{
							...
							'actionUrl':'/thurly/tools/html_editor_action.php',
							'cssIframePath':'/thurly/js/fileman/html_editor/iframe-style.css?1412693817',
							'bodyClass':'',
							'bodyId':'',
							'spellcheck_path':'/thurly/js/fileman/html_editor/html-spell.js?v=1412693817',
							'usePspell':'N',
							'useCustomSpell':'Y',
							'bbCode':false,
				...
				if (is_array($fieldValue))
				{
					...
					foreach ($fieldValue as $r)
					{
						...
						if ($arImg = $dbImg->Fetch())
							$result[] = "[url=/thurly/tools/bizproc_show_file.php?f=".urlencode($arImg["FILE_NAME"])."&i=".$r."&h=".md5($arImg["SUBDIR"])."]".htmlspecialcharsbx($arImg["ORIGINAL_NAME"])."[/url]";
					}
				}
				else
				{
					...
					if ($arImg = $dbImg->Fetch())
						$result = "[url=/thurly/tools/bizproc_show_file.php?f=".urlencode($arImg["FILE_NAME"])."&i=".$fieldValue."&h=".md5($arImg["SUBDIR"])."]".htmlspecialcharsbx($arImg["ORIGINAL_NAME"])."[/url]";
				}
				break;
	...
	*/
	public function GetDocumentAdminPage($documentId)
	{
		...
		if ($ar = $db->Fetch())
		{
			...
			return "/thurly/admin/iblock_element_edit.php?view=Y&ID=".$documentId."&IBLOCK_ID=".$ar["IBLOCK_ID"]."&type=".$ar["IBLOCK_TYPE_ID"];
		}
				...
				elseif ($propertyValue["PROPERTY_TYPE"] == "F")
				{
					...
					foreach ($arPropertyValue as $v)
					{
						...
						if ($ar)
						{
							...
							$arResult["PROPERTY_".$propertyKey."_printable"][intval($v)] = "[url=/thurly/tools/bizproc_show_file.php?f=".urlencode($ar["FILE_NAME"])."&i=".$v."&h=".md5($ar["SUBDIR"])."]".htmlspecialcharsbx($ar["ORIGINAL_NAME"])."[/url]";
						}
					}
				}
				else
	...
	public function GetDocumentFieldTypes($documentType)
	{
		...
		if (class_exists('\Thurly\Bizproc\BaseType\InternalSelect'))
		{
			$arResult[\Thurly\Bizproc\FieldType::INTERNALSELECT] = array(
		...
		foreach (CIBlockProperty::GetUserType() as  $ar)
		{
			...
			$arResult[$t] = array("Name" => $ar["DESCRIPTION"], "BaseType" => "string", 'typeClass' => '\Thurly\Iblock\BizprocType\UserTypeProperty');
			if ($t == "S:employee")
			{
				if (COption::GetOptionString("bizproc", "employee_compatible_mode", "N") != "Y")
					$arResult[$t]["BaseType"] = "user";
				$arResult[$t]['typeClass'] = '\Thurly\Iblock\BizprocType\UserTypePropertyEmployee';
			}
			elseif ($t == "E:EList")
			{
				$arResult[$t]["BaseType"] = "string";
				$arResult[$t]["Complex"] = true;
				$arResult[$t]['typeClass'] = '\Thurly\Iblock\BizprocType\UserTypePropertyElist';
			}
			elseif ($t == 'S:HTML')
			{
				$arResult[$t]['typeClass'] = '\Thurly\Iblock\BizprocType\UserTypePropertyHtml';
			}
			elseif($t == 'S:DiskFile')
			{
				$arResult[$t]["BaseType"] = "int";
				$arResult[$t]['typeClass'] = '\Thurly\Iblock\BizprocType\UserTypePropertyDiskFile';
			}
			elseif($t == 'E:ECrm')
			{
				$arResult[$t]["BaseType"] = "string";
				$arResult[$t]["Complex"] = true;
				$arResult[$t]['typeClass'] = '\Thurly\Iblock\BizprocType\ECrm';
			}
			elseif($t == 'S:Money')
			{
				$arResult[$t]["BaseType"] = "string";
				$arResult[$t]['typeClass'] = '\Thurly\Iblock\BizprocType\Money';
			}
		}
	...
	public function PublishDocument($documentId)
	{
		...
		if($ar_element = $db_element->Fetch())
		{
			...
			if($PARENT_ID)
			{
				...
				\Thurly\Iblock\PropertyIndex\Manager::updateElementIndex($ar_element["IBLOCK_ID"], $PARENT_ID);
				return $PARENT_ID;
			}
			else
			{
				...
				\Thurly\Iblock\PropertyIndex\Manager::updateElementIndex($ar_element["IBLOCK_ID"], $ID);
				return $ID;
			}
		}
		return false;
