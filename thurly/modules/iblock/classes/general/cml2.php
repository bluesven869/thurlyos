...
class CIBlockCMLImport
{
	...
	function Init(&$next_step, $files_dir = false, $use_crc = true, $preview = false, $detail = false, $use_offers = false, $use_iblock_type_id = false, $table_name = "b_xml_tree")
	{
		...
		if ($this->bCatalog)
		{
			$catalogsIterator = \Thurly\Catalog\CatalogIblockTable::getList(array(
	...
	function ImportMetaData($xml_root_id, $IBLOCK_TYPE, $IBLOCK_LID, $bUpdateIBlock = true)
	{
		...
		if($ar)
		{
			...
			//Also check for non thurly xml file
	...
	function ImportPropertyDirectory($arProperty, $arEnumXmlNodes)
	{
		...
		{
			$hlblock = Thurly\Highloadblock\HighloadBlockTable::getList(array(
				"filter" => array(
			...
			if (!$hlblock)
			{
				...
				$result = Thurly\Highloadblock\HighloadBlockTable::add($data);
		...
		$entity = Thurly\Highloadblock\HighloadBlockTable::compileEntity($hlblock);
	...
	function ImportElements($start_time, $interval)
	{
		...
		if($this->next_step["XML_ELEMENTS_PARENT"])
		{
			...
			while($arParent = $rsParents->Fetch())
			{
				...
				if($ID)
				{
					$ipropValues = new \Thurly\Iblock\InheritedProperty\ElementValues($this->next_step["IBLOCK_ID"], $ID);
		...
		elseif(isset($arXMLElement[$this->mess["IBLOCK_XML2_NAME"]]))
		{
			...
			if(isset($arXMLElement[$this->mess["IBLOCK_XML2_PROPERTIES_VALUES"]]))
			{
				foreach($arXMLElement[$this->mess["IBLOCK_XML2_PROPERTIES_VALUES"]] as $value)
				{
					...
					//check for thurly extended format
	...
	function ImportElementPrices($arXMLElement, &$counter, $arParent = false)
	{
		...
		if($arDBElement)
		{
			...
			if(isset($arElement["PRICES"]) && $this->bCatalog)
			{
				...
				$this->SetProductPrice($arElement["ID"], $arElement["PRICES"], $arElement["DISCOUNTS"]);
				\Thurly\Iblock\PropertyIndex\Manager::updateElementIndex($IBLOCK_ID, $arElement["ID"]);
			}
			elseif(
				$this->bCatalog
...
class CIBlockCMLExport
{
	...
	function ExportSections(&$SECTION_MAP, $start_time, $INTERVAL, $FILTER = "", $PROPERTY_MAP = array())
	{
		...
		while($arSection = $rsSections->Fetch())
		{
			...
			if($this->bExtended)
			{
				...
				$sectionTemplates = new \Thurly\Iblock\InheritedProperty\SectionTemplates($this->arIBlock["ID"], $arSection["ID"]);
	...
	function StartExportCatalog($with_metadata = true, $changes_only = false)
	{
		...
		if($with_metadata)
		{
			...
			if($this->bExtended)
			{
				...
				$iblockTemplates = new \Thurly\Iblock\InheritedProperty\IblockTemplates($this->arIBlock["ID"]);
	...
	function exportInheritedTemplates($level, \Thurly\Iblock\InheritedProperty\BaseTemplate $template)
	{
	...
	function exportElement($arElement, $SECTION_MAP, $PROPERTY_MAP)
	{
		...
		if(!$this->only_price)
		{
			...
			if($this->bExtended)
			{
				$elementTemplates = new \Thurly\Iblock\InheritedProperty\ElementTemplates($this->arIBlock["ID"], $arElement["ID"]);
