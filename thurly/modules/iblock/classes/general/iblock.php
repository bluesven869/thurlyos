...
class CAllIBlock
{
	...
	function AddPanelButtons($mode, $componentName, $arButtons)
	{
		...
		$arImages = array(
			"add_element" => (defined("PANEL_ADD_ELEMENT_BTN")) ? PANEL_ADD_ELEMENT_BTN : "/thurly/images/iblock/icons/new_element.gif",
			"edit_element" => (defined("PANEL_EDIT_ELEMENT_BTN")) ? PANEL_EDIT_ELEMENT_BTN : "/thurly/images/iblock/icons/edit_element.gif",
			"edit_iblock" => (defined("PANEL_EDIT_IBLOCK_BTN")) ? PANEL_EDIT_IBLOCK_BTN : "/thurly/images/iblock/icons/edit_iblock.gif",
			"history_element" => (defined("PANEL_HISTORY_ELEMENT_BTN")) ? PANEL_HISTORY_ELEMENT_BTN : "/thurly/images/iblock/icons/history.gif",
			"edit_section" => (defined("PANEL_EDIT_SECTION_BTN")) ? PANEL_EDIT_SECTION_BTN : "/thurly/images/iblock/icons/edit_section.gif",
			"add_section" => (defined("PANEL_ADD_SECTION_BTN")) ? PANEL_ADD_SECTION_BTN : "/thurly/images/iblock/icons/new_section.gif",
			"element_list" => "/thurly/themes/.default/icons/iblock/mnu_iblock_el.gif",
			"section_list" => "/thurly/themes/.default/icons/iblock/mnu_iblock_sec.gif",
		);
		...
		if(count($arButtons[$mode]) > 0)
		{
			...
			if(strlen($componentName) <= 0 && function_exists("debug_backtrace"))
			{
				...
				foreach($arTrace as $arCallInfo)
				{
					if(array_key_exists("file", $arCallInfo))
					{
						...
						if(preg_match("#.*/thurly/components/(.+?)/(.+?)/#", $file, $match))
						{
			...
			$arPanelButton = array(
				"SRC" => "/thurly/images/iblock/icons/iblock.gif",
				"ALT" => $componentName,
				"TEXT" => $componentName,
				"MAIN_SORT" => 300,
				"SORT" => 30,
	...
	{
		$arImages = array(
			"add_element" => "/thurly/images/iblock/icons/new_element.gif",
			"edit_element" => "/thurly/images/iblock/icons/edit_element.gif",
			"edit_iblock" => "/thurly/images/iblock/icons/edit_iblock.gif",
			"history_element" => "/thurly/images/iblock/icons/history.gif",
			"edit_section" => "/thurly/images/iblock/icons/edit_section.gif",
			"add_section" => "/thurly/images/iblock/icons/new_section.gif",
			"element_list" => "/thurly/themes/.default/icons/iblock/mnu_iblock_el.gif",
			"section_list" => "/thurly/themes/.default/icons/iblock/mnu_iblock_sec.gif",
		);
	...
	public static function GetPanelButtons($IBLOCK_ID=0, $ELEMENT_ID=0, $SECTION_ID=0, $arOptions=array())
	{
		...
		if ($useCatalogButtons || (isset($arOptions["CATALOG"]) && $arOptions["CATALOG"] == true))
		{
			if (self::$catalogIncluded === null)
				self::$catalogIncluded = \Thurly\Main\Loader::includeModule('catalog');
		...
		foreach($return_url as $key => $url)
		{
			if(empty($url))
			{
				if(empty($str))
				{
					$str = \Thurly\Main\Context::getCurrent()->getServer()->getRequestUri();
		...
		if (self::$workflowIncluded === null)
			self::$workflowIncluded = \Thurly\Main\Loader::includeModule('workflow');
		...
		{
			$url = "/thurly/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, $ELEMENT_ID, array(
			...
			if($bWorkflow)
			{
				$url = "/thurly/admin/iblock_history_list.php?type=".$arIBlock["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&IBLOCK_ID=".$IBLOCK_ID."&ELEMENT_ID=".$ELEMENT_ID."&filter_section=".$SECTION_ID."&return_url=".UrlEncode($return_url["history_element"]);
				$arButton = array(
		...
		if(CIBlockSectionRights::UserHasRightTo($IBLOCK_ID, $SECTION_ID, "section_element_bind"))
		{
			...
			{
				$url = "/thurly/admin/".CIBlock::GetAdminElementEditLink($IBLOCK_ID, null, $params);
				$action = $APPLICATION->GetPopupLink(
					array(
						"URL" => $url,
						"PARAMS" => $windowParams,
		...
		if($ELEMENT_ID <= 0 && $bSectionButtons)
		{
			...
			if(($arIBTYPE = $rsIBTYPE->Fetch()) && ($arIBTYPE["SECTIONS"] == "Y"))
			{
				if($SECTION_ID > 0 && CIBlockSectionRights::UserHasRightTo($IBLOCK_ID, $SECTION_ID, "section_edit"))
				{
					...
					$url = "/thurly/admin/".CIBlock::GetAdminSectionEditLink($IBLOCK_ID, $SECTION_ID, array(
						"force_catalog" => $bCatalog,
						"filter_section" => $SECTION_ID,
						"bxpublic" => "Y",
						"from_module" => "iblock",
				...
				{
					$url = "/thurly/admin/".CIBlock::GetAdminSectionEditLink($IBLOCK_ID, null, array(
						"force_catalog" => $bCatalog,
						"IBLOCK_SECTION_ID" => $SECTION_ID,
						"filter_section" => $SECTION_ID,
						"bxpublic" => "Y",
		...
		if( ($IBLOCK_ID > 0) && CIBlockRights::UserHasRightTo($IBLOCK_ID, $IBLOCK_ID, "iblock_admin_display") )
		{
			...
			if($SECTION_ID > 0)
				$url = "/thurly/admin/".CIBlock::GetAdminElementListLink($IBLOCK_ID , array('find_section_section'=>$SECTION_ID));
			else
				$url = "/thurly/admin/".CIBlock::GetAdminElementListLink($IBLOCK_ID , array('find_el_y'=>'Y'));
			...
			$url = "/thurly/admin/".CIBlock::GetAdminSectionListLink($IBLOCK_ID, array('find_section_section'=>$SECTION_ID));
			$arButton = array(
			...
			{
				$url = "/thurly/admin/iblock_edit.php?type=".$arIBlock["IBLOCK_TYPE_ID"]."&lang=".LANGUAGE_ID."&ID=".$IBLOCK_ID."&return_url=".UrlEncode($return_url["edit_iblock"]);
				$arButton = array(
	...
	function Add($arFields)
	{
		...
		else
		{
			...
			{
				$ipropTemplates = new \Thurly\Iblock\InheritedProperty\IblockTemplates($ID);
...
**************/
	...
	public static function GetAdminSubElementEditLink($IBLOCK_ID, $ELEMENT_ID, $SUBELEMENT_ID, $arParams = array(), $strAdd = '', $absoluteUrl = false)
	{
		...
		$url = ($absoluteUrl ? '/thurly/admin/' : '').'iblock_subelement_edit.php?IBLOCK_ID='.(int)$IBLOCK_ID.'&type='.urlencode(CIBlock::GetArrayByID($IBLOCK_ID, 'IBLOCK_TYPE_ID'));
