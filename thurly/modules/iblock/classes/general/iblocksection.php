...
class CAllIBlockSection
{
	...
	function Add($arFields, $bResort=true, $bUpdateSearch=true, $bResizePictures=false)
	{
		...
		$ipropTemplates = new \Thurly\Iblock\InheritedProperty\SectionTemplates($arFields["IBLOCK_ID"], 0);
		if(is_set($arFields, "PICTURE"))
		{
			...
			else
			{
				...
				$arFields["PICTURE"]["name"] = \Thurly\Iblock\Template\Helper::makeFileName(
					$ipropTemplates
		...
		else
		{
			...
			{
				$ipropTemplates = new \Thurly\Iblock\InheritedProperty\SectionTemplates($arIBlock["ID"], $ID);
	...
	function Update($ID, $arFields, $bResort=true, $bUpdateSearch=true, $bResizePictures=false)
	{
		...
		$ipropTemplates = new \Thurly\Iblock\InheritedProperty\SectionTemplates($db_record["IBLOCK_ID"], $db_record["ID"]);
		if(is_set($arFields, "PICTURE"))
		{
			if (
	...
	public static function Delete($ID, $bCheckPermissions = true)
	{
		...
		if($s = $s->Fetch())
		{
			...
			$ipropTemplates = new \Thurly\Iblock\InheritedProperty\SectionTemplates($s["IBLOCK_ID"], $ID);
