...
if($this->startResultCache(false, array($arParams)))
{
	...
	if($arResult = $rsIBlock->GetNext())
	{
		...
		if($arParams["SENDER_CHAIN_ID"] > 0 && Loader::includeModule('sender'))
		{
			$postingDb = \Thurly\Sender\PostingTable::getList(array(
				...
				'filter' => array(
					...
					'=STATUS' => array(
						\Thurly\Sender\PostingTable::STATUS_SENT,
						\Thurly\Sender\PostingTable::STATUS_SENT_WITH_ERRORS
					)
				),
		...
		if($arParams["PARENT_SECTION"]>0)
		{
			...
			{
				$ipropValues = new \Thurly\Iblock\InheritedProperty\SectionValues($arParams["IBLOCK_ID"], $arPath["ID"]);
				$arPath["IPROPERTY_VALUES"] = $ipropValues->getValues();
				$arResult["SECTION"]["PATH"][] = $arPath;
			}

			$ipropValues = new \Thurly\Iblock\InheritedProperty\SectionValues($arResult["ID"], $arParams["PARENT_SECTION"]);
		...
		{
			if(class_exists('\Thurly\Main\Mail\StopException'))
			{
		...
		while($obElement = $rsElement->GetNextElement())
		{
			...
			$ipropValues = new \Thurly\Iblock\InheritedProperty\ElementValues($arItem["IBLOCK_ID"], $arItem["ID"]);
		...
		/** @var CThurlyComponent $navComponentObject */
