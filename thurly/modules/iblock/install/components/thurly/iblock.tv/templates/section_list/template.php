...
?>
	<div id="bx_tv_block_<?=$arResult['PREFIX']?>" style="width: <?=$arParams['WIDTH']?>px;">
		...
		<div id="thurly_tv_flv_cont_<?= $arResult["PREFIX"]?>" style="display: none">
		<?$APPLICATION->IncludeComponent(
			...
			Array(
				...
				"PLAYER_ID" => "thurly_tv_flv_".$arResult["PREFIX"]
			),
			$component,
			Array("HIDE_ICONS" => "Y")
		);?>
		...
		<div id="thurly_tv_wmv_cont_<?= $arResult["PREFIX"]?>" style="display: none">
		<?$APPLICATION->IncludeComponent(
			"thurly:player",
			"",
			Array(
				...
				"PLAYER_ID" => "thurly_tv_wmv_".$arResult["PREFIX"]
			),
			$component,
			Array("HIDE_ICONS" => "Y")
		);?>
		...
		<div class="player_section_list" id="player_section_list">
			<?foreach ($arResult['SECTIONS'] as $keySection=>$valSection):?>
				<?if(isset($arResult["RAW_SECTIONS"][$keySection])):?>
					<a href="javascript:void(0)" onclick="jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].BuildTreeExt(<?=$i++?>);"><?=htmlspecialcharsbx($arResult["RAW_SECTIONS"][$keySection]["NAME"]);?></a><?=($i>0 && $i<count($arResult["SECTIONS"]))?" | ":"";?>
				<?endif;?>
			<?endforeach;?>
		</div>
		<?endif;?>
...
<script>
	...
	jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].LanguagePhrases = {
		'duration':'<?=GetMessageJS("THURLYTV_TEMPLATE_DURATION")?>',
		'title':'<?=GetMessageJS("THURLYTV_TEMPLATE_TITLE")?>',
		'description':'<?=GetMessageJS("THURLYTV_TEMPLATE_DESCRIPTION")?>',
		'file':'<?=GetMessageJS("THURLYTV_TEMPLATE_FILE")?>',
		'download':'<?=GetMessageJS("THURLYTV_TEMPLATE_DOWNLOAD")?>',
		'size_mb':'<?=GetMessageJS("THURLYTV_TEMPLATE_BXTV_SIZE_MB")?>',
		'play':'<?=GetMessageJS("THURLYTV_TEMPLATE_BXTV_PLAY")?>',
		'edit':'<?=GetMessageJS("THURLYTV_TEMPLATE_BXTV_EDIT")?>'
	};
	...
	jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].AddPlayerListener(
		...
		function(txt, i, j)
		{
			...
			'<div onmouseover="jsPublicTVCollector.add[<?=$arResult['PREFIX']?>].HoverListItem(this)" onmouseout="jsPublicTVCollector.add[<?=$arResult['PREFIX']?>].HoverListItem(this)" class="normal-tv-item">'
				+'<div class="thurly-tv-small-image" onclick="jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].PlayFile('+i+','+j+',true,true)">'
				...
				+'</div>'
				+'<div class="thurly-tv-tree-item-description">'
	...
	if(jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>])
	{
		jsPublicTVCollector.tv[<?=$arResult['PREFIX']?>].Init(
			...
			'tv_description_<?=$arResult['PREFIX']?>',
			{
				block_id:
				{
					wmv: 'thurly_tv_wmv_cont_<?=$arResult['PREFIX']?>',
					flv: 'thurly_tv_flv_cont_<?=$arResult['PREFIX']?>'
				},
				obj_id:
				{
					wmv: 'thurly_tv_wmv_<?=$arResult['PREFIX']?>',
					flv: 'thurly_tv_flv_<?=$arResult['PREFIX']?>_div'
				},
