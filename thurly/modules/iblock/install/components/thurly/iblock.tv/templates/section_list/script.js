		...
		StatEvent: function(player, oldstate, newstate)
		{
			...
			)
			{
				...
				for(var file in this.files)
				{
					...
					{
						BX.ajax.post(
							'/thurly/components/thurly/iblock.tv/stat.php',
							{id: this.files[file]},
							function(data){}
						);
...
function jsPublicTV()
{
	...
	{
			id:'-roll',
			styles:['thurly-tv-section-closed','thurly-tv-section-title']
	};
...
jsPublicTV.prototype.Init = function(arValues, TBID, DBID, arStart)
{
	...
	this.TreeStyleCodes =
	{
		...
		description_end:'</div>',
		tree_image_start:'<div class="thurly-tv-small-image">',
		tree_image_end:'</div>',
		tree_description_start:'<div class="thurly-tv-tree-item-description">',
...
jsPublicTV.prototype.GeneratePlayer = function()
{
	...
	if(this.PlayerConfig.width && this.PlayerConfig.height && this.PlayerConfig.obj_id[curItem.Type])
	{
		if(curItem.Type == 'wmv') //WMV
		{
			...
			this.Player.wmv = new jeroenwijering.Player(BX(this.PlayerConfig.obj_id.wmv), '/thurly/components/thurly/player/wmvplayer/wmvplayer.xaml',  this.PlayerConfig);
			this.PlayerConfig.height +=4; //height fix
...
jsPublicTV.prototype.BuildItem = function(i,j)
{
	...
	if(ParentBlock)
	{
		...
		item.className = 'thurly-tv-tree-item';
		var txt =
...
jsPublicTV.prototype.EffectCreateOverlay = function()
{
	...
	if (!this.EffectOverlay && this.EffectGetOpacityProperty())
	{
		...
		this.EffectOverlay.className = "thurly-tv-overlay";
		this.EffectOverlay.id = 'thurly-tv-overlay';
			...
			if(dxShadow > 0)
			{
				...
				if(!img)
				{
					if(BX.browser.IsIE())
					{
			 			...
			 			img.style.filter = "progid:DXImageTransform.Microsoft.AlphaImageLoader(src='/thurly/themes/"+phpVars.ADMIN_THEME_ID+"/images/shadow.png',sizingMethod='scale')";
					}
					else
					{
			 			img = document.createElement("IMG");
						img.src = '/thurly/themes/'+phpVars.ADMIN_THEME_ID+'/images/shadow.png';
					}
