...
if($arParams["SHOW_WORKFLOW"] || $this->StartResultCache(false, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups())))
{
	...
	if($ELEMENT_ID)
	{
		...
		if($obElement = $rsElement->GetNextElement())
		{
			...
			$ipropValues = new \Thurly\Iblock\InheritedProperty\ElementValues($arResult["IBLOCK_ID"], $arResult["ID"]);
	...
	if(isset($arResult["ID"]))
	{
		...
		foreach (array("NEXT", "PREV") as $type)
		{
			...
			{
				$ipropValues = new \Thurly\Iblock\InheritedProperty\ElementValues($value["IBLOCK_ID"], $value["ID"]);
