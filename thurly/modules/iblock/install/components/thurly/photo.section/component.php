...
if($arParams["SECTION_ID"] > 0 && $arParams["SECTION_ID"]."" != $arParams["~SECTION_ID"])
{
	...
	{
		\Thurly\Iblock\Component\Tools::process404(
...
if($this->StartResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $arNavigation, $bUSER_HAVE_ACCESS, $pagerParameters)))
{
	...
	if($arResult)
	{
		...
		$ipropValues = new \Thurly\Iblock\InheritedProperty\SectionValues($arResult["IBLOCK_ID"], $arResult["ID"]);
...
if(isset($arResult["ID"]))
{
	...
	$browserTitle = \Thurly\Main\Type\Collection::firstNotEmpty(
	...
	$metaKeywords = \Thurly\Main\Type\Collection::firstNotEmpty(
	...
	$metaDescription = \Thurly\Main\Type\Collection::firstNotEmpty(
