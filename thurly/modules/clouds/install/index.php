...
Class clouds extends CModule
{
	...
	function InstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/clouds/install/db/".strtolower($DB->type)."/install.sql");
		}
	...
	function UnInstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/clouds/install/db/".strtolower($DB->type)."/uninstall.sql");
			$this->UnInstallTasks();
		}
	...
	function InstallFiles($arParams = array())
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/clouds/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/clouds/install/themes", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes", true, true);
		}
		return true;
	}
	...
	function UnInstallFiles()
	{
		if($_ENV["COMPUTERNAME"]!='BX')
		{
			DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/clouds/install/admin/", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
			DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/clouds/install/themes/.default/", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes/.default");
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if($USER->IsAdmin())
		{
			...
			if($step < 2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("CLO_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/clouds/install/step1.php");
			}
			elseif($step==2)
			{
				if($this->InstallDB())
				...
				$APPLICATION->IncludeAdminFile(GetMessage("CLO_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/clouds/install/step2.php");
			}
		}
	}
	...
	function DoUninstall()
	{
		...
		if($USER->IsAdmin())
		{
			...
			if($step < 2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("CLO_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/clouds/install/unstep1.php");
			}
			elseif($step == 2)
			{
				...
				$this->UnInstallFiles();
				$GLOBALS["errors"] = $this->errors;
				$APPLICATION->IncludeAdminFile(GetMessage("CLO_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/clouds/install/unstep2.php");
			}
		}
	}
}
