...
class CAllWorkflow
{
	...
	public static function OnPanelCreate()
	{
		...
		// Document history
		$flow_link_hist = "/thurly/admin/workflow_history_list.php?lang=".LANGUAGE_ID. "&find_filename=".urlencode($cur_page)."&find_filename_exact_match=Y&set_filter=Y";
		...
		if($create_permission)
		{
			...
			$APPLICATION->AddPanelButtonMenu("create", array(
				"SRC" => "/thurly/images/workflow/new_page.gif",
		...
		if($edit_permission)
		{
			$APPLICATION->AddPanelButtonMenu("edit", array(
				"SRC" => "/thurly/images/workflow/edit_flow_public.gif",
		...
		if($history_permission)
		{
			$flow_link_hist = "/thurly/admin/workflow_history_list.php?lang=".LANGUAGE_ID. "&find_filename=".urlencode($cur_page)."&find_filename_exact_match=Y&set_filter=Y";
			$APPLICATION->AddPanelButtonMenu("edit", array(
				"SRC" => "/thurly/images/workflow/history.gif",
	...
	public static function GetEditLink($FILENAME, &$status_id, &$status_title, $template="", $lang=LANGUAGE_ID, $return_url="")
	{
		...
		if($USER->CanDoFileOperation('fm_edit_in_workflow', array($SITE_ID, $FILENAME)))
		{
			...
			$link = "/thurly/admin/workflow_edit.php?lang=".$lang."&site=".$SITE_ID."&fname=".$FILENAME;
