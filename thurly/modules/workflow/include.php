...
function GetDefaultProlog($title)
{
	return "<"."?\n".
		...
		"require(\$_SERVER[\"DOCUMENT_ROOT\"].\"/thurly/modules/main/include/prolog_after.php\");\n".
		"?".">\n";
}

function GetDefaultEpilog()
{
	return "\n<"."?require(\$_SERVER[\"DOCUMENT_ROOT\"].\"/thurly/modules/main/include/epilog.php\");?".">";
}

function PathToWF($text, $DOCUMENT_ID)
{
	return preg_replace("'(<img[^>]+?src\\s*=\\s*\")(\\S+)(\"[^>]*>)'i", "\\1/thurly/admin/workflow_get_file.php?did=".$DOCUMENT_ID."&fname=\\2\\3", $text);
}
...
{
	return preg_replace_callback("'(<img[^>]+?src\\s*=\\s*[\"\'])/thurly/admin/workflow_get_file.php\\?([^>]+)([\"\'][^>]*>)'i", "convert_image", $text);
}
