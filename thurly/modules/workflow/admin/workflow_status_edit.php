...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $WORKFLOW_RIGHT=="W" && check_thurly_sessid())
{
	...
	if($res)
	{
		...
		if($apply != "")
			LocalRedirect("/thurly/admin/workflow_status_edit.php?ID=".$ID."&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
			LocalRedirect("/thurly/admin/workflow_status_list.php?lang=".LANG);
	}
	else
	{
...
if (intval($ID) > 0)
{
	...
	if ($WORKFLOW_RIGHT == "W" && intval($ID) > 1)
	{
		$aMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('".GetMessage("FLOW_DELETE_STATUS_CONFIRM")."')) window.location='workflow_status_list.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		);
	}
}
...
<?endif;?>
	...
	<tr>
		<td class="adm-detail-valign-top"><?=GetMessage('FLOW_MOVE_RIGHTS');?><br><img src="/thurly/images/workflow/mouse.gif" width="44" height="21" border=0 alt=""></td>
		<td><?echo SelectBoxM("arPERMISSION_M[]", CGroup::GetDropDownList(""), $arPERMISSION_M,"",true,8);?></td>
	</tr>
	<tr>
		<td class="adm-detail-valign-top"><?echo GetMessage('FLOW_EDIT_RIGHTS');?><br><img src="/thurly/images/workflow/mouse.gif" width="44" height="21" border=0 alt=""></td>
