...
if ($del_id > 0 && $WORKFLOW_RIGHT > "R" && check_thurly_sessid())
{
	if (CWorkflow::IsAllowEdit($del_id, $locked_by, $date_lock))
	{
		CWorkflow::Delete($del_id);
		LocalRedirect("/thurly/admin/workflow_list.php?lang=".LANGUAGE_ID);
	}
	else
	{
			...
			if (!$message)
			{
				...
				if (strlen($save) > 0 || $STATUS_ID == 1)
				{
					if (strlen($return_url) > 0)
						...
						LocalRedirect("/thurly/admin/workflow_list.php?lang=".LANGUAGE_ID."&set_default=Y&strNote=".urlencode($strNote));
				}
				elseif (strlen($apply) > 0)
				{
					LocalRedirect("/thurly/admin/workflow_edit.php?lang=".LANGUAGE_ID."&ID=".$ID."&strNote=".urlencode($strNote)."&".$tabControl->ActiveTabParam().(strlen($return_url) ? "&return_url=".urlencode($return_url) : ""));
				}
			}
		}
	}
...
if (intval($ID) > 0)
{
	...
	if (intval($locked_by) <= 0 || intval($locked_by) == $USER->GetID())
	{
		$aMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('".GetMessage("FLOW_DELETE_DOCUMENT_CONFIRM")."')) window.location='workflow_edit.php?del_id=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
...
function ShowFile(did, fname)
{
	width=650;
	height=500;
	window.open('/thurly/admin/workflow_get_file.php?did='+did+ '&fname='+fname,'','scrollbars=yes,resizable=yes,width='+width+',height='+height+',left='+Math.floor((screen.width - width)/2)+',top='+Math.floor((screen.height - height)/5));
}
</script>
...
?>
	...
	<tr>
		...
		<td><select name="template" onchange="window.location='/thurly/admin/workflow_edit.php?lang=<?echo LANG?>&amp;fname=<?echo UrlEncode($fname)?>&amp;template='+escape(this[this.selectedIndex].value)"><?
		foreach($arTemplates as $Template)
		{
		...
		<td colspan="2">
			<table border="0" cellspacing="0" cellpadding="0" width="100%" class="internal">
				...
				<tr>
					...
					if($USER->IsAdmin() || !in_array($ext, $arExt)) :
						?>&nbsp;&nbsp;<a href="workflow_file_download.php?did=<?=$ID?>&amp;fname=<?echo $zr["FILENAME"]?>" title="<?=GetMessage("FLOW_DOWNLOAD_FILE")?>"><img onmouseover="this.src='/thurly/images/workflow/download_file.gif'" onmouseout="this.src='/thurly/images/workflow/download_file_t.gif'" src="/thurly/images/workflow/download_file_t.gif" width="16" height="16" border=0></a><?
					endif;
					?></td>
					<td><?=$zr["FILESIZE"]?></td>
