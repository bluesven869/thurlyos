...
?'.'>';
		...
		$cache = \Thurly\Main\Config\Configuration::getValue('cache');
		if ($isOnline)
		{
			if (
				!is_array($cache)
			...
			{
				\Thurly\Main\Config\Configuration::setValue('cache', array(
					'type' => array(
		...
		else
		{
			...
			{
				\Thurly\Main\Config\Configuration::setValue('cache', null);
	...
	public static function getServerList()
	{
		...
		foreach (CClusterMemcache::LoadConfig() as $arData)
		{
			$result[] = array(
				...
				"HOST" => $arData["HOST"],
				"DEDICATED" => "Y",
				"EDIT_URL" => "/thurly/admin/cluster_memcache_edit.php?lang=".LANGUAGE_ID."&group_id=".$arData["GROUP_ID"]."&ID=".$arData["ID"],
			);
		}
		return $result;
	}
