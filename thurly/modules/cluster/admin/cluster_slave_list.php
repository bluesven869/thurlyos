...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/cluster/include.php");
		...
		if($arRes["STATUS"] == "READY" && $group_id > 0)
		{
			...
			if($arRes["ROLE_ID"] == "MASTER")
				$arActions[] = array(
					...
					"ACTION" => "javascript:WizardWindow.Open('thurly:cluster.master_start','".thurly_sessid()."&__wiz_node_id=".$arRes["ID"]."&__wiz_group_id=".$arRes["GROUP_ID"]."')",
				);
			else
				$arActions[] = array(
					"TEXT" => GetMessage("CLU_SLAVE_LIST_START_USING_DB"),
					"ACTION" => "javascript:WizardWindow.Open('thurly:cluster.slave_start','".thurly_sessid()."&__wiz_node_id=".$arRes["ID"]."&__wiz_group_id=".$arRes["GROUP_ID"]."')",
				);
		}
	}
...
if($bHasMaster && $group_id > 0)
{
	$aContext[] = array(
		...
		"LINK" => "javascript:WizardWindow.Open('thurly:cluster.slave_add','".thurly_sessid()."&__wiz_group_id=".$group_id."')",
...
elseif($group_id > 1)
{
	$aContext[] = array(
		...
		"LINK" => "javascript:WizardWindow.Open('thurly:cluster.master_add','".thurly_sessid()."&__wiz_group_id=".$group_id."')",
