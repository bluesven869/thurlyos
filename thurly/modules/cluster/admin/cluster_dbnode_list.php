		...
		if($arRes["STATUS"] == "READY")
		{
			if($DB->type == "MYSQL")
				$arActions[] = array(
					...
					"ACTION" => "javascript:WizardWindow.Open('thurly:cluster.module_move','".thurly_sessid()."&__wiz_node_id=".$arRes["ID"]."&__wiz_status=".$arRes["STATUS"]."')",
				);
...
if($DB->type == "MYSQL")
{
	$link = "javascript:WizardWindow.Open('thurly:cluster.dbnode_add','".thurly_sessid()."')";
	$title = GetMessage("CLU_DBNODE_LIST_ADD_TITLE1");
}
else
{
	$link = "/thurly/admin/cluster_dbnode_edit.php?lang=".LANGUAGE_ID;
