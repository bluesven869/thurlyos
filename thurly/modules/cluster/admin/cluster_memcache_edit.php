	...
	)
	{
		...
		if($res)
		{
			if(isset($_REQUEST["apply"]) && $_REQUEST["apply"] != "")
				LocalRedirect("/thurly/admin/cluster_memcache_edit.php?ID=".$res."&lang=".LANGUAGE_ID.'&group_id='.$group_id."&".$tabControl->ActiveTabParam());
			else
				LocalRedirect("/thurly/admin/cluster_memcache_list.php?lang=".LANGUAGE_ID.'&group_id='.$group_id);
		}
		else
		{
