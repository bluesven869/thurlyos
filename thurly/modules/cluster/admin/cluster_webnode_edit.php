...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/cluster/prolog.php");
...
if($_SERVER["REQUEST_METHOD"] == "POST" && check_thurly_sessid())
{
	...
	)
	{
		...
		if($res)
		{
			if(isset($_REQUEST["apply"]) && $_REQUEST["apply"] != "")
				LocalRedirect("/thurly/admin/cluster_webnode_edit.php?ID=".$ID."&lang=".LANGUAGE_ID.'&group_id='.$group_id."&".$tabControl->ActiveTabParam());
			else
				LocalRedirect("/thurly/admin/cluster_webnode_list.php?lang=".LANGUAGE_ID.'&group_id='.$group_id);
		}
		else
		{
	...
	elseif((isset($_REQUEST["delete"]) && $_REQUEST["delete"] != "") && $ID > 1)
	{
		...
		$res = $ob->Delete($ID);
		if($res)
			LocalRedirect("/thurly/admin/cluster_dbnode_list.php?lang=".LANG.'&group_id='.$group_id);
		else
			$bVarsFromForm = true;
	}
}
