...
if (strlen($fatalErrorMessage) <= 0)
{
	...
	while ($arWorkflowTemplate = $dbWorkflowTemplate->GetNext())
	{
		...
		$arWorkflowTemplates[$arWorkflowTemplate["ID"]]["URL"] = htmlspecialcharsex($APPLICATION->GetCurPageParam("workflow_template_id=".$arWorkflowTemplate["ID"].'&'.thurly_sessid_get(), Array("workflow_template_id", "sessid")));
;
	}

	if ($workflowTemplateId > 0 && check_thurly_sessid() && strlen($_POST["CancelStartParamWorkflow"]) <= 0
		&& array_key_exists($workflowTemplateId, $arWorkflowTemplates))
	{
		...
		if ($bCanStartWorkflow)
		{
			...
			else
			{
				...
				if (strlen($backUrl) <= 0)
					$backUrl = "/thurly/admin/bizproc_log.php?ID=#WF#";
