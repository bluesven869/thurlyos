...
class CBPDocument
{
	...
	*/
	...
	public static function killWorkflow($workflowId, $terminate = true, $documentId = null)
	{
		...
		if (!$errors)
		{
			\Thurly\Bizproc\WorkflowInstanceTable::delete($workflowId);
...
<script>
	function BPAShowSelector(id, type, mode, arCurValues, arDocumentType)
	{
		...
		if (mode == "only_users")
		{
			...
			(new BX.CDialog({
				'content_url': '/thurly/admin/'+module
...
<?
	...
	public static function _ReplaceTaskURL($str, $documentType)
	{
		...
		return str_replace(
			...
			Array($_SERVER['HTTP_HOST'], ($documentType[0]=="iblock"?"/thurly/admin/bizproc_task.php?workflow_id={=Workflow:id}":"/company/personal/bizproc/{=Workflow:id}/"), $baseHref),
			$str
			);
	}
	...
	public static function AddDefaultWorkflowTemplates($documentType, $additionalModuleId = null)
	{
		...
		{
			if (file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/'.$sModule.'/templates'))
			{
				if($handle = opendir($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/'.$sModule.'/templates'))
				{
					$bIn = true;
					while(false !== ($file = readdir($handle)))
					{
						if(!is_file($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/'.$sModule.'/templates/'.$file))
							continue;
						$arFields = false;
						include($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/'.$sModule.'/templates/'.$file);
	...
	public static function onAfterTMDayStart($data)
	{
		...
		$iterator = \Thurly\Bizproc\WorkflowInstanceTable::getList(
			array(
				'select' => array(new \Thurly\Main\Entity\ExpressionField('CNT', 'COUNT(\'x\')')),
				'filter' => array(
					'=STATE.STARTED_BY' => $userId,
					'<OWNED_UNTIL' => date($DB->DateFormatToPHP(FORMAT_DATETIME),
						time() - \Thurly\Bizproc\WorkflowInstanceTable::LOCKED_TIME_INTERVAL)
				),
			)
		);
		$row = $iterator->fetch();
		if (!empty($row['CNT']))
		{
			$path = \Thurly\Main\Config\Option::get("bizproc", "locked_wi_path", '');
			if (!$path)
			{
				$path = IsModuleInstalled('thurlyos') ? '/bizproc/bizproc/?type=is_locked' : '/services/bp/instances.php?type=is_locked';
			}
	...
	public static function sendB24LimitsNotifyToAdmins($ts = 0)
	{
		...
		if (!CModule::IncludeModule('thurlyos') || !CModule::IncludeModule("im"))
			return '';

		$userIds = \CThurlyOS::getAllAdminId();
		if (!$userIds)
			return '';
