...
class CBPHelper
{
				...
				if (is_array($result))
				{
					...
					foreach ($result as $r)
					{
						...
						if ($arImg = $dbImg->Fetch())
							$newResult[] = "[url=/thurly/tools/bizproc_show_file.php?f=".urlencode($arImg["FILE_NAME"])."&i=".$r."]".htmlspecialcharsbx($arImg["ORIGINAL_NAME"])."[/url]";
					}
				}
				else
				{
					...
					if ($arImg = $dbImg->Fetch())
						$newResult = "[url=/thurly/tools/bizproc_show_file.php?f=".urlencode($arImg["FILE_NAME"])."&i=".$result."]".htmlspecialcharsbx($arImg["ORIGINAL_NAME"])."[/url]";
				}
				break;
	...
	{
		$selectorProps = \Thurly\Main\Web\Json::encode(array(
			'controlId' => $controlId,
			'baseType' => $baseType
		));
