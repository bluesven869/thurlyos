...
SequenceActivity = function()
{
	...
	{
		this.parentNode.style.backgroundImage = 'url(/thurly/images/bizproc/arr_over.gif)';
	}

	ob.LineMouseOut = function (e)
	{
		this.parentNode.style.backgroundImage = 'url(/thurly/images/bizproc/arr.gif)';
	}
		...
		*/
		...
		for (groupId in arAllActGroups)
		{
			...
			for(var act_i in arAllActivities)
			{
				...
				oSubMenu.push({'ICON': 'url('+arAllActivities[act_i]['ICON']+')', 'TEXT': '<img src="'+(arAllActivities[act_i]['ICON']?arAllActivities[act_i]['ICON']:'/thurly/images/bizproc/act_icon.gif')+'" align="left" style="margin-right: 7px;margin-left: 0px">' + '<b>' + HTMLEncode(arAllActivities[act_i]['NAME']) + '</b><br>' + HTMLEncode(arAllActivities[act_i]['DESCRIPTION']), 
		...
		if (arUserParams && BX.type.isArray(arUserParams['SNIPPETS']))
		{
			...
			for(var isn in arUserParams['SNIPPETS'])
			{
				...
				if (!icon)
				{
					icon = '/thurly/images/bizproc/act_icon.gif';
				}
	...
	ob.CreateLine = function(ind)
	{
		...
		ob.childsContainer.rows[ind*2 + ob.iHead].cells[0].style.background = 'url(/thurly/images/bizproc/arr.gif) no-repeat scroll 50% 50%';

		var i = ob.childsContainer.rows[ind * 2 + ob.iHead].cells[0].appendChild(document.createElement('IMG'));
		i.src = '/thurly/images/1.gif';
		i.width = '28';
		i.height = '21';
