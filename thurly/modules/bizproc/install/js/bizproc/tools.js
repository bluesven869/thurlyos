...
if (typeof BX.Bizproc.doInlineTask === 'undefined')
{
	BX.Bizproc.doInlineTask = function (parameters, callback, scope)
	{
		...
		if (!parameters || !parameters['TASK_ID'])
			return false;
		parameters['sessid'] = BX.thurly_sessid();
		BX.ajax({
			method:'POST',
			dataType: 'json',
			url:'/thurly/tools/bizproc_do_task_ajax.php',
			data: parameters,
	...
	BX.Bizproc.showTaskPopup = function (taskId, callback, userId, scope, useIframe)
	{
		...
		BX.ajax({
			method: 'GET',
			dataType: 'html',
			url: '/thurly/components/thurly/bizproc.task/popup.php?site_id='+BX.message('SITE_ID')+'&TASK_ID='
			...
			onsuccess: function (HTML)
			{
				...
				BX.load(['/thurly/components/thurly/bizproc.task/templates/.default/style.css'], function()
				{
					BX.Bizproc.taskPopupInstance.show();
				});
			}
	...
	BX.Bizproc.showDelegationPopup = function(scope, taskId, userId)
	{
		...
		BX.ajax({
			method: 'GET',
			dataType: 'html',
			url: '/thurly/components/thurly/bizproc.task/delegate.php?SITE_ID=' + BX.message('SITE_ID')
	...
	BX.Bizproc.delegateTask = function(taskId, fromUserId, toUserId)
	{
		...
		BX.ajax({
			...
			method:'POST',
			dataType: 'json',
			url:'/thurly/components/thurly/bizproc.task/delegate.php',
			data: parameters,
			onsuccess: function(response)
			{
	...
	BX.Bizproc.showWorkflowInfoPopup = function (workflowId)
	{
		BX.ajax({
			method: 'GET',
			dataType: 'html',
			url: '/thurly/components/thurly/bizproc.workflow.info/popup.php?site_id='+BX.message('SITE_ID')+'&WORKFLOW_ID=' + workflowId,
			onsuccess: function (HTML)
			{
				BX.load(['/thurly/components/thurly/bizproc.workflow.info/templates/.default/style.css'], function()
				{
					var wrapper = BX.create('div', {
						style: {width: '800px'}
					});
	...
	BX.Bizproc.showWorkflowLogPopup = function (workflowId, params)
	{
		...
		BX.ajax({
			method: 'GET',
			dataType: 'html',
			url: '/thurly/components/thurly/bizproc.log/popup.php?site_id='+BX.message('SITE_ID')+'&WORKFLOW_ID=' + workflowId,
			onsuccess: function (HTML)
			{
				var wrapper = BX.create('div', {
					style: {width: '800px'}
	...
	BX.Bizproc.postTaskForm = function (form, e)
	{
		...
		form.action = '/thurly/tools/bizproc_do_task_ajax.php';
		form.BPRUNNING = true;
