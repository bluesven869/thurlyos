...
$pathToTemplates = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/bizproc/templates_bp";
...
if (strlen($arResult["FatalErrorMessage"]) <= 0)
{
	...
	if ($arResult["Step"] > 1 && !check_thurly_sessid())
	{
		$arResult["Step"] = 1;
...
if (strlen($arResult["FatalErrorMessage"]) <= 0)
{
	if ($arResult["Step"] == 1)
	{
		...
		$arResult["ComponentTemplates"] = array(
			"Start" => CComponentUtil::GetTemplatesList("thurly:bizproc.wizards.start"),
			"List" => CComponentUtil::GetTemplatesList("thurly:bizproc.wizards.list"),
			"View" => CComponentUtil::GetTemplatesList("thurly:bizproc.wizards.view"),
		);
	}
