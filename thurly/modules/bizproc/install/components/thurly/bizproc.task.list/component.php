...
$isAdmin = $USER->IsAdmin() || (CModule::IncludeModule('thurlyos') && CThurlyOS::IsPortalAdmin($USER->GetID()));
...
if (strlen($arResult["FatalErrorMessage"]) <= 0 && !$arParams['COUNTERS_ONLY'])
{
	...
	$arResult['DOCUMENT_TYPES'] = array(
		...
		'lists' => array('NAME' => GetMessage('BPATL_FILTER_DOCTYPE_LISTS'), 'FILTER' => array('MODULE_ID' => 'lists', 'ENTITY' => 'Thurly\Lists\BizprocDocumentLists'), 'COUNTER_KEY' => 'iblock')
	);
	...
	if (!empty($_REQUEST['action_button_'.$arResult["GRID_ID"]]) && check_thurly_sessid())
	{
