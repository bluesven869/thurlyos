...
use Thurly\Bizproc\WorkflowInstanceTable;
...
class BizprocWorkflowInstances extends \CThurlyComponent
{
	...
	protected function isAdmin()
	{
		...
		{
			$this->isAdmin = $USER->IsAdmin() || (Loader::includeModule('thurlyos') && \CThurlyOS::IsPortalAdmin($USER->GetID()));
		}
		return $this->isAdmin;
	}
	...
	protected function getDocumentTypes()
	{
		return array(
			...
			'lists' => array('NAME' => Loc::getMessage('BPWI_MODULE_IBLOCK'), 'MODULE_ID' => 'lists', 'ENTITY' => 'Thurly\Lists\BizprocDocumentLists')
		);
	}
	...
	public function executeComponent()
	{
		...
		if (!empty($_POST['ID']) && check_thurly_sessid() && $this->isAdmin())
		{
	...
	{
		if ($moduleId == 'lists' && $entity == 'Thurly\Lists\BizprocDocumentLists')
		{
			$moduleId = 'iblock';
		}
