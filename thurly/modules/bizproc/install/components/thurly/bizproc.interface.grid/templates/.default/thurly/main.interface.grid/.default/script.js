	...
	this.InitTable = function()
	{
		...
		for(i=0; i<nCols; i++)
		{
			...
			for(j=0; j<2; j++)
			{
				...
				if(j==1)
				{
					if(cell.className && (cell.className == 'bx-bizproc-actions-col' || cell.className == 'bx-bizproc-checkbox-col'))
						continue;
		...
		for(i=0; i<n; i++)
		{
			...
			if(row.className && row.className == 'bx-bizproc-grid-footer')
				continue;

			/*first and last columns style classes*/
			row.cells[0].className += ' bx-bizproc-left';
			row.cells[row.cells.length-1].className += ' bx-bizproc-right';
...
//					row.className += ' bx-even';
		...
		if(tbl.rows.length > 2)
		{
			...
			if(r.className && r.className == 'bx-bizproc-grid-footer')
				r = tbl.rows[tbl.rows.length-2];
			r.className += ' bx-bizproc-bottom';
		}
	};
	...
	this.HighlightGutter = function(cell, on)
	{
		...
		var bSorted = (gutter.className.indexOf('bx-bizproc-sorted') != -1);
		if(on)
		{
			if(bSorted)
			{
	...
	this.OnClickRow = function(e)
	{
		...
		else
		{
			var tr = BX.findParent(obj, {
				tagName: 'tr',
				className: 'bx-bizproc-table-body'
			}, this.tableNode);
			if(tr)
			{
				var td = BX.findChild(tr, {
					tagName: 'td',
					className: 'bx-bizproc-checkbox-col'
				});
				if(td)
				{
		...
		if(this.IsActionEnabled('edit'))
		{
			...
			for(var i=0; i<ids.length; i++)
			{
				...
				if(el.checked)
				{
					...
					if(td.className == 'bx-bizproc-actions-col')
						td = jsUtils.FindNextSibling(td, "td");
					...
					for(var col_id in this.oColsMeta)
					{
						if(this.oColsMeta[col_id].editable == true && this.oEditData[row_id][col_id] !== false)
						{
							...
							switch(this.oColsMeta[col_id].type)
							{
								...
								default:
									var props = {
										className: 'bx-bizproc-folder-title list',
										'type':'text',
										'name':name,
										'value':data,
	...
	this.Reload = function(url)
	{
		...
		{
			BX.ajax.get(
				url + (url.indexOf('?') == -1? '?':'&') + 'bxajaxid=' + this.vars.ajax.AJAX_ID,
	...
	this.SetTheme = function(menuItem, theme)
	{
		...
		BX(_this.table_id).className = 'bx-bizproc-interface-grid bx-bizproc-interface-grid-theme-'+theme;
	...
	this.DragFinish = function(obDrag, x, y, e)
	{
		...
		for(var i=0; i < 2; i++)
		{
			...
			if(cell.className && (cell.className.indexOf('bx-bizproc-actions-col') != -1 || cell.className.indexOf('bx-bizproc-checkbox-col') != -1))
				delta ++;
		}
...
if (typeof(BX.Bizproc.GridPageSizeControl) === "undefined")
{
	BX.Bizproc.GridPageSizeControl = (function(){
