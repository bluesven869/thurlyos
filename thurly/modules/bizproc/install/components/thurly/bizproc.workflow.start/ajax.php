...
if (!check_thurly_sessid() || !is_object($user) || !$user->IsAuthorized() || !CModule::IncludeModule('bizproc'))
{
	die();
}
...
$writeResponse = function(\Thurly\Main\Result $data)
{
...
$sendError = function($error) use ($writeResponse)
{
	...
	foreach ($errors as $e)
	{
		$result->addError(new \Thurly\Main\Error($e));
	}
