...
if ($REQUEST_METHOD == "GET" && strlen($RestoreDefaults) > 0 && $bizprocPerms == "W" && check_thurly_sessid())
{
...
$strWarning = "";
if ($REQUEST_METHOD == "POST" && strlen($Update) > 0 && $bizprocPerms == "W" && check_thurly_sessid())
{
	...
	\Thurly\Main\Config\Option::set("bizproc", "use_gzip_compression", $_REQUEST["use_gzip_compression"]);
	\Thurly\Main\Config\Option::set("bizproc", "locked_wi_path", $_REQUEST["locked_wi_path"]);
...
?>
	...
	<?endfor;?>
		<tr>
			...
			<td width="50%" valign="top">
				<select name="use_gzip_compression">
					<?$useGZipCompression = \Thurly\Main\Config\Option::get("bizproc", "use_gzip_compression", "");?>
		...
		<tr>
			...
			<td width="50%" valign="top">
				<?$path = \Thurly\Main\Config\Option::get("bizproc", "locked_wi_path", "/services/bp/instances.php?type=is_locked");?>
...
function RestoreDefaults()
{
	if (confirm('<?echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>'))
		window.location = "<?= $APPLICATION->GetCurPage() ?>?RestoreDefaults=Y&lang=<?= LANG ?>&mid=<?= urlencode($mid) ?>&<?= thurly_sessid_get() ?>";
}
</script>
