...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/database.php");
...
abstract class CDatabaseMysql extends CAllDatabase
{
	...
	function Query($strSql, $bIgnoreErrors=false, $error_position="", $arOptions=array())
	{
		...
		//or replicated one
		$connectionPool = \Thurly\Main\Application::getInstance()->getConnectionPool();
		...
		if(!$result)
		{
			...
			if(!$bIgnoreErrors)
			{
				...
				elseif(file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/dbquery_error.php"))
					include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/dbquery_error.php");
				else
...
{
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/mysql/database_mysqli.php");
}
else
{
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/mysql/database_mysql.php");
}
