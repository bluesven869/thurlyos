	...
	public static function BuildRating($ID)
	{
		...
		if ($arRating && $arRating['ACTIVE'] == 'Y')
		{
			...
			$strSql  = "
				...
				WHERE 
					U.ACTIVE = 'Y' 
					AND (CASE WHEN U.EXTERNAL_AUTH_ID IN ('".join("', '", \Thurly\Main\UserTable::getExternalUserTypes())."') THEN 'Y' ELSE 'N' END) = 'N'
					AND RU.ID IS NULL	";
			$res = $DB->Query($strSql, false, $err_mess.__LINE__);
			// authority calc
	...
	public static function GetRatingVoteListSQL($arParam, $bplus, $bIntranetInstalled)
	{
		...
		return "
			...
			FROM
				...
				b_user U
			WHERE
				(CASE WHEN U.EXTERNAL_AUTH_ID IN ('".join("', '", \Thurly\Main\UserTable::getExternalUserTypes())."') THEN 'Y' ELSE 'N' END) = 'N'
