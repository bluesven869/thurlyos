...
if (!defined("US_SHARED_KERNEL_PATH"))
	define("US_SHARED_KERNEL_PATH", "/thurly");

class CUpdateSystem
{
	public static function IsInCommonKernel()
	{
		return (!defined("US_THURLY24_MODE") || !US_THURLY24_MODE) && file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/update_db_updater.php");
	}
	...
	public static function RegisterVersion(&$strError, $lang = false, $stableVersionsOnly = "Y")
	{
		...
		if (strlen($strError_tmp)<=0)
		{
			if (!($fp1 = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", "wb")))
				$strError_tmp .= "[URV02] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates", GetMessage("SUPP_RV_ER_TEMP_FILE")).".<br>";
		}

		if (strlen($strError_tmp)<=0)
		{
			if (!fwrite($fp1, $fcontent))
				$strError_tmp .= "[URV03] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", GetMessage("SUPP_RV_WRT_TEMP_FILE")).".<br>";
		...
		if (strlen($strError_tmp)<=0)
		{
			$updates_dir_full = $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/".$temporary_updates_dir;
		...
		if (strlen($strError_tmp)<=0)
		{
			if (!is_writeable($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include.php"))
			{
				$strError_tmp .= "[URV09] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include.php", GetMessage("SUPP_RV_NO_WRITE")).".<br>";
			}
		}

		if (strlen($strError_tmp) <= 0)
		{
			if (!copy($updates_dir_full."/include.php", $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include.php"))
				$strError_tmp .= "[URV10] ".GetMessage("SUPP_RV_ERR_COPY").".<br>";
			@chmod($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include.php", BX_FILE_PERMISSIONS);
		}
		...
		if (strlen($strError_tmp) <= 0)
		{
			...
			if ($strongUpdateCheck == "Y")
			{
				...
				$crc32_new = dechex(crc32(file_get_contents($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include.php")));
				if ($crc32_new != $crc32_old)
					$strError_tmp .= "[URV1011] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include.php", GetMessage("SUPP_UGA_FILE_CRUSH")).".<br>";
			}
		}
	...
	public static function UpdateUpdate(&$strError, $lang = false, $stableVersionsOnly = "Y")
	{
		...
		{
			if (!($fp1 = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", "wb")))
				$strError_tmp .= "[UUU02] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates", GetMessage("SUPP_RV_ER_TEMP_FILE")).".<br>";
		}

		if (strlen($strError_tmp)<=0)
		{
			if (!fwrite($fp1, $fcontent))
				$strError_tmp .= "[UUU03] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", GetMessage("SUPP_RV_WRT_TEMP_FILE")).".<br>";
		...
		*/
		...
		if (strlen($strError_tmp)<=0)
		{
			CUpdateSystem::CopyDirFiles($updates_dir_full."/main", $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main", $strError_tmp);
		}
	...
	public static function LoadSources(&$strError, $lang = false, $stableVersionsOnly = "Y")
	{
		...
		{
			if (!($fp1 = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", "wb")))
				$strError_tmp .= "[ULS02] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates", GetMessage("SUPP_RV_ER_TEMP_FILE")).".<br>";
		}

		if (strlen($strError_tmp)<=0)
		{
			if (!fwrite($fp1, $fcontent))
				$strError_tmp .= "[ULS03] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", GetMessage("SUPP_RV_WRT_TEMP_FILE")).".<br>";
		...
		if (strlen($strError_tmp)<=0)
		{
			foreach ($arClientModules as $module_id => $version)
			{
				...
				$to_dir = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$module_id;
	...
	public static function GetAvailableUpdateTypes(&$strError, $lang = false, $stableVersionsOnly = "Y")
	{
		...
		CUpdateSystem::CheckDirPath($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/", true);
	...
	public static function LoadModuleUpdates($arModules, &$strError, $lang = false, $stableVersionsOnly = "Y")
	{
		...
		if (strlen($strError_tmp)<=0)
		{
			if (!($fp1 = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", "wb")))
			{
				$strError_tmp .= "[ULMU04] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates", GetMessage("SUPP_RV_ER_TEMP_FILE")).".<br>";
			}
		}
		...
		if (strlen($strError_tmp)<=0)
		{
			...
			if (!file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz")
				|| !is_file($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz"))
			{
				$strError_tmp .= "[ULMU05] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", GetMessage("SUPP_LMU_NO_TMP_FILE")).".<br>";
			}
		}
	...
	public static function LoadLangsUpdates($arLangs, &$strError, $lang = false, $stableVersionsOnly = "Y")
	{
		...
		if (strlen($strError_tmp)<=0)
		{
			if (!($fp1 = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", "wb")))
			{
				$strError_tmp .= "[ULLU04] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates", GetMessage("SUPP_RV_ER_TEMP_FILE")).".<br>";
			}
		}
		...
		if (strlen($strError_tmp)<=0)
		{
			...
			if (!file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz")
				|| !is_file($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz"))
			{
				$strError_tmp .= "[ULLU05] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", GetMessage("SUPP_LMU_NO_TMP_FILE")).".<br>";
			}
		}
	...
	public static function LoadHelpUpdates($arHelp, &$strError, $lang = false, $stableVersionsOnly = "Y")
	{
		...
		{
			if (!($fp1 = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", "wb")))
				$strError_tmp .= "[ULHU04] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates", GetMessage("SUPP_RV_ER_TEMP_FILE")).".<br>";
		}
		...
		if (strlen($strError_tmp)<=0)
		{
			...
			if (!file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz")
				|| !is_file($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz"))
			{
				$strError_tmp .= "[ULHU05] ".str_replace("#FILE#", $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz", GetMessage("SUPP_LMU_NO_TMP_FILE")).".<br>";
			}
		}
	...
	public static function UnGzipArchive(&$updates_dir, &$strError, $DelArch = "Y")
	{
		...
		$archiveFileName = $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/update_archive.gz";
		...
		if (strlen($strError_tmp)<=0)
		{
			...
			$updates_dir_full = $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/".$updates_dir;
		...
		{
			if ($bCompressionUsed)
				$flabel = gzread($zp, strlen("THURLY"));
			else
				$flabel = fread($zp, strlen("THURLY"));
				...
				if ($arSelectedModules===false || in_array($arModulesList[$i]["@"]["ID"], $arSelectedModules))
				{
						...
						&& count($arModulesList[$i]["#"]["VERSION_CONTROL"])>0)
					{
						for ($j = 0, $nj = count($arModulesList[$i]["#"]["VERSION_CONTROL"]); $j < $nj; $j++)
						{
							...
							if ($bNeedInstall)
							{
								$strModule_tmp_dir = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$strModule_tmp;
	...
	public static function CheckUpdatability($updates_dir, &$strError)
	{
		...
		if ($handle = @opendir($updates_dir_full))
		{
			while (($file = readdir($handle)) !== false)
			{
				...
				{
					$strRealPath = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".substr($updates_dir."/".$file, strpos($updates_dir."/".$file, "/"));
	...
	public static function UpdateKernel($updates_dir, $arModules, &$strError, &$arErrorModules, &$arSuccessModules)
	{
		...
		if (strlen($strError_tmp)<=0)
		{
			for ($i = 0, $ni = count($arModules); $i < $ni; $i++)
			{
				...
				$to_dir = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$arModules[$i];
				...
				if (strlen($strError_tmp1) <= 0)
				{
					for ($i1 = 0, $ni1 = count($arUpdaters); $i1 < $ni1; $i1++)
					{
						...
						{
							$strError_tmp2 = "";
							CUpdateSystem::RunUpdaterScript($from_dir.$arUpdaters[$i1][0], $strError_tmp2, "/thurly/updates/".$updates_dir."/".$arModules[$i], $arModules[$i]);
	...
	public static function UpdateLangs($updates_dir, $arLangs, &$strError, &$arErrorLangs, &$arSuccessLangs)
	{
		...
		if (strlen($strError_tmp) <= 0)
		{
			$to_dir1 = $_SERVER["DOCUMENT_ROOT"]."/thurly/components/thurly";
		...
		if (strlen($strError_tmp)<=0)
		{
			$to_dir = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules";
	...
	public static function UpdateHelp($updates_dir, $arHelp, &$strError, &$arErrorHelp, &$arSuccessHelp)
	{
		...
		$updates_dir_full = $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/".$updates_dir;
		$help_dir_full = $_SERVER["DOCUMENT_ROOT"]."/thurly/help";
	...
	public static function AddMessage2Log($sText, $sErrorCode = "")
	{
		...
		$READ_PSIZE = 8000;
		$LOG_FILE = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/updater.log";
		$LOG_FILE_TMP = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/updater_tmp1.log";
	...
	public static function GetModules(&$strError, $arSelected = false)
	{
		...
		$handle = @opendir($_SERVER["DOCUMENT_ROOT"]."/thurly/modules");
		if ($handle)
		{
			...
			while (false !== ($dir = readdir($handle)))
			{
				if (is_dir($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$dir)
					&& $dir!="." && $dir!=".." && $dir!="main")
				{
					if ($arSelected===false || is_array($arSelected) && in_array($dir, $arSelected))
					{
						$module_dir = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$dir;
	...
	public static function GetLanguages(&$strError, $arSelected = false)
	{
		...
		$strLangPath = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/lang";
	...
	public static function GetHelps(&$strError, $arSelected = false)
	{
		...
		$strHelpPath = $_SERVER["DOCUMENT_ROOT"]."/thurly/help";
	...
	public static function GetModuleVersion($module)
	{
		...
		$strModule_tmp_dir = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$module;
		if (file_exists($strModule_tmp_dir) && is_dir($strModule_tmp_dir))
		{
			if ($module != "main")
			{
	...
	public static function EraseOldFolders($iCnt = 1)
	{
		...
		$path = $_SERVER["DOCUMENT_ROOT"]."/thurly/updates";
		$arFolders = array();
		if ($handle = @opendir($path))
		{
			while (($file = readdir($handle)) !== false)
