...
abstract class CAllUser extends CDBResult
{
	...
	public function RequiredHTTPAuthBasic($Realm = "Thurly")
	{
	...
	public function LoginByDigest($arDigest)
	{
		...
		if($arUser = $res->Fetch())
		{
			...
			if(($appPassword = \Thurly\Main\Authentication\ApplicationPasswordTable::findDigestPassword($arUser["ID"], $arDigest)) !== false)
			{
		...
		if($user_id > 0)
		{
			if($applicationId === null && CModule::IncludeModule("security"))
			{
				...
				*/
				...
				$doAuthorize = \Thurly\Security\Mfa\Otp::verifyUser($arParams);
			}
	...
	public function LoginByOtp($otp, $remember_otp = "N", $captcha_word = "", $captcha_sid = "")
	{
		...
		$userParams = \Thurly\Security\Mfa\Otp::getDeferredParams();
		...
		if(!\Thurly\Security\Mfa\Otp::verifyUser($userParams))
		{
	...
	public function AuthorizeWithOtp($user_id)
	{
		...
		if(CModule::IncludeModule("security"))
		{
			...
			When authorization is disallowed the OTP form will be shown on the next hit.
			*/
			$doAuthorize = \Thurly\Security\Mfa\Otp::verifyUser(array("USER_ID" => $user_id));
		}
	...
	public function Register($USER_LOGIN, $USER_NAME, $USER_LAST_NAME, $USER_PASSWORD, $USER_CONFIRM_PASSWORD, $USER_EMAIL, $SITE_ID = false, $captcha_word = "", $captcha_sid = 0, $bSkipConfirm = false)
	{
		...
		{
			$agreementObject = new \Thurly\Main\UserConsent\Agreement($agreementId);
			if ($agreementObject->isExist() && $agreementObject->isActive() && $_REQUEST["USER_AGREEMENT"] == "Y")
			{
				\Thurly\Main\UserConsent\Consent::addByContext($agreementId, "main/reg", "register");
			}
		}
	...
	public function Update($ID, $arFields, $authActions = true)
	{
		...
		{
			\Thurly\Main\UserTable::indexRecord($ID);

			if(defined("BX_COMP_MANAGED_CACHE"))
			{
				$userData = \Thurly\Main\UserTable::getById($ID)->fetch();
				$isRealUser = !$userData['EXTERNAL_AUTH_ID'] || !in_array($userData['EXTERNAL_AUTH_ID'], \Thurly\Main\UserTable::getExternalUserTypes());
	...
	public static function Delete($ID)
	{
		...
		if(defined("BX_COMP_MANAGED_CACHE"))
		{
			$isRealUser = !$arUser['EXTERNAL_AUTH_ID'] || !in_array($arUser['EXTERNAL_AUTH_ID'], \Thurly\Main\UserTable::getExternalUserTypes());
		...
		\Thurly\Main\UserTable::deleteIndexRecord($ID);
	...
	public static function SetLastActivityDateByArray($arUsers)
	{
		...
		$event = new \Thurly\Main\Event("main", "OnUserSetLastActivityDate", array($arUsers));
		$event->send();
	...
	{
		return \Thurly\Main\UserTable::getSecondsForLimitOnline();
	}
	...
	public static function GetOnlineStatus($userId, $lastseen, $now = false)
	{
		...
		if ($lastseen instanceof \Thurly\Main\Type\DateTime)
		{
	...
	public static function FormatLastActivityDate($timestamp, $now = false)
	{
		...
		if ($timestamp instanceof \Thurly\Main\Type\DateTime)
		{
