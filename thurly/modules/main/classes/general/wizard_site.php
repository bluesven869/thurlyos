...
class CWizard
{
	...
	function Install()
	{
		...
		elseif ($this->pathToScript)
		{
			...
			{
				$lang = LANGUAGE_ID;
				$wizardPath = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main";
	...
	function __SetTemplate()
	{
		...
		foreach ($this->arDescription["TEMPLATES"] as $arTemplate)
		{
			if($arTemplate["SCRIPT"]=="wizard_sol")
			{
				...
				include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/wizard_sol/template.php");
	...
	function __InstallSite($siteID)
	{
		...
		require_once($_SERVER['DOCUMENT_ROOT']."/thurly/modules/main/include.php");
	...
	function __MoveDirFiles(&$arFiles)
	{
		...
		foreach ($arFiles["FILES"] as $arFile)
		{
			...
			{
				if ($arFile["DELETE"] == "/" || $arFile["DELETE"] == "/thurly" || $arFile["DELETE"] == "/thurly/")
					continue;
