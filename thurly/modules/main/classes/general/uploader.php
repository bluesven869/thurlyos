...
class CImageUploader
{
	public static function ShowScript($Params = array())
	{
		...
		$APPLICATION->AddHeadScript('/thurly/image_uploader/bximageuploader.js');
		$APPLICATION->SetAdditionalCSS('/thurly/image_uploader/bximageuploader.css');

		$APPLICATION->AddHeadScript('/thurly/image_uploader/aurigma.uploader.js');
		$APPLICATION->AddHeadScript('/thurly/image_uploader/aurigma.uploader.installationprogress.js');
		$id = self::GetId();
		...
		var bxp = {
			...
			activeXControl:
			{
				...
				codeBase: '/thurly/image_uploader/ImageUploader7.cab',
				codeBase64: '/thurly/image_uploader/ImageUploader7_x64.cab',
				progId: 'Thurly.ImageUploader.7',
				version: '7.0.38.0'
			},
			javaControl:
			{
				className: 'com.thurlysoft.imageuploader.ImageUploader',
				codeBase: '/thurly/image_uploader/ImageUploader7.jar',
				version: '7.0.38.0'
			},
			metadata: {},
			events:{
	...
	{
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/image_uploader/ImageUploaderPHP/UploadHandler.class.php");
		//include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/image_uploader/ImageUploaderPHP/UploadedFile.class.php");
	}
	...
	public static function SetTmpPath($PackageGuid, $pathToTmp)
	{
		...
		self::$sTmpPath = $pathToTmp.md5(serialize(array("PackageGuid " => $PackageGuid, "sessid" => thurly_sessid())));
	...
	{
		if (!defined('BX_UTF'))
			$url = \Thurly\Main\Text\Encoding::convertEncoding($url, SITE_CHARSET, "UTF-8");
...
class CFlashUploader extends CImageUploader
{
	public static function ShowScript($Params = array())
	{
		...
		$APPLICATION->AddHeadScript('/thurly/image_uploader/bximageuploader.js');
		$APPLICATION->SetAdditionalCSS('/thurly/image_uploader/bximageuploader.css');
		$APPLICATION->AddHeadScript('/thurly/image_uploader/flash/aurigma.imageuploaderflash.js');
		$id = self::GetId();
		?>
		<script>
		...
		var bxp = {
			...
			converters: [],
			flashControl: {
				codeBase: '/thurly/image_uploader/flash/aurigma.imageuploaderflash.swf',
				wmode: "opaque"
			},
			uploadSettings: {
				actionUrl: '<?= CUtil::JSEscape(self::StrangeUrlEncode($Params['actionUrl']))?>',
	...
	public static function InitUploaderHandler()
	{
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/image_uploader/ImageUploaderFlashPHP/UploadHandler.class.php");
	}
...
class CFileUploader extends \Thurly\Main\UI\Uploader\Uploader {
	function __construct($params, $doCheckPost = true)
	{
