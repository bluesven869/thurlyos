...
class CThurlyComponent
{
	...
	/** @var \Thurly\Main\Composite\StaticArea[] */
	 ...
	 *
	 * @param CThurlyComponent $component
	 */
	public function __construct($component = null)
	{
		if(is_object($component) && ($component instanceof cthurlycomponent))
		{
	...
	/**
	 * Function returns component name in form thurly:component.name
	 *
	...
	/**
	 * Function returns path to component in form /thurly/component.name
	 *
	 * <p>Note: component must be inited by initComponent method.</p>
	 * @return string
	 *
	...
	/**
	 * Function returns path to component relative to Web server DOCUMENT_ROOT in form /thurly/components/thurly/component.name
	 *
	 * <p>Note: component must be inited by initComponent method.</p>
	 * @return string
	 *
	 ...
	 * The list contains parameters which are presented in  \CThurlyComponent::listKeysSignedParameters().
	 *
	 * @see \CThurlyComponent::listKeysSignedParameters()
	 *
	 * @return string|null
	 */
	...
	{
		$this->signedParameters = \Thurly\Main\Component\ParameterSigner::signParameters($this->getName(), $params);
	 ...
	 *
	 * <p>Note: component must be inited by initComponent method.</p>
	 * @return CThurlyComponentTemplate
	 *
	 */
	...
	/**
	 * Function includes class of the component by component name thurly:component.base
	 *
	 ...
	 * Then if there is subsclass of CThurlyComponent found? it's name is returned.</p>
	...
	final private function __getClassForPath($componentPath)
	{
		if (!isset(self::$__classes_map[$componentPath]))
		{
			...
			if (file_exists($fname) && is_file($fname))
			{
				...
				{
					if (is_subclass_of($afterClasses[$i], "cthurlycomponent"))
	...
	final public function includeComponentLang($relativePath = "", $lang = false)
	{
		...
		if($lang === false)
		{
			\Thurly\Main\Localization\Loc::loadMessages($path);
		}
		else
		{
			\Thurly\Main\Localization\Loc::loadLanguageFile($path, $lang);
		}
	}
	/**
	 ...
	 * @param array[string]mixed $arParams
	 * @param CThurlyComponent $parentComponent
	 * @return mixed
	 *
	 */
	final public function includeComponent($componentTemplate, $arParams, $parentComponent)
	{
		...
		if($this->classOfComponent)
		{
			...
			$componentFrame = new \Thurly\Main\Composite\Internals\AutomaticArea($component);
		...
		else
		{
			...
			$componentFrame = new \Thurly\Main\Composite\Internals\AutomaticArea($this);
			$componentFrame->start();
		...
		if (!$frameMode)
		{
			$page = \Thurly\Main\Composite\Page::getInstance();
	...
	final public function initComponentTemplate($templatePage = "", $siteTemplate = false, $customTemplatePath = "")
	{
		...
		$this->__template = new CThurlyComponentTemplate();
	...
	final public function startResultCache($cacheTime = false, $additionalCacheID = false, $cachePath = false)
	{
		...
		$this->__cache = \Thurly\Main\Data\Cache::createInstance();
			...
			if (array_key_exists("templateCachedData", $arCache))
			{
				...
				if ($templateCachedData && is_array($templateCachedData))
				{
					...
					if (array_key_exists("frames", $templateCachedData) && is_array($templateCachedData["frames"]))
					{
						...
						{
							$frame = \Thurly\Main\Composite\StaticArea::applyCachedData($frameState);
							if ($this->__parent)
							{
					...
					)
					{
						...
						{
							\Thurly\Main\Composite\StaticArea::applyCachedData($frame);
						}
					}
	...
	final public static function clearComponentCache($componentName, $siteId = "")
	{
		...
		if ($componentRelativePath != "")
		{
			...
			if ($siteId == "")
			{
				$rsSite = \Thurly\Main\SiteTable::getList(array('order' => array('SORT' => 'ASC')));
	 ...
	 *
	 * @param \Thurly\Main\Composite\StaticArea $frame
	 * @return void
	 *
	 */
	 ...
	 * @return string
	 *
	 * @see \Thurly\Main\Type\RandomSequence::randString
	 */
	public function randString($length = 6)
	{
		if (!$this->randomSequence)
		{
			...
			$this->randomSequence = new \Thurly\Main\Type\RandomSequence($seed);
		}
	...
	public function getDefaultFrameMode()
	{
		...
		$compositeOptions = \Thurly\Main\Composite\Helper::getOptions();
