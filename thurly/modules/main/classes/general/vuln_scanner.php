	...
	private function dependencyHave($tokens, $type)
	{
		...
		for ($i = 1, $tokens_count = count($tokens) - 1; $i < $tokens_count; $i++)
		{
			if(is_array($tokens[$i]))
			{
				...
				{
					case 'XSS':
						if($tokens[$i][0] === T_STRING && $tokens[$i - 1] !== '!' && $tokens[$i][1] === 'check_thurly_sessid')
							return true;
						break;
					case '!XSS':
						if($tokens[$i][0] === T_STRING && $tokens[$i - 1] === '!' && $tokens[$i][1] === 'check_thurly_sessid')
					...
					if(in_array($token, $this->tokens_type['INCLUDES']))
					{
						...
						if($token == T_INCLUDE_COMPONENT)
						{
							if(!$this->mp_mode) //not done yet
							{
								...
								if(empty($component_name)) // || strpos($component_name, 'thurly:') === 0)
									continue;

								$component_name = "/".str_replace(":", "/", $component_name);

								if(is_file($this->arParams['doc_root_path'].'/thurly/components'.$component_name.'/component.php'))
									$inc_file = $this->arParams['doc_root_path'].'/thurly/components'.$component_name.'/component.php';
								else
									continue;
						...
						elseif($token == T_INCLUDE_RESULT_MODIFIER)
						{
							...
							{
								$skip = 1;
								$component_path = array_pop(explode('thurly/components', dirname($this->current_file)));
								$component_template = (!empty($this->tokens[$i][3]) ? $this->tokens[$i][3] : '.default');
								if(is_file($this->arParams['doc_root_path'].'/thurly/templates/'.$this->template.'/components'.$component_path.'/'.$component_template.'/result_modifier.php')) 
									$inc_file = $this->arParams['doc_root_path'].'/thurly/templates/'.$this->template.'/components'.$component_path.'/'.$component_template.'/result_modifier.php';
								elseif(is_file($this->arParams['doc_root_path'].'/thurly/templates/'.$component_template.'/components'.$component_path.'/.default/result_modifier.php'))
									$inc_file = $this->arParams['doc_root_path'].'/thurly/templates/'.$component_template.'/components'.$component_path.'/.default/result_modifier.php';
								else
						...
						elseif($token == T_INCLUDE_COMPONENTTEMPLATE)
						{
							if(!$this->mp_mode)
							{
								...
								$component_path = array_pop(explode('thurly/components', dirname($this->current_file)));
								if(is_file($this->arParams['doc_root_path'].'/thurly/templates/'.$this->template.'/components'.$component_path.'/'.$component_template.'/'.$template_name.'.php'))
									$inc_file = $this->arParams['doc_root_path'].'/thurly/templates/'.$this->template.'/components'.$component_path.'/'.$component_template.'/'.$template_name.'.php';
								elseif(is_file($this->arParams['doc_root_path'].'/thurly/templates/.default/components'.$component_path.'/'.$component_template.'/'.$template_name.'.php'))
									$inc_file = $this->arParams['doc_root_path'].'/thurly/templates/.default/components'.$component_path.'/'.$component_template.'/'.$template_name.'.php';
								else
						...
						//not including thurly core, too hard:(
...
class CQAACheckListTests
{
	...
	static private function defineScanParams()
	{
		...
		$arResult = Array(
			...
			'PREG_FOR_SKIP_INCLUDE' => '/\/modules\/(thurly\.)?[^\W\.]+\//is',
			'PREG_FOR_SKIP_SCAN' => '/(\/thurly\/modules\/)|(\/thurly\/components\/thurly\/)|('.implode('$)|(', $SKIPDIR).'$)|('.implode('$)|(', $SKIPFILE).'$)/is',
			//	'VERBOSITY' => 1,
			'MAX_TRACE' => 30,
