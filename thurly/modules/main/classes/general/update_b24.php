...
class CB24Updater
{
	...
	const versionsFileNameConst = "/thurly/modules/versions.php";
	private $versionsFileName = "";

	const versionsDatabaseFolderConst = "/thurly/updates/thurlyos_clients/";
	...
	private static function GetDatabaseVersions()
	{
		...
		$dbResult = $DB->Query("SELECT VALUE FROM b_option WHERE MODULE_ID='main' AND NAME='THURLY24_VERSIONS' AND SITE_ID IS NULL", true);
	...
	{
		if(is_array($arDBVersions))
			self::SetOption("THURLY24_VERSIONS", serialize($arDBVersions));
	}
	...
	private function InitializeFileData()
	{
		...
		{
			include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/update_client.php");
	...
	{
		return self::GetOption("THURLY24_GENERATION_DATE_".($this->isProcessingMain ? "M" : "N"), 0);
	}
	...
	{
		self::SetOption("THURLY24_GENERATION_DATE_".($this->isProcessingMain ? "M" : "N"), $generationDate);
	}
