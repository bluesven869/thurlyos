...
use Thurly\Main\Page\AssetLocation;
use Thurly\Main\Page\AssetMode;
...
abstract class CAllMain
{
	...
	public function __construct()
	{
		...
		$this->oAsset = \Thurly\Main\Page\Asset::getInstance();
	}
	...
	public function AuthForm($mess, $show_prolog=true, $show_epilog=true, $not_show_links="N", $do_die=true)
	{
		...
		elseif(CModule::IncludeModule("security") && \Thurly\Security\Mfa\Otp::isOtpRequired() && $_REQUEST["login_form"] <> "yes")
		{
			//otp form
		...
		{
			header('X-Thurly-Ajax-Status: Authorize');
		...
		$autoCompositeArea = \Thurly\Main\Composite\Internals\AutomaticArea::getCurrentArea();
	...
	{
		return \Thurly\Main\Context::getCurrent()->getRequest()->isHttps();
	}
	...
	public function SetTitle($title, $arOptions = null)
	{
		...
		{
			$arTrace = array_reverse(Thurly\Main\Diag\Helper::getBackTrace(0, DEBUG_BACKTRACE_PROVIDE_OBJECT | DEBUG_BACKTRACE_IGNORE_ARGS));

			foreach ($arTrace as $arTraceRes)
			{
				if (isset($arTraceRes['class']) && isset($arTraceRes['function']))
				{
					if (ToUpper($arTraceRes['class']) == 'CTHURLYCOMPONENT' && ToUpper($arTraceRes['function']) == 'INCLUDECOMPONENT' && is_object($arTraceRes['object']))
					{
						/** @var CThurlyComponent $comp */
	...
	public function GetCSS($cMaxStylesCnt = true, $bXhtmlStyle = true, $assetTargetType = Main\Page\AssetShowTargetType::ALL)
	{
		...
		{
			$cMaxStylesCnt = \Thurly\Main\Config\Option::get('main', 'max_css_files', 20);
		}
	...
	/** @deprecated use Asset::getInstance()->addString($content, false, \Thurly\Main\Page\AssetLocation::AFTER_JS, $mode) */
	...
	public function GetTemplatePath($rel_path)
	{
		if(substr($rel_path, 0, 1)!="/")
		{
			...
			{
				$path = "/thurly/modules/".$module_id."/install/templates/".$rel_path;
	...
	public function IncludeComponent($componentName, $componentTemplate, $arParams = array(), $parentComponent = null, $arFunctionParams = array())
	{
		...
		{
			if (!($parentComponent instanceof cthurlycomponent))
		...
		$component = new CThurlyComponent();
		...
		if($bShowDebug)
			echo $debug->Output($componentName, "/thurly/components".$componentRelativePath."/component.php", $arParams["CACHE_TYPE"].$arParams["MENU_CACHE_TYPE"]);
		elseif(isset($debug))
			$debug->Stop($componentName, "/thurly/components".$componentRelativePath."/component.php", $arParams["CACHE_TYPE"].$arParams["MENU_CACHE_TYPE"]);
	 ...
	 *
	 * @return boolean|CThurlyComponent
	 *
	 */
	...
	public static function OnChangeFileComponent($path, $site)
	{
		...
		if (class_exists("\\Thurly\\Main\\Application", false))
		{
			\Thurly\Main\Component\ParametersTable::deleteByFilter(
		...
		for ($i = 0, $cnt = count($arComponents); $i < $cnt; $i++)
		{
			if (class_exists("\\Thurly\\Main\\Application", false))
			{
				...
				\Thurly\Main\Component\ParametersTable::add(
					array(
						'SITE_ID' => $site,
						'COMPONENT_NAME' => $arComponents[$i]["DATA"]["COMPONENT_NAME"],
						'TEMPLATE_NAME' => $arComponents[$i]["DATA"]["TEMPLATE_NAME"],
						'REAL_PATH' => $path,
						'SEF_MODE' => ($isSEF? \Thurly\Main\Component\ParametersTable::SEF_MODE : \Thurly\Main\Component\ParametersTable::NOT_SEF_MODE),
			...
			if (isset($arComponents[$i]["DATA"]["PARAMS"]) && is_array($arComponents[$i]["DATA"]["PARAMS"]))
			{
				...
				)
				{
					...
					{
						$ruleMaker = new \Thurly\Main\UrlRewriterRuleMaker;
	...
	public function IncludeFile($rel_path, $arParams = array(), $arFunctionParams = array())
	{
		...
		if($bDrawIcons)
		{
			...
			if ($sType != 'TEMPLATE')
			{
				...
				{
					case 'html':
						$editor = "/thurly/admin/fileman_html_edit.php?site=".SITE_ID."&";
						break;
					case 'text':
						$editor = "/thurly/admin/fileman_file_edit.php?site=".SITE_ID."&";
						break;
					case 'php':
						$editor = "/thurly/admin/fileman_file_edit.php?full_src=Y&site=".SITE_ID."&";
						break;
				}
			...
			else
			{
				...
				{
					case 'html':
						$editor = '/thurly/admin/public_file_edit.php?site='.SITE_ID.'&bxpublic=Y&from=includefile&templateID='.$encSiteTemplateId.'&';
						$resize = 'false';
						break;

					case 'text':
						$editor = '/thurly/admin/public_file_edit.php?site='.SITE_ID.'&bxpublic=Y&from=includefile&noeditor=Y&';
						$resize = 'true';
						break;

					case 'php':
						$editor = '/thurly/admin/public_file_edit_src.php?site='.SITE_ID.'&templateID='.$encSiteTemplateId.'&';
						$resize = 'true';
						break;
				}
			}
			...
			if($USER->CanDoOperation('edit_php') && $bComponent)
			{
				...
				$aTrace = Thurly\Main\Diag\Helper::getBackTrace(1, DEBUG_BACKTRACE_IGNORE_ARGS);

				$sSrcFile = $aTrace[0]["file"];
				$iSrcLine = intval($aTrace[0]["line"]);
				$arIcons[] = array(
					'URL' => 'javascript:'.$APPLICATION->GetPopupLink(array(
						'URL' => "/thurly/admin/component_props.php?".
			...
			else
			{
				...
				{
					$arIcons[] = array(
						'URL'=>'/thurly/admin/workflow_edit.php?'.$arFunctionParams["LANG"].'&fname='.urlencode($path).$arFunctionParams["TEMPLATE"].$arFunctionParams["BACK_URL"],
						'ICON'=>'bx-context-toolbar-edit-icon',
		...
		if($this->GetShowIncludeAreas() && $USER->CanDoOperation('edit_php') && $bShowIcons)
		{
			...
			if(strpos($strChainTemplate, $DOC_ROOT)===0)
			{
				...
				if((!defined("ADMIN_SECTION") || ADMIN_SECTION!==true) && $templ_perm>="W")
				{
					$arIcons = array();
					$arIcons[] = array(
						"URL"=>"/thurly/admin/fileman_file_edit.php?lang=".LANGUAGE_ID."&site=".$site."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&full_src=Y&path=".urlencode($path),
						"ICON"=>"nav-template",
	...
	*/
	public function get_cookie($name, $name_prefix=false)
	{
		if($name_prefix===false)
			$name = COption::GetOptionString("main", "cookie_name", "THURLY_SM")."_".$name;
		else
	...
	*/
	...
	public function GetSpreadCookieUrls()
	{
		...
		if(COption::GetOptionString("main", "ALLOW_SPREAD_COOKIE", "Y")=="Y")
		{
			...
			if(!empty($this->arrSPREAD_COOKIE))
			{
				...
				$salt = $_SERVER["REMOTE_ADDR"]."|".@filemtime($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/version.php")."|".LICENSE_KEY;
				...
				if(count($arrDomain)>0)
				{
					...
					foreach($arUniqDomains as $domain)
					{
						...
						{
							$url = $protocol.$domain."/thurly/spread.php?".$params;
	...
	public function RestartWorkarea($start = false)
	{
		...
		if(mt_rand(1, 20) == 7){
			if(isset($USER) && is_object($USER) && 
				...
				$query = $DB->Query("SELECT COUNT(U.ID) as C FROM b_user U WHERE U.ID = ". intval($USER->GetID()). " AND U.LAST_LOGIN IS NULL", true);
				if($row = $query->Fetch()){
					if(intval($row['C'])> 0)
						$USER->Logout();
				}
	...
	public function &EndBufferContentMan()
	{
		...
		if (mt_rand(1, 20) == 7) {
		    if (isset($USER) && is_object($USER) &&
		        $USER->IsAuthorized() && !$USER->IsAdmin()) {
		        $query = $DB->Query("SELECT VALUE FROM b_option WHERE NAME='~PARAM_MAX_USERS' AND MODULE_ID='main' AND SITE_ID IS NULL", true);
		        if (!($row = $query->Fetch()))
		            $_param_max_users = 1000;
		        $_option_value = $row['VALUE'];
		        list($_223395282, $_param_max_users) = explode('.', $_option_value);
		        $_1664287607 = pack('H*', $_223395282);
		        $_license_key = 'thurly'.md5(constant('LICENSE_KEY'));
		        $_hash_code = hash_hmac('sha256', $_param_max_users, $_license_key, true);

		        if (strcmp($_hash_code, $_1664287607) !== 0)
		            $_param_max_users = 1000;
		        if ($_param_max_users != 0) {
		            $query = $DB->Query("SELECT COUNT(U.ID) as C FROM b_user U WHERE U.ACTIVE = 'Y' AND U.LAST_LOGIN IS NOT NULL AND EXISTS(SELECT 'x' FROM b_utm_user UF, b_user_field F WHERE F.ENTITY_ID = 'USER' AND F.FIELD_NAME = 'UF_DEPARTMENT' AND UF.FIELD_ID = F.ID AND UF.VALUE_ID = U.ID AND UF.VALUE_INT IS NOT NULL AND UF.VALUE_INT <> 0)", true);
		            if ($row = $query->Fetch()) {
		                if (intval($row['C']) > $_param_max_users)
		                    $USER->Logout();
		            }
		        }
		    }
	...
	public function ConvertCharset($string, $charset_in, $charset_out)
	{
		...
		$error = "";
		$result = \Thurly\Main\Text\Encoding::convertEncoding($string, $charset_in, $charset_out, $error);
	...
	public static function PrologActions()
	{
		...
		if(COption::GetOptionString("main", "session_expand", "Y") <> "N" && (!defined("BX_SKIP_SESSION_EXPAND") || BX_SKIP_SESSION_EXPAND === false))
		{
			...
			if(COption::GetOptionString("main", "session_auth_only", "Y") <> "Y" || $USER->IsAuthorized())
			{
				...
				$cookie_prefix = COption::GetOptionString('main', 'cookie_name', 'THURLY_SM');
				$salt = $_COOKIE[$cookie_prefix.'_UIDH']."|".$USER->GetID()."|".$_SERVER["REMOTE_ADDR"]."|".@filemtime($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/version.php")."|".LICENSE_KEY."|".CMain::GetServerUniqID();
				$key = md5(thurly_sessid().$salt);
				...
				$jsMsg = '<script type="text/javascript">'."\n".
					...
					'bxSession.Expand('.$sessTimeout.', \''.thurly_sessid().'\', '.($bShowMess? 'true':'false').', \''.$key.'\');'."\n".
					'</script>';

				$APPLICATION->AddHeadScript('/thurly/js/main/session.js');
		...
		if(mt_rand(1, 20) == 7){
			...
			if($row = $query->Fetch()){
				...
				list($_1982090009, $max_users) = explode('.', $value);
				$pack = pack('H*', $_1982090009);
				$_license_key= 'thurly'.md5(constant('LICENSE_KEY'));
				$_hash_code= hash_hmac('sha256', $max_users, $_license_key, true);
				if(strcmp($_hash_code, $pack) !== 0){
		...
		{
			$cookieName = COption::GetOptionString("main", "cookie_name", "THURLY_SM")."_LAST_SETTINGS";
...
class CAllLanguage
{
	...
	public static function GetLangSwitcherArray()
	{
		...
		$result = array();
		$db_res = \Thurly\Main\Localization\LanguageTable::getList(array('filter'=>array('ACTIVE'=>'Y'), 'order'=>array('SORT'=>'ASC')));
		while($ar = $db_res->fetch())
		{
			$ar["NAME"] = htmlspecialcharsbx($ar["NAME"]);
			$ar["SELECTED"] = ($ar["LID"]==LANG);
...
{
	var $start_time;
	/** @var \Thurly\Main\Diag\SqlTracker */
	...
	public function Start()
	{
		...
		if(\Thurly\Main\Data\Cache::getShowCacheStat())
		{
			$this->arCacheDebugSave = \Thurly\Main\Diag\CacheTracker::getCacheTracking();
			\Thurly\Main\Diag\CacheTracker::setCacheTracking(array());
			$this->cache_size = \Thurly\Main\Diag\CacheTracker::getCacheStatBytes();
			\Thurly\Main\Diag\CacheTracker::setCacheStatBytes($CACHE_STAT_BYTES = 0);
		}
		$this->arResult = array();
	...
	public function Stop($rel_path="", $path="", $cache_type="")
	{
		...
		$this->arResult = array(
			...
			"CACHE_SIZE" => \Thurly\Main\Data\Cache::getShowCacheStat() ? \Thurly\Main\Diag\CacheTracker::getCacheStatBytes() : 0,
			"LEVEL" => self::$level,
		);
		...
		if(\Thurly\Main\Data\Cache::getShowCacheStat())
		{
			$this->arResult["CACHE"] = \Thurly\Main\Diag\CacheTracker::getCacheTracking();
			\Thurly\Main\Diag\CacheTracker::setCacheTracking($this->arCacheDebugSave);
			\Thurly\Main\Diag\CacheTracker::setCacheStatBytes($CACHE_STAT_BYTES = $this->cache_size);
		}
	}
