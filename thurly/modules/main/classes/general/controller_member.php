...
class CControllerClient
{
	...
	{
		return file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/update_db_updater.php");
	}
	...
	public static function OnExternalLogin(&$arParams)
	{
		...
		if(is_array($arUser) && strtolower($arUser['LOGIN']) == strtolower($login))
		{
			...
			if (
				$ar_mem
				&& $USER_ID
				&& class_exists("\\Thurly\\Controller\\AuthLogTable")
				&& \Thurly\Controller\AuthLogTable::isEnabled()
			)
			{
				\Thurly\Controller\AuthLogTable::logSiteToControllerAuth(
					$ar_mem["ID"],
					$USER_ID,
					true,
					'CONTROLLER_MEMBER',
	...
	public static function UpdateUser($arFields = Array(), $FORMAT_DATE = false, $FORMAT_DATETIME = false)
	{
		...
		$arFields["ACTIVE"] = "Y";
		$arFields["PASSWORD"] = \Thurly\Main\Security\Random::getString(32);
	...
	public static function InitTicket($controller_url)
	{
		...
		$member_id = "m".\Thurly\Main\Security\Random::getString(31);
		COption::SetOptionString("main", "controller_member_id", $member_id);

		$member_secret_id = "m".\Thurly\Main\Security\Random::getString(31);
		COption::SetOptionString("main", "controller_member_secret_id", $member_secret_id);

		$ticket_id = "m".\Thurly\Main\Security\Random::getString(31);
	...
	public static function UpdateCounters()
	{
		...
		else
		{
			...
			if($oResponse == false)
				throw new \Thurly\Main\SystemException("CControllerClient::UpdateCounters: unknown error");
			elseif(!$oResponse->OK())
				throw new \Thurly\Main\SystemException("CControllerClient::UpdateCounters: ".$oResponse->text);
...
class __CControllerPacket
{
	 ...
	 * By default logging will be into /thurly/controller_logs/ directory
	...
	public function getDebugLogDirectory()
	{
		if ($this->debug_file_const && defined($this->debug_file_const))
			...
			return $_SERVER['DOCUMENT_ROOT'].'/thurly/controller_logs/';
	}
	...
	public function getDebugLogFileName()
	{
		if ($this->session_id)
			...
			return "gen_".\Thurly\Main\Security\Random::getString(32).".log";
...
class __CControllerPacketRequest extends __CControllerPacket
{
	...
	public function Send($url = "", $page = "")
	{
		...
		$strRequest .= "User-Agent: ThurlyControllerMember\r\n";
		$strRequest .= "Accept: */*\r\n";
...
class CControllerClientRequestTo extends __CControllerPacketRequest
{
	...
	public function __construct($operation, $arParameters = Array())
	{
		...
		$this->arParameters = $arParameters;
		$this->session_id = \Thurly\Main\Security\Random::getString(32);
	}

	function SendWithCheck($page="/thurly/admin/controller_ws.php")
	{
	...
	public function Send($url = "", $page = "/thurly/admin/controller_ws.php")
	{
		$this->Sign();
...
class CControllerTools
{
	public static function PackFileArchive($path)
	{
		...
		if (file_exists($path))
		{
			...
			$arcname = CTempFile::GetFileName(\Thurly\Main\Security\Random::getString(32).'.tar.gz');
