...
if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/admin/update_system.php"))
{
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/admin/update_system.php");
	die();
}
...
{
	require($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/update_update5.php");
	die();
}

include(GetLangFileName($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/lang/", "/classes/general/update_update.php", $lang));
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/prolog.php");
...
if ($TYPE=="SAVE_KEY" && $MAIN_RIGHT=="W")
{
	...
	{
		if (!($fp = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/license_key.php", "w")))
		{
...
if ($TYPE=="UPDATE" && $STEP==1)
{
			...
			<table width="100%" border="0" cellspacing="1" cellpadding="3" class="edittable">
				...
				<tr>
					<td valign="top" align="right" colspan="2">
						<img src="/thurly/images/1.gif" width="1" height="8">
					</td>
				</tr>
				<tr>
		...
		|| $bLicenseNotFound))
	{
		...
		{
			?>
			<?= str_replace("#URL#", "http://".(($lang=="ru") ? "www.thurlysoft.ru/ru" : "www.thurlysoft.com" )."/bsm_register.php?dt=".UrlEncode($SiteExpireDate), GetMessage("SUP_NO_KEY_PROMT")) ?>
			<br><br>

			<a href="http://<?= (($lang=="ru") ? "www.thurlysoft.ru/ru" : "www.thurlysoft.com" ) ?>/bsm_register.php?dt=<?echo UrlEncode($SiteExpireDate) ?>" target="_blank"><img src="/thurly/images/main/icon2.gif" width="15" height="15" border="0" alt="<?= GetMessage("SUP_NO_KEY_ACT_ALT") ?>"> <?= GetMessage("SUP_NO_KEY_ACT") ?> &gt;&gt;</a>
			<br><br><br>

			<?= str_replace("#URL#", "/thurly/admin/settings.php?mid_SELECTED=yes&mid=main", GetMessage("SUP_NO_KEY_ENTER_PROMT")) ?>
			<?
		}
		else
		{
			?>
			<?= str_replace("#URL_SET#", "/thurly/admin/settings.php?mid_SELECTED=yes&mid=main", str_replace("#URL#", "http://www.thurlysoft.".(($lang=="ru") ? "ru" : "com")."/support/", GetMessage("SUP_NO_KEY_PROMT_SRC"))) ?>
			<?
		}
		?>
		...
		&& isset($arTypesUpdates["UPDATE_SYSTEM"]))
	{
		...
		<a href="sysupdate.php?TYPE=UPDUPD&STEP=1&lang=<?= htmlspecialchars($lang) ?>"><img src="/thurly/images/main/icon2.gif" width="15" height="15" border="0" alt="<?= GetMessage("SUP_UPD_UPD_ACT_ALT") ?>"> <?= GetMessage("SUP_UPD_UPD_ACT") ?> &gt;&gt;</a>
		<br><br><br>
		<?
	}
	?>
	...
	if (!$bLockUpdateSystemKernel && $newLicense == "Y")
	{
				...
				|| $arTypesUpdates["CLIENT"]["@"]["ENC_TYPE"]=="E"))
		{
			...
			<a href="sysupdate.php?TYPE=REGVER&STEP=1&lang=<?= htmlspecialchars($lang) ?>"><img src="/thurly/images/main/icon2.gif" width="15" height="15" border="0" alt="<?= GetMessage("SUP_REG_ACT_ALT") ?>"> <?= GetMessage("SUP_REG_ACT") ?> &gt;&gt;</a>
			<br><br><br>
			<?
		}
		?>
			...
			&& IntVal($arTypesUpdates["MODULES"]["@"]["COUNT"])>0)
		{
			...
			<a href="sysupdate.php?STEP=2&TYPE=UPDATE&lang=<?= htmlspecialchars($lang) ?>"><img src="/thurly/images/main/icon2.gif" width="15" height="15" border="0" alt="<?= GetMessage("SUP_MUPD_ACT_ALT") ?>"> <?= GetMessage("SUP_MUPD_ACT") ?> &gt;&gt;</a>
			<br><br><br>
			<?
		}
		else
			...
			&& ($arTypesUpdates["CLIENT"]["@"]["ENC_TYPE"]=="F"))
		{
			...
			<?if (!isset($arTypesUpdates["MODULES"])
				|| IntVal($arTypesUpdates["MODULES"]["@"]["COUNT"])<=0):?>
				<a href="sysupdate.php?TYPE=LOADSRC&STEP=1&lang=<?= htmlspecialchars($lang) ?>"><img src="/thurly/images/main/icon2.gif" width="15" height="15" border="0" alt="<?= GetMessage("SUP_SRC_ACT_ALT") ?>"> <?= GetMessage("SUP_SRC_ACT") ?> &gt;&gt;</a>
				<br><br>
			<?endif;?>
			<br>
			<?
			...
			)
		{
			...
			<a href="sysupdate.php?STEP=2&TYPE=LANGS&lang=<?= htmlspecialchars($lang) ?>"><img src="/thurly/images/main/icon2.gif" width="15" height="15" border="0" alt="<?= GetMessage("SUP_LUPD_ACT_ALT") ?>"> <?= GetMessage("SUP_LUPD_ACT") ?> &gt;&gt;</a>
			<br><br><br>
			<?
		}
		?>
			...
			)
		{
			...
			<a href="sysupdate.php?STEP=2&TYPE=HELP&lang=<?= htmlspecialchars($lang) ?>"><img src="/thurly/images/main/icon2.gif" width="15" height="15" border="0" alt="<?= GetMessage("SUP_HUPD_ACT_ALT") ?>"> <?= GetMessage("SUP_HUPD_ACT") ?> &gt;&gt;</a>
			<br><br><br>
			<?
		}
		?>
			...
			&& $arTypesUpdates["CLIENT"]["@"]["RESERVED"]!="Y")
		{
			...
			<a href="sysupdate.php?TYPE=ADD_SITE&STEP=1&lang=<?= htmlspecialchars($lang) ?>"><img src="/thurly/images/main/icon2.gif" width="15" height="15" border="0" alt="<?= GetMessage("SUP_SITES_ACT_ALT") ?>"> <?= GetMessage("SUP_CHECK_ACT") ?> &gt;&gt;</a>
			<br><br>
			<br>
			<?
		}
	...
	elseif (!$bLockUpdateSystemKernel)
	{
		...
		<form method="POST" action="sysupdate.php" name="license_form">
			...
			<?
			if (LANG == "ru")
				$license_text_file = "http://www.thurlysoft.ru/license-ru.htm";
			else
	...
	if (!$bLockUpdateSystemKernel)
	{
		...
		<a href="sysupdate.php?TYPE=TURN_STABILITY&STEP=1&lang=<?= htmlspecialchars($lang) ?>"><img src="/thurly/images/main/icon2.gif" width="15" height="15" border="0" alt="<?= (($stableVersionsOnly == "N") ? GetMessage("SUP_STABLE_TURN_ON") : GetMessage("SUP_STABLE_TURN_OFF")) ?>"> <?= (($stableVersionsOnly == "N") ? GetMessage("SUP_STABLE_TURN_ON") : GetMessage("SUP_STABLE_TURN_OFF")) ?> &gt;&gt;</a>
		<br><br>
		<br>
		<?
	}
		...
		for ($i = 0; $i < count($arModulesUpdates["MODULES"]["#"]["MODULE"]); $i++)
		{
			if (array_key_exists($arModulesUpdates["MODULES"]["#"]["MODULE"][$i]["@"]["ID"], $arErrorModules))
			{
				...
				if ($arModulesUpdates["MODULES"]["#"]["MODULE"][$i]["@"]["NEW"]=="Y")
				{
					...
					$strErrorModules .= "<a href=\"module_admin.php?lang=".LANG."&id=".urlencode($arModulesUpdates["MODULES"]["#"]["MODULE"][$i]["@"]["ID"])."&".thurly_sessid_get()."&install=".urlencode(GetMessage("SUP_STEP4_INST"))."\" target=\"_blank\" title=\"".GetMessage("SUP_STEP4_INST_ALT")."\">".GetMessage("SUP_STEP4_INST_DO")."</a>";
			...
			elseif (array_key_exists($arModulesUpdates["MODULES"]["#"]["MODULE"][$i]["@"]["ID"], $arSuccessModules))
			{
				...
				if ($arModulesUpdates["MODULES"]["#"]["MODULE"][$i]["@"]["NEW"]=="Y")
				{
					...
					$strSuccessModules .= "<a href=\"module_admin.php?lang=".LANG."&id=".urlencode($arModulesUpdates["MODULES"]["#"]["MODULE"][$i]["@"]["ID"])."&".thurly_sessid_get()."&install=".urlencode(GetMessage("SUP_STEP4_INST"))."\" target=\"_blank\" title=\"".GetMessage("SUP_STEP4_INST_ALT")."\">".GetMessage("SUP_STEP4_INST_DO")."</a>";
...
if ($TYPE=="HISTORY")
{
	...
	$iMaxNumRecs = 20;
	$arLogRecs = array();
	if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/updater.log")
		&& is_file($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/updater.log")
		&& is_readable($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/updater.log"))
	{
		$logf = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/updater.log", "r");
		while (!feof($logf))
		{
