...
class CModule
{
	...
	public static function AddAutoloadClasses($module, $arParams = array())
	{
		...
		\Thurly\Main\Loader::registerAutoLoadClasses($module, $arParams);
		return true;
	}

	public static function AutoloadClassDefined($className)
	{
		return \Thurly\Main\Loader::isAutoLoadClassRegistered($className);
	}

	public static function RequireAutoloadClass($className)
	{
		\Thurly\Main\Loader::autoLoad($className);
	}

	public static function _GetCache()
	{
		return \Thurly\Main\ModuleManager::getInstalledModules();
	}
	...
	{
		return \Thurly\Main\ModuleManager::isModuleInstalled($this->MODULE_ID);
	}
	...
	function Remove()
	{
		\Thurly\Main\ModuleManager::delete($this->MODULE_ID);
	}

	function Add()
	{
		\Thurly\Main\ModuleManager::add($this->MODULE_ID);
	}
	...
	{
		return \Thurly\Main\Loader::includeModule($module_name);
	}

	public static function IncludeModuleEx($module_name)
	{
		return \Thurly\Main\Loader::includeSharewareModule($module_name);
	}
...
{
	\Thurly\Main\ModuleManager::registerModule($id);
}

function UnRegisterModule($id)
{
	\Thurly\Main\ModuleManager::unRegisterModule($id);
}
...
{
	return \Thurly\Main\ModuleManager::isModuleInstalled($module_id);
}
