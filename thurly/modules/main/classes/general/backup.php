...
class CBackup
{
	...
	public static function ignorePath($path)
	{
		...
		if (strpos(self::$DOCUMENT_ROOT_SITE.BX_ROOT, $path) !== false) // на пути к /thurly
			return false;

		if (strpos($path, self::$DOCUMENT_ROOT_SITE.BX_ROOT) === false) // за пределами /thurly
			return !$dump_file_public;

		$path_root = substr($path, strlen(self::$DOCUMENT_ROOT_SITE));
		if (preg_match('#^/thurly/(.settings.php|php_interface|templates)/([^/]*)#',$path_root.'/',$regs))
			return !$dump_file_public;
	
		if (preg_match('#^/thurly/(activities|components|gadgets|wizards)/([^/]*)#',$path_root.'/',$regs))
		{
			if (!$regs[2])
				return false;
			if ($regs[2] == 'thurly')
		...
		// всё остальное в папке thurly - ядро
		return !$dump_file_kernel;
	}
...
class CDirRealScan extends CDirScan
{
	...
	function ProcessFile($f)
	{
		...
		{
			$f = str_replace('\\', '/', $f);
			if (preg_match('#/thurly/(php_interface/dbconn.php|.settings.php)$#', $f, $regs))
			{
