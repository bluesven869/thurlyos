...
class CSmile
{
	...
	const TYPE_SMILE = 'S';
	const TYPE_ICON = 'I';
	const PATH_TO_SMILE = "/thurly/images/main/smiles/";
	const PATH_TO_ICON = "/thurly/images/main/icons/";
...
class CSmileGallery
{
	...
	public static function getDefaultId()
	{
		...
		if ($galleryId == 0)
		{
			$gallery = CSmileGallery::getByStringId('thurly');
	...
	public static function installGallery()
	{
		...
		if ($smileGalleryId)
		{
			$smileSet = CSmileSet::getByStringId('thurly_main');
			if ($smileSet)
			{
			...
			CSmile::import(array('FILE' => $_SERVER["DOCUMENT_ROOT"].'/thurly/modules/main/install/smiles/smiles_default.zip', 'SET_ID' => $smileSetId, 'IMPORT_IF_FILE_EXISTS' => 'Y'));
		}
	}
	...
	public static function convertGallery()
	{
		...
		if (COption::GetOptionInt("main", "smile_gallery_converted", 0) == 0)
		{
			...
			if (!(($smileCount == 0 || $smileCount == 15) && $smileCount == $smileOriginalCount))
			{
				...
				$smileSet = CSmileGallery::getByStringId('thurly_convert');
				if (!$smileSet)
				{
					$smileCustomGalleryId = CSmileGallery::add(Array(
						'STRING_ID' => 'thurly_convert',
						'SORT' => 300,
						'LANG' => $arLang3,
					));
