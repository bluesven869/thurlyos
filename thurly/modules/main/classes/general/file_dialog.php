	...
	public static function ShowScript($arConfig)
	{
		...
		if (strlen($functionError) <= 0)
		{
			...
			window.<?= CUtil::JSEscape($arConfig['event'])?> = function(bLoadJS, Params)
			{
				...
				{
					if (bLoadJS !== false)
						BX.loadScript('<?=CUtil::GetAdditionalFileURL("/thurly/js/main/file_dialog.js")?>');
...
if (window.jsUtils)
{
	jsUtils.addEvent(window, 'load', function(){jsUtils.loadJSFile('<?=CUtil::GetAdditionalFileURL("/thurly/js/main/file_dialog.js")?>');}, false);
}
<?
		}
	}
			...
			<table class="bx-fd-top-contr-tbl">
				<tr>
					...
					<td nowrap style="width:170px !important; padding: 0 2px 0 2px !important;">
						<img src="/thurly/images/1.gif" class="fd_iconkit go_button" id="__bx_dir_path_go" title="<?=GetMessage("FD_GO_TO")?>"/>
						<img src="/thurly/images/1.gif" __bx_disable="Y" class="fd_iconkit path_back_dis" title="<?=GetMessage("FD_GO_BACK")?>" id="__bx_dir_path_back"/>
						<img src="/thurly/images/1.gif" __bx_disable="Y" class="fd_iconkit path_forward_dis" title="<?=GetMessage("FD_GO_FORWARD")?>" id="__bx_dir_path_forward"/>
						<img src="/thurly/images/1.gif" class="fd_iconkit dir_path_up" title="<?=GetMessage("FD_GO_UP")?>" id="__bx_dir_path_up" />
						<img src="/thurly/images/1.gif" class="fd_iconkit dir_path_root" title="<?=GetMessage("FD_GO_TO_ROOT")?>" id="__bx_dir_path_root" />
						<img src="/thurly/images/1.gif" class="fd_iconkit new_dir" title="<?=GetMessage("FD_NEW_FOLDER")?>" id="__bx_new_dir" />
						<img src="/thurly/images/1.gif" class="fd_iconkit refresh" title="<?=GetMessage("FD_REFRESH")?>" onclick="oBXDialogControls.RefreshOnclick();"/>
						<?
						$arViews = Array(
						...
						$u->Show();
						?>
						<img onclick="oBXDialogControls.ViewSelector.OnClick();" src="/thurly/images/1.gif" id="__bx_view_selector" class="fd_iconkit view_selector"  title="<?=GetMessage("FD_SELECT_VIEW")?>"/>
					</td>
					...
					<td style="width:20px !important; padding: 0 6px 0 3px !important;">
						<img src="/thurly/images/1.gif" class="fd_iconkit sort_up" title="<?=GetMessage("FD_CHANGE_SORT_ORDER")?>" __bx_value="asc" id="__bx_sort_order" />
					</td>
				</tr>
			</table>
		</div>
			...
			<tr>
				<td style="height:30px">
					<table class="fd_tab_title">
						<tr>
							<td class="icon"><img class="bxfd-add-to-menu-icon" src="/thurly/images/1.gif" width="32" height="32"/></td>
...
<div id="__bx_get_real_size_cont"></div>
	...
	{
		global $APPLICATION;
		$fd_engine_js_src = '/thurly/js/main/file_dialog_engine.js';
		$fd_css_src = '/thurly/themes/.default/file_dialog.css';
		$arSites = $Params['arSites'];
		?>
<script>
...
<form name="frmLoad" action="file_dialog.php?action=uploader&lang=<?=$lang?>&site=<?=$site?>&<?=thurly_sessid_get()?>" onsubmit="return parent.oBXDialogControls.Uploader.OnSubmit();" method="post" enctype="multipart/form-data">
