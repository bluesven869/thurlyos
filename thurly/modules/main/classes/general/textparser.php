...
class CTextParser
{
	...
	public function getAnchorSchemes()
	{
		if($this->anchorSchemes === null)
		{
			...
			if($schemes === null)
			{
				$schemes = \Thurly\Main\Config\Option::get("main", "~parser_anchor_schemes", "http|https|news|ftp|aim|mailto|file|tel|callto|skype|viber");
			}
	...
	{
		return \Thurly\Main\Text\Encoding::convertEncoding($a, 'cp1251', SITE_CHARSET);
	}
	...
	protected function convert_video($arParams)
	{
		...
		elseif (in_array($arParams["TYPE"], $trustedProviders))
		{
			...
			$uri = new \Thurly\Main\Web\Uri($arParams["PATH"]);
			if(\Thurly\Main\UrlPreview\UrlPreview::isHostTrusted($uri) || $uri->getHost() == \Thurly\Main\Application::getInstance()->getContext()->getServer()->getServerName())
			{
		...
		else
		{
			...
			$playerComponent = 'thurly:player';
			if ($this->bMobile)
			{
				$playerComponent = 'thurly:mobile.player';
			}
	...
	function convert_user($userId = 0, $userName = "")
	{
		...
		if (\Thurly\Main\Loader::includeModule('socialnetwork'))
		{
			...
			{
				$arExtranetUser = \Thurly\Socialnetwork\ComponentHelper::getExtranetUserIdList();
			}

			if ($arEmailUser === false)
			{
				$arEmailUser = \Thurly\Socialnetwork\ComponentHelper::getEmailUserIdList();
			}
		}
