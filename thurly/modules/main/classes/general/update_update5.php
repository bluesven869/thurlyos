...
//*********************************
		...
		|| $bLicenseNotFound))
	{
		...
		{
			$sMess = GetMessage("SUP_NO_KEY_PROMT", array("#URL#"=>"http://".(LANGUAGE_ID=="ru"? "www.thurlysoft.ru" : "www.thurlysoft.com" )."/bsm_register.php?dt=".UrlEncode($SiteExpireDate)));
			$sLink = '<a href="http://'.(LANGUAGE_ID=="ru"? "www.thurlysoft.ru" : "www.thurlysoft.com").'/bsm_register.php?dt='.UrlEncode($SiteExpireDate).'" target="_blank">'.GetMessage("SUP_NO_KEY_ACT").'</a>';
			echo CUpdateOutput::ShowBlock($sMess, $sLink, "icon-licence");
		}

		if(defined("DEMO") && DEMO == "Y")
			$sMess = GetMessage("SUP_NO_KEY_ENTER_PROMT", array("#URL#"=>"/thurly/admin/settings.php"));
		else
			$sMess = GetMessage("SUP_NO_KEY_PROMT_SRC", array("#URL_SET#"=>"/thurly/admin/settings.php", "#URL#"=>"http://www.thurlysoft.".(LANGUAGE_ID=="ru"? "ru" : "com")."/support/"));
		ob_start();
?>
