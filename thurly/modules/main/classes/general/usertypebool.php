...
class CUserTypeBoolean extends \Thurly\Main\UserField\TypeBase
{
	...
	function GetSettingsHTML($arUserField = false, $arHtmlControl, $bVarsFromForm)
	{
		...
		<tr>
			...
			<td><table border="0">
				<tr><td>'.GetMessage('MAIN_YES').':</td><td><input type="text" name="'.$arHtmlControl["NAME"].'[LABEL][1]" value="'.\Thurly\Main\Text\HtmlFilter::encode($value[1]).'"></td></tr>
				<tr><td>'.GetMessage('MAIN_NO').':</td><td><input type="text" name="'.$arHtmlControl["NAME"].'[LABEL][0]" value="'.\Thurly\Main\Text\HtmlFilter::encode($value[0]).'"></td></tr>
			</table></td>
		</tr>
		';
		if($bVarsFromForm)
		...
		<tr>
			...
			<td><input type="text" name="'.$arHtmlControl["NAME"].'[LABEL_CHECKBOX]" value="'.\Thurly\Main\Text\HtmlFilter::encode($value).'"></td>
		</tr>
		';
		...
		switch($arUserField["SETTINGS"]["DISPLAY"])
		{
			case "DROPDOWN":
				...
				return '
					...
					<option value="1"'.($arHtmlControl["VALUE"]? ' selected': '').'>'.\Thurly\Main\Text\HtmlFilter::encode($label[1]).'</option>
					<option value="0"'.(!$arHtmlControl["VALUE"]? ' selected': '').'>'.\Thurly\Main\Text\HtmlFilter::encode($label[0]).'</option>
					</select>
				';
			case "RADIO":
				return '
					<label><input type="radio" value="1" name="'.$arHtmlControl["NAME"].'"'.($arHtmlControl["VALUE"]? ' checked': '').($arUserField["EDIT_IN_LIST"]!="Y"? ' disabled="disabled"': '').'>'.\Thurly\Main\Text\HtmlFilter::encode($label[1]).'</label><br>
					<label><input type="radio" value="0" name="'.$arHtmlControl["NAME"].'"'.(!$arHtmlControl["VALUE"]? ' checked': '').($arUserField["EDIT_IN_LIST"]!="Y"? ' disabled="disabled"': '').'>'.\Thurly\Main\Text\HtmlFilter::encode($label[0]).'</label>
				';
			default:
				$arHtmlControl["VALIGN"] = "middle";
				...
				return '
					...
					<label><input type="checkbox" value="1" name="'.$arHtmlControl["NAME"].'"'.($arHtmlControl["VALUE"]? ' checked': '').' id="'.$arHtmlControl["NAME"].'"'.($arUserField["EDIT_IN_LIST"]!="Y"? ' disabled="disabled"': '').'>'.\Thurly\Main\Text\HtmlFilter::encode($label).'</label>
				';
		}
	}
	...
	function GetFilterHTML($arUserField, $arHtmlControl)
	{
		...
		return '
			...
			<option value="1"'.($arHtmlControl["VALUE"]? ' selected': '').'>'.\Thurly\Main\Text\HtmlFilter::encode($label[1]).'</option>
			<option value="0"'.(strlen($arHtmlControl["VALUE"])>0 && !$arHtmlControl["VALUE"]? ' selected': '').'>'.\Thurly\Main\Text\HtmlFilter::encode($label[0]).'</option>
			</select>
		';
	}
	...
	function GetAdminListViewHTML($arUserField, $arHtmlControl)
	{
		...
		if($arHtmlControl["VALUE"])
			return \Thurly\Main\Text\HtmlFilter::encode($label[1]);
		else
			return \Thurly\Main\Text\HtmlFilter::encode($label[0]);
	}
	...
	public static function GetPublicView($arUserField, $arAdditionalParameters = array())
	{
		...
		foreach($value as $res)
		{
			...
			{
				$res = '<a href="'.htmlspecialcharsbx(str_replace('#VALUE#', $res, $arUserField['PROPERTY_VALUE_LINK'])).'">'.\Thurly\Main\Text\HtmlFilter::encode($res ? $label[1] : $label[0]).'</a>';
			}
			else
			{
				$res = \Thurly\Main\Text\HtmlFilter::encode($res ? $label[1] : $label[0]);
			}
		...
		foreach($value as $res)
		{
			...
			switch($arUserField['SETTINGS']['DISPLAY'])
			{
				...
				default:
					...
					$tag .= '<label><input '.static::buildTagAttributes($attrList).'>'.\Thurly\Main\Text\HtmlFilter::encode($label).'</label>';
