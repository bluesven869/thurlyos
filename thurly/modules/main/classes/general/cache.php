...
{
	/**
	 * @var Thurly\Main\Data\Cache
	 */
	private $cache;

	public function __construct()
	{
		$this->cache = \Thurly\Main\Data\Cache::createInstance();
	}
...
class CPageCache
{
	...
	{
		$this->_cache = \Thurly\Main\Data\Cache::createCacheEngine();
	}
	...
	function Clean($uniq_str, $initdir = false, $basedir = "cache")
	{
		if(isset($this) && is_object($this) && is_object($this->_cache))
		{
			...
			if (\Thurly\Main\Data\Cache::getShowCacheStat())
				\Thurly\Main\Diag\CacheTracker::add(0, "", $basedir, $initdir, "/".$filename, "C");
	...
	function CleanDir($initdir = false, $basedir = "cache")
	{
		$basedir = BX_PERSONAL_ROOT."/".$basedir."/";
		if (\Thurly\Main\Data\Cache::getShowCacheStat())
			\Thurly\Main\Diag\CacheTracker::add(0, "", $basedir, $initdir, "", "C");
...
//		$GLOBALS["CACHE_STAT_BYTES"] += $this->_cache->read;
		if (\Thurly\Main\Data\Cache::getShowCacheStat())
		{
			$read = 0;
			$path = '';
			if ($this->_cache instanceof \Thurly\Main\Data\ICacheEngineStat)
			{
			...
			\Thurly\Main\Diag\CacheTracker::addCacheStatBytes($read);
			\Thurly\Main\Diag\CacheTracker::add($read, $path, $this->basedir, $this->initdir, $this->filename, "R");
		}
		return true;
	}
	...
	function EndDataCache()
	{
		...
		if (\Thurly\Main\Data\Cache::getShowCacheStat())
		{
			...
			\Thurly\Main\Diag\CacheTracker::addCacheStatBytes($written);
			\Thurly\Main\Diag\CacheTracker::add($written, $path, $this->basedir, $this->initdir, $this->filename, "W");
		}
...
class CCacheManager
{
	/** @var Thurly\Main\Data\ManagedCache */
	private $managedCache;

	/** @var Thurly\Main\Data\TaggedCache */
	private $taggedCache;
	...
	{
		\Thurly\Main\Data\ManagedCache::finalize();
	}
...
class CStackCacheEntry
{
	...
	{
		global $DB;
		$objCache = \Thurly\Main\Data\Cache::createInstance();
	...
	function Save()
	{
		if (defined("THURLY_SKIP_STACK_CACHE") && THURLY_SKIP_STACK_CACHE)
			return;
