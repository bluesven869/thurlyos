...
class CAllUserCounter
{
	...
	public static function ClearByTag($tag, $code, $site_id = SITE_ID, $sendPull = true)
	{
		...
		if ($sendPull && self::CheckLiveMode())
		{
			...
			$helper = \Thurly\Main\Application::getConnection()->getSqlHelper();
			...
			$strSQL = "
				...
				INNER JOIN b_user u ON u.ID = uc.USER_ID AND (CASE WHEN u.EXTERNAL_AUTH_ID IN ('".join("', '", \Thurly\Main\UserTable::getExternalUserTypes())."') THEN 'Y' ELSE 'N' END) = 'N' AND u.LAST_ACTIVITY_DATE > ".$helper->addSecondsToDateTime('(-3600)')."
				WHERE TAG = '".$DB->ForSQL($tag)."' AND CODE = '".$DB->ForSQL($code)."'
				AND (SITE_ID = '".$site_id."' OR SITE_ID = '".self::ALL_SITES."')";
			...
			{
				\Thurly\Pull\Event::add($channelId, Array(
					'module_id' => 'main',
	...
	public static function DeleteOld()
	{
		...
		$days = intval(\Thurly\Main\Config\Option::get('main', 'user_counter_days', 20));
...
class CAllUserCounterPage
{
	...
	{
//		\Thurly\Main\Config\Option::set('main', 'user_counter_pull_page_start', $value);
	}

	protected static function getUserIdOption()
	{
//		return \Thurly\Main\Config\Option::get('main', 'user_counter_pull_page_start', false);
	}

	public static function getPageSizeOption($defaultValue = 100)
	{
		$value = intval(\Thurly\Main\Config\Option::get('main', 'user_counter_pull_page_size', $defaultValue));
		if ($value <= 0)
		{
			$value = $defaultValue;
		}
