...
class CZip implements IBXArchive
{
	...
	*/
	...
	private function _addFile($filename, &$arHeader, $addDir, $removeDir, $removeAllDir, &$arParams)
	{
		...
		$arHeader['filename']          = \Thurly\Main\Text\Encoding::convertEncoding($filename, $this->fileSystemEncoding, "cp866");
		$arHeader['filename_len']      = self::$bMbstring ? mb_strlen(\Thurly\Main\Text\Encoding::convertEncoding($filename, $this->fileSystemEncoding, "cp866"), "latin1") : strlen(\Thurly\Main\Text\Encoding::convertEncoding($filename, $this->fileSystemEncoding, "cp866"));
		$arHeader['flag']              = 0;
		$arHeader['index']             = -1;
		$arHeader['internal']          = 0;
		...
		$arHeader['status']            = 'ok';
		$arHeader['stored_filename']   = \Thurly\Main\Text\Encoding::convertEncoding($storedFilename, $this->fileSystemEncoding, "cp866");
		$arHeader['version']           = 20;
	...
	private function _extractFile(&$arEntry, $path, $removePath, $removeAllPath, &$arParams)
	{
		...
		$arEntry["filename"] = \Thurly\Main\Text\Encoding::convertEncoding($arEntry["filename"], "cp866", $this->fileSystemEncoding);
		$arEntry["stored_filename"] = \Thurly\Main\Text\Encoding::convertEncoding($arEntry["stored_filename"], "cp866", $this->fileSystemEncoding);
