	...
	implements IBXVirtualIO, IBXGetErrors
{
	...
	public static function ConvertCharset($string, $direction = 1, $skipEvents = false)
	{
		...
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/charset_converter.php");
		if ($direction == self::directionEncode)
			$result = \Thurly\Main\Text\Encoding::convertEncoding($string, self::$serverEncoding, self::$systemEncoding);
		else
			$result = \Thurly\Main\Text\Encoding::convertEncoding($string, self::$systemEncoding, self::$serverEncoding);
