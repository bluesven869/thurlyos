...
class CComponentAjax
{
	...
	public function __construct($componentName, $componentTemplate, &$arParams, $parentComponent)
	{
		...
		if ($USER->IsAdmin())
		{
			if ($_GET['thurly_disable_ajax'] == 'N')
			{
				unset($_SESSION['thurly_disable_ajax']);
			}

			if ($_GET['thurly_disable_ajax'] == 'Y' || $_SESSION['thurly_disable_ajax'] == 'Y')
			{
				$_SESSION['thurly_disable_ajax'] = 'Y';
				return null;
			}
		}
	...
	function LocalRedirectHandler(&$url)
	{
		...
		{
			if (!$this->bIFrameMode)
				Header('X-Thurly-Ajax-Status: OK');
		}
		else
		{
			...
			if (!$this->bIFrameMode)
				Header('X-Thurly-Ajax-Status: Redirect');
	...
	function __PrepareLinks(&$data)
	{
		...
		for($iData = 1; $iData < $cData; $iData += 2)
		{
			...
			{
				$url = \Thurly\Main\Text\Converter::getHtmlConverter()->decode($arLinkParams[3][$url_key]);
