...
class CUserTypeManager
{
	...
	function getUtsDBColumnType($arUserField)
	{
		...
		{
			$sqlHelper = \Thurly\Main\Application::getConnection()->getSqlHelper();
	...
	*/
	...
	function EditFormShowTab($entity_id, $bVarsFromForm, $ID)
	{
		...
		{
			echo "<tr colspan=\"2\"><td align=\"left\"><a href=\"/thurly/admin/userfield_edit.php?lang=".LANG."&ENTITY_ID=".urlencode($entity_id)."&back_url=".urlencode($APPLICATION->GetCurPageParam("", array("bxpublic"))."&tabControl_active_tab=user_fields_tab")."\">".GetMessage("USER_TYPE_EDIT_TAB_HREF")."</a></td></tr>";
		}
	...
	function AdminListAddFilter($entityId, &$arFilter)
	{
		...
		foreach ($arUserFields as $fieldName => $arUserField)
		{
			...
			)
			{
				...
				if ($this->IsNotEmpty($value1) && \Thurly\Main\Type\Date::isCorrect($value1))
				{
					$date = new \Thurly\Main\Type\Date($value1);
					$arFilter['>='.$fieldName] = $date;
				}
				if ($this->IsNotEmpty($value2) && \Thurly\Main\Type\Date::isCorrect($value2))
				{
					$date = new \Thurly\Main\Type\Date($value2);
	...
	{
		$filterOption = new Thurly\Main\UI\Filter\Options($filterId);
	...
	function ShowScript()
	{
		...
		$APPLICATION->AddHeadScript("/thurly/js/main/usertype.js");
	...
	function GetEditFormHTML($bVarsFromForm, $form_value, $arUserField)
	{
		...
		if($arUserField["USER_TYPE"])
		{
			if($this->GetRights($arUserField["ENTITY_ID"]) >= "W")
				$edit_link = ($arUserField["HELP_MESSAGE"]? htmlspecialcharsex($arUserField["HELP_MESSAGE"]).'<br>': '').'<a href="'.htmlspecialcharsbx('/thurly/admin/userfield_edit.php?lang='.LANG.'&ID='.$arUserField["ID"].'&back_url='.urlencode($APPLICATION->GetCurPageParam("", array("bxpublic")).'&tabControl_active_tab=user_fields_tab')).'">'.htmlspecialcharsex(GetMessage("MAIN_EDIT")).'</a>';
			else
				$edit_link = '';
	...
	{
		$event = new \Thurly\Main\Event("main", "onBeforeGetPublicView", array(&$arUserField, &$arAdditionalParameters));
		$event->send();

		$arType = $this->GetUserType($arUserField["USER_TYPE_ID"]);

		$html = null;
		$event = new \Thurly\Main\Event("main", "onGetPublicView", array($arUserField, $arAdditionalParameters));
		$event->send();
		foreach ($event->getResults() as $evenResult)
		{
			if ($evenResult->getType() == \Thurly\Main\EventResult::SUCCESS)
			{
		...
		$event = new \Thurly\Main\Event("main", "onAfterGetPublicView", array($arUserField, $arAdditionalParameters, &$html));
		$event->send();
	...
	{
		$event = new \Thurly\Main\Event("main", "onBeforeGetPublicEdit", array(&$arUserField, &$arAdditionalParameters));
		$event->send();

		$arType = $this->GetUserType($arUserField["USER_TYPE_ID"]);

		$html = null;
		$event = new \Thurly\Main\Event("main", "onGetPublicEdit", array($arUserField, $arAdditionalParameters));
		$event->send();
		foreach ($event->getResults() as $evenResult)
		{
			if ($evenResult->getType() == \Thurly\Main\EventResult::SUCCESS)
		...
		$event = new \Thurly\Main\Event("main", "onAfterGetPublicEdit", array($arUserField, $arAdditionalParameters, &$html));
		$event->send();
	...
	function Update($entity_id, $ID, $arFields, $user_id = false)
	{
		...
		foreach($arUserFields as $FIELD_NAME=>$arUserField)
		{
			if(array_key_exists($FIELD_NAME, $arFields))
			{
				...
				elseif(is_array($arFields[$FIELD_NAME]))
				{
					...
					{
						$serialized = \Thurly\Main\UserFieldTable::serializeMultipleDatetime($arInsert[$arUserField["ID"]]);
					}
					elseif ($arUserField['USER_TYPE_ID'] == 'date')
					{
						$serialized = \Thurly\Main\UserFieldTable::serializeMultipleDate($arInsert[$arUserField["ID"]]);
					}
					else
					{
		...
		foreach($arInsert as $FieldId=>$arField)
		{
			...
			{
				if ($value instanceof \Thurly\Main\Type\Date)
				{
					// little hack to avoid timezone vs 00:00:00 ambiguity. for utm only
					$value = new \Thurly\Main\Type\DateTime($value->format('Y-m-d H:i:s'), 'Y-m-d H:i:s');
				}
	...
	public function getEntityField($arUserField, $fieldName = null, $fieldParameters = array())
	{
		...
		switch ($arUserField['USER_TYPE']['BASE_TYPE'])
		{
			...
			default:
				throw new \Thurly\Main\ArgumentException(sprintf(
...
class CAllSQLWhere
{
	...
	*/
	...
	public static function splitWords($string)
	{
		...
		if($encoding === null)
		{
			$encoding = \Thurly\Main\Context::getCurrent()->getCulture()->getCharset();
		}
	...
	public static function GetMinTokenSize()
	{
		...
		{
			$config = \Thurly\Main\Application::getConnection()->getConfiguration();
	...
	function addDateFilter(&$result, $isMultiple, $FIELD_NAME, $operation, $value, $format)
	{
		...
		if (is_array($value))
		{
			...
			foreach ($value as $val)
			{
				if ($val instanceof \Thurly\Main\Type\Date)
				{
		...
		elseif ($value instanceof \Thurly\Main\Type\Date)
		{
 ...
 * @deprecated  use \Thurly\Main\DB\SqlExpression instead
 * @see \Thurly\Main\DB\SqlExpression
 */
class CSQLWhereExpression
{
	protected
