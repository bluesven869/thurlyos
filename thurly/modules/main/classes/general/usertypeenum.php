...
class CUserTypeEnum extends \Thurly\Main\UserField\TypeBase
{
	...
	function GetEditFormHTML($arUserField, $arHtmlControl)
	{
		...
		if($arUserField["SETTINGS"]["DISPLAY"]=="UI")
		{
			...
			$items = \Thurly\Main\Web\Json::encode($itemList);
			$value = \Thurly\Main\Web\Json::encode($startValue);
			...
			$result .= '<input type="hidden" name="'.$arHtmlControl["NAME"].'" value="'.\Thurly\Main\Text\HtmlFilter::encode($startValue['VALUE']).'" id="'.$valueContainerId.'">';
...
EOT;
	...
	function GetEditFormHTMLMulty($arUserField, $arHtmlControl)
	{
		...
		if($arUserField["SETTINGS"]["DISPLAY"] == "UI")
		{
			...
			$result .= '<span id="'.\Thurly\Main\Text\HtmlFilter::encode($valueContainerId).'" style="display: none">';

			for($i = 0, $n = count($startValue); $i < $n; $i++)
			{
				$result .= '<input type="hidden" name="'.$arHtmlControl["NAME"].'" value="'.\Thurly\Main\Text\HtmlFilter::encode($startValue[$i]['VALUE']).'" />';
			}
...
EOT;
	...
	public function getPublicEdit($arUserField, $arAdditionalParameters = array())
	{
		...
		if($arUserField["SETTINGS"]["DISPLAY"] == "UI")
		{
			...
			$items = \Thurly\Main\Web\Json::encode($itemList);
			$currentValue = \Thurly\Main\Web\Json::encode($startValue);
