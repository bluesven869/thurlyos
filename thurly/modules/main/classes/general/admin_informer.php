...
<script type="text/javascript">
	...
	public static function InsertMainItems()
	{
		...
		if ($USER->CanDoOperation("cache_control") && !\Thurly\Main\Composite\Helper::isOn())
		{
			self::AddItem(array(
				"TITLE" => GetMessage("top_panel_ai_composite_title"),
				"HTML" => GetMessage("top_panel_ai_composite_desc"),
				"COLOR" => "red",
				"FOOTER" => '<a href="/thurly/admin/composite.php?lang='.LANGUAGE_ID.'">'.GetMessage("top_panel_ai_composite_switch_on").'</a>',
				"ALERT" => true,
				"SORT" => 1
			));
		}
		...
		if($USER->IsAdmin() || $USER->CanDoOperation('install_updates'))
		{
			...
			if ($update_res['result'] === true && $update_res['tooltip'])
			{
				...
				$updAIParams["FOOTER"] = '<a href="/thurly/admin/update_system.php?lang='.LANGUAGE_ID.'">'.GetMessage("top_panel_ai_upd_instl").'</a>';
			...
			else if ($update_res['error'] <> '') // update error
			{
				...
				$updAIParams["FOOTER"] = '<a href="/thurly/admin/update_system.php?refresh=Y&lang='.LANGUAGE_ID.'">'.GetMessage("top_panel_ai_upd_chk").'</a>';
				$updAIParams["ALERT"] = true;
			}
						...
						if($module["TMS"]+3600*24*14 < time())
						{
							...
							function hideMpAnswer(el, module)
							{
								if(el.parentNode.parentNode.parentNode)
									...
									BX.ajax({
										'method': 'POST',
										'dataType': 'html',
										'url': '/thurly/admin/partner_modules.php',
										'data': 'module='+module+'&".thurly_sessid_get()."&act=unnotify',
										'async': true,
										'processData': false
							...
							$arParams = array(
								...
								'FOOTER' => "<a href=\"javascript:void(0)\" onclick=\"hideMpAnswer(this, '".CUtil::JSEscape($module["ID"])."')\" ".
									...
									"<a href=\"http://marketplace.1c-thurly.".LANGUAGE_ID."/solutions/".$module["ID"]."/#tab-rating-link\" target=\"_blank\" ".
