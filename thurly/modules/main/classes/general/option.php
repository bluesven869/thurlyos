...
class CAllOption
{
	...
	public static function GetOptionString($module_id, $name, $def="", $site=false, $bExactSite=false)
	{
		...
		try
		{
			if ($bExactSite)
			{
				$v = \Thurly\Main\Config\Option::getRealValue($module_id, $name, $site);
				return $v === null ? false : $v;
			}

			$v = \Thurly\Main\Config\Option::get($module_id, $name, $def, $site);
		}
		catch (\Thurly\Main\ArgumentNullException $e)
		{
	...
	{
		\Thurly\Main\Config\Option::set($module_id, $name, $value, $site);
		return true;
	}
	...
	public static function RemoveOption($module_id, $name="", $site=false)
	{
		...
		if (strlen($site) > 0)
			$filter["site_id"] = $site;
		\Thurly\Main\Config\Option::delete($module_id, $filter);
	}
