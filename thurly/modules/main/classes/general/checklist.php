...
class CCheckList
{
	function __construct($ID = false)
	{
		...
		$this->report_info = "";
		$this->checklist_path = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/checklist_structure.php";
	...
	{//getting description of sections and points
		$file = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/lang/".LANG."/admin/checklist/".$ID.".html";
		$arHowTo = "";
		if(file_exists($file))
		{
			if ($f = @fopen($file, "r"))
...
class CCheckListResult
{
	...
	public static function GetList($arOrder = array(), $arFilter = array())
	{
		...
		{
			$arSqlWhere = array();
			$arSqlFields = array("ID", "REPORT", "HIDDEN", "SENDED_TO_THURLY");
...
class CAutoCheck
{
	...
	{
		$arFolders = array(
			$_SERVER['DOCUMENT_ROOT']."/thurly/templates",
	...
	public static function CheckKernel($arParams)
	{
		...
		global $DB;
		$arCompare = array(
			"install/components/thurly/" => "/thurly/components/thurly/",
			"install/js/" => "/thurly/js/",
			"install/activities/" => "/thurly/activities/",
			"install/admin/" => "/thurly/admin/",
			"install/wizards/" => "/thurly/wizards/",
		);
		...
		$module_folder = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$module_id."/";
		...
		$ver = \Thurly\Main\ModuleManager::getVersion($module_id);
		...
		else
		{
			if(count($NS["FILE_LIST"]) == 0)
			{
				...
				$data = $http->get("http://".$sHost."/thurly/updates/checksum.php?check_sum=Y&module_id=".$module_id."&ver=".$ver."&dbtype=".$dbtype."&mode=2");
	...
	public static function CheckQueryString($arParams = array())
	{
		...
		$arPath = array(
			...
			$_SERVER["DOCUMENT_ROOT"]."/thurly/templates/",
			$_SERVER["DOCUMENT_ROOT"]."/thurly/php_interface/",
			$_SERVER["DOCUMENT_ROOT"]."/thurly/components/"
		);
		$arExept = array(
			"FOLDERS" => array("images", "thurly", "upload", ".svn"),
			"EXT" => array("php"),
			"FILES" => array(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/php_interface/dbconn.php",
				"after_connect.php"
			)
		);
	...
	public static function CheckVMThurly(){
		$arResult = array();
		$arResult["STATUS"] = false;

		$http = new \Thurly\Main\Web\HttpClient();
		$ver = $http->get("http://www.1c-thurly.ru/download/vm_thurly.ver");

		if (version_compare(getenv('THURLY_VA_VER'), $ver) >= 0)
		{
			$arResult["STATUS"] = true;
			$arResult["MESSAGE"] = array(
				'PREVIEW' => GetMessage("CL_VMTHURLY_ACTUAL"),
			);
		}
		else
		{
			$arResult["MESSAGE"] = array(
				'PREVIEW' => GetMessage("CL_VMTHURLY_NOT_ACTUAL"),
			);
		}
