<?php
namespace Thurly\Main\Composite;

use Thurly\Main\Composite\Debug;
use Thurly\Main\Composite\Debug\Logger;
use Thurly\Main\Composite\Internals\Model\PageTable;
use Thurly\Main\Composite\Internals\Locker;
use Thurly\Main\Composite\Internals\PageManager;
...
use Thurly\Main\Text\BinaryString;
...
final class Engine
{
	...
	private static function processPageContent($content)
	{
		...
		if (self::isAjaxRequest())
		{
			...
			header("X-Thurly-Composite: Ajax ".($htmlCacheChanged ? "(changed)" : "(stable)"));
	...
	/**
	 * Replaces thurly sessid in the $content
	 *
	 * @param string $content
	 */
	private static function replaceSessid(&$content)
	{
		$methodInvocations = thurly_sessid_post("sessid", true);
		if ($methodInvocations > 0)
		{
			$content = str_replace("value=\"".thurly_sessid()."\"", "value=\"\"", $content);
		}
	}
	...
	public static function onBeforeLocalRedirect(&$url, $skip_security_check, $isExternal)
	{
		...
		header("X-Thurly-Composite: Ajax (error:redirect)");
		self::sendRandHeader();
	...
	private static function getAjaxError($errorMsg = null)
	{
		...
		header("X-Thurly-Composite: Ajax (error:".$error.")");
		self::sendRandHeader();
...
JS;
	...
	public static function getInjectedCSS()
	{
		...
		return <<<CSS
			...
			.bx-composite-btn {
				background: url(/thurly/images/main/composite/sprite-1x.png) no-repeat right 0 #e94524;
				border-radius: 15px;
				color: #fff !important;
			...
			@media screen 
  				...
  				and (min-resolution: 192dpi) {
					.bx-composite-btn {
						background-image: url(/thurly/images/main/composite/sprite-2x.png);
			...
			.bx-composite-loading {
				...
				width: 40px;
				height: 40px;
				background: url(/thurly/images/main/composite/loading.gif);
			}
CSS;
	}
	...
	public static function onEpilog()
	{
		...
		if (\Thurly\Main\Data\Cache::shouldClearCache())
		{
	 ...
	 *
	 * @deprecated
	 * @see \Thurly\Main\Composite\StaticArea::getCurrentDynamicId
	 * @return string|false
	 */
...
if (!class_exists("Thurly\\Main\\Page\\Frame", false))
{
	class_alias("Thurly\\Main\\Composite\\Engine", "Thurly\\Main\\Page\\Frame");
}
