...
class AppCache
{
	...
	const MANIFEST_CHECK_FILE = "/thurly/tools/check_appcache.php";
	...
	{
		//use \Thurly\Main\Composite\AppCache::getInstance();
	}
		...
		if (array_key_exists("css", $arFilesByType))
		{
			...
			for ($j = 0; $j < $cssCount; $j++)
			{
				...
				)
				{
					...
					$file = new  \Thurly\Main\IO\File(Application::getDocumentRoot() . $fileUrl['path']);
					...
					{
						$cssFileRelative = new \Thurly\Main\IO\File($cssFilePath);
...
class_alias("Thurly\\Main\\Composite\\AppCache", "Thurly\\Main\\Data\\AppCacheManifest");