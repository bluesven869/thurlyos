...
class Asset
{
	...
	private function __construct()
	{
		...
		$this->dbType = ToUpper(\Thurly\Main\Application::getInstance()->getConnection()->getType());
	}
	...
	public static function fixCssIncludes($content, $path)
	{
		...
		$content = preg_replace_callback(
			...
			create_function('$matches', 'return $matches[1].Thurly\Main\Page\Asset::replaceUrlCSS($matches[3], $matches[2], "'.addslashes($path).'").")";'),
			$content
		);
	...
	private function prepareCss()
	{
		...
		foreach($this->css as $css => $set)
		{
			...
			else
			{
				...
				elseif (strncmp($cssInfo['PATH'], '/thurly/js/', 11) != 0 /*||*/ )
				{
					$cssInfo['SKIP'] = !(
						strncmp($cssInfo['PATH'], '/thurly/panel/', 14) != 0
						&& strncmp($cssInfo['PATH'], '/thurly/themes/', 15) != 0
						&& strncmp($cssInfo['PATH'], '/thurly/modules/', 16) != 0
					);
				}
			}
	...
	private function prepareJs()
	{
		...
		foreach($this->js as $js => $set)
		{
			...
			else
			{
				...
				elseif (strncmp($jsInfo['PATH'], '/thurly/js/', 11) != 0)
				{
					$jsInfo['SKIP'] = !(
						strncmp($jsInfo['PATH'], '/thurly/panel/', 14) != 0
						&& strncmp($jsInfo['PATH'], '/thurly/themes/', 15) != 0
						&& strncmp($jsInfo['PATH'], '/thurly/modules/', 16) != 0
					);
				}
			}
	...
	function isKernelCSS($css)
	{
		...
		if (!($this->sliceKernel() && $this->optimizeCss()))
		{
			return (strncmp($css, '/thurly/css/', 12) == 0);
		}
		...
		elseif (strncmp($css, '/thurly/css/', 12) == 0)
		{
			$tmp = explode('/', $css);
			$moduleID = $tmp['3'];
			unset($tmp);
	...
	function isKernelJS($js)
	{
		...
		if (!($this->sliceKernel() && $this->optimizeJs()))
		{
			return (strncmp($js, '/thurly/js/', 11) == 0);
		}
		...
		elseif (strncmp($js, '/thurly/js/', 11) == 0)
		{
			$tmp = explode('/', $js);
			$moduleID = $tmp['3'];
			unset($tmp);
