...
class Mail
{
	...
	public static function send($mailParams)
	{
		...
		$event = new \Thurly\Main\Event("main", "OnBeforeMailSend", array($mailParams));
		$event->send();
	...
	public function initSettings()
	{
		...
		$this->thurlyDirectory = \Thurly\Main\Application::getInstance()->getPersonalRoot();
	}
	...
	protected function getReplacedImageSrc($matches)
	{
		...
		else if(substr($srcTrimmed,0, 1) == "/")
		{
			...
			{
				$io = \CBXVirtualIo::GetInstance();
				$filePath = $io->GetPhysicalName(\Thurly\Main\Application::getDocumentRoot().$srcTrimmed);
				foreach($this->attachment as $attach)
				{
					if($filePath == $attach['PATH'])
					{
