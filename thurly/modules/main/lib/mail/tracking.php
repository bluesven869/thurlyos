...
use Thurly\Main\Context as MainContext;
use Thurly\Main\EventResult;
use Thurly\Main\Security\Sign\BadSignatureException;
use Thurly\Main\Security\Sign\Signer;
use Thurly\Main\SystemException;

/**
 * Class Tracking
 * @package Thurly\Main\Mail
 */
class Tracking
{
	 ...
	 * @return array
	 * @throws \Thurly\Main\Security\Sign\BadSignatureException
	 */
	...
	public static function getSubscriptionList($data)
	{
		...
		$event = new \Thurly\Main\Event("main", "OnMailEventSubscriptionList", array($data['FIELDS']), $filter);
		$event->send();
	...
	public static function subscribe($data)
	{
		...
		$event = new \Thurly\Main\Event("main", "OnMailEventSubscriptionEnable", array($data['FIELDS']), array($data['MODULE_ID']));
		$event->send();
		foreach ($event->getResults() as $eventResult)
		{
			if ($eventResult->getType() == EventResult::ERROR)
	...
	public static function unsubscribe($data)
	{
		...
		$event = new \Thurly\Main\Event("main", "OnMailEventSubscriptionDisable", array($data['FIELDS']), array($data['MODULE_ID']));
		$event->send();
		foreach ($event->getResults() as $eventResult)
		{
			if ($eventResult->getType() == EventResult::ERROR)
	...
	public static function click(array $data)
	{
		...
		$event = new \Thurly\Main\Event("main", "OnMailEventMailClick", array($data['FIELDS']), $filter);
		$event->send();
		foreach ($event->getResults() as $eventResult)
		{
			if ($eventResult->getType() == EventResult::ERROR)
	...
	public static function read(array $data)
	{
		...
		$event = new \Thurly\Main\Event("main", "OnMailEventMailRead", array($data['FIELDS']), $filter);
		$event->send();
		foreach ($event->getResults() as $eventResult)
		{
			if ($eventResult->getType() == EventResult::ERROR)
