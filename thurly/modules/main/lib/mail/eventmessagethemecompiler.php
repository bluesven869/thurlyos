...
use Thurly\Main\Mail\Internal as MailInternal;
use Thurly\Main\Config as Config;
use Thurly\Main\IO as IO;
use Thurly\Main\ObjectNotFoundException as ObjectNotFoundException;
...
class EventMessageThemeCompiler
{
	...
	public function getStylesString()
	{
		...
		{
			$pathFull = \Thurly\Main\Application::getDocumentRoot().$path;
	...
	public function showStyles()
	{
		...
		{
			$identificator = '%THURLY_MAIL_EVENT_TEMPLATE_THEME_CALLBACK_STYLE%';
	...
	protected function setTheme($site_template_id)
	{
		if(strlen($site_template_id)>0)
		{
			...
			if($templateFields = $result->Fetch())
			{
				...
				$template_path_header = \Thurly\Main\Application::getDocumentRoot().$templateFields['PATH'].'/header.php';
				$template_path_footer = \Thurly\Main\Application::getDocumentRoot().$templateFields['PATH'].'/footer.php';
	...
	protected function executePhp($template, $resourceOrder = 100)
	{
		...
		try
		{
			...
			$result = eval('use \Thurly\Main\Mail\EventMessageThemeCompiler; ob_start();?>' . $template . '<? return ob_get_clean();');
		}
	 ...
	 *
	 * @return void
	 * @throws \Thurly\Main\Mail\StopException
	 */
