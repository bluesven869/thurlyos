...
class FrameCache
{
	...
	public static function setEnable($isEnable = true)
	{
		if ($isEnable && !self::$isEnable)
		{
			...
			$actionType = \Thurly\Main\Context::getCurrent()->getServer()->get("HTTP_BX_ACTION_TYPE");
	...
	public static function onBeforeEndBufferContent()
	{
		...
		$manifest = \Thurly\Main\Data\AppCacheManifest::getInstance();
	...
	public static function onEndBufferContent(&$content)
	{
		...
		if (self::getUseAppCache() == true) //Do we use html5 application cache?
			\Thurly\Main\Data\AppCacheManifest::onEndBufferContent($content);
		else
			\Thurly\Main\Data\AppCacheManifest::checkObsoleteManifest();//it checks if the manifest is still alive.
		...
		if (self::$isBackgroundRequest) //Is it a check request?
		{
			...
			$content = Array(
				"isManifestUpdated" => \Thurly\Main\Data\AppCacheManifest::getInstance()->getIsModified(),
