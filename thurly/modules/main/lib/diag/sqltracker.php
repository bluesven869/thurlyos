...
class SqlTracker implements \Iterator
{
	 ...
	 *
	 * @return \Thurly\Main\Diag\SqlTrackerQuery
	 */
	 ...
	 *
	 * @return void
	 * @see \Thurly\Main\Diag\SqlTracker->stopFileLog
	 * @see \Thurly\Main\Diag\SqlTracker->writeFileLog
	 */
	 ...
	 *
	 * @return void
	 * @see \Thurly\Main\Diag\SqlTracker->startFileLog
	 * @see \Thurly\Main\Diag\SqlTracker->stopFileLog
	 */
	public function writeFileLog($sql, $executionTime = 0.0, $additional = "", $traceSkip = 2)
	{
		if ($this->logFilePath)
		{
			...
			$trace = $this->formatTrace(\Thurly\Main\Diag\Helper::getBackTrace($this->depthBackTrace, null, $traceSkip));
			...
			$message =
				...
				"\n".$footer.
				"\n";
			\Thurly\Main\IO\File::putFileContents($this->logFilePath, $message, \Thurly\Main\IO\File::APPEND);
		}
	}
