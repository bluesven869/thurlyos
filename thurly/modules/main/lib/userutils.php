...
class UserUtils
{
	...
	public static function getUserSearchFilter(array $fields)
	{
		...
		if (UserTable::getEntity()->fullTextIndexEnabled('SEARCH_USER_CONTENT'))
		{
			...
			if (array_key_exists('FIND', $fields))
			{
				...
				if (\Thurly\Main\Search\Content::isIntegerToken($find))
				{
					$find = \Thurly\Main\Search\Content::prepareIntegerToken($find);
				}
				else
				{
					$find = \Thurly\Main\Search\Content::prepareStringToken($find);
				}
			}
			else
			{
				...
				foreach ($fields as $key => $value)
				{
					...
					{
						if (\Thurly\Main\Search\Content::isIntegerToken($value))
						{
							$find .= ' '.\Thurly\Main\Search\Content::prepareIntegerToken($value);
						}
						else
						{
							$find .= ' '.\Thurly\Main\Search\Content::prepareStringToken($value);
						}
						$find = trim($find);
					}
				}
				...
				if (array_key_exists('UF_DEPARTMENT_NAME', $fields))
				{
					...
					if (\Thurly\Main\Search\Content::isIntegerToken($fields['UF_DEPARTMENT_NAME']))
					{
						$find .= ' '.\Thurly\Main\Search\Content::prepareIntegerToken($fields['UF_DEPARTMENT_NAME']);
					}
					else
					{
						$find .= ' '.\Thurly\Main\Search\Content::prepareStringToken($fields['UF_DEPARTMENT_NAME']);
					}
					$find = trim($find);
				}
			}

			if (\Thurly\Main\Search\Content::canUseFulltextSearch($find, \Thurly\Main\Search\Content::TYPE_MIXED))
			{
	...
	public static function getDepartmentNames($departmentIds)
	{
		...
		else if ($iblockStructureId <= 0 || !\Thurly\Main\Loader::includeModule('iblock'))
		{
			return $result;
		}
		else
		{
			$orm = \Thurly\Iblock\SectionTable::getList(Array('select' => Array('ID', 'NAME', 'DEPTH_LEVEL', 'IBLOCK_SECTION_ID'), 'filter' => Array('=IBLOCK_ID' => $iblockStructureId, '=ACTIVE' => 'Y',),));
