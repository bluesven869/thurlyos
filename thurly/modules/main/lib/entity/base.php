...
class Base
{
	...
	{
		/** @var $className \Thurly\Main\Entity\DataManager */
	...
	public function postInitialize()
	{
		...
		if (is_null($this->dbTableName))
		{
			...
			{
				if ($i == 0 && $_pathElem == 'Thurly')
				{
					// skip thurly namespace
					continue;
				}
	...
	protected function appendField(Field $field)
	{
		...
		if ($field instanceof UField && $field->getTypeId() == 'iblock_section')
		{
			...
			$newFieldInfo = array(
				'data_type' => 'Thurly\Iblock\Section',
	...
	public static function compileEntity($entityName, $fields = null, $parameters = array())
	{
		...
		// build entity code
		$classCode = $classCode."class {$entityName} extends \\Thurly\\Main\\Entity\\DataManager "."{";
		$classCodeEnd = '}'.$classCodeEnd;
