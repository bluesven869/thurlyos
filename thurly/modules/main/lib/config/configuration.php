	...
	implements \ArrayAccess, \Iterator, \Countable
{
	...
	const CONFIGURATION_FILE_PATH = "/thurly/.settings.php";
	const CONFIGURATION_FILE_PATH_EXTRA = "/thurly/.settings_extra.php";
	...
	public static function wnc()
	{
		...
		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/cluster/memcache.php"))
		{
			$arList = null;
			include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/cluster/memcache.php");
		...
		if ($DBType == 'mysql')
			$dbClassName = defined('BX_USE_MYSQLI') && BX_USE_MYSQLI === true ? "\\Thurly\\Main\\DB\\MysqliConnection" : "\\Thurly\\Main\\DB\\MysqlConnection";
		elseif ($DBType == 'mssql')
		...
		$filename1 = $_SERVER["DOCUMENT_ROOT"]."/thurly/php_interface/after_connect.php";
		$filename2 = $_SERVER["DOCUMENT_ROOT"]."/thurly/php_interface/after_connect_d7.php";
		if (file_exists($filename1) && !file_exists($filename2))
		{
			...
			if (strtolower(substr($source, 0, 5)) == '<?php')
				$pos = 5;
			$source = substr($source, 0, $pos)."\n".'$connection = \Thurly\Main\Application::getConnection();'.substr($source, $pos);
