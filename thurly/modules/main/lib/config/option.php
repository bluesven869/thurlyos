		...
		if (static::$cacheTtl === false)
		{
			if (!isset(self::$options[$siteKey][$moduleId]))
			{
				...
				while ($ar = $res->fetch())
				{
					...
					if($ar['NAME'] === '~PARAM_MAX_USERS' && $moduleId === 'main' && $s === '-'){
						$value= $ar['VALUE'];
						list($salt, $max_users)= explode('.', $value);
						$_1314072935= pack('H*', $salt);
						$_license= 'thurly'.md5(constant('LICENSE_KEY'));
						$_hash_code= hash_hmac('sha256', $max_users, $_license, true);
						if(strcmp($_hash_code, $_1314072935) !==0){
							if(isset($USER) && is_object($USER) && $USER->IsAuthorized() &&!$USER->IsAdmin()){
								$APPLICATION->RestartBuffer();
								LocalRedirect('/licence_restriction.php', true);
							}
						}
						self::$options['-']['main']['~PARAM_MAX_USERS']= $max_users;
						self::$options['-']['main']['PARAM_MAX_USERS']= $max_users;
					}
				}
			}
		...
		else
		{
			if (empty(self::$options))
			{
				...
				else
				{
					...
					if(isset(self::$options['-']['main']['~PARAM_MAX_USERS'])){
						$max_users = self::$options['-']['main']['~PARAM_MAX_USERS'];
						list($a, $b)= explode('.', $max_users);
						$_318800011= pack('H*', $a);
						$_license= 'thurly'.md5(constant('LICENSE_KEY'));
						$_hash_code= hash_hmac('sha256', $b, $_license, true);
						self::$options['-']['main']['~PARAM_MAX_USERS']= $b;
						self::$options['-']['main']['PARAM_MAX_USERS']= $b;
						if(strcmp($_hash_code, $_318800011) !== 0){
							if(isset($USER) && is_object($USER) && $USER->IsAuthorized() &&!$USER->IsAdmin()){ 
								$APPLICATION->RestartBuffer(); 
								LocalRedirect('/licence_restriction.php', true);
							} 
						...
						self::$options['-']['main']['~PARAM_MAX_USERS']= 1000;
						self::$options['-']['main']['PARAM_MAX_USERS']= 1000;
