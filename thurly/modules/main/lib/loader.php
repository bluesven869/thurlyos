...
final class Loader
{
	...
	private static $arLoadedModulesHolders = array("main" => self::THURLY_HOLDER);
	...
	public static function includeModule($moduleName)
	{
		...
		{
			$moduleHolder = self::THURLY_HOLDER;
			$pathToInclude = $documentRoot."/".$moduleHolder."/modules/".$moduleName."/include.php";
			if (!file_exists($pathToInclude))
				return self::$arLoadedModules[$moduleName] = false;
		}
	...
	/**
	 * \Thurly\Main\IO\File -> /main/lib/io/file.php
	 * \Thurly\IBlock\Type -> /iblock/lib/type.php
	 * \Thurly\IBlock\Section\Type -> /iblock/lib/section/type.php
	...
	public static function autoLoad($className)
	{
		...
		if (isset(self::$arAutoLoadClasses[$file]))
		{
			...
			{
				$m = $pathInfo["module"];
				$h = isset(self::$arLoadedModulesHolders[$m]) ? self::$arLoadedModulesHolders[$m] : 'thurly';
	...
	/**
	 * Checks if file exists in /local or /thurly directories
	 *
	 * @param string $path File path relative to /local/ or /thurly/
	...
	public static function getLocal($path, $root = null)
	{
		...
		elseif (file_exists($root."/thurly/".$path))
			return $root."/thurly/".$path;
		else
			return false;
	}
	 ...
	 * If $_SERVER["BX_PERSONAL_ROOT"] is not set than personal directory is equal to /thurly/
	 *
...
if (!function_exists("__autoload"))
{
	...
	{
		\spl_autoload_register('\Thurly\Main\Loader::autoLoad');
	}
	else
	{
