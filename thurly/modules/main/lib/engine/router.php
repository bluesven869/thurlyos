...
namespace Thurly\Main\Engine;


use Thurly\Main\Component\ParameterSigner;
use Thurly\Main\Config\Configuration;
use Thurly\Main\Engine\Component\ComponentController;
use Thurly\Main\Engine\Contract\Controllerable;
use Thurly\Main\HttpRequest;
use Thurly\Main\Loader;
use Thurly\Main\Security;
...
final class Router
{
	...
	{
		$class = \CThurlyComponent::includeComponentClass($componentName);
		if (!is_subclass_of($class, 'CThurlyComponent'))
		{
			return null;
		}
		...
		if (!($component instanceof Controllerable))
		{
			throw new SystemException(
				"The component {$this->component} must be implement interface \Thurly\Main\Engine\Contract\Controllerable"
			);
		}
	...
	public function getControllerAndAction()
	{
		...
		/** @see \Thurly\Main\Engine\Controller::__construct */
	...
	{
		$filename = \Thurly\Main\Application::getDocumentRoot() . $componentPath . '/ajax.php';
