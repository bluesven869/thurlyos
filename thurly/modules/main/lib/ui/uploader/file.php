...
class File
{
	...
	public function saveFile(&$file, Storable $storage, array $copies)
	{
		...
		if ($result->isSuccess())
		{
			...
			if ($info["uploadStatus"] == "uploaded")
			{
				...
				$info["~url"] = $this->getUrl("view", $code, \COption::GetOptionString("main.fileinput", "entryPointUrl", "/thurly/tools/upload.php"));
	...
	{
		$uri = is_null($uri) ? \Thurly\Main\Context::getCurrent()->getRequest()->getRequestUri() : $uri;
	...
	public static function view(array $fileData, $options = array())
	{
		...
		{
			$file = new \Thurly\Main\IO\File($fileData["tmp_name"]);
			try
			{
				$src = $file->open(\Thurly\Main\IO\FileStreamOpenMode::READ);
			}
			catch(\Thurly\Main\IO\IoException $e)
			{
				return false;
			}
		...
		else
		{
			...
			if($fastDownload && ($fromClouds || strpos($fileData["tmp_name"], Application::getInstance()->getContext()->getServer()->getDocumentRoot()) === 0))
			{
				...
				{
					header('X-Accel-Redirect: '.\Thurly\Main\Text\Encoding::convertEncoding($fileData["tmp_name"], SITE_CHARSET, "UTF-8"));
				}
			}
			else if ($src)
			{
			...
			{
				$src = new \Thurly\Main\Web\HttpClient();
