	...
	implements ICacheEngine, ICacheEngineStat
{
	...
	{
		$cacheConfig = \Thurly\Main\Config\Configuration::getValue("cache");
	...
	private static function addAgent()
	{
		...
		{
			$agentAdded = true;
			$agents = \CAgent::GetList(array("ID" => "DESC"), array("NAME" => "\\Thurly\\Main\\Data\\CacheEngineFiles::delayedDelete(%"));
			if (!$agents->Fetch())
			{
				$res = \CAgent::AddAgent(
					"\\Thurly\\Main\\Data\\CacheEngineFiles::delayedDelete();",
					"main", //module
					"Y", //period
					1 //interval
				);
	 ...
	 *
	 * @param string $baseDir Base cache directory (usually /thurly/cache).
	...
	public static function delayedDelete($count = 1)
	{
		...
		if ($toDeleteCount > 0)
			return "\\Thurly\\Main\\Data\\CacheEngineFiles::delayedDelete(".$count.");";
		else
			return "";
	}
}
