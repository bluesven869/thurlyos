...
class ConnectionPool
{
	 ...
	 * @return Connection
	 * @throws \Thurly\Main\Config\ConfigurationException
	 */
	...
	protected function getConnectionParameters($name)
	{
		...
		{
			$params["include_after_connected"] = \Thurly\Main\Loader::getPersonal("php_interface/after_connect_d7.php");
		}
	...
	public function getDefaultConnectionType()
	{
		...
		if (isset($params[static::DEFAULT_CONNECTION_NAME]) && !empty($params[static::DEFAULT_CONNECTION_NAME]))
		{
			...
			if ($cn === "\\Thurly\\Main\\DB\\MysqlConnection" || $cn === "\\Thurly\\Main\\DB\\MysqliConnection")
				return 'MYSQL';
			elseif ($cn === "\\Thurly\\Main\\DB\\OracleConnection")
				return 'ORACLE';
			else
				return 'MSSQL';
		}
	...
	protected function getDbConnConnectionParameters()
	{
		...
		if($type == 'mysql')
		{
			$className = "\\Thurly\\Main\\DB\\MysqlConnection";
		}
		elseif($type == 'mssql')
		{
			$className = "\\Thurly\\Main\\DB\\MssqlConnection";
		}
		elseif($type == 'oracle')
		{
			$className = "\\Thurly\\Main\\DB\\OracleConnection";
		}
