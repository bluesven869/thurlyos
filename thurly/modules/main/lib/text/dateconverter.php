...
class DateConverter
{
	 ...
	 * @param string $text
	 * @param integer $limit
	 * @return \Thurly\Main\Text\DateConverterResult[]
	 */
	public static function decode($text, $limit = 0)
	{
		...
		$workTimeStart = explode('.', \Thurly\Main\Config\Option::get('calendar', 'work_time_start', '9'));
		...
		if (preg_match_all("/(".implode('|', $pattern).")/", $text, $match, PREG_OFFSET_CAPTURE))
		{
			foreach ($match[0] as $matchPattern)
			{
				...
				{
					$position = defined("BX_UTF")? strlen(\Thurly\Main\Text\BinaryString::getSubstring($text, 0, $matchPattern[1])): $matchPattern[1];
	 ...
	 * @param \Thurly\Main\Type\DateTime $date If not specified, the modified date is got from the current date.
	 * @return \Thurly\Main\Type\DateTime
	 *
	 */
	private static function createDateUsingMetrics($type, $metric, &$metricModificator = array(), \Thurly\Main\Type\DateTime $date = null)
	{
		$defaultDate = $date? $date: new \Thurly\Main\Type\DateTime();
		...
		switch($type)
		{
			case 1:
				...
				foreach ($metricModificator as $currentModificator)
				{
					...
					else if ($metric['TYPE'] == 'MONTH')
					{
						if (in_array($currentModificator['TYPE'], Array('BEFORE', 'PREVIOUS')))
						{
							...
							$metricDate = new \Thurly\Main\Type\DateTime($metricDate->format('Y-m').'-'.$numberOfMonth.' '.$timeOfEndDate.':00', 'Y-m-d H:i:s');
							$modificator = true;
						}
						else if (in_array($currentModificator['TYPE'], Array('AFTER', 'NEXT')))
						{
						...
						else if ($currentModificator['TYPE'] == 'MIDDLEOF')
						{
							...
							$metricDate = new \Thurly\Main\Type\DateTime($metricDate->format('Y-m').'-'.$numberOfHalfMonth.' '.$timeOfEndDate.':00', 'Y-m-d H:i:s');
							$modificator = true;
						}
						else if ($currentModificator['TYPE'] == 'STARTOF')
						{
							$metricDate = new \Thurly\Main\Type\DateTime($metricDate->format('Y-m').'-01 '.$timeOfStartDate.':00', 'Y-m-d H:i:s');
							$modificator = true;
						}
						else if ($currentModificator['TYPE'] == 'ENDOF')
						{
				...
				$result = new \Thurly\Main\Type\DateTime($metricDate->format('Y-m-d').' '.$metricTime.':00', 'Y-m-d H:i:s');
			...
			case 4:
				...
				$result = new \Thurly\Main\Type\DateTime($metric['VALUE'].' '.$metricTime.':00', 'Y-m-d H:i:s');
	 ...
	 *
	 */
	private static function getDateOfDayOfCurrentWeek($nameOfday, \Thurly\Main\Type\DateTime $date = null)
	{
		$date = $date? $date: new \Thurly\Main\Type\DateTime();
