...
class CAdminList
{
	...
	function ShowSettings($aAllCols, $aCols, $aOptions)
	{
		...
		require($_SERVER['DOCUMENT_ROOT']."/thurly/modules/main/interface/settings_admin_list.php");
	...
	{
		if($_SERVER['REQUEST_METHOD']=='POST' && isset($_REQUEST['save'])  && check_thurly_sessid())
		{
	...
	public function ActionDoGroup($id, $action_id, $add_params='')
	{
		/** @global CMain $APPLICATION */
		global $APPLICATION;
		return $this->table_id.".GetAdminList('".CUtil::AddSlashes($APPLICATION->GetCurPage())."?ID=".CUtil::AddSlashes($id)."&action_button=".CUtil::AddSlashes($action_id)."&lang=".LANGUAGE_ID."&".thurly_sessid_get().($add_params<>""?"&".CUtil::AddSlashes($add_params):"")."');";
	}
	...
	/**
	 * @param \Thurly\Main\UI\PageNavigation $nav
	 * @param string $title
