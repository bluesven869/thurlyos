...
use Thurly\Main\Grid\Editor\Types;
...
class CAdminUiList extends CAdminList
{
	...
	{
		$gridOptions = new Thurly\Main\Grid\Options($this->table_id);
	...
	public function setNavigation(\Thurly\Main\UI\PageNavigation $nav, $title, $showAllways = true, $post = false)
	{
		global $APPLICATION;
	...
	public function EditAction()
	{
		if($_SERVER["REQUEST_METHOD"] == "POST" &&
			!empty($_REQUEST["action_button_".$this->table_id]) && check_thurly_sessid())
		{
	...
	{
		$filterOption = new Thurly\Main\UI\Filter\Options($this->table_id);
		...
		foreach ($this->arActions as $actionKey => $action)
		{
			...
			else
			{
				$actionList[] = array(
					"NAME" => $action,
					"VALUE" => $actionKey,
					"ONCHANGE" => Thurly\Main\Grid\Panel\Actions::RESET_CONTROLS,
				);
			}
		}
	...
	{
		$filterOptions = new Thurly\Main\UI\Filter\Options($this->table_id);
	...
	public function DisplayList($arParams = array())
	{
		...
		$gridParameters = array(
			...
			"SHOW_PAGESIZE" => true,
			"AJAX_ID" => CAjax::getComponentID('thurly:main.ui.grid', '.default', ''),
		...
		$gridOptions = new Thurly\Main\Grid\Options($gridParameters["GRID_ID"]);
		$gridColumns = $gridOptions->getVisibleColumns();
...
class CAdminUiResult extends CAdminResult
{
	...
	public function GetNavSize($tableId = false, $nPageSize = 20, $listUrl = '')
	{
		...
		$gridOptions = new Thurly\Main\Grid\Options($tableId);
		$navParams = $gridOptions->getNavParams();
		return $navParams["nPageSize"];
	}
}
