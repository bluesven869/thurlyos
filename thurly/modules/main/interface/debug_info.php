...
if($bShowStat || $bShowCacheStat)
{
	...
	{
		$arCacheDebug = \Thurly\Main\Diag\CacheTracker::getCacheTracking();
		if (!empty($arCacheDebug))
		{
			echo '<a title="'.GetMessage("debug_info_query_title").'" href="javascript:BX_DEBUG_INFO_CACHE.Show(); BX_DEBUG_INFO_CACHE.ShowDetails(\'BX_DEBUG_INFO_CACHE_m_0\');">'.GetMessage("debug_info_cache_size")."</a> "." ",CFile::FormatSize(\Thurly\Main\Diag\CacheTracker::getCacheStatBytes(), 0)." (".count($arCacheDebug).")<br>";
		}
		else
		{
			echo GetMessage("debug_info_cache_size")." ",CFile::FormatSize(\Thurly\Main\Diag\CacheTracker::getCacheStatBytes(), 0)."<br>";
		}
	}
}
...
if ($bShowStat || $bShowCacheStat) //2
{
	...
	if (!empty($arCacheDebug))
	{
		...
		?>
		<p><?echo GetMessage("debug_info_cache_size")?> <?=CFile::FormatSize(\Thurly\Main\Diag\CacheTracker::getCacheStatBytes(), 0)?></p>
		<?
			...
			<table id="cacheDebug" cellpadding="2" cellspacing="0" border="0">
				...
				foreach($arCacheDebug as $j => $cacheDebug)
				{
					if (substr($cacheDebug["path"], 0, $DOCUMENT_ROOT_LEN) === $_SERVER["DOCUMENT_ROOT"])
						$path = '<a target="blank" href="/thurly/admin/fileman_file_view.php?path='.urlencode(substr($cacheDebug["path"], $DOCUMENT_ROOT_LEN)).'&lang='.LANGUAGE_ID.'">'.htmlspecialcharsEx(substr($cacheDebug["path"], $DOCUMENT_ROOT_LEN)).'</a>';
					else
						$path = '&nbsp;';
				?>
				<tr class="cache-row">
...
?>
	...
	<p><?echo GetMessage("debug_info_comps_cache")?> <?if(COption::GetOptionString("main", "component_cache_on", "Y")=="Y") echo GetMessage("debug_info_comps_cache_on"); else echo "<a href=\"/thurly/admin/cache.php\"><font class=\"errortext\">".GetMessage("debug_info_comps_cache_off")."</font></a>";?>.</p>
	<p><?
	if(\Thurly\Main\Data\Cache::getShowCacheStat())
		echo GetMessage("debug_info_cache_size")." ",CFile::FormatSize(\Thurly\Main\Diag\CacheTracker::getCacheStatBytes(), 0);
	else
		echo "&nbsp;";
	?></p>
<?
