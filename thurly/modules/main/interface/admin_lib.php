...
define("ADMIN_THEMES_PATH", "/thurly/themes");
...
class CAdminPage
{
	...
	public function ShowCSS()
	{
		...
		$arCSS = array_merge(array(
				...
				'/thurly/panel/main/adminstyles_fixed.css',
				'/thurly/panel/main/admin.css',
			),
			$this->GetModulesCSS(),
			$APPLICATION->GetCSSArray()
		);
...
var phpVars = {
	...
	'cookiePrefix': '".CUtil::JSEscape(COption::GetOptionString("main", "cookie_name", "THURLY_SM"))."',
...
";
		...
		$APPLICATION->AddHeadScript('/thurly/js/main/admin_search.js');
		$APPLICATION->AddHeadScript('/thurly/js/main/hot_keys.js');

		$APPLICATION->SetAdditionalCSS('/thurly/panel/main/hot_keys.css');
	...
	function ShowSound()
	{
		...
		$res = '';
		if($USER->IsAuthorized() && !isset($_COOKIE[COption::GetOptionString("main", "cookie_name", "THURLY_SM").'_SOUND_LOGIN_PLAYED']))
		{
			$aUserOptGlobal = CUserOptions::GetOption("global", "settings");
			if($aUserOptGlobal["sound"] == 'Y')
			{
				if($aUserOptGlobal["sound_login"] == '')
					$aUserOptGlobal["sound_login"] = "/thurly/sounds/main/thurly_tune.mp3";

				ob_start();
				$APPLICATION->IncludeComponent("thurly:player",	"audio",
					Array(
...
';
	...
	public function getSSOSwitcherButton()
	{
		...
		else
		{
			...
			{
				if(class_exists('CThurlyOSNetTransport'))
				{
		...
		if(is_array($queryResult))
		{
			...
			if(isset($queryResult['error']))
			{
				if(
					...
					&& class_exists("Thurly\\Socialservices\\Network")
					&& method_exists("Thurly\\Socialservices\\Network", "getAuthUrl")
				)
				{
					$n = new \Thurly\Socialservices\Network();
					$ssoMenu[] =  array(
						"TEXT" => \Thurly\Main\Localization\Loc::getMessage("admin_lib_sso_auth"),
						"TITLE" => \Thurly\Main\Localization\Loc::getMessage("admin_lib_sso_auth_title"),
			...
			{
				$currentHost = \Thurly\Main\Context::getCurrent()->getRequest()->getHttpHost();
	...
	function _SetActiveItems(&$aMenu, $aSections=array())
	{
		...
		if($aMenu["url"] <> "" || $bMoreUrl)
		{
			...
			for($j = 0; $j < $n; $j++)
			{
				...
				if(strpos($all_links[$j], "/thurly/admin/") !== 0)
					$tested_link = "/thurly/admin/".$all_links[$j];
				else
...
class CAdminResult extends CDBResult
{
	...
	public function GetNavPrint($title, $show_allways=true, $StyleText="", $template_path=false, $arDeleteParam=false)
	{
		if($template_path === false)
			$template_path = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/interface/navigation.php";
...
<?
		...
		{
			$openerUrl = '/thurly/admin/get_start_menu.php?skip_recent=Y&lang='.LANGUAGE_ID.($item['ID'] ? '&mode=chain&admin_mnu_menu_id='.urlencode($item['ID']) : '');
...
class CAdminMainChain extends CAdminChain
{
	...
	function Init()
	{
		...
		parent::AddItem(array("TEXT"=> GetMessage("admin_lib_navchain_first"), "LINK"=>"/thurly/admin/index.php?lang=".LANGUAGE_ID, "CLASS" => "adm-navchain-item-desktop"));
