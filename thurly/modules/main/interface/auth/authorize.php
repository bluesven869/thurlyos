...
if(
	...
	&& class_exists("Thurly\\Socialservices\\Network") // just to check if socserv update installed
	&& method_exists("Thurly\\Socialservices\\Network", "displayAdminPopup")
)
{
	$authLink = \Thurly\Socialservices\Network::getAuthUrl("popup", array("admin"));
}
?>
	...
	<div class="login-popup">
		...
		<div class="login-popup-field" id="authorize_password">
			...
			<div class="login-loading">
				<img class="login-waiter" alt="" src="/thurly/panel/main/images/login-waiter.gif">
			</div>
		</div>
<?
$CAPTCHA_CODE = '';
...
?>
		...
		<div class="login-popup-field login-captcha-field">
			...
			<div class="login-input-wrap">
				<span class="login-captcha-wrap" id="captcha_image"><?if($bNeedCaptcha):?><img src="/thurly/tools/captcha.php?captcha_sid=<?=$CAPTCHA_CODE?>" width="180" height="40" alt="CAPTCHA" /><?endif;?></span><input type="text" onfocus="BX.addClass(this.parentNode, 'login-input-active')" onblur="BX.removeClass(this.parentNode, 'login-input-active')" name="captcha_word" class="login-input" tabindex="5" autocomplete="off">
				<div class="login-inp-border"></div>
			</div>
		</div>
<?
...
?>
		<div class="login-popup-field login-auth-serv-icons">
			<a href="" class="login-ss-button thurlyosnet-button thurlyosnet-button-ru"></a>
		</div>
