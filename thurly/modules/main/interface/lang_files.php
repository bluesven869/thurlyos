		...
		if(strlen($_REQUEST["srchlngfil"])>0)
		{
			...
			foreach($MESS as $k=>$v)
			{
				if(strpos($v, $_REQUEST["srchlngfil"])!==false)
				{
					$bFound = true;
					$k = "#$k";
					$stf .= ' <a href="/thurly/admin/translate_edit.php?lang='.LANGUAGE_ID.'&file='.$NEW_LANGS[$i].$k.'">'.htmlspecialcharsbx($v)."</a><br>";
				}
			}
			$MESS = $MESS_t;
			if(!$bFound)
...
<a href="/thurly/admin/translate_edit.php?lang=<?=LANGUAGE_ID?>&file=<?=$NEW_LANGS[$i]?>"><?=$NEW_LANGS[$i]?></a>
</font>
</td>
