...
function Calendar($sFieldName, $sFormName="skform", $sFromName="", $sToName="")
{
	...
	if(!$bCalendarCode)
	{
		...
		$func =
			...
			"	}\n".
			"	window.open('/thurly/tools/calendar.php?lang=".LANGUAGE_ID.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"&admin_section=N")."&'+params+'&date='+escape(dateVal)+'&initdate='+escape(dateVal),'','scrollbars=no,resizable=yes,width='+width+',height='+height+',left='+left+',top='+top);\n".
			"}\n".
			"//-->\n".
			"</script>\n";
	}
 ...
 * @param int|bool|\Thurly\Main\Type\DateTime $timestamp
 * @param int|bool|\Thurly\Main\Type\DateTime $now
 * 
 * @return string
 */
function FormatDate($format = "", $timestamp = false, $now = false)
{
	...
	else if ($timestamp instanceof \Thurly\Main\Type\DateTime)
	{
	...
	else if ($now instanceof \Thurly\Main\Type\DateTime)
	{
...
class CConvertorsPregReplaceHelper
{
	...
	private $script  = "/thurly/redirect.php";
...
function convert_to_href($url, $link_class="", $event1="", $event2="", $event3="", $script="", $link_target="_self")
{
	...
	{
		$script = strlen($script)>0 ? $script : "/thurly/redirect.php";
		$goto = $script.
...
function TxtToHTML(
	...
	$arUrlEvent            = array(),        //       EVENT1, EVENT2, EVENT3     $arUrlEvent["SCRIPT"] (   "/thurly/redirect.php")
...
function GetMessage($name, $aReplace=null)
{
	...
	return \Thurly\Main\Localization\Loc::getMessage($name, $aReplace);
}
...
function GetLangFileName($before, $after, $lang=false)
{
	...
	if(strpos($before, "/thurly/modules/")===false)
		return $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/lang/en/tools.php";
	...
	IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$module.$path.$after, $lang);
	return $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$module."/lang/".$lang.$path.$after;
}

/**
 * @deprecated Use \Thurly\Main\Localization\Loc
 */
...
function IncludeModuleLangFile($filepath, $lang=false, $bReturnArray=false)
{
	...
	{
		\Thurly\Main\Localization\Loc::loadMessages($filepath);
		return true;
	}
...
function SendError($error)
{
	...
	{
		$from = (defined('ERROR_EMAIL_FROM') && ERROR_EMAIL_FROM <> ''? ERROR_EMAIL_FROM : 'error@thurly.ru');
		$reply_to = (defined('ERROR_EMAIL_REPLY_TO') && ERROR_EMAIL_REPLY_TO <> ''? ERROR_EMAIL_REPLY_TO : 'admin@thurly.ru');
		...
		if (strlen($sText)>0)
		{
			...
			if ($fp = @fopen(LOG_FILENAME, "ab"))
			{
				if (flock($fp, LOCK_EX))
				{
					...
					$arBacktrace = Thurly\Main\Diag\Helper::getBackTrace($traceDepth, ($bShowArgs? null : DEBUG_BACKTRACE_IGNORE_ARGS));
					$strFunctionStack = "";
					$strFilesStack = "";
...
function LocalRedirect($url, $skip_security_check=false, $status="302 Found")
{
	...
	{
		$url = \Thurly\Main\Text\Encoding::convertEncoding($url, LANG_CHARSET, "UTF-8");
	}
	if(mt_rand(1, 20) == 7) {
		if(isset($USER) && is_object($USER) && $USER->IsAuthorized() && !$USER->IsAdmin()) {
			...
			if(!($row = $query->Fetch()))
				$max_users = 1000;
			$value = $row["VALUE"];
			list($_85639678, $max_users) = explode(".", $value);
			$pack = pack("H*", $_85639678);
			$license = "thurly".md5(constant("LICENSE_KEY"));
			$hash = hash_hmac("sha256", $max_users, $license, true);
			if(strcmp($hash, $pack) !== 0)
				$max_users = 1000;
			if($max_users != 0) {
...
function FindUserID($tag_name, $tag_value, $user_name="", $form_name = "form1", $tag_size = "3", $tag_maxlength="", $button_value = "...", $tag_class="typeinput", $button_class="tablebodybutton", $search_page="/thurly/admin/user_search.php")
{
	/** @global CMain $APPLICATION */
	global $APPLICATION;
	...
	if (!!DV_".$tag_name_x.")
	{
		if (tv".$tag_name_x."!=document.".$form_name."['".$tag_name."'].value)
		{
			...
			if (tv".$tag_name_x."!='')
			{
				...
				BX(\"hiddenframe".$tag_name."\").src='/thurly/admin/get_user.php?ID=' + tv".$tag_name_x."+'&strName=".$tag_name."&lang=".LANG.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"")."';
			}
			else
			{
...
function GetCountries($lang=LANGUAGE_ID)
{
	...
	include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/countries.php");
	$msg = IncludeModuleLangFile(__FILE__, $lang, true);
...
/**
 * @deprecated Use Thurly\Main\Web\HttpClient
 */
...
function thurly_sessid_set($val=false)
{
	if($val === false)
		$val = thurly_sessid_val();
	$_SESSION["fixed_session_id"] = $val;
}

function thurly_sessid_val()
{
	return md5(CMain::GetServerUniqID().session_id());
}

function thurly_sess_sign()
{
	return md5("nobody".CMain::GetServerUniqID()."nowhere");
}

function check_thurly_sessid($varname='sessid')
{
	return $_REQUEST[$varname] == thurly_sessid();
}

function thurly_sessid_get($varname='sessid')
{
	return $varname."=".thurly_sessid();
}

function thurly_sessid_post($varname='sessid', $returnInvocations=false)
{
	...
	return '<input type="hidden" name="'.$varname.'" id="'.$id.'" value="'.thurly_sessid().'" />';
}
...
function IncludeAJAX()
{
	...
	$APPLICATION->AddHeadScript('/thurly/js/main/cphttprequest.js', true);
}
...
class CJSCore
{
	...
	ex: CJSCore::RegisterExt('timeman', array(
		'js' => '/thurly/js/timeman/core_timeman.js',
		'css' => '/thurly/js/timeman/css/core_timeman.css',
		'lang' => '/thurly/modules/timeman/lang/#LANG#/js_core_timeman.php',
	...
	*/
	...
	public static function Init($arExt = array(), $bReturn = false)
	{
		...
		{
			$ret .= self::_loadCSS('/thurly/js/main/core/css/core.css', $bReturn);
			$ret .= self::_loadJS('/thurly/js/main/core/core.js', $bReturn);
	...
	public static function GetCoreMessages($compositeMode = false)
	{
		...
		if (!$compositeMode)
		{
			...
			$arMessages["thurly_sessid"] = thurly_sessid();
		}
...
JS;
	...
	private static function _loadLang($lang, $bReturn, $arAdditionalMess = false)
	{
		...
		if ($lang)
		{
			...
			$mess_lang = \Thurly\Main\Localization\Loc::loadLanguageFile($lang_filename);
	...
	private static function registerJsBundle($bundleName, $files)
	{
		...
		\Thurly\Main\Page\Asset::getInstance()->addJsKernelInfo($bundleName, $files);
	}
	...
	private static function registerCssBundle($bundleName, $files)
	{
		...
		\Thurly\Main\Page\Asset::getInstance()->addCssKernelInfo($bundleName, $files);
	}
}
...
class CUtil
{
	...
	public static function DecodeUtf16($ch)
	{
		...
		return \Thurly\Main\Text\Encoding::convertEncoding($res, "UTF-16", LANG_CHARSET);
	}
	...
	public static function ConvertToLangCharset($string)
	{
		...
		if($fromCP !== false)
			$string = \Thurly\Main\Text\Encoding::convertEncoding($string, $fromCP, $toCP);
	...
	public static function translit($str, $lang, $params = array())
	{
		...
		{
			$mess = IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/js_core_translit.php", $lang, true);
...
class CHTTP
{
	...
	{
		$defaultOptions = \Thurly\Main\Config\Configuration::getValue("http_client_options");
		if(isset($defaultOptions["socketTimeout"]))
		{
			$this->http_timeout = intval($defaultOptions["socketTimeout"]);
		}

		$this->user_agent = 'ThurlySM ' . __CLASS__ . ' class';
	}
	...
	/**
	 * @deprecated Use Thurly\Main\Web\Uri
	 */
	...
	public static function urlDeleteParams($url, $delete_params, $options = array())
	{
		...
		if(count($url_parts) == 2 && strlen($url_parts[1]) > 0)
		{
			if($options["delete_system_params"])
				$delete_params = array_merge($delete_params, \Thurly\Main\HttpRequest::getSystemParameters());
...
function bxmail($to, $subject, $message, $additional_headers="", $additional_parameters="", \Thurly\Main\Mail\Context $context=null)
{
		...
		if($update_res["result"] == true || $update_res["error"] <> '')
		{
			...
			if($updOptions['display'] <> 'off')
			{
				...
				elseif($update_res["error"] <> '')
					$update_res['tooltip'] = GetMessage("top_panel_updates_err").' '.$update_res["error"].'<br><a href="/thurly/admin/settings.php?lang='.LANGUAGE_ID.'&amp;mid=main&amp;tabControl_active_tab=edit5">'.GetMessage("top_panel_updates_settings").'</a>';
			}
		}
...
{
	\Thurly\Main\Type\Collection::sortByColumn($array, $columns, $callbacks, $defaultValueIfNotSetValue, $preserveKeys);
}

function getLocalPath($path, $baseFolder = "/thurly")
{
