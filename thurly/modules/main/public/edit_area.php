...
class CComponentPanel
{
	...
	public function __construct($component, $componentName, $componentTemplate, $parentComponent, $bComponentEnabled)
	{
		...
		$aTrace = Thurly\Main\Diag\Helper::getBackTrace(2);
		...
		if($this->iSrcLine > 0 && $this->sSrcFile <> "")
		{
			...
			{
				//thurly outside
				$realThurly = strtolower(str_replace("\\", "/", realpath($_SERVER["DOCUMENT_ROOT"]."/thurly")));
				if(strpos($lowerScrFile, substr($realThurly, 0, -6)) === 0)
				{
					$this->sSrcFile = substr($this->sSrcFile, strlen($realThurly) - 7);
					$this->bSrcFound = true;
				}
				else
				{
	...
	public function GetIcons()
	{
		...
		if($bComponentAccess)
		{
			...
			if(!is_null($template))
			{
				...
				if($this->bSrcFound && $template->IsInTheme() == false)
				{
					...
					$urlCopy = "/thurly/admin/template_copy.php?".
						"lang=".urlencode(CUtil::addslashes(LANGUAGE_ID)).
				...
				if($USER->CanDoOperation('edit_php') && strlen($template->GetSiteTemplate()) > 0)
				{
					...
					$arIcons[] = array(
						'URL' => 'javascript:'.$APPLICATION->GetPopupLink(array(
							'URL' => "/thurly/admin/public_file_edit_src.php?site=".SITE_ID."&".'path='.urlencode($template->GetFile())."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&lang=".LANGUAGE_ID,
							'PARAMS' => array(
								'width' => 770,
								'height' => 470,
								'resize' => true,
					...
					if(strlen($template->GetFolder()) > 0)
					{
						if(file_exists($_SERVER["DOCUMENT_ROOT"].$template->GetFolder()."/style.css"))
						{
							...
							$arIcons[] = array(
								'URL' => 'javascript:'.$APPLICATION->GetPopupLink(array(
									'URL' => "/thurly/admin/public_file_edit_src.php?site=".SITE_ID."&".'path='.urlencode($template->GetFolder()."/style.css")."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&lang=".LANGUAGE_ID,
									'PARAMS' => array(
										'width' => 770,
										'height' => 470,
										'resize' => true,
						...
						if(file_exists($_SERVER["DOCUMENT_ROOT"].$template->GetFolder()."/result_modifier.php"))
						{
							...
							$arIcons[] = array(
								'URL' => 'javascript:'.$APPLICATION->GetPopupLink(array(
									'URL' => "/thurly/admin/public_file_edit_src.php?site=".SITE_ID."&".'path='.urlencode($template->GetFolder()."/result_modifier.php")."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&lang=".LANGUAGE_ID,
									'PARAMS' => array(
										'width' => 770,
										'height' => 470,
										'resize' => true,
						...
						if(file_exists($_SERVER["DOCUMENT_ROOT"].$template->GetFolder()."/component_epilog.php"))
						{
							...
							$arIcons[] = array(
								'URL' => 'javascript:'.$APPLICATION->GetPopupLink(array(
									'URL' => "/thurly/admin/public_file_edit_src.php?site=".SITE_ID."&".'path='.urlencode($template->GetFolder()."/component_epilog.php")."&back_url=".urlencode($_SERVER["REQUEST_URI"])."&lang=".LANGUAGE_ID,
									'PARAMS' => array(
										'width' => 770,
										'height' => 470,
										'resize' => true,
		...
		if($arComponentDescription && is_array($arComponentDescription))
		{
			...
			if(array_key_exists("CACHE_PATH", $arComponentDescription) && $USER->CanDoOperation('cache_control'))
			{
				...
				{
					$arIcons[] = array(
						"URL" => "javascript:jsComponentUtils.ClearCache('component_name=".urlencode(CUtil::addslashes($this->componentName))."&site_id=".SITE_ID."&".thurly_sessid_get()."');",
						"ICON" => "del-cache",
