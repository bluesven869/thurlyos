...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_REQUEST["save"]) && $strWarning == "")
{
	...
	if (!$fileContent)
		$fileContent = '<'.'?require($_SERVER["DOCUMENT_ROOT"]."/thurly/header.php");?'.'>'.GetMessage("PAGE_NEW_CONTENT").'<'.'?require($_SERVER["DOCUMENT_ROOT"]."/thurly/footer.php");?>';
...
<script>
	...
	(new BX.CEditorDialog({
		content_url: "/thurly/admin/public_file_edit.php?<?=$_GET['subdialog'] == 'Y' ? "subdialog=Y" : ""?>&bxpublic=Y&lang=<?=CUtil::JSEscape($lang)?>" + "&path=<?=urlencode($pathToEdit)?>&site=<?=urlencode($site)?>&templateID=<?=urlencode($_REQUEST['templateID'])?>&back_url=<?=urlencode($relativePath)?>&edit_new_file_undo=<?= $ID?>",
		height: 470,
		width: 780,
		resizable: true,
		min_width: 780,
...
<?if (IsModuleInstalled("fileman")):?>
	<?if ($createNewFolder):?>
		<p><a href="/thurly/admin/fileman_newfolder.php?lang=<?=urlencode($lang)?>&site=<?=urlencode($site)?>&path=<?=urlencode($path)?>&back_url=<?=urlencode($back_url)?>"><?=GetMessage("PAGE_NEW_EDIT_IN_ADMIN")?></a></p>
	<?else:?>
		<p><a href="/thurly/admin/fileman_html_edit.php?lang=<?=urlencode($lang)?>&site=<?=urlencode($site)?>&path=<?=urlencode($path)?>&new=Y&back_url=<?=urlencode($back_url)?>"><?=GetMessage("PAGE_NEW_EDIT_IN_ADMIN")?></a></p>
	<?endif?>
<?endif?>
...
<table class="bx-width100" id="bx_new_page_common">
	...
	<tr>
		...
		{
			include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/fileman/classes/general/fileman_utils.php");
...
<?if(!empty($arEditGroups)):?>
				<div style="margin-top:4px">
					...
					<label for="bx_acc_lim_who_editors"><?= GetMessage("PAGE_NEW_LIMIT_ACCESS_EDITORS")?> (<a href="/thurly/admin/settings.php?lang=<?=LANGUAGE_ID?>&amp;mid=fileman&amp;tabControl_active_tab=edit3#limitaccess" target="_blank"><?echo GetMessage("page_new_limit_settings")?></a>)</label>
				</div>
...
<div style="margin-top:8px">
	<a href="/thurly/admin/settings.php?lang=<?=LANGUAGE_ID?>&amp;mid=fileman&amp;tabControl_active_tab=edit3#limitaccess" target="_blank"><?echo GetMessage("PAGE_NEW_EDITORS_DEF")?></a>
</div>
<?endif?>
<?
else: //$bAdmin
