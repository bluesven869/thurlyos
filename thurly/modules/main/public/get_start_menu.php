	...
	if (is_array($menu))
	{
		foreach($menu as $item)
		{
			...
			if($item["url"] <> "")
			{
				...
				if(strpos($link, "/thurly/admin/") !== 0)
					$link = "/thurly/admin/".$link;
			...
			{
				$aItem["MENU_URL"] = '/thurly/admin/get_start_menu.php?mode=dynamic&lang='.LANGUAGE_ID.'&admin_mnu_module_id='.urlencode($item['module_id']).'&admin_mnu_menu_id='.urlencode($item['items_id']).($bSkipRecent?'&skip_recent=Y':'').($_REQUEST["back_url_pub"]<>''? '&back_url_pub='.urlencode($_REQUEST["back_url_pub"]):'').'&'.thurly_sessid_get();
...
else
{
	...
	foreach($adminMenu->aGlobalMenu as $menu)
	{
		$aPopup[] = array(
			...
			"LINK"=>$menu['url'] ? '/thurly/admin/'.$menu['url'] : '',
	...
	if($USER->CanDoOperation('edit_own_profile') || $USER->CanDoOperation('edit_other_settings') || $USER->CanDoOperation('view_other_settings'))
	{
		...
		while($db_fav_arr = $db_fav->Fetch())
		{
			...
			if ($db_fav_arr["URL"])
			{
				...
				{
					$aItem["LINK"] = '/thurly/admin/'.$aItem["LINK"];
				}
			...
			if ($db_fav_arr['MENU_ID'])
			{
				...
				if(!is_array($aSubmenu) || empty($aSubmenu))
				{
					$aItem["MENU_URL"] = '/thurly/admin/get_start_menu.php?mode=dynamic&lang='.LANGUAGE_ID.'&admin_mnu_module_id='.urlencode($db_fav_arr['MODULE_ID']).'&admin_mnu_menu_id='.urlencode($db_fav_arr['MENU_ID']).($_REQUEST["back_url_pub"]<>''? '&back_url_pub='.urlencode($_REQUEST["back_url_pub"]):'').'&'.thurly_sessid_get();
					$aItem['MENU_PRELOAD'] = false;
				}
