...
if($_GET['ajax'] == 'Y')
{
	if($_GET['action'] == 'delfolder' && check_thurly_sessid() && $bFileman)
	{
	...
	elseif(($_GET['action'] == 'copy' || $_GET['action'] == 'move') && check_thurly_sessid() && $bFileman)
	{
...
?>
<script src="/thurly/js/main/dd.js<?echo '?'.filemtime($_SERVER["DOCUMENT_ROOT"].'/thurly/js/main/dd.js')?>" type="text/javascript"></script>
<script>
...
window.structGetSubDir = function(el, div_id, path, dirsonly)
{
	...
	if(div.innerHTML == '')
	{
		...
		CHttpRequest.Send('/thurly/admin/public_structure.php?ajax=Y&<?="lang=".$encLang."&site=".$encSite?>&load_path='+path+(dirsonly? '&dirsonly=Y':''));
	}
...
window.structReload = function(path, params)
{
	...
	CHttpRequest.Send('/thurly/admin/public_structure.php?ajax=Y&reload=Y&<?="lang=".$encLang."&site=".$encSite?>&path='+path+(params? '&'+params:''));
};
...
window.structReloadDirs = function(path)
{
	...
	setTimeout(ShowWaitWindow, 50);
	CHttpRequest.Send('/thurly/admin/public_structure.php?ajax=Y&reload=Y&<?="lang=".$encLang."&site=".$encSite?>&dirsonly=Y&path='+path);
};
...
<?
	$url = $APPLICATION->GetPopupLink(array(
		"URL"=>"/thurly/admin/public_file_new.php?subdialog=Y&lang=".$encLang."&site=".$encSite."&templateID=".$encTemplateID."&path=#PATH#", 
...
<?
	$url = $APPLICATION->GetPopupLink(Array(
		"URL"=>"/thurly/admin/public_access_edit.php?subdialog=Y&lang=".$encLang."&site=".$encSite."&path=#PATH#",
		"PARAMS" => Array("min_width"=>450, "min_height" => 250)), "subdialog");
...
<?
	$url = $APPLICATION->GetPopupLink(array(
		"URL"=>"/thurly/admin/public_folder_edit.php?subdialog=Y&lang=".$encLang."&site=".$encSite."&path=#PATH#",
		"PARAMS" => Array("min_width"=>450, "min_height" => 250)), "subdialog");
	$url = str_replace("#PATH#", "'+path+'", $url);
	echo $url.";";
?>
...
{
//	structShowSubDialog();
//	jsPopup_editor.ShowDialog('/thurly/admin/public_file_edit.php?bxpublic=Y&subdialog=Y&lang=<?=$encLang?>&site=<?=$encSite?>&templateID=<?=$encTemplateID?>&path='+path, {width: 780, height: 570, resize: false});
<?
	$url = $APPLICATION->GetPopupLink(Array(
		"URL"=>"/thurly/admin/public_file_edit.php?bxpublic=Y&subdialog=Y&lang=".$encLang."&path=#PATH#&site=".$encSite, 
...
<?
	$url = $APPLICATION->GetPopupLink(Array(
		"URL"=>"/thurly/admin/public_file_edit.php?bxpublic=Y&subdialog=Y&lang=".$encLang."&noeditor=Y&path=#PATH#&site=".$encSite, 
		"PARAMS"=>array("width"=>780, "height"=>570, "resize"=>true)), "editor");
	$url = str_replace("#PATH#", "'+path+'", $url);
	echo $url.";";
?>
...
<?
	$url = $APPLICATION->GetPopupLink(Array(
		"URL"=>"/thurly/admin/public_file_property.php?subdialog=Y&lang=".$encLang."&site=".$encSite."&path=#PATH#",
		"PARAMS" => Array("min_width"=>450, "min_height" => 250)), "subdialog");
	$url = str_replace("#PATH#", "'+path+'", $url);
	echo $url.";";
?>
...
<?
	$url = $APPLICATION->GetPopupLink(array(
		"URL" => "/thurly/admin/public_file_delete.php?subdialog=Y&lang=".$encLang."&site=".$encSite."&path=#PATH#",
...
window.structDelFolder = function(path)
{
	if(confirm('<?=CUtil::JSEscape(GetMessage("pub_struct_folder_del_confirm"))?>'))
		structReload(path, 'action=delfolder&<?="lang=".$encLang."&site=".$encSite."&".thurly_sessid_get()?>&path='+path);
};
...
<?if($bFileman):?>
	...
	items[items.length] = {'SEPARATOR':true};
	items[items.length] = {'TEXT': '<?=CUtil::JSEscape(GetMessage("pub_struct_cp"))?>', 'ONCLICK': 'jsUtils.Redirect(arguments, \'/thurly/admin/fileman_admin.php?lang=<?=$encLang?>&site=<?=$encSite?>&path='+path+'\')', 'TITLE': '<?=CUtil::JSEscape(GetMessage("pub_struct_cp_title"))?>'};
<?endif;?>
...
window.structOpenDirs = function(el)
{
	...
	BX.showWait(strDiv);
	BX.ajax.get(
		'/thurly/admin/public_structure.php?ajax=Y&reload=Y&<?="lang=".$encLang."&site=".$encSite."&path=".$encPath?>&dirsonly=Y', 
		function(result)
		{
...
window.Struct_DragFinish = function(obDrag, x, y, e)
{
	...
	if(confirm(mess))
		structReload(enc_to, 'action='+(e.ctrlKey? 'copy':'move')+'&from='+enc_from+'&to='+enc_to+'&<?=thurly_sessid_get()?>');
