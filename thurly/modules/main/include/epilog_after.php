...
if(isset($_GET["show_lang_files"]) || isset($_SESSION["SHOW_LANG_FILES"]))
	include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/interface/lang_files.php");
...
$bShowCacheStat = (\Thurly\Main\Data\Cache::getShowCacheStat() && ($canEditPHP || $_SESSION["SHOW_CACHE_STAT"]=="Y"));
...
if(($bShowStat || $bShowCacheStat) && !$USER->IsAuthorized())
{
	...
	$GLOBALS["APPLICATION"]->AddHeadString('<script type="text/javascript" src="/thurly/js/main/public_tools.js"></script>');
...
\Thurly\Main\Context::getCurrent()->getResponse()->flush($r);
...
if(!IsModuleInstalled("compression") && !defined('PUBLIC_AJAX_MODE') && ($_REQUEST["mode"] != 'excel'))
{
	if($bShowTime || $bShowStat || $bShowCacheStat)
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/interface/debug_info.php");
}
