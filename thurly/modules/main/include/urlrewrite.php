...
{
	$requestPage = \Thurly\Main\Text\Encoding::convertEncoding($requestPage, "utf-8", (defined("BX_DEFAULT_CHARSET")? BX_DEFAULT_CHARSET : "windows-1251"));
}

$requestUri = $requestPage.$requestParams;

include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/virtual_io.php");
...
if((isset($_SERVER['REDIRECT_STATUS']) && $_SERVER['REDIRECT_STATUS'] == '404') || isset($_GET["SEF_APPLICATION_CUR_PAGE_URL"]))
{
	...
	if (isset($_GET["SEF_APPLICATION_CUR_PAGE_URL"])
		&& (
			...
			|| ($requestUriWithoutParams != "/thurly/urlrewrite.php")
		)
	)
	{
	...
	define("POST_FORM_ACTION_URI", htmlspecialcharsbx("/thurly/urlrewrite.php?SEF_APPLICATION_CUR_PAGE_URL=".urlencode($uriPath)));
}
...
if (!CHTTP::isPathTraversalUri($_SERVER["REQUEST_URI"]))
{
	foreach($arUrlRewrite as $val)
	{
		if(preg_match($val["CONDITION"], $requestUri))
		{
			...
			if (($urlTmp7 == "upload/" || ($urlTmp7 == "thurly/" && substr($urlTmp, 0, 16) != "thurly/services/" && substr($urlTmp, 0, 18) != "thurly/groupdavphp")))
				continue;
...
//admin section 404
if(strpos($requestUri, "/thurly/admin/") === 0)
{
	$_SERVER["REAL_FILE_PATH"] = "/thurly/admin/404.php";
	include($_SERVER["DOCUMENT_ROOT"]."/thurly/admin/404.php");
	die();
}
