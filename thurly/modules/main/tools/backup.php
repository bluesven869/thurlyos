...
{
	IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/admin/dump.php');
	...
	if (!file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/backup'))
		mkdir($_SERVER['DOCUMENT_ROOT'].'/thurly/backup');
...
elseif (!CLI) // hit from thurlycloud service
{
...
if (!file_exists(DOCUMENT_ROOT.'/thurly/backup'))
	mkdir(DOCUMENT_ROOT.'/thurly/backup');

if (!file_exists(DOCUMENT_ROOT."/thurly/backup/index.php"))
{
	$f = fopen(DOCUMENT_ROOT."/thurly/backup/index.php","w");
...
if (!$NS['step'])
{
	...
	if ($dump_bucket_id == -1)
	{
		if (!$bThurlyCloud || !$NS['dump_encrypt_key'])
		{
			$dump_bucket_id = 0;
			ShowBackupStatus('ThurlyCloud is not available');
		}
	}
...
if ($NS['step'] == 4)
{
	...
	if ($arParams['dump_file_public'] || $arParams['dump_file_kernel'])
	{
		...
		foreach($NS['site_path_list'] as $SITE_ID => $DOCUMENT_ROOT_SITE)
		{
			...
			if ($NS['multisite'])
			{
				$tar->prefix = 'thurly/backup/sites/'.$SITE_ID.'/';
				$DirScan->arSkip[rtrim($DOCUMENT_ROOT_SITE, '/').'/thurly'] = true;
...
function RaiseErrorAndDie($strError, $errCode = 0, $ITEM_ID = '')
{
	...
	if (is_object($DB))
	{
		...
		$link = '/thurly/admin/event_log.php?set_filter=Y&find_type=audit_type_id&find_audit_type[]=BACKUP_ERROR';
		$ar = Array(
		...
		{
			\Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/admin/dump.php', $lang);
			$ar["LANG"][$lang] = \Thurly\Main\Localization\Loc::getMessage('DUMP_ERR_AUTO', array('#MESSAGE#' => $strError, '#LINK#' => $link), $lang);
		}
		CAdminNotify::Add($ar);
	}
	die();
