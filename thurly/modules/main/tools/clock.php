...
class CClock
{
	...
	public static function Show($arParams)
	{
		...
		{
			?><a href="javascript:void(0);" onclick="bxShowClock_<?=$arParams['inputId']?>()" title="<?=GetMessage('BX_CLOCK_TITLE')?>" onmouseover="this.className='bxc-icon-hover';" onmouseout="this.className='';"><img id="<?=$arParams['inputId']?>_icon" src="/thurly/images/1.gif" class="bx-clock-icon bxc-iconkit-c"></a><?
		}
		...
		function bxLoadClock_<?=$arParams['inputId']?>(callback)
		{
			...
			if (!window.JCClock)
			{
				...
				else
				{
					...
					return BX.load(
						[
							'<?=CUtil::GetAdditionalFileURL("/thurly/js/main/clock.js")?>',
							'<?=CUtil::GetAdditionalFileURL("/thurly/themes/.default/clock.css")?>'
						],
						function() {bxLoadClock_<?=$arParams['inputId']?>(callback)}
					);
				}
