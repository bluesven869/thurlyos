...
?>		
	...
	function SaveStatus(_this)
	{
		...
		BX.ajax.post("/thurly/admin/checklist.php?bxpublic=Y&<?=thurly_sessid_get()?>",query_str,TestResultSimple);
	}
		...
		var callback = function(data)
		{
			try
			{
				...
				else if (json_data.IN_PROGRESS == "Y")
				{
					...
					BX.ajax.post("/thurly/admin/checklist.php","ACTION=update&autotest=Y&bxpublic=Y&TEST_ID="+testID+"&STEP="+(++step)+"&lang=<?=LANG;?>&<?=thurly_sessid_get()?>",callback);
				}
				else
				{
	...
	function Move(action)
	{
		...
		ShowWaitWindow();
		BX.ajax.post(
			"/thurly/admin/checklist_detail.php?TEST_ID="+arStates["POINTS"][current].TEST_ID+"&lang=<?=LANG;?>&bxpublic=Y&<?=thurly_sessid_get()?>",
			data,
			function(data)
			{
				ReCalc(current);
