...
if ($queryType == "M")
{
	...
	if (strlen($errorMessage) <= 0)
	{
		if (!CUpdateClient::UpdateStepModules($temporaryUpdatesDir, $errorMessage, defined("US_THURLY24_MODE") && US_THURLY24_MODE))
		{
	...
	{
		if (defined("US_THURLY24_MODE") && US_THURLY24_MODE)
		{
			COption::SetOptionString("main", "THURLY24_VERSIONS", "");
			@unlink($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/versions.php");
		}
			...
			else
			{
				if (isset($arStepUpdateInfo["DATA"]["#"]["ITEM"]))
				{
					...
					for ($i = 0, $cnt = count($arStepUpdateInfo["DATA"]["#"]["ITEM"]); $i < $cnt; $i++)
					{
						...
						{
							$cultureId = false;
							if(class_exists('\Thurly\Main\Localization\CultureTable'))
							{
								$culture = \Thurly\Main\Localization\CultureTable::getRow(array('filter'=>array(
								...
								{
									$addResult = \Thurly\Main\Localization\CultureTable::add(array(
