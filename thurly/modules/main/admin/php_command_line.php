...
$remove = 0;
if (isset($_REQUEST["remove"]) && preg_match('/^tab(\d+)$/', $_REQUEST["remove"], $match) && check_thurly_sessid())
{
	$remove = $match[1];
}

if (isset($_REQUEST["query_count"]) && $_REQUEST["query_count"] > 1 && check_thurly_sessid())
{
...
if (isset($_REQUEST["save"]) && check_thurly_sessid())
{
...
function TabAction(action, param, showWait)
{
	...
	BX.ajax.post(
		'php_command_line.php?lang=' + phpVars.LANGUAGE_ID + '&sessid=' + phpVars.thurly_sessid, data,
		function(result){
...
function __FPHPSubmit()
{
	if(confirm('<?=GetMessageJS("php_cmd_confirm")?>'))
	{
		...
		ShowWaitWindow();
		BX.ajax.post(
			'php_command_line.php?lang=' + phpVars.LANGUAGE_ID + '&sessid=' + phpVars.thurly_sessid,
			{
