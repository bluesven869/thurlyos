...
$arSettings = array(
	...
	"arTTFFiles" => array(
		...
		array(
//			"font.ttf" => "font.ttf",
//			"thurly_captcha.ttf" => "thurly_captcha.ttf",
		),
		array("font.ttf"),
...
if($REQUEST_METHOD=="POST" && (strlen($save)>0 || strlen($apply)>0) && check_thurly_sessid() && $isAdmin)
{
...
{
	var changed = false;
	var url = '/thurly/admin/captcha.php?Preview=Y&captcha_sid=<?echo $CAPTCHA_CODE?>';
	var ctl, b;
...
function set_presets()
{
	...
	if(document.getElementById('presets').value == '2')
	{
		...
		for(var i = 0; i < ctl.length; i++)
		{
			ctl[i].selected = (ctl[i].value == 'thurly_captcha.ttf');
		}
	}
...
<tr>
	...
	<td valign="top" rowspan="<?echo count($arSettings)+1?>">
		<?for($i=0;$i < 12; $i++):?>
			<img id="CAPTCHA_<?echo $i?>" src="/thurly/admin/captcha.php?Preview=Y&amp;captcha_sid=<?echo $CAPTCHA_CODE?>&amp;i=<?echo $i?>&amp;j=0" width="180" height="40" alt="CAPTCHA" /><br><br><br>
		<?endfor?>
	</td>
</tr>
<?foreach($arSettings as $key => $value):?>
