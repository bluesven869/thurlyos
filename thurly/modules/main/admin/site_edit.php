...
if($_SERVER["REQUEST_METHOD"] == "POST" && ($_POST["save"] <> '' || $_POST["apply"] <> '') && $isAdmin && check_thurly_sessid())
{
	$arFields = array(
	...
	else
	{
		...
		if ($bNew && $_POST["START_SITE_WIZARD"] == "Y")
		{
			...
			$indexContent = '<'.'?
				...
				include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/wizard/wizard.php");
				?'.'>';
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/thurly/admin/site_admin.php?lang=".LANGUAGE_ID."&set_default=Y",
...
if(!$bNew)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/site_edit.php?lang=".LANGUAGE_ID,
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/site_edit.php?lang=".LANGUAGE_ID."&amp;COPY_ID=".urlencode($str_LID),
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".CUtil::JSEscape(GetMessage("MAIN_DELETE_RECORD_CONF"))."')) window.location='/thurly/admin/site_admin.php?ID=".urlencode(urlencode($str_LID))."&lang=".LANGUAGE_ID."&action=delete&".thurly_sessid_get()."';",
...
?>
	...
	<tr class="adm-detail-required-field">
		...
		<td>
<?
$cultureRes = \Thurly\Main\Localization\CultureTable::getList(array('order'=>array('NAME'=>'ASC')));
$cultures = array();
	...
	<tr id="ID_HIDDENABLE_TR"<?= ($bNew && ($str_START_SITE_WIZARD != "N")) ? "style='display:none'" : ""?>>
		<td colspan="2" align="center">
			...
			while($arRes = $db_res->GetNext())
			{
				...
				$templateSigns[$arRes["ID"]] = $signer->sign($arRes["ID"], "template_preview".thurly_sessid());
			}
				...
				<td>
					...
					<script type="text/javascript">
						function bx_preview_template(index)
						{
							...
							{
								window.open(url + '?thurly_preview_site_template='+templateSigns[sel.value]);
							}
							return false;
						}
					</script>
					<?endif?>
					<a title="<?=GetMessage('MAIN_PREVIEW_TEMPLATE')?>" href="javascript:void(0)" onclick="bx_preview_template('<?=$i?>')"><img src="/thurly/images/main/preview.gif" width="16" height="16" border="0"></a>
				</td>
