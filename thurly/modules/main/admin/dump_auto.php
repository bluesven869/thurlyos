...
if (!preg_match('/^[a-z0-9\.\-]+$/i', $server_name)) // cyrillic domain hack
{
	...
	if (!preg_match('#--p1ai$#', $host)) // trying to guess
		$host = $converter->Encode(\Thurly\Main\Text\Encoding::convertEncoding($server_name, 'utf-8', 'windows-1251'));
	$server_name = $host;
}
$url = (CMain::IsHTTPS() ? "https://" : "http://").$server_name;
define('LOCK_FILE', $_SERVER['DOCUMENT_ROOT'].'/thurly/backup/auto_lock');
...
if($_REQUEST['save'])
{
	...
	else
	{
		...
		if ($bThurlyCloud)
			CThurlyCloudBackup::getInstance()->deleteBackupJob();
		...
		if ($dump_auto_set)
		{
			...
			if ($dump_auto_set == 2)
			{
				...
				$strError = CThurlyCloudBackup::getInstance()->addBackupJob(
					$backup_secret_key,
					$url,
					$sec,
					$arWeekDays
				);
				$strMessage .= '<br>'.GetMessage('DUMP_CHECK_THURLYCLOUD', array('#LINK#' => '/thurly/admin/thurlycloud_backup_job.php?lang='.LANGUAGE_ID));
			}
		}
		elseif ($_REQUEST['dump_auto_green_button'])
			$strError = GetMessage('DUMP_WARN_NO_THURLYCLOUD');
		else
...
{
	// Location
	BX("<?=$bThurlyCloud ? 'thurlycloud' : 'localfolder'?>").checked = true;

	// Starter
	BX("<?=$bThurlyCloud ? 'dump_auto_thurly' : ($bCron ? 'dump_auto_cron' : 'dump_auto_disable')?>").checked = true;

	if (ob = document.fd1.dump_encrypt)
		ob.checked = false;

	BX("<?=$bThurlyCloud ? 'dump_delete_old_1' : 'dump_delete_old_4' ?>").checked = true;
...
{
	var ob;
	if (BX('thurlycloud').checked || (ob = document.fd1.dump_encrypt) && ob.checked)
	{
		if (!PasswordDialog)
		{
...
if ($BUCKET_ID == -1 && !$bThurlyCloud)
	$BUCKET_ID = 0;
?>
<tr>
	<td class="adm-detail-valign-top" width=40%><?=GetMessage('MAIN_DUMP_ARC_LOCATION')?></td>
	<td>
		<div><label><input type=radio name=dump_bucket_id value="-1" <?=$BUCKET_ID == -1 ? "checked" : ""?> id="thurlycloud" <?=!$bThurlyCloud ? 'disabled' : ''?> onclick="CheckEncrypt()"> <?=GetMessage('DUMP_MAIN_IN_THE_BXCLOUD')?></label><?=$strBXError ? ' <span style="color:red">('.$strBXError.')</span>' : ''?></div>
...
<tr>
	...
	<td>
		<?
		?>
		<div><label><input type="radio" name="dump_auto_enable" id="dump_auto_thurly" value="2" <?= $dump_auto_enable == 2 ? 'checked' : '' ?> <?=$bThurlyCloud ? '' : 'disabled'?> onclick="CheckEnabled()"> <?=GetMessage('AUTO_EXEC_FROM_THURLY')?> (<a href="/thurly/admin/settings.php?lang=<?=LANGUAGE_ID?>&mid=main" target=_blank><?=GetMessage('AUTO_URL')?></a>: <?=htmlspecialcharsbx($url)?>)</div>
		...
		<div><label><input type="radio" name="dump_auto_enable" id="dump_auto_disable" value="0" <?= $dump_auto_enable == 0 ? 'checked' : '' ?> onclick="CheckEnabled()"> <?=GetMessage('AUTO_EXEC_FROM_MAN', array('#SCRIPT#' => '<b>/thurly/modules/main/tools/backup.php</b>'))?></div>
	</td>
</tr>
<tr>
