...
if (StrLen($site_id) <= 0)
	LocalRedirect("/thurly/admin/urlrewrite_list.php?lang=".LANG);

if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $isAdmin && check_thurly_sessid())
{
	...
	{
		if (strlen($apply) <= 0)
			LocalRedirect("/thurly/admin/urlrewrite_list.php?lang=".LANG."&filter_site_id=".UrlEncode($site_id)."&".GetFilterParams("filter_", false));
	}
	else
	{
...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/urlrewrite_list.php?lang=".LANG."&filter_site_id=".UrlEncode($site_id)."&".GetFilterParams("filter_", false),
		"ICON"	=> "btn_list",
...
if (StrLen($CONDITION) > 0)
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/urlrewrite_edit.php?lang=".LANG."&site_id=".UrlEncode($site_id)."&".GetFilterParams("filter_", false),
		"ICON"	=> "btn_new",
	...
	$aMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".GetMessage("MURL_ACT_DEL_CONF")."')) window.location='/thurly/admin/urlrewrite_list.php?ID=".urlencode(urlencode($CONDITION))."&filter_site_id=".urlencode(urlencode($site_id))."&action=delete&lang=".LANG."&".thurly_sessid_get()."';",
		"WARNING" => "Y",
		"ICON"	=> "btn_delete"
	);
}
...
$tabControl->Buttons(
	array(
		"disabled" => !$isAdmin,
		"back_url" => "/thurly/admin/urlrewrite_list.php?lang=".LANG."&filter_site_id=".UrlEncode($site_id)."&".GetFilterParams("filter_", false)
	)
);
?>
<?
