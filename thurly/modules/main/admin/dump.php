...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/backup.php");
$strBXError = '';
...
if($_REQUEST['process'] == "Y")
{
	...
	if($_REQUEST['action'] == 'start')
	{
		...
		if(!file_exists(DOCUMENT_ROOT.BX_ROOT."/backup/index.php"))
		{
			...
			fwrite($f,"<head><meta http-equiv=\"REFRESH\" content=\"0;URL=/thurly/admin/index.php\"></head>");
			fclose($f);
		}
...
//				$NS['dump_encrypt_key'] = $APPLICATION->ConvertCharset('windows-1251','utf-8',$NS['dump_encrypt_key']);
		...
		if ($NS['BUCKET_ID']) // send to the [thurly]cloud
			$NS['total_steps']++;
			...
			if (haveTime())
			{
				...
				if (is_array($NS['dump_site_id']))
				{
					...
					if ($f = $rs->Fetch())
					{
						...
						if ($NS['multisite'])
						{
							$tar->prefix = 'thurly/backup/sites/'.$f['LID'].'/';
							$DirScan->arSkip[$DOCUMENT_ROOT_SITE.'/thurly'] = true;
		...
		if ($NS['BUCKET_ID'])
		{
			if (haveTime())
			{
				...
				if ($NS['BUCKET_ID'] == -1)
				{
					...
					if (!$NS['obBucket'])
					{
						$backup = CThurlyCloudBackup::getInstance();
						$q = $backup->getQuota();
...
//				GetMessage('TIME_SPENT').' '.HumanTime($NS["time"]),
		...
		?>
		<script>
			window.setTimeout("if(!stop)AjaxSend('?process=Y&<?=thurly_sessid_get()?>')",<?=1000 * IntOption("dump_max_exec_time_sleep")?>);
		</script>
		<?
	}
	else // Finish
	{
		...
		if ($NS["arc_size"])
		{
			...
			elseif ($NS['BUCKET_ID'] == -1)
				$l = GetMessage('DUMP_MAIN_THURLY_CLOUD');
			else
...
if (defined('DUMP_DEBUG_MODE'))
	echo '<div style="color:red">DEBUG MODE</div><input type=button value=Next onclick="AjaxSend(\'?process=Y&'.thurly_sessid_get().'\')">';

?><div id="dump_result_div"></div><?
$aMenu = array(
	array(
		"TEXT"	=> GetMessage("MAIN_DUMP_LIST_PAGE_TITLE"),
		"LINK"	=> "/thurly/admin/dump_list.php?lang=".LANGUAGE_ID,
...
function GetLicenseInfo()
{
	...
	CHttpRequest.Send('?action=key_info&lang=<?=LANGUAGE_ID?>&<?=thurly_sessid_get()?>');
}
...
BX.ready(
	function()
	{
		CheckExpert();
		<?
			if ($_REQUEST['from'] == 'thurlycloud')
				echo 'StartDump();';
			elseif ($_REQUEST['action'] == 'cloud_send' && check_thurly_sessid())
				echo "AjaxSend('?process=Y&action=cloud_send&f_id=".CUtil::JSEscape($_REQUEST['f_id'])."&dump_encrypt_key=".CUtil::JSEscape($_REQUEST['dump_encrypt_key'])."&dump_bucket_id=".CUtil::JSEscape($_REQUEST['dump_bucket_id'])."&".thurly_sessid_get()."');";
			elseif ($_REQUEST['action'] == 'check_archive' && check_thurly_sessid())
				echo "AjaxSend('?process=Y&action=check_archive&f_id=".CUtil::JSEscape($_REQUEST['f_id'])."&dump_encrypt_key=".CUtil::JSEscape($_REQUEST['dump_encrypt_key'])."&".thurly_sessid_get()."');";
		?>
	}
);
...
{
	var ob;
	if (BX('thurlycloud').checked || (document.fd1.dump_expert.checked && (ob = document.fd1.dump_encrypt) && ob.checked))
	{
		if (!PasswordDialog)
		{
...
function RetryRequest()
{
	if (ob = BX('retry_button'))
		ob.disabled=true;
	AjaxSend('?process=Y&<?=thurly_sessid_get()?>');
}
</script>
	...
	if (!COption::GetOptionInt("main", 'dump_auto_enable_auto', 0))
	{
		...
		<tr>
			<td colspan=2><?
			echo BeginNote();
			echo GetMessage('MAIN_DUMP_AUTO_WARN', array('#LINK#' => '/thurly/admin/dump_auto.php?lang='.LANGUAGE_ID));
			echo EndNote();
			?></td>
		</tr>
		<?
	...
	?>
	<tr>
		<td class="adm-detail-valign-top" width="40%"><?=GetMessage('DUMP_MAIN_THURLY_CLOUD_DESC')?><span class="required"><sup>1</sup></span>:</td>
		<td width="60%">
		<?
			$backup = CThurlyCloudBackup::getInstance();
			CAdminMessage::ShowMessage(array(
	...
	<tr>
		...
		<td>
			<div><input type=radio name=dump_bucket_id value="-1" <?=$bThurlyCloud ? "checked" : ""?> id="thurlycloud" <?=$bThurlyCloud ? '' : 'disabled'?> onclick="CheckEncrypt(this)"> <label for="thurlycloud"><?=GetMessage('DUMP_MAIN_IN_THE_BXCLOUD')?></label><?=$strBXError ? ' <span style="color:red">('.$strBXError.')</span>' : ''?></div>
			<div><input type=radio name=dump_bucket_id value="0"  <?=!$bThurlyCloud ? "checked" : ""?> id="dump_bucket_id_0" onclick="CheckEncrypt(this)"> <label for="dump_bucket_id_0"><?=GetMessage('MAIN_DUMP_LOCAL_DISK')?></label></div>
			<?
...
?>
	...
	<tr>
		...
		<td><input type="checkbox" name="dump_encrypt" value="Y" <?=(IntOption("dump_encrypt", 0) ? "checked" : "")?> <?=$bMcrypt && !$bThurlyCloud  ? '' : 'disabled'?>></td>
	</tr>
	<tr>
