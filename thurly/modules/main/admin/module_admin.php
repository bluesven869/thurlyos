...
if($isAdmin && !$fb && check_thurly_sessid())
{
	...
	elseif(isset($_REQUEST["action"]) && $_REQUEST["action"] == "version_down")
	{
		...
		if($_REQUEST["id"] == "main")
			$fn = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/version.php";
		else
...
function DoAction(oEvent, action, module_id)
{
	if (oEvent.ctrlKey || BX.browser.IsMac() && (oEvent.altKey || oEvent.metaKey))
	{
		...
		if(action == 'version_down')
		{
			ShowWaitWindow();
			BX.ajax.post(
				'module_admin.php?lang=<?echo LANGUAGE_ID?>&id='+module_id+'&count='+(oEvent.shiftKey? 10: 1)+'&<?echo thurly_sessid_get()?>&action='+action,
				null,
				function(result){
					CloseWaitWindow();
