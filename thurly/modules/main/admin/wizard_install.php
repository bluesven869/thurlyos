...
elseif (!check_thurly_sessid()):?>
	...
	<form action="<?=$APPLICATION->GetCurPageParam(thurly_sessid_get(), Array("sessid"))?>" method="post">
...
<?
	...
	{
		$path = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$arWizardName[0]."/install/wizards/".$arWizardName[1]."/".$arWizardName[2];

		CopyDirFiles(
			$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$arWizardName[0]."/install/wizards/".$arWizardName[1]."/".$arWizardName[2],
			$_SERVER["DOCUMENT_ROOT"]."/thurly/wizards/".$arWizardName[1]."/".$arWizardName[2],
			true,
			true,
			false,
			""
