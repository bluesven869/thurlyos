...
$res=false;
if(strlen($Reindex)>0 && check_thurly_sessid())
{
	...
	if (class_exists("\\Thurly\\Main\\Application", false))
		$res = \Thurly\Main\UrlRewriter::reindexAll(($NS["stepped"]=="Y"? $NS["max_execution_time"]:0), $NS);
	else
...
function DoNext()
{
	...
	if(newNS!=savedNS)
	{
		...
		queryString+='&<?echo thurly_sessid_get()?>';
		savedNS=newNS;
		//alert(queryString);
