...
if($_SERVER["REQUEST_METHOD"] == "POST" && ($_REQUEST["save"] <> '' || $_REQUEST["apply"] <> '') && $USER->CanDoOperation('edit_groups') && check_thurly_sessid())
{
	if($ID <= 2 && $ID != 0)
		$ACTIVE = "Y";
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/thurly/admin/group_admin.php?lang=".LANGUAGE_ID."&set_default=Y",
		"ICON"	=> "btn_list"
...
if($USER->CanDoOperation('edit_groups'))
{
	if(intval($ID)>0 && $COPY_ID<=0)
	{
		...
		$aMenu[] = array(
			...
			"LINK"	=> "/thurly/admin/group_edit.php?lang=".LANGUAGE_ID,
			"ICON"	=> "btn_new"
		);
		if($ID>1)
		{
			$aMenu[] = array(
				...
				"LINK"	=> "/thurly/admin/group_edit.php?lang=".LANGUAGE_ID."&amp;COPY_ID=".$ID,
				"ICON"	=> "btn_copy"
			);
		}
		...
		if($ID>2)
		{
			$aMenu[] = array(
				...
				"LINK"	=> "javascript:if(confirm('".CUtil::JSEscape(GetMessage("MAIN_DELETE_RECORD_CONF"))."')) window.location='/thurly/admin/group_admin.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
				"ICON"	=> "btn_delete"
			);
		}
	}
		...
		<table border="0" cellpadding="0" cellspacing="0" class="internal">
			...
			while ($arUsers = $dbUsers->Fetch())
			{
				...
				<tr>
					...
					<td align="left"><label for="USER_ID_ACT_ID_<?=$ind?>">[<a href="/thurly/admin/user_edit.php?ID=<?=$arUsers["ID"]?>&lang=<?=LANGUAGE_ID?>" title="<?=GetMessage("MAIN_VIEW_USER")?>"><?=$arUsers["ID"]?></a>] (<?=htmlspecialcharsbx($arUsers["LOGIN"])?>) <?=htmlspecialcharsbx($arUsers["NAME"])?> <?=htmlspecialcharsbx($arUsers["LAST_NAME"])?></label></td>
					<td>
			...
			else
			{
				...
				)
				{
					foreach ($arSites["reference_id"] as $i => $site_id_tmp)
					{
						...
						if (strlen($v) > 0)
						{
							?><tr>
								...
								?></td>
								<td style="padding: 3px;"><a href="javascript:void(0)" onClick="settingsDeleteRow(this)"><img src="/thurly/themes/.default/images/actions/delete_button.gif" border="0" width="20" height="20"></a></td>
							</tr><?
						}
					}
					...
					<tr id="hidden-rights-row" style="display: none;">
						...
						<td><a href="javascript:void(0)" onClick="settingsDeleteRow(this)"><img src="/thurly/themes/.default/images/actions/delete_button.gif" border="0" width="20" height="20"></a></td>
					</tr>
					<?
				}
