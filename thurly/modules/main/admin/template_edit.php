...
$isEditingMessageThemePage = $APPLICATION->GetCurPage() == '/thurly/admin/message_theme_edit.php';
...
if($_SERVER["REQUEST_METHOD"] == "POST" && ($_POST["save"] <> '' || $_POST["apply"] <> '') && check_thurly_sessid() && ($edit_php || $lpa))
{
	$strError = "";
	if ($lpa)
	{
...
$APPLICATION->AddHeadScript("/thurly/js/main/template_edit.js");
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/thurly/admin/".($isEditingMessageThemePage ? "message_theme_admin.php" : "template_admin.php")."?lang=".LANGUAGE_ID."&set_default=Y",
...
if (strlen($ID)>0 && $edit_php)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/".($isEditingMessageThemePage ? "message_theme_edit.php" : "template_edit.php")."?lang=".LANGUAGE_ID,
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/".($isEditingMessageThemePage ? "message_theme_admin.php" : "template_admin.php")."?lang=".LANGUAGE_ID."&ID=".urlencode($ID)."&action=copy&".thurly_sessid_get(),
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/thurly/admin/".($isEditingMessageThemePage ? "message_theme_admin.php" : "template_admin.php")."?ID=".urlencode(urlencode($ID))."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."&action=delete';",
...
if (($USER->CanDoOperation('edit_other_settings') || $USER->CanDoOperation('lpa_template_edit')) && !empty($ID) && !$isEditingMessageThemePage):
	...
	$sign = $signer->sign($ID, "template_preview".thurly_sessid());
?>
<input type="button" value="<?=GetMessage('FILEMAN_PREVIEW_TEMPLATE')?>" name="template_preview" onclick="preview_template('<?=htmlspecialcharsbx(CUtil::JSEscape($ID))?>', '<?= thurly_sessid()?>', '<?=htmlspecialcharsbx(CUtil::JSEscape($sign))?>');" title="<?=GetMessage('FILEMAN_PREVIEW_TEMPLATE_TITLE')?>">
<?
endif;
?>
