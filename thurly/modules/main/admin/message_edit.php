...
if($REQUEST_METHOD=="POST" && (strlen($save)>0 || strlen($apply)>0)&& $isAdmin && check_thurly_sessid())
{
	...
	else
	{
		...
		if(is_array($FILES_del))
		{
			...
			{
				$deleteFileDb = \Thurly\Main\Mail\Internal\EventMessageAttachmentTable::getList(array(
				...
				while($arDeleteFile = $deleteFileDb->fetch())
				{
					...
					\Thurly\Main\Mail\Internal\EventMessageAttachmentTable::delete($ID);
				}
			}
		}
		...
		foreach($arFiles as $file)
		{
			if(strlen($file["name"])>0 and intval($file["size"])>0)
			{
				...
				if($fid > 0)
				{
					$resultAddAttachFile = \Thurly\Main\Mail\Internal\EventMessageAttachmentTable::add(array(
						'EVENT_MESSAGE_ID' => $ID,
						'FILE_ID' => $fid
					));
...
else
{
	...
	$attachmentFileDb = \Thurly\Main\Mail\Internal\EventMessageAttachmentTable::getList(array(
		'select' => array('FILE_ID'),
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/thurly/admin/message_admin.php?lang=".LANGUAGE_ID."&set_default=Y",
...
if (intval($ID)>0 && $COPY_ID<=0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/message_edit.php?lang=".LANGUAGE_ID,
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/message_edit.php?lang=".LANGUAGE_ID.htmlspecialcharsbx("&COPY_ID=").$ID,
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/thurly/admin/message_admin.php?ID=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."&action=delete';",
	...
	<tr>
		...
		<td>
			<select name="LANGUAGE_ID">
				...
				<?
				$languages = \Thurly\Main\Localization\LanguageTable::getList(array(
				...
				<? while($language = $languages->fetch()): ?>
					<option value="<?=$language["LID"]?>"<? if($str_LANGUAGE_ID == $language["LID"]) echo " selected" ?>>
						<?=\Thurly\Main\Text\HtmlFilter::encode($language["NAME"])?>
					</option>
				<? endwhile ?>
			</select>
		</td>
