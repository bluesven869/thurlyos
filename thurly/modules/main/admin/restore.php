...
if (getenv('THURLY_VA_VER'))
	define('VMTHURLY', 'defined');
...
if(isset($_SERVER["BX_PERSONAL_ROOT"]) && $_SERVER["BX_PERSONAL_ROOT"] <> "")
	...
	define("BX_PERSONAL_ROOT", "/thurly");

if(!defined("START_EXEC_TIME"))
	define("START_EXEC_TIME", microtime(true));

define("STEP_TIME", defined('VMTHURLY') ? 30 : 15);
# define("DELAY", defined('VMTHURLY') ? 0 : 3); // reserved
# xdebug_start_trace();
define('RESTORE_FILE_LIST', $_SERVER['DOCUMENT_ROOT'].'/thurly/tmp/restore.file_list.php');
define('RESTORE_FILE_DIR', $_SERVER['DOCUMENT_ROOT'].'/thurly/tmp/restore.removed');
...
$dbconn = $_SERVER['DOCUMENT_ROOT']."/thurly/php_interface/dbconn.php";
...
$mArr_ru =  array(
			...
			"ARC_DOWN_THURLYCLOUD" => "Развернуть резервную копию из облака &quot;1С-Битрикс&quot;",
			...
			"WARN_CLEARED" => "При распаковке ядра продукта в папке /thurly/modules были обнаружены файлы, которых не было в архиве. Эти файлы перенесены в /thurly/tmp/restore.removed",
			"WARN_SITES" => "Вы распаковали многосайтовый архив, файлы дополнительных сайтов следует скопировать вручную из папки /thurly/backup/sites",
			"WARNING" => "Внимание!",
...
$mArr_en = array(
			...
			"ARC_DOWN_THURLYCLOUD" => "Restore the backup from the Thurly Cloud",
			...
			"DB_ENV" => "restoring in Thurly Environment",
			...
			"ERR_LOAD_FILE_LIST" => "Wrong Thurlysoft server response",
			...
			"WARN_CLEARED" => "Some files were found in /thurly/modules which don't present in the backup. They were moved to /thurly/tmp/restore.removed",
			"WARN_SITES" => "You have extracted the multisite archive, please copy files of additional sites from /thurly/backup/sites to an appropriate place",
			"WARNING" => "Warning!",
...
if ($_REQUEST['LoadFileList'])
{
	...
	if (LoadFile("https://www.1c-thurly.ru/buy_tmp/backup.php?license=".md5(trim($_REQUEST['license_key']))."&lang=".LANG."&action=get_info", $file = $_SERVER['DOCUMENT_ROOT'].'/file_list.xml') && ($str = file_get_contents($file)))
	{
		if (preg_match_all('/<file name="([^"]+)" size="([^"]+)".*?\\/>/', $str, $regs))
		{
			...
			echo getMsg('SELECT_ARC').':&nbsp;<select name="thurlycloud_backup">';
			foreach($arFiles as $name => $size)
				echo '<option value="'.htmlspecialcharsbx($name).'" '.($_REQUEST['thurlycloud_backup'] == $name ? 'selected' : '').'>'.htmlspecialcharsbx($name).' ('.floor($size/1024/1024), ' Mb'.($arParts[$name] > 1 ? ', '.getMsg('CNT_PARTS').': '.$arParts[$name] : '').')</option>';
			echo '</select><br>';
...
elseif ($Step == 2 && !$bSelectDumpStep)
{
	...
	if ($source == 'thurlycloud')
	{
		$source = 'download';
		$strLog = '';
		if (LoadFile('https://www.1c-thurly.ru/buy_tmp/backup.php?license='.md5(trim($_REQUEST['license_key'])).'&lang='.LANG.'&action=read_file&file_name='.urlencode($_REQUEST['thurlycloud_backup']).'&check_word='.CTar::getCheckword($_REQUEST['EncryptKey']), $file = $_SERVER['DOCUMENT_ROOT'].'/file_info.xml') && ($str = file_get_contents($file)))
		{
			unlink($file);
...
//			$port = preg_match('#<port>([^<]+)</port>#i',$str,$regs) ? $regs[1] : false;
		...
		if (!$_REQUEST['try_next'] && $strErrMsg)
		{
			...
			'<input type="hidden" name="source" value="thurlycloud">'.
			'<input type="hidden" name="thurlycloud_backup" value="'.htmlspecialcharsbx($_REQUEST['thurlycloud_backup']).'">'.
	...
	if ($source == 'download')
	{
		...
		if ($res)
		{
			...
			if ($res == 2)
			{
				...
				$text .= '<input type=hidden name="thurlycloud_backup" value="'.htmlspecialcharsbx($_REQUEST['thurlycloud_backup']).'">';
			...
			else
			{
				...
				if (count($arHeaders)) // thurlycloud
				{
					$text .= '<input type=hidden name=source value=thurlycloud>';
					$text .= '<input type=hidden name="thurlycloud_backup" value="'.htmlspecialcharsbx($tar->getNextName($_REQUEST['thurlycloud_backup'])).'">';
				}
				else
				{
					$text .= '<input type=hidden name=source value=download>';
		...
		elseif ($_REQUEST['try_next']) // пробовали новую часть
		{
			...
			if ($_REQUEST['thurlycloud_backup'])
				$text .= '<input type=hidden name=arc_name value="'.htmlspecialcharsbx(CTar::getFirstName($_REQUEST['thurlycloud_backup'])).'">';
			else
				$text .= '<input type=hidden name=arc_name value="'.htmlspecialcharsbx(CTar::getFirstName($arc_name)).'">';
		}
		else
		{
			if ($_REQUEST['source'] != 'thurlycloud' && $replycode == 403 && count($arHeaders)) // Retry for thurlycloud
			{
				$text = getMsg('ARC_DOWN_PROCESS').' <b>'.htmlspecialcharsbx($arc_name).'</b>' . $status .
					'<input type=hidden name=Step value=2>'.
					'<input type=hidden name=continue value=Y>'.
...
if(!$Step)
{
	$ar = array(
		...
		'TEXT' =>
			...
			getMsg('BEGIN'),
		'BOTTOM' =>
		(defined('VMTHURLY') ? '<input type=button value="'.getMsg('BUT_TEXT_BACK').'" onClick="document.location=\'/\'"> ' : '').
...
elseif($Step == 1)
{
	...
	$ar = array(
		...
		'TEXT' =>
				...
				'<input type="hidden" name="Step" value="2">'.
				'<div class=t_div>
					<label><input type=radio name=x_source onclick="div_show(0)" '.($_REQUEST['thurlycloud_backup'] ? 'checked' : '').'>'.getMsg("ARC_DOWN_THURLYCLOUD", LANG).'</label>
	...
	<script>
		...
		function div_show(i)
		{
			...
			arSources = [ 'thurlycloud','download','upload','local','dump' ];
		...
		function LoadFileList()
		{
			...
			query = 'LoadFileList=Y&lang=<?=LANG?>&thurlycloud_backup=<?=htmlspecialcharsbx($_REQUEST['thurlycloud_backup'])?>&license_key=' + document.getElementById('license_key').value;
		...
		elseif ($_REQUEST['thurlycloud_backup'])
		{
			?>
...
elseif($Step == 2)
{
	...
	if ($bClearUnusedStep)
	{
		if (file_exists(RESTORE_FILE_LIST))
		{
			...
			$res = $ds->Scan($_SERVER['DOCUMENT_ROOT'].'/thurly/modules');
		...
		if (file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/backup/sites'))
	...
	if ($bSelectDumpStep)
	{
		...
		else
		{
			...
			if (!$_REQUEST['DBName'])
			{
				...
				{
					$DBHost = 'localhost'.(file_exists($_SERVER['DOCUMENT_ROOT'].'/../ThurlyEnv.exe') ? ':31006' : '');
					$DBLogin = 'root';
					$DBPassword = '';
					$DBName = 'thurly_'.(rand(11,99));
					$create_db = "Y";
				}
			}
			else
			...
			if(count($arDName))
			{
				$ar = array(
					...
					'TEXT' =>
						...
						(
						file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/backup/clouds') ?
...
elseif($Step == 3)
{
	...
	if (!isset($_REQUEST['d_pos'])) // start
	{
		...
		if (defined('VMTHURLY') && !preg_match('#^[ \t]*define..BX_CRONTAB_SUPPORT#mi', $strFile))
			$strFile = '<'.'?define("BX_CRONTAB_SUPPORT", true);?'.'>'.$strFile;

		file_put_contents($dbconn, $strFile);

		if (file_exists($config = $_SERVER['DOCUMENT_ROOT']."/thurly/.settings.php"))
		{
			ob_start();
			$ar = include($config);
			ob_end_clean();
			...
			if (is_array($ar))
			{
				...
				rename($config, $_SERVER['DOCUMENT_ROOT']."/thurly/.settings.restore.php"); // workaround for bug #47641
		}
	...
	if($r && !$oDB->is_end())
	{
		...
		$oDB->close();
		$arc_name = $_REQUEST["arc_name"];
		$dump_name = preg_replace('#\.[0-9]+$#', '', $_SERVER['DOCUMENT_ROOT'].'/thurly/backup/'.$_REQUEST['dump_name']);
		$dump_size = 0;
...
elseif($Step == 4) // последний экран: удалять или нет? 
{
	...
	if ($_REQUEST['check_site_path'])
	{
		...
		if ($oDB->Connect())
		{
			...
			$rs = $oDB->Query('SHOW TABLES LIKE "b_thurlycloud_option"');
			if ($oDB->Fetch($rs))
			{
				$rs = $oDB->Query('SELECT * FROM b_thurlycloud_option WHERE NAME="cdn_config_active" AND PARAM_VALUE=1');
				if ($oDB->Fetch($rs))
				{
					$rs = $oDB->Query('SELECT * FROM b_thurlycloud_option WHERE NAME="cdn_config_domain"');
					if (($f = $oDB->Fetch($rs)) && $f['PARAM_VALUE'] != $_SERVER['HTTP_HOST'])
					{
						$oDB->Query('UPDATE b_thurlycloud_option SET PARAM_VALUE=0 WHERE NAME="cdn_config_active"');
						$oDB->Query('UPDATE b_thurlycloud_option SET PARAM_VALUE='.(time() + 86400 * 3650).' WHERE NAME="cdn_config_expire_time"');
						$strWarning .= '<li>'.getMsg('CDN_WARN');
					}
				}
			}

			if ($rs = $oDB->Query('SELECT * FROM b_module_to_module WHERE FROM_MODULE_ID="main" AND MESSAGE_ID="OnPageStart" AND TO_CLASS="Thurly\\\\Security\\\\HostRestriction"'))
			{
...
elseif($Step == 5)
{
	@unlink($_SERVER['DOCUMENT_ROOT'].'/thurlysetup.php');
	$ok = unlink($_SERVER["DOCUMENT_ROOT"]."/restore.php");

	if ($_REQUEST['dump_name'])
	{
		$ok = unlink($_SERVER["DOCUMENT_ROOT"]."/thurly/backup/".$_REQUEST["dump_name"]) && $ok;
		$ok = unlink($_SERVER["DOCUMENT_ROOT"]."/thurly/backup/".str_replace('.sql','_after_connect.sql',$_REQUEST["dump_name"])) && $ok;
	}

	if($_REQUEST['arc_name'] && strpos($_REQUEST['arc_name'],'thurly/') === false)
	{
	...
	foreach(array('cache','stack_cache','managed_cache') as $dir)
		DeleteDirRec($_SERVER['DOCUMENT_ROOT'].'/thurly/'.$dir);
...
class CDBRestore
{
	...
	public function __construct($DBHost, $DBName, $DBLogin, $DBPassword, $DBdump, $d_pos)
	{
		...
		$this->DBName = $DBName;
		$this->DBdump = $_SERVER["DOCUMENT_ROOT"]."/thurly/backup/".$DBdump;
		$this->d_pos = $d_pos;
	...
	function restore()
	{
		...
		{
			if (defined('VMTHURLY')) // избавимся от MyISAM
			{
		...
		if ($this->LocalCloud && $this->f_end)
		{
			...
			if (!file_exists($f = $_SERVER['DOCUMENT_ROOT'].'/upload'))
				mkdir($f);
			if (rename($_SERVER['DOCUMENT_ROOT'].'/thurly/backup/clouds', $_SERVER['DOCUMENT_ROOT'].'/upload/'.$name))
			{
...
{
	$arDump = array();
	if (is_dir($back_dir = $_SERVER["DOCUMENT_ROOT"]."/thurly/backup"))
	{
...
function html($ar)
{
	$isCrm = getenv('THURLY_ENV_TYPE') == 'crm';
?>
	<html>
	<head>
...
class CTar
{
	...
	const BX_SIGNATURE = 'Thurly Encrypted File';
	var $BufferSize;
	var $Buffer;
...
//						$this->Error('Wrong GZIP Extra Field');
	...
	{
		return md5('THURLYCLOUDSERVICE'.$key);
	}
...
class CTarRestore extends CTar
{
	function readHeader($Long = false)
	{
		...
		if (!$Long && is_array($header))
		{
			...
			elseif ($f == $dr.'thurlyphp_interfacedbconn.php' && file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/php_interface/dbconn.php'))
...
function img($name)
{
	if (file_exists($_SERVER['DOCUMENT_ROOT'].'/images/'.$name))
		return '/images/'.$name;
	return 'https://www.1c-thurly.ru/images/thurly_setup/'.$name;
}
...
<IfModule mod_rewrite.c>
	...
	RewriteCond %{REQUEST_FILENAME} !/thurly/urlrewrite.php$
	RewriteRule ^(.*)$ /thurly/urlrewrite.php [L]
