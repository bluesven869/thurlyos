...
if ($REQUEST_METHOD == "POST" && (strlen($save) > 0 || strlen($apply) > 0))
{
	...
	if (empty($arError))
	{
		...
		if ($arSmileSet['STRING_ID'] == 'thurly' || $_REQUEST["STRING_ID"] == 'thurly')
		{
...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/smile_gallery.php?lang=".LANG."&".GetFilterParams("filter_", false),
		"ICON" => "btn_list",
	)
);
...
if ($ID > 0)
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/smile_gallery_edit.php?lang=".LANG."&".GetFilterParams("filter_", false),
		"ICON" => "btn_new",
	);
	if($arSmileSet["STRING_ID"] != 'thurly')
	{
		$aMenu[] = array(
			"TEXT" => GetMessage("SMILE_BTN_DELETE"),
			"LINK" => "javascript:if(confirm('".GetMessage("SMILE_BTN_DELETE_CONFIRM")."')) window.location='/thurly/admin/smile_gallery.php?action=delete&ID[]=".$ID."&lang=".LANG."&".thurly_sessid_get()."#tb';",
			"ICON" => "btn_delete",
		);
	}
}
...
$tabControl->Buttons(array(
	"back_url" => "/thurly/admin/smile_gallery.php?lang=".LANG."&".GetFilterParams("filter_", false)));
?>
</form>
<?
$tabControl->End();
