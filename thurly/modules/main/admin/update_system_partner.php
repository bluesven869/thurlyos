...
$arMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/update_system_partner.php?refresh=Y&amp;lang=".LANGUAGE_ID."&amp;addmodule=".urlencode($myaddmodule),
		"ICON" => "btn_update_partner",
	),
	array("SEPARATOR" => "Y"),
	array(
		"TEXT" => GetMessage("SUP_SETTINGS"),
		"LINK" => "/thurly/admin/settings.php?lang=".LANGUAGE_ID."&mid=main&tabControl_active_tab=edit5&back_url_settings=%2Fthurly%2Fadmin%2Fupdate_system_partner.php%3Flang%3D".LANGUAGE_ID."",
	),
);
			...
			if ($arUpdateList)
			{
				...
				if ($arUpdateList !== false && isset($arUpdateList["REG"]) && count($arUpdateList["REG"]) > 0)
				{
					...
					function RegisterSystem()
					{
						...
						updRand++;
						CHttpRequest.Send('/thurly/admin/update_system_partner_act.php?query_type=register&<?= thurly_sessid_get() ?>&updRand=' + updRand);
					}
					//-->
					</SCRIPT>
					<?
				...
				function __InstallUpdates()
				{
					...
					updRand++;
					CHttpRequest.Send('/thurly/admin/update_system_partner_call.php?' + aStrParams + "&<?= thurly_sessid_get() ?>&query_type=" + param + "&updRand=" + updRand);
				}
							...
							if(modNewCount >= 1)
							{
								...
								for (i=0; i<modNewCount; i++)
								{
									if(document.getElementById('md_name_'+moduleNew[i]))
									{
										...
										updSuccessDivText.innerHTML += '<br />' + n +'&nbsp;&nbsp;<input type="button" onclick="window.open(\'/thurly/admin/partner_modules.php?lang=<?=LANGUAGE_ID?>&amp;id='+moduleNew[i]+'&amp;install=Y&amp;<?=thurly_sessid_get()?>\');" value="<?=GetMessageJS("SUP_SU_UPD_MP_NEW_INST")?>">';
									}
								}
									...
									<td>
										<table border="0" cellspacing="1" cellpadding="3">
											...
											<tr>
												...
												<td><b><?= md5("THURLY".CUpdateClientPartner::GetLicenseKey()."LICENCE"); ?></b></td>
											</tr>
											<tr>
			...
			if ($arUpdateList)
			{
				...
				<!--
					...
					function ShowLicence()
					{
						...
						txt += '<iframe name="license_text" src="//www.1c-thurly.ru/license.php?module='+name+'&free_module='+freeModule+'&updatesystem=Y" style="width:622px; height:410px; display:block;"></iframe>';
		...
		<td colspan="2">
			...
			if (!$bLockUpdateSystemKernel)
			{
				...
				function ActivateCoupon()
				{
					...
					{
						updRand++;
						CHttpRequest.Send('/thurly/admin/update_system_partner_act.php?query_type=coupon&<?= thurly_sessid_get() ?>&COUPON=' + escape(param) + "&updRand=" + updRand);
					}
					else
					{
						document.getElementById("id_coupon_btn").disabled = false;
