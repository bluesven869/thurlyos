...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/checklist.php");
IncludeModuleLangFile(__FILE__);

$APPLICATION->SetAdditionalCSS("/thurly/themes/.default/check-list-style.css");
...
if ((($res = CCheckListResult::GetList(Array(),Array("REPORT"=>"N"))->Fetch()) || ($_POST["bx_start_test"] == "Y") || $_REQUEST["ACTION"]) && check_thurly_sessid())
{
	...
	elseif ($_REQUEST["ACTION"] == "RESETTHURLYSTATUS")
	{
		$dbReport = CCheckListResult::GetList(Array(),Array("REPORT"=>"Y", "SENDED_TO_THURLY" => 'Y'));
		if ($arReport = $dbReport->Fetch())
		{
			CCheckListResult::Update($arReport['ID'], array('SENDED_TO_THURLY' => 'N'));
		}
		LocalRedirect($APPLICATION->GetCurPage()."?lang=".LANG,true);
	}
...
//		CCheckListResult::Update($report_id, $arFields);
	...
	elseif ($_REQUEST["ACTION"] == "ADDSENDREPORT")//add report and send to thurly
	{
		if (isset($_REQUEST['report_id']))
		{
			$report_id = intval($_REQUEST['report_id']);
			...
			if ($arReport = $dbReport->Fetch())
			{
				...
				CCheckListResult::Update($report_id, array('SENDED_TO_THURLY' => 'Y'));
...
?>
	...
	<script type="text/javascript">
			...
			Dialog = new BX.CAdminDialog(
				{
					...
					head: "",
					content_url: "/thurly/admin/checklist_detail.php?TEST_ID="+testID+"&lang=<?=LANG;?>&bxpublic=Y",
					icon: "head-block",
					resizable: false,
					draggable: true,
					height: "530",
		...
		function AutoTest(testID,callback,step)
		{
			...
			BX.ajax.post("/thurly/admin/checklist.php"+"?lang=<?=LANG;?>&bxpublic=Y&<?=thurly_sessid_get()?>",data,callback);
		}

		/*function SaveReport()
		{
			window.location = 'checklist.php?lang=<?=LANG?>&ACTION=ADDREPORT&<?=thurly_sessid_get()?>';
		}*/
	...
	function ShowHideReports ()
	{
		if (showHiddenReports == 'Y')
			showHiddenReports = 'N';
				...
				showHiddenReports = 'Y';
		window.location = 'checklist.php?lang=<?=LANG?>&ACTION=CHANGELISTPROP&showHiddenReports='+showHiddenReports+'&<?=thurly_sessid_get()?>';
	}
	...
	<script>
		...
		function RefreshReportStatuses ()
		{
			BX.ajax.get("https://www.1c-thurly.ru/buy_tmp/partner_check_key_for_qc.php?key=" + "<?=md5(trim(LICENSE_KEY))?>", {}, function (data) {
				...
				{
					window.location = 'checklist.php?lang=<?=LANG?>&ACTION=RESETTHURLYSTATUS'+'&<?=thurly_sessid_get()?>';
				}
			});
		}
		function ShowHint (el)
		...
		{
			window.location = 'checklist.php?lang=<?=LANG?>&ACTION=SHOWHIDEELEMENTS&report_id='+report_id+'&report_action=hide&<?=thurly_sessid_get()?>';
		}

		function showReport (report_id)
		{
			window.location = 'checklist.php?lang=<?=LANG?>&ACTION=SHOWHIDEELEMENTS&report_id='+report_id+'&report_action=show&<?=thurly_sessid_get()?>';
		}
	</script>
...
function ShowReportList()
{
	...
	<?
	$exists_sended_to_thurly = CCheckListResult::GetList(Array(),Array("SENDED_TO_THURLY"=>"Y"))->Fetch();
			...
			<table class="checklist-archive-table" cellspacing="0">
				...
				<?foreach ($arReports as $k=>$arReport):?>
					<tr class="">
						...
						<td><a class="checklist-archive-table-detail" href="/thurly/admin/checklist_report.php?ID=<?=$arReport["ID"];?>&lang=<?=LANG;?>"><?=GetMessage("CL_REPORT_TABLE_DETAIL");?></a></td>
						<td>
							<?if ($arReport["SENDED_TO_THURLY"] == 'N' && $k == 0) {?>
								<?if(!$exists_sended_to_thurly) {?>
									...
									<?}?>
								<?}?>
							<?} elseif ($arReport["SENDED_TO_THURLY"] == 'Y') {?>
	...
	<div>
		...
		<div>
			<?if($exists_sended_to_thurly && count($arReports)>0) {?>
...
function SendReportToThurly ($arFields)
{
	/** @global CMain $APPLICATION */
	global $APPLICATION;
