...
$filterOption = new Thurly\Main\UI\Filter\Options($sTableID);
$filterData = $filterOption->getFilter($filterFields);
if (!empty($filterData["FIND"]))
{
	$userQuery->setFilter(\Thurly\Main\UserUtils::getAdminSearchFilter(array("FIND" => $filterData["FIND"])));
}
...
foreach ($listRatingColumn as $ratingColumn)
{
	if (preg_match('/^RATING_(\d+)$/i', $ratingColumn, $matches))
	{
		...
		$userQuery->registerRuntimeField("RR".$ratingId, array(
			"data_type" => "Thurly\Main\Rating\ResultsTable",
			"reference" => array(
...
while ($userData = $result->fetch())
{
	...
	if($USER->CanDoOperation('edit_php'))
	{
		...
		$arActions[] = array(
			...
			"LINK" => "user_admin.php?lang=".LANGUAGE_ID."&ID=".$userId."&action=authorize&".thurly_sessid_get()
		);
	}
