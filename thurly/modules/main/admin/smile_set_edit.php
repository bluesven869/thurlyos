...
if ($_SERVER["REQUEST_METHOD"] == "POST" && (strlen($save) > 0 || strlen($apply) > 0))
{
	...
	if (empty($arError))
	{
		...
		if (in_array($arSmileSet["STRING_ID"], Array('thurly_main')))
		{
...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/smile_set.php?GALLERY_ID=".$arSmileSet['PARENT_ID']."&lang=".LANG."&".GetFilterParams("filter_", false),
		"ICON" => "btn_list",
	)
);
...
if ($ID > 0)
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/smile_set_edit.php?GALLERY_ID=".$arSmileSet['PARENT_ID']."&lang=".LANG."&".GetFilterParams("filter_", false),
		"ICON" => "btn_new",
	);
	if (!in_array($arSmileSet["STRING_ID"], Array('thurly_main')))
	{
		$aMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('".GetMessage("SMILE_BTN_DELETE_CONFIRM")."')) window.location='/thurly/admin/smile_set.php?GALLERY_ID=".$arSmileSet['PARENT_ID']."&action=delete&ID[]=".$ID."&lang=".LANG."&".thurly_sessid_get()."#tb';",
			"ICON" => "btn_delete",
		);
	}
}
...
$tabControl->Buttons(array(
	"back_url" => "/thurly/admin/smile_set.php?lang=".LANG."&".GetFilterParams("filter_", false)));
?>
</form>
<?
$tabControl->End();
