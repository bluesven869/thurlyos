...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if ($isAdmin)
	{
		...
		if ($startType == "POPUP")
			$arActions[] = array("DEFAULT" => "Y", "ICON"=>"install", "TEXT" => GetMessage("MAIN_WIZARD_ADMIN_INSTALL"), "ACTION"=>"WizardWindow.Open('".$f_ID."','".thurly_sessid()."')");
		else if ($startType == "WINDOW")
			$arActions[] = Array(
				"DEFAULT" => "Y", 
				"ICON"=>"install", 
				"TEXT" => GetMessage("MAIN_WIZARD_ADMIN_INSTALL"), 
				"ACTION"=>"window.open('wizard_install.php?lang=".LANGUAGE_ID."&wizardName=".$f_ID."&".thurly_sessid_get()."');"
			);
	}
...
{
	window.open("wizard_export.php?ID="+val+"&<?=thurly_sessid_get()?>");
}
//-->
</script>
