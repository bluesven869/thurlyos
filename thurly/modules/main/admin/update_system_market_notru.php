...
IncludeModuleLangFile("/thurly/modules/main/admin/update_system_market.php");
...
if ($_REQUEST["action"] == "load" && strlen($_REQUEST["id"]) > 0 && check_thurly_sessid())
{
	$errorMessage = "";
	...
	{
		LocalRedirect("/thurly/admin/module_admin.php?lang=".LANG."&id=".urlencode($_REQUEST["id"])."&".thurly_sessid_get()."&install=Y");
	}
}
...
while ($arResultItem = $dbResultList->Fetch())
{
	...
	if (!array_key_exists($arResultItem["ID"], $arCurrentModules))
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect('update_system_market.php?id='.$arResultItem["ID"].'&action=load&'.thurly_sessid_get()),
			"DEFAULT" => false
		);
	}
