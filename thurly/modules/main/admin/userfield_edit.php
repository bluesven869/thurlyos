...
if($_SERVER["REQUEST_METHOD"] == "POST" && ($_POST["save"] != "" || $_POST["apply"] != "") && ($RIGHTS >= "W") && check_thurly_sessid())
{
	$arFields = array(
	...
	{
		if($_POST["apply"] != "")
			LocalRedirect("/thurly/admin/userfield_edit.php?ID=".$ID."&lang=".LANG."&back_url=".urlencode($back_url)."&".$tabControl->ActiveTabParam());
		elseif($back_url)
			LocalRedirect($back_url);
		else
			LocalRedirect("/thurly/admin/userfield_admin.php?lang=".LANG);
	}
	else
	{
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"javascript:if(confirm('".GetMessage("USER_TYPE_DELETE_CONF")."'))window.location='userfield_admin.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
