	...
	$context = new CAdminContextMenu(
		array(
			array(
				...
				"LINK"	=> "/thurly/admin/type_admin.php?lang=".LANGUAGE_ID,
	...
	$context = new CAdminContextMenu(
		array(
			...
			array(
				...
				"LINK"	=> "/thurly/admin/type_edit.php?lang=".LANGUAGE_ID,
				"TITLE"	=> GetMessage("MAIN_NEW_RECORD_TITLE"),
				"ICON"	=> "btn_new"
			),
			array(
				"TEXT"	=> GetMessage("MAIN_DELETE_RECORD"),
				"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/thurly/admin/type_admin.php?ID=".urlencode($arParams["EVENT_NAME"])."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."&action=delete';",
...
?><tr>
	<td colspan="2">[<a href="/thurly/admin/message_edit.php?ID=<?=$v["ID"]?>"><?=$v["ID"]?></a>]<?=(strlen(trim($v["SUBJECT"])) > 0 ? " " : "").htmlspecialcharsEx($v["SUBJECT"])?>
	<?
	$arLID = array();
