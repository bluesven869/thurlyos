...
use Thurly\Main\Composite\Internals\AutomaticArea;
...
$APPLICATION->SetAdditionalCSS("/thurly/panel/main/composite.css");
...
)
{
	...
	LocalRedirect("/thurly/admin/composite.php?lang=".LANGUAGE_ID."&".$tabControl->ActiveTabParam());
}
	...
	<td>
		...
		<select name="composite_storage" id="composite_storage" style="width:300px;" onchange="onStorageSelect(this)">
			...
			foreach ($storages as $storageId => $storage):
				...
				if (isset($storage["module"]) && !\Thurly\Main\ModuleManager::isModuleInstalled($storage["module"]))
				{
					$disabled = " disabled";
...
<tr id="composite_cluster_hint_row" <?if ($compositeOptions["STORAGE"] !== "memcached_cluster") echo 'style="display:none"'?>>
	...
	<td><?=GetMessage("MAIN_COMPOSITE_CLUSTER_HINT", array(
			"#A_START#" => "<a href=\"/thurly/admin/cluster_memcache_list.php?lang=".LANGUAGE_ID."&group_id=".(defined("BX_CLUSTER_GROUP") ? BX_CLUSTER_GROUP : 1)."\">",
			"#A_END#" => "</a>"
		));?></td>
</tr>
...
<tr>
	<td></td>
	<td>
		<a href="/thurly/admin/cache.php?lang=<?=LANGUAGE_ID?>&cachetype=html&tabControl_active_tab=fedit2"><?=GetMessage("MAIN_COMPOSITE_CLEAR_CACHE")?></a>
	</td>
</tr>
<?
$tabControl->BeginNextTab();
...
<tr>
	<td>
		<img class="adm-composite-patent" src="/thurly/panel/main/images/composite/patent_composit.jpg" alt="">
	</td>
</tr>
