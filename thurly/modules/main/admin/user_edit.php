...
//authorize as user
if($_REQUEST["action"] == "authorize" && check_thurly_sessid() && $USER->CanDoOperation('edit_php'))
{
	$USER->Logout();
...
if($canViewUserList)
{
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/user_admin.php?lang=".LANGUAGE_ID."&apply_filter=Y",
		"ICON"	=> "btn_list",
...
if($USER->CanDoOperation('edit_php') && $ID != $USER->GetID())
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/user_edit.php?lang=".LANGUAGE_ID."&ID=".$ID."&action=authorize&".thurly_sessid_get()
	);
}
...
if($USER->CanDoOperation('edit_all_users') || $USER->CanDoOperation('edit_subordinate_users'))
{
	if ($ID>0 && $COPY_ID<=0)
	{
		...
		$aMenu[] = array(
			...
			"LINK"	=> "/thurly/admin/user_edit.php?lang=".LANGUAGE_ID,
			"ICON"	=> "btn_new",
			"TITLE"	=> GetMessage("MAIN_NEW_RECORD_TITLE"),
		);
		$aMenu[] = array(
			"TEXT"	=> GetMessage("MAIN_COPY_RECORD"),
			"LINK"	=> "/thurly/admin/user_edit.php?lang=".LANGUAGE_ID.htmlspecialcharsbx("&COPY_ID=").$ID,
			"ICON"	=> "btn_copy",
		...
		if ($ID!=1)
		{
			$aMenu[] = array(
				...
				"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/thurly/admin/user_admin.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
				"ICON"	=> "btn_delete",
...
<?
	...
	$languages = \Thurly\Main\Localization\LanguageTable::getList(array("filter" => array("ACTIVE" => "Y"), "order" => array("SORT" => "ASC", "NAME" => "ASC")));
	while($language = $languages->fetch())
	{
		$langOptions[$language["LID"]] = \Thurly\Main\Text\HtmlFilter::encode($language["NAME"]);
	}
		...
		<td colspan="2" align="center">
			...
			while ($arGroups = $dbGroups->Fetch())
			{
				...
				<tr>
					...
					<td class="align-left">
						<label for="GROUP_ID_ACT_ID_<?= $ind ?>"><?=htmlspecialcharsbx($arGroups["NAME"])?> [<a href="/thurly/admin/group_edit.php?ID=<?=$arGroups["ID"]?>&lang=<?=LANGUAGE_ID?>" title="<?=GetMessage("MAIN_VIEW_GROUP")?>"><?echo intval($arGroups["ID"])?></a>]</label>
					</td>
					<td>
