...
if(($arID = $lAdmin->GroupAction()) && $isAdmin && check_thurly_sessid())
{
...
$resultMessageByTypeDb = \Thurly\Main\Mail\Internal\EventMessageTable::getList(array('select' => array('ID', 'EVENT_NAME')));
...
{
	$runtimeList[] = new \Thurly\Main\Entity\ReferenceField(
		'MESSAGE',
		'Thurly\Main\Mail\Internal\EventMessageTable',
		array('=this.EVENT_NAME' => 'ref.EVENT_NAME')
	);
	$arFilter['MESSAGE.ID'] = $arFilter['MESSAGE_ID'];
	unset($arFilter['MESSAGE_ID']);
}
$resultDb = \Thurly\Main\Mail\Internal\EventTypeTable::getList(array(
	'filter' => $arFilter,
	'runtime' => $runtimeList,
...
$nav = new \Thurly\Main\UI\AdminPageNavigation("nav-event-type");
