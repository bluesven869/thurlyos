...
define('DEBUG_FLAG', str_replace('\\','/',$_SERVER['DOCUMENT_ROOT'] . '/thurly/site_checker_debug'));
require($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/classes/general/site_checker.php');
...
if ($_REQUEST['unique_id'])
{
	...
	switch ($_GET['test_type'])
	{
		...
		break;
		case 'fast_download':
			header('X-Accel-Redirect: /thurly/tmp/success.txt');
		break;
		case 'dbconn_test':
			ob_start();
			define('NOT_CHECK_PERMISSIONS', true);
		...
		case 'upload_test':
			...
			$dir = $_SERVER['DOCUMENT_ROOT'].'/thurly/tmp';
			if (!file_exists($dir))
				mkdir($dir);
	...
	if ($fix_mode = intval($_GET['fix_mode']))
	{
		...
		include_once($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/lang/'.LANGUAGE_ID.'/admin/site_checker.php');
		InitPureDB();
...
{
	if (defined('NOT_CHECK_PERMISSIONS') && NOT_CHECK_PERMISSIONS ===true || check_thurly_sessid())
	{
		$ob = new CSearchFiles;
		$ob->TimeLimit = 10;
		...
		elseif($check_type == 'kernel')
			$path = $_SERVER['DOCUMENT_ROOT'].'/thurly';
		...
		echo '<h1>Permission denied: THURLY SESSID ERROR</h1>';
	exit;
}
...
elseif ($fix_mode = intval($_REQUEST['fix_mode']))
{
	...
	<script>
		...
		BX.ajax.get('site_checker.php?fix_mode=' + fix_mode + '&test_start=Y&lang=<?=LANGUAGE_ID?>&charset=<?=LANG_CHARSET?>&<?=thurly_sessid_get()?>&unique_id=<?=checker_get_unique_id()?>', fix_onload);
	</script>
	<?
	exit;
}
...
?>
	...
	<style>
		.sc_help_link {
			background: url("/thurly/themes/.default/icons/status_icons.png") no-repeat scroll -12px -235px transparent;
			cursor:pointer;
			float:right;
			width:25px;
			height:25px;
		...
		.sc_icon_success {
			background: url("/thurly/themes/.default/icons/status_icons.png") no-repeat scroll -14px -19px transparent;
		}

		.sc_icon_warning{
			background: url("/thurly/themes/.default/icons/status_icons.png") no-repeat scroll -12px -212px transparent;
		}

		.sc_icon_error{
			background: url("/thurly/themes/.default/icons/status_icons.png") no-repeat scroll -12px -73px transparent;
		}
	...
	<script>
		...
		function show_popup(title, link, confirm_text)
		{
			...
			var d = new BX.CAdminDialog({
				'title': title,
				'content_url': '/thurly/admin/site_checker.php' + link,
				'draggable': true,
				'resizable': true,
		...
		function help_me(title, id)
		{
			...
			html += '<br><br><?=CUtil::JSEscape(GetMessage('SC_READ_MORE_ANC'))?>'.replace('#LINK#', '/thurly/admin/site_checker.php?lang=<?=LANGUAGE_ID?>&read_log=Y&anchor=' + id + '#' + id);
			html += '</div>';
		...
		function fix_onload(result)
		{
			...
			try
			{
				...
				if (strNextRequest)
					BX.ajax.get('site_checker.php?fix_mode=' + fix_mode + '&test_start=Y&lang=<?=LANGUAGE_ID?>&charset=<?=LANG_CHARSET?>&<?=thurly_sessid_get()?>&unique_id=<?=checker_get_unique_id()?>' + strNextRequest, fix_onload);
			}
			catch(e)
			{
				console.log(e);
		...
		function set_start(val)
		{
			...
			if (val)
			{
				...
				BX.ajax.get('site_checker.php?test_start=Y&lang=<?=LANGUAGE_ID?>&<?=thurly_sessid_get()?>', test_onload);
			}
			else
				CloseWaitWindow();
		}
		...
		function test_onload(result)
		{
			...
			if (document.getElementById('test_start').disabled) // Stop was not pressed
			{
				...
				if (strNextRequest)
				{
					...
					{
						?>
						BX.ajax.get('site_checker.php?HTTP_HOST=<?=urlencode($_GET['HTTP_HOST'])?>&SERVER_PORT=<?=urlencode($_GET['SERVER_PORT'])?>&HTTPS=<?=urlencode($_GET['HTTPS'])?>&test_start=Y&lang=<?=LANGUAGE_ID?>&<?=thurly_sessid_get()?>' + strNextRequest, test_onload);
						<?
					}
					else
					{
						?>
						BX.ajax.get('site_checker.php?HTTP_HOST=' + window.location.hostname + '&SERVER_PORT=' + window.location.port + '&HTTPS=' + (window.location.protocol == 'https:' ? 'on' : '') + '&test_start=Y&lang=<?=LANGUAGE_ID?>&<?=thurly_sessid_get()?>' + strNextRequest, test_onload);
						<?
					}?>
				}
				else // Finish
		...
		function ExpressTest(result, begin)
		{
			...
			if (strNextRequest || begin)
				BX.ajax.get('site_checker.php?test_start=Y&fast=1&lang=<?=LANGUAGE_ID?>&<?=thurly_sessid_get()?>&HTTP_HOST=' + HTTP_HOST + '&SERVER_PORT=' + SERVER_PORT + '&HTTPS=' + HTTPS + strNextRequest, ExpressTest);
			else
			{
...
<input type="hidden" name="license_key" value="<?=(LICENSE_KEY == "DEMO"? "DEMO" : md5("THURLY".LICENSE_KEY."LICENCE"))?>">
