...
if($_SERVER["REQUEST_METHOD"]=="POST" && ($_POST["save"] <> '' || $_POST["apply"] <> '') && $USER->CanDoOperation('edit_tasks') && check_thurly_sessid())
{
	$aMsg = Array();
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/thurly/admin/task_admin.php?lang=".LANGUAGE_ID."&amp;set_default=Y",
		"ICON"	=> "btn_list"
	)
);
...
if($ID > 0 && $COPY_ID <= 0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/task_edit.php?lang=".LANGUAGE_ID,
		"ICON"	=> "btn_new"
	);
	$aMenu[] = array(
		"TEXT"	=> GetMessage("MAIN_COPY_RECORD"),
		"TITLE"	=> GetMessage("MAIN_COPY_RECORD_TITLE"),
		"LINK"	=> "/thurly/admin/task_edit.php?lang=".LANGUAGE_ID."&amp;COPY_ID=".$ID,
		"ICON"	=> "btn_copy"
	);
	if ($USER->CanDoOperation('edit_tasks') && $str_SYS != 'Y')
	{
		$aMenu[] = array(
			...
			"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_DELETE_RECORD_CONF")."')) window.location='/thurly/admin/task_admin.php?del_id=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
			"ICON"	=> "btn_delete"
		);
	}
}
