...
if($USER->CanDoOperation('view_other_settings') || $USER->CanDoOperation('manage_short_uri') || $USER->CanDoOperation('lpa_template_edit') || $USER->CanDoOperation('edit_own_profile') || $USER->CanDoOperation('cache_control'))
{
	...
	if($USER->CanDoOperation('view_other_settings'))
	{
		...
		{
			if($adminMenu->IsSectionActive("menu_module_settings") || ($APPLICATION->GetCurPage() == "/thurly/admin/settings.php" && $_REQUEST["mid_menu"]<>"") || defined('BX_SEARCH_ADMIN') && BX_SEARCH_ADMIN === true)
			{
				$adminPage->Init();
...
if ($USER->CanDoOperation("view_other_settings") && \Thurly\Main\Analytics\SiteSpeed::isRussianSiteManager())
{
	AddEventHandler("main", "OnBuildGlobalMenu", array("\\Thurly\\Main\\Analytics\\SiteSpeed", "onBuildGlobalMenu"));
}
