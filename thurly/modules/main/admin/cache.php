...
)
{
	...
	if(!check_thurly_sessid() || !$isAdmin)
	{
		?>
		<script>
			window.location = '/thurly/admin/cache.php?lang=<?echo LANGUAGE_ID?>&tabControl_active_tab=fedit2';
		</script>
		<?
	...
	if(
		...
		|| \Thurly\Main\Data\Cache::getCacheEngineType() == "cacheenginefiles"
	)
	{
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/cache_files_cleaner.php");
	...
	if($_REQUEST["cachetype"] === "html")
	{
		...
		\Thurly\Main\Composite\Helper::updateCacheFileSize(-$space_freed);
	}
	elseif(\Thurly\Main\Data\Cache::getCacheEngineType() == "cacheenginefiles")
	{
		$obCacheCleaner->Start();
		while($file = $obCacheCleaner->GetNextFile())
		{
...
if ($REQUEST_METHOD=="POST" && ($cache_on=="Y" || $cache_on=="N") && check_thurly_sessid() && $isAdmin)
{
...
if($REQUEST_METHOD=="POST" && ($managed_cache_on=="Y" || $managed_cache_on=="N") && check_thurly_sessid() && $isAdmin)
{
