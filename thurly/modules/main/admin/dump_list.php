...
$bThurlyCloud = $bMcrypt && CModule::IncludeModule('thurlycloud') && CModule::IncludeModule('clouds');
...
if ($_REQUEST['action'])
{
	...
	if ($_REQUEST['action'] == 'download')
	{
		...
		echo "links=".\Thurly\Main\Web\Json::encode($arLink).";";
		die();
	}
	...
	elseif ($_REQUEST['action'] == 'restore')
	{
		...
		else
		{
			...
			if ($BUCKET_ID == -1)
					$url = 'thurlycloud_backup='.htmlspecialcharsbx(basename($name));
			elseif ($BUCKET_ID > 0)
			{
				if (CModule::IncludeModule('clouds'))
				{
	...
	try
	{
		foreach($backup->listFiles() as $ar)
		{
			$arTmpFiles[] = array(
				...
				'BUCKET_ID' => -1,
				'PLACE' => GetMessage('DUMP_MAIN_THURLY_CLOUD')
			);
		}
	}
	catch (Exception $e)
...
while($f = $rsDirContent->NavNext(true, "f_"))
{
	...
	if (defined('DUMP_DEBUG_MODE'))
	{
		$arActions[] = array(
			"ICON" => "clouds",
			"TEXT" => 'DEBUG - '.GetMessage("MAIN_DUMP_SEND_CLOUD").' Thurly',
			"ACTION" => "if(k=prompt('".CUtil::JSEscape(GetMessage("MAIN_DUMP_SEND_FILE_CLOUD"))."?')) document.location=\"/thurly/admin/dump.php?f_id=".urlencode($f['NAME'])."&action=cloud_send&dump_bucket_id=-1&".thurly_sessid_get().'&dump_encrypt_key="+k;'
		);
		$arActions[] = array(
			"ICON" => "archive",
			...
			"ACTION" => 
			strpos($f['NAME'], '.enc.') ?
			"if(k=prompt('".CUtil::JSEscape(GetMessage("INTEGRITY_CHECK"))."?')) document.location=\"/thurly/admin/dump.php?f_id=".urlencode($f['NAME'])."&action=check_archive&".thurly_sessid_get().'&dump_encrypt_key="+k;'
			:
			"if(confirm('".CUtil::JSEscape(GetMessage("INTEGRITY_CHECK"))."?')) document.location=\"/thurly/admin/dump.php?f_id=".urlencode($f['NAME'])."&action=check_archive&".thurly_sessid_get().'";'
		);
	}
	...
	if (!preg_match('#\.sql$#i',$f['NAME']))
	{
		if ($BUCKET_ID != -1)
		{
			$arActions[] = array(
				...
				"ACTION" => "PartList('/thurly/admin/dump_list.php?action=download&f_id=".$f['NAME']."&BUCKET_ID=".$BUCKET_ID."&".thurly_sessid_get()."')"
			);
			$arActions[] = array(
				"ICON" => "link",
				"TEXT" => GetMessage("MAIN_DUMP_GET_LINK"),
				"ACTION" => "AjaxSend('/thurly/admin/dump_list.php?action=link&f_id=".$f['NAME']."&BUCKET_ID=".$BUCKET_ID."&".thurly_sessid_get()."')"
			);
		}
		...
		$arActions[] = array(
			...
			"ACTION" => "if(confirm('".CUtil::JSEscape(GetMessage("MAIN_RIGHT_CONFIRM_EXECUTE"))."')) AjaxSend('/thurly/admin/dump_list.php?action=restore&f_id=".$f['NAME']."&BUCKET_ID=".$BUCKET_ID."&".thurly_sessid_get()."')"
		);
		...
		if ($BUCKET_ID == 0)
		{
			if ($arWriteBucket)
			{
				...
				foreach($arWriteBucket as $arBucket)
					$arActions[] = array(
						...
						"ACTION" => "if(confirm('".CUtil::JSEscape(GetMessage("MAIN_DUMP_SEND_FILE_CLOUD"))."?')) ".$lAdmin->ActionRedirect("/thurly/admin/dump.php?f_id=".urlencode($f['NAME'])."&action=cloud_send&dump_bucket_id=".$arBucket['ID']."&".thurly_sessid_get())
					);
			}
			...
			$arActions[] = array(
				...
				"ACTION" => "if(name=prompt('".CUtil::JSEscape(GetMessage("MAIN_DUMP_ARC_NAME_W_O_EXT"))."','".htmlspecialcharsbx($arName['name'])."')) tbl_dump.GetAdminList('/thurly/admin/dump_list.php?ID=".urlencode($f['NAME'])."&action=rename&lang=".LANGUAGE_ID."&".thurly_sessid_get()."&BUCKET_ID=".$BUCKET_ID."&name='+name);"
			);
		}
	}
