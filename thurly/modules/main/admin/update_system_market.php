...
if(!in_array(LANGUAGE_ID, array("ru", "ua", "bg")))
{
	...
	include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/admin/update_system_market_notru.php");
}
else
{
	$m = array();
	...
	$APPLICATION->SetAdditionalCSS("/thurly/panel/main/marketplace.css");
				...
				if(count($arModules) > 0)
				{
					...
					foreach($arModules as $Item)
					{
						...
						if($USER->CanDoOperation('install_updates'))
							...
							$arM["urlInstall"] = $arM["url"]."&instadm=Y&".thurly_sessid_get();
														...
														<div id="scrollable" class="scrollable" style="visibility: visible; overflow: hidden; position: relative; z-index: 2; left: 0px; width: 735px;">
															<ul style="margin: 0pt; padding: 0pt; position: relative; list-style-type: none; z-index: 1; width: 1470px; left: -490px;">
																<?foreach($arM["moreItems"]["item"] as $moreItem)
																{
																	...
																	$moreItem["urlClick"] = str_replace("#module#", $moreItem["code"], $sTableID.".GetAdminList('/thurly/admin/update_system_market.php?module=#module#&lang=".LANGUAGE_ID."&".thurly_sessid_get()."&table_id=".$sTableID.((intval($category) > 0) ? "&category=".$category : "")."'); return false;");
	...
	if($_REQUEST["instadm"] == "Y" && check_thurly_sessid() && strlen($moduleCode) > 0 && !empty($arM))
	{
