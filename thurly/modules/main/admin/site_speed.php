...
$APPLICATION->SetAdditionalCSS("/thurly/panel/main/site_speed.css");
	...
	<div class="site-speed-analytics" id="site-speed-analytics">
		...
		<div class="site-speed-indicator-block" id="site-speed-indicator-block">
			...
			<div class="site-speed-perf" id="site-speed-perf">
				<?
				if (\Thurly\Main\ModuleManager::isModuleInstalled("perfmon")):
					$mark = (double)COption::GetOptionString("perfmon", "mark_php_page_rate", "");
				?>
					<a href="/thurly/admin/perfmon_panel.php?lang=<?=LANGUAGE_ID?>" class="site-speed-perf-label"><?=Loc::getMessage("MAIN_SITE_SPEED_PERF")?></a>:<span class="site-speed-perf-value"><?if ($mark > 0):?><?=$mark?><?else:?><?=Loc::getMessage("MAIN_SITE_SPEED_PERF_NO_RES")?><?endif?></span>
				<?endif?>

				<?
					$compositeStatus = \Thurly\Main\Composite\Helper::isCompositeEnabled() ? Loc::getMessage("MAIN_SITE_SPEED_ENABLED") : Loc::getMessage("MAIN_SITE_SPEED_DISABLED");
				?>
				<a href="/thurly/admin/composite.php?lang=<?=LANGUAGE_ID?>" class="site-speed-perf-label"><?=Loc::getMessage("MAIN_SITE_SPEED_COMPOSITE_SITE")?></a>:<span class="site-speed-perf-value"><?=$compositeStatus?></span>
				<?

				if (\Thurly\Main\Loader::includeModule("thurlycloud")):
					$cdnStatus = CThurlyCloudCDN::IsActive() ? Loc::getMessage("MAIN_SITE_SPEED_ENABLED") : Loc::getMessage("MAIN_SITE_SPEED_DISABLED");
				?>
				<a href="/thurly/admin/thurlycloud_cdn.php?lang=<?=LANGUAGE_ID?>" class="site-speed-perf-label"><?=Loc::getMessage("MAIN_SITE_SPEED_CDN")?></a>:<span class="site-speed-perf-value"><?=$cdnStatus?></span>
				<?endif?>
			</div>
		</div>
		...
		<div class="site-speed-indicator-block">
			...
			<iframe id="site-speed-clients-map" scrolling="no" width="100%" height="400"
				...
				name="<?=htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($mapIframeLang))?>"
			></iframe>
		</div>
...
(function() {
	...
	function drawStat(host)
	{
		...
		BX('site-speed-clients-map').src = "https://analytics.thurly.info/cstats/v1_0/maps_client/index.php?"
