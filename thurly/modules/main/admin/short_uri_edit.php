...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $isAdmin && check_thurly_sessid())
{
	$arFields = Array(
		"URI" => $URI,
		"SHORT_URI" => $SHORT_URI,
	...
	{
		if($apply!="")
			LocalRedirect("/thurly/admin/short_uri_edit.php?ID=".$ID."&mess=ok&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
			LocalRedirect("/thurly/admin/short_uri_admin.php?lang=".LANG);
	}
	else
	{
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"javascript:if(confirm('".GetMessage("SU_EF_mnu_del_conf")."'))window.location='short_uri_admin.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
