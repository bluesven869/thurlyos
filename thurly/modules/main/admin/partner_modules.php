...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/update_client_partner.php");
...
if($isAdmin && $_SERVER["REQUEST_METHOD"] == "POST" && strlen($_POST["module"]) > 0 && check_thurly_sessid())
{
	...
	if(strlen($moduleId) > 0)
	{
		...
		elseif($_POST["act"] == "add_opinion")
		{
			$arF = Array(
					...
					"lkey" => md5("THURLY".CUpdateClientPartner::GetLicenseKey()."LICENCE"),
...
if(LANGUAGE_ID == "ru")
{
	...
	$linkToBuyUpdate = "https://marketplace.1c-thurly.ru"."/tobasket.php?ID=#CODE#&lckey=".md5("THURLY".CUpdateClientPartner::GetLicenseKey()."LICENCE");
}
...
if((strlen($_REQUEST["uninstall"])>0 || strlen($_REQUEST["install"])>0 || strlen($_REQUEST["clear"])>0) && $isAdmin && !$fb && check_thurly_sessid())
{
...
while($info = $rsData->Fetch())
{
	...
	if(!empty($modules[$info["MODULE_ID"]]["VERSION"]))
		$status .= "<br /><a href=\"/thurly/admin/update_system_partner.php?tabControl_active_tab=tab2&addmodule=".$info["MODULE_ID"]."\" style=\"color:green;\">".GetMessage("MOD_SMP_NEW_UPDATES")."</a>";
	...
	if(!empty($modules[$info["MODULE_ID"]]) && !empty($modules[$info["MODULE_ID"]]["VERSION"]))
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect("/thurly/admin/update_system_partner.php?tabControl_active_tab=tab2&addmodule=".$info["MODULE_ID"]),
		);
	}
	...
	if($info["IsInstalled"])
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?id=".htmlspecialcharsbx($info["MODULE_ID"])."&lang=".LANG."&uninstall=Y&".thurly_sessid_get()),
		);
	}
	else
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?id=".htmlspecialcharsbx($info["MODULE_ID"])."&lang=".LANG."&install=Y&".thurly_sessid_get()),
		);
		$arActions[] = array(
			"ICON" => "delete",
			"DEFAULT" => false,
			"TEXT" => GetMessage("MOD_SMP_DELETE"),
			"ACTION" => "if(confirm('".GetMessageJS('MOD_CLEAR_CONFIRM', Array("#NAME#" => htmlspecialcharsbx($info["MODULE_NAME"])))."')) ".$lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?id=".htmlspecialcharsbx($info["MODULE_ID"])."&lang=".LANG."&clear=Y&".thurly_sessid_get()),
		);
	}
...
while($info = $rsData->Fetch())
{
	...
	if($info["UPDATE_END"] != "Y")
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin1->ActionRedirect("/thurly/admin/update_system_partner.php?tabControl_active_tab=tab2&addmodule=".$info["MODULE_ID"]),
		);
	}
