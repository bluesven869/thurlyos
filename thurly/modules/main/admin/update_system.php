...
$APPLICATION->SetAdditionalCSS("/thurly/themes/".ADMIN_THEME_ID."/sysupdate.css");
...
$arMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/update_system.php?refresh=Y&lang=".LANGUAGE_ID,
		"ICON"=>"btn_update",
	),
	array("SEPARATOR" => "Y"),
	/*array(
		"TEXT" => GetMessage("SUP_CHECK_UPDATES_PARTNER"),
		"LINK" => "/thurly/admin/update_system_partner.php?refresh=Y&lang=".LANGUAGE_ID,
		"ICON"=>"btn_update",
	),
	array("SEPARATOR" => "Y"),*/
	array(
		"TEXT" => GetMessage("SUP_SETTINGS"),
		"LINK" => "/thurly/admin/settings.php?lang=".LANGUAGE_ID."&mid=main&tabControl_active_tab=edit5&back_url_settings=%2Fthurly%2Fadmin%2Fupdate_system.php%3Flang%3D".LANGUAGE_ID."",
	),
	array("SEPARATOR" => "Y"),
	array(
		"TEXT" => GetMessage("SUP_HISTORY"),
		"LINK" => "/thurly/admin/sysupdate_log.php?lang=".LANGUAGE_ID,
		"ICON"=>"btn_update_log",
	)
);
		...
		if ($arUpdateList = CUpdateClient::GetUpdatesList($errorMessage, LANG, $stableVersionsOnly))
		{
			...
			if (isset($arUpdateList["REPAIR"]))
			{
				if ($refreshStep < 5)
				{
					...
					LocalRedirect("/thurly/admin/update_system.php?refresh=Y&refresh_step=".$refreshStep."&lang=".LANGUAGE_ID);
				}
				else
				{
...
{
	$errorMessage .= "<br>".GetMessage("SUP_HTTP_BX_MASTER", array("#ADDR#" => "http://".$_SERVER["SERVER_ADDR"].":8890/thurly/admin/update_system.php"));
}
...
if ($DB->TableExists('b_sale_order') || $DB->TableExists('B_SALE_ORDER'))
{
	if (COption::GetOptionString("main", "~sale_converted_15", "N") != "Y")
	{
		if (isset($arClientModules["sale"])
			...
			$systemMessage .= GetMessage("SUP_SALE_1500_HINT", array("#ADDR#" => "/thurly/admin/sale_converter.php?lang=".LANG));
	}
}
			...
			if ($arUpdateList)
			{
				...
				if ($newLicenceSigned != "Y")
				{
					...
					function AgreeLicence()
					{
						...
						updRand++;
						CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=licence&<?= thurly_sessid_get() ?>&updRand=' + updRand);
					}
				...
				if ($bLicenseNotFound  || (defined("DEMO") && DEMO == "Y" && !$bFullVersion))
				{
					...
					<div id="upd_key_div">
						<table border="0" cellspacing="1" cellpadding="3" width="100%" class="internal">
							...
							<?if($bLicenseNotFound):?>
									...
									<a href="http://<?= ((LANGUAGE_ID == "ru") ? "www.thurlysoft.ru" : "www.thurlysoft.com") ?>/bsm_register.php" target="_blank"><?= GetMessage("SUP_SUBK_GET_KEY") ?></a>
							<?else:?>
					...
					function LicenceKeyFormSubmit()
					{
						...
						CHttpRequest.Action = function(result)
						{
							...
							if (result == "Y")
							{
								window.location.href = "/thurly/admin/update_system.php?lang=<?= LANG ?>";
						...
						updRand++;
						CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=key&<?= thurly_sessid_get() ?>&NEW_LICENSE_KEY=' + escape(document.licence_key_form.NEW_LICENSE_KEY.value) + "&updRand=" + updRand);
					}
					//-->
					</SCRIPT>
					<?
				...
				if (!$bLicenseNotFound)
				{
					if (isset($arUpdateList["CLIENT"]) && !isset($arUpdateList["UPDATE_SYSTEM"]) && count($arUpdateList["CLIENT"]) > 0 && $arUpdateList["CLIENT"][0]["@"]["RESERVED"] == "Y")
					{
						...
						function ActivateFormSubmit()
						{
							...
							var bEr = false;
							var erImg = '<img src="/thurly/themes/.default/images/icon_warn.gif" width="20" height="20" alt="Error" title="Error" align="left" />';
							...
							if(bEr)
							{
								...
								CloseWaitWindow();
								document.getElementById('errorDiv').innerHTML = '<table style="color:red;"><tr><td><img src="/thurly/themes/.default/images/icon_error.gif" width="32" height="32" alt="Error" title="Error" align="left" valign="center"/></td><td><b><?=GetMessage("SUP_SUBA_CONFIRM_ERROR")?></b></td></tr></table>';
							...
							else
							{
								...
								CHttpRequest.Action = function(result)
								{
									...
									{
										document.getElementById("id_activate_form_button").disabled = false;
										document.getElementById('errorDiv').innerHTML = '<table style="color:red;"><tr><td><img src="/thurly/themes/.default/images/icon_error.gif" width="32" height="32" alt="Error" title="Error" align="left" valign="center"/></td><td><b>'+result+'</b></td></tr></table>';
										document.getElementById('errorDiv').style.border = "1px solid red";
								...
								updRand++;
								CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=activate&<?= thurly_sessid_get() ?>&' + param + "&updRand=" + updRand);
								return true;
...
/*
						...
						function ActivateFormSubmit()
						{
							...
							CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=activate&<?= thurly_sessid_get() ?>&' + param);
						}
*/
					...
					else
					{
						if ($arUpdateList !== false && isset($arUpdateList["UPDATE_SYSTEM"]))
						{
							...
							function UpdateUpdate()
							{
								...
								updRand++;
								CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=updateupdate&<?= thurly_sessid_get() ?>&updRand=' + updRand);
							}
							//-->
							</SCRIPT>
							<?
					...
					&& ($arUpdateList["CLIENT"][0]["@"]["ENC_TYPE"] == "F" || $arUpdateList["CLIENT"][0]["@"]["ENC_TYPE"] == "E"))
				{
					...
					function RegisterSystem()
					{
						...
						updRand++;
						CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=register&<?= thurly_sessid_get() ?>&updRand=' + updRand);
					}
					//-->
					</SCRIPT>
					<?
					...
					&& ($arUpdateList["CLIENT"][0]["@"]["ENC_TYPE"] == "F"))
				{
					...
					function __LoadSources()
					{
						...
						{
							updRand++;
							CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=sources&<?= thurly_sessid_get() ?>&requested_modules=' + requestedModules + "&updRand=" + updRand);
						}
						else
						{
				...
				if ($arUpdateList !== false && (isset($_REQUEST["BX_SUPPORT_MODE"]) && ($_REQUEST["BX_SUPPORT_MODE"] == "Y")) && isset($arUpdateList["CLIENT"]) && !isset($arUpdateList["UPDATE_SYSTEM"]))
				{
					...
					function __LoadSupport()
					{
						...
						{
							updRand++;
							CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=support_full_load&<?= thurly_sessid_get() ?>&requested_modules=' + requestedModules + "&updRand=" + updRand);
						}
						else
						{
				...
				function __InstallUpdates()
				{
					...
					updRand++;
					CHttpRequest.Send('/thurly/admin/update_system_call.php?' + aStrParams + "&<?= thurly_sessid_get() ?>&query_type=" + param + "&updRand=" + updRand);
				}
		...
		<td colspan="2">
			...
			if (!$bLockUpdateSystemKernel)
			{
				...
				function ActivateCoupon()
				{
					...
					{
						updRand++;
						CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=coupon&<?= thurly_sessid_get() ?>&COUPON=' + escape(param) + "&updRand=" + updRand);
					}
					else
					{
						document.getElementById("id_coupon_btn").disabled = false;
		...
		<td colspan="2">
			...
			if (!$bLockUpdateSystemKernel)
			{
				...
				function SwithStability()
				{
					...
					updRand++;
					CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=stability&<?= thurly_sessid_get() ?>&STABILITY=' + escape("<?= $stableVersionsOnly ?>") + "&updRand=" + updRand);
				}
				//-->
				</SCRIPT>
				...
				function SubscribeMail()
				{
					...
					{
						updRand++;
						CHttpRequest.Send('/thurly/admin/update_system_act.php?query_type=mail&<?= thurly_sessid_get() ?>&EMAIL=' + escape(param) + "&updRand=" + updRand);
					}
					else
					{
						CloseWaitWindow();
