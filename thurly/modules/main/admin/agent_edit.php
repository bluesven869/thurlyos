...
if($REQUEST_METHOD=="POST" && (strlen($save)>0 || strlen($apply)>0) && $isAdmin && check_thurly_sessid())
{
	...
	{
		if(strlen($save) > 0)
			LocalRedirect("/thurly/admin/agent_list.php");
		elseif(strlen($apply) > 0)
			LocalRedirect("/thurly/admin/agent_edit.php?&ID=".$ID."&".$editTab->ActiveTabParam());
	}
}
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/thurly/admin/agent_list.php?lang=".LANG,
		"ICON"	=> "btn_list",
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/agent_edit.php?lang=".LANGUAGE_ID,
		"ICON"	=> "btn_new",
		"TITLE"	=> GetMessage("MAIN_AGENT_NEW_RECORD_TITLE"),
	);
	$aMenu[] = array(
		"TEXT"	=> GetMessage("MAIN_AGENT_DEL_RECORD_TITLE"),
		"LINK"	=> "javascript:if(confirm('".GetMessage("MAIN_AGENT_DELETE_RECORD_CONF")."')) window.location='/thurly/admin/agent_list.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		"ICON"	=> "btn_delete",
