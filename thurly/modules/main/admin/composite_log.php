...
use Thurly\Main\Composite\Debug\Model\LogTable;
use Thurly\Main\Config\Option;
use Thurly\Main\Context;
use Thurly\Main\Localization\Loc;
use Thurly\Main\UI\AdminPageNavigation;
...
if ($request->getPost("debug_form") && check_thurly_sessid())
{
...
if ($request->get("action") === "clear_all" && check_thurly_sessid())
{
	LogTable::deleteAll();
	localRedirect($APPLICATION->getCurPage()."?lang=".LANGUAGE_ID);
}
...
$toolbar = array(
	array(
		...
		"LINK" => $APPLICATION->getCurPage()."?lang=".LANGUAGE_ID."&action=clear_all&".thurly_sessid_get(),
...
<form method="POST" action="<?=$APPLICATION->getCurPage()?>" name="enable_debug_form">
	...
	<div style="margin: 10px 0 0 1px;">
		...
		<?=Loc::getMessage("MAIN_COMPOSITE_LOG_PLUGIN_AD", array(
			"#LINK_START#" =>
				'<a href="https://chrome.google.com/webstore/detail/thurly-composite-notifier/'.
