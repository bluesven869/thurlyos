...
BX.admin.panel = {
	...
	__view_mode_toggle: function(e)
	{
		...
		if (this1.className=='bx-panel-toggle-on')
		{
			...
			this1.href = this1.href.replace('thurly_include_areas=N', 'thurly_include_areas=Y');
		}
		else
		{
			...
			this1.href = this1.href.replace('thurly_include_areas=Y', 'thurly_include_areas=N');
		}
...
BX.admin.toggle = {
	...
	_changePosition : function(on)
	{
		...
		if (BX.admin.dynamic_mode)
		{
			...
			{
				BX.admin.dynamic_mode_show_borders = true;
				this.toggle.href = this.toggle.href.replace('thurly_include_areas=Y', 'thurly_include_areas=N');
			}
			else
			{
				BX.admin.dynamic_mode_show_borders = false;
				this.toggle.href = this.toggle.href.replace('thurly_include_areas=N', 'thurly_include_areas=Y');
			}
...
{
	BX.ajax.get('/thurly/admin/get_start_menu.php', {
		mode: 'save_recent',
		url: itemInfo['LINK'],
		text: itemInfo['TEXT'],
...
{
	window.location.href = '/thurly/admin/favorite_edit.php?lang='+BX.message('LANGUAGE_ID')+'&name='+BX.util.urlencode(document.title)+'&addurl='+BX.util.urlencode(window.location.href)+'&encoded=Y' + (!!back_url ? '&back_url_pub=' + BX.util.urlencode(back_url) : '');
}
