;(function(window){
	...
	BX.Uploader = function(params)
	{
		...
		else
		{
			...
			this.controlID = this.controlId = (params["controlId"] || "thurlyUploader");
	...
	BX.Uploader.prototype = {
		...
		preparePost : function(data, prepareForm)
		{
			...
			post.data["sessid"] = BX.thurly_sessid();
			post.size += (6 + BX.thurly_sessid().length);
			if (data)
			{
...
//				this.queue.restoreFiles(pack.repo);
	...
	BX.UploaderStream.prototype =
	{
		...
		onfailure : function(status, e)
		{
			...
			{
				BX.ajax.post(
					"/thurly/tools/upload.php?action=error",
					{
