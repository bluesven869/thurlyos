...
$APPLICATION->SetAdditionalCSS('/thurly/gadgets/thurly/admin_security/styles.css');

$aGlobalOpt = CUserOptions::GetOption("global", "settings", array());
$bShowSecurity = (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/security/install/index.php") && $aGlobalOpt['messages']['security'] <> 'N');
...
if($bSecModuleInstalled){
	...
	include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/security/install/version.php");
...
$eventLogUrl = '/thurly/admin/event_log.php?set_filter=Y&find_type=audit_type_id&find_audit_type[]=SECURITY_VIRUS&find_audit_type[]=SECURITY_FILTER_SQL&find_audit_type[]=SECURITY_FILTER_XSS&find_audit_type[]=SECURITY_FILTER_XSS2&find_audit_type[]=SECURITY_FILTER_PHP&find_audit_type[]=SECURITY_REDIRECT&find_audit_type[]=SECURITY_HOST_RESTRICTION&mod=security&lang='.LANGUAGE_ID;
$securityScannerUrl = '/thurly/admin/security_scanner.php?lang='.LANGUAGE_ID;
	...
	?><tr class="bx-gadget-bottom-cont<?=((!$bSecModuleInstalled && $USER->CanDoOperation('edit_other_settings')) || ($bSecModuleInstalled && $APPLICATION->GetGroupRight("security") >= "W") ? " bx-gadget-bottom-button-cont" : "")?>"><?
		...
		if (!$bSecModuleInstalled && $USER->CanDoOperation('edit_other_settings'))
		{
			?><td class="bx-gadgets-colourful-cell"><?
				?><a class="bx-gadget-button bx-gadget-button-clickable" href="/thurly/admin/module_admin.php?id=security&install=Y&lang=<?=LANGUAGE_ID?>&<?=thurly_sessid_get()?>">
