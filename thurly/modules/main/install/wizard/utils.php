...
class BXInstallServices
{
	...
	public static function LoadWizardData($wizard)
	{
		...
		if (count($ar) > 2)
			$path = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$ar[0]."/install/wizards/".$ar[1]."/".$ar[2];
		elseif (count($ar) == 2)
			$path = $_SERVER["DOCUMENT_ROOT"]."/thurly/wizards/".$ar[0]."/".$ar[1];
		else
			return false;
		...
		if (strlen($arWizardDescription["IMAGE"]) > 0)
		{
			if (count($ar) > 2)
			{
				BXInstallServices::CopyDirFiles(
					...
					$_SERVER["DOCUMENT_ROOT"]."/thurly/tmp/".$ar[1]."/".$ar[2]."/".$arWizardDescription["IMAGE"],
					true
				);
				$arWizardDescription["IMAGE"] = "/thurly/tmp/".$ar[1]."/".$ar[2]."/".$arWizardDescription["IMAGE"];
			}
			else
			{
				$arWizardDescription["IMAGE"] = "/thurly/wizards/".$ar[0]."/".$ar[1]."/".$arWizardDescription["IMAGE"];
			}
		}
	...
	public static function GetWizardsList($moduleName = "")
	{
		...
		{
			$path = $_SERVER["DOCUMENT_ROOT"]."/thurly/wizards";
			if ($h1 = opendir($path))
			{
		...
		$path = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules";
		if ($h1 = opendir($path))
		{
			while (($f1 = readdir($h1)) !== false)
			{
		...
		if(LANGUAGE_ID != 'ru')
			unset($arWizardsList['thurly:demo']);
	...
	public static function GetDBTypes()
	{
		...
		if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/mysql/database.php"))
	...
	public static function Add2Log($sText, $sErrorCode = "")
	{
		...
		$READ_PSIZE = 8000;
		$LOG_FILE = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/install.log";
		$LOG_FILE_TMP = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/install_tmp.log";
	...
	public static function GetDemoWizard()
	{
		...
		foreach ($arWizards as $arWizard)
		{
			...
			if ($wizardID == "thurly:demo")
			{
				$defaultWizard = "thurly:demo";
				continue;
			}
	...
	public static function CheckShortInstall()
	{
		...
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/".$DBType."/main.php");
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/tools.php");
		IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/main.php");
		...
		if ($DBType == 'mysql')
		{
			...
			{
				$dbClassName = "\\Thurly\\Main\\DB\\MysqliConnection";
			}
			else
			{
				$dbClassName = "\\Thurly\\Main\\DB\\MysqlConnection";
			}
		}
		elseif ($DBType == 'mssql')
		{
			$dbClassName = "\\Thurly\\Main\\DB\\MssqlConnection";
		}
		else
		{
			$dbClassName = "\\Thurly\\Main\\DB\\OracleConnection";
		}

		$conPool->setConnectionParameters(
			\Thurly\Main\Data\ConnectionPool::DEFAULT_CONNECTION_NAME,
			array(
		...
		if ($databaseStep->IsThurlyInstalled())
		...
		while (($file = readdir($handle)) !== false)
		{
			...
			else
			{
				if (
					...
					strpos($filePath, "/thurly/modules/main/lang/".LANGUAGE_ID."/install.php") !== false
				)
					continue;
	...
	public static function GetRegistrationKey($lic_key_user_name, $lic_key_user_surname, $lic_key_email, $DBType)
	{
		...
		if ($fp)
		{
			...
			fputs($fp, "User-Agent: thurlyKeyReq\r\n");
	...
	public static function CreateLicenseFile($licenseKey)
	{
		...
		$filePath = $_SERVER["DOCUMENT_ROOT"]."/thurly/license_key.php";
