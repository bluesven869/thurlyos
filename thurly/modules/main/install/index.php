...
class main extends CModule
{
	...
	function InstallDB()
	{
		...
		$errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/".$DBType."/install.sql");
		if ($errors !== false)
		{
		...
		if (strtolower($DB->type) == 'mysql')
		{
			...
			{
				\Thurly\Main\UserTable::getEntity()->enableFullTextIndex("SEARCH_USER_CONTENT");
			}
			if($DB->Query("CREATE fulltext index IXF_B_USER_INDEX_2 on b_user_index (SEARCH_DEPARTMENT_CONTENT)", true))
			{
				\Thurly\Main\UserTable::getEntity()->enableFullTextIndex("SEARCH_DEPARTMENT_CONTENT");
			}
			if($DB->Query("CREATE fulltext index IXF_B_USER_INDEX_3 on b_user_index (SEARCH_ADMIN_CONTENT)", true))
			{
				\Thurly\Main\UserTable::getEntity()->enableFullTextIndex("SEARCH_ADMIN_CONTENT");
			}
		}
		...
		if (LANGUAGE_ID <> "de" && file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/lang/de/install/index.php'))
		{
			$addResult = CultureTable::add(array(
				"NAME" => "de",
				"CODE" => "de",
		...
		if (LANGUAGE_ID <> "ua" && file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/lang/ua/install/index.php'))
		{
			$addResult = CultureTable::add(array(
				"NAME" => "ua",
				"CODE" => "ua",
		...
		if (LANGUAGE_ID <> "ru" && file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/lang/ru/install/index.php'))
		{
			$addResult = CultureTable::add(array(
				"NAME" => "ru",
				"CODE" => "ru",
		...
		RegisterModuleDependences("sender", "OnConnectorList", "main", "\\Thurly\\Main\\SenderEventHandler", "onConnectorListUser");
		...
		RegisterModuleDependences("main", "OnUserTypeBuildList", "main", "\\Thurly\\Main\\UrlPreview\\UrlPreviewUserType", "getUserTypeDescription", 210);

		RegisterModuleDependences("main", "OnBeforeEndBufferContent", "main", "\\Thurly\\Main\\Analytics\\Counter", "onBeforeEndBufferContent");
		RegisterModuleDependences("main", "OnBeforeRestartBuffer", "main", "\\Thurly\\Main\\Analytics\\Counter", "onBeforeRestartBuffer");

		RegisterModuleDependences("disk", "onAfterAjaxActionCreateFolderWithSharing", "main", "\\Thurly\\Main\\FinderDestTable", "onAfterDiskAjaxAction");
		RegisterModuleDependences("disk", "onAfterAjaxActionAppendSharing", "main", "\\Thurly\\Main\\FinderDestTable", "onAfterDiskAjaxAction");
		RegisterModuleDependences("disk", "onAfterAjaxActionChangeSharingAndRights", "main", "\\Thurly\\Main\\FinderDestTable", "onAfterDiskAjaxAction");
		...
		if (LANGUAGE_ID == "ru")
			COption::SetOptionString("main", "vendor", "1c_thurly");
		else
			COption::SetOptionString("main", "vendor", "thurly");
		...
		CAgent::AddAgent('\\Thurly\\Main\\Analytics\\CounterDataTable::submitData();', "main", "N", 60);

		$eventManager = \Thurly\Main\EventManager::getInstance();

		$eventManager->registerEventHandler("sale", "OnSaleBasketItemSaved", "main", "\\Thurly\\Main\\Analytics\\Catalog", "catchCatalogBasket");
		$eventManager->registerEventHandler("sale", "OnSaleOrderSaved", "main", "\\Thurly\\Main\\Analytics\\Catalog", "catchCatalogOrder");
		$eventManager->registerEventHandler("sale", "OnSaleOrderPaid", "main", "\\Thurly\\Main\\Analytics\\Catalog", "catchCatalogOrderPayment");
		...
		{
			GeoIp\HandlerTable::add(array('SORT' => 90, 'ACTIVE' => 'Y', 'CLASS_NAME' => '\Thurly\Main\Service\GeoIp\Extension'));
		}

		GeoIp\HandlerTable::add(array('SORT' => 100, 'ACTIVE' => 'N', 'CLASS_NAME' => '\Thurly\Main\Service\GeoIp\MaxMind'));
		GeoIp\HandlerTable::add(array('SORT' => 110, 'ACTIVE' => 'Y', 'CLASS_NAME' => '\Thurly\Main\Service\GeoIp\SypexGeo'));
	...
	private static function InstallDesktop()
	{
		...
		$info_table .= '	<td class="bx-gadgets-info-site-logo" rowspan="5"><img src="'.'/thurly/components/thurly/desktop/templates/admin/images/site_logo.png'.'"></td>';
		$info_table .= '</tr>';
		$info_table .= '<tr>';
		...
		switch (LANGUAGE_ID)
		{
			case "ru":
				...
				break;
			case "de":
				$rss_url = "http://www.thurly.de/company/news/rss/";
				break;
			default:
		...
		if ( // 111
			file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/index.php")
			&& !file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/intranet/install/index.php")
		)
		{
			if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/install/index.php"))
			{
				$arOptions = array(
					array(
						"GADGETS" => array(
		...
		elseif (!file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/intranet/install/index.php")) // 222
		{
			if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/install/index.php"))
			{
				$arOptions = array(
		...
		elseif (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/index.php")) // 333
		{
			$arOptions = array(
				array(
					"GADGETS" => array(
	...
	private static function InstallSmiles()
	{
		...
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/file.php");
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/".$DBType."/file.php");
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/archive.php");
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/csv_data.php");
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/file_temp.php");
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/smile.php");
	...
	function InstallEvents()
	{
		...
		$arMessages[] = array(
			...
			'MESSAGE'          => "<? EventMessageThemeCompiler::includeComponent('thurly:main.mail.confirm', '', \$arParams); ?>",
			'BODY_TYPE'        => 'html',
	...
	{
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/thurly", $_SERVER["DOCUMENT_ROOT"]."/thurly", true, true);
		CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/main/install/admin", $_SERVER['DOCUMENT_ROOT']."/thurly/admin");
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/tools", $_SERVER["DOCUMENT_ROOT"]."/thurly/tools", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/services", $_SERVER["DOCUMENT_ROOT"]."/thurly/services", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/images", $_SERVER["DOCUMENT_ROOT"]."/thurly/images", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/js", $_SERVER["DOCUMENT_ROOT"]."/thurly/js", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/themes", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/sounds", $_SERVER["DOCUMENT_ROOT"]."/thurly/sounds", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/components/thurly", $_SERVER["DOCUMENT_ROOT"]."/thurly/components/thurly", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/gadgets/thurly", $_SERVER["DOCUMENT_ROOT"]."/thurly/gadgets/thurly", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/image_uploader", $_SERVER["DOCUMENT_ROOT"]."/thurly/image_uploader", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/panel", $_SERVER["DOCUMENT_ROOT"]."/thurly/panel", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/fonts", $_SERVER["DOCUMENT_ROOT"]."/thurly/fonts", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/css", $_SERVER["DOCUMENT_ROOT"]."/thurly/css", true, true);
