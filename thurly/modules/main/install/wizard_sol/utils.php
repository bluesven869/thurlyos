...
class WizardServices
{
	function PatchHtaccess($path)
	{
		if (strtoupper(substr(PHP_OS, 0, 3)) === "WIN")
		{
			...
			if ($io->FileExists($fnhtaccess))
			{
				...
				if (strpos($ffhtaccessContent, "/thurly/virtual_file_system.php") === false)
				{
					$ffhtaccessContent = preg_replace('/RewriteEngine On/is', "RewriteEngine On\r\n\r\n".
						...
						"RewriteCond %{REQUEST_FILENAME} !/thurly/virtual_file_system.php$\r\n".
						"RewriteRule ^(.*)$ /thurly/virtual_file_system.php [L]", $ffhtaccessContent);
	...
	function ImportIBlockFromXML($xmlFile, $iblockCode, $iblockType, $siteID, $permissions = Array())
	{
		...
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/iblock/classes/".strtolower($GLOBALS["DB"]->type)."/cml2.php");
	...
	{
		global $DBType;
		require_once($_SERVER['DOCUMENT_ROOT']."/thurly/modules/main/classes/".strtolower($DBType)."/favorites.php");
