...
class CWizardSolPanel
{
	function ShowPanel()
	{
		...
		if($USER->IsAdmin())
		{
			if($_REQUEST['add_new_site_sol']=='sol' && check_thurly_sessid())
			{
			...
			if($_REQUEST['delete_button_sol']=='sol' && check_thurly_sessid())
			{
			...
			$arMenu = Array(
				Array(		
					"ACTION" => "jsUtils.Redirect([], '".CUtil::JSEscape(SITE_DIR)."?add_new_site_sol=sol&".thurly_sessid_get()."')",
			...
			$arMenu[] = Array(		
				"ACTION" => "if(confirm('".AddSlashes(GetMessage("SOL_BUTTON_CONFIRM_W2"))."')) jsUtils.Redirect([], '".CUtil::JSEscape(SITE_DIR)."?delete_button_sol=sol&".thurly_sessid_get()."');",
			...
			$APPLICATION->AddPanelButton(array(
				"HREF" => SITE_DIR."?add_new_site_sol=sol&".thurly_sessid_get(),
				"ID" => "solutions_wizard",
