		...
		var d = function(node, type, container, formats) {
			...
			d.prototype = {
				...
				callback : function(data) {
					...
					this.node.value = BX.date.format(this.format.thurly[this.type], d);
				...
				init : function(formats) {
					var DATETIME_FORMAT = BX.date.convertThurlyFormat(BX.message("FORMAT_DATETIME")),
						DATE_FORMAT = BX.date.convertThurlyFormat(BX.message("FORMAT_DATE")),
						TIME_FORMAT;
					if ((DATETIME_FORMAT.substr(0, DATE_FORMAT.length) == DATE_FORMAT))
						...
						TIME_FORMAT = BX.date.convertThurlyFormat(DATETIME_FORMAT.indexOf('T') >= 0 ? 'H:MI:SS T' : 'HH:MI:SS');
					this.format.thurly.datetime = DATETIME_FORMAT;

					this.format.thurly.date = DATE_FORMAT;
					this.format.thurly.time = TIME_FORMAT;
