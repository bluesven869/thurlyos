 ...
 * Class CMainUIGrid
 */
class CMainUIGrid extends CThurlyComponent
{
	/** @var \CGridOptions $gridOptions */
	protected $gridOptions;

	/** @var Thurly\Main\Web\Uri $uri */
	protected $uri;
	...
	protected function prepareMessages($messages = array())
	{
		...
		$isAssociative = ($isArray && \Thurly\Main\Type\Collection::isAssociative($messages));
	...
	protected function applyColumnsCustomNames()
	{
		if (is_array($this->getCustomNames()) && count($this->getCustomNames()) > 0)
		{
			foreach ($this->getCustomNames() as $key => $value)
			{
				...
				{
					$this->arResult["COLUMNS_ALL"][$key]["name"] = Thurly\Main\Text\Converter::getHtmlConverter()->decode($value);
				}
			}
		}
	...
	protected function prepareNavString()
	{
		...
		if (!is_string($this->arResult["NAV_STRING"]))
		{
			...
			elseif(is_object($this->arParams["NAV_OBJECT"]))
			{
				...
				if(($nav = $this->arParams["NAV_OBJECT"]) instanceof \Thurly\Main\UI\PageNavigation)
				{
					$params = array_merge(
						array(
							"NAV_OBJECT" => $nav,
	 ...
	 * Gets uri object
	 * @method getUri
	 * @return Thurly\Main\Web\Uri instance with current request uri
	 */
