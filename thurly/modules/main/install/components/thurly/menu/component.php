...
if($this->startResultCache(false, false, ($arParams["MENU_CACHE_USE_USERS"] === "Y"? $this->getGenerationCachePath($USER->GetID()): false)))
{
	if(defined("BX_COMP_MANAGED_CACHE"))
		$CACHE_MANAGER->registerTag("thurly:menu");
	//Read root menu
	...
	else
	{
		...
		{
			//Menu from iblock (thurly:menu.sections)
...
if($USER->IsAuthorized())
{
	...
	if($bMenuEdit)
	{
		$menu_edit_url = $APPLICATION->GetPopupLink(array(
			"URL"=> "/thurly/admin/public_menu_edit.php?lang=".LANGUAGE_ID.
	...
	if ($bMenuAdd)
	{
		...
		$menu_edit_url = $APPLICATION->GetPopupLink(array(
			"URL" => "/thurly/admin/public_menu_edit.php?new=Y&lang=".LANGUAGE_ID.
				"&site=".SITE_ID."&back_url=".urlencode($_SERVER["REQUEST_URI"]).
	...
	if ($bMenuDelete)
	{
		$menu_del_url = "if(confirm('".CUtil::JSEscape(GetMessage('menu_comp_del_conf', array('#MENU_TITLE#' => (isset($arMenuTypes[$menuType]) ? $arMenuTypes[$menuType] : $menuType)))).
			"')) {BX.showWait(); BX.ajax.get('/thurly/admin/public_menu_edit.php?lang=".LANGUAGE_ID.
			"&site=".SITE_ID."&back_url=".urlencode($_SERVER["REQUEST_URI"]).
			"&path=".urlencode($menuDir)."&name=".$menuType."&action=delete&".thurly_sessid_get()."')}";
