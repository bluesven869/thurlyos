...
use \Thurly\Main\UI\Uploader\Uploader;
use \Thurly\Main\UI\FileInputUtility;
use \Thurly\Main\UI\Uploader\File;
...
class MFIController
{
	...
	{
		if (\Thurly\Main\ModuleManager::isModuleInstalled($moduleId))
	...
	/**
	 * @throws \Thurly\Main\AccessDeniedException
	 * @return void
	 */
	public function checkRequest($controlId=null)
	{
		...
		if (!preg_match('/^[a-f01-9]{32}$/', $cid))
			throw new \Thurly\Main\AccessDeniedException("CID has wrong format.");
		...
		if (is_null($registeredControlId))
			throw new \Thurly\Main\AccessDeniedException("CID is not registered.");
		if (!check_thurly_sessid())
			throw new \Thurly\Main\AccessDeniedException("Bad sessid.");
		...
		if ($action == "upload")
		{
			if ($this->getUploader() === null)
				throw new \Thurly\Main\AccessDeniedException("Uploading is forbidden.");
	...
	protected function executeActionUpload()
	{
		...
		for ($i = 0; $i < $count; $i++)
		{
			...
			{
				if (strlen($res) > 0)
					throw new \Thurly\Main\ArgumentException($res);
	...
	public function getUrlDownload($id)
	{
		...
		return "/thurly/components/thurly/main.file.input/ajax.php?" . http_build_query($query);
	}
	/**
	 * @param integer $id
	 * @return string
