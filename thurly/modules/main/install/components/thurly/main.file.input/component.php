...
{
	$arParams["sign"] = new \Thurly\Main\Security\Sign\Signer;
}
			...
			if (strlen($res) <= 0)
			{
				...
				if ($fileID)
				{
					...
					if ($file)
					{
						...
						$tmp['fileURL'] = "/thurly/components/thurly/main.file.input/file.php?" . http_build_query($query);
...
if (is_array($arParams['INPUT_VALUE']) && strlen(implode(",", $arParams["INPUT_VALUE"])) > 0)
{
	...
	while ($arFile = $dbRes->GetNext())
	{
		...
		$arFile['URL'] = "/thurly/components/thurly/main.file.input/file.php?" . http_build_query($query);
