...
CComponentUtil::__IncludeLang("/thurly/components/thurly/desktop/", "/include.php");
		...
		foreach($folders as $folder)
		{
			...
			if(is_dir($gdDir) && ($handle = opendir($gdDir)))
			{
				while(false !== ($item = readdir($handle)))
					if(is_dir($gdDir."/".$item) && $item != "." && $item != ".." && $item != "thurly")
						$arGdNS[] = $item;
				closedir($handle);
			}
			...
			foreach($arGdNS as $NS)
			{
				...
				if(is_dir($gdDir."/".$id))
				{
					...
					if($arDescription["ICON"] && substr($arDescription["ICON"], 0, 1)!="/")
						$arDescription["ICON"] = "/thurly/gadgets/".$NS."/".$id."/".$arDescription["ICON"];
