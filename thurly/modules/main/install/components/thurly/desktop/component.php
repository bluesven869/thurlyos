...
include_once($_SERVER['DOCUMENT_ROOT'].'/thurly/components/thurly/desktop/include.php');
...
if($USER->IsAuthorized() && $arResult["PERMISSION"]>"R" && check_thurly_sessid())
{
...
foreach($arGadgets as $gadget)
{
	...
	{
		$aGlobalOpt = CUserOptions::GetOption("global", "settings", array());
		$bShowPerfmon = (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/perfmon/install/index.php") && $aGlobalOpt['messages']['perfmon'] <> 'N');
		if (!$bShowPerfmon)
			continue;
	}
	if (
