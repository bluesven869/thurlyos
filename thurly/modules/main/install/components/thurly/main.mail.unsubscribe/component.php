...
try
{
	$arTag = \Thurly\Main\Mail\Tracking::parseSignedTag(is_string($_REQUEST['tag']) ? $_REQUEST['tag'] : '');
	$arTag['IP'] = $_SERVER['REMOTE_ADDR'];

	$arResult = array();
	$arResult['FORM_URL'] = $APPLICATION->getCurPageParam("",array('success'));
	$arResult['LIST'] = \Thurly\Main\Mail\Tracking::getSubscriptionList($arTag);

	if($_SERVER['REQUEST_METHOD'] == 'POST' && array_key_exists('MAIN_MAIL_UNSUB_BUTTON', $_POST) && check_thurly_sessid())
	{
		...
		if(!empty($arUnsubscribeList))
		{
			...
			$result = \Thurly\Main\Mail\Tracking::unsubscribe($arTag);
			if ($result)
			{
				$messageResult = '0';
			}
...
$siteName = '';
/*
$siteName = \COption::getOptionString('thurlyos', 'site_title', '');
if (!$siteName)
{
