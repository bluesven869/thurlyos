...
?><select name="<?=$arParams["arUserField"]["FIELD_NAME"]?>">
	<option value="1"<?=($res? ' selected': '')?>><?=\Thurly\Main\Text\HtmlFilter::encode($arResult['FIELD_LABEL'][1])?></option>
	<option value="0"<?=(!$res? ' selected': '')?>><?=\Thurly\Main\Text\HtmlFilter::encode($arResult['FIELD_LABEL'][0])?></option>
</select><?
			break;
			case "RADIO":
?><label><input type="radio" value="1" name="<?=$arParams["arUserField"]["FIELD_NAME"]?>"
	<?=($res ? ' checked': '')?>><?=\Thurly\Main\Text\HtmlFilter::encode($arResult['FIELD_LABEL'][1])?></label><br>
<label><input type="radio" value="0" name="<?=$arParams["arUserField"]["FIELD_NAME"]?>"
	<?=(!$res ? ' checked': '')?>><?=\Thurly\Main\Text\HtmlFilter::encode($arResult['FIELD_LABEL'][0])?></label><?
			break;
			default:
?><input type="hidden" value="0" name="<?=$arParams["arUserField"]["FIELD_NAME"]?>">
				<label><input type="checkbox" value="1" name="<?=$arParams["arUserField"]["FIELD_NAME"]?>" <?=($res ? "checked=\"checked\"": "")?>> <?=\Thurly\Main\Text\HtmlFilter::encode($arResult['FIELD_LABEL'][1])?></label><?
			break;
		}
?></div><?
endforeach;
