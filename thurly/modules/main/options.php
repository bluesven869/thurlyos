...
if($_SERVER["REQUEST_METHOD"] == "GET" && $USER->IsAdmin() && $_REQUEST["RestoreDefaults"] <> '' && check_thurly_sessid())
{
	$aSaveVal = array(
		...
		array("NAME"=>"vendor", "DEF"=>"1c_thurly"),
...
if($_SERVER["REQUEST_METHOD"] == "GET" && $USER->CanDoOperation('edit_other_settings') && $_REQUEST["GenKey"] <> '' && check_thurly_sessid())
{
...
$arAllOptions = array(
	"main" => Array(
		...
		Array("cookie_name", GetMessage("MAIN_PREFIX"), "THURLY_SM", Array("text", 30)),
...
if (\Thurly\Main\Analytics\SiteSpeed::isRussianSiteManager())
{
...
if(CRsaSecurity::Possible())
{
	...
	$mess .= '<br><br><input type="button" name="" value="'.GetMessage("MAIN_OPT_SECURE_GENKEY").'" onclick="window.location=\'/thurly/admin/settings.php?GenKey=Y&lang='.LANGUAGE_ID.'&mid='.urlencode($mid).'&'.thurly_sessid_get().'&tabControl_active_tab=edit6\'">';
...
$intl = new \Thurly\Main\UserConsent\Intl(LANGUAGE_ID);
$listAgreement = array("" => GetMessage("MAIN_REGISTER_AGREEMENT_DEFAUTL_VALUE"));
$listAgreementObject = \Thurly\Main\UserConsent\Internals\AgreementTable::getList(array(
...
$arAllOptions["auth"][] = Array("new_user_registration_email_confirmation", GetMessage("MAIN_REGISTER_EMAIL_CONFIRMATION", array("#EMAIL_TEMPLATES_URL#" => "/thurly/admin/message_admin.php?lang=".LANGUAGE_ID."&set_filter=Y&find_type_id=NEW_USER_CONFIRM")), "N", Array("checkbox", "Y"));
...
$SET_LICENSE_KEY = "";
if($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["Update"])>0 && ($USER->CanDoOperation('edit_other_settings') && $USER->CanDoOperation('edit_groups')) && check_thurly_sessid())
{
	...
	if($_REQUEST["back_url_settings"] <> "" && $_REQUEST["Apply"] == "")
		...
		LocalRedirect("/thurly/admin/settings.php?lang=".LANGUAGE_ID."&mid=".urlencode($mid)."&tabControl_active_tab=".urlencode($_REQUEST["tabControl_active_tab"])."&back_url_settings=".urlencode($_REQUEST["back_url_settings"]));
}

if($SET_LICENSE_KEY == "")
	$SET_LICENSE_KEY = LICENSE_KEY;

if ($_SERVER["REQUEST_METHOD"]=="POST" && $_POST["stop_site"]=="Y" && $USER->CanDoOperation('edit_other_settings') && check_thurly_sessid())
{
	COption::SetOptionString("main", "site_stopped", "Y");
	CAdminMessage::ShowNote(GetMessage("MAIN_OPTION_PUBL_CLOSES"));
}

if ($_SERVER["REQUEST_METHOD"]=="POST" && $_POST["start_site"]=="Y" && $USER->CanDoOperation('edit_other_settings') && check_thurly_sessid())
{
...
?>
	<tr valign="top">
		<td><?=$value["NAME"]." [<a title=\"".GetMessage("MAIN_USER_GROUP_TITLE")."\" href=\"/thurly/admin/group_edit.php?ID=".$value["ID"]."&amp;lang=".LANGUAGE_ID."\">".$value["ID"]."</a>]:"?></td>
		<td>
		<?
...
function RestoreDefaults()
{
	if(confirm('<?echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>'))
		window.location = "<?echo $APPLICATION->GetCurPage()?>?RestoreDefaults=Y&lang=<?=LANGUAGE_ID?>&mid=<?echo urlencode($mid)?>&<?echo thurly_sessid_get()?>";
}
...
)
{
	...
	if($_POST["controller_login"] == '' || $_POST["controller_password"] == '')
	{
		LocalRedirect($controller_url."/thurly/admin/controller_member_edit.php?lang=".LANGUAGE_ID.'&act=unregister&member_id='.urlencode(COption::GetOptionString("main", "controller_member_id", "")).'&back_url='.urlencode(($APPLICATION->IsHTTPS()?"https://":"http://").$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']));
	}
	else
	{
...
function DoNext(name, id, recount)
{
	...
	if (result['stop'] == false)
	{
		if (result['done'] == true)
		{
			...
			if (recount == 'begin')
				str = '&recount=begin';
			document.getElementById('frame_disk_quota').src='/thurly/admin/quota.php?name=' + id + '&id=' + name + str + '&<?echo thurly_sessid_get()?>';
		}
	}
	else
	{
