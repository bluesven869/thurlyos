...
$MESS["SC_CRON_WARN"] = "The constant BX_CRONTAB_SUPPORT is defined in /thurly/php_interface/dbconn.php, this requires running agents using cron.";
...
$MESS["SC_BX_UTF"] = "Use the following code in <i>/thurly/php_interface/dbconn.php</i>:
...
$MESS["SC_HELP_CHECK_INSTALL_SCRIPTS"] = "Users may occasionally forget to delete the installation scripts (restore.php, thurlysetup.php) after system recovery or installation. This may become a serious security threat and result in website hijacking. If you have ignored the autodelete warning, remember to remove these files manually.";
$MESS["SC_HELP_CHECK_MAIL_PUSH"] = "The <a href=\"http://helpdesk.thurlyos.com/?url=open/1602367/\" target=_blank>Message Relay</a> feature will post messages from e-mail to Activity Stream making it possible to involve in discussion any user who does not have an account on your ThurlyOS.
...
All the Thurly modules use the <i>BX_UTF</I> constant to resolve the current encoding. A UTF-8 website requires the following code in <i>/thurly/php_interface/dbconn.php</i>:
<code>define('BX_UTF', true);</code>
";
$MESS["SC_HELP_CHECK_SITES"] = "Verifies general multisite parameters. If a website specifies the root directory path (which is required only for websites existing on different domains), that directory must contain a symbolic link to writable \"thurly\" folder.
...
$MESS["SC_HELP_CHECK_DBCONN_SETTINGS"] = "This test will compare database connection parameters specified in <i>/thurly/php_interface/dbconn.php</i> with those in <i>/thurly/.settings.php</i>. 
...
$MESS["SC_HELP_CHECK_UPLOAD"] = "This will attempt to connect to the web server and send a chunk of binary data as a file. The server will then compare the received data with the original sample. If a problem arises, it may be caused by some parameter in <i>php.ini</I> prohibiting binary data transfer, or by inaccessible temporary folder (or <i>/thurly/tmp</i>).
...
Alternatively, you can use cron to send the e-mails. To do so, add <code>define('BX_CRONTAB_SUPPORT', true);</code> to dbconn.php. Then, set cron to execute <i>php /var/www/thurly/modules/main/tools/cron_events.php</I> every minute (replace <i>/var/www</i> with your website root).

If the call to mail() has failed, you cannot send e-mail from your server using conventional methods.

If your hosting provider offers alternative e-mail sending services, you can use them by calling the function \"custom_mail\". Define this function in <i>/thurly/php_interface/dbconn.php</I>. If the system find this function definition, it will use the latter instead of PHP's \"mail\" with the same input parameters.";
...
If the current value is less than that, add this line of code to <i>/thurly/php_interface/dbconn.php</i>.
";
...
Configure PHP to run as an Apache module if possible.";
$MESS["SC_HELP_CHECK_BX_CRONTAB"] = "To migrate the non-periodic agents and e-mail to cron, add the following constant to <i>/thurly/php_interface/dbconn.php</i>:
<code>define('BX_CRONTAB_SUPPORT', true);</code>

With this constant set to \"true\", the system will run only the periodic agents when a hit occurs. Now add a task to cron to execute <i>/var/www/thurly/modules/main/tools/cron_events.php</i> every minute (replace <i>/var/www</i> with the website root path).
...
Set the PHP time zone in <i>/thurly/php_interface/dbconn.php</i>:
<code>date_default_timezone_set(&quot;Europe/London&quot;);</code> (use your region and city).

Set the database time zone by adding the following code to <i>/thurly/php_interface/after_connect_d7.php</i>:
<code>\$connection = Thurly\\Main\\Application::getConnection(); 
...
$MESS["SC_HELP_CHECK_MYSQL_MODE"] = "The parameter <i>sql_mode</i> specifies the MySQL operation mode. Note that it may accept values incompatible with Thurly solutions. Add the following code to <i>/thurly/php_interface/after_connect_d7.php</I> to set the default mode:
<code>\$connection = Thurly\\Main\\Application::getConnection(); \$connection->queryExecute(&quot;SET sql_mode=''&quot;);</code>";
...
To change the connection charset (for example, set it to UTF-8), add the following code to <i>/thurly/php_interface/after_connect_d7.php</i>:
<code>\$connection = Thurly\\Main\\Application::getConnection();
...
If you don't need features provided by this module, simply <a href=\"/thurly/admin/module_admin.php\">uninstall it</a>.";
...
Enable this option in the <a href=\"/thurly/admin/settings.php?mid=main\">Kernel settings</a>. <a href=\"http://www.thurlysoft.com/products/virtual_appliance/\">Thurly Virtual Appliance</a> supports fast file downloads by default.
...
If the server does not support HTML page compression, the Thurly Compression module is used instead. Remember that this module <a href=\"/thurly/admin/module_admin.php\">should not be installed</a> otherwise.";
...
$MESS["SC_HELP_CHECK_SEARCH"] = "The system can search text in documents in Open XML format (introduced in Microsoft Office 2007) out of the box. To support other file formats, specify paths to parsing applications <a href=\"/thurly/admin/settings.php?mid=intranet\">in the Intranet module settings</a>. Otherwise, the system will be able to search filenames only.

<a href=\"http://www.1c-thurly.ru/products/vmthurly/index.php\">Thurly Virtual Appliance</a> supports it by default.";
...
$MESS["MAIN_SC_NO_ACCESS"] = "Cannot access Thurly, Inc. server. Updates and Thurly Cloud Service are unavailable.";
...
$MESS["MAIN_SC_NO_SOCIAL_SERVICES_24NET"] = "thurlyos.net integration is not configured in the Social Service module settings.";
...
$MESS["MAIN_SC_WARN_EXPAND_SESSION"] = "If the Instant Messenger module is installed, disable the session keep alive feature in the <a href='/thurly/admin/settings.php?mid=main' target=_blank>kernel settings</a> to reduce server load.";
...
$MESS["MAIN_SC_MBSTRING_SETTIGNS_DIFFER"] = "mbstring preferences in <i>/thurly/.settings.php</i> (utf_mode) and <i>/thurly/php_interface/dbconn.php</i> (BX_UTF) are different.";
$MESS["SC_ERR_NO_SETTINGS"] = "Configuration file /thurly/.settings.php was not found";
