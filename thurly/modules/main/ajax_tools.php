...
class CAjax
{
	...
	{
		$aTrace = Thurly\Main\Diag\Helper::getBackTrace(0, DEBUG_BACKTRACE_IGNORE_ARGS);
		...
		if($iSrcLine > 0 && $sSrcFile <> "")
		{
			...
			{
				//outside
				$sRealThurly = strtolower(str_replace("\\", "/", realpath($_SERVER["DOCUMENT_ROOT"]."/thurly")));

				if(strpos($sSrcFile, substr($sRealThurly, 0, -6)) === 0)
				{
					$sSrcFile = substr($sSrcFile, strlen($sRealThurly) - 7);
					$bSrcFound = true;
				}
				else
				{
					// special hack
					$sRealThurlyModules = strtolower(str_replace("\\", "/", realpath($_SERVER["DOCUMENT_ROOT"]."/thurly/modules")));
					if(strpos($sSrcFile, $sRealThurlyModules) === 0)
					{
						$sSrcFile = "/thurly/modules".substr($sSrcFile, strlen($sRealThurlyModules));
						$bSrcFound = true;
					}
					else
					{
						// next special hack
						$matches = array();
						if (preg_match("#(?<=/modules/)[^/]+(?=/install/components/thurly/)#", $sSrcFile, $matches))
						{
							$sRealThurlyComponentsDir = strtolower(str_replace("\\", "/", realpath($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$matches[0]."/install/components/thurly")));
							if(strpos($sSrcFile, $sRealThurlyComponentsDir) === 0)
							{
								$sSrcFile = "/thurly/components/thurly".substr($sSrcFile, strlen($sRealThurlyComponentsDir));
								$bSrcFound = true;
							}
						}
						elseif (preg_match("#/thurly/modules#", $sSrcFile))
						{
							$sRealDocRoot = substr($sSrcFile, 0, strpos($sSrcFile, "/thurly/modules/"));
	...
	public static function GetForm($form_params, $container_id, $ajax_id, $bReplace = true, $bShadow = true)
	{
		...
		if (!$rndGenerator)
			$rndGenerator = new \Thurly\Main\Type\RandomSequence("BX_AJAX");
...
if (top.BX(\''.BX_AJAX_PARAM_ID.'_'.$ajax_id.'_'.$rnd.'\'))
	...
	public static function decodeURI($str)
	{
		...
		if (preg_match("'^(/thurly/|http://|https://|ftp://)'i", $str))
		{
			$APPLICATION->ThrowException(GetMessage('AJAX_REDIRECTOR_BAD_URL'));
			return false;
		}
