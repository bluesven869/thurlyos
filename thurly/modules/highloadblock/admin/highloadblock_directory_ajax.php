...
if ($USER->IsAuthorized() && check_thurly_sessid() && isset($_REQUEST['IBLOCK_ID']))
{
	if (!Loader::includeModule('highloadblock') || !Loader::includeModule('iblock'))
	{
		echo Thurly\Main\Web\Json::encode(array('ERROR' => 'SS_MODULE_NOT_INSTALLED'));
	...
	if ($iblockID < 0)
	{
		echo Thurly\Main\Web\Json::encode(array('ERROR' => 'SS_IBLOCK_ID_ABSENT'));
		require($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/include/epilog_admin_after.php');
		die();
	}
	elseif ($iblockID === 0)
	{
		if (!$USER->IsAdmin())
		{
			echo Thurly\Main\Web\Json::encode(array('ERROR' => 'SS_NO_ADMIN'));
			require($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/include/epilog_admin_after.php');
			die();
		}
	}
	else
	{
		...
		if ($arIBlock)
		{
			...
			{
				echo Thurly\Main\Web\Json::encode(array('ERROR' => 'SS_ACCESS_DENIED'));
				require($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/include/epilog_admin_after.php');
				die();
			}
		}
		...
		{
			echo Thurly\Main\Web\Json::encode(array('ERROR' => 'SS_IBLOCK_ABSENT'));
			require($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/include/epilog_admin_after.php');
			die();
		}
	}
	...
	{
		$hlblock = Thurly\Highloadblock\HighloadBlockTable::getList(array("filter" => array("=TABLE_NAME" => $hlBlockID)))->fetch();
	...
	if (isset($_REQUEST['addEmptyRow']) && $_REQUEST['addEmptyRow'] === 'Y')
	{
		...
		echo Thurly\Main\Web\Json::encode(addTableRow($rowNumber, $defaultValues, $hlblockFields, true));
	}
	else
	{
		if ($hlBlockID !== '')
