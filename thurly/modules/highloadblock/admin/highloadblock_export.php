...
if ($request->get('start') == 'Y' && $server->getRequestMethod() == 'POST')
{
	...
	// create / open file
	$export = new \Thurly\Main\XmlWriter(array(
	...
	else
	{
		...
		if ($NS['finish'])
		{
			...
			$details .= '<br/>'.Loc::getMessage('ADMIN_TOOLS_PROCESS_FINAL', array(
				'#xml_link#' => '<a href="/thurly/admin/fileman_admin.php?lang='.LANG.'&amp;path='.htmlspecialcharsbx(urlencode($pathInfo['dirname'])).'&amp;set_filter=Y&amp;find_name='.htmlspecialcharsbx(urlencode($pathInfo['basename'])).'" target="_blank">'.
			...
			if ($NS['has_files'])
			{
				$details .= '<br/>'.Loc::getMessage('ADMIN_TOOLS_PROCESS_FILES_FINAL', array(
					'#files_link#' => '<a href="/thurly/admin/fileman_admin.php?lang='.LANG.'&amp;path='.htmlspecialcharsbx(urlencode($pathInfo['dirname'])).'&amp;set_filter=Y&amp;find_name='.htmlspecialcharsbx(urlencode($pathInfo['files_dir'])).'" target="_blank">'.
