...
class CUserTypeHlblock extends CUserTypeEnum
{
	...
	function getEntityReferences($userfield, \Thurly\Main\Entity\ScalarField $entityField)
	{
		if ($userfield['SETTINGS']['HLBLOCK_ID'])
		{
			$hlblock = \Thurly\Highloadblock\HighloadBlockTable::getById($userfield['SETTINGS']['HLBLOCK_ID'])->fetch();

			if ($hlblock)
			{
				if (class_exists($hlblock['NAME'].'Table'))
				{
					$hlentity = \Thurly\Main\Entity\Base::getInstance($hlblock['NAME']);
				}
				else
				{
					$hlentity = \Thurly\Highloadblock\HighloadBlockTable::compileEntity($hlblock);
				}
	...
	public static function getHlRows($userfield, $clearValues = false)
	{
		...
		if (!empty($hlblock_id))
		{
			$hlblock = \Thurly\Highloadblock\HighloadBlockTable::getById($hlblock_id)->fetch();
		}
		...
		if (!empty($hlblock))
		{
			...
			if ($userfield)
			{
				...
				$hlDataClass = \Thurly\Highloadblock\HighloadBlockTable::compileEntity($hlblock)->getDataClass();
	...
	public static function getDropDownData()
	{
		...
		$hlblocks = \Thurly\Highloadblock\HighloadBlockTable::getList(array('order' => 'NAME'))->fetchAll();
