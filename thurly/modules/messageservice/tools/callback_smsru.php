...
foreach ($_POST["data"] as $entry)
{
	...
	$smsId = $lines[1];
	$statusCode = \Thurly\MessageService\Sender\Sms\SmsRu::resolveStatus($lines[2]);
...
if ($smsStatuses)
{
	...
	$tableName = \Thurly\MessageService\Internal\Entity\MessageTable::getTableName();
	...
	//send pull message
	if(\Thurly\MessageService\Integration\Pull::canUse())
	{
		$query = new \Thurly\Main\Entity\Query(\Thurly\MessageService\Internal\Entity\MessageTable::getEntity());
		$query->setSelect(array('ID', 'STATUS_ID'));
		$query->addFilter('=SENDER_ID', 'smsru');
		$query->addFilter('@EXTERNAL_ID', array_keys($smsStatuses));

		\Thurly\MessageService\Integration\Pull::onMessagesUpdate($query->exec()->fetchAll());
	}
}
