...
class RestService extends \IRestService
{
	...
	private static function checkAdminPermissions()
	{
		...
		if (!isset($USER)
			|| !is_object($USER)
			|| (!$USER->isAdmin() && !(Loader::includeModule('thurlyos') && \CThurlyOS::isPortalAdmin($USER->getID())))
		)
		{
	...
	{
		$iterator = \Thurly\Rest\AppTable::getList(
			array(
				'filter' => array(
					'=CLIENT_ID' => $clientId
				),
		...
		$orm = \Thurly\Rest\AppLangTable::getList(array(
			'filter' => array(
				'=APP_ID' => $app['ID']
			),
