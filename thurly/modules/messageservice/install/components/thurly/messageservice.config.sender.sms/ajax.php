...
$isAdmin = \Thurly\MessageService\Context\User::isAdmin();
if (!$isAdmin || !check_thurly_sessid() || $_SERVER['REQUEST_METHOD'] != 'POST')
{
	return;
}
...
/** @var \Thurly\MessageService\Sender\BaseConfigurable $sender */
$sender = \Thurly\MessageService\Sender\SmsManager::getSenderById($_REQUEST['sender_id']);
...
)
{
	...
	$message = \Thurly\MessageService\Sender\SmsManager::createMessage(array(
...
else if ($action === 'disable_demo')
{
	...
	$sendResponse(array(
		'success' => false,
		'errors' => array(\Thurly\Main\Localization\Loc::getMessage('MESSAGESERVICE_CONFIG_SENDER_SMS_DISABLE_DEMO_ERROR'))
	));
}
