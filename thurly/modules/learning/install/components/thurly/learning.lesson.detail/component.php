...
if ($this->StartResultCache(false,  $additionalCacheID))
{
	...
	$APPLICATION->AddHeadScript('/thurly/js/learning/scorm.js');
...
if ($bCanUserEdit || $bCanUserRemove)
{
	...
	if ($bCanUserRemove)
	{
		...
		$arAreaButtons[] = array(
				...
				"URL" => 'javascript:'.$APPLICATION->GetPopupLink(
					array(
						"URL" => "/thurly/admin/learn_unilesson_edit.php?LESSON_ID=" . $arParams["LESSON_ID"]
							. '&' . $strUrlencodedLessonPath
	...
	if ($bCanUserEdit)
	{
		$arAreaButtons[] = array(
				...
				"URL" => "javascript:if(confirm('".GetMessage("LEARNING_COURSES_LESSON_DELETE_CONF")."'))jsUtils.Redirect([], '".CUtil::JSEscape("/thurly/admin/learn_unilesson_admin.php?ID=".$arParams["LESSON_ID"]."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."&COURSE_ID=".$arParams["COURSE_ID"]).(strlen($deleteReturnUrl) ? "&return_url=".urlencode($deleteReturnUrl) : "")."')",
