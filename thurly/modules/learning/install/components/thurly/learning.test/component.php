...
if (!sizeof($errors))
{
	...
	//Redirect page
	if (!empty($_REQUEST["back_page"]) && check_thurly_sessid())
...
if ($bCanEdit)
{
	...
	$arAreaButtons = array(
		array(
			...
			"URL" => 'javascript:'.$APPLICATION->GetPopupLink(
				array(
					"URL" => "/thurly/admin/learn_test_edit.php?lang=".LANGUAGE_ID."&ID=".$arResult["TEST"]["ID"]."&COURSE_ID=".$arResult["TEST"]["COURSE_ID"]."&bxpublic=Y&from_module=learning",
					"PARAMS" => array(
		...
		array(
			...
			"URL" => "javascript:if(confirm('".GetMessage("LEARNING_COURSES_TEST_DELETE_CONF")."'))jsUtils.Redirect([], '".CUtil::JSEscape("/thurly/admin/learn_test_admin.php?ID=".$arParams["TEST_ID"]."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."&COURSE_ID=".$arParams["COURSE_ID"]).(strlen($deleteReturnUrl) ? "&return_url=".urlencode($deleteReturnUrl) : "")."')",
	...
	if ($arResult["QUESTION"]["ID"])
	{
		array_unshift($arAreaButtons, array(
			...
			"URL" => 'javascript:'.$APPLICATION->GetPopupLink(
				array(
					"URL" => "/thurly/admin/learn_question_edit.php?lang=".LANGUAGE_ID."&ID=".$arResult["QUESTION"]["ID"]."&COURSE_ID=".$arResult["TEST"]["COURSE_ID"]."&LESSON_ID=".$arResult["QUESTION"]["LESSON_ID"]."&bxpublic=Y&from_module=learning",
					"PARAMS" => array(
						"width" => 700, 'height' => 500, 'resize' => false,
					),
				)
