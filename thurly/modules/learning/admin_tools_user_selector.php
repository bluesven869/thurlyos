...
class Learning_CIBlockPropertyUserID
{
	function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
	{
		...
		{
			$select = "CU";
			$res = "[<a title='".GetMessage("LEARNING_USER_SELECTOR_USER_PROFILE")."'  href='/thurly/admin/user_edit.php?ID=".$USER->GetID()."&lang=".LANG."'>".$USER->GetID()."</a>] (".htmlspecialcharsbx($USER->GetLogin()).") ".htmlspecialcharsbx($USER->GetFirstName())." ".htmlspecialcharsbx($USER->GetLastName());
		}
		elseif ($default_value > 0)
		{
			...
			if ($arUser = $rsUsers->Fetch())
				$res = "[<a title='".GetMessage("LEARNING_USER_SELECTOR_USER_PROFILE")."'  href='/thurly/admin/user_edit.php?ID=".$arUser["ID"]."&lang=".LANG."'>".$arUser["ID"]."</a>] (".htmlspecialcharsbx($arUser["LOGIN"]).") ".htmlspecialcharsbx($arUser["NAME"])." ".htmlspecialcharsbx($arUser["LAST_NAME"]);
			else
...
function Learning_FindUserIDNew(
	...
	$search_page = "/thurly/admin/user_search.php"
)
{
	...
	if (!!DV_".$tag_name_x.")
	{
		...
		)
		{
			...
			if (tv".$tag_name_x."!='')
			{
				...
				{
					DV_".$tag_name_x.".innerHTML = '".CUtil::JSEscape("[<a title=\"".GetMessage("LEARNING_USER_SELECTOR_USER_PROFILE")."\" class=\"tablebodylink\" href=\"/thurly/admin/user_edit.php?ID=".$USER->GetID()."&lang=".LANG."\">".$USER->GetID()."</a>] (".htmlspecialcharsbx($USER->GetLogin()).") ".htmlspecialcharsbx($USER->GetFirstName())." ".htmlspecialcharsbx($USER->GetLastName()))."';
