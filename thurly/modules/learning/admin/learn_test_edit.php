...
if ($_SERVER["REQUEST_METHOD"] == "POST" && strlen($_POST["Update"])>0 && check_thurly_sessid() && $isCreateOrEditAccess)
{
	$test = new CTest;
	...
	if (!$bVarsFromForm)
	{
		...
		if(strlen($apply)<=0)
		{
			...
			{
				LocalRedirect("/thurly/admin/learn_test_admin.php?lang=".LANG
					. "&COURSE_ID=" . $COURSE_ID
					. '&PARENT_LESSON_ID=' . ($_GET['PARENT_LESSON_ID'] + 0)
					. '&LESSON_PATH=' . urlencode($_GET['LESSON_PATH'])
					.GetFilterParams("filter_", false));
		...
		LocalRedirect("/thurly/admin/learn_test_edit.php?lang=" . LANG 
			. "&COURSE_ID=" . $COURSE_ID
			. '&PARENT_LESSON_ID=' . ($_GET['PARENT_LESSON_ID'] + 0)
			. '&LESSON_PATH=' . urlencode($_GET['LESSON_PATH'])
			. "&ID=" . $ID
...
if ($ID > 0)
{
	...
	$returnUrl = "/thurly/admin/learn_test_admin.php?lang=" . LANG
		. "&amp;COURSE_ID=" . $COURSE_ID
		. '&amp;PARENT_LESSON_ID=' . ($_GET['PARENT_LESSON_ID'] + 0)
		. '&amp;LESSON_PATH=' . urlencode($_GET['LESSON_PATH'])
		. GetFilterParams("filter_", false);
	...
	$aContext[] = 	array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("LEARNING_CONFIRM_DEL_MESSAGE")."'))window.location='learn_test_admin.php?lang=".LANG
			. "&COURSE_ID=" . $COURSE_ID . "&action=delete&ID=" . $ID . "&" . thurly_sessid_get() . urlencode(GetFilterParams("filter_", false)) 
			...
			<table cellpadding="0" cellspacing="0" width="100%" class="internal" id="marks-table">
				...
				<?php
					...
					<tr>
						...
						<td align="center"><a href="javascript:void(0);" onclick="BX.remove(this.parentNode.parentNode)"><img src="/thurly/themes/.default/images/actions/delete_button.gif" border="0" width="20" height="20"/></a><input type="hidden" name="ANSWER_HIDDEN_ID[]" value="<?php echo (int) $i; ?>"></td>
					</tr>
				<?php endforeach?>
				</tbody>
			</table>
