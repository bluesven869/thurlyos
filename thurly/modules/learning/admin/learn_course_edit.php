...
if ( ($_SERVER["REQUEST_METHOD"] == "POST") && ($Perm >= "X") && (strlen($_POST["Update"])>0) && check_thurly_sessid() )
{
	$course = new CCourse;
	...
	else
	{
		...
		if(!$bVarsFromForm)
		{
			...
			LocalRedirect("/thurly/admin/learn_course_edit.php?lang=".LANG
				. $returnPath
				. "&COURSE_ID=".$COURSE_ID
	...
	if ($linkedLessonId !== false)
	{
		$aContext[] = array(
			...
			"LINK"	=> "javascript:if(confirm('".GetMessage("LEARNING_CONFIRM_DEL_MESSAGE")
				...
				."&lang=".LANG
				. $returnPathUrlencoded
				."&".thurly_sessid_get().urlencode(GetFilterParams("filter_", false))."';",
		);
	}
...
$tabControl->Buttons(
	array(
		...
		"back_url" => "/thurly/admin/learn_unilesson_admin.php?lang=".LANG."&PARENT_LESSON_ID=-1"
			. $uriParentLessonPath
			. GetFilterParams("filter_", false)
		)
	);
