...
if (!$bBadAttempt && $_SERVER["REQUEST_METHOD"] == "POST" && strlen($Update)>0 && check_thurly_sessid())
{
	$ta = new CTestAttempt;
	...
	else
	{
		if(strlen($apply)<=0)
		{
			if (strlen($return_url)>0)
				...
				LocalRedirect("/thurly/admin/learn_attempt_admin.php?lang=". LANG.GetFilterParams("filter_", false));
		}

		LocalRedirect("/thurly/admin/learn_attempt_edit.php?ID=".$ID."&tabControl_active_tab=".urlencode($tabControl_active_tab).GetFilterParams("filter_", false));
	}
}
...
if ($ID > 0)
{
	$aContext[] = 	array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("LEARNING_CONFIRM_DEL_MESSAGE")."'))window.location='learn_attempt_admin.php?lang=".LANG."&action=delete&ID=".$ID."&lang=".LANG."&".thurly_sessid_get().urlencode(GetFilterParams("filter_", false))."';",
	);
