...
{
	require($_SERVER['DOCUMENT_ROOT'] . '/thurly/modules/main/include/prolog_admin_after.php'); // second system's prolog
	...
	require($_SERVER['DOCUMENT_ROOT'] . '/thurly/modules/main/include/epilog_admin.php');	// system's epilog
	exit();
}

require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/learning/prolog.php");
...
if (!$bBadCourse && $_SERVER["REQUEST_METHOD"] == "POST" && strlen($Update)>0 && check_thurly_sessid())
{
	...
	else
	{
		...
		if (!$bVarsFromForm)
		{
			if(strlen($apply)<=0)
			{
				if ($from == "learn_admin")
				{
					LocalRedirect("/thurly/admin/learn_unilesson_admin.php?lang=" . LANG
				...
				{
					LocalRedirect("/thurly/admin/learn_question_admin.php?lang=" . LANG
			...
			LocalRedirect("/thurly/admin/learn_question_edit.php?lang=" . LANG
				. '&LESSON_PATH=' . $uriLessonPath
				. "&ID=" . $ID
...
<script type="text/javascript">
	...
	function _mediaParser(_str, pMainObj)
	{
		...
		var ReplaceWMV = function(str, b1, b2, b3, b4)
		{
			...
			return '<img  id="' + pMainObj.SetBxTag(false, {tag: 'media', params: arTagParams}) + '" src="/thurly/images/1.gif" style="border: 1px solid rgb(182, 182, 184); background-color: rgb(226, 223, 218); background-image: url(/thurly/images/learning/icons/media.gif); background-position: center center; background-repeat: no-repeat; width: '+w+'px; height: '+h+'px;" width="'+w+'" height="'+h+'" />';
		}
	...
	function _mediaUnParser(_node, pMainObj)
	{
		...
		if (bxTag && bxTag.tag && bxTag.tag == "media")
		{
			...
			if (!bWM) // FL
			{
				...
				str += '>';
				str += '<param name="movie" value="/thurly/components/thurly/player/mediaplayer/player">';

				var embed = '<embed src="/thurly/components/thurly/player/mediaplayer/player" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" pluginspage="http:/' + '/www.macromedia.com/go/getflashplayer" ';
				...
				var arParams = {
					...
					"flashvars" : {
						...
						"logo.hide" : "true",
						"skin": "/thurly/components/thurly/player/mediaplayer/skins/thurly.swf",
						"repeat" : "N",
						"bufferlength" : "10",
						"dock" : "true"
					}
			...
			{
				str = '<script type="text/javascript" src="/thurly/components/thurly/player/wmvplayer/silverlight.js" /><\/script>' +
				'<script type="text/javascript" src="/thurly/components/thurly/player/wmvplayer/wmvplayer.js"><\/script>' +
				'<div id="' + _node.arAttributes["id"] + '">WMV Player</div>' +
				'<script type="text/javascript">new jeroenwijering.Player(document.getElementById("' + _node.arAttributes["id"] + '"), "/thurly/components/thurly/player/wmvplayer/wmvplayer.xaml", {';
		...
		'action': function() {
			...
			if (path.length > 0 && parseInt(width) > 0 && parseInt(height) > 0)
			{
				...
				else
				{
					...
					pEditor.insertHTML('<img id="' + pEditor.SetBxTag(false, {tag: 'media', params: arParams}) + '" src="/thurly/images/1.gif" style="border: 1px solid rgb(182, 182, 184); background-color: rgb(226, 223, 218); background-image: url(/thurly/images/learning/icons/media.gif); background-position: center center; background-repeat: no-repeat; width: '+width+'px; height: '+height+'px;" width="'+width+'" height="'+height+'" />');
				}
			}
			pElement = null;
		}
	...
	var pMediaButton = [
		...
		{
			id : 'media',
			src : '/thurly/images/learning/icons/media.gif',
				...
				?>
					<tr>
						...
						<td align="center" width="40" style="text-align:center;">
							...
							<?else:?>
								<a href="javascript:void(0);" onclick="BX.remove(this.parentNode.parentNode)"><img src="/thurly/themes/.default/images/actions/delete_button.gif" border="0" width="20" height="20"/></a>
							<?endif?>
						...
						var row = BX.create("tr", {
							children: [
								...
								BX.create('td', {
									...
									html : '<a href="javascript:void(0);" onclick="BX.remove(this.parentNode.parentNode)"><img src="/thurly/themes/.default/images/actions/delete_button.gif" border="0" width="20" height="20"/></a><input type="hidden" name="ANSWER_HIDDEN_ID[]" value="n' + nextNum + '">'
								})
							]
						});
