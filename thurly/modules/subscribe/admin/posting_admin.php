...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/include.php");
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/prolog.php");
...
if($_REQUEST["action"]=="js_send" && check_thurly_sessid())
{
...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	if($f_STATUS=="D" && $POST_RIGHT=="W")
		$arActions[] = array(
			...
			"ACTION"=>"if(confirm('".GetMessage("post_conf")."')) window.location='".$APPLICATION->GetCurPage()."?ID=".$f_ID."&action=send&lang=".LANG."&".thurly_sessid_get()."'"
		);
	if($f_STATUS=="W" && $POST_RIGHT=="W")
		$arActions[] = array(
			"ICON"=>"",
			"TEXT"=>GetMessage("posting_continue_act"),
			"ACTION"=>"if(confirm('".GetMessage("posting_continue_conf")."')) window.location='".$APPLICATION->GetCurPage()."?ID=".$f_ID."&action=send&lang=".LANG."&".thurly_sessid_get()."'"
		);
	if($f_STATUS=="P" && $POST_RIGHT=="W")
		$arActions[] = array(
			"ICON"=>"",
			"TEXT"=>GetMessage("posting_stop_act"),
			"ACTION"=>"if(confirm('".GetMessage("posting_stop_conf")."')) window.location='".$APPLICATION->GetCurPage()."?ID=".$f_ID."&action=stop&lang=".LANG."&".thurly_sessid_get()."'"
		);
...
if($_REQUEST['action']=="send"):
	...
	<script>
		...
		function MoveProgress()
		{
			...
			var url = 'posting_admin.php?lang=<?echo LANGUAGE_ID?>&ID=<?echo $ID?>&<?echo thurly_sessid_get()?>&action=js_send';
			ShowWaitWindow();
			BX.ajax.post(
				url,
				null,
