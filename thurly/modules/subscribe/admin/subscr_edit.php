...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $POST_RIGHT>="W" && check_thurly_sessid())
{
	...
	{
		if($apply!="")
			LocalRedirect("/thurly/admin/subscr_edit.php?ID=".$ID."&mess=ok&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
			LocalRedirect("/thurly/admin/subscr_admin.php?lang=".LANG);
	}
	else
	{
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"javascript:if(confirm('".GetMessage("subscr_mnu_del_conf")."'))window.location='subscr_admin.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
