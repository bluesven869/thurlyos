...
if($REQUEST_METHOD == "POST" && ($save.$apply.$Send.$Resend.$Continue!="") && $POST_RIGHT=="W" && check_thurly_sessid())
{
	...
	if($res)
	{
		...
		{
			LocalRedirect("posting_admin.php?ID=".$ID."&action=send&lang=".LANG."&".thurly_sessid_get());
		}
...
if($ID>0 && !$bCopy)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"javascript:if(confirm('".GetMessage("post_mnu_confirm")."'))window.location='posting_admin.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
		...
		<?
			...
			?>
				<div class="adm-list-item">
					...
					<div class="adm-list-label"><label for="GROUP_ID_<?echo $ar["ID"]?>"><?echo $ar["NAME"]?>&nbsp;[<a href="/thurly/admin/group_edit.php?ID=<?echo $ar["ID"]?>&amp;lang=<?echo LANGUAGE_ID?>"><?echo $ar["ID"]?></a>]</label></div>
				</div>
			<?
				$n++;
			endwhile;
