...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $POST_RIGHT=="W" && check_thurly_sessid())
{
	...
	{
		if($apply!="")
			LocalRedirect("/thurly/admin/rubric_edit.php?ID=".$ID."&mess=ok&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
			LocalRedirect("/thurly/admin/rubric_admin.php?lang=".LANG);
	}
	else
	{
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"javascript:if(confirm('".GetMessage("rubric_mnu_del_conf")."'))window.location='rubric_admin.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
	$aMenu[] = array("SEPARATOR"=>"Y");
	$aMenu[] = array(
		...
		<tr>
			...
			<td>
				...
				<?if(IsModuleInstalled("fileman")):?>
					<a title="<?=GetMessage("rub_manage")?>" href="/thurly/admin/fileman_admin.php?path=<?=urlencode("/".$arTemplate["PATH"])?>"><?=$arTemplate["PATH"]?></a>
				<?else:?>
					<?=$arTemplate["PATH"]?>
				<?endif?>
			</td>
