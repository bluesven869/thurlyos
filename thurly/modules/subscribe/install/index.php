...
class subscribe extends CModule
{
	...
	function subscribe()
	{
		...
		$this->MODULE_CSS = "/thurly/modules/subscribe/styles.css";
	}
	...
	function InstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/db/".$DBType."/install.sql");
		}
		...
		else
		{
			...
			RegisterModuleDependences("sender", "OnConnectorList", "subscribe", "Thurly\\Subscribe\\SenderEventHandler", "onConnectorListSubscriber");
	...
	function UnInstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/db/".$DBType."/uninstall.sql");
		...
		UnRegisterModuleDependences("sender", "OnConnectorList", "subscribe", "Thurly\\Subscribe\\SenderEventHandler", "onConnectorListSubscriber");
	...
	function InstallEvents()
	{
		...
		if($ar["C"] <= 0)
		{
			include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/events.php");
		}
		return true;
	}
	...
	function InstallFiles($arParams = array())
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/themes", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes", false, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", True, True);
		}

		if(array_key_exists("install_auto_templates", $arParams) && $arParams["install_auto_templates"] == "Y")
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/php_interface", $_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/php_interface", false, true);
		}
		...
		)
		{
			...
			{
				$source = $_SERVER['DOCUMENT_ROOT']."/thurly/modules/subscribe/public/";
	...
	function UnInstallFiles()
	{
		...
		{
			//admin files
			DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
			//css
			DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/themes/.default/", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes/.default");
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if($POST_RIGHT == "W")
		{
			...
			if($step < 2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("inst_inst_title"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/inst1.php");
			}
			elseif($step==2)
			{
				if($this->InstallDB())
				...
				$APPLICATION->IncludeAdminFile(GetMessage("inst_inst_title"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/inst2.php");
			}
		}
	}
	...
	function DoUninstall()
	{
		...
		if($POST_RIGHT == "W")
		{
			...
			if($step < 2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("inst_uninst_title"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/uninst1.php");
			}
			elseif($step == 2)
			{
				...
				$this->UnInstallFiles();
				$GLOBALS["errors"] = $this->errors;
				$APPLICATION->IncludeAdminFile(GetMessage("inst_uninst_title"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/subscribe/install/uninst2.php");
			}
		}
	}
