...
namespace Thurly\Forum\Integration\SocialNetwork;

use Thurly\Forum\MessageTable;
	...
	public static function onIndexGetContent(Event $event)
	{
		...
		if (intval($sourceId) > 0)
		{
			...
			if (
				...
				&& \Thurly\Main\ModuleManager::isModuleInstalled('disk')
			)
			{
		...
		if ($message)
		{
			...
			{
				$metadata = \Thurly\Main\UrlPreview\UrlMetadataTable::getRowById($message['UF_FORUM_MES_URL_PRV']);
				if (
					$metadata
