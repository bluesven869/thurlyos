...
class User {
	...
	protected function __construct($id)
	{
		...
		{
			$user = \Thurly\Main\UserTable::getList(array(
				'select' => array('*'),
	...
	public function setLastVisit()
	{
		...
		$merge = $helper->prepareMerge(
			...
			array(
				...
				"ALLOW_POST" => "Y",
				"USER_ID" => $this->getId(),
				"DATE_REG" => new \Thurly\Main\DB\SqlExpression($helper->getCurrentDateTimeFunction()),
				"LAST_VISIT" => new \Thurly\Main\DB\SqlExpression($helper->getCurrentDateTimeFunction())
			),
			array(
				"LAST_VISIT" => new \Thurly\Main\DB\SqlExpression($helper->getCurrentDateTimeFunction())
			)
