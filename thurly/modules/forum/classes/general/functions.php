...
{
	global $MESS;
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/lang/".LANGUAGE_ID."/errors.php");
	$msg = $MESS[$error["MSG"]];
	echo "Error: ".$msg;
}
...
class forumTextParser extends CTextParser
{
	...
	function convert_attachment($fileID = "", $p = "", $type = "", $text = "")
	{
		...
		if (!array_key_exists($fileID, $this->arFilesParsed) || empty($this->arFilesParsed[$fileID][$type]))
		{
			...
			{
				$path = '/thurly/components/thurly/forum.interface/show_file.php?fid='.$arFile["ID"];
...
class textParser
{
	...
	function textParser($strLang = False, $pathToSmile = false)
	{
		...
		$pathToSmile = ($pathToSmile === false ? "/thurly/images/forum/smile/" : $pathToSmile);
...
//		$text = str_replace(array("  ", "\t", ), array("&nbsp;&nbsp;", "&nbsp;&nbsp;&nbsp;"), $text);
	...
	function convert_image_tag($url = "", $type = "html")
	{
		...
		$bErrorIMG = False;
		if (strpos($url, "/thurly/components/thurly/forum.interface/show_file.php?fid=") === false)
		{
...
?>
		...
		else
		{
			...
			BX.loadScript("<?=CUtil::GetAdditionalFileURL("/thurly/js/forum/autosave.js")?>");
		}
		</script>
<?
		ob_end_flush();
