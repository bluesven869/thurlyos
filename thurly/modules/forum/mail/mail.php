...
class CForumEMail
{
	...
	function OnGetSocNetFilterList()
	{
		return Array(
			...
			"ACTION_INTERFACE"		=>	$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/mail/action.php",
...
//		$arParams["LOG_ID"], $arParams["LANG"] ....
	...
	{
		$MESS = Array();
		if(file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/forum/lang/'.$lang.'/mail/mail.php'))
			include($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/forum/lang/'.$lang.'/mail/mail.php');
		if($MESS[$ID])
			return $MESS[$ID];
		include($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/forum/lang/en/mail/mail.php');
		return $MESS[$ID];
	}
}
