...
class CCommentRatings
{
	...
	function OnCommentsDisplayTemplate($output, $arParams, $arResult)
	{
		...
		{
			$GLOBALS["APPLICATION"]->SetAdditionalCSS("/thurly/components/thurly/rating.vote/templates/like/popup.css");
			$GLOBALS["APPLICATION"]->SetAdditionalCSS("/thurly/components/thurly/rating.vote/templates/".$arParams["RATING_TYPE"]."/style.css");
		}
	}
		...
		ob_start();
			...
			else
			{
				...
				<?
					...
					$GLOBALS["APPLICATION"]->IncludeComponent("thurly:rating.vote", $arParams["RATING_TYPE"], $arRatingParams, $this->component, array("HIDE_ICONS" => "Y"));
			if ($top)
			{
				?>
				</div>
