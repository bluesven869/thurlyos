...
Class forum extends CModule
{
	...
	function InstallDB()
	{
		...
		{
			$this->errors = $GLOBALS["DB"]->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/".$GLOBALS["DBType"]."/install.sql");
		...
		RegisterModuleDependences('conversion', 'OnGetCounterTypes' , 'forum', '\Thurly\Forum\Internals\ConversionHandlers', 'onGetCounterTypes');
		RegisterModuleDependences('conversion', 'OnGetRateTypes' , 'forum', '\Thurly\Forum\Internals\ConversionHandlers', 'onGetRateTypes');
		RegisterModuleDependences('forum', 'onAfterTopicAdd', 'forum', '\Thurly\Forum\Internals\ConversionHandlers', 'onTopicAdd');
		RegisterModuleDependences('forum', 'onAfterMessageAdd', 'forum', '\Thurly\Forum\Internals\ConversionHandlers', 'onMessageAdd');

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->registerEventHandler('socialnetwork', 'onLogIndexGetContent', 'forum', '\Thurly\Forum\Integration\Socialnetwork\Log', 'onIndexGetContent');
		$eventManager->registerEventHandler('socialnetwork', 'onLogCommentIndexGetContent', 'forum', '\Thurly\Forum\Integration\Socialnetwork\LogComment', 'onIndexGetContent');
		$eventManager->registerEventHandler('socialnetwork', 'onContentViewed', 'forum', '\Thurly\Forum\Integration\Socialnetwork\ContentViewHandler', 'onContentViewed');
		...
		if ($GLOBALS["DB"]->TableExists("b_forum_pm_folder") || $GLOBALS["DB"]->TableExists("B_FORUM_PM_FOLDER"))
		{
			...
			{
				$this->errors = $GLOBALS["DB"]->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/".$GLOBALS["DBType"]."/install2.sql");
			}
		}
		if ($arInstall["INSTALL_FILTER"] == "Y")
		{
				...
				($GLOBALS["DB"]->TableExists("b_forum_filter") || $GLOBALS["DB"]->TableExists("B_FORUM_FILTER")))
			{
				...
				while($site = $sites->Fetch())
				{
					...
					{
						if(file_exists(	$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/".$GLOBALS["DBType"]."/".$site["LID"]."/".$site["LID"].".sql"))
							$this->errors = $GLOBALS["DB"]->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/".$GLOBALS["DBType"]."/".$site["LID"]."/".$site["LID"].".sql");
					}
					if ($site["LID"] == "ru")
					{
						GenPatternAll(1, 2);
	...
	function UnInstallDB($arParams = array())
	{
		...
		if(array_key_exists("savedata", $arParams) && $arParams["savedata"] != "Y")
		{
			...
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/".$GLOBALS["DBType"]."/uninstall.sql");
		}
		...
		UnRegisterModuleDependences('conversion', 'OnGetCounterTypes' , 'forum', '\Thurly\Forum\Internals\ConversionHandlers', 'onGetCounterTypes');
		UnRegisterModuleDependences('conversion', 'OnGetRateTypes' , 'forum', '\Thurly\Forum\Internals\ConversionHandlers', 'onGetRateTypes');
		UnRegisterModuleDependences('forum', 'onAfterTopicAdd', 'forum', '\Thurly\Forum\Internals\ConversionHandlers', 'onTopicAdd');
		UnRegisterModuleDependences('forum', 'onAfterMessageAdd', 'forum', '\Thurly\Forum\Internals\ConversionHandlers', 'onMessageAdd');

		UnRegisterModuleDependences("main", "OnAfterUserUpdate", "forum", "CForumUser", "OnAfterUserUpdate");

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->unregisterEventHandler('socialnetwork', 'onLogIndexGetContent', 'forum', '\Thurly\Forum\Integration\Socialnetwork\Log', 'onIndexGetContent');
		$eventManager->unregisterEventHandler('socialnetwork', 'onLogCommentIndexGetContent', 'forum', '\Thurly\Forum\Integration\Socialnetwork\LogComment', 'onIndexGetContent');
		$eventManager->unregisterEventHandler('socialnetwork', 'onContentViewed', 'forum', '\Thurly\Forum\Integration\Socialnetwork\ContentViewHandler', 'onContentViewed');
	...
	{
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/events.php");
		return true;
	}
	...
	function InstallFiles()
	{
		...
		{
			CheckDirPath($_SERVER["DOCUMENT_ROOT"]."/thurly/images/forum/", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/images",  $_SERVER["DOCUMENT_ROOT"]."/thurly/images/forum", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/themes", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/js", $_SERVER["DOCUMENT_ROOT"]."/thurly/js/forum", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/public/templates", $_SERVER["DOCUMENT_ROOT"]."/thurly/templates", true, true);
		}
		return true;
	}
	...
	function UnInstallFiles()
	{
		if($_SERVER["DevServer"] != "Y" && $_ENV["COMPUTERNAME"]!="BX")
		{
			DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
			DeleteDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/themes/.default/",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/themes/.default");//css
			DeleteDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/public/templates/.default/page_templates/forum/",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/templates/.default/page_templates/forum/");//page template
			DeleteDirFilesEx("/thurly/themes/.default/icons/forum/");//icons
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if($step != 2)
			$GLOBALS["APPLICATION"]->IncludeAdminFile(GetMessage("FORUM_INSTALL1"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/do_install1.php");
		else
		{
			// Check Fatal errors
			if (!$this->InstallDB() || !empty($this->errors))
			{
				...
				$GLOBALS["APPLICATION"]->IncludeAdminFile(GetMessage("FORUM_INSTALL2"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/do_install2.php");
			}
	...
	function DoUninstall()
	{
		...
		$step = IntVal($_REQUEST["step"]);
		if($step<2)
			$GLOBALS["APPLICATION"]->IncludeAdminFile(GetMessage("FORUM_DELETE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/do_uninstall1.php");
		else
		{
			...
			$GLOBALS["errors"] = $this->errors;
			$GLOBALS["APPLICATION"]->IncludeAdminFile(GetMessage("FORUM_DELETE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/install/do_uninstall2.php");
		}
	}
}
?>
