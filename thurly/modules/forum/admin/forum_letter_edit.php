...
********************************************************************/
	...
	$arFields = array();
/*******************************************************************/
	if ($REQUEST_METHOD=="POST" && ($Update = 'Y') && $forumPermWrite && check_thurly_sessid())
	{
		$erMsg = array();
...
/*******************************************************************/
	...
	$aMenu = array(
		array(
			...
			"LINK" => "/thurly/admin/forum_letter.php?DICTIONARY_ID=".$DICTIONARY_ID."&lang=".LANG,
			"ICON" => "btn_list",
		)
	);
	...
	if ($ID > 0 && $forumPermWrite)
	{
		...
		$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/forum_dictionary_edit.php?DICTIONARY_ID=".$DICTIONARY_ID."&lang=".LANG,
			"ICON" => "btn_new",
		);
		$aMenu[] = array(
			"TEXT" => GetMessage("FLTR_DEL"), 
			"LINK" => "javascript:if(confirm('".GetMessage("FLTR_DEL_CONFIRM")."')) window.location='/thurly/admin/forum_letter.php?DICTIONARY_ID=".$DICTIONARY_ID."&lang=".LANG."&action=delete&ID[]=".$ID."&".thurly_sessid_get()."';",
			"ICON" => "btn_delete",
		);
	}
...
<?$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/forum_letter.php?DICTIONARY_ID=".$DICTIONARY_ID."&lang=".LANG
			)
	);?>
<?$tabControl->End();?>
</form>
