...
********************************************************************/
	...
	$arFields = array();
/*******************************************************************/
	if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && (CFilterUnquotableWords::FilterPerm()) && check_thurly_sessid())
	{
...
/*******************************************************************/
	...
	$aMenu = array(
		array(
			...
			"LINK" => "/thurly/admin/forum_words.php?DICTIONARY_ID=".$DICTIONARY_ID."&lang=".LANG,
			"ICON" => "btn_list",
		)
	);
	...
	if ($ID > 0 && $forumPermissions == "W")
	{
		...
		$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/forum_words_edit.php?DICTIONARY_ID=".$DICTIONARY_ID."&lang=".LANG,
			"ICON" => "btn_new",
		);
		$aMenu[] = array(
			"TEXT" => GetMessage("FLTR_DEL"), 
			"LINK" => "javascript:if(confirm('".GetMessage("FLTR_DEL_CONFIRM")."')) window.location='/thurly/admin/forum_words.php?DICTIONARY_ID=".$DICTIONARY_ID."&lang=".LANG."&action=delete&ID[]=".$ID."&".thurly_sessid_get()."';",
			"ICON" => "btn_delete",
		);
	}
...
<?$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/forum_words.php?DICTIONARY_ID=".$DICTIONARY_ID."&lang=".LANG
			)
	);?>
<?$tabControl->End();?>
</form>
