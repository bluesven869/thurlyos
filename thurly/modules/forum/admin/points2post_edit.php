...
$message = false;
$bInitVars = false;
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $forumPermissions=="W" && check_thurly_sessid())
{
...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/forum_points2post.php?lang=".LANG."&".GetFilterParams("filter_", false),
		"ICON" => "btn_list",
	)
);
...
if ($ID > 0 && $forumPermissions == "W")
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/forum_points2post_edit.php?lang=".LANG."&".GetFilterParams("filter_", false),
		"ICON" => "btn_new",
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("FPPN_DELETE_POINT"), 
		"LINK" => "javascript:if(confirm('".GetMessage("FPPN_DELETE_POINT_CONFIRM")."')) window.location='/thurly/admin/forum_points2post.php?action=delete&ID[]=".$ID."&lang=".LANG."&".thurly_sessid_get()."#tb';",
		"ICON" => "btn_delete",
	);
}
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/forum_points2post.php?lang=".LANG."&".GetFilterParams("filter_", false)
			)
	);
$tabControl->End();
?>
