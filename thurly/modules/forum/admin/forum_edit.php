...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/include.php");
IncludeModuleLangFile(__FILE__);
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/forum/prolog.php");
...
/********************************************************************
				Action
********************************************************************/
if ($_SERVER['REQUEST_METHOD'] == "POST" && $forumPermissions >= "W" && $_REQUEST["Update"] == "Y" && check_thurly_sessid())
{
...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/forum_admin.php?lang=".LANG."&".GetFilterParams("filter_", false),
		"ICON" => "btn_list",
	)
);
...
if ($ID > 0 && $forumPermissions >= "W")
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/forum_edit.php?lang=".LANG."&".GetFilterParams("filter_", false),
		"ICON" => "btn_new",
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("FEN_DELETE_FORUM"), 
		"LINK" => "javascript:if(confirm('".GetMessage("FEN_DELETE_FORUM_CONFIRM")."')) window.location='/thurly/admin/forum_admin.php?action=delete&ID[]=".$ID."&lang=".LANG."&".thurly_sessid_get()."';",
		"ICON" => "btn_delete",
	);
}
...
<?
	...
	while ($res = $db_res->GetNext())
	{
		...
		?>
		<tr>
			<td width="40%"><?=$res["NAME"]?>&nbsp;[<a  href="/thurly/admin/group_edit.php?ID=<?=$res["ID"]?>&lang=<?=LANGUAGE_ID?>"><?=$res["ID"]?></a>]:</td>
			<td width="60%">
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/forum_admin.php?lang=".LANG."&".GetFilterParams("filter_", false)
			)
	);
?>
...
if(IsModuleInstalled("search"))
{
	?><?=GetMessage("REQUIRE_REINDEX",array("#LINK#" => "/thurly/admin/search_reindex.php"))?> <?
}
