	...
	function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName)
	{
		if (CModule::IncludeModule("forum"))
		{
			...
			else
			{
				...
				if ($arTopic)
					$res = "[<a title='".GetMessage("IBLOCK_PROP_FORUM_VIEW_TOPIC")."' class='tablebodylink' href='/thurly/admin/forum_topics.php?lang=".LANG."'>".intVal($arTopic["ID"])."</a>] (".htmlspecialcharsbx($arTopic["TITLE"]).") ";
				else
...
function FindTopicID($tag_name, $tag_value, $topic_name="", $form_name = "form1", $tag_size = "3", $tag_maxlength="", $button_value = "...", $tag_class="typeinput", $button_class="tablebodybutton", $search_page="/thurly/admin/forum_topics_search.php")
{
	global $APPLICATION;
...
")."
		if (val!==false && top.tv".$tag_name_x."!=val) {
			...
			if (top.tv".$tag_name_x."!='')
			{
				...
				top.BX('hiddenframe".$tag_name_js."').src='/thurly/admin/get_topics.php?ID='+top.tv".$tag_name_x."+'&strName=".$tag_name_js."&lang=".LANG.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"")."';
			}
			else
