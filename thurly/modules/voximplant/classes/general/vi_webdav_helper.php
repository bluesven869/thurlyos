...
class CVoxImplantDiskHelper
{
	...
	public static function GetStorageModel($siteId = SITE_ID)
	{
		...
		$storageModel = \Thurly\Disk\Driver::getInstance()->getStorageByCommonId('shared_files_'.$siteId);
		if (!$storageModel)
		{
			return false;
		}
	...
	public static function GetRootFolder($siteId = SITE_ID)
	{
		...
		{
			// Access codes
			$rightsManager = \Thurly\Disk\Driver::getInstance()->getRightsManager();
	...
	/**
	 * @param \Thurly\Main\Type\DateTime $recordDate
	 * @return \Thurly\Disk\Folder|false
	 */
	public static function GetRecordsFolder($folderName, $siteId = SITE_ID)
	{
		...
		$subFolder = \Thurly\Disk\Folder::load(array(
			'=NAME' => $folderName,
	...
	public static function CheckParams($arHistory, $arFile)
	{
		...
		if (!($arHistory['CALL_START_DATE'] instanceof Thurly\Main\Type\DateTime))
			return false;
