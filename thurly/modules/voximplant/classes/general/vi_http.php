...
class CVoxImplantHttp
{
	...
	private $controllerUrl = 'https://telephony.thurly.info/telephony/portal.php';
	private $licenceCode = '';
	private $domain = '';
	private $type = '';
	private $error = null;
	...
	public function StartOutgoingCall($userId, $phoneNumber)
	{
		$query = $this->Query(
			'StartOutgoingCall',
			Array('TYPE' => 'phone', 'USER_ID' => intval($userId), 'NUMBER' => $phoneNumber, 'IP' => Thurly\Main\Application::getInstance()->getContext()->getRequest()->getRemoteAddress())
		);
		if (isset($query->error))
		{
			$this->error = new CVoxImplantError(__METHOD__, $query->error->code, $query->error->msg);
	...
	public function AddPhoneOrder($params)
	{
		$query = $this->Query(
			'AddPhoneOrder',
			Array('FORM_DATA' => Thurly\Main\Web\Json::encode($params))
		);
	...
	public function GetDocumentStatus()
	{
		...
		foreach ($query as $key => $verification)
		{
			if (isset($verification->DOCUMENTS))
			{
				...
				{
					$query[$key]->DOCUMENTS[$id]->REVIEWER_COMMENT = \Thurly\Main\Text\Encoding::convertEncodingToCurrent($document->REVIEWER_COMMENT);
				}
			}
		}
	...
	public function setAutoPayAllowed($allowAutoPay)
	{
		...
		{
			throw new \Thurly\Main\ArgumentException('$autoPayAllowed should be bool', '$autoPayAllowed');
		}
	...
	private function Query($command, $params = array())
	{
		...
		$httpClient->setHeader('User-Agent', 'Thurly Telephony');
		$httpClient->setCharset(\Thurly\Main\Context::getCurrent()->getCulture()->getCharset());
	...
	public static function GetPortalSign()
	{
		if(defined('BX24_HOST_NAME') || defined('VOXIMPLANT_HOST_NAME'))
		{
			return self::RequestSign(self::TYPE_THURLY24, defined('BX24_HOST_NAME')? md5(BX24_HOST_NAME): md5(VOXIMPLANT_HOST_NAME));
		}
		else
		{
	...
	{
		return CVoxImplantHttp::GetServerAddress().'/thurly/tools/voximplant/receiver.php?b24_direct=y';
	}
