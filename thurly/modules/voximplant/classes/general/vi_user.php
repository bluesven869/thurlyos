...
use Thurly\Voximplant\Security\Helper;
use Thurly\Voximplant\Security\Permissions;
...
class CVoxImplantUser
{
	...
	public function getAuthorizationInfo($userId, $autoRegister = false)
	{
		...
		if ($userId <= 0)
		{
			$result->addError(new \Thurly\Main\Error('userId is empty'));
			return $result;
		}

		if(!\Thurly\Voximplant\Integration\ThurlyOS::isEmailConfirmed())
		{
			$result->addError(new \Thurly\Main\Error(GetMessage('VI_ERROR_EMAIL_NOT_CONFIRMED_2')));
			return $result;
		}

		$arUser = \Thurly\Main\UserTable::getRow(array(
			'select' => array('UF_VI_PASSWORD', 'UF_DEPARTMENT'),
			'filter' => array('=ID' => $userId)
		));
		if(!$arUser)
		{
			$result->addError(new \Thurly\Main\Error(GetMessage('VI_ERROR_USER_NOT_FOUND')));
			return $result;
		}

		if(static::IsExtranet($arUser))
		{
			$result->addError(new \Thurly\Main\Error(GetMessage('VI_ERROR_USER_NO_EXTRANET')));
			return $result;
		}
		...
		if($arUser['UF_VI_PASSWORD'] == '')
		{
			if(!$autoRegister)
			{
				$result->addError(new \Thurly\Main\Error(GetMessage('VI_ERROR_USER_NOT_REGISTERED')));
				return $result;
			}
			$registerResult = $this->GetUser($userId);
			if (!$registerResult || $this->GetError()->error)
			{
				$result->addError(new \Thurly\Main\Error($this->GetError()->msg, $this->GetError()->code));
				return $result;
			}
		...
		$result->setData(array(
			'server' => str_replace('voximplant.com', 'thurlyphone.com', $viAccount->GetCallServer()),
			'login' => 'user'.$userId,
	...
	public function GetUserInfo($userId, $getPhoneAccess = false)
	{
		...
		return Array(
			'call_server' => str_replace('voximplant.com', 'thurlyphone.com', $viAccount->GetCallServer()),
	...
	{
		$viAccount = new CVoxImplantAccount();
		return str_replace('voximplant.com', 'thurlyphone.com', $viAccount->GetCallServer($update));
	}
	...
	{
		$cache = \Thurly\Main\Application::getInstance()->getManagedCache();
		...
		{
			$outgoingLine = '';
			$userData = \Thurly\Main\UserTable::getRow(array(
	...
	public static function GetList($params)
	{
		...
		$query->registerRuntimeField('', new \Thurly\Main\Entity\ExpressionField('IS_ONLINE_CUSTOM', 'CASE WHEN LAST_ACTIVITY_DATE > '.self::GetLastActivityDateAgo().' THEN \'Y\' ELSE \'N\' END'));
	...
	public static function GetLastActivityDateAgo()
	{
		...
		return Thurly\Main\Application::getConnection()->getSqlHelper()->addSecondsToDateTime('(-'.$lastActivityDate.')');
	}
	...
	public static function GetByPhone($phone, $type = CVoxImplantPhone::PHONE_USER_INNER)
	{
		...
		$result = Thurly\Voximplant\PhoneTable::getList(Array(
	...
	public static function hasMobile($userId)
	{
		...
		$cursor = \Thurly\Pull\PushTable::getList(Array(
			'select' => Array('ID'),
	...
	{
		if(!\Thurly\Voximplant\Limits::canSelectLine())
			return true;
	...
	public static function getAllowedLines($userId)
	{
		...
		$cursor = \Thurly\Voximplant\Model\LineAccessTable::getList(array(
			'select' => array(
				new \Thurly\Main\Entity\ExpressionField('DISTINCT_SEARCH_ID', 'DISTINCT %s', 'LINE_SEARCH_ID'),
		...
		{
			if($row['DISTINCT_SEARCH_ID'] == $defaultLineId) $defaultLineFound = true;
			$result[] = $row['DISTINCT_SEARCH_ID'];
		}
		if(!$defaultLineFound)
		{
	 ...
	 * @return array
	 */
	public static function getUsersWithNotDefaultNumber()
	{
		$result = array();
		...
		$cursor = \Thurly\Main\UserTable::getList(array(
			...
			'filter' => array(
				'=UF_VI_BACKPHONE' => '',
				'!=EXTERNAL_AUTH_ID' => array("bot", "email", "controller", "replica", "imconnector")
			),
			'order' => array(
				'ID' => 'asc'
			)
