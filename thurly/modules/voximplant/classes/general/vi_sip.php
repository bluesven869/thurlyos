...
class CVoxImplantSip
{
	...
	public function Add($fields)
	{
		...
		{
			$countQuery = new \Thurly\Main\Entity\Query(VI\SipTable::getEntity());
	...
	public function Update($configId, $fields)
	{
		...
		if (isset($arUpdate['SEARCH_ID']))
		{
			$result = \Thurly\Voximplant\ConfigTable::getById($configId);
	...
	public function Get($configId, $params = Array())
	{
		...
		else
		{
			if (empty($row['INCOMING_SERVER']) && empty($row['INCOMING_LOGIN']) && empty($row['INCOMING_PASSWORD']))
			{
				...
				if ($result)
				{
					$row['INCOMING_SERVER'] = str_replace(Array('incoming.', '.voximplant.com'), Array('ip.', '.thurlyphone.com'), $result->server);
			...
			{
				$row['INCOMING_SERVER'] = str_replace(Array('incoming.', '.voximplant.com'), Array('ip.', '.thurlyphone.com'), $row['INCOMING_SERVER']);
			}
			unset($row['REG_ID']);
		}
