...
class CVoxImplantIncoming
{
	...
	public static function GetConfig($params)
	{
		...
		$isBlacklistAutoEnable = Thurly\Main\Config\Option::get("voximplant", "blacklist_auto", "N") == "Y";
	...
	public static function Init($params)
	{
		...
		if ($config['CRM'] == 'Y')
		{
			...
			if ($crmData['FOUND'] == 'N'
				...
				&& ($config['CRM_CREATE_CALL_TYPE'] == CVoxImplantConfig::CRM_CREATE_CALL_TYPE_INCOMING || $config['CRM_CREATE_CALL_TYPE'] == CVoxImplantConfig::CRM_CREATE_CALL_TYPE_ALL)
			)
			{
	...
	public static function SendCommand($params, $waitResponse = false)
	{
		...
		if (!$call)
		{
			$result->addError(new \Thurly\Main\Error('Call not found', 'NOT_FOUND'));
			return $result;
		}
		...
		if($waitResponse)
		{
			if ($queryResult === false)
			{
				...
				if(count($httpClientErrors) > 0)
				{
					...
					{
						$result->addError(new \Thurly\Main\Error($message, $code));
					}
				}
			}
			...
			{
				$result->addError(new \Thurly\Main\Error('Call scenario is not running', 'NOT_FOUND'));
			}
			else
			{
				$result->addError(new \Thurly\Main\Error("Scenario server returns code " . $http->getStatus()));
	...
	public static function RegisterCall($config, $params)
	{
		...
		if($call)
		{
			VI\CallTable::update($call['ID'], array(
				'CONFIG_ID' => $config['ID'],
				'USER_ID' => 0,
				'STATUS' => Thurly\Voximplant\CallTable::STATUS_WAITING,
		...
		else
		{
			...
			$insertResult = Thurly\Voximplant\CallTable::add($call);
	...
	{
		$blackListTime = Thurly\Main\Config\Option::get("voximplant", "blacklist_time", 5);
		$blackListCount = Thurly\Main\Config\Option::get("voximplant", "blacklist_count", 5);

		$minTime = new Thurly\Main\Type\DateTime();
		...
		while($dbData->fetch())
		{
			...
			if ($callsCount >= $blackListCount)
			{
				...
				$messageUserId = Thurly\Main\Config\Option::get("voximplant", "blacklist_user_id", "");
			...
			'filter' => array(
				...
				array(
					...
					array(
						...
						'=QUEUE.\Thurly\Voximplant\Model\QueueUserTable:QUEUE.USER_ID' => $userId
					),
					array('@USER_ID' => new \Thurly\Main\DB\SqlExpression("
						SELECT 
							QU.USER_ID 
						FROM 
