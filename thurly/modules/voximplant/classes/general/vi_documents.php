...
class CVoxImplantDocuments
{
	...
	public function  GetUploadUrl($country_code = 'RU', $address_type = null, $phone_category = null, $phone_region_code = null)
	{
		if($country_code == 'RU')
		{
			...
			return 'https://verify.voximplant.com/?account_id='.$params['ACCOUNT_ID'].'&admin_user_name='.$params['ADMIN_NAME'].'&verification_name='.$country_code.'&session_id='.$params['SESSION_ID'].'&vendor=thurly';
		}
		else
		{
			...
			$language = Thurly\Main\Context::getCurrent()->getLanguage();
	...
	public function GetStatus()
	{
		...
		foreach ($result as $key => $verification)
		{
			...
			{
				$data = new Thurly\Main\Type\DateTime($verification->UNVERIFIED_HOLD_UNTIL, 'Y-m-d H:i:s');
				$verifications[$key]['UNVERIFIED_HOLD_UNTIL'] = $data->format(Thurly\Main\Type\Date::getFormat());
			}
			else
			{
			...
			if (isset($verification->DOCUMENTS))
			{
				...
				{
					$data = new Thurly\Main\Type\DateTime($document->UPLOADED, 'Y-m-d H:i:s');

					$verifications[$key]['DOCUMENTS'][] = array(
						'UPLOADED' => $data->format(Thurly\Main\Type\DateTime::getFormat()),
	...
	public function setFilledByUser($userId)
	{
		...
		\Thurly\Main\Config\Option::set('voximplant', 'documents_filled_by', $userId);
	}
	...
	{
		$lastFilledBy = (int)\Thurly\Main\Config\Option::get('voximplant', 'documents_filled_by');
