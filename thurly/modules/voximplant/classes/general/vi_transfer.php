...
class CVoxImplantTransfer
{
	...
	public static function Invite($callId, $transferType, $transferUserId = 0, $transferPhone = '')
	{
		if($transferType !== self::TYPE_USER && $transferType !== self::TYPE_PHONE)
			throw new \Thurly\Main\ArgumentException('unsupported parameter value', 'transferType');
		...
		if($transferType == self::TYPE_PHONE && $transferPhone == '')
			throw new \Thurly\Main\ArgumentException('transferPhone is empty', 'transferPhone');

		if($transferType == self::TYPE_USER && $transferUserId == 0)
			throw new \Thurly\Main\ArgumentException('transferUserId is empty', 'transferUserId');
	...
	public static function Wait($callId)
	{
		...
		if (!$call)
		{
			$result->addError(new \Thurly\Main\Error('Call is not found', 'ERROR_NOT_FOUND'));
			return $result;
		}

		if ($call['TRANSFER_TYPE'] == '')
		{
			$result->addError(new \Thurly\Main\Error('Wrong call state', 'ERROR_WRONG_STATE'));
			return $result;
		}
		...
		$http->post($call['ACCESS_URL'], \Thurly\Main\Web\Json::encode($command));
		self::SendPullEvent(Array(
			'COMMAND' => 'waitTransfer',
	...
	public static function SendPullEvent($params)
	{
		...
		\Thurly\Pull\Event::add($params['USER_ID'],
			Array(
	 ...
	 * @see \Thurly\Voximplant\CallTable.
	 * Return array.
	 */
	protected function getTransferConfig(array $callFields)
	{
		$callConfig = VI\ConfigTable::getById($callFields['CONFIG_ID'])->fetch();
		if($callConfig == false)
		{
			throw new \Thurly\Main\SystemException('Config not found for call ' .$callFields['CALL_ID']);
		}
