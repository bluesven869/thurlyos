...
class CVoxImplantPhone
{
	...
	public static function SynchronizeUserPhones()
	{
		...
		$result = \Thurly\Main\UserTable::getList(Array(
			...
			'filter' => Array('=Thurly\Voximplant\Phone:USER.USER_ID'=>0),
			'limit' => 100,
			'offset' => $offset,
			'order' => 'ID'
		));
	...
	public static function GetCallerId()
	{
		...
		if ($result && !empty($result->result))
		{
			...
			$arResult['PHONE_NUMBER_FORMATTED'] = \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($phone->callerid_number)->format();
	...
	public static function GetRentNumbers()
	{
		...
		if ($result && !empty($result->result))
		{
			foreach ($result->result as $value)
			{
				...
				{
					$data = new Thurly\Main\Type\DateTime($value->phone_next_renewal.' 00:00:00', 'Y-m-d H:i:s');
					$renewalDate = $data->format(Thurly\Main\Type\Date::getFormat());
				...
				{
					$data = new Thurly\Main\Type\DateTime($value->unverified_hold_until.' 00:00:00', 'Y-m-d H:i:s');
					$unverifiedHoldDate = $data->format(Thurly\Main\Type\Date::getFormat());
				...
				$arResult[$value->phone_number] = Array(
					...
					'FORMATTED_NUMBER' => \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($value->phone_number)->format(),
	 ...
	 * @return \Thurly\Voximplant\Result
	 */
	...
	{
		$result = new \Thurly\Voximplant\Result();
		if (!in_array($category, self::GetCategories()))
		{
			$result->addError(new \Thurly\Main\Error(Loc::getMessage("VI_PHONE_ATTACH_ERROR_UNKNOWN_REGION")));
			return $result;
		}
		...
		else
		{
			...
			$result->addError(new \Thurly\Main\Error($errorMessage, $errorCode));
			return $result;
		}
	...
	public static function CheckDeleteAgent($configId)
	{
		$result = VI\ConfigTable::getList(Array(
			'select' => Array('ID'),
			'filter' => Array('=ID' => intval($configId), '=TO_DELETE' => 'Y', '<DATE_DELETE' => new Thurly\Main\Type\DateTime())
		));
		while($row = $result->fetch())
		{
