...
use Thurly\Voximplant\Rest;
use Thurly\Voximplant\Integration;
...
class CVoxImplantRestService extends IRestService
{
		...
		return array(
			'telephony' => array(
				...
				CRestUtil::EVENTS => array(
					'OnVoximplantCallInit' => array('voximplant', 'onCallInit', array(__CLASS__, 'onCallInit'), array("category" => \Thurly\Rest\Sqs::CATEGORY_TELEPHONY)),
					'OnVoximplantCallStart' => array('voximplant', 'onCallStart', array(__CLASS__, 'onCallStart'), array("category" => \Thurly\Rest\Sqs::CATEGORY_TELEPHONY)),
					'OnVoximplantCallEnd' => array('voximplant', 'onCallEnd', array(__CLASS__, 'onCallEnd'), array("category" => \Thurly\Rest\Sqs::CATEGORY_TELEPHONY)),
					Rest\Helper::EVENT_START_EXTERNAL_CALL => array('voximplant', 'onExternalCallStart', array(__CLASS__, 'filterApp'), array("category" => \Thurly\Rest\Sqs::CATEGORY_TELEPHONY)),
					Rest\Helper::EVENT_START_EXTERNAL_CALLBACK => array('voximplant', 'onExternalCallBackStart', array(__CLASS__, 'filterApp'), array("category" => \Thurly\Rest\Sqs::CATEGORY_TELEPHONY)),
				),
	...
	public static function sipGet($arParams, $nav, $server)
	{
		...
		{
			throw new \Thurly\Rest\AccessException();
		}
		...
		$dbResCnt = \Thurly\Voximplant\SipTable::getList(array(
			'filter' => $arFilter,
		...
		if ($arResCnt && $arResCnt["CNT"] > 0)
		{
			...
			$dbRes = \Thurly\Voximplant\SipTable::getList(array(
				'order' => $arSort,
				'select' => Array('*', 'TITLE'),
				'filter' => $arFilter,
	...
	public static function sipAdd($arParams, $nav, $server)
	{
		...
		{
			throw new Thurly\Rest\RestException($viSip->GetError()->msg, $viSip->GetError()->code, CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function sipUpdate($arParams, $nav, $server)
	{
		...
		if (!$arResCnt || $arResCnt["CNT"] <= 0)
		{
			throw new Thurly\Rest\RestException("Specified CONFIG_ID is not found", Thurly\Rest\RestException::ERROR_NOT_FOUND, CRestServer::STATUS_NOT_FOUND);
		}
	...
	public static function sipDelete($arParams, $nav, $server)
	{
		...
		if (!$arResCnt || $arResCnt["CNT"] <= 0)
		{
			throw new Thurly\Rest\RestException("Specified CONFIG_ID is not found", Thurly\Rest\RestException::ERROR_NOT_FOUND, CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function sipConnectorStatus()
	{
		...
		{
			throw new Thurly\Rest\RestException($ViHttp->GetError()->msg, $ViHttp->GetError()->code, CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function statisticGet($arParams, $nav, $server)
	{
		...
		$dbResCnt = \Thurly\Voximplant\StatisticTable::getList(array(
			'filter' => $arFilter,
			'select' => array("CNT" => new Thurly\Main\Entity\ExpressionField('CNT', 'COUNT(1)')),
		));
		$arResCnt = $dbResCnt->fetch();
		if ($arResCnt && $arResCnt["CNT"] > 0)
		{
			...
			$dbRes = \Thurly\Voximplant\StatisticTable::getList(array(
				'order' => $arSort,
				'filter' => $arFilter,
				'limit' => $arNavParams['limit'],
				'offset' => $arNavParams['offset'],
			...
			while ($arData = $dbRes->fetch())
			{
				...
				if ($arData['CALL_WEBDAV_ID'] > 0 && \Thurly\Main\Loader::includeModule('disk'))
				{
					$fileId = (int)$arData['CALL_WEBDAV_ID'];
					$file = \Thurly\Disk\File::loadById($fileId);
					if (!is_null($file))
						$arData['CALL_RECORD_URL'] = \Thurly\Disk\Driver::getInstance()->getUrlManager()->getUrlForDownloadFile($file, true);
				}
				$result[] = $arData;
			}
	...
	protected static function checkStatisticFilter($arFilter)
	{
		...
		foreach ($arFilter as $key => $value)
		{
			...
			else if (preg_match('/^([^a-zA-Z]*)(.*)/', $key, $matches))
			{
				$operation = $matches[1];
				$field = $matches[2];
	...
	public static function lineOutgoingSipSet($arParams)
	{
		...
		if (!$result)
		{
			throw new Thurly\Rest\RestException('Specified CONFIG_ID is not found', Thurly\Rest\RestException::ERROR_ARGUMENT, CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	{
		return \Thurly\Voximplant\Tts\Language::getList();
	}
	...
	public static function getUser($params, $n, $server)
	{
		...
		{
			throw new \Thurly\Rest\RestException('Parameter USER_ID is not set');
		}
		...
		if (count($userIds) == 0)
			throw new \Thurly\Rest\AccessException('You have no permission to query selected users');

		if (\Thurly\Voximplant\Integration\ThurlyOS::isInstalled())
		{
			$admins = \Thurly\Voximplant\Integration\ThurlyOS::getAdmins();
		}
		else
		{
			$admins = array();
		...
		$server->requestConfirmation(
			...
			GetMessage(
				...
				array('#APPLICATION_NAME#' => \Thurly\Voximplant\Rest\Helper::getRestAppName($server->getClientId()))
			)
		);
		...
		while ($row = $cursor->Fetch())
		{
			$result[] = array(
				...
				'SIP_SERVER' => str_replace('voximplant.com', 'thurlyphone.com', $account->GetCallServer()),
	...
	public static function activatePhone($params, $n, $server)
	{
		...
		if (!CVoxImplantUser::canModify($userId, $permissions))
			throw new \Thurly\Rest\RestException('You are not allowed to modify user\'s settings');
	...
	public static function getAuthorization($params, $n, $server)
	{
		if ($server->getAuthType() !== \Thurly\Rest\SessionAuth\Auth::AUTH_TYPE)
			throw new \Thurly\Rest\RestException("This method is only available for internal usage.", "WRONG_AUTH_TYPE", \CRestServer::STATUS_FORBIDDEN);
		...
		if(!$result->isSuccess())
		{
			...
			throw new Thurly\Rest\RestException($errors[0]->getMessage(), $errors[0]->getCode());
		}
		$data = $result->getData();
		return array(
			'SERVER' => $data['server'],
	...
	public static function signOneTimeKey($params, $n, $server)
	{
		...
		if (!$result)
			throw new \Thurly\Rest\RestException($voxMain->GetError()->msg, $voxMain->GetError()->code);
	...
	public static function startCallWithDevice($params, $n, $server)
	{
		...
		if (!CVoxImplantUser::GetPhoneActive($userId))
			throw new \Thurly\Rest\RestException("User has no phone.", "NO_PHONE", \CRestServer::STATUS_NOT_FOUND);
	 ...
	 * @param \CRestServer $server
	 * @return array
	 * @throws \Thurly\Rest\RestException
	 */
	public static function sendWait($params, $n, $server)
	{
		...
		$call = \Thurly\Voximplant\CallTable::getByCallId($params['CALL_ID']);
		if (!$call)
			throw new Thurly\Rest\RestException("Call is not found, or already finished", Thurly\Rest\RestException::ERROR_NOT_FOUND, CRestServer::STATUS_NOT_FOUND);

		if($call['STATUS'] !== \Thurly\Voximplant\CallTable::STATUS_WAITING)
			throw new Thurly\Rest\RestException("Call is already answered", "ERROR_WRONG_STATE");
	...
	public static function sendReady($params, $n, $server)
	{
		...
		if($call)
		{
			\Thurly\Voximplant\CallTable::update($call['ID'], array(
				'STATUS' => \Thurly\Voximplant\CallTable::STATUS_CONNECTING
			));
		}
	...
	public static function answerCall($params, $n, $server)
	{
		...
		$userId = static::getCurrentUserId();
		$call = \Thurly\Voximplant\CallTable::getByCallId($callId);
		if (!$call)
			throw new Thurly\Rest\RestException("Call is not found, or already finished", Thurly\Rest\RestException::ERROR_NOT_FOUND, CRestServer::STATUS_NOT_FOUND);
	...
	public static function startCallViaRest($params, $n, $server)
	{
		...
		$userId = static::getCurrentUserId();
		$startResult = \Thurly\Voximplant\Rest\Helper::startCall(
			$params['NUMBER'],
			$userId,
			$params['LINE_ID'],
			$callParams
	...
	public static function getCall($params, $n, $server)
	{
		...
		if(!$call)
			throw new \Thurly\Rest\RestException("Call is not found, or finished", "NOT_FOUND", \CRestServer::STATUS_NOT_FOUND);
	...
	public static function startCallback($params, $n, $server)
	{
		...
		if(!$callbackResult->isSuccess())
			throw new \Thurly\Rest\RestException(implode('; ', $callbackResult->getErrorMessages()));
	...
	public static function startInfoCallWithText($params, $n, $server)
	{
		...
		if(!$infoCallResult->isSuccess())
			throw new \Thurly\Rest\RestException(implode('; ', $infoCallResult->getErrorMessages()));
	...
	public static function searchCrmEntities($params, $n, $server)
	{
		...
		if($phoneNumber == '')
		{
			throw new \Thurly\Rest\RestException('PHONE_NUMBER is empty');
		}

		$result = Rest\Helper::searchCrmEntities($phoneNumber);
		if(!$result->isSuccess())
			throw new \Thurly\Rest\RestException(implode('; ', $result->getErrorMessages()));
	...
	public static function registerExternalCall($params, $n, $server)
	{
		...
		$row = \Thurly\Rest\AppTable::getByClientId($clientId);
		$appId = $row['ID'];
		...
		if(!$userId)
			throw new \Thurly\Rest\RestException('USER_ID or USER_PHONE_INNER should be set');

		if(!in_array($params['TYPE'], CVoxImplantMain::getCallTypes()))
			throw new \Thurly\Rest\RestException('Unknown TYPE');

		if(isset($params['CALL_START_DATE']))
			$startDate = new \Thurly\Main\Type\DateTime(CRestUtil::unConvertDateTime($params['CALL_START_DATE']));
		else
			$startDate = new \Thurly\Main\Type\DateTime();
	...
	public static function attachRecord($params, $n, $server)
	{
		...
		{
			throw new \Thurly\Rest\RestException('Required parameters are not set. Request should contain or URL or FILENAME parameter');
		}
