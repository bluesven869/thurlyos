...
use Thurly\Main\Type as FieldType;
...
class CVoxImplantMain
{
		...
		else if ($openChat)
		{
			...
			else
			{
				...
				$chatId = $CIMChat->Add(Array(
					'TITLE' => $chatTitle != ''? $chatTitle: \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($phone)->format(),
					'USERS' => false,
			...
			if ($getPhoto && intval($avatarId) > 0)
			{
				...
				$hrPhoto[$dialogId] = empty($arPhotoHrTmp['src'])? '/thurly/js/im/images/hidef-avatar-v2.png': $arPhotoHrTmp['src'];
			}
		}
		else if ($userId)
		{
	...
	public static function UpdateChatInfo($dialogId, array $additionalData)
	{
		...
		if (substr($dialogId, 0, 4) == 'chat')
		{
			...
			if($updateResult)
			{
				$relationCursor = \Thurly\Im\Model\RelationTable::getList(array(
				...
				while ($relation = $relationCursor->fetch())
				{
					if (
						...
						\Thurly\Im\User::getInstance($relation['USER_ID'])->isConnector()
					)
					{
						continue;
					}
	...
	{
		if(!VI\Integration\ThurlyOS::isEmailConfirmed())
		{
		...
		return Array(
			'SERVER' => str_replace('voximplant.com', 'thurlyphone.com', $userInfo['call_server']),
	...
	public static function CallStart($callId, $userId, $callDevice = 'WEBRTC', $external = false)
	{
		...
		$callRecord = array('STATUS' => Thurly\Voximplant\CallTable::STATUS_CONNECTED);
		if(!$call['PORTAL_USER_ID'])
			$callRecord['USER_ID'] = $userId;

		Thurly\Voximplant\CallTable::update($call['ID'], $callRecord);
	 ...
	 * @return bool
	 * @deprecated Use Thurly\Voximplant\Security\Permissions instead.
	 */
	public static function CheckAccess()
	{
		...
		{
			if (is_object($USER) && intval($USER->GetID()) && $USER->CanDoOperation('thurlyos_config'))
			{
				$result = true;
			}
		}
	...
	public static function GetTrialText($popupId = 'main')
	{
		...
		throw new \Thurly\Main\ArgumentException('Unknown trial popup id', 'popupId');
	}

	public static function GetTrialTextMain()
	{
		if(\Thurly\Main\Loader::includeModule('thurlyos'))
			$transcriptionLimited = !in_array(CThurlyOS::getLicensePrefix(), VI\Transcript::getAllowedRegions());
		else
	...
	public static function GetBuyLink()
	{
		...
		if (IsModuleInstalled('thurlyos'))
		{
			if ($accountLang == "ua")
			{
				return 'https://www.thurlyos.ua/prices/telephony.php';
			}
		...
		else
		{
			...
			{
				return 'https://www.thurlyos.de/prices/self-hosted-telephony.php';
			}
			else
			{
	...
	public static function GetProLink()
	{
		if(LANGUAGE_ID == "ru" || LANGUAGE_ID == "kz" || LANGUAGE_ID == "by")
			return "https://www.thurlyos.ru/pro/call.php";
		else if(LANGUAGE_ID == "de")
			return "https://www.thurlyos.de/pro/call.php";
		else if(LANGUAGE_ID == "ua")
			return "https://www.thurlyos.ua/pro/call.php";
		else
	...
	{
		if(LANGUAGE_ID == "ru" || LANGUAGE_ID == "kz" || LANGUAGE_ID == "by")
			return "https://www.thurlyos.ru/prices/calls.php";
		else if(LANGUAGE_ID == "de")
			return "https://www.thurlyos.de/prices/self-hosted-telephony.php";
		else if(LANGUAGE_ID == "ua")
			return "https://www.thurlyos.ua/prices/calls.php";
		else
			return "https://www.thurlyos.com/prices/self-hosted-telephony.php";
	}
	...
	{
		return (int)\Thurly\Main\Config\Option::get('voximplant', 'default_group_id');
	}
