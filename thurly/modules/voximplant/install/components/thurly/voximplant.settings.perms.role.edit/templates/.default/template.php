						...
						foreach ($actions as $action => $availablePermissions)
						{
							?>
								<tr class="<?=($firstAction ? 'tr-first' : '')?>">
									<td class="table-blue-td-name"><?=($firstAction ? htmlspecialcharsbx(\Thurly\Voximplant\Security\Permissions::getEntityName($entity)) : '&nbsp;')?></td>
									<td class="table-blue-td-param"><?=htmlspecialcharsbx(\Thurly\Voximplant\Security\Permissions::getActionName($action))?></td>
										...
										<select name="PERMISSIONS[<?=$entity?>][<?=$action?>]" class="table-blue-select">
											<?foreach ($availablePermissions as $permission):?>
												<option value="<?=$permission?>" <?=($permission === $arResult['PERMISSIONS'][$entity][$action] ? 'selected' : '')?>>
													<?=htmlspecialcharsbx(\Thurly\Voximplant\Security\Permissions::getPermissionName($permission))?>
												</option>
											<?endforeach;?>
										</select>
									</td>
