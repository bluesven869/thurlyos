...
use Thurly\Voximplant\Security\RoleManager;

Loc::loadMessages(__FILE__);

class CVoximplantRoleEditComponent extends CThurlyComponent
{
	protected $new = false;
	...
	protected function prepareData()
	{
		if($this->errors->isEmpty())
		{
			...
			if(!$role)
			{
				$this->errors[] = new \Thurly\Main\Error(Loc::getMessage('VOXIMPLANT_ROLE_NOT_FOUND'));
				$this->new = true;
			}
			else
			{
		...
		$this->arResult['CAN_EDIT'] = \Thurly\Voximplant\Security\Helper::canUse();
	...
	protected function save()
	{
		...
		if($roleName == '')
		{
			$this->errors[] = new \Thurly\Main\Error(Loc::getMessage('VOXIMPLANT_ROLE_ERROR_EMPTY_NAME'));
			return false;
		}
		...
		{
			$this->errors[] = new \Thurly\Main\Error(Loc::getMessage('VOXIMPLANT_ROLE_SAVE_ERROR'));
			return false;
		}
	...
	public function executeComponent()
	{
		...
		if($this->saveMode)
		{
			if(\Thurly\Voximplant\Security\Helper::canUse())
			{
				if($this->save())
				{
			...
			{
				$this->errors[] = new \Thurly\Main\Error(Loc::getMessage('VOXIMPLANT_ROLE_LICENSE_ERROR'));
			}
		}
