(function()
{
	...
	var AJAX_URL = '/thurly/components/thurly/voximplant.ivr.edit/ajax.php';
	var ivrDefaults = {
		VOICE_LIST: null,
		SPEED_LIST: null,
		VOLUME_LIST: null,
	...
	{
		var self = this;
		BX.addCustomEvent(window, 'onViGroupEditInit', this._onGroupEditorInit.bind(this));
	...
	BX.IvrEditor.prototype.render = function()
	{
		...
		var ivrNode = BX.create("div", {props: {className: 'ivr-wrap'}, children: [
			...
			BX.create("div", {props: {className: 'ivr-root-lvl'}, children: [
				BX.create("div", {props: {className: 'ivr-block'}, children: [
					...
					BX.create("div", {props: {className: 'ivr-block-row'}, children: [
						BX.create("input", {props: {className: 'ivr-input-text', value: this.ivrData.NAME}, events: {
							bxchange: function(e)
							{
			...
			BX.create("div", {props: {className: 'tel-set-footer-btn'}, children: [
				this.elements.saveButton = !ivrDefaults.IS_ENABLED ? null : BX.create("a", {
					props: {href: "javascript:void(0);", className: "webform-small-button webform-small-button-accept"},
					children: [
						BX.create("span", {props: {className: 'webform-small-button-left'}}),
						BX.create("span", {props: {className: 'webform-small-button-text'}, text: BX.message('VOX_IVR_EDIT_SAVE')}),
				...
				BX.create("a", {props: {className: 'webform-small-button-link'}, attrs: {href: ivrDefaults.IVR_LIST_URL}, text: BX.message('VOX_IVR_EDIT_RETURN')})
			]})
		]});
	...
	BX.IvrEditor.prototype.renderAction = function (action, itemDescriptor)
	{
		...
		var subItemNode;
		var resultNode;
		var groupSettingsNode;
		var groupSettingsPlaceholderNode;
		var groupSelectNode;
				...
				BX.create("div", {
					...
					children: [
						BX.create("div", {props: {className: 'ivr-block-row'}, children: [
								...
								events: {
									bxchange: function (e)
									{
										if(e.target.value == 'new')
										{
											...
											{
												self.showGroupSettings(groupSettingsNode, groupSettingsPlaceholderNode, groupSelectNode);
											}
										}
										else
										{
							...
							groupSettingsNode = BX.create("span", {
								...
								events: {
									click: function(e)
									{
										if(groupSettingsNode.dataset.settingsOpen == "Y")
											self.hideGroupSettings(groupSettingsNode, groupSettingsPlaceholderNode, groupSelectNode);
										else
											self.showGroupSettings(groupSettingsNode, groupSettingsPlaceholderNode, groupSelectNode);
									}
								}
							}),
							groupSettingsPlaceholderNode = BX.create("div", {props: {className: 'ivr-group-settings-container'}, style: {'max-height': '0px'}})
						]})
					]
				}),
				BX.create("div", {
	...
	BX.IvrEditor.prototype._onSaveButtonClick = function()
	{
		...
		BX.ajax({
			...
			onfailure: function()
			{
				BX.removeClass(self.elements.saveButton, "webform-small-button-wait webform-small-button-active");
				BX.remove(waitNode);
				BX.debug('Failed to save ivr structure');
				self.saving = false;
			}
		})
	};
	...
	BX.IvrEditor.prototype.showGroupSettings = function (button, placeholder, groupSelect)
	{
		var self = this;
		var groupId = groupSelect.value;
		var requestId = BX.util.getRandomString(16);
		button.innerText = BX.message('VOX_IVR_HIDE_GROUP_SETTINGS');
		...
		BX.ajax({
			...
			{
				button.dataset.settingsOpen = "Y";
				button.dataset.requestId = requestId;
				placeholder.innerHTML = HTML;
				placeholder.style.maxHeight = '800px';

				self.groupsSettingsRequests[requestId] = {
					button: button,
	...
	BX.IvrEditor.prototype.hideGroupSettings = function (button, placeholder, groupSelect)
	{
		var self = this;
		var externalRequestId = button.dataset.requestId;
		...
		setTimeout(function()
		{
			button.innerText = BX.message('VOX_IVR_GROUP_SETTINGS');
			button.dataset.settingsOpen = "N";
			placeholder.innerHTML = '';
			if(self.groupsSettingsRequests[externalRequestId])
			{
				delete self.groupsSettingsRequests[externalRequestId];
			}
		}, 300)
	};

	BX.IvrEditor.prototype.selectNewGroup = function (groupOptions, externalRequestId)
	{
		if(this.groupsSettingsRequests[externalRequestId])
		{
			var groupSelect = this.groupsSettingsRequests[externalRequestId].groupSelect;
			if(groupOptions && groupOptions.GROUP)
			{
				var groupFields = groupOptions.GROUP;
				if(!groupFields.ID)
					return;
	...
	BX.IvrEditor.prototype._onGroupEditorInit = function(groupEditor)
	{
		var self = this;
		this.groupsSettingsRequests[groupEditor.externalRequestId].groupEditor = groupEditor;

		BX.addCustomEvent(groupEditor, 'onSave', function(groupOptions, editRequestId)
		{
			self.selectNewGroup(groupOptions, editRequestId);
			self.onSettingsEditFinished(editRequestId);
		});

		BX.addCustomEvent(groupEditor, 'onCancel', function(editRequestId)
		{
			self.onSettingsEditFinished(editRequestId);
		});
	};

	BX.IvrEditor.prototype.onSettingsEditFinished = function(externalRequestId)
	{
		if(this.groupsSettingsRequests[externalRequestId])
		{
			var request = this.groupsSettingsRequests[externalRequestId];
			this.hideGroupSettings(request.button, request.placeholder, request.groupSelect);
		}
	};
	...
	Uploader.prototype.render = function()
	{
		...
		{
			uploaderNode = BX.create("div", {children: [
				BX.create('span', {
		...
		else
		{
			...
			setTimeout(function()
			{
				jwplayer(self.playerId).setup({
					...
					flashplayer: '/thurly/components/thurly/player/mediaplayer/player'
				});
			}, 10)
		}
	...
	Uploader.prototype.upload = function(file, next)
	{
		...
		formData.append('sessid', BX.thurly_sessid());
