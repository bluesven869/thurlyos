...
CJSCore::Init(array('socnetlogdest', 'phone_number'));
\Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/components/thurly/player/mediaplayer/jwplayer.js');
\Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/components/thurly/voximplant.main/templates/.default/telephony.css');
\Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/components/thurly/voximplant.config.edit/templates/.default/style.css');
\Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/css/main/font-awesome.css');
...
<script>
	BX.message({
		...
		'VOX_IVR_NOT_ALLOWED_TITLE': '<?= CUtil::JSEscape(\Thurly\Voximplant\Ivr\Helper::getLicensePopupHeader())?>',
		'VOX_IVR_NOT_ALLOWED_TEXT': '<?= CUtil::JSEscape(\Thurly\Voximplant\Ivr\Helper::getLicensePopupContent())?>',
	...
	BX.ready(function()
	{
		BX.IvrEditor.setDefaults({
			'VOICE_LIST': <?= CUtil::PhpToJSObject(\Thurly\Voximplant\Tts\Language::getList())?>,
			'SPEED_LIST': <?= CUtil::PhpToJSObject(\Thurly\Voximplant\Tts\Speed::getList())?>,
			'VOLUME_LIST': <?= CUtil::PhpToJSObject(\Thurly\Voximplant\Tts\Volume::getList())?>,
			'DEFAULT_VOICE': '<?=\Thurly\Voximplant\Tts\Language::getDefaultVoice(\Thurly\Main\Application::getInstance()->getContext()->getLanguage())?>',
			'DEFAULT_SPEED': '<?=\Thurly\Voximplant\Tts\Speed::getDefault()?>',
			'DEFAULT_VOLUME': '<?=\Thurly\Voximplant\Tts\Volume::getDefault()?>',
			...
			'IS_ENABLED': <?= CUtil::PhpToJSObject(\Thurly\Voximplant\Ivr\Ivr::isEnabled())?>,
			'MAX_DEPTH': <?= \Thurly\Voximplant\Limits::getIvrDepth()?>,
			'MAX_GROUPS': <?= \Thurly\Voximplant\Limits::getMaximumGroups()?>
		});
