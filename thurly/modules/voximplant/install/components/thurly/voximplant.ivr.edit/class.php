...
class CVoximplantIvrEditComponent extends \CThurlyComponent
{
	const MAX_DEPTH = 20;
	...
	public static function saveIvr(array $ivr)
	{
		...
		{
			$result->addError(new \Thurly\Main\Error(GetMessage('VOX_IVR_EDIT_ERROR_IVR_NOT_AVAILABLE')));
			return $result;
		}
		...
		if($ivr['ID'] > 0)
		{
			\Thurly\Voximplant\Model\IvrTable::update($ivr['ID'], $ivrFields);
		}
		else
		{
			$addResult = \Thurly\Voximplant\Model\IvrTable::add($ivrFields);
			$ivr['ID'] = $addResult->getId();
		}
		static::$ivrId = $ivr['ID'];

		$cursor = \Thurly\Voximplant\Model\IvrItemTable::getList(array(
			'select' => array('ID'),
			'filter' => array(
				'IVR_ID' => $ivr['ID']
			)
		...
		\Thurly\Voximplant\Model\IvrTable::update($ivr['ID'], array(
			'FIRST_ITEM_ID' => $rootItemId
		));

		foreach (self::$itemsToDelete as $itemIdToDelete => $v)
		{
			\Thurly\Voximplant\Model\IvrActionTable::deleteByItemId($itemIdToDelete);
			\Thurly\Voximplant\Model\IvrItemTable::delete($itemIdToDelete);
		}
	...
	protected static function saveItem(array $fields, $depth)
	{
		...
		{
			$result->addError(new \Thurly\Main\Error(GetMessage('VOX_IVR_EDIT_ERROR_IVR_DEPTH_TOO_LARGE')));
			return $result;
		}
		...
		$itemFields = array(
			...
			'TIMEOUT_ACTION' => $fields['TIMEOUT_ACTION'] ?: \Thurly\Voximplant\Ivr\Item::TIMEOUT_ACTION_EXIT,
		...
		if($fields['ID'] > 0)
		{
			\Thurly\Voximplant\Model\IvrItemTable::update((int)$fields['ID'], $itemFields);
			$itemFields['ID'] = (int)$fields['ID'];
			unset(static::$itemsToDelete[$itemFields['ID']]);
		}
		else
		{
			$addResult = \Thurly\Voximplant\Model\IvrItemTable::add($itemFields);
			$itemFields['ID'] = $addResult->getId();
		}

		$actionsToDelete = array();
		$cursor = \Thurly\Voximplant\Model\IvrActionTable::getList(array(
			'select' => array('ID'),
		...
		if(is_array($fields['ACTIONS']))
		{
			foreach ($fields['ACTIONS'] as $action)
			{
				...
				if($action['ACTION'] == \Thurly\Voximplant\Ivr\Action::ACTION_ITEM && !is_array($action['ITEM']))
					continue;
		...
		{
			\Thurly\Voximplant\Model\IvrActionTable::delete($actionId);
		}
	...
	protected static function saveAction(array $fields, $depth)
	{
		...
		{
			case \Thurly\Voximplant\Ivr\Action::ACTION_ITEM:
				...
				break;
			case \Thurly\Voximplant\Ivr\Action::ACTION_PHONE:
				$actionFields['PARAMETERS']['PHONE_NUMBER'] = $fields['PARAMETERS']['PHONE_NUMBER'];
				break;
			case \Thurly\Voximplant\Ivr\Action::ACTION_QUEUE:
				$actionFields['PARAMETERS']['QUEUE_ID'] = $fields['PARAMETERS']['QUEUE_ID'];
				break;
			case \Thurly\Voximplant\Ivr\Action::ACTION_USER:
				$actionFields['PARAMETERS']['USER_ID'] = $fields['PARAMETERS']['USER_ID'];
				break;
			case \Thurly\Voximplant\Ivr\Action::ACTION_VOICEMAIL:
				$actionFields['PARAMETERS']['USER_ID'] = $fields['PARAMETERS']['USER_ID'];
				break;
		}
		...
		if($fields['ID'] > 0)
		{
			\Thurly\Voximplant\Model\IvrActionTable::update((int)$fields['ID'], $actionFields);
			$actionFields['ID'] = (int)$fields['ID'];
		}
		else
		{
			$addResult = \Thurly\Voximplant\Model\IvrActionTable::add($actionFields);
	...
	public static function uploadFile($fields)
	{
		...
		{
			$result->addError(new \Thurly\Main\Error(Loc::getMessage('VOX_IVR_FILE_UPLOAD_ERROR')));
			return $result;
		}

		$uploadedFileDescriptor = $_FILES['FILE'];
		$messageFile = new Thurly\Main\IO\File($uploadedFileDescriptor['tmp_name']);
		$messageFile->open('rb');

		if($messageFile->getSize() > self::MAX_FILE_SIZE)
		{
			$result->addError(new \Thurly\Main\Error(Loc::getMessage('VOX_IVR_FILE_TOO_LARGE')));
			return $result;
		}
	...
	protected function prepareData()
	{
		...
		if($this->id > 0)
		{
			$ivr = new \Thurly\Voximplant\Ivr\Ivr($this->id);
		...
		$result['TELEPHONY_GROUPS'] = \Thurly\Voximplant\Model\QueueTable::getList(array(
		...
		if($ivr instanceof \Thurly\Voximplant\Ivr\Ivr)
		{
	...
	{
		$licenseMaxDepth = \Thurly\Voximplant\Limits::getIvrDepth();
