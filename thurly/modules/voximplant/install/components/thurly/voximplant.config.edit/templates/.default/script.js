;(function(window){
	...
	var ajaxUrl = '/thurly/components/thurly/voximplant.config.edit/ajax.php';
	...
	BX.VoxImplantConfigEdit = function(params)
	{
		...
		this._onGroupSavedHandler = this._onGroupSaved.bind(this);
		this._onGroupEditCanceledHandler = this._onViGroupEditCanceled.bind(this);
		this._onGroupEditDestroyHandler = this._onGroupEditDestroy.bind(this);
	...
	BX.VoxImplantConfigEdit.prototype.bindEvents = function ()
	{
		...
		BX.bind(this.getNode('show-group-config'), 'click', this.onShowSettingsClick.bind(this));
		...
		BX.addCustomEvent(window, 'onViGroupEditInit', this._onGroupEditInit.bind(this));
	};
	...
	BX.VoxImplantConfigEdit.prototype.onShowSettingsClick = function (e)
	{
	...
	BX.VoxImplantConfigEdit.prototype.showGroupSetting = function (params)
	{
		this.settingsOpened = true;
		var placeholderWrapper = BX('vi-group-settings-placeholder-wrapper');
		var placeholder = BX('vi-group-settings-placeholder');
		var groupId = params.groupId;
		var button = this.getNode('show-group-config');
		...
		BX.ajax({
			...
			onsuccess: function (HTML)
			{
				...
				setTimeout(function()
				{
					placeholderWrapper.style.maxHeight = placeholder.scrollHeight + 'px';
				}, 50);
			}
		});
	};

	BX.VoxImplantConfigEdit.prototype.hideGroupSetting = function (immediate)
	{
		var self = this;
		var placeholderWrapper = BX('vi-group-settings-placeholder-wrapper');
		var placeholder = BX('vi-group-settings-placeholder');
		...
		placeholderWrapper.style.maxHeight = 0;
		if(immediate)
		{
	...
	BX.VoxImplantConfigEdit.prototype._onSubmitClick = function(e)
	{
		...
		if(this.groupEditor)
		{
			this.groupEditor.save(function(params)
			{
				self.hideGroupSetting(true);
				var groupFields = params.GROUP;
				if (!groupFields.ID)
					return;

				self.afterGroupSaved(groupFields);
	...
	BX.VoxImplantConfigEdit.prototype._onGroupEditInit = function (groupEditor)
	{
		this.groupEditor = groupEditor;
		BX.addCustomEvent(this.groupEditor, 'onSave', this._onGroupSavedHandler);
		BX.addCustomEvent(this.groupEditor, 'onCancel', this._onGroupEditCanceledHandler);
		BX.addCustomEvent(this.groupEditor, 'onDestroy', this._onGroupEditDestroyHandler);
	};

	BX.VoxImplantConfigEdit.prototype._onGroupEditDestroy = function ()
	{
		BX.removeCustomEvent(this.groupEditor, 'onSave', this._onGroupSavedHandler);
		BX.removeCustomEvent(this.groupEditor, 'onCancel', this._onGroupEditCanceledHandler);
		BX.removeCustomEvent(this.groupEditor, 'onDestroy', this._onGroupEditDestroyHandler);
		this.groupEditor = null;
	};

	BX.VoxImplantConfigEdit.prototype._onGroupSaved = function (params)
	{
		this.hideGroupSetting();
		this.groupEditor.destroy();
		var groupFields = params.GROUP;
		if (!groupFields.ID)
			return;
	...
	BX.VoxImplantConfigEdit.prototype._onViGroupEditCanceled = function ()
	{
		this.hideGroupSetting();
		this.groupEditor.destroy();
		...
		if (groupId === 'new')
		{
			groupSelect.value = this.previousGroupId;
			this.currentGroupId = this.previousGroupId;
		}
	};
	...
	BX.ViCallerId.connectPhone = function(number)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'VI_CONNECT': 'Y', 'NUMBER': number, 'VI_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
	...
	BX.ViCallerId.verifyPhone = function()
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'VI_VERIFY': 'Y', 'VI_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
	...
	BX.ViCallerId.activatePhone = function(code)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'VI_ACTIVATE': 'Y', 'CODE': code, 'VI_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				BX.ViCallerId.blockVerify = false;
				BX.closeWait();
	...
	BX.ViCallerId.removePhone = function()
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'VI_REMOVE': 'Y', 'VI_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				BX.closeWait();
				BX.ViCallerId.blockAjax = false;
...
(function()
{
	...
	BX.Voximplant.Sip.prototype.checkStatus = function()
	{
		...
		BX.showWait();
		BX.ajax({
			url: '/thurly/components/thurly/voximplant.config.edit/ajax.php?VI_SIP_CHECK',
			method: 'POST',
			dataType: 'json',
			timeout: 60,
			data: {'VI_SIP_CHECK': 'Y', 'REG_ID': this.regId, 'VI_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function(data)
			{
				if (data.ERROR == '')
				{
