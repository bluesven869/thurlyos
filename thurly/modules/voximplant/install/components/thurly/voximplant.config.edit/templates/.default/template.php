...
\Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/voximplant/common.js');

CJSCore::RegisterExt('voximplant_config_edit', array(
	'js' => '/thurly/components/thurly/voximplant.config.edit/templates/.default/script.js',
	'lang' => '/thurly/components/thurly/voximplant.config.edit/templates/.default/lang/'.LANGUAGE_ID.'/template.php',
));
CJSCore::Init(array('access', 'voximplant_config_edit', 'phone_number'));
\Thurly\Voximplant\Ui\Helper::initLicensePopups();
$i = 0;
$defaultM = $arResult["DEFAULT_MELODIES"];
$melodiesToLoad = array();
?>
<form action="<?=POST_FORM_ACTION_URI?>" method="POST" id="config_edit_form">
<?=thurly_sessid_post()?>
							...
							<table class="tel-set-sip-table" cellpadding="0" cellspacing="0">
								...
								<tbody id="vi-tel-sip-additional-fields" class="tel-set-sip-additional-fields-hidden tel-connect-pbx-animate">
									<tr>
										...
										<td class="tel-set-sip-td-r"><input type="text" name="SIP[AUTH_USER]" value="<?=htmlspecialcharsbx($arResult['SIP_CONFIG']['AUTH_USER'])?>" class="tel-set-inp tel-set-inp-sip" /></td>
									</tr>
									<tr>
										<td class="tel-set-sip-td-l"><?=GetMessage('VI_CONFIG_SIP_T_OUTBOUND_PROXY')?></td>
										<td class="tel-set-sip-td-r"><input type="text" name="SIP[OUTBOUND_PROXY]" value="<?=htmlspecialcharsbx($arResult['SIP_CONFIG']['OUTBOUND_PROXY'])?>" class="tel-set-inp tel-set-inp-sip"/></td>
									</tr>
								</tbody>
							</table>
						</div>
						...
						<div class="tel-set-item-cont">
							...
							<div class="tel-set-item-select-block">
								...
								<select name="DIRECT_CODE_RULE" class="tel-set-inp tel-set-item-select">
									...
									<option value="<?=CVoxImplantIncoming::RULE_VOICEMAIL?>" <?=($arResult["ITEM"]["DIRECT_CODE_RULE"] == CVoxImplantIncoming::RULE_VOICEMAIL ? " selected" : "")?>><?=GetMessage("VI_CONFIG_EDIT_DEALING_WITH_OMITTED_CALL_2")?></option>
								</select>
							</div>
						</div>
					</div>
			...
			<?if($arResult['SHOW_IVR']):?>
				<div class="tel-set-item">
					...
					<div class="tel-set-item-cont-block">
						...
						<select name="IVR_ID" class="tel-set-inp tel-set-item-select" >
						...
						<? if(!\Thurly\Voximplant\Ivr\Ivr::isEnabled()): ?>
					...
					<?if (!IsModuleInstalled("timeman")):?>
						<script type="text/javascript">
							BX.bind(BX('vi_timeman'), 'change', function(e){
								...
								alert('<?=GetMessage(IsModuleInstalled("thurlyos")? "VI_CONFIG_EDIT_TIMEMAN_SUPPORT_B24": "VI_CONFIG_EDIT_TIMEMAN_SUPPORT_CP")?>');
							});
						</script>
					<?endif;?>
				...
				<div class="tel-set-item-cont-block">
					...
					<div id="vi-recording-details" class="tel-set-height-animated" style="<?=($arResult["ITEM"]["RECORDING"] == "Y" ? 'max-height: 160px' : 'max-height: 0')?>">
						...
						<input id="vi_transcribe"
							   ...
							   <?if(!\Thurly\Voximplant\Transcript::isEnabled()) { ?>disabled<? }?>
						/>
						...
						<?if (!\Thurly\Voximplant\Transcript::isEnabled() || \Thurly\Voximplant\Transcript::isDemo()):?>
							<div class="tel-lock-holder-select" title="<?=GetMessage("VI_CONFIG_LOCK_ALT")?>"><div onclick="BX.Voximplant.showLicensePopup('main')" class="tel-lock <?=(\Thurly\Voximplant\Transcript::isDemo()? 'tel-lock-demo': '')?>"></div></div>
						<?endif;?>
									...
									<div class="tel-set-item-cont">
										...
										<div class="tel-set-melody-block">
											...
											<span class="tel-set-file-wrap">
												<?$APPLICATION->IncludeComponent('thurly:main.file.input', '.default',
													array(
			...
			<div class="tel-set-item">
				<div class="tel-set-item-num">&nbsp;
					<input id="vi_can_be_selected"
						   ...
						   data-locked="<?=(\Thurly\Voximplant\Limits::canSelectLine() ? "N" : "Y")?>"
					/>
				</div>
				<div class="tel-set-item-cont-block">
					...
					<?if (!\Thurly\Voximplant\Limits::canSelectLine() || CVoxImplantAccount::IsDemo()):?>
						<div class="tel-lock-holder-select" title="<?=GetMessage("VI_CONFIG_LOCK_ALT")?>">
					...
					<div class="tel-set-item-cont">
						<div class="tel-set-item-select-block">
							...
							<select class="tel-set-inp tel-set-item-select" name="MELODY_LANG">
								<?foreach (array("RU", "EN", "DE", "UA") as $k):?>
...
foreach ($melodies as $ii => $group):?>
	...
	<?else:?>
	<div class="tel-melody-group-open"><span class="webform-small-button" onclick="viShowMelodyGroup(this)"><?=GetMessage('VI_CONFIG_MORE_TONES')?></span></div>
	<div class="tel-melody-group tel-melody-group-hide tel-melody-group-<?=$ii?>">
	<?endif?>
<?
	foreach ($group as $i => $melody):
	$id = 'voximplant'.$ii.$i;
	CHTTP::URN2URI($APPLICATION->GetCurPageParam("mfi_mode=down&fileID=".$fileID."&cid=".$cid."&".thurly_sessid_get(), array("mfi_mode", "fileID", "cid")))
?>
...
<!-- //melody -->
		...
		<div class="tel-set-footer-btn">
			<span class="webform-button webform-button-accept" data-role="config-edit-submit">
...
<script>
	...
	BX.ready(function(e)
	{
		...
		test = new BX.VoxImplantConfigEdit({
