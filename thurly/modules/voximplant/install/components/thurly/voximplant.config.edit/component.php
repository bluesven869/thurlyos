...
********************************************************************/
$arResult = array(
	"ITEM" => Thurly\Voximplant\ConfigTable::getById($arParams["ID"])->fetch(),
	"CALLER_ID" => CVoxImplantPhone::GetCallerId(),
	"QUEUES" => \Thurly\Voximplant\Model\QueueTable::getList(array('select' => array('ID', 'NAME')))->fetchAll(),
	"IVR_MENUS" => \Thurly\Voximplant\Model\IvrTable::getList(array('select' => array('ID', 'NAME')))->fetchAll(),
	"TRANSCRIBE_LANGUAGES" => \Thurly\Voximplant\Asr\Language::getList(),
	"SIP_CONFIG" => array(),
...
if ($arResult["ITEM"])
{
	...
	$arResult['ITEM']['PHONE_NAME_FORMATTED'] = \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($arResult['ITEM']['PHONE_NAME'])->format();
	...
	if($arResult["ITEM"]["IVR"] == "Y" && !\Thurly\Voximplant\Ivr\Ivr::isEnabled())
		$arResult["ITEM"]["IVR"] = "N";

	if($arResult['ITEM']['TRANSCRIBE_LANG'] == '')
	{
		$arResult['ITEM']['TRANSCRIBE_LANG'] = \Thurly\Voximplant\Asr\Language::getDefault(\Thurly\Main\Context::getCurrent()->getLanguage());
	}

	if($arResult["ITEM"]["CAN_BE_SELECTED"] == "Y" && !\Thurly\Voximplant\Limits::canSelectLine())
...
if ($_REQUEST["action"] == "save" && check_thurly_sessid())
{
	...
	if (!\Thurly\Voximplant\Transcript::isEnabled())
	{
		$post['TRANSCRIBE'] = 'N';
	...
	$arFields = Array(
		...
		"IVR" => \Thurly\Voximplant\Ivr\Ivr::isEnabled() ?  $post["IVR"] : "N",
		"IVR_ID" => $post["IVR_ID"],
	...
	else
	{
		...
		\Thurly\Voximplant\Model\LineAccessTable::deleteBySearchId($arResult["ITEM"]["SEARCH_ID"]);
		...
		if (($res = Thurly\Voximplant\ConfigTable::update($arParams["ID"], $arFields)) && $res->isSuccess())
		{
...
$arResult['DEFAULTS'] = array(
	'MAXIMUM_GROUPS' => \Thurly\Voximplant\Limits::getMaximumGroups()
);
