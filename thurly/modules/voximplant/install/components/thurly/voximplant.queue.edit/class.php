...
class CVoximplantQueueEditComponent extends \CThurlyComponent
{
	private $action;
	...
	protected function prepareEditData()
	{
		...
		$result['MAXIMUM_GROUP_MEMBERS'] = \Thurly\Voximplant\Limits::getMaximumGroupMembers();

		$userIds = array();
		if($id > 0)
		{
			$result['ITEM'] = \Thurly\Voximplant\Model\QueueTable::getById($id)->fetch();

			$cursor = \Thurly\Voximplant\Model\QueueUserTable::getList(array(
		...
		$result['QUEUE_LIST'] = \Thurly\Voximplant\Model\QueueTable::getList(array(
	 ...
	 * @throws Exception
	 * @throws \Thurly\Voximplant\Model\ArgumentException
	 */
	public static function save($request)
	{
		...
		$queueFields = array(
			...
			'ALLOW_INTERCEPT' => ($request['ALLOW_INTERCEPT'] === 'Y' && \Thurly\Voximplant\Limits::canInterceptCall() ? 'Y' : 'N'),
		);
		
		if($queueFields['NAME'] == '')
		{
			$result->addError(new \Thurly\Main\Error(Loc::getMessage('VI_CONFIG_ERROR_EMPTY_NAME')));
			return $result;
		}
		...
		if($id > 0)
			$dbResult = \Thurly\Voximplant\Model\QueueTable::update($id, $queueFields);
		else
			$dbResult = \Thurly\Voximplant\Model\QueueTable::add($queueFields);

		if(!$dbResult->isSuccess())
		{
			$result->addError(new \Thurly\Main\Error('DB error'));
			return $result;
		}
		...
		$queueFields['ID'] = $id;
		\Thurly\Voximplant\Model\QueueUserTable::deleteByQueueId($id);
		$users = (array)$request['USERS'];
		$maximumUsers = \Thurly\Voximplant\Limits::getMaximumGroupMembers();
		$currentUserCount = 0;
		foreach ($users as $userId)
		{
			$userId = (int)$userId;
			$dbResult = \Thurly\Voximplant\Model\QueueUserTable::add(array(
				'QUEUE_ID' =>  $id,
				'USER_ID' => $userId
			));
			if(!$dbResult->isSuccess())
