...
$arResult['SHOW_LINES'] = $permissions->canPerform(\Thurly\Voximplant\Security\Permissions::ENTITY_LINE, \Thurly\Voximplant\Security\Permissions::ACTION_MODIFY);
$arResult['SHOW_STATISTICS'] = $permissions->canPerform(\Thurly\Voximplant\Security\Permissions::ENTITY_CALL_DETAIL, \Thurly\Voximplant\Security\Permissions::ACTION_VIEW);
...
if($arResult['SHOW_STATISTICS'])
{
	...
	$statCursor = \Thurly\Voximplant\StatisticTable::getList(array(
		...
		'runtime' => array(
			new Thurly\Main\Entity\ExpressionField('YEAR','YEAR(%s)', array('CALL_START_DATE')),
			new Thurly\Main\Entity\ExpressionField('MONTH','MONTH(%s)', array('CALL_START_DATE')),
		)
	));
...
function getStatisticsStartDate()
{
	...
	return \Thurly\Main\Type\Date::createFromPhp($startDate);
}
