;(function()
{
	...
	BX.Voximplant.rentPhone.prototype.getCountry = function ()
	{
		...
		if (!this.country)
		{
			...
			BX.ajax({
				url: '/thurly/components/thurly/voximplant.config.rent/ajax.php?VI_GET_COUNTRY',
				method: 'POST',
				dataType: 'json',
				timeout: 60,
				data: {'VI_GET_COUNTRY': 'Y', 'VI_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()},
	...
	BX.Voximplant.rentPhone.prototype.getState = function ()
	{
		...
		if (this.currentCountry !== '-' && this.currentCountryCategory !== '-' && (!this.countryStates[this.currentCountry] || !this.countryStates[this.currentCountry][this.currentCountryCategory]))
		{
			...
			BX.showWait(this.nodes.container);
			BX.ajax({
				url: '/thurly/components/thurly/voximplant.config.rent/ajax.php?VI_GET_STATE',
				method: 'POST',
				dataType: 'json',
				timeout: 60,
				data: {
					...
					'VI_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()
				},
				onsuccess: BX.delegate(function (data)
				{
					BX.closeWait(self.nodes.container);
	...
	BX.Voximplant.rentPhone.prototype.getRegion = function ()
	{
		...
		if (this.currentCountry != '-' && this.currentCountryState != '-' && this.currentCountryCategory != '-' && (!this.countryRegion[this.currentCountry] || !this.countryRegion[this.currentCountry][this.currentCountryCategory] || !this.countryRegion[this.currentCountry][this.currentCountryCategory][this.currentCountryState]))
		{
			...
			BX.showWait(this.nodes.container);
			BX.ajax({
				url: '/thurly/components/thurly/voximplant.config.rent/ajax.php?VI_GET_REGION',
				method: 'POST',
				dataType: 'json',
				timeout: 60,
				data: {
	...
	BX.Voximplant.rentPhone.prototype.getVerifiedAddresses = function ()
	{
		...
		if (currentRegionParameters && currentRegionParameters.REGULATION_ADDRESS_TYPE != '')
		{
			BX.showWait(this.nodes.container);
			BX.ajax({
				url: '/thurly/components/thurly/voximplant.config.rent/ajax.php?VI_GET_VERIFIED_ADDRESSES',
				method: 'POST',
				dataType: 'json',
				timeout: 60,
				data: {
	...
	BX.Voximplant.rentPhone.prototype.getNumbers = function ()
	{
		...
		{
			BX.ajax({
				url: '/thurly/components/thurly/voximplant.config.rent/ajax.php?VI_GET_PHONE_NUMBERS',
				method: 'POST',
				dataType: 'json',
				timeout: 60,
				data: {
	...
	BX.Voximplant.rentPhone.prototype.attachPhone = function (type)
	{
		...
		this.blockAjax = true;
		BX.ajax({
			url: '/thurly/components/thurly/voximplant.config.rent/ajax.php?VI_RENT_NUMBER',
			method: 'POST',
			dataType: 'json',
			timeout: 60,
			data: {
			...
			onsuccess: BX.delegate(function (answer)
			{
				...
				{
					location.href = self.publicFolder + 'edit.php?ID=' + answer.DATA[0]['ID'] + '&NEW=Y'
				}
				else 
				{
	...
	BX.Voximplant.rentPhone.prototype.unlinkPhone = function (id)
	{
		...
		this.blockAjax = true;
		BX.ajax({
			url: '/thurly/components/thurly/voximplant.config.rent/ajax.php?VI_UNLINK_NUMBER',
			method: 'POST',
			dataType: 'json',
			timeout: 60,
			data: {'VI_UNLINK_NUMBER': 'Y', 'NUMBER_ID': id, 'VI_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: function (data)
			{
				BX.closeWait(self.nodes.container);
				self.blockAjax = false;
	...
	BX.Voximplant.rentPhone.prototype.cancelUnlinkPhone = function (id)
	{
		...
		this.blockAjax = true;
		BX.ajax({
			url: '/thurly/components/thurly/voximplant.config.rent/ajax.php?VI_CANCEL_UNLINK_NUMBER',
			method: 'POST',
			dataType: 'json',
			timeout: 60,
			data: {'VI_CANCEL_UNLINK_NUMBER': 'Y', 'NUMBER_ID': id, 'VI_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: function (data)
			{
				BX.closeWait(self.nodes.container);
				self.blockAjax = false;
