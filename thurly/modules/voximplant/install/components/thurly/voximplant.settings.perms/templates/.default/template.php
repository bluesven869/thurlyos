...
\Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/css/main/table/style.css');
...
<form method="POST">
				...
				<table class="table-blue">
					...
					<?foreach ($arResult['ROLES'] as $role):?>
						<tr data-role-id="<?=htmlspecialcharsbx($role['ID'])?>">
							...
							<td class="table-blue-td-action">
								<a class="table-blue-edit" title="<?=GetMessage('VOXIMPLANT_PERM_EDIT')?>" href="<?=$role['EDIT_URL']?>"></a>
					...
					<tr>
						<td colspan="2" class="table-blue-td-link">
							<a href="<?=$arResult['ADD_URL']?>" class="table-blue-link"><?=GetMessage('VOXIMPLANT_PERM_ADD')?></a>
						</td>
					</tr>
				</table>
			</td>
	...
	<?if($arResult['CAN_EDIT']):?>
		<input type="submit" class="webform-small-button webform-small-button-accept" value="<?=GetMessage('VOXIMPLANT_PERM_SAVE')?>">
	<?else:?>
		<span class="webform-small-button webform-small-button-accept" onclick="viOpenTrialPopup('vi_crm_source')">
			<?=GetMessage('VOXIMPLANT_PERM_SAVE')?>
