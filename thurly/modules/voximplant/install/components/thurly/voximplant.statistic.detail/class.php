...
use Thurly\Voximplant;
use Thurly\Voximplant\Security\Permissions;
use Thurly\Voximplant\Security\Helper;

Loc::loadMessages(__FILE__);

class CVoximplantStatisticDetailComponent extends \CThurlyComponent
{
	...
	protected function init()
	{
		...
		$this->gridOptions = new \Thurly\Main\Grid\Options($this->gridId);
	...
	protected function getFilter(array $gridFilter)
	{
		...
		if (strlen($filter["START_DATE_from"]) > 0)
		{
			try
			{
				$result[">=CALL_START_DATE"] = new \Thurly\Main\Type\DateTime($filter["START_DATE_from"]);
			} catch (Exception $e)
			{
			}
		}
		...
		if (strlen($filter["START_DATE_to"]) > 0)
		{
			try
			{
				$result["<=CALL_START_DATE"] = new \Thurly\Main\Type\DateTime($filter["START_DATE_to"]);
			} catch (Exception $e)
			{
			}
		}
		...
		{
			if(\Thurly\Voximplant\StatisticTable::getEntity()->fullTextIndexEnabled('COMMENT'))
	...
	protected function getRuntimeFields(array $gridFilter)
	{
		...
		if (isset($filter["TRANSCRIPT_TEXT"]) && Voximplant\Model\TranscriptLineTable::getEntity()->fullTextIndexEnabled('MESSAGE'))
		{
			...
			if($transcriptText != '')
			{
				$result["HAS_TRANSCRIPT"] = new \Thurly\Main\Entity\ExpressionField(
					"HAS_TRANSCRIPT",
	...
	{
		$query = new Query(\Thurly\Voximplant\Model\TranscriptLineTable::getEntity());
		$query->addSelect('ID');
	...
	protected function getCrmHtml($statisticRecord, $crmFields)
	{
		...
		{
			$hasActivity = true;
			$result .= '<div><a href="javascript:void(0);" onclick="'.$this->getActivityShowCode($statisticRecord['CRM_ACTIVITY_ID']).'" target="_blank">' . $activityDescription . '</a></div>';
		}
	...
	protected function getLock()
	{
		...
		$lockTimestamp = (int)\Thurly\Main\Config\Option::get(self::MODULE, self::LOCK_OPTION);

		if($lockTimestamp > 0)
		{
			if($currentTimestamp - $lockTimestamp > 60)
			{
				\Thurly\Main\Config\Option::set(self::MODULE, self::LOCK_OPTION, $currentTimestamp);
				return true;
			}
			else
			{
	...
	protected function releaseLock()
	{
		...
		\Thurly\Main\Config\Option::set(self::MODULE, self::LOCK_OPTION);
	}

	protected static function formatPhoneNumber($number)
	{
		return \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($number)->format();
	}

	protected function formatDate(\Thurly\Main\Type\DateTime $date)
	{
		if (!$date)
		{
			return '-';
