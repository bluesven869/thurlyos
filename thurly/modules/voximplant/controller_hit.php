...
)
{
	...
	else if($params["COMMAND"] == "HangupCall")
	{
		...
		if ($call)
		{
			$res = \Thurly\Voximplant\Model\QueueUserTable::getList(Array(
				'filter' => Array('=QUEUE_ID' => $call['QUEUE_ID']),
			));
	...
	else if($params["COMMAND"] == "GetNextInQueue")
	{
		...
		else if(isset($params['CALL_ID']))
		{
			...
			if(is_array($call))
			{
				...
				{
					$config = \Thurly\Voximplant\ConfigTable::getById($call['CONFIG_ID'])->fetch();
		...
		if($queueId > 0)
			$queueParams = \Thurly\Voximplant\Model\QueueTable::getById($queueId)->fetch();
	...
	elseif($params["COMMAND"] == "InviteUsers")
	{
		...
		\Thurly\Voximplant\Integration\Pull::sendInvite($users, $callId);
	}
	...
	elseif (isset($params['BX_TYPE']))
	{
		...
		{
			$params['OPERATOR_CONTRACT'] = \Thurly\Main\Text\Encoding::convertEncodingToCurrent($params['OPERATOR_CONTRACT']);
		...
		{
			$res = Thurly\Voximplant\CallTable::getList(Array(
				'filter' => Array('=CALL_ID' => $params['CALL_ID_TMP']),
			));
			if ($call = $res->fetch())
			{
				Thurly\Voximplant\CallTable::delete($call['ID']);
		...
		{
			$params['REVIEWER_COMMENT'] = \Thurly\Main\Text\Encoding::convertEncodingToCurrent($params['REVIEWER_COMMENT']);
		...
		else if($params["COMMAND"] == "NotifyAdmins")
		{
			...
			if(!is_array($buttons))
				$buttons = array();
			\Thurly\Voximplant\Integration\Im::notifyAdmins($message, $buttons);
		}
		else if($params["COMMAND"] == "TranscriptionComplete")
		{
			\Thurly\Voximplant\Transcript::onTranscriptionComplete(array(
				'SESSION_ID' => $params['SESSION_ID'],
