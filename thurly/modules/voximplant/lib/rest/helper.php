...
namespace Thurly\Voximplant\Rest;
...
use Thurly\Main\PhoneNumber\Format;
use Thurly\Main\PhoneNumber\Parser;
use Thurly\Main\SystemException;
use Thurly\Main\Type\DateTime;
use Thurly\Main\UserTable;
use Thurly\Rest\AppTable;
use Thurly\Rest\EventTable;
use Thurly\Voximplant\CallTable;
use Thurly\Voximplant\Integration\Im;
use Thurly\Voximplant\Model\ExternalLineTable;
use Thurly\Voximplant\PhoneTable;
use Thurly\Voximplant\Result;
use Thurly\Voximplant\Security;
use Thurly\Voximplant\StatisticTable;
...
class Helper
{
	...
	public static function registerExternalCall(array $fields)
	{
		...
		{
			$crmEntity = \CVoxImplantCrmHelper::GetCrmEntity($fields['PHONE_NUMBER'], \Thurly\Voximplant\Security\Helper::getCurrentUserId());
	...
	public static function uploadRecord($callId)
	{
		...
		$statisticRecord = \Thurly\Voximplant\StatisticTable::getByCallId($callId);
		if(!$statisticRecord)
		{
			$result->addError(new Error("Call is not found in the statistic table. Looks like it is not finished yet."));
			return $result;
