...
class CMobileHelper
{
	public static function InitFileStorage()
	{
		...
		if (!$bInited)
		{
			...
			if ($bDiskEnabled)
			{
				$storage = \Thurly\Disk\Driver::getInstance()->getStorageByUserId($GLOBALS["USER"]->GetID());
				if (!$storage)
				{
					$arResult = array(
		...
		if ($type == "blog")
		{
			$arCommentParams = Array(
				...
				"URL" => array(
					...
					"MODERATE" => str_replace(
						array("#source_post_id#", "#post_id#", "#comment_id#", "&".thurly_sessid_get()),
			...
			if ($arFields["SHOW_RATING"] == "Y")
			{
				...
				$GLOBALS["APPLICATION"]->IncludeComponent(
					"thurly:rating.vote", $arFields["RATING_TYPE"],
					Array(
				...
				ob_start();
				$GLOBALS["APPLICATION"]->IncludeComponent(
					"thurly:rating.vote", "mobile_comment_".$arFields["RATING_TYPE"],
					Array(
						"ENTITY_TYPE_ID" => "BLOG_COMMENT",
						"ENTITY_ID" => $arFields["arComment"]["ID"],
						"OWNER_ID" => $arFields["arComment"]["AUTHOR_ID"],
				...
				)
				{
					...
					foreach($arUF[$ufCode]["VALUE"] as $attachedId)
					{
						...
						if($attachedObject)
						{
							...
							$fileUrl = str_replace("/thurly/tools/disk/uf.php", SITE_DIR."mobile/ajax.php", $fileUrl);
							$fileUrl = $fileUrl.(strpos($fileUrl, "?") === false ? "?" : "&")."mobile_action=disk_uf_view&filename=".$fileName;

							if (
								\Thurly\Disk\TypeFile::isImage($file)
							...
							)
							{
								$previewImageUrl = $urlManager->getUrlUfController(
									...
									array(
										...
										'height' => 144,
										'exact' => 'Y',
										'signature' => \Thurly\Disk\Security\ParameterSigner::getImageSignature($attachedId, 144, 144)
									)
								);
							}
							else
							...
							$iconUrl = CComponentEngine::makePathFromTemplate('/thurly/components/thurly/mobile.disk.file.detail/images/'.$icon);

							$fileFata = array(
								'type' => $file->getExtension(),
								'ufCode' => $ufCode,
								'id' => $attachedId,
								'objectId' => \Thurly\Disk\Uf\FileUserType::NEW_FILE_PREFIX.$file->GetId(),
	...
	public static function createLink($tag)
	{
		...
		else if (substr($tag, 0, 11) == 'IM|MENTION|')
		{
			$params = explode("|", $tag);
			$result = SITE_DIR.'mobile/im/dialog.php';
			$uniqueParams = "{dialogId: 'chat".$params[2]."'}";
			$unique = true;
		}
