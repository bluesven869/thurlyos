...
Class mobile extends CModule
{
	...
	{
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"] . "/thurly/modules/mobile/public/mobile/", $_SERVER["DOCUMENT_ROOT"] . "/mobile/", True, True);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"] . "/thurly/modules/mobile/install/templates/", $_SERVER["DOCUMENT_ROOT"] . "/thurly/templates/", True, True);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"] . "/thurly/modules/mobile/install/js/", $_SERVER["DOCUMENT_ROOT"] . "/thurly/js/", true, true);
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"] . "/thurly/modules/mobile/install/js/", $_SERVER["DOCUMENT_ROOT"] . "/thurly/js/", true, true);
		CopyDirFiles($_SERVER['DOCUMENT_ROOT'] . '/thurly/modules/mobile/install/tools/', $_SERVER['DOCUMENT_ROOT'] . '/thurly/tools/', true, true);
		CopyDirFiles($_SERVER['DOCUMENT_ROOT'] . '/thurly/modules/mobile/install/services/', $_SERVER['DOCUMENT_ROOT'] . '/thurly/services/', true, true);
		COption::SetOptionString("socialnetwork", "urlToPost", "/mobile/log/index.php");
		CopyDirFiles(
			$_SERVER["DOCUMENT_ROOT"] . "/thurly/modules/mobile/install/components/",
	...
	function UnInstallFiles()
	{
		if ($_ENV["COMPUTERNAME"] != 'BX')
		{
			...
			DeleteDirFilesEx('/thurly/templates/mobile_app/');
		}
		return true;
	}
	...
	{
		if (!IsModuleInstalled('pull') && file_exists($_SERVER['DOCUMENT_ROOT'] . "/thurly/modules/pull/install/index.php"))
		{
			include_once($_SERVER['DOCUMENT_ROOT'] . "/thurly/modules/pull/install/index.php");
			$pull = new pull();
			$pull->InstallFiles();
			$pull->InstallDB();
		}
	...
	{
		if (!IsModuleInstalled('mobileapp') && file_exists($_SERVER['DOCUMENT_ROOT'] . "/thurly/modules/mobileapp/install/index.php"))
		{
			include_once($_SERVER['DOCUMENT_ROOT'] . "/thurly/modules/mobileapp/install/index.php");
			$pull = new mobileapp();
			$pull->InstallFiles();
			$pull->InstallDB();
		}
	...
	function DoInstall()
	{
		...
		$APPLICATION->IncludeAdminFile(GetMessage("APP_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"] . "/thurly/modules/mobile/install/step.php");
	}
	...
	function DoUninstall()
	{
		...
		if ($USER->IsAdmin())
		{
			...
			if ($step < 2)
			{
				...
				$APPLICATION->IncludeAdminFile(GetMessage("APP_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"] . "/thurly/modules/mobile/install/unstep1.php");
			}
			elseif ($step == 2)
			{
				$this->UnInstallDB();
				...
				$APPLICATION->IncludeAdminFile(GetMessage("APP_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"] . "/thurly/modules/mobile/install/unstep.php");
			}
		}
	}
}
