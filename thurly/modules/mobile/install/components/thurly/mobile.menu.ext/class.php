 ...
 * Class MobileMenu
 */
class MobileMenu extends \CThurlyComponent
{
	...
	public function executeComponent()
	{
		...
		else
		{
			...
			$arResult["MENU"] = include(\Thurly\Main\Application::getDocumentRoot() . $this->getPath() . "/.mobile_menu.php");
			$host = Thurly\Main\Context::getCurrent()->getServer()->getHttpHost();
		...
		$events = \Thurly\Main\EventManager::getInstance()->findEventHandlers("mobile", "onMobileMenuStructureBuilt");
		if (count($events) > 0)
		{
		...
		if ($arResult["USER"]["AVATAR"])
		{
			...
			\Thurly\Main\Data\AppCacheManifest::getInstance()->addFile($file);
		}
