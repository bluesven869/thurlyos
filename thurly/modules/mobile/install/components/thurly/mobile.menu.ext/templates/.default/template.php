...
$langMessages = \Thurly\Main\Localization\Loc::loadLanguageFile(\Thurly\Main\Application::getDocumentRoot().$this->GetFile());
$jsLangMessages = CUtil::PhpToJSObject($langMessages);
$langMessageJS = <<<JS
	(window.BX||top.BX).message($jsLangMessages);
JS;

\Thurly\Main\Page\Asset::getInstance()->addString("<script type=\"text/javascript\">$langMessageJS</script>");
...
$initData = array(
	...
	"siteDir" => SITE_DIR,
	"canInvite" => (IsModuleInstalled("thurlyos") && $USER->CanDoOperation('thurlyos_invite')),
...
{
	$userColor = Thurly\Im\Color::getColorByNumber($initData['userId']);
...
<div class="menu-user" id="menu-user"
	...
	<div class="menu-user-actions<? if (!$showHelpIcon): ?> menu-user-actions-50<? endif ?>">
		<div class="menu-user-action menu-user-accounts" id="menu-user-accounts">
			<span><?= GetMessage("MB_MY_THURLY24") ?></span>
		</div>
		<? if ($showHelpIcon): ?>
		...
		<? if (!$bExtranet && IsModuleInstalled("timeman")): ?>
			<? $APPLICATION->IncludeComponent('thurly:timeman', 'mobile', array(), $component, array("HIDE_ICONS" => "Y")) ?>
		<? endif ?>
