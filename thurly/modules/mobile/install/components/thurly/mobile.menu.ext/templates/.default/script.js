			...
			if(BX.localStorage.get("carddav_ask") != "Y")
			{
				...
				app.confirm({
					...
					callback:function(buttonIndex){
						if(buttonIndex == 1)
						{
							BX.ajax.get("/thurly/tools/dav_profile.php?action=token&params[resources]=carddav", {}, function(json){
								console.log(json);
								var response  = JSON.parse(json);
								if(response.token)
								{
									var urlPath = "/thurly/tools/dav_profile.php?action=payload&params[resources]=carddav&params[access_token]=";
	...
	BX.MobileUI.List.addListener(
		function(eventName, params, listObject)
		{
			...
			else if(eventName == "createLink")
			{
				if(params.ID)
				{
					BX.ajax.post("/thurly/components/thurly/disk.folder.list/ajax.php?action=generateExternalLink",{
						objectId:params.ID,
