...
function RegisterNewUser($SITE_ID, $arFields)
{
	...
	{
		CUserOptions::SetOption("thurlyos", "invite_message_text", $arFields["MESSAGE_TEXT"]);
	}
	...
	{
		$event = new CEvent;
		$event->SendImmediate("THURLY24_USER_INVITATION", $SITE_ID, array(
			"EMAIL_FROM" => $USER->GetEmail(),
			"EMAIL_TO" => $userData["EMAIL"],
			"LINK" => CHTTP::URN2URI("/thurly/tools/intranet_invite_dialog.php?user_id=".$userData["ID"]."&checkword=".urlencode($userData["CONFIRM_CODE"])),
		...
		foreach ($arEmailToRegister as $userData)
		{
			...
			else
			{
				...
				$event->SendImmediate("THURLY24_USER_INVITATION", $SITE_ID, array(
					"EMAIL_FROM" => $USER->GetEmail(),
					"EMAIL_TO" => $userData["EMAIL"],
					"LINK" => CHTTP::URN2URI("/thurly/tools/intranet_invite_dialog.php?user_id=".$ID."&checkword=".urlencode($arUser["CONFIRM_CODE"])),
					"USER_TEXT" => $messageText,
				));
			}
		}
...
function ReinviteUser($SITE_ID, $USER_ID)
{
	...
	{
		$messageText = ($userMessageText = CUserOptions::GetOption("thurlyos", "invite_message_text")) ? htmlspecialcharsbx($userMessageText) : GetMessage("BX24_INVITE_DIALOG_INVITE_MESSAGE_TEXT");

		$event = new CEvent;
		$event->SendImmediate("THURLY24_USER_INVITATION", $SITE_ID, array(
			"EMAIL_FROM" => $USER->GetEmail(),
			"EMAIL_TO" => $arUser["EMAIL"],
			"LINK" => CHTTP::URN2URI("/thurly/tools/intranet_invite_dialog.php?user_id=".$USER_ID."&checkword=".urlencode($arUser["CONFIRM_CODE"])),
			"USER_TEXT" => $messageText,
		));
		return true;
	}
