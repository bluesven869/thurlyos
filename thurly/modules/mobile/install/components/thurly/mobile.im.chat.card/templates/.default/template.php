...
<script type="text/javascript">
	BX.bind(BX('chat-write'), 'click', function(){
		BXMobileApp.PageManager.loadPageUnique({
			'url' : (BX.message('MobileSiteDir') ? BX.message('MobileSiteDir') : '/') + 'mobile/im/dialog.php'+(!app.enableInVersion(11)? "?id=chat<?=$arResult['CHAT']['id']?>":""),
			'bx24ModernStyle' : true,
			'data': {dialogId: 'chat<?=$arResult['CHAT']['id']?>'}
		});
	});
		...
		else if (data.command == 'chatAvatar')
		{
			if (<?=$arResult['CHAT_ID']?> == data.params.chatId)
			{
				BX.ajax({
					...
					dataType: 'json',
					timeout: 60,
					data: {'IM_GET_MOBILE_CHAT_AVATAR' : 'Y', 'CHAT_ID' : data.params.chatId, 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
