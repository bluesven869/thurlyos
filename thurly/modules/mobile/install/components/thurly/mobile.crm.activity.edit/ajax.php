...
if(!function_exists('__CrmMobileActivityEditEndResonse'))
{
	function __CrmMobileActivityEditEndResonse($result)
	{
...
if($action === 'SAVE_ENTITY')
{
	...
	if(empty($commData))
	{
		__CrmMobileActivityEditEndResonse(array('ERROR' => GetMessage('CRM_ACTIVITY_SELECT_COMMUNICATION')));
	}
	...
	if($ownerTypeName === '' || $ownerID <= 0)
	{
		__CrmMobileActivityEditEndResonse(array('ERROR' => GetMessage('CRM_ACTIVITY_OWNER_NOT_FOUND')));
	}
	...
	if($typeID === CCrmActivityType::Call || $typeID === CCrmActivityType::Meeting)
	{
		...
		{
			//$DB->Rollback();
			__CrmMobileActivityEditEndResonse(array('ERROR' => $errorMessage));
		}
	}
	elseif($typeID === CCrmActivityType::Email)
	{
		...
		if($from === '')
		{
			...
			{
				__CrmMobileActivityEditEndResonse(array('ERROR' => GetMessage('CRM_ACTIVITY_EDIT_EMAIL_EMPTY_FROM_FIELD')));
			}
		}
		elseif(!check_email($from))
		{
			__CrmMobileActivityEditEndResonse(array('ERROR' => GetMessage('CRM_ACTIVITY_EDIT_INVALID_EMAIL', array('#VALUE#' => $from))));
		}
