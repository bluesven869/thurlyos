...
global $APPLICATION;
$APPLICATION->AddHeadString('<script type="text/javascript" src="' . CUtil::GetAdditionalFileURL(SITE_TEMPLATE_PATH . '/crm_mobile.js') . '"></script>', true, \Thurly\Main\Page\AssetLocation::AFTER_JS_KERNEL);
...
$title = $isNew ? GetMessage('M_CRM_ACTIVITY_EDIT_NEW_CALL') : $entity['SUBJECT'];
...
<div class="crm_toppanel">
	<div class="crm_filter"><span class="crm_tel_icon"></span><?=htmlspecialcharsbx($title)?></div>
</div>
<div class="crm_wrapper">
	<div class="crm_block_container aqua_style comments">
		<div class="crm_arrow">
			<div class="crm_block_title">
				<?=htmlspecialcharsbx(GetMessage('M_CRM_ACTIVITY_EDIT_CALL_FIELD_TIME'))?>: <span id="<?=$prefix?>_start_time_text"><?=isset($entity['START_TIME']) ? $entity['START_TIME'] : htmlspecialcharsbx(GetMessage('M_CRM_ACTIVITY_EDIT_CALL_TIME_NOT_SPECIFIED'))?></span>
			</div>
	...
	<div class="crm_block_container blue_style_footer">
		<div class="crm_card">
			<table><tbody><tr>
				...
				<td class="tal fwb" style="font-size: 13px; color: #75818b;"><?=htmlspecialcharsbx(GetMessage('M_CRM_ACTIVITY_EDIT_CALL_FIELD_NOTIFY'))?></td>
				<td class="tal fwb">
	...
	<div class="crm_block_container">
		<div class="crm_block_title fln"><?=htmlspecialcharsbx(GetMessage('M_CRM_ACTIVITY_EDIT_CALL_DETAIL_SECTION'))?></div>
		<hr>
		<div class="crm_meeting_info">
			<input id="<?=$prefix?>_subject" type="text" class="crm_input_text" placeholder="<?=htmlspecialcharsbx(GetMessage('M_CRM_ACTIVITY_EDIT_CALL_FIELD_SUBJECT'))?>" value="<?=htmlspecialcharsbx($entity['SUBJECT'])?>"/>
			<textarea id="<?=$prefix?>_description" class="crm_input_text" placeholder="<?=htmlspecialcharsbx(GetMessage('M_CRM_ACTIVITY_EDIT_CALL_FIELD_DESCRIPTION'))?>"><?=htmlspecialcharsbx($entity['DESCRIPTION'])?></textarea>
...
<script type="text/javascript">
	BX.ready(
		function()
		{
			...
			?>
			var editor = BX.CrmCallEditor.create(
				"<?=CUtil::JSEscape($UID)?>",
				{
			...
			context.createButtons(
				{
					back:
					{
						...
						style: 'custom',
						position: 'left',
						name: '<?=GetMessageJS('M_CRM_ACTIVITY_EDIT_CALL_CANCEL_BTN')?>',
					...
					save:
					{
						...
						style: 'custom',
						position: 'right',
						name: '<?=GetMessageJS("M_CRM_ACTIVITY_EDIT_CALL_{$mode}_BTN")?>',
