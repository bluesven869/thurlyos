...
var REVISION = 1; // api revision - check module/pull/include.php
...
Connection.prototype.broadcastMessages = function (messages)
{
	...
	messages.forEach(function (message)
	{
		...
		message.extra.server_time_unix = parseInt((new Date(message.extra.server_time)).getTime()/1000);
		message.extra.server_time_ago = parseInt(((new Date()).getTime()-(new Date(message.extra.server_time)).getTime())/1000)-this.config.server.timeShift;
...
function AppCounters()
{
	...
	this.database = new ReactDatabase("communication_counters", CONFIG.USER_ID);

	this.loadFromCache();
}

AppCounters.prototype.onUpdateUserCounters = function(counters, merge)
{
	this.userCounters = merge? Utils.objectMerge(this.userCounters, counters): counters;
...
AppCounters.prototype.onPull = function(command, params)
{
	if (command == 'user_counter')
	{
		this.onUpdateUserCounters(params, true);
	}
};
...
{
	this.database.table(tables.counters).then(table =>
	{
		table.get().then(items =>
		{
			if (items.length <= 0 || this.total != 0)
			{
				return true;
			}

			this.counters = JSON.parse(items[0].VALUE);
			...
			for (let element in this.counters)
			{
				...
				this.total += this.counters[element];
			}

			console.info("AppCounters.loadFromCache: counters load from cache", this.counters);
			if (this.total > 0)
			{
				Application.setBadges(this.counters);
				if (!Application.isBackground() && typeof Application.setIconBadge != 'undefined')
				{
					Application.setIconBadge(this.total);
				}
			}
...
{
	clearTimeout(this.refreshCountersTimeout);
	this.refreshCountersTimeout = setTimeout(() =>
	{
		this.database.table(tables.counters).then(table =>
		{
			table.delete().then(() =>
			{
				table.add({value : this.counters}).then(() =>
				{
					console.info("AppCounters.updateCache: counters updated", "count: "+this.total);
				});
			})
		});
...
var PushNotifications = {
	...
	actions: ["post", "tasks", "comment", "mention", "share", "share2users"],
	init:function(){
