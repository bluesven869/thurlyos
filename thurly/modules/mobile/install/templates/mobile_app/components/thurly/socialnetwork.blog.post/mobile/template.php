...
include_once($_SERVER["DOCUMENT_ROOT"].SITE_TEMPLATE_PATH."/components/thurly/socialnetwork.blog.post/mobile/functions.php");
	...
	else
	{
		...
		if ($arParams["IS_LIST"])
		{
			?><script type="text/javascript">
				arLogTs.entry_<?=intval($arParams["LOG_ID"])?> = <?=intval($arParams["LAST_LOG_TS"])?>;
			</script><?
		}
		else
		{
			?><script>
				BX.ready(function()
				{
					...
					oMSL.InitDetail({
						...
						bUseTasks: <?=(isset($arResult["bTasksAvailable"]) && $arResult["bTasksAvailable"] ? 'true' : 'false')?>,
						entryParams: {
					...
					if ($arResultVote)
					{
						...
						?><span id="rating_button_<?=$arParams["LOG_ID"]?>"><?
							?><div class="post-item-informers post-item-inform-likes<?=($arResultVote['USER_HAS_VOTED'] == 'Y' ? '-active' : '')?>" id="bx-ilike-button-<?=CUtil::JSEscape(htmlspecialcharsbx($arResultVote['VOTE_ID']))?>"><?
								...
								$like = COption::GetOptionString("main", "rating_text_like_y", GetMessage("BLOG_MOBILE_LIKE_ACTION"));
								$like2 = str_replace('#LIKE#', $like, GetMessage("BLOG_MOBILE_LIKE2_ACTION_PATTERN"));
