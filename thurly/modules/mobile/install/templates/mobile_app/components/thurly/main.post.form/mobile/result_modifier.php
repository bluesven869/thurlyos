...
$arResult["diskEnabled"] = (\Thurly\Main\Config\Option::get('disk', 'successfully_converted', false) && CModule::includeModule('disk'));
...
//				'INPUT_VALUE' => $arResult["VALUE"],
			...
			$GLOBALS["APPLICATION"]->IncludeComponent('thurly:mobile.file.upload', '', $componentParams, false, Array("HIDE_ICONS" => "Y"));
		}
				...
				elseif ($FIELD_NAME == "UF_BLOG_POST_FILE")
				{
					if ($arResult["diskEnabled"])
					{
						...
						if (is_array($arPostField["VALUE"]))
						{
							...
							{
								$oAttachedModel = \Thurly\Disk\AttachedObject::loadById($val, array('OBJECT.STORAGE', 'VERSION'));
								if ($oAttachedModel)
								{
						...
						{
							$oAttachedModel = \Thurly\Disk\AttachedObject::loadById($arPostField["VALUE"], array('OBJECT.STORAGE', 'VERSION'));
							if ($oAttachedModel)
							{
								$oDiskFile = $oAttachedModel->getFile();
								if ($oDiskFile)
...
$bAllowToAll = (
	...
	&& \Thurly\Socialnetwork\ComponentHelper::getAllowToAllDestination()
);
