		...
		foreach ($arResult['TASK']['PARAMETERS']['REQUEST'] as $request)
		{
			...
			if (!in_array($type, $checkedTypes))
			{
				if (isset($arResult['TypesMap'][$type]))
				{
					...
					if (!$typeClass::canRenderControl(\Thurly\Bizproc\FieldType::RENDER_MODE_MOBILE))
					{
						$arResult["SKIP_BP"] = 'Y';
						break;
					}
...
if (!empty($arResult["TASK"]["DESCRIPTION"]))
{
	$arResult["TASK"]["DESCRIPTION"] = preg_replace_callback(
		'|<a href="/thurly/tools/bizproc_show_file.php\?([^"]+)"\starget=\'_blank\'>|',
		function($matches)
		{
	...
	$arResult["TASK"]["DESCRIPTION"] = preg_replace_callback(
		'|<a href="/thurly/tools/disk/uf.php\?([^"]+)"\starget=\'_blank\'>([^<]+)|',
		function($matches)
		{
			parse_str($matches[1], $query);
