...
(function ()
{
	...
	BX.MessengerMobileLegacy.prototype.openSearch = function ()
	{
		app.openTable({
			...
			callback: function (data)
			{
				...
				app.loadPageBlank({
					'url': (BX.message('MobileSiteDir') ? BX.message('MobileSiteDir') : '/') + 'mobile/im/dialog.php?id=' + user_id,
					'unique': true
				});
	...
	BX.MessengerMobileLegacy.prototype.sendMessageAjax = function (messageTmpIndex, recipientId, messageText, sendMessageToChat)
	{
		...
		BX.ajax({
			...
			method: 'POST',
			dataType: 'json',
			data: {'IM_SEND_MESSAGE': 'Y', 'CHAT': sendMessageToChat ? 'Y' : 'N', 'ID': 'temp' + messageTmpIndex, 'RECIPIENT_ID': recipientId, 'MESSAGE': messageText, 'TAB': recipientId, 'MOBILE': 'Y', 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()},
	...
	BX.MessengerMobileLegacy.prototype.drawMessage = function (message, appendTop)
	{
		...
		if (message.params && message.params.FILE_ID)
		{
			for (var i = 0; i < message.params.FILE_ID.length; i++)
			{
				...
				if (file['urlPreview'] && file['urlPreview']['mobile'])
				{
					file['name'] = '<a href="'+file['urlShow']['mobile']+'">'+file['name']+'</a>';
				}
		...
		else
		{
			...
			this.timeoutAnimation = setTimeout(function ()
			{
				...
				this.intervalAnimation = ThurlyAnimation.animate({
					duration: 1000,
	...
	BX.MessengerMobileLegacy.prototype.readMessage = function (userId, lastId)
	{
		...
		this.timeoutReadMessage = setTimeout(function ()
		{
			BX.ajax({
				...
				method: 'POST',
				dataType: 'json',
				data: {'IM_READ_MESSAGE': 'Y', 'USER_ID': userId, 'LAST_ID': lastId, 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function (data)
				{
					if (data.ERROR.length == 0)
					{
	...
	BX.MessengerMobileLegacy.prototype.updateStateLight = function ()
	{
		...
		this.timeoutUpdateStateLight = setTimeout(BX.delegate(function ()
		{
			BX.ajax({
				...
				dataType: 'json',
				timeout: 20,
				data: {'IM_UPDATE_STATE_LIGHT': 'Y', 'MOBILE': 'Y', 'SITE_ID': BX.message('SITE_ID'), 'NOTIFY': 'Y', 'MESSAGE': 'Y', 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()},
				onsuccess: BX.delegate(function (data)
				{
					if (data.ERROR.length == 0)
					{
	...
	BX.MessengerMobileLegacy.prototype.getHistory = function (userId)
	{
		...
		BX.ajax({
			...
			method: 'POST',
			dataType: 'json',
			data: {'IM_HISTORY_LOAD_MORE': 'Y', 'USER_ID': userId, 'PAGE_ID': this.historyPage, 'MOBILE': 'Y', 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()},
			onsuccess: BX.delegate(function (data)
			{
				var count = 0;
				var height = 0;
	...
	BX.MessengerMobileLegacy.formatDate = function (timestamp)
	{
		var format = [
			...
			["", BX.date.convertThurlyFormat(BX.message("IM_FORMAT_DATETIME"))]
		];
	...
	BX.MessengerMobileLegacy.prototype.sendWriting = function (userId)
	{
		if (parseInt(userId) > 0 && !this.writingSendList[userId])
		{
			...
			BX.ajax({
				...
				method: 'POST',
				dataType: 'json',
				data: {'IM_START_WRITING': 'Y', 'DIALOG_ID': DIALOG_ID, 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()}
			});
