...
class Action
{
	...
	{
		$this->actions = include(\Thurly\Main\Application::getDocumentRoot()."/thurly/modules/mobile/ajax_action.php");
	}
	...
	public function executeAction($name, $params = array())
	{
		...
		if ($actionDesc)
		{
			...
			$isSessidValid = true;
			if ($actionDesc["needThurlySessid"] == true || (array_key_exists("sessid", $_REQUEST) && strlen($_REQUEST["sessid"]) > 0))
			{
