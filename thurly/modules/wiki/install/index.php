...
Class wiki extends CModule
{
	...
	function InstallDB()
	{
		...
		$eventManager->registerEventHandler('socialnetwork', 'onLogIndexGetContent', 'wiki', '\Thurly\Wiki\Integration\Socialnetwork\Log', 'onIndexGetContent');
		return true;
	}
	...
	function UnInstallDB()
	{
		...
		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->unRegisterEventHandler('socialnetwork', 'onLogIndexGetContent', 'wiki', '\Thurly\Wiki\Integration\Socialnetwork\Log', 'onIndexGetContent');
		return true;
	}
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/install/admin', $_SERVER['DOCUMENT_ROOT'].'/thurly/admin', true, true);
			CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/install/images', $_SERVER['DOCUMENT_ROOT'].'/thurly/images/wiki', true, true);
			CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/install/themes', $_SERVER['DOCUMENT_ROOT'].'/thurly/themes', true, true);
			CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/install/components', $_SERVER['DOCUMENT_ROOT'].'/thurly/components', true, true);
		}
		return true;
	}
	...
	{
		DeleteDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/install/admin', $_SERVER['DOCUMENT_ROOT'].'/thurly/admin');
		DeleteDirFilesEx('/thurly/images/wiki/');
		DeleteDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/install/themes/.default/', $_SERVER['DOCUMENT_ROOT'].'/thurly/themes/.default');//css
		DeleteDirFilesEx('/thurly/themes/.default/icons/wiki/');//icons
		return true;
	}
	...
	function DoInstall()
	{
		...
		if(!CBXFeatures::IsFeatureEditable('Wiki'))
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage('WIKI_INSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/install/step3.php');
		}
		elseif ($step < 2)
			$APPLICATION->IncludeAdminFile(GetMessage('WIKI_INSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/install/step.php');
		elseif ($step == 2)
			$APPLICATION->IncludeAdminFile(GetMessage('WIKI_INSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/install/step2.php');
		else
		{
			$this->InstallDB();
			$this->InstallFiles();
	...
	function DoUninstall()
	{
		...
		{
			require_once($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/include.php');
			CWikiSocnet::EnableSocnet(false);
		}
		$this->UnInstallFiles();
		$this->UnInstallDB();
		CBXFeatures::SetFeatureEnabled('Wiki', false);
		$APPLICATION->IncludeAdminFile(GetMessage('WIKI_UNINSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/wiki/install/unstep.php');
	}
