...
)
{
	...
	elseif (IntVal($_GET["show_comment_id"]) > 0)
	{
		...
		$arTagInline = \Thurly\Socialnetwork\Util::detectTags($arComment, array('POST_TEXT'));
			...
			else
			{
				if(check_thurly_sessid())
				{
					if($commentID = CBlogComment::Update($arComment["ID"], Array(
						...
						"SEARCH_GROUP_ID" => \Thurly\Main\Config\Option::get("socialnetwork", "userbloggroup_id", false, SITE_ID)
					)))
					{
						BXClearCache(true, "/blog/comment/".intval($arParams["ID"] / 100)."/".$arParams["ID"]."/");
		...
		if($arResult["Perm"] >= BLOG_PERMS_PREMODERATE)
		{
			if(check_thurly_sessid())
			{
				...
				if($_POST["act"] != "edit")
				{
					...
					{
						include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/captcha.php");
					...
					{
						if (!\Thurly\Blog\Item\Comment::checkDuplicate(array(
					...
					if(strlen($strErrorMessage)<=0)
					{
						...
						if(
							...
							&& ($urlPreviewValue = \Thurly\Socialnetwork\ComponentHelper::getUrlPreviewValue($arFields["POST_TEXT"]))
						)
						{
							$arFields["UF_BLOG_COMM_URL_PRV"] = $urlPreviewValue;
						}
						$arTagInline = \Thurly\Socialnetwork\Util::detectTags($arFields, array('POST_TEXT'));
							...
							if($commentID = CBlogComment::Update($commentID, $arFields))
							{
								...
								)
								{
									...
									if (!empty($arUserIdToShare))
									{
										...
										\Thurly\Socialnetwork\ComponentHelper::processBlogPostShare(
											array(
												"POST_ID" => $arPost["ID"],
	...
	if($arResult["Perm"]>=BLOG_PERMS_READ)
	{
		...
		else
		{
			...
			)
			{
				...
				$cache_id = "blog_comment_".$USER->IsAuthorized()."_".md5(serialize($arCacheID))."_".LANGUAGE_ID."_".$arParams["DATE_TIME_FORMAT"]."_".Thurly\Main\Context::getCurrent()->getCulture()->getDateTimeFormat();
				...
				)
				{
					...
					if (!empty($arResult["Assets"]))
					{
						if (!empty($arResult["Assets"]["CSS"]))
						{
							...
							{
								\Thurly\Main\Page\Asset::getInstance()->addCss($cssFile);
							}
						}
						...
						if (!empty($arResult["Assets"]["JS"]))
						{
							...
							{
								\Thurly\Main\Page\Asset::getInstance()->addJs($jsFile);
							}
						}
					}

					CThurlyComponentTemplate::ApplyCachedData($Vars["templateCachedData"]);
					$cache->Output();
				}
				else
				{
						...
						if($arPost["HAS_COMMENT_IMAGES"] != "N")
						{
							...
							while ($aImg = $res->Fetch())
							{
								...
								$arResult["arImages"][$aImg["COMMENT_ID"]][$aImg['ID']] = Array(
									"small" => "/thurly/components/thurly/blog/show_file.php?fid=".$aImg['ID']."&width=".$arParams["ATTACHED_IMAGE_MAX_WIDTH_SMALL"]."&height=".$arParams["ATTACHED_IMAGE_MAX_HEIGHT_SMALL"]."&type=square"
								);
								...
								$arResult["arImages"][$aImg["COMMENT_ID"]][$aImg['ID']]["full"] = (
									$arParams["MOBILE"] == "Y"
										...
										: "/thurly/components/thurly/blog/show_file.php?fid=".$aImg['ID']."&width=".$arParams["ATTACHED_IMAGE_MAX_WIDTH_FULL"]."&height=".$arParams["ATTACHED_IMAGE_MAX_HEIGHT_FULL"]
								);
							}
						}
									...
									if (count($arPostFields) > 0)
									{
										foreach ($arPostFields as $FIELD_NAME => $arPostField)
										{
											...
											)
											{
												...
												$urlPreviewText = \Thurly\Socialnetwork\ComponentHelper::getUrlPreviewContent($arPostField, array(
		...
		if(is_array($arResult["CommentsResult"]))
		{
			...
			$handlerManager = new Thurly\Socialnetwork\CommentAux\HandlerManager();
			$newFound = false;
				...
				else // check for old shares
				{
					...
					)
					{
						...
						if(!empty($dest))
						{
							...
							if($bDestCut)
							{
								if ($handler = \Thurly\Socialnetwork\CommentAux\Base::init(\Thurly\Socialnetwork\CommentAux\Share::getType(), array(
		...
		$arResult["urlToDelete"] = $arResult["urlToPost"]."delete_comment_id=#comment_id#&comment_post_id=#post_id#&".thurly_sessid_get();
		$arResult["urlToHide"] = $arResult["urlToPost"]."hide_comment_id=#comment_id#&comment_post_id=#post_id#&".thurly_sessid_get();
		$arResult["urlToShow"] = $arResult["urlToPost"]."show_comment_id=#comment_id#&comment_post_id=#post_id#&".thurly_sessid_get();
		$arResult["urlToAnswer"] = $arResult["urlToPost"]."answer_user_id=#user_id#&answer_post_id=#post_id#&".thurly_sessid_get();
