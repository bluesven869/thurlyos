...
else
{
	if ($arParams["ID"] > 0)
	{
		...
		else
		{
			...
			if ($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["delete"]) > 0 && $arResult["CurrentUserPerms"]["IsCurrentUser"] && check_thurly_sessid())
			{
				$errorMessage = "";
				...
				if ($dbFriends)
				{
					...
					while ($arFriends = $dbFriends->GetNext())
					{
						...
						if (intval($arParams["THUMBNAIL_LIST_SIZE"]) > 0)
						{
							...
							$arImage = CSocNetTools::InitImage($arFriends[$pref."_USER_PERSONAL_PHOTO"], 150, "/thurly/images/socialnetwork/nopic_user_150.gif", 150, $pu, $canViewProfile);
