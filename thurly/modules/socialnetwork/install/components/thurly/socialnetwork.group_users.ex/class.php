...
final class SocialnetworkGroupUsersEx extends CThurlyComponent
{
	protected function getUserList($key, $arParams, $arResult, $arNavParams)
	{
		...
		switch($key)
		{
			case "Users":
			case "UsersAuto":
				$arFilter["<=ROLE"] = \Thurly\Socialnetwork\UserToGroupTable::ROLE_USER;
				break;
			case "Moderators":
				$arFilter["<=ROLE"] = \Thurly\Socialnetwork\UserToGroupTable::ROLE_MODERATOR;
				break;
			case "Ban":
				$arFilter["=ROLE"] = \Thurly\Socialnetwork\UserToGroupTable::ROLE_BAN;
				break;
			default:
				$arFilter["<=ROLE"] = \Thurly\Socialnetwork\UserToGroupTable::ROLE_USER;
		}
		...
		if ($dbRequests)
		{
			...
			while ($arRequests = $dbRequests->GetNext())
			{
				...
				if ($key == "Moderators")
				{
					$record["IS_OWNER"] = ($arRequests["ROLE"] == \Thurly\Socialnetwork\UserToGroupTable::ROLE_OWNER);
				}
