...
else
{
	...
	{
		$arContext = \Thurly\Socialnetwork\ComponentHelper::getUrlContext();
		$arParams['PATH_TO_USER_EDIT'] = \Thurly\Socialnetwork\ComponentHelper::addContextToUrl($arParams['PATH_TO_USER_EDIT'], $arContext);
		...
		$arResult["CurrentUserPerms"] = CSocNetUserPerms::InitUserPerms(
			$USER->GetID(), 
			$arResult["User"]["ID"], 
			CSocNetUser::IsCurrentUserModuleAdmin(SITE_ID, !IsModuleInstalled("thurlyos"))
		);
		...
		if (CModule::IncludeModule('mail'))
		{
			...
			if (
				$arParams['ID'] == IntVal($USER->GetID())
				&& method_exists('Thurly\Mail\User','getForwardTo')
			)
			{
				$arResult['User']['EMAIL_FORWARD_TO'] = array();
				if (\Thurly\Main\ModuleManager::isModuleInstalled('blog'))
				{
					$res = Thurly\Mail\User::getForwardTo(SITE_ID, $arParams['ID'], 'BLOG_POST');
					if (is_array($res))
					{
				...
				if (\Thurly\Main\ModuleManager::isModuleInstalled('tasks'))
				{
					$res = Thurly\Mail\User::getForwardTo(SITE_ID, $arParams['ID'], 'TASKS_TASK');
					if (is_array($res))
					{
						list($emailForwardTo) = $res;
						if ($emailForwardTo)
		...
		$arImage = CSocNetTools::InitImage($arResult["User"]["PERSONAL_PHOTO"], $iSize, "/thurly/images/socialnetwork/nopic_user_150.gif", 150, "", false);
						...
						|| in_array($userFieldName, $arParams["USER_FIELDS_PERSONAL"]))
					{
						...
						switch ($userFieldName)
						{
							...
							case 'WORK_LOGO':
								...
								{
									$iSize = 150;
									$arImage = CSocNetTools::InitImage($val, $iSize, "/thurly/images/1.gif", 1, "", false);
									$val = $arImage["IMG"];
								}
								break;
					...
					if ($dbFriends)
					{
						...
						while ($arFriends = $dbFriends->GetNext())
						{
							...
							if (intval($arParams["THUMBNAIL_LIST_SIZE"]) > 0)
							{
								...
								$arImage = CSocNetTools::InitImage($arFriends[$pref."_USER_PERSONAL_PHOTO"], $arParams["THUMBNAIL_LIST_SIZE"], "/thurly/images/socialnetwork/nopic_30x30.gif", 30, $pu, $canViewProfile);
							}
							else // old
								$arImage = CSocNetTools::InitImage($arFriends[$pref."_USER_PERSONAL_PHOTO"], 50, "/thurly/images/socialnetwork/nopic_user_50.gif", 50, $pu, $canViewProfile);
		...
		//otp
		if (CModule::IncludeModule("security") && Thurly\Security\Mfa\Otp::isOtpEnabled())
		{
			...
			$arResult["User"]["OTP"]["ARE_RECOVERY_CODES_ENABLED"] = Thurly\Security\Mfa\Otp::isRecoveryCodesEnabled();
