...
<?
$mailModuleInstalled = \Thurly\Main\Loader::includeModule('mail');
...
&& strlen($arResult["LOG_ENTRY"]["AVATAR_SRC"]) > 0
	...
	: '/thurly/components/thurly/socialnetwork.log.entry.mail/templates/.default/images/userpic.gif'
);
...
if (!empty($arResult["LOG_ENTRY"]["ATTACHMENTS"]))
{
	...
	)
	{
		...
		{
			$backUrl = \Thurly\Socialnetwork\ComponentHelper::getReplyToUrl($attachment["URL"], $arParams["RECIPIENT_ID"], 'ATTACHED_OBJECT', $attachment["ID"], $arResult["SITE"]["ID"], $arParams["URL"]);
			if ($backUrl)
			{
		...
		$arResult["LOG_ENTRY"]["EVENT"]["MESSAGE_FORMATTED"] = \Thurly\Socialnetwork\ComponentHelper::convertMailDiskFileBBCode($arResult["LOG_ENTRY"]["EVENT"]["MESSAGE_FORMATTED"], $arResult["LOG_ENTRY"]["ATTACHMENTS"]);
	}
}
...
if (!empty($arResult["COMMENTS"]))
{
	...
	{
		$dateCreateFormatted = \Thurly\Socialnetwork\ComponentHelper::formatDateTimeToGMT($comment['EVENT']['LOG_DATE'], $comment['EVENT']['USER_ID']);
		if (
			isset($arParams["RECIPIENT_ID"])
			&& intval($arParams["RECIPIENT_ID"]) > 0
			&& $mailModuleInstalled
		)
		{
			...
			$comment["EVENT"]["MESSAGE_FORMATTED"] = \Thurly\Socialnetwork\ComponentHelper::convertMailDiskFileBBCode($comment["EVENT"]["MESSAGE_FORMATTED"], $comment["ATTACHMENTS"]);
		}
...
{
	$uri = new \Thurly\Main\Web\Uri($arResult["LOG_ENTRY_URL"]);
