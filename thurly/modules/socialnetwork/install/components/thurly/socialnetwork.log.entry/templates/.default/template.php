...
else
{
	...
	if (!defined("SONET_LOG_JS"))
	{
		...
		$message = array(
			'sonetLEGetPath' => '/thurly/components/thurly/socialnetwork.log.entry/ajax.php',
			'sonetLESetPath' => '/thurly/components/thurly/socialnetwork.log.entry/ajax.php',
...
*/
				...
				{
					$APPLICATION->SetAdditionalCSS('/thurly/components/thurly/rating.vote/templates/like/popup.css');
					?><div class="feed-post-text-block feed-info-block"><?
						?><div class="feed-post-text-block-inner feed-post-contentview" id="<?=(!empty($arResult["CONTENT_ID"]) ? "feed-post-contentview-".htmlspecialcharsBx($arResult["CONTENT_ID"]) : "")?>" bx-content-view-xml-id="<?=htmlspecialcharsBx($arResult["CONTENT_ID"])?>"><?
						...
						if (strlen($arEvent["EVENT"]["PARAMS"]) > 0)
						{
							...
							)
							{
								...
								?><?$APPLICATION->IncludeComponent(
									"thurly:photogallery.detail.list.ex",
									"",
									Array(
				...
				) // all other events
				{
					?><div class="feed-post-text-block"><?
						...
						{
							?><div class="feed-post-text-block-inner feed-post-contentview" id="feed-post-text-contentview-<?=htmlspecialcharsBx($arResult["CONTENT_ID"])?>" bx-content-view-xml-id="<?=htmlspecialcharsBx($arResult["CONTENT_ID"])?>"><?
								?><div class="feed-post-text-block-inner-inner" id="log_entry_body_<?=$arEvent["EVENT"]["ID"]?>"><?=$arEvent["EVENT_FORMATTED"]["MESSAGE"]?></div><?
							?></div><?
						}
						else
						{
							?><div class="feed-post-text-block-inner feed-post-contentview"><?
								?><div class="feed-post-text-block-inner-inner" id="log_entry_body_<?=$arEvent["EVENT"]["ID"]?>"><?=$arEvent["EVENT_FORMATTED"]["MESSAGE"]?></div><?
							?></div><?
						}
				...
				?><div class="feed-post-informers"><?
					...
					)
					{
						...
						$APPLICATION->IncludeComponent(
							"thurly:rating.vote", $arParams["RATING_TYPE"],
							Array(
					...
					)
					{
						...
						$serverName = (CMain::IsHTTPS() ? "https" : "http")."://".Thurly\Main\Text\HtmlFilter::encode((defined("SITE_SERVER_NAME") && strlen(SITE_SERVER_NAME) > 0) ? SITE_SERVER_NAME : COption::GetOptionString("main", "server_name", ""));
					...
					?><span class="feed-post-time-wrap"><?
						...
						{
							$APPLICATION->IncludeComponent(
								"thurly:socialnetwork.contentview.count", "",
								Array(
			...
			)
			{
				...
				$arResult["OUTPUT_LIST"] = $APPLICATION->IncludeComponent(
					...
					array(
						...
						"NAV_STRING" => ($tmp ? "/thurly/components/thurly/socialnetwork.log.entry/ajax.php?".http_build_query(array(
								"action" => "get_comments",
						...
						"MODERATE_URL" => "",
						"DELETE_URL" => '/thurly/components/thurly/socialnetwork.log.entry/ajax.php?lang='.LANGUAGE_ID.'&action=delete_comment&delete_comment_id=#ID#&post_id='.$arEvent["EVENT"]["ID"].'&site='.SITE_ID,
