...
if ($arParams["StartWorkflowPermission"] == "Y")
{
	...
	$url .= (strpos($url, "?") === false ? "?" : "&").thurly_sessid_get()."&back_url=".
			...
			<th colspan="2">
				...
				<?
					...
					<span class="bizproc-document-control-first">
						<a href="<?=$APPLICATION->GetCurPageParam("id=".$arDocumentState["ID"]."&action=stop_bizproc&".thurly_sessid_get().
					...
					<?elseif ($arParams["DropWorkflowPermission"] == "Y"): 
						$tmp = true;?>
					<span class="bizproc-document-control-first">
						<a href="<?=$APPLICATION->GetCurPageParam("id=".$arDocumentState["ID"]."&action=del_bizproc&".thurly_sessid_get().
						(!empty($arParams["back_url"]) ? "&back_url=".urlencode($arParams["back_url"]) : ""), 
