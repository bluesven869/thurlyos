...
$arComponentParameters = array(
	"PARAMETERS" => array(
		...
		"PATH_TO_SMILE" => Array(
			...
			"TYPE" => "STRING",
			"MULTIPLE" => "N",
			"DEFAULT" => "/thurly/images/socialnetwork/smile/",
		),
		"PATH_TO_BLOG_SMILE" => Array(
			...
			"TYPE" => "STRING",
			"MULTIPLE" => "N",
			"DEFAULT" => "/thurly/images/blog/smile/",
		),
...
if (CModule::IncludeModule("forum"))
{
	...
	$arThemes = array();
	$dir = trim(preg_replace("'[\\\\/]+'", "/", $_SERVER['DOCUMENT_ROOT']."/thurly/components/thurly/forum/templates/.default/themes/"));
	...
	if (IsModuleInstalled("vote"))
		{
			...
			if ($arCurrentValues["SHOW_VOTE"] == "Y" && CModule::IncludeModule("vote"))
			{
				...
				if ($rVoteChannels)
				{
					\Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/components/thurly/voting.current/.parameters.php");
					...
					$arComponentParameters["PARAMETERS"]["VOTE_UNIQUE_IP_DELAY"] = array(
						...
						"TYPE" => "CUSTOM",
						"JS_FILE" => "/thurly/js/vote/comp_props.js",
...
$arSMThemes = array();
$dir = trim(preg_replace("'[\\\\/]+'", "/", $_SERVER['DOCUMENT_ROOT']."/thurly/components/thurly/socialnetwork.menu/templates/.default/themes/"));
if (is_dir($dir) && $directory = opendir($dir)):
	while (($file = readdir($directory)) !== false)
	{
		if ($file != "." && $file != ".." && is_dir($dir.$file))
