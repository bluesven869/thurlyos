...
if (
	$arResult['BLOG_POST_TASKS']
	&& \Thurly\Main\Loader::includeModule('thurlyos')
	&& !\CThurlyOSBusinessTools::isToolAvailable($USER->getId(), 'tasks')
)
{
	$arResult['BLOG_POST_TASKS'] = false;
}
...
$arBlog = \Thurly\Blog\Item\Blog::getByUser(array(
		...
		if($arPost = CBlogPost::GetByID($delId))
		{
			$perms = (
				$arPost["AUTHOR_ID"] == $user_id
					? Thurly\Blog\Item\Permissions::FULL
					: CBlogPost::GetSocNetPostPerms($_GET["delete_blog_post_id"], true)
			);

			if (
				$perms < Thurly\Blog\Item\Permissions::FULL
				&& (
			...
			if($perms >= Thurly\Blog\Item\Permissions::FULL)
			{
				...
				BXClearCache(true, CComponentEngine::MakeComponentPath("thurly:socialnetwork.blog.blog"));
...
)
{
	...
	$bAllowToAll = \Thurly\Socialnetwork\ComponentHelper::getAllowToAllDestination();
	...
	)
	{
		...
		)
		{
			...
			{
				$arBlog = \Thurly\Socialnetwork\ComponentHelper::createUserBlog(array(
	...
	else
	{
		...
		if ($_POST["save"] == "Y" && $_POST["changePostFormTab"] == "calendar" && (check_thurly_sessid() || $arParams["IS_REST"] == "Y"))
		{
		...
		if($_POST["save"] == "Y" && $_POST["changePostFormTab"] == "lists" && (check_thurly_sessid() || $arParams["IS_REST"] == "Y"))
		{
			$redirectUrl = CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_BLOG"], array("user_id" => $arBlog["OWNER_ID"]));
			LocalRedirect($redirectUrl);
		}
		...
		) // Save on button click
		{
			...
			)
			{
				if(strlen($arResult["ERROR_MESSAGE"]) <= 0)
				{
					...
					$postItem = new \Thurly\Blog\Item\Post;
					...
					$arTagInline = \Thurly\Socialnetwork\Util::detectTags($arFields, $codeList);
					...
					{
						\Thurly\Socialnetwork\ComponentHelper::processBlogPostNewMailUser($_POST, $arResult);
					...
					if(!$bError)
					{
						...
						$arFields["SEARCH_GROUP_ID"] = \Thurly\Main\Config\Option::get("socialnetwork", "userbloggroup_id", false, SITE_ID);
						...
						else
						{
							...
							if(
								...
								&& ($urlPreviewValue = \Thurly\Socialnetwork\ComponentHelper::getUrlPreviewValue($arFields["DETAIL_TEXT"]))
							)
							{
						...
						)
						{
							...
							\Thurly\Blog\Util::sendBlogPing(array(
								'siteId' => SITE_ID,
					...
					{
						$eventId = \Thurly\Blog\Integration\Socialnetwork\Log::EVENT_ID_POST;
						$arPostFields = $USER_FIELD_MANAGER->GetUserFields("BLOG_POST", $newID, LANGUAGE_ID);
						...
						{
							$eventId = \Thurly\Blog\Integration\Socialnetwork\Log::EVENT_ID_POST_IMPORTANT;
						}
						elseif (
						...
						{
							$eventId = \Thurly\Blog\Integration\Socialnetwork\Log::EVENT_ID_POST_VOTE;
						}
						elseif (
							isset($arPostFields["UF_GRATITUDE"])
						...
						{
							$eventId = \Thurly\Blog\Integration\Socialnetwork\Log::EVENT_ID_POST_GRAT;
						}
						...
						)
						{
							...
							$res = \Thurly\Socialnetwork\LogTable::getList(array(
								'filter' => array(
						...
						)
						{
							...
							if ($post = \Thurly\Blog\Item\Post::getById($newID))
							{
						...
						if($arFields["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH)
						{
							...
							\Thurly\Socialnetwork\ComponentHelper::notifyBlogPostCreated(array(
								'post' => array(
									'ID' => $newID,
		...
		if(!empty($arBlog) && ($arPost["ID"] > 0 || strlen($arResult["ERROR_MESSAGE"]) > 0))
		{
			...
			while($aImg = $res->Fetch())
			{
				...
				$aImgNew["DEL_URL"] = $APPLICATION->GetCurPageParam(
					"del_image_id=".$aImg["ID"]."&".thurly_sessid_get(),
		...
		)
		{
			...
			foreach($_REQUEST["WFILES"] as $val)
			{
				...
				if($isDiskProperty)
				{
					//@see Thurly\Disk\Uf\FileUserType::NEW_FILE_PREFIX
					$val = 'n' . $val;
				}
		...
		$arResult["urlToDelImage"] = $APPLICATION->GetCurPageParam("del_image_id=#del_image_id#&".thurly_sessid_get(), Array("sessid", "image_upload_frame", "image_upload", "do_upload","del_image_id"));
		...
		if (strLen($serverName) <=0)
		{
			$serverName = (
				...
				&& strlen (SITE_SERVER_NAME) > 0
					? SITE_SERVER_NAME
					: COption::GetOptionString("main", "server_name", "www.thurlysoft.com")
			);
	...
	if ($arResult["SHOW_FULL_FORM"])
	{
		...
		$limitReached = false;
		$arResult["PostToShow"]["FEED_DESTINATION"]['SONETGROUPS'] = \Thurly\Socialnetwork\ComponentHelper::getSonetGroupAvailable(array(
			'limit' => 100
		), $limitReached);
		...
		else
		{
			...
			{
				\Thurly\Socialnetwork\ComponentHelper::fillSelectedUsersToInvite($_POST, $arParams, $arResult);
		...
		foreach($arDepartmentCodesSelected as $selectedDepartmentCode)
		{
			...
			if (
				...
				&& \Thurly\Main\Loader::includeModule('iblock')
				&& (($structureIBlockId = \Thurly\Main\Config\Option::get('intranet', 'iblock_structure', 0)) > 0)
			)
			{
		...
		$arResult["PostToShow"]["FEED_DESTINATION"]["USERS_VACATION"] = Thurly\Socialnetwork\Integration\Intranet\Absence\User::getDayVacationList();
