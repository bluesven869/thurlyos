	...
	foreach ($arMessages as $res):
		...
		<div class="forum-info-box forum-post-vote">
			<div class="forum-info-box-inner">
				...
				?><?$GLOBALS["APPLICATION"]->IncludeComponent("thurly:voting.current", $arParams["VOTE_TEMPLATE"],
				array(
					"VOTE_ID" => $res["PARAM2"],
					...
					<div class="forum-post-entry">
						...
						if (!empty($res["FILES"]))
						{
							...
							foreach ($res["FILES"] as $arFile)
							{
								...
								{
									$arFileTemplate = $GLOBALS["APPLICATION"]->IncludeComponent("thurly:forum.interface", "show_file",
										Array(
											"FILE" => $arFile,
						...
						if (is_array($res["PROPS"])) {
							foreach ($res["PROPS"] as $arPostField)
							{
								if(!empty($arPostField["VALUE"]))
								{
									...
									?><?$GLOBALS["APPLICATION"]->IncludeComponent("thurly:system.field.view", $arPostField["USER_TYPE"]["USER_TYPE_ID"],
					...
					if ($arParams["PERMISSION_ORIGINAL"] >= "Q"):
						...
						<div class="forum-post-entry forum-user-additional forum-user-moderate-info">
								...
								<span><?=GetMessage("F_SOURCE_ID")?>: <?
									if (!empty($res["MAIL_HEADER"])):
										if ($res["PANELS"]["MAIL"] == "Y" && !empty($res["XML_ID"])):
											$res["MAIL_HEADER"] .= "<br /><a href=\"/thurly/admin/mail_message_view.php?MSG_ID=".$res["XML_ID"]."\">".GetMessage("F_ORIGINAL_MESSAGE")."</a>";
										endif;
							...
							if ($res["PANELS"]["STATISTIC"] == "Y"):
								?>
								<span><?=GetMessage("F_USER_ID")?>: <span><a href="/thurly/admin/guest_list.php?lang=<?=LANG_ADMIN_LID?><?
							...
							if ($res["PANELS"]["MAIN"] == "Y"):
								...
								<span><?=GetMessage("F_USER_ID_USER")?>: <span><?
									?><a href="/thurly/admin/user_edit.php?lang=<?=LANG_ADMIN_LID?>&amp;ID=<?=$res["AUTHOR_ID"]?>"><?=$res["AUTHOR_ID"];?></a></span></span>
								<?
							endif;
							?>
						</div>
