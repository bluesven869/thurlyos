...
else
{
	...
	/***********************  ACTIONS  *******************************/
	if ($_REQUEST["action"] == "close" && check_thurly_sessid() && IntVal($_REQUEST["eventID"]) > 0)
	{
		$errorMessage = "";
	...
	if ($_REQUEST["action"] == "delete" && check_thurly_sessid() && IntVal($_REQUEST["eventID"]) > 0)
	{
		$errorMessage = "";
	...
	if ($_REQUEST["action"] == "ban" && check_thurly_sessid() && IntVal($_REQUEST["userID"]) > 0)
	{
		$errorMessage = "";
	...
	if ($_SERVER["REQUEST_METHOD"]=="POST" && (strlen($_POST["do_read"]) > 0 || strlen($_POST["do_delete"]) > 0) && check_thurly_sessid())
	{
		$errorMessage = "";
	...
	$arResult["Urls"]["ReadAll"] = htmlspecialcharsbx($APPLICATION->GetCurUri("eventID=".$arMessages["ID"]."&action=close&".thurly_sessid_get().""));
	...
	while ($arMessages = $dbMessages->GetNext())
	{
		...
		$arImage = CSocNetTools::InitImage($arMessages["FROM_USER_PERSONAL_PHOTO"], 150, "/thurly/images/socialnetwork/nopic_user_150.gif", 150, $pu, $canViewProfile);
		...
		$arResult["Events"][] = array(
			...
			"READ_LINK" => htmlspecialcharsbx($APPLICATION->GetCurUri("eventID=".$arMessages["ID"]."&action=close&".thurly_sessid_get()."")),
			"DELETE_LINK" => htmlspecialcharsbx($APPLICATION->GetCurUri("eventID=".$arMessages["ID"]."&action=delete&".thurly_sessid_get()."")),
			"BAN_LINK" => htmlspecialcharsbx($APPLICATION->GetCurUri("userID=".$arMessages["FROM_USER_ID"]."&action=ban&".thurly_sessid_get()."")),
