		...
		else
		{
			...
			else
			{
				...
				elseif ($arResult["CurrentUserPerms"]["UserRole"] && $_REQUEST["EventType"] != "GroupRequest")
				{
					...
					if ($arResult["CurrentUserPerms"]["UserRole"] == SONET_ROLES_REQUEST)
					{
						...
						if ($arUserRequests = $dbUserRequests->GetNext())
						{
							...
							$arEventTmp["Urls"]["FriendAdd"] = htmlspecialcharsbx($APPLICATION->GetCurUri("EventType=GroupRequest&eventID=".$arUserRequests["ID"]."&action=add&".thurly_sessid_get().""));
							$arEventTmp["Urls"]["FriendReject"] = htmlspecialcharsbx($APPLICATION->GetCurUri("EventType=GroupRequest&eventID=".$arUserRequests["ID"]."&action=reject&".thurly_sessid_get().""));
				...
				else
				{
					...
					if ($_REQUEST["EventType"] == "GroupRequest" && check_thurly_sessid() && IntVal($_REQUEST["eventID"]) > 0)
					{
