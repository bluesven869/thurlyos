...
ThurlyLFFilter.prototype.initLentaMenu = function(params)
{
...
ThurlyLFFilter.prototype.showLentaMenu = function(params)
{
	var
	...
	if (
		...
		|| params.siteTemplateId != 'thurlyos'
	)
	{
	...
	this.popupMenu = BX.PopupMenu.create("lenta-sort-popup", bindElement, BX.util.array_merge((!short ? BX.util.array_merge(this.menuItems.preset, this.menuItems.filter) : []), this.menuItems.actions), {
		offsetTop: (params.siteTemplateId == 'thurlyos' ? -5 : 2),
		offsetLeft: (params.siteTemplateId == 'thurlyos' ? 17 : 43),
		angle : true,
		events : {
...
ThurlyLFFilter.prototype.initFilter = function(params)
{
	var version = (
...
ThurlyLFFilter.prototype.initDestination = function(params)
{
...
ThurlyLFFilter.prototype.clearInput = function()
{
...
ThurlyLFFilter.prototype.onSelectDestination = function(item, type, search, bUndeleted)
{
...
ThurlyLFFilter.prototype.onUnSelectDestination = function(item)
{
...
ThurlyLFFilter.prototype.ShowFilterPopup = function(bindElement)
{
...
ThurlyLFFilter.prototype.__SLFShowExpertModePopup = function(bindObj)
{
...
ThurlyLFFilter.prototype.onClickMenuItem = function(params)
{
...
ThurlyLFFilter.prototype.closeHint = function(element)
{

};

ThurlyLFFilterDestinationSelectorManager = {
	controls: {},
	...
	onSelect: function(item, type, search, bUndeleted, name, state)
	{
		...
		var control = ThurlyLFFilterDestinationSelectorManager.controls[name];
		if(control)
		{
...
ThurlyLFFilterDestinationSelector = function ()
{
	this.id = "";
	this.filterId = "";
	this.settings = {};
...
ThurlyLFFilterDestinationSelector.create = function(id, settings)
{
	var self = new ThurlyLFFilterDestinationSelector(id, settings);
	self.initialize(id, settings);
	BX.onCustomEvent(window, 'BX.Livefeed.Filter:create', [ id ]);
	return self;
};

ThurlyLFFilterDestinationSelector.prototype.getSetting = function(name, defaultval)
{
	return this.settings.hasOwnProperty(name) ? this.settings[name] : defaultval;
};

ThurlyLFFilterDestinationSelector.prototype.getSearchInput = function()
{
	return this.control ? this.control.getLabelNode() : null;
};

ThurlyLFFilterDestinationSelector.prototype.initialize = function(id, settings)
{
	this.id = id;
...
ThurlyLFFilterDestinationSelector.prototype.open = function()
{
	var name = this.id;
...
ThurlyLFFilterDestinationSelector.prototype.close = function()
{
	BX.SocNetLogDestination.closeDialog();
	this.opened = false;
	this.closed = true;
...
ThurlyLFFilterDestinationSelector.prototype.onCustomEntitySelectorOpen = function(control)
{
	...
	else
	{
		...
		ThurlyLFFilterDestinationSelectorManager.controls[this.id] = this.control;
...
ThurlyLFFilterDestinationSelector.prototype.onCustomEntitySelectorClose = function(control)
{
	if(
...
ThurlyLFFilterDestinationSelector.prototype.onGetStopBlur = function(event, result)
{
...
ThurlyLFFilterDestinationSelector.prototype.onCustomEntityRemove = function(control)
{
...
ThurlyLFFilterDestinationSelector.prototype.onBeforeSwitchTabFocus = function(ob)
{
	if(this.id === ob.id)
	{
		ob.blockFocus = true;
...
ThurlyLFFilterDestinationSelector.prototype.onBeforeSelectItemFocus = function(ob)
{
	if(this.id === ob.id)
	{
		ob.blockFocus = true;
...
ThurlyLFFilterDestinationSelector.prototype.onBeforeInitDialog = function(params)
{
	if (
		typeof params.id == 'undefined'
		|| params.id != this.id
