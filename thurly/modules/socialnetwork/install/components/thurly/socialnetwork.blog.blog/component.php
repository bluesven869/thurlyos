...
$cache_path = CComponentEngine::MakeComponentPath("thurly:socialnetwork.blog.blog");
...
if(is_array($_REQUEST["options"]) && !empty($_REQUEST["options"]) &&
	check_thurly_sessid() && $USER->IsAuthorized() && CModule::IncludeModule("blog"))
{
	...
	foreach ($arPostsAll as $arPost)
	{
		...
		if($arPost["perms"] >= BLOG_PERMS_WRITE)
		{
			...
			if($arPost["perms"] >= BLOG_PERMS_MODERATE)
				$arPost["urlToHide"] = htmlspecialcharsex($APPLICATION->GetCurPageParam("hide_id=".$arPost["ID"]."&".thurly_sessid_get(), Array("del_id", "sessid", "success", "hide_id")));
			if($arPost["perms"] >= BLOG_PERMS_FULL)
				$arPost["urlToDelete"] = htmlspecialcharsex($APPLICATION->GetCurPageParam("del_id=".$arPost["ID"]."&".thurly_sessid_get(), Array("del_id", "sessid", "success", "hide_id")));
		}
