...
if ($arParams["IS_CRM"] !== "Y")
{
	...
	if (
		SITE_TEMPLATE_ID == "thurlyos"
		&& (
			(
				ModuleManager::isModuleInstalled('thurlyos')
				&& \CThurlyOS::isPortalAdmin($arResult["currentUserId"])
			)
			|| (
				!ModuleManager::isModuleInstalled('thurlyos')
				&& $USER->IsAdmin()
	...
	{
		\Thurly\Socialnetwork\ComponentHelper::getLivefeedStepper();
	}
}
...
{
	$APPLICATION->AddHeadScript("/thurly/components/thurly/socialnetwork.log.entry/templates/.default/scripts.js");
	...
	if ($arParams["HIDE_EDIT_FORM"] != "Y" && IntVal($arResult["MICROBLOG_USER_ID"]) > 0 && $USER->IsAuthorized())
	{
		...
		if (defined("THURLY24_INDEX_COMPOSITE"))
		{
		...
		$staticHtmlCache = \Thurly\Main\Data\StaticHtmlCache::getInstance();
		...
		$APPLICATION->IncludeComponent(
			"thurly:socialnetwork.blog.post.edit",
			"",
	...
	if ($arParams["SHOW_EVENT_ID_FILTER"] == "Y")
	{
		...
		$APPLICATION->IncludeComponent(
			"thurly:socialnetwork.log.filter",
	...
	if (defined("THURLY24_INDEX_COMPOSITE"))
	{
		$dynamicArea = new \Thurly\Main\Page\FrameStatic("live-feed");
...
?><script>
	...
	if (!$arResult["AJAX_CALL"])
	{
		?>
		BX.message({
			sonetLGetPath: '<?=CUtil::JSEscape('/thurly/components/thurly/socialnetwork.log.ex/ajax.php')?>',
			sonetLSetPath: '<?=CUtil::JSEscape('/thurly/components/thurly/socialnetwork.log.ex/ajax.php')?>',
			sonetLESetPath: '<?=CUtil::JSEscape('/thurly/components/thurly/socialnetwork.log.entry/ajax.php')?>',
			sonetLEPath: '<?=CUtil::JSEscape($arParams["PATH_TO_LOG_ENTRY"])?>',
			sonetLSessid: '<?=thurly_sessid_get()?>',
			...
			sonetLIsB24: '<?=(SITE_TEMPLATE_ID == "thurlyos" ? "Y" : "N")?>',
			...
			sonetLFCreateTaskTaskPath: '<?=CUtil::JSEscape(\Thurly\Main\Config\Option::get('socialnetwork', 'user_page', SITE_DIR.'company/personal/'))?>user/#user_id#/tasks/task/view/#task_id#/',
	...
	if ($arResult["AJAX_CALL"] && $arParams["SHOW_RATING"] == "Y")
	{
		...
		{
			?>
			BX.loadCSS('/thurly/components/thurly/rating.vote/templates/<?=$arParams["RATING_TYPE"]?>/popup.css');
			<?
		}
		?>
		BX.loadCSS('/thurly/components/thurly/rating.vote/templates/<?=$arParams["RATING_TYPE"]?>/style.css');
		<?
	}
	...
	foreach ($arResult["Events"] as $arEvent)
	{
		if (!empty($arEvent))
		{
			...
			else
			{
				...
				$APPLICATION->IncludeComponent(
					"thurly:socialnetwork.log.entry",
					"",
					$arComponentParams,
					$component
				);
	...
	BX.ready(function(){
		...
		if (!!window["FCForm"])
		{
			...
			window["SLEC"] = {
				...
				actionUrl : '/thurly/urlrewrite.php?SEF_APPLICATION_CUR_PAGE_URL=<?=str_replace("%23", "#", urlencode($arResult["urlToPost"]))?>',
