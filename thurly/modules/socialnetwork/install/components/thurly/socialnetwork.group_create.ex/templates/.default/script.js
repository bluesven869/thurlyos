			...
			document.forms.sonet_group_create_popup_form,
			{
				...
				onsuccess: function(obResponsedata)
				{
					...
					else if (obResponsedata["MESSAGE"] == 'SUCCESS')
					{
						...
						{
							window.top.BX.onCustomEvent("BX.ThurlyOS.PageSlider:close", [false]);
...
BX.BXGCE.init = function(params) {
	...
	if (
		typeof top.BX.ThurlyOS != 'undefined'
		&& top.BX.ThurlyOS.Slider.getCurrentPage()
	)
	{
		BX.addCustomEvent(
			top.BX.ThurlyOS.Slider.getCurrentPage().getWindow(),
			"BX.ThurlyOS.PageSlider:onClose",
			function (slider)
			{
				setTimeout(function(){ top.BX.ThurlyOS.Slider.destroy(slider.getUrl()) }, 500);
			}
		);
	}
