...
use Thurly\Socialnetwork\UserToGroupTable;
use Thurly\Socialnetwork\Item\UserToGroup;
use Thurly\Main\ModuleManager;
use Thurly\Main\Loader;
use Thurly\Main\Config\Option;
use Thurly\Socialnetwork\Item\WorkgroupTemplate;
use Thurly\Main\Localization\Loc;

require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/components/thurly/socialnetwork.group_create.ex/include.php");
...
else
{
	...
	else
	{
		...
		$arResult["POST"]["IMAGE_ID_IMG"] = '<img src="/thurly/images/1.gif" height="60" class="sonet-group-create-popup-image" id="sonet_group_create_popup_image" border="0">';
	}
	...
	$arResult['Types'] = (
		$arResult["USE_PRESETS"] == 'Y'
			? \Thurly\Socialnetwork\Item\Workgroup::getTypes(array(
	...
	if (StrLen($arResult["FatalError"]) <= 0)
	{
		...
		)
		{
			...
			)
			{
				...
				)
				{
					if (
						...
						&& in_array($_POST['GROUP_IMAGE_ID'], \Thurly\Main\UI\FileInputUtility::instance()->checkFiles('GROUP_IMAGE_ID', array($_POST['GROUP_IMAGE_ID'])))
					)
					{
			...
			)
			{
				...
				else
				{
					...
					else
					{
						...
						if (!empty($plusList))
						{
							$memberRelationIdList = $otherRelationIdList = array();
							...
							{
								CSocNetUserToGroup::TransferMember2Moderator($arResult["currentUserId"], $arParams["GROUP_ID"], $memberRelationIdList, CSocNetUser::IsCurrentUserModuleAdmin());
							}

							foreach($moderatorIdList as $moderatorId)
							{
								if (!array_key_exists($moderatorId, $memberRelationIdList))
								{
									if (array_key_exists($moderatorId, $otherRelationIdList))
									{
										CSocNetUserToGroup::update($otherRelationIdList[$moderatorId], array(
						...
						{
							BXClearCache(true, "/".$arSite["ID"]."/thurly/search.tags.cloud/");
						}
					}
				}
			}
			...
			elseif ($arResult["GROUP_ID"] > 0)
			{
				...
				)
				{
					...
					$typeCode = \Thurly\Socialnetwork\Item\Workgroup::getTypeCodeByParams(array(
				...
				)
				{
					...
					{
						$externalAuthIdList = \Thurly\Socialnetwork\ComponentHelper::checkPredefinedAuthIdList(array('bot', 'imconnector', 'replica'));
		...
		if ($arResult["ShowForm"] == "Input")
		{
			if (!array_key_exists("TAB", $arResult) || $arResult["TAB"] == "edit")
			{
				...
				$arResult["InitiatePermsProject"] = array(
					UserToGroupTable::ROLE_OWNER => GetMessage("SONET_GCE_IP_OWNER_PROJECT"),
					UserToGroupTable::ROLE_MODERATOR => GetMessage("SONET_GCE_IP_MOD_PROJECT"),
					UserToGroupTable::ROLE_USER => GetMessage("SONET_GCE_IP_USER_PROJECT"),
				);
