...
$arResult['CONTEXT'] = \Thurly\Socialnetwork\ComponentHelper::getUrlContext();
$arParams['PATH_TO_USER'] = \Thurly\Socialnetwork\ComponentHelper::addContextToUrl($arParams['PATH_TO_USER'], $arResult["CONTEXT"]);
...
$CurrentUserPerms = CSocNetUserPerms::InitUserPerms(
	$USER->GetID(),
	$arParams["ID"],
	CSocNetUser::IsCurrentUserModuleAdmin(SITE_ID, (CModule::IncludeModule("thurlyos") && CThurlyOS::IsPortalAdmin($USER->GetID()) ? false : true))
);

$dbUser = CUser::GetByID($arParams["ID"]);
$arResult["User"] = $dbUser->GetNext();

if (in_array($arResult["User"]["EXTERNAL_AUTH_ID"], \Thurly\Socialnetwork\ComponentHelper::checkPredefinedAuthIdList(array('bot', 'imconnector'))))
{
...
if (in_array($arResult['User']['EXTERNAL_AUTH_ID'], \Thurly\Socialnetwork\ComponentHelper::checkPredefinedAuthIdList('email')))
{
...
else
{
	...
	if($arResult['bEdit'] == 'Y' && $_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["submit"])>0 && check_thurly_sessid())
	{
		if(CModule::IncludeModule("socialservices"))
		{
			$arPerm = array();
		...
		if (
			...
			&& in_array($_POST['PERSONAL_PHOTO_ID'], \Thurly\Main\UI\FileInputUtility::instance()->checkFiles(
		...
		if (
			...
			&& in_array($_POST['WORK_LOGO_ID'], \Thurly\Main\UI\FileInputUtility::instance()->checkFiles(
				'WORK_LOGO_IMAGE_ID',
		...
		foreach ($arFields as $key)
		{
			...
			elseif ('GROUP_ID' == $key)
			{
				...
				if (
					...
					&& !CThurlyOS::isMoreAdminAvailable()
				)
				{
		...
		else
		{
			...
			if(IsModuleInstalled("thurlyos") && isset($arFieldsValue["GROUP_ID"]) && defined("BX_COMP_MANAGED_CACHE"))
			{
	...
	if($arResult['bEdit'] == 'Y' && $_SERVER["REQUEST_METHOD"]=="POST" && (strlen($_POST["submit_fire"])>0 || strlen($_POST["submit_recover"])>0) && check_thurly_sessid())
	{
