...
if (
	...
	&& !in_array($arResult["User"]["EXTERNAL_AUTH_ID"], \Thurly\Socialnetwork\ComponentHelper::checkPredefinedAuthIdList(array('bot', 'email', 'imconnector')))
)
				...
				case 'SOCSERVICES':
					if(!empty($arResult["arSocServ"])):
						...
						<div class="bx-sonet-profile-field-socserv">
							<?
							$APPLICATION->IncludeComponent("thurly:socserv.auth.split", "twitpost", array(
									"SHOW_PROFILES" => "Y",
									"ALLOW_DELETE" => "Y",
									"USER_ID" => $arParams['ID'],
								),
