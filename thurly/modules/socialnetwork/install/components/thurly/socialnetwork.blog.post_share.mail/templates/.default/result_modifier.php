...
foreach($arResult["AUTHORS"] as $authorId => $arAuthor)
{
	$arAuthor["AVATAR_URL"] = (
		...
		&& strlen($arAuthor["PERSONAL_PHOTO_resized"]["src"]) > 0
			...
			: '/thurly/components/thurly/socialnetwork.blog.post_share.mail/templates/.default/images/userpic.gif'
	);
...
)
{
	if (!empty($arResult["POST"]["ATTACHMENTS"]))
	{
		...
		{
			$backUrl = \Thurly\Socialnetwork\ComponentHelper::getReplyToUrl($attachment["URL"], $arParams["RECIPIENT_ID"], 'ATTACHED_OBJECT', $attachment["ID"], $arResult["POST"]["BLOG_GROUP_SITE_ID"], $arParams["URL"]);
			if ($backUrl)
			{
		...
		$arResult["POST"]["DETAIL_TEXT_FORMATTED"] = \Thurly\Socialnetwork\ComponentHelper::convertMailDiskFileBBCode($arResult["POST"]["DETAIL_TEXT_FORMATTED"], $arResult["POST"]["ATTACHMENTS"]);
	}
	...
	if (!empty($arResult["COMMENTS"]))
	{
		foreach ($arResult["COMMENTS"] as $commentKey => $arComment)
		{
			if (!empty($arComment["ATTACHMENTS"]))
			{
				...
				$arResult["COMMENTS"][$commentKey]["POST_TEXT_FORMATTED"] = \Thurly\Socialnetwork\ComponentHelper::convertMailDiskFileBBCode($arResult["COMMENTS"][$commentKey]["POST_TEXT_FORMATTED"], $arResult["COMMENTS"][$commentKey]["ATTACHMENTS"]);
			}
		}
	}
}
