...
else
{
	if (!$arResult["AJAX_CALL"])
	{
		...
		$APPLICATION->IncludeComponent(
			"thurly:socialnetwork.group.iframe.popup",
			".default",
			array(
		...
		)
		{
			...
			{
				$APPLICATION->IncludeComponent(
					"thurly:socialnetwork.user_groups.link.add",
					".default",
					array(
		...
		?><script>
			BX.ready(function() {
				oSUG.init({
					keyboardPageNavigation: <?=(SITE_TEMPLATE_ID == 'thurlyos' ? 'true' : 'false')?>,
	...
	?><div id="sonet-groups-content-wrap" class="sonet-groups-content-wrap<?=(!$notEmptyList ? " no-groups" : "")?>">
		<div class="sonet-groups-content-sort-container"><?
			...
			?><span class="sonet-groups-search"><?
				$APPLICATION->IncludeComponent(
					"thurly:socialnetwork.user_groups.search_form",
					".default",
					array(),
					null,
					array("HIDE_ICONS" => "Y")
		...
		if (
			...
			&& (
				(
					...
					&& \CThurlyOS::isPortalAdmin($USER->getId())
				)
				|| (
					!ModuleManager::isModuleInstalled('thurlyos')
					&& $USER->isAdmin()
		...
		)
		{
			echo Stepper::getHtml(array(
				'socialnetwork' => array('Thurly\Socialnetwork\Update\WorkgroupIndex')
							...
							?><span class="sonet-groups-group-text"><?
								?><span class="sonet-groups-group-title<?=($group["IS_EXTRANET"] == "Y" ? " sonet-groups-group-title-extranet" : "")?>"><?
									?><span class="sonet-groups-group-title-text"><?
										...
										?><?=($group["IS_EXTRANET"] == "Y" && SITE_TEMPLATE_ID != "thurlyos" ? '<span class="sonet-groups-group-signature">'.GetMessage("SONET_C33_T_IS_EXTRANET").'</span>' : '')?><?
									?></span><?
								...
								)
								{
									?><span class="sonet-groups-group-btn-container"><?
										...
										$requestSent = (isset($group['ROLE']) && $group['ROLE'] == \Thurly\Socialnetwork\UserToGroupTable::ROLE_REQUEST);
