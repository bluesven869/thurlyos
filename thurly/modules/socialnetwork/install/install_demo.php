...
if (!function_exists("GetSocNetMessageLocal"))
{
	function GetSocNetMessageLocal($message, $lang)
	{
		...
		{
			$langFile = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/lang/".$lang."/install/install.php";
			if (!file_exists($langFile))
			{
				$langFile = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/lang/en/install/install.php";
				if (!file_exists($langFile))
					$langFile = "";
			}
...
if (CModule::IncludeModule("socialnetwork"))
{
	...
	if (IntVal($cnt) <= 0)
	{
		...
		if (!$bSonetError)
		{
			$pathToImages = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/public/images/";
...
if (StrLen($installPath) > 0)
{
	...
	CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/public/".$folder."/".$lng, $arSite['ABS_DOC_ROOT'].$arSite["DIR"].$installPath, $installRewrite, true);
