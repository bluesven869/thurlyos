...
Class socialnetwork extends CModule
{
	...
	function InstallDB($install_wizard = true)
	{
		...
		{
			$errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/db/".$DBType."/install.sql");
		}
		...
		RegisterModuleDependences("main", "OnAfterUserAdd", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\UserToGroup", "onAfterUserAdd");
		RegisterModuleDependences("main", "OnAfterUserUpdate", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\UserToGroup", "onAfterUserUpdate");
		RegisterModuleDependences("iblock", "OnBeforeIBlockSectionUpdate", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\Workgroup", "onBeforeIBlockSectionUpdate");
		RegisterModuleDependences("iblock", "OnAfterIBlockSectionUpdate", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\Workgroup", "onAfterIBlockSectionUpdate");
		RegisterModuleDependences("iblock", "onBeforeIBlockSectionDelete", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\Workgroup", "onBeforeIBlockSectionDelete");
		RegisterModuleDependences("iblock", "OnAfterIBlockSectionDelete", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\Workgroup", "onAfterIBlockSectionDelete");

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->registerEventHandler('mail', 'onReplyReceivedLOG_ENTRY', 'socialnetwork', '\Thurly\Socialnetwork\Internals\MailHandler', 'handleReplyReceivedLogEntry');
		$eventManager->registerEventHandler('main', 'OnUISelectorActionProcessAjax', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Main\UISelector\Handler', 'OnUISelectorActionProcessAjax');
		$eventManager->registerEventHandler('main', 'OnUISelectorEntitiesGetList', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Main\UISelector\Handler', 'OnUISelectorEntitiesGetList');
		$eventManager->registerEventHandler('tasks', 'onTaskUpdateViewed', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Tasks\Task', 'onTaskUpdateViewed');
		$eventManager->registerEventHandler('calendar', 'onViewEvent', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Calendar\CalendarEvent', 'onViewEvent');
		$eventManager->registerEventHandler('main', 'onRatingListViewed', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Main\RatingVoteList', 'onViewed');
		$eventManager->registerEventHandler('mobile', 'onSetContentView', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Mobile\LogEntry', 'onSetContentView');
		$eventManager->registerEventHandler('mobile', 'onGetContentId', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Mobile\LogEntry', 'onGetContentId');
		$eventManager->registerEventHandler('pull', 'onGetMobileCounter', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Pull\Counter', 'onGetMobileCounter');
		$eventManager->registerEventHandler('pull', 'onGetMobileCounterTypes', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Pull\Counter', 'onGetMobileCounterTypes');
		$eventManager->registerEventHandler('intranet', 'onEmployeeDepartmentsChanged', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Intranet\Structure\Employee', 'onEmployeeDepartmentsChanged');
		...
		if(strtolower($DB->type) == 'mysql')
		{
			...
			{
				\Thurly\Socialnetwork\LogIndexTable::getEntity()->enableFullTextIndex("CONTENT");
			}

			if ($DB->Query("CREATE fulltext index IXF_SONET_GROUP on b_sonet_group (SEARCH_INDEX)", true))
			{
				\Thurly\Socialnetwork\WorkgroupTable::getEntity()->enableFullTextIndex("SEARCH_INDEX");
			}
		}
		...
		if($install_smiles == "Y" || $install_wizard)
		{
			...
			if(!($dbSmile->Fetch()))
			{
				...
				{
					IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/smiles.php", $arLangs["LID"]);
	...
	function UnInstallDB($arParams = Array())
	{
		...
		if(array_key_exists("savedata", $arParams) && $arParams["savedata"] != "Y")
		{
			...
			$errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/db/".$DBType."/uninstall.sql");
		...
		UnRegisterModuleDependences("main", "OnAfterUserAdd", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\UserToGroup", "onAfterUserAdd");
		UnRegisterModuleDependences("main", "OnAfterUserUpdate", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\UserToGroup", "onAfterUserUpdate");
		UnRegisterModuleDependences("iblock", "OnBeforeIBlockSectionUpdate", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\Workgroup", "OnBeforeIBlockSectionUpdate");
		UnRegisterModuleDependences("iblock", "OnAfterIBlockSectionUpdate", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\Workgroup", "onAfterIBlockSectionUpdate");
		UnRegisterModuleDependences("iblock", "onBeforeIBlockSectionDelete", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\Workgroup", "onBeforeIBlockSectionDelete");
		UnRegisterModuleDependences("iblock", "OnAfterIBlockSectionDelete", "socialnetwork", "\\Thurly\\Socialnetwork\\Item\\Workgroup", "onAfterIBlockSectionDelete");

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->unregisterEventHandler('mail', 'onReplyReceivedLOG_ENTRY', 'socialnetwork', '\Thurly\Socialnetwork\Internals\MailHandler', 'handleReplyReceivedLogEntry');
		$eventManager->unregisterEventHandler('main', 'OnUISelectorActionProcessAjax', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Main\UISelector\Handler', 'OnUISelectorActionProcessAjax');
		$eventManager->unregisterEventHandler('main', 'OnUISelectorEntitiesGetList', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Main\UISelector\Handler', 'OnUISelectorEntitiesGetList');
		$eventManager->unregisterEventHandler('tasks', 'onTaskUpdateViewed', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Tasks\Task', 'onTaskUpdateViewed');
		$eventManager->unregisterEventHandler('calendar', 'onViewEvent', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Calendar\CalendarEvent', 'onViewEvent');
		$eventManager->unregisterEventHandler('main', 'onRatingListViewed', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Main\RatingVoteList', 'onViewed');
		$eventManager->unregisterEventHandler('mobile', 'onSetContentView', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Mobile\LogEntry', 'onSetContentView');
		$eventManager->unregisterEventHandler('mobile', 'onGetContentId', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Mobile\LogEntry', 'onGetContentId');
		$eventManager->unregisterEventHandler('pull', 'onGetMobileCounter', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Pull\Counter', 'onGetMobileCounter');
		$eventManager->unregisterEventHandler('pull', 'onGetMobileCounterTypes', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Pull\Counter', 'onGetMobileCounterTypes');
		$eventManager->unregisterEventHandler('intranet', 'onEmployeeDepartmentsChanged', 'socialnetwork', '\Thurly\Socialnetwork\Integration\Intranet\Structure\Employee', 'onEmployeeDepartmentsChanged');
		...
		if (!!$id)
		{
			...
			if ($id != "webdav")
			{
				...
				if (IsModuleInstalled("blog"))
				{
					...
					{
						$messages = IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/index.php", $arLang["LID"], true);
	...
	function InstallEvents()
	{
		...
		if($ar["C"] <= 0)
		{
			...
			{
				if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/intranet"))
					$pathInMessage = "/company/personal/";
				else
					$pathInMessage = "/club/";
			}
			include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/events/set_events.php");
		}
		return true;
	}
	...
	{
		global $DB;
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/events/del_events.php");
		return true;
	}
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin", true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/images",  $_SERVER["DOCUMENT_ROOT"]."/thurly/images/socialnetwork", true, True);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/sounds",  $_SERVER["DOCUMENT_ROOT"]."/thurly/sounds", true, True);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/themes", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/gadgets", $_SERVER["DOCUMENT_ROOT"]."/thurly/gadgets", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/tools", $_SERVER["DOCUMENT_ROOT"]."/thurly/tools", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/js", $_SERVER["DOCUMENT_ROOT"]."/thurly/js", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/activities", $_SERVER["DOCUMENT_ROOT"]."/thurly/activities", true, true);
		}
		return true;
	}
	...
	function InstallPublic()
	{
		...
		foreach ($arSites as $site)
		{
			...
			include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/install_demo.php");
		}
	}

	function UnInstallFiles()
	{
		DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
		DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/themes/.default/", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes/.default");//css
		DeleteDirFilesEx("/thurly/themes/.default/icons/socialnetwork/");//icons
		DeleteDirFilesEx("/thurly/images/socialnetwork/");//images
		DeleteDirFilesEx("/thurly/sounds/socialnetwork/");//sounds
	...
	function DoInstall()
	{
		...
		$step = IntVal($step);
		if ($step < 2)
			$APPLICATION->IncludeAdminFile(GetMessage("SONET_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/step1.php");
		elseif($step==2)
		{
			$this->InstallFiles();
			...
			$APPLICATION->IncludeAdminFile(GetMessage("SONET_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/step2.php");
		}
	}
	...
	function DoUninstall()
	{
		...
		$step = IntVal($step);
		if($step<2)
			$APPLICATION->IncludeAdminFile(GetMessage("SONET_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/unstep1.php");
		elseif($step==2)
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("SONET_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/install/unstep2.php");
		}
	}
