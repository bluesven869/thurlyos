...
class LogTagTable extends Entity\DataManager
{
	...
	public static function deleteByLogId($params = array())
	{
		...
		\Thurly\Main\Application::getConnection()->queryExecute('DELETE FROM '.self::getTableName().' WHERE LOG_ID = '.intval($params['logId']));
		return true;
	}
	...
	public static function deleteByItem($params = array())
	{
		...
		\Thurly\Main\Application::getConnection()->queryExecute('DELETE FROM '.self::getTableName().' WHERE ITEM_TYPE = "'.$params['itemType'].'" AND ITEM_ID = '.intval($params['itemId']));
		return true;
	}
