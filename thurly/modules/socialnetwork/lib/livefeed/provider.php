<?php
namespace Thurly\Socialnetwork\Livefeed;

use Thurly\Main;
use Thurly\Main\Loader;
use Thurly\Main\Localization\Loc;
use Thurly\Socialnetwork\Item\Subscription;
use Thurly\Socialnetwork\UserContentViewTable;
use Thurly\Socialnetwork\Item\UserContentView;
...
abstract class Provider
{
	...
	final private static function getProvider($entityType)
	{
		switch ($entityType)
		{
			case self::DATA_ENTITY_TYPE_BLOG_POST:
				$provider = new \Thurly\Socialnetwork\Livefeed\BlogPost();
				break;
			case self::DATA_ENTITY_TYPE_BLOG_COMMENT:
				$provider = new \Thurly\Socialnetwork\Livefeed\BlogComment();
				break;
			case self::DATA_ENTITY_TYPE_TASKS_TASK:
				$provider = new \Thurly\Socialnetwork\Livefeed\TasksTask();
				break;
			case self::DATA_ENTITY_TYPE_FORUM_POST:
				$provider = new \Thurly\Socialnetwork\Livefeed\ForumPost();
				break;
			case self::DATA_ENTITY_TYPE_CALENDAR_EVENT:
				$provider = new \Thurly\Socialnetwork\Livefeed\CalendarEvent();
				break;
			case self::DATA_ENTITY_TYPE_LOG_ENTRY:
				$provider = new \Thurly\Socialnetwork\Livefeed\LogEvent();
				break;
			case self::DATA_ENTITY_TYPE_LOG_COMMENT:
				$provider = new \Thurly\Socialnetwork\Livefeed\LogComment();
				break;
			case self::DATA_ENTITY_TYPE_RATING_LIST:
				$provider = new \Thurly\Socialnetwork\Livefeed\RatingVoteList();
				break;
			case self::DATA_ENTITY_TYPE_PHOTOGALLERY_ALBUM:
				$provider = new \Thurly\Socialnetwork\Livefeed\PhotogalleryAlbum();
				break;
			case self::DATA_ENTITY_TYPE_PHOTOGALLERY_PHOTO:
				$provider = new \Thurly\Socialnetwork\Livefeed\PhotogalleryPhoto();
				break;
			case self::DATA_ENTITY_TYPE_LISTS_ITEM:
				$provider = new \Thurly\Socialnetwork\Livefeed\ListsItem();
				break;
			default:
				$provider = false;
		}
	...
	protected static function cloneUfValues(array $values)
	{
		...
		{
			$result = \Thurly\Disk\Driver::getInstance()->getUserFieldManager()->cloneUfValuesFromAttachedObject($values, $USER->getId());
		}
	...
	public function getDiskObjects($entityId, $clone = false)
	{
		...
		if ($clone)
		{
			...
			)
			{
				foreach($result as $clonedDiskObjectId)
				{
					...
					{
						$attachedObject = \Thurly\Disk\AttachedObject::loadById($attachedDiskObjectId);
						if ($attachedObject)
						{
							$this->diskObjectsCloned[\Thurly\Disk\Uf\FileUserType::NEW_FILE_PREFIX.$attachedObject->getObjectId()] = $this->attachedDiskObjectsCloned[$attachedDiskObjectId];
						}
					}
				}
			}
		...
		else
		{
			...
			)
			{
				...
				{
					$attachedObject = \Thurly\Disk\AttachedObject::loadById($attachedObjectId);
					if ($attachedObject)
					{
						$result[] = \Thurly\Disk\Uf\FileUserType::NEW_FILE_PREFIX.$attachedObject->getObjectId();
					}
				}
			}
		}
