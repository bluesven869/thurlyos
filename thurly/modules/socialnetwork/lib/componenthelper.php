...
use Thurly\Socialnetwork\CommentAux\CreateTask;
use Thurly\Main\Update\Stepper;
use Thurly\Socialnetwork\Item\Log;
use Thurly\Socialnetwork\Livefeed\BlogPost;
	...
	*/
	public static function getBlogCommentListData($postId, $params, $languageId, &$authorIdList = array())
	{
		...
		else
		{
			...
			if ($connection instanceof \Thurly\Main\DB\MysqlCommonConnection)
			{
...
//					"SHARE_DEST" => false
	...
	*/
	private static function processCommentData(&$comment, $languageId, $p, $params = array())
	{
		...
		if ($commentAuxProvider = \Thurly\Socialnetwork\CommentAux\Base::findProvider(
			$comment,
			array(
	...
	*/
	public static function getReplyToUrl($url, $userId, $entityType, $entityId, $siteId, $backUrl = null)
	{
		...
		{
			$urlRes = \Thurly\Mail\User::getReplyTo(
				$siteId,
				intval($userId),
				$entityType,
				$entityId,
	...
	*/
	public static function getAttachmentsData($valueList, $siteId = false)
	{
		...
		$driver = \Thurly\Disk\Driver::getInstance();
		$urlManager = $driver->getUrlManager();

		foreach ($valueList as $key => $value)
		{
			$attachedObject = \Thurly\Disk\AttachedObject::loadById($value, array('OBJECT'));
			if(
				!$attachedObject
			...
			$result[$value] = array(
				...
				"IS_IMAGE" => \Thurly\Disk\TypeFile::isImage($attachedObject->getFile())
			);
		}
	...
	*/
	public static function getAttachmentUrlList($valueList = array(), $entityType = '', $entityId = 0, $authorId = 0, $attachmentList = array())
	{
		...
		$userFieldManager = \Thurly\Disk\Driver::getInstance()->getUserFieldManager();
		...
		foreach($valueList as $value)
		{
			...
			list($type, $realValue) = \Thurly\Disk\Uf\FileUserType::detectType($value);
			if ($type == \Thurly\Disk\Uf\FileUserType::TYPE_NEW_OBJECT)
			{
				$attachedObject = \Thurly\Disk\AttachedObject::load(array(
			...
			)
			{
				...
				else
				{
					if (!$attachedObject)
					{
						$attachedObject = \Thurly\Disk\AttachedObject::loadById($attachedFileId, array('OBJECT'));
					}
					...
					if ($attachedObject)
					{
						...
						{
							/** @var \Thurly\Disk\ExternalLink $externalLink */
						...
						if ($externalLink)
						{
							...
							$result[$value] = array(
								'TYPE' => 'image',
								'URL' => \Thurly\Disk\Driver::getInstance()->getUrlManager()->getUrlExternalLink(
									array(
	...
	*/
	public static function formatDateTimeToGMT($dateTimeSource, $authorId)
	{
		...
		if (!empty($dateTimeSource))
		{
			...
			$result = \FormatDate(
				preg_replace('/[\/.,\s:][s]/', '', \Thurly\Main\Type\Date::convertFormatToPhp(FORMAT_DATETIME)),
	...
	*/
	public static function createUserBlog($params)
	{
		...
		$res = \Thurly\Main\UserTable::getList(array(
			'order' => array(),
			'filter' => array(
				"ID" => $params["USER_ID"]
			),
	...
	*/
	public static function getUrlPreviewValue($text, $html = true)
	{
		...
		{
			$url = (
				!\Thurly\Main\Application::isUtfMode()
					? \Thurly\Main\Text\Encoding::convertEncoding($res[1][0], 'UTF-8', \Thurly\Main\Context::getCurrent()->getCulture()->getCharset())
					: $res[1][0]
			);

			$metaData = \Thurly\Main\UrlPreview\UrlPreview::getMetadataAndHtmlByUrl($url, true, false);
			if (
				!empty($metaData)
				&& !empty($metaData["ID"])
			...
			)
			{
				...
				$value = $signer->sign($metaData["ID"].'', \Thurly\Main\UrlPreview\UrlPreview::SIGN_SALT);
			}
		}
	...
	*/
	...
	public static function getEmailUserIdList()
	{
		...
		$cache = new \CPHPCache;
		$cacheDir = '/thurly/sonet/user_email';
	...
	public static function getExtranetSonetGroupIdList()
	{
		...
		$cache = new \CPHPCache;
		$cacheDir = '/thurly/sonet/group_ex';
	...
	public static function processBlogPostShare($fields, $params)
	{
		...
		if (Loader::includeModule('blog'))
		{
			...
			if(\CBlogPost::update($postId, array("SOCNET_RIGHTS" => $sonetRights, "HAS_SOCNET_ALL" => "N")))
			{
				...
				\Thurly\Blog\Broadcast::send(array(
				...
				\Thurly\Main\FinderDestTable::merge(array(
					"CONTEXT" => "blog_post",
					"CODE" => \Thurly\Main\FinderDestTable::convertRights($newRights)
				));
				...
				{
					$provider = \Thurly\Socialnetwork\CommentAux\Base::init(\Thurly\Socialnetwork\CommentAux\Share::getType(), array(
	...
	public static function processBlogCreateTask($params)
	{
		...
		if ($task = \Thurly\Tasks\Manager\Task::get($USER->getId(), $taskId))
		{
			$task = $task['DATA'];
		}
		...
		if (
			...
			|| !\Thurly\Socialnetwork\Livefeed\BlogPost::canRead(array(
				'POST' => $post
			))
		)
		{
		...
		if ($logId > 0)
		{
			...
			)
			{
				...
				$provider = \Thurly\Socialnetwork\CommentAux\Base::init(\Thurly\Socialnetwork\CommentAux\CreateTask::getType(), array(
					'liveParamList' => $auxLiveParamList
				));
	...
	public static function getSonetGroupAvailable($params = array(), &$limitReached = false)
	{
		...
		$currentCache = \Thurly\Main\Data\Cache::createInstance();
		...
		)
		{
			...
			$res = $APPLICATION->includeComponent(
				...
				array(
					...
					"MODERATE_URL" => "",
					"DELETE_URL" => '/thurly/components/thurly/socialnetwork.log.entry/ajax.php?lang='.$params["LANGUAGE_ID"].'&action=delete_comment&delete_comment_id=#ID#&post_id='.$logEntry["ID"].'&site='.$params["SITE_ID"],
					"AUTHOR_URL" => '',
		...
		)
		{
			...
			if ($UESent) // process emails
			{
				foreach ($HTTPPost["SPERM"]["UE"] as $key => $userEmail)
				{
					...
					$invitedUserId = \Thurly\Mail\User::create($userFields);
...
//				unset($HTTPPost["SPERM"]["UE"]);
	...
	public static function getUserSonetGroupIdList($userId = false, $siteId = false)
	{
		...
		if($currentCache->startDataCache($cacheTtl, $cacheId, $cacheDir))
		{
			...
			$res = \Thurly\Socialnetwork\UserToGroupTable::getList(array(
				'filter' => array(
					'<=ROLE' => \Thurly\Socialnetwork\UserToGroupTable::ROLE_USER,
					'=USER_ID' => $userId,
					'=GROUP.ACTIVE' => 'Y',
	...
	public static function getLivefeedStepper()
	{
		...
		{
			$res["blog"] = array('Thurly\Blog\Update\LivefeedIndexPost', 'Thurly\Blog\Update\LivefeedIndexComment');
		}
		if (ModuleManager::isModuleInstalled('tasks'))
		{
			$res["tasks"] = array('Thurly\Tasks\Update\LivefeedIndexTask');
		}
		if (ModuleManager::isModuleInstalled('calendar'))
		{
			$res["calendar"] = array('Thurly\Calendar\Update\LivefeedIndexCalendar');
		}
		if (ModuleManager::isModuleInstalled('forum'))
		{
			$res["forum"] = array('Thurly\Forum\Update\LivefeedIndexMessage', 'Thurly\Forum\Update\LivefeedIndexComment');
		}
		if (ModuleManager::isModuleInstalled('xdimport'))
		{
			$res["xdimport"] = array('Thurly\XDImport\Update\LivefeedIndexLog', 'Thurly\XDImport\Update\LivefeedIndexComment');
		}
		if (ModuleManager::isModuleInstalled('wiki'))
		{
			$res["wiki"] = array('Thurly\Wiki\Update\LivefeedIndexLog', 'Thurly\Wiki\Update\LivefeedIndexComment');
		}
		if (!empty($res))
		{
	...
	public static function checkProfileRedirect($userId = 0)
	{
		...
		if ($userFields = $res->fetch())
		{
			...
			{
				if($eventResult->getType() == \Thurly\Main\EventResult::SUCCESS)
				{
	...
	public static function setBlogPostLimitedViewStatus($params = array())
	{
		...
		if ($show)
		{
			...
			\Thurly\Blog\Integration\Socialnetwork\CounterPost::increment(array(
			...
			\Thurly\Socialnetwork\ComponentHelper::notifyBlogPostCreated($notificationParamsList);
