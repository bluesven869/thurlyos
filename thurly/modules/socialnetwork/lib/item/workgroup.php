...
class Workgroup
{
		...
		if ($groupId > 0)
		{
			...
			else
			{
				...
				if ($fields = $res->fetch())
				{
					...
					if ($groupFields['DATE_CREATE'] instanceof \Thurly\Main\Type\DateTime)
					{
						$groupFields['DATE_CREATE'] = $groupFields['DATE_CREATE']->toString();
					}
					if ($groupFields['DATE_UPDATE'] instanceof \Thurly\Main\Type\DateTime)
					{
						$groupFields['DATE_UPDATE'] = $groupFields['DATE_UPDATE']->toString();
					}
					if ($groupFields['DATE_ACTIVITY'] instanceof \Thurly\Main\Type\DateTime)
					{
	...
	private static function getDelayedSubordination()
	{
		return (
			...
			&& method_exists('Thurly\Intranet\Internals\UserSubordinationTable', 'getDelay')
	...
	public static function onAfterIBlockSectionUpdate($section)
	{
		...
		if (!empty($groupsToCheck))
		{
			...
			{
				$groupItem = \Thurly\Socialnetwork\Item\Workgroup::getById($groupId, false);
	...
	private static function disconnectSection($sectionId)
	{
		...
		foreach($groupList as $group)
		{
			...
			$groupItem = \Thurly\Socialnetwork\Item\Workgroup::getById($group['ID'], false);
			$groupItem->syncDeptConnection();
		}
	}
