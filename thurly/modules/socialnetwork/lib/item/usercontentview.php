...
/*
		...
		{
			$result = (
				!in_array(\CThurlyOS::getLicenseType(), array('project'), true)
...
*/
	...
	public static function getUserList($params = array())
	{
		...
		$res = \Thurly\Socialnetwork\UserContentViewTable::getList($queryParams);
		...
		while ($fields = $res->fetch())
		{
			...
			$dateView = ($fields['DATE_VIEW'] instanceof \Thurly\Main\Type\DateTime ? $fields['DATE_VIEW']->toString() : '');
	...
	public static function deleteNoDemand($userId = 0)
	{
		...
		try
		{
			\Thurly\Main\Application::getConnection()->queryExecute("DELETE FROM ".UserContentViewTable::getTableName()." WHERE USER_ID = ".$userId);
		}
