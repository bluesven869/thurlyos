...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/main/admin/settings.php');
IncludeModuleLangFile(__FILE__);

CModule::IncludeModule('socialnetwork');

if ($REQUEST_METHOD=="GET" && strlen($RestoreDefaults)>0 && $SONET_RIGHT=="W" && check_thurly_sessid())
{
...
)
{
	...
	for ($i = 0; $i < $tmp_count; $i++)
	{
		...
		{
			\Thurly\Socialnetwork\LogIndexTable::getEntity()->enableFullTextIndex("CONTENT", ($val == 'Y'));
		}
			...
			if ($arAllOptionsCommon[$i][0] == 'use_lf_fulltext_index')
			{
				...
				)
				{
					if (!$DB->IndexExists("b_sonet_log_index", array("CONTENT")))
					{
						...
						{
							\Thurly\Socialnetwork\LogIndexTable::getEntity()->enableFullTextIndex("CONTENT", false);
	...
	if (count($arAllOptionsCommon) > 0)
	{
		...
		for ($i = 0; $i < $tmp_count; $i++):
			...
			if (
				...
				&& $val == 'Y'
				&& !\Thurly\Socialnetwork\LogIndexTable::getEntity()->fullTextIndexEnabled("CONTENT")
			)
			{
				$val = 'N';
			}
