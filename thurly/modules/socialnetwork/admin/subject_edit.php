...
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $socialnetworkModulePermissions>="W" && check_thurly_sessid())
{
	$arFields = array(
		"NAME" => $NAME,
		"SITE_ID" => $SITE_ID,
	...
	{
		if (strlen($apply) <= 0)
			LocalRedirect("/thurly/admin/socnet_subject.php?lang=".LANG."&".GetFilterParams("filter_", false));
	}
	else
	{
		$bVarsFromForm = true;
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/socnet_subject.php?lang=".LANG."&".GetFilterParams("filter_", false)
	)
);
...
if ($ID > 0 && $socialnetworkModulePermissions >= "W")
{
	...
	$aMenu[] = array(
			...
			"ICON" => "btn_new",
			"LINK" => "/thurly/admin/socnet_subject_edit.php?lang=".LANG."&".GetFilterParams("filter_", false)
		);

	$aMenu[] = array(
			"TEXT" => GetMessage("SONETE_DELETE_SUBJECT"), 
			"ICON" => "btn_delete",
			"LINK" => "javascript:if(confirm('".GetMessage("SONETE_DELETE_SUBJECT_CONFIRM")."')) window.location='/thurly/admin/socnet_subject.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."#tb';",
			"WARNING" => "Y"
		);
}
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/socnet_subject.php?lang=".LANG."&".GetFilterParams("filter_", false)
			)
	);
?>
