...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/include.php");

IncludeModuleLangFile(__FILE__);

require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/socialnetwork/prolog.php");
...
$strErrorMessage = "";
$bInitVars = false;
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $sonetPermissions=="W" && check_thurly_sessid())
{
	$SORT = IntVal($SORT);
	if ($SORT<=0) $SORT = 150;
	...
	if (strlen($strErrorMessage)<=0)
	{
		...
		if (is_set($_FILES, "IMAGE1") && strlen($_FILES["IMAGE1"]["name"])>0)
		{
			...
			if (strlen($strErrorMessage)<=0)
			{
				$strDirName = $_SERVER["DOCUMENT_ROOT"]."/thurly/images/socialnetwork/";
				...
				else
				{
					...
					{
						$strDirNameOld = $_SERVER["DOCUMENT_ROOT"]."/thurly/images/socialnetwork/";
...
$aMenu = array(
		array(
			...
			"ICON" => "btn_list",
			"LINK" => "/thurly/admin/socnet_smile.php?lang=".LANG."&".GetFilterParams("filter_", false)
		)
	);
...
if ($ID > 0 && $sonetPermissions == "W")
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/socnet_smile_edit.php?lang=".LANG."&".GetFilterParams("filter_", false)
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("FSN_DELETE_SMILE"), 
		"LINK" => "javascript:if(confirm('".GetMessage("FSN_DELETE_SMILE_CONFIRM")."')) window.location='/thurly/admin/socnet_smile.php?action=delete&ID[]=".$ID."&lang=".LANG."&".thurly_sessid_get()."#tb';",
		"WARNING" => "Y"
	);
}
...
<tr class="adm-detail-required-field">
	...
	<td>
		...
		if (strlen($f_IMAGE)>0)
		{
			?><div style="padding-top: 10px;"><img src="/thurly/images/socialnetwork/<?echo ($f_SMILE_TYPE=="I")?"icon":"smile" ?>/<?echo $f_IMAGE?>" border="0" <?echo (IntVal($f_IMAGE_WIDTH)>0) ? "width=\"".$f_IMAGE_WIDTH."\"" : "" ?> <?echo (IntVal($f_IMAGE_WIDTH)>0) ? "height=\"".$f_IMAGE_HEIGHT."\"" : "" ?>></div><?
		}
	?></td>
</tr><?
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/socnet_smile.php?lang=".LANG."&".GetFilterParams("filter_", false)
			)
	);
