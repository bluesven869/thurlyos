...
while ($arSocNet = $dbResultList->NavNext(true, "f_"))
{
	...
	$row->AddField("ID", '<a href="/thurly/admin/socnet_smile_edit.php?ID='.$f_ID.'&lang='.LANGUAGE_ID.'" title="'.GetMessage("SONET_EDIT_DESCR").'">'.$f_ID.'</a>');
	...
	$row->AddField("ICON", "<img src=\"/thurly/images/socialnetwork/".(($f_SMILE_TYPE=="I")?"icon":"smile")."/".$f_IMAGE."\" border=\"0\" ".((IntVal($f_IMAGE_WIDTH) > 0) ? "width=\"".$f_IMAGE_WIDTH."\"" : "")." ".((IntVal($f_IMAGE_WIDTH) > 0) ? "height=\"".$f_IMAGE_HEIGHT."\"" : "" ).">");
