...
class CSocNetLogPages
{
	...
	public static function Set($user_id, $page_last_date, $page_size, $page_num = 1, $site_id = SITE_ID, $group_code = '**', $traffic_avg = false, $traffic_cnt = false)
	{
		...
		$page_last_date = new \Thurly\Main\Type\DateTime($page_last_date);
		...
		if ($traffic_cnt)
		{
			...
			$arInsertFields["TRAFFIC_LAST_DATE"] = $arUpdateFields["TRAFFIC_LAST_DATE"] = new \Thurly\Main\DB\SqlExpression($helper->getCurrentDateTimeFunction());
		}
