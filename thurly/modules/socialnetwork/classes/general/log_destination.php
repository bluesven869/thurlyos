...
class CSocNetLogDestination
{
	...
	*/
	...
	public static function GetUsers($arParams = Array(), $bSelf = true)
	{
		...
		$userId = intval($USER->GetID());
		$extranetUserIdList = \Thurly\Socialnetwork\ComponentHelper::getExtranetUserIdList();
	...
	public static function SearchUsers($search, &$nt = "", $bSelf = true, $bEmployeesOnly = false, $bExtranetOnly = false, $departmentId = false)
	{
		...
		$bThurlyOSEnabled = IsModuleInstalled('thurlyos');

		$bEmailUsersAll = ($bMailEnabled && \Thurly\Main\Config\Option::get('socialnetwork', 'email_users_all', 'N') == 'Y');
		...
		if ($bIntranetEnabled)
		{
			...
			)
			{
				...
				$rsUser = \Thurly\Main\FinderDestTable::getList(array(
					'order' => array(),
		...
		) // consider extranet collaboration
		{
			CExtranet::fillUserListFilterORM(
				array(
					...
					"INTRANET_ONLY" => ($bEmployeesOnly || ($bThurlyOSEnabled && !$bExtranetEnabled)),
		...
		$arSelect = array(
			...
			"PERSONAL_GENDER",
			"EXTERNAL_AUTH_ID",
			new \Thurly\Main\Entity\ExpressionField('MAX_LAST_USE_DATE', 'MAX(%s)', array('\Thurly\Main\FinderDest:CODE_USER_CURRENT.LAST_USE_DATE'))
		);
		...
		)
		{
			...
			$rsUser = \Thurly\Main\UserTable::getList(array(
				...
				'select' => array(
					...
					"PERSONAL_GENDER",
					"EXTERNAL_AUTH_ID",
					new \Thurly\Main\Entity\ExpressionField('EMAIL_OK', 'CASE WHEN UPPER(%s) = "'.$DB->ForSql(strtoupper(str_replace('%', '%%', $search))).'" THEN 1 ELSE 0 END', 'EMAIL')
				),
				'limit' => 10
			));
	...
	private static function GetSocnetGroupFilteredByInitiatePerms(&$arGroups)
	{
		...
		$res = \Thurly\Socialnetwork\WorkgroupTable::getList(array(
			'filter' => array(
				'@ID' => $arGroupsIDs
			),
		...
		foreach ($arGroups as $key => $group)
		{
			...
			$canInitiate = (
				(
					...
					&& $groupsList[$groupId]["INITIATE_PERMS"] == \Thurly\Socialnetwork\UserToGroupTable::ROLE_OWNER
					&& $userId == $groupsList[$groupId]["OWNER_ID"]
				)
				|| (
					isset($groupsList[$groupId])
					&& $groupsList[$groupId]["INITIATE_PERMS"] == \Thurly\Socialnetwork\UserToGroupTable::ROLE_MODERATOR
					&& isset($userRolesList[$groupId])
					&& in_array($userRolesList[$groupId], array(
						\Thurly\Socialnetwork\UserToGroupTable::ROLE_OWNER,
						\Thurly\Socialnetwork\UserToGroupTable::ROLE_MODERATOR
					))
				)
				|| (
					isset($groupsList[$groupId])
					&& $groupsList[$groupId]["INITIATE_PERMS"] == \Thurly\Socialnetwork\UserToGroupTable::ROLE_USER
					&& isset($userRolesList[$groupId])
					&& in_array($userRolesList[$groupId], array(
						\Thurly\Socialnetwork\UserToGroupTable::ROLE_OWNER,
						\Thurly\Socialnetwork\UserToGroupTable::ROLE_MODERATOR,
						\Thurly\Socialnetwork\UserToGroupTable::ROLE_USER
					))
				)
			);
	...
	public static function GetDestinationUsers($arCodes, $fetchUsers = false)
	{
		...
		foreach($arCodes as $code)
		{
			...
			elseif (substr($code, 0, 2) === 'DR')
			{
				...
				$res = \Thurly\Intranet\Util::getDepartmentEmployees(array(
					'DEPARTMENTS' => $depId,
					'RECURSIVE' => 'Y',
					'ACTIVE' => 'Y',
					'SELECT' => $fields
	...
	public static function GetDestinationSort($arParams = array())
	{
		...
		else
		{
			...
			if (!empty($arParams["DEST_CONTEXT"]))
			{
				...
				$arRuntime = array(
					new \Thurly\Main\Entity\ExpressionField('CONTEXT_SORT', "CASE WHEN CONTEXT = '".$helper->forSql($arParams["DEST_CONTEXT"])."' THEN 1 ELSE 0 END")
				);
			...
			$rsDest = \Thurly\Main\FinderDestTable::getList(array(
				'order' => $arOrder,
				'filter' => $arFilter,
				'select' => array(
					'CONTEXT',
		...
		if (is_array($arDestinationSort))
		{
			...
			)
			{
				...
				else
				{
					...
					$selectList[] = new \Thurly\Main\Entity\ExpressionField('MAX_LAST_USE_DATE', 'MAX(%s)', array('\Thurly\Main\FinderDest:CODE_USER_CURRENT.LAST_USE_DATE'));
	...
	public static function getUsersAll($arParams)
	{
		...
		$maxCount = (IsModuleInstalled('thurlyos') ? 200 : 500);
		$resultCount = 0;
		$countExceeded = false;
		$arUsers = array();
