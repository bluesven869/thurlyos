...
class CAllSocNetUser
{
	public static function OnUserDelete($ID)
	{
		...
		if ($bSuccess)
		{
			...
			\Thurly\Socialnetwork\Item\UserContentView::deleteNoDemand($ID);
	...
	public static function IsUserModuleAdmin($userID, $site_id = SITE_ID)
	{
		...
		$arModuleAdmin = \Thurly\Socialnetwork\User::getModuleAdminList($site_id);
			...
			else // check by log USER_ID field / author
			{
				if (in_array($userId, $arLogEntryUserId))
				{
					...
					)
					{
						...
						$rsDepartmentUserId = \Thurly\Intranet\Util::getDepartmentEmployees(array(
