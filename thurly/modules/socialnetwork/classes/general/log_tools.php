...
class CSocNetLogTools
{
	...
	{
		$name = $GLOBALS["APPLICATION"]->IncludeComponent("thurly:main.user.link",
			'',
			array(
				"ID" => $arEntityDesc["ID"],
	...
	public static function FormatEvent_GetCreatedBy($arFields, $arParams, $bMail, $bFirstCaps = false)
	{
		if (intval($arFields["USER_ID"]) > 0)
		{
			...
			)
			{
				...
				$suffix = (SITE_TEMPLATE_ID != "thurlyos" ? GetMessage("SONET_LOG_EXTRANET_SUFFIX") : "");
			}
			else
			{
	...
	public static function DeleteComment_Forum($arFields)
	{
		...
		)
		{
			...
			if (
				true || // we are not ready to use \Thurly\Forum\Comments\Feed yet
	...
	function AddComment_Review_UpdateElement_Forum($arElement, $topicID, $bNewTopic)
	{
		...
		ForumClearComponentCache("thurly:forum.topic.reviews");
	}
	...
	public static function FormatDestinationFromRights($arRights, $arParams, &$iMoreCount = false)
	{
		...
		{
			$rsGroupTmp = \Thurly\Main\GroupTable::getList(array(
		...
		if (!empty($arUserIdToGet))
		{
			...
			$rsUserTmp = \Thurly\Main\UserTable::getList(array(
				'order' => array(),
				'filter' => array(
					"ID" => $arUserIdToGet
				),
	...
	public static function GetDataFromRatingEntity($rating_entity_type_id, $rating_entity_id, $bCheckRights = true)
	{
		...
		switch ($rating_entity_type_id)
		{
			...
			case "INTRANET_NEW_USER_COMMENT":
				...
				break;
			case "THURLY24_NEW_USER":
				$log_type = "log";
				$log_event_id = array("thurlyos_new_user");
				break;
			case "THURLY24_NEW_USER_COMMENT":
				$log_type = "comment";
				$log_event_id = array("thurlyos_new_user_comment");
				break;
			case "VOTING":
				$log_type = "log";
				if ($blogPostEventIdList === null)
				...
				?><div class="feed-com-files">
					...
					foreach ($arResult["VALUE"] as $fileID)
					{
						...
						if($arFile)
						{
							...
							$arFile["LINK"] = "/thurly/components/thurly/socialnetwork.log.ex/show_file.php?fid=".$fileID."&ltype=".$type;
...
//			"blog_post" => array("BLOG", "BG"),
	...
	function AddComment_Tasks($arFields)
	{
		...
		if ($arLog = $dbResult->Fetch())
		{
			...
			{
				$forumID = \Thurly\Tasks\Integration\SocialNetwork\Task::getCommentForumId();
				if ($forumID)
				{
					$userID = $USER->GetID();
...
class CSocNetLogComponent
{
	...
	public static function ConvertPresetToFilters($arPreset, $arParams = array())
	{
		...
		{
			$isExtranetAvailable = (
				\Thurly\Main\ModuleManager::isModuleInstalled('extranet')
			);
			$isExtranetSite = (
				$isExtranetAvailable
			...
			$isTasksAvailable = (
				\Thurly\Main\ModuleManager::isModuleInstalled('tasks')
			);
			$isCrmAvailable = (
				\Thurly\Main\ModuleManager::isModuleInstalled('crm')
			);
			$isListsAvailable = (
				\Thurly\Main\ModuleManager::isModuleInstalled('lists')
				&& \Thurly\Main\ModuleManager::isModuleInstalled('bizproc')
				&& \Thurly\Main\ModuleManager::isModuleInstalled('intranet')
			);
			$isTimemanAvailable = (
				\Thurly\Main\ModuleManager::isModuleInstalled('timeman')
			);
			$inited = true;
		}
		...
		foreach ($arPreset as $key => $arPresetFilterTmp)
		{
			...
			if (!empty($arPresetFilterTmp["ID"]))
			{
				...
				(
					...
					&& $isCrmAvailable
					&& \Thurly\Main\Config\Option::get("crm", "enable_livefeed_merge", "N") == "Y"
	...
	public static function saveFileToUF($arFile = array(), &$type, $userId = false)
	{
		...
		if (!isset($arDiskData[$userId]))
		{
			...
			{
				if (
					($storage = \Thurly\Disk\Driver::getInstance()->getStorageByUserId($userId))
		...
		if(is_array($arFile))
		{
			...
			if ($arDiskData[$userId]["ENABLED"] == "Y")
			{
				...
				if ($file)
				{
					$resultId = \Thurly\Disk\Uf\FileUserType::NEW_FILE_PREFIX.$file->getId();
				}
			}
	...
	public static function getCommentByRequest($commentId, $postId, $action = false, $checkPerms = true, $checkAdminSession = true)
	{
		...
		if ($arLog = $rsLog->Fetch())
		{
			$hasSource = \Thurly\Socialnetwork\ComponentHelper::hasCommentSource(array(
