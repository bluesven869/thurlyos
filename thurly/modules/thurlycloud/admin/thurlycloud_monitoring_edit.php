...
/* @global CUser $USER */
if (!$USER->CanDoOperation("thurlycloud_monitoring") || !CModule::IncludeModule("thurlycloud"))
...
try
{
	...
	)
	{
		...
		{
			$bVarsFromForm = true;
			throw new CThurlyCloudException($result);
		}

		if ($apply != "")
			LocalRedirect("/thurly/admin/thurlycloud_monitoring_edit.php?lang=".LANGUAGE_ID."&domain=".urlencode($_REQUEST["domain"]));
		else
			LocalRedirect("/thurly/admin/thurlycloud_monitoring_admin.php?lang=".LANGUAGE_ID);
	}

	$arResult = $monitoring->getList();
	if (is_string($arResult))
	{
		throw new CThurlyCloudException($arResult);
		$arResult = array();
	}
}
catch (Exception $e)
...
<?$tabControl->Buttons(array(
	"disabled" => false,
	"back_url" => "thurlycloud_monitoring_admin.php?lang=".LANGUAGE_ID,
));?>
