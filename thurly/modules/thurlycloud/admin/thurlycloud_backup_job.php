...
/* @global CUser $USER */
if (!$USER->CanDoOperation("thurlycloud_backup") || !CModule::IncludeModule("thurlycloud"))
...
try
{
	$sTableID = "t_thurlycloud_backup_job";
	...
	if ($arID = $lAdmin->GroupAction())
	{
		foreach ($arID as $ID)
		{
			...
			{
			case "delete":
				$strError = CThurlyCloudBackup::getInstance()->deleteBackupJob();
				break;
			}
		}
	}
	...
	)
	{
		...
		$strError = CThurlyCloudBackup::getInstance()->addBackupJob($backup_secret_key, $_POST["URL"], $time, $_POST["WEEK_DAYS"]);
		if ($strError == "")
			LocalRedirect("/thurly/admin/thurlycloud_backup_job.php?lang=".LANGUAGE_ID);
	}
	...
	$arJobs = CThurlyCloudBackup::getInstance()->getBackupJob();
	if (is_string($arJobs))
		throw new CThurlyCloudException($arJobs);
