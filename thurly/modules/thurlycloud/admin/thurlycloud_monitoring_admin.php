<?
define("ADMIN_MODULE_NAME", "thurlycloud");
...
{
	CThurlyCloudMonitoringResult::setExpirationTime(0);
...
$sTableID = "t_thurlycloud_monitoring";
...
try
{
	...
	{
		throw new CThurlyCloudException($monitoringResults);
	}
...
while($arRes = $rsData->GetNext())
{
	...
	$isOK = true;
	/** @var CThurlyCloudMonitoringDomainResult $domainResults */
	$domainResults = $monitoringResults[$arRes["DOMAIN"]];
	$html = '<table width="100%">';
	/** @var CThurlyCloudMonitoringTest $test_http_response_time */
	$test_http_response_time = $domainResults["test_http_response_time"];
	if ($test_http_response_time)
	{
		if ($test_http_response_time->getStatus() === CThurlyCloudMonitoringResult::RED_LAMP)
		{
			$isOK = false;
	...
	/** @var CThurlyCloudMonitoringTest $test_domain_registration */
	$test_domain_registration = $domainResults["test_domain_registration"];
	if ($test_domain_registration)
	{
		if ($test_domain_registration->getStatus() === CThurlyCloudMonitoringResult::RED_LAMP)
		{
			$isOK = false;
			$indicatorStyle = 'style="color:red"';
		}
	...
	/** @var CThurlyCloudMonitoringTest $test_lic */
	$test_lic = $domainResults["test_lic"];
	if ($test_lic)
	{
		if ($test_lic->getStatus() === CThurlyCloudMonitoringResult::RED_LAMP)
		{
			$isOK = false;
			$indicatorStyle = 'style="color:red"';
		}
	...
	/** @var CThurlyCloudMonitoringTest $test_ssl_cert_validity */
	$test_ssl_cert_validity = $domainResults["test_ssl_cert_validity"];
	if ($test_ssl_cert_validity)
	{
		if ($test_ssl_cert_validity->getStatus() === CThurlyCloudMonitoringResult::RED_LAMP)
		{
			$isOK = false;
			$indicatorStyle = 'style="color:red"';
		}
	...
	$arActions = array(
		array(
			...
			"ACTION" => $lAdmin->ActionRedirect("thurlycloud_monitoring_edit.php?domain=".urlencode($arRes["DOMAIN"])),
		),
		array(
			"SEPARATOR" => "Y",
		),
...
if (!empty($localDomains))
{
	...
	foreach ($localDomains as $punyName => $domainName)
	{
		$aContext[] = array(
			...
			"LINK" => "thurlycloud_monitoring_edit.php?lang=".LANGUAGE_ID."&domain=".urlencode($punyName),
			"TITLE" => "",
		);
	}
	$aContext = array(
