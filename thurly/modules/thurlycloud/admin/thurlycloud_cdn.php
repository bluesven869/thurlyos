...
)
{
	if (isset($_POST["save"]) || isset($_POST["apply"]))
	{
		...
		else
		{
			...
			CThurlyCloudCDNConfig::getInstance()->setDebug($_POST["debug"] === "y");
			if (!CThurlyCloudCDN::SetActive($_POST["cdn_active"] === "Y", true))
			{
	...
	else
	{
		...
		LocalRedirect("/thurly/admin/thurlycloud_cdn.php?lang=".LANGUAGE_ID.($_GET["return_url"] ? "&return_url=".urlencode($_GET["return_url"]) : "")."&".$tabControl->ActiveTabParam());
	}
}
$cdn_config = CThurlyCloudCDNConfig::getInstance()->loadFromOptions();
...
if ($bVarsFromForm)
{
	...
	$optimize = $_POST["optimize"]==="y";
	$kernel_folder = $_POST["thurly_folder"]!=="n";
...
{
	$active = CThurlyCloudCDN::IsActive();
...
?>
<form method="POST" action="thurlycloud_cdn.php?lang=<?echo LANGUAGE_ID ?><?echo $_GET["return_url"] ? "&amp;return_url=".urlencode($_GET["return_url"]) : "" ?>" enctype="multipart/form-data" name="editform">
<?
$tabControl->Begin();
...
$tabControl->Buttons(array(
	"back_url" => $_GET["return_url"] ? $_GET["return_url"] : "thurlycloud_cdn.php?lang=".LANGUAGE_ID,
));
?>
