...
class CThurlyCloudBackupWebService extends CThurlyCloudWebService
{
	...
	protected function getActionURL($arParams = /*.(array[string]string).*/ array())
	{
		...
		$url = COption::GetOptionString("thurlycloud", "backup_policy_url");
	 ...
	 */
	protected function backup_action($action) /*. throws CThurlyCloudException .*/
	{
		...
		{
			throw new CThurlyCloudException(GetMessage("BCL_BACKUP_WS_SERVER", array(
				"#STATUS#" => "-1",
	 ...
	 *
	 */
	public function actionGetInformation() /*. throws CThurlyCloudException .*/
	{
		$this->addStr = "";
	 ...
	 *
	 */
	public function actionReadFile($check_word, $file_name) /*. throws CThurlyCloudException .*/
	{
		$this->addStr = "";
		$this->addParams = array(
	 ...
	 *
	 */
	public function actionWriteFile($check_word, $file_name) /*. throws CThurlyCloudException .*/
	{
		$this->addStr = "";
		$this->addParams = array(
			"file_name" => $file_name,
	 ...
	 *
	 */
	public function actionAddBackupJob($secret_key, $url, $time = 0, $weekdays = array()) /*. throws CThurlyCloudException .*/
	{
		if ($secret_key == "")
		{
			throw new CThurlyCloudException(GetMessage("BCL_BACKUP_EMPTY_SECRET_KEY"), "");
		}
		...
		{
			throw new CThurlyCloudException(GetMessage("BCL_BACKUP_WRONG_URL"), "");
		}

		$time = intval($time);
		if ($time < 0 || $time >= 24*3600)
		{
			throw new CThurlyCloudException(GetMessage("BCL_BACKUP_WRONG_TIME"), "");
		}
		...
		if (!$weekdaysIsOk)
		{
			throw new CThurlyCloudException(GetMessage("BCL_BACKUP_WRONG_WEEKDAYS"), "");
		}
