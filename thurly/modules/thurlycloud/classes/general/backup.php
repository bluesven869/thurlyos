...
class CThurlyCloudBackup
{
	private static $instance = /*.(CThurlyCloudBackup).*/ null;
	private $init = false;
	 ...
	 *
	 * @return CThurlyCloudBackup
	 *
	 */
	public static function getInstance()
	{
		if (!isset(self::$instance))
			self::$instance = new CThurlyCloudBackup;
	...
	private function _getInformation($force = false)
	{
		...
		try
		{
			$web_service = new CThurlyCloudBackupWebService();
		...
		if (is_object($node))
			$this->quota = CThurlyCloudCDNQuota::parseSize($node->textContent());
		...
		if (is_object($node))
		{
			...
			foreach($nodeFiles as $nodeFile)
			{
				...
				$size = CThurlyCloudCDNQuota::parseSize($nodeFile->getAttribute("size"));
	 ...
	 *
	 */
	public function listFiles() /*. throws CThurlyCloudException .*/
	{
	 ...
	 *
	 */
	public function getQuota() /*. throws CThurlyCloudException .*/
	{
		$this->_getInformation();
		return $this->quota;
	}
	 ...
	 *
	 */
	public function getUsage() /*. throws CThurlyCloudException .*/
	{
		$this->_getInformation();
	 ...
	 *
	 */
	public function getLastTimeBackup() /*. throws CThurlyCloudException .*/
	{
		$this->_getInformation();
	...
	private function _getBucket($operation, $check_word, $file_name)
	{
		...
		return new CThurlyCloudBackupBucket(
			$bucket_name,
			$prefix,
			$access_key,
			$secret_key,
	...
	{
		CThurlyCloudOption::getOption("backup_files")->delete();
		CThurlyCloudOption::getOption("backup_quota")->delete();
		CThurlyCloudOption::getOption("backup_total_size")->delete();
		CThurlyCloudOption::getOption("backup_last_backup_time")->delete();
		return $this;
	}
	/**
	...
	public function saveToOptions()
	{
		...
		ksort($arFiles);
		CThurlyCloudOption::getOption("backup_files")->setArrayValue($arFiles);
		CThurlyCloudOption::getOption("backup_quota")->setStringValue((string)$this->quota);
		CThurlyCloudOption::getOption("backup_total_size")->setStringValue((string)$this->total_size);
		CThurlyCloudOption::getOption("backup_last_backup_time")->setStringValue((string)$this->last_backup_time);
		return $this;
	}
	/**
	...
	{
		$this->files = /*.(array[int][string]string).*/ array();
		foreach(CThurlyCloudOption::getOption("backup_files")->getArrayValue() as $FILE_NAME => $FILE_SIZE)
		{
			$this->files[] = array(
				"FILE_NAME" => $FILE_NAME,
				"FILE_SIZE" => $FILE_SIZE,
		...
		$this->quota = doubleval(CThurlyCloudOption::getOption("backup_quota")->getStringValue());
		$this->total_size = doubleval(CThurlyCloudOption::getOption("backup_total_size")->getStringValue());
		$this->last_backup_time = intval(CThurlyCloudOption::getOption("backup_last_backup_time")->getStringValue());
		$this->init = true;
		return $this;
	}
	/**
	...
	public static function OnAdminInformerInsertItems()
	{
		...
		$last_request_time_option = CThurlyCloudOption::getOption("backup_last_backup_time");
		try
		{
			if (
				$backup->getQuota() <= 0
		...
		if (empty($arFiles))
		{
			...
			if ($USER->CanDoOperation("thurlycloud_backup") && $USER->CanDoOperation('edit_php'))
			{
				$CDNAIParams["FOOTER"] = '<a href="/thurly/admin/dump.php?lang='.LANGUAGE_ID.'">'.GetMessage("BCL_BACKUP_AI_DO_BACKUP_STRONGLY").'</a>';
			}
		}
		...
		else
		{
			...
			if ($USER->CanDoOperation("thurlycloud_backup") && $USER->CanDoOperation('edit_php'))
			{
				$CDNAIParams["FOOTER"] = '<a href="/thurly/admin/dump.php?lang='.LANGUAGE_ID.'">'.GetMessage("BCL_BACKUP_AI_DO_BACKUP").'</a>';
			}
		}
