<?php
abstract class CAllThurlyCloudOption
{
	private $name = "";
	...
	private function _write_db($value)
	{
		...
		if (is_array($value))
		{
			...
			{
				$DB->Add("b_thurlycloud_option", array(
					"NAME" => $this->name,
					"SORT" => (string)$sort,
					"PARAM_KEY" => $key,
					"PARAM_VALUE" => $val,
	...
	private function _update_db($value)
	{
		...
		while (list($key, $val) = each($value))
		{
			if ($db_row = $rs->fetch())
			{
				...
				{
					$DB->Query("
						UPDATE b_thurlycloud_option SET
		...
		{
			$DB->Query("
				DELETE FROM b_thurlycloud_option
	...
	public function getArrayValue()
	{
		...
		if (!isset($this->value))
		{
			if (CACHED_b_thurlycloud_option <= 0)
			{
				$this->value = $this->_read_db();
			}
			else
			{
				if (!$CACHE_MANAGER->Read(CACHED_b_thurlycloud_option, "b_thurlycloud_option"))
				{
					$arOptions = $this->_read_all_db();
					$CACHE_MANAGER->Set("b_thurlycloud_option", $arOptions);
				}
				else
				{
					$arOptions = $CACHE_MANAGER->Get("b_thurlycloud_option");
				}
	...
	public function setArrayValue($value)
	{
		...
		if (strlen($this->name) > 0)
		{
			...
			if ($stored !== $value)
			{
				...
				if (CACHED_b_thurlycloud_option !== false)
					$CACHE_MANAGER->Clean("b_thurlycloud_option");
			}
		}
	}
	/**
