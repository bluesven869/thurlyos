...
class CThurlyCloudMonitoring
{
	private static $instance = /*.(CThurlyCloudMonitoring).*/ null;
	private $result = /*.(CThurlyCloudMonitoringResult).*/null;
	private $interval = 0;
	/**
	 * Returns proxy class instance (singleton pattern)
	 *
	 * @return CThurlyCloudMonitoring
	 *
	 */
	public static function getInstance()
	{
		if (!isset(self::$instance))
			self::$instance = new CThurlyCloudMonitoring;
	...
	{
		$web_service = new CThurlyCloudMonitoringWebService();
	...
	public function getInterval()
	{
		if ($this->interval <= 0)
		{
			$this->interval = intval(COption::GetOptionInt("thurlycloud", "monitoring_interval"));
			if (
				$this->interval != 7
				&& $this->interval != 30
				&& $this->interval != 90
	...
	public function getMonitoringResults($interval = false)
	{
		...
		if ($this->result === null)
		{
			try
			{
				if (CThurlyCloudMonitoringResult::isExpired())
				{
					$web_service = new CThurlyCloudMonitoringWebService();
					...
					{
						$this->result = CThurlyCloudMonitoringResult::fromXMLNode($domains);
						$control = $xml->SelectNodes("/control");
						if (is_object($control))
						{
							$this->result->saveToOptions();
							CThurlyCloudMonitoringResult::setExpirationTime(strtotime($control->getAttribute("expires")));
						}
					}
				}
				else
				{
					$this->result = CThurlyCloudMonitoringResult::loadFromOptions();
				}
			}
			catch (CThurlyCloudException $e)
			{
				CThurlyCloudMonitoringResult::setExpirationTime(time() + 1800);
				return $e->getMessage();//."[".htmlspecialcharsEx($e->getErrorCode())."]";
			}
		}
		return $this->result;
	...
	{
		return CThurlyCloudOption::getOption("monitoring_alert")->getArrayValue();
	}
	public function storeAlertsCurrentResult()
	{
		...
		if (is_array($alerts))
		{
			CThurlyCloudOption::getOption("monitoring_alert")->setArrayValue($alerts);
		}
	}
				...
				foreach ($this->result as $domainName => $domainResult)
				{
					foreach ($domainResult as $testId => $testResult)
					{
						if (
							...
							&& $testResult->getStatus() === CThurlyCloudMonitoringResult::RED_LAMP
						)
						{
