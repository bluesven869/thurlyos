...
if($_SERVER["REQUEST_METHOD"] == "POST" && array_key_exists("IM_AJAX_CALL", $_REQUEST) && $_REQUEST["IM_AJAX_CALL"] === "Y" && $_POST['IM_OPEN_LINES_CLIENT'] == 'Y')
{
	...
	if ($userId <= 0 || !(IsModuleInstalled('imopenlines') && \Thurly\Main\Loader::includeModule('im') && \Thurly\Im\User::getInstance($userId)->isConnector()))
	{
		echo \Thurly\ImOpenLines\Common::objectEncode(Array('ERROR' => 'AUTHORIZE_ERROR'));
		CMain::FinalActions();
		die();
	}
	...
	{
		echo \Thurly\ImOpenLines\Common::objectEncode(Array(
	...
	if ($_POST['COMMAND'] == "sendLivechatForm")
	{
		...
		$control = new \Thurly\ImOpenLines\LiveChatForm($chatId, $userId);
...
else if($_SERVER["REQUEST_METHOD"] == "POST" && array_key_exists("IM_AJAX_CALL", $_REQUEST) && $_REQUEST["IM_AJAX_CALL"] === "Y" && $_POST['IM_OPEN_LINES'] == 'Y')
{
	...
	{
		$control = new \Thurly\ImOpenLines\Operator($chatId, $userId);
	...
	{
		$control = new \Thurly\ImOpenLines\Operator(0, $userId);
	...
	{
		$control = new \Thurly\ImOpenLines\Operator($_POST['CHAT_ID'], $userId);
