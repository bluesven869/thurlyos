...
class ImOpenLinesComponentStatisticsDetail extends CThurlyComponent
{
	protected $gridId = "imopenlines_statistic_v3";
	protected $filterId = "imopenlines_statistic_detail_filter";

	/** @var  \Thurly\Main\Grid\Options */
	...
	private function init()
	{
		...
		if ($request->get('CONFIG_ID'))
		{
			...
			$config = \Thurly\ImOpenLines\Config::getInstance()->get($request->get('CONFIG_ID'));
	...
	{
		if(!$this->userPermissions->canPerform(\Thurly\ImOpenlines\Security\Permissions::ENTITY_SESSION, \Thurly\ImOpenlines\Security\Permissions::ACTION_VIEW))
		{
	...
	private static function getCrmLink($row)
	{
		...
		return \Thurly\ImOpenLines\Crm::getLink($row['CRM_ENTITY_TYPE'], $row['CRM_ENTITY_ID']);
	}
	...
	private static function getCrmActivityLink($row)
	{
		...
		return \Thurly\ImOpenLines\Crm::getLink('ACTIVITY', $row['CRM_ACTIVITY_ID']);
	}
	...
	private function getFilterDefinition()
	{
		...
		$result = array_merge($result, array(
			...
			'SOURCE' => array(
				...
				"type" => "list",
				"items" => \Thurly\ImConnector\Connector::getListConnector(),
				"default" => true,
				"params" => array(
					"multiple" => "Y"
				)
	...
	private function getFilter(array $filterDefinition)
	{
		...
		$filterOptions = new \Thurly\Main\UI\Filter\Options($this->filterId);
		...
		$allowedUserIds = \Thurly\ImOpenlines\Security\Helper::getAllowedUserIds(
			\Thurly\ImOpenlines\Security\Helper::getCurrentUserId(),
			$this->userPermissions->getPermission(\Thurly\ImOpenlines\Security\Permissions::ENTITY_SESSION, \Thurly\ImOpenlines\Security\Permissions::ACTION_VIEW)
		);
		...
		if (strlen($filter["DATE_CREATE_from"]) > 0)
		{
			try
			{
				$result[">=DATE_CREATE"] = new \Thurly\Main\Type\DateTime($filter["DATE_CREATE_from"]);
			}
			catch (Exception $e)
			{
			}
		...
		if (strlen($filter["DATE_CREATE_to"]) > 0)
		{
			try
			{
				$result["<=DATE_CREATE"] = new \Thurly\Main\Type\DateTime($filter["DATE_CREATE_to"]);
			}
			catch (Exception $e)
			{
			}
		...
		if (strlen($filter["DATE_CLOSE_from"]) > 0)
		{
			try
			{
				$result[">=DATE_CLOSE"] = new \Thurly\Main\Type\DateTime($filter["DATE_CLOSE_from"]);
			} catch (Exception $e){}
		}
		if (strlen($filter["DATE_CLOSE_to"]) > 0)
		{
			try
			{
				$result["<=DATE_CLOSE"] = new \Thurly\Main\Type\DateTime($filter["DATE_CLOSE_to"]);
			} catch (Exception $e){}
		}
		...
		if(isset($filter['CRM_ENTITY']) && $filter['CRM_ENTITY'] != '')
		{
			...
			try
			{
				$crmFilter = \Thurly\Main\Web\Json::decode($filter['CRM_ENTITY']);
		...
		if(isset($filter['FIND']) && \Thurly\Main\Search\Content::canUseFulltextSearch($filter['FIND'], \Thurly\Main\Search\Content::TYPE_MIXED))
		{
			global $DB;
			if (!\Thurly\Imopenlines\Model\SessionIndexTable::getEntity()->fullTextIndexEnabled('SEARCH_CONTENT') && $DB->IndexExists("b_imopenlines_session_index", array("SEARCH_CONTENT"), true))
			{
				\Thurly\Imopenlines\Model\SessionIndexTable::getEntity()->enableFullTextIndex("SEARCH_CONTENT");
			}
			if (\Thurly\Imopenlines\Model\SessionIndexTable::getEntity()->fullTextIndexEnabled('SEARCH_CONTENT'))
			{
				if (\Thurly\Main\Search\Content::isIntegerToken($filter['FIND']))
				{
					$result['*INDEX.SEARCH_CONTENT'] = \Thurly\Main\Search\Content::prepareIntegerToken($filter['FIND']);
				}
				else
				{
					$result['*INDEX.SEARCH_CONTENT'] = \Thurly\Main\Search\Content::prepareStringToken($filter['FIND']);
				}
			}
		}
	...
	public function executeComponent()
	{
		...
		{
			$this->arResult['TRIAL'] = \Thurly\ImOpenlines\Security\Helper::getTrialText(); // TODO restrict
		}
		...
		$cursor = \Thurly\ImOpenLines\Model\SessionTable::getList(array(
			'order' => $sorting["sort"],
		...
		$this->showHistory = \Thurly\ImOpenlines\Security\Helper::getAllowedUserIds(
			\Thurly\ImOpenlines\Security\Helper::getCurrentUserId(),
			$this->userPermissions->getPermission(\Thurly\ImOpenlines\Security\Permissions::ENTITY_HISTORY, \Thurly\ImOpenlines\Security\Permissions::ACTION_VIEW)
		);
		$configManager = new \Thurly\ImOpenLines\Config();

		$arUsers = $this->getUserData($userId);
		$arSources = \Thurly\ImConnector\Connector::getListConnector();
