BX.ViPermissionEdit = function(element)
{
	...
	this.ajaxUrl = '/thurly/components/thurly/imopenlines.settings.perms/ajax.php';
	this.init();
};
...
BX.ViPermissionEdit.prototype =
{
	...
	handleDeleteRoleClick: function(e)
	{
		...
		self.confirm(BX.message('IMOL_PERM_ROLE_DELETE'), BX.message('IMOL_PERM_ROLE_DELETE_CONFIRM'), function(e)
		{
			if(!e.confirmed)
				return;
			...
			BX.ajax({
				...
				onsuccess: function(data)
				{
					...
					if(!data || data.ERROR)
					{
						self.notify(BX.message('IMOL_PERM_ERROR'), BX.message('IMOL_PERM_ROLE_DELETE_ERROR'));
						return;
					}
		...
		var popupWindow = new BX.PopupWindow(popupId, null, {
			...
			buttons: [
				new BX.PopupWindowButton({
					text : BX.message('IMOL_PERM_ROLE_OK'),
				...
				new BX.PopupWindowButtonLink({
					text : BX.message('IMOL_PERM_ROLE_CANCEL'),
