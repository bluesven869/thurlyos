...
namespace Thurly\Imopenlines\Ui;
...
class Helper
{
	...
	{
		global $APPLICATION;
		Asset::getInstance()->addJs('/thurly/js/imopenlines/common.js');
		...
		{
			Asset::getInstance()->addJs('/thurly/js/crm/common.js');
			Asset::getInstance()->addJs('/thurly/js/crm/crm.js');
			Asset::getInstance()->addJs('/thurly/js/crm/interface_grid.js');
		...
		foreach($userSelectors as $userSelector)
		{
			...
			?>BX.ready(
				function()
				{
					BX.ImOpenLines.UserSelector.create(
						"<?=\CUtil::JSEscape($selectorID)?>",
						{
							fieldId: "<?=\CUtil::JSEscape($fieldID)?>",
	...
	{
		$res = array(
			"imopenlines" => array('Thurly\Imopenlines\Update\Session')
		);
