...
namespace Thurly\ImOpenlines\Security;
...
class Helper
{
	...
	{
		if (\Thurly\Main\ModuleManager::isModuleInstalled('thurlyos') && !$GLOBALS['USER']->CanDoOperation('thurlyos_config'))
		{
			return false;
		}
		...
		if (is_null(self::$showWidgetLink))
		{
			...
			if (\Thurly\Main\Loader::includeModule('crm') && \CCrmPerms::IsAccessEnabled())
			{
	...
	{
		$checkCursor = \Thurly\ImOpenlines\Model\RoleTable::getList(array('limit' => 1));
		...
		{
			$addResult = \Thurly\ImOpenlines\Model\RoleTable::add(array(
				'NAME' => $role['NAME'],
				'XML_ID' => $roleCode,
			));
			...
			if ($roleId)
			{
				...
				\Thurly\ImOpenlines\Security\RoleManager::setRolePermissions($roleId, $role['PERMISSIONS']);
			}
		}

		if (isset($roleIds['ADMIN']))
		{
			\Thurly\ImOpenlines\Model\RoleAccessTable::add(array(
