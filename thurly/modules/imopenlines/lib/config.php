...
class Config
{
	...
	{
		$companyName = \Thurly\Main\Config\Option::get("main", "site_name", "");
		...
		if ($fields['QUEUE_TYPE'] == self::QUEUE_TYPE_ALL && !\Thurly\Imopenlines\Limit::canUseQueueAll())
		{
		...
		if ($fields['VOTE_MESSAGE'] == 'Y' && !\Thurly\Imopenlines\Limit::canUseVoteClient())
		{
	...
	public function create($params = Array())
	{
		...
		if ($fields['TEMPORARY'] == 'Y')
		{
			...
			$date->add('8 HOUR');
			\CAgent::AddAgent('\Thurly\ImOpenLines\Config::deleteTemporaryConfigAgent('.$id.');', "imopenlines", "N", 28800, "", "Y", $date);
		}
	...
	public function delete($id)
	{
		...
		try
		{
			...
			{
				\Thurly\ImConnector\Output::deleteLine($id);
			}
		}
		catch (\Exception $e)
		{}
	...
	{
		if (!\Thurly\Imopenlines\Limit::canUseVoteHead())
		{
			return false;
		}
	...
	public static function getQueueList($userId = 0, $emptyIsNotOperator = true)
	{
		...
		if ($userId > 0)
		{
			...
			$runtime[] = new \Thurly\Main\Entity\ReferenceField(
				'QUEUE',
				'\Thurly\ImOpenlines\Model\QueueTable',
				array(
	...
	public static function sendUpdateForQueueList($data)
	{
		...
		\Thurly\Pull\Event::add(array_values($channelId), Array(
