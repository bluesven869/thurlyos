...
class LiveChat
{
	...
	public function openSession()
	{
		...
		else
		{
			...
			$licence = md5("THURLY".\CUpdateClient::GetLicenseKey()."LICENCE");

			$this->sessionId = md5(time().thurly_sessid().$licence);
		}
	...
	private function getChatForUser()
	{
		...
		if ($this->userId)
		{
			$orm = \Thurly\Main\UserTable::getById($this->userId);
			if ($user = $orm->fetch())
			{
				...
				$userName = \Thurly\Im\User::getInstance($this->userId)->getFullName(false);
				$chatColorCode = \Thurly\Im\Color::getCodeByNumber($this->userId);
				if (\Thurly\Im\User::getInstance($this->userId)->getGender() == 'M')
				{
	...
	private function getGuestUser($userId = null)
	{
		...
		if ($userId && \Thurly\Im\User::getInstance($userId)->isExists())
		{
			if (\Thurly\Im\User::getInstance($userId)->isConnector())
			{
				return $userId;
			}
	...
	{
		$messages = \Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/imopenlines/js_livechat.php', $lang);
