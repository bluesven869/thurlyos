...
class Chat
{
	...
	public function __construct($chatId = 0, $params = Array())
	{
		...
		if ($chatId > 0)
		{
			$chat = \Thurly\Im\Model\ChatTable::getById($chatId)->fetch();
	...
	public function load($params)
	{
		...
		$orm = \Thurly\Im\Model\ChatTable::getList(array(
			'filter' => array(
				'=ENTITY_TYPE' => 'LINES',
		...
		if ($params['USER_ID'])
		{
			$orm = \Thurly\Main\UserTable::getById($params['USER_ID']);
			if ($user = $orm->fetch())
			{
				...
				{
					$userName = \Thurly\Im\User::getInstance($params['USER_ID'])->getFullName(false);
				}
				$chatColorCode = \Thurly\Im\Color::getCodeByNumber($params['USER_ID']);
				if (\Thurly\Im\User::getInstance($params['USER_ID'])->getGender() == 'M')
				{
					$replaceColor = \Thurly\Im\Color::getReplaceColors();
		...
		$orm = \Thurly\Im\Model\ChatTable::getById($id);
		$this->chat = $orm->fetch();
		$this->isCreated = true;
		$this->isDataLoaded = true;
	...
	public function answer($userId, $skipSession = false, $skipMessage = false)
	{
		...
		else if ($session && !$session->isNowCreated())
		{
			...
			if (!\Thurly\Im\User::getInstance($userId)->isBot() && $session->getData('DATE_OPERATOR_ANSWER') <= 0)
			{
		...
		foreach ($relations as $relation)
		{
			...
			if (\Thurly\Im\User::getInstance($relation['USER_ID'])->isConnector())
				continue;

			if ($config['WELCOME_BOT_LEFT'] == Config::BOT_LEFT_CLOSE && \Thurly\Im\User::getInstance($relation['USER_ID'])->isBot())
				continue;
		...
		if (!$skipMessage)
		{
			$userAnswer = \Thurly\Im\User::getInstance($userId);
	...
	public function intercept($userId)
	{
		...
		$previousOwner = \Thurly\Im\User::getInstance($previousOwnerId);
		$newOwner = \Thurly\Im\User::getInstance($userId);
	...
	public function skip($userId = 0)
	{
		...
		if ($userId)
		{
			$userSkip = \Thurly\Im\User::getInstance($userId);
		...
		if ($session->getConfig('QUEUE_TYPE') == Config::QUEUE_TYPE_ALL && $userId)
		{
			...
			foreach ($relations as $relation)
			{
				...
				if (\Thurly\Im\User::getInstance($relation['USER_ID'])->isBot())
					continue;
	...
	public function endBotSession()
	{
		...
		if ($result && \Thurly\Im\User::getInstance($session->getData('OPERATOR_ID'))->isBot())
		{
	...
	public function transfer($params)
	{
		...
		if (substr($params['TO'], 0, 5) == 'queue')
		{
			...
			$config = \Thurly\ImOpenlines\Model\ConfigTable::getById($queueId)->fetch();
			if (!$config)
			{
				return false;
			}
			...
			$userFrom = \Thurly\Im\User::getInstance($params['FROM']);

			\Thurly\ImOpenLines\Model\OperatorTransferTable::Add(Array(
		...
		else
		{
			...
			{
				if (
					\Thurly\Im\User::getInstance($transferUserId)->isBot() ||
					\Thurly\Im\User::getInstance($transferUserId)->isExtranet() ||
					\Thurly\Im\User::getInstance($transferUserId)->isConnector() ||
					$transferUserId <= 0
				)
				{
					return false;
			...
			$userFrom = \Thurly\Im\User::getInstance($params['FROM']);
			$userTo = \Thurly\Im\User::getInstance($transferUserId);
	...
	{
		$users = Array();
		$relations = \Thurly\Im\Chat::getRelation($this->chat['ID'], Array(
		...
		\Thurly\Pull\Event::add($users, Array(
			'module_id' => 'imopenlines',
	...
	public function setSilentMode($active = true)
	{
		...
		\Thurly\Im\Model\ChatTable::update($this->chat['ID'], Array(
	...
	public function getTitle($lineName, $userName = '', $userColor = '')
	{
		...
		if (!$userColor)
		{
			$userColor = \Thurly\Im\Color::getRandomCode();
		}
	...
	public function updateFieldData($field, $fieldData)
	{
		...
		if ($field == self::FIELD_SESSION)
		{
			...
			{
				$data['DATE_CREATE'] = $fieldData['DATE_CREATE'] instanceof \Thurly\Main\Type\DateTime? $fieldData['DATE_CREATE']->getTimestamp(): intval($fieldData['DATE_CREATE']);
			}
		...
		else if ($field == self::FIELD_LIVECHAT)
		{
			...
			{
				$data['READED_TIME'] = $fieldData['READED_TIME'] instanceof \Thurly\Main\Type\DateTime? date('c', $fieldData['READED_TIME']->getTimestamp()): false;
			}
		...
		{
			if (
				\Thurly\Im\User::getInstance($relation['USER_ID'])->isBot() ||
				\Thurly\Im\User::getInstance($relation['USER_ID'])->isNetwork() ||
				$field != self::FIELD_LIVECHAT && \Thurly\Im\User::getInstance($relation['USER_ID'])->isConnector()
			)
			{
				continue;
			}
	...
	public function update($fields)
	{
		...
		\Thurly\Im\Model\ChatTable::update($this->chat['ID'], $fields);
		...
		foreach ($relations as $rel)
		{
			...
			if ($updateManager)
			{
				...
				{
					\Thurly\Im\Model\RelationTable::update($rel['ID'], Array('MANAGER' => 'N'));
				}
				if ($rel['USER_ID'] == $fields['AUTHOR_ID'])
				{
					\Thurly\Im\Model\RelationTable::update($rel['ID'], Array('MANAGER' => 'Y'));
				}
			}
		}
	...
	public static function getGuestName($chatColorCode = '')
	{
		...
		if (\Thurly\Im\Color::isEnabled())
		{
			if (!$chatColorCode)
			{
				...
				$chatColorCode = \Thurly\Im\Color::getCodeByNumber($chatColorId);
			}
			...
			$userName = Loc::getMessage('IMOL_CHAT_CHAT_NAME_COLOR_GUEST', Array("#COLOR#" => \Thurly\Im\Color::getName($chatColorCode), "#NUMBER#" => $chatColorCodeCount+1));
		}
		else
		{
	...
	public function sendJoinMessage($userList)
	{
		...
		{
			$toUserId = $userList[0];
			$userName = \Thurly\Im\User::getInstance($toUserId)->getFullName(false);
	...
	public static function sendRatingNotify($type, $sessionId, $rating, $toUserId, $fromUserId = null)
	{
		...
		{
			$userName = \Thurly\Im\User::getInstance($fromUserId)->getFullName(false);
			$userGender = \Thurly\Im\User::getInstance($fromUserId)->getGender();
