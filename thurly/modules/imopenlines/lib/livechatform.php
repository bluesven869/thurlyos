...
class LiveChatForm
{
	...
	public function saveForm($type, $fields)
	{
		...
		$user = \Thurly\Im\User::getInstance($this->userId);
		...
		if ($type == self::FORM_WELCOME || $type == self::FORM_OFFLINE)
		{
			...
			if (isset($fields['PHONE']) && $user->getPhone(\Thurly\Im\User::PHONE_MOBILE) != $fields['PHONE'])
			{
		...
		else if ($type == self::FORM_HISTORY)
		{
			...
			if (isset($fields['EMAIL']) && $chatFieldSession['SESSION_ID'])
			{
				...
				\Thurly\ImOpenLines\Mail::sendSessionHistory($chatFieldSession['SESSION_ID'], $fields['EMAIL']);
			}
		...
		if (!empty($userUpdate))
		{
			...
			\Thurly\Im\User::clearStaticCache();
		}
