...
use Thurly\Imopenlines\Model\TrackerTable;
use Thurly\Main,
...
class Tracker
{
	...
	public function message($params)
	{
		...
		/* @var \Thurly\ImOpenLines\Session $session */
		$session = $params['SESSION'];
		if (!($session instanceof \Thurly\ImOpenLines\Session))
			return false;
		...
		if (isset($params['CRM']['ENTITY_TYPE']) && isset($params['CRM']['ENTITY_ID']))
		{
			...
			if (
				...
				\Thurly\Im\User::getInstance($session->getData('USER_ID'))->isConnector() &&
				\Thurly\Im\User::getInstance($session->getData('USER_ID'))->getName() == ''
			)
			{
		...
		if ($current['CHANGE_NAME'] == 'Y' && $entityData['NAME'] && $entityData['LAST_NAME'])
		{
			...
			\Thurly\Pull\Event::add(array_keys($relations), Array(
				...
				'command' => 'updateUser',
				'params' => Array(
					'user' => \Thurly\Im\User::getInstance($session->getData('USER_ID'))->getFields()
				),
				'extra' => Array(
		...
		if ($current['ACTION'] == self::ACTION_CREATE)
		{
			...
			$keyboard = new \Thurly\Im\Bot\Keyboard();
	...
	public function cancel($messageId)
	{
		...
		if ($action == self::ACTION_CREATE)
		{
			...
			$entityTime = new \Thurly\Main\Type\DateTime($entityData['DATE_CREATE']);
	...
	public function change($messageId, $newEntityType, $newEntityId)
	{
		...
		if ($result)
		{
			$messageData = \Thurly\Im\Model\MessageTable::getById($messageOriginId)->fetch();
			$this->message(Array(
				'SESSION' => $session,
				'MESSAGE' => Array(
					'ID' => $messageData["ID"],
