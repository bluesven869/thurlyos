...
class SessionTable extends Main\Entity\DataManager
{
	...
	public static function getMap()
	{
		return array(
			...
			'INDEX' => array(
				'data_type' => 'Thurly\ImOpenLines\Model\SessionIndex',
				'reference' => array('=this.ID' => 'ref.SESSION_ID'),
				'join_type' => 'INNER',
			),
			'CONFIG' => array(
				'data_type' => 'Thurly\ImOpenLines\Model\Config',
				'reference' => array('=this.CONFIG_ID' => 'ref.ID'),
			),
			'CHECK' => array(
				'data_type' => 'Thurly\ImOpenLines\Model\SessionCheck',
				'reference' => array('=this.ID' => 'ref.SESSION_ID'),
			),
			'LIVECHAT' => array(
				'data_type' => 'Thurly\ImOpenLines\Model\Livechat',
	...
	public static function generateSearchContent(array $fields)
	{
		if($fields['CRM_ENTITY_TYPE'] != '' && $fields['CRM_ENTITY_ID'] > 0)
			$crmEntityCaption = \Thurly\ImOpenLines\Crm::getEntityCaption($fields['CRM_ENTITY_TYPE'], $fields['CRM_ENTITY_ID']);
		else
			$crmEntityCaption = '';

		$userId = array();

		if($fields['CHAT_ID'] > 0 && $fields['CLOSED'] == 'Y' && \Thurly\Main\Loader::includeModule('im'))
		{
			$userId[$fields['OPERATOR_ID']] = $fields['OPERATOR_ID'];
			$userId[$fields['USER_ID']] = $fields['USER_ID'];

			$transcriptLines = Array();
			$cursor = \Thurly\Im\Model\MessageTable::getList(array(
			...
			$transcriptLines = \Thurly\Im\Text::removeBbCodes($transcriptLines);
		...
		$result = \Thurly\Main\Search\MapBuilder::create()
			->addUser($userId)
	...
	{
		return new \Thurly\Main\Type\DateTime();
	}
}