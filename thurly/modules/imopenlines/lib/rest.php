...
class Rest extends \IRestService
{
	...
	public static function operatorAnswer($arParams, $n, \CRestServer $server)
	{
		...
		$control = new \Thurly\ImOpenLines\Operator($arParams['CHAT_ID']);
		$result = $control->answer();
		if (!$result)
		{
			throw new \Thurly\Rest\RestException($control->getError()->msg, $control->getError()->code, \CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function botSessionOperator($arParams, $n, \CRestServer $server)
	{
		...
		$chat = new \Thurly\Imopenlines\Chat($arParams['CHAT_ID']);
		$result = $chat->endBotSession();
		if (!$result)
		{
			throw new \Thurly\Rest\RestException("Operator is not a bot", "WRONG_CHAT", \CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function botSessionTransfer($arParams, $n, \CRestServer $server)
	{
		...
		else if (isset($arParams['QUEUE_ID']))
		{
			...
			if ($arParams['USER_ID'] <= 0)
			{
				throw new \Thurly\Rest\RestException("QUEUE ID can't be empty", "USER_ID_EMPTY", \CRestServer::STATUS_WRONG_REQUEST);
			}
		...
		{
			throw new \Thurly\Rest\RestException("Queue ID or User ID can't be empty", "TRANSFER_ID_EMPTY", \CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (!$result)
		{
			throw new \Thurly\Rest\RestException("You can not redirect to this operator", "OPERATOR_WRONG", \CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	{
		return array(
			'SERVER_ADDRESS' => \Thurly\ImOpenLines\Common::getServerAddress(),
			'PUBLIC_PATH' => \Thurly\ImOpenLines\Common::getPublicFolder()
		);
	}
	...
	public static function networkJoin($arParams, $n, \CRestServer $server)
	{
		...
		{
			throw new \Thurly\Rest\RestException("You entered an invalid code", "CODE", \CRestServer::STATUS_WRONG_REQUEST);
		}

		if (!\Thurly\Main\Loader::includeModule('imbot'))
		{
			throw new \Thurly\Rest\RestException("Module IMBOT is not installed", "IMBOT_ERROR", \CRestServer::STATUS_WRONG_REQUEST);
		}

		if (\Thurly\ImBot\Bot\Network::isFdcCode($arParams['CODE']))
		{
			throw new \Thurly\Rest\RestException("Line not found", "NOT_FOUND", \CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (!$result)
		{
			throw new \Thurly\Rest\RestException($network->getError()->msg, $network->getError()->code, \CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function networkMessageAdd($arParams, $n, \CRestServer $server)
	{
		...
		$isThurlyOS = \Thurly\Main\Loader::includeModule('thurlyos');
		if (!$isThurlyOS || !\CThurlyOS::IsNfrLicense())
		{
			$dateLimit = new \Thurly\Main\Type\DateTime();
			$dateLimit->add('-1 WEEK');

			$check = \Thurly\Imopenlines\Model\RestNetworkLimitTable::getList(Array(
				'filter' => Array(
			...
			if ($check)
			{
				throw new \Thurly\Rest\RestException("You cant send more than one message per week to each user.", "USER_MESSAGE_LIMIT", \CRestServer::STATUS_WRONG_REQUEST);
			}
		}
		...
		$id = \Thurly\Im\Bot::addMessage(array('BOT_ID' => $networkBot['BOT_ID']), $arMessageFields);
		if (!$id)
		{
			throw new \Thurly\Rest\RestException("Message isn't added", "WRONG_REQUEST", \CRestServer::STATUS_WRONG_REQUEST);
		}

		\Thurly\Imopenlines\Model\RestNetworkLimitTable::add(Array('BOT_ID' => $networkBot['BOT_ID'], 'USER_ID' => $arMessageFields['DIALOG_ID']));
