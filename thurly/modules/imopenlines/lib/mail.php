...
class Mail
{
	public static function addSessionToMailQueue($sessionId, $withCheck = true)
	{
		...
		$mailData = new \Thurly\Main\Type\DateTime();
		$mailData->add('1 MINUTE');
	
		\Thurly\ImOpenlines\Model\SessionCheckTable::update($sessionId, Array(
			'DATE_MAIL' => $mailData
		));
		
		Session::deleteQueueFlagCache(Session::CACHE_MAIL);
		
		$event = new \Thurly\Main\Event("imopenlines", "OnSessionToMailQueueAdd", Array('SESSION_ID' => $sessionId));
		$event->send();
	...
	public static function sendOperatorAnswer($sessionId)
	{
		...
		if ($sessionId <= 0 || !\Thurly\Main\Loader::includeModule('im'))
		{
			return false;
		}
		...
		{
			\Thurly\ImOpenlines\Model\SessionCheckTable::update($session['ID'], Array(
				'DATE_MAIL' => null
			));
		}
		...
		$email = \Thurly\Im\User::getInstance($session['USER_ID'])->getEmail();
		if (!$email)
		{
			return false;
		}
		...
		if (empty($widgetUrl))
		{
			...
			{
				$widgetUrl = \Thurly\ImOpenLines\Common::getServerAddress().'/online/'.$session['LIVECHAT_URL_CODE_PUBLIC'];
			}
			else if (!empty($session['LIVECHAT_URL_CODE']))
			{
				$widgetUrl = \Thurly\ImOpenLines\Common::getServerAddress().'/online/'.$session['LIVECHAT_URL_CODE'];
			}
		}
		...
		$arFields = array(
			"EMAIL_TO" => $email,
			"EMAIL_TITLE" => $title,
			"TEMPLATE_SERVER_ADDRESS" => \Thurly\ImOpenLines\Common::getServerAddress(),
	...
	public static function prepareOperatorAnswerForTemplate($sessionId, $setSendFlag = true)
	{
		...
		$relation = \Thurly\Im\Model\RelationTable::getList(Array(
			'filter' => Array(
				'=CHAT_ID' => $chatId,
		...
		\Thurly\Main\Type\Collection::sortByColumn($messages, Array('id' => SORT_ASC), '', null, true);
	...
	private static function prepareMessagesForTemplate($session, $history, $language = null)
	{
		...
		$userTzOffset = \Thurly\Im\User::getInstance($session['USER_ID'])->getTzOffset();
		...
		foreach ($history['message'] as $messageId => $message)
		{
			...
			if ($message['senderId'] > 0)
			{
				...
				{
					$authorName = \Thurly\ImOpenLines\Connector::getOperatorName($session['CONFIG_ID'], $message['senderId']);
				}
				
				$authorAvatar = \Thurly\Im\User::getInstance($message['senderId'])->getAvatar();
				if ($authorAvatar == '/thurly/js/im/images/blank.gif')
				{
					$authorAvatar = '';
				}
				else
				{
					$authorAvatar = substr($authorAvatar, 0, 4) != 'http'? \Thurly\ImOpenLines\Common::getServerAddress().$authorAvatar: $authorAvatar;
				}
			...
			$date = \Thurly\Main\Type\DateTime::createFromTimestamp($message['date']);
