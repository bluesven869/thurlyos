...
class Queue
{
	...
	public function getNextUser($manual = false)
	{
		...
		{
			if (!\Thurly\Im\User::getInstance($queueUser['USER_ID'])->isActive())
			{
				continue;
			}
			
			if (\Thurly\Im\User::getInstance($queueUser['USER_ID'])->isAbsent())
			{
			...
			{
				Model\QueueTable::update($queueUser['ID'], Array('LAST_ACTIVITY_DATE' => new \Thurly\Main\Type\DateTime()));
			}
		...
		{
			Model\QueueTable::update($result['FIRST_IN_QUEUE_ID'], Array('LAST_ACTIVITY_DATE' => new \Thurly\Main\Type\DateTime()));
		}
	...
	{
		$lastActivityDate = \IsModuleInstalled('thurlyos')? 1440: 180;
		$timeHelper = \Thurly\Main\Application::getConnection()->getSqlHelper()->addSecondsToDateTime('(-'.$lastActivityDate.')');

		$query = new \Thurly\Main\Entity\Query(\Thurly\ImOpenLines\Model\QueueTable::getEntity());
		$query->registerRuntimeField('', new \Thurly\Main\Entity\ExpressionField('IS_ONLINE_CUSTOM', 'CASE WHEN %s > '.$timeHelper.' THEN \'Y\' ELSE \'N\' END', Array('USER.LAST_ACTIVITY_DATE')));
