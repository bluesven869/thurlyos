...
use Thurly\ImOpenlines\QuickAnswers\ListsDataManager;
use Thurly\ImOpenlines\QuickAnswers\QuickAnswer;
use Thurly\Main,
	Thurly\Main\Localization\Loc;

use Thurly\ImOpenlines\Security\Permissions;
use Thurly\ImOpenlines\Security\Helper;
...
class Operator
{
	...
	private function checkAccess()
	{
		...
		$orm = \Thurly\Im\Model\RelationTable::getList(array(
		...
		{
			$ormChat = \Thurly\Im\Model\ChatTable::getById($this->chatId);
			if($chat = $ormChat->fetch())
			{
				if($chat['TYPE'] == IM_MESSAGE_OPEN_LINE)
				{
					...
					if(!\Thurly\ImOpenLines\Config::canJoin($lineId, ($fieldData[0] == 'Y'? $fieldData[1]: null), ($fieldData[0] == 'Y'? $fieldData[2]: null)))
					{
	...
	{
		if (\Thurly\Im\User::getInstance($this->userId)->isExtranet())
			return false;
		...
		if ($result)
		{
			...
			if ($chat->getData('AUTHOR_ID') != $this->userId)
			{
				...
				if (!\Thurly\ImOpenLines\Config::canJoin($lineId, $sessionField['CRM_ENTITY_TYPE'], $sessionField['CRM_ENTITY_ID']))
				{
					$result = false;
				}
			}
	...
	public function getSessionHistory($sessionId)
	{
		...
		$result['canJoin'] = \Thurly\ImOpenLines\Config::canJoin($session['CONFIG_ID'])? 'Y':'N';
		$result['canVoteAsHead'] = \Thurly\ImOpenLines\Config::canVoteAsHead($session['CONFIG_ID'])? 'Y':'N';
