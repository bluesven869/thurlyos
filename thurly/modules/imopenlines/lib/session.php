...
class Session
{
	...
	public function load($params)
	{
		...
		if(\Thurly\ImOpenLines\Connector::isEnableGroupByChat($parsedUserCode['CONNECTOR_ID']))
		{
	...
	public function start($params)
	{
		...
		$select = \Thurly\ImOpenLines\Model\SessionTable::getSelectFieldsPerformance();
		...
		while($row = $orm->fetch())
		{
			...
			else
			{
				...
				$updateDuplicate['CLOSED'] = new \Thurly\Main\DB\SqlExpression("?# + 1", "CLOSED");
				$updateDuplicate['IN_WORK'] = new \Thurly\Main\DB\SqlExpression("?# - 1", "IN_WORK");
		...
		if ($result)
		{
			...
			if ($params['VOTE_SESSION'] == 'Y' && $result['CLOSED'] == 'Y')
			{
				...
				$updateStatisticTable['CLOSED'] = new \Thurly\Main\DB\SqlExpression("?# - 1", "CLOSED");
				$updateStatisticTable['IN_WORK'] = new \Thurly\Main\DB\SqlExpression("?# + 1", "IN_WORK");
				Model\ConfigStatisticTable::update($this->session['CONFIG_ID'], $updateStatisticTable);

				$dateClose = new \Thurly\Main\Type\DateTime();
				...
				Model\SessionTable::update($result['ID'], Array(
					...
					'PAUSE' => 'N',
					'END_ID' => 0,
					'STATUS' => self::STATUS_CLIENT_AFTER_OPERATOR
				));
		...
		if ($fields['MODE'] == self::MODE_INPUT)
		{
			...
			{
				if ($this->config['WELCOME_BOT_JOIN'] == \Thurly\ImOpenLines\Config::BOT_JOIN_ALWAYS)
				{
			...
			{
				$fields['DATE_OPERATOR'] = new \Thurly\Main\Type\DateTime();
		...
		$updateStats = Array(
			'IN_WORK' => new \Thurly\Main\DB\SqlExpression("?# + 1", "IN_WORK"),
			'SESSION' => new \Thurly\Main\DB\SqlExpression("?# + 1", "SESSION"),
		);
		if ($fields['CRM_CREATE'] == 'Y')
		{
			$updateStats['LEAD'] = new \Thurly\Main\DB\SqlExpression("?# + 1", "LEAD");
		}
		...
		if ($fields['MODE'] == self::MODE_INPUT)
		{
			$dateQueue = new \Thurly\Main\Type\DateTime();
			if ($this->session['JOIN_BOT'])
			{
		...
		$event = new \Thurly\Main\Event("imopenlines", "OnSessionStart", $eventData);
		$event->send();
	...
	{
		$result = \Thurly\Imopenlines\Model\UserRelationTable::getByPrimary($params['USER_CODE'])->fetch();
		if ($result)
		{
			if ($result['CHAT_ID'])
			{
		...
		else if ($params['SKIP_CREATE'] != 'Y')
		{
			...
			\Thurly\Imopenlines\Model\UserRelationTable::add(Array(
			...
			if (!$this->chat->isDataLoaded())
			{
				\Thurly\Imopenlines\Model\UserRelationTable::delete($params['USER_CODE']);
				return false;
			}

			\Thurly\Imopenlines\Model\UserRelationTable::update($params['USER_CODE'], Array('CHAT_ID' => $this->chat->getData('ID')));
	...
	public function finish($auto = false, $force = false, $hideChat = true)
	{
		...
		$currentDate = new \Thurly\Main\Type\DateTime();

		if ($this->session['CHAT_ID'])
		{
			$chatData = \Thurly\Im\Model\ChatTable::getById($this->session['CHAT_ID'])->fetch();
		...
		{
			$messageData = \Thurly\Im\Model\MessageTable::getById($lastMessageId)->fetch();
			if ($messageData)
			{
		...
		else
		{
			...
			else
			{
				...
				if (!$waitAction)
				{
					$userSkip = \Thurly\Im\User::getInstance($this->chat->getData('OPERATOR_ID'));
				...
				if (!\Thurly\Im\User::getInstance($this->session['OPERATOR_ID'])->isBot())
				{
		...
		if ($update['CLOSED'] == 'Y')
		{
			...
			if (\Thurly\Im\User::getInstance($this->session['OPERATOR_ID'])->isBot() && $this->session['TIME_BOT'] <= 0 && $this->session['DATE_CREATE'])
			{
	...
	public function createLead($params)
	{
		...
		if ($id)
		{
			\Thurly\Imopenlines\Limit::increaseTracker();
	...
	public function update($fields)
	{
		...
		{
			$dateActivityClose = new \Thurly\Main\Type\DateTime();
			...
			if ($this->session['PAUSE'] == 'N' || $fields['PAUSE'] == 'N')
			{
				...
				if ($this->session['WAIT_ACTION'] == 'N' && isset($fields['USER_ID']) && \Thurly\Im\User::getInstance($fields['USER_ID'])->isConnector())
				{
					$dateClose = new \Thurly\Main\Type\DateTime();
		...
		if (isset($fields['CLOSED']) && $fields['CLOSED'] == 'Y')
		{
			...
			{
				if (\Thurly\Main\Loader::includeModule('im') && \Thurly\Im\User::getInstance($this->session['USER_ID'])->isOnline())
				{
					\CAgent::AddAgent('\Thurly\ImOpenLines\Mail::sendOperatorAnswerAgent('.$this->session['ID'].');', "imopenlines", "N", 60, "", "Y", \ConvertTimeStamp(time()+\CTimeZone::GetOffset()+60, "FULL"));
				}
				else
				{
					\Thurly\ImOpenLines\Mail::sendOperatorAnswer($this->session['ID']);
				}
			}
		...
		else if (isset($fields['PAUSE']))
		{
			if ($fields['PAUSE'] == 'Y')
			{
				$datePause = new \Thurly\Main\Type\DateTime();
		...
		{
			$fields["MESSAGE_COUNT"] = new \Thurly\Main\DB\SqlExpression("?# + 1", "MESSAGE_COUNT");
			$updateStatisticTable['MESSAGE'] = new \Thurly\Main\DB\SqlExpression("?# + 1", "MESSAGE");
		}

		if (isset($fields['CRM_CREATE']) && $fields['CRM_CREATE'] == 'Y')
		{
			$updateStatisticTable['LEAD'] = new \Thurly\Main\DB\SqlExpression("?# + 1", "LEAD");
		}
	...
	public function checkWorkTime()
	{
		...
		$numberDate = new \Thurly\Main\Type\DateTime(null, null, $timezone);
	...
	public function execAutoAction($params)
	{
		...
		if ($this->chat->isNowCreated() && $this->config['AGREEMENT_MESSAGE'] == 'Y' && $enableSystemMessage)
		{
			Im::addMessage(Array(
				...
				"MESSAGE" => Loc::getMessage('IMOL_SESSION_AGREEMENT_MESSAGE', Array(
					'#LINK_START#' => '[URL='.\Thurly\ImOpenLines\Common::getAgreementLink($this->config['AGREEMENT_ID'])."]",
					'#LINK_END#' => '[/URL]'
				)),
				"SYSTEM" => 'Y',
				"IMPORTANT_CONNECTOR" => 'Y',
		...
		$update['DATE_MODIFY'] = new \Thurly\Main\Type\DateTime();
	...
	public function transferToNextInQueue($manual = true)
	{
		...
		if ($queue['RESULT'])
		{
			if ($queue['USER_ID'] && $this->session['OPERATOR_ID'] != $queue['USER_ID'])
			{
				$this->chat->transfer(Array(
					...
					'LEAVE' => $this->config['WELCOME_BOT_LEFT'] == Config::BOT_LEFT_CLOSE && \Thurly\Im\User::getInstance($this->session['OPERATOR_ID'])->isBot()? 'N':'Y'
				));
			}
		...
		{
			if ($this->session['OPERATOR_ID'] <= 0 || \Thurly\Im\User::getInstance($this->session['OPERATOR_ID'])->isBot())
			{
				$this->chat->transfer(Array(
					'FROM' => $this->session['OPERATOR_ID'],
	...
	public static function transferToNextInQueueAgent($nextExec)
	{
		if (self::getQueueFlagCache(self::CACHE_QUEUE))
			return '\Thurly\ImOpenLines\Session::transferToNextInQueueAgent(0);';
		...
		$res = Model\SessionCheckTable::getList(Array(
			'select' => $select,
			'filter' => Array(
				'<=DATE_QUEUE' => new \Thurly\Main\Type\DateTime()
			),
			'limit' => 100
		));
		while ($row = $res->fetch())
		...
		return '\Thurly\ImOpenLines\Session::transferToNextInQueueAgent(1);';
	}

	public static function closeByTimeAgent($nextExec)
	{
		if (self::getQueueFlagCache(self::CACHE_CLOSE))
			return '\Thurly\ImOpenLines\Session::closeByTimeAgent(0);';
		...
		$res = Model\SessionCheckTable::getList(Array(
			'select' => $select,
			'filter' => Array(
				'<=DATE_CLOSE' => new \Thurly\Main\Type\DateTime()
			),
			'limit' => 100
		));
		while ($row = $res->fetch())
		...
		return '\Thurly\ImOpenLines\Session::closeByTimeAgent(1);';
	}

	public static function mailByTimeAgent($nextExec)
	{
		if (self::getQueueFlagCache(self::CACHE_MAIL))
			return '\Thurly\ImOpenLines\Session::mailByTimeAgent(0);';
		...
		$res = Model\SessionCheckTable::getList(Array(
			...
			'filter' => Array(
				'<=DATE_MAIL' => new \Thurly\Main\Type\DateTime()
			),
			'limit' => 100
		));
		while ($row = $res->fetch())
		{
			\Thurly\ImOpenLines\Mail::sendOperatorAnswer($row['SESSION_ID']);
		}

		return '\Thurly\ImOpenLines\Session::mailByTimeAgent(1);';
	}
	...
	public static function voteAsUser($sessionId, $action, $userId = null)
	{
		...
		\Thurly\ImOpenLines\Chat::sendRatingNotify(\Thurly\ImOpenLines\Chat::RATING_TYPE_CLIENT, $sessionData['ID'], $voteValue, $sessionData['OPERATOR_ID'], $sessionData['USER_ID']);
	...
	public static function voteAsHead($sessionId, $voteValue, $userId = null)
	{
		...
		if ($voteValue > 0)
		{
			\Thurly\ImOpenLines\Chat::sendRatingNotify(\Thurly\ImOpenLines\Chat::RATING_TYPE_HEAD, $sessionData['ID'], $voteValue, $sessionData['OPERATOR_ID'], $userId);
		}
		...
		if (\Thurly\Main\Loader::includeModule("pull"))
		{
			...
			\Thurly\Pull\Event::add(array_keys($relations), $pullMessage);
