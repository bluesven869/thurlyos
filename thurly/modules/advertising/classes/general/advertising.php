...
class CAdvBanner_all
{
	...
	public static function __innerExtractThurlyDates($arBanner)
	{
	...
	{
		$dt = CAdvBanner_all::__innerExtractThurlyDates($arBanner);
		if (!$dt["to"]) return 0;
				...
				switch($file_type)
				{
					case "FLASH":
						...
						if ($arBanner["FLASH_JS"] != 'Y')
						{
							...
							{
								$strReturn .= ($bNoIndex? '<noindex>':'').'<div style="position:absolute; z-index:100;"><a href="'.$url.'"'.$a_target.$a_title.($bNoIndex? ' rel="nofollow"':'').'><img src="/thurly/images/1.gif" width="'.$arImage["WIDTH"].'" height="'.$arImage["HEIGHT"].'" style="border:0;" alt="'.htmlspecialcharsEx($alt).'" /></a></div>'.($bNoIndex? '</noindex>':'');
							}
							$strReturn .=
'<OBJECT
		...
		<EMBED
			...
			PLUGINSPAGE="http://www.macromedia.com/go/getflashplayer">
						...
						else
						{
							...
							{
								$bScript = true;
								$strReturn .= '<script type="text/javascript" src="'.CUtil::GetAdditionalFileURL('/thurly/js/advertising/flash.js').'"></script>';
							}
		...
		if($USER->IsAuthorized() && $APPLICATION->GetShowIncludeAreas())
		{
			if(CAdvContract::IsManager() || CAdvContract::IsAdmin())
			{
					...
					if (!empty($arBanner) && isset($arBanner["ID"]))
					{
						$curIcon = array(
								"URL" => 'javascript:'.$APPLICATION->GetPopupLink(
												array(
														'URL' => "/thurly/admin/adv_banner_edit.php?bxpublic=Y&from_module=advertising&lang=".LANGUAGE_ID."&ID=".$arBanner["ID"]. "&CONTRACT_ID=".$arBanner["CONTRACT_ID"],
														'PARAMS' => array(
																'width' => 700,
																'height' => 400,
																'resize' => false,
				...
				else
				{
						...
						foreach($arContracts as $arContract)
						{
							if (array_key_exists("ALL", $arContractTypes[$arContract["ID"]]) || array_key_exists($TYPE_SID, $arContractTypes[$arContract["ID"]]))
							{
								$arSubMenu[] = array(
										"URL" => 'javascript:'.$APPLICATION->GetPopupLink(
														array(
																'URL' => "/thurly/admin/adv_banner_edit.php?bxpublic=Y&from_module=advertising&lang=".LANGUAGE_ID."&TYPE_SID=".$TYPE_SID."&CONTRACT_ID=".$arContract["ID"],
																'PARAMS' => array(
																		'width' => 700,
																		'height' => 400,
																		'resize' => false,
					...
					$arIcons[] = array(
							"URL" => "/thurly/admin/adv_banner_list.php?lang=".LANGUAGE_ID."&find_contract_id[]=".$arBanner["CONTRACT_ID"]. "&find_type_sid[]=".$arBanner["TYPE_SID"]."&set_filter=Y",
							"SRC" => "/thurly/themes/.default/icons/advertising/comp_view.gif",
