...
if ((strlen($save)>0 || strlen($apply)>0) && check_thurly_sessid() && $REQUEST_METHOD=="POST")
{
	$arrWEEKDAY = array(
		"SUNDAY"	=> $arrSUNDAY,
		"MONDAY"	=> $arrMONDAY,
	...
	if ($isAdmin || $isDemo)
	{
		...
		if ($ID>1)
		{
			$arMenuActions[] = array(
				...
				"LINK"	=> "javascript:if(confirm('".GetMessage("AD_DELETE_CONTRACT_CONFIRM")."'))window.location='adv_contract_list.php?ID=".$ID."&lang=".LANGUAGE_ID."&action=delete&sessid=".thurly_sessid()."';",
			);
		}
	}
		...
		<td>
			<table cellspacing=1 cellpadding=0 border=0>
				...
				<tr>
					...
					<td><a href="/thurly/admin/adv_banner_list.php?find_contract_id[]=<?echo $ID?>&find_status_sid[]=<?echo $status_sid?>&set_filter=Y&lang=<?=LANGUAGE_ID?>" title='<?=GetMessage("AD_BANNER_ALT")?>'><?=$count?></a></td>
				</tr>
				<? endwhile;?>
				<tr>
					<td><b><?=GetMessage("AD_TOTAL")?>&nbsp;</b></td>
					<td><a href="/thurly/admin/adv_banner_list.php?find_contract_id[]=<?echo $ID?>&set_filter=Y&lang=<?=LANGUAGE_ID?>" title='<?=GetMessage("AD_BANNER_ALT")?>'><?echo $str_BANNER_COUNT?></a></td>
				</tr>
			</table></td>
	</tr>
	<?endif;?>
...
<?$tabControl->BeginNextTab();?>
			...
			if ($isEditMode) :?>
				...
				while(list($sid, $arrS) = each($arrSites)):
					...
					<div class="adm-list-item">
						...
						<div class="adm-list-label"><?echo '[<a href="/thurly/admin/site_edit.php?LID='.urlencode($sid).'&lang='.LANGUAGE_ID.'" title="'.GetMessage("AD_SITE_ALT").'">'.htmlspecialcharsex($sid).'</a>]&nbsp;<label for="site_'.htmlspecialcharsbx($sid).'">'.htmlspecialcharsex($arrS["NAME"])?></label></div>
					</div>
				<?endwhile;?>
				</div>
			<?else:
	...
	<tr valign="top">
		...
		<td width="60%">
			<div class="adm-list">
				<div class="adm-list-item">
					<div class="adm-list-control"><input name="arrTYPE[]" type="checkbox" value="ALL" onclick="OnSelectAll_typies(this.checked)" <?if ($ID>0 && in_array("ALL", $arrTYPE) || $ID<=0) echo 'checked="checked"'?> id="alltypies"></div>
					<div class="adm-list-label"><label for="alltypies"><?=GetMessage("AD_ALL_TYPIES")?></label></div>
				</div>
		...
		while ($arType = $rsType->GetNext())
		{
			...
			<div class="adm-list-item">
				...
				<div class="adm-list-label">
					<?="[<a href='/thurly/admin/adv_type_edit.php?lang=".LANGUAGE_ID."&SID=".$arType["SID"]."&action=view' title='".GetMessage("AD_TYPE_ALT")."'>".$arType["SID"]."</a>] "."<label for='arType_".$i."'>".$arType["NAME"]."</label>"?>
				</div>
			</div>
			<?
			$i++;
	...
	<tr valign="top">
		...
		<td><?
			...
			foreach ($arContractTypes as $sid => $name):
				if ($sid == "ALL") continue;
			?>
				[<a href="/thurly/admin/adv_type_edit.php?lang=<?=LANGUAGE_ID?>&SID=<?=$sid?>&action=view" title="<?=GetMessage("AD_TYPE_ALT")?>"><?=$sid?></a>]
...
<?$tabControl->BeginNextTab();?>
	...
	?>
	<tr valign="top">
		<td width="40%"><?=GetMessage("AD_VIEW_STATISTICS")?><br><IMG SRC="/thurly/images/advertising/mouse.gif" WIDTH="44" HEIGHT="21" BORDER=0 ALT=""></td>
		<td width="60%"><?
	...
	<?if ($isEditRightsMode):?>
	<tr valign="top">
		<td><?=GetMessage("AD_MANAGE_BANNERS")?><br><IMG SRC="/thurly/images/advertising/mouse.gif" WIDTH="44" HEIGHT="21" BORDER=0 ALT=""></td>
		<td><?
	...
	<?if ($isEditMode && ($isAdmin || ($isDemo && !$isOwner))):?>
	<tr valign="top">
		<td><?=GetMessage("AD_EDIT_CONTRACT")?><br><IMG SRC="/thurly/images/advertising/mouse.gif" WIDTH="44" HEIGHT="21" BORDER=0 ALT=""></td>
