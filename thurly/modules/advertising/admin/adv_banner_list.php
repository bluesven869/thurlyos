...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if (is_array($arrSITE))
	{
		foreach($arrSITE as $sid)
		{
			...
			{
				if ($isAdmin)
					$sites .= htmlspecialcharsbx($arrSites[$sid]["NAME"]).' [<a href="/thurly/admin/site_edit.php?LID='.htmlspecialcharsbx($sid).'&amp;lang='.LANGUAGE_ID.'" title="'.GetMessage("ADV_SITE_VIEW").'">'.htmlspecialcharsbx($sid).'</a>]<br>';
				else
