...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	if (is_array($arrSITE)):
		foreach($arrSITE as $sid):
			if ($isAdmin)
				$sites .= '<a href="/thurly/admin/site_edit.php?LID='.htmlspecialcharsbx($sid).'&amp;lang='.LANGUAGE_ID.'" title="'.GetMessage("ADV_SITE_VIEW").'">'.htmlspecialcharsbx($arrSites[$sid]["NAME"]).'</a><br>';
			else
	...
	$row->AddViewField("BANNER_COUNT", '<a href="/thurly/admin/adv_banner_list.php?find_contract_id[]='.$f_ID.'&set_filter=Y" title="'.GetMessage("ADV_BANNER_LIST").'">'.$f_BANNER_COUNT.'</a>');
...
<tr>
	<td valign="top"><?=GetMessage("AD_F_SITE")?>:<br><img src="/thurly/images/advertising/mouse.gif" width="44" height="21" border=0 alt=""></td>
	<td><?
	$ref = array();
	$ref_id = array();
