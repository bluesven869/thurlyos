...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/advertising/prolog.php");
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/advertising/include.php");
	...
	if (strlen($_POST["name"]) > 0)
	{
		...
		if (count($properties['parameters']['PROPS']) > 0)
		{
			foreach ($properties['parameters']['PROPS'] as $id => $prop)
			{
				...
				$arPropsTemplate[$id] = CComponentUtil::GetTemplateProps('thurly:advertising.banner.view', $_POST["name"], '', $arCurVal[$id]);
		...
		else if ($_POST["action"] == 'refreshAll')
		{
			if (empty($arCurVal))
			{
				...
				$arPropsTemplate[0] = CComponentUtil::GetTemplateProps('thurly:advertising.banner.view', $_POST["name"], '', $arCurVal);
			...
			else
			{
				...
				{
					$arPropsTemplate[$id] = CComponentUtil::GetTemplateProps('thurly:advertising.banner.view', $_POST["name"], '', $curVal);
					uasort($arPropsTemplate[$id]["PARAMETERS"], 'pr_comp');
				}
			}
		}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["action"] == 'getCleanTemplate' && isset($_POST["name"]) && check_thurly_sessid())
{
	$GLOBALS['APPLICATION']->RestartBuffer();
	if (strlen($_POST["name"]) > 0)
	{
		$i = intval($_POST["index"]);
		$arPropsTemplate = CComponentUtil::GetTemplateProps('thurly:advertising.banner.view', $_POST["name"], '', array('EXTENDED_MODE' => $_POST["mode"]));
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && ($_POST["save"] <> '' || $_POST["apply"] <> '') && check_thurly_sessid())
{
	...
	if ($ID = CAdvBanner::Set($arFields, $ID))
	{
		...
		{
			if ($_POST["save"] <> '')
				LocalRedirect("/thurly/admin/adv_banner_list.php?lang=".LANGUAGE_ID);
			else
				LocalRedirect("/thurly/admin/adv_banner_edit.php?ID=".$ID."&CONTRACT_ID=".$CONTRACT_ID."&lang=".LANGUAGE_ID."&action=".$action."&".$tabControl->ActiveTabParam());
		}
	}
...
if ($str_AD_TYPE == 'template')
{
	...
	if (is_array($arCurVal) && !empty($arCurVal))
	{
		foreach ($arCurVal as $id => $prop)
		{
			...
			$arPropsTemplate[$id] = CComponentUtil::GetTemplateProps('thurly:advertising.banner.view', $templateName, '', $arCurVal[$id]);
			uasort($arPropsTemplate[$id]["PARAMETERS"], 'pr_comp');
		}
	}
	else
	{
		...
		$arPropsTemplate[0] = CComponentUtil::GetTemplateProps('thurly:advertising.banner.view', $templateName, '', $arCurVal);
...
if (intval($ID) > 0 && !$bCopy)
{
	...
	if ($isAdmin || ($isDemo && !$isOwner) || $isManager || ($isAdvertiser && in_array("ADD", $arrPERM)))
	{
		...
		$arMenuActions[] = array(
			...
			"LINK"	=> "javascript:if (confirm('".GetMessage("AD_DELETE_BANNER_CONFIRM")."'))window.location='adv_banner_list.php?ID=".$ID."&lang=".LANGUAGE_ID."&sessid=".thurly_sessid()."&action=delete';",
			"ICON"	=> "btn_delete"
		);
	}
}
...
<? if (strlen($strError) > 0)
	...
	<form name="bx_adv_edit_form" method="POST" action="<?=$APPLICATION->GetCurPage()?>" enctype="multipart/form-data">
				...
				<tr valign="top">
					...
					<td width="60%"><?=$str_DATE_CREATE?><?
						if (intval($str_CREATED_BY) > 0) :
							...
							$arUser = $rsUser->Fetch();
							echo "&nbsp;&nbsp;[<a href='/thurly/admin/user_edit.php?ID=".$str_CREATED_BY."&lang=".LANGUAGE_ID."' title='".GetMessage("AD_USER_ALT")."'>".$str_CREATED_BY."</a>]&nbsp;(".htmlspecialcharsbx($arUser["LOGIN"]).") ".htmlspecialcharsbx($arUser["NAME"])." ".htmlspecialcharsbx($arUser["LAST_NAME"]);
						endif;
						?></td>
				</tr>
			<? endif ?>
				...
				<tr valign="top">
					...
					<td><?=$str_DATE_MODIFY?><?
						if (intval($str_MODIFIED_BY) > 0) :
							...
							$arUser = $rsUser->Fetch();
							echo "&nbsp;&nbsp;[<a href='/thurly/admin/user_edit.php?ID=".$str_MODIFIED_BY."&lang=".LANGUAGE_ID."' title='".GetMessage("AD_USER_ALT")."'>".$str_MODIFIED_BY."</a>]&nbsp;(".htmlspecialcharsbx($arUser["LOGIN"]).") ".htmlspecialcharsbx($arUser["NAME"])." ".htmlspecialcharsbx($arUser["LAST_NAME"]);
						endif;
						?></td>
				</tr>
			<? endif ?>
		...
		<tr<? if (!$isEditMode) echo ' style="display: none;"'; ?> id='banner_type' valign="top">
			...
			<td align="left">
				<select onchange='setType(this);' name="AD_TYPE">
					...
					<?$arTemplates = CComponentUtil::GetTemplatesList('thurly:advertising.banner.view');?>
		...
		<?
		$APPLICATION->IncludeComponent('thurly:main.colorpicker', '', array('SHOW_BUTTON' => 'N'), false, array('HIDE_ICONS' => 'Y'));

		CJSCore::RegisterExt('adv_dragdrop', array('js' => '/thurly/js/main/core/core_dragdrop.js'));
		...
		<tr id="eTemplateComponent" valign="top" style="display: none;">
			<? if ((!defined('BX_PUBLIC_MODE') || BX_PUBLIC_MODE != 1) && COption::GetOptionString('advertising', 'SHOW_COMPONENT_PREVIEW') == "Y"): ?>
				<td align="center" colspan="2">
					...
					<iframe src="adv_banner_preview.php?id=<?=intval($_GET['ID'])?>&name=<?=$str_TEMPLATE["NAME"]?>&thurly_include_areas=N" id="componentIframe" style="width:100%;display:none;" scrolling="no" frameBorder="0" onLoad="iFrameAutoResize('componentIframe');"></iframe>
				</td>
			<? endif ?>
		</tr>
							...
							while (list($sid, $arrS) = each($arrSites)):
								if (in_array($sid, $arrContractSite)) :
									...
									<div class="adm-list-item">
										...
										<div class="adm-list-label"><?='[<a href="/thurly/admin/site_edit.php?LID='.urlencode($sid).'&lang='.LANGUAGE_ID.'" title="'.GetMessage("AD_SITE_ALT").'">'.htmlspecialcharsex($sid).'</a>]&nbsp;<label for="site_'.htmlspecialcharsbx($sid).'">'.htmlspecialcharsex($arrS["NAME"])?></label></div>
									</div>
									<?
								endif;
							endwhile;?>
					...
					<? else:
						...
						if (is_array($arrSITE)):
							foreach ($arrSITE as $sid):
								if (in_array($sid, $arrContractSite)) :
									$arS = $arrSites[$sid];
									echo '[<a href="/thurly/admin/site_edit.php?LID='.urlencode($arS["ID"]).'&lang='.LANGUAGE_ID.'" title="'.GetMessage("AD_SITE_ALT").'">'.htmlspecialcharsex($arS["ID"]).'</a>] '.htmlspecialcharsex($arS["NAME"]).'<br>';
								endif;
							endforeach;
						endif;
		...
		<? if ($isEditMode):
			...
			?>
			<tr valign="top">
				<td><?=GetMessage("AD_USER_GROUPS");?><br><img src="/thurly/images/advertising/mouse.gif" width="44" height="21" border=0 alt=""><br><?=GetMessage("AD_SELECT_WHAT_YOU_NEED")?></td>
				<td>
		...
		if (CModule::IncludeModule("statistic")):
				...
				<td>
					...
					<? if ($isEditMode): ?>
						<script>
							...
							{
								if (V_STAT_TYPE == 'CITY')
									jsUtils.OpenWindow('/thurly/admin/stat_city_multiselect.php?lang=<?=LANGUAGE_ID?>&form=bx_adv_edit_form&field=STAT_TYPE_VALUES[]', 600, 600);
								else if (V_STAT_TYPE == 'REGION')
									jsUtils.OpenWindow('/thurly/admin/stat_region_multiselect.php?lang=<?=LANGUAGE_ID?>&form=bx_adv_edit_form&field=STAT_TYPE_VALUES[]', 600, 600);
								else
									jsUtils.OpenWindow('/thurly/admin/stat_country_multiselect.php?lang=<?=LANGUAGE_ID?>&form=bx_adv_edit_form&field=STAT_TYPE_VALUES[]', 600, 600);
							}
						</script>
			...
			if ($isAdmin || ($isDemo && !$isOwner)):
				...
				if ($isEditMode):
					?>
					<tr valign="top">
						<td><?=GetMessage("AD_STAT_ADV")?><br><img src="/thurly/images/advertising/mouse.gif" width="44" height="21" border=0 alt=""><br><?=GetMessage("AD_SELECT_WHAT_YOU_NEED")?></td>
		...
		$tabControl->Buttons(array("disabled" => $disable, "back_url"=>"/thurly/admin/adv_banner_list.php?lang=".LANGUAGE_ID));
		$tabControl->End();
		?>
	</form>
	<script>
