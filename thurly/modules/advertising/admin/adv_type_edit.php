...
if(strlen($SID)>0)
{
	...
	if ($isAdmin || $isDemo)
	{
		...
		$aMenu[] = array(
			...
			"LINK"	=> "javascript:if(confirm('".GetMessage("AD_DELETE_TYPE_CONFIRM")."'))window.location='adv_type_list.php?ID=".$SID."&lang=".LANGUAGE_ID."&action=delete&sessid=".thurly_sessid()."';",
			"ICON"	=> "btn_delete"
			);
	}
}
...
?>
	...
	<tr valign="top">
		...
		if (intval($str_CREATED_BY)>0) :
			...
			$arUser = $rsUser->Fetch();
			echo "&nbsp;&nbsp;[<a href='/thurly/admin/user_edit.php?ID=".$str_CREATED_BY."' title='".GetMessage("AD_USER_ALT")."'>".$str_CREATED_BY."</a>]&nbsp;(".htmlspecialcharsbx($arUser["LOGIN"]).") ".htmlspecialcharsbx($arUser["NAME"])." ".htmlspecialcharsbx($arUser["LAST_NAME"]);
		endif;
		?></td>
	</tr>
	<?endif;?>
	...
	<tr valign="top">
		...
		if (intval($str_MODIFIED_BY)>0) :
			...
			$arUser = $rsUser->Fetch();
			echo "&nbsp;&nbsp;[<a href='/thurly/admin/user_edit.php?ID=".$str_MODIFIED_BY."' title='".GetMessage("AD_USER_ALT")."'>".$str_MODIFIED_BY."</a>]&nbsp;(".htmlspecialcharsbx($arUser["LOGIN"]).") ".htmlspecialcharsbx($arUser["NAME"])." ".htmlspecialcharsbx($arUser["LAST_NAME"]);
		endif;
		?></td>
	</tr>
	<?endif;?>
