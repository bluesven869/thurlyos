				...
				foreach($arShow as $ctype) :
					...
					if ( ${"sum_".strtolower($counter_type)}>0):
						...
						<table cellspacing=0 cellpadding=2 class="graph">
							<tr>
								<td valign="top"><img class="graph" src="/thurly/admin/adv_diagram.php?<?=GetFilterParams($FilterArr)?>&diagram_type=<?echo $diagram_type?>&counter_type=<?echo $counter_type?>" width="<?echo $diameter?>" height="<?echo $diameter?>">
								</td>
								<td valign="top">
									<table cellpadding=0 cellspacing=0 border=0 class="legend">
										...
										<tr>
											...
											<td  nowrap><?echo "[<a href='/thurly/admin/adv_contract_edit.php?find_id=".$arrS["ID"]."&lang=".LANGUAGE_ID."&action=view' title='".GetMessage("AD_CONTRACT_VIEW")."'>".$arrS["ID"]."</a>] ".$arrS["NAME"];?></td>
										</tr>
										<?
											endif;
										endwhile;
