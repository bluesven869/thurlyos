			...
			if (!function_exists("ImageCreate")) :
				...
				foreach($arShow as $ctype) :
					...
					<table cellspacing=0 cellpadding=0 class="graph">
						...
						<tr>
							<td valign="top"><img class="graph" src="/thurly/admin/adv_diagram.php?<?=GetFilterParams($FilterArr)?>&diagram_type=<?echo $diagram_type?>&counter_type=<?echo $counter_type?>" width="<?echo $diameter?>" height="<?echo $diameter?>"></td>
							<td valign="top">
								<table cellpadding=0 cellspacing=0 border=0 class="legend">
									...
									<tr>
										...
										<td valign="center">
											<div style="background-color: <?="#".$color?>"><img src="/thurly/images/1.gif" width="12" height="12" border=0></div>
										</td>
										<td align="right" nowrap><?echo sprintf("%01.2f", $procent)."%"?></td>
										<td nowrap>(<?=$counter?>)</td>
										<td  nowrap><?echo '[<a href="/thurly/admin/adv_banner_edit.php?ID='.$arrS["ID"].'&lang='.LANGUAGE_ID.'&action=view" title="'.GetMessage("AD_BANNER_VIEW").'">'.$arrS["ID"].'</a>] '.htmlspecialcharsEx($arrS["NAME"]);?>
										</td>
									</tr>
									<?
										endif;
