...
if($isPost && isset($_POST['IMPORT_REPORT']))
{
	if(is_uploaded_file($_FILES['IMPORT_REPORT_FILE']['tmp_name']))
	{
		...
		if(!empty($reportData) && is_array($reportData))
		{
			...
			Thurly\Report\ReportTable::add($report);
		}
		else
		{
...
$result = Thurly\Report\ReportTable::getList(array(
	'order' => array('ID' => 'ASC'),
...
$result = Thurly\Report\Internals\SharingTable::getList(array(
...
$queryObject = Thurly\Report\ReportTable::getList(array(
