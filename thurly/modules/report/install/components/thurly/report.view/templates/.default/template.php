...
/**
 * @param CThurlyComponentTemplate &$component
...
{
	$GLOBALS['APPLICATION']->SetAdditionalCSS('/thurly/js/report/css/report.css');
...
if ($arParams['USE_CHART'] && $arResult['settings']['chart']['display'])
{
	...
	// amCharts
	$GLOBALS['APPLICATION']->AddHeadScript('/thurly/js/main/amcharts/3.3/amcharts.js');
	$GLOBALS['APPLICATION']->AddHeadScript('/thurly/js/main/amcharts/3.3/serial.js');
	$GLOBALS['APPLICATION']->AddHeadScript('/thurly/js/main/amcharts/3.3/pie.js');
}
...
function getResultColumnDataType(&$viewColumnInfo, &$customColumnTypes = array(), $helperClassName)
{
	...
	{
		/** @var Thurly\Main\Entity\Field[] $viewColumnInfo */
...
<div class="reports-result-list-wrap">
	...
	<script type="text/javascript">
		...
		<? if (\Thurly\Main\Page\Frame::isAjaxRequest()):?>
		drawChart();
		<? else: ?>
...
<div id="report-chfilter-examples" style="display: none;">
	...
	<div class="filter-field filter-field-user chfilter-field-\Thurly\Main\User" callback="RTFilter_chooseUser">
	...
	<div class="filter-field filter-field-user chfilter-field-\Thurly\Socialnetwork\Workgroup" callback="RTFilter_chooseGroup">
		<label for="user-email" class="filter-field-title">%TITLE% "%COMPARE%"</label>
		<span class="webform-field-textbox-inner">
			<input id="%ID%" type="text" class="webform-field-textbox" caller="true" />
			<input type="hidden" name="%NAME%" value=""/>
	...
	<div class="filter-field chfilter-field-datetime">
		...
		<input type="text" value="%VALUE%" name="%NAME%" value="" class="filter-field-calendar" id="" /><a class="filter-date-interval-calendar" href="" title="<?=GetMessage('TASKS_PICK_DATE')?>"><img border="0" src="/thurly/js/main/core/images/calendar-icon.gif" alt="<?=GetMessage('TASKS_PICK_DATE')?>"></a>
	</div>
	...
	<div class="sidebar-block-inner">
		...
		<div class="filter-block filter-field-date-combobox filter-field-date-combobox-interval">
				...
				<span class="filter-date-interval<?php
							...
							?>"><span class="filter-date-interval-from"><input type="text" class="filter-date-interval-from" name="F_DATE_FROM" id="REPORT_INTERVAL_F_DATE_FROM"
									...
									class="filter-date-interval-calendar" href="" title="<?php echo GetMessage("TASKS_PICK_DATE")?>" id="filter-date-interval-calendar-from"><img border="0"
																src="/thurly/js/main/core/images/calendar-icon.gif"
									...
									border="0" src="/thurly/js/main/core/images/calendar-icon.gif"
			...
			<?
				...
				if (!empty($ufInfoUsed))
				{
					$APPLICATION->IncludeComponent(
						'thurly:report.filter.field.selector',
						'',
			...
			BX.ready(function(){
				...
				// User controls
				var controls = BX.findChildren(BX('report-rewrite-filter'), {className: /chfilter-field-(\\Thurly\\Main\\User|employee)/}, true);
				if (controls != null)
				{
					for (i in controls)
					{
				...
				// Group controls
				var controls = BX.findChildren(BX('report-rewrite-filter'), {className:'chfilter-field-\\Thurly\\Socialnetwork\\Workgroup'}, true);
				if (controls != null)
				{
					for (i in controls)
					{
			...
			foreach ($arResult['changeableFilters'] as $chFilter)
			{
				/** @var \Thurly\Main\Entity\ReferenceField[] $chFilter */
				if (($chFilter['field'] && $chFilter['field'] instanceof \Thurly\Main\Entity\ReferenceField
						&& $chFilter['field']->getRefEntityName() == '\Thurly\Main\User')
				...
				else if ($chFilter['field'] && $chFilter['field'] instanceof \Thurly\Main\Entity\ReferenceField
					&& $chFilter['field']->getRefEntityName() == '\Thurly\Socialnetwork\Workgroup')
				{
					// group selector
					$name = $APPLICATION->IncludeComponent(
						"thurly:socialnetwork.group.selector",
						".default",
						array(
