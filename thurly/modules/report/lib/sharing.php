...
use Thurly\Report\Internals\SharingTable;
...
class Sharing
{
	...
	public static function getUserData($userId)
	{
		...
		if ($user)
		{
			...
			$userData['avatar'] = $avatar ? $avatar['src'] : '/thurly/js/report/css/images/default_avatar.png';
		...
		elseif($type == self::TYPE_SOCNET_GROUP)
		{
			if(Loader::includeModule('socialnetwork'))
			{
				...
				while($group = $query->fetch())
				{
					...
					$typeData['avatar'] = $avatar?$avatar['src']:'/thurly/js/report/css/images/default_groupe.png';
				}
			}
		}
		elseif($type == self::TYPE_DEPARTMENT)
		{
			if(Loader::includeModule('socialnetwork'))
			{
				...
				if(!empty($structure['department']['DR' . $id]))
				{
					...
					$typeData['avatar'] = '/thurly/js/report/css/images/default_groupe.png';
				}
			}
		}
