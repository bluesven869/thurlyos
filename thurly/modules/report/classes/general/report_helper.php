...
abstract class CReportHelper
{
	...
	{
		throw new \Thurly\Main\SystemException('Method "getEntityName" must be defined in child class.');
	}

	public static function getOwnerId()
	{
		throw new \Thurly\Main\SystemException('Method "getOwnerId" must be defined in child class.');
	}

	public static function getColumnList()
	{
		throw new \Thurly\Main\SystemException('Method "getColumnList" must be defined in child class.');
	}

	public static function getDefaultColumns()
	{
		throw new \Thurly\Main\SystemException('Method "getDefaultColumns" must be defined in child class.');
	}

	public static function getPeriodFilter($date_from, $date_to)
	{
		throw new \Thurly\Main\SystemException('Method "getPeriodFilter" must be defined in child class.');
	}
	...
	public static function detectUserField($field)
	{
		...
		if ($field instanceof \Thurly\Main\Entity\ExpressionField && is_array(self::$ufInfo) && count(self::$ufInfo) > 0)
		{
			$ufKey = $field->getName();
	...
	public static function getCompareVariations()
	{
		return array(
			...
			'\Thurly\Socialnetwork\Workgroup' => array(
				'EQUAL'
			),
			'enum' => array(
				'EQUAL',
	...
	public static function fillFilterReferenceColumn(&$filterElement, Entity\ReferenceField $field)
	{
		...
		else if ($field->getRefEntityName() == '\Thurly\Socialnetwork\Workgroup')
		{
			// GROUP
			if ($filterElement['value'])
			{
	...
	public static function collectUFValues($rows, $columnInfo, $total)
	{
		...
		if (count($employeeColumns) > 0)
		{
			...
			foreach ($arEmployeeID as $employeeID)
			{
				...
				if ($cnt === $stepCnt || $i === $nIDs)
				{
					$res = \Thurly\Main\UserTable::getList(
						array(
							'filter' => array('ID' => $arID),
	...
	public static function formatResultValue($k, &$v, &$row, &$cInfo, $total, &$customChartValue = null)
	{
		...
		{
			$v = ($v instanceof \Thurly\Main\Type\DateTime || $v instanceof \Thurly\Main\Type\Date) ? ConvertTimeStamp($v->getTimestamp(), 'SHORT') : '';
		}
