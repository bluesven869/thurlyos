...
if ($params['BX_IFRAME'] == 'Y')
{
	if ($params['BX_IFRAME_ACTION'] == 'REGISTER')
	{
		...
		if (\Thurly\Im\App::getUserHash($params['USER_ID'], $hash) != $params['USER_HASH'])
		{
			echo "Code: 403";
			die();
		}
		
		$apps = \Thurly\Im\App::update(
...
)
{
	...
	{
		$result = \Thurly\ImBot\Controller::sendToService($params['BX_SERVICE_NAME'], $params['BX_COMMAND'], $params);
	}
	else
	{
		$result = \Thurly\ImBot\Controller::sendToBot($params['BX_BOT_NAME'], $params['BX_COMMAND'], $params);
	}
	if (is_null($result))
	{
