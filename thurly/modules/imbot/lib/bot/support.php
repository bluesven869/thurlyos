...
{
	class Support extends \Thurly\ThurlyOS\SupportBot
	{
	}
}
else
	...
	class Support extends Network
	{
		...
		public static function register(array $params = Array())
		{
			...
			if ($botId)
			{
				...
				$user = new \CUser;
				$user->Update($botId, array('PERSONAL_PHOTO' => self::uploadAvatar('http://helpdesk.thurlyos.com/images/support/bot.png')));

				$rs = \CUser::GetList($by = 'id', $order = 'asc', $arFilter = array('ACTIVE' => 'Y', 'GROUPS_ID' => 1));
				while($f = $rs->Fetch())
					\CIMMessage::GetChatId($f['ID'], $botId);

				\Thurly\ImBot\Bot\Network::removeFdc(0);
			}
		...
		{
			if (!$url = \Thurly\Main\Config\Option::get(self::MODULE_ID, "portal_url", ""))
				return false;
			if (!$ar = parse_url($url))
				return false;
			$host = $ar['host'];
			...
			$httpClient->setHeader('User-Agent', 'Thurly Support Bot');
		...
		public static function onChatStart($dialogId, $joinFields)
		{
			...
			\Thurly\Im\Bot::addMessage(Array('BOT_ID' => $messageFields['BOT_ID']), Array(
				...
				'MESSAGE' => Loc::getMessage('SUPPORT_WELCOME_MESSAGE', array('#USERNAME#' => \Thurly\Im\User::getInstance($messageFields['USER_ID'])->getFullName(false))),
				'SYSTEM' => 'N',
				'URL_PREVIEW' => 'N'
			));
			return true;
		...
		{
			return \Thurly\ImBot\Bot\Network::getNetworkBotId(self::getCode());
		}

		public static function getCode()
		{
			if ($f = \Thurly\Main\Localization\CultureTable::getList(array('filter' => array('=CODE' => 'ru')))->fetch())
