...
class Designer
{
	...
	public function __construct($portalId, $dialogId, $botId, $userId)
	{
		...
		\Thurly\Main\Loader::includeModule('imopenlines');
		...
		$this->userData = \Thurly\Im\User::getInstance($userId);
	...
	private function getCurrentMenu()
	{
		...
		if (\Thurly\Main\IO\File::isFileExists($_SERVER['DOCUMENT_ROOT'].'/upload/imopenlines/itr/'.$this->cacheId.'.cache'))
		{
			$this->currentMenu = intval(\Thurly\Main\IO\File::getFileContents($_SERVER['DOCUMENT_ROOT'].'/upload/imopenlines/itr/'.$this->cacheId.'.cache'));
		}
	}
	...
	private function setCurrentMenu($id)
	{
		...
		\Thurly\Main\IO\File::putFileContents($_SERVER['DOCUMENT_ROOT'].'/upload/imopenlines/itr/'.$this->cacheId.'.cache', $this->currentMenu);
	}
	...
	private function execMenuItem($itemId = '')
	{
		...
		if (isset($menuItemAction['TEXT']))
		{
			...
			\Thurly\Im\Bot::addMessage(Array('BOT_ID' => $this->botId), Array(
		...
		{
			$chat = new \Thurly\Imopenlines\Chat(substr($this->dialogId, 4));
			$chat->endBotSession();
		}
		else if ($menuItemAction['TYPE'] == Item::TYPE_USER)
		{
			...
			$chat->transfer(Array(
				...
				'MODE' => \Thurly\Imopenlines\Chat::TRANSFER_MODE_BOT,
