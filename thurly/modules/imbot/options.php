...
if(strlen($_POST['Update'])>0 && check_thurly_sessid())
{
	...
	else if(strlen($_POST['Update'])>0)
	{
		...
		{
			if (!\Thurly\ImBot\Bot\Giphy::getBotId())
			{
		...
		else
		{
			if (\Thurly\ImBot\Bot\Giphy::getBotId())
			{
				\Thurly\ImBot\Bot\Giphy::unRegister();
			}
		}
		if (isset($_POST['BOT_PROPERTIES']))
		{
			if (!\Thurly\ImBot\Bot\Properties::getBotId())
			{
		...
		else
		{
			if (\Thurly\ImBot\Bot\Properties::getBotId())
			{
				\Thurly\ImBot\Bot\Properties::unRegister();
			}
		}
		if (isset($_POST['BOT_PROPERTIESUA']))
		{
			if (!\Thurly\ImBot\Bot\PropertiesUa::getBotId())
			{
		...
		else
		{
			if (\Thurly\ImBot\Bot\PropertiesUa::getBotId())
			{
				\Thurly\ImBot\Bot\PropertiesUa::unRegister();
			}
		}
		...
		if (!\CModule::IncludeModule('thurlyos'))
		{
			...
			{
				if (!\Thurly\ImBot\Bot\Support::checkPublicUrl())
				{
					$APPLICATION->ThrowException(\Thurly\Main\Localization\Loc::getMessage('SUPPORT_ERROR_URL'));
					\Thurly\ImBot\Bot\Support::unRegister();
				}
				elseif (!\Thurly\ImBot\Bot\Support::getBotId())
				{
					\Thurly\ImBot\Bot\Support::register();
				}
			}
			else
			{
				if (\Thurly\ImBot\Bot\Support::getBotId())
				{
					\Thurly\ImBot\Bot\Support::unRegister();
				}
			}
...
<tr>
	...
	<td width="60%"><input type="text" name="PUBLIC_URL" value="<?=htmlspecialcharsbx(\Thurly\ImBot\Http::getServerAddress())?>" /></td>
</tr>
...
<tr>
	<td width="40%"><?=\Thurly\ImBot\Bot\Giphy::getLangMessage('IMBOT_GIPHY_BOT_NAME')?>:</td>
	<td width="60%"><input type="checkbox" name="BOT_GIPHY" value="Y" <?=(\Thurly\ImBot\Bot\Giphy::getBotId()? 'checked':'')?> /></td>
</tr>
<tr>
	<td width="40%"><?=\Thurly\ImBot\Bot\Properties::getLangMessage('IMBOT_PROPERTIES_BOT_NAME')?>:</td>
	<td width="60%"><input type="checkbox" name="BOT_PROPERTIES" value="Y" <?=(\Thurly\ImBot\Bot\Properties::getBotId()? 'checked':'')?> /></td>
</tr>
<tr>
	<td width="40%"><?=\Thurly\ImBot\Bot\PropertiesUa::getLangMessage('IMBOT_PROPERTIESUA_BOT_NAME').' ('.\Thurly\Main\Localization\Loc::getMessage('IMBOT_BOT_POSTFIX_UA').')'?>:</td>
	<td width="60%"><input type="checkbox" name="BOT_PROPERTIESUA" value="Y" <?=(\Thurly\ImBot\Bot\PropertiesUa::getBotId()? 'checked':'')?> /></td>
</tr>
<? if (!\CModule::IncludeModule('thurlyos')): ?>
<tr>
	<td width="40%"><?=\Thurly\ImBot\Bot\Properties::getLangMessage('IMBOT_SUPPORT_BOT_NAME')?>:</td>
	<td width="60%"><input type="checkbox" name="BOT_SUPPORT" value="Y" <?=(\Thurly\ImBot\Bot\Support::getBotId()? 'checked':'')?> /></td>
</tr>
<?endif;?>
<?$tabControl->Buttons();?>
