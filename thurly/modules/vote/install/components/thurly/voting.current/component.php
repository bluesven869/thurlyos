...
else if ($arParams["PERMISSION"] >= 4 && $arParams["VOTE_ID"] > 0 && check_thurly_sessid())
{
	if ($this->request->getPost("stopVoting") == $arParams["VOTE_ID"])
	{
		\Thurly\Vote\Vote::loadFromId($arParams["VOTE_ID"])->stop();
		$arResult["VOTE"]["LAMP"] = "red";
	}
	else if ($this->request->getPost("resumeVoting") == $arParams["VOTE_ID"])
	{
		\Thurly\Vote\Vote::loadFromId($arParams["VOTE_ID"])->resume();
		$arResult["VOTE"]["LAMP"] = "green";
	}
	else if ($this->request->getQuery("exportVoting") == $arParams["VOTE_ID"])
	{
		\Thurly\Vote\Vote::loadFromId($arParams["VOTE_ID"])->exportExcel();
	}
}
$arParams["VOTED"] = \Thurly\Vote\User::getCurrent()->isVotedFor($arResult["VOTE"]["ID"]);
