;(function(window) {
	...
	if (BX["Vote"])
		return;
	var actionUrl = (window["app"] ? BX.message("SITE_DIR") + "mobile/?mobile_action=vote" : "/thurly/tools/vote/uf.php");
	...
	BX.Vote = (function() {
		...
		d.prototype = {
			...
			exportXls : function (e) {
				BX.eventCancelBubble(e);
				top.location.href = BX.util.add_url_param(this.url, {action : "exportXls", attachId : this.id, sessid : BX.thurly_sessid()});
				return BX.PreventDefault(e);
			},
			...
			checkMobileUsers : function(e) {
				if (this.node && this.node.getAttribute("data-bx-vote-form") !== "shown")
				{
					...
					if (i && parseInt(i.innerHTML) > 0)
					{
						BX.PreventDefault(e);
						app.openBXTable({
							url: BX.util.add_url_param(this.url, {action : "getMobileVoted", attachId : this.id, answerId : node.getAttribute("data-bx-vote-answer"), sessid : BX.thurly_sessid()}),
							TABLE_SETTINGS : {
								markmode : false,
								cache: false
							}
