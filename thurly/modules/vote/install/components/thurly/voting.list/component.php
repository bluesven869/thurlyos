...
if ($db_res)
{
	...
	$votedUser = \Thurly\Vote\User::getCurrent();
...
if ($GLOBALS["APPLICATION"]->GetGroupRight("vote") == "W" && CModule::IncludeModule("intranet") && is_object($GLOBALS['INTRANET_TOOLBAR']))
{
	if ($channelID )
		$GLOBALS['INTRANET_TOOLBAR']->AddButton(array(
			...
			'ICON' => 'add',
			'HREF' => '/thurly/admin/vote_edit.php?lang='.LANGUAGE_ID."&CHANNEL_ID=".$channelID,
			'SORT' => '100',
		));
	$GLOBALS['INTRANET_TOOLBAR']->AddButton(array(
		...
		'ICON' => 'settings',
		'HREF' => '/thurly/admin/vote_list.php?lang='.LANGUAGE_ID,
		'SORT' => '200',
	));
}
