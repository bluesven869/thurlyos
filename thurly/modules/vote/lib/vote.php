...
use Thurly\Vote\Base\BaseObject;
...
class VoteTable extends Entity\DataManager
{
	...
	public static function getMap()
	{
		...
		return array(
			...
			'CHANNEL' => array(
				'data_type' => '\Thurly\Vote\ChannelTable',
				'reference' => array(
			...
			'QUESTION' => array(
				'data_type' => '\Thurly\Vote\QuestionTable',
				'reference' => array(
					'=this.ID' => 'ref.VOTE_ID',
				),
				'join_type' => 'LEFT',
...
class Vote extends BaseObject implements \ArrayAccess
{
	...
	public function getStatistic()
	{
		...
		$dbRes = \Thurly\Vote\EventTable::getList(array(
			'select' => array(
				'V_' => '*',
				'Q_' => 'QUESTION.*',
				'A_' => 'QUESTION.ANSWER.*',
	...
	{
		$format = 'Y-m-d H:i:s';
		VoteTable::update($this->id, array("DATE_END" => new \Thurly\Main\Type\DateTime(date($format, (time() + (365 * 86400))), $format)));
		$this->clearCache();
	}
	...
	{
		$format = 'Y-m-d H:i:s';
		VoteTable::update($this->id, array("DATE_END" => new \Thurly\Main\Type\DateTime(date($format, (time() - 3600)), $format)));
		$this->clearCache();
	}
	...
	public function voteFor(array $request)
	{
		...
		if (!empty($sqlAnswers) && $this->errorCollection->isEmpty())
		{
			...
			$eventFields = array(
				"VOTE_ID"			=> $voteId,
				"VOTE_USER_ID"		=> \Thurly\Vote\User::getCurrent()->setVotedUserId(true),
				"DATE_VOTE"			=> new \Thurly\Main\Type\DateTime(),
			...
			if (($eventId = EventTable::add($eventFields)->getId()) && $eventId > 0)
			{
				...
				if (!empty($ids))
				{
					...
					if (\CModule::IncludeModule("statistic"))
					{
						$event3 = $this["EVENT3"];
						if (!empty($event3)):
							$event3 = "http://" . $_SERVER["HTTP_HOST"] . "/thurly/admin/vote_user_results.php?EVENT_ID=" . $eventId . "&lang=" . LANGUAGE_ID;
						endif;
	...
	public function canRead($userId)
	{
		...
		else if (parent::canRead($userId))
		{
			...
			$dbRes = \Thurly\Vote\Channel::getList(array(
				'select' => array("*"),
				'filter' => array(
					"ACTIVE" => "Y",
					"HIDDEN" => "N",
	...
	{
		throw new \Thurly\Main\NotSupportedException('Model provide ArrayAccess only for reading');
	}
	/**
	 * Is not supported.
	 * @param string $offset Key for vote or attach data array.
