...
use Thurly\Vote\Vote;
use Thurly\Vote\Channel;
use Thurly\Vote\AttachTable;
...
class Attach implements \ArrayAccess
{
	...
	public function save($data, $createdBy = 0)
	{
		...
		if ($voteId > 0)
		{
			...
			list($attach, $vote) = \Thurly\Vote\Attach::getData($id);
			$this->attach = $attach;
			$this->vote = $vote;
		}
	...
	public static function loadFromAttachId($id)
	{
		...
		$res = \Thurly\Vote\Attach::getData($id);
		if (is_null($res))
		{
	...
	public static function loadFromEntity(array $filter)
	{
		...
		$return = array();
		$res = \Thurly\Vote\Attach::getData($filter);
		if (is_array($res))
		{
