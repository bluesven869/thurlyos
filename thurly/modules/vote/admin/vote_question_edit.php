...
$aMenu = array(
	array(
		...
		"LINK"	=> "/thurly/admin/vote_question_list.php?lang=".LANGUAGE_ID."&VOTE_ID=".$voteId,
		"ICON" => "btn_list"));
...
if ($ID > 0)
{
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/vote_question_edit.php?VOTE_ID=$voteId&lang=".LANGUAGE_ID,
		"ICON" => "btn_new");

	$aMenu[] = array(
		"TEXT"	=> GetMessage("VOTE_DELETE"), 
		"TITLE"	=> GetMessage("VOTE_DELETE_RECORD"),
		"LINK"	=> "javascript:if(confirm('".GetMessage("VOTE_DELETE_RECORD_CONFIRM")."')) window.location='/thurly/admin/vote_question_list.php?action=delete&ID=$ID&VOTE_ID=$voteId&".thurly_sessid_get()."&lang=".LANGUAGE_ID."';",
		"ICON" => "btn_delete");
}
...
?>
		<td bgcolor='<?=$t_curCOL?>'><a href='javascript:void(0)' onmousedown='javascript:col_set("<?=$t_curCOL?>")' <?
			?>onmouseover='javascript:col_show("<?=$t_curCOL?>")'><img src=/thurly/images/1.gif border="0" width="10" height="10"></a></td>
<?
}
?>
</table>
...
<tr>
	<td width=50% style="border:1px solid black" id="t_fillCOL"><img src=/thurly/images/1.gif style="width:100%;height:5px"></td>
