...
if ((!empty($save) || !empty($apply)) && $REQUEST_METHOD == "POST" && $VOTE_RIGHT>="W" && check_thurly_sessid())
{
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/thurly/admin/vote_channel_list.php?lang=".LANGUAGE_ID,
		"ICON" => "btn_list"
	)
);
if ($ID>0)
{
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/vote_list.php?lang=".LANGUAGE_ID."&find_channel_id=$ID&set_filter=Y",
	);
	...
	if ($VOTE_RIGHT=="W")
	{
		$aMenu[] = array(
			...
			"LINK"	=> "/thurly/admin/vote_channel_edit.php?lang=".LANGUAGE_ID,
			"ICON" => "btn_new");

		$aMenu[] = array(
			"TEXT"	=> GetMessage("VOTE_DELETE"),
			"TITLE"	=> GetMessage("VOTE_DELETE_RECORD"),
			"LINK"	=> "javascript:if(confirm('".GetMessage("VOTE_DELETE_RECORD_CONFIRM")."')) window.location='/thurly/admin/vote_channel_list.php?action=delete&ID=".$ID."&".thurly_sessid_get()."&lang=".LANGUAGE_ID."';",
			"ICON" => "btn_delete");
	}
	...
	if ($VOTE_RIGHT=="W")
	{
		$aMenu[] = array(
			...
			"LINK"	=> "/thurly/admin/vote_edit.php?lang=".LANGUAGE_ID."&CHANNEL_ID=$ID",
			"ICON" => "btn_new");
	}
}
	...
	<tr valign="top" class="adm-detail-required-field">
		...
		while(list($sid, $arrS) = each($arrSites)):
			...
			<div class="adm-list-item">
				...
				<div class="adm-list-label"><label for="<?=htmlspecialcharsbx($sid)?>"><?echo '[<a title="'.GetMessage("VOTE_SITE_EDIT").'" href="/thurly/admin/site_edit.php?LID='.htmlspecialcharsbx($sid).'&lang='.LANGUAGE_ID.'">'.htmlspecialcharsbx($sid).'</a>]&nbsp;'.htmlspecialcharsex($arrS["NAME"])?></label></div>
			</div>
			<?
		endwhile;
		?></div></td>
