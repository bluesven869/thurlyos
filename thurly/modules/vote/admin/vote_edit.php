...
$db_res = \Thurly\Vote\Channel::getList(array(
	'select' => array("*"),
...
try
{
		...
		($request->getPost("save") || $request->getPost("apply"))))
	{
		if (!check_thurly_sessid())
			throw new \Thurly\Main\ArgumentException("Bad sessid.");
		$action = true;
	...
	{
		$copyVote = \Thurly\Vote\Vote::loadFromId($copyVoteId);
		global $USER;
	...
	/* @var \Thurly\Vote\Channel $channel */
	$channel = \Thurly\Vote\Channel::loadFromId($channelId);
...
if (isset($vote))
{
	...
	$context = new CAdminContextMenu(array(
		array(
			...
			"LINK"	=> "/thurly/admin/vote_user_votes_table.php?lang=".LANGUAGE_ID."&VOTE_ID=".$ID,
			"ICON" => "btn_list"),
		array(
			"TEXT"	=> GetMessage("VOTE_COPY"),
			"TITLE"	=> GetMessage("VOTE_COPY_TITLE"),
			"LINK"	=> "vote_edit.php?lang=".LANGUAGE_ID."&amp;COPY_ID=$ID&".thurly_sessid_get(),
			"ICON" => "btn_copy"),
		array(
			"TEXT"	=> GetMessage("VOTE_DELETE"),
			"TITLE"	=> GetMessage("VOTE_DELETE_RECORD"),
			"LINK"	=> "javascript:if(confirm('".GetMessage("VOTE_DELETE_RECORD_CONFIRM")."')) window.location='/thurly/admin/vote_list.php?action=delete&ID=".$ID."&".thurly_sessid_get()."&lang=".LANGUAGE_ID."';",
			"ICON" => "btn_delete"),
		array(
			"TITLE"	=> GetMessage("VOTE_RESET_RECORD"),
			"LINK"	=> "javascript:if(confirm('".GetMessage("VOTE_RESET_RECORD_CONFIRM")."')) window.location='/thurly/admin/vote_list.php?reset_id=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
	...
	$context = new CAdminContextMenu(array(
		array(
			...
			"LINK"	=> "/thurly/admin/vote_question_list.php?lang=".LANGUAGE_ID."&VOTE_ID=".$ID),
		array(
			"TEXT"	=> GetMessage("VOTE_QUESTIONS_ADD"),
			"TITLE"	=> GetMessage("VOTE_QUESTIONS_ADD_TITLE"),
			"LINK"	=> "/thurly/admin/vote_question_edit.php?lang=".LANGUAGE_ID."&VOTE_ID=$ID",
			"ICON" => "btn_new")));
	$context->Show();
}
if ($message)
...
if ($fields["AUTHOR_ID"] > 0 && ($arAuthor = CUser::GetByID($fields["AUTHOR_ID"])->Fetch()))
{
	...
	<tr>
		...
		<td width="60%">
			<a href="/thurly/admin/user_edit.php?ID=<?=$fields["AUTHOR_ID"]?>&lang=<?=LANG?>"> [<?=$fields["AUTHOR_ID"]?>] <?=$arAuthor["NAME"]?></a>
		</td>
	</tr>
	<tr>
