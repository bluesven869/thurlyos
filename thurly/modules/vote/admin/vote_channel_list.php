...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if(is_array($arrSITE))
	{
		foreach($arrSITE as $sid)
			$str .= '<a title="'.GetMessage("VOTE_SITE_EDIT").'" href="/thurly/admin/site_edit.php?LID='.$sid.'&lang='.LANGUAGE_ID.'">'.$sid.'</a>, ';
	};
	...
	if($f_ID!='1' && $VOTE_RIGHT=="W")
	{
		...
		$arActions[] = array("ICON"=>"delete", "TEXT"=>GetMessage("MAIN_ADMIN_MENU_DELETE"), "ACTION"=>"if(confirm('".GetMessage("VOTE_CONFIRM_DEL_CHANNEL")."')) window.location='vote_channel_list.php?lang=".LANGUAGE_ID."&action=delete&ID=$f_ID&".thurly_sessid_get()."'");
	}
...
<tr valign="top">
	<td><?=GetMessage("VOTE_F_SITE")?><br><img src="/thurly/images/vote/mouse.gif" width="44" height="21" border=0 alt=""></td>
	<td><?
	$ref = array();
	$ref_id = array();
