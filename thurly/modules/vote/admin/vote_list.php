...
/********************************************************************
				ACTIONS
********************************************************************/
if (intval($request->getQuery("reset_id")) > 0 && $VOTE_RIGHT >= "W" && check_thurly_sessid()):
...
if ($lAdmin->EditAction() && $VOTE_RIGHT >= "W" && check_thurly_sessid())
{
...
// Group actions
if(($arID = $lAdmin->GroupAction()) && $VOTE_RIGHT>="W" && check_thurly_sessid())
{
...
while($res = $rsData->getNext())
{
	...
	if (in_array($res["CHANNEL_ID"], $channelsToAdmin))
	{
		...
		$arActions[] = array(
			...
			"ACTION" => "if(confirm('".GetMessage("VOTE_CONFIRM_RESET_VOTE")."')) window.location='vote_list.php?lang=".LANGUAGE_ID."&find_channel_id=".$arFilter["CHANNEL_ID"]."&reset_id={$res["ID"]}&".thurly_sessid_get()."'");
		$arActions[] = array("SEPARATOR"=>true);
		$arActions[] = array("ICON" => "delete", "TEXT" => GetMessage("MAIN_ADMIN_MENU_DELETE"),
			"ACTION"=>"if(confirm('".GetMessage("VOTE_CONFIRM_DEL_VOTE")."')) window.location='vote_list.php?lang=".LANGUAGE_ID."&find_channel_id=".$arFilter["CHANNEL_ID"]."&action=delete&ID={$res["ID"]}&".thurly_sessid_get()."'");
		$arActions[] = array("SEPARATOR"=>true);
