...
/***************** /Event ******************************************/
	...
	public static function Delete($ID)
	{
		...
		\Thurly\Vote\AttachTable::deleteByFilter(array("OBJECT_ID" => $ID));
		// delete vote images
	...
	{
		\Thurly\Vote\Event::resetStatistic($ID);
	...
	{
		return \Thurly\Vote\User::getCurrent()->setVotedUserId();
	}
	...
	{
		global $USER;
		/** @var $r \Thurly\Main\HttpRequest */
		$r = \Thurly\Main\Context::getCurrent()->getRequest();
		...
		try
		{
			if (empty($request["vote"]) || $PUBLIC_VOTE_ID <= 0 || !check_thurly_sessid())
				throw new \Thurly\Main\ArgumentException(GetMessage("VOTE_NOT_FOUND"), "bad_params");

			$vote = new \Thurly\Vote\Vote($PUBLIC_VOTE_ID);
			...
			if ($channel["USE_CAPTCHA"] == "Y" && !$USER->IsAuthorized())
			{
					...
					empty($request["captcha_code"]) && !$cpt->CheckCode($request["captcha_word"], 0))
				{
					...
					throw new \Thurly\Main\ArgumentException(GetMessage("VOTE_BAD_CAPTCHA"), "captcha");
				}
			}
