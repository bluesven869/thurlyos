...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/xmpp/include.php");
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/xmpp/prolog.php");
...
if (strlen($_REQUEST["server_action"]) > 0 && in_array($_REQUEST["server_action"], array("start", "stop", "query", "clearcache", "dump")))
{
	if (check_thurly_sessid())
	{
		...
		elseif ($_REQUEST["server_action"] == "start")
		{
			...
			$serverPath = COption::GetOptionString("xmpp", "server_path", "./thurly/modules/xmpp/xmppd.php");
			...
			if (strlen($startErrorMessage) <= 0)
			{
				if ($bWindowsHosting)
				{
					pclose(popen("start ".$phpPath." ".$serverPath.(extension_loaded('thurlyos') ? " thurlyos" : ""), "r"));
				}
				else
				{
					$cmd = 'nohup '.$phpPath.' '.$_SERVER['DOCUMENT_ROOT'].ltrim($serverPath, '.').(extension_loaded('thurlyos') ? " thurlyos" : "").' > /dev/null &';
					exec($cmd, $op);
				}
			}
...
function StartServer()
{
	...
	ShowWaitWindow();
	CHttpRequest.Send('xmpp_server.php?lang=<?echo htmlspecialchars(LANG)?>&server_action=start&<?echo thurly_sessid_get()?>');
}
...
function StopServer()
{
	...
	ShowWaitWindow();
	CHttpRequest.Send('xmpp_server.php?lang=<?echo htmlspecialchars(LANG)?>&server_action=stop&<?echo thurly_sessid_get()?>');
}
...
function ClearCacheServer()
{
	...
	ShowWaitWindow();
	CHttpRequest.Send('xmpp_server.php?lang=<?echo htmlspecialchars(LANG)?>&server_action=clearcache&<?echo thurly_sessid_get()?>');
}
...
function DumpServer()
{
	...
	ShowWaitWindow();
	CHttpRequest.Send('xmpp_server.php?lang=<?echo htmlspecialchars(LANG)?>&server_action=dump&<?echo thurly_sessid_get()?>');
}
...
function QueryServer()
{
	...
	ShowWaitWindow();
	CHttpRequest.Send('xmpp_server.php?lang=<?echo htmlspecialchars(LANG)?>&server_action=query&<?echo thurly_sessid_get()?>');
}
