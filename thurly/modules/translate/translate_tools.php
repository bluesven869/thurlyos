	...
	if($handle)
	{
		...
		while (false!==($file=readdir($handle)))
		{
			if ($file != "." && $file != ".." && $file != ".access.php" && $file != ".htaccess" && $file != '.svn' && $file != '.hg')
			{
				...
				if ($IS_DIR=="Y" &&
					($path_prepared=="/thurly/updates" ||
					$path_prepared=="/thurly/updates_enc" ||
					$path_prepared=="/thurly/updates_enc5" ||
					$path_prepared=="/thurly/help" ||
					$path_prepared=="/thurly/cache" ||
					$path_prepared=="/thurly/cache_image" ||
					$path_prepared=="/thurly/managed_cache" ||
					$path_prepared=="/thurly/stack_cache"
					)
				)
					continue;
...
function SaveTCSVFile()
{
	global $APPLICATION;
	$errors = array();
	if ($APPLICATION->GetGroupRight("translate") == 'W' && check_thurly_sessid())
	{
		if (
...
function TR_BACKUP($file)
{
	...
	if (COption::GetOptionString('translate', 'BACKUP_FILES', 'N') == 'Y')
	{
		...
		$backUPPath = $_SERVER["DOCUMENT_ROOT"].'/thurly/tmp/translate/_backup'.dirname($file).'/';
