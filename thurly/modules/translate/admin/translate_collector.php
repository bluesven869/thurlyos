...
$strErrorMessage = "";
$strOKMessage = "";
if ($_SERVER["REQUEST_METHOD"]=="POST" && $_REQUEST["start_collect"]=="Y" && check_thurly_sessid())
{
	...
	{
		$targetLanguagePath = $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/_langs/".$language_id;
	...
	if (strlen($strErrorMessage)<=0)
	{
		DeleteDirFilesEx("/thurly/updates/_langs/".$language_id);
		clearstatcache();
	...
	if ($strErrorMessage == '')
	{
		__ReWalkDirs($_SERVER["DOCUMENT_ROOT"]."/thurly/modules", $targetLanguagePath, $language_id, $bConvert, $strEncodingIn, $strEncodingOut);
	...
	if (strlen($strErrorMessage)<=0)
	{
		if ($pack_files=="Y")
		{
			@unlink($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/_langs/file-".$language_id.".tar.gz");

			$oArc = new CArchiver($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/_langs/file-".$language_id.".tar.gz", $bUseCompression);
			$oArc->_strSeparator = '|';
			$res = $oArc->Add($_SERVER["DOCUMENT_ROOT"]."/thurly/updates/_langs/".$language_id, false, $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/_langs");
			if (!$res)
			{
			...
			else
			{
				$strOKMessage = GetMessage('TR_LANGUAGE_COLLECTED_ARCHIVE', array('%LANG%' => $language_id,
																				'%FILE_PATH%' => $_SERVER["DOCUMENT_ROOT"]."/thurly/updates/_langs/file-".$language_id.".tar.gz",
																				'%LINK%' =>	"<a href=\"/thurly/updates/_langs/file-".$language_id.".tar.gz\">file-".$language_id.".tar.gz</a>"
				));
			}
		}
	}
...
else if ($_SERVER["REQUEST_METHOD"]=="POST" && $_REQUEST["start_download"]=="Y" && check_thurly_sessid())
{
	...
	$tempLanguagePathNoRoot = '/thurly/tmp/translate/'.time().'/';
	...
	if (strlen($strErrorMessage)<=0)
	{
		...
		__ReWalkDirs($tempLanguagePath.$language_id.'/', $_SERVER["DOCUMENT_ROOT"]."/thurly/modules", $language_id,
...
<?=thurly_sessid_post()?>
	<tr class="adm-required-field">
		...
		<select name="language_id">
			...
			{
				if (is_dir($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/lang/".$arLang['LANGUAGE_ID']))
				{
