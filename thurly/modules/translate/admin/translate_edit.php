...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/translate/prolog.php");
...
if($strError == "")
{
	...
	if (is_array($arSlash))
	{
		...
		foreach($arSlash_tmp as $dir)
		{
			...
			if ($i==1)
			{
				$chain .= "<a href=\"translate_list.php?lang=".LANGUAGE_ID."&path=/"."&".thurly_sessid_get()."\" title=\"".GetMessage("TRANS_CHAIN_FOLDER_ROOT")."\">..</a> / ";
			}
			else
			{
				$arPath[] = htmlspecialcharsbx($dir);
				if ($i>2) $chain .= " / ";
				$chain .= "<a href=\"translate_list.php?lang=".LANGUAGE_ID."&path="."/".implode("/",$arPath)."/"."&".thurly_sessid_get()."\" title=\"".GetMessage("TRANS_CHAIN_FOLDER")."\">".htmlspecialcharsbx($dir)."</a>";
			}
		}
	}
	...
	if ($_SERVER['REQUEST_METHOD'] == "POST" && (strlen($save)>0 || strlen($apply)>0) && $TRANS_RIGHT=="W" && check_thurly_sessid())
	{
		if (is_array($KEYS))
		{
			$arTEXT = array();
			...
			if (strlen($save)>0)
				LocalRedirect("translate_list.php?lang=".LANGUAGE_ID."&path=".$path_back."&".thurly_sessid_get());
			else
...
$aMenu[] = array(
	...
	"LINK"	=> "/thurly/admin/translate_list.php?lang=".LANGUAGE_ID."&path=/".implode("/",$arPath)."/"."&".thurly_sessid_get(),
...
if ($ONLY_ERROR=="N")
{
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/translate_edit.php?file=".htmlspecialcharsbx($file)."&lang=".LANGUAGE_ID."&show_error=Y",
...
elseif ($ONLY_ERROR=="Y")
{
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/translate_edit.php?file=".htmlspecialcharsbx($file)."&lang=".LANGUAGE_ID,
...
if ($boolGetUntranslate)
{
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/translate_csv_download.php?lang=".LANGUAGE_ID."&download_translate_lang=N&path=".htmlspecialcharsbx($path_back)."&file=".htmlspecialcharsbx($file)."&".thurly_sessid_get(),
...
$arSubMenu = array(
	array(
		...
		"LINK"	=> "/thurly/admin/translate_show_php.php?file=".htmlspecialcharsbx($file)."&lang=".LANGUAGE_ID,
		"TITLE"	=> GetMessage("TR_FILE_SHOW_TITLE"),
	),
	array(
		"TEXT"	=> GetMessage("TR_FILE_EDIT"),
		"LINK"	=> "/thurly/admin/translate_edit_php.php?file=".htmlspecialcharsbx($file)."&lang=".LANGUAGE_ID,
...
<table border="0" cellspacing="3" cellpadding="3" width="100%">
	<tr>
		<td valign="top" align="right" width="0%" nowrap colspan="2"><img src="/thurly/images/1.gif" width="1" height="8"></td>
	</tr>
	<tr>
	...
	if (is_array($arKEYS))
	{
		while (list($i,$key)=each($arKEYS))
		{
			...
			if (($ONLY_ERROR=="Y" && $red) || $ONLY_ERROR=="N")
			{
				...
				$intShowCount++;
?>
			<tr><td colspan="3"><img src="/thurly/images/1.gif" width="1" height="10"><hr><img src="/thurly/images/1.gif" width="1" height="3"></td></tr>
			<tr>
				<td>ID:</td>
				<td>
<?
...
?>
			...
			<tr>
				<td colspan="3"><img src="/thurly/images/1.gif" width="1" height="5"></td>
			</tr>
<?
				reset($arTLanguages);
				$rows = "2";
...
<?$tabControl->Buttons(array("disabled" => ($TRANS_RIGHT<"W"), "back_url"=>"translate_list.php?lang=".LANGUAGE_ID."&path=".urlencode($path_back)."&".thurly_sessid_get()));
$tabControl->End();
?></form>
