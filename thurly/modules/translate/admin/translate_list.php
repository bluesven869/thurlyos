...
if (strlen($path)>0 && !$arSearchParam)
{
	...
	$row->AddViewField("TRANS_FILE_NAME", '<a href="?lang='.LANGUAGE_ID.'&path='.urlencode($last_path).'" title="'.GetMessage("TR_UP_TITLE").'">
			<img src="/thurly/images/translate/up.gif" width="11" height="13" border=0 alt=""></a>'.
	...
	foreach($arLangDirFiles as $key => $ar) :
		...
		if (in_array($ar["PARENT"],$arPath) || $arSearchParam) :
			...
			if ($is_dir=="Y") :
				...
				$row->AddViewField("TRANS_FILE_NAME", '<a href="?lang='.LANGUAGE_ID.'&path='.$fpath.'" title="'.GetMessage("TR_FOLDER_TITLE").'"><img src="/thurly/images/translate/folder.gif" width="16" height="16" border=0 alt=""></a>'.'&nbsp;<a href="?lang='.LANGUAGE_ID.'&path='.$fpath.'" title="'.GetMessage("TR_FOLDER_TITLE").'">'.$ftitle.'</a>');
			else :
				...
				$row->AddViewField("TRANS_FILE_NAME", '<a href="translate_edit.php?lang='.LANGUAGE_ID.'&file='.$fpath.'&show_error='.$show_error.'" title="'.GetMessage("TR_FILE_TITLE").'"><img src="/thurly/images/translate/file.gif" width="16" height="16" border=0 alt=""></a>'.
...
$url = $APPLICATION->GetPopupLink(
					array(
						'URL' => "/thurly/admin/translate_search.php?lang=".LANGUAGE_ID."&bxpublic=Y&path=".urlencode($path),
								'PARAMS' => array(
									'width' => 470,
									'height' => 310,
									'resizable' => false
...
<?$tabControl->EndTab();?>
	...
	<form action="translate_csv_download.php" name="form4" method="POST">
		...
		<?
		$customScriptsFile = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/langs.txt";
		if(file_exists($customScriptsFile)):
		?>
		<tr>
			<td width="10%" valign="top" nowrap>Only /thurly/modules/langs.txt files:</td>
			<td valign="top" width="90%">
