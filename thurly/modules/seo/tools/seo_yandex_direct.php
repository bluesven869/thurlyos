...
if (!$USER->CanDoOperation('seo_tools') || !check_thurly_sessid())
	...
	try
	{
		switch($_REQUEST['action'])
		{
			...
			case 'banner_stats':
				...
				if($finish)
				{
					...
					if(!$res['error'] || $res["code"] == Engine\YandexDirect::ERROR_NO_STATS)
					{
						...
						if(
							\Thurly\Main\ModuleManager::isModuleInstalled('sale')
							&& \Thurly\Main\ModuleManager::isModuleInstalled('catalog')
						...
						)
						{
							...
							$res["order_sum_format"] = \CCurrencyLang::CurrencyFormat(doubleval($res["order_sum"]), \Thurly\Currency\CurrencyManager::getBaseCurrency(), true);
						}
					}
				}
				else
			...
			case 'banner_stat_detail':
				...
				$bSale = \Thurly\Main\ModuleManager::isModuleInstalled('sale')
					&& \Thurly\Main\ModuleManager::isModuleInstalled('catalog')
					&& Loader::includeModule('currency');
