...
$bShowYandexServices =
	COption::GetOptionString('main', 'vendor', '') == '1c_thurly'
...
if($REQUEST_METHOD=="POST" && strlen($Update.$Apply.$RestoreDefaults)>0 && check_thurly_sessid())
{
...
<tr>
	<td>
<?
		\Thurly\Main\Localization\Loc::loadMessages(dirname(__FILE__).'/admin/seo_search.php');
		\Thurly\Main\Localization\Loc::loadMessages(dirname(__FILE__).'/admin/seo_adv.php');

		$engine = new \Thurly\Seo\Engine\YandexDirect();
		require_once(dirname(__FILE__)."/admin/tab/seo_search_yandex_direct_auth.php");

		if(\Thurly\Seo\Service::isRegistered())
		{
?>
...
<script>
	function clearCloudAdvRegister()
	{
		...
		{
			BX.ajax.loadJSON('/thurly/tools/seo_yandex_direct.php?action=unregister&sessid=' + BX.thurly_sessid(), function(result)
			{
				if(result['result'])
				{
					BX.reload();
