...
use Thurly\Seo\SitemapIblock;
use Thurly\Seo\SitemapTable;
use Thurly\Seo\SitemapIndex;
use Thurly\Seo\SitemapRuntime;
...
if($_REQUEST['action'] == 'sitemap_run' && check_thurly_sessid())
{
	$arValueSteps = array(
		'init' => 0,
		'files' => 40,
...
//							remove or replace SERVER_NAME
	...
	if($v == $arValueSteps['index'])
	{
		...
		SitemapTable::update($ID, array('DATE_RUN' => new Thurly\Main\Type\DateTime()));
	}
