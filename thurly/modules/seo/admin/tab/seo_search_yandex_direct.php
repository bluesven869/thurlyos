...
?>
<a href="/thurly/admin/seo_search_yandex_direct_edit.php?lang=<?=LANGUAGE_ID?>&back_url=<?=urlencode($APPLICATION->GetCurPageParam('form_element_'.$iblockElementInfo["IBLOCK"]["ID"].'_active_tab=seo_adv_seo_adv', array('form_element_'.$iblockElementInfo["IBLOCK"]["ID"].'_active_tab')))?>"><?=Loc::getMessage("SEO_CREATE_NEW_CAMPAIGN")?></a>
<?
}
else
{
...
span.yandex-delete {
	...
	line-height: 20px;
	cursor: pointer;
	background: url("/thurly/panel/main/images/bx-admin-sprite-small-1.png") no-repeat scroll 5px -2446px rgba(0, 0, 0, 0);
}
...
.seo-adv-block-icon
{
	top: 12px;
	left: 12px;
	background: url('/thurly/panel/seo/images/faces.png') no-repeat scroll 0 0;
	display: block;
	position: absolute;
	height: 42px;
	width: 42px;
	...
	<tr>
		...
		<td width="60%">
			<select id="seo_adv_campaign" style="width:400px" onchange="updateNewBannerLink()">
				...
				href="/thurly/admin/seo_search_yandex_direct_edit.php?lang=<?=LANGUAGE_ID?>&back_url=<?=urlencode($APPLICATION->GetCurPageParam('form_element_'.$iblockElementInfo["IBLOCK"]["ID"].'_active_tab=seo_adv_seo_adv', array('form_element_'.$iblockElementInfo["IBLOCK"]["ID"].'_active_tab')))?>"><?=Loc::getMessage("SEO_CREATE_NEW_CAMPAIGN")?></a>
		</td>
	</tr>
	<tr>
		...
		<td>
			<select id="seo_adv_banner" style="width:400px" disabled="disabled"
				...
				<option value="0"><?=Loc::getMessage("SEO_CAMPAIGN_CHOOSE_OPTION")?></option>
					...
					href="/thurly/admin/seo_search_yandex_direct_banner_edit.php?lang=<?=LANGUAGE_ID?>&element=<?=$iblockElementInfo['ID']?>&back_url=<?=urlencode($APPLICATION->GetCurPageParam('form_element_'.$iblockElementInfo["IBLOCK"]["ID"].'_active_tab=seo_adv_seo_adv', array('form_element_'.$iblockElementInfo["IBLOCK"]["ID"].'_active_tab')));?>" style="display: none;"><?=Loc::getMessage('SEO_CREATE_NEW_BANNER')?></a>
...
<script>
	...
	function updateNewBannerLink()
	{
		...
		if (campaign > 0 && typeof bannerList[campaign] == 'undefined')
		{
			...
			BX.ajax.loadJSON('/thurly/tools/seo_yandex_direct.php?action=banners_get&campaign=' + campaign + '&sessid=' + BX.thurly_sessid(), function (result)
			{
	...
	function createLink()
	{
		...
		if (bannerSelect.value > 0)
		{
			...
			BX.ajax.loadJSON('/thurly/tools/seo_yandex_direct.php', {
				action: 'link_create',
	...
	function deleteLink(bannerId, el)
	{
		...
		{
			el._loading = true;
			el.style.background = 'url("/thurly/panel/main/images/waiter-small-white.gif") no-repeat scroll 3px center';

			BX.ajax.loadJSON('/thurly/tools/seo_yandex_direct.php?action=link_delete&banner='+bannerId+'&link=<?=$iblockElementInfo["ID"]?>&link_type=<?=\Thurly\Seo\Adv\LinkTable::TYPE_IBLOCK_ELEMENT?>&get_list_html=1&sessid='+BX.thurly_sessid(), function(res)
			{
				if(res.result)
				{
					BX('adv_banner_list').innerHTML = res.list_html;
	...
	function deleteLinkStat(bannerId, el)
	{
		if(!el._loading)
		{
			...
			BX.ajax.loadJSON('/thurly/tools/seo_yandex_direct.php?action=link_delete&banner='+bannerId+'&link=<?=$iblockElementInfo["ID"]?>&link_type=<?=\Thurly\Seo\Adv\LinkTable::TYPE_IBLOCK_ELEMENT?>&get_list_html=0&sessid='+BX.thurly_sessid(), function(res)
			{
	...
	function loadBannerStats(bannerId, dateStart, session)
	{
		...
		BX.ajax.loadJSON('/thurly/tools/seo_yandex_direct.php', queryData, function(res)
		{
	...
	function stopBanner(campaignId, bannerId)
	{
		...
		BX.ajax({
			url: '/thurly/admin/seo_search_yandex_direct_banner.php?ID='+bannerId+'&action_button=stop&sessid='+BX.thurly_sessid()+'&campaign='+campaignId+'&mode=list&table_id=tbl_yandex_direct_banner',
			processData: false,
			onsuccess: function()
			{
				loadStat(bannerId);
	...
	function resumeBanner(campaignId, bannerId)
	{
		BX('yandex_direct_action_button_' + bannerId).disabled = true;
		BX.ajax({
			url: '/thurly/admin/seo_search_yandex_direct_banner.php?ID='+bannerId+'&action_button=resume&sessid='+BX.thurly_sessid()+'&campaign='+campaignId+'&mode=list&table_id=tbl_yandex_direct_banner',
			processData: false,
			onsuccess: function()
			{
				loadStat(bannerId);
