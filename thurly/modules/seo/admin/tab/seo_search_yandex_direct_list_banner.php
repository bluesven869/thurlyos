...
<script>
	function deleteLink(linkId, linkType, el)
	{
		...
		{
			el._loading = true;
			el.style.background = 'url("/thurly/panel/main/images/waiter-small-white.gif") no-repeat scroll center center';

			BX.ajax.loadJSON('/thurly/tools/seo_yandex_direct.php?action=link_delete&banner=<?=$ID?>&link='+linkId+'&link_type='+linkType+'&get_list_html=2&sessid='+BX.thurly_sessid(), function(res)
			{
