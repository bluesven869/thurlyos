...
use Thurly\Seo\RobotsFile;
use Thurly\Seo\SitemapTable;
use Thurly\Seo\SitemapEntityTable;
use Thurly\Seo\SitemapIblockTable;
use Thurly\Seo\SitemapForumTable;
use Thurly\Seo\SitemapRuntimeTable;
...
if($ID > 0)
{
	...
	{
		if($_REQUEST['action'] == 'delete' && check_thurly_sessid())
		{
...
if(strlen($SITE_ID) > 0)
{
	...
	else
	{
		...
		$dbDomains = Thurly\Main\SiteDomainTable::getList(
			array(
		...
		$arSite['DOMAINS'][] = \Thurly\Main\Config\Option::get('main', 'server_name', '');
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_thurly_sessid() && (strlen($_POST["save"]) > 0 || strlen($_POST['apply']) > 0 || strlen($_POST['save_and_add']) > 0))
{
	...
	if(empty($errors))
	{
		...
		if($result->isSuccess())
		{
			...
			{
				LocalRedirect(BX_ROOT."/admin/seo_sitemap.php?lang=".LANGUAGE_ID."&run=".$ID."&".thurly_sessid_get());
			}
			else
			{
...
if(isset($_REQUEST['dir']) && check_thurly_sessid())
{
...
if($bIBlock && isset($_REQUEST['iblock']) && check_thurly_sessid())
{
...
$APPLICATION->SetAdditionalCSS("/thurly/panel/seo/sitemap.css");
...
$aMenu[] = array(
	...
	"LINK"	=> "/thurly/admin/seo_sitemap.php?lang=".LANGUAGE_ID,
	"ICON"	=> "btn_list",
...
if ($ID > 0)
{
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".Loc::getMessage("SITEMAP_DELETE_CONFIRM")."')) window.location='/thurly/admin/seo_sitemap_edit.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		"ICON"	=> "btn_delete",
...
function loadDir(bLogical, sw, dir, div, depth, checked)
{
	...
	{
		BX.ajax.get('<?=$APPLICATION->GetCurPageParam('', array('dir', 'depth'))?>', {dir:dir,depth:depth,checked:checked?'Y':'N',log:bLogical?'Y':'N',sessid:BX.thurly_sessid()}, function(res)
		{
...
function loadIblock(sw, iblock, section, div, section_checked, element_checked)
{
	...
	else
	{
		...
		BX.ajax.get('<?=$APPLICATION->GetCurPageParam('', array('dir', 'iblock', 'section', 'depth'))?>', {iblock:iblock,section:section,section_checked:section_checked?'Y':'N',element_checked:element_checked?'Y':'N',sessid:BX.thurly_sessid()}, function(res)
		{
			BX(div).innerHTML = res;
			BX(row).style.display = '';
			if(sw)
