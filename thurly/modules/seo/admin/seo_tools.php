...
?>
	<tr>
		<td colspan="2"><img src="/thurly/admin/searcher_graph.php?&lang=<?echo LANGUAGE_ID?>&find_date1_DAYS_TO_BACK=90<?foreach ($arSearchers as $key => $ar) echo '&find_searchers[]='.$key;?>&mode=list&find_summa=N&width=576&height=300" border="0" width="576" height="300" border="0" /><br /><br />
...
?>
			<tr>
				<td><img src="/thurly/admin/graph_legend.php?color=<?=$color?>" width="45" height="2"></td>
				<td nowrap="nowrap">[<a href="/thurly/admin/searcher_list.php?lang=<?=LANGUAGE_ID?>&amp;find_id=<?=$keyL?>&amp;set_filter=Y"><?=$keyL?></a>]&nbsp;<a  href="/thurly/admin/searcher_dynamic_list.php?lang=<?=LANGUAGE_ID?>&amp;find_searcher_id=<?=$keyL?>&amp;find_date1=<?echo $arFilter["DATE1"]?>&amp;find_date2=<?=$arFilter["DATE2"]?>&amp;set_filter=Y"><?=$arrL["NAME"]?></a></td>
			</tr>
<?
					endforeach;
?>
...
?>
					...
					<div>
						<a href="/thurly/admin/phrase_list.php?lang=<?echo LANGUAGE_ID?>"><?echo GetMessage('SEO_PAGE_GOTO_CP')?></a>
					</div>
<?
				}
				else
		...
		case 'referers':
			...
			<table width="100%" class="referers-table bx-seo-words-table internal" id="bx_seo_words_table">
				...
				<?
					...
					{
						$percent = number_format($arData['PERCENT'], 2);
						$domainEnc = \Thurly\Main\Text\Converter::getHtmlConverter()->encode($domain);
						?>
						<tr>
			...
			<div>
				<a href="/thurly/admin/referer_list.php?lang=<?echo LANGUAGE_ID?>"><?echo GetMessage('SEO_PAGE_GOTO_CP')?></a>
			</div>
<?
//			have not references
			else:
...
?>
<form name="seo_form" method="POST" action="/thurly/admin/public_seo_tools.php" enctype="multipart/form-data">
...
window.BXCallUpdateKeywordsStats = function(keywords)
{
	BX.showWait();
	BX.ajax({
		url: '/thurly/tools/seo_page_parser.php?lang=<?=LANGUAGE_ID?>&site=<?=$site?>&url=<?echo CUtil::JSEScape(urlencode($back_url))?>&callback=set_keywords_stats&sessid=' + BX.thurly_sessid(),
...
window.BXCallPageStats = function()
{
	...
	BX.ajax({
		url: '/thurly/tools/seo_page_parser.php?lang=<?=LANGUAGE_ID?>&first=Y&site=<?=$site?>&url=<?echo CUtil::JSEScape(urlencode($back_url))?>&callback=set_stats&sessid=' + BX.thurly_sessid(),
		data: 'keywords=' + BX.util.urlencode(keywords),
		method: 'POST',
		dataType: 'script',
		emulateOnload: false
