...
if($bShowStats)
{
	...
	if(count($statsBanners) > 0)
	{
		...
		foreach($statsBanners as $banner)
		{
			...
			if($bShowOrderStats)
			{
				$row->AddViewField("BANNER_SUM_ORDER", \CCurrencyLang::CurrencyFormat(doubleval($banner['BANNER_SUM_ORDER']), \Thurly\Currency\CurrencyManager::getBaseCurrency(), true));
			}
		}
	}
...
if($request->isPost() && ($request["save"]<>'' || $request["apply"]<>'') && check_thurly_sessid() && !$bReadOnly)
{
	...
	if($result->isSuccess())
	{
		...
		if($request["apply"]<>'')
		{
			LocalRedirect('/thurly/admin/seo_search_yandex_direct_edit.php?lang='.LANGUAGE_ID.'&ID='.$ID.'&'.$tabControl->ActiveTabParam());
		}
		else
		{
			if($back_url == '')
			{
				LocalRedirect("/thurly/admin/seo_search_yandex_direct.php?lang=".LANGUAGE_ID);
			}
			else
			{
...
if($ID > 0)
{
	...
	if(!$bReadOnly)
	{
		$aMenu[] = array(
			...
			"ICON" => "btn_delete",
			"LINK" => "seo_search_yandex_direct.php?lang=".LANGUAGE_ID."&ID=".$ID."&action=delete&".thurly_sessid_get()
		);
	}
}
...
<script>
	...
	function loadGraphData(session)
	{
		...
		BX.ajax.loadJSON(
			'/thurly/tools/seo_yandex_direct.php',
			queryData,
			function (res)
			{
				if (!!res.session)
...
function updateCampaign(btn, campaignId)
{
	...
	var url = '/thurly/tools/seo_yandex_direct.php?action=campaign_update&campaign=' + BX.util.urlencode(campaignId);
