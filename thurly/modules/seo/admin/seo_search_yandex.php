...
if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_REQUEST['CODE']) && check_thurly_sessid())
{
	try
	{
...
$dbSites = new Thurly\Main\DB\ArrayResult(\CSeoUtils::getDomainsList());
	...
	if(window.lastSeoResult)
	{
		...
		else
		{
			var wnd = new BX.CAdminDialog({
				content_url: "/thurly/admin/seo_search_yandex_detail.php?lang="+BX.message('LANGUAGE_ID')+"&domain="+domain+"&bxpublic=Y"
			});
			wnd.Show();
		}
	}
...
function bindDomain(domain, dir, node)
{
	...
	BX.ajax.loadJSON('/thurly/tools/seo_yandex.php?action=site_add&domain='+BX.util.urlencode(domain)+'&dir='+BX.util.urlencode(dir)+'&sessid=' + BX.thurly_sessid(), updateCallback);
}

function verifyDomain(domain, dir, node)
{
	node.innerHTML = '<?=CUtil::JSEscape(Loc::getMessage('SEO_LOADING'))?>';
	BX.ajax.loadJSON('/thurly/tools/seo_yandex.php?action=site_verify&domain='+BX.util.urlencode(domain)+'&dir='+BX.util.urlencode(dir)+'&sessid=' + BX.thurly_sessid(), updateCallback);
}


function updateInfo()
{
	BX.ajax.loadJSON('/thurly/tools/seo_yandex.php?action=sites_feed&sessid=' + BX.thurly_sessid(), updateCallback);
}
...
function makeNewAuth()
{
	...
	BX.ajax.loadJSON('/thurly/tools/seo_yandex.php?action=nullify_auth&sessid=' + BX.thurly_sessid(), function(){
		window.lastSeoResult = null;
...
<div id="auth_code" style="display: none;">
	<form name="auth_code_form" action="<?=Converter::getHtmlConverter()->encode($APPLICATION->getCurPageParam("", array("CODE", "oauth")))?>" method="POST"><?=thurly_sessid_post();?><?=Loc::getMessage('SEO_AUTH_CODE')?>: <input type="text" name="CODE" style="width: 200px" /> <input type="submit" name="send_code" value="<?=Loc::getMessage('SEO_AUTH_CODE_SUBMIT')?>"></form></div>
<?
if(!$bNeedAuth)
{
