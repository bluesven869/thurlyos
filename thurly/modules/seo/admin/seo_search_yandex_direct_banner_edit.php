...
if(!$bReadOnly && $request->isPost() && ($request["save"]<>'' || $request["apply"]<>'') && check_thurly_sessid())
{
	...
	if($result->isSuccess())
	{
		...
		if($request["apply"]<>'')
		{
			LocalRedirect('/thurly/admin/seo_search_yandex_direct_banner_edit.php?lang='.LANGUAGE_ID.'&ID='.$ID.'&'.$tabControl->ActiveTabParam());
		}
		else
		{
			if($back_url == '')
			{
				LocalRedirect("/thurly/admin/seo_search_yandex_direct_banner.php?lang=".LANGUAGE_ID.'&campaign='.$campaign['ID']);
			}
			else
			{
...
if($ID > 0)
{
	if(!$bReadOnly)
	{
		...
		$aMenu[] = array(
			...
			"ICON" => "btn_delete",
			"LINK" => "seo_search_yandex_direct_banner.php?lang=".LANGUAGE_ID."&campaign=".$campaignId."&ID=".$ID."&action=delete&".thurly_sessid_get()
		);
	}
}
...
?>
	<tr>
		...
		<td colspan="3">
			...
			<a href="javascript:void(0)" onclick="BX.util.popup('/thurly/admin/iblock_element_search.php?lang=ru&n=new_link_container', 1000, 700);"><?=Loc::getMessage('SEO_BANNER_LINK_CREATE_ITEM')?></a><br />
		</td>
	</tr>
<?
if($bShowAuto):
...
?>
	<script>
		...
		{
			BX.ajax.loadJSON('/thurly/tools/seo_yandex_direct.php?action=link_create&banner=<?=$ID?>&link='+linkId+'&link_type='+linkType+'&get_list_html=2&sessid='+BX.thurly_sessid(), function(res)
			{
...
function updateBanner(btn, bannerId)
{
	...
	var url = '/thurly/tools/seo_yandex_direct.php?action=banner_update&campaign=<?=$campaignId?>&banner=' + BX.util.urlencode(bannerId);

	BX.ajax.loadJSON(url + '&sessid=' + BX.thurly_sessid(), function(res){
...
{
	background: url("/thurly/panel/main/images/bx-admin-sprite.png") no-repeat scroll -10px -233px rgba(0, 0, 0, 0);
}

.region-opened > .openbutton
{
	background: url("/thurly/panel/main/images/bx-admin-sprite.png") no-repeat scroll -10px -211px rgba(0, 0, 0, 0);
}

.openbutton.empty
{
	background: url("/thurly/panel/main/images/bx-admin-sprite-small-1.png") no-repeat scroll -3px -1132px rgba(0, 0, 0, 0) !important;
}

span.loading-message-text
{
	background: url('/thurly/panel/main/images/waiter-small-white.gif') no-repeat left center;
	padding-left: 20px;
	margin: 0 0 0 15px;
	line-height: 15px;
	font-size: 11px;
...
if($bShowStats)
{
	...
	if(!$bLoadStats && $bSale)
	{
		$orderStats = Adv\OrderTable::getList(array(
			...
			'runtime' => array(
				new \Thurly\Main\Entity\ExpressionField('BANNER_SUM', 'SUM(SUM)'),
			),
		));
...
<tr>
	...
	<td><span id="banner_profit"><?=\CCurrencyLang::CurrencyFormat(doubleval($bannerProfit), \Thurly\Currency\CurrencyManager::getBaseCurrency(), true)?></span></td>
</tr>
<?
	endif;
?>
			...
			"categoryAxis": {
				...
				"labelFunction": function(val, date)
				{
					return BX.date.format(
						BX.date.convertThurlyFormat('<?=FORMAT_DATE?>'), date
					);
				}
			},
			"chartCursor": {
