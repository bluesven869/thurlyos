...
function bindDomain(domain, dir, node)
{
	...
	BX.ajax.loadJSON('/thurly/tools/seo_google.php?action=site_add&domain='+BX.util.urlencode(domain)+'&dir='+BX.util.urlencode(dir)+'&sessid=' + BX.thurly_sessid(), updateCallback);
}

function verifyDomain(domain, dir, node)
{
	node.innerHTML = '<?=CUtil::JSEscape(Loc::getMessage('SEO_LOADING'))?>';
	BX.ajax.loadJSON('/thurly/tools/seo_google.php?action=site_verify&domain='+BX.util.urlencode(domain)+'&dir='+BX.util.urlencode(dir)+'&sessid=' + BX.thurly_sessid(), updateCallback);
}

function updateInfo()
{
	BX.ajax.loadJSON('/thurly/tools/seo_google.php?action=sites_feed&sessid=' + BX.thurly_sessid(), updateCallback);
}
...
function makeNewAuth()
{
	...
	BX.ajax.loadJSON('/thurly/tools/seo_google.php?action=nullify_auth&sessid=' + BX.thurly_sessid(), function(){
		window.lastSeoResult = null;
