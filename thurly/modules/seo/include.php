...
class CSeoEventHandlers
{
	function SeoOnPanelCreate()
	{
		...
		$APPLICATION->AddPanelButton(array(
			"HREF"=> 'javascript:'.$APPLICATION->GetPopupLink(
				array(
					"URL"=>"/thurly/admin/public_seo_tools.php?lang=".LANGUAGE_ID."&bxpublic=Y&from_module=seo&site=".SITE_ID
	...
	{
		if (COption::GetOptionString('main', 'vendor', '') == '1c_thurly' && defined('ADMIN_SECTION'))
		{
?>
<script>
;(function(){
	...
	var originalTextBtn = new BX.CWindowButton({
			...
			action: function()
			{
				...
				if(content.length < <?=\Thurly\Seo\Engine\Yandex::ORIGINAL_TEXT_MIN_LENGTH?>)
				{
					alert('<?=GetMessageJS('SEO_YANDEX_ORIGINAL_TEXT_TOO_SHORT', array('#NUM#' => \Thurly\Seo\Engine\Yandex::ORIGINAL_TEXT_MIN_LENGTH))?>');
				}
				else if(content.length > <?=\Thurly\Seo\Engine\Yandex::ORIGINAL_TEXT_MAX_LENGTH?>)
				{
					alert('<?=GetMessageJS('SEO_YANDEX_ORIGINAL_TEXT_TOO_LONG', array('#NUM#' => \Thurly\Seo\Engine\Yandex::ORIGINAL_TEXT_MAX_LENGTH))?>');
				}
				else
				{
					...
					BX.ajax({
						method: 'POST',
						dataType: 'json',
						url: '/thurly/tools/seo_yandex.php',
						data: {
	...
	var originalTextHandler = function()
	{
		...
		{
			BX.showWait();
			BX.ajax.get('/thurly/tools/seo_yandex.php?get=original_text_form&sessid=' + BX.thurly_sessid(), BX.proxy(originalTextLoader, this));
		}
	...
	{
		id : 'SeoUniqText',
		src : '/thurly/panel/seo/images/icon_editor_toolbar.png',
	...
	;(function(){
		...
		var originalTextHandler = function(editor)
		{
			if(!originalTextWnd)
			{
				BX.showWait();
				BX.ajax.get(
					'/thurly/tools/seo_yandex.php?get=original_text_form&sessid=' + BX.thurly_sessid(),
		...
		function applyForEditor(editor)
		{
			editor.AddButton(
				{
					id : 'SeoUniqText',
					src : '/thurly/panel/seo/images/icon_editor_toolbar_2.png',
					name : '<?=CUtil::JSEscape(GetMessage('SEO_UNIQUE_TEXT_YANDEX'))?>',
					codeEditorMode : true,
