...
class CSeoUtils
{
	...
	public static function getDomainsList()
	{
		...
		if($arDomains === null)
		{
			...
			$dbSites = Thurly\Main\SiteDomainTable::getList(
				array(
					'select'=>array(
						'DOMAIN', 'LID',
			...
			$defaultDomain = CBXPunycode::ToASCII(Thurly\Main\Config\Option::getRealValue('main', 'server_name'), $e = null);
			...
			{
				$dbDefSite = Thurly\Main\SiteTable::getList(array(
					'filter' => array('DEF' => 'Y'),
	...
	public static function getDirStructure($bLogical, $site, $path)
	{
		...
		if($USER->CanDoFileOperation('fm_view_listing', array($site, $path)))
		{
			...
			for($i=0,$l = count($arDirContent_t);$i<$l;$i++)
			{
				...
				$f = $file['TYPE'] == 'F'
					? new \Thurly\Main\IO\File($file['PATH'], $site)
					: new \Thurly\Main\IO\Directory($file['PATH'], $site);

				$p = $f->getName();

				if($f->isSystem()
					|| $file['TYPE'] == 'F' && in_array($p, array("urlrewrite.php"))
					|| $file['TYPE'] == 'D' && preg_match("/\/(thurly|".\COption::getOptionString("main", "upload_dir", "upload").")\//", "/".$p."/")
				)
				{
					continue;
				}
