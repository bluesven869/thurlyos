...
if (!$canWrite || !check_thurly_sessid())
{
...
function WFSFSave()
{
	var perm = 'save=Y&document_type=<?= urlencode($_POST['document_type']) ?>&<?= thurly_sessid_get() ?>';
	...
	BX.showWait();
	BX.ajax({
		'url': '/thurly/admin/<?= MODULE_ID ?>_bizproc_wf_settings.php?lang=<?= LANGUAGE_ID ?>&entity=<?= ENTITY ?>',
		'method': 'POST',
		'data': perm,
		'dataType': 'json',
		'timeout': 10,
