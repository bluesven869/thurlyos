...
while ($arResultItem = $dbResultList->NavNext(true, "f_"))
{
	...
	$row->AddField("USER", '[<a href="/thurly/admin/user_edit.php?ID='.$f_USER_ID.'&document_type='.urlencode($documentType[2]).'&lang='.LANGUAGE_ID.'" title="'.GetMessage("BPATT_USER_PROFILE").'">'.$f_USER_ID.'</a>] ('.$f_USER_LOGIN.') '.$f_USER_NAME." ".$f_USER_LAST_NAME);
...
$arSubMenu[] = array(
	...
	"ACTION"	=> "window.location='/thurly/admin/".MODULE_ID."_bizproc_workflow_edit.php?lang=".LANGUAGE_ID."&init=statemachine&entity=".urlencode(ENTITY)."&document_type=".urlencode($documentType[2]).'&back_url_list='.urlencode($backUrl)."';"
);

$arSubMenu[] = array(
	"TEXT"	=> GetMessage("BPATT_SUBMENU2_TEXT"),
	"TITLE"	=> GetMessage("BPATT_SUBMENU2_TEXT_TITLE"),
	"ACTION"	=> "window.location='/thurly/admin/".MODULE_ID."_bizproc_workflow_edit.php?lang=".LANGUAGE_ID."&entity=".urlencode(ENTITY)."&document_type=".urlencode($documentType[2]).'&back_url_list='.urlencode($backUrl)."';"
);
