...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/bizproc/prolog.php");
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/bizproc/include.php");
...
// AJAX
//////////////////////////////////////////
$back_url = "/thurly/admin/".MODULE_ID."_bizproc_workflow_admin.php?lang=".LANGUAGE_ID."&entity=".urlencode(ENTITY)."&document_type=".$document_type."&back_url_list=/".urlencode("/".ltrim(trim($_REQUEST["back_url_list"]), "\\/"));
if(strlen($_REQUEST["back_url"])>0)
	$back_url = "/".ltrim(trim($_REQUEST["back_url"]), "/\\");

if($_SERVER['REQUEST_METHOD']=='POST' && $_REQUEST['saveajax']=='Y' && check_thurly_sessid())
{
	...
	if($_REQUEST['saveuserparams']=='Y')
	{
		...
		if (\Thurly\Main\Text\BinaryString::getLength($d) > 64000)
		{
			?><!--SUCCESS--><script>
...
//		"ACTIVE" 		=> $_POST["ACTIVE"],
	...
	function wfeexception_handler($e)
	{
		...
		(function(){
			...
			errors = <?=\Thurly\Main\Web\Json::encode($errors);?>;
	...
	?><!--SUCCESS--><script type="text/javascript">
		...
		window.location = '<?=($_REQUEST["apply"]=="Y"?Cutil::JSEscape("/thurly/admin/".MODULE_ID."_bizproc_workflow_edit.php?lang=".LANGUAGE_ID."&entity=".AddSlashes(ENTITY)."&ID=".$ID."&back_url_list=".urlencode($_REQUEST["back_url_list"])) : Cutil::JSEscape($back_url))?>';
	</script><?
	die();
}

if($_SERVER['REQUEST_METHOD']=='GET' && $_REQUEST['export_template']=='Y' && check_thurly_sessid())
{
...
if($_SERVER['REQUEST_METHOD']=='POST' && $_REQUEST['import_template']=='Y' && check_thurly_sessid())
{
	$APPLICATION->RestartBuffer();
	...
	<?else:?>
		<?$ID = $r;?>
	<?endif;?>
	window.location = '<?=CUtil::JSEscape('/thurly/admin/'.MODULE_ID.'_bizproc_workflow_edit.php?'.($ID>0?"ID=".$ID."&":"")
...
$aMenu[] = array(
	...
	"LINK"=>"/thurly/admin/".MODULE_ID."_bizproc_workflow_admin.php?lang=".LANGUAGE_ID."&entity=".AddSlashes(ENTITY)."&document_type=".AddSlashes($document_type)."",
	"ICON"=>"btn_list",
);
...
$arSubMenu[] = array(
	...
	"ACTION"	=> "if(confirm('".GetMessage("BIZPROC_WFEDIT_MENU_ADD_WARN")."'))window.location='".CUtil::JSEscape("/thurly/admin/".MODULE_ID."_bizproc_workflow_edit.php?lang=".LANGUAGE_ID."&init=statemachine&entity=".AddSlashes(ENTITY)."&document_type=".AddSlashes($document_type))."';",
	"TITLE"	    => GetMessage("BIZPROC_WFEDIT_MENU_ADD_STATE_TITLE"),
);

$arSubMenu[] = array(
	"TEXT"	=> GetMessage("BIZPROC_WFEDIT_MENU_ADD_SEQ"),
	"ACTION"	=> "if(confirm('".GetMessage("BIZPROC_WFEDIT_MENU_ADD_WARN")."'))window.location='".CUtil::JSEscape("/thurly/admin/".MODULE_ID."_bizproc_workflow_edit.php?lang=".LANGUAGE_ID."&entity=".AddSlashes(ENTITY)."&document_type=".AddSlashes($document_type))."';",
...
function BCPProcessExport()
{
	...
	window.open('<?=CUtil::JSEscape('/thurly/admin/'.MODULE_ID.'_bizproc_workflow_edit.php?'.($ID>0?"ID=".$ID."&":"")
			.'entity='.AddSlashes(urlencode(ENTITY)).'&document_type='.AddSlashes(urlencode($document_type))
			.'&lang='.LANGUAGE_ID.'&'.thurly_sessid_get().'&export_template=Y')?>');
}
...
function BCPSaveUserParams()
{
	...
	jsExtLoader.startPost('<?=CUtil::JSEscape('/thurly/admin/'.MODULE_ID.'_bizproc_workflow_edit.php?'.($ID>0?"ID=".$ID."&":"")
			.'entity='.AddSlashes(urlencode(ENTITY)).'&document_type='.AddSlashes(urlencode($document_type))
			.'&lang='.LANGUAGE_ID.'&'.thurly_sessid_get().'&saveajax=Y&saveuserparams=Y')?>', data);
}
...
function BCPSaveTemplate(save)
{
	...
	jsExtLoader.startPost('<?=CUtil::JSEscape('/thurly/admin/'.MODULE_ID.'_bizproc_workflow_edit.php?'.($ID>0?"ID=".$ID."&":"")
			.'entity='.AddSlashes(urlencode(ENTITY)).'&document_type='.AddSlashes(urlencode($document_type))
			.'&lang='.LANGUAGE_ID.'&'.thurly_sessid_get().'&saveajax=Y')?>'+
...
{
	(new BX.CAdminDialog({
		'content_url': "/thurly/admin/<?=MODULE_ID?>_bizproc_wf_settings.php?mode=public&bxpublic=Y&lang=<?=LANGUAGE_ID?>&entity=<?=ENTITY?>", 
...
.activity a.activitydel {background: url(/thurly/images/bizproc/act_button_del.gif) 50% center no-repeat;}
.activity a.activityset {background: url(/thurly/images/bizproc/act_button_sett.gif) 50% center no-repeat;}
.activity a.activitymin {background: url(/thurly/images/bizproc/act_button_min.gif) 50% center no-repeat;}
...
td.statdel {background: url(/thurly/images/bizproc/stat_del.gif) 50% center no-repeat;}
td.statset {background: url(/thurly/images/bizproc/stat_sett.gif) 50% center no-repeat;}

.activity {}
.activity .activityhead {background: url(/thurly/images/bizproc/act_h.gif) left top repeat-x; height: 17px; overflow-y: hidden; background-color: #fec260;}
.activity .activityheadr {background: url(/thurly/images/bizproc/act_hr.gif) right top no-repeat;}
.activity .activityheadl {background: url(/thurly/images/bizproc/act_hl.gif) left top no-repeat; height:17px; padding-left: 3px;}

.activityerr {}
.activityerr .activityhead {background: url(/thurly/images/bizproc/err_act_h.gif) left top repeat-x; height: 17px; overflow-y: hidden; background-color: #ffb3b3;}
.activityerr .activityheadr {background: url(/thurly/images/bizproc/err_act_hr.gif) right top no-repeat;}
.activityerr .activityheadl {background: url(/thurly/images/bizproc/err_act_hl.gif) left top no-repeat; height:17px; padding-left: 3px;}

.activityerr a.activitydel {background: url(/thurly/images/bizproc/err_act_button_del.gif) 50% center no-repeat;}
.activityerr a.activityset {background: url(/thurly/images/bizproc/err_act_button_sett.gif) 50% center no-repeat;}
...
<script src="/thurly/js/main/public_tools.js"></script>
<script src="/thurly/js/bizproc/bizproc.js"></script>
