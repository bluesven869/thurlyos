...
if ($STAT_RIGHT>="R"):
	...
	if ($REQUEST_METHOD=="POST" && $STAT_RIGHT=="W" && strlen($RestoreDefaults)>0 && check_thurly_sessid())
	{
	...
	$cookie_name = COption::GetOptionString("main", "cookie_name", "THURLY_SM");
	...
	$arOPTIONS =	Array(
		...
		"TAB3" => Array(
			...
			"EVENT_GID_SITE_ID" => Array("EVENT_GID_SITE_ID", GetMessage("STAT_OPT_STORAGE2_EVENT_GID_SITE_ID", array("#HREF#"=>'/thurly/admin/event_edit.php?lang='.LANGUAGE_ID)), Array("text", 20)),
			"USER_EVENTS_LOAD_HANDLERS_PATH" => Array("USER_EVENTS_LOAD_HANDLERS_PATH", GetMessage("STAT_OPT_STORAGE2_USER_EVENTS_LOAD_HANDLERS_PATH", array("#HREF#"=>'/thurly/admin/event_edit.php?lang='.LANGUAGE_ID)), Array("text", 40)),
	...
	if($REQUEST_METHOD=="POST" && strlen($Update.$Apply)>0 && $STAT_RIGHT>="W" && check_thurly_sessid())
	{
	...
	if (strlen($cleanup)>0 && $REQUEST_METHOD=="POST" && $STAT_RIGHT>="W" && check_thurly_sessid())
	{
	...
	if(strlen($runsql)>0 && $REQUEST_METHOD=="POST" && $STAT_RIGHT>="W" && check_thurly_sessid())
	{
		set_time_limit(0);
		ignore_user_abort(true);
		$bDone = true;
	...
	if (strlen($optimize)>0 && $REQUEST_METHOD=="POST" && $STAT_RIGHT>="W" && check_thurly_sessid())
	{
		set_time_limit(0);
		ignore_user_abort(true);
		$fname = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/db/".strtolower($statDB->type). "/optimize.sql";
	...
	<tr>
		<td align="center" colspan=2>[ <a href="/thurly/admin/fileman_file_edit.php?lang=<?=LANGUAGE_ID?>&amp;full_src=Y&amp;path=%2Fthurly%2Factivity_limit.php"><?echo GetMessage("STAT_OPT_GRABBER_EDIT_503_TEMPLATE_LINK")?></a> ]</td>
	</tr>
	<?endif;?>
	<tr>
	...
	<tr>
		<td nowrap><label for="DEFENCE_LOG"><?echo GetMessage("STAT_OPT_DEFENCE_LOG", array("#HREF#"=>"/thurly/admin/event_log.php?lang=".LANGUAGE_ID."&set_filter=Y&find_type=audit_type_id&find_audit_type[]=STAT_ACTIVITY_LIMIT"))?></label></td>
	...
	foreach($arOPTIONS["TAB4"] as $key => $Option):
		if(!is_array($Option)):
			...
			<?elseif($key == "SKIP_STATISTIC_GROUPS"):
				...
				?>
				<tr>
					<td class="adm-detail-valign-top" width="40%"><?=GetMessage("STAT_OPT_SKIP_GROUPS_LABEL")?>:<br><img src="/thurly/images/statistic/mouse.gif" width="44" height="21" border=0 alt=""></td>
