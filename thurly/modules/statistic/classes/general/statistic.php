<?php
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/classes/general/keepstatistic.php");
...
class CAllStatistics extends CKeepStatistics
{
	...
	public static function StartBuffer()
	{
		...
		if ($STAT_RIGHT>="R")
		{
			...
			$arButtons[] = array(
				...
				"IMAGE" => "/thurly/images/statistic/page_traffic.gif",
				"ACTION" => "javascript:window.open('/thurly/admin/section_graph_list.php?lang=". LANGUAGE_ID."&public=Y&width=".$width."&height=".$height."&section=".urlencode($CURRENT_PAGE)."&set_default=Y','','target=_blank,scrollbars=yes,resizable=yes,width=".$width. ",height=".$height.",left='+Math.floor((screen.width - ".$width.")/2)+',top='+Math.floor((screen.height- ".$height.")/2))",
			);
			...
			if ($_SESSION["SHOW_LINK_STAT"]=="Y")
			{
				...
				$arButtons[] = array(
					...
					"IMAGE" => "/thurly/images/statistic/link_stat_panel.gif",
			...
			$arButtons[] = array(
				"TEXT" => $alt,
				"TITLE" => $alt,
				"IMAGE" => "/thurly/images/statistic/link_stat_show.gif",
				...
				if((count($arHashLink) > 0) && ($SUM > 0))
				{
					...
					<table border="0" cellspacing="0" cellpadding="0" width="100%">
						...
						<tr>
							...
							<td valign="top" nowrap width="40%"><img src="/thurly/images/statistic/votebar.gif" width="<?echo ($w==0) ? "0" : $w."%"?>" height="10" border=0 alt=""></td>
						</tr>
						<?endforeach?>
						<tr>
				...
				if (intval($STACK_TIME)>0)
				{
					...
					if (intval($STACK_HITS)>$MAX_STACK_HITS)
					{
						...
						if ($_SESSION["ACTIVITY_EXCEEDING_NOTIFIED"]!="Y")
						{
							...
							$SESSION_LINK = intval($_SESSION["SESS_SESSION_ID"])>0? "/thurly/admin/session_list.php?lang=". $arSite["LANGUAGE_ID"]."&find_id=".$_SESSION["SESS_SESSION_ID"]."&find_id_exact_match=Y&set_filter=Y": "";
							$VISITOR_LINK = intval($_SESSION["SESS_GUEST_ID"])>0? "/thurly/admin/guest_list.php?lang=". $arSite["LANGUAGE_ID"]."&find_id=".$_SESSION["SESS_GUEST_ID"]."&find_id_exact_match=Y&set_filter=Y": "";

							$arr = explode(".",$_SERVER["REMOTE_ADDR"]);
							$STOPLIST_LINK = "/thurly/admin/stoplist_edit.php?lang=". $arSite["LANGUAGE_ID"]."&net1=".intval($arr[0])."&net2=".intval($arr[1])."&net3=". intval($arr[2])."&net4=".intval($arr[3])."&user_agent=".urlencode($_SERVER["HTTP_USER_AGENT"]);

							$SEARCHER_LINK = intval($_SESSION["SESS_SEARCHER_ID"])>0? "/thurly/admin/hit_searcher_list.php?lang=". $arSite["LANGUAGE_ID"]."&find_searcher_id=".$_SESSION["SESS_SEARCHER_ID"]."&set_filter=Y": "";
	...
	public static function CleanUp($cleanup_date="", &$arErrors)
	{
		...
		{
			$fname = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/db/".strtolower($DB->type)."/clean_up.sql";
			if (file_exists($fname))
			{
				$arErrors = $DB->RunSQLBatch($fname);
				if (!$arErrors)
				{
					$fname = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/db/".strtolower($DB->type)."/adv.sql";
