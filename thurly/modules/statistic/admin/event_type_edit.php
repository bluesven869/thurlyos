...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $STAT_RIGHT=="W" && check_thurly_sessid())
{
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessageJS("STAT_RESET_EVENT_TYPE_CONFIRM")."'))window.location='event_type_list.php?ID=".$ID."&action=clear&lang=".LANG."&".thurly_sessid_get()."';",
	);
	$aMenu[] = array(
		"TEXT"=>GetMessage("STAT_DELETE"),
		"TITLE"=>GetMessage("STAT_DELETE_EVENT_TYPE"),
		"LINK"	=> "javascript:if(confirm('".GetMessageJS("STAT_DELETE_EVENT_TYPE_CONFIRM")."'))window.location='event_type_list.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
