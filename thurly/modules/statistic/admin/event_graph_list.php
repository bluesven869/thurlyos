			...
			<td>
				<table border="0" cellspacing="0" cellpadding="0" class="legend">
					...
					<tr>
						...
						<td>
							[<a class="stat_link" href="<?echo htmlspecialcharsbx("/thurly/admin/event_type_list.php?lang=".urlencode(LANGUAGE_ID)."&find_id=".urlencode($keyL)."&set_filter=Y")?>"><?=$keyL?></a>]&nbsp;<a class="stat_link" title="<?echo GetMessage("STAT_EVENT_DYNAMIC")?>" href="<?echo htmlspecialcharsbx("/thurly/admin/event_dynamic_list.php?lang=".urlencode(LANGUAGE_ID)."&find_event_id=".urlencode($keyL)."&find_date1=".urlencode($arFilter["DATE1"])."&find_date2=".urlencode($arFilter["DATE2"])."&set_filter=Y")?>"><?=htmlspecialcharsEx($arrL["NAME"])?></a>
						</td>
					</tr>
					<?endforeach;?>
				</table>
...
<tr valign="top">
	<td><?echo GetMessage("STAT_F_EVENTS")?><br><IMG SRC="/thurly/images/statistic/mouse.gif" WIDTH="44" HEIGHT="21" BORDER=0 ALT=""></td>
	<td><?
