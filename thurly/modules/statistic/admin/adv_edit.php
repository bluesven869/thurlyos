...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $STAT_RIGHT>="W" && check_thurly_sessid())
{
...
if(intval($ID)>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessageJS("STAT_RESET_ADV_CONFIRM")."'))window.location='adv_list.php?clear_id=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
	);
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessageJS("STAT_DELETE_ADV_CONFIRM")."'))window.location='adv_list.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
	);
}
...
<a name="tb"></a>
<?if (strlen($base_currency)<=0) : ?>
<p><?=GetMessage("STAT_BASE_CURRENCY_NOT_INSTALLED").$base_currency?>&nbsp;[&nbsp;<a href="/thurly/admin/settings.php?lang=<?=LANGUAGE_ID?>&mid=statistic"><?=GetMessage("STAT_CHOOSE_CURRENCY")?></a>&nbsp;]</p>
<?endif;?>
...
?>
	...
	<tr valign="top">
		<td valign="top"><?=GetMessage("STAT_SEARCHERS")?><br><IMG SRC="/thurly/images/statistic/mouse.gif" WIDTH="44" HEIGHT="21" BORDER=0 ALT=""><br><?=GetMessage("STAT_SELECT_WHAT_YOU_NEED")?></td>
		<td><?
			$ref = $ref_id = array();
