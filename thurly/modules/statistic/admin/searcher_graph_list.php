		...
		<td>
			...
			?>
				<tr>
					...
					<?
					if ($arrL["COUNTER_TYPE"]=="DETAIL") :
						?>[<a title="<?=GetMessage("STAT_SEARCHER_LIST_OPEN")?> " href="<?htmlspecialcharsbx("/thurly/admin/searcher_list.php?lang=".urlencode(LANGUAGE_ID)."&find_id=".urlencode($keyL)."&set_filter=Y")?>"><?=$keyL?></a>]&nbsp;<a title="<?=GetMessage("STAT_SEARCHER_DYNAMIC")?>" href="<?htmlspecialcharsbx("/thurly/admin/searcher_dynamic_list.php?lang=".urlencode(LANGUAGE_ID)."&find_searcher_id=".urlencode($keyL)."&find_date1=".urlencode($arFilter["DATE1"])."&find_date2=".urlencode($arFilter["DATE2"])."&set_filter=Y")?>"><?=$arrL["NAME"]?></a><?
					else :
...
?>
<tr valign="top">
	<td width="0%" nowrap><?echo GetMessage("STAT_F_SEACHERS")?><br><IMG SRC="/thurly/images/statistic/mouse.gif" WIDTH="44" HEIGHT="21" BORDER=0 ALT=""></td>
	<td width="0%" nowrap><?
