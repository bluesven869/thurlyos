...
if ($Load!="" && $tabControl_active_tab=="load_csv_tab" && $REQUEST_METHOD=="POST" && $STAT_RIGHT>="W" && check_thurly_sessid())
{
...
if ($Load!="" && $tabControl_active_tab=="load_manual_tab" && $STAT_RIGHT>="W" && check_thurly_sessid())
{
	...
	&& check_thurly_sessid())
{
	if(preg_match("/^(".preg_quote($upload_dir."/", "/").")[0-9a-fA-F]{32}_(in|out)\\.csv$/", $csvfile))
	{
		...
		{
			$success="N";
			$link .= "&csvfile=".urlencode($csvfile)."&time_step=".intval($time_step). "&next_line=".$next_line."&total_loaded=".$total_loaded."&total_rows=".$total_rows."&check_unique=".$check_unique."&total_duplicate=".$total_duplicate."&total_processed=".$total_processed."&next_pos=".$next_pos."&preview=".$preview."&".$tab."&".thurly_sessid_get();
		}
		elseif ($all_loaded=="Y")
		{
			$success="N";
...
if (strlen($success) <= 0 || $success == "Y")
{
	...
	{
		CAdminMessage::ShowMessage(array(
			"DETAILS" => GetMessage("STAT_BASE_CURRENCY_NOT_INSTALLED").'<br><a href="/thurly/admin/settings.php?lang='.LANGUAGE_ID.'&amp;mid=statistic">'.GetMessage("STAT_CHOOSE_CURRENCY").'</a>',
			"HTML" => true,
			"TYPE" => "ERROR",
		));
	}
...
function addNewRow(currentId)
{
	...
	oCell = oRow.insertCell(i++);
	oCell.innerHTML = '<a href="javascript: Copy('+id+')"><img src="/thurly/images/statistic/copy.gif" width="15" height="15" border=0 class="tb2" hspace="2" alt="<?echo GetMessage("STAT_COPY")?>"></a>';
...
function SelectHandler()
{
	...
	fileman_edit_link = "/thurly/admin/fileman_file_edit.php?lang=<?=LANGUAGE_ID?>&full_src=Y&path=";
	<?=$strJavaUserHandlerArray?>
...
<?$tabControl->BeginNextTab();?>
	<tr>
		...
		<tr>
			...
			<td><a href="javascript: Copy(<?=$i?>)"><img src="/thurly/images/statistic/copy.gif" width="15" height="15" border=0 class="tb2" hspace="2" alt="<?echo GetMessage("STAT_COPY")?>"></a></td>
		</tr>
		<?
		$i++;
		endwhile;
