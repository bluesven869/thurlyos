...
if(intval($ID)>0)
{
	...
	if ($STAT_RIGHT>="W")
	{
		$aMenu[] = array(
			...
			"LINK"	=> "javascript:if(confirm('".GetMessageJS("STAT_DELETE_STOPLIST_CONFIRM")."'))window.location='stoplist_list.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		);
	}
}
...
<?
$tabControl->Buttons(Array("disabled" =>$STAT_RIGHT<"W" ,"back_url" =>"/thurly/admin/stoplist_list.php?lang=".LANG."&set_filter=Y"));
$tabControl->End();
?>
</form>
