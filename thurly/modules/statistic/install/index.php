...
Class statistic extends CModule
{
	...
	function statistic()
	{
		...
		$this->MODULE_CSS = "/thurly/modules/statistic/statistic.css";
	}
	...
	function InstallDB($arParams = array())
	{
		...
		if($no_tables == "Y")
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/db/".strtolower($DB->type)."/install.sql");
		}
		...
		if($no_tables=="Y")
		{
			$arAllErrors[] = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]. "/thurly/modules/statistic/install/db/".strtolower($DB->type)."/searchers.sql");
			$arAllErrors[] = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]. "/thurly/modules/statistic/install/db/".strtolower($DB->type)."/browsers.sql");
			$arAllErrors[] = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]. "/thurly/modules/statistic/install/db/".strtolower($DB->type)."/adv.sql");
		}

		// ip-to-country
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/ip_tools.php");
		i2c_load_countries();
		...
		$fname = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/db/".strtolower($DB->type)."/optimize.sql";
	...
	function UnInstallDB($arParams = array())
	{
		...
		if(is_object($DB))
		{
			...
			{
				$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/db/".strtolower($DB->type)."/uninstall.sql");
	...
	function InstallEvents()
	{
		...
		if($ar["C"] <= 0)
		{
			include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/events/set_events.php");
		}
		return true;
	}
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/public/thurly", $_SERVER["DOCUMENT_ROOT"]."/thurly", true, true);//all from thurly
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/components/thurly", $_SERVER["DOCUMENT_ROOT"]."/thurly/components/thurly", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/gadgets/thurly", $_SERVER["DOCUMENT_ROOT"]."/thurly/gadgets/thurly", true, true);
		}
		return true;
	}
	...
	function UnInstallFiles()
	{
		if($_ENV["COMPUTERNAME"]!='BX')
		{
			DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/public/thurly/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
			DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/public/thurly/themes/.default/", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes/.default");//css
			DeleteDirFilesEx("/thurly/themes/.default/icons/statistic/");//icons
			DeleteDirFilesEx("/thurly/images/statistic/");//images
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if(!CBXFeatures::IsFeatureEditable("Analytics"))
		{
			...
			$APPLICATION->ThrowException(implode("<br>", $this->errors));
			$APPLICATION->IncludeAdminFile(GetMessage("STAT_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/step2.php");
		}
		elseif($step < 2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage("STAT_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/step1.php");
		}
		elseif($step == 2)
		{
	...
	function DoUninstall()
	{
		...
		if ($STAT_RIGHT>="W")
		{
			...
			if($step < 2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("STAT_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/unstep1.php");
			}
			elseif($step == 2)
			{
				...
				$GLOBALS["errors"] = $this->errors;
				$APPLICATION->IncludeAdminFile(GetMessage("STAT_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/statistic/install/unstep2.php");
			}
		}
	}
