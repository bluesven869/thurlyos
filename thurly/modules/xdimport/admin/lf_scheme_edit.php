	...
	)
	{
		...
		if($res)
		{
			...
			if(isset($_REQUEST["apply"]))
				LocalRedirect("/thurly/admin/xdi_lf_scheme_edit.php?ID=".$res."&lang=".LANGUAGE_ID);
			else
				LocalRedirect("/thurly/admin/xdi_lf_scheme_list.php?lang=".LANGUAGE_ID);
		}
		else
		{
...
if($ID > 0)
{
	...
	if ($arRes = $rs->fetch())
	{
		...
		if ($arRes["TYPE"] == "XML")
		{
			if ($arRes["PAGE"] == "/thurly/tools/stat_gadget.php" && $arRes["METHOD"] == "GetLiveFeedData")
			{
				$predefined = "stat";
			}
			if ($arRes["PAGE"] == "/thurly/tools/sale_gadget.php" && $arRes["METHOD"] == "GetLiveFeedData")
			{
				$predefined = "sale";
			}
		}
					...
					<span id="LF_RIGHTS_USER_US" style="display: <?=(is_array($arRes["RIGHTS_USER_ID"]) && !in_array($arRes["RIGHTS_USER_ID"][0], array("UA", "UN")) ? "block" : "none")?>;">
						...
						)
						{
							...
							foreach($arRes["RIGHTS_USER_ID"] as $key=>$user_id_tmp)
							{
								?><tr>
									<td>
										<a href="javascript:void(0)" onClick="lfpDropRow(this)" style="display: inline-block; vertical-align: middle; cursor: pointer;"><img border="0" width="20" height="20" src="/thurly/themes/.default/images/actions/delete_button.gif"></a><?
										$sUser = "";
										if($user_id_tmp > 0)
										{
							...
							var q = function () {
								...
								if (tv_xxx != BX('RIGHTS_USER_ID_' + iTmp).value)
								{
									...
									if (tv_xxx != '')
									{
										...
										BX("hiddenframeRIGHTS_USER_ID_" + iTmp).src='/thurly/admin/get_user.php?ID=' + tv_xxx+'&strName=RIGHTS_USER_ID_' + iTmp + '&lang=<?=LANGUAGE_ID?>&admin_section=Y';
									}
									else
									{
										DV_xxx.innerHTML = '';
									...
									BX.create('td', {
										children: [
												...
												children: [
													BX.create('img', {
														attrs: {
															...
															'width': '20',
															'height': '20',
															'src': '/thurly/themes/.default/images/actions/delete_button.gif'
														}
													})
												]
											}),
											...
											BX.create('input', {
												...
												events: {
													'click': function() { window.open('/thurly/admin/user_search.php?lang=<?=LANGUAGE_ID?>&FN=editform&FC=RIGHTS_USER_ID_' + iTmp, '', 'scrollbars=yes,resizable=yes,width=760,height=500,top='+Math.floor((screen.height - 560)/2-14)+',left='+Math.floor((screen.width - 760)/2-5)); }
												}
											}),
											BX.create('span', {
												html: ' '
							...
							function ___LFPChangePredefined(el)
							{
								...
								)
								{
									...
									if (el.options[el.selectedIndex].value == "stat")
										BX("LF_PAGE").value = "/thurly/tools/stat_gadget.php";
									else
										BX("LF_PAGE").value = "/thurly/tools/sale_gadget.php";
								}
								else
								{
	...
	elseif (in_array($scheme_type, array("POST")))
	{
		...
		<tr>
			...
			<td><?
				...
				$point_page = $protocol."://".$server_name.COption::GetOptionString("xdimport", "point_page", "/thurly/tools/xdi_livefeed.php");
				?><?=$point_page?></td>
		</tr>
		<tr>
