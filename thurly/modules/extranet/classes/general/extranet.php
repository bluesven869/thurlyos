...
class CExtranet
{
	...
	*/
	public static function getMyGroupsUsersSimple($extranetSite, $params = array())
	{
		...
		if (CModule::IncludeModule('socialnetwork'))
		{
			$query = new \Thurly\Main\Entity\Query(\Thurly\Socialnetwork\UserToGroupTable::getEntity());
			...
			$res = \Thurly\Socialnetwork\UserToGroupTable::getList(array(
				'order' => array(),
				'filter' => array(
					'@GROUP_ID' => new \Thurly\Main\DB\SqlExpression($subQuery),
					'<=ROLE' => \Thurly\Socialnetwork\UserToGroupTable::ROLE_USER,
					'=USER.ACTIVE' => 'Y',
				),
	...
	public static function fillUserListFilterORM($arParams = array(), &$arFilter = array(), &$bFilteredByUserId)
	{
		...
		$bEmailUsersAll = (
			isset($arParams["EMAIL_USERS_ALL"])
				...
				: (IsModuleInstalled('mail') && \Thurly\Main\Config\Option::get('socialnetwork', 'email_users_all', 'N') == 'Y')
		);
