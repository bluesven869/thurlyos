...
while ($arInstance = $dbRes->Fetch())
{
	...
	)
	{
		...
		if($res)
		{
			$fileList = \Thurly\Main\UI\FileInputUtility::instance()->checkFiles('MEETING_ITEM_REPORT_FILES_'.$arInstance['ID'], $_REQUEST['FILES']);
			$deletedFileList = \Thurly\Main\UI\FileInputUtility::instance()->checkDeletedFiles('MEETING_ITEM_REPORT_FILES_'.$arInstance['ID']);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_REQUEST['save_item'] && check_thurly_sessid())
{
	...
	$arFields['FILES'] = is_array($_REQUEST['ITEM_FILES'])
		? \Thurly\Main\UI\FileInputUtility::instance()->checkFiles('MEETING_ITEM_FILES_'.$arParams['ITEM_ID'], $_REQUEST['ITEM_FILES'])
		: array();
