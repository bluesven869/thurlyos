...
Class meeting extends CModule
{
	...
	function InstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/meeting/install/db/".strtolower($DB->type)."/install.sql");
		}
	...
	function UnInstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/meeting/install/db/".strtolower($DB->type)."/uninstall.sql");
	...
	function InstallFiles($arParams = array())
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/meeting/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", True, True);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/meeting/install/js", $_SERVER["DOCUMENT_ROOT"]."/thurly/js", True, True);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/meeting/install/tools", $_SERVER["DOCUMENT_ROOT"]."/thurly/tools", True, True);
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if(!CBXFeatures::IsFeatureEditable("Meeting"))
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("MEETING_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/meeting/install/step2.php");
		}
		elseif($step < 2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage("MEETING_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/meeting/install/step1.php");
		}
		elseif($step==2)
		{
	...
	function DoUninstall()
	{
		...
		if($USER->IsAdmin())
		{
			...
			if($step < 2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("MEETING_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/meeting/install/unstep1.php");
			}
			elseif($step == 2)
			{
				...
				$GLOBALS["errors"] = $this->errors;
				$APPLICATION->IncludeAdminFile(GetMessage("MEETING_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/meeting/install/unstep2.php");
			}
		}
	}
}
