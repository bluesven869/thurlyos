...
class CBlogNotifySchema
{
	...
	public static function CBlogEventsIMCallback($module, $tag, $text, $arNotify)
	{
		if ($module == "blog")
		{
			...
			$perm = \Thurly\Blog\Item\Permissions::DENY;

			if($post["AUTHOR_ID"] == $currentUserId)
			{
				$perm = \Thurly\Blog\Item\Permissions::FULL;
			}
			else
			{
				$postPerm = CBlogPost::getSocNetPostPerms($post["ID"]);
				if ($postPerm > \Thurly\Blog\Item\Permissions::DENY)
				{
			...
			if ($perm == \Thurly\Blog\Item\Permissions::PREMODERATE)
			{
