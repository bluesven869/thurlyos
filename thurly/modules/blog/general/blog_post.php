...
//			preserve collision between numeric code and post ID.
	...
	public static function Notify($arPost, $arBlog, $arParams)
	{
		...
		)
		{
			...
			$arSoFields = Array(
				"EVENT_ID" => (
					...
					&& intval($arPost["UF_BLOG_POST_IMPRTNT"]) > 0
						? \Thurly\Blog\Integration\Socialnetwork\Log::EVENT_ID_POST_IMPORTANT
						: \Thurly\Blog\Integration\Socialnetwork\Log::EVENT_ID_POST
				),
				"=LOG_DATE" => (
			...
			$post = \Thurly\Blog\Item\Post::getById($arPost["ID"]);
			...
			if (intval($logID) > 0)
			{
				$socnetPerms = \Thurly\Socialnetwork\ComponentHelper::getBlogPostSocNetPerms(array(
					'postId' => $arPost["ID"],
				...
				\Thurly\Main\FinderDestTable::merge(array(
					"CONTEXT" => "blog_post",
					"CODE" => \Thurly\Main\FinderDestTable::convertRights($socnetPerms, array("U".$arPost["AUTHOR_ID"]))
				));
				...
				if (preg_match_all('/\[DISK\s+FILE\s+ID\s*=\s*([n]*\d+)\s*\]/is'.BX_UTF_PCRE_MODIFIER, $postFields['DETAIL_TEXT'], $matches))
				{
					if (!empty($matches[1]))
					{
						...
						foreach($inlineFileList as $key => $value)
						{
							...
							{
								$res = \Thurly\Disk\AttachedObject::getList(array(
									'filter' => array(
										'=ENTITY_TYPE' => \Thurly\Disk\Uf\BlogPostConnector::className(),
				...
				$hasVideoTransforming = (
					...
					&& Thurly\Blog\Integration\Disk\Transformation::getStatus(array(
	...
	public static function NotifyImReady($arParams = array())
	{
		...
		if (!empty($moderatorList))
		{
			...
			$bTitleEmpty = $arTitle['IS_TITLE_EMPTY'];
			$serverName = (CMain::IsHTTPS() ? "https" : "http")."://".((defined("SITE_SERVER_NAME") && strlen(SITE_SERVER_NAME) > 0) ? SITE_SERVER_NAME : COption::GetOptionString("main", "server_name", ""));
			$moderationUrl = \Thurly\Main\Config\Option::get('socialnetwork', 'workgroups_page', SITE_DIR.'workgroups/').'group/#group_id#/blog/moderation/';
	...
	public static function NotifyMail($arFields)
	{
		...
		$arEmail = \Thurly\Mail\User::getUserData($arFields["userId"], $nameTemplate);
		if (empty($arEmail))
		{
			return false;
		}
		...
		$defaultEmailFrom = \Thurly\Mail\User::getDefaultEmailFrom();
		...
		foreach ($arEmail as $userId => $arUser)
		{
			...
			$res = \Thurly\Mail\User::getReplyTo(
				$arFields["siteId"],
				$userId,
				'BLOG_POST',
				$arFields["postId"],
