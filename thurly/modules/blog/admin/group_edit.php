...
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $blogModulePermissions>="W" && check_thurly_sessid())
{
	...
	if (strlen($errorMessage) <= 0)
	{
		...
		if (strlen($apply) <= 0)
			LocalRedirect("/thurly/admin/blog_group.php?lang=".LANG."&".GetFilterParams("filter_", false));
	}
	else
	{
		$bVarsFromForm = true;
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/blog_group.php?lang=".LANG."&".GetFilterParams("filter_", false)
	)
);
...
if ($ID > 0 && $blogModulePermissions >= "W")
{
	...
	$aMenu[] = array(
			...
			"ICON" => "btn_new",
			"LINK" => "/thurly/admin/blog_group_edit.php?lang=".LANG."&".GetFilterParams("filter_", false)
		);

	$aMenu[] = array(
			"TEXT" => GetMessage("BLGE_DELETE_GROUP"), 
			"ICON" => "btn_delete",
			"LINK" => "javascript:if(confirm('".GetMessage("BLGE_DELETE_GROUP_CONFIRM")."')) window.location='/thurly/admin/blog_group.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."#tb';",
			"WARNING" => "Y"
		);
}
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/blog_group.php?lang=".LANG."&".GetFilterParams("filter_", false)
			)
	);
?>
