...
while($arSite = $dbSite->Fetch())
{
	...
	$serverName = \Thurly\Main\Text\HtmlFilter::encode($serverName);
...
while ($arComment = $dbResultList->Fetch())
{
	...
	if(IntVal($arComment["AUTHOR_ID"]) > 0)
		$row->AddField("AUTHOR_ID", "[<a href=\"/thurly/admin/user_edit.php?ID=".$arComment["AUTHOR_ID"]."&lang=".LANG."\">".$arComment["AUTHOR_ID"]."</a>] ".htmlspecialcharsEx("(".$arComment["USER_LOGIN"].") ".$arComment["USER_NAME"]." ".$arComment["USER_LAST_NAME"]));
	...
	if(IntVal($arComment["BLOG_ID"]) > 0)
		$row->AddField("BLOG_ID", "[<a href=\"/thurly/admin/blog_blog_edit.php?ID=".$arComment["BLOG_ID"]."&lang=".LANG."\">".$arComment["BLOG_ID"]."</a>] ".htmlspecialcharsEx($arComment["BLOG_NAME"].""));
