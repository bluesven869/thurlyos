...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/include.php");
...
$ID = IntVal($ID);
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $blogModulePermissions>="W" && check_thurly_sessid())
{
	$arFields = array(
		"NAME" => $NAME,
	...
	if (strlen($errorMessage) <= 0)
	{
		...
		if (strlen($apply) <= 0)
			LocalRedirect("/thurly/admin/blog_blog.php?lang=".LANG.GetFilterParams("filter_", false));
		else
			LocalRedirect("/thurly/admin/blog_blog_edit.php?lang=".LANG."&ID=".$ID."&".$tabControl->ActiveTabParam());
	}
	else
	{
		$bVarsFromForm = true;
...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/prolog.php");
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/blog_blog.php?lang=".LANG."&".GetFilterParams("filter_", false)
	)
);
...
if ($ID > 0 && $blogModulePermissions >= "W")
{
	...
	$aMenu[] = array(
			...
			"ICON" => "btn_new",
			"LINK" => "/thurly/admin/blog_blog_edit.php?lang=".LANG."&".GetFilterParams("filter_", false)
		);

	$aMenu[] = array(
			"TEXT" => GetMessage("BLBE_DELETE_BLOG"), 
			"ICON" => "btn_delete",
			"LINK" => "javascript:if(confirm('".GetMessage("BLBE_DELETE_BLOG_CONFIRM")."')) window.location='/thurly/admin/blog_blog.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."#tb';",
			"WARNING" => "Y"
		);
}
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/blog_blog.php?lang=".LANG."&".GetFilterParams("filter_", false)
			)
	);
?>
