...
use Thurly\Socialnetwork\LogFollowTable;
...
class Broadcast
{
	...
	private function sendRequest($value, $siteId = SITE_ID)
	{
		...
		if (Loader::includeModule('im'))
		{
			...
			$moduleAdminList = array_keys(\Thurly\Socialnetwork\User::getModuleAdminList(array($siteId, false)));
	...
	public static function send($params)
	{
		...
		if ($found)
		{
			$userList = \Thurly\Intranet\Util::getEmployeesList($userListParams);
		}
		...
		)
		{
			...
			if (!empty($userIdList))
			{
				...
				\Thurly\Pull\Push::add($userIdListPush, Array(
					'module_id' => 'blog',
					'push' => Array(
						'message' => $message,
						'params' => array(
