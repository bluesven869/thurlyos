...
namespace Thurly\Blog\Internals;

use Thurly\Blog\Item\Blog;
use Thurly\Main;
use Thurly\Main\Loader;
use Thurly\Main\Localization\Loc;
use Thurly\Main\Config;
use Thurly\Blog\Item\Permissions;
...
final class MailHandler
{
	 ...
	 * @return int|false
	 */
	public static function handleReplyReceivedBlogPost(\Thurly\Main\Event $event)
	{
		...
		if (!empty($siteId))
		{
			$fields["SEARCH_GROUP_ID"] = \Thurly\Main\Config\Option::get("socialnetwork", "userbloggroup_id", false, $siteId);
		}
		...
		{
			\Thurly\Disk\Uf\FileUserType::setValueForAllowEdit("BLOG_COMMENT", true);
		}
		...
		if ($commentId)
		{
			\Thurly\Blog\Item\Comment::actionsAfter(array(
				'MESSAGE' => $message,
				'BLOG_ID' => $blogPost["BLOG_ID"],
	 ...
	 * @return int|false
	 */
	public static function handleForwardReceivedBlogPost(\Thurly\Main\Event $event)
	{
		$userId = intval($event->getParameter('from'));
		$message = trim($event->getParameter('content'));
		...
		if ($blog)
		{
			...
			if (Loader::includeModule('disk'))
			{
				\Thurly\Disk\Uf\FileUserType::setValueForAllowEdit("BLOG_POST", true);
			}
