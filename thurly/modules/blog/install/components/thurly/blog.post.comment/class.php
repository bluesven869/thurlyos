...
class CBlogPostCommentEdit extends CThurlyComponent
{
						...
						else
						{
							...
							if(IntVal($this->arParams["ID"]) > 0)
							{
								...
								while ($arImage = $res->Fetch())
								{
									...
									$currImage = array(
										"small" => "/thurly/components/thurly/blog/show_file.php?fid=".$arImage['ID']."&width=70&height=70&type=square",
										"full" => "/thurly/components/thurly/blog/show_file.php?fid=".$arImage['ID']."&width=1000&height=1000"
									);
								...
								$blogUser = new Thurly\Blog\BlogUser($this->arParams["CACHE_TIME"]);
								$blogUser->setBlogId($arBlog["ID"]);
								$commentsUsers = $blogUser->getUsers(\Thurly\Blog\BlogUser::getCommentAuthorsIdsByPostId($arPost['ID']));
...
//
											if(empty($this->arResult["USER_CACHE"][$arComment["AUTHOR_ID"]]))
											{
												...
												$arUsrTmp["AuthorName"] = Thurly\Blog\BlogUser::GetUserNameEx(
...
//									from user cache
										...
										if($this->arResult["canModerate"])
										{
											...
											{
												if(IntVal($arComment["AUTHOR_ID"]) > 0)
													$arComment["urlToSpam"] = "/thurly/admin/blog_comment.php?lang=ru&set_filter=Y&filter_author_id=".$arComment["AUTHOR_ID"];
												elseif(strlen($arComment["AUTHOR_IP"]) > 0)
													$arComment["urlToSpam"] = "/thurly/admin/blog_comment.php?lang=ru&set_filter=Y&filter_author_anonym=Y&filter_author_ip=".$arComment["AUTHOR_IP"];
												else
													$arComment["urlToSpam"] = "/thurly/admin/blog_comment.php?lang=ru&set_filter=Y&filter_author_anonym=Y&filter_author_email=".$arComment["AUTHOR_EMAIL"];
											}
										}
...
//				default lifetime - 1 day
	...
	private function isUserGivenConsent()
	{
		...
		{
			$this->arParams["USER_CONSENT_WAS_GIVEN"] = \Thurly\Blog\BlogUser::isUserGivenConsent(
