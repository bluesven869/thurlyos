...
else
{
	...
	?><script>window.BX = top.BX;
		...
		{
			?>
				BX('captcha').src='/thurly/tools/captcha.php?captcha_code=' + '<?=$arResult["CaptchaCode"]?>';
				...
				if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
				{
					...
					<div class="blog-comment-content">
						...
						if(strlen($comment["urlToHide"])>0)
						{
							...
							<span class="blog-comment-show">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a href="javascript:void(0)" onclick="return hideShowComment('<?=$comment["urlToHide"]."&".thurly_sessid_get()?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_HIDE")?>">
								<?else:?>
									<a href="<?=$comment["urlToHide"]."&".thurly_sessid_get()?>" title="<?=GetMessage("BPC_MES_HIDE")?>">
								<?endif;?>
						...
						if(strlen($comment["urlToDelete"])>0)
						{
							...
							<span class="blog-comment-delete">
								<?if($arParams["AJAX_POST"] == "Y"):?>
									<a href="javascript:void(0)" onclick="if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) deleteComment('<?=$comment["urlToDelete"]."&".thurly_sessid_get()?>', '<?=$comment["ID"]?>');" title="<?=GetMessage("BPC_MES_DELETE")?>">
								<?else:?>
									<a href="javascript:if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$comment["urlToDelete"]."&".thurly_sessid_get()?>'" title="<?=GetMessage("BPC_MES_DELETE")?>">
								<?endif;?>
