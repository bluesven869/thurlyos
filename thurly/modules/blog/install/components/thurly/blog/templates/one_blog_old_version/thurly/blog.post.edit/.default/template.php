...
{	
	...
	else
	{
		...
		<?=thurly_sessid_post();?>
								...
								<table class="blog-blog-edit-table-smiles">
									...
									foreach($arResult["Smiles"] as $arSmiles)
									{
										if ($ind == 0)
											echo "<tr>";
										?>
											<td align="center"><img src="/thurly/images/blog/smile/<?=$arSmiles["IMAGE"]?>" width="<?=$arSmiles["IMAGE_WIDTH"]?>" height="<?=$arSmiles["IMAGE_HEIGHT"]?>" title="<?=$arSmiles["LANG_NAME"]?>" OnClick="emoticon('<?=$arSmiles["TYPE"]?>')" style="cursor:pointer"></td>
										<?
										if($ind == 1)
										{
											echo "</tr>";
			...
			{
				?>
				<script type="text/javascript" src="/thurly/js/main/ajax.js"></script>
				<script type="text/javascript" src="/thurly/js/main/admin_tools.js"></script>
				<script type="text/javascript" src="/thurly/js/main/utils.js"></script>
				<?
				//$APPLICATION->AddHeadScript("/thurly/js/main/ajax.js");
				...
				$APPLICATION->SetAdditionalCSS("/thurly/admin/htmleditor2/editor.css");
				...
				<td>
					...
					*/?>
					<div id="category-new">
						...
						{
							$APPLICATION->IncludeComponent("thurly:search.tags.input", ".default", Array(
								"NAME"	=>	"TAGS",
			...
			<?if($USER->IsAdmin()):?>
				<tr>
					<td colspan="2"><a href="/thurly/admin/userfield_edit.php?ENTITY_ID=BLOG_POST&back_url=<?=$arResult["CUR_PAGE"]?>"><?=GetMessage("BLOG_POST_PROPERTY_ADD")?></a></th>
				</tr>
			<?endif;?>
			</table>
			<?
