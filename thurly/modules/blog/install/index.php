<?
global $MESS;
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/index.php");
...
Class blog extends CModule
{
	...
	function InstallDB($install_wizard = true)
	{
		...
		{
			$errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/".$DBType."/install.sql");
		...
		RegisterModuleDependences('conversion', 'OnGetCounterTypes' , 'blog', '\Thurly\Blog\Internals\ConversionHandlers', 'onGetCounterTypes');
		RegisterModuleDependences('conversion', 'OnGetRateTypes' , 'blog', '\Thurly\Blog\Internals\ConversionHandlers', 'onGetRateTypes');
		RegisterModuleDependences('blog', 'OnPostAdd', 'blog', '\Thurly\Blog\Internals\ConversionHandlers', 'onPostAdd');

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->registerEventHandler('mail', 'onReplyReceivedBLOG_POST', 'blog', '\Thurly\Blog\Internals\MailHandler', 'handleReplyReceivedBlogPost');
		$eventManager->registerEventHandler('mail', 'onForwardReceivedBLOG_POST', 'blog', '\Thurly\Blog\Internals\MailHandler', 'handleForwardReceivedBlogPost');
		$eventManager->registerEventHandler('socialnetwork', 'onLogIndexGetContent', 'blog', '\Thurly\Blog\Integration\Socialnetwork\Log', 'onIndexGetContent');
		$eventManager->registerEventHandler('socialnetwork', 'onLogCommentIndexGetContent', 'blog', '\Thurly\Blog\Integration\Socialnetwork\LogComment', 'onIndexGetContent');
		$eventManager->registerEventHandler('socialnetwork', 'onContentViewed', 'blog', '\Thurly\Blog\Integration\Socialnetwork\ContentViewHandler', 'onContentViewed');
	...
	function UnInstallDB($arParams = Array())
	{
		...
		if(array_key_exists("savedata", $arParams) && $arParams["savedata"] != "Y")
		{
			...
			$errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/".$DBType."/uninstall.sql");
		...
		UnRegisterModuleDependences('conversion', 'OnGetCounterTypes' , 'blog', '\Thurly\Blog\Internals\ConversionHandlers', 'onGetCounterTypes');
		UnRegisterModuleDependences('conversion', 'OnGetRateTypes' , 'blog', '\Thurly\Blog\Internals\ConversionHandlers', 'onGetRateTypes');
		UnRegisterModuleDependences('blog', 'OnPostAdd', 'blog', '\Thurly\Blog\Internals\ConversionHandlers', 'onPostAdd');

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->unregisterEventHandler('mail', 'onReplyReceivedBLOG_POST', 'blog', '\Thurly\Blog\Internals\MailHandler', 'handleReplyReceivedBlogPost');
		$eventManager->unregisterEventHandler('mail', 'onForwardReceivedBLOG_POST', 'blog', '\Thurly\Blog\Internals\MailHandler', 'handleForwardReceivedBlogPost');
		$eventManager->unregisterEventHandler('socialnetwork', 'onLogIndexGetContent', 'blog', '\Thurly\Blog\Integration\Socialnetwork\Log', 'onIndexGetContent');
		$eventManager->unregisterEventHandler('socialnetwork', 'onLogCommentIndexGetContent', 'blog', '\Thurly\Blog\Integration\Socialnetwork\LogComment', 'onIndexGetContent');
		$eventManager->unregisterEventHandler('socialnetwork', 'onContentViewed', 'blog', '\Thurly\Blog\Integration\Socialnetwork\ContentViewHandler', 'onContentViewed');
	...
	function InstallEvents()
	{
		...
		if($ar["C"] <= 0)
		{
			include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/events/set_events.php");
		}
		return true;
	}
	...
	{
		global $DB;
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/events/del_events.php");
		return true;
	}
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin", true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/images",  $_SERVER["DOCUMENT_ROOT"]."/thurly/images/blog", true, True);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/themes", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/public/templates", $_SERVER["DOCUMENT_ROOT"]."/thurly/templates", true, true);
		}
		...
		foreach($arSite as $fSite)
		{
			...
			if (${"install_public_".$fSite["SITE_ID"]} == "Y" && !empty(${"public_path_".$fSite["SITE_ID"]}))
			{
				...
				CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/public/".$folder, $fSite['ABS_DOC_ROOT'].$fSite["DIR"].$public_dir, $bReWritePublicFiles, true);
				if ($folder == "SEF")
				{
	...
	function UnInstallFiles()
	{
		if($_ENV["COMPUTERNAME"]!='BX')
		{
			DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
			DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/themes/.default/", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes/.default");//css
			DeleteDirFilesEx("/thurly/themes/.default/icons/blog/");//icons
			DeleteDirFilesEx("/thurly/images/blog/");//images
		}
	...
	function DoInstall()
	{
		...
		$step = IntVal($step);
		if ($step < 2)
			$APPLICATION->IncludeAdminFile(GetMessage("BLOG_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/step1.php");
		elseif($step==2)
		{
			$this->InstallFiles();
			...
			$APPLICATION->IncludeAdminFile(GetMessage("BLOG_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/step2.php");
		}
	}
	...
	function DoUninstall()
	{
		...
		$step = IntVal($step);
		if($step<2)
			$APPLICATION->IncludeAdminFile(GetMessage("BLOG_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/unstep1.php");
		elseif($step==2)
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("BLOG_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/blog/install/unstep2.php");
		}
	}
