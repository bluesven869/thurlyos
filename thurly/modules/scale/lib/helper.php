...
class Helper
{
	...
	{
		if(intval($minutes) <= 0)
			throw new \Thurly\Main\ArgumentNullException("minutes");

		$now = time();

		$contents = file_get_contents(\Thurly\Main\Application::getDocumentRoot().'/thurly/modules/scale/server_off.html');
	...
	{
		if(strlen($DBHost) <= 0)
			throw new \Thurly\Main\ArgumentNullException("DBHost");
		if(strlen($DBName) <= 0)
			throw new \Thurly\Main\ArgumentNullException("DBName");
		if(strlen($DBLogin) <= 0)
			throw new \Thurly\Main\ArgumentNullException("DBLogin");

		$filename = \Thurly\Main\Application::getDocumentRoot()."/thurly/php_interface/dbconn.php";
		...
		file_put_contents(\Thurly\Main\Application::getDocumentRoot()."/thurly/php_interface/dbconn.php.bak", $content);
	...
	{
		$filename = $_SERVER['DOCUMENT_ROOT']."/thurly/.settings-test.php";
		...
		rename($filename, $_SERVER['DOCUMENT_ROOT']."/thurly/.settings-test.php.bak");
