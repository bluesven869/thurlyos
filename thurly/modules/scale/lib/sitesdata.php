...
use Thurly\Main\SiteDomainTable;
...
class SitesData
{
	...
	public static function getSite($siteName, $dbName = false)
	{
		if(strlen($siteName) <= 0)
			throw new \Thurly\Main\ArgumentNullException("siteName");
		...
		if($hitCache === null)
		{
			...
			if($execRes)
			{
				...
				while ($site = $rsSite->fetch())
				{
					...
					{
						$docRoot = strlen($site["DOC_ROOT"]) > 0 ? $site["DOC_ROOT"] : \Thurly\Main\Application::getDocumentRoot();
