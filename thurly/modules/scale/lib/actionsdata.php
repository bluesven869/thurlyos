...
class ActionsData
{
	...
	public static function getAction($actionId)
	{
		if(strlen($actionId) <= 0)
			throw new \Thurly\Main\ArgumentNullException("actionId");
	...
	public static function getActionObject($actionId, $serverHostname = "", array $userParams = array(), array $freeParams = array(), array $actionParams = array())
	{
		...
		if(!is_array($userParams))
			throw new \Thurly\Main\ArgumentTypeException("userParams", "array");

		if(!is_array($userParams))
			throw new \Thurly\Main\ArgumentTypeException("freeParams", "array");

		if(!is_array($actionParams))
			throw new \Thurly\Main\ArgumentTypeException("actionParams", "array");
	 ...
	 * @param string $bid -     action thurly idetifyer
	 * @return array
	 */
	...
	public static function getList($checkConditions = false)
	{
		...
		if($def == null)
		{
			$filename = \Thurly\Main\Application::getDocumentRoot()."/thurly/modules/scale/include/actionsdefinitions.php";
