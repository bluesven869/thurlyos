...
class Monitoring
{
	...
	{
		$dir = new \Thurly\Main\IO\Directory(static::$rrdPath."/".$hostname);
		return $dir->isExists();
	}
	...
	public static function getLoadBarValue($hostname, $roleId)
	{
		...
		if(strlen($hostname) <= 0)
			throw new \Thurly\Main\ArgumentNullException("hostname");
		...
		$file = new \Thurly\Main\IO\File($rrdPath);

		if(!$file->isExists())
			throw new \Thurly\Main\IO\FileNotFoundException($rrdPath);
	...
	public static function getInfoTableCategory($hostname, $categoryId)
	{
		...
		if(strlen($categoryId) <= 0)
			throw new \Thurly\Main\ArgumentNullException("paramId");
		...
		$result["MEMORY"] = array(
			...
			"PARAMS" => array(
				array(
					...
					"ITEMS" => array(
						array(
							"VALUE_FUNC" => '\Thurly\Scale\Monitoring::getMemoryUsage',
							"FUNC_PARAMS" => array($hostname)
						),
						array(
							"VALUE_FUNC" => '\Thurly\Scale\Monitoring::getMemoryUsageValue',
							"FUNC_PARAMS" => array($hostname),
							"TYPE"=>"LOADBAR"
						)
					)
	...
	public static function getValue($hostname, $categoryId, $param)
	{
		...
		if(strlen($categoryId) <= 0)
			throw new \Thurly\Main\ArgumentNullException("categoryId");

		if(strlen($param) <= 0)
			throw new \Thurly\Main\ArgumentNullException("param");
		...
		if(isset($arData["ansible_facts"]["ansible_mounts"]))
		{
			foreach($arData["ansible_facts"]["ansible_mounts"] as $mountId => $mount)
			{
				$result[$mountId] = array(
					...
					"ITEMS" => array(
						array(
							"VALUE_FUNC" => '\Thurly\Scale\Monitoring::getHddsValues',
							"FUNC_PARAMS" => array(
								$hostname,
								$mountId
							)
						...
						array(
							"TYPE" => "LOADBAR",
							"VALUE_FUNC" => '\Thurly\Scale\Monitoring::getHddsUsed',
							"FUNC_PARAMS" => array(
								$hostname,
								$mountId
							)
		...
		foreach ($arChildren as $child)
		{
			...
			if($pos1 !== false && $pos2 !== false)
			{
				...
				$result[$dev] = array(
					...
					{
						$result = \Thurly\Scale\Monitoring::formatSize($data["calcpr"][0]/600).
							"&nbsp;/&nbsp;".
							\Thurly\Scale\Monitoring::formatSize($data["calcpr"][1]/600)."&nbsp;'.Helper::nbsp(Loc::getMessage("SCALE_MONITORING_NET_SEC")).'";
					}
					return $result;'
				);
			}
						...
						array(
							...
							"DATA_FUNC" => '
								...
								if(isset($data["calcpr"][0]) && isset($data["calcpr"][1]))
								{
									$result = \Thurly\Scale\Monitoring::formatSize($data["calcpr"][0]/600).
										"&nbsp;/&nbsp;".
										\Thurly\Scale\Monitoring::formatSize($data["calcpr"][1]/600)."&nbsp;'.Loc::getMessage("SCALE_MONITORING_NET_SEC").'";
								}
								return $result;'
						),
						array(
