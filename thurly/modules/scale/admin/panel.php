...
$APPLICATION->AddHeadScript("/thurly/js/scale/core.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/communicator.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/collection.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/actionsparamstypes.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/actionprocessdialog.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/actionparamsdialog.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/actionresultdialog.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/loadbar.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/role.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/action.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/infotable.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/itloadbar.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/server.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/provider.js");
$APPLICATION->AddHeadScript("/thurly/js/scale/admin_frame.js");

$APPLICATION->SetAdditionalCSS("/thurly/js/scale/css/scale-page-style.css");
...
$serversList = \Thurly\Scale\ServersData::getList();
$runningAction = \Thurly\Scale\ActionsData::checkRunningAction(); //If one of the actions runs now - get it params, to show dialog, and block page
$pullCreateAction = 'CREATE_PULL';

if(empty($serversList))
{
	$netIfaces = \Thurly\Scale\Helper::getNetworkInterfaces();
...
if(\Thurly\Scale\Helper::checkBxEnvVersion())
{
	?>
	...
	<div class="adm-scale-blocks-wrapper" id="adm-scale-blocks-wrapper"<?=empty($serversList) ? ' style="width:100%;"' : ''?>>
		<?if(Thurly\Main\ModuleManager::isModuleInstalled("cluster") || empty($serversList)):?>
			<?if(!empty($serversList)):?>
	...
	<script type="text/javascript">
		BX.ready(function(){
			...
			BX.Scale.actionsCollection = new BX.Scale.Collection(BX.Scale.Action, <?=CUtil::PhpToJSObject( Thurly\Scale\ActionsData::getList(true))?>);
			BX.Scale.rolesList = <?=CUtil::PhpToJSObject( \Thurly\Scale\RolesData::getList())?>;
			BX.Scale.sitesList = <?=CUtil::PhpToJSObject( \Thurly\Scale\SitesData::getList())?>;
			BX.Scale.monitoringEnabled = <?=Thurly\Scale\Monitoring::isEnabled() ? "true" : "false"?>;
			BX.Scale.thurlyEnvType = "<?=getenv('THURLY_ENV_TYPE')?>";
			BX.Scale.monitoringCategories = {};
			BX.Scale.isMonitoringDbCreated = {};

			<?foreach($serversList as $hostname => $server):?>
				BX.Scale.monitoringCategories["<?=$hostname?>"] = <?=CUtil::PhpToJSObject(\Thurly\Scale\Monitoring::getInfoTableCategoriesList($hostname))?>;
				BX.Scale.isMonitoringDbCreated["<?=$hostname?>"] = <?=Thurly\Scale\Monitoring::isDatabaseCreated($hostname) ? "true" : "false"?>;
			<?endforeach;?>

			BX.Scale.serversCollection = new BX.Scale.Collection(BX.Scale.Server, <?=CUtil::PhpToJSObject($serversList)?>);

			BX.Scale.Communicator.url = "/thurly/admin/scale_ajax.php";
			...
			<?if(Thurly\Scale\Monitoring::isEnabled()):?>
