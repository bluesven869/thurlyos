	...
	foreach($orderIds as $orderId)
	{
		...
		switch($request['action_button'])
		{
			case "add_to_pull":
				...
				$result = \Thurly\Scale\Provider::addToPullFromOrder($providerId, $orderId);
				...
				else
				{
					...
					{
						// add to monitoring
						$actionUpdateMonitoring = \Thurly\Scale\ActionsData::getActionObject("MONITORING_UPDATE");
...
$ordersList = \Thurly\Scale\Provider::getOrdersList();
$orders = array();
...
while($order = $data->Fetch())
{
	...
	$date = \Thurly\Main\Type\Date::createFromTimestamp($order["mtime"]);
