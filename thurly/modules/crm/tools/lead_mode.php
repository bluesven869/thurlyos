...
if($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["action"]) > 0 && check_thurly_sessid())
{
	...
	\Thurly\Main\Config\Option::set('crm', 'crm_lead_enabled_show', "N");
	...
	if (strlen($_POST["crmType"]) > 0 && \Thurly\Main\Loader::includeModule("crm"))
	{
		if ($_POST["crmType"] == "simple")
		{
			$res = \Thurly\Crm\Settings\LeadSettings::enableLead(false);
			if ($res)
			{
				__CrmShowEndJsonResonse(array('success' => "Y"));
			}
		...
		{
			$res = \Thurly\Crm\Settings\LeadSettings::enableLead(true);
			if ($res)
			{
				__CrmShowEndJsonResonse(array('success' => "Y"));
			}
