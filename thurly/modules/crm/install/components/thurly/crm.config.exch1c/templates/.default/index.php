	...
	<div class="crm-detail-lead-wrap">
		...
		<div class="crm-instant-editor-fld-block">
			<div class="crm-detail-comments-text" style="padding-left: 0;">
				<p><a href="<?=htmlspecialcharsbx($arResult['PATH_TO_EXCH1C_INVOICE'])?>"><?= htmlspecialcharsbx(GetMessage('CRM_EXCH1C_INV_SETTINGS_LINK_TITLE')) ?></a></p>
				<p><a href="<?=htmlspecialcharsbx($arResult['PATH_TO_EXCH1C_CATALOG'])?>"><?= htmlspecialcharsbx(GetMessage('CRM_EXCH1C_CAT_SETTINGS_LINK_TITLE')) ?></a></p>
			</div>
		</div>
	</div>
</div>
		...
		if (form && url && check)
		{
			BX.bind(check, 'click',
				function () {
					...
					BX.ajax.post(url, {"sessid": "<?=CUtil::JSEscape(thurly_sessid())?>", "CRM_EXCH1C_ENABLE": checked}, function () {});
				}
			);
		}
	});
