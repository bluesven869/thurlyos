...
switch ($action)
{
	case 'SET_LOAD_TARGET':
		...
		$options = \Thurly\Main\Config\Option::get('crm', $optionName);
		if ($options !== '')
		...
		if (count($options) === 0)
		{
			\Thurly\Main\Config\Option::delete('crm', array('name' => $optionName));
		}
		else
		{
			\Thurly\Main\Config\Option::set('crm', $optionName, serialize($options));
		}
