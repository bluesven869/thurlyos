<?php
use Thurly\Crm\Activity\CommunicationWidgetPanel;
	...
	<div class="crm-portriat">
		...
		<div class="crm-portrait-item crm-portrait-item-show">
			<div class="crm-portrait-item-wrapper">
				<div class="crm-portrait-load"
					...
					data-loaddata="<?=htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($primaryBar['data']))?>"
					data-context="<?=htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($primaryBar['context']))?>"
		...
		<div class="crm-portrait-item" data-role="loadbarmenu-container">
			...
			?><div class="crm-portrait-item-wrapper">
				...
				<div class="crm-portrait-load"
					data-role="loadbar"
					data-loaddata="<?=htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($bar['data']))?>"
					data-context="<?=htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($bar['context']))?>"
					<?if ($arResult['CAN_WRITE_CONFIG']):?> style="margin-right: 40px;"<?endif;?>>
					<div class="crm-portrait-load-item-blue" data-role="level-1" title="<?=GetMessage('CRM_CLIENT_PORTRAIT_LOAD_ITEM_IDEAL')?>">
						<div class="crm-portrait-load-item-text"><span class="crm-portriat-load-tem-text-inner"><?=GetMessage('CRM_CLIENT_PORTRAIT_LOAD_ITEM_IDEAL')?></span></div>
					</div>
					...
					<table class="crm-portrait-user-table">
						...
						<tr>
							...
							<td class="crm-portrait-user-table-dark">
								<a href="<?=htmlspecialcharsbx($element['ASSIGNED_BY_URL'])?>" id="balloon_portrait<?=$element['ID']?>">
									<?=htmlspecialcharsbx($element['ASSIGNED_BY_FORMATTED_NAME'])?>
								</a>
								<script type="text/javascript">BX.tooltip(<?=(int)$element['ASSIGNED_BY_ID']?>, "balloon_portrait<?=(int)$element['ID']?>", "");</script>
							</td>
						</tr>
						<?endif?>
					</table>
		...
		<div class="bx-crm-view" data-role="crm-portrait-widgets">
			<?$APPLICATION->IncludeComponent(
				...
				array(
					...
					'ROWS' => $rowData,
					'USE_DEMO' => 'N',
					'RENDER_HEAD_INTO_VIEW' => (SITE_TEMPLATE_ID === 'thurlyos') ? null : 'widget_panel_head',
					'MAX_WIDGET_COUNT' => 30
				)
			);
			?>
		...
		if(BX.CrmWidgetPanel && BX.CrmWidgetPanel.current)
		{
			BX.CrmWidgetPanel.current.reloadWindow = function()
			{
				...
				BX.ajax({
					method: 'POST',
					dataType: 'html',
					url: '/thurly/components/thurly/crm.client.portrait/lazyload.ajax.php',
					data: {
