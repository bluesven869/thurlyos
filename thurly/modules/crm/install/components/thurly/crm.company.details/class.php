...
use Thurly\Crm\Format\CompanyAddressFormatter;
...
class CCrmCompanyDetailsComponent extends CThurlyComponent
{
	/** @var string */
	protected $guid = '';
	/** @var int */
	...
	public function executeComponent()
	{
		...
		$this->enableOutmodedFields = \Thurly\Crm\Settings\CompanySettings::getCurrent()->areOutmodedRequisitesEnabled();
		...
		$this->isMyCompany = is_string($isMyCompany) && strtoupper($isMyCompany) === 'Y';
		...
		$this->enableDupControl = $this->arResult['DUPLICATE_CONTROL']['enabled'] =
			!$this->isEditMode && \Thurly\Crm\Integrity\DuplicateControl::isControlEnabledFor(CCrmOwnerType::Company);

		if($this->enableDupControl)
		{
			$this->arResult['DUPLICATE_CONTROL']['serviceUrl'] = '/thurly/components/thurly/crm.company.edit/ajax.php?'.thurly_sessid_get();
...
//			),
			...
			elseif($userField['USER_TYPE_ID'] === 'file')
			{
				$fieldInfo['ADDITIONAL'] = array(
					'URL_TEMPLATE' => \CComponentEngine::MakePathFromTemplate(
						'/thurly/components/thurly/crm.company.show/show_file.php?ownerId=#owner_id#&fieldName=#field_name#&fileId=#file_id#',
						array(
	...
	public function prepareEntityData()
	{
		...
		else
		{
			...
			$this->isMyCompany = isset($this->entityData['IS_MY_COMPANY']) && $this->entityData['IS_MY_COMPANY'] === 'Y';
		}
		...
		if($this->entityID > 0)
		{
			$contactIDs = Thurly\Crm\Binding\ContactCompanyTable::getCompanyContactIDs($this->entityID);
		}
