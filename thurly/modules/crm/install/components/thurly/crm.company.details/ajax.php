...
if($action === 'SAVE')
{
	...
	foreach($fieldsInfo as $name => $info)
	{
		...
		else if(isset($_POST[$name]))
		{
			...
			else if($name === 'LOGO')
			{
				...
				{
					$fileID = $fields[$name];
					$allowedFileIDs = \Thurly\Main\UI\FileInputUtility::instance()->checkFiles(
				...
				if(isset($_POST[$removeFlag]) && isset($presentFields[$name]) && $_POST[$removeFlag] == $presentFields[$name])
				{
					$fields[$removeFlag] = $_POST[$removeFlag];
				}
			}
		}
	}
	...
	{
		\Thurly\Crm\EntityRequisite::intertalizeFormData(
			$_POST['REQUISITES'],
	...
	if(!empty($fields) || !empty($entityRequisites) || !empty($entityBankDetails))
	{
		...
		if($isCopyMode)
		{
			...
			$merger = new \Thurly\Crm\Merger\CompanyMerger($currentUserID, false);
		...
		if($isNew)
		{
			...
			{
				$fields['OPENED'] = \Thurly\Crm\Settings\CompanySettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N';
			}
	...
	CThurlyComponent::includeComponentClass('thurly:crm.company.details');
