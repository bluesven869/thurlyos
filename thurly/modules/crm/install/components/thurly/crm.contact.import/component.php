...
if(!function_exists('__CrmImportCreateCustom'))
{
	function __CrmImportCreateCustom($origin, $headerLangId)
	{
		...
		return \Thurly\Crm\Import\CsvFileImportFactory::createByTypeName(
			$origin,
			array(
...
$userNameFormats = \Thurly\Crm\Format\PersonNameFormatter::getAllDescriptions();
...
elseif (isset($_REQUEST['import']) && isset($_SESSION['CRM_IMPORT_FILE']))
{
	...
	$userNameFormat = isset($_SESSION['CRM_IMPORT_IMPORT_NAME_FORMAT'])
		&& \Thurly\Crm\Format\PersonNameFormatter::isDefined($_SESSION['CRM_IMPORT_IMPORT_NAME_FORMAT'])
			? intval($_SESSION['CRM_IMPORT_IMPORT_NAME_FORMAT'])
			: \Thurly\Crm\Format\PersonNameFormatter::FirstLast;
	...
	$dupChecker = new \Thurly\Crm\Integrity\ContactDuplicateChecker();
	$processedQty = 0;
	...
	while($arData = $csvFile->Fetch())
	{
					...
					elseif ($currentKey == 'ASSIGNED_BY_ID')
					{
						...
						else
						{
							...
							else
							{
								...
								{
									$nameParts = array();
									if(\Thurly\Crm\Format\PersonNameFormatter::tryParseName($data, $userNameFormat, $nameParts))
									{
										$userFilter = array();
				...
				|| $enableDupCtrlByEmail || $enableDupCtrlByRequisite))
		{
			...
			$adapter = \Thurly\Crm\EntityAdapterFactory::create($arContact, CCrmOwnerType::Contact);
			$dups = $dupChecker->findDuplicates($adapter, new \Thurly\Crm\Integrity\DuplicateSearchParams($fieldNames));
							...
							foreach($valueTypes as $valueType)
							{
								foreach($multiFieldData as $multiFieldItem)
								{
									...
									if($itemValueType === $valueType && $itemValue !== '')
									{
										...
										$multiFieldValues[$type][$valueType][] = array(
											'VALUE' => $itemValue,
											'CODE' => \Thurly\Crm\Integrity\DuplicateCommunicationCriterion::prepareCode($type, $itemValue)
										);
									}
								}
							}
										...
										if(isset($item['FM'][$type]) && !empty($item['FM'][$type]))
										{
											...
											foreach($values as &$value)
											{
												...
												foreach($item['FM'][$type] as $k => &$v)
												{
													if($v['VALUE_TYPE'] === $valueType)
													{
														if($code === \Thurly\Crm\Integrity\DuplicateCommunicationCriterion::prepareCode($type, $v['VALUE']))
														{
															$isFound = true;
															break;
														}
	...
	if($arResult['error'] > 0)
	{
		$arResult['errata_url'] = SITE_DIR.'thurly/components/thurly/crm.contact.import/show_file.php?name=errata';
	}

	if($enableDupFile && $arResult['duplicate'] > 0)
	{
		$arResult['duplicate_url'] = SITE_DIR.'thurly/components/thurly/crm.contact.import/show_file.php?name=duplicate';
	}
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_thurly_sessid())
{
	if(isset($_POST['next']))
	{
			...
			else
			{
				...
				else
				{
					...
					$_SESSION['CRM_IMPORT_IMPORT_NAME_FORMAT'] = isset($_POST['IMPORT_NAME_FORMAT'])
						&& \Thurly\Crm\Format\PersonNameFormatter::isDefined($_POST['IMPORT_NAME_FORMAT'])
		...
		else if ($arResult['STEP'] == 3)
		{
			...
			ob_start();
			?>
			<div class="crm_import_entity"><?=GetMessage('CRM_IMPORT_FINISH')?>: <span id="crm_import_entity">0</span> <span id="crm_import_entity_progress"><img src="/thurly/components/thurly/crm.contact.import/templates/.default/images/wait.gif" align="absmiddle"></span></div>
...
if(!$enableFixedOrigin)
{
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'type' => 'list',
		'value' => \Thurly\Crm\Format\PersonNameFormatter::FirstLast
	);
