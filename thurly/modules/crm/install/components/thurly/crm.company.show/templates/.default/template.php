				...
				if(BX(bpContainerId))
				{
					BX.CrmBizprocDispatcher.create(
						"<?=CUtil::JSEscape($bizprocDispatcherID)?>",
						{
							...
							entityTypeName: "<?=CCrmOwnerType::CompanyName?>",
							entityID: <?=$arResult['ELEMENT_ID']?>,
							serviceUrl: "/thurly/components/thurly/crm.company.show/bizproc.php?company_id=<?=$arResult['ELEMENT_ID']?>&post_form_uri=<?=urlencode($arResult['POST_FORM_URI'])?>&<?=thurly_sessid_get()?>",
							formID: "<?=CUtil::JSEscape($arResult['FORM_ID'])?>",
