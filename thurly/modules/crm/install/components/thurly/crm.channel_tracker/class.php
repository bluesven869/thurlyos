...
use Thurly\Crm\Integration\Channel\IChannelInfo;
use Thurly\Crm\Integration\Channel\IChannelGroupInfo;
use Thurly\Crm\Integration\Channel\ChannelTrackerManager;
use Thurly\Crm\Integration\Channel\ChannelType;
use Thurly\Crm\Widget\Filter;
use Thurly\Crm\Widget\FilterPeriodType;
use Thurly\Crm\Widget\Data\Activity\ChannelStatistics as ActivityChannelStatistics;
use Thurly\Crm\Widget\Data\Company\DealSumStatistics;
use Thurly\Main\Entity;

Loc::loadMessages(__FILE__);

class CCrmStartPageComponent extends CThurlyComponent
{
	/** @var string */
	protected $guid = '';
	/** @var string */
	...
	/** @var Thurly\Crm\Widget\Filter|null */
	...
	public function executeComponent()
	{
		...
		if (!$this->commonFilter->isEmpty())
		{
			...
			/* @var \Thurly\Main\Type\Date $period["START"] */
			if ($period["START"] instanceof \Thurly\Main\Type\Date)
				$this->arResult["PERIOD"]["START"] = $period["START"]->format("Y-m-d");
			/* @var \Thurly\Main\Type\Date $period["END"] */
...
class CCrmStartPageComponentCRMCounters {
	...
	public function setFilter(Filter $filter)
	{
		...
		{
			Thurly\Crm\Widget\Filter::merge($filter, $this->filter, array('overridePeriod' => true));
		}
	}
	...
	private function getPersonalCountersQueries($endTime)
	{
		...
		while ($pointer)
		{
			if (self::$personalCounterData['dbRes'] === null)
			{
				...
				{
					$dbRes = Thurly\Crm\ActivityTable::getList(array(
						'select' => array(
					...
					if (!empty($users))
					{
						self::$personalCounterData['dbRes'] = Thurly\Crm\ActivityTable::getList(array(
							'select' => array(
								new ExpressionField('CNT', "COUNT(*)"),
								'RESPONSIBLE_ID',
								'OWNER_TYPE_ID',
				...
				{
					self::$personalCounterData['dbRes'] = Thurly\Crm\LeadTable::getList(array(
						'select' => array(
							new ExpressionField('CNT', "COUNT(*)"),
				...
				{
					self::$personalCounterData['dbRes'] = Thurly\Crm\DealTable::getList(array(
						'select' => array(
							new ExpressionField('CNT', "COUNT(*)"),
	...
	public function getPersonalCounters($previousDay = false)
	{
		...
		if (!empty($result))
		{
			if ($dbRes = Thurly\Crm\ActivityTable::getList(array(
				'select' => array(
					new ExpressionField('CNT', "COUNT(*)"),
					'RESPONSIBLE_ID',
					'OWNER_TYPE_ID'
	...
	public static function sendJsonResponse($response)
	{
		...
		echo \Thurly\Main\Web\Json::encode($response);
