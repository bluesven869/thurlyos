		...
		*/
		...
		if (empty($arResult['ERROR_MESSAGE']))
		{
			...
			if (CCrmDeal::CheckCreatePermission($userPermissions, 0))
			{
				if ($bConvertDeal)
				{
					...
					if($iDealId > 0)
					{
						...
						//Region automation
						\Thurly\Crm\Automation\Factory::runOnAdd(\CCrmOwnerType::Deal, $iDealId);
						//end region
					}
				}
			}
			...
			if($CCrmLead->Update($arParams['ELEMENT_ID'], $arFields['LEAD'], true, true, array('REGISTER_SONET_EVENT' => true)))
			{
				...
				//Region automation
				\Thurly\Crm\Automation\Factory::runOnStatusChanged(\CCrmOwnerType::Lead, $arParams['ELEMENT_ID']);
				//end region
			}
