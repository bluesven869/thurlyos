...
foreach($arResult['LIST_DATA'] as $key => &$listRow)
{
	...
	$gridDataItem = array(
		...
		'columns' => array(
			...
			'IN_SHORT_LIST' => $listRow['IN_SHORT_LIST'] == 'Y' ? GetMessage('MAIN_YES') : GetMessage('MAIN_NO')
		),
		'editable' => array()
	);
	$rows[] = $gridDataItem;
...
<script type="text/javascript">
	...
	BX.Crm[presetFieldListManagerId] = new BX.Crm.PresetFieldListManagerClass({
		...
		userFieldServiceUrl: "<?= ('/thurly/components/thurly/crm.requisite.edit/uf.ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get()) ?>",
		messages: {
