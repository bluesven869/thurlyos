...
if (isset($_REQUEST['conv_deal_id']) && $_REQUEST['conv_deal_id'] > 0)
{
	...
	if($srcDealId > 0)
	{
		$conversionWizard = \Thurly\Crm\Conversion\DealConversionWizard::load($srcDealId);
...
elseif ($bEdit || $bCopy)
{
	...
	if ($arParams['IS_RECURRING'] === "Y")
	{
		...
		{
			$result = Recurring\Manager::exposeInvoices(
				array(
		...
		$recurData = Thurly\Crm\InvoiceRecurTable::getList(
			array(
...
if ($conversionWizard !== null || $bEdit || $bCopy)
{
	...
	{
		if ($bCreateFromDeal || $conversionWizard instanceof \Thurly\Crm\Conversion\DealConversionWizard)
		{
...
$bProcessPost = $_SERVER['REQUEST_METHOD'] == 'POST' && check_thurly_sessid();
		...
		elseif (isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']) || isset($_POST['continue']) || $bAjaxSubmit)
		{
			...
			if ($bAjaxSubmit)
			{
				...
				if ($personTypeId > 0)
				{
					...
					{
						$paySystem = \Thurly\Sale\PaySystem\Manager::getById($paySystemValue);
						if ($paySystem)
			...
			if (empty($arResult['ERROR_MESSAGE']))
			{
				...
				if ($bEdit)
				{
					...
					if ($_POST['RECUR_PARAM']['RECURRING_SWITCHER'] === 'Y' && Recurring\Manager::isAllowedExpose(Recurring\Manager::INVOICE))
					{
						...
						if (
							($_POST['RECUR_PARAM']['REPEAT_TILL'] === 'times' || $_POST['RECUR_PARAM']['REPEAT_TILL'] === Thurly\Crm\Recurring\Entity\Invoice::LIMITED_BY_TIMES)
							&& (int)$recurringList['LIMIT_REPEAT'] > 0)
						{
							$recurringList['IS_LIMIT'] = Thurly\Crm\Recurring\Entity\Invoice::LIMITED_BY_TIMES;
						}
						elseif(
							$_POST['RECUR_PARAM']['REPEAT_TILL'] === 'date'
							|| $_POST['RECUR_PARAM']['REPEAT_TILL'] === Thurly\Crm\Recurring\Entity\Invoice::LIMITED_BY_DATE)
						{
							$recurringList['IS_LIMIT'] = Thurly\Crm\Recurring\Entity\Invoice::LIMITED_BY_DATE;
						}
						else
						{
							$recurringList['IS_LIMIT'] = Thurly\Crm\Recurring\Entity\Invoice::NO_LIMITED;
						}
						...
						$recur = \Thurly\Crm\InvoiceRecurTable::getList(
							array(
				...
				else
				{
					if ($_POST['RECUR_PARAM']['RECURRING_SWITCHER'] === 'Y' && Recurring\Manager::isAllowedExpose(Recurring\Manager::INVOICE))
					{
						...
						if (
							...
							&& ($_POST['RECUR_PARAM']['REPEAT_TILL'] === 'date'|| $_POST['RECUR_PARAM']['REPEAT_TILL'] === Thurly\Crm\Recurring\Entity\Invoice::LIMITED_BY_DATE)
						)
						{
							$recurringList['LIMIT_DATE'] = new \Thurly\Main\Type\Date($_POST['RECUR_PARAM']['END_DATE']);
							$recurringList['IS_LIMIT'] = Thurly\Crm\Recurring\Entity\Invoice::LIMITED_BY_DATE;
				...
				if ($bSuccess)
				{
					...
					{
						\Thurly\Crm\Requisite\EntityLink::unregister(CCrmOwnerType::Invoice, $arResult['ELEMENT']['ID']);
					}
				}
...
$arResult['FIELDS']['tab_1'][] = array(
	...
	'componentParams' => array(
		...
		'SERVICE_URL' => '/thurly/components/thurly/crm.invoice.edit/ajax.php?'.thurly_sessid_get(),
...
{
	$paySystem = \Thurly\Sale\PaySystem\Manager::getById($arResult['ELEMENT']['PAY_SYSTEM_ID']);
	if ($paySystem)
...
else
{
	...
	$fileViewer = new \Thurly\Crm\UserField\FileViewer(CCrmOwnerType::Invoice, $arResult['ELEMENT']['ID']);
}
...
include_once($_SERVER['DOCUMENT_ROOT'].'/thurly/components/thurly/crm.invoice/include/nav.php');
?>
