...
/** @var array $arParams */
/** @var array $arResult */
/** @var \Thurly\Crm\PresetListComponent $component */
...
foreach($arResult['LIST_DATA'] as $key => &$listRow)
{
	...
	$canDelete = true;
	if ($arResult['ENTITY_TYPE_ID'] === \Thurly\Crm\EntityPreset::Requisite)
	{
				...
				|| (isset($row['~REQUISITE_DEF_FOR_CONTACT']) && $row['~REQUISITE_DEF_FOR_CONTACT'] === 'Y')))
		{
			$presetSetDefForCompanyUrl = CHTTP::urlAddParams(
				...
				array('action_'.$arResult['GRID_ID'] => 'set_def_for_company', 'ID' => $key, 'sessid' => thurly_sessid())
			);
			$presetSetDefForContactUrl = CHTTP::urlAddParams(
				...
				array('action_'.$arResult['GRID_ID'] => 'set_def_for_contact', 'ID' => $key, 'sessid' => thurly_sessid())
			);
			$itemActions[] =  array(
				'ICONCLASS' => 'edit',
