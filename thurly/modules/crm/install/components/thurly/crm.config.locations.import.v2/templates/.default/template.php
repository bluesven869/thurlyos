...
<?else:?>
	...
	$APPLICATION->AddHeadScript('/thurly/js/sale/core_iterator.js');
	$APPLICATION->AddHeadScript('/thurly/js/sale/core_ui_etc.js');
	$APPLICATION->AddHeadScript('/thurly/js/sale/core_ui_itemtree.js');
	$APPLICATION->AddHeadScript('/thurly/js/sale/core_ui_fileasyncloader.js');
	?>
	...
	<div id="location-import">
		...
		$arTabs = array(
			'tab_params' => array(
				...
				'icon' => '',
				'onselect_callback' => 'BX.crmLocationsImportInstance.setTab'
			),
			'tab_cleanup' => array(
				'id' => 'tab_cleanup',
		...
		*/
		...
		if(in_array(LANGUAGE_ID, array('ru', 'ua', 'de')))
		{
			...
			?>
				<label>
					<input type="checkbox" value="1" name="EXCLUDE_COUNTRY_DISTRICT" class="bx-ui-loc-i-option" checked="checked" />
	...
	<script>
		BX.crmLocationsImportInstance = new BX.Crm.component.config.locations.import(<?=CUtil::PhpToJSObject(array(
