...
if (!($activity['PARENT_ID'] > 0) && 'BATCH' == $activity['__message_type'])
{
	$this->setViewTarget('planner_slider_header');
...
<form id="<?=htmlspecialcharsbx($formId) ?>" action="/thurly/components/thurly/crm.activity.editor/ajax.php?action=save_email" method="POST">
		...
		array(
			...
			'FIELDS' => array(
				...
				array(
					...
					'hidden'      => 'BATCH' == $activity['__message_type'],
...
BX.ready(function ()
{
	BXCrmActivityEmailController.init({
		...
		type: 'edit',
		templates: <?=\Thurly\Main\Web\Json::encode($arParams['TEMPLATES_BY_TYPE']) ?>
	});
