...
class CrmActivityEmailComponent extends CThurlyComponent
{
	...
	protected function executeEdit()
	{
		...
		$userFields = \Thurly\Main\UserTable::getList(array(
		...
		if (empty($activity['__parent']))
		{
			...
			if (\CCrmOwnerType::Invoice == $activity['OWNER_TYPE_ID'] && $activity['OWNER_ID'] > 0)
			{
				...
				if (!empty($invoice))
				{
					...
					if ($activity['STORAGE_TYPE_ID'] == \Thurly\Crm\Integration\StorageType::Disk)
			...
			{
				// encapsulate to StorageManager
				$storageElementId = \Thurly\Crm\Integration\StorageManager::saveEmailAttachment(
					\CFile::getFileArray($pdfFileId),
					\Thurly\Crm\Integration\StorageType::Disk
				);
				if ($storageElementId > 0)
				{
					$storageElement = \Thurly\Disk\File::loadById($storageElementId, array('STORAGE'));

					$diskRightsManager = \Thurly\Disk\Driver::getInstance()->getRightsManager();
					$diskRightsManager->append(
						$storageElement,
						array(array(
							'ACCESS_CODE' => 'U'.$USER->getId(),
							'TASK_ID' => $diskRightsManager->getTaskIdByName(\Thurly\Disk\RightsManager::TASK_READ),
						))
					);
		...
		else
		{
			...
			{
				$authorFields = \Thurly\Main\UserTable::getList(array(
		...
		{
			$diskSecurityContext = new \Thurly\Disk\Security\DiskSecurityContext($USER->getId());
			$diskRightsManager = \Thurly\Disk\Driver::getInstance()->getRightsManager();
			foreach ($activity['STORAGE_ELEMENT_IDS'] as $item)
			{
				$storageElement = \Thurly\Disk\File::loadById($item, array('STORAGE'));
				if ($storageElement && !$storageElement->canRead($diskSecurityContext))
				{
					$diskRightsManager->append(
						$storageElement,
						array(array(
					...
					\Thurly\Crm\Integration\StorageManager::registerInterRequestFile($item, \Thurly\Crm\Integration\StorageType::Disk);
		...
		$this->arParams['DOCS_READONLY'] = \CCrmOwnerType::Lead == $activity['OWNER_TYPE_ID'];
	...
	public static function prepareActivityRcpt(&$activity, $parent = null)
	{
		...
		// FROM for display
		{
			...
			else
			{
				...
				$activity['ITEM_FROM_EMAIL'] = $activityEmailMeta['__email'] != $author['NAME_FORMATTED']
