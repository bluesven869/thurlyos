...
if(!function_exists('__CrmQuickPanelViewEndResonse'))
{
	function __CrmQuickPanelViewEndResonse($result)
	{
...
{
	__CrmQuickPanelViewEndResonse(array('ERROR' => 'Could not include crm module.'));
}
...
{
	__CrmQuickPanelViewEndResonse(array('ERROR' => 'Access denied.'));
}
if ($_SERVER['REQUEST_METHOD'] != 'POST')
{
	__CrmQuickPanelViewEndResonse(array('ERROR' => 'Request method is not allowed.'));
}
...
if($action === 'GET_CLIENT_INFOS')
{
	...
	if($entityTypeName === '')
	{
		__CrmQuickPanelViewEndResonse(array('ERROR' => 'Entity type is not specified.'));
	}

	$entityTypeID = CCrmOwnerType::ResolveID($entityTypeName);
	if($entityTypeID === CCrmOwnerType::Undefined)
	{
		__CrmQuickPanelViewEndResonse(array('ERROR' => 'Undefined entity type is specified.'));
	}

	$ownerTypeName = isset($params['OWNER_TYPE_NAME']) ? $params['OWNER_TYPE_NAME'] : '';
	if($ownerTypeName === '')
	{
		__CrmQuickPanelViewEndResonse(array('ERROR' => 'Owner type is not specified.'));
	}

	$ownerTypeID = CCrmOwnerType::ResolveID($ownerTypeName);
	if($ownerTypeID === CCrmOwnerType::Undefined)
	{
		__CrmQuickPanelViewEndResonse(array('ERROR' => 'Undefined owner type is specified.'));
	}

	$ownerID = isset($params['OWNER_ID']) ? (int)$params['OWNER_ID'] : 0;
	if($ownerID <= 0)
	{
		__CrmQuickPanelViewEndResonse(array('ERROR' => 'Owner ID is not specified.'));
	}
	...
	{
		$entityIDs = \Thurly\Crm\Binding\ContactCompanyTable::getContactCompanyIDs($ownerID);
	}
	elseif($ownerTypeID === CCrmOwnerType::Deal && $entityTypeID === CCrmOwnerType::Contact)
	{
		$entityIDs = \Thurly\Crm\Binding\DealContactTable::getDealContactIDs($ownerID);
	}
	elseif($ownerTypeID === CCrmOwnerType::Quote && $entityTypeID === CCrmOwnerType::Contact)
	{
		$entityIDs = \Thurly\Crm\Binding\QuoteContactTable::getQuoteContactIDs($ownerID);
	}
	...
	__CrmQuickPanelViewEndResonse(array('DATA' => $data));
}
else
{
	__CrmQuickPanelViewEndResonse(array('ERROR' => "Action '{$action}' is not supported in current context."));
}
