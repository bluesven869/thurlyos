...
$arResult['PERMISSION_ENTITY_TYPE'] = Thurly\Crm\Category\DealCategory::convertToPermissionEntityType($arResult['CATEGORY_ID']);
...
/** @var \Thurly\Crm\Conversion\EntityConversionWizard */
$conversionWizard = null;
if(isset($_REQUEST['lead_id']) && $_REQUEST['lead_id'] > 0)
{
	$srcLeadID = (int)$_REQUEST['lead_id'];
	$conversionWizard = \Thurly\Crm\Conversion\LeadConversionWizard::load($srcLeadID);
...
elseif (isset($_REQUEST['conv_quote_id']) && $_REQUEST['conv_quote_id'] > 0)
{
	...
	if($srcQuoteID > 0)
	{
		$conversionWizard = \Thurly\Crm\Conversion\QuoteConversionWizard::load($srcQuoteID);
		if($conversionWizard !== null)
		{
...
if ($conversionWizard !== null)
{
	...
	if($_SERVER['REQUEST_METHOD'] === 'GET')
	{
		...
		if(isset($arFields['CATEGORY_ID'])
			&& Thurly\Crm\Category\DealCategory::isEnabled($arFields['CATEGORY_ID'])
...
elseif ($isEditMode || $isCopyMode)
{
	...
	if(is_array($arFields))
	{
		...
		$arFields['CONTACT_BINDINGS'] = \Thurly\Crm\Binding\DealContactTable::getDealBindings($arParams['ELEMENT_ID']);
	...
	if ($arParams['IS_RECURRING'] === "Y")
	{
		...
		elseif ($_REQUEST['expose'] === 'Y')
		{
			...
			$result = Thurly\Crm\Recurring\Manager::expose(
				array(
		...
		$recurData = Thurly\Crm\DealRecurTable::getList(
			array(
...
$arResult['FORM_ID'] = Thurly\Crm\Category\DealCategory::prepareFormID(
	$arResult['CATEGORY_ID'],
...
else
{
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && check_thurly_sessid())
	{
		...
		elseif(isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']) || isset($_POST['continue']))
		{
			...
			if(isset($_POST['COMMENTS']))
			{
				...
				{
					$comments = \Thurly\Crm\Format\TextHelper::sanitizeHtml($comments);
				}
			...
			elseif(!$isEditMode && !$isCopyMode)
			{
				$arFields['OPENED'] = \Thurly\Crm\Settings\DealSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N';
			}
			...
			if (empty($arResult['ERROR_MESSAGE']))
			{
				...
				if ($isEditMode)
				{
					...
					if ($_POST['RECUR_PARAM']['RECURRING_SWITCHER'] === 'Y' && Thurly\Crm\Recurring\Manager::isAllowedExpose(Thurly\Crm\Recurring\Manager::DEAL))
					{
						if (strlen($_POST['RECUR_PARAM']['START_DATE']) > 0)
							$recurringList['START_DATE'] = new \Thurly\Main\Type\Date($_POST['RECUR_PARAM']['START_DATE']);

						if (strlen($_POST['RECUR_PARAM']['DEAL_DATEPICKER_BEFORE']) > 0)
						{
							$recurringList['START_DATE'] = new \Thurly\Main\Type\Date($_POST['RECUR_PARAM']['DEAL_DATEPICKER_BEFORE']);
						}

						if (strlen($_POST['RECUR_PARAM']['END_DATE']) > 0)
						{
							$recurringList['LIMIT_DATE'] = new \Thurly\Main\Type\Date($_POST['RECUR_PARAM']['END_DATE']);
						}
						else
						{
						...
						if (
							$_POST['RECUR_PARAM']['REPEAT_TILL'] === \Thurly\Crm\Recurring\Entity\Deal::LIMITED_BY_TIMES
							&& (int)$recurringList['LIMIT_REPEAT'] > 0
						)
						{
							$recurringList['IS_LIMIT'] = \Thurly\Crm\Recurring\Entity\Deal::LIMITED_BY_TIMES;
						}
						elseif($_POST['RECUR_PARAM']['REPEAT_TILL'] === \Thurly\Crm\Recurring\Entity\Deal::LIMITED_BY_DATE)
						{
							$recurringList['IS_LIMIT'] = \Thurly\Crm\Recurring\Entity\Deal::LIMITED_BY_DATE;
						}
						else
						{
							$recurringList['IS_LIMIT'] = \Thurly\Crm\Recurring\Entity\Deal::NO_LIMITED;
						}

						$recurringList['PARAMS'] = $_POST['RECUR_PARAM'];
						$recur = \Thurly\Crm\DealRecurTable::getList(
							array(
								"filter" => array("=DEAL_ID" => $arResult['ELEMENT']['ID'])
							)
						)->fetch();

						$res = \Thurly\Crm\Recurring\Manager::update(
							$recur['ID'],
							$recurringList,
							\Thurly\Crm\Recurring\Manager::DEAL
						);
						...
						if ($res->isSuccess())
						{
							\Thurly\Crm\Recurring\Manager::exposeToday(null, Thurly\Crm\Recurring\Manager::DEAL);
						}
					}
				}
				else
				{
					...
					if ($_POST['RECUR_PARAM']['RECURRING_SWITCHER'] === 'Y' && Thurly\Crm\Recurring\Manager::isAllowedExpose(Thurly\Crm\Recurring\Manager::DEAL))
					{
						...
						$result = \Thurly\Crm\Recurring\Manager::createEntity(
							$arFields,
							$recurringList,
							\Thurly\Crm\Recurring\Manager::DEAL
						);
				...
				if ($bSuccess)
				{
					...
					{
						\Thurly\Crm\Requisite\EntityLink::unregister(CCrmOwnerType::Deal, $arResult['ELEMENT']['ID']);
					}
				}
			...
			if ($_POST['RECUR_PARAM']['RECURRING_SWITCHER'] !== 'Y')
			{
				...
				if (!$isEditMode)
				{
					\Thurly\Crm\Automation\Factory::runOnAdd(\CCrmOwnerType::Deal, $arResult['ELEMENT']['ID']);
				}
				elseif (isset($arFields['STAGE_ID']) && $arSrcElement['STAGE_ID'] != $arFields['STAGE_ID'])
				{
					\Thurly\Crm\Automation\Factory::runOnStatusChanged(\CCrmOwnerType::Deal, $arResult['ELEMENT']['ID']);
				}
				//end automation
			}
			...
			{
				$callList = \Thurly\Crm\CallList\CallList::createWithId($arResult['CALL_LIST_ID']);
				if($callList && $ID > 0)
				{
			...
			else
			{
				...
				if(isset($arResult['EXTERNAL_CONTEXT']) && $arResult['EXTERNAL_CONTEXT'] !== '')
				{
					...
					{
						\Thurly\Pull\Event::add($USER->GetID(), array(
							'module_id' => 'crm',
	...
	elseif (isset($_GET['delete']) && check_thurly_sessid())
	{
		if ($isEditMode)
		{
...
$arResult['~STAGE_LIST'] = Thurly\Crm\Category\DealCategory::getStageList($arResult['CATEGORY_ID']);
...
$arResult['FIELDS']['tab_1'][] = array(
	...
	'value' => isset($arResult['ELEMENT']['OPENED'])
		? $arResult['ELEMENT']['OPENED'] : (\Thurly\Crm\Settings\DealSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N'),
...
if(CCrmCompany::CheckReadPermission(0, $userPermissions) || CCrmContact::CheckReadPermission(0, $userPermissions))
{
	...
	{
		$contactBindings = \Thurly\Crm\Binding\DealContactTable::getDealBindings($arParams['ELEMENT_ID']);
	}
	...
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'componentParams' => array(
			...
			'SERVICE_URL' => '/thurly/components/thurly/crm.deal.edit/ajax.php?'.thurly_sessid_get(),
			'REQUISITE_SERVICE_URL' => '/thurly/components/thurly/crm.requisite.edit/settings.php?'.thurly_sessid_get(),
...
$APPLICATION->IncludeComponent('thurly:crm.interface.form.recurring',
	'edit',
	array(
		'DATA' => $data,
		'ID' => $arResult['ELEMENT']['ID'],
		'ENTITY_TYPE' => Thurly\Crm\Recurring\Manager::DEAL,
...
if($conversionWizard !== null)
{
	...
	$fileViewer = new \Thurly\Crm\Conversion\EntityConversionFileViewer(
		CCrmOwnerType::Deal,
		CCrmOwnerType::Lead,
		$arResult['LEAD_ID']
	);
...
else
{
	...
	$fileViewer = new \Thurly\Crm\UserField\FileViewer(CCrmOwnerType::Deal, $arResult['ELEMENT']['ID']);
}
