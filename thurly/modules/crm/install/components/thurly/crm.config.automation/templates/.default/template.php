...
<?if ($arResult['CATEGORIES'] && count($arResult['CATEGORIES']) > 1):?>
	<div class="crm-config-automation-button-container">
		<div class="crm-interface-toolbar-button" data-role="category-selector" data-categories="<?=htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($arResult['CATEGORIES']))?>">
		...
		if (categorySelector)
		{
			...
			for (i = 0; i < categories.length; ++i)
			{
				menuItems.push({
					...
					{
						window.location.href = '/crm/configs/automation/<?=CUtil::JSEscape($arResult['ENTITY_TYPE_NAME'])?>/'
							+ item.category.id + '/';
		...
		if (helpContainer)
		{
			...
			if (closeButton)
			{
				BX.bind(closeButton, 'click', function()
				{
					BX.ajax(
						{
							url: "/thurly/components/thurly/crm.config.automation/ajax.php?<?=thurly_sessid_get()?>",
							method: "POST",
							dataType: "json",
							data:
								{ "ACTION" : "HIDE_HELP"}
