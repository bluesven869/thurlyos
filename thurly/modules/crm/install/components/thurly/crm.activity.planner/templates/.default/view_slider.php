...
$optionsJson = \Thurly\Main\Web\Json::encode($options);
...
?><!DOCTYPE html>
	...
	<div class="crm-activity-planner-slider-wrap" data-role="options" data-options="<?=htmlspecialcharsbx($optionsJson)?>">
								...
								<div class="crm-task-list-person-info-phone-block">
									<span class="crm-task-list-person-info-phone"><?=GetMessage('CRM_ACTIVITY_PLANNER_TEL')?>:</span>
										...
										<a href="<?=htmlspecialcharsbx($link['HREF'])?>" onclick="<?=htmlspecialcharsbx($link['ONCLICK'])?>">
											<?=htmlspecialcharsbx(
												\Thurly\Main\PhoneNumber\Parser::getInstance()->parse($fm['VALUE'])->format()
											)?>
										</a>
									</span>
								</div>
				...
				callback : {
					select: function(item, type, search, undeleted, name, state)
					{
						...
						if ('init' != state)
						{
							BX.ajax({
								method: 'POST',
								url: '/thurly/components/thurly/crm.activity.editor/ajax.php?id=<?=$activity['ID'] ?>&action=docs',
								data: data,
								'dataType': 'json'
							});
						}
