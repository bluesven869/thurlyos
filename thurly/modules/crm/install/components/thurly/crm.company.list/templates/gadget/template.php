...
else
{
	foreach($arResult['COMPANY'] as $arCompany)
	{
		...
		<script type="text/javascript">BX.tooltip('COMPANY_<?=$arCompany['~ID']?>', "balloon_<?=$arResult['GADGET_ID']?>_CO_<?=$arCompany['ID']?>", "/thurly/components/thurly/crm.company.show/card.ajax.php", "crm_balloon_company", true);</script>
		<?
	}
}
?>