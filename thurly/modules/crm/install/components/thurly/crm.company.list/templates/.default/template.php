...
$APPLICATION->SetAdditionalCSS("/thurly/themes/.default/crm-entity-show.css");
if(SITE_TEMPLATE_ID === 'thurlyos')
{
	$APPLICATION->SetAdditionalCSS("/thurly/themes/.default/thurlyos/crm-entity-show.css");
}
if (CModule::IncludeModule('thurlyos') && !\Thurly\Crm\CallList\CallList::isAvailable())
{
...
Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/crm/autorun_proc.js');
Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/js/crm/css/autorun_proc.css');
...
if($arResult['NEED_FOR_TRANSFER_REQUISITES']):
	?><div id="transferRequisitesMsg" class="crm-view-message">
		<?=Thurly\Crm\Requisite\EntityRequisiteConverter::getIntroMessage(
			array(
...
$gridManagerCfg = array(
	...
	'serviceUrl' => '/thurly/components/thurly/crm.activity.editor/ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
	'filterFields' => array()
);
$prefix = $arResult['GRID_ID'];
	...
	if($isMyCompanyMode)
	{
		if(!(isset($arCompany['IS_DEF_MYCOMPANY']) && $arCompany['IS_DEF_MYCOMPANY'] === 'Y'))
		{
			...
			$companySetDefMyCompany = CHTTP::urlAddParams(
				...
				array('action_'.$arResult['GRID_ID'] => 'set_def_mycompany', 'ID' => $sKey, 'sessid' => thurly_sessid())
			);
			$arActions[] = array(
	...
	&& ($allowWrite || $allowDelete || $callListUpdateMode))
{
	...
	{
		//region Add Email
		if (!$isMyCompanyMode && IsModuleInstalled('subscribe'))
		{
			$actionList[] = array(
				'NAME' => GetMessage('CRM_COMPANY_SUBSCRIBE'),
				'VALUE' => 'subscribe',
				'ONCHANGE' => array(
					array(
						'ACTION' => Thurly\Main\Grid\Panel\Actions::CREATE,
		...
		if(!Thurly\Main\Grid\Context::isInternalRequest())
		{
		...
		$actionList[] = array(
			...
			'ONCHANGE' => array(
				array(
					...
					'DATA' => array(
						array(
							'TYPE' => Thurly\Main\Grid\Panel\Types::TEXT,
							'ID' => 'action_assigned_by_search',
							'NAME' => 'ACTION_ASSIGNED_BY_SEARCH'
						),
						array(
							'TYPE' => Thurly\Main\Grid\Panel\Types::HIDDEN,
	...
	if($callListUpdateMode)
	{
		$controlPanel['GROUPS'][0]['ITEMS'][] = array(
			"TYPE" => \Thurly\Main\Grid\Panel\Types::BUTTON,
		...
		array(
				...
				array(
					'ITEMS' => array(
						...
						array(
							//'icon' => 'chart',
							'id' => 'widget',
							'name' => GetMessage('CRM_COMPANY_LIST_FILTER_NAV_BUTTON_WIDGET'),
							'active' => false,
							'url' => $arParams['PATH_TO_COMPANY_WIDGET']
						)
					),
					'BINDING' => array(
			...
			'EXTENSION' => array(
				...
				'CONFIG' => array(
					...
					'gridId' => $arResult['GRID_ID'],
					'activityEditorId' => $activityEditorID,
					'activityServiceUrl' => '/thurly/components/thurly/crm.activity.editor/ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
					'taskCreateUrl'=> isset($arResult['TASK_CREATE_URL']) ? $arResult['TASK_CREATE_URL'] : '',
					'serviceUrl' => '/thurly/components/thurly/crm.company.list/list.ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
...
<script type="text/javascript">
	BX.ready(
		function()
		{
			...
			var mgr = BX.CrmDuplicateManager.create("mgr", { entityTypeName: "<?=CUtil::JSEscape(CCrmOwnerType::CompanyName)?>", serviceUrl: "<?=SITE_DIR?>thurly/components/thurly/crm.company.list/list.ajax.php?&<?=thurly_sessid_get()?>" });
			BX.addCustomEvent(
				mgr,
		...
		BX.ready(
			function()
			{
				...
				var manager = BX.AutorunProcessManager.create("rebuildCompanySearch",
					{
						serviceUrl: "<?='/thurly/components/thurly/crm.company.list/list.ajax.php?'.thurly_sessid_get()?>",
		...
		function()
		{
			...
			var converter = BX.CrmRequisiteConverter.create(
				"converter",
				{
					...
					serviceUrl: "<?=SITE_DIR?>thurly/components/thurly/crm.company.list/list.ajax.php?&<?=thurly_sessid_get()?>"
				}
			);
