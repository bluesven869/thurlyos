...
if(!function_exists('__CrmCompanyStExportEndResonse'))
{
	function __CrmCompanyStExportEndResonse($result)
	{
...
{
	__CrmCompanyStExportEndResonse(array('ERROR' => 'Site ID is not specified.'));
}


if (!CModule::IncludeModule('crm'))
{
	__CrmCompanyStExportEndResonse(array('ERROR' => 'Could not include crm module.'));
}
...
if ($action === 'STEXPORT')
{
	...
	if($entityTypeName === '')
	{
		__CrmCompanyStExportEndResonse(array('ERROR' => 'Entity type is not specified.'));
	}

	$exportType = isset($params['EXPORT_TYPE']) ? $params['EXPORT_TYPE'] : '';
	if(!in_array($exportType, array('csv', 'excel', true)))
	{
		__CrmCompanyStExportEndResonse(
	...
	{
		__CrmCompanyStExportEndResonse(array('ERROR' => 'Undefined entity type is specified.'));
	}
	...
	{
		__CrmCompanyStExportEndResonse(array('ERROR' => 'Access denied.'));
	}
	...
	if($processToken === '')
	{
		__CrmCompanyStExportEndResonse(array('ERROR' => 'Process token is not specified.'));
	}
	...
	$progressData = CUserOptions::GetOption('crm', 'crm_stexport_'.$optionSuffix, '');
	...
	if (!is_string($filePath) || strlen($filePath) === 0 || !CheckDirPath($filePath))
	{
		...
		CUserOptions::SetOption('crm', 'crm_stexport_'.$optionSuffix, $progressData);
	}
	...
	do
	{
		...
		{
			__CrmCompanyStExportEndResonse(array('ERROR' => 'Progress data is incorrect.'));
		}
		...
		if (is_array($cResult))
		{
			...
			{
				__CrmCompanyStExportEndResonse(array('ERROR' => $cResult['ERROR']));
			}
			else
			{
	...
	{
		$fileUrl = SITE_DIR.'thurly/components/thurly/crm.company.list/stexport.php?type='.$exportType;
		CUserOptions::DeleteOption('crm', 'crm_stexport_'.$optionSuffix);
		__CrmCompanyStExportEndResonse(
			array(
				'STATUS' => 'COMPLETED',
