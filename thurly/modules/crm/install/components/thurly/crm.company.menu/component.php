...
if($arParams['TYPE'] === 'list')
{
	...
	if ($bExport)
	{
		...
		$randomSequence = new Thurly\Main\Type\RandomSequence($stExportId);
		...
		$arResult['STEXPORT_PARAMS'] = array(
			...
			'initialOptions' => array(
				'REQUISITE_MULTILINE' => array(
					...
					'type' => 'checkbox',
					'title' => GetMessage('COMPANY_STEXPORT_OPTION_REQUISITE_MULTILINE'),
					'value' => 'N'
				)
			),
			...
			'messages' => array(
				'stExportExcelDlgTitle' => GetMessage('COMPANY_EXPORT_EXCEL_TITLE'),
				'stExportExcelDlgSummary' => GetMessage('COMPANY_STEXPORT_SUMMARY'),
				'stExportCsvDlgTitle' => GetMessage('COMPANY_EXPORT_CSV_TITLE'),
				'stExportCsvDlgSummary' => GetMessage('COMPANY_STEXPORT_SUMMARY')
			)
		);
		if ($isMyCompanyMode)
		{
			$arResult['STEXPORT_PARAMS']['myCompanyMode'] = true;
		}

		$arResult['BUTTONS'][] = array(
			'TITLE' => GetMessage('COMPANY_EXPORT_CSV_TITLE'),
			'TEXT' => GetMessage('COMPANY_EXPORT_CSV'),
			'ONCLICK' => "BX.Crm.StExportManager.items['".CUtil::JSEscape($stExportManagerId)."'].startExport('csv')",
			'ICON' => 'btn-export'
		);
		...
		$arResult['BUTTONS'][] = array(
			...
			'ONCLICK' => "BX.Crm.StExportManager.items['".CUtil::JSEscape($stExportManagerId)."'].startExport('excel')",
			'ICON' => 'btn-export'
		);
...
{
	$plannerButton = \Thurly\Crm\Activity\Planner::getToolbarButton($arParams['ELEMENT_ID'], CCrmOwnerType::Company);
	if($plannerButton)
	{
	...
	if(!$isResponsible)
	{
		...
		$arResult['SONET_SUBSCRIBE'] = array(
			...
			'SERVICE_URL' => CComponentEngine::makePathFromTemplate(
				'#SITE_DIR#thurly/components/thurly/crm.company.edit/ajax.php?site_id=#SITE#&sessid=#SID#',
