...
$APPLICATION->SetAdditionalCSS('/thurly/components/thurly/crm.product.section.tree/templates/.default/style.css');
$APPLICATION->SetAdditionalCSS('/thurly/components/thurly/crm.product.section.crumbs/templates/.default/style.css');
$APPLICATION->SetAdditionalCSS('/thurly/components/thurly/crm.interface.grid/templates/flat/thurly/main.interface.grid/.default/style.css');
$APPLICATION->SetAdditionalCSS('/thurly/components/thurly/crm.product.search.dialog/templates/.default/thurly/catalog.product.search/.default/style.css');

$APPLICATION->AddHeadScript('/thurly/js/crm/crm.js');
...
?>
	<table id="<?= $productContainerID ?>" class="crm-items-table" style="<?= $nProductRows === 0 ? 'display: none;' : '' ?>">
		...
		$measures = \Thurly\Crm\Measure::getMeasures(100);
		...
		$productEditorCfg = array(
			...
			'serviceUrl'=> '/thurly/components/thurly/crm.product_row.list/ajax.php?'.thurly_sessid_get(),
			'productSearchUrl'=> '/thurly/components/thurly/crm.product.list/list.ajax.php?'.thurly_sessid_get(),
			...
			'discountTypeText' => array(
				\Thurly\Crm\Discount::UNDEFINED => '-',
				\Thurly\Crm\Discount::PERCENTAGE => '%',
				\Thurly\Crm\Discount::MONETARY => $currencyText
			),
		...
		for ($i = 0; $i < $nProductRows; $i++)
		{
			...
			/*if ($row['DISCOUNT_TYPE_ID'] === \Thurly\Crm\Discount::MONETARY)
				$discountValue = number_format(doubleval($row['DISCOUNT_SUM']), 2, '.', '');
			else if ($row['DISCOUNT_TYPE_ID'] === \Thurly\Crm\Discount::PERCENTAGE)
			...
			// DISCOUNT TYPE
			/*$htmlValues['DISCOUNT_TYPE_TEXT'] = ($row['DISCOUNT_TYPE_ID'] === \Thurly\Crm\Discount::MONETARY) ? $currencyText : '%';*/
