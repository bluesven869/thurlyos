...
foreach($arResult['PRODUCT_ROWS'] as &$row)
{
	...
	if ($arResult['INVOICE_MODE'])
	{
		...
		$row['CUSTOMIZED'] = ($row['CUSTOM_PRICE'] === 'Y') ? 'Y' : 'N';
		...
		$row['DISCOUNT_TYPE_ID'] = \Thurly\Crm\Discount::MONETARY;

		$row['DISCOUNT_SUM'] = round((double)$row['DISCOUNT_PRICE'], 2);
		$row['DISCOUNT_RATE'] = \Thurly\Crm\Discount::calculateDiscountRate(
	...
	if(!isset($row['PRICE_NETTO']) || $row['PRICE_NETTO'] == 0.0)
	{
		...
		if($discountTypeID === \Thurly\Crm\Discount::MONETARY)
		{
		...
		else
		{
			...
			$discoutSum = $discoutRate < 100
				? \Thurly\Crm\Discount::calculateDiscountSum($row['PRICE_EXCLUSIVE'], $discoutRate)
