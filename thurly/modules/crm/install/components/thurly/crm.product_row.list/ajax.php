...
if(!function_exists('__CrmPropductRowListEndResonse'))
{
	function __CrmPropductRowListEndResonse($result)
	{
...
if(!isset($mode[0]))
{
	__CrmPropductRowListEndResonse(array());
}

$ownerType = isset($_POST['OWNER_TYPE']) ? $_POST['OWNER_TYPE'] : '';
if(!isset($ownerType[0]))
{
	__CrmPropductRowListEndResonse(array('ERROR'=>'OWNER_TYPE_NOT_FOUND'));
}
...
if($mode === 'SET_OPTION')
{
	...
	__CrmPropductRowListEndResonse($arResponse);
}
elseif($mode === 'CALC_PRODUCT_PRICES')
{
	if(!\Thurly\Crm\Security\EntityAuthorization::checkReadPermission($ownerTypeID, $ownerID, $perms))
	{
		__CrmPropductRowListEndResonse(array('ERROR'=>'PERMISSION_DENIED'));
	}

	$data = isset($_POST['DATA']) && is_array($_POST['DATA']) ? $_POST['DATA'] : array();
	if(count($data) == 0)
	{
		__CrmPropductRowListEndResonse(array('ERROR'=>'SOURCE_DATA_NOT_FOUND'));
	}
...
//	$dstExchRate = ($dstCurrency = CCrmCurrency::GetByID($dstCurrencyID)) ? $dstCurrency['EXCH_RATE'] : 1.0;
	...
	if(count($arProducts) > 0)
	{
		foreach($arProducts as &$arProduct)
		{
			...
			if (isset($arProduct['DISCOUNT_TYPE_ID']) && isset($arProduct['DISCOUNT_VALUE'])
				&& intval($arProduct['DISCOUNT_TYPE_ID']) === \Thurly\Crm\Discount::MONETARY
	...
	__CrmPropductRowListEndResonse(
		array(
...
elseif($mode === 'CONVERT_MONEY')
{
	...
	__CrmPropductRowListEndResonse(array('SUM' => CCrmCurrency::ConvertMoney($srcSum, $srcCurrencyID, $dstCurrencyID)));
}
elseif($mode === 'ADD_PRODUCT')
{
	...
	if($ownerID <= 0)
	{
		__CrmPropductRowListEndResonse(array('ERROR'=>'OWNER_ID_NOT_FOUND'));
	}

	if(!\Thurly\Crm\Security\EntityAuthorization::checkUpdatePermission($ownerTypeID, $ownerID, $perms))
	{
		__CrmPropductRowListEndResonse(array('ERROR'=>'PERMISSION_DENIED'));
	}
	...
	if($fields['PRODUCT_ID'] === 0)
	{
		...
		{
			__CrmPropductRowListEndResonse(array('ERROR' => 'CUSTOM_PRODUCT_NAME_NOT_ASSIGNED'));
		}
	}
	...
	if(isset($_POST['DISCOUNT_TYPE_ID']))
	{
		$fields['DISCOUNT_TYPE_ID'] = isset($_POST['DISCOUNT_TYPE_ID'])
			? intval($_POST['DISCOUNT_TYPE_ID']) : \Thurly\Crm\Discount::UNDEFINED;

		if(!\Thurly\Crm\Discount::isDefined($fields['DISCOUNT_TYPE_ID']))
		{
			$fields['DISCOUNT_TYPE_ID'] = \Thurly\Crm\Discount::PERCENTAGE;
		}
	}
	...
	if(!$ID)
	{
		__CrmPropductRowListEndResonse(array('ERROR' => CCrmProductRow::GetLastError()));
	}
	else
	{
		__CrmPropductRowListEndResonse(array('PRODUCT_ROW' => array('ID' => $ID)));
	}
}
elseif($mode === 'UPDATE_PRODUCT')
{
	$hasPermission = $ownerID > 0
		? \Thurly\Crm\Security\EntityAuthorization::checkUpdatePermission($ownerTypeID, $ownerID, $perms)
		: \Thurly\Crm\Security\EntityAuthorization::checkCreatePermission($ownerTypeID, $perms);
	...
	if($ID <= 0)
	{
		__CrmPropductRowListEndResonse(array('ERROR'=>'ID_NOT_FOUND'));
	}
...
elseif($mode === 'SAVE_PRODUCTS')
{
	...
	foreach ($arProducts as $arProduct)
	{
		...
		if(isset($arProduct['DISCOUNT_TYPE_ID']))
		{
			$fields['DISCOUNT_TYPE_ID'] = isset($arProduct['DISCOUNT_TYPE_ID'])
				? intval($arProduct['DISCOUNT_TYPE_ID']) : \Thurly\Crm\Discount::UNDEFINED;
	...
	if(!$bSuccess)
	{
		__CrmPropductRowListEndResonse(array('ERROR'=>'PRODUCT_ROWS_SAVING_ERROR'));
	}
	else
	{
		...
		__CrmPropductRowListEndResonse(array('PRODUCT_ROW_IDS' => $arProductRowIDs));
	}
}
elseif($mode === 'REMOVE_PRODUCT')
{
	...
	{
		__CrmPropductRowListEndResonse(array('DELETED_PRODUCT_ID' => $ID));
	}
}
elseif($mode === 'CALCULATE_TOTALS')
{
	...
	foreach($productRows as &$productRow)
	{
		...
		$productRow['DISCOUNT_TYPE_ID'] =
			(isset($productRow['DISCOUNT_TYPE_ID'])
				&& \Thurly\Crm\Discount::isDefined(intval($productRow['DISCOUNT_TYPE_ID']))) ?
				intval($productRow['DISCOUNT_TYPE_ID']) : \Thurly\Crm\Discount::PERCENTAGE;
	...
	{
		__CrmPropductRowListEndResonse(array('ERROR' => 'PERSON_TYPES_NOT_DEFINED'));
	}

	$personTypeID = isset($personTypeIDs[$clientTypeName]) ? intval($personTypeIDs[$clientTypeName]) : 0;
	if($personTypeID <= 0)
	{
		__CrmPropductRowListEndResonse(array('ERROR' => 'COULD_NOT_FIND_PERSON_TYPE'));
	}
