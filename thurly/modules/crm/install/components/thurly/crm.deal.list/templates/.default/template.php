...
foreach($arResult['DEAL'] as $sKey =>  $arDeal)
{
	...
	if(!$isInternal && $arParams['IS_RECURRING'] !== 'Y')
	{
		if($arResult['CAN_CONVERT'])
		{
			...
			{
				$arSchemeDescriptions = \Thurly\Crm\Conversion\DealConversionScheme::getJavaScriptDescriptions(true);
				$arSchemeList = array();
		...
		'columns' => array(
			...
			'STAGE_ID' => CCrmViewHelper::RenderDealStageControl(
				array(
					...
					'SERVICE_URL' => '/thurly/components/thurly/crm.deal.list/list.ajax.php',
...
if(isset($arResult['ERRORS']) && is_array($arResult['ERRORS']))
{
	...
	{
		$messages[] = array(
    		'TYPE' => \Thurly\Main\Grid\MessageType::ERROR,
    		'TITLE' => $error['TITLE'],
    		'TEXT' => $error['TEXT']
		);
	}
	...
	array(
		...
		'EXTENSION' => array(
			...
			'CONFIG' => array(
				...
				'serviceUrl' => '/thurly/components/thurly/crm.deal.list/list.ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
		...
		BX.ready(
			function()
			{
				BX.CrmDealConversionScheme.messages =
					<?=CUtil::PhpToJSObject(\Thurly\Crm\Conversion\DealConversionScheme::getJavaScriptDescriptions(false))?>;
				...
				{
					serviceUrl: "<?='/thurly/components/thurly/crm.deal.show/ajax.php?action=convert&'.thurly_sessid_get()?>",
		...
		BX.ready(
			function()
			{
				...
				var manager = BX.AutorunProcessManager.create("rebuildDealSearch",
					{
						serviceUrl: "<?='/thurly/components/thurly/crm.deal.list/list.ajax.php?'.thurly_sessid_get()?>",
