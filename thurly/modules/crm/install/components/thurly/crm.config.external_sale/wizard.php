...
//require($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/interface/prolog_jspopup_admin.php");
...
if ($processActions && !check_thurly_sessid())
...
if ($currentStep == 3 && $processActions && empty($errorMessage))
{
	...
	if (empty($errorMessage))
	{
		...
		else
		{
			$request = array(
				"METHOD" => "GET",
				"PATH" => "/thurly/admin/sale_order_new.php",
				"HEADERS" => array(),
				"BODY" => array()
			);
			...
			elseif (strpos($response["BODY"], "/thurly/") === false)
			{
				$errorMessage .= GetMessage("CRM_EXT_SALE_C1NO_THURLY")."<br>";
			}
		}
	}
}
...
<form method="post">
	...
	elseif ($currentStep == 4)
	{
		...
		<table align="center" id="id_es_progress_stop">
			<tr>
				<td align="center">
					<img src="/thurly/components/thurly/crm.config.external_sale/images/pb1.gif" alt="Loading..."/><br />
		...
		<script type="text/javascript">
			...
			function ExtSaleDoSync()
			{
				...
				BX.ajax.get(
					"/thurly/components/thurly/crm.config.external_sale/ajax.php",
					{
						id:<?=$id?>,
						skip_bp:'Y',
						skip_notify:'Y',
