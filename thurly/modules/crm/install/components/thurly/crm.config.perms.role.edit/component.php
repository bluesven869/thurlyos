...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && (isset($_POST['save']) || isset($_POST['apply'])) && check_thurly_sessid())
{
	$bVarsFromForm = true;
	$arFields = array(
		'NAME' => $_POST['NAME'],
...
else if ($_SERVER['REQUEST_METHOD'] == 'GET' && isset($_GET['delete']) && check_thurly_sessid() && $arParams['ROLE_ID'] > 0)
{
	$CCrmRole = new CCrmRole();
...
$dealCategoryConfigs = Thurly\Crm\Category\DealCategory::getPermissionRoleConfigurations();
...
$arPerms = array('READ', 'ADD', 'WRITE', 'DELETE', 'EXPORT', 'IMPORT');
...
$arResult['ROLE_PERM']['COMPANY'] = $arResult['ROLE_PERM']['CONTACT'] = $permissionSet;
$arResult['ROLE_PERM']['WEBFORM'] = $arResult['ROLE_PERM']['BUTTON'] = array(
	BX_CRM_PERM_NONE => GetMessage('CRM_PERMS_TYPE_'.BX_CRM_PERM_NONE),
	BX_CRM_PERM_ALL => GetMessage('CRM_PERMS_TYPE_'.BX_CRM_PERM_ALL)
);
...
$arResult['PATH_TO_ROLE_DELETE'] =  CHTTP::urlAddParams(CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_ROLE_EDIT'],
	...
	array('delete' => '1', 'sessid' => thurly_sessid())
);
