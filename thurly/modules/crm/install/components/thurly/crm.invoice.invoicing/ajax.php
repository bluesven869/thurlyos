...
CThurlyComponent::includeComponentClass("thurly:crm.invoice.invoicing");
$orderPayment = new CrmInvoiceInvoicingComponent();

$result = array();
if (check_thurly_sessid() && $request->isPost() && $request->isAjaxRequest())
{
	$service = null;
	...
	if ($request->get('PAY_SYSTEM_ID') === null)
	{
		...
		if ($paySystemList)
		{
			...
			if ($paySystemId > 0)
				$service = \Thurly\Sale\PaySystem\Manager::getObjectById($paySystemId);
		}
	}
	else
	{
		$service = \Thurly\Sale\PaySystem\Manager::getObjectById($request->get('PAY_SYSTEM_ID'));
	}
