var CrmButtonEditor = function(params) {
	...
	this.initToolTips = function(context)
	{
		...
		this.initToolTipNodes(nodeList);
	};

	this.initToolTipNodes = function(nodeList)
	{
		var _this = this;
	...
	this.showTooltip = function(node, text)
	{
		if (!this.popupTooltip)
		{
			this.popupContentNode = BX.create("div", {attrs: {style: "padding-right: 5px; width: 250px;" }});
			this.popupTooltip = new BX.PopupWindow('bx-crm-site-button-tooltip', node, {
				lightShadow: true,
				autoHide: false,
				darkMode: true,
				offsetLeft: 9,
				...
				zIndex: 200,
				content: this.popupContentNode
			});
			this.popupTooltip.setAngle({offset:13, position: 'bottom'});
		}

		this.popupContentNode.innerHTML = text;
		this.popupTooltip.setBindElement(node);
	...
	this.sendActionRequest = function(action, sendData, callbackSuccess, callbackFailure)
	{
		...
		if (sendData instanceof FormData)
		{
			...
			sendData.append('sessid', BX.thurly_sessid());
		...
		else
		{
			...
			sendData.sessid = BX.thurly_sessid();
		}
	...
	this.initSlider = function()
	{
		...
		this.slider.bindClose(BX('CRM_BUTTON_EDIT_TO_LIST'));
		this.slider.bindClose(BX('CRM_BUTTON_EDIT_TO_LIST_BOTTOM'));
		this.slider.setHrefTargetTop(BX('crm_button_main_form'));
	};

	this.slider = {
		setHrefTargetTop: function (context)
		{
			var elements = context.querySelectorAll('[data-bx-slider-href]');
			elements = BX.convert.nodeListToArray(elements);
		...
		{
			e.preventDefault();
			window.top.BX.ThurlyOS.PageSlider.close();
		},
		open: function (url)
		{
			window.top.BX.ThurlyOS.PageSlider.open(url);
		},
	...
	onChangeSelect: function(node)
	{
		...
		if (type == 'openline')
		{
			...
			connectors.forEach(function (connector) {
				...
				BX.addClass(node, 'connector-icon-' + connector.code.replace('.', '-'));
...
CrmButtonEditLineManager.prototype =
{
	...
	createLine: function ()
	{
		...
		this.caller.caller.slider.setHrefTargetTop(lineNode);
...
CrmButtonEditLine.prototype =
{
	...
	{
		node = node.querySelector('[data-hint]');
		if (!node)
		{
			return;
		}
