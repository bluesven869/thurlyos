...
use Thurly\Crm\Settings\ContactSettings;
/**
 * Thurly vars
 *
 * @var array $arParams
...
if($leadID > 0)
{
	$conversionWizard = \Thurly\Crm\Conversion\LeadConversionWizard::load($leadID);
...
else
{
	...
	if (isset($_GET['company_id']))
	{
		...
		$arFields['COMPANY_BINDINGS'] = \Thurly\Crm\Binding\EntityBinding::prepareEntityBindings(
...
else
{
	if($_SERVER['REQUEST_METHOD'] == 'POST' && check_thurly_sessid())
	{
		...
		elseif(isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']) || isset($_POST['continue']))
		{
			...
			if(isset($_POST['COMPANY_IDS']))
			{
				...
				\Thurly\Crm\Binding\EntityBinding::markFirstAsPrimary($arFields['COMPANY_BINDINGS']);
			}
			...
			elseif(!$isEditMode)
			{
				$arFields['OPENED'] = \Thurly\Crm\Settings\ContactSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N';
			}
			...
			if(isset($_POST['REQUISITE']) && is_array($_POST['REQUISITE']))
			{
				...
				{
					$requisiteFields = \Thurly\Crm\EntityRequisite::parseFormData($requisiteForm);
					if(is_array($requisiteFields))
					{
						...
						if (is_array($requisiteForm['BANK_DETAILS']) && !empty($requisiteForm['BANK_DETAILS']))
						{
							...
							{
								$bankDetailFields = \Thurly\Crm\EntityBankDetail::parseFormData($bankDetailForm);
	...
	else if (isset($_GET['delete']) && check_thurly_sessid())
	{
		if ($isEditMode)
		{
...
$enableDupControl = $arResult['DUPLICATE_CONTROL']['ENABLED'] =
	!$isEditMode && \Thurly\Crm\Integrity\DuplicateControl::isControlEnabledFor(CCrmOwnerType::Contact);
...
if($enableDupControl)
{
	...
	$countriesInfo = Thurly\Crm\EntityPreset::getCountriesInfo();
...
ob_start();
$APPLICATION->IncludeComponent('thurly:crm.field_multi.edit',
	'new',
	array(
		'FM_MNEMONIC' => 'CONFM',
		'ENTITY_ID' => 'CONTACT',
...
if (CCrmCompany::CheckReadPermission(0, $userPermissions))
{
	...
	{
		$companyBindings = \Thurly\Crm\Binding\ContactCompanyTable::getContactBindings($arParams['ELEMENT_ID']);
	}
	elseif(isset($arResult['ELEMENT']['COMPANY_ID']))
	{
		//For backward compatibility
		$companyBindings = \Thurly\Crm\Binding\EntityBinding::prepareEntityBindings(
			CCrmOwnerType::Company,
	...
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'componentParams' => array(
			...
			'ENTITY_IDS' => \Thurly\Crm\Binding\EntityBinding::prepareEntityIDs(CCrmOwnerType::Company, $companyBindings),
...
if($enableOutmodedFields)
{
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'type' => 'address',
		'componentParams' => array(
			'SERVICE_URL' => '/thurly/components/thurly/crm.contact.edit/ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
			'DATA' => array(
...
$arResult['FIELDS']['tab_1'][] = array(
	...
	'value' => isset($arResult['ELEMENT']['OPENED'])
		? $arResult['ELEMENT']['OPENED'] : (\Thurly\Crm\Settings\ContactSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N'),
...
foreach ($bankDetailFormData as $requisiteId => $requisiteBankDetails)
{
	if (is_array($requisiteBankDetails) && !empty($requisiteBankDetails) && isset($requisiteFormData[$requisiteId]))
	{
		...
		foreach ($requisiteBankDetails as $pseudoId => $bankDetailFields)
		{
			...
			{
				if ($fValue instanceof \Thurly\Main\Type\DateTime)
...
ob_start();
$GLOBALS['APPLICATION']->IncludeComponent('thurly:crm.requisite.form.editor',
	'',
	array(
		'PRESET_ENTITY_TYPE_ID' => \Thurly\Crm\EntityPreset::Requisite,
...
else
{
	...
	$fileViewer = new \Thurly\Crm\UserField\FileViewer(CCrmOwnerType::Contact, $arResult['ELEMENT']['ID']);
}
...
include_once($_SERVER['DOCUMENT_ROOT'].'/thurly/components/thurly/crm.contact/include/nav.php');
