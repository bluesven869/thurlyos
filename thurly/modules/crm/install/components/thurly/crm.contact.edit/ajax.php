...
if(!function_exists('__CrmContactEditEndResonse'))
{
	function __CrmContactEditEndResonse($result)
	{
...
{
	__CrmContactEditEndResonse(array('ERROR' => 'Could not include crm module.'));
}
/*
...
{
	__CrmContactEditEndResonse(array('ERROR' => 'Access denied.'));
}
if ($_SERVER['REQUEST_METHOD'] != 'POST')
{
	__CrmContactEditEndResonse(array('ERROR' => 'Request method is not allowed.'));
}
...
elseif($action === 'FIND_DUPLICATES')
{
	...
	if($entityTypeName === '')
	{
		__CrmContactEditEndResonse(array('ERROR' => 'Entity type is not specified.'));
	}

	$entityTypeID = CCrmOwnerType::ResolveID($entityTypeName);
	if($entityTypeID === CCrmOwnerType::Undefined)
	{
		__CrmContactEditEndResonse(array('ERROR' => 'Undefined entity type is specified.'));
	}

	if($entityTypeID !== CCrmOwnerType::Contact)
	{
		__CrmContactEditEndResonse(array('ERROR' => "The '{$entityTypeName}' type is not supported in current context."));
	}
	...
	$checker = new \Thurly\Crm\Integrity\ContactDuplicateChecker();
	$groupResults = array();
	...
	__CrmContactEditEndResonse(array('GROUP_RESULTS' => $groupResults));
}
elseif($action === 'FIND_LOCALITIES')
{
	...
	if($localityType === 'COUNTRY')
	{
		...
		__CrmContactEditEndResonse(array('DATA' => array('ITEMS' => $result)));
	}
	else
	{
		__CrmContactEditEndResonse(array('ERROR' => "Locality '{$localityType}' is not supported in current context."));
	}
}
else
{
	__CrmContactEditEndResonse(array('ERROR' => "Action '{$action}' is not supported in current context."));
}
