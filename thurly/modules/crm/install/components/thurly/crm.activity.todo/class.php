...
class CrmActivityTodoComponent extends \CThurlyComponent
{
	private $ids = 0;
	private $typeId = 0;
	...
	private function getActivity()
	{
		...
		while ($activity = $res->getNext())
		{
			...
			if ($activity['DEADLINE'] && $activity['COMPLETED'] != 'Y')
			{
				...
				$date = \Thurly\Main\Type\DateTime::createFromTimestamp($dateTime);
		...
		//waiter
		$wait = \Thurly\Crm\Pseudoactivity\WaitEntry::getRecentByOwner(
			$this->typeId,
			$id
		);
		if ($wait)
