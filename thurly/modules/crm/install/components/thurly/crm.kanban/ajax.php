...
{
	$result = $APPLICATION->IncludeComponent('thurly:crm.kanban', '', array(
		'IS_AJAX' => 'Y',
		'ENTITY_TYPE' => $type,
		'GET_AVATARS' => 'Y',
		'ADDITIONAL_FILTER' =>
...
elseif ($action == 'status' && !empty($id) && !empty($newState))
{
	...
	$result = $APPLICATION->IncludeComponent('thurly:crm.kanban', '', $params);
}
//activity items
elseif ($action == 'activities' && !empty($id))
{
	$APPLICATION->IncludeComponent('thurly:crm.activity.todo', '', array(
		'OWNER_TYPE_ID' => $type,
		'OWNER_ID' => $id,
		'IS_AJAX' => 'Y',
		'COMPLETED' => 'N'
