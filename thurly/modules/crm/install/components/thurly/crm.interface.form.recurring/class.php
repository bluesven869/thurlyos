...
use Thurly\Main\Localization\Loc,
	...
	Thurly\Crm\Category\DealCategory,
	Thurly\Crm\Recurring\Manager,
	Thurly\Crm\Recurring\Entity\Base;

Loc::loadMessages(__FILE__);

class CrmInterfaceFormRecurring extends CThurlyComponent
{
	private $data = array();
	private $type = array();
	...
	public function executeComponent()
	{
		...
		{
			$this->arResult['NEXT_EXECUTION_HINT'] = Loc::getMessage('NEXT_EXECUTION_' . $this->arParams['ENTITY_TYPE'] . '_HINT', array('#DATE_EXECUTION#' => Calculator::getNextDate($this->data)));
		}
		else
		{
		...
		$this->arResult['AJAX_URL'] = '/thurly/components/thurly/crm.interface.form.recurring/ajax.php?siteID=' . SITE_ID . '&' . thurly_sessid_get();
		...
		if (CModule::IncludeModule('thurlyos'))
		{
			...
			switch (LANGUAGE_ID)
			{
				...
				case "kz":
				case "by":
					$link = 'https://www.thurlyos.ru/pro/crm.php ';
					break;
				case "de":
					$link = 'https://www.thurlyos.de/pro/crm.php';
					break;
				case "ua":
					$link = 'https://www.thurlyos.ua/pro/crm.php';
					break;
				default:
	...
	protected function getDealPeriodHint($data)
	{
		...
		{
			$typeElement = \Thurly\Crm\MessageHelper::getNumberDeclension(
	...
	protected function getConstraintHint($data)
	{
		if ((int)$data['LIMIT_REPEAT'] > 0 && ($data['REPEAT_TILL'] === 'times' || $data['REPEAT_TILL'] === Base::LIMITED_BY_TIMES))
		{
			...
			{
				$form = \Thurly\Crm\MessageHelper::getNumberDeclension(
					$times,
