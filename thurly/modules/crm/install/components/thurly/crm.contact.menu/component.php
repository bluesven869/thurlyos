...
if($arParams['TYPE'] === 'list')
{
	...
	if ($bExport)
	{
		...
		if (CModule::IncludeModule('webservice') && class_exists("\\Thurly\\WebService\\StsSync"))
		{
			...
			$arResult['BUTTONS'][] = array(
				...
				'ONCLICK' => \Thurly\WebService\StsSync::getUrl('contacts', 'contacts_crm', $APPLICATION->GetCurPage(), $sPrefix, GetMessage('CRM_OUTLOOK_TITLE_CONTACTS'), $GUID),
				'ICON' => 'btn-export'
			);
		}
	}
...
{
	$plannerButton = \Thurly\Crm\Activity\Planner::getToolbarButton($arParams['ELEMENT_ID'], CCrmOwnerType::Contact);
	if($plannerButton)
	{
	...
	if(!$isResponsible)
	{
		...
		$arResult['SONET_SUBSCRIBE'] = array(
			...
			'SERVICE_URL' => CComponentEngine::makePathFromTemplate(
				'#SITE_DIR#thurly/components/thurly/crm.contact.edit/ajax.php?site_id=#SITE#&sessid=#SID#',
