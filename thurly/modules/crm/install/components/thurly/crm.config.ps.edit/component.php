...
$documentRoot = \Thurly\Main\Application::getDocumentRoot();
...
if ($psID > 0)
{
	$arPaySys = \Thurly\Sale\PaySystem\Manager::getById($psID);
...
if (check_thurly_sessid())
{
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && (isset($_POST['save']) || isset($_POST['apply'])))
	{
		...
		if (strlen($errorMessage) <= 0)
		{
			...
			if ($errorMessage === '')
			{
				...
				$path = \Thurly\Sale\PaySystem\Manager::getPathToHandlerFolder($handler);
				if (\Thurly\Main\IO\File::isFileExists($documentRoot.$path.'/handler.php'))
				{
					require_once $documentRoot.$path.'/handler.php';

					$className = \Thurly\Sale\PaySystem\Manager::getClassNameFromPath($path);
				...
				if ($psID === 0)
				{
					$image = '/thurly/images/sale/sale_payments/'.$handler.'.png';
				...
				if ($errorMessage === '')
				{
					if ($psID > 0)
					{
						$result = \Thurly\Sale\Internals\PaySystemActionTable::update($psID, $arActFields);
					}
					else
					{
						$result = \Thurly\Sale\Internals\PaySystemActionTable::add($arActFields);
						if ($result->isSuccess())
						{
							$psID = $result->getId();
							\Thurly\Sale\Internals\PaySystemActionTable::update($psID, array('PAY_SYSTEM_ID' => $psID));
						}
					}
					...
					if (array_key_exists('YANDEX_PUBLIC_KEY', $_FILES) && $_FILES['YANDEX_PUBLIC_KEY']['tmp_name'])
					{
						...
						{
							$shopId = \Thurly\Sale\BusinessValue::get('YANDEX_INVOICE_SHOP_ID', 'PAYSYSTEM_'.$psID, $personTypeId);
							if (strlen($shopId) > 0)
							{
								...
								if ($dbRes->fetch())
									...
									\Thurly\Sale\Internals\YandexSettingsTable::add(array('SHOP_ID' => $shopId, 'PUB_KEY' => $publicKey));
							}
						}
						else
						{
	...
	elseif ($_SERVER['REQUEST_METHOD'] == 'GET' &&  isset($_GET['delete']))
	{
		...
		$result = \Thurly\Sale\PaySystem\Manager::delete($psID);
		if (!$result->isSuccess())
...
if (array_key_exists('AJAX', $_REQUEST) && $_REQUEST['AJAX'] == 'Y')
{
	...
	{
		$data = \Thurly\Sale\PaySystem\Manager::getById($psID);
		$personTypeList = \Thurly\Sale\PaySystem\Manager::getPersonTypeIdList($psID);
...
$fieldsGroups = \Thurly\Sale\BusinessValue::getGroups();
...
$service = new \Thurly\Sale\PaySystem\Service($arResult['PAY_SYSTEM']);
if ($service)
{
