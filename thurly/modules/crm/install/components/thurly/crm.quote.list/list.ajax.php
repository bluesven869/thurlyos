...
{
	$agent = \Thurly\Crm\Agent\Search\QuoteSearchContentRebuildAgent::getInstance();
	if(!$agent->isEnabled())
	{
		__CrmQuoteListEndResonse(array('STATUS' => 'COMPLETED'));
	}

	$progressData = $agent->getProgressData();
	__CrmQuoteListEndResonse(
		array(
			'STATUS' => 'PROGRESS',
...
elseif ($action === 'SAVE_PROGRESS')
{
	...
	{
		__CrmQuoteListEndResonse(array('ERROR' => 'Invalid data!'));
	}
	...
	{
		__CrmQuoteListEndResonse(array('ERROR' => 'Not found!'));
	}
...
__CrmQuoteListEndResonse(array());
