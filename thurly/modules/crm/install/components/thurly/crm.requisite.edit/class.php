...
use \Thurly\Crm\Integration\ClientResolver;
...
class CCrmRequisiteEditComponent extends \CThurlyComponent
{
	protected $site;
	protected $elementId;
	...
	protected $presetId;
	/** @var \Thurly\Crm\EntityPreset $preset */
	protected $preset;
	...
	/** @var \Thurly\Crm\EntityBankDetail $bankDetail */
	...
	public function executeComponent()
	{
		...
		include_once($_SERVER['DOCUMENT_ROOT'].'/thurly/components/thurly/crm.requisite/include/nav.php');
	...
	protected function parseParams()
	{
		...
		$fieldsOfActivePresetsInfo = $this->preset->getSettingsFieldsOfPresets(
			\Thurly\Crm\EntityPreset::Requisite,
			'active'
		);
		...
		$this->bVarsFromForm =
			$this->externalFormData === null
			&& $_SERVER['REQUEST_METHOD'] === 'POST' && check_thurly_sessid()
			&& (isset($_POST['saveAndView'])
				...
				|| isset($_POST['reload'])
				|| isset($_POST['verify']));
		$this->bDeleteAction = (isset($_REQUEST['delete']) && check_thurly_sessid());
		...
		if (!$this->bVarsFromForm && !$this->bDeleteAction)
		{
			...
			if ($jsonData)
			{
				try
				{
					$requisiteData = \Thurly\Main\Web\Json::decode($jsonData);
		...
		{
			$this->enableDupControl = Thurly\Crm\Integrity\DuplicateControl::isControlEnabledFor($this->entityTypeId);
		}
	...
	protected function prepareExistingFields()
	{
		...
		if ($this->bCopy)
		{
			$curDateTime = new \Thurly\Main\Type\DateTime();
	...
	protected function processFormAction()
	{
		if (in_array($this->formAction, $this->validActions, true))
		{
			...
			if ($this->bEdit)
			{
				$this->fields['DATE_MODIFY'] = new \Thurly\Main\Type\DateTime();
			...
			if($result && $result->isSuccess())
			{
				...
				if ($result && $result->isSuccess() && is_array($this->fields))
				{
					...
					$fieldsInView = array_intersect(
						array_keys($this->presetFieldsInShortList),
						array_keys($this->fieldsAllowed)
					);
	...
	protected function prepareResult()
	{
		...
		if (is_array($this->requisiteData))
		{
			...
			try
			{
				$jsonData = \Thurly\Main\Web\Json::encode($this->requisiteData);
			}
			catch (\Thurly\Main\SystemException $e)
			{
			}
		...
		foreach ($fieldsInfo as $fieldName => $fieldInfo)
		{
			if ($fieldInfo['isRQ'] && !$fieldInfo['isUF'])
			{
				...
				if (!empty($fieldTitle))
				{
					...
					{
						$addressTypeInfos = Thurly\Crm\RequisiteAddress::getClientTypeInfos();
						$addressTypeInfosDesc = Thurly\Crm\RequisiteAddress::getTypeInfos();
						...
						$rqFields[] = array(
							...
							'componentParams' => array(
								...
								'SERVICE_URL' => '/thurly/components/thurly/crm.requisite.edit/ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
								'SCHEME' => array(
		...
		// append user fields
		$fileViewer = new \Thurly\Crm\Requisite\FileViewer();
