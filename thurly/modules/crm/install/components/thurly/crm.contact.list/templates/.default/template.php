	...
	&& ($allowWrite || $allowDelete || $callListUpdateMode))
{
	...
	if($allowWrite)
	{
		...
		{
			$actionList[] = array(
				'NAME' => GetMessage('CRM_CONTACT_SUBSCRIBE'),
				'VALUE' => 'subscribe',
				'ONCHANGE' => array(
					array(
	...
	array(
		...
		'EXTENSION' => array(
			...
			'CONFIG' => array(
				...
				'serviceUrl' => '/thurly/components/thurly/crm.contact.list/list.ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
...
BX.ready(
	function()
	{
		...
		var mgr = BX.CrmDuplicateManager.create("mgr", { entityTypeName: "<?=CUtil::JSEscape(CCrmOwnerType::ContactName)?>", serviceUrl: "<?=SITE_DIR?>thurly/components/thurly/crm.contact.list/list.ajax.php?&<?=thurly_sessid_get()?>" });
		BX.addCustomEvent(
			mgr,
		...
		BX.ready(
			function()
			{
				...
				var manager = BX.AutorunProcessManager.create("rebuildContactSearch",
					{
						serviceUrl: "<?='/thurly/components/thurly/crm.contact.list/list.ajax.php?'.thurly_sessid_get()?>",
	...
	function()
	{
		...
		var converter = BX.CrmRequisiteConverter.create(
			"converter",
			{
				...
				serviceUrl: "<?=SITE_DIR?>thurly/components/thurly/crm.contact.list/list.ajax.php?&<?=thurly_sessid_get()?>"
			}
		);
