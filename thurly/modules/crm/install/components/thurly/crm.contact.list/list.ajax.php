...
if ($_REQUEST['MODE'] == 'SEARCH')
{
	...
	__CrmContactListEndResonse($arData);
}
elseif ($action === 'REBUILD_SEARCH_CONTENT')
{
	$agent = \Thurly\Crm\Agent\Search\ContactSearchContentRebuildAgent::getInstance();
	if(!$agent->isEnabled())
	{
		__CrmContactListEndResonse(array('STATUS' => 'COMPLETED'));
	}
...
{
	$agent = \Thurly\Crm\Agent\Timeline\ContactTimelineBuildAgent::getInstance();
	if(!$agent->isEnabled())
	{
		__CrmContactListEndResonse(array('STATUS' => 'COMPLETED'));
	}
...
elseif ($action === 'GET_REQUISITE_PRESETS')
{
	...
	$presetEntity = new \Thurly\Crm\EntityPreset();
...
elseif ($action === 'BUILD_REQUISITES')
{
	...
	if($presetID <= 0)
	{
		__CrmContactListEndResonse(array('ERROR' => "Preset ID is not specified."));
	}
...
elseif ($action === 'CONVERT_REQUISITES')
{
	...
	if($itemQty > 0)
	{
		foreach($converters as $converter)
		{
			...
			{
				__CrmContactListEndResonse(array('ERROR' => $e->getLocalizedMessage()));
			}
			catch(Exception $e)
			{
				__CrmContactListEndResonse(array('ERROR' => $e->getMessage()));
			}
		}
