...
use Thurly\Crm\Format\ContactAddressFormatter;
use Thurly\Crm\Format\AddressSeparator;
...
class CCrmContactDetailsComponent extends CThurlyComponent
{
	/** @var string */
	protected $guid = '';
	/** @var int */
	...
	public function executeComponent()
	{
		...
		$this->enableOutmodedFields = \Thurly\Crm\Settings\ContactSettings::getCurrent()->areOutmodedRequisitesEnabled();
		...
		$this->enableDupControl = $this->arResult['DUPLICATE_CONTROL']['enabled'] =
			!$this->isEditMode && \Thurly\Crm\Integrity\DuplicateControl::isControlEnabledFor(CCrmOwnerType::Contact);

		if($this->enableDupControl)
		{
			$this->arResult['DUPLICATE_CONTROL']['serviceUrl'] = '/thurly/components/thurly/crm.contact.edit/ajax.php?'.thurly_sessid_get();
		...
		$this->arResult['EDITOR_CONFIG_ID'] = isset($this->arParams['EDITOR_CONFIG_ID'])
			? $this->arParams['EDITOR_CONFIG_ID'] : 'contact_details';
		//endregion
		...
		{
			$APPLICATION->SetTitle($this->entityData['FORMATTED_NAME']);
		}
		elseif(!$this->isEditMode)
		{
		...
		if($this->entityID > 0)
		{
			$this->arResult['TABS'][] = array(
				...
				'loader' => array(
					'serviceUrl' => '/thurly/components/thurly/crm.deal.list/lazyload.ajax.php?&site'.SITE_ID.'&'.thurly_sessid_get(),
					'componentData' => array(
						'template' => '',
						'params' => array(
							'DEAL_COUNT' => '20',
			...
			$this->arResult['TABS'][] = array(
				...
				'loader' => array(
					'serviceUrl' => '/thurly/components/thurly/crm.client.portrait/lazyload.ajax.php?&site='.SITE_ID.'&'.thurly_sessid_get(),
					'componentData' => array(
						'template' => '.default',
						'params' => array(
		...
		$this->arResult['ENTITY_FIELDS'] = array(
			...
			array(
				...
				'data' => array(
					'map' => array(
						'secondaryEntityType' => '',
						'secondaryEntityIds' => 'COMPANY_IDS'
					),
					'enablePrimaryEntity' => false,
					'secondaryEntityTypeName' => CCrmOwnerType::CompanyName,
					'secondaryEntityInfo' => 'COMPANY_DATA',
					'secondaryEntityLegend' => Loc::getMessage('CRM_CONTACT_FIELD_COMPANY_LEGEND'),
					'loaders' => array(
						'primary' => array(
			...
			array(
				...
				'data' => array(
					'presets'=> \CCrmInstantEditorHelper::PrepareListOptions(
						\Thurly\Crm\EntityPreset::getActiveItemList()
					)
				)
			)
		);
		...
		if($this->enableOutmodedFields)
		{
			$this->arResult['ENTITY_FIELDS'][] = array(
				...
				'data' => array(
					...
					'labels' => \Thurly\Crm\EntityAddress::getLabels(),
					'view' => 'ADDRESS_HTML'
				)
			);
		}
			...
			elseif($userField['USER_TYPE_ID'] === 'file')
			{
				$fieldInfo['ADDITIONAL'] = array(
					'URL_TEMPLATE' => \CComponentEngine::MakePathFromTemplate(
						'/thurly/components/thurly/crm.contact.show/show_file.php?ownerId=#owner_id#&fieldName=#field_name#&fileId=#file_id#',
						array(
	...
	public function prepareEntityData()
	{
		...
		{
			$this->entityData = array();
			$this->entityData['OPENED'] = \Thurly\Crm\Settings\ContactSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N';
		...
		if($this->entityID > 0)
		{
			$companyIDs = Thurly\Crm\Binding\ContactCompanyTable::getContactCompanyIDs($this->entityID);
		}
		...
		$this->entityData['COMPANY_DATA'] = $companyData;
