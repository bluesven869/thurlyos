...
class CCrmConfigEmailtrackerComponent extends CThurlyComponent
{
	...
	public function executeComponent()
	{
		...
		$res = \Thurly\Mail\MailServicesTable::getList(array(
		...
		while ($service = $res->fetch())
		{
			$this->arParams['SERVICES'][$service['ID']] = array(
				...
				'name'       => $service['NAME'],
				'link'       => $service['LINK'],
				'icon'       => \Thurly\Mail\MailServicesTable::getIconSrc($service['NAME'], $service['ICON']),
		...
		if (!empty($this->arParams['MAILBOX']))
		{
			...
			if (in_array('crm_preconnect', $options['flags']))
			{
				...
				$imapDirs = \Thurly\Mail\Helper::listImapDirs($mailbox, $error, $errors);
				if ($imapDirs !== false)
				{
					$mbData = array(
						'OPTIONS' => $options,
			...
			{
				$this->arParams['LEAD_RESP_SELECTED'] = \Thurly\Main\UserTable::getList(array(
					'filter' => array(
