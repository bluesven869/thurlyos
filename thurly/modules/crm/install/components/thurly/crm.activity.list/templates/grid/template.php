...
Thurly\Main\Page\Asset::getInstance()->addCss("/thurly/themes/.default/crm-entity-show.css");
...
use Thurly\Crm\Activity\Provider\ProviderManager;
...
foreach($arResult['ITEMS'] as &$item)
{
	...
	elseif($typeID === CCrmActivityType::Provider && $provider !== null):
		...
		if (
			...
			in_array($item['ORIGINATOR_ID'], array('THURLY', 'WORDPRESS', 'DRUPAL', 'JOOMLA', 'MAGENTO'))
		)
		{
...
if($enableToolbar)
{
	$isSingleButtonMode = SITE_TEMPLATE_ID === 'thurlyos' && !$isInternal;
	...
	array(
		...
		'EXTENSION' => array(
			...
			'CONFIG' => array(
				...
				'gridId' => $arResult['UID'],
				'serviceUrl' => '/thurly/components/thurly/crm.activity.list/list.ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
		...
		BX.ready(
			function()
			{
				...
				var manager = BX.AutorunProcessManager.create("rebuildActivitySearch",
						{
						serviceUrl: "<?='/thurly/components/thurly/crm.activity.list/list.ajax.php?'.thurly_sessid_get()?>",
