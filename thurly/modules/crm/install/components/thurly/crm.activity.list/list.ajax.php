...
{
	$agent = \Thurly\Crm\Agent\Search\ActivitySearchContentRebuildAgent::getInstance();
	if(!$agent->isEnabled())
	{
		__CrmActivityListEndResonse(array('STATUS' => 'COMPLETED'));
	}

	$progressData = $agent->getProgressData();
	__CrmActivityListEndResonse(
		array(
			'STATUS' => 'PROGRESS',
...
{
	$agent = \Thurly\Crm\Agent\Timeline\ActivityTimelineBuildAgent::getInstance();
	if(!$agent->isEnabled())
	{
		__CrmActivityListEndResonse(array('STATUS' => 'COMPLETED'));
	}
...
elseif ($action === 'REBUILD_ACT_STATISTICS')
{
	...
	if($itemQty > 0)
	{
		\Thurly\Crm\Statistics\ActivityStatisticEntry::rebuild($itemIDs);
