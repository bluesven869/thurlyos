...
use Thurly\Crm\Integrity\DuplicateIndexType;
...
$itemData = array();
$itemNum = 0;
/** @var Thurly\Crm\Integrity\Duplicate $item **/
		...
		function()
		{
			...
			var list = BX.CrmDedupeList.create(
				"<?=CUtil::JSEscape($listID)?>",
				{
					...
					serviceUrl: "<?='/thurly/components/thurly/crm.dedupe.list/ajax.php?'.thurly_sessid_get()?>",
		...
		function()
		{
			...
			var mgr = BX.CrmDuplicateManager.create(
				"mgr",
				{
					...
					serviceUrl: "<?=SITE_DIR?>thurly/components/thurly/crm.<?=strtolower($entityTypeName)?>.list/list.ajax.php?&<?=thurly_sessid_get()?>"
				}
			);
			BX.addCustomEvent(
				mgr,
