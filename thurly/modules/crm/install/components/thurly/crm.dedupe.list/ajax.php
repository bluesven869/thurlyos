...
use Thurly\Crm\Integrity;
use Thurly\Crm\Merger;
global $APPLICATION;
...
if (!$currentUser || !$currentUser->IsAuthorized() || !check_thurly_sessid())
{
		...
		foreach ($columnList as $columnInfo)
		{
			if (is_array($columnInfo) && isset($columnInfo['GROUP_NAME']))
			{
				...
				if ($isRequisite || $isBankDetail)
				{
					...
					{
						if ($isRequisite)
							Thurly\Crm\EntityRequisite::prepareEntityInfoBatch($entityTypeID, $entityInfos, $scope, $fieldName);
						else
							Thurly\Crm\EntityBankDetail::prepareEntityInfoBatch($entityTypeID, $entityInfos, $scope, $fieldName);
					}
				}
			}
		}
...
elseif($action === 'REBUILD_DEDUPE_INDEX')
{
	...
	if(empty($typeIDs))
	{
		__CrmDedupeListEndResonse(array('ERROR' => 'Index type names are not defined or invalid.'));
	}
	...
	{
		__CrmDedupeListEndResonse(array('ERROR' => 'Scope is invalid.'));
	}
