...
if (!($hasReadPermission && \Thurly\Crm\Restriction\RestrictionManager::isDuplicateControlPermitted()))
{
...
else
{
	...
	foreach ($typeScopeMap as $typeID => $scopes)
	{
		foreach (array_keys($scopes) as $scope)
		{
			...
			if(($typeID & Integrity\DuplicateIndexType::REQUISITE) === $typeID)
			{
				Thurly\Crm\EntityRequisite::prepareEntityInfoBatch($entityTypeID, $entityInfos, $scope, $typeName);
			}
			elseif(($typeID & Integrity\DuplicateIndexType::BANK_DETAIL) === $typeID)
			{
				Thurly\Crm\EntityBankDetail::prepareEntityInfoBatch($entityTypeID, $entityInfos, $scope, $typeName);
			}
		}
	}
