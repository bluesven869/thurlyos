...
use Thurly\Crm\Settings\InvoiceSettings;
use Thurly\Crm\Settings\QuoteSettings;
use Thurly\Crm\Counter\EntityCounterFactory;
...
{
	$settingsClass = '\\Thurly\\Crm\\Settings\\' . $vewCode . 'Settings';
...
$isSliderEnabled = \Thurly\Crm\Settings\LayoutSettings::getCurrent()->isSliderEnabled();
...
if($isAdmin || CCrmLead::CheckReadPermission(0, $userPermissions))
{
	...
	$stdItems['LEAD'] = array(
		...
		'ACTIONS' => $actions,
		'IS_DISABLED' => !\Thurly\Crm\Settings\LeadSettings::isEnabled()
	);
}
...
if($isAdmin || !$userPermissions->HavePerm('INVOICE', BX_CRM_PERM_NONE, 'READ'))
{
	...
	if (IsModuleInstalled('thurlyos'))
	{
		...
		{
			$dbRes = \Thurly\Sale\PaySystem\Manager::getList(array('filter' => array('ACTION_FILE' => 'alfabankb2b')));
			while ($data = $dbRes->fetch())
			{
				$service = \Thurly\Sale\PaySystem\Manager::getObjectById($data['ID']);
