...
{
	if ($_SERVER["REQUEST_METHOD"] == "GET" && strlen($_REQUEST["delete_id"]) > 0 && check_thurly_sessid())
	{
...
if (strlen($arResult["FatalErrorMessage"]) <= 0)
{
	...
	while ($arRecord = $dbRecordsList->GetNext())
	{
		...
		$aActions = array(
			...
			array("ICONCLASS"=>"delete", "TEXT"=>GetMessage("JHGFDC_STOP"), "ONCLICK"=>"if(confirm('".GetMessage("JHGFDC_STOP_ALT")."')) window.location='".$APPLICATION->GetCurPageParam("delete_id=".$arRecord["ID"]."&".thurly_sessid_get(), array("sessid", "delete_id", "check_id", "sync_id"))."';")
		);
