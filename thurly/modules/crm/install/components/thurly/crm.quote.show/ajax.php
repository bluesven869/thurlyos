...
use Thurly\Crm\Conversion\QuoteConversionConfig;
use Thurly\Crm\Conversion\QuoteConversionWizard;

$currentUser = CCrmSecurityHelper::GetCurrentUser();
$currentUserPermissions = CCrmPerms::GetCurrentUserPermissions();
if (!$currentUser || !check_thurly_sessid() || $_SERVER['REQUEST_METHOD'] != 'POST')
{
	return;
}
...
if($mode === 'SAVE_PDF')
{
	...
	$storageTypeID = \Thurly\Crm\Integration\StorageType::getDefaultTypeID();
	if($storageTypeID !== \Thurly\Crm\Integration\StorageType::File)
	{
		$storageFileID = \Thurly\Crm\Integration\StorageManager::saveEmailAttachment($fileArray, $storageTypeID);
		$fileInfo = $storageFileID > 0 ? \Thurly\Crm\Integration\StorageManager::getFileInfo($storageFileID, $storageTypeID) : null;
		if(is_array($fileInfo))
		{
			\Thurly\Crm\Integration\StorageManager::registerInterRequestFile($storageFileID, $storageTypeID);
			if($storageTypeID === \Thurly\Crm\Integration\StorageType::WebDav)
			{
				__CrmQuoteShowEndJsonResonse(array('webdavelement' => $fileInfo));
			}
			elseif($storageTypeID === \Thurly\Crm\Integration\StorageType::Disk)
			{
...
if($mode === 'CONVERT')
{
	if(!\Thurly\Crm\Restriction\RestrictionManager::isConversionPermitted())
	{
...
if($mode === 'GET_BINGINGS')
{
	...
	__CrmQuoteShowEndJsonResonse(
		array(
			'DATA' => Thurly\Crm\Binding\BindingHelper::prepareBindingInfos(
				$ownerTypeID,
				$ownerID,
				$entityTypeID,
				$formID
...
if($mode === 'SAVE_SELECTED_BINDING')
{
	...
	if($currentUserID > 0)
	{
		\Thurly\Crm\Config\EntityConfig::set(
			$ownerTypeID,
			$ownerID,
			$currentUserID,
