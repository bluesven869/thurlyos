...
if (isset($arResult['ELEMENT']['PAY_SYSTEM_ID']))
{
	...
	{
		$data = \Thurly\Sale\PaySystem\Manager::getById($arResult['ELEMENT']['PAY_SYSTEM_ID']);
		if ($data)
	...
	'value' => isset($arResult['ELEMENT']['UF_DEAL_TITLE'])
		? (!CCrmDeal::CheckReadPermission($dealID)
			...
			'<a href="'.$arResult['PATH_TO_DEAL_SHOW'].'" id="balloon_'.$arResult['GRID_ID'].'_D_'.$dealID.'">'.htmlspecialcharsbx($arResult['ELEMENT']['UF_DEAL_TITLE']).'</a>'.
				'<script type="text/javascript">BX.tooltip("DEAL_'.$dealID.'", "balloon_'.$arResult['GRID_ID'].'_D_'.$dealID.'", "/thurly/components/thurly/crm.deal.show/card.ajax.php", "crm_balloon_deal", true);</script>'
	...
	'value' => isset($arResult['ELEMENT']['UF_QUOTE_TITLE'])
		? (!CCrmQuote::CheckReadPermission($quoteID)
			...
			'<a href="'.$arResult['PATH_TO_QUOTE_SHOW'].'" id="balloon_'.$arResult['GRID_ID'].'_Q_'.$quoteID.'">'.htmlspecialcharsbx($arResult['ELEMENT']['UF_QUOTE_TITLE']).'</a>'.
				'<script type="text/javascript">BX.tooltip("QUOTE_'.$quoteID.'", "balloon_'.$arResult['GRID_ID'].'_Q_'.$quoteID.'", "/thurly/components/thurly/crm.quote.show/card.ajax.php", "crm_balloon_quote", true);</script>'
...
if(CCrmCompany::CheckReadPermission(0, $userPermissions) || CCrmContact::CheckReadPermission(0, $userPermissions))
{
	...
	$requisteBindings = Thurly\Crm\Requisite\EntityLink::getByEntity(CCrmOwnerType::Invoice, $arParams['ELEMENT_ID']);
...
$arResult['USER_FIELD_COUNT'] = $CCrmUserType->AddFields(
	...
	array(
		'FILE_URL_TEMPLATE' =>
			"/thurly/components/thurly/crm.invoice.show/show_file.php?ownerId=#owner_id#&fieldName=#field_name#&fileId=#file_id#",
		'IS_TACTILE' => true
	)
);
