 ...
 */
class CCurrencyClassifierComponent extends \CThurlyComponent
{
	/**
	...
	private function prepareDataForDatabase($lastValues, $formMode)
	{
		...
		if ($formMode == 'ADD')
		{
			...
			foreach ($this->arResult['LANGUAGE_IDS'] as $languageId)
			{
				...
				$fields['LANG'][$upperLanguageId] = $currencyLangProps;
			}
		}
		else
		{
			foreach ($lastValues['EDIT'] as $type => $values)
			{
				...
				else
				{
					...
					$fields['LANG'][$type] = $currencyLangProps;
				}
			}
		}
	...
	private function processFormActions()
	{
		...
		if ($request->isPost() && ($request->get('save') || $request->get('apply')) && check_thurly_sessid())
		{
	 ...
	 * @param string $formMode - Form Mode
	 * @param \Thurly\Main\HttpRequest $request - Request
	 * @return array
	 */
	private function getPostCurrencyLangSettings($formMode, \Thurly\Main\HttpRequest $request)
	{
