	...
	foreach($arResult['EVENT'] as $arEvent)
	{
		...
		$arColumns = array(
			'CREATED_BY_FULL_NAME' => $arEvent['CREATED_BY_FULL_NAME'] == ''? '' :
				...
				'<script type="text/javascript">BX.tooltip('.$arEvent['USER_ID'].', "balloon_'.$arResult['GRID_ID'].'_'.$arEvent['ID'].'", "");</script>',
...
//					'<script type="text/javascript">BX.tooltip("'.$arEvent['ENTITY_TYPE'].'_'.$arEvent['ENTITY_ID'].'", "balloon_'.$arResult['GRID_ID'].'_I_'.$arEvent['ID'].'", "/thurly/components/thurly/crm.'.strtolower($arEvent['ENTITY_TYPE']).'.show/card.ajax.php", "crm_balloon'.($arEvent['ENTITY_TYPE'] == 'LEAD' || $arEvent['ENTITY_TYPE'] == 'DEAL' || $arEvent['ENTITY_TYPE'] == 'QUOTE' ? '_no_photo': '_'.strtolower($arEvent['ENTITY_TYPE'])).'", true);</script>'
//					: '';
//			}
//			else
//			{
...
//				'ONCLICK' => "javascript:(new BX.CDialog({'content_url':'/thurly/components/thurly/crm.event.add/box.php?FORM_ID=".strtoupper($arResult['FORM_ID'])."&ENTITY_TYPE=".$arResult['ENTITY_TYPE']."&ENTITY_ID=".$arResult['ORDER_ID']."', 'width':'498', 'height':'275', 'resizable':false })).Show()",
//				'ICON' => 'btn-new'
//			);
//		}
