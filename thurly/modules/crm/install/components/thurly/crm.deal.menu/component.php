...
if($arParams['TYPE'] === 'details')
{
	...
	if($arParams['IS_RECURRING'] !== 'Y')
	{
		...
		{
			$schemeID = \Thurly\Crm\Conversion\DealConversionConfig::getCurrentSchemeID();
			$arResult['BUTTONS'][] = array(
				...
				'PARAMS' => array(
					...
					'SCHEME_ID' => $schemeID,
					'SCHEME_NAME' => \Thurly\Crm\Conversion\DealConversionScheme::resolveName($schemeID),
					'SCHEME_DESCRIPTION' => \Thurly\Crm\Conversion\DealConversionScheme::getDescription($schemeID),
		...
		if(!empty($moveToCategoryIDs))
		{
			...
			$arResult['CATEGORY_CHANGER'] = array(
				...
				'SERVICE_URL' => CComponentEngine::makePathFromTemplate(
					'/thurly/components/thurly/crm.deal.details/ajax.php?site_id=#SITE#&sessid=#SID#',
...
if($arParams['TYPE'] === 'list')
{
			...
			$arResult['BUTTONS'][] = array(
				...
				'LINK' =>
					CCrmUrlUtil::AddUrlParams(
						CComponentEngine::MakePathFromTemplate(
							$arParams[\Thurly\Crm\Settings\LayoutSettings::getCurrent()->isSliderEnabled() ? 'PATH_TO_DEAL_DETAILS' : 'PATH_TO_DEAL_EDIT'],
							array('deal_id' => 0)
						),
	...
	if ($bExport)
	{
		$filterParams = Thurly\Crm\Widget\Data\DealDataSource::extractDetailsPageUrlParams($_REQUEST);
		$arResult['BUTTONS'][] = array(
...
{
	$plannerButton = \Thurly\Crm\Activity\Planner::getToolbarButton($arParams['ELEMENT_ID'], CCrmOwnerType::Deal);
	if($plannerButton)
	{
...
if (($arParams['TYPE'] == 'show') && $bRead && $arParams['ELEMENT_ID'] > 0 && $arParams['IS_RECURRING'] !== 'Y')
{
	...
	if(!$isResponsible)
	{
		...
		$arResult['SONET_SUBSCRIBE'] = array(
			'ID' => $subscriptionID,
			'SERVICE_URL' => CComponentEngine::makePathFromTemplate(
				'/thurly/components/thurly/crm.deal.edit/ajax.php?site_id=#SITE#&sessid=#SID#',
				array('SID' => thurly_sessid())
			),
