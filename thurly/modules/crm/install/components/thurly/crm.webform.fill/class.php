...
use Thurly\Crm\SiteButton\Guest;
...
class CCrmWebFormFillComponent extends \CThurlyComponent
{
	...
	protected function getAddResultParameters()
	{
		...
		if ($fromUrl)
		{
			$uri = new \Thurly\Main\Web\Uri($fromUrl);
			if ($uri->getLocator())
			{
				if ($uri->getQuery())
				{
					...
					foreach ($queryParamList as $queryParamKey => $queryParamVal)
					{
						...
						$placeholders[$queryParamKey] = \Thurly\Main\Text\Encoding::convertEncoding(
		...
		if ($presetsString)
		{
			...
			foreach ($presets as $presetKey => $presetVal)
			{
				...
				$placeholders[$presetKey] = \Thurly\Main\Text\Encoding::convertEncoding(
		...
		$utmDictionary = \Thurly\Crm\UtmTable::getCodeList();
			...
			else
			{
				...
				if($this->arResult['FORM']['IS_PAY'] == 'Y')
				{
					...
					if($resultEntity && $resultEntity->getInvoiceId())
					{
						...
						{
							$uri = new \Thurly\Main\Web\Uri($resultRedirectUrl);
	...
	protected function processPostGuest(\Thurly\Crm\WebForm\Result $result)
	{
	...
	protected function processPostCaptcha()
	{
		...
		$isSuccess = $reCaptcha->verify(
			...
			\Thurly\Main\Context::getCurrent()->getServer()->get('REMOTE_ADDR')
		);
	...
	protected function answerJson($answer = array())
	{
		...
		echo \Thurly\Main\Web\Json::encode($answer);
	...
	public function prepareResult()
	{
		...
		{
			if($field['type'] == \Thurly\Crm\WebForm\Internals\FieldTable::TYPE_ENUM_PHONE)
			{
