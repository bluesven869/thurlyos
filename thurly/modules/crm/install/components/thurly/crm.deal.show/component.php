...
$arResult['STAGE_LIST'] = Thurly\Crm\Category\DealCategory::getStageList(
...
$arFields['CONTACT_IDS'] = \Thurly\Crm\Binding\DealContactTable::getDealContactIDs($arResult['ELEMENT_ID']);
$arFields['CONTACT_COUNT'] = count($arFields['CONTACT_IDS']);

if($currentUserID > 0)
{
	$entitySettings = \Thurly\Crm\Config\EntityConfig::get(
		CCrmOwnerType::Deal,
		$arResult['ELEMENT_ID'],
		$currentUserID
	);
...
$arResult['TACTILE_FORM_ID'] = Thurly\Crm\Category\DealCategory::prepareFormID(
...
elseif (!empty($arResult['RECURRING_DATA']))
{
	...
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'type' => 'label',
		'value' => ($arResult['RECURRING_DATA']['NEXT_EXECUTION']) instanceof \Thurly\Main\Type\Date? $arResult['RECURRING_DATA']['NEXT_EXECUTION']->toString()  : '',
		'isTactile' => true
	);
	...
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'type' => 'label',
		'value' => ($arResult['RECURRING_DATA']['LAST_EXECUTION']) instanceof \Thurly\Main\Type\Date? $arResult['RECURRING_DATA']['LAST_EXECUTION']->toString()  : '',
		'isTactile' => true
	);
...
if(CCrmCompany::CheckReadPermission(0, $userPermissions) || CCrmContact::CheckReadPermission(0, $userPermissions))
{
	...
	else
	{
		...
		$primaryBinding = \Thurly\Crm\Binding\EntityBinding::findPrimaryBinding($contactBindings);
	...
	$requisteBindings = Thurly\Crm\Requisite\EntityLink::getByEntity(CCrmOwnerType::Deal, $arParams['ELEMENT_ID']);
	...
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'componentParams' => array(
			...
			'SECONDARY_ENTITY_IDS' => \Thurly\Crm\Binding\EntityBinding::prepareEntityIDs(CCrmOwnerType::Contact, $contactBindings),
...
// UTM -->
ob_start();
$APPLICATION->IncludeComponent('thurly:crm.utm.entity.view', '',
	array(
	...
	'value' => isset($arResult['ELEMENT']['QUOTE_TITLE'])
		? (!CCrmQuote::CheckReadPermission(0, $userPermissions)
			...
			'<a href="'.$arResult['PATH_TO_QUOTE_SHOW'].'" id="balloon_'.$arResult['GRID_ID'].'_Q_'.$arResult['ELEMENT']['QUOTE_ID'].'">'.$arResult['ELEMENT']['QUOTE_TITLE'].'</a>'.
				'<script type="text/javascript">BX.tooltip("QUOTE_'.$arResult['ELEMENT']['~QUOTE_ID'].'", "balloon_'.$arResult['GRID_ID'].'_Q_'.$arResult['ELEMENT']['QUOTE_ID'].'", "/thurly/components/thurly/crm.quote.show/card.ajax.php", "crm_balloon_quote", true);</script>'
		) : '',
	'type' => 'custom',
	'isTactile' => true
);
...
if ($arParams['IS_RECURRING'] !== 'Y')
{
	...
	if($arParams['ELEMENT_ID'] > 0)
	{
		...
		{
			ob_start();
			$APPLICATION->IncludeComponent('thurly:crm.entity.livefeed',
				'',
				array(
...
if (IsModuleInstalled('bizproc') && CModule::IncludeModule('bizproc') && CBPRuntime::isFeatureEnabled() && $arParams['IS_RECURRING'] !== 'Y')
{
	...
	{
		ob_start();
		$APPLICATION->IncludeComponent('thurly:bizproc.log',
			'',
			Array(
				'MODULE_ID' => 'crm',
	...
	else
	{
		...
		{
			ob_start();
			$APPLICATION->IncludeComponent('thurly:bizproc.document',
				'',
				Array(
					'MODULE_ID' => 'crm',
					'ENTITY' => 'CCrmDocumentDeal',
...
if (\Thurly\Crm\Automation\Factory::isAutomationAvailable(CCrmOwnerType::Deal) && $arParams['IS_RECURRING'] !== 'Y')
{
...
if(!isset($_REQUEST['bxajaxid']) && \Thurly\Crm\Settings\HistorySettings::getCurrent()->isViewEventEnabled())
{
