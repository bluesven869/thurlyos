...
use Thurly\Crm\Format\LeadAddressFormatter;
use Thurly\Crm\Settings\HistorySettings;
use Thurly\Crm\Context\GridContext;
...
{
	AddEventHandler('crm', 'onCrmLeadListItemBuildMenu', array('\Thurly\Crm\CallList\CallList', 'handleOnCrmLeadListItemBuildMenu'));
}

CCrmLead::PrepareConversionPermissionFlags(0, $arResult, $userPermissions);
if($arResult['CAN_CONVERT'])
{
	$config = \Thurly\Crm\Conversion\LeadConversionManager::create(array())->getConfig();
...
if (!$bInternal)
{
	...
	$arResult['FILTER'] = array(
		...
		array(
			...
			'name' => GetMessage('CRM_COLUMN_IS_RETURN_CUSTOMER'),
			'params' => array('multiple' => 'N'),
			'default' => true,
			'type' => 'list',
			'items' => $arResult['BOOLEAN_VALUES_LIST']
		),*/
	...
	$arResult['FILTER'][] = Thurly\Crm\PhaseSemantics::getListFilterInfo(
		CCrmOwnerType::Lead,
		array(
	...
	$arResult['FILTER_PRESETS'] = array(
		'filter_my_in_work' => array(
			...
			'fields' => array_merge(
				...
				array(
					...
					'STATUS_SEMANTIC_ID' => array(Thurly\Crm\PhaseSemantics::PROCESS)
				)
			)
		),
		'filter_in_work' => array(
			...
			'fields' => array_merge(
				$defaultFilter,
				array('STATUS_SEMANTIC_ID' => array(Thurly\Crm\PhaseSemantics::PROCESS))
			)
		),
		'filter_closed' => array(
			...
			'fields' => array_merge(
				$defaultFilter,
				array('STATUS_SEMANTIC_ID' => array(Thurly\Crm\PhaseSemantics::SUCCESS))
			)
		)
	);
}
...
$arResult['HEADERS'] = array_merge(
	...
	array(
		...
		array('id' => 'HONORIFIC', 'name' => GetMessage('CRM_COLUMN_HONORIFIC'), 'sort' => false, 'type' => 'list', 'editable' => array('items' => CCrmStatus::GetStatusList('HONORIFIC'))),
...
$arResult['HEADERS'] = array_merge($arResult['HEADERS'], array(
	...
	/*
	array('id' => 'IS_RETURN_CUSTOMER', 'name' => GetMessage('CRM_COLUMN_IS_RETURN_CUSTOMER'), 'sort' => 'is_return_customer', 'default' => false, 'type' => 'list')
	*/
));
...
Thurly\Crm\Search\SearchEnvironment::convertEntityFilterValues(CCrmOwnerType::Company, $arFilter);
...
if ($isInExportMode)
{
	...
	$productRowsEnabled = \Thurly\Crm\Settings\LeadSettings::getCurrent()->isProductRowExportEnabled();
...
else
{
	...
	if(!empty($addressSort))
	{
		...
		$navDbResult = \Thurly\Crm\LeadAddress::getEntityList(
...
{
	$waitingInfos = \Thurly\Crm\Pseudoactivity\WaitEntry::getRecentInfos(CCrmOwnerType::Lead, $aclivitylessItems);
...
$CCrmUserType->ListAddEnumFieldsValue(
	...
	array(
		'FILE_URL_TEMPLATE' =>
			'/thurly/components/thurly/crm.lead.show/show_file.php?ownerId=#owner_id#&fieldName=#field_name#&fileId=#file_id#'
	)
);
...
if ($sExportType == '')
{
	$schemeID = \Thurly\Crm\Conversion\LeadConversionManager::create(array())->getCurrentSchemeID();
	$arResult['CONVERSION_SCHEME'] = array(
		'ORIGIN_URL' => $currentPage,
		'SCHEME_ID' => $schemeID,
		'SCHEME_NAME' => \Thurly\Crm\Conversion\LeadConversionScheme::resolveName($schemeID),
		'SCHEME_DESCRIPTION' => \Thurly\Crm\Conversion\LeadConversionScheme::getDescription($schemeID),
	...
	if(!$bInternal)
	{
		...
		$arResult['NEED_FOR_BUILD_TIMELINE'] = \Thurly\Crm\Agent\Timeline\LeadTimelineBuildAgent::getInstance()->isEnabled();
		$arResult['NEED_FOR_REFRESH_ACCOUNTING'] = \Thurly\Crm\Agent\Accounting\LeadAccountSyncAgent::getInstance()->isEnabled();
	...
	include_once($_SERVER['DOCUMENT_ROOT'].'/thurly/components/thurly/crm.lead/include/nav.php');
