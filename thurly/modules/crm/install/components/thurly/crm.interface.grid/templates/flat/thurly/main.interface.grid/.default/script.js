	...
	this.InitTable = function()
	{
		...
		for(i=0; i<nCols; i++)
		{
			...
			for(j=0; j<2; j++)
			{
				...
				if(j==1)
				{
					if(cell.className && (cell.className == 'bx-crm-actions-col' || cell.className == 'bx-crm-checkbox-col'))
						continue;
		...
		for(i=0; i<n; i++)
		{
			...
			if(row.className && row.className == 'bx-crm-grid-footer')
				continue;

			/*first and last columns style classes*/
			row.cells[0].className += ' bx-crm-left';
			row.cells[row.cells.length-1].className += ' bx-crm-right';
...
//					row.className += ' bx-even';
		...
		if(tbl.rows.length > 2)
		{
			...
			if(r.className && r.className == 'bx-crm-grid-footer')
				r = tbl.rows[tbl.rows.length-2];
			r.className += ' bx-crm-bottom';
		}
	};
	...
	this.HighlightGutter = function(cell, on)
	{
		...
		var bSorted = (gutter.className.indexOf('bx-crm-sorted') != -1);
		if(on)
		{
			if(bSorted)
			{
	...
	this.OnClickRow = function(e)
	{
		...
		else
		{
			var tr = BX.findParent(obj, {
				tagName: 'tr',
				className: 'bx-crm-table-body'
			}, this.tableNode);
			if(tr)
			{
				var td = BX.findChild(tr, {
					tagName: 'td',
					className: 'bx-crm-checkbox-col'
				});
				if(td)
				{
		...
		if(this.IsActionEnabled('edit'))
		{
			...
			for(var i=0; i<ids.length; i++)
			{
				...
				if(el.checked)
				{
					...
					if(td.className == 'bx-crm-actions-col')
						td = jsUtils.FindNextSibling(td, "td");
						...
						if(this.oColsMeta[col_id].editable == true && this.oEditData[row_id][col_id] !== false)
						{
							...
							switch(this.oColsMeta[col_id].type)
							{
								...
								case 'list':
									var options = [];
									var list_val, li;
									for (li=0; li<this.oColsMeta[col_id].items.length; li++)
									{
										list_val = this.oColsMeta[col_id].items[li].val;
										options[options.length] = BX.create('OPTION', {
												'props': {'value':list_val, 'selected':(list_val == data)},
												'text': this.oColsMeta[col_id].items[li].ttl}
										);
									}
								...
								default:
									var props = {
										className: 'bx-crm-folder-title list',
										'type':'text',
										'name':name,
										'value':data,
	...
	this.SaveColumns = function(columns)
	{
		...
		BX.ajax.get(
			'/thurly/components'+_this.vars.component_path+'/settings.php?GRID_ID='+_this.table_id+'&action=showcolumns&columns='+sCols+'&sessid='+_this.vars.sessid,
			BX.delegate(_this._onColumnsSaved, _this)
		);
	};

	this._onColumnsSaved = function()
	{
	...
	this.Reload = function(url)
	{
		...
		{
			BX.ajax.get(
				url + (url.indexOf('?') == -1? '?':'&') + 'bxajaxid=' + this.vars.ajax.AJAX_ID + '&sessid=' + BX.thurly_sessid(),
				BX.delegate(this.onAjaxReload, this)
			);
		}
	};

	this.onAjaxReload = function(data)
	{
		BX('comp_' + this.vars.ajax.AJAX_ID).innerHTML = data;
		this.InitTable();
		BX.onCustomEvent(window, 'BXInterfaceGridAfterReload', []);
	};

	this.SetTheme = function(menuItem, theme)
	{
		BX.loadCSS(this.vars.template_path+'/themes/'+theme+'/style.css');
		BX(_this.table_id).className = 'bx-crm-interface-grid bx-crm-interface-grid-theme-'+theme;

		var themeMenu = this.menu.GetMenuByItemId(menuItem.id);
		themeMenu.SetAllItemsIcon('');
		themeMenu.SetItemIcon(menuItem, 'checked');

		BX.ajax.get('/thurly/components'+_this.vars.component_path+'/settings.php?GRID_ID='+_this.table_id+'&action=settheme&theme='+theme+'&sessid='+_this.vars.sessid);
	};
	...
	this.SetView = function(view_id)
	{
		...
		BX.ajax.get('/thurly/components'+_this.vars.component_path+'/settings.php?GRID_ID='+_this.table_id+'&action=setview&view_id='+view_id+'&sessid='+_this.vars.sessid, func);
	};
	...
	this.EditCurrentView = function()
	{
		...
		this.ShowSettings(this.oOptions.views[this.oOptions.current_view],
			function()
			{
				var data = self.SaveSettings(self.oOptions.current_view, true);

				self.oOptions.views[self.oOptions.current_view] = {
					'name':data.name,
	...
	this.DeleteView = function(view_id)
	{
		...
		BX.ajax.get('/thurly/components'+this.vars.component_path+'/settings.php?GRID_ID='+this.table_id+'&action=delview&view_id='+view_id+'&sessid='+_this.vars.sessid);
	};
	...
	this.SaveSettings = function(view_id, doReload)
	{
		...
		BX.ajax.post('/thurly/components'+_this.vars.component_path+'/settings.php', data, handler);
	...
	this.DragStart = function()
	{
		...
		var innerDiv = document.body.appendChild(document.createElement("DIV"));
		...
		if (innerDiv.clientHeight < div.clientHeight)
		{
			var marginTop = Math.floor((div.clientHeight - innerDiv.clientHeight) / 2);
			if (marginTop > 0)
	...
	{
		if (obDrag === null || typeof(obDrag) !== "object" || !obDrag.hasOwnProperty("__dragArrowDiv"))
			return false;
	...
	this.DragFinish = function(obDrag, x, y, e)
	{
		...
		for(var i=0; i < 2; i++)
		{
			...
			if(cell.className && (cell.className.indexOf('bx-crm-actions-col') != -1 || cell.className.indexOf('bx-crm-checkbox-col') != -1))
				delta ++;
		}
	...
	this.SaveFilterRows = function()
	{
		...
		BX.ajax.get('/thurly/components'+this.vars.component_path+'/settings.php?GRID_ID='+this.table_id+'&action=filterrows&rows='+sRows+'&sessid='+this.vars.sessid);
	};
	...
	this.SwitchFilter = function(a)
	{
		...
		BX.ajax.get('/thurly/components'+this.vars.component_path+'/settings.php?GRID_ID='+this.table_id+'&action=filterswitch&show='+(on? 'N':'Y')+'&sessid='+this.vars.sessid);
	};
	...
	this.ClearFilter = function(form)
	{
		...
		{
			form = document.forms['filter_'+this.table_id];
		}
	...
	this.DeleteFilter = function(filter_id)
	{
		...
		BX.ajax.get('/thurly/components'+this.vars.component_path+'/settings.php?GRID_ID='+this.table_id+'&action=delfilter&filter_id='+filter_id+'&sessid='+_this.vars.sessid);
	};
	...
	this.SetFilterFields = function(form, fields)
	{
		...
		for(var i=0, n = form.elements.length; i<n; i++)
		{
			...
			var name = el.name;
			var val = fields[name] ? fields[name] : '';
			...
			switch(el.type.toLowerCase())
			{
				...
				case 'select-multiple':
				{
					...
					val = typeof(fields[name]) === 'object' && fields[name] ? fields[name] : {};
					var selected = false;
					for(var j = 0, l = el.options.length; j < l; j++)
					{
						var option = el.options[j];
						var k = 'sel' + option.value;
						var sel = val[k] ? val[k] : null;
	...
	this.SaveFilter = function(filter_id)
	{
		...
		BX.ajax.post('/thurly/components'+_this.vars.component_path+'/settings.php', data);
...
BX.namespace("BX.Crm");
if (typeof(BX.Crm.GridPageSizeControl) === "undefined")
{
	BX.Crm.GridPageSizeControl = (function(){
		var GridPageSizeControl = function (settings)
		{
			...
			if (this.nodeId)
			{
				...
				if (this.node)
				{
					BX.bind(this.node, "change", BX.proxy(this.handlePageSizeControlChange, this));
				}
			}
		};

		GridPageSizeControl.prototype = {
			handlePageSizeControlChange: function (e)
			{
				e = e || window.event;
				...
				if (this.grid)
				{
					this.grid.oOptions.views[this.grid.oOptions.current_view].page_size = parseInt(node.value);
					this.saveGridSettings(this.grid.oOptions.current_view, true);
				}
			},
			saveGridSettings: function(view_id, doReload)
			{
				var data = {
					'GRID_ID': this.grid.table_id,
					'view_id': view_id,
					'action': 'savesettings',
					'sessid': this.grid.vars.sessid,
					'name': this.grid.oOptions.views[view_id].name,
					'columns': this.grid.oOptions.views[view_id].columns,
					'sort_by': this.grid.oOptions.views[view_id].sort_by,
					'sort_order': this.grid.oOptions.views[view_id].sort_order,
					'page_size': this.grid.oOptions.views[view_id].page_size,
					'saved_filter': this.grid.oOptions.views[view_id].saved_filter
				};
				...
				if(doReload === true)
				{
					handler = function()
					{
						if(data.saved_filter && self.grid.oOptions.filters[data.saved_filter])
						{
							self.grid.ApplyFilter(data.saved_filter);
						}
						else
						{
							self.grid.Reload();
				...
				BX.ajax.post('/thurly/components'+self.grid.vars.component_path+'/settings.php', data, handler);

				return data;
			}
		};

		return GridPageSizeControl;
	})();
}