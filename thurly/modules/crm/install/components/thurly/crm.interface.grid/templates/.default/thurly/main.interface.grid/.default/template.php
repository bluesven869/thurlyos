...
$APPLICATION->AddHeadScript('/thurly/js/crm/common.js');
...
if(!empty($arParams["FILTER"])):
	...
	array(
		...
		"NAVIGATION_BAR" => $arParams["~FILTER_NAVIGATION_BAR"],
...
<?=thurly_sessid_post();?>
<?endif?>
<div class="bx-crm-interface-grid">
...
if($header["sort"] <> ''):
	...
	$headerClassName = 'bx-sortable';
	if($header["sort_state"])
		$headerClassName .= ' bx-sorted';
	if(isset($header['class']))
		$headerClassName .= ' bx-'.$header['class'];
?>
...
foreach($arParams["ROWS"] as $index=>$aRow):
	...
	$emphasized = isset($aRow['emphasized']) && $aRow['emphasized'] === true;
?>
	<tr<?= $emphasized ? ' class="crm-interface-grid-row-emphasized"' : ''?> oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.oActions[<?=$index?>]"<?if($sDefAction <> ''):?> ondblclick="<?=$sDefAction?>"<?endif?> >
<?if($arResult["ALLOW_EDIT"]):?>
	<?
...
<?
	...
	foreach($arResult["HEADERS"] as $id=>$header):
	$columnClass = 	$header["sort_state"] !== '' ? 'bx-sorted' : '';
	...
	<tr class="bx-grid-footer">
		<td colspan="<?=$colspan?>">
			...
			<?foreach($arParams["FOOTER"] as $footer):?>
					...
					<?elseif(isset($footer["custom_html"])):?>
						<?=$footer["custom_html"]?>
					<?else:?>
		...
		<td colspan="2" align="center">
			<table>
				<tr>
					...
					<td style="background-image:none">
						<div style="margin-bottom:5px"><input type="button" name="up_btn" value="<?echo GetMessage("interface_grid_view_up")?>" title="<?echo GetMessage("interface_grid_view_up_title")?>" class="bx-grid-btn" style="width:100px;" disabled onclick="jsSelectUtils.moveOptionsUp(this.form.view_cols)"></div>
						<div style="margin-bottom:5px"><input type="button" name="down_btn" value="<?echo GetMessage("interface_grid_view_down")?>" title="<?echo GetMessage("interface_grid_view_down_title")?>" class="bx-grid-btn" style="width:100px;" disabled onclick="jsSelectUtils.moveOptionsDown(this.form.view_cols)"></div>
						<div style="margin-bottom:5px"><input type="button" name="rename_btn" value="<?echo GetMessage("interface_grid_name_btn")?>" title="<?echo GetMessage("interface_grid_name_btn_title")?>" class="bx-grid-btn" style="width:100px;" disabled onclick="bxGrid_<?=$arParams["GRID_ID"]?>.RenameColumn()"></div>
					</td>
				</tr>
			</table>
		</td>
...
<?
$ajaxLoaderParams = isset($arParams['AJAX_LOADER']) ? $arParams['AJAX_LOADER'] : null;
if(is_array($ajaxLoaderParams))
{
	if(!(isset($ajaxLoaderParams['url']) && $ajaxLoaderParams['url'] !== ''))
	{
	...
	?><script type="text/javascript">
		BX.ready(
			function()
			{
				var id = "<?=CUtil::JSEscape($arParams["AJAX_ID"])?>";
				BX.GridAjaxLoader.remove(id);
				var loader = BX.GridAjaxLoader.create(id, <?=CUtil::PhpToJSObject($ajaxLoaderParams)?>);
				bxGrid_<?=$arParams["GRID_ID"]?>.setLoader(loader);
			}
		);
	</script>
	<?
