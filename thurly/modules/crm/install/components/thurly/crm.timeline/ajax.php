...
if (!$currentUser || !$currentUser->IsAuthorized() || !check_thurly_sessid() || $_SERVER['REQUEST_METHOD'] != 'POST')
{
	return;
}
...
CThurlyComponent::includeComponentClass("thurly:crm.timeline");
...
if($action == 'SAVE_COMMENT')
{
	...
	$entryID = \Thurly\Crm\Timeline\CommentEntry::create(
		array(
			'TEXT' => $text,
			'SETTINGS' => $settings,
			'AUTHOR_ID' => $authorID,
	...
	$item = Thurly\Crm\Timeline\CommentController::getInstance()->onCreate($entryID, $saveData);

	__CrmTimelineEndResonse(array('HISTORY_ITEM' => $item));
}
elseif($action == 'SAVE_WAIT')
{
	...
	$arActivity = null;
	if($ID > 0 && !\Thurly\Crm\Pseudoactivity\WaitEntry::exists($ID))
	{
	...
	if(!\Thurly\Crm\Security\EntityAuthorization::checkUpdatePermission($ownerTypeID, $ownerID))
	{
	...
	if($typeId === 2 && $effectiveFieldName !== '')
	{
		...
		if(is_array($fields))
		{
			...
			if($targetDate !== '')
			{
				...
				$currentDate = new \Thurly\Main\Type\Date();
				$endDate = \Thurly\Main\Type\Date::createFromTimestamp($endTime);
				$end = \Thurly\Main\Type\Date::createFromTimestamp($endTime);
	...
	if($end === null)
	{
		$end = new \Thurly\Main\Type\DateTime();
		$end->add("{$duration}D");
	}
	...
	if($ID <= 0)
	{
		$result = \Thurly\Crm\Pseudoactivity\WaitEntry::add($arFields);
		if($result->isSuccess())
		{
...
elseif($action == 'COMPLETE_WAIT')
{
	...
	{
		$fields = \Thurly\Crm\Pseudoactivity\WaitEntry::getByID($ID);
		if(!is_array($fields))
		{
	...
	$result = \Thurly\Crm\Pseudoactivity\WaitEntry::complete($ID, $completed);
	if($result->isSuccess())
	{
...
elseif($action == 'POSTPONE_WAIT')
{
	...
	$result = \Thurly\Crm\Pseudoactivity\WaitEntry::postpone($ID, $offset);
	if($result->isSuccess())
	{
...
elseif($action == 'GET_HISTORY_ITEMS')
{
	...
	$lastItemTime = \Thurly\Main\Type\DateTime::tryParse(
		isset($params['LAST_ITEM_TIME']) ? $params['LAST_ITEM_TIME'] : '',
		'Y-m-d H:i:s'
	);

	if(!\Thurly\Crm\Security\EntityAuthorization::checkReadPermission($ownerTypeID, $ownerID))
	{
...
elseif($action == 'SAVE_SMS_MESSAGE')
{
	...
	$result = \Thurly\Crm\Integration\SmsManager::sendMessage(array(
		'SENDER_ID' => $senderId,
		'AUTHOR_ID' => $responsibleID,
	...
	if ($result->isSuccess())
	{
		$activityID = \Thurly\Crm\Activity\Provider\Sms::addActivity(array(
			'AUTHOR_ID' => $responsibleID,
...
elseif($action == 'CHANGE_FASTEN_ITEM')
{
	...
	$checkData = Thurly\Crm\Timeline\Entity\TimelineTable::getList(
		array(
			'select' => array('ID'),
			'filter' => array(
				'=ID' => $id,
			...
			'runtime' => array(
				new \Thurly\Main\Entity\ReferenceField(
					'BINDING',
					'\Thurly\Crm\Timeline\Entity\TimelineBindingTable',
	...
	if (!$checkData->fetch())
	{
		__CrmTimelineEndResonse(array('ERROR' => 'Entity is not found'));
	}

	$resultUpdating = Thurly\Crm\Timeline\Entity\TimelineBindingTable::update(
		array('OWNER_ID' => $id, 'ENTITY_ID' => $ownerID , 'ENTITY_TYPE_ID' => $ownerTypeID),
		array('IS_FIXED' => $value));

	if ($resultUpdating->isSuccess())
	{
		$items = array($id => \Thurly\Crm\Timeline\TimelineEntry::getByID($id));
		\Thurly\Crm\Timeline\TimelineManager::prepareDisplayData($items);
		if(\Thurly\Main\Loader::includeModule('pull') && CPullOptions::GetQueueServerStatus())
		{
			$tag = \Thurly\Crm\Timeline\TimelineEntry::prepareEntityPushTag($ownerTypeID, $ownerID);
		...
		__CrmTimelineEndResonse(array('ID' => $id));
	}
	else
	{
		__CrmTimelineEndResonse(array('ERROR' => 'Could not fasten item.'));
	}
...
elseif($action == 'UPDATE_COMMENT')
{
	...
	$resultCheck = Thurly\Crm\Timeline\Entity\TimelineTable::getList(
		array(
			'filter' => array('=ID' => $id, '=TYPE_ID' => Thurly\Crm\Timeline\TimelineType::COMMENT),
		)
	);
	$oldMentions = array();
	if ($commentData = $resultCheck->fetch())
	{
		$oldMentions = \Thurly\Crm\Timeline\CommentController::getMentionIds($commentData['COMMENT']);
	}
	else
	{
		__CrmTimelineEndResonse(array('ERROR' => 'Comment is not found.'));
	}

	$resultBind = Thurly\Crm\Timeline\Entity\TimelineBindingTable::getList(
		array(
	...
	if (!$isExistBinding)
		__CrmTimelineEndResonse(array('ERROR' => 'Could not update comment.'));
	...
	if (count($bindings) > 1)
	{
		$newId = \Thurly\Crm\Timeline\CommentEntry::create(array(
		...
		$bindingDelete = \Thurly\Crm\Timeline\Entity\TimelineBindingTable::delete(array(
			'OWNER_ID' => $id,
			'ENTITY_ID' => $ownerID,
			'ENTITY_TYPE_ID' => $ownerTypeID,
		));
	...
	$resultUpdating = \Thurly\Crm\Timeline\CommentEntry::update($id, array('COMMENT' => $text, 'SETTINGS' => $settings));
	...
	if ($resultUpdating->isSuccess())
	{
		...
		$item = Thurly\Crm\Timeline\CommentController::getInstance()->onModify($id, $saveData);
		__CrmTimelineEndResonse(array('HISTORY_ITEM' => $item));
	}
	else
	{
...
elseif($action == 'DELETE_COMMENT')
{
	...
	if (!$isExistBinding)
		__CrmTimelineEndResonse(array('ERROR' => 'Could not delete comment.'));
	...
	if (count($bindings) > 1)
	{
		\Thurly\Crm\Timeline\Entity\TimelineBindingTable::delete(array(
			'OWNER_ID' => $id,
			'ENTITY_ID' => $ownerID,
			'ENTITY_TYPE_ID' => $ownerTypeID,
		));
	...
	{
		Thurly\Crm\Timeline\CommentEntry::delete($id);
	}

	Thurly\Crm\Timeline\CommentController::getInstance()->onDelete($id, array(
		'ENTITY_TYPE_ID' => $ownerTypeID,
		'ENTITY_ID' => $ownerID,
		'DELETE_FILE_BINDING' => $filesDelete
	));
...
elseif($action == 'GET_COMMENT_CONTENT')
{
	...
	{
		$html = \Thurly\Crm\Timeline\CommentController::getFileBlock($id);
	}
	else
	{
		$commentData = \Thurly\Crm\Timeline\TimelineEntry::getByID($id);
		$data = \Thurly\Crm\Timeline\CommentController::convertToHtml($commentData, array("INCLUDE_FILES" => 'Y'));
		$html = $data['COMMENT'];
	}

	if (empty($html))
		__CrmTimelineEndResonse(array('ERROR' => "Content is empty"));

	__CrmTimelineEndResonse(array('BLOCK' => $html));
}