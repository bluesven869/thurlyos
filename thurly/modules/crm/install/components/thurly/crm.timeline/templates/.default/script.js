if(typeof(BX.CrmTimelineManager) === "undefined")
{
	...
	BX.CrmTimelineManager.prototype =
	{
		initialize: function(id, settings)
		{
			...
			{
				throw "BX.CrmTimeline. A required parameter 'activityEditorId' is missing.";
			}
			...
			this._fixedHistory = BX.CrmFixedHistory.create(
				this._id,
				{
					...
					commentEditorUrl: this.getSetting("commentEditorUrl"),
					currentUrl: currentUrl,
					readOnly: this._readOnly
				}
			);
		...
		onPullEvent: function(command, params)
		{
			...
			if(command === "timeline_activity_add")
			{
		...
		loadMediaPlayer: function(id, filePath, mediaType, node)
		{
			var playerUrl = this.getSetting("playerUrl", "");
			if(playerUrl === "")
			{
				return;
			}
			...
			var url = BX.util.add_url_param(
				playerUrl,
				{ id: BX.util.urlencode(id), path: BX.util.urlencode(filePath), type: BX.util.urlencode(mediaType) }
			);

			//BX.ajax.Setup({ denyShowWait: true }, true);
			//BX.ajax.insertToNode(url, node);

			BX.ajax.get(url, function(data) { node.innerHTML = data; });
		},
...
if(typeof(BX.CrmTimeline) === "undefined")
{
	...
	BX.CrmTimeline.prototype =
	{
		initialize: function(id, settings)
		{
			...
			this._isStubMode = this.getSetting("isStubMode", false);
			this._readOnly = BX.prop.getBoolean(this._settings, "readOnly", false);
			this.doInitialize();
		},
		getId: function()
...
if(typeof(BX.CrmHistory) === "undefined")
{
	...
	BX.CrmHistory.prototype.doInitialize = function()
	{
		...
		this._timeFormat = BX.date.convertThurlyFormat(timeFormat);
	...
	BX.CrmHistory.prototype.layout = function()
	{
		...
		if(!this.isStubMode())
		{
			...
			if(length)
			{
				this._lastLoadedItemTimestamp = this._items[length - 1].getCreatedTimestamp();
				this._loadingWaiter = this._items[length - 1].getWrapper();
	...
	BX.CrmHistory.prototype.formatTime = function(time)
	{
		return BX.date.format(this._timeFormat, time);
	};
	...
	BX.CrmHistory.prototype.formatDate = function(date)
	{
		return (
			BX.date.format(
				[
	...
	BX.CrmHistory.prototype.onItemsLoad = function(sender, result)
	{
		var now = BX.prop.extractDate(new Date());
		var itemData = BX.prop.getArray(result, "HISTORY_ITEMS");
		var i, length, item;
		var timestamp = "";
		var proceccedCount = 0;
		for(i = 0, length = itemData.length; i < length; i++)
		{
			...
			timestamp = BX.prop.getString(itemData[i], "CREATED_SERVER", "");
		...
		if(proceccedCount > 0)
		{
			if(timestamp !== "")
			{
				this._lastLoadedItemTimestamp = timestamp;
			}
...
if(typeof(BX.CrmFixedHistory) === "undefined")
{
	...
	BX.CrmFixedHistory.prototype.setItems = function()
	{
		var itemData = this.getSetting("itemData");
		if(!BX.type.isArray(itemData))
		{
...
if(typeof BX.CrmSchedule === "undefined")
{
	...
	{
		var length = this._items.length;
		var historyItemCount = this._history ? this._history.getItemCount() : 0;
		if(length === 0)
		{
			this.addStub();

			if(historyItemCount > 0 || this._history.isStubMode())
			{
...
if(typeof BX.CrmTimelineBaseEditor === "undefined")
{
	...
	BX.CrmTimelineBaseEditor.prototype =
	{
		...
		resizeForm: function()
		{
			...
			ghost.innerHTML = this._input.value.replace(/[\r\n]{1}/g, '<br>');
...
if(typeof BX.CrmTimelineCommentEditor === "undefined")
{
	...
	{
		this._serviceUrl = this.getSetting("serviceUrl", "");
		this._commentEditorUrl = this.getSetting("commentEditorUrl");
	...
	BX.CrmTimelineCommentEditor.prototype.loadEditor = function()
	{
		...
		if (!this._commentEditorUrl || this._postForm)
			return;

		var url = BX.util.add_url_param(
			this._commentEditorUrl,
			{
				name: this._editorName
			}
		);

		BX.ajax.get(url, BX.delegate(this.onLoadEditorSuccess, this));
	};
	BX.CrmTimelineCommentEditor.prototype.onLoadEditorSuccess = function(result)
	{
		var promise = BX.html(this._editorContainer, result);
...
if(typeof(BX.CrmTimelineEmailAction) === "undefined")
{
	...
	BX.CrmTimelineEmailAction.prototype.onClick = function(e)
	{
		...
		if (BX.CrmActivityProvider && top.BX.ThurlyOS && top.BX.ThurlyOS.Slider)
		{
...
if(typeof(BX.CrmTimelineItem) === "undefined")
{
	...
	BX.CrmTimelineItem.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(!(this._activityEditor instanceof BX.CrmActivityEditor))
			{
				throw "BX.CrmTimelineItem. A required parameter 'activityEditor' is missing.";
			}
		...
		prepareCutOffElements: function(text, length, clickHandler)
		{
			...
			return(
				[
					text.substring(0, offset) + "&hellip;&nbsp;" ,
					BX.create("A",
						{
...
if(typeof(BX.CrmHistoryItemComment) === "undefined")
{
	...
	BX.CrmHistoryItemComment.prototype.prepareContent = function()
	{
		...
		if (this._mode !== BX.Crm.TimelineEditorMode.edit)
			BX.bind(wrapper, "dblclick", BX.delegate(this.switchToEditMode, this));
	...
	BX.CrmHistoryItemComment.prototype.onLoadEditorSuccess = function(result)
	{
		...
		this._editorContainer.innerHTML = result;
...
if(typeof(BX.CrmHistoryItemEmail) === "undefined")
{
	...
	{
		return this.getMessage('emailRemove').replace("#TITLE#", this.getTitle());
	};
...
if(typeof(BX.CrmHistoryItemCall) === "undefined")
{
	...
	BX.CrmHistoryItemCall.prototype.prepareContent = function()
	{
		...
		if(hasCallInfo)
		{
			...
			if(this._mediaFileInfo !== null)
			{
				...
				// var player = new BX.Fileman.Player('history_' + this.getId(), {
				// 	autostart: false,
				// 	width: 350,
				// 	height: 30,
				// 	sources: [
				// 		{
				// 			src: this._mediaFileInfo["URL"],
				// 			type: this._mediaFileInfo["TYPE"]
				// 		}
				// 	]
				// });
				// player.isAudio = true;
				// var node = player.createElement();
				// this._playerWrapper.appendChild(node);
			...
			if(callComment)
			{
				detailWrapper.appendChild(BX.create("DIV",
					{
						attrs: { className: "crm-entity-stream-content-detail-description"},
						text: BX.util.htmlspecialchars(callComment)
					}
				));
			}
		}
	...
	BX.CrmHistoryItemCall.prototype.getRemoveMessage = function()
	{
		...
		return this.getMessage(messageName).replace("#TITLE#", this.getTitle());
	};
...
if(typeof(BX.CrmHistoryItemMeeting) === "undefined")
{
	...
	{
		return this.getMessage('meetingRemove').replace("#TITLE#", this.getTitle());
	};
...
if(typeof(BX.CrmHistoryItemTask) === "undefined")
{
	...
	{
		return this.getMessage('taskRemove').replace("#TITLE#", this.getTitle());
	};
...
if(typeof(BX.CrmHistoryItemSender) === "undefined")
{
	...
	{
		var entityData = this.getAssociatedEntityData();
		var description = this.getMessage('title') + ': ' + (!this.isRemoved() ? this.getDataSetting('letterTitle') : this.getMessage('removed'));
...
if(typeof(BX.CrmHistoryItemActivityRestApplication) === "undefined")
{
	...
	BX.CrmHistoryItemActivityRestApplication.prototype.prepareContent = function()
	{
		...
		wrapper.appendChild(
			BX.create("DIV", { attrs: { className: "crm-entity-stream-section-icon crm-entity-stream-section-icon-rest" } })
		);
...
if(typeof(BX.CrmScheduleItemActivity) === "undefined")
{
	...
	BX.CrmScheduleItemActivity.prototype.prepareContent = function(options)
	{
		...
		var descriptionHtml = BX.prop.getString(entityData, "DESCRIPTION_RAW", "");
		...
		{
			//trim leading spaces
			descriptionHtml = descriptionHtml.replace(/^\s+/,'');
		}
		...
		detailWrapper.appendChild(
			BX.create("DIV",
				{
					attrs: { className: "crm-entity-stream-content-detail-description" },
					html: this.cutOffText(descriptionHtml, 128)
				}
			)
		);
...
if(typeof(BX.CrmTimelineMenuBar) === "undefined")
{
	...
	BX.CrmTimelineMenuBar.prototype =
	{
		initialize: function(id, settings)
		{
			...
			this._activityEditor = BX.prop.get(this._settings, "activityEditor");
			if(!(this._activityEditor instanceof BX.CrmActivityEditor))
			{
				throw "BX.CrmTimelineMenuBar. A required parameter 'activityEditor' is missing.";
			}
