...
Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/crm/activity.js');
Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/crm/message.js');

\Thurly\Main\UI\Extension::load("ui.buttons");

//HACK: Preloading files for prevent trembling of player afer load.
Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/js/crm/timeline_player/timeline_player.css');
Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/components/thurly/player/js/fileman_player.js');
Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/components/thurly/player/videojs/video.js');
Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/components/thurly/player/videojs/video-js.css');

//HACK: Preloading files for comment editor.
Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/components/thurly/main.post.form/templates/.default/script.js');
Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/components/thurly/main.post.form/templates/.default/style.css');
if(\Thurly\Main\Loader::includeModule('disk'))
{
	Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/components/thurly/disk.uf.file/templates/.default/script.js');
}
if(\Thurly\Main\Loader::includeModule('blog'))
{
	Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/components/thurly/socialnetwork.blog.blog/templates/.default/style.css');
}

$jsLibraries = array('crm_visit_tracker', 'viewer');
...
{
	if (\Thurly\Main\Loader::includeModule('fileman') && !CJSCore::IsExtRegistered('html_editor'))
	{
		$baseHtmlEditorPath = '/thurly/js/fileman/html_editor/';
		CJSCore::RegisterExt('html_editor', array(
			'js' => array(
				$baseHtmlEditorPath.'range.js',
				$baseHtmlEditorPath.'html-actions.js',
				$baseHtmlEditorPath.'html-views.js',
				$baseHtmlEditorPath.'html-parser.js',
				$baseHtmlEditorPath.'html-base-controls.js',
				$baseHtmlEditorPath.'html-controls.js',
				$baseHtmlEditorPath.'html-components.js',
				$baseHtmlEditorPath.'html-snippets.js',
				$baseHtmlEditorPath.'html-editor.js',
				'/thurly/js/main/dd.js'
			),
			'css' => $baseHtmlEditorPath.'html-editor.css',
			'rel' => array('date', 'timer')
		));
		$jsLibraries[] = 'html_editor';
	}

	$spotlight = new \Thurly\Main\UI\Spotlight("CRM_TIMELINE_FASTEN_SWITCHER");
				...
				<div class="crm-entity-stream-section-content crm-entity-stream-section-content-new">
					<div id="<?=htmlspecialcharsbx($menuBarContainerID)?>" class="crm-entity-stream-section-content-new-header">
						...
						<?if(count($arResult['ADDITIONAL_TABS']) > 0)
						{
							...
							{?>
								<a data-item-id="<?=$tab['id']?>" data-item-title="<?=\Thurly\Main\Text\HtmlFilter::encode($tab['name'])?>" class="crm-entity-stream-section-new-action" href="#">
									<?=\Thurly\Main\Text\HtmlFilter::encode($tab['name'])?>
								</a>
							<?}
						}?>
					...
					<div id="<?=htmlspecialcharsbx($smsContainerID)?>" class="crm-entity-stream-content-new-detail focus" style="display: none;">
						...
						<div class="crm-entity-stream-content-new-sms-btn-container">
							<button id="<?=htmlspecialcharsbx($smsButtonID)?>" class="crm-entity-stream-content-new-sms-submit">
								<?=GetMessage('CRM_TIMELINE_SEND')?>
							</button>
							<a id="<?=htmlspecialcharsbx($smsCancelButtonID)?>" href="#" class="crm-entity-stream-content-new-sms-cancel"><?=GetMessage('CRM_TIMELINE_CANCEL_BTN')?></a>
	...
	BX.ready(
		function()
		{
			...
			BX.CrmHistoryItemMark.messages =
			{
				...
				taskSuccessMark: "<?=GetMessageJS('CRM_TIMELINE_TASK_SUCCESSMARK')?>",
			...
			BX.CrmTimelineManager.create(
				"<?=CUtil::JSEscape($guid)?>",
				{
					...
					serviceUrl: "/thurly/components/thurly/crm.timeline/ajax.php?&site=<?=SITE_ID?>&<?=thurly_sessid_get()?>",
					playerUrl:"/thurly/components/thurly/crm.timeline/player.ajax.php?&site=<?=SITE_ID?>&<?=thurly_sessid_get()?>",
					commentEditorUrl:"/thurly/components/thurly/crm.timeline/editor.php?&site=<?=SITE_ID?>&<?=thurly_sessid_get()?>",
