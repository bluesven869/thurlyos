		...
		elseif(isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']) || isset($_POST['continue']))
		{
			...
			if(isset($_POST['CONTACT_ID']))
			{
				...
				$currentContactIDs = isset($arResult['ELEMENT']['ID']) && $arResult['ELEMENT']['ID'] > 0
					?\Thurly\Crm\Binding\ContactCompanyTable::getCompanyContactIDs($arResult['ELEMENT']['ID'])
					: array();
...
$enableDupControl = $arResult['DUPLICATE_CONTROL']['ENABLED'] =
	!$isEditMode && \Thurly\Crm\Integrity\DuplicateControl::isControlEnabledFor(CCrmOwnerType::Company);

// Fix for #26945. Suppress binding of contacts to new compnany. Contacts will be binded to source company.
if($isEditMode)
{
	$arResult['CONTACT_ID'] = \Thurly\Crm\Binding\ContactCompanyTable::getCompanyContactIDs($arResult['ELEMENT']['ID']);
}
...
if($enableOutmodedFields)
{
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'type' => 'address',
		'componentParams' => array(
			'SERVICE_URL' => '/thurly/components/thurly/crm.company.edit/ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
			'DATA' => array(
...
if (CCrmContact::CheckReadPermission(0, $userPermissions))
{
	...
	{
		$contactIDs = \Thurly\Crm\Binding\ContactCompanyTable::getCompanyContactIDs($arResult['ELEMENT']['ID']);
	}
	else
	{
		$contactIDs = array();
...
else
{
	...
	$fileViewer = new \Thurly\Crm\UserField\FileViewer(CCrmOwnerType::Company, $arResult['ELEMENT']['ID']);
}
