...
if (isset($_SERVER["REQUEST_URI"]) && strlen($_SERVER["REQUEST_URI"]) > 0)
	$path = substr($_SERVER["REQUEST_URI"], strlen("/thurly/tools/crm_sale_proxy.php"));
else
...
if (isset($_REQUEST["__BX_CRM_QUERY_STRING_PREFIX"]))
{
	...
	if (substr($prefix, 0, strlen("/thurly/tools/crm_sale_proxy.php?")) == "/thurly/tools/crm_sale_proxy.php?")
		$prefix = substr($prefix, strlen("/thurly/tools/crm_sale_proxy.php?"));
...
$pathRegexs = array(
	"/^\/thurly\/admin\/[a-z0-9_.-]+\.php$/i",
	"/^\/thurly\/tools(?:\/[^\/]+)*\/[a-z0-9_.-]+\.php$/i",
	"/^\/thurly\/components\/thurly\/(?:[a-z0-9_.-]+\/)+[a-z0-9_.-]+\.(?:php|js|css)$/i",
	"/^\/thurly\/js\/(?:[a-z0-9_.-]+\/)+[a-z0-9_.-]+\.(?:js|css)$/i"
);
...
$body = preg_replace(
	"#(\"|')(/thurly/([a-z0-9_.-]+/)*?([a-z0-9_.-]+?\.(gif|png)))#i",
	"$1".($proxy->GetUrl())."$2",
	$body
);

$body = preg_replace(
	"#(\"|')(/thurly/([a-z0-9_.-]+/)*?(sale\.css))#i",
	"$1".($proxy->GetUrl())."$2",
	$body
);
...
$body = preg_replace(
	"#(<a\s[^>]*?)(href\s*=\s*(\"|'))(/thurly/([a-z0-9_.-]+/)*([a-z0-9_.-]+\.php)(?<!sale_order_edit\.php|ajax\.php|sale_order_new\.php|sale_order_detail\.php|sale_order_print\.php|sale_print\.php|sale_product_search\.php|user_search\.php))#i",
	"$1target=\"_blank\" $2".($proxy->GetUrl())."$4",
	$body
);

$body = preg_replace(
    "#(\"|')(/thurly/([a-z0-9_.-]+/)*([a-z0-9_.-]+\.(?:php)))#i",
    "$1/thurly/tools/crm_sale_proxy.php?".$externalSaleId."$2",
    $body
);

$body = preg_replace(
	"#(<a\s[^>]*?)(href\s*=\s*(\"|'))(?!/thurly/)(/([a-z0-9_.-]+/)*([a-z0-9_.-]+)/?)(\"|')#i",
