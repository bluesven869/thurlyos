...
(function(BX)
{
	...
	var DEFAULT_AJAX_URL = '/thurly/components/thurly/crm.activity.planner/ajax.php?site_id=' + BX.message('SITE_ID');
	var COMMUNICATIONS_AJAX_URL = '/thurly/components/thurly/crm.activity.editor/ajax.php?siteID='+BX.message('SITE_ID')+'&sessid='+BX.thurly_sessid();
	...
	Planner.util = {
		...
		{
			var dateFormat = BX.date.convertThurlyFormat(BX.message('FORMAT_DATE')).replace(/:?\s*s/, ''),
				timeFormat = BX.date.convertThurlyFormat(BX.message('FORMAT_DATETIME')).replace(/:?\s*s/, ''),
		...
		convertDateTime: function(date)
		{
			...
			var format = BX.date.convertThurlyFormat(BX.type.isNotEmptyString(f) ? f : 'DD.MM.YYYY HH:MI:SS');
	...
	{
		params['sessid'] = BX.thurly_sessid();
	...
	Planner.prototype._createEditSlider = function(params, next)
	{
		...
		top.BX.ThurlyOS.Slider.open(
			'/thurly/components/thurly/crm.activity.planner/slider.php?site_id=' + BX.message('SITE_ID')
	...
	Planner.prototype.showEdit = function (params)
	{
		...
		if (top.BX.ThurlyOS && top.BX.ThurlyOS.Slider)
		{
	...
	if(typeof(BX.CrmActivityProvider) == 'undefined')
	{
		...
		BX.CrmActivityProvider.prototype =
		{
			...
			openDialog: function (mode)
			{
				...
				{
					if (top.BX.ThurlyOS && top.BX.ThurlyOS.Slider && self.getType() == BX.CrmActivityType.email)
					{
						params.ajax_action = 'ACTIVITY_EDIT';
				...
				BX.ajax({
					method: 'POST',
					dataType: 'html',
					url: '/thurly/components/thurly/crm.activity.planner/ajax.php?site_id=' + BX.message('SITE_ID'),
					data: params,
					onsuccess: function (HTML)
					{
						if (BX.CrmActivityProvider.dialogs[dlgId])
