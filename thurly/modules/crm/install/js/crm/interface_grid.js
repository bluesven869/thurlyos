if(typeof(BX.CrmInterfaceGridManager) == 'undefined')
{
	...
	BX.CrmInterfaceGridManager.prototype =
	{
		...
		{
			return this.getSetting('serviceUrl', '/thurly/components/thurly/crm.activity.editor/ajax.php');
		},
...
if(typeof(BX.CrmUIGridExtension) === "undefined")
{
	...
	BX.CrmUIGridExtension.prototype =
	{
		...
		processApplyButtonClick: function()
		{
			...
			else if(actionName === "subscribe")
			{
				if (top.BX.ThurlyOS && top.BX.ThurlyOS.Slider)
				{
					this.createEmailFor({
						...
						'ITEMS': selectedIds.map(function (value) {
			...
			else if(actionName === "move_to_category")
			{
				this.openMoveToCaregoryDialog();
			}
			else
			{
				grid.sendSelected();
			...
			var dlg = new BX.CDialog(
				{
					content_url: BX.util.add_url_param(
						"/thurly/components/thurly/crm.event.add/box.php",
		...
		openMoveToCaregoryDialog: function()
		{
		...
		onExternalEvent: function(params)
		{
			...
			if(BX.prop.getString(BX.prop.getObject(params, "value", {}), "entityTypeName", "") === this.getOwnerTypeName())
			{
				BX.Main.gridManager.reload(this.getGridId());
			}
		}
