...
if(typeof(BX.CmrSelectorMenuItem) === "undefined")
{
	...
	BX.CmrSelectorMenuItem.prototype =
	{
		...
		createMenuItem: function(encode)
		{
			...
			return({ text: text, onclick: BX.delegate(this._onClick, this) });
		},
...
if(typeof(BX.CrmDupCtrlCommunication) === "undefined")
{
	...
	{
		return this.getFieldCount() > 0 ? this.prepareFieldId(0) : "";
	};
...
if(typeof(BX.CrmDealCategorySelectDialog) === "undefined")
{
	...
	BX.CrmDealCategorySelectDialog.prototype =
	{
		...
		getFilteredCagegories: function()
		{
		...
		prepareContent: function()
		{
			...
			var items = BX.CrmDealCategory.getListItems(this.getFilteredCagegories());
...
BX.Crm.Page =
{
	...
	isSliderEnabled: function(url)
	{
		...
		if(typeof(window.top.BX.ThurlyOS) === "undefined")
		{
			return false;
		}
	...
	openSlider: function(url, params)
	{
		...
		window.top.BX.ThurlyOS.Slider.open (
	...
	{
		//HACK: close slider before destroy due to BX.ThurlyOS.Slider.destroy bug
		var current = window.top.BX.ThurlyOS.Slider.getCurrentPage();
		...
		if(!keepalive)
		{
			window.top.BX.ThurlyOS.Slider.destroy(url);
		}
	}
};
...
if(typeof(BX.CrmLeadMode) == "undefined")
{
			...
			if (typeof params == 'object' && params)
	{
				...
				this.needRobotsNotify = params.needRobotsNotify == "Y";
			...
			for(var i=0, l=this.typeBlocks.length; i<l; i++)
		{
			...
			{
					BX.addClass(subBlock, "bx-lead-info-popup-btn-active");
					this.currentCrmType = this.typeBlocks[i].getAttribute("data-crm-type");
				}
				else
				{
					BX.removeClass(subBlock, "bx-lead-info-popup-btn-active");
				}
			}
		},
		...
		confirmRobotsDelete: function ()
		{
			BX.PopupWindowManager.create('confirmRobotsPopup', null, {
				closeIcon : true,
				lightShadow : true,
				overlay : true,
				titleBar: this.message["CRM_ROBOTS_TITLE"],
				zIndex: -970,
				buttons:  [
					new BX.PopupWindowButton({
						text : this.message["CRM_TYPE_TURN_ON"],
						className : 'popup-window-button-create',
				...
				content: '<div style="width:300px;">' + this.message["CRM_ROBOTS_TEXT"] + '</div>'
			}).show();
		},
		...
		showPopup: function ()
		{
			BX.PopupWindowManager.create('leadFirstPopup', null, {
				...
				buttons: this.isAdmin ? [
					new BX.PopupWindowButton({
						...
						events : { click : BX.proxy(function()
						{
							...
							if (this.currentCrmType == "simple" && this.needRobotsNotify)
							{
								this.confirmRobotsDelete();
						...
						this.isLeadEnabled ? new BX.PopupWindowButtonLink({
						text : this.message["CRM_TYPE_CANCEL"],
						events: { click : function()
						{
							this.popupWindow.close();
				...
				content: '<div style="width:600px;height:550px; background: url(/thurly/js/crm/images/waiter-white-64px.gif) no-repeat center;"></div>',
				events : {
							...
							onsuccess: function(result)
		{
							...
							onfailure: function()
			{
					...
					onAfterPopupShow: BX.proxy(function()
					{
						...
						BX.ajax.post(
							'/thurly/tools/crm_lead_mode.php',
							{
		},
