function BxCrmInterfaceForm(name, aTabs)
{
	...
	this.ToggleTabs = function(bSkipSave)
	{
		...
		if(bSkipSave !== true)
			BX.ajax.get('/thurly/components'+this.vars.component_path+'/settings.php?FORM_ID='+this.name+'&action=expand&expand='+(this.bExpandTabs? 'Y':'N')+'&sessid='+this.vars.sessid);
	};
	...
	this.SetTheme = function(menuItem, theme)
	{
		...
		BX.ajax.get('/thurly/components'+_this.vars.component_path+'/settings.php?FORM_ID='+this.name+'&GRID_ID='+this.vars.GRID_ID+'&action=settheme&theme='+theme+'&sessid='+this.vars.sessid);
	};
	...
	this.SaveSettings = function()
	{
		...
		BX.ajax.post('/thurly/components'+_this.vars.component_path+'/settings.php', data, function(){_this.Reload()});
	};
	...
	this.SaveSettings = function(options)
	{
		...
		var url = '/thurly/components' + _this.vars.component_path + '/settings.php';
		if(callback)
		{
	...
	{
		var url = '/thurly/components' + this.vars.component_path + '/settings.php?FORM_ID=' + this.name + '&action=enable&enabled=' + (enabled? 'Y':'N') + '&sessid=' + this.vars.sessid;
...
if(typeof(BX.CrmEntityInfo) === "undefined")
{
	...
	BX.CrmEntityInfo.prototype =
	{
		...
		getTypeName: function()
		{
			...
			{
				result = BX.CrmEntityType.verifyName(this.getSetting("type", ""));
			}
			return result;
		},
		getTypeId: function()
...
if(typeof(BX.CrmClientPanelCommunication) === "undefined")
{
				...
				this._link = BX.create("A",
					{
						...
						props: { href: (callToFormat === BX.CrmCalltoFormat.standard ? "callto://" : "callto:") + BX.util.urlencode(value) },
						text: value
					}
				);

				if(callToFormat === BX.CrmCalltoFormat.thurly)
				{
	...
	{
		BX.CrmClientPanelCommunication.callToFormat = BX.CrmCalltoFormat.thurly;
	}
	...
	BX.CrmCustomDragItem.prototype =
	{
		...
		_scrollIfNeed: function()
		{
			...
			for(;;)
			{
				...
				if(clientRect.bottom < borderBottom && clientRect.top > borderTop)
				{
					break;
				}

				if(clientRect.bottom >= borderBottom)
				{
				...
				else //if(clientRect.top <= borderTop)
				{
					if(scrollTop <= 0)
					{
						break;
...
BX.Crm.EntityEditorBlockClass = (function ()
{
	var EntityEditorBlockClass = function (parameters)
	{
		...
		this.ajaxUrl = "/thurly/components/thurly/crm.requisite.edit/settings.php";
				...
				if (requisiteId > 0)
				{
					if (this.requisiteIndex[requisiteId])
					{
						...
						if (entityTypeId > 0 && entityId > 0)
						{
							...
							var url = BX.util.add_url_param(this.ajaxUrl, {sessid: BX.thurly_sessid()});
							var data = {
								"action": "savelastselectedrequisite",
