if(typeof(BX.CrmActivityEditor) == 'undefined')
{
	...
	BX.CrmActivityEditor.prototype =
	{
		...
		openActivityDialog: function(mode, itemId, options, onCloseCallBack)
		{
			...
			else if(typeID === BX.CrmActivityType.email)
			{
				...
				if (this._markAsCompletedOnView && !item.getSetting('completed', false) && !(BX.ThurlyOS && BX.ThurlyOS.Slider))
				{
	...
	BX.CrmActivityEditor.getDateTimeFormat = function()
	{
		...
		return BX.date.convertThurlyFormat(BX.type.isNotEmptyString(f) ? f : 'DD.MM.YYYY HH:MI:SS');
	};
	BX.CrmActivityEditor.getDateFormat = function()
	{
		var f = BX.message('FORMAT_DATE');
		return BX.date.convertThurlyFormat(BX.type.isNotEmptyString(f) ? f : 'DD.MM.YYYY');
	};
	BX.CrmActivityEditor.getTimeFormat = function()
	{
		...
		return BX.date.convertThurlyFormat(BX.type.isNotEmptyString(t) ? t : 'HH:MI:SS');
	};
...
//			);
	...
	BX.CrmActivityEmail.prototype =
	{
		...
		openDialog: function(mode)
		{
			...
			if (BX.CrmActivityProvider && (mode == BX.CrmDialogMode.view || top.BX.ThurlyOS && top.BX.ThurlyOS.Slider))
			{
