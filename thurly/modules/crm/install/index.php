...
Class crm extends CModule
{
	...
	function InstallUserFields()
	{
		...
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/crm/classes/general/crm_usertypecrm.php");
...
*/
	...
	function InstallDB()
	{
		...
		require_once($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/include.php');

		$this->errors = false;
		if (!$DB->Query("SELECT 'x' FROM b_crm_lead", true))
		{
			$this->errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/db/'.strtolower($DB->type).'/install.sql');
			...
			\Thurly\Crm\Honorific::installDefault();
		...
		if($qty === 0)
		{
			...
			{
				$langFile = $_SERVER["DOCUMENT_ROOT"].'/thurly/modules/crm/lang/'.$langID."/lib/businesstype.php";
		...
		if($DB->type === "MYSQL")
		{
			...
			{
				\Thurly\Main\Config\Option::set("main", "~ft_b_crm_lead", serialize(array("SEARCH_CONTENT" => true)));
			}

			if($DB->Query("CREATE FULLTEXT INDEX IX_B_CRM_DEAL_SEARCH ON b_crm_deal(SEARCH_CONTENT)", true))
			{
				\Thurly\Main\Config\Option::set("main", "~ft_b_crm_deal", serialize(array("SEARCH_CONTENT" => true)));
			}

			if($DB->Query("CREATE FULLTEXT INDEX IX_B_CRM_QUOTE_SEARCH ON b_crm_quote(SEARCH_CONTENT)", true))
			{
				\Thurly\Main\Config\Option::set("main", "~ft_b_crm_quote", serialize(array("SEARCH_CONTENT" => true)));
			}

			if($DB->Query("CREATE FULLTEXT INDEX IX_B_CRM_CONTACT_SEARCH ON b_crm_contact(SEARCH_CONTENT)", true))
			{
				\Thurly\Main\Config\Option::set("main", "~ft_b_crm_contact", serialize(array("SEARCH_CONTENT" => true)));
			}

			if($DB->Query("CREATE FULLTEXT INDEX IX_B_CRM_COMPANY_SEARCH ON b_crm_company(SEARCH_CONTENT)", true))
			{
				\Thurly\Main\Config\Option::set("main", "~ft_b_crm_company", serialize(array("SEARCH_CONTENT" => true)));
			}

			if($DB->Query("CREATE FULLTEXT INDEX IX_B_CRM_ACT_SEARCH ON b_crm_act(SEARCH_CONTENT)", true))
			{
				\Thurly\Main\Config\Option::set("main", "~ft_b_crm_act", serialize(array("SEARCH_CONTENT" => true)));
			}
		}
		//endregion

		\Thurly\Main\Config\Option::set('crm', 'enable_slider', 'Y');
		\Thurly\Crm\EntityRequisite::installDefaultPresets();
		...
		RegisterModuleDependences('main', 'OnUserDelete', 'crm', '\Thurly\Crm\Kanban\SortTable', 'clearUser');
		...
		RegisterModuleDependences('rest', 'OnRestServiceBuildDescription', 'crm', '\Thurly\Crm\SiteButton\Rest', 'onRestServiceBuildDescription');
		...
		RegisterModuleDependences('imconnector', 'OnAddStatusConnector', 'crm', '\Thurly\Crm\SiteButton\Manager', 'onImConnectorChange');
		RegisterModuleDependences('imconnector', 'OnUpdateStatusConnector', 'crm', '\Thurly\Crm\SiteButton\Manager', 'onImConnectorChange');
		RegisterModuleDependences('imconnector', 'OnDeleteStatusConnector', 'crm', '\Thurly\Crm\SiteButton\Manager', 'onImConnectorChange');

		RegisterModuleDependences("main", "OnApplicationsBuildList", "main", '\Thurly\Crm\Integration\Application', "OnApplicationsBuildList", 100, "modules/crm/lib/integration/application.php");
		RegisterModuleDependences('disk', 'onAfterDeleteFile', 'crm', '\Thurly\Crm\Integration\DiskManager', 'OnDiskFileDelete');
		...
		RegisterModuleDependences('crm', 'OnAfterCrmInvoiceSetStatus', 'crm', '\Thurly\Crm\Automation\Trigger\InvoiceTrigger', 'onAfterCrmInvoiceSetStatus');
		RegisterModuleDependences('disk', 'onBuildAdditionalConnectorList', 'crm', '\Thurly\Crm\Integration\DiskManager', 'onBuildConnectorList');

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->registerEventHandler('main', 'OnAfterSetOption_~crm_webform_max_activated', 'crm', '\Thurly\Crm\WebForm\Form', 'onAfterSetOptionCrmWebFormMaxActivated');
		$eventManager->registerEventHandler('main', 'OnAfterSetOption_crm_deal_category_limit', 'crm', '\Thurly\Crm\Restriction\RestrictionManager', 'onDealCategoryLimitChange');
		...
		$eventManager->registerEventHandler('sale', 'OnSalePsServiceProcessRequestBeforePaid', 'crm', '\Thurly\Crm\InvoiceTable', 'redeterminePaySystem');
		$eventManager->registerEventHandler('sale', 'OnSaleGetHandlerDescription', 'crm', '\CCrmPaySystem', 'getHandlerDescriptionEx');

		$eventManager->registerEventHandler('iblock', 'OnIBlockPropertyBuildList', 'crm', '\Thurly\Crm\Integration\IBlockElementProperty', 'getUserTypeDescription');

		$eventManager->registerEventHandler('socialnetwork', 'onUserProfileRedirectGetUrl', 'crm', '\Thurly\Crm\Integration\Socialnetwork', 'onUserProfileRedirectGetUrl');
		$eventManager->registerEventHandler('main', 'OnUserConsentProviderList', 'crm', '\Thurly\Crm\Integration\UserConsent', 'onProviderList');
		$eventManager->registerEventHandler('main', 'OnUserConsentDataProviderList', 'crm', '\Thurly\Crm\Integration\UserConsent', 'onDataProviderList');

		$eventManager->registerEventHandler('main', 'OnAfterUserTypeAdd', 'crm', '\Thurly\Crm\UserField\UserFieldHistory', 'onAdd');
		$eventManager->registerEventHandler('main', 'OnAfterUserTypeUpdate', 'crm', '\Thurly\Crm\UserField\UserFieldHistory', 'onUpdate');
		$eventManager->registerEventHandler('main', 'OnAfterUserTypeDelete', 'crm', '\Thurly\Crm\UserField\UserFieldHistory', 'onDelete');
	...
	function UnInstallDB($arParams = array())
	{
		...
		UnRegisterModuleDependences('main', 'OnUserDelete', 'crm', '\Thurly\Crm\Kanban\SortTable', 'clearUser');
		...
		UnRegisterModuleDependences('rest', 'OnRestServiceBuildDescription', 'crm', '\Thurly\Crm\SiteButton\Rest', 'onRestServiceBuildDescription');
		...
		UnRegisterModuleDependences('imconnector', 'OnAddStatusConnector', 'crm', '\Thurly\Crm\SiteButton\Manager', 'onImConnectorChange');
		UnRegisterModuleDependences('imconnector', 'OnUpdateStatusConnector', 'crm', '\Thurly\Crm\SiteButton\Manager', 'onImConnectorChange');
		UnRegisterModuleDependences('imconnector', 'OnDeleteStatusConnector', 'crm', '\Thurly\Crm\SiteButton\Manager', 'onImConnectorChange');

		UnRegisterModuleDependences('forum', 'OnAfterCommentAdd', 'crm', 'CCrmLiveFeed', 'onAfterCommentAdd');
		UnRegisterModuleDependences('disk', 'onAfterDeleteFile', 'crm', '\Thurly\Crm\Integration\DiskManager', 'OnDiskFileDelete');
		...
		UnRegisterModuleDependences('crm', 'OnAfterCrmInvoiceSetStatus', 'crm', '\Thurly\Crm\Automation\Trigger\InvoiceTrigger', 'onAfterCrmInvoiceSetStatus');
		UnRegisterModuleDependences('disk', 'onBuildAdditionalConnectorList', 'crm', '\Thurly\Crm\Integration\DiskManager', 'onBuildConnectorList');

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->unRegisterEventHandler('main', 'OnAfterSetOption_~crm_webform_max_activated', 'crm', '\Thurly\Crm\WebForm\Form', 'onAfterSetOptionCrmWebFormMaxActivated');
		...
		$eventManager->unregisterEventHandler('sale', 'OnSalePsServiceProcessRequestBeforePaid', 'crm', '\Thurly\Crm\InvoiceTable', 'redeterminePaySystem');

		$eventManager->unRegisterEventHandler('iblock', 'OnIBlockPropertyBuildList', 'crm', '\Thurly\Crm\Integration\IBlockElementProperty', 'getUserTypeDescription');

		$eventManager->unRegisterEventHandler('socialnetwork', 'onUserProfileRedirectGetUrl', 'crm', '\Thurly\Crm\Integration\Socialnetwork', 'onUserProfileRedirectGetUrl');
		$eventManager->unRegisterEventHandler('main', 'OnUserConsentProviderList', 'crm', '\Thurly\Crm\Integration\UserConsent', 'onProviderList');
		$eventManager->unRegisterEventHandler('main', 'OnUserConsentDataProviderList', 'crm', '\Thurly\Crm\Integration\UserConsent', 'onDataProviderList');

		$eventManager->unRegisterEventHandler('main', 'OnAfterUserTypeAdd', 'crm', '\Thurly\Crm\UserField\UserFieldHistory', 'onAdd');
		$eventManager->unRegisterEventHandler('main', 'OnAfterUserTypeUpdate', 'crm', '\Thurly\Crm\UserField\UserFieldHistory', 'onUpdate');
		$eventManager->unRegisterEventHandler('main', 'OnAfterUserTypeDelete', 'crm', '\Thurly\Crm\UserField\UserFieldHistory', 'onDelete');
		//endregion
		...
		if (!array_key_exists('savedata', $arParams) || $arParams['savedata'] != 'Y')
		{
			...
			$this->errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/db/'.strtolower($DB->type).'/uninstall.sql');
		...
		while ($lang = $langs->fetch())
		{
			...
			$eventTypes = array(
				array(
					...
					'NAME'        => getMessage('CRM_EMAIL_CONFIRM_TYPE_NAME'),
					'DESCRIPTION' => getMessage('CRM_EMAIL_CONFIRM_TYPE_DESC'),
					'SORT'        => 1,
				)
			);
			...
			if (count($sitesIds) > 0)
			{
				$eventMessages = array(
					array(
						...
						'EMAIL_TO'   => '#EMAIL#',
						'SUBJECT'    => getMessage('CRM_EMAIL_CONFIRM_EVENT_NAME'),
						'MESSAGE'    => getMessage('CRM_EMAIL_CONFIRM_EVENT_DESC'),
						'BODY_TYPE'  => 'html',
	...
	{
		CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/components', $_SERVER['DOCUMENT_ROOT'].'/thurly/components', true, true);
		CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/gadgets', $_SERVER['DOCUMENT_ROOT'].'/thurly/gadgets', true, true);
		CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/js', $_SERVER['DOCUMENT_ROOT'].'/thurly/js', true, true);
		CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/admin', $_SERVER['DOCUMENT_ROOT'].'/thurly/admin', true, true);
		CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/tools/', $_SERVER['DOCUMENT_ROOT'].'/thurly/tools', true, true);
		CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/activities/', $_SERVER['DOCUMENT_ROOT'].'/thurly/activities', true, true);
		CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/themes/', $_SERVER['DOCUMENT_ROOT'].'/thurly/themes', true, true);
		CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/services/', $_SERVER['DOCUMENT_ROOT'].'/thurly/services', true, true);
		...
		$APPLICATION->SetFileAccessPermission('/thurly/admin/crm_bizproc_activity_settings.php', array('2' => 'R'));
		$APPLICATION->SetFileAccessPermission('/thurly/admin/crm_bizproc_selector.php', array('2' => 'R'));
		$APPLICATION->SetFileAccessPermission('/thurly/admin/crm_bizproc_wf_settings.php', array('2' => 'R'));

		\Thurly\Main\Loader::includeModule('crm');
		\Thurly\Crm\Preview\Route::setCrmRoutes();
	...
	function UnInstallFiles()
	{
		...
		{
			DeleteDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/js', $_SERVER['DOCUMENT_ROOT'].'/thurly/js');
			DeleteDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/themes', $_SERVER['DOCUMENT_ROOT'].'/thurly/themes');
			DeleteDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/gadgets', $_SERVER['DOCUMENT_ROOT'].'/thurly/gadgets');
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if (!CBXFeatures::IsFeatureEditable('crm'))
		{
			...
			$GLOBALS['errors'] = $this->errors;
			$APPLICATION->IncludeAdminFile(GetMessage('CRM_INSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/step3.php');
		}
		elseif($step < 2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage('CRM_INSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/step1.php');
		}
		elseif($step == 2)
		{
			$this->InstallDB();
...
/*		elseif ($step == 3)
	...
	function DoUninstall()
	{
		...
		if ($step < 2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage('CRM_UNINSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/unstep1.php');
		}
		elseif ($step == 2)
		{
			...
			$GLOBALS['errors'] = $this->errors;
			$APPLICATION->IncludeAdminFile(GetMessage('CRM_UNINSTALL_TITLE'), $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/unstep2.php');
		}
	}
