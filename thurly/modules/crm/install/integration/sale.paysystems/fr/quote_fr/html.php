...
<html>
<head>
<title><?= Loc::getMessage('SBLP_Q_FR_TEXT_QUOTE', null, $lng) ?></title>
	...
	<tr><?
		...
		<td><?
			...
			<? if (CSalePaySystemAction::GetParamValue("SELLER_PHONE", false)) { ?>
				<?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_PHONE', null, $lng).": %s", CSalePaySystemAction::GetParamValue("SELLER_PHONE", false)); ?><br>
			<? } ?>
			<? if (CSalePaySystemAction::GetParamValue("SELLER_EMAIL", false)) { ?>
				<?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_EMAIL', null, $lng).": %s", CSalePaySystemAction::GetParamValue("SELLER_EMAIL", false)); ?>
			<? } ?>
		</td>
	</tr>
</table>
...
<div style="margin: 0pt; padding: 0pt;">
	<? if (CSalePaySystemAction::GetParamValue("BUYER_NAME", false)) { ?>
		...
		if ($buyerPayerName)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_CONTACT_PERSON', null, $lng).": %s", $buyerPayerName); ?><br><?
		}
		unset($buyerPayerName);
		$buyerPhone = CSalePaySystemAction::GetParamValue("BUYER_PHONE", false);
		if ($buyerPhone)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_PHONE', null, $lng).": %s", $buyerPhone); ?><br><?
		}
		unset($buyerPhone);
		$buyerFax = CSalePaySystemAction::GetParamValue("BUYER_FAX", false);
		if ($buyerFax)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_FAX', null, $lng).": %s", $buyerFax); ?><br><?
		}
		unset($buyerFax);
		$buyerEmail = CSalePaySystemAction::GetParamValue("BUYER_EMAIL", false);
		if ($buyerEmail)
		{
			?><?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_EMAIL', null, $lng).": %s", $buyerEmail); ?><br><?
		}
		unset($buyerEmail);
		?>
	<? } ?>
	...
	<tr>
		<td>
			<span style="font-size: 1.5em; font-weight: bold; text-align: center;">
				<?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_QUOTE', null, $lng).' '.Loc::getMessage('SBLP_Q_FR_TEXT_NUMBER', null, $lng).' %s',
					htmlspecialcharsbx($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["ACCOUNT_NUMBER"])); ?>
			</span>
		</td>
		<td align="right">
			<?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_ISSUE_DATE', null, $lng).': %s',
...
if (!empty($arBasketItems))
{
	...
	foreach ($columnList as $column)
	{
		...
		{
			$arCols[$column] = array(
				'NAME' => CSalePaySystemAction::GetParamValue('QUOTE_FR_COLUMN_'.$column.'_TITLE'),
	...
	if(is_array($arBasketItems))
	{
		foreach($arBasketItems as &$arBasket)
		{
			...
			if ($productName == "OrderDelivery")
				$productName = Loc::getMessage('SBLP_Q_FR_TEXT_SHIPPING', null, $lng);
			else if ($productName == "OrderDiscount")
				$productName = Loc::getMessage('SBLP_Q_FR_TEXT_DISCOUNT', null, $lng);
			...
			foreach ($arCols as $columnId => $caption)
			{
				...
				switch ($columnId)
				{
					...
					case 'QUANTITY':
						...
						break;
					case 'MEASURE':
						$data = $arBasket["MEASURE_NAME"] ? htmlspecialcharsbx($arBasket["MEASURE_NAME"]) : Loc::getMessage('SBLP_Q_FR_TEXT_PCS', null, $lng);
						break;
					case 'PRICE':
		...
		foreach ($arCols as $columnId => $caption)
		{
			...
			switch ($columnId)
			{
				...
				case 'MEASURE':
					...
					break;
				case 'PRICE':
					$data = SaleFormatCurrency($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE_DELIVERY"] / (1 + $vat), $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["CURRENCY"], false);
					break;
				case 'VAT_RATE':
	...
	if (CSalePaySystemAction::GetParamValue('QUOTE_FR_TOTAL_SHOW') == 'Y')
	{
		if ($sum < $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE"])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = Loc::getMessage('SBLP_Q_FR_TEXT_SUBTOTAL', null, $lng).":";
		...
		if (!empty($vats))
		{
			...
			foreach ($vats as $vatRate => $vatSum)
			{
				...
				$arCells[$n][$arColumnKeys[$columnCount-2]] = sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_TAX', null, $lng)." (%s%%):", roundEx($vatRate * 100, SALE_VALUE_PRECISION));
		...
		if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["DISCOUNT_VALUE"]) > 0)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = Loc::getMessage('SBLP_Q_FR_TEXT_DISCOUNT', null, $lng).":";
		...
		$arCells[$n][$arColumnKeys[$columnCount-2]] = Loc::getMessage('SBLP_Q_FR_TEXT_TOTAL', null, $lng).":";
...
<div><?=sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_DUE_DATE', null, $lng).': %s', ConvertDateTime(CSalePaySystemAction::GetParamValue("DATE_PAY_BEFORE", false), FORMAT_DATE))?></div>
<br>
<? } ?>
