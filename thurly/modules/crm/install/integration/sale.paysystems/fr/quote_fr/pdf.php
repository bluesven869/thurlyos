...
if (CSalePaySystemAction::GetParamValue('QUOTE_FR_HEADER_SHOW') == 'Y')
{
	...
	if($sellerPhone)
	{
		$sellerPhone = sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_PHONE', null, $lng).": %s", $sellerPhone);
	...
	if($sellerEmail)
	{
		$sellerEmail = sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_EMAIL', null, $lng).": %s", $sellerEmail);
	...
	if($buyerPayerName)
	{
		$buyerPayerName = sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_CONTACT_PERSON', null, $lng)." %s", $buyerPayerName);
	...
	if($buyerPhone)
	{
		$buyerPhone = sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_PHONE', null, $lng).": %s", $buyerPhone);
	...
	if($buyerEmail)
	{
		$buyerEmail = sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_EMAIL', null, $lng).": %s", $buyerEmail);
	...
	$pdf->Cell(0.35*$width, 15, CSalePdf::prepareToPdf(sprintf(
		Loc::getMessage('SBLP_Q_FR_TEXT_QUOTE', null, $lng).' '.Loc::getMessage('SBLP_Q_FR_TEXT_NUMBER', null, $lng).' %s',
		$GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["ACCOUNT_NUMBER"]
	)));

	$pdf->SetFont($fontFamily, 'B', $fontSize);
	$pdf->Cell(0, 15, CSalePdf::prepareToPdf(sprintf(
		Loc::getMessage('SBLP_Q_FR_TEXT_ISSUE_DATE', null, $lng).': %s',
...
if (!empty($arBasketItems))
{
	...
	{
		if (CSalePaySystemAction::GetParamValue('QUOTE_FR_COLUMN_'.$column.'_SHOW') == 'Y')
		{
			$arCols[$column] = array(
				'NAME' => CSalePdf::prepareToPdf(CSalePaySystemAction::GetParamValue('QUOTE_FR_COLUMN_'.$column.'_TITLE')),
				'SORT' => CSalePaySystemAction::GetParamValue('QUOTE_FR_COLUMN_'.$column.'_SORT')
			);
		}
	}
	...
	foreach($arBasketItems as &$arBasket)
	{
		...
		if ($productName == "OrderDelivery")
			$productName = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_FR_TEXT_SHIPPING', null, $lng));
		else if ($productName == "OrderDiscount")
			$productName = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_FR_TEXT_DISCOUNT', null, $lng));
		...
		foreach ($arCols as $columnId => $col)
		{
			...
			switch ($columnId)
			{
				...
				case 'QUANTITY':
					...
					break;
				case 'MEASURE':
					$data = CSalePdf::prepareToPdf($arBasket["MEASURE_NAME"] ? $arBasket["MEASURE_NAME"] : Loc::getMessage('SBLP_Q_FR_TEXT_PCS', null, $lng));
					$arCols[$columnId]['IS_DIGIT'] = true;
					break;
				case 'PRICE':
	...
	if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE_DELIVERY"]) > 0)
	{
		...
		$sDeliveryItem = Loc::getMessage('SBLP_Q_FR_TEXT_SHIPPING', null, $lng);
	...
	$items = $n;
	if (CSalePaySystemAction::GetParamValue('QUOTE_FR_TOTAL_SHOW') == 'Y')
	{
		if ($sum < $GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["PRICE"])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_FR_TEXT_SUBTOTAL', null, $lng).":");
		...
		if (!empty($vats))
		{
			...
			foreach ($vats as $vatRate => $vatSum)
			{
				...
				$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(sprintf(Loc::getMessage('SBLP_Q_FR_TEXT_TAX', null, $lng)." (%s%%):", roundEx($vatRate * 100, SALE_VALUE_PRECISION)));
			...
			if(!empty($arTaxList))
			{
				foreach($arTaxList as &$arTaxInfo)
				{
					...
					$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(sprintf(
						"%s%s%s:",
						($arTaxInfo["IS_IN_PRICE"] == "Y") ? Loc::getMessage('SBLP_Q_FR_TEXT_INCLUDED', null, $lng)." " : "",
						$arTaxInfo["TAX_NAME"],
		...
		if (DoubleVal($GLOBALS["SALE_INPUT_PARAMS"]["ORDER"]["DISCOUNT_VALUE"]) > 0)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_FR_TEXT_DISCOUNT', null, $lng).":"));
		...
		$arCells[$n][$arColumnKeys[$columnCount-2]] = CSalePdf::prepareToPdf(Loc::getMessage('SBLP_Q_FR_TEXT_TOTAL', null, $lng).":");
...
for ($n = 1; $n <= $rowsCnt; $n++)
{
	...
	if (CSalePaySystemAction::GetParamValue('QUOTE_FR_COLUMN_NAME_SHOW') == 'Y')
	{
...
if (CSalePaySystemAction::GetParamValue("DATE_PAY_BEFORE", false))
{
	$pdf->Cell(
		...
		$pdf->prepareToPdf(
			sprintf(
				Loc::getMessage('SBLP_Q_FR_TEXT_DUE_DATE', null, $lng).': %s',
...
if (CSalePaySystemAction::GetParamValue('QUOTE_FR_SIGN_SHOW') == 'Y')
{
...
return $pdf->Output(
	sprintf(
		Loc::getMessage('SBLP_Q_FR_TEXT_QUOTE', null, $lng).' '.Loc::getMessage('SBLP_Q_FR_TEXT_NUMBER', null, $lng).' %s ('.Loc::getMessage('SBLP_Q_FR_TEXT_ISSUE_DATE', null, $lng).' %s).pdf',
		str_replace(
			array(
