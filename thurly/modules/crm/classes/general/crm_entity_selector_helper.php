...
class CCrmEntitySelectorHelper
{
	...
	{
		$enableSlider = \Thurly\Crm\Settings\LayoutSettings::getCurrent()->isSliderEnabled();
				...
				if($arRes = $obRes->Fetch())
				{
						...
						while($arRes = $obRes->Fetch())
						{
							if ($arRes['TYPE_ID'] === 'PHONE' || $arRes['TYPE_ID'] === 'EMAIL')
							{
								$formattedValue = $arRes['TYPE_ID'] === 'PHONE'
									? \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($arRes['VALUE'])->format()
									: $arRes['VALUE'];
					...
					if($requireBindings)
					{
						$result[$advancedInfoKey][$bindingDataKey][CCrmOwnerType::CompanyName] =
							\Thurly\Crm\Binding\ContactCompanyTable::getContactCompanyIDs($entityID);
					}
	...
	public static function PrepareRequisiteData($entityTypeId, $entityId, $options = array())
	{
		...
		if ($requisite->validateEntityReadPermission($entityTypeId, $entityId))
		{
			...
			if (array_search(Thurly\Crm\EntityRequisite::ADDRESS, $select, true))
			{
				$needLoadAddresses = true;
				unset($select[Thurly\Crm\EntityRequisite::ADDRESS]);
			}
			...
			if (is_array($select) && !empty($select))
			{
				...
				while ($row = $res->fetch())
				{
					if ($needLoadAddresses)
						$row[Thurly\Crm\EntityRequisite::ADDRESS] = array();
				...
				if (!empty($requisiteList))
				{
					...
					if ($needLoadAddresses)
					{
						$rqAddr = new Thurly\Crm\RequisiteAddress();
						$res = $rqAddr->getList(
							array(
								'filter' => array(
						...
						while ($row = $res->fetch())
						{
							...
							$requisiteList[$requisiteId][Thurly\Crm\EntityRequisite::ADDRESS][$typeId] = $row;
						}
					}
					...
					foreach ($requisiteList as $requisiteId => $fields)
					{
						...
						if ($bankDetailCountryId <= 0)
							$bankDetailCountryId = \Thurly\Crm\EntityPreset::getCurrentCountryId();
						...
						if (is_array($requisiteData))
						{
							...
							try
							{
								$jsonData = \Thurly\Main\Web\Json::encode($requisiteData);
							}
