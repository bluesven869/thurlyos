...
class CCrmPerms
{
	...
	public static function IsAdmin($userID = 0)
	{
		...
		if($userID <= 0)
		{
			...
			try
			{
				...
				{
					if(class_exists('CThurlyOS')
						&& method_exists('CThurlyOS', 'IsPortalAdmin'))
					{
						// New style check
						$result = CThurlyOS::IsPortalAdmin($userID);
					}
					else
					{
	...
	static public function GetUserAttr($iUserID)
	{
		...
		if (!empty($arResult[$iUserID]['INTRANET']) && Thurly\Main\Loader::includeModule('intranet'))
		{
	...
	static public function BuildSqlForEntitySet(array $enityTypes, $aliasPrefix, $permType, $options = array())
	{
		...
		$enityOptions = array_merge($options, array('RAW_QUERY' => true));
		...
		for($i = 0; $i < $total; $i++)
		{
			...
			if($sql === '')
			{
				if($sqlType === 'WHERE')
				{
					$unrestrictedQueries[] = "SELECT {$aliasPrefix}P.ENTITY_ID FROM b_crm_entity_perms {$aliasPrefix}P WHERE {$aliasPrefix}P.ENTITY = '{$enityType}'";
				}
				else
				{
					$unrestrictedQueries[] = "SELECT {$aliasPrefix}P.ENTITY_ID FROM b_crm_entity_perms {$aliasPrefix}P WHERE {$aliasPrefix}P.ENTITY = '{$enityType}' GROUP BY {$aliasPrefix}P.ENTITY_ID";
				}
			}
			else
			{
		...
		if($rawQueryParam === true || is_array($rawQueryParam))
		{
			if(is_array($rawQueryParam) && isset($rawQueryParam['TOP']) && $rawQueryParam['TOP'] > 0)
			{
				...
				$querySql = \Thurly\Main\Application::getConnection()->getSqlHelper()->getTopSql(
	...
	static public function BuildSql($permEntity, $sAliasPrefix, $mPermType, $arOptions = array())
	{
		...
		if(isset($arOptions['RAW_QUERY']) && ($arOptions['RAW_QUERY'] === true || is_array($arOptions['RAW_QUERY'])))
		{
			if(is_array($arOptions['RAW_QUERY']) && isset($arOptions['RAW_QUERY']['TOP']) && $arOptions['RAW_QUERY']['TOP'] > 0)
			{
				...
				$subQuerySql = \Thurly\Main\Application::getConnection()->getSqlHelper()->getTopSql(
