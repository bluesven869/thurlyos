...
use Thurly\Crm\Integration\OpenLineManager;
use Thurly\Crm\Color\PhaseColorScheme;
...
class CCrmViewHelper
{
	...
	public static function PrepareEntityBaloonHtml($arParams)
	{
		...
		if($entityTypeID === CCrmOwnerType::Company)
		{
			...
			return '<a href="'.htmlspecialcharsbx($showPath).'" id="'.$baloonID.'"'.($className !== '' ? ' class="'.htmlspecialcharsbx($className).'"' : '').'>'.htmlspecialcharsbx($title).'</a>'.
				'<script type="text/javascript">BX.tooltip("COMPANY_'.$entityID.'", "'.$baloonID.'", "/thurly/components/thurly/crm.company.show/card.ajax.php", "crm_balloon_company", true);</script>';
		}
		elseif($entityTypeID === CCrmOwnerType::Contact)
		{
			...
			return '<a href="'.htmlspecialcharsbx($showPath).'" id="'.$baloonID.'"'.($className !== '' ? ' class="'.htmlspecialcharsbx($className).'"' : '').'>'.htmlspecialcharsbx($title).'</a>'.
			'<script type="text/javascript">BX.tooltip("CONTACT_'.$entityID.'", "'.$baloonID.'", "/thurly/components/thurly/crm.contact.show/card.ajax.php", "crm_balloon_contact", true);</script>';
		}
		elseif($entityTypeID === CCrmOwnerType::Lead)
		{
			...
			return '<a href="'.htmlspecialcharsbx($showPath).'" id="'.$baloonID.'"'.($className !== '' ? ' class="'.htmlspecialcharsbx($className).'"' : '').'>'.htmlspecialcharsbx($title).'</a>'.
				'<script type="text/javascript">BX.tooltip("LEAD_'.$entityID.'", "'.$baloonID.'", "/thurly/components/thurly/crm.lead.show/card.ajax.php", "crm_balloon_no_photo", true);</script>';
		}
		elseif($entityTypeID === CCrmOwnerType::Deal)
		{
			...
			return '<a href="'.htmlspecialcharsbx($showPath).'" id="'.$baloonID.'"'.($className !== '' ? ' class="'.htmlspecialcharsbx($className).'"' : '').'>'.htmlspecialcharsbx($title).'</a>'.
				'<script type="text/javascript">BX.tooltip("DEAL_'.$entityID.'", "'.$baloonID.'", "/thurly/components/thurly/crm.deal.show/card.ajax.php", "crm_balloon_no_photo", true);</script>';
		...
		elseif($entityTypeID === CCrmOwnerType::Quote)
		{
			...
			return '<a href="'.htmlspecialcharsbx($showPath).'" id="'.$baloonID.'"'.($className !== '' ? ' class="'.htmlspecialcharsbx($className).'"' : '').'>'.htmlspecialcharsbx($title).'</a>'.
				'<script type="text/javascript">BX.tooltip("QUOTE_'.$entityID.'", "'.$baloonID.'", "/thurly/components/thurly/crm.quote.show/card.ajax.php", "crm_balloon_no_photo", true);</script>';
	...
	public static function PrepareMultiFieldHtml($typeName, $arParams, $arOptions = array())
	{
		...
		if($typeName === 'PHONE')
		{
			...
			$valueUrl = \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($value)->format();
			$additionalHtml = '';
	...
	public static function RenderResponsiblePanel($arParams)
	{
		...
		else
		{
			...
			if($enableLazyLoad)
			{
				$GLOBALS['APPLICATION']->AddHeadScript('/thurly/components/thurly/intranet.user.selector.new/templates/.default/users.js');
				$GLOBALS['APPLICATION']->SetAdditionalCSS('/thurly/components/thurly/intranet.user.selector.new/templates/.default/style.css');
			}
			else
			{
	...
	public static function RenderWidgetFilterPeriod(array $arParams)
	{
		...
		$periodType = isset($config['periodType'])
			? $config['periodType']
			: Thurly\Crm\Widget\FilterPeriodType::UNDEFINED;
		...
		echo '<span id="', $controls['quarterWrap'] ,'" class="bx-select-wrap" style="margin: 0 0 5px;',
			($periodType !== Thurly\Crm\Widget\FilterPeriodType::QUARTER ? ' display:none;' : ''),'">',
		...
		echo '<span id="', $controls['monthWrap'] ,'" class="bx-select-wrap" style="margin: 0 0 5px;',
			($periodType !== Thurly\Crm\Widget\FilterPeriodType::MONTH ? ' display:none;' : ''),' ">',
			'<select id="', $controls['month'], '" class="bx-select"></select>',
			'</span>';

		echo '<span id="', $controls['yearWrap'] ,'" class="bx-select-wrap" style="margin: 0 0 5px;',
			($periodType !== Thurly\Crm\Widget\FilterPeriodType::YEAR
				&& $periodType !== Thurly\Crm\Widget\FilterPeriodType::MONTH
				&& $periodType !== Thurly\Crm\Widget\FilterPeriodType::QUARTER ? ' display:none;' : ''),'">',
	...
	protected static function PrepareDealStages($categoryID = 0)
	{
		...
		$scheme = Thurly\Crm\Color\DealStageColorScheme::getByCategory($categoryID);
	...
	public static function AreDealStageColorsEnabled($categoryID = 0)
	{
		...
		{
			self::$ENABLE_DEAL_STAGE_COLORS[$categoryID] = Thurly\Crm\Color\DealStageColorScheme::getByCategory($categoryID)->isPersistent();
		}
	...
	public static function AreLeadStatusColorsEnabled()
	{
		...
		{
			self::$ENABLE_LEAD_STATUS_COLORS = Thurly\Crm\Color\LeadStatusColorScheme::getCurrent()->isPersistent();
		}
	...
	protected static function PrepareLeadStatuses()
	{
		...
		$scheme = Thurly\Crm\Color\LeadStatusColorScheme::getCurrent();
		if(!$scheme->isPersistent())
		{
	...
	public static function RenderInvoiceStatusSettings()
	{
		...
		$settings = array(
			'imagePath' => '/thurly/js/crm/images/',
	...
	public static function RenderProgressControl($arParams)
	{
		...
		CCrmComponentHelper::RegisterScriptLink('/thurly/js/crm/progress_control.js');
	...
	protected static function PrepareQuoteStatuses()
	{
		...
		$scheme = Thurly\Crm\Color\QuoteStatusColorScheme::getCurrent();
		if(!$scheme->isPersistent())
		{
