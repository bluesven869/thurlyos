...
use Thurly\Main;
use Thurly\Rest\AccessException;
use Thurly\Rest\RestException;
use Thurly\Rest\UserFieldProxy;
use Thurly\Crm\Integration\StorageFileType;
use Thurly\Crm\Integration\StorageType;
use Thurly\Crm\Integration\DiskManager;
use Thurly\Crm\Integration\ThurlyOSManager;
use Thurly\Crm\Color\PhaseColorSchemeElement;
use Thurly\Crm\Color\DealStageColorScheme;
use Thurly\Crm\Color\LeadStatusColorScheme;
use Thurly\Crm\Color\QuoteStatusColorScheme;
use Thurly\Crm\Category\DealCategory;
use Thurly\Crm\Rest;
use Thurly\Crm\Settings\RestSettings;
use Thurly\Crm\EntityPreset;
use Thurly\Crm\EntityRequisite;
use Thurly\Crm\EntityBankDetail;
use Thurly\Crm\EntityAddress;
use Thurly\Crm\EntityAddressType;
use Thurly\Crm\Binding\EntityBinding;
use Thurly\Crm\Binding\DealContactTable;
use Thurly\Crm\Binding\QuoteContactTable;
use Thurly\Crm\Binding\ContactCompanyTable;
...
final class CCrmRestService extends IRestService
{
	...
	public static function onRestServiceMethod($arParams, $nav, CRestServer $server)
	{
		...
		if(!$proxy)
		{
			...
			{
				$proxy = self::$PROXIES[$typeName] = new \Thurly\Crm\Automation\Rest\Proxy();
			}
			else
			{
...
abstract class CCrmRestProxyBase implements ICrmRestProxy
{
	...
	protected function externalizeFile($ownerTypeID, $ownerID, $fieldName, $fileID, $dynamic = true)
	{
		...
		$handlerUrl = "/thurly/components/thurly/crm.{$ownerTypeName}.show/show_file.php";
	...
	{
		$handlerUrl = "/thurly/components/thurly/crm.product.file/download.php";
		$showUrl = CComponentEngine::MakePathFromTemplate(
	...
	{
		return \Thurly\Crm\Format\TextHelper::sanitizeHtml($html);
	}
	...
	{
		return !ThurlyOSManager::isEnabled() || ThurlyOSManager::isRestBizProcEnabled();
	}
	...
	{
		return array('category' => \Thurly\Rest\Sqs::CATEGORY_CRM);
	}
	protected static function createEventInfo($moduleName, $eventName, array $callback)
	{
		return array($moduleName, $eventName, $callback, array('category' => \Thurly\Rest\Sqs::CATEGORY_CRM));
	}
}
	...
	protected function innerGetList($order, $filter, $select, $navigation, &$errors)
	{
		...
		if($ownerType === 'I')
		{
			...
			foreach($productRows as $productRow)
			{
				...
				$discountRate = \Thurly\Crm\Discount::calculateDiscountRate(($exclusivePrice + $discountSum), $exclusivePrice);
				...
				$result[] = array(
					...
					'DISCOUNT_TYPE_ID' => \Thurly\Crm\Discount::MONETARY,
...
class CCrmLeadRestProxy extends CCrmRestProxyBase
{
	...
	protected function innerAdd(&$fields, &$errors, array $params = null)
	{
		...
		//Region automation
		\Thurly\Crm\Automation\Factory::runOnAdd(\CCrmOwnerType::Lead, $result);
		//End region
...
class CCrmDealRestProxy extends CCrmRestProxyBase
{
	...
	protected function innerAdd(&$fields, &$errors, array $params = null)
	{
		...
		//Region automation
		\Thurly\Crm\Automation\Factory::runOnAdd(\CCrmOwnerType::Deal, $result);
		//End region
...
class CCrmDealCategoryProxy extends CCrmRestProxyBase
{
	...
	public function processMethodRequest($name, $nameDetails, $arParams, $nav, $server)
	{
		...
		if($name === 'DEFAULT')
		{
			...
			if($nameSuffix === 'GET')
			{
				return array('ID' => 0, 'NAME' => \Thurly\Crm\Category\DealCategory::getDefaultCategoryName());
			}
			elseif($nameSuffix === 'SET')
			{
				...
				if(is_string($name))
				{
					\Thurly\Crm\Category\DealCategory::setDefaultCategoryName($name);
				}
				return true;
			}
		}
...
class CCrmActivityRestProxy extends CCrmRestProxyBase
{
	...
	protected function innerAdd(&$fields, &$errors, array $params = null)
	{
		...
		$isRestActivity = isset($fields['PROVIDER_ID']) && $fields['PROVIDER_ID'] === \Thurly\Crm\Activity\Provider\RestApp::getId();

		if ($isRestActivity)
		{
			$clientId = $this->getServer()->getClientId();
			$application = $clientId ? \Thurly\Rest\AppTable::getByClientId($clientId) : null;
	...
	protected function innerUpdate($ID, &$fields, &$errors, array $params = null)
	{
		...
		if ($currentFields['PROVIDER_ID'] === \Thurly\Crm\Activity\Provider\RestApp::getId())
		{
			$clientId = $this->getServer()->getClientId();
			$application = $clientId ? \Thurly\Rest\AppTable::getByClientId($clientId) : null;
	...
	protected function externalizeFields(&$fields, &$fieldsInfo)
	{
		...
		if(isset($fields['STORAGE_ELEMENT_IDS']))
		{
			...
			if($storageTypeID === Thurly\Crm\Integration\StorageType::Disk)
			{
				$fields['FILES'] = $fields['STORAGE_ELEMENT_IDS'];
			}
			elseif($storageTypeID === Thurly\Crm\Integration\StorageType::WebDav)
			{
	...
	public function processMethodRequest($name, $nameDetails, $arParams, $nav, $server)
	{
		...
		if(strtoupper($name) === 'FINDBYCOMM')
		{
			...
			foreach($values as $value)
			{
				...
				$criterion = new \Thurly\Crm\Integrity\DuplicateCommunicationCriterion($type, $value);
				$isExists = false;
				foreach($criterions as $curCriterion)
				{
					/** @var \Thurly\Crm\Integrity\DuplicateCriterion $curCriterion */
	...
	public function processMethodRequest($name, $nameDetails, $arParams, $nav, $server)
	{
		...
		if($name === 'ADD')
		{
			...
			if (
				...
				&& ($storage = \Thurly\Disk\Driver::getInstance()->getStorageByUserId($USER->getID()))
			...
			)
			{
				...
				foreach($fields["FILES"] as $tmp)
				{
					...
					if(is_array($arFile))
					{
						...
						if ($file)
						{
							$arResultFile[] = \Thurly\Disk\Uf\FileUserType::NEW_FILE_PREFIX.$file->getId();
						}
					}
				}
...
class CCrmUserFieldRestProxy extends UserFieldProxy implements ICrmRestProxy
{
	...
	public function processMethodRequest($name, $nameDetails, $arParams, $nav, $server)
	{
		...
		elseif($name === 'TYPES' && method_exists('\Thurly\Rest\UserFieldProxy', 'getTypes'))
		{
...
class CCrmRequisiteLinkRestProxy extends CCrmRestProxyBase
{
	...
	protected function getFieldsInfo()
	{
		if(!$this->FIELDS_INFO)
		{
			$this->FIELDS_INFO = Thurly\Crm\Requisite\EntityLink::getFieldsInfo();
		}
		return $this->FIELDS_INFO;
	}
	...
	protected function innerGetList($order, $filter, $select, $navigation, &$errors)
	{
		if (!Thurly\Crm\Requisite\EntityLink::checkReadPermissionOwnerEntity())
		{
			$errors[] = 'Access denied.';
			return false;
		}
		...
		$result = Thurly\Crm\Requisite\EntityLink::getList(
			array(
				'order' => $order,
				'filter' => $filter,
				'select' => $select,
	...
	protected function innerRegister(&$fields, &$errors, array $params = null)
	{
		...
		if (!Thurly\Crm\Requisite\EntityLink::checkUpdatePermissionOwnerEntity($entityTypeId, $entityId))
		{
			$errors[] = 'Access denied.';
			return false;
		}
		...
		try
		{
			Thurly\Crm\Requisite\EntityLink::checkConsistence(
				$entityTypeId, $entityId,
				$requisiteId, $bankDetailId,
				$mcRequisiteId, $mcBankDetailId
			);

			Thurly\Crm\Requisite\EntityLink::register(
				$entityTypeId, $entityId,
				$requisiteId, $bankDetailId,
				$mcRequisiteId, $mcBankDetailId
			);
	...
	protected function innerUnregister($entityTypeId, $entityId, &$errors)
	{
		...
		try
		{
			Thurly\Crm\Requisite\EntityLink::unregister($entityTypeId, $entityId);
		}
		catch (Main\SystemException $e)
		{
			$errors[] = $e->getMessage();
	...
	{
		if(!Thurly\Crm\Requisite\EntityLink::checkReadPermissionOwnerEntity($entityTypeId, $entityId))
		{
			$errors[] = 'Access denied.';
			return false;
		}

		$linkInfo = Thurly\Crm\Requisite\EntityLink::getByEntity($entityTypeId, $entityId);
...
class CCrmPaySystemRestProxy extends CCrmRestProxyBase
{
	...
	protected function innerGetList($order, $filter, $select, $navigation, &$errors)
	{
		...
		if (!empty($personTypeIds))
		{
			...
			$res = \Thurly\Sale\PaySystem\Manager::getList(
				array(
					'order' => $order,
					'filter' => array(
						'LOGIC' => 'AND',
						$filter,
						array('!ID' => \Thurly\Sale\PaySystem\Manager::getInnerPaySystemId()),
					),
					'select' => $select
				)
			);
			...
			while ($row = $res->fetch())
			{
				...
				if (isset($row['ID']) && $row['ID'] > 0)
					$paySystemPersonTypes = \Thurly\Sale\PaySystem\Manager::getPersonTypeIdList($row['ID']);
...
class CCrmMeasureRestProxy extends CCrmRestProxyBase
{
	...
	protected function getFieldsInfo()
	{
		return Thurly\Crm\Measure::getFieldsInfo();
	}
...
class CCrmWebformRestProxy implements ICrmRestProxy
{
	...
	public function processMethodRequest($name, $nameDetails, $arParams, $nav, $server)
	{
		...
		if ($name === 'CONFIGURATION')
		{
			...
			if($nameSuffix === 'GET')
			{
				return array('URL' => CCrmUrlUtil::ToAbsoluteUrl(Thurly\Crm\WebForm\Manager::getUrl()));
			}
		}
		throw new RestException("Resource '{$name}' is not supported in current context.");
	}
...
class CCrmSiteButtonRestProxy implements ICrmRestProxy
{
	...
	public function processMethodRequest($name, $nameDetails, $arParams, $nav, $server)
	{
		...
		if ($name === 'CONFIGURATION')
		{
			...
			if($nameSuffix === 'GET')
			{
				return array('URL' => CCrmUrlUtil::ToAbsoluteUrl(Thurly\Crm\SiteButton\Manager::getUrl()));
			}
		}
		throw new RestException("Resource '{$name}' is not supported in current context.");
	}
