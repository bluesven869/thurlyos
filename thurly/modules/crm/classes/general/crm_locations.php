	...
	public static function getLocationString($locID)
	{
		if(CSaleLocation::isLocationProMigrated())
		{
			...
			if((string) $locID === (string) intval($locID))
				return \Thurly\Sale\Location\Admin\LocationHelper::getLocationStringById($locID);
			else
				return \Thurly\Sale\Location\Admin\LocationHelper::getLocationStringByCode($locID);
		}
		else
		{
			if(!is_int($locID))
