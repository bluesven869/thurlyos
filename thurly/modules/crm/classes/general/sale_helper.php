	...
	private static function PrepareShoppingCartItems(&$productRows, $currencyID, $siteId)
	{
		...
		foreach($productRows as $k => &$v)
		{
			...
			$item['CRM_PR_FIELDS']['DISCOUNT_TYPE_ID'] = isset($v['DISCOUNT_TYPE_ID']) ?
				intval($v['DISCOUNT_TYPE_ID']) : \Thurly\Crm\Discount::PERCENTAGE;
			...
			{
				if($item['CRM_PR_FIELDS']['DISCOUNT_TYPE_ID'] === \Thurly\Crm\Discount::MONETARY)
				{
				...
				else
				{
					...
					$discoutSum = $discoutRate < 100
						? \Thurly\Crm\Discount::calculateDiscountByDiscountPrice($exclusivePrice, $discoutRate)
