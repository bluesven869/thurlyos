...
class CCrmCallToUrl
{
	...
	public static function IsDefined($typeID)
	{
		...
		return $typeID > self::Undefined && $typeID <= self::Thurly;
	}
	...
	{
		$value = self::NormalizeNumberIfRequired($value);
		$format = self::GetFormat(self::Thurly);

		if($format === self::Thurly)
		{
			$paramsString = '';
			if (is_array($params) && count($params) > 0)
			{
				$paramsString = ', '.\Thurly\Main\Web\Json::encode($params);
			}
			return array(
				'HREF' => "callto://{$value}",
	...
	public static function GetAllDescriptions()
	{
		if(!self::$ALL_DESCRIPTIONS)
		{
			...
			self::$ALL_DESCRIPTIONS = array(
				...
				self::Thurly => GetMessage('CRM_CALLTO_URL_THURLY'),
