...
class CCrmFieldMulti
{
		...
		if(self::$ENTITY_TYPES === null)
		{
			self::$ENTITY_TYPES = Array(
				...
				'IM' => Array(
					...
					'THURLY24' => Array('FULL' => GetMessage('CRM_FM_ENTITY_IM_THURLY24'), 'SHORT' => GetMessage('CRM_FM_ENTITY_IM_THURLY24_SHORT'), 'TEMPLATE' => '#VALUE_HTML#'),
	...
	public function SetFields($entityId, $elementId, $arFieldData)
	{
		...
		foreach($arFieldData as $typeId => $arValues)
		{
			...
			foreach($arValues as $id => $arValue)
			{
				$currentValue = isset($arValue['VALUE']) ? trim((string)$arValue['VALUE']) : '';
				$currentValueType = isset($arValue['VALUE_TYPE']) ? trim((string)$arValue['VALUE_TYPE']) : '';
				if (substr($id, 0, 1) == 'n')
				{
					if ($currentValue === '')
					{
				...
				else
				{
					...
					elseif($currentValue !== $arList[$id]['VALUE'] || $currentValueType !== $arList[$id]['VALUE_TYPE'])
					{
						$this->Update(
							$id,
							array(
	...
	public static function PrepareEntityDataBulk($typeID, $entityID, array $elementIDs, array $options = null)
	{
		...
		$elementSql = implode(',', $elementIDs);
		$sql = "SELECT * FROM b_crm_field_multi WHERE TYPE_ID = '{$typeID}' AND ENTITY_ID = '{$entityID}' AND ELEMENT_ID IN({$elementSql})";
		...
		if(is_object($dbResult))
		{
			while($fields = $dbResult->Fetch())
			{
				...
				if($typeID === static::PHONE)
					$fields['VALUE_FORMATTED'] = \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($fields['VALUE'])->format();
				else
