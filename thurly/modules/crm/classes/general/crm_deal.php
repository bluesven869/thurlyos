...
use Thurly\Crm\Category\DealCategoryChangeError;
use Thurly\Crm\Binding\EntityBinding;
use Thurly\Crm\Binding\DealContactTable;
use Thurly\Crm\Settings\DealSettings;
use Thurly\Crm\Settings\HistorySettings;
use Thurly\Crm\History\DealStageHistoryEntry;
use Thurly\Crm\Statistics\DealSumStatisticEntry;
use Thurly\Crm\Statistics\DealInvoiceStatisticEntry;
use Thurly\Crm\Statistics\LeadConversionStatisticsEntry;
use Thurly\Crm\Statistics\DealActivityStatisticEntry;
use Thurly\Crm\Counter\EntityCounterType;
use Thurly\Crm\Counter\EntityCounterManager;
use Thurly\Crm\Integration\Channel\DealChannelBinding;
...
class CAllCrmDeal
{
	...
	public function Add(array &$arFields, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			$arFields['~BEGINDATE'] = $DB->CharToDateFunction(
				isset($arFields['BEGINDATE']) ? $arFields['BEGINDATE'] : $now,
				'SHORT',
				false
			);
			...
			{
				Thurly\Crm\Timeline\DealController::getInstance()->onCreate(
					$ID,
					array(
						'FIELDS' => $arFields,
			...
			Thurly\Crm\Search\SearchContentBuilderFactory::create(CCrmOwnerType::Deal)->build($ID);
			//endregion
			...
			\Thurly\Crm\Kanban\SupervisorTable::sendItem($ID, CCrmOwnerType::DealName, 'kanban_add');
		}
	...
	public function Update($ID, array &$arFields, $bCompare = true, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			if($arRow['STAGE_SEMANTIC_ID'] !== $currentFields['STAGE_SEMANTIC_ID']
				&& $currentFields['STAGE_SEMANTIC_ID'] !== Thurly\Crm\PhaseSemantics::PROCESS)
			{
			...
			Thurly\Crm\Timeline\DealController::getInstance()->onModify(
				$ID,
				array(
			...
			\Thurly\Crm\Kanban\SupervisorTable::sendItem($ID, CCrmOwnerType::DealName, 'kanban_update');
		}
		return $bResult;
	}
	...
	public function Delete($ID, $arOptions = array())
	{
		...
		if (is_object($dbRes) && $dbRes->AffectedRowsCount() > 0)
		{
			...
			Thurly\Crm\Kanban\SortTable::clearEntity($ID, \CCrmOwnerType::DealName);
			...
			\Thurly\Crm\Requisite\EntityLink::unregister(CCrmOwnerType::Deal, $ID);
			\Thurly\Crm\Pseudoactivity\WaitEntry::deleteByOwner(CCrmOwnerType::Deal, $ID);
			\Thurly\Crm\Timeline\TimelineEntry::deleteByOwner(CCrmOwnerType::Deal, $ID);

			if ($arFields['IS_RECURRING'] === "Y")
			{
				$recurDealData = \Thurly\Crm\DealRecurTable::getList(
					array(
				...
				{
					\Thurly\Crm\DealRecurTable::delete($recurring['ID']);
				}
			}
	...
	public static function GetPermissionAttributes(array $IDs, $categoryID = 0)
	{
	...
	public static function GetSemanticID($stageID, $categoryID = -1)
	{
		if($stageID === '')
		{
			return Thurly\Crm\PhaseSemantics::UNDEFINED;
		}
	...
	public static function MoveToCategory($ID, $newCategoryID)
	{
		...
		if($semanticID === Thurly\Crm\PhaseSemantics::SUCCESS)
		{
			$newStageID = $successStageID;
		}
		elseif($semanticID === Thurly\Crm\PhaseSemantics::FAILURE)
		{
			$newStageID = $failureStageID;
		}
		else//if($semanticID === Thurly\Crm\PhaseSemantics::PROCESS)
		{
	...
	public static function FindByCommunication($entityTypeID, $type, $value, $checkPermissions = true, array $select = null, array $order = null)
	{
		...
		/** @var \Thurly\Crm\Integrity\Duplicate $duplicate */
	...
	public static function RebuildSemantics(array $IDs, array $options = null)
	{
		...
		while($fields = $dbResult->Fetch())
		{
			...
			$entity->Update($ID, $updateFields, false, false, array('SYNCHRONIZE_STAGE_SEMANTICS' => true, 'REGISTER_SONET_EVENT' => false));
		}
	}
