...
class CAllCrmQuote
{
	...
	public function Add(&$arFields, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			{
				$contactIDs = \Thurly\Crm\Binding\EntityBinding::prepareEntityIDs(
			...
			elseif(is_array($contactIDs))
			{
				...
				\Thurly\Crm\Binding\EntityBinding::markFirstAsPrimary($contactBindings);
			}
			//endregion
			...
			Thurly\Crm\Search\SearchContentBuilderFactory::create(CCrmOwnerType::Quote)->build($ID);
			//endregion
			...
			\Thurly\Crm\Kanban\SupervisorTable::sendItem($ID, CCrmOwnerType::QuoteName, 'kanban_add');
		}
	...
	public function Update($ID, &$arFields, $bCompare = true, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			$originalContactIDs = \Thurly\Crm\Binding\EntityBinding::prepareEntityIDs(CCrmOwnerType::Contact, $originalContactBindings);
			$contactBindings = isset($arFields['CONTACT_BINDINGS']) && is_array($arFields['CONTACT_BINDINGS'])
				? $arFields['CONTACT_BINDINGS'] : null;
			$contactIDs = isset($arFields['CONTACT_IDS']) && is_array($arFields['CONTACT_IDS'])
				? $arFields['CONTACT_IDS'] : null;
			...
			*/
			...
			if(is_array($contactBindings))
			{
				...
				\Thurly\Crm\Binding\EntityBinding::prepareBindingChanges(
					CCrmOwnerType::Contact,
				...
				$addedContactIDs = \Thurly\Crm\Binding\EntityBinding::prepareEntityIDs(
					CCrmOwnerType::Contact,
					$addedContactBindings
				);

				$removedContactIDs = \Thurly\Crm\Binding\EntityBinding::prepareEntityIDs(
					CCrmOwnerType::Contact,
					$removedContactBindings
				);
			}
			...
			\Thurly\Crm\Kanban\SupervisorTable::sendItem($ID, CCrmOwnerType::QuoteName, 'kanban_update');
		}
		return $bResult;
	}
	...
	public function Delete($ID, $options = array())
	{
		...
		{
			Thurly\Crm\Kanban\SortTable::clearEntity($ID, \CCrmOwnerType::QuoteName);
			...
			\Thurly\Crm\Requisite\EntityLink::unregister(CCrmOwnerType::Quote, $ID);

			// delete utm fields
			UtmTable::deleteEntityUtm(CCrmOwnerType::Quote, $ID);

			if(Thurly\Crm\Settings\HistorySettings::getCurrent()->isQuoteDeletionEventEnabled())
			{
	...
	public static function PrepareConversionPermissionFlags($ID, array &$params, $userPermissions = null)
	{
		...
		$restriction = \Thurly\Crm\Restriction\RestrictionManager::getConversionRestriction();
	...
	public static function ResolvePersonType(array $arFields, array $types = null)
	{
		...
		$contactId = \Thurly\Crm\Binding\EntityBinding::getPrimaryEntityID(CCrmOwnerType::Contact, $contactBindings);
		...
		if ($companyId > 0)
		{
			if (is_array($arCompany) && count($arCompany) >0)
			{
				foreach (self::$clientFields as $k)
				{
					...
					{
						$v = Thurly\Crm\Format\CompanyAddressFormatter::format(
							$arCompany,
		...
		elseif ($contactId > 0)
		{
			if (is_array($arContact) && count($arContact) >0)
			{
				foreach (self::$clientFields as $k)
				{
					...
					elseif ($k === 'CLIENT_ADDR')
					{
						$v = Thurly\Crm\Format\ContactAddressFormatter::format($arContact);
					}
					elseif ($k === 'CLIENT_EMAIL')
					{
						$v = $contactEMail;
		...
		if ($requisiteId > 0 && ($personTypeId == $personTypeContact || $personTypeId == $personTypeCompany))
		{
			...
			$requisiteToClientFieldsMap = array(
				$personTypeCompany => array(
					...
					'RQ_ADDR_'.Thurly\Crm\EntityAddress::Registered => 'CLIENT_ADDR',
					'RQ_INN' => 'CLIENT_TP_ID',
				...
				$personTypeContact => array(
					...
					'RQ_EMAIL' => 'CLIENT_EMAIL',
					'RQ_PHONE' => 'CLIENT_PHONE',
					'RQ_ADDR_'.Thurly\Crm\EntityAddress::Primary => 'CLIENT_ADDR'
				),
			);
	...
	{
		return \Thurly\Crm\Integration\StorageManager::makeFileArray($arElementIDs, $storageTypeID);
	}

	private static function ResolveStorageElementName($storageTypeID, $elementID)
	{
		return \Thurly\Crm\Integration\StorageManager::getFileName($elementID, $storageTypeID);
	}
		...
		if (CModule::IncludeModule('iblock'))
		{
			if ($calculatedOrder['BASKET_ITEMS'])
			{
				...
				if ($productIds)
				{
					...
					{
						$catalogId = isset($product['CATALOG_ID']) ? intval($product['CATALOG_ID']) : \CCrmCatalog::EnsureDefaultExists();
