...
class CCrmLiveFeed
{
	...
	public static function FormatEvent($arFields, $arParams, $bMail = false)
	{
		...
		if ($arFields['ENTITY_TYPE'] == CCrmLiveFeedEntity::Activity)
		{
			if ($arActivity = CCrmActivity::GetByID($arFields["ENTITY_ID"], false))
			{
				...
				$arComponentReturn = $APPLICATION->IncludeComponent('thurly:crm.livefeed.activity', '', array(
					'FIELDS' => $arFields,
					'ACTIVITY' => $arActivity,
					'PARAMS' => $arParams
		...
		elseif ($arFields['ENTITY_TYPE'] == CCrmLiveFeedEntity::Invoice)
		{
			if ($arInvoice = CCrmInvoice::GetByID($arFields["ENTITY_ID"]))
			{
				...
				$arComponentReturn = $APPLICATION->IncludeComponent('thurly:crm.livefeed.invoice', '', array(
					'FIELDS' => $arFields,
					'INVOICE' => $arInvoice,
					'PARAMS' => $arParams
				), null, array('HIDE_ICONS' => 'Y'));
		...
		{
			$arComponentReturn = $APPLICATION->IncludeComponent('thurly:crm.livefeed', '', array(
				'FIELDS' => $arFields,
				'PARAMS' => $arParams
			), null, array('HIDE_ICONS' => 'Y'));
		}
		...
		else
		{
			...
			$arResult["CACHED_CSS_PATH"] = array(
				"/thurly/themes/.default/crm-entity-show.css"
			);

			$arResult["CACHED_JS_PATH"] = array(
				"/thurly/js/crm/progress_control.js",
				"/thurly/js/crm/activity.js",
				"/thurly/js/crm/common.js"
			);
			
			if (IsModuleInstalled("tasks"))
			{
				$arResult["CACHED_CSS_PATH"][] = "/thurly/js/tasks/css/tasks.css";
			}
	...
	static public function OnBuildSocNetLogPerms(&$perms, $params)
	{
		...
		if(empty($affectedEntityTypes))
		{
			...
			$dealPermissionTypes = array_merge(
				...
				\Thurly\Crm\Category\DealCategory::getPermissionEntityTypeList()
			);
	...
	static public function OnBuildSocNetLogFilter(&$filter, &$params, &$componentParams)
	{
		...
		if(isset($filter['SITE_ID']) && \Thurly\Main\ModuleManager::isModuleInstalled('thurlyos'))
		{
			//HACK: Clear filter by SITE_ID in thurlyos context.
	...
	static public function OnAfterSocNetLogEntryCommentAdd($arLogFields, $arParams = array())
	{
		...
		)
		{
			...
			)
			{
				...
				{
					\Thurly\Socialnetwork\Util::notifyMail(array(
						"type" => "LOG_COMMENT",
				...
				{
					\Thurly\Crm\Activity\Provider\Livefeed::addComment($arComment, $arLog);
				}
			}
		}
	}
		...
		if(is_int($logEventID) && $logEventID > 0)
		{
			...
			if (!empty($arSocnetRights))
			{
				...
				\Thurly\Main\FinderDestTable::merge(array(
					"CONTEXT" => "crm_post",
					"CODE" => \Thurly\Main\FinderDestTable::convertRights($arSocnetRights, array("U".$userID))
				));
			...
			\Thurly\Crm\Timeline\LiveFeedController::getInstance()->onMessageCreate(
				$logEventID,
				array('FIELDS' => $fields)
			);
	...
	static public function ConvertTasksLogEvent($arFields)
	{
		...
		$logId = CSocNetLog::Update(
			intval($arFields["LOG_ID"]),
			array(
				"ENTITY_TYPE" => CCrmLiveFeedEntity::Activity,
				"ENTITY_ID" => intval($arFields["ACTIVITY_ID"]),
				...
				"RATING_TYPE_ID" => "LOG_ENTRY",
				"RATING_ENTITY_ID" => intval($arFields["LOG_ID"])
			)
		);
	...
	static public function revertTasksLogEvent($params = array())
	{
		...
		$task = (isset($params['TASK']) ? $params['TASK'] : false);
		$activity = (isset($params['ACTIVITY']) ? $params['ACTIVITY'] : false);
	...
	static public function CounterIncrement($arLogFields)
	{
		...
		$arUserID = isset($data['USERS']) ? $data['USERS'] : array();
		...
		foreach($arUserIDChunks as $i => $arUserIDChunk)
		{
			$sql = "SELECT U.ID as ID, 1 as CNT, '**' as SITE_ID ,'CRM_**' as CODE, 0 as SENT
				FROM b_user U
				WHERE
					(U.ID IN (SELECT USER_ID FROM b_user_access WHERE ACCESS_CODE = 'G1' AND USER_ID <> ".$authorID.")
					".(!empty($arUserIDChunk) ? " OR U.ID IN (".implode(",", $arUserIDChunk).") " : "").")".
				(
	...
	public static function onAfterCommentAdd($entityType, $entityId, $arData)
	{
		...
		if (
			...
			|| !CModule::IncludeModule('tasks')
			|| !CModule::IncludeModule('socialnetwork')
			|| !\Thurly\Tasks\Integration\SocialNetwork::isEnabled()
		)
		{
			return;
		}
		...
		if ($arLog = $dbResult->Fetch())
		{
			...
			if ($arCrmActivity = $dbCrmActivity->Fetch())
			{
				...
				)
				{
					...
					if ($arTaskFields = $dbTask->Fetch())
					{
						$FORUM_ID = \Thurly\Tasks\Integration\SocialNetwork\Task::getCommentForumId();
						...
						$feed = new \Thurly\Forum\Comments\Feed(
							$FORUM_ID,
							array(
								"type" => "TK",
								"id" => $arTaskFields['ID'],
						...
						\Thurly\Tasks\Integration\SocialNetwork::disable(); // disable socnet on comment add to avoid recursion
						$message = $feed->add($arFieldsMessage);
						\Thurly\Tasks\Integration\SocialNetwork::enable(); // enable it back
	...
	public static function SyncTaskEvent($arEntity, $arAllTaskFields)
	{
		...
		if (
			...
			&& intval($arAllTaskFields['GROUP_ID']) > 0
			&& $arEntity
			&& !empty($arEntity['ID'])
			&& intval($arEntity['ID']) > 0
		)
		{
...
class CCrmLiveFeedComponent
{
	...
	public function __construct($params)
	{
		...
		$this->arSipServiceUrl = array(
			CCrmOwnerType::Lead => SITE_DIR.'thurly/components/thurly/crm.lead.show/ajax.php?'.thurly_sessid_get(),
			CCrmOwnerType::Company => SITE_DIR.'thurly/components/thurly/crm.company.show/ajax.php?'.thurly_sessid_get(),
			CCrmOwnerType::Contact => SITE_DIR.'thurly/components/thurly/crm.contact.show/ajax.php?'.thurly_sessid_get()
		);
	...
	public function showField($arField, $arUF = array())
	{
		...
		switch($arField["FORMAT"])
		{
			...
			case "TEXT_FORMATTED_BOLD":
				...
				{
					$text_formatted = $this->ParseText(htmlspecialcharsback($arField["VALUE"]), $arUF, $this->params);
	...
	public static function ProcessLogEventEditPOST($arPOST, $entityTypeID, $entityID, &$arResult, $arUfCode = array())
	{
		...
		$allowToAll = \Thurly\Socialnetwork\ComponentHelper::getAllowToAllDestination($arResult['USER_ID']);
...
*/
				...
				)
				{
					...
					{
						\Thurly\Crm\Activity\Provider\Livefeed::addActivity(array(
							"TYPE" => "ENTRY",
	...
	public static function ProcessLogEventEditPOSTCrmEmailUsers(&$arPOST, &$arCrmEmailEntities = array())
	{
		...
		)
		{
			foreach ($arPOST['SPERM']['UE'] as $key => $userEmail)
			{
				...
				$rsUser = \Thurly\Main\UserTable::getList(array(
					'order' => array('ID' => 'ASC'),
					'filter' => $arFilter,
	...
	public static function OnSonetLogCounterClear($counterType, $timeStamp)
	{
		...
		{
			\Thurly\Crm\Activity\Provider\Livefeed::readComments(array(
				"USER_ID" => $USER->GetId(),
				"TIMESTAMP" => $timeStamp
			));
		}
	...
	public static function processCrmBlogComment($params = array())
	{
		...
		$blogPostId = (isset($params["POST_ID"]) ? intval($params["POST_ID"]) : 0);
		$blogCommentId = (isset($params["COMMENT_ID"]) ? intval($params["COMMENT_ID"]) : 0);
		...
		if (Main\Loader::includeModule('socialnetwork'))
		{
			...
			if ($log = $res->Fetch())
			{
				$res = CCrmActivity::getList(
					array(),
					array(
						'=PROVIDER_ID' => \Thurly\Crm\Activity\Provider\Livefeed::PROVIDER_ID,
						'=PROVIDER_TYPE_ID' => \Thurly\Crm\Activity\Provider\Livefeed::PROVIDER_TYPE_ID_ENTRY,
