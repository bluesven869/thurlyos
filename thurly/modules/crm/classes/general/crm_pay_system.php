...
class CCrmPaySystem
{
	...
	private static $arCrmCompatibleActs = array(
		...
		'paypal', 'yandexinvoice', 'yandexreferrer', 'alfabankb2b'
	);
	...
	public static function getPSCorrespondence($actFile)
	{
		...
		$data = \Thurly\Sale\PaySystem\Manager::getHandlerDescription($actFile);
	...
	public static function rewritePSCorrByRqSource($personTypeId, &$params, $options = array())
	{
		...
		$countryId = \Thurly\Crm\EntityPreset::getCurrentCountryId();
		$psaCode = '';
		$psType = '';
		...
		if (!empty($psaCode))
		{
			...
			{
				if (Thurly\Crm\EntityPreset::isUTFMode())
				...
				if ($countryId)
				{
					$convMap = array(
						'PROPERTY' => array(
							...
							'COMPANY_ADR' => array(
								'TYPE' => 'REQUISITE',
								'VALUE' => 'RQ_ADDR_'.\Thurly\Crm\EntityAddress::Registered.'|'.$countryId
							),
							'PHONE' => array(
								'TYPE' => 'REQUISITE',
							...
							'ADDRESS' => array(
								'TYPE' => 'REQUISITE',
								'VALUE' => 'RQ_ADDR_'.\Thurly\Crm\EntityAddress::Primary.'|'.$countryId
							)
						)
					);
	...
	*/
	...
	public static function getRequisiteFieldSelectItems($fieldsUsedInSettings)
	{
		...
		foreach(Thurly\Crm\RequisiteAddress::getClientTypeInfos() as $typeInfo)
		{
		...
		{
			if ($fieldName === \Thurly\Crm\EntityRequisite::ADDRESS)
			{
		...
		$currentCountryId = \Thurly\Crm\EntityPreset::getCurrentCountryId();
		...
		$rqbcFields = array(
			...
			'RQ_INN' => true,
			'RQ_KPP' => true,
			'RQ_ADDR_'.\Thurly\Crm\EntityAddress::Primary => true,
			'RQ_ADDR_'.\Thurly\Crm\EntityAddress::Registered => true,
			'RQ_EMAIL' => true,
			'RQ_PHONE' => true,
			'RQ_FAX' => true,
			'RQ_CONTACT' => true,
		...
		foreach (\Thurly\Crm\EntityPreset::getCountryList() as $k => $v)
			$countryTitleList[$k] = $v;

		$result[] = array('id' => '', 'title' => GetMessage('CRM_PS_SELECT_NONE'));
		$addressPrefix = \Thurly\Crm\EntityRequisite::ADDRESS;
		$isUTFMode = \Thurly\Crm\EntityPreset::isUTFMode();
	...
	{
		$curPresetCountryId = Thurly\Crm\EntityPreset::getCurrentCountryId();
	...
	/**
	 * @param \Thurly\Main\Event $event
	 * @return array
	 */
	public function getHandlerDescriptionEx(\Thurly\Main\Event $event)
	{
