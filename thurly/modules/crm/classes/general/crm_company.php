...
class CAllCrmCompany
{
	...
	public static function GetTopIDs($top, $sortType = 'ASC', $userPermissions = null)
	{
		...
		$sql = $permissionSql === ''
			? $connection->getSqlHelper()->getTopSql("SELECT ID FROM b_crm_company ORDER BY ID {$sortType}", $top)
			: "SELECT L.ID FROM b_crm_company L INNER JOIN ($permissionSql) LP ON L.ID = LP.ENTITY_ID";
	...
	public function Add(array &$arFields, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			//Statistics & History -->
			Thurly\Crm\Statistics\CompanyGrowthStatisticEntry::register($ID, $arFields);
			//<-- Statistics & History

			Thurly\Crm\Timeline\CompanyController::getInstance()->onCreate(
				$ID,
			...
			{
					\Thurly\Crm\Binding\ContactCompanyTable::bindContactIDs($arFields['ID'], $arFields['CONTACT_ID']);
			...
			Thurly\Crm\Search\SearchContentBuilderFactory::create(CCrmOwnerType::Company)->build($ID);
			//endregion
	...
	public function Update($ID, array &$arFields, $bCompare = true, $bUpdateSearch = true, $arOptions = array())
	{
		...
		else
		{
			...
			if(isset($arFields['LOGO']))
			{
				...
				elseif(is_array($arFields['LOGO']) && strlen(CFile::CheckImageFile($arFields['LOGO'])) === 0)
				{
			...
			Thurly\Crm\Statistics\CompanyGrowthStatisticEntry::synchronize($ID, array(
			...
			if (isset($arFields['CONTACT_ID']) && is_array($arFields['CONTACT_ID']))
			{
				...
				{
					\Thurly\Crm\Binding\ContactCompanyTable::unbindAllContacts($arFields['ID']);
				}
				else
				{
					$arCurrentContact = \Thurly\Crm\Binding\ContactCompanyTable::getCompanyContactIDs($arFields['ID']);
					$arAdd = array_diff($arFields['CONTACT_ID'], $arCurrentContact);
					$arDelete = array_diff($arCurrentContact, $arFields['CONTACT_ID']);

					\Thurly\Crm\Binding\ContactCompanyTable::bindContactIDs($arFields['ID'], $arAdd);
					\Thurly\Crm\Binding\ContactCompanyTable::unbindContactIDs($arFields['ID'], $arDelete);
			...
			Thurly\Crm\Timeline\CompanyController::getInstance()->onModify(
				$ID,
				array(
	...
	public function Delete($ID, $arOptions = array())
	{
		...
		if (is_object($obRes) && $obRes->AffectedRowsCount() > 0)
		{
			...
			\Thurly\Crm\Binding\ContactCompanyTable::unbindAllContacts($ID);
			...
			{
				\Thurly\Crm\Integrity\DuplicateIndexBuilder::markAsJunk(CCrmOwnerType::Company, $ID);
			}
			...
			\Thurly\Crm\Statistics\CompanyGrowthStatisticEntry::unregister($ID);
			//<-- Statistics & History
			...
			\Thurly\Crm\Timeline\TimelineEntry::deleteByOwner(CCrmOwnerType::Company, $ID);

			if(Thurly\Crm\Settings\HistorySettings::getCurrent()->isCompanyDeletionEventEnabled())
			{
