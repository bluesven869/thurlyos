...
use Thurly\Crm;
use Thurly\Crm\UtmTable;
use Thurly\Crm\Integrity\DuplicateCommunicationCriterion;
use Thurly\Crm\Integrity\DuplicatePersonCriterion;
use Thurly\Crm\Integrity\DuplicateOrganizationCriterion;
use Thurly\Crm\Integrity\DuplicateEntityRanking;
use Thurly\Crm\Integrity\DuplicateIndexMismatch;
use Thurly\Crm\Counter\EntityCounterType;
use Thurly\Crm\Counter\EntityCounterManager;
use Thurly\Crm\Integration\Channel\LeadChannelBinding;
...
class CAllCrmLead
{
	...
	public static function GetFields($arOptions = null)
	{
		...
		if(!(is_array($arOptions) && isset($arOptions['DISABLE_ADDRESS']) && $arOptions['DISABLE_ADDRESS']))
		{
			$addrJoin = 'LEFT JOIN b_crm_addr ADDR ON L.ID = ADDR.ENTITY_ID AND ADDR.TYPE_ID = '
				.\Thurly\Crm\EntityAddress::Primary.' AND ADDR.ENTITY_TYPE_ID = '.CCrmOwnerType::Lead;
	...
	public function Add(array &$arFields, $bUpdateSearch = true, $options = array())
	{
		...
		if(isset($arFields['BIRTHDATE']))
		{
			if($arFields['BIRTHDATE'] !== '')
			{
				...
				$arFields['BIRTHDAY_SORT'] = \Thurly\Crm\BirthdayReminder::prepareSorting($birthDate);
			}
			else
			{
				$arFields['BIRTHDAY_SORT'] = \Thurly\Crm\BirthdayReminder::prepareSorting('');
			}
		...
		\Thurly\Crm\EntityAddress::register(
			CCrmOwnerType::Lead,
			$ID,
			\Thurly\Crm\EntityAddress::Primary,
			array(
		...
		//Statistics & History -->
		Thurly\Crm\Statistics\LeadSumStatisticEntry::register($ID, $arFields);
		Thurly\Crm\History\LeadStatusHistoryEntry::register($ID, $arFields, array('IS_NEW' => true));
		if($arFields['STATUS_ID'] === 'CONVERTED')
		{
			Thurly\Crm\Statistics\LeadConversionStatisticsEntry::register($ID, $arFields, array('IS_NEW' => true));
		}
		//Thurly\Crm\Statistics\LeadProcessStatisticsEntry::register($ID, $arFields, array('IS_NEW' => true));
		//<-- Statistics & History

		Thurly\Crm\Timeline\LeadController::getInstance()->onCreate(
			$ID,
		...
		Thurly\Crm\Search\SearchContentBuilderFactory::create(CCrmOwnerType::Lead)->build($ID);
		//endregion
		...
		\Thurly\Crm\Kanban\SupervisorTable::sendItem($ID, CCrmOwnerType::LeadName, 'kanban_add');
	...
	public function Update($ID, array &$arFields, $bCompare = true, $bUpdateSearch = true, $options = array())
	{
		...
		$arFields['IS_RETURN_CUSTOMER'] = isset($arFields['IS_RETURN_CUSTOMER']) ? $arFields['IS_RETURN_CUSTOMER'] : $arRow['IS_RETURN_CUSTOMER'];
		...
		else
		{
			...
			if ($bCompare)
			{
				...
				$arEvents = self::CompareFields($arRow, $arFields, $options);
			...
			if($arRow['STATUS_SEMANTIC_ID'] !== $currentFields['STATUS_SEMANTIC_ID']
				&& $currentFields['STATUS_SEMANTIC_ID'] !== Thurly\Crm\PhaseSemantics::PROCESS)
			{
				$providerIDs = array();
				...
				foreach(\Thurly\Crm\Activity\Provider\ProviderManager::getCompletableProviderList() as $providerInfo)
				{
					...
					if(!isset($activityCompetionConfig[$providerID]) || $activityCompetionConfig[$providerID])
					{
				...
				if($providerQty > 0)
				{
					\CCrmActivity::SetAutoCompletedByOwner(
						CCrmOwnerType::Lead,
						$ID,
						$providerQty < count($activityCompetionConfig) ? $providerIDs : array()
					);
				}
			}
			//endregion
			//region Statistics & History
			Thurly\Crm\Statistics\LeadSumStatisticEntry::register($ID, $currentFields);
			Thurly\Crm\History\LeadStatusHistoryEntry::synchronize($ID, $currentFields);
			LeadChannelBinding::synchronize($ID, $currentFields);
			if(isset($arFields['STATUS_ID']))
			{
				Thurly\Crm\History\LeadStatusHistoryEntry::register($ID, $currentFields, array('IS_NEW' => false));

				if(($arFields['STATUS_ID'] === 'CONVERTED' && $arRow['STATUS_ID'] !== 'CONVERTED')
					|| ($arFields['STATUS_ID'] !== 'CONVERTED' && $arRow['STATUS_ID'] === 'CONVERTED'))
				{
					Thurly\Crm\Statistics\LeadConversionStatisticsEntry::register($ID, $currentFields, array('IS_NEW' => false));
				}
			}
			//endregion
			...
			Thurly\Crm\Timeline\LeadController::getInstance()->onModify(
				$ID,
			...
			\Thurly\Crm\Kanban\SupervisorTable::sendItem($ID, CCrmOwnerType::LeadName, 'kanban_update');
		}
	...
	public function Delete($ID, $arOptions = array())
	{
		...
		if (is_object($obRes) && $obRes->AffectedRowsCount() > 0)
		{
			...
			Thurly\Crm\Kanban\SortTable::clearEntity($ID, \CCrmOwnerType::LeadName);
			...
			Thurly\Crm\History\LeadStatusHistoryEntry::unregister($ID);
			Thurly\Crm\Statistics\LeadSumStatisticEntry::unregister($ID);
			Thurly\Crm\Statistics\LeadActivityStatisticEntry::unregister($ID);
			//Thurly\Crm\Statistics\LeadProcessStatisticsEntry::unregister($ID);
			LeadChannelBinding::unregisterAll($ID);

			if($isConverted)
			{
				Thurly\Crm\Statistics\LeadConversionStatisticsEntry::unregister($ID);
			}

			Thurly\Crm\EntityAddress::unregister(CCrmOwnerType::Lead, $ID, \Thurly\Crm\EntityAddress::Primary);
			...
			{
				Thurly\Crm\Integrity\DuplicateIndexBuilder::markAsJunk(CCrmOwnerType::Lead, $ID);
			}
			...
			\Thurly\Crm\Pseudoactivity\WaitEntry::deleteByOwner(CCrmOwnerType::Lead, $ID);
			\Thurly\Crm\Timeline\TimelineEntry::deleteByOwner(CCrmOwnerType::Lead, $ID);
			...
			if(Thurly\Crm\Settings\HistorySettings::getCurrent()->isLeadDeletionEventEnabled())
			{
	...
	public function CheckFields(&$arFields, $ID = false, $options = array())
	{
		...
		if (isset($arFields['IS_RETURN_CUSTOMER']) && $arFields['IS_RETURN_CUSTOMER'] == 'Y')
		{
			...
			if (!$hasClient)
			{
				$this->LAST_ERROR .= GetMessage('CRM_LEAD_FIELD_REPEAT_CUSTOMER_NO_CLIENT_ERROR')."<br />";
			}
		}
	...
	public static function CompareFields(array $arFieldsOrig, array $arFieldsModif, array $arOptions = null)
	{
		...
		$arMsg = array_merge(
			$arMsg,
			\Thurly\Crm\LeadAddress::prepareChangeEvents(
				$arFieldsOrig,
				$arFieldsModif,
				\Thurly\Crm\ContactAddress::Primary,
				$addressOptions
			)
		);
	...
	public static function RebuildStatistics(array $IDs, array $options = null)
	{
		...
		while($fields = $dbResult->Fetch())
		{
			...
			if($statusID !== '')
			{
				...
				//--> History
				if($enableHistory && ($forced || !Thurly\Crm\History\LeadStatusHistoryEntry::isRegistered($ID)))
				{

					Thurly\Crm\History\LeadStatusHistoryEntry::register(
						$ID,
						$fields,
				...
				if($enableConversionStatistics
						&& ($forced || !Thurly\Crm\Statistics\LeadConversionStatisticsEntry::isRegistered($ID))
						&& $statusID === 'CONVERTED')
				{
					Thurly\Crm\Statistics\LeadConversionStatisticsEntry::register(
						$ID,
						$fields,
						array('IS_NEW' => $isNew)
					);
			...
			//--> Statistics
			if($enableSumStatistics && ($forced || !Thurly\Crm\Statistics\LeadSumStatisticEntry::isRegistered($ID)))
			{
				Thurly\Crm\Statistics\LeadSumStatisticEntry::register($ID, $fields, array('FORCED' => $forced));
			}

			if($enableActivityStatistics)
			{
				$timeline = Thurly\Crm\Statistics\LeadActivityStatisticEntry::prepareTimeline($ID);
				foreach($timeline as $date)
				{
					Thurly\Crm\Statistics\LeadActivityStatisticEntry::register($ID, $fields, array('DATE' => $date));
				}
			}
			//<-- Statistics
		}
