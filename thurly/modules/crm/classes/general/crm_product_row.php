...
class CAllCrmProductRow
{
	...
	public static function Add($arFields, $checkPerms = true, $regEvent = true)
	{
		...
		$arFields['DISCOUNT_TYPE_ID'] = isset($arFields['DISCOUNT_TYPE_ID'])
			? intval($arFields['DISCOUNT_TYPE_ID']) : \Thurly\Crm\Discount::UNDEFINED;

		if(!\Thurly\Crm\Discount::isDefined($arFields['DISCOUNT_TYPE_ID']))
		{
			$arFields['DISCOUNT_TYPE_ID'] = \Thurly\Crm\Discount::PERCENTAGE;
		}

		if($arFields['DISCOUNT_TYPE_ID'] === \Thurly\Crm\Discount::MONETARY)
		{
			$arFields['DISCOUNT_SUM'] = round(doubleval($arFields['DISCOUNT_SUM']), 2);
			$arFields['DISCOUNT_RATE'] = \Thurly\Crm\Discount::calculateDiscountRate(
				($arFields['PRICE'] + $arFields['DISCOUNT_SUM']),
				$arFields['PRICE']
			);
		}
		else if($arFields['DISCOUNT_TYPE_ID'] === \Thurly\Crm\Discount::PERCENTAGE)
		{
			$arFields['DISCOUNT_RATE'] = round(doubleval($arFields['DISCOUNT_RATE']), 2);
			$arFields['DISCOUNT_SUM'] = round(\Thurly\Crm\Discount::calculateDiscountSum(
				$arFields['PRICE'],
		...
		$defaultMeasureInfo = \Thurly\Crm\Measure::getDefaultMeasure();

		if($arFields['MEASURE_CODE'] <= 0)
		{
			if($productID > 0)
			{
				$measureInfos = \Thurly\Crm\Measure::getProductMeasures($productID);
		...
		{
			$measureInfo = \Thurly\Crm\Measure::getMeasureByCode($arFields['MEASURE_CODE']);
	...
	public static function Update($ID, $arFields, $checkPerms = true, $regEvent = true)
	{
		...
		else
		{
			...
			if(!\Thurly\Crm\Discount::isDefined($discountTypeID))
			{
				$discountTypeID = isset($arParams['DISCOUNT_TYPE_ID'])
					? intval($arParams['DISCOUNT_TYPE_ID']) : \Thurly\Crm\Discount::UNDEFINED;
			}
			...
			elseif($arFields['DISCOUNT_TYPE_ID'] === \Thurly\Crm\Discount::PERCENTAGE)
			{
	...
	private static function CheckFields($sAction, &$arFields, $ID)
	{
		if($sAction == 'ADD')
		{
			...
			$discountTypeID = isset($arFields['DISCOUNT_TYPE_ID'])
				&& \Thurly\Crm\Discount::isDefined($arFields['DISCOUNT_TYPE_ID'])
				? intval($arFields['DISCOUNT_TYPE_ID']) : \Thurly\Crm\Discount::UNDEFINED;

			if($discountTypeID !== \Thurly\Crm\Discount::UNDEFINED
				&& !\Thurly\Crm\Discount::isDefined($discountTypeID))
			{
				self::RegisterError("Discount type ID (DISCOUNT_TYPE_ID) '{$discountTypeID}' is not supported in current context.");
			}
			else if($discountTypeID === \Thurly\Crm\Discount::MONETARY && !isset($arFields['DISCOUNT_SUM']))
			{
				self::RegisterError("Discount Sum (DISCOUNT_SUM) is required if Monetary Discount Type (DISCOUNT_TYPE_ID) is defined.");
			}
			else if($discountTypeID === \Thurly\Crm\Discount::PERCENTAGE && !isset($arFields['DISCOUNT_RATE']))
			{
	...
	public static function LoadRows($ownerType, $ownerID, $assoc = false)
	{
		...
		while($ary = $dbRes->Fetch())
		{
			...
			$ary['DISCOUNT_TYPE_ID'] = isset($ary['DISCOUNT_TYPE_ID'])
				? (int)$ary['DISCOUNT_TYPE_ID'] : \Thurly\Crm\Discount::UNDEFINED;
		...
		{
			$defaultMeasureInfo = \Thurly\Crm\Measure::getDefaultMeasure();
			$measureInfos = \Thurly\Crm\Measure::getProductMeasures($measurelessProductIDs);
	...
	public static function SaveRows($ownerType, $ownerID, $arRows, $accountContext = null, $checkPerms = true, $regEvent = true, $syncOwner = true, $totalInfo = array())
	{
		...
		foreach($arRows as &$arRow)
		{
			...
			if(!\Thurly\Crm\Discount::isDefined($arRow['DISCOUNT_TYPE_ID']))
			{
				$arRow['DISCOUNT_TYPE_ID'] = \Thurly\Crm\Discount::PERCENTAGE;
			...
			if($discountTypeID === \Thurly\Crm\Discount::PERCENTAGE)
			{
				...
				{
					$discountSum = round(\Thurly\Crm\Discount::calculateDiscountSum($exclusivePrice, $discountRate), 2);
				}
			}
			else//if($discountTypeID === \Thurly\Crm\Discount::MONETARY)
			{
				if(!isset($arRow['DISCOUNT_SUM']))
				{
					self::RegisterError("Discount Sum (DISCOUNT_SUM) is required if Monetary Discount Type (DISCOUNT_TYPE_ID) is defined.");
	...
	public static function GetByID($ID)
	{
		...
		if(is_array($arResult))
		{
			...
			$arResult['DISCOUNT_TYPE_ID'] = isset($arResult['DISCOUNT_TYPE_ID'])
				? intval($arResult['DISCOUNT_TYPE_ID']) : \Thurly\Crm\Discount::UNDEFINED;
