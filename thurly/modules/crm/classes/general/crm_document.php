...
class CCrmDocument
{
	...
	static public function GetDocumentFieldTypes($documentType)
	{
		...
		foreach ($arTypes as $arType)
		{
			...
			{
				$arResult['UF:'.$arType['USER_TYPE_ID']]['typeClass'] = '\Thurly\Bizproc\BaseType\Date';
				...
				foreach ($fieldValue as $key => $value)
				{
					...
					if (strpos($arFieldType['Type'], 'UF:') === 0)
					{
						...
						if ($sType == 'crm')
						{
							?>
							<script>
							BX.loadCSS('/thurly/js/crm/css/crm.css');
							</script>
							<?
						}
						$arUserFieldType = $USER_FIELD_MANAGER->GetUserType($sType);
	...
	protected static function getWebFormSelectOptions()
	{
		if (self::$webFormSelectList === null)
		{
			...
			$result = \Thurly\Crm\WebForm\Internals\FormTable::getList(array(
	...
	{
		if (($rootActivity->getDocumentEventType() & CBPDocumentEventType::Manual) === 0)
			return;
		...
		\Thurly\Crm\Timeline\BizprocController::getInstance()->onWorkflowStatusChange(
			$workflowId,
			$status
		);
	}
