	...
	implements IBPWorkflowDocument
{
	...
	{
		\Thurly\Main\Localization\Loc::loadMessages($_SERVER['DOCUMENT_ROOT'].BX_ROOT.'/components/thurly/crm.'.
	...
	{
		$arFields['FULL_ADDRESS'] = Thurly\Crm\Format\LeadAddressFormatter::format(
			$arFields,
			array('SEPARATOR' => Thurly\Crm\Format\AddressSeparator::Comma)
		);
		$arFields['COMPANY_IDS'] = \Thurly\Crm\Binding\ContactCompanyTable::getContactCompanyIDs($arFields['ID']);
	}
	...
	static public function CreateDocument($parentDocumentId, $arFields)
	{
		...
		{
			$presetID = \Thurly\Crm\EntityRequisite::getDefaultPresetId(CCrmOwnerType::Contact);
			if($presetID > 0)
			{
				$converter = new \Thurly\Crm\Requisite\AddressRequisiteConverter(CCrmOwnerType::Contact, $presetID, false);
