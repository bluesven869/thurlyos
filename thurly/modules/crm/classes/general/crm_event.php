...
class CCrmEvent
{
	...
	*/
	...
	static public function RegisterViewEvent($entityTypeID, $entityID, $userID = 0)
	{
		...
		$query = new Thurly\Main\Entity\Query(Thurly\Crm\EventTable::getEntity());
		...
		$subQuery = new Thurly\Main\Entity\Query(Thurly\Crm\EventRelationsTable::getEntity());
		$subQuery->addSelect('EVENT_ID');
		$subQuery->addFilter('=ENTITY_TYPE', $entityTypeName);
		$subQuery->addFilter('=ENTITY_ID', $entityID);
		$query->addFilter('@ID', new Thurly\Main\DB\SqlExpression($subQuery->getQuery()));
