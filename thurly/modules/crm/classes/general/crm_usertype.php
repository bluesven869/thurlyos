...
class CCrmUserType
{
	...
	public function AddFields(&$arFilterFields, $ID, $sFormName = 'form1', $bVarsFromForm = false, $bShow = false, $bParentComponent = false, $arOptions = array())
	{
		...
		/** @var \Thurly\Crm\UserField\FileViewer|null $fileViewer */
		...
		foreach($arUserFields as $FIELD_NAME => &$arUserField)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						// In view mode we have to use custom rendering for hide real file URL's ('thurly:system.field.view' can't do it)
					...
					else
					{
						...
						ob_start();
						$APPLICATION->IncludeComponent(
							'thurly:system.field.'.($viewMode ? 'view' : 'edit'),
							$userTypeID,
							array(
								'arUserField' => $arUserField,
								'bVarsFromForm' => $bVarsFromForm,
						...
						foreach ($FIELD_VALUE as $FIELD_VALUE_NAME => $FIELD_VALUE_ID)
						{
							...
							else if ($KEY == 'crm')
							{
								foreach($FIELD_VALUE_ID as $CID)
								{
									...
									if(!$textonly)
									{
										$tooltip = '<script type="text/javascript">BX.tooltip('.$CID.', "balloon_'.$ID.'_'.$FIELD_NAME.'_'.$FIELD_VALUE_NAME.'_'.$CID.'", "/thurly/components/thurly/crm.'.strtolower($FIELD_VALUE_NAME).'.show/card.ajax.php", "crm_balloon'.($FIELD_VALUE_NAME == 'LEAD' || $FIELD_VALUE_NAME == 'DEAL' || $FIELD_VALUE_NAME == 'QUOTE' ? '_no_photo': '_'.strtolower($FIELD_VALUE_NAME)).'", true);</script>';
	...
	public static function onBeforeGetPublicEdit(\Thurly\Main\Event $event)
	{
