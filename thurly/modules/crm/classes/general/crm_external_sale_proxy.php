...
class CCrmExternalSaleProxy
{
	...
	private $userAgent = 'ThurlyCRM client';
	/** @var \Thurly\Main\Web\HttpClient */
	private $client = null;
	...
	public function Send(array $request)
	{
		...
		if($method !== \Thurly\Main\Web\HttpClient::HTTP_GET && $method !== \Thurly\Main\Web\HttpClient::HTTP_POST)
		{
			throw new Thurly\Main\ArgumentException("Could not find 'METHOD'.", 'request');
		}

		$path = isset($request['PATH']) && is_string($request['PATH']) ? $request['PATH'] : '';
		if($path === '')
		{
			throw new Thurly\Main\ArgumentException("Could not find 'PATH'.", 'request');
		}

		$postData = $method === \Thurly\Main\Web\HttpClient::HTTP_POST
			&& isset($request['BODY'])
			? $request['BODY'] : null;

		if(!$this->client)
		{
			$this->client = new \Thurly\Main\Web\HttpClient();
		}

		$this->client->setRedirect(false);

		if($method === \Thurly\Main\Web\HttpClient::HTTP_POST && is_array($postData))
		{
			//Force UTF encoding
			$this->client->setCharset('UTF-8');
			if ((!isset($request['UTF']) || !$request['UTF']) && !defined('BX_UTF'))
			{
				$postData = \Thurly\Main\Text\Encoding::convertEncodingArray($postData, SITE_CHARSET, 'UTF-8');
			}
		}
		...
		{
			/**@var \Thurly\Main\Web\HttpHeaders*/
