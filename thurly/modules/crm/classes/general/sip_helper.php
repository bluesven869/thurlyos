...
class CCrmSipHelper
{
	...
	public static function findByPhoneNumber($number, $params = array())
	{
		if (!is_string($number))
		{
			throw new \Thurly\Main\ArgumentTypeException('number', 'string');
		}

		if ($number === '')
		{
			throw new \Thurly\Main\ArgumentException('Is empty', 'number');
		}
		...
		$dups = array();
		$criterion = new \Thurly\Crm\Integrity\DuplicateCommunicationCriterion('PHONE', $number);
		...
		elseif ($entityTypeID === CCrmOwnerType::Lead)
		{
			...
			if (is_array($entityFields))
			{
				...
				$fields = array(
					...
					'IS_FINAL' => \Thurly\Crm\PhaseSemantics::isFinal(CCrmLead::GetSemanticID($entityFields['STATUS_ID']))
				);
			}
		}
