...
use Thurly\Crm\Integrity\DuplicateBankDetailCriterion;
use Thurly\Crm\Integrity\DuplicateRequisiteCriterion;
...
class CAllCrmContact
{
	...
	public function Add(array &$arFields, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			elseif (!empty($arFields['FACE_ID']) && Main\Loader::includeModule('faceid'))
			{
				...
				$face = \Thurly\Faceid\FaceTable::getRowById($arFields['FACE_ID']);
				if (!empty($face))
				{
			...
			*/
			...
			//Statistics & History -->
			Thurly\Crm\Statistics\ContactGrowthStatisticEntry::register($ID, $arFields);
			//<-- Statistics & History

			//region Save companies
			if (is_array($companyBindings))
			{
				\Thurly\Crm\Binding\ContactCompanyTable::bindCompanies($ID, $companyBindings);
			}
			...
			{
				Thurly\Crm\Statistics\LeadConversionStatisticsEntry::processBindingsChange($arFields['LEAD_ID']);
			}
			//endregion

			Thurly\Crm\Timeline\ContactController::getInstance()->onCreate(
				$ID,
			...
			Thurly\Crm\Search\SearchContentBuilderFactory::create(CCrmOwnerType::Contact)->build($ID);
			//endregion
	...
	public function Update($ID, array &$arFields, $bCompare = true, $bUpdateSearch = true, $arOptions = array())
	{
		...
		else
		{
			...
			//region Preparation of companies
			$originalCompanyBindings = \Thurly\Crm\Binding\ContactCompanyTable::getContactBindings($ID);
			...
			*/
			...
			{
				\Thurly\Crm\Binding\ContactCompanyTable::unbindCompanies($ID, $removedCompanyBindings);
			}

			if(!empty($addedCompanyBindings))
			{
				\Thurly\Crm\Binding\ContactCompanyTable::bindCompanies($ID, $addedCompanyBindings);
			...
			if($oldLeadID != $curLeadID)
			{
				if($oldLeadID > 0)
				{
					Thurly\Crm\Statistics\LeadConversionStatisticsEntry::processBindingsChange($oldLeadID);
				}

				if($curLeadID > 0)
				{
					Thurly\Crm\Statistics\LeadConversionStatisticsEntry::processBindingsChange($curLeadID);
				}
			}
			Thurly\Crm\Statistics\ContactGrowthStatisticEntry::synchronize($ID, array(
			...
			Thurly\Crm\Timeline\ContactController::getInstance()->onModify(
				$ID,
				array(
			...
			if($bResult && $bCompare && $registerSonetEvent && !empty($sonetEventData))
			{
				...
				$parentCompanyIDs = is_array($companyIDs)
					? $companyIDs : \Thurly\Crm\Binding\ContactCompanyTable::getContactCompanyIDs($ID);
				//endregion
	...
	public function Delete($ID, $arOptions = array())
	{
		...
		if (is_object($obRes) && $obRes->AffectedRowsCount() > 0)
		{
			...
			\Thurly\Crm\Binding\ContactCompanyTable::unbindAllCompanies($ID);
			\Thurly\Crm\Binding\DealContactTable::unbindAllDeals($ID);
			\Thurly\Crm\Binding\QuoteContactTable::unbindAllQuotes($ID);
			...
			{
				\Thurly\Crm\Integrity\DuplicateIndexBuilder::markAsJunk(CCrmOwnerType::Contact, $ID);
			}
			...
			if($leadID)
			{
				\Thurly\Crm\Statistics\LeadConversionStatisticsEntry::processBindingsChange($leadID);
			}
			\Thurly\Crm\Statistics\ContactGrowthStatisticEntry::unregister($ID);
			//<-- Statistics & History
			...
			\Thurly\Crm\Timeline\TimelineEntry::deleteByOwner(CCrmOwnerType::Contact, $ID);

			if(\Thurly\Crm\Settings\HistorySettings::getCurrent()->isContactDeletionEventEnabled())
			{
	...
	/**
	 * @deprecated
	 * @see Thurly\Crm\Binding\ContactCompanyTable::getCompanyContactIDs
	 * @param $companyID
	 * @return CDBResult
	 */
	...
	/**
	 * @deprecated
	 * @see Thurly\Crm\Binding\ContactCompanyTable::bindContactIDs
	 * @param array $arIDs
	 * @param int $companyID
	 * @return bool
	 */
