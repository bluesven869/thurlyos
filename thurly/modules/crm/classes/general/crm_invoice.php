...
class CAllCrmInvoice
{
	...
	public function Add($arFields, &$arRecalculated = false, $siteId = SITE_ID, $options = array())
	{
		...
		else
		{
			...
			if (count($productIds) > 0)
				$productMeasures = \Thurly\Crm\Measure::getProductMeasures($productIds);
			unset($productIds);
			...
			foreach ($arProduct as &$productRow)
			{
				...
				else
				{
					...
					$ri = new \Thurly\Main\Type\RandomSequence($productRow["PRODUCT_XML_ID"]);
				...
				if (!isset($productRow['MEASURE_CODE']) || intval($productRow['MEASURE_CODE']) <= 0)
				{
					if ($defaultMeasure === null)
						$defaultMeasure = \Thurly\Crm\Measure::getDefaultMeasure();
				...
				{
					$measure = \Thurly\Crm\Measure::getMeasureByCode($productRow['MEASURE_CODE']);
		...
		if (intval($orderID) > 0)
		{
			...
			if($newDealID)
			{
				Thurly\Crm\Statistics\DealInvoiceStatisticEntry::register($newDealID);
			}
			if($oldDealID > 0 && $oldDealID !== $newDealID)
			{
				Thurly\Crm\Statistics\DealInvoiceStatisticEntry::register($oldDealID);
			}
		...
		if ($orderID > 0)
		{
			...
			{
				Thurly\Crm\History\InvoiceStatusHistoryEntry::register($orderID, null, array('IS_NEW' => $isNew));
				Thurly\Crm\Statistics\InvoiceSumStatisticEntry::register($orderID, null);
			}
			//<-- Statistics & History
			...
			\Thurly\Crm\Kanban\SupervisorTable::sendItem($orderID, CCrmOwnerType::InvoiceName, ($tmpOrderId <= 0) ? 'kanban_add' : 'kanban_update');
		}
	...
	public function Delete($ID)
	{
		...
		if($result)
		{
			Thurly\Crm\Kanban\SortTable::clearEntity($ID, \CCrmOwnerType::InvoiceName);
			...
			if($dealID > 0)
			{
				Thurly\Crm\Statistics\DealInvoiceStatisticEntry::register($dealID);
			}
			CCrmSearch::DeleteSearch('INVOICE', $ID);

			\Thurly\Crm\Requisite\EntityLink::unregister(CCrmOwnerType::Invoice, $ID);

			Thurly\Crm\History\InvoiceStatusHistoryEntry::unregister($ID);
			Thurly\Crm\Statistics\InvoiceSumStatisticEntry::unregister($ID);
			...
			{
				$invoice = \Thurly\Crm\InvoiceRecurTable::getList(
					array(
				...
				{
					\Thurly\Crm\InvoiceRecurTable::delete($invoiceData['ID']);
				}
			}
		}
	...
	public function SetStatus($ID, $statusID, $statusParams = false, $options = array())
	{
		...
		if ($result)
		{
			...
			if ($result)
			{
				...
				if ($result && $curStatusID != $statusID)
				{
					...
					Thurly\Crm\History\InvoiceStatusHistoryEntry::register($ID);
					Thurly\Crm\Statistics\InvoiceSumStatisticEntry::register($ID, null);
				}
		...
		\Thurly\Crm\Kanban\SupervisorTable::sendItem($ID, CCrmOwnerType::InvoiceName, 'kanban_update');
	...
	public static function getPropertiesList($idInvoices)
	{
		...
		$valueData = \Thurly\Sale\Internals\OrderPropsValueTable::getList(
			array(
		...
		$propsData = \Thurly\Sale\Internals\OrderPropsTable::getList(
			array(
				"filter" => array(
					"ID" => $propsIdList,
					"ACTIVE" => "Y"
		...
		if ($companyId > 0)
		{
			if (is_array($arCompany) && count($arCompany) >0)
			{
				foreach ($arInvoiceProperties as $propertyKey => $property)
				{
					...
					{
						$curVal = Thurly\Crm\Format\CompanyAddressFormatter::format(
							$arCompany,
							array('TYPE_ID' => \Thurly\Crm\EntityAddress::Registered)
						);
					}
		...
		elseif ($contactId > 0)
		{
			if (is_array($arContact) && count($arContact) >0)
			{
				foreach ($arInvoiceProperties as $propertyKey => $property)
				{
					...
					{
						$curVal = Thurly\Crm\Format\ContactAddressFormatter::format($arContact);
					}
	...
	public static function rewritePropsFromRequisite($personTypeId, $requisiteId, &$arInvoiceProperties)
	{
		...
		if ($requisiteId > 0)
		{
			...
			)
			{
				...
				$preset = new \Thurly\Crm\EntityPreset();
				...
				if (is_array($row))
				{
					if (isset($row['PRESET_ID']) && $row['PRESET_ID'] > 0)
					{
						...
						$res = $preset->getList(array(
							...
							'filter' => array(
								'=ENTITY_TYPE_ID' => \Thurly\Crm\EntityPreset::Requisite,
						...
						{
							$valueKey = Thurly\Crm\EntityRequisite::ADDRESS.'_'.$addrTypeId;
							$requisiteValues[$valueKey] =
								Thurly\Crm\Format\EntityAddressFormatter::format(
									$addrFields,
									array('SEPARATOR' => Thurly\Crm\Format\AddressSeparator::Comma)
								);
						}
					}
				}
				...
				if (empty($fullName))
				{
					...
					if (!empty($firstName) || !empty($lastName) || !empty($secondName))
					{
						$fullName = CUser::FormatName(
							\Thurly\Crm\Format\PersonNameFormatter::getFormat(),
							array(
								'LOGIN' => '[]',
								'NAME' => $firstName,
								'LAST_NAME' => $lastName,
				...
				$requisiteToPropsMap = array(
					$personTypeCompany => array(
						...
						'RQ_ADDR_'.Thurly\Crm\EntityAddress::Registered => 'COMPANY_ADR',
						'RQ_INN' => 'INN',
						'RQ_KPP' => 'KPP',
					...
					$personTypeContact => array(
						...
						'RQ_EMAIL' => 'EMAIL',
						'RQ_PHONE' => 'PHONE',
						'RQ_ADDR_'.Thurly\Crm\EntityAddress::Primary => 'ADDRESS'
					),
				);
	...
	*/
	public static function installExternalEntities()
	{
		...
		if (COption::GetOptionString('crm', '~CRM_SALE_STATUS_UPDATE_15_5_8', 'N') === 'Y')
		{
			if (COption::GetOptionString('crm', '~CRM_INVOICE_EXCH1C_UPDATE_16_1_8', 'N') !== 'Y')
			{
					...
					&& COption::GetOptionString('crm', '~CRM_INVOICE_EXCH1C_UPDATE_12_5_17', 'N') === 'Y')
				{
					try
					{
						require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/crm/install/exch1c.php");
					}
					catch (Exception $e)
					{
						$errMsg[] = $e->getMessage();
		...
		{
			\Thurly\Main\Config\Option::set('sale', 'format_quantity', '4');
			...
			if (!empty($statuses))
			{
				foreach ($arActiveLangs as &$language)
					$statusLangFiles[$language] = \Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/crm/install/status.php', $language);
				unset($language);
		...
		if (COption::GetOptionString('crm', '~CRM_INVOICE_INSTALL_12_5_7', 'N') !== 'Y')
		{
			try
			{
				require($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/crm/install/sale_link.php");
			}
			catch(Exception $e)
			{
				$errMsg[] = $e->getMessage();
	...
	private static function installOrderPropertyLocationUa()    // Fix 0081306.
	{
		...
		if (IsModuleInstalled('thurlyos')
			&& CModule::IncludeModule('thurlyos')
			&& method_exists('CThurlyOS', 'getLicensePrefix'))
		{
		...
		$dbSaleOrderPropsGroup = CSaleOrderPropsGroup::GetList(
			...
			array(
				...
				"NAME" => Thurly\Main\Localization\Loc::getMessage("CRM_ORD_PROP_GROUP_UR2")
			),
		...
		$arProps[] = array(
			"PERSON_TYPE_ID" => $companyPTID,
			"NAME" => Thurly\Main\Localization\Loc::getMessage("CRM_ORD_PROP_2"),
			"TYPE" => "LOCATION",
			"REQUIED" => "Y",
			"DEFAULT_VALUE" => "",
	...
	public static function ProductRows2BasketItems($arProductRows, $srcCurrencyID = '', $dstCurrencyID = '')
	{
		...
		foreach ($arProductRows as $row)
		{
			...
			$discountTypeID = isset($row['DISCOUNT_TYPE_ID']) ? intval($row['DISCOUNT_TYPE_ID']) : \Thurly\Crm\Discount::UNDEFINED;
			if ($discountTypeID !== \Thurly\Crm\Discount::PERCENTAGE && $discountTypeID !== \Thurly\Crm\Discount::MONETARY)
				$discountTypeID = \Thurly\Crm\Discount::PERCENTAGE;
			if ($discountTypeID === \Thurly\Crm\Discount::PERCENTAGE)
			{
				$discountRate = isset($row['DISCOUNT_RATE']) ? round(doubleval($row['DISCOUNT_RATE']), 2) : 0.0;
				if ($discountRate < 100.00)
					$freshRow['DISCOUNT_PRICE'] = round(\Thurly\Crm\Discount::calculateDiscountSum($exclusivePrice, $discountRate), 2);
				else
	...
	public static function GetSemanticID($statusID)
	{
		if($statusID === 'P')
		{
			return Thurly\Crm\PhaseSemantics::SUCCESS;
		}

		if($statusID === 'D')
		{
			return Thurly\Crm\PhaseSemantics::FAILURE;
		}

		return (self::GetStatusSort($statusID) > self::GetFinalStatusSort())
			? Thurly\Crm\PhaseSemantics::FAILURE : Thurly\Crm\PhaseSemantics::PROCESS;
	}
	...
	public static function RebuildStatistics(array $IDs, array $options = null)
	{
		...
		while($fields = $dbResult->Fetch())
		{
			$ID = (int)$fields['ID'];
			//--> History
			if($enableHistory && ($forced || !Thurly\Crm\History\InvoiceStatusHistoryEntry::isRegistered($ID)))
			{
				$created = isset($fields['DATE_INSERT']) ? $fields['DATE_INSERT'] : '';
				$createdTime = null;
				try
				{
					$createdTime = new Thurly\Main\Type\DateTime(
						$created,
						Thurly\Main\Type\DateTime::convertFormatToPhp(FORMAT_DATETIME));
				}
				catch(Thurly\Main\ObjectException $e)
				{
				}
				...
				if($modified !== '')
				{
					try
					{
						$modifiedTime = new Thurly\Main\Type\DateTime(
							$modified,
							Thurly\Main\Type\DateTime::convertFormatToPhp(FORMAT_DATETIME));
					}
					catch(Thurly\Main\ObjectException $e)
				...
				{
					Thurly\Crm\History\InvoiceStatusHistoryEntry::register(
						$ID,
						$fields,
			...
			//--> Statistics
			if($enableSumStatistics && ($forced || !Thurly\Crm\Statistics\InvoiceSumStatisticEntry::isRegistered($ID)))
			{
				Thurly\Crm\Statistics\InvoiceSumStatisticEntry::register($ID, $fields, array('FORCED' => $forced));
			}
			//<-- Statistics
		}
	}
	...
	public static function PrepareFilter(&$arFilter, $arFilter2Logic = null)
	{
		...
		foreach ($arFilter as $k => $v)
		{
			...
			{
				$v = Thurly\Main\Web\Json::decode($v);
				if(count($v) > 0)
				{
	...
	public static function PrepareSalePaymentData(array &$arOrder, $options = array())
	{
		...
		$mcRequisiteId = 0;
		$mcBankDetailId = 0;
		if ($row = \Thurly\Crm\Requisite\EntityLink::getList(
			array(
				'filter' => array(
					'=ENTITY_TYPE_ID' => CCrmOwnerType::Invoice,
					'=ENTITY_ID' => $ID
		...
		{
			$defLink = Thurly\Crm\Requisite\EntityLink::getDefaultMyCompanyRequisiteLink();
			if (is_array($defLink))
			{
			...
			if (is_array($row))
			{
				if (isset($row['PRESET_ID']) && $row['PRESET_ID'] > 0)
				{
					...
					if ($presetCountryId > 0)
					{
						...
						foreach ($requisite->getAddresses($requisiteId) as $addrTypeId => $addrFields)
						{
							$valueKey = Thurly\Crm\EntityRequisite::ADDRESS.'_'.$addrTypeId.'|'.$presetCountryId;
							$requisiteValues[$valueKey] =
								Thurly\Crm\Format\EntityAddressFormatter::prepareLines(
									$addrFields,
									array(
										'SEPARATOR' => Thurly\Crm\Format\AddressSeparator::NewLine,
										'NL2BR' => false
									)
								);
						}
		...
		if ($bankDetailId > 0)
		{
			$bankDetail = new \Thurly\Crm\EntityBankDetail();
		...
		if ($presetCountryId > 0)
			...
			$countryIds = \Thurly\Crm\EntityRequisite::getAllowedRqFieldCountries();
		$arPersonTypes = CCrmPaySystem::getPersonTypeIDs();
		if ($arPersonTypes['COMPANY'] != "" && $arPersonTypes['CONTACT'] != "")
		{
			$personTypeCompany = $arPersonTypes['COMPANY'];
			...
			if ($requisiteConverted)
			{
				...
				$propsToRequisiteMap = array(
					$personTypeCompany => array(
						...
						'COMPANY_ADR' => 'RQ_ADDR_'.Thurly\Crm\EntityAddress::Registered,
						'INN' => 'RQ_INN',
						'KPP' => 'RQ_KPP',
					...
					$personTypeContact => array(
						...
						'EMAIL' => 'RQ_EMAIL',
						'PHONE' => 'RQ_PHONE',
						'ADDRESS' => 'RQ_ADDR_'.Thurly\Crm\EntityAddress::Primary,
					),
				);
			...
			if (is_array($row))
			{
				if (isset($row['PRESET_ID']) && $row['PRESET_ID'] > 0)
				{
					...
					if ($mcPresetCountryId > 0)
					{
						...
						{
							$valueKey = Thurly\Crm\EntityRequisite::ADDRESS.'_'.$addrTypeId.'|'.$mcPresetCountryId;
	 ...
	 * @throws \Thurly\Main\ObjectNotFoundException
	 */
	public static function getPublicLink($invoiceId)
	{
		if ($invoiceId > 0)
		{
			$order = \Thurly\Sale\Order::load($invoiceId);
			if ($order)
			{
	...
	public static function savePdf($invoice_id, &$error = null)
	{
		...
		$service = \Thurly\Sale\PaySystem\Manager::getObjectById($arOrder["PAY_SYSTEM_ID"]);
		if ($service !== null)
		{
			...
			$order = \Thurly\Sale\Order::load($invoice_id);
			if ($order)
			{
				...
				if ($collection)
				{
					...
					foreach ($collection as $payment)
					{
						if (!$payment->isInner())
						{
							...
							$initResult = $service->initiatePay($payment, null, \Thurly\Sale\PaySystem\BaseServiceHandler::STRING);
