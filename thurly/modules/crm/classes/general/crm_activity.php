...
use Thurly\Crm;
use Thurly\Crm\Integration\StorageManager;
use Thurly\Crm\Integration\StorageType;
use Thurly\Crm\Integration\StorageFileType;
use Thurly\Crm\Settings\ActivitySettings;
use Thurly\Crm\Counter\EntityCounterType;
use Thurly\Crm\Counter\EntityCounterManager;
use Thurly\Disk\SpecificFolder;
...
class CAllCrmActivity
{
	...
	public static function Add(&$arFields, $checkPerms = true, $regEvent = true, $options = array())
	{
		...
		{
			$deadline = new \Thurly\Main\Type\DateTime($deadline);
			...
			foreach($arBindings as $arBinding)
			{
				...
				if($curOwnerID > 0)
				{
					...
					{
						Thurly\Crm\Statistics\DealActivityStatisticEntry::register($curOwnerID, null, array('DATE' => $deadline));
					}
					elseif($curOwnerTypeID === CCrmOwnerType::Lead)
					{
						Thurly\Crm\Statistics\LeadActivityStatisticEntry::register($curOwnerID, null, array('DATE' => $deadline));
					}
				}
			}
			unset($arBinding);
		...
		\Thurly\Crm\Activity\CommunicationStatistics::registerActivity($arFields);
		if ($deadline)
			\Thurly\Crm\Statistics\ActivityStatisticEntry::register($ID, $arFields);

		\Thurly\Crm\Timeline\ActivityController::getInstance()->onCreate($ID, array('FIELDS' => $arFields));

		//region Search content index
		Thurly\Crm\Search\SearchContentBuilderFactory::create(CCrmOwnerType::Activity)->build($ID);
		//endregion
		...
		if(is_int($ID) && $ID > 0)
		{
			...
			\Thurly\Crm\Activity\Provider\ProviderManager::processCreation(
				$arFields,
				array('BINDINGS' => $arBindings)
			);

			\Thurly\Crm\Pseudoactivity\WaitEntry::processActivityCreation(
				$arFields,
				array('BINDINGS' => $arBindings)
			);
			...
			{
				\Thurly\Pull\Event::add($arFields['RESPONSIBLE_ID'], array(
								'module_id' => 'crm',
	...
	public static function Update($ID, $arFields, $checkPerms = true, $regEvent = true, $options = array())
	{
		...
		{
			$deadline = new \Thurly\Main\Type\DateTime($prevDeadline);
			$deadline->setTime(0, 0, 0);
		...
		{
			$deadline = new \Thurly\Main\Type\DateTime($curDeadline);
			$deadline->setTime(0, 0, 0);
			foreach($arBindings as $arBinding)
			{
		...
		\Thurly\Crm\Activity\CommunicationStatistics::updateActivity($arCurEntity, $arPrevEntity);
		if ($curDeadline)
		{
			\Thurly\Crm\Statistics\ActivityStatisticEntry::register($ID, $arCurEntity);
		}
		\Thurly\Crm\Integration\Channel\ActivityChannelBinding::synchronize($ID, $arCurEntity);
		...
		\Thurly\Crm\Timeline\ActivityController::getInstance()->onModify(
			$ID,
			array(
	...
	public static function Delete($ID, $checkPerms = true, $regEvent = true, $options = array())
	{
		...
		if(is_array($arBindings))
		{
			...
			if(!$skipStatistics)
			{
				...
				if($completed && $deadline)
				{
					...
					foreach($arBindings as $arBinding)
					{
						...
						if($curOwnerID > 0)
						{
							...
							{
								Thurly\Crm\Statistics\LeadActivityStatisticEntry::register($curOwnerID, null, array('DATE' => $deadline));
								//Thurly\Crm\Statistics\LeadProcessStatisticsEntry::register($curOwnerID, null, array('IS_NEW' => false, 'DATE' => $deadline));
							}
						}
					}
					unset($arBinding);
				...
				\Thurly\Crm\Activity\CommunicationStatistics::unregisterActivity($ary);
			}
		...
		\Thurly\Crm\Statistics\ActivityStatisticEntry::unregister($ID);
		\Thurly\Crm\Integration\Channel\ActivityChannelBinding::unregisterAll($ID);
		\Thurly\Crm\Timeline\ActivityController::getInstance()->onDelete(
			$ID,
			array('FIELDS' => $ary, 'BINDINGS' => $arBindings)
		);
	...
	protected static function DeleteStorageElements($ID)
	{
		...
		{
			\Thurly\Main\Loader::includeModule('disk');
			...
			foreach($arFileIDs as $fileID)
			{
				...
				{
					$file->delete(\Thurly\Disk\SystemUser::SYSTEM_USER_ID);
				}
			}
		}
	...
	/**
	 * @return \Thurly\Crm\Activity\Provider\Base[] - List of providers.
	 */
	public static function GetProviders()
	{
		if (static::$PROVIDERS === null)
		{
			static::$PROVIDERS = \Thurly\Crm\Activity\Provider\ProviderManager::getProviders();
		}
	 ...
	 * @return null|\Thurly\Crm\Activity\Provider\Base
	 */
	...
	public static function GetProviderByType($typeId)
	{
		...
		switch ($typeId)
		{
			case CCrmActivityType::Meeting:
				$provider = \Thurly\Crm\Activity\Provider\Meeting::className();
				break;
			case CCrmActivityType::Call:
				$provider = \Thurly\Crm\Activity\Provider\Call::className();
				break;
			case CCrmActivityType::Task:
				$provider = \Thurly\Crm\Activity\Provider\Task::className();
				break;
			case CCrmActivityType::Email:
				$provider = \Thurly\Crm\Activity\Provider\Email::className();
				break;
		}
		return $provider;
	}
	...
	public static function Rebind($ownerTypeID, $oldOwnerID, $newOwnerID)
	{
		...
		\Thurly\Crm\Activity\CommunicationStatistics::rebuild($ownerTypeID, array($newOwnerID));
	}
	...
	public static function ChangeOwner($oldOwnerTypeID, $oldOwnerID, $newOwnerTypeID, $newOwnerID)
	{
		...
		\Thurly\Crm\Activity\CommunicationStatistics::rebuild($newOwnerTypeID, array($newOwnerID));
	}

	public static function AttachBinding($srcOwnerTypeID, $srcOwnerID, $targOwnerTypeID, $targOwnerID)
	{
		$dbResult = \Thurly\Main\Application::getConnection()->query(
		...
		foreach($itemFields as $fields)
		{
			...
			\Thurly\Crm\Activity\CommunicationStatistics::rebuild($targOwnerTypeID, array($targOwnerID));
		}
	...
	public static function PrepareCommunicationInfos(array $activityIDs)
	{
		...
		$nameTemplate = \Thurly\Crm\Format\PersonNameFormatter::getFormat();
		...
		while($comm = $dbResult->fetch())
		{
			...
			if($comm['TYPE'] === Crm\CommunicationType::PHONE_NAME && $comm['VALUE'])
				$formattedValue = \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($comm['VALUE'])->format();
			else
	...
	public static function PrepareClientInfos($IDs, $arOptions = null)
	{
		$nameTemplate = is_array($arOptions) && isset($arOptions['NAME_TEMPLATE'])
			...
			? $arOptions['NAME_TEMPLATE'] : \Thurly\Crm\Format\PersonNameFormatter::getFormat();
	...
	public static function PrepareStorageElementInfo(&$arFields)
	{
		...
		elseif($storageTypeID === StorageType::Disk)
		{
			...
			foreach($storageElementIDs as $elementID)
			{
				$infos[] = Thurly\Crm\Integration\DiskManager::getFileInfo(
					$elementID,
					false,
	...
	public static function SetAutoCompletedByOwner($ownerTypeID, $ownerID, array $providerIDs = null)
	{
		$ownerID = (int)$ownerID;
		...
		$query = new \Thurly\Main\Entity\Query(\Thurly\Crm\ActivityTable::getEntity());
		$query->addSelect('ID');
		$query->registerRuntimeField('',
			new \Thurly\Main\Entity\ReferenceField('B',
				\Thurly\Crm\ActivityBindingTable::getEntity(),
				array(
					'=ref.ACTIVITY_ID' => 'this.ID',
					'=ref.OWNER_ID' => new \Thurly\Main\DB\SqlExpression($ownerID),
					'=ref.OWNER_TYPE_ID' => new \Thurly\Main\DB\SqlExpression($ownerTypeID)
				),
		...
		while($fields = $dbResult->fetch())
		{
			self::SetAutoCompleted($fields['ID']);
		}
	}
	...
	{
		return \Thurly\Crm\Activity\Provider\Task::createFromTask($taskID, $arTaskFields, $checkPerms, $regEvent);
	}
	...
	{
		\Thurly\Crm\Activity\Provider\Task::onTaskAdd($taskID, $arTaskFields);
	}
	...
	{
		\Thurly\Crm\Activity\Provider\Task::onBeforeTaskAdd($arTaskFields);
	}
	...
	{
		\Thurly\Crm\Activity\Provider\Task::onTaskUpdate($taskID, $arCurrentTaskFields, $arPreviousTaskFields);
	}
	...
	{
		\Thurly\Crm\Activity\Provider\Task::onTaskDelete($taskID);
	}
	...
	public static function DeleteByOwner($ownerTypeID, $ownerID)
	{
		...
		\Thurly\Crm\Activity\CommunicationStatistics::unregisterByOwner($ownerTypeID, $ownerID);
		//endregion
	...
	public static function DeleteUnbound($arBindings = null)
	{
		...
		if(is_array($arBindings) && !empty($arBindings))
		{
			foreach($arBindings as &$arBinding)
			{
				...
				self::SynchronizeUserActivity($arBinding['OWNER_TYPE_ID'], $arBinding['OWNER_ID'], 0);
				\Thurly\Crm\Activity\CommunicationStatistics::rebuild(
					$arBinding['OWNER_TYPE_ID'],
	...
	{
		return \Thurly\Crm\Integration\StorageManager::makeFileArray($arFileIDs, $storageTypeID);
	}
	...
	/**
	 * @deprecated use \Thurly\Crm\Activity\Provider\Task::updateAssociatedEntity
	 * @param array $arFields Activity data.
	 * @return \Thurly\Main\Result|bool
	 */
	protected static function SaveTask(&$arFields)
	{
		...
		//compatible
		$result = \Thurly\Crm\Activity\Provider\Task::updateAssociatedEntity($associatedEntityID, $arFields);
	...
	protected static function DeleteTask(&$arFields)
	{
		...
		//compatible
		$result = \Thurly\Crm\Activity\Provider\Task::deleteAssociatedEntity($associatedEntityID, $arFields);
		return $result->isSuccess();
	}
	...
	private static function RegisterLiveFeedEvent(&$arFields)
	{
		...
		if (!empty($arStorageElementID))
		{
			...
			else if ($arFields["STORAGE_TYPE_ID"] == StorageType::Disk)
			{
				...
				//We have to add prefix Thurly\Disk\Uf\FileUserType::NEW_FILE_PREFIX to file ID
		...
		if ($arFields["TYPE_ID"] == CCrmActivityType::Email)
		{
			...
			$liveFeedFields['CONTEXT_USER_ID'] = 1;
		}
	...
	public static function OnBeforeIntantMessangerChatAdd(\Thurly\Main\Entity\Event $event)
	{
		$result = new \Thurly\Main\Entity\EventResult();
	...
	{
		return Thurly\Crm\Integration\ThurlyOSEmail::addSignature($message, $contentType);
	}
	...
	public static function PrepareDescriptionFields(array &$fields, array $options = null)
	{
		...
		if($descriptionType === \CCrmContentType::Html)
		{
			...
			$fields['DESCRIPTION_RAW'] = html_entity_decode(
				strip_tags(
...
/**
 * @deprecated Please use \Thurly\Crm\Integration\StorageType
 */
...
class CCrmActivityEmailSender
{
	...
	public static function TrySendEmail($ID, &$arFields, &$arErrors)
	{
		...
		{
			$res = \Thurly\Mail\MailboxTable::getList(array(
			...
			$defaultFrom = \Thurly\Mail\User::getDefaultEmailFrom();
		}
		...
		if (!empty($userImap['need_sync']) || !empty($crmImap['need_sync']))
		{
			...
			class_exists('Thurly\Mail\Helper');

			$rcpt = '';
			foreach ($to as $item)
				$rcpt[] = \Thurly\Mail\DummyMail::encodeHeaderFrom($item, SITE_CHARSET);
			$rcpt = join(', ', $rcpt);

			$outgoing = new \Thurly\Mail\DummyMail(array(
				'CONTENT_TYPE' => 'html',
			...
			if (!empty($userImap['need_sync']))
				\Thurly\Mail\Helper::addImapMessage($userImap, (string) $outgoing, $err);
			if (!empty($crmImap['need_sync']))
				\Thurly\Mail\Helper::addImapMessage($crmImap, (string) $outgoing, $err);
		}
