...
class CCrmTemplateMapper extends CCrmTemplateMapperBase
{
		...
		elseif($typeID === CCrmOwnerType::System)
		{
			switch ($fieldName)
			{
				...
				case 'PERSONAL_MOBILE':
					$result = \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($fields[$fieldName])->format();
					break;
			}
		}
			...
			else if (array_key_exists($fieldName, $entityInfo['FM']))
			{
				...
				switch ($fieldName)
				{
					...
					case 'FM_PHONE':
						...
						{
							return \Thurly\Main\PhoneNumber\Parser::getInstance()->parse($value)->format();
	...
	private static function MapHtmlFieldAsBbCode(&$fields, $fieldID)
	{
		...
		return \Thurly\Crm\Format\TextHelper::convertHtmlToBbCode($fields[$fieldID]);
	}
