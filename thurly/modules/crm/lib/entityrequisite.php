...
use Thurly\Crm\Requisite\EntityLink;
use Thurly\Main;
...
class EntityRequisite
{
						...
						&& !empty($data['REQUISITE_DATA_SIGN']))
					{
						...
						if ($isValidData)
						{
							...
							if ($requisiteId === 0)
							{
								...
								try
								{
									$requisiteData = \Thurly\Main\Web\Json::decode($data['REQUISITE_DATA']);
								}
	...
	public function getFieldsTitles($countryId = 0)
	{
		...
		Loc::loadMessages(Main\Application::getDocumentRoot().'/thurly/modules/crm/lib/requisite.php');
			...
			foreach (array_keys($countryIds) as $countryId)
			{
				...
				if (!empty($langId))
				{
					$messages = Loc::loadLanguageFile(
						Main\Application::getDocumentRoot().'/thurly/modules/crm/lib/requisite.php',
						$langId
					);
	...
	public static function installDefaultPresets()
	{
		...
		$thurlyosPath = Main\Application::getDocumentRoot().'/thurly/modules/thurlyos/';
		$thurlyos = Main\IO\Directory::isDirectoryExists($thurlyosPath);
		unset($thurlyosPath);
		$languageId = '';
		if ($thurlyos)
		{
