<?php
namespace Thurly\Crm\Widget\Data\Activity;
use Thurly\Main;
...
//						ActivityChannelStatistics::GROUP_BY_USER,
		...
		if (\Thurly\Crm\Integration\Channel\VoxImplantTracker::getInstance()->isEnabled() &&
			\Thurly\Crm\Integration\Channel\VoxImplantTracker::getInstance()->isInUse())
		{
			$deleteItems = array(
				\Thurly\Crm\Integration\Channel\ChannelType::VOXIMPLANT_NAME => 0,
				\Thurly\Crm\Integration\Channel\ChannelType::VOXIMPLANT_NAME.'|'.\CVoxImplantConfig::LINK_BASE_NUMBER => 0);
		...
		else if (\Thurly\Crm\Integration\Channel\VoxImplantTracker::getInstance()->isEnabled())
		{
			unset($items[\Thurly\Crm\Integration\Channel\ChannelType::VOXIMPLANT_NAME]);
		}
			...
			if ($item = reset($items))
			{
				...
				do
				{
							...
							isset($item['IS_IN_USE']) && !$item['IS_IN_USE']))
					{
						...
						{
							$inactiveItems[] = $item;
							if ($groupID !== \Thurly\Crm\Integration\Channel\VoxImplantTracker::GROUP_ID)
							{
