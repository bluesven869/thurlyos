...
class CompanyActivityStatisticEntry
{
	/**
	* @return array|null
	*/
	...
	public static function register($ownerID, array $entityFields = null, array $options = null)
	{
		...
		if ($value === 0)
		{
			CompanyActivityStatisticsTable::delete(array(
				'OWNER_ID' => $ownerID,
				'DEADLINE_DATE' => $date,
		...
		if(is_array($present))
		{
			...
			{
				CompanyActivityStatisticsTable::synchronize(
					$ownerID,
		...
		CompanyActivityStatisticsTable::upsert($data);
	...
	*/
	public static function unregister($ownerID)
	{
		...
		CompanyActivityStatisticsTable::deleteByOwner($ownerID);
	}
	/**
	* @return boolean
	*/
