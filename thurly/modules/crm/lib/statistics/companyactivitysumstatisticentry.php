...
use Thurly\Crm;
use Thurly\Crm\Statistics\Entity\CompanyActivitySumStatisticsTable;
class CompanyActivitySumStatisticEntry
{
	/**
	* @return array|null
	*/
	...
	public static function register($ownerID, array $entityFields = null, array $options = null)
	{
		...
		if ($sumTotal === 0)
		{
			CompanyActivitySumStatisticsTable::delete(array(
				'OWNER_ID' => $ownerID,
				'DEADLINE_DATE' => $date,
		...
		if(is_array($present))
		{
			...
			{
				CompanyActivitySumStatisticsTable::synchronize(
					$ownerID,
		...
		CompanyActivitySumStatisticsTable::upsert($data);
	...
	*/
	public static function unregister($ownerID)
	{
		...
		CompanyActivitySumStatisticsTable::deleteByOwner($ownerID);
	}
	/**
	* @return boolean
	*/
