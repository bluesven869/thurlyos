...
use Thurly\Crm;
use Thurly\Crm\Statistics\Entity\ContactActivityStreamStatisticsTable;
class ContactActivityStreamStatisticEntry
{
	/**
	* @return array|null
	*/
	...
	public static function register($ownerID, array $entityFields = null, array $options = null)
	{
		...
		{
			ContactActivityStreamStatisticsTable::delete(array(
				'OWNER_ID' => $ownerID,
				'DEADLINE_DATE' => $date,
		...
		if(is_array($present))
		{
			...
			if($responsibleID !== (int)$present['RESPONSIBLE_ID'])
			{
				ContactActivityStreamStatisticsTable::synchronize(
					$ownerID,
		...
		ContactActivityStreamStatisticsTable::upsert($data);
	...
	*/
	public static function unregister($ownerID)
	{
		...
		ContactActivityStreamStatisticsTable::deleteByOwner($ownerID);
	}
	/**
	* @return boolean
	*/
