...
use Thurly\Crm;
use Thurly\Crm\Statistics\Entity\ContactGrowthStatisticsTable;
class ContactGrowthStatisticEntry
{
	/**
	 * @param $ownerID
	 * @param Date $date
	...
	public static function register($ownerID, array $entityFields = null, array $options = null)
	{
		...
		if(is_array($present))
		{
			...
			if($responsibleID !== (int)$present['RESPONSIBLE_ID'])
			{
				ContactGrowthStatisticsTable::synchronize(
					$ownerID,
		...
		ContactGrowthStatisticsTable::upsert($data);
	...
	*/
	public static function unregister($ownerID)
	{
		...
		ContactGrowthStatisticsTable::deleteByOwner($ownerID);
	}
	/**
	* @return boolean
	*/
