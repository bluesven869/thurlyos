...
use Thurly\Crm;
use Thurly\Crm\Statistics\Entity\ContactActivityStatusStatisticsTable;
class ContactActivityStatusStatisticEntry
{
	/**
	* @return array|null
	*/
	...
	public static function register($ownerID, array $entityFields = null, array $options = null)
	{
		...
		{
			ContactActivityStatusStatisticsTable::delete(array(
				'OWNER_ID' => $ownerID,
				'DEADLINE_DATE' => $date,
		...
		if(is_array($present))
		{
			...
			if($responsibleID !== (int)$present['RESPONSIBLE_ID'])
			{
				ContactActivityStatusStatisticsTable::synchronize(
					$ownerID,
		...
		ContactActivityStatusStatisticsTable::upsert($data);
	...
	*/
	public static function unregister($ownerID)
	{
		...
		ContactActivityStatusStatisticsTable::deleteByOwner($ownerID);
	}
	/**
	* @return boolean
	*/
