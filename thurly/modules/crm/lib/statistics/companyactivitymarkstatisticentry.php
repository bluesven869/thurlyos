...
class CompanyActivityMarkStatisticEntry
{
	/**
	 * @param $ownerID
	 * @param Date $date
	...
	public static function register($ownerID, array $entityFields = null, array $options = null)
	{
		...
		{
			CompanyActivityMarkStatisticsTable::delete(array(
				'OWNER_ID' => $ownerID,
				'DEADLINE_DATE' => $date,
		...
		if(is_array($present))
		{
			...
			if($responsibleID !== (int)$present['RESPONSIBLE_ID'])
			{
				CompanyActivityMarkStatisticsTable::synchronize(
					$ownerID,
		...
		CompanyActivityMarkStatisticsTable::upsert($data);
	...
	*/
	public static function unregister($ownerID)
	{
		...
		CompanyActivityMarkStatisticsTable::deleteByOwner($ownerID);
	}
	/**
	* @return boolean
	*/
