...
class OpenLine extends Base
{
	...
	public static function isActive()
	{
		if (is_null(static::$isActive))
		{
			...
			if ($active)
			{
				static::$activeLine = \Thurly\ImOpenLines\Model\ConfigTable::getCount(array('=TEMPORARY' => 'N'));
	...
	public static function getStatusAnchor()
	{
		...
		return array(
			'TEXT' => $text,
			'URL' => $included? \Thurly\ImOpenLines\Common::getPublicFolder(): ''
		);
	}
	...
	public static function renderView(array $activity)
	{
		...
		{
			$closeSliderOnClick = 'if (top.BX.ThurlyOS && top.BX.ThurlyOS.Slider) {top.BX.ThurlyOS.Slider.closeAll();};';
		}
	...
	public static function getResultSources()
	{
		...
		return \Thurly\ImConnector\Connector::getListConnector();
	}
