<?php
namespace Thurly\Crm\Pseudoactivity;
...
class WaitEntry
{
	...
	public static function add(array $fields)
	{
		...
		*/
		...
		$ID = $result->getId();
		$fields['ID'] = $ID;
		\Thurly\Crm\Timeline\WaitController::getInstance()->onCreate($ID, array('FIELDS' => $fields));
		...
		while($fields = $dbResult->fetch())
		{
			$results[] = array(
				...
				'TITLE' => Loc::getMessage(
					'CRM_WAIT_INFO_TITLE',
					array('#END_TIME#' => $fields['END_TIME']->format($format))
				)
			);
		}
		return $results;
	...
	public static function update($ID, array $fields)
	{
		...
		if($result->isSuccess())
		{
			\Thurly\Crm\Timeline\WaitController::getInstance()->onModify(
				$ID,
				array(
	...
	public static function complete($ID, $completed)
	{
		return self::update($ID, array('COMPLETED' => $completed ? 'Y' : 'N'));
	}
