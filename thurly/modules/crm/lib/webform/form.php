...
use Thurly\Crm\WebForm\Internals\FieldDependenceTable;
use Thurly\Crm\WebForm\Internals\FormStartEditTable;
use Thurly\Crm\WebForm\Internals\FormTable;
use Thurly\Crm\WebForm\Internals\FieldTable;
use Thurly\Crm\WebForm\Internals\FormViewTable;
use Thurly\Crm\WebForm\Internals\FormCounterTable;
use Thurly\Crm\WebForm\Internals\PresetFieldTable;
...
use Thurly\Main\Entity\Result as EntityResult;
use Thurly\Main\Type\DateTime;
use Thurly\Main\Mail\Event as MailEvent;
...
class Form
{
	...
	public function save($onlyCheck = false)
	{
		...
		foreach($fields as $field)
		{
			...
			if($field['ID'] > 0)
			{
				$fieldResult = Internals\FieldTable::update($field['ID'], $field);
				$newFieldList[] = $field['ID'];
			}
			else
			{
	...
	public function addResult($resultFields, $resultParameters = array())
	{
		...
		foreach($resultFields as $fieldKey => $field)
		{
			...
			foreach($field['items'] as $item)
			{
				...
				$resultProducts[] = array(
					'ID' => is_numeric($item['value']) ? $item['value'] : 0,
					'NAME' => $item['title'],
					'PRICE' => $item['price'],
				);
			}
	...
	public static function onAfterSetOptionCrmWebFormMaxActivated(\Thurly\Main\Event $event)
	{
