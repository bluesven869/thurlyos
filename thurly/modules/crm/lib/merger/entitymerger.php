		...
		foreach($fieldInfos as $fieldID => &$fieldInfo)
		{
			...
			if($typeID === 'file')
			{
				...
				else
				{
					...
					{
						$previousFileIDs = isset($targ[$fieldID]) && is_array($targ[$fieldID]) ? $targ[$fieldID] : array();
						$targ[$fieldID] = array();
						...
						foreach($seed[$fieldID] as $fileID)
						{
							$file = null;
					...
					{
						//HACK: Convert file IDs to file informations for preventing error during UF check.
