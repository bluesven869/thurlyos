...
use Thurly\Main,
	...
	Thurly\Crm\DealRecurTable,
	Thurly\Crm\Automation,
	Thurly\Crm\Timeline\DealRecurringController,
...
class Deal extends Base
{
		...
		try
		{
			...
			foreach ($idListChunks as $idList)
			{
				...
				while ($deal = $dealsData->Fetch())
				{
					...
					$reCalculate = false;
					$resultId = $newDeal->Add($deal, $reCalculate, array('DISABLE_TIMELINE_CREATION' => 'Y'));
					...
					{
						$result->addError(new Main\Error(Loc::getMessage("CRM_RECUR_DEAL_ERROR_GET_EMPTY")));
					}
				}
			}
	...
	public function isAllowedExpose()
	{
		if (Main\Loader::includeModule('thurlyos'))
			return !in_array(\CThurlyOS::getLicenseType(), array('project', 'tf')) || Main\Config\Option::get('crm', 'recurring_deal_enabled', 'N') === 'Y';
