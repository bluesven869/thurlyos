<?php
namespace Thurly\Crm\Requisite\Conversion;
...
class PSRequisiteConverter
{
		...
		if (self::$paySystemData === null)
		{
			...
			$dbPaySystems = \CSalePaySystem::GetList(
				...
				array(
					...
					"!ID" => \Thurly\Sale\PaySystem\Manager::getInnerPaySystemId()
				)
			);
	...
	private static function updateInvoices()
	{
		...
		if (is_array($progressData))
		{
			...
			{
				$countInvoiceUpdated = \Thurly\Sale\Internals\OrderTable::getCount(
					array(
						'=PAY_SYSTEM_ID' => array_keys(self::$psEntityTreeMap),
						'<ID' => $nextInvoiceId
					)
			...
			{
				$countInvoice = \Thurly\Sale\Internals\OrderTable::getCount(
					array(
						'=PAY_SYSTEM_ID' => array_keys(self::$psEntityTreeMap)
					)
				);
		...
		$res = \Thurly\Sale\Internals\OrderTable::getList(
			array(
				'order' => array('ID' => 'ASC'),
				'filter' => array(
					'=PAY_SYSTEM_ID' => array_keys(self::$psEntityTreeMap),
	...
	private static function updateInvoice($info)
	{
		if (is_array(self::$psEntityTreeMap) && is_array($info))
		{
			...
			if ($id > 0 && $psId > 0 && is_array(self::$psEntityTreeMap[$psId]))
			{
				...
				if ($myCompanyId > 0)
				{
					\Thurly\Sale\Internals\OrderTable::update($id, array('UF_MYCOMPANY_ID' => $myCompanyId));
				}
				if ($mcRequisiteId > 0)
				{
