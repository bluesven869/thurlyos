...
class ImportHelper
{
	...
	public static function prepareEntityImportRequisiteInfo($entityTypeId, $options = array())
	{
		...
		if (EntityRequisite::checkEntityType($entityTypeId))
		{
			...
			$fieldList = $preset->getSettingsFieldsOfPresets(\Thurly\Crm\EntityPreset::Requisite, 'active', $opts);
			unset($opts);
	...
	public function __construct($entityTypeId, $headerIndex, $headerInfo, $options)
	{
		...
		if (!is_array($headerIndex) || empty($headerIndex))
			throw new Main\ArgumentException('Incorrect headers index.', 'headerIndex');

		if (!is_array($headerInfo) || empty($headerInfo))
			throw new Main\ArgumentException('Incorrect headers info.', 'headerInfo');
		...
		// check key index
		$keyIndexSpecified = false;
		foreach ($this->entityKeyFields as $fieldName)
		{
			if (isset($headerIndex[$fieldName]))
			{
				$keyIndexSpecified = true;
				break;
			}
		}
		if (!$keyIndexSpecified)
			throw new Main\ArgumentException('No index is specified for the key fields', 'headerIndex');
