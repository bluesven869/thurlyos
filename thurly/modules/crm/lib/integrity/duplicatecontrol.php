...
class DuplicateControl
{
	...
	public function save()
	{
		...
		\Thurly\Main\Config\Option::delete('crm', array('name'=>'dup_ctrl'));
		\Thurly\Main\Config\Option::set('crm', 'dup_ctrl', serialize(self::$currentSettings));
	}
	private static function loadCurrentSettings()
	{
		if(self::$currentSettings === null)
		{
			$s = \Thurly\Main\Config\Option::get('crm', 'dup_ctrl');
			if(is_string($s) && $s !== '')
			{
				$ary = unserialize($s);
				if(is_array($ary))
