...
class SupervisorTable extends Entity\DataManager
{
		...
		)
		{
			...
			if (!empty($users))
			{
				...
				if (!empty($send['users']))
				{
					...
					\Thurly\Pull\Event::add($send['users'], array(
						'module_id' => 'crm',
						'command' => $command,
						'params' => $send['info']
					));
	...
	protected static function buildItem(array $row, $type)
	{
		...
		if ($row['CONTACT_ID'] > 0)
		{
			...
			while ($fm = $res->fetch())
			{
				...
				if (empty($langMess))
				{
					Loc::loadMessages('/thurly/components/thurly/crm.kanban/class.php');
					$langMess = array(
	...
	protected static function getRedefinedPriceField($type)
	{
		...
		{
			$prices[$type] = '';
			$slots = \Thurly\Crm\Statistics\StatisticEntryManager::prepareSlotBingingData($type .  '_SUM_STATS');
