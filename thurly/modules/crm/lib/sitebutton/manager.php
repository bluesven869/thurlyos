...
namespace Thurly\Crm\SiteButton;
...
use Thurly\Imopenlines;

Loc::loadMessages(__FILE__);

/**
 * Class Manager
 * @package Thurly\Crm\SiteButton
 */
class Manager
{
	...
	public static function getLanguages()
	{
		...
		if (ModuleManager::isModuleInstalled("thurlyos"))
		{
			...
			{
				$list = \Thurly\Main\Text\Encoding::convertEncoding($b24Languages, 'UTF-8', SITE_CHARSET);
			}
		}
		else
		{
			$langDir = Application::getDocumentRoot() . '/thurly/modules/crm/lang/';
	...
	public static function updateScriptCacheAgent($fromButtonId = null)
	{
		...
		if ($resultButtonId)
		{
			return '\\Thurly\\Crm\\SiteButton\\Manager::updateScriptCacheAgent(' . $resultButtonId . ');';
		}
		else
		{
			return '';
	...
	public static function onImConnectorChange()
	{
		...
		$isAdded = true;
		\CAgent::addAgent('\\Thurly\\Crm\\SiteButton\\Manager::updateScriptCacheAgent();', "crm", "N", 60, "", "Y", \ConvertTimeStamp(time()+\CTimeZone::getOffset(), "FULL"));
	}
	...
	public static function getList($params = array('order' => array('ID' => 'DESC'), 'cache' => array('ttl' => 36000)))
	{
		...
		$domain = \Thurly\Crm\WebForm\Script::getDomain();
		while($buttonData = $buttonDb->fetch())
		{
			$button = new Button();
			$button->loadByData($buttonData);
	...
	{
		return \Thurly\Crm\WebForm\Manager::isInUse('Y');
	}
	...
	public static function canUseMultiLines()
	{
		...
		return Imopenlines\Limit::getLinesLimit() == 0;
	}
	 ...
	 * @return bool
	 */
	public static function onThurlyOSLicenseChange($licenseType)
	{
		if ($licenseType)
		{
			$isChanged = false;
			if(!self::canRemoveCopyright())
			{
				...
				if ($isChanged)
				{
					\CAgent::addAgent('\\Thurly\\Crm\\SiteButton\\Manager::updateScriptCacheAgent();', "crm", "N", 60, "", "Y", \ConvertTimeStamp(time()+\CTimeZone::getOffset()+60, "FULL"));
				}
			}
		}
	}
