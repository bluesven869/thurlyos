...
class Field
{
	...
	public static function prepareFieldDataForFilter(array $field)
	{
		...
		elseif($field['TYPE'] == 'E')
		{
			$result = array(
				...
				'type' => 'custom_entity',
				'filterable' => '',
				'customFilter' => array('Thurly\Iblock\Helpers\Filter\Property', 'addFilter')
			);
		}
	...
	protected static function renderCustomDefaultValueDiskFile(array $field)
	{
		...
		foreach($field['VALUE'] as $value)
		{
			list($type, $realId) = \Thurly\Disk\Uf\FileUserType::detectType($value);
			if($type == \Thurly\Disk\Uf\FileUserType::TYPE_NEW_OBJECT)
			{
				$fileModel = \Thurly\Disk\File::loadById($realId, array('STORAGE'));
				if(!$fileModel)
				{
					return '';
				}
