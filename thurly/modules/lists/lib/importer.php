 ...
 *	{
 *      $id = iblock ID
 *		$datum = \Thurly\Lists\Importer::export($id);
 *
 ...
 *		fclose($f);
 *
 *      \Thurly\Lists\Importer::import("iblock type", $datum);
 *  }
 *
 */
class Importer
{
	...
	{
		$iblockType = Main\Config\Option::get("lists", "livefeed_iblock_type_id", "thurly_processes");
		if (empty($iblockType))
			$iblockType = "thurly_processes";
	...
	protected static function getDocumentType($iblockType, $iblockId)
	{
		if ($iblockType == static::getIBlockType())
			...
			$documentType = array('lists', 'Thurly\Lists\BizprocDocumentLists', 'iblock_'.$iblockId);
	...
	const PATH = "/thurly/modules/lists/install/bizproc/process/";
	const PATH_USER_PROCESSES = "/thurly/lists/processes/";
	...
	protected static function createIBlockType()
	{
		...
		if (!$res)
		{
			...
			$con->queryExecute("
				...
				select 'thurly_processes', p.GROUP_ID
				from
					b_lists_permission p
					left join b_lists_permission p2 on p2.GROUP_ID = p.GROUP_ID and p2.IBLOCK_TYPE_ID = 'thurly_processes'
				where
		...
		Main\Config\Option::set("lists", "livefeed_iblock_type_id", "thurly_processes");
	}
	...
	public static function onAgent($lang)
	{
		...
		{
			return '\Thurly\Lists\Importer::onAgent("'.$lang.'");';
		}
	}
	...
	public static function migrateList($id)
	{
		...
		\CBPDocument::MigrateDocumentType(
			array("lists", 'Thurly\Lists\BizprocDocumentLists', "iblock_".$id),
