...
class BizprocDocument extends CIBlockDocument
{
	...
	public static function generateDocumentComplexType($iblockType, $iblockId)
	{
		if($iblockType == COption::GetOptionString("lists", "livefeed_iblock_type_id"))
			...
			return array('lists', 'Thurly\Lists\BizprocDocumentLists', self::generateDocumentType($iblockId));
	}
	...
	public static function getDocumentComplexId($iblockType, $documentId)
	{
		if($iblockType == COption::getOptionString("lists", "livefeed_iblock_type_id"))
			...
			return array('lists', 'Thurly\Lists\BizprocDocumentLists', $documentId);
	}
				...
				foreach ($propertyArray as $v)
				{
					...
					if ($userArray)
					{
						...
						$result['PROPERTY_'.$propertyId.'_printable'][intval($v)] =
							"[url=/thurly/tools/bizproc_show_file.php?f=".
				...
				if (is_array($fieldValue))
				{
					...
					foreach ($fieldValue as $r)
					{
						...
						if ($img = $imgQuery->fetch())
							$result[] = "[url=/thurly/tools/bizproc_show_file.php?f=".urlencode($img["FILE_NAME"])."&i=".$r."&h=".md5($img["SUBDIR"])."]".htmlspecialcharsbx($img["ORIGINAL_NAME"])."[/url]";
					}
				}
				else
				{
					...
					if ($img = $imgQuery->fetch())
						$result = "[url=/thurly/tools/bizproc_show_file.php?f=".urlencode($img["FILE_NAME"])."&i=".$fieldValue."&h=".md5($img["SUBDIR"])."]".htmlspecialcharsbx($img["ORIGINAL_NAME"])."[/url]";
				}
				break;
