...
Class lists extends CModule
{
	...
	function InstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/db/".strtolower($DB->type)."/install.sql");
		}
		...
		else
		{
			...
			RegisterModuleDependences('rest', 'onRestServiceBuildDescription', 'lists', '\Thurly\Lists\Rest\RestService', 'onRestServiceBuildDescription');
	...
	function installDemoData()
	{
		...
		unset($currentPermissions["thurly_processes"]);
		...
		if (empty($currentPermissions))
		{
			...
			CLists::SetPermission('thurly_processes', array(1));
		}
		...
		\Thurly\Lists\Importer::installProcesses($defaultLang);
		\Thurly\Main\Config\Option::set("lists", "livefeed_url", "/bizproc/processes/");
	...
	function UnInstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/db/".strtolower($DB->type)."/uninstall.sql");
		}
		...
		UnRegisterModuleDependences('rest', 'onRestServiceBuildDescription', 'lists', '\Thurly\Lists\Rest\RestService', 'onRestServiceBuildDescription');
	...
	function InstallFiles($arParams = array())
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin", true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", True, True);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/images", $_SERVER["DOCUMENT_ROOT"]."/thurly/images/lists", True, True);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/js", $_SERVER["DOCUMENT_ROOT"]."/thurly/js", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/activities", $_SERVER["DOCUMENT_ROOT"]."/thurly/activities", true, true);
		}
		return true;
	}
	...
	function UnInstallFiles()
	{
		if($_ENV["COMPUTERNAME"]!='BX')
		{
			DeleteDirFilesEx("/thurly/images/lists/");
			DeleteDirFilesEx("/thurly/js/lists/");
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if(!CBXFeatures::IsFeatureEditable("Lists"))
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("LISTS_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/step2.php");
		}
		elseif($step < 2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage("LISTS_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/step1.php");
		}
		elseif($step==2)
		{
	...
	function DoUninstall()
	{
		...
		if($USER->IsAdmin())
		{
			...
			if($step < 2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("LISTS_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/unstep1.php");
			}
			elseif($step == 2)
			{
				...
				$GLOBALS["errors"] = $this->errors;
				$APPLICATION->IncludeAdminFile(GetMessage("LISTS_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/lists/install/unstep2.php");
			}
		}
	}
