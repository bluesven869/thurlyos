...
Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/js/lists/css/autorun_progress_bar.css');
Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/lists/js/autorun_progress_bar.js');

if($arResult["PROCESSES"] && $arResult["USE_COMMENTS"])
	\Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/bizproc/tools.js');
...
{
	if($icons = $APPLICATION->IncludeComponent('thurly:sharepoint.link', '', array(
...
foreach($arResult["FILTER_CUSTOM_ENTITY"] as $fieldType => $listField)
{
	...
	if($filterId)
	{
		echo Thurly\Iblock\Helpers\Filter\Property::render($filterId, $fieldType, $listField);
	}
}
	...
	<script>
		BX.ready(function(){
			...
			var manager = BX.Lists.AutorunProcessManager.create("rebuildSeachableContent",
				{
					serviceUrl: "<?='/thurly/components/thurly/lists.list/ajax.php'?>",
					ajaxAction: "rebuildSeachableContent",
					dataForAjax: <?=Thurly\Main\Web\Json::encode($dataForAjax)?>,
...
<script type="text/javascript">
	BX.ready(function(){
		BX.Lists['<?= $arResult['JS_OBJECT'] ?>'] = new BX.Lists.ListClass({
			...
			listAction: <?=\Thurly\Main\Web\Json::encode($listAction)?>,
			listActionAdd: <?=\Thurly\Main\Web\Json::encode($listActionAdd)?>,
