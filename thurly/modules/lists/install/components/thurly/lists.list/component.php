...
$grid_options = new Thurly\Main\Grid\Options($arResult["GRID_ID"]);
...
foreach($listFields as $fieldId => $field)
{
	...
	$preparedField = Thurly\Lists\Field::prepareFieldDataForFilter($field);
...
while($obElement = $rsElements->GetNextElement())
{
	...
	foreach($arResult["FIELDS"] as $fieldId => $field)
	{
		...
		$columns[$fieldId] = \Thurly\Lists\Field::renderField($field);
	}
	...
	if($arResult["BIZPROC"] == "Y")
	{
		...
		foreach($arDocumentTemplates as $arWorkflowTemplate)
		{
			...
			))
			{
				...
				$url .= ((strpos($url, "?") === false) ? "?" : "&").thurly_sessid_get();
				$arBPStart[] = array(
