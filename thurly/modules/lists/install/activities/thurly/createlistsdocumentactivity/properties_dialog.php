...
<tr>
	...
	<td width="60%">
		<select name="lists_document_type" onchange="BPCLDA_changeDocumentType(this.value)">
			...
			<?php
			$processesType = COption::getOptionString("lists", "livefeed_iblock_type_id", 'thurly_processes');
			$types = array(
			...
			while ($row = $iterator->fetch()):
				$value = 'lists@'.($type == $processesType ? 'BizprocDocument' : 'Thurly\Lists\BizprocDocumentLists').'@iblock_'.$row['ID'];
...
<script>
	...
	var BPCLDA_changeDocumentType = function(documentType)
	{
		...
		BX.ajax.post(
			'/thurly/tools/bizproc_activity_ajax.php',
			{
