...
if($_SERVER['REQUEST_METHOD'] == "POST" && ($save!="" || $apply!="") && $POST_RIGHT=="W" && check_thurly_sessid())
{
	$arError = array();
	...
	if(!$isUserHavePhpAccess)
	{
		...
		if($ID>0)
		{
			$mailingChainOld = \Thurly\Sender\MailingChainTable::getRowById(array('ID' => $ID));
			if($mailingChainOld)
			{
	...
	switch($SEND_TYPE)
	{
		...
		case 'TIME':
			...
			if(!\Thurly\Main\Type\DateTime::isCorrect($AUTO_SEND_TIME))
				$arError[] = GetMessage("sender_chain_edit_error_time_format");
			else
				$arFields["AUTO_SEND_TIME"] = new \Thurly\Main\Type\DateTime($AUTO_SEND_TIME);

			if ($ID <= 0)
			{
				$arFields["STATUS"] = \Thurly\Sender\MailingChainTable::STATUS_SEND;
			}
			else
			{
				$arMailingChainOld = \Thurly\Sender\MailingChainTable::getRowById(array('ID' => $ID));
				if($arMailingChainOld['STATUS'] == \Thurly\Sender\MailingChainTable::STATUS_NEW)
					$arFields["STATUS"] = \Thurly\Sender\MailingChainTable::STATUS_SEND;
			}
			break;
		case 'REITERATE':
			...
			if ($ID <= 0)
			{
				$arFields["STATUS"] = \Thurly\Sender\MailingChainTable::STATUS_WAIT;
			}
			else
			{
				$arMailingChainOld = \Thurly\Sender\MailingChainTable::getRowById(array('ID' => $ID));
	...
	if(empty($arError))
	{
		...
		if ($ID > 0)
		{
			...
			$mailingUpdateDb = \Thurly\Sender\MailingChainTable::update(array('ID' => $ID), $arFields);
		...
		elseif ($MAILING_ID > 0)
		{
			$mailingAddDb = \Thurly\Sender\MailingChainTable::add($arFields);
			if ($mailingAddDb->isSuccess())
			{
				$ID = $mailingAddDb->getId();
				\Thurly\Sender\MailingChainTable::initPosting($ID);
				$res = ($ID > 0);
			}
			else
			{
	...
	if($res)
	{
		...
		if(is_array($FILES_del))
		{
			...
			{
				$deleteFileDb = \Thurly\Sender\MailingAttachmentTable::getList(array(
				...
				while($arDeleteFile = $deleteFileDb->fetch())
				{
					if(!empty($arDeleteFile))
					{
						...
						\Thurly\Sender\MailingAttachmentTable::delete($arDeleteFile);
					}
				}
			}
		}
		...
		if(array_key_exists("NEW_FILE", $_POST) && is_array($_POST["NEW_FILE"]))
		{
			foreach($_POST["NEW_FILE"] as $index=>$value)
			{
				...
				else
				{
					...
					$docRoot = \Thurly\Main\Application::getDocumentRoot();
		...
		foreach($arFiles as $file)
		{
			if(strlen($file["name"])>0 and intval($file["size"])>0)
			{
				...
				if($fid > 0)
				{
					$resultAddAttachFile = \Thurly\Sender\MailingAttachmentTable::add(array(
						'CHAIN_ID' => $ID,
						'FILE_ID' => $fid
					));
		...
		\Thurly\Sender\MailingChainTable::setEmailFromToList($EMAIL_FROM);
		...
		if(isset($TEMPLATE_ACTION_SAVE) && $TEMPLATE_ACTION_SAVE == 'Y')
		{
			if(!empty($TEMPLATE_ACTION_SAVE_NAME) && !empty($MESSAGE))
			{
				...
				if($TEMPLATE_TYPE && $TEMPLATE_ID)
				{
					...
					$chainTemplate = \Thurly\Sender\Preset\Template::getById($TEMPLATE_TYPE, $TEMPLATE_ID);

					if($chainTemplate && $chainTemplate['HTML'])
					{
						$CONTENT = \Thurly\Fileman\Block\Editor::fillTemplateBySliceContent($chainTemplate['HTML'], $CONTENT);
				...
				$addResult = \Thurly\Sender\TemplateTable::add(array(
				...
				{
					\Thurly\Sender\MailingChainTable::update(
						array('ID' => $ID),
		...
		if($apply!="")
			LocalRedirect("/thurly/admin/sender_mailing_chain_edit.php?MAILING_ID=".$MAILING_ID."&ID=".$ID."&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
			LocalRedirect("/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=".$MAILING_ID."&lang=".LANG);
	}
	else
	{
...
if($ID>0)
{
	$rubric = new CDBResult(\Thurly\Sender\MailingChainTable::getList(array(
		'select' => array('*'),
		'filter' => array('ID' => $ID)
	)));
	...
	if($ID>0)
	{
		$postingDb = \Thurly\Sender\PostingTable::getList(array(
			'select' => array('*'),
		...
		$attachmentFileDb = \Thurly\Sender\MailingAttachmentTable::getList(array(
...
if($MAILING_ID>0)
{
	$mailingSiteDb = \Thurly\Sender\MailingTable::getList(array('select' => array('SITE_ID'), 'filter' => array('ID' => $MAILING_ID)));
	if($mailingSite = $mailingSiteDb->fetch())
	{
		$mailingSiteCharsetDb = \Thurly\Main\SiteTable::getList(array(
...
$templateListHtml = \Thurly\Sender\Preset\Template::getTemplateListHtml('tabControl_layout');
$templateName = '';
$template = \Thurly\Sender\Preset\Template::getById($str_TEMPLATE_TYPE, $str_TEMPLATE_ID);
if($template)
{
...
{
	$statistics = \Thurly\Sender\Stat\Statistics::create()->filter('mailingId', $MAILING_ID);
...
$aMenu = array(
	array(
		...
		"LINK"=>"/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=".$MAILING_ID."&lang=".LANG,
		"ICON"=>"btn_list",
	)
);
if($ID>0 && $POST_RIGHT>="W")
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"/thurly/admin/sender_mailing_chain_edit.php?MAILING_ID=".$MAILING_ID."&lang=".LANG,
		"ICON"=>"btn_new",
	);
	$aMenu[] = array(
		"TEXT"=>GetMessage("sender_chain_edit_action_copy"),
		"TITLE"=>GetMessage("sender_chain_edit_action_copy_title"),
		"LINK"=>"/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=".$MAILING_ID."&ID=".$ID."&action=copy&lang=".LANG."&".thurly_sessid_get(),
		"ICON"=>"btn_copy",
	);
	$aMenu[] = array(
		"TEXT"=>GetMessage("sender_chain_edit_action_del"),
		"TITLE"=>GetMessage("sender_chain_edit_action_del_title"),
		"LINK"=>"javascript:if(confirm('".GetMessage("sender_chain_edit_action_del_confirm")."'))window.location='/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=".$MAILING_ID."&ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
	$aMenu[] = array("SEPARATOR"=>"Y");
}
...
\Thurly\Sender\PostingRecipientTable::setPersonalizeList(\Thurly\Sender\MailingTable::getPersonalizeList($MAILING_ID));
$arMailing = \Thurly\Sender\MailingTable::getRowById($MAILING_ID);
...
?>
	...
	<script>
		function SendTestMailing()
		{
			...
			var url = '/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=<?echo $MAILING_ID?>&ID=<?echo $ID?>&lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&action=js_send';
			ShowWaitWindow();
			BX.ajax.post(
				url,
				data,
...
?>
			...
			<div class="sender-mailing-status">
				...
				<span class="sender-mailing-status-text sender-mailing-status-text-<?=(strtolower($str_STATUS))?>"" >
					<span>
						...
						if(!empty($str_STATUS))
						{
							$arStatus = \Thurly\Sender\MailingChainTable::getStatusList();
					...
					<?if(!empty($str_DATE_SENT) && in_array($str_STATUS, array(\Thurly\Sender\MailingChainTable::STATUS_END))):?>
					...
					<?if(!empty($str_CREATED_BY)):?>
						<span class="sender-mailing-status-creator">
							<?=GetMessage("sender_chain_edit_field_author")?> <?$arUser = \Thurly\Main\UserTable::getRowById($str_CREATED_BY);echo htmlspecialcharsbx($arUser['NAME'].' '.$arUser['LAST_NAME']);?>
						</span>
					<?endif;?>
				</span>
				<span>
					...
					if($ID>0 && $POST_RIGHT>="W"):
						...
						if(\Thurly\Sender\MailingChainTable::isReadyToSend($ID))
						{
							?>
							<input style="margin-left: 80px; margin-right: 15px;" type="button"
								value="<?echo GetMessage("sender_chain_edit_btn_send")?>"
								onclick="window.location='/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=<?=$MAILING_ID?>&ID=<?=$ID?>&action=send&lang=<?=LANGUAGE_ID?>'"
								title="<?echo GetMessage("sender_chain_edit_btn_send_desc")?>" />
							<?

							if($str_STATUS == \Thurly\Sender\MailingChainTable::STATUS_PAUSE)
							{
								?>
								...
								<input style="margin-left: 15px;" type="button"
									...
									onclick="window.location='/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=<?=$MAILING_ID?>&ID=<?=$ID?>&action=stop&lang=<?=LANGUAGE_ID?>'"
									title="<?echo GetMessage("sender_chain_edit_btn_stop_desc")?>" />
								<?
							}

						}
						elseif(\Thurly\Sender\MailingChainTable::isManualSentPartly($ID))
						{
							?>
							<input style="margin-left: 80px; margin-right: 15px;" type="button"
						...
						elseif(in_array($str_STATUS, array(\Thurly\Sender\MailingChainTable::STATUS_SEND, \Thurly\Sender\MailingChainTable::STATUS_WAIT)))
						{
							?>
							<input style="margin-left: 80px;" type="button"
								value="<?echo GetMessage("sender_chain_edit_btn_pause")?>"
								onclick="window.location='/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=<?=$MAILING_ID?>&ID=<?=$ID?>&action=pause&lang=<?=LANGUAGE_ID?>'"
								title="<?echo GetMessage("sender_chain_edit_btn_pause_desc")?>" />
							<?
						}
						elseif($str_STATUS == \Thurly\Sender\MailingChainTable::STATUS_END && \Thurly\Sender\MailingChainTable::canReSendErrorRecipients($ID))
						{
							?>
							<input style="margin-left: 80px;" type="button"
								value="<?echo GetMessage("sender_chain_edit_btn_send_err")?>"
								onclick="window.location='/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=<?=$MAILING_ID?>&ID=<?=$ID?>&action=send_error&lang=<?=LANGUAGE_ID?>'"
					...
					if($ID > 0)
					{
						if(in_array(
							$str_STATUS,
							array(
								\Thurly\Sender\MailingChainTable::STATUS_SEND,
								\Thurly\Sender\MailingChainTable::STATUS_PAUSE
							))
						)
						{
							echo '<span class="sender-mailing-status-creator" style="margin-left: 30px;">(' .
								GetMessage("sender_chain_edit_btn_send_stat_prcnt") . ': ' .
								\Thurly\Sender\PostingTable::getSendPercent($str_POSTING_ID) .
								'%)</span>';
						}
					}
					?>
	...
	<tr class="adm-detail-required-field">
		<td colspan="2">
			<br>
			<?
			$arEmailFromList = \Thurly\Sender\MailingChainTable::getEmailToMeList();
...
?>
	...
	<tr class="hidden-when-show-template-list" <?=(empty($str_MESSAGE) ? 'style="display: none;"' : '')?>>
		...
		<td>
			<?
			$arPersonalizeList = \Thurly\Sender\PostingRecipientTable::getPersonalizeList();
			?>
	...
	<tr class="hidden-when-show-template-list" <?=(empty($str_MESSAGE) ? 'style="display: none;"' : '')?>>
		...
		<td>
			<?
			$arEmailFromList = \Thurly\Sender\MailingChainTable::getEmailFromList();
			?>
	...
	<tr class="adm-detail-required-field hidden-when-show-template-list" <?=(empty($str_MESSAGE) ? 'style="display: none;"' : '')?>>
		<td colspan="2" align="left">
			...
			<?=\Thurly\Sender\TemplateTable::initEditor(array(
				'FIELD_NAME' => 'MESSAGE',
				'FIELD_VALUE' => $str_MESSAGE,
				//'CONTENT_URL' => '/thurly/admin/sender_mailing_chain_admin.php?action=get_template&ID=' . $ID . '&lang=' . LANGUAGE_ID . '&' . thurly_sessid_get(),
	...
	<tr class="hidden-when-show-template-list" <?=(empty($str_MESSAGE) ? 'style="display: none;"' : '')?>>
		...
		<td>
			...
			{
				echo \Thurly\Main\UI\FileInput::createInstance((
					array(
						"name" => "NEW_FILE[n#IND#]",
						"upload" => true,
						"medialib" => true,
...
?>
	...
	<tr>
		<td colspan="2">
			...
			?>
			<?if($ID<=0 || $str_STATUS == \Thurly\Sender\MailingChainTable::STATUS_NEW):?>
					...
					<div class="sender-box-list-item-block">
						<div class="sender-box-list-item-block-item">
							...
							<tr>
								...
								<td>
									<?if($ID>0 && $str_STATUS == \Thurly\Sender\MailingChainTable::STATUS_END):?>
										<?=$str_AUTO_SEND_TIME?>
...
$tabControl->Buttons(array(
	...
	"back_url"=>"/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=".$MAILING_ID."&lang=".LANG,
));
?>
