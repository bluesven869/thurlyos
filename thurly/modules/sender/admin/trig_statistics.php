...
CJSCore::RegisterExt('sender_stat', array(
	'js' => array(
		...
		'/thurly/js/main/amcharts/3.3/funnel.js',
...
{
	$i = 1;
	$chainList = \Thurly\Sender\MailingTable::getChain($MAILING_ID);
	foreach($chainList as $chain)
	{
		...
		$statRawDb = \Thurly\Sender\PostingTable::getList(array(
			'select' => array(
			...
			'runtime' => array(
				new \Thurly\Main\Entity\ExpressionField('CNT', 'SUM(%s)', 'COUNT_SEND_SUCCESS'),
				new \Thurly\Main\Entity\ExpressionField('READ_CNT', 'SUM(%s)', 'COUNT_READ'),
				new \Thurly\Main\Entity\ExpressionField('CLICK_CNT', 'SUM(%s)', 'COUNT_CLICK'),
				new \Thurly\Main\Entity\ExpressionField('UNSUB_CNT', 'SUM(%s)', 'COUNT_UNSUB')
			),
		));
		...
		$statRawDb = \Thurly\Sender\PostingRecipientTable::getList(array(
			'select' => array('ID'),
			'filter' => array(
				'=POSTING.MAILING_CHAIN_ID' => $chain['ID'],
				'=STATUS' => array(
					\Thurly\Sender\PostingRecipientTable::SEND_RESULT_SUCCESS,
					\Thurly\Sender\PostingRecipientTable::SEND_RESULT_DENY,
				),
				'!DATE_DENY' => null
			)
		));
	...
	<script>
		BX.ready(function(){
			var chart = AmCharts.makeChart("chartdiv", {
				"theme": "dark",
				"type": "serial",
				"pathToImages": "/thurly/js/main/amcharts/3.3/images/",
...
<?$oFilter->Begin();?>
	...
	<tr>
		...
		<td valign="middle">
			<?
			$arr = array();
			$mailingChainDb = \Thurly\Sender\MailingChainTable::getList(array(
