...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $POST_RIGHT=="W" && check_thurly_sessid())
{
	...
	if(is_array($CONNECTOR_SETTING) && count($CONNECTOR_SETTING)>0)
	{
		if($ID > 0)
		{
			$groupUpdateDb = \Thurly\Sender\GroupTable::update($ID, $arFields);
	...
	if($res)
	{
		...
		if($apply!="")
			LocalRedirect("/thurly/admin/sender_group_edit.php?ID=".$ID."&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
			LocalRedirect("/thurly/admin/sender_group_admin.php?lang=".LANG);
	}
	else
	{
		if(!empty($arError))
...
if($ID>0)
{
	$rubric = new CDBResult(\Thurly\Sender\GroupTable::getById($ID));
...
if($ID>0)
{
	$groupConnectorDb = \Thurly\Sender\GroupConnectorTable::getList(array('filter'=>array('GROUP_ID'=>$ID)));
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"javascript:if(confirm('".GetMessage("sender_group_del_confirm")."'))window.location='sender_group_admin.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
