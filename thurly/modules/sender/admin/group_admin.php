...
if($lAdmin->EditAction() && $POST_RIGHT=="W")
{
	foreach($FIELDS as $ID=>$arFields)
	{
		...
		$arData = \Thurly\Sender\GroupTable::getRowById($dataPrimary);
		if($arData)
		{
			...
			unset($arData['ID']);
			$dataUpdateDb = \Thurly\Sender\GroupTable::update($dataPrimary, $arData);
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT=="W")
{
	...
	{
		$dataListDb = \Thurly\Sender\GroupTable::getList(array(
			'select' => array('ID'),
			'filter' => $arFilter,
	...
	foreach($arID as $ID)
	{
		...
		switch($_REQUEST['action'])
		{
			case "delete":
				@set_time_limit(0);
				$DB->StartTransaction();
				$dataDeleteDb = \Thurly\Sender\GroupTable::delete($dataPrimary);
			...
			case "deactivate":
				...
				$dataUpdateDb = \Thurly\Sender\GroupTable::update($dataPrimary, $arFields);
				if(!$dataUpdateDb->isSuccess())
				{
					$LAST_ERROR = $dataUpdateDb->getErrorMessages();
					$LAST_ERROR = $LAST_ERROR[0];
...
$groupListDb = \Thurly\Sender\GroupTable::getList(array(
	'filter' => $arFilter,
	'order' => array($by=>$order)
));
...
<tr>
	...
	<tr>
		...
		<td>
			<?
			$arr = array();
			$mailingDb = \Thurly\Sender\MailingTable::getList(array('select'=>array('REFERENCE'=>'NAME','REFERENCE_ID'=>'ID'), 'filter' => array('IS_TRIGGER' => 'N')));
