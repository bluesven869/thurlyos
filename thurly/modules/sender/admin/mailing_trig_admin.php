...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sender/admin/mailing_admin.php');
...
if($lAdmin->EditAction() && $POST_RIGHT=="W")
{
	foreach($FIELDS as $ID=>$arFields)
	{
		...
		$arData = \Thurly\Sender\MailingTable::getRowById($dataPrimary);
		if($arData)
		{
			...
			unset($arData['ID']);
			$dataUpdateDb = \Thurly\Sender\MailingTable::update($dataPrimary, $arData);
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT=="W")
{
	...
	foreach($arID as $ID)
	{
		...
		case "delete":
			@set_time_limit(0);
			$DB->StartTransaction();
			$dataDeleteDb = \Thurly\Sender\MailingTable::delete($dataPrimary);
		...
		case "deactivate":
			...
			$dataUpdateDb = \Thurly\Sender\MailingTable::update($dataPrimary, $arFields);
			if(!$dataUpdateDb->isSuccess())
			{
				$LAST_ERROR = $dataUpdateDb->getErrorMessages();
				$LAST_ERROR = $LAST_ERROR[0];
...
$groupListDb = \Thurly\Sender\MailingTable::getList(array(
