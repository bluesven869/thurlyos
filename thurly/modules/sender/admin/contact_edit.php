...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sender/admin/template_edit.php');
...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $POST_RIGHT=="W" && check_thurly_sessid())
{
	$arError = array();
	...
	if($ID > 0)
	{
		$contactUpdateDb = \Thurly\Sender\ContactTable::update($ID, $arFields);
	...
	{
		$contactAddDb = \Thurly\Sender\ContactTable::add($arFields);
	...
	if($res)
	{
		\Thurly\Sender\ContactListTable::delete(array('CONTACT_ID' => $ID));
		foreach($listsSelected as $listId)
		{
			if (is_numeric($listId))
			{
				\Thurly\Sender\ContactListTable::add(array('CONTACT_ID' => $ID, 'LIST_ID' => $listId));
			}
		}
		...
		foreach($subSelected as $mailingId)
		{
			...
			{
				\Thurly\Sender\MailingSubscriptionTable::addSubscription(array('CONTACT_ID' => $ID, 'MAILING_ID' => $mailingId));
			}
		}
		...
		foreach($unsubSelected as $mailingId)
		{
			if (is_numeric($mailingId))
			{
				\Thurly\Sender\MailingSubscriptionTable::addUnSubscription(array('CONTACT_ID' => $ID, 'MAILING_ID' => $mailingId));
			}
		}

		$mailingDeleteList = array();
		$mailingDeleteDb = \Thurly\Sender\MailingSubscriptionTable::getList(array(
		...
		{
			\Thurly\Sender\MailingSubscriptionTable::delete(array('CONTACT_ID' => $ID, 'MAILING_ID' => $mailingId));
		}
	}
	...
	{
		if($apply!="")
			LocalRedirect("/thurly/admin/sender_contact_edit.php?ID=".$ID."&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
			LocalRedirect("/thurly/admin/sender_contact_admin.php?lang=".LANG);
	}
	else
	{
		if(!empty($arError))
...
if($ID > 0)
{
	$rubric = new CDBResult(\Thurly\Sender\ContactTable::getById($ID));
	if(!$rubric->ExtractFields("str_"))
		$ID=0;
}

$mailingSubList = \Thurly\Sender\MailingTable::getList(array('filter' => array('IS_TRIGGER' => 'N'),'order' => array('SITE_ID' => 'ASC', 'NAME' => 'ASC')))->fetchAll();
$mailingUnSubList = \Thurly\Sender\MailingTable::getList(array('order' => array('SITE_ID' => 'ASC', 'NAME' => 'ASC')))->fetchAll();
$lists = \Thurly\Sender\ListTable::getList(array('order' => array('NAME' => 'ASC')))->fetchAll();
if($ID > 0 )
{
	$listSelectedDb = \Thurly\Sender\ContactListTable::getList(array(
	...
	$subSelectedDb = \Thurly\Sender\MailingSubscriptionTable::getSubscriptionList(array(
		'select' => array('MAILING_ID'),
		'filter' => array('=CONTACT_ID' => $ID),
	));
	...
	$unsubSelectedDb = \Thurly\Sender\MailingSubscriptionTable::getUnSubscriptionList(array(
		'select' => array('MAILING_ID'),
		'filter' => array('=CONTACT_ID' => $ID),
	));
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"javascript:if(confirm('".GetMessage("sender_contact_edit_bnt_del_confirm")."'))window.location='sender_contact_admin.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
	$aMenu[] = array("SEPARATOR"=>"Y");
