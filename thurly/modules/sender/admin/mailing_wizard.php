...
use \Thurly\Sender\MailingChainTable;
use \Thurly\Sender\PostingTable;
use \Thurly\Sender\PostingRecipientTable;
...
if($step=='mailing')
{
	...
	if($REQUEST_METHOD == "POST" && !$isPostedFormProcessed && check_thurly_sessid())
	{
		$NAME = trim($NAME);
		if($MAILING_TYPE == 'NEW')
		{
			...
			$mailingAddDb = \Thurly\Sender\MailingTable::add($arFields);
		...
		{
			$mailing = \Thurly\Sender\MailingTable::getRowById($MAILING_ID);
			if(!$mailing)
	...
	$arMailingList = array();
	$groupDb = \Thurly\Sender\MailingTable::getList(array(
...
if($step=='group')
{
	...
	if(!isset($group_create) && $REQUEST_METHOD == "POST" && !$isPostedFormProcessed && check_thurly_sessid())
	{
		$res = false;
		$NAME = trim($NAME);
		if(isset($popup_create_group) && $popup_create_group == 'Y')
		{
			require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/prolog_admin_js.php");
		...
		{
			$groupAddDb = \Thurly\Sender\GroupTable::add($arFields);
		...
		if($res)
		{
			if(is_array($CONNECTOR_SETTING))
			{
				...
				\Thurly\Sender\GroupConnectorTable::delete(array('GROUP_ID' => $ID));
				...
				foreach ($arEndpointList as $endpoint)
				{
					...
					if ($connector)
					{
						...
						$groupConnectorAddDb = \Thurly\Sender\GroupConnectorTable::add($arGroupConnectorAdd);
				...
				\Thurly\Sender\GroupTable::update($ID, array('ADDRESS_COUNT' => $groupConnectorsDataCount));
			}
		}
		...
		if(empty($arError))
		{
			...
			if(isset($popup_create_group) && $popup_create_group == 'Y')
			{
				...
				<?
				require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/epilog_admin_js.php");
				exit();
			}
			else
			{
	...
	{
		$arConnectorSettings = \Thurly\Sender\ConnectorManager::getFieldsFromEndpoint($endpointList);
	}

	$arAvailableConnectors = array();
	$arExistedConnectors = array();
	$arConnector = \Thurly\Sender\ConnectorManager::getConnectorList();
	/** @var \Thurly\Sender\Connector $connector */
...
if($step=='mailing_group')
{
	...
	if($REQUEST_METHOD == "POST" && !$isPostedFormProcessed && check_thurly_sessid())
	{
		...
		if($MAILING_ID>0)
		{
			...
			\Thurly\Sender\MailingGroupTable::delete(array('MAILING_ID' => $ID));
			foreach($GROUP as $arGroup)
			{
				\Thurly\Sender\MailingGroupTable::add($arGroup);
			}
		}
	...
	else
	{
		...
		$groupDb = \Thurly\Sender\MailingGroupTable::getList(array(
	...
	$GROUP_EXIST = array();
	$groupDb = \Thurly\Sender\GroupTable::getList(array(
...
if($step=='trig_mailing')
{
	...
	if($REQUEST_METHOD == "POST" && !$isPostedFormProcessed && check_thurly_sessid())
	{
		...
		{
			$presetMailingList = \Thurly\Sender\MailingTable::getPresetMailingList(array('CODE' => $MAILING_TEMPLATE_CODE));
			...
			if(!empty($presetMailing))
			{
				...
				$result = new \Thurly\Main\Entity\Result;
				\Thurly\Sender\MailingTable::checkFieldsChain($result, null, $chainList);
				if(!$result->isSuccess())
		...
		if(empty($arError))
		{
			if($MAILING_ID > 0)
			{
				$updateDb = \Thurly\Sender\MailingTable::update($MAILING_ID, $arFields);
			...
			{
				$arFields['ACTIVE'] = 'N';
				$addDb = \Thurly\Sender\MailingTable::add($arFields);
				if($addDb->isSuccess())
				{
					$MAILING_ID = $addDb->getId();
					$resultDb = \Thurly\Sender\MailingTable::updateChain($MAILING_ID, $chainList);
					$resultDb->isSuccess();
				}
				else
	...
	else
	{
		ClearVars();
		$str_SORT = 100;
		$rubric = new CDBResult(\Thurly\Sender\MailingTable::getById($MAILING_ID));
	...
	$presetMailingList = \Thurly\Sender\MailingTable::getPresetMailingList();
}
...
if($step=='trig_mailing_group')
{
	...
	$triggerList = \Thurly\Sender\TriggerManager::getList();
	...
	foreach($triggerList as $trigger)
	{
		foreach(array('START', 'END') as $type)
		{
			...
			$triggerListForJS[$type][$trigger->getId()] = \Thurly\Sender\TriggerSettings::getArrayFromTrigger($trigger);
	...
	if(!empty($mailing['TRIGGER_FIELDS']))
	{
		foreach($triggerListExists as $type => $value)
		{
			...
			$trigger = \Thurly\Sender\TriggerManager::getOnce($mailing['TRIGGER_FIELDS'][$type]);
			if ($trigger)
			{
	...
	if($REQUEST_METHOD == "POST" && !$isPostedFormProcessed && check_thurly_sessid())
	{
		...
		foreach($triggerListExists as $type => $value)
		{
			$trigger = \Thurly\Sender\TriggerManager::getOnce($ENDPOINT[$type]);
			if($trigger)
			{
				$triggerListExists[$type] = $ENDPOINT[$type] + \Thurly\Sender\TriggerSettings::getArrayFromTrigger($trigger);
			}
		}

		$updateDb = \Thurly\Sender\MailingTable::update($MAILING_ID, array('TRIGGER_FIELDS' => $triggerListExists));
		if(!$updateDb->isSuccess())
			$arError = array_merge($arError, $updateDb->getErrorMessages());
		...
		{
			$isPostedFormProcessed = true;
			LocalRedirect('/thurly/admin/sender_mailing_trig_edit.php?ID=' . $MAILING_ID . "&lang=".LANGUAGE_ID);
		}
		...
		<table class="adm-detail-content-table edit-table">
			...
			<tr>
				<td colspan="2">
					...
					?>
					<?if($str_STATUS != \Thurly\Sender\MailingChainTable::STATUS_SEND):?>
						<div class="sender-box-selector">
