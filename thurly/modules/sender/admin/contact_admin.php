...
if($_REQUEST["action"]=="js_pull" && check_thurly_sessid() && $POST_RIGHT>="W")
{
	...
	if($connector)
	{
		...
		else
		{
			...
			$arPullResult = \Thurly\Sender\ContactTable::addFromConnector($connector, $connectorDataPage, $timeout);
...
if($lAdmin->EditAction() && $POST_RIGHT=="W")
{
	foreach($FIELDS as $ID=>$arFields)
	{
		...
		$arData = \Thurly\Sender\ContactTable::getRowById($dataPrimary);
		if($arData)
		{
			...
			unset($arData['ID']);
			$dataUpdateDb = \Thurly\Sender\ContactTable::update($dataPrimary, $arData);
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT=="W")
{
	...
	{
		$dataDb = \Thurly\Sender\ContactTable::getList(array(
			'select' => array('ID'),
			'filter' => $arFilter,
	...
	foreach($arID as $ID)
	{
		...
		switch($_REQUEST['action'])
		{
			case "delete":
				...
				$connection = \Thurly\Main\Application::getInstance()->getConnection();
				$connection->startTransaction();
				$dataDeleteDb = \Thurly\Sender\ContactTable::delete($dataPrimary);
...
$nav = new \Thurly\Main\UI\AdminPageNavigation("nav-sender-contact");
$selectParams = array(
...
$contactListDb = \Thurly\Sender\ContactTable::getList($selectParams);
$nav->setRecordCount($contactListDb->getCount());
$lAdmin->setNavigation($nav, \Thurly\Main\Localization\Loc::getMessage("contact_nav"));
...
while($contact = $contactListDb->fetch())
{
	...
	$list = array();
	$listDb = \Thurly\Sender\ListTable::getList(array(
...
$aContext[] = array(
	...
	"LINK"=>"/thurly/admin/sender_list_admin.php?lang=".LANG,
...
$mailingDb = \Thurly\Sender\MailingTable::getList(array('select'=>array('IS_TRIGGER', 'REFERENCE'=>'NAME','REFERENCE_ID'=>'ID')));
...
<tr>
	...
	<td>
		...
		$listDb = \Thurly\Sender\ListTable::getList(array('select'=>array('REFERENCE'=>'NAME','REFERENCE_ID'=>'ID')));
...
if($_REQUEST['action']=="pull"):
	...
	$connectorClassList = \Thurly\Sender\ConnectorManager::getConnectorClassList();
	if(!empty($connectorClassList)):
		...
		<script>
			...
			function MoveProgress()
			{
				...
				var url = '/thurly/admin/sender_contact_admin.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&action=js_pull';
				ShowWaitWindow();
				BX.ajax.post(
					url,
					data,
