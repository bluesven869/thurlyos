...
if (CheckFilter() || $ID>0)
{
	...
	if($find_sent && in_array($find_sent, array_keys(\Thurly\Sender\PostingRecipientTable::getStatusList())))
	{
...
$nav = new \Thurly\Main\UI\AdminPageNavigation("nav-sender-recipient");
$recipientListDb = \Thurly\Sender\PostingRecipientTable::getList(array(
	'select' => array('NAME', 'EMAIL', 'CALC_IS_READ', 'CALC_IS_CLICK', 'CALC_IS_UNSUB'),
	'filter' => $arFilter,
	'runtime' => array(
		new \Thurly\Main\Entity\ExpressionField('CALC_IS_READ', 'MAX(%s)', 'IS_READ'),
		new \Thurly\Main\Entity\ExpressionField('CALC_IS_CLICK', 'MAX(%s)', 'IS_CLICK'),
		new \Thurly\Main\Entity\ExpressionField('CALC_IS_UNSUB', 'MAX(%s)', 'IS_UNSUB'),
	),
...
$lAdmin->setNavigation($nav, \Thurly\Main\Localization\Loc::getMessage("rub_nav"));
...
<tr>
	...
	<td>
		<?
		$arRecipientStatus = \Thurly\Sender\PostingRecipientTable::getStatusList();
		$arr = array(
