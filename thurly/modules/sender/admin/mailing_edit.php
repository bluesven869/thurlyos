...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $POST_RIGHT=="W" && check_thurly_sessid())
{
	...
	if($ID > 0)
	{
		$mailingUpdateDb = \Thurly\Sender\MailingTable::update($ID, $arFields);
	...
	{
		if($apply!="")
			LocalRedirect("/thurly/admin/sender_mailing_edit.php?ID=".$ID."&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
			LocalRedirect("/thurly/admin/sender_mailing_admin.php?lang=".LANG);
	}
	else
	{
		if(!empty($arError))
...
if($ID>0)
{
	$rubric = new CDBResult(\Thurly\Sender\MailingTable::getById($ID));
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"javascript:if(confirm('".GetMessage("sender_mailing_edit_del_confirm")."'))window.location='sender_mailing_admin.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
	$aMenu[] = array("SEPARATOR"=>"Y");
}
