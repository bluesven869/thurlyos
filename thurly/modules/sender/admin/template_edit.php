...
if($REQUEST_METHOD == "POST" && ($save!="" || $apply!="") && $POST_RIGHT=="W" && check_thurly_sessid())
{
	...
	if(!$isUserHavePhpAccess)
	{
		...
		if($ID>0)
		{
			$templateOld = \Thurly\Sender\TemplateTable::getRowById(array('ID' => $ID));
			if($templateOld)
			{
	...
	if($ID > 0)
	{
		$mailingUpdateDb = \Thurly\Sender\TemplateTable::update($ID, $arFields);
	...
	{
		$mailingAddDb = \Thurly\Sender\TemplateTable::add($arFields);
	...
	{
		if($apply!="")
			LocalRedirect("/thurly/admin/sender_template_edit.php?ID=".$ID."&lang=".LANG."&".$tabControl->ActiveTabParam());
		else
			LocalRedirect("/thurly/admin/sender_template_admin.php?lang=".LANG);
	}
	else
	{
		if(!empty($arError))
...
if($ID>0)
{
	$rubric = new CDBResult(\Thurly\Sender\TemplateTable::getById($ID));
...
if($ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"=>"javascript:if(confirm('".GetMessage("sender_tmpl_edit_action_del_confirm")."'))window.location='sender_template_admin.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
	$aMenu[] = array("SEPARATOR"=>"Y");
	...
	<tr class="adm-detail-required-field hidden-when-show-template-list" <?=(empty($str_CONTENT) ? 'style="display: none;"' : '')?>>
		<td colspan="2" align="left">
			...
			<?=\Thurly\Sender\TemplateTable::initEditor(array(
				...
				'CONTENT_URL' => '/thurly/admin/sender_template_admin.php?action=get_template&template_type=USER&template_id=' . $blockTemplateId . '&lang=' . LANGUAGE_ID . '&' . thurly_sessid_get(),
