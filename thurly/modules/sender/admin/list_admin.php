...
if($lAdmin->EditAction() && $POST_RIGHT=="W")
{
	foreach($FIELDS as $ID=>$arFields)
	{
		...
		$arData = \Thurly\Sender\ListTable::getRowById($dataPrimary);
		if($arData)
		{
			...
			unset($arData['ID']);
			$dataUpdateDb = \Thurly\Sender\ListTable::update($dataPrimary, $arData);
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT=="W")
{
	...
	{
		$dataListDb = \Thurly\Sender\ListTable::getList(array(
			'select' => array('ID'),
			'filter' => $arFilter,
	...
	foreach($arID as $ID)
	{
		...
		switch($_REQUEST['action'])
		{
			case "delete":
				@set_time_limit(0);
				$DB->StartTransaction();
				$dataDeleteDb = \Thurly\Sender\ListTable::delete($dataPrimary);
			...
			case "deactivate":
				...
				$dataUpdateDb = \Thurly\Sender\ListTable::update($dataPrimary, $arFields);
				if(!$dataUpdateDb->isSuccess())
				{
					$LAST_ERROR = $dataUpdateDb->getErrorMessages();
					$LAST_ERROR = $LAST_ERROR[0];
