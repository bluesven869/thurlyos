...
if($request->get('action') == 'get_template')
{
	...
	{
		$template = \Thurly\Sender\Preset\Template::getById($templateType, $templateId);
		if ($template)
		{
			echo \Thurly\Fileman\Block\Editor::getHtmlForEditor($template['HTML'], $templateCharset);
		}
	}
...
if($lAdmin->EditAction() && $POST_RIGHT=="W")
{
	foreach($FIELDS as $ID=>$arFields)
	{
		...
		$arData = \Thurly\Sender\TemplateTable::getRowById($dataPrimary);
		if($arData)
		{
			...
			unset($arData['ID']);
			$dataUpdateDb = \Thurly\Sender\TemplateTable::update($dataPrimary, $arData);
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT=="W")
{
	...
	{
		$dataListDb = \Thurly\Sender\MailingTable::getList(array(
			'select' => array('ID'),
			'filter' => $arFilter,
	...
	foreach($arID as $ID)
	{
		...
		case "delete":
			@set_time_limit(0);
			$DB->StartTransaction();
			$dataDeleteDb = \Thurly\Sender\TemplateTable::delete($dataPrimary);
		...
		case "deactivate":
			...
			$dataUpdateDb = \Thurly\Sender\TemplateTable::update($dataPrimary, $arFields);
			if(!$dataUpdateDb->isSuccess())
			{
				$LAST_ERROR = $dataUpdateDb->getErrorMessages();
				$LAST_ERROR = $LAST_ERROR[0];
...
$templateListDb = \Thurly\Sender\TemplateTable::getList(array(
	'select' => array('*'),
	'filter' => $arFilter,
	'order' => array($by=>$order)
));
