...
if($_REQUEST["action"]=="copy" && check_thurly_sessid() && $POST_RIGHT>="W")
{
	$copiedId = \Thurly\Sender\MailingChainTable::copy($ID);
	if ($copiedId)
	{
		\Thurly\Sender\MailingChainTable::update(array('ID' => $copiedId), array('CREATED_BY' => $USER->GetID()));
		LocalRedirect("sender_mailing_chain_edit.php?MAILING_ID=" . $MAILING_ID . "&ID=" . $copiedId . "&mess=copied");
	}
}

if($_REQUEST["action"]=="send_to_me" && check_thurly_sessid() && $POST_RIGHT>="W")
{
	$arResult = array();
	...
	try
	{
		...
		while($mailingChain = $mailingChainDb->fetch())
		{
			...
			foreach($sendToMeAddr as $address)
			{
				...
				if(!empty($address))
				{
					$sendResult = \Thurly\Sender\PostingManager::sendToAddress($mailingChain['ID'], $address);
					if ($sendResult == \Thurly\Sender\PostingManager::SEND_RESULT_SENT)
					{
						$arResult[] = $address;
...
if(check_thurly_sessid() && $POST_RIGHT>="W" && isset($_REQUEST["action"]) && in_array($_REQUEST["action"], array('js_pause', 'js_stop', 'js_send_error')))
{
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/prolog_admin_js.php");
	$message = null;
...
if($_REQUEST["action"]=="js_send" && check_thurly_sessid() && $POST_RIGHT>="W")
{
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/prolog_admin_js.php");
		...
		else
		{
			switch($arMailingChain["STATUS"])
			{
				...
				case MailingChainTable::STATUS_SEND:
					...
					\Thurly\Sender\MailingManager::chainSend($mailingChainId);
					$sendErrors = \Thurly\Sender\MailingManager::getErrors();
					if(empty($sendErrors))
					{
						$messageDetails = '<br><p>' . GetMessage("MAILING_ADM_SENDING_NOTE_LINE1") . '<br>' . GetMessage("MAILING_ADM_SENDING_NOTE_LINE2") . '</p>';
						$message = array(
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT=="W")
{
	...
	{
		$dataListDb = \Thurly\Sender\MailingChainTable::getList(array(
			'select' => array('ID'),
			'filter' => $arFilter,
	...
	foreach($arID as $ID)
	{
		...
		case "delete":
			@set_time_limit(0);
			$DB->StartTransaction();
			$dataDeleteDb = \Thurly\Sender\MailingChainTable::delete($dataPrimary);
...
$groupListDb = \Thurly\Sender\MailingChainTable::getList(array(
	'select' => array(
...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	$row->AddViewField("NAME", '<a href="/thurly/admin/sender_mailing_chain_edit.php?MAILING_ID='.$MAILING_ID.'&ID='.$f_ID.'&amp;lang='.LANG.'">'.$f_NAME.'</a>');
	$arUser = \Thurly\Main\UserTable::getRowById(intval($f_CREATED_BY));
	$row->AddViewField("CREATED_BY", '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$f_CREATED_BY.'">'.htmlspecialcharsbx($arUser['NAME']." ".$arUser['LAST_NAME'])."</a>");
	...
	if ($POST_RIGHT>="W")
	{
		$arActions[] = array(
			...
			"ACTION"=>$lAdmin->ActionRedirect("sender_mailing_chain_admin.php?action=copy&MAILING_ID=".$MAILING_ID."&ID=".$f_ID."&".thurly_sessid_get())
		);
			...
			case MailingChainTable::STATUS_NEW:
				if ($POST_RIGHT>="W")
				{
					$arActions[] = array(
						...
						"ACTION" => $lAdmin->ActionRedirect("/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=".$MAILING_ID."&ID=".$f_ID."&action=send&lang=" . LANGUAGE_ID)
					);
				}
				break;
			...
			case MailingChainTable::STATUS_SEND:
				if ($POST_RIGHT>="W")
				{
					$arActions[] = array(
						...
						"ACTION" => $lAdmin->ActionRedirect("/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=".$MAILING_ID."&ID=".$f_ID."&action=pause&lang=" . LANGUAGE_ID)
					);
				}
				break;
			case MailingChainTable::STATUS_PAUSE:
				if ($POST_RIGHT>="W")
				{
					...
					$arActions[] = array(
						...
						"ACTION" => $lAdmin->ActionRedirect("/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=".$MAILING_ID."&ID=".$f_ID."&action=stop&lang=" . LANGUAGE_ID)
					);
				}
				break;
		}
...
$aContext = array(
	array(
		...
		"LINK"=>"/thurly/admin/sender_mailing_chain_edit.php?MAILING_ID=".$MAILING_ID."&lang=".LANGUAGE_ID,
...
if(in_array($jsAction, array('send', 'pause', 'stop', 'send_error'))):
	...
	if($jsAction == 'send')
	{
		$canSend = \Thurly\Sender\MailingChainTable::isReadyToSend($ID);
		if(!$canSend)
			$canSend = \Thurly\Sender\MailingChainTable::isManualSentPartly($ID);
	}
	...
	if($canSend):
		...
		<script>
			...
			function MoveProgress()
			{
				...
				var url = '/thurly/admin/sender_mailing_chain_admin.php?lang=<?echo LANGUAGE_ID?>&MAILING_ID=<?echo $MAILING_ID?>&ID=<?echo $ID?>&<?echo thurly_sessid_get()?>&action=js_<?=htmlspecialcharsbx($jsAction)?>';
				ShowWaitWindow();
				BX.ajax.post(
					url,
					null,
