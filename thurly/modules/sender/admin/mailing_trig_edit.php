		...
		<div class="sender_letter_container" id="SENDER_TRIGGER_CHAIN_<?=$i?>">
					...
					<?elseif(!empty($chain['ID'])):?>
						...
						<span>
							...
							echo GetMessage("sender_chain_edit_field_created_text", array(
								'%DATE_CREATE%' => htmlspecialcharsbx(is_object($chain['DATE_INSERT']) ? \Thurly\Main\Type\Date::createFromTimestamp($chain['DATE_INSERT']->getTimestamp()) : $chain['DATE_INSERT']),
								'%AUTHOR%' => '<a class="sender_letter_container-author" href="/thurly/admin/user_edit.php?ID='.htmlspecialcharsbx($chain['CREATED_BY']).'&lang='.LANGUAGE_ID.'">'.htmlspecialcharsbx($chain['CREATED_BY_NAME']).' '.htmlspecialcharsbx($chain['CREATED_BY_LAST_NAME']).'</a>',
							));
							?>
						</span>
					<?else:?>
			...
			<div class="sender_letter_container_body" <?=((strlen($chain['SUBJECT'])>0 && strlen($chain['MESSAGE'])>0) ? 'style="display:none;"' : '')?>>
				<div class="sender_letter_container_body_tmpl" id="CHAIN_TEMPLATE_NUM_<?=$i?>" <?=(strlen($chain['MESSAGE'])>0 ? 'style="display:none;"' : '')?>>
					<?=\Thurly\Sender\Preset\Template::getTemplateListHtml('SENDER_TRIGGER_CHAIN_'.$i)?>
				</div>
...
$personalizeList = \Thurly\Sender\MailingTable::getChainPersonalizeList($ID);
\Thurly\Sender\PostingRecipientTable::setPersonalizeList($personalizeList);
if($_REQUEST["action"]=="get_vr" && check_thurly_sessid())
{
	$letterTemplate = array(
		'BODY' => getSenderItemContainer($ID),
		'FIELDS' => array(
			'MESSAGE' =>  \Thurly\Sender\TemplateTable::initEditor(array(
...
$mailingDb = \Thurly\Sender\MailingTable::getList(array(
...
if(!$mailing = $mailingDb->fetch())
{
	...
	ShowError(\Thurly\Main\Localization\Loc::getMessage("SENDER_MAILING_TRIG_ERROR_NOT_FOUND"));
...
if(in_array($_REQUEST["action"], array('activate', 'deactivate')) && check_thurly_sessid() && $POST_RIGHT>="W")
{
	$fields = array();
	...
	\Thurly\Sender\MailingTable::update($ID, $fields);
	LocalRedirect('/thurly/admin/sender_mailing_trig_edit.php?ID=' . $ID . '&lang=' . LANGUAGE_ID);
}

if($_SERVER['REQUEST_METHOD'] == 'POST' && is_array($CHAIN) && ($save!="" || $apply!="") && $POST_RIGHT=="W" && check_thurly_sessid())
{
	...
	// save chain
	$result = \Thurly\Sender\MailingTable::updateChain($ID, $arTriggerSettings);
	$errorList = array_merge($errorList, $result->getErrorMessages());

	if(empty($errorList))
	{
		if($save != "")
			LocalRedirect('/thurly/admin/sender_mailing_trig_admin.php?lang=' . LANGUAGE_ID);
		else
			LocalRedirect('/thurly/admin/sender_mailing_trig_edit.php?ID=' . $ID . '&lang=' . LANGUAGE_ID);
	}
...
else
{
	...
	$chainFieldsList = \Thurly\Sender\MailingTable::getChain($ID);
}
...
$aMenu = array(
	array(
		...
		"LINK"=>"/thurly/admin/sender_mailing_trig_admin.php?MAILING_ID=".$MAILING_ID."&lang=".LANG,
		"ICON"=>"btn_list",
	)
);
...
CJSCore::RegisterExt('sender_dragdrop', array('js' => '/thurly/js/main/core/core_dragdrop.js'));
...
?>
	<script>
		...
		function SendTestMailing()
		{
			...
			var url = '/thurly/admin/sender_mailing_chain_admin.php?MAILING_ID=<?echo $ID?>&IS_TRIGGER=Y&lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&action=js_send';
			ShowWaitWindow();
			BX.ajax.post(
				url,
				data,
				...
				<span>
					<?if($POST_RIGHT>="W" && $mailing['ACTIVE'] != 'Y'):?>
						<input style="margin-left: 80px;" type="button"
							...
							onclick="window.location='/thurly/admin/sender_mailing_trig_edit.php?ID=<?=$ID?>&<?=thurly_sessid_get()?>&action=activate&lang=<?=LANGUAGE_ID?>'"
							title="<?echo GetMessage("sender_chain_edit_btn_send_desc")?>" />
					<?elseif($POST_RIGHT>="W"):?>
						<input style="margin-left: 80px;" type="button"
							value="<?echo GetMessage("sender_chain_edit_field_status_btn_n")?>"
							onclick="window.location='/thurly/admin/sender_mailing_trig_edit.php?ID=<?=$ID?>&<?=thurly_sessid_get()?>&action=deactivate&lang=<?=LANGUAGE_ID?>'"
							title="<?echo GetMessage("sender_chain_edit_btn_send_desc")?>" />
					<?endif;?>
				</span>
			</div>
	...
	<tr>
		<td colspan="2">
			...
			<script>
				...
				var letterTemplate = {};
				BX.ajax.loadJSON(
					'/thurly/admin/sender_mailing_trig_edit.php?ID=<?=$ID?>&lang=<?=LANGUAGE_ID?>&action=get_vr&<?echo thurly_sessid_get()?>',
					{},
					function(data){
						letterTemplate = data;
					}
...
$tabControl->Buttons(array(
	...
	"back_url"=>"/thurly/admin/sender_mailing_trig_admin.php?ID=".$ID."&lang=".LANG,
));
?>
