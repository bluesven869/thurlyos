...
class TemplateTable extends Entity\DataManager
{
	...
	{
		return \Thurly\Fileman\Block\Editor::isContentSupported($content);
	}
	...
	public static function initEditor(array $params)
	{
		...
		\CJSCore::RegisterExt("editor_mailblock", Array(
			"js" => array(
				"/thurly/js/sender/editor_mailblock.js",
			),
			"rel" => array()
		));
		...
		<?
		if(\Thurly\Main\Config\Option::get('fileman', 'use_editor_3') == 'Y'):
		...
		<script>
			BX.ready(function(){
				<?if(!$isInit): $isInit = true;?>
					...
					letterManager.setMailBlockList(<?=\CUtil::PhpToJSObject(\Thurly\Sender\Preset\MailBlock::getBlockForVisualEditor());?>);
					letterManager.setPlaceHolderList(<?=\CUtil::PhpToJSObject(\Thurly\Sender\PostingRecipientTable::getPersonalizeList());?>);
				<?endif;?>
			});
		...
		<?
			...
			?>
			<br>
			<?=Loc::getMessage("SENDER_ENTITY_TEMPLATE_NOTE_OLD_EDITOR", array("%LINK_START%" => '<a href="/thurly/admin/settings.php?mid=fileman&lang=' . LANGUAGE_ID . '">',	"%LINK_END%" => '</a>'))?>
			<br>
			<br>
		...
		<div id="bx-sender-block-editor-<?=$fieldName?>" style="<?if(!$isDisplayBlockEditor):?>display: none;<?endif;?>">
			...
			if($isDisplayBlockEditor)
			{
				...
				{
					$url = '/thurly/admin/sender_template_admin.php?';
					$url .= 'action=get_template&template_type=' . $templateType . '&template_id=' . $templateId;
					$url .= '&lang=' . LANGUAGE_ID . '&' . thurly_sessid_get();
				}
				else
				{
					$url = $contentUrl;
			...
			echo \Thurly\Fileman\Block\EditorMail::show(array(
				'id' => $fieldName,
				'charset' => $charset,
				'site' => $site,
