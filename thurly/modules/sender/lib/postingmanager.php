...
class PostingManager
{
	...
	public static function click($recipientId, $url)
	{
		...
		if ($recipient && $recipient['ID'])
		{
			...
			if ($postingDb->fetch())
			{
				$uri = new \Thurly\Main\Web\Uri($url);
	 ...
	 * @return string
	 * @throws \Thurly\Main\ArgumentException
	 * @throws \Thurly\Main\DB\Exception
	 */
	protected static function sendInternal($mailingChainId, array $params)
	{
		...
		if(static::$currentMailingChainFields === null)
		{
			...
			$charset = false;
			$siteDb = \Thurly\Main\SiteTable::getList(array(
			...
			{
				throw new \Thurly\Main\DB\Exception(Loc::getMessage('SENDER_POSTING_MANAGER_ERR_SITE', array('#SITE_ID#' => $mailingChain['SITE_ID'])));
			}

			if(!$charset)
			{
				throw new \Thurly\Main\DB\Exception(Loc::getMessage('SENDER_POSTING_MANAGER_ERR_CHARSET', array('#SITE_ID#' => "[".$mailingChain['SITE_ID']."]".$site['NAME'])));
			}

			$attachmentList = array();
			$attachmentDb = \Thurly\Sender\MailingAttachmentTable::getList(array(
			...
			{
				$chainTemplate = \Thurly\Sender\Preset\Template::getById($mailingChain['TEMPLATE_TYPE'], $mailingChain['TEMPLATE_ID']);
				if($chainTemplate && $chainTemplate['HTML'])
				{
					$document = new \Thurly\Main\Web\DOM\Document;
					$document->loadHTML($chainTemplate['HTML']);
					\Thurly\Main\Loader::includeModule('fileman');
					if(\Thurly\Fileman\Block\Editor::fillDocumentBySliceContent($document, $mailingChain['MESSAGE']))
					{
						\Thurly\Main\Web\DOM\StyleInliner::inlineDocument($document);
			...
			static::$currentMailingChainFields['MESSAGE'] = array(
				...
				'MESSAGE_PHP' => \Thurly\Main\Mail\Internal\EventMessageTable::replaceTemplateToPhp($mailingChain['MESSAGE']),
			);
			...
			static::$currentMailingChainFields['LINK_PROTOCOL'] = \Thurly\Main\Config\Option::get("sender", "link_protocol", 'http');
		...
		if(!empty($params['FIELDS']['UNSUBSCRIBE_LINK']))
		{
			if(substr($params['FIELDS']['UNSUBSCRIBE_LINK'], 0, 4) !== 'http')
			{
				...
				{
					$serverName = \Thurly\Main\Config\Option::get("main", "server_name", $GLOBALS["SERVER_NAME"]);
				}
		...
		{
			// inline styles
			$mailBody = \Thurly\Main\Web\DOM\StyleInliner::inlineHtml($message->getMailBody());
		}
	 ...
	 * @throws \Thurly\Main\ArgumentException
	 * @throws \Thurly\Main\DB\Exception
	 * @throws \Thurly\Main\Db\SqlQueryException
	 * @throws \Exception
	 */
	public static function send($id, $timeout=0, $maxMailCount=0)
	{
		...
		{
			throw new \Thurly\Main\DB\Exception(Loc::getMessage('SENDER_POSTING_MANAGER_ERR_LOCK'));
		}
		...
		while($recipientData = $recipientDataDb->fetch())
		{
			...
			catch(\Thurly\Main\Mail\StopException $e)
			{
				$isStopped = true;
				break;
			}
	...
	public static function lockPosting($id)
	{
		...
		elseif($connection instanceof \Thurly\Main\DB\MssqlConnection)
		{
			//Clean up locks
		...
		elseif($connection instanceof \Thurly\Main\DB\OracleConnection)
		{
			try
			{
			...
			catch(\Thurly\Main\Db\SqlQueryException $exception)
			{
