...
class MailingTable extends Entity\DataManager
{
	...
	public static function getMap()
	{
		return array(
			...
			'EMAIL_FROM' => array(
				...
				'validation' => array('Thurly\Sender\MailingChainTable', 'validateEmailForm'),
			),
			'CHAIN' => array(
				'data_type' => 'Thurly\Sender\MailingChainTable',
				'reference' => array('=this.ID' => 'ref.MAILING_ID'),
			),
			'POSTING' => array(
				'data_type' => 'Thurly\Sender\PostingTable',
				'reference' => array('=this.ID' => 'ref.MAILING_ID'),
			),
			'MAILING_GROUP' => array(
				'data_type' => 'Thurly\Sender\MailingGroupTable',
				'reference' => array('=this.ID' => 'ref.MAILING_ID'),
			),
			'MAILING_SUBSCRIPTION' => array(
				'data_type' => 'Thurly\Sender\MailingSubscriptionTable',
				'reference' => array('=this.ID' => 'ref.MAILING_ID'),
			),
		);
	}
	...
	{
		$resultList = array();
		$event = new \Thurly\Main\Event('sender', 'OnPresetMailingList');
		$event->send();
	...
	public static function checkFieldsChain(\Thurly\Main\Entity\Result $result, $primary = null, array $fields)
	{
		$id = $primary;
		$errorList = array();
		...
		foreach($fields as $item)
		{
			...
			if($chainId > 0)
			{
				$chain = \Thurly\Sender\MailingChainTable::getRowById(array('ID' => $chainId));
				if($chain && $chain['STATUS'] != \Thurly\Sender\MailingChainTable::STATUS_WAIT)
				{
			...
			if(empty($chainFields['STATUS']))
				$chainFields['STATUS'] = \Thurly\Sender\MailingChainTable::STATUS_WAIT;

			$chainFields['ID'] = $chainId;

			$resultItem = new \Thurly\Main\Entity\Result;
			\Thurly\Sender\MailingChainTable::checkFields($resultItem, null, $chainFields);
		...
		{
			/* @var \Thurly\Main\Entity\FieldError[] $errors*/
	...
	public static function updateChainTrigger($id)
	{
		...
		foreach($triggerFields as $key => $point)
		{
			...
			$settingsList[] = new \Thurly\Sender\TriggerSettings($point);
		}
		...
		{
			/* @var \Thurly\Sender\TriggerSettings $settings */
			$trigger = \Thurly\Sender\TriggerManager::getOnce($settings->getEndpoint());
			if($trigger)
			{
	...
	public static function getPersonalizeList($id)
	{
		...
		$groupConnectorDb = \Thurly\Sender\MailingGroupTable::getList(array(
			'select' => array(
		...
		while($groupConnector = $groupConnectorDb->fetch())
		{
			...
			{
				$connector = \Thurly\Sender\ConnectorManager::getConnector($groupConnector['CONNECTOR_ENDPOINT']);
			}
...
class MailingGroupTable extends Entity\DataManager
{
	...
	public static function getMap()
	{
		return array(
			...
			'MAILING' => array(
				'data_type' => 'Thurly\Sender\MailingTable',
				'reference' => array('=this.MAILING_ID' => 'ref.ID'),
			),
			'GROUP' => array(
				'data_type' => 'Thurly\Sender\GroupTable',
...
class MailingSubscriptionTable extends Entity\DataManager
{
	...
	public static function getMap()
	{
		return array(
			...
			'CONTACT' => array(
				'data_type' => 'Thurly\Sender\ContactTable',
