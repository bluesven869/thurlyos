...
class PostingTable extends Entity\DataManager
{
	...
	public static function getMap()
	{
		return array(
			...
			'POSTING_RECIPIENT' => array(
				'data_type' => 'Thurly\Sender\PostingRecipientTable',
				'reference' => array('=this.ID' => 'ref.POSTING_ID'),
			),
			'POSTING_READ' => array(
				'data_type' => 'Thurly\Sender\PostingReadTable',
				'reference' => array('=this.ID' => 'ref.POSTING_ID'),
			),
			'POSTING_CLICK' => array(
				'data_type' => 'Thurly\Sender\PostingClickTable',
				'reference' => array('=this.ID' => 'ref.POSTING_ID'),
			),
			'POSTING_UNSUB' => array(
				'data_type' => 'Thurly\Sender\PostingUnsubTable',
				'reference' => array('=this.ID' => 'ref.POSTING_ID'),
			),
		);
	}
	...
	{
		$posting = \Thurly\Sender\PostingTable::getRowById(array('ID' => $postingId));
		if(!$posting)
			return false;
		...
		if(!$checkDuplicate)
		{
			if($posting['STATUS'] == \Thurly\Sender\PostingTable::STATUS_NEW)
			{
				$primary = array('POSTING_ID' => $postingId);
				\Thurly\Sender\PostingRecipientTable::delete($primary);
		...
		$unSubEmailDb = \Thurly\Sender\MailingSubscriptionTable::getUnSubscriptionList(array(
		...
		$statusRecipientNone = \Thurly\Sender\PostingRecipientTable::SEND_RESULT_NONE;
		...
		$groupConnectorList[] = array(
			...
			'GROUP_ID' => null,
			'CONNECTOR' => new \Thurly\Sender\SenderConnectorSubscriber
		);
		...
		foreach($groupConnectorList as $group)
		{
			...
			elseif(is_array($group['CONNECTOR_ENDPOINT']))
				$connector = \Thurly\Sender\ConnectorManager::getConnector($group['CONNECTOR_ENDPOINT']);
			...
			//\Thurly\Sender\GroupConnectorTable::update(array('ID' => $group['GROUP_CONNECTOR_ID']), array('ADDRESS_COUNT' => $connectorDataCount));
		...
		{
			\Thurly\Sender\GroupTable::update($groupId, array('ADDRESS_COUNT' => $groupDataCount));
		}
	...
	public static function onAfterAdd(Entity\Event $event)
	{
		...
		if($resultDb->getSelectedRowsCount() == 1)
		{
			PostingTable::update(array('ID' => $data['POSTING_ID']), array(
				'COUNT_READ' => new \Thurly\Main\DB\SqlExpression('?# + 1', 'COUNT_READ')
			));
		}
...
class PostingClickTable extends Entity\DataManager
{
	...
	public static function onAfterAdd(Entity\Event $event)
	{
		...
		{
			PostingTable::update(array('ID' => $data['POSTING_ID']), array(
				'COUNT_CLICK' => new \Thurly\Main\DB\SqlExpression('?# + 1', 'COUNT_CLICK')
			));
		}
...
class PostingUnsubTable extends Entity\DataManager
{
	...
	public static function onAfterAdd(Entity\Event $event)
	{
		...
		{
			PostingTable::update(array('ID' => $data['POSTING_ID']), array(
				'COUNT_UNSUB' => new \Thurly\Main\DB\SqlExpression('?# + 1', 'COUNT_UNSUB')
			));
		}
