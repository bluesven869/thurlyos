...
class TriggerManager
{
	...
	{
		$args = func_get_args();
		if (isset($args[0]) && $args[0] instanceof \Thurly\Main\Event)
		{
			$event = $args[0];
	...
	protected static function send($chain)
	{
		...
		{
			$autoSendTime = new \Thurly\Main\Type\DateTime;
	...
	protected static function preventMailEvent(array $emailEvent)
	{
		if(isset($emailEvent['EVENT_NAME']) && strlen($emailEvent['EVENT_NAME'])>0)
		{
			...
			{
				\Thurly\Sender\MailEventHandler::prevent($emailEvent['EVENT_NAME'], $emailEvent['FILTER']);
			}
		}
	}
	...
	public static function actualizeHandler(array $params, $activate = null)
	{
		...
		if($activate)
		{
			\Thurly\Main\EventManager::getInstance()->registerEventHandler(
				$moduleId, $eventType, 'sender', __CLASS__, 'handleEvent'
			);
		}
		else
		{
			\Thurly\Main\EventManager::getInstance()->unRegisterEventHandler(
				$moduleId, $eventType, 'sender', __CLASS__, 'handleEvent'
			);
		}
	}
	...
	public static function getList(array $endpointList = null)
	{
		...
		{
			/** @var \Thurly\Sender\Trigger $trigger */
	 ...
	 *
	 * @param array
	 * @return \Thurly\Sender\Trigger|null
	 */
	...
	public static function getClassList(array $endpointList = null)
	{
		...
		foreach ($event->getResults() as $eventResult)
		{
			...
			if($eventResultParameters && array_key_exists('TRIGGER', $eventResultParameters))
			{
				...
				{
					if(!is_subclass_of($connectorClassName,  '\Thurly\Sender\Trigger'))
					{
						continue;
					}
					...
					if(is_subclass_of($connectorClassName,  '\Thurly\Sender\TriggerConnectorClosed'))
	...
	{
		if(static::$debug)
			\Thurly\Main\Diag\Debug::writeToFile($mess, "", "__bx_sender_trigger.log");
	}
	...
	*/
	...
	{
		return '\Thurly\Sender\TriggerManager::fireClosedEventAgent("' . $moduleId . '","' . $eventType .'","' . $chainId .'");';
	}
	...
	*/
	 ...
	 * @return void
	 */
	public static function onAfterMailingChainSend(\Thurly\Main\Event $event)
	{
	 ...
	 * @return void
	 */
	public static function onAfterPostingSendRecipient(\Thurly\Main\Event $event)
	{
		$data = $event->getParameter(0);
	...
	{
		$data['TRIGGER'] = array(
			'Thurly\Sender\SenderTriggerUserAuth',
			'Thurly\Sender\SenderTriggerUserDontAuth',
		);
