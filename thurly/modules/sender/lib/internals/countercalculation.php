...
namespace Thurly\Sender\Internals;
...
class CounterCalculation
{
	...
	public static function updateRecipientReadCounters($type)
	{
		$params = array(
			...
			'runtime' => array(
				new \Thurly\Main\Entity\ReferenceField(
					'UPDATE_RECIPIENT',
					'Thurly\Sender\PostingRecipientTable',
		...
		{
			case 'READ':
				$dataDb = \Thurly\Sender\PostingReadTable::getList($params);
				break;
			case 'CLICK':
				$dataDb = \Thurly\Sender\PostingClickTable::getList($params);
				break;
			case 'UNSUB':
				$dataDb = \Thurly\Sender\PostingUnsubTable::getList($params);
				break;
		}
		...
		while($item = $dataDb->fetch())
		{
			...
			\Thurly\Sender\PostingRecipientTable::update(array('ID' => $item['RECIPIENT_ID']), array('IS_' . $type => 'Y'));
		}
	...
	public static function updatePostingStatusCounters()
	{
		...
		$resultDb = \Thurly\Sender\PostingRecipientTable::getList(array(
			...
			'runtime' => array(
				new \Thurly\Main\Entity\ExpressionField('CALC_COUNT', 'COUNT(%s)', 'ID'),
				new \Thurly\Main\Entity\ReferenceField(
					'UPDATE_POSTING',
					'Thurly\Sender\PostingTable',
		...
		while(!$stopRun)
		{
			...
			if(!$data || $lastPostingId != $data['POSTING_ID'])
			{
				...
				if($updateFields)
				{
					\Thurly\Sender\PostingTable::update(array('ID' => $lastPostingId), $updateFields);
				}
	...
	{
		$dataDb = \Thurly\Sender\PostingRecipientTable::getList(array(
			'select' => array(
				'POSTING_ID',
				'CNT'
			),
			...
			'runtime' => array(
				...
				new \Thurly\Main\Entity\ExpressionField('CNT', 'COUNT(%s)', 'ID')
			),
		...
		while($item = $dataDb->fetch())
		{
			...
			\Thurly\Sender\PostingTable::update(array('ID' => $item['POSTING_ID']), array('COUNT_' . $type => $item['CNT']));
		}
	...
	public static function updateMailingSubscription()
	{
		...
		$existedSubDb = \Thurly\Sender\MailingSubscriptionTable::getList(array(
		...
		$dataDb = \Thurly\Sender\PostingUnsubTable::getList(array(
			'select' => array(
		...
		while($data = $dataDb->fetch())
		{
			...
			$contactId = \Thurly\Sender\ContactTable::addIfNotExist(array('EMAIL' => $data['EMAIL']));
			$primary = array('MAILING_ID' => $data['MAILING_ID'], 'CONTACT_ID' => $contactId);
			$fields = array('IS_UNSUB' => 'Y');
			$row = \Thurly\Sender\MailingSubscriptionTable::getRowById($primary);
			if(!$row)
			{
				$result = \Thurly\Sender\MailingSubscriptionTable::add($fields + $primary);
				$result->isSuccess();
			}
		}
	...
	private static function getPostingStatusUpdateFields($postingId, $statusList)
	{
		...
		$map = \Thurly\Sender\PostingTable::getRecipientStatusToPostingFieldMap();
