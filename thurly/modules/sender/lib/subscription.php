...
class Subscription
{
	...
	{
		return \Thurly\Main\Mail\Tracking::getLinkUnsub(static::MODULE_ID, $fields, \Thurly\Main\Config\Option::get('sender', 'unsub_link'));
	}
	...
	{
		$tag = \Thurly\Main\Mail\Tracking::getSignedTag(static::MODULE_ID, $fields);
		$urlPage = \Thurly\Main\Config\Option::get('sender', 'sub_link');
		if($urlPage == "")
		{
			$thurlyDirectory = \Thurly\Main\Application::getInstance()->getPersonalRoot();
			$result = $thurlyDirectory.'/tools/sender_sub_confirm.php?sender_subscription=confirm&tag='.urlencode($tag);
		}
		else
		{
			...
			if(!empty($unsub))
			{
				...
				if(!$unsubExists)
				{
					...
					if($unsubResult->isSuccess())
					{
						...
						$event = new \Thurly\Main\Event('sender', 'OnAfterRecipientUnsub', array($eventData));
						$event->send();
					}
				}
	...
	public static function sendEventConfirm($email, array $mailingIdList, $siteId)
	{
		...
		\Thurly\Main\Mail\Event::send($eventSendFields);
	}
