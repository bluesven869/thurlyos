 ...
 */
namespace Thurly\Sender\Preset;
...
use Thurly\Fileman\Block\Editor;
use Thurly\Sender\TemplateTable;
...
class Template
{
	...
	public static function getTemplateListHtml($containerId = 'TEMPLATE_CONTAINER')
	{
		...
		if(!$templateListByType)
			$templateListByType = \Thurly\Sender\Preset\Template::getListByType();

		$templateTypeList = \Thurly\Sender\Preset\Template::getTypeList();
						...
						<div class="sender-template-list-container">
							<?foreach($templateTypeList as $templateType => $templateTypeName):?>
								<div id="sender-template-list-type-container-<?=$templateType?>" class="sender-template-list-type-container sender-template-list-type-container-<?=$templateType?>" style="display: none;">
									...
									{
										\Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/panel/main/marketplace.css');
										?>
										<script>
											...
											BX.ready(function(){
												BX.ajax.insertToNode(
													'/thurly/admin/update_system_market.php?category=55&lang=ru&mode=list',
...
class TemplateBase
{
	...
	public static function getById($templateName)
	{
		...
		$fullPathOfIcon = \Thurly\Main\Loader::getLocal($localPathOfIcon);

		$fullPathOfFile = \Thurly\Main\Loader::getLocal(static::LOCAL_DIR_TMPL . bx_basename($templateName) . '.php');
		if ($fullPathOfFile)
		...
		{
			\Thurly\Main\Loader::includeModule('fileman');
			if(\Thurly\Fileman\Block\Editor::isContentSupported($fileContent))
			{
			...
			$result = array(
				...
				'ICON' => (!empty($fullPathOfIcon) ? '/thurly'.$localPathOfIcon : ''),
				'HTML' => $fileContent
			);
		}
	...
	protected static function replaceTemplateByDefaultData($template)
	{
		...
		{
			$logoHeader = '/thurly/images/sender/preset/blocked1/logo.png';
		}
		if(!File::isFileExists(Application::getDocumentRoot() . $logoFooter))
		{
			$logoFooter = '/thurly/images/sender/preset/blocked1/logo_m.png';;
		}
		...
		$themeContent = File::getFileContents(\Thurly\Main\Loader::getLocal(static::LOCAL_DIR_TMPL . 'theme.php'));
		return str_replace(
			array(
