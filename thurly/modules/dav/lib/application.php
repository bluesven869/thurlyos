...
class Application extends \Thurly\Main\Authentication\Application
{
	...
	{
		$this->validUrls = array("/thurly/groupdav.php", "/index.php", "/.well-known");

		$site = \Thurly\Main\Application::getInstance()->getContext()->getSite();
		$urls = UrlRewriter::getList($site);
		foreach ($urls as $url)
		{
			if (in_array($url['ID'], array('thurly:socialnetwork_user', 'thurly:socialnetwork_group', 'thurly:disk.common')))
