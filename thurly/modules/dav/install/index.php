...
Class dav extends CModule
{
	...
	function InstallDB($install_wizard = true)
	{
		...
		if (!$DB->Query("SELECT 'x' FROM b_dav_locks", true))
			$errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/dav/install/db/".$DBType."/install.sql");
		...
		RegisterModuleDependences("main", "OnApplicationsBuildList", "main", '\Thurly\Dav\Application', "onApplicationsBuildList", 100, "modules/dav/lib/application.php"); // main here is not a mistake
	...
	function UnInstallDB($arParams = Array())
	{
		...
		{
			$errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/dav/install/db/".$DBType."/uninstall.sql");
		...
		UnRegisterModuleDependences("main", "OnApplicationsBuildList", "main", '\Thurly\Dav\Application', "onApplicationsBuildList", "modules/dav/lib/application.php"); // main here is not a mistake
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/dav/install/thurly",  $_SERVER["DOCUMENT_ROOT"]."/thurly", true, True);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/dav/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", true, true);
		}
	...
	function DoInstall()
	{
		...
		$APPLICATION->IncludeAdminFile(GetMessage("DAV_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/dav/install/step2.php");
	}
	...
	function DoUninstall()
	{
		...
		if($step<2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage("DAV_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/dav/install/unstep1.php");
		}
		elseif($step==2)
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("DAV_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/dav/install/unstep2.php");
		}
	}
