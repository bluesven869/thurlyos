<?
if (strpos($_SERVER['SCRIPT_NAME'], "/thurly/groupdav.php") === 0)
	return;

if ($_SERVER['REQUEST_METHOD'] == 'PROPFIND' || $_SERVER['REQUEST_METHOD'] == 'OPTIONS')
{
	if (preg_match("/(thurly|coredav|iphone|davkit|dataaccess|sunbird|lightning|cfnetwork|zideone|webkit|khtml|ical4ol|ios\\/([5-9]|10|11)|mac\\sos|mac_os_x|carddavthurlyos|caldavthurlyos|mac\\+os\\+x)/i", $_SERVER['HTTP_USER_AGENT']))
	{
		CHTTP::SetStatus("302 Found");
		header('Location: /thurly/groupdav.php/');
		die();
	}
}

if (\Thurly\Main\Config\Option::get('disk', 'successfully_converted', false) != 'Y' || !CModule::includeModule('disk'))
	return;
...
if (!defined("STOP_WEBDAV") || !STOP_WEBDAV)
{
	...
	{
		if (CModule::includeModule('ldap') && CLdapUtil::isThurlyVMAuthSupported())
		{
			CLdapUtil::thurlyVMAuthorize();
		}
