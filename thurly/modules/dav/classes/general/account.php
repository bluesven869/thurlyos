...
class CDavAccount
{
	...
	public static function GetAccountById($account)
	{
		...
		$dbUsers = \Thurly\Main\UserTable::getList($params);
		if ($arUser = $dbUsers->Fetch())
		{
			$arResult = self::ExtractAccountFromUser($arUser);
			self::$accountsCache["users"][$arUser["ID"]] = $arResult;
				...
				if (array_key_exists('ID', $arFilter) || empty($arFilter['XML_ID']))
				{
					...
					else
					{
						$extranet_site = isModuleInstalled('extranet')
							...
							: (isModuleInstalled('thurlyos') ? 'ex' : false);
	...
	public static function GetAddressbookContactsList($collectionId, $arFilter = array())
	{
			...
			if (empty($xmlIds) || !empty($arFilter['ID']))
			{
				...
				$params = array(
					...
					'select' => array(
						...
						'WORK_ZIP', 'WORK_COUNTRY', 'TIMESTAMP_X', 'UF_DEPARTMENT'
					)
				);
				$dbUsers = \Thurly\Main\UserTable::getList($params);
				while ($arUser = $dbUsers->fetch())
		...
		if ($USER->isAuthorized() and !empty($xmlIds) || empty($arFilter['ID']) and CModule::includeModule('mail'))
		{
			...
			$extranetSite = isModuleInstalled('extranet') ? COption::getOptionString('extranet', 'extranet_site') : (isModuleInstalled('thurlyos') ? 'ex' : false);
			...
			while ($arSite = $rsSite->fetch())
			{
				...
				if (empty($xmlIds) || in_array($xmlId, $xmlIds) and isModuleInstalled('blog'))
				{
					$arResult[] = array(
						'ID'             => $xmlId,
						'LAST_NAME'      => getMessage('DAV_BLOG_POST_CONTACT_NAME'),
						'EMAIL'          => reset(Thurly\Mail\User::getForwardTo($arSite['LID'], $USER->getId(), 'BLOG_POST')),
						'WORK_WWW'       => $server_name,
						'TIMESTAMP_X'    => $timestampX,
						'PERSONAL_PHOTO' => array('src' => '/thurly/modules/dav/images/addressbook/feed.png')
					);
				}
				...
				if (empty($xmlIds) || in_array($xmlId, $xmlIds) and isModuleInstalled('tasks'))
				{
					$arResult[] = array(
						'ID'             => $xmlId,
						'LAST_NAME'      => getMessage('DAV_TASK_CONTACT_NAME'),
						'EMAIL'          => reset(Thurly\Mail\User::getForwardTo($arSite['LID'], $USER->getId(), 'TASKS_TASK')),
						'WORK_WWW'       => $server_name,
						'TIMESTAMP_X'    => $timestampX,
						'PERSONAL_PHOTO' => array('src' => '/thurly/modules/dav/images/addressbook/task.png')
					);
				}
			}
		}
