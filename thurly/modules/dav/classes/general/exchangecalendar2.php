...
if (!class_exists("CDavExchangeCalendar"))
{
	...
	\Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/dav/classes/general/exchangecalendar.php");
		...
		private function ConvertCalendarToArray($calendarItem)
		{
			...
			if ($arResultItem["IS_MEETING"])
			{
				...
				if(count($arRequiredAttendees) > 0)
				{
					...
					for($i = 0; $i < count($arRequiredAttendees); $i++)
					{
						$email = $arRequiredAttendees[$i]->GetPath("/Attendee/Mailbox/EmailAddress");
						if(count($email) > 0)
						{
							$arResultItem["ATTENDEES_EMAIL_LIST"][] = $email[0]->GetContent();
						}
					}
				}
							...
							foreach ($arModifiedUserCalendarItems as $value)
							{
								...
								if (is_array($arModifiedCalendarItem) && count($arModifiedCalendarItem) > 0)
								{
									...
									$arModifyEventArray = array(
										...
										"DESCRIPTION" => $arModifiedCalendarItem["DESCRIPTION"],
										"XML_ID" => $arModifiedCalendarItem["XML_ID"],
										"PROPERTY_LOCATION" => $arModifiedCalendarItem["PROPERTY_LOCATION"],
										"DATE_FROM" => $arModifiedCalendarItem["ACTIVE_FROM"],
										"DATE_TO" => $arModifiedCalendarItem["ACTIVE_TO"],
										"SKIP_TIME" => $arModifiedCalendarItem["SKIP_TIME"],
										...
										"PRIVATE_EVENT" => strtolower($arModifiedCalendarItem["PROPERTY_SENSITIVITY"]) == 'private',
										"ATTENDEE_EMAIL_LIST" => $arModifiedCalendarItem["ATTENDEES_EMAIL_LIST"]
									);

									if ($arModifiedCalendarItem["IS_RECURRING"])
									{
										if ($arModifiedCalendarItem["RECURRING_TYPE"] == "MONTHLY_ABSOLUTE" ||
											$arModifiedCalendarItem["RECURRING_TYPE"] == "MONTHLY_RELATIVE" ||
											$arModifiedCalendarItem["RECURRING_TYPE"] == "MONTHLY")
											$arModifyEventArray["PROPERTY_PERIOD_TYPE"] = "MONTHLY";
										elseif ($arModifiedCalendarItem["RECURRING_TYPE"] == "YEARLY_ABSOLUTE" ||
											$arModifiedCalendarItem["RECURRING_TYPE"] == "YEARLY_RELATIVE" ||
											$arModifiedCalendarItem["RECURRING_TYPE"] == "YEARLY")
										...
										$arModifyEventArray["PROPERTY_EVENT_LENGTH"] = MakeTimeStamp($arModifyEventArray["DATE_TO"]) - MakeTimeStamp($arModifyEventArray["DATE_FROM"]);
										if ($arModifyEventArray["PROPERTY_EVENT_LENGTH"] <= 0)
											$arModifyEventArray["PROPERTY_EVENT_LENGTH"] = 86400;

										if (isset($arModifiedCalendarItem["RECURRING_NUMBEROFOCCURRENCES"]) && $arModifiedCalendarItem["RECURRING_NUMBEROFOCCURRENCES"] > 0)
										{
											$arModifyEventArray["PROPERTY_RRULE_COUNT"] = intval($arModifiedCalendarItem["RECURRING_NUMBEROFOCCURRENCES"]);
										}
										elseif (isset($arModifiedCalendarItem["RECURRING_ENDDATE"]))
										{
											$arModifyEventArray["PROPERTY_PERIOD_UNTIL"] = $arModifiedCalendarItem["RECURRING_ENDDATE"];
										}
										else
										{
											$arModifyEventArray["PROPERTY_PERIOD_UNTIL"] = ConvertTimeStamp(mktime(0, 0, 0, 12, 31, 2025), "FULL");
										}
									}

									CCalendarSync::ModifyEvent($userCalendar["CALENDAR_ID"], $arModifyEventArray);
