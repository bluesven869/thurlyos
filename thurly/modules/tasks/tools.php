 ...
 * @deprecated
 */
class TasksException extends \Thurly\Tasks\Exception
{
...
function tasksFormatDate($in_date)
{
	...
	else
	{
		...
		{
			$strDate = \Thurly\Tasks\UI::formatDateTime($date, FORMAT_DATE);
		}
		else
...
{
	return \Thurly\Tasks\UI\Task\Template::makeReplicationPeriodString($arParams);
}
...
function tasksRenderListItem($task, $childrenCount, $arPaths, $depth = 0, 
	...
	$componentName = "thurly:tasks.list.item", $componentTemplate = ".default", 
			...
			files: [
				<?php
					foreach($arTask["FILES"] as $file):
						$i++;
				?>
				{ name : '<?php echo CUtil::JSEscape($file["ORIGINAL_NAME"])?>', url : '/thurly/components/thurly/tasks.task.detail/show_file.php?fid=<?=intval($file["ID"])?>', size : '<?php echo CUtil::JSEscape(CFile::FormatSize($file["FILE_SIZE"]))?>' }<?php if ($i != sizeof($arTask["FILES"])):?>,<?php endif?>
				<?php endforeach?>
			],
		<?php endif?>
