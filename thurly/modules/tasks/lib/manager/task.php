...
namespace Thurly\Tasks\Manager;

use Thurly\Tasks\Integration\SocialNetwork\Group;
use Thurly\Tasks\Internals\Task\ParameterTable;
use Thurly\Tasks\Manager\Task\Accomplice;
use Thurly\Tasks\Manager\Task\Auditor;
use Thurly\Tasks\Manager\Task\Checklist;
use Thurly\Tasks\Manager\Task\ElapsedTime;
use Thurly\Tasks\Manager\Task\Log;
use Thurly\Tasks\Manager\Task\Originator;
use Thurly\Tasks\Manager\Task\Parameter;
use Thurly\Tasks\Manager\Task\ParentTask;
use Thurly\Tasks\Manager\Task\Project;
use Thurly\Tasks\Manager\Task\ProjectDependence;
use Thurly\Tasks\Manager\Task\RelatedTask;
use Thurly\Tasks\Manager\Task\Reminder;
use Thurly\Tasks\Manager\Task\Responsible;
use Thurly\Tasks\Manager\Task\Tag;
use Thurly\Tasks\Manager\Task\Template;
use Thurly\Tasks\Util\Error\Collection;
use Thurly\Tasks\Util\UserField\Task as UserField;

final class Task extends \Thurly\Tasks\Manager
{
	...
	private static function doAdd($userId, array $data, array $parameters)
	{
		...
		if ($taskId)
		{
			if (!\Thurly\Tasks\Integration\Extranet\User::isExtranet($userId) && $data[ "ADD_TO_TIMEMAN" ] == "Y")
			{
				// add the task to planner only if the user this method executed under is current and responsible for the task
				if ($userId == $data[ 'RESPONSIBLE_ID' ] && $userId == \Thurly\Tasks\Util\User::getId())
				{
	...
	private static function doUpdate($userId, $taskId, array $data, array $parameters)
	{
		...
		if (!empty($cleanData))
		{
			...
			{
				\Thurly\Tasks\Item\Task\Parameter::deleteByParent($taskId, array());
		...
		$adminUserId = \Thurly\Tasks\Util\User::getAdminId();
	...
	public static function makeItem($data, $userId = 0)
	{
		...
		return new \Thurly\Tasks\Item\Task($data, $userId);
	}
	...
	public static function getFullRights($userId)
	{
		...
		return array(
			...
			'DAYPLAN.ADD' => !\Thurly\Tasks\Integration\Extranet\User::isExtranet($userId)
							 && \Thurly\Tasks\Integration\Timeman::canUse()
		);
	}
	...
	private static function injectDayPlanFields($userId, array $parameters, array &$data, array &$can)
	{
		...
		$extranetSite = \Thurly\Tasks\Integration\Extranet::isExtranetSite();
		$extranetUser = \Thurly\Tasks\Integration\Extranet\User::isExtranet($userId);
		...
		foreach ($data as &$task)
		{
			...
			$can[ $task[ 'ID' ] ][ 'ACTION' ][ 'ADD_TO_DAY_PLAN' ] = $can[ $task[ 'ID' ] ][ 'ACTION' ][ 'DAYPLAN.ADD' ] =
				!$extranetUser && $canAddToPlan && \Thurly\Tasks\Integration\Timeman::canUse();
		}
		unset($task);
...
//                'DEADLINE',
		...
		foreach ($arFields as $field)
		{
			switch ($field)
			{
				...
				// custom fields
				case 'CHECKLIST':
					/** Thurly\Tasks\Item\Task\Collection\CheckList */
					...
					break;
				case 'RESPONSIBLE':
					$index[] = \Thurly\Tasks\Util\User::getUserName($arTask[ 'RESPONSIBLE_ID' ]);
					break;
				case 'ORIGINATOR':
					$index[] = \Thurly\Tasks\Util\User::getUserName($arTask[ 'CREATED_BY' ]);
					break;
				case 'AUDITORS':
					if(array_key_exists('AUDITORS', $arTask))
					{
						...
						{
							$index[] = \Thurly\Tasks\Util\User::getUserName($userId);
						}
					}
					break;
				case 'ACCOMPLICES':
...
//                    break;
				case 'CRM':
					if(\Thurly\Main\ModuleManager::isModuleInstalled('crm'))
					{
