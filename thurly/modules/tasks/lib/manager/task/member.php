...
use Thurly\Tasks\Integration\SocialServices;

abstract class Member extends \Thurly\Tasks\Manager
{
	...
	public static function extendData(array &$data, array $knownMembers = array())
	{
		...
		if(static::getIsMultiple())
		{
			...
			foreach($data[$code] as $k => $item)
			{
				...
				{
					$data[$code][$k] = \Thurly\Tasks\Util\User::extractPublicData($knownMembers[$item['ID']]);
				}
				else
				{
		...
		else
		{
			...
			{
				$data[$code] = \Thurly\Tasks\Util\User::extractPublicData($knownMembers[$data[$code]['ID']]);
			}
			else
			{
				// user might be is about to invite, do not erase
			...
			if(static::getIsMultiple())
			{
				foreach($data[$code] as $i => $user)
				{
					if(!intval($user['ID']))
					{
						...
						{
							$data[$code][$i]['ID'] = static::inviteUser($user, $errors);
						}
