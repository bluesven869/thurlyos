...
class TaskHandler extends \Thurly\Replica\Client\BaseHandler
{
	...
	{
		$this->fileHandler = new \Thurly\Tasks\Replica\TaskAttachmentHandler();
	}
	...
	public function afterInsertTrigger(array $newRecord)
	{
		...
		$list = \Thurly\Tasks\Internals\Task\MemberTable::getList(array(
				"filter" => array(
...
//		\CTaskCountersProcessor::onAfterTaskAdd($newRecord);
		\Thurly\Tasks\Internals\Counter::onAfterTaskAdd($newRecord);
...
//		\CTaskCountersProcessor::onAfterTaskUpdate($oldRecord, $newRecord);
		\Thurly\Tasks\Internals\Counter::onAfterTaskUpdate($oldRecord, $newRecord);
...
//		\CTaskCountersProcessor::onAfterTaskDelete($oldRecord);
		\Thurly\Tasks\Internals\Counter::onAfterTaskUpdate($oldRecord);
