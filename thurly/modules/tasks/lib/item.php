...
use Thurly\Tasks\Internals\DataBase\LazyAccess;
use Thurly\Tasks\Item\Access;
use Thurly\Tasks\Item\Context;
use Thurly\Tasks\Item\Converter;
use Thurly\Tasks\Item\Exporter\Canonical;
use Thurly\Tasks\Item\Result;
use Thurly\Tasks\Item\State;
use Thurly\Tasks\Item\Field;
...
//		}
	...
	public function save($settings = array())
	{
		...
		if($canPerform)
		{
			...
			if($result->isSuccess() && $this->doPreActions($state) && $this->executeHooksBefore($state))
			{
				...
				$authContext = new \Thurly\Main\Authentication\Context();
				...
				if($dbResult->isSuccess())
				{
					...
					if ($this->id)
					{
						\Thurly\Tasks\Kanban\StagesTable::pinInStage($this->id);
					}
	...
	private static function fixGlobalUser($userId)
	{
		...
		{
			$GLOBALS['USER'] = \Thurly\Tasks\Util\User\Mock::getInstance($userId);
			$fixed = true;
		}
	 ...
	 * @param array $parameters
	 * @param null $settings
	 * @return array|\Thurly\Tasks\Item\Collection
	 * @throws NotImplementedException
	 * @throws SystemException
	 */
	 ...
	 * @return \Thurly\Tasks\Util\Collection
	 */
	...
	protected static function generateMap(array $parameters = array())
	{
		...
		/**
		 * @var mixed[]|\Thurly\Main\Entity\BooleanField|\Thurly\Main\Entity\DateTimeField|\Thurly\Main\Entity\ScalarField $v
		 */
		foreach($dc::getMap() as $k => $v)
		{
			...
			$isBoolean = is_object($v) ? is_a($v, '\\Thurly\\Main\\Entity\\BooleanField') : $v['data_type'] == 'boolean';
			$isDate = is_object($v) ? is_a($v, '\\Thurly\\Main\\Entity\\DateTimeField') : $v['data_type'] == 'datetime';
			$isReference = is_object($v) ? is_a($v, '\\Thurly\\Main\\Entity\\ReferenceField') : isset($v['reference']);
			$isExpression = is_object($v) ? is_a($v, '\\Thurly\\Main\\Entity\\ExpressionField') : isset($v['expression']);
	...
	public function prepareData($result)
	{
		...
		$this->searchIndex = \Thurly\Tasks\Manager\Task::prepareSearchIndex($this->getData());
		...
		if($ufc)
		{
			...
			if($getValue)
			{
				foreach($scheme as $field => $fieldDesc)
				{
					...
					if($settings['COLLECTION_VALUE_TO_ARRAY'] && \Thurly\Tasks\Util\Collection::isA($fieldValue))
					{
	...
	protected function callCanMethod($name, $arguments)
	{
		...
		if(is_callable($method))
		{
			...
			/** @var \Thurly\Tasks\Util\Result $mainResult */
