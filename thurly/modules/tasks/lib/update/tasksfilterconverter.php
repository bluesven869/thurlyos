...
use Thurly\Tasks\Ui\Filter\Convert;
	...
	public function execute(array &$result)
	{
		...
		if ($params["count"] > 0)
		{
			...
			while ($arData = $res->Fetch())
			{
				...
				$fields = \Thurly\Tasks\Ui\Filter\Convert\Filter::prepareFilter($serializedFilter);
				if ($fields)
				{
			...
			foreach(array_keys($userFilters) as $userId)
			{
				...
				if(!$filters)
				{
					...
					$filters["default_presets"] = \Thurly\Tasks\UI\Filter\Task::getPresets();
					$filters["default"] = \Thurly\Main\UI\Filter\Options::findDefaultPresetId(
