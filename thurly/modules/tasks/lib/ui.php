...
class UI
{
	...
	{
		return 'UTC '.($offset > 0 ? '+' : ($offset != 0 ? '-' : '')).static::formatTimeAmount($offset, 'HH:MI');
	}
	...
	{
		$dayTime = \Thurly\Tasks\Util::trim($dayTime);
	...
	public static function getHintState()
	{
		...
		$options = \Thurly\Tasks\Util\User::getOption('task_hints');
		if(is_array($options))
		{
	...
	public static function convertBBCodeToHtml($value, array $parameters = array())
	{
		...
		if($preset == 'FULL')
		{
			$parser = \Thurly\Tasks\Util::getParser($parameters);
	...
	{
		$id = AddEventHandler("main", "TextParserBeforeAnchorTags", Array("\\Thurly\\Tasks\\UI", "convertHtmlToBBCodeHack"));

		$TextParser = \Thurly\Tasks\Util::getParser();
	...
	private static function processObjects($data)
	{
		...
		if(is_object($data) && is_subclass_of($data, '\Thurly\Main\Type\DateTime'))
		{
