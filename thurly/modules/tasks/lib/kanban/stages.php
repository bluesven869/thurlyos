<?php
namespace Thurly\Tasks\Kanban;

use \Thurly\Tasks\Internals\TaskTable as Task;
use \Thurly\Tasks\Internals\Task\SortingTable;
use \Thurly\Tasks\Internals\Task\FavoriteTable;
use \Thurly\Tasks\Internals\Task\ViewedTable;
		...
		if ($task['GROUP_ID'] > 0 && !empty($checkStages) && ($newTask || $refreshGroup))
		{
			...
			if ($row = $res->fetch())
			{
				SortingTable::setSorting(
					\Thurly\Tasks\Util\User::getId() > 0 ? \Thurly\Tasks\Util\User::getId() : $task['CREATED_BY'],
					$task['GROUP_ID'],
					$taskId,
					$row['ID'],
