...
namespace Thurly\Tasks\Processor\Task;
...
use Thurly\Tasks\Processor\Task\Scheduler\RelationManager;
use Thurly\Tasks\Processor\Task\Scheduler\RelationManager\Project;
use Thurly\Tasks\Processor\Task\Scheduler\RelationManager\SubTask;

use Thurly\Tasks\Item;

final class Scheduler extends \Thurly\Tasks\Processor
{
	 ...
	 * @return \Thurly\Tasks\Processor\Task\Result
	 */
	public function processEntity($id, $data = array(), array $settings = array())
	{
		...
		// todo: Impact class is TEMPORAL, it should be replaced with (or at least inherited from) \Thurly\Tasks\Item\Task when ready
		...
		if($inSubTask || $inProject)
		{
			...
			while(count($this->queue) && $times < 10000)
			{
				...
				/** @var \Thurly\Tasks\Processor\Task\Scheduler\RelationManager\Project|\Thurly\Tasks\Processor\Task\Scheduler\RelationManager\SubTask $nextProcessor */
				...
				/** @var \Thurly\Tasks\Processor\Task\Scheduler\Result\Impact $impact */
