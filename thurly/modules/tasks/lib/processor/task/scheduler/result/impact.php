...
namespace Thurly\Tasks\Processor\Task\Scheduler\Result;
...
final class Impact extends \Thurly\Tasks\Processor\Task\Result\Impact
{
	...
	/**
	 * todo: when inherited from \Thurly\Tasks\Item\Task, create method reSchedule() there, to be able to use
	 * todo: \Thurly\Tasks\Item\Task instance with dates recalculation according to the MATCH_WORK_TIME flag
	 *
	 * @param array $data
	 */
	...
	public function save()
	{
		...
		try
		{
			...
			// todo: get rid of CTaskItem, use \Thurly\Tasks\Item\Task when ready
	...
	{
		if (!$this->getEndDatePlan()) // endless task
		{
			return 0;
		}
	...
	private static function castToDateTimeGmt($date)
	{
		...
		elseif ($date instanceof \Thurly\Main\Type\DateTime)
		{
			// to local time ...
			$date = $date->toString();
