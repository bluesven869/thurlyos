...
namespace Thurly\Tasks\Processor\Task\Scheduler\RelationManager;

use Thurly\Tasks\Internals\DataBase\Structure\ClosureTree\Fragment;
use Thurly\Tasks\Internals\Helper\Task\Dependence;
...
final class SubTask extends \Thurly\Tasks\Processor\Task\Scheduler\RelationManager
{
	...
	/**
	 * @param \Thurly\Tasks\Processor\Task\Scheduler\Result\Impact $rootImpact
	 * @param Result $result
		...
		foreach($miniMaxes as $taskId => $dates)
		{
			...
			if($taskData['INHERIT_DATES'] == 'Y') // resize only "bracket" tasks
			{
				...
				if($impact)
				{
					...
					if($impact['START_DATE_PLAN']->isNotEqualTo($dates['MIN']) || $impact['END_DATE_PLAN']->isNotEqualTo($dates['MAX']))
					{
