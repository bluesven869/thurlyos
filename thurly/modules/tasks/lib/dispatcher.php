...
use Thurly\Tasks\Dispatcher\ExecutionResult;
use Thurly\Tasks\Dispatcher\Operation;
use Thurly\Tasks\Dispatcher\PublicAction;
use Thurly\Tasks\Dispatcher\ToDo;
use Thurly\Tasks\Util\Error;
use \Thurly\Tasks\Util\Error\Collection;
use \Thurly\Tasks\Util\Error\Filter;
...
final class Dispatcher
{
	...
	public static function restRegister($rest)
	{
		...
		$classPrefix = '\\Thurly\\Tasks\\Dispatcher\\Publicaction\\';
	...
	public static function restGateway()
	{
		...
		{
			throw new \Thurly\Rest\RestException(
				'Method not found!',
		...
		$plan->import(array(
			array(
				...
				'ARGUMENTS' => \CRestUtil::getRequestData()
			)
		));
	...
	protected function getOperationsByPlan($plan, $result)
	{
		...
		foreach($plan as $op)
		{
			...
			$opClassPrefix = '\\Thurly\\Tasks\\Dispatcher\\';
	...
	private function wrapOpCall($operation)
	{
		...
		catch(\TasksException $e) // old-style tasks exception
		{
			...
			if($errorCode !== false)
			{
				...
				{
					$errors = \Thurly\Tasks\Util\Type::unSerializeArray($e->getMessage());
					foreach ($errors as $error)
					{
		...
		catch(\Thurly\Tasks\ActionNotAllowedException $e)
		{
		...
		catch(\Thurly\Tasks\ActionFailedException $e)
		{
	 ...
	 * @param \Thurly\Main\SystemException $e
	 * @return string
	 */
	...
	// todo: rewirite this on \Thurly\Main\IO functions
