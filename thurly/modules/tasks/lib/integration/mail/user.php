...
final class User extends \Thurly\Tasks\Integration\Mail
{
	...
	public static function getData(array $ids, $siteId = '')
	{
		...
		$data = \Thurly\Mail\User::getUserData($ids, \Thurly\Tasks\Util\Site::getUserNameFormat($siteId));
		if (empty($data))
		{
			return array();
		}
	...
	public static function create(array $data, Collection $errors = null)
	{
		...
		$id = \Thurly\Mail\User::create(array(
			'EMAIL' => $email,
