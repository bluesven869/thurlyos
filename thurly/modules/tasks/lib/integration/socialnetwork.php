...
abstract class SocialNetwork extends \Thurly\Tasks\Integration
{
	...
	public static function getLogDestination($context = 'TASKS', array $parameters = array())
	{
		...
		{
			throw new \Thurly\Main\SystemException('Global user is not defined');
		}
		...
		if (\Thurly\Tasks\Integration\Extranet\User::isExtranet())
		{
		...
		if (\Thurly\Tasks\Integration\Extranet::isConfigured())
		{
		...
		{
			$destination['USERS_VACATION'] = \Thurly\Socialnetwork\Integration\Intranet\Absence\User::getDayVacationList();
		}
	...
	public static function formatDateTimeToGMT($time, $userId)
	{
		...
		return \Thurly\Socialnetwork\ComponentHelper::formatDateTimeToGMT($time, $userId);
	}
	...
	public static function getMemberList($groupId)
	{
		...
		try
		{
			$res = \Thurly\Socialnetwork\UserToGroupTable::getList(
				array(
					'order' => array(
						'DATE_CREATE' => 'ASC'
					),
			...
			foreach ($res as $item)
			{
				$user = array(
					...
					'FORMATTED_NAME' => \Thurly\Tasks\Util\User::getUserName($item['USER_ID']),
