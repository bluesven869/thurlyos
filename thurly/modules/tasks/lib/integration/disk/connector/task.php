...
namespace Thurly\Tasks\Integration\Disk\Connector;

use Thurly\Main\Localization\Loc;

use Thurly\Disk\Ui;
use Thurly\Disk\Uf\StubConnector;
...
class Task extends StubConnector
{
	...
	public function addComment($authorId, array $data)
	{
		...
		// todo: move to \Thurly\Tasks\Item\Task
	...
	public static function getFilesCount($id)
	{
		...
		$res = \Thurly\Disk\Internals\AttachedObjectTable::getList(array(
			'select' => array(
				'TASK_ID' => 'ENTITY_ID',
				new \Thurly\Main\Entity\ExpressionField('CNT', 'COUNT(ENTITY_ID)')
			),
			'filter' => array(
	...
	public static function getCover($id, $width=0, $height=0)
	{
		...
		$res = \Thurly\Disk\Internals\AttachedObjectTable::getList(array(
			...
			'runtime' => array(
				'FILE' => new \Thurly\Main\Entity\ReferenceField(
					'FILE',
					'\Thurly\Main\FileTable',
			...
			'filter' => array(
				'=ENTITY_TYPE' => self::className(),
				'ENTITY_ID' => $id,
				'OBJECT.TYPE_FILE' => \Thurly\Disk\TypeFile::IMAGE,
				//'>=FILE.WIDTH' => $width,
				//'>=FILE.HEIGHT' => $height
			),
			'order' => array(
		...
		while ($row = $res->fetch())
		{
			$result[$row['TASK_ID']] = \Thurly\Disk\UrlManager::getUrlToActionShowUfFile(
				$row['ID'],
				$width*$height > 0
				? array(
					'width' => $width,
