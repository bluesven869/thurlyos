...
final class Comment extends \Thurly\Tasks\Integration\Forum\Comment
{
	...
	public static function add($taskId, array $data)
	{
		...
		{
			$data['AUTHOR_ID'] = \Thurly\Tasks\Util\User::getId();
		}
		...
		foreach($data as $k => $v)
		{
			if(\Thurly\Tasks\Util\UserField::isUFKey($k))
			{
				$GLOBALS[$k] = $data[$k];
				unset($data[$k]);
			}
	 ...
	 *
	 * This function WILL send notifications in case of comment add through thurly:forum.comments component
	...
	public static function onAfterAdd($entityType, $taskId, $arData)
	{
		...
		$oTask = \CTaskItem::getInstance($taskId, \Thurly\Tasks\Util\User::getAdminId());
		$arTask = $oTask->getData();

		$arRecipientsIDs = static::getTaskMembersByTaskId($taskId, $excludeUser = $occurAsUserId);

		// sonet log
		if (\Thurly\Tasks\Integration\Socialnetwork::includeModule() && \Thurly\Tasks\Integration\SocialNetwork::isEnabled())
		{
			$bCrmTask = (
				...
				case "SHOW":
					...
					if (!$arLogComment)
					{
						...
						if ($arMessage)
						{
							...
							elseif (\Thurly\Tasks\Integration\CRM::includeModule())
							{
...
//									"URL" => str_replace("?IFRAME=Y", "", str_replace("&IFRAME=Y", "", str_replace("IFRAME=Y&", "", $strURL))),
	...
	{
		$oTask = \CTaskItem::getInstance((int)$taskId, \Thurly\Tasks\Util\User::getAdminId());
	...
	{
		if(empty($toUsers) || !\Thurly\Tasks\Integration\IM::includeModule())
		{
			return false;
		}
		...
		$message = \Thurly\Tasks\Util::trim(\CTextParser::clearAllTags($messageData['POST_MESSAGE']));
		...
		\CTaskNotifications::SendMessageEx($taskData["ID"], $fromUser, $toUsers, array(
			...
			'PUSH' => \CTaskNotifications::cropMessage($messageTemplatePush, array(
				'USER_NAME' => 			\Thurly\Tasks\Util\User::formatName($user),
	...
	private static function getUserId()
	{
		...
		if(!$userId)
		{
			$userId = \Thurly\Tasks\Util\User::getAdminId();
		}
	...
	private static function addWebDavFileRights($taskId, $arFilesIds)
	{
		...
		$oTask  = new \CTaskItem((int)$taskId, \Thurly\Tasks\Util\User::getAdminId());
		$arTask = $oTask->getData(false);
	...
	{
		return \Thurly\Tasks\Integration\Forum\Task\Topic::getFileCount($topicId, $forumId);
	}
}