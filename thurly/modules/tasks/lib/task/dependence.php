...
final class DependenceTable extends Mesh
{
	...
	public static function getLinkCountForUser($userId = 0)
	{
		...
		$item = \Thurly\Main\HttpApplication::getConnection()->query("select count(*) as CNT from ".static::getTableName()." where ".static::getDIRECTColumnName()." = '1' and CREATOR_ID = '".intval($userId)."'")->fetch();
	...
	public static function getSubTreeSql($id)
	{
		...
		// (in \Thurly\Tasks\TaskTable there is different alias naming)
		...
		*/
		...
		// look also at \Thurly\Tasks\Util\Scheduler::initializeTargetTask()
	...
	// unused
	/*
	protected static function getRelationHash($id, array $parameters = array())
	{
		$id = intval($id);
		$result = array();
	...
	*/
	...
	public static function getMap()
	{
		...
		$entityName = '\\Thurly\\Tasks\\TaskTable';
		if(func_num_args() > 0)
		{
		...
		), parent::getMap('\\Thurly\\Tasks\\Task\\Dependence'));
