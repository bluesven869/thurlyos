...
namespace Thurly\Tasks\Ui\Filter\Convert;
...
final class Filter
{
	...
	public static function prepareFilter($filter)
	{
		...
		if(array_key_exists('RESPONSIBLE_ID', $filter))
		{
			...
			{
				$filter['RESPONSIBLE_ID_label'][$k] = \Thurly\Tasks\Util\User::getUserName($userId);
			}
		}
		if(array_key_exists('CREATED_BY', $filter))
		{
			...
			{
				$filter['CREATED_BY_label'][$k] = \Thurly\Tasks\Util\User::getUserName($userId);
			}
		}
		if(array_key_exists('AUDITOR', $filter))
		{
			...
			{
				$filter['AUDITOR_label'][$k] = \Thurly\Tasks\Util\User::getUserName($userId);
			}
		}
		if(array_key_exists('ACCOMPLICE', $filter))
		{
			...
			{
				$filter['ACCOMPLICE_label'][$k] = \Thurly\Tasks\Util\User::getUserName($userId);
			}
		}
		...
		if(array_key_exists('GROUP_ID', $filter))
		{
			...
			{
				$group = \Thurly\Tasks\Integration\SocialNetwork\Group::getData(array($groupId));
