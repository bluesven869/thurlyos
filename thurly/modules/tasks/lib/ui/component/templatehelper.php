 ...
 */
namespace Thurly\Tasks\UI\Component;
...
final class TemplateHelper
{
	...
	public function __construct($name, $template, array $parameters = array())
	{
		...
		if($template instanceof \CThurlyComponentTemplate)
		{
	...
	public function initializeExtension(array $data = array())
	{
		...
		$data = array_merge((is_array($jsData) ? $jsData : array()), $data, array(
			...
			'hintState' => \Thurly\Tasks\UI::getHintState(), // todo: when collection implemented, move this outside, leave handy shortcut in component.js
		...
		?>
		<script>new BX.Tasks.Component.<?=$this->name?>(<?=\Thurly\Tasks\UI::toJSON($data)?>);</script>
		<?
	}
	...
	public function fillTemplate($template, $data)
	{
		...
		foreach($data as $k => $v)
		{
			if(array_key_exists('CAN_SHOW_URL', $data))
			{
				...
				{
					$v = \Thurly\Tasks\UI::sanitizeString($v, array('a' => array('href'), 'img' => array('src')));
					$v = \Thurly\Tasks\UI::convertBBCodeToHtml($v, array('PRESET' => 'BASIC'));
				}
