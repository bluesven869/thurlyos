 ...
 * 
 * @access private
 * todo: create \Thurly\Tasks\Internals\DataBase\Structure\ClosureMesh, and then make this class deprecated
 */
namespace Thurly\Tasks\Internals\DataBase;
...
abstract class Mesh extends Tree
{
	...
	{
		throw new \Thurly\Tasks\Exception('Can not add a new edge', array('AUX' => array('MESSAGE' => array('FROM' => $from, 'TO' => $to, 'DIRECT' => $direct, 'TYPE' => $type))));
	}
	...
	private static function insertEdge($from, $to, $direct = true, $increment = 0, array $behaviour = array('INCREMENT_MP_WHEN_EXISTS' => true))
	{
		...
		$merge = $helper->prepareMerge(
			...
			$values,
			array(
				$mpcityColName => new \Thurly\Main\DB\SqlExpression($mpcityColName.$plus),
			)
		);
