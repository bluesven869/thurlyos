...
namespace Thurly\Tasks\Internals\RunTime;
...
use Thurly\Tasks\Internals\DataBase\Helper;

final class Task extends \Thurly\Tasks\Internals\Runtime
{
	 ...
	 *
	 * @param $parameters
	 * @return \Thurly\Main\Entity\ReferenceField[]
	...
	public static function getLegacyFilter(array $parameters)
	{
		...
		$query = new \Thurly\Main\Entity\Query('Thurly\\Tasks\\Task');
		$query->setFilter(
			array(
		...
		$result['runtime'][] = new Entity\ReferenceField(
			$parameters['NAME'],
			\Thurly\Main\Entity\Base::getInstanceByQuery($query),
			array(
				'=this.'.((string) $rf != '' ? $rf : 'ID') => 'ref.ID'
			),
	...
	public static function getLegacyRightsChecker(array $parameters)
	{
		...
		if (!\Thurly\Tasks\Util\User::isSuper($parameters['USER_ID']))
		{
	...
	{
		$conditions = array();
		$helper = \Thurly\Main\HttpApplication::getConnection()->getSqlHelper();
