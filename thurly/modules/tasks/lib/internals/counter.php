...
use Thurly\Tasks\Internals\Counter\Agent;
...
class Counter
{
	...
	{
		$reflect = new \ReflectionClass('\Thurly\Tasks\Internals\Counter\Name');
	...
	private function processRecalculate($plan)
	{
		/** @var Collection $plan */
		$plan = $plan->export();
	...
	private function getKpi()
	{
		...
		$effective = round(Effective::getByRange($dateFrom, $dateTo, $this->userId, $groupId));
	...
	public static function onAfterTaskAdd(array $fields)
	{
		...
		if ($accomplice->count() > 0)
		{
			foreach ($fields['ACCOMPLICES'] as $userId)
			{
				...
				if($fields['DEADLINE'])
				{
					\Thurly\Tasks\Internals\Counter\Agent::add($fields['ID'], new DateTime($fields['DEADLINE']));
				}
				else
				{
					Effective::modify(
	...
	public static function onAfterTaskDelete($fields)
	{
		...
		if ($responsible->count() > 0)
		{
			...
			\Thurly\Tasks\Internals\Effective::modify(
				$fields['RESPONSIBLE_ID'],
				'R',
				$instance,
				$fields['GROUP_ID'],
		...
		\Thurly\Tasks\Internals\Effective::repair($fields['ID']);
	}
