...
use Thurly\Main\HttpApplication;
use Thurly\Main\Entity;
use Thurly\Main\Localization\Loc;

use Thurly\Tasks\Util\Assert;
use Thurly\Tasks\TaskTable;
...
class FavoriteTable extends Entity\DataManager
{
	 ...
	 *
	 * @param mixed[] Primary key for \Thurly\Tasks\Task\FavoriteTable entity
	 *
	 * @return boolean
	 */
	 ...
	 *
	 * @return \Thurly\Main\Entity\AddResult|\Thurly\Main\Entity\DeleteResult Returns \Thurly\Main\Entity\AddResult if task was not favorite, but became such, \Thurly\Main\Entity\DeleteResult otherwise
	 */
	...
	public static function add(array $data)
	{
		...
		// already there, we dont want \Thurly\Main\DB\SqlQueryException "Duplicate entry" crumble everywhere
		if ($behaviour['CHECK_EXISTENCE'] && static::check($data))
		{
			$result = new \Thurly\Main\Entity\AddResult();
		}
		else
		{
	...
	/**
	 * @param mixed[] Primary key for \Thurly\Tasks\Task\FavoriteTable entity
	 * @return mixed[] Primary key for \Thurly\Tasks\Task\FavoriteTable entity
	...
	protected static function processPrimary($primary)
	{
		if (is_array($primary) && !array_key_exists('USER_ID', $primary))
		{
			...
			{
				throw new \Thurly\Main\ArgumentException('Cannot set USER_ID automatically, global USER object does not look good. Specify $primary[USER_ID] manually.');
			}
		}
