...
use Thurly\Tasks\Integration\Search;
use Thurly\Tasks\Integration\Pull;
use Thurly\Tasks\Integration\SocialNetwork\Group;
use Thurly\Tasks\Internals\Task\FavoriteTable;

use Thurly\Tasks\Internals\Task\LogTable;
...
use Thurly\Tasks\Item\Converter\Task\ToTemplate as TaskToTemplate;
use Thurly\Tasks\Item\Converter\Task\ToTask as TaskToTask;

Loc::loadMessages(__FILE__);

/**
 * Class Task
 * @package Thurly\Tasks\Item
 *
 * @property string $title
 */
final class Task extends \Thurly\Tasks\Item
{
	...
	protected function doPostActions($state)
	{
		if($state->isModeCreate())
		{
			...
			// todo: we can use \Thurly\Tasks\Util\Type\StructureChecker to check such option structure, set default one, etc...
			...
			\CTasks::__updateViewed($id, $userId, $onTaskAdd = true); // todo: create processor instead of it
//			\CTaskCountersProcessor::onAfterTaskAdd($data, $userId);
			\Thurly\Tasks\Internals\Counter::onAfterTaskAdd($data);
			// note that setting occur as is deprecated. use access checker switch off instead
			$occurAsUserId = Util\User::getOccurAsId();
			if(!$occurAsUserId)
			{
			...
			\CTaskNotifications::sendAddMessage(
				array_merge($data, array('CHANGED_BY' => $occurAsUserId)),
			...
			if($parentId)
			{
				\Thurly\Tasks\Internals\Helper\Task\Dependence::attachNew($this->getId(), $parentId);
			}
	...
	/**
	 * Creates new virtual (not presented in database) instance of \Thurly\Tasks\Item\Task\Template based on
	 * data from $this
	 *
	...
	/**
	 * Creates new virtual (not presented in database) instance of \Thurly\Tasks\Item\Task based on
	 * data from $this
	 *
	 * @return Converter\Result
	 */
	...
	private function prepareLegacyData($pristine = false, $onlyModified = false)
	{
		...
		/** @var \Thurly\Tasks\Item\Task\Collection\Tag $tags */
		$tags = $this['SE_TAG'];
		if($tags)
		{
	...
	private function processSchedulerBefore()
	{
		...
		{
			$scheduler = \Thurly\Tasks\Processor\Task\Scheduler::getInstance($this->getUserId());
	...
	private function processSchedulerAfter()
	{
		...
		if($shiftResult instanceof \Thurly\Tasks\Processor\Task\Result)
		{
