...
abstract class UserField // todo: extends Dictionary, to iterate over user field scheme
{
	...
	public static function getDefaultValue($code, $userId = false)
	{
		...
		$typeClass = \Thurly\Tasks\Util\UserField\Type::getClass($field['USER_TYPE_ID']);
	...
	/**
	 * @param $data
	 * @param \Thurly\Tasks\Util\UserField $dstUFController
	 * @param int $userId
	...
	public function cloneValues($data, $dstUFController, $userId = 0, array $parameters = array() /*todo: configure object later */)
	{
		...
		foreach($scheme as $field => $desc)
		{
			...
			)
			{
				...
				$typeClass = \Thurly\Tasks\Util\UserField\Type::getClass($scheme[$field]['USER_TYPE_ID']);
...
//			$item = \Thurly\Main\UserFieldTable::getList(array('limit' => 1, 'filter' => array(
//				'=FIELD_NAME' => $name,
