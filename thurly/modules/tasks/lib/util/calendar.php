...
final class Calendar
{
	...
	public static function getSettings($siteId = false)
	{
		...
		$calendarSettings = \Thurly\Tasks\Integration\Calendar::getSettings();
	 ...
	 * @return bool
	 */
	public static function isDateInRange($date, $rangeStart = '', $rangeEnd = '')
	{
		$date = Datetime::createFrom($date);
		$rangeStart = Datetime::createFrom($rangeStart);
		$rangeEnd = Datetime::createFrom($rangeEnd);

		if (($rangeStart && $rangeStart->checkGT($date)) || ($rangeEnd && $rangeEnd->checkLT($date)))
		{
			return false;
		}
