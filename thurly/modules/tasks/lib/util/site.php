...
final class Site
{
	...
	{
		return \Thurly\Main\Config\Option::get("main", "server_name");
	}
		...
		if(empty(self::$cache['SITE']))
		{
			...
			while($site = $res->Fetch())
			{
				$siteList['LIST'][$site['ID']] = array(
					...
					'SERVER_NAME' => (strlen(trim($site["SERVER_NAME"])) > 0 ? $site["SERVER_NAME"] : \Thurly\Main\Config\Option::get("main", "server_name", $_SERVER["HTTP_HOST"])),
				);
