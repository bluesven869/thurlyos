...
final class Operation
{
	...
	protected function checkClass()
	{
		...
		if(class_exists($this->parsed['CLASS']) && is_subclass_of($this->parsed['CLASS'], 'TasksBaseComponent'))
		{
			...
			{
				throw new \Thurly\Tasks\Exception('Method '.$class.'::allowedMethods() returned a non-array value, too frightful to execute');
			}
			else
			{
		...
		else
		{
			...
			if(class_exists($this->parsed['CLASS']) && is_subclass_of($this->parsed['CLASS'], '\Thurly\Tasks\Dispatcher\PublicAction'))
			{
				$class = $this->parsed['CLASS'];
				$forbiddenMethods = $class::getForbiddenMethods();
				if(!is_array($forbiddenMethods))
				{
					throw new \Thurly\Tasks\Exception('Method '.$class.'::getForbiddenMethods() returned a non-array value, too frightful to execute');
				}
				else
				{
