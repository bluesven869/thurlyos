...
BX.Tasks.Kanban.Item.prototype = {
	...
	{
		var data = this.getData();
		var format = BX.date.convertThurlyFormat(BX.message("FORMAT_DATETIME"));
	...
	render: function()
	{
		...
		if (data.tags && data.tags.length > 0)
		{
			...
			for (var i = 0, c = data.tags.length; i < c; i++)
			{
				this.tag = BX.create("span", {
					text: "#" + data.tags[i],
					events: {
						click: BX.delegate(this.setFilterTag, this)
					}
				});
		...
		{
			BX.bind(this.date_deadline, "click", BX.delegate(this.deadlineTask, this));
		}
		else
		{
			BX.unbind(this.date_deadline, "click", BX.delegate(this.deadlineTask, this));
		}
			...
			this.author.appendChild(BX.create("div", {
				...
				events: {
					click: data.allow_edit
							? BX.delegate(this.changeAuthorTask, this)
							: function() {}
				}
			}));
			...
			this.responsible.appendChild(BX.create("div", {
				...
				events: {
					click: data.allow_delegate
							? BX.delegate(this.delegateTask, this)
							: function() {}
				}
			}));
		...
		this.container = BX.create("div", {
			...
			events: {
				dblclick: function()
				{
					...
					{
						BX.ThurlyOS.PageSlider.open(this.getTaskUrl(this.getId()));
					}
				}.bind(this)
			}
		});
