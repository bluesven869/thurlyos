...
BX.Tasks.Util.Query = BX.Tasks.Util.Base.extend({
    options: {
        url: '/thurly/components/thurly/tasks.base/ajax.php',
        autoExec: false,
    ...
    methods: {
        ...
        execute: function(params)
        {
            ...
            {
	            this.vars.prevLocal = null;
	            this.vars.prevLocal = this.vars.local;
	            this.vars.local = null;
                ...
                BX.ajax({
                    ...
                    start: true,
                    data: {
	                    'sessid': BX.thurly_sessid(), // make security filter feel happy, call variable "sessid" instead of "csrf"
	    ...
	    {
		    this.executeDone(res, params.done, params.pr); // params.done is a callback
		    this.fireEvent('executed', [res]);
	    },

        executeDone: function(res, done, pr)
        {
	        ...
	        if(res.success) // request was parsed successfully
	        {
		        for(var m in data)
		        {
			        if(data.hasOwnProperty(m))
			        {
				        ...
				        // fulfill promise
				        this.vars.prevLocal[m].pr.fulfill(new BX.Tasks.Util.Query.Result(privateErrors, data[m].RESULT));
	        ...
	        else
	        {
		        ...
		        BX.Tasks.each(this.vars.prevLocal, function(request){
