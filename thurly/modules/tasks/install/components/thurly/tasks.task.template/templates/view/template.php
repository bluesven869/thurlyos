			...
			<div class="task-detail-content">
				<? if (strlen($template["DESCRIPTION"])):
					$extraDesc =
						...
						(isset($userFields[$diskUfCode]) && !Thurly\Tasks\Util\UserField::isValueEmpty($userFields[$diskUfCode]["VALUE"]))
					?>
				...
				<?// files\pictures ?>
				<?if (isset($userFields[$diskUfCode]) && !Thurly\Tasks\Util\UserField::isValueEmpty($userFields[$diskUfCode]["VALUE"])):?>
			...
			<div class="task-detail-sidebar-content">
				...
				<?if($template["DEADLINE_AFTER"]
					...
					<?if($template["DEADLINE_AFTER"]):?>
						<div class="task-detail-sidebar-item task-detail-sidebar-item-deadline">
							...
							<div class="task-detail-sidebar-item-value"><?=$helper->formatDateAfter($template["DEADLINE_AFTER"])?></div>
						</div>
					<?endif?>
					...
					<?if($template["START_DATE_PLAN_AFTER"]):?>
						<div class="task-detail-sidebar-item">
							...
							<div class="task-detail-sidebar-item-value"><?=$helper->formatDateAfter($template["START_DATE_PLAN_AFTER"])?></div>
						</div>
					<?endif?>
					...
					<?if($template["END_DATE_PLAN_AFTER"]):?>
						<div class="task-detail-sidebar-item">
							...
							<div class="task-detail-sidebar-item-value"><?=$helper->formatDateAfter($template["END_DATE_PLAN_AFTER"])?></div>
						</div>
					<?endif?>
					...
					<?if($template["ALLOW_TIME_TRACKING"] === "Y" && $template["TIME_ESTIMATE"] > 0):?>
						<div class="task-detail-sidebar-item">
							...
							<div class="task-detail-sidebar-item-value" id="task-detail-estimate-time-<?=$template["ID"]?>">
								<?=\Thurly\Tasks\UI::formatTimeAmount($template["TIME_ESTIMATE"]);?>
							</div>
						</div>
					<?endif?>
				...
				<?if(!$arParams["PUBLIC_MODE"]):?>
					...
					<?if($canUpdate || count($tags)):?>
						...
						<?$tagString = \Thurly\Tasks\UI\Task\Tag::formatTagString($tags);?>
