...
class CTasksDepartmentsOverviewComponent extends CThurlyComponent
{
	public function executeComponent()
	{
		...
		foreach ($arDepartmentsData as $departmentId => $departmentName)
		{
			...
			$rsUsers = \Thurly\Tasks\Integration\Intranet\User::getByDepartments(
				array($departmentId),
				array(
					'ID', 'PERSONAL_PHOTO', 'NAME',
					'LAST_NAME', 'SECOND_NAME', 'LOGIN', 'WORK_POSITION'
				)
			);
			...
			if ($departmentHead > 0)
			{
				$arImmediateEmployees = \Thurly\Tasks\Integration\Intranet\User::getSubordinate(
					$departmentHead,
	...
	private static function getCounts($arUsersIds)
	{
		...
		foreach ($arUsersIds as $userId)
		{
			...
			$effective = \Thurly\Tasks\Internals\Effective::getMiddleCounter($userId, 0, $dateFrom, $dateTo);
		...
		else
		{
			...
			$startFromDepartmentsDraft = array_unique(
				array_filter(
					array_map(
						'intval',
						\Thurly\Tasks\Integration\Intranet\Department::getSubordinateIds(
