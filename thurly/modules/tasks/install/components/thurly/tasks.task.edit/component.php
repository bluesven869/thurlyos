...
if($_SERVER["REQUEST_METHOD"] == "POST" && check_thurly_sessid() && ($arResult["ACTION"] == "create" || $arResult["ACTION"] == "edit"))
{
		...
		function lambda_sgkrg455d_funcCreateSubtasks($arFields, $arAllResponsibles, $index, $loggedInUserId, $woStepper = false, $parameters = array())
		{
			...
			foreach($arResponsibles as $responsible)
			{
				...
				try
				{
					...
					// transform UF files
					if(is_array($arFieldsToSave['UF_TASK_WEBDAV_FILES']) && !empty($arFieldsToSave['UF_TASK_WEBDAV_FILES']) && \Thurly\Main\Loader::includeModule('disk'))
					{
	...
	if (!$bDuplicatePostRequest)
	{
		...
		if (isset($_POST["save"]) || isset($_POST["apply"]))
		{
			...
			if (isset($_POST['MARK']))
				$arFields["MARK"] = (in_array($_POST['MARK'], array('P', 'N')) ? $_POST['MARK'] : false);
			if (
						...
						if(intval($templateId))
						{
							...
							if(is_array($_POST['CHECKLIST_ITEM_ID']))
							{
								...
								if(!empty($items))
								{
									...
									try
									{
										\Thurly\Tasks\Template\CheckListTable::updateForTemplate($templateId, $items);
									}
			...
			if ($arTemplate = $rsTemplate->GetNext())
			{
				if ($arTemplate["CREATED_BY"] == $loggedInUserId)
				{
					...
					if ($arTemplate["DEADLINE_AFTER"])
					{
						...
						$arTemplate["DEADLINE"] = \Thurly\Tasks\UI::formatDateTime(strtotime(date("Y-m-d 00:00")." +".$deadlineAfter." days"));
					}

					// check list items
					$res = \Thurly\Tasks\Template\CheckListTable::getList(array(
