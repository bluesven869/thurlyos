...
var tasksEditNS = {
		...
		this.checkWarningsId = window.setTimeout(
			function()
			{
				...
				var oAjaxReply = BX.ajax({
					method   : 'POST',
					dataType : 'html',
					url      : '/thurly/components/thurly/tasks.task.edit/ajax.php',
					data     :  data,
					async    :  false
				});
	...
	onFilesChange : function()
	{
		...
		var form = BX.create("form", {
			props : {
				method : "post",
				action : "/thurly/components/thurly/tasks.task.edit/upload.php",
			...
			children : [
				...
				BX.create("input", {
					props : {
						type : "hidden",
						name : "sessid",
						value : BX.message("thurly_sessid")
					}
				}),
				BX.create("input", {
					props : {
...
function fillPlanDates() {
	if ((duration = parseInt(BX("task-duration").value, 10)) && duration > 0)
	{
		...
		if (BX("task-start-date").value.length > 0)
		{
			...
			var value = BX.date.format(BX.date.convertThurlyFormat(BX.message('FORMAT_DATETIME')), endDate);
		...
		else if(BX("task-end-date").value.length > 0)
		{
			...
			var value = BX.date.format(BX.date.convertThurlyFormat(BX.message('FORMAT_DATETIME')), startDate);
	...
	_deleteFile : function (e)
	{
		...
		if (confirm(BX.message("TASKS_DELETE_CONFIRM"))) {
			if (!BX.hasClass(this.parentNode, "saved"))
			{
				...
				var url = "/thurly/components/thurly/tasks.task.edit/upload.php";
