...
use \Thurly\Tasks\Kanban\StagesTable;
use \Thurly\Tasks\Kanban\TaskStageTable;
use \Thurly\Tasks\ProjectsTable;
use \Thurly\Tasks\Helper\Filter;
use \Thurly\Tasks\Internals\Task;
use \Thurly\Tasks\Integration\Disk\Connector\Task as ConnectorTask;

class TasksKanbanComponent extends \CThurlyComponent
{
	...
	protected function init()
	{
		...
		if ($init && ($this->userId = (int) \Thurly\Tasks\Util\User::getId()) <= 0)
		{
...
//		);
	...
	protected function getAdmins()
	{
		$users = array();
		// site admins
		$res = \Thurly\Main\UserGroupTable::getList(array(
			'filter' => array(
				'GROUP_ID' => 1
			)
		));
	...
	protected function getLastGroupId()
	{
		...
		$res = \Thurly\Socialnetwork\WorkgroupViewTable::getList(array(
			'select' => array(
				'GROUP_ID'
			),
			'filter' => array(
	...
	{
		$context = Application::getInstance()->getContext();
		$uri = new \Thurly\Main\Web\Uri($context->getRequest()->getRequestUri());
	...
	protected function getTimeStarted(array $items)
	{
		...
		$res = \Thurly\Tasks\Internals\Task\TimerTable::getList(array(
			'filter' => array(
				'TASK_ID' => array_keys($items),
				'>TIMER_STARTED_AT' => 0
			)
	...
	{
		$event = new \Thurly\Main\Event('tasks', $codeEvent, array(
			'DATA' => $data,
		));
		$event->send();
		foreach ($event->getResults() as $result)
		{
			if ($result->getResultType() != \Thurly\Main\EventResult::ERROR)
			{
	...
	public function executeComponentAjax()
	{
		...
		if (is_array($return))
		{
			...
			echo \Thurly\Main\Web\Json::encode($return);
		}
		// else simple html
		else
		{
	...
	protected function actionChangeGroup()
	{
		...
		{
			\Thurly\Socialnetwork\WorkgroupViewTable::set(array(
				'GROUP_ID' => $this->arParams['GROUP_ID'],
				'USER_ID' => $this->userId
			));
		}
