...
$GLOBALS['APPLICATION']->AddHeadScript("/thurly/components/thurly/tasks.templates.list/templates/.default/script.js");
...
var ajaxUrl = "/thurly/components/thurly/tasks.templates.list/ajax.php?SITE_ID=<?php echo SITE_ID?>";
</script>
	...
	<?if($arParams['SHOW_GROUP_ACTIONS'] !== 'N'):?>
		...
		<form action="<?=POST_FORM_ACTION_URI?>" id="task-list-group-operations">
			<div class="task-table-footer">
				<input type="checkbox" <?//onclick="active_btn(this)"?> id="task_list_group_action_all" class="task-table-foot-checkbox" title="<?=GetMessage('TASKS_LIST_TOOLTIP_FOR_ALL_TEMPLATE_ITEMS')?>"><label class="task-table-foot-label" for="task_list_group_action_all"><?=GetMessage('TASKS_LIST_GROUP_ACTION_FOR_ALL')?>
	...
	$APPLICATION->IncludeComponent(
		...
		array(
			...
			'ADD_BUTTON' => array(
				...
				).(isset($arParams['BASE_TEMPLATE_ID']) ? '?BASE_TEMPLATE='.intval($arParams['BASE_TEMPLATE_ID']) : '')
			)
		)
	);
}
...
var tasksListAjaxUrl = "/thurly/components/thurly/tasks.templates.list/ajax.php";
