...
if (check_thurly_sessid())
{
	...
	{
		$userIsAdmin = \Thurly\Tasks\Integration\SocialNetwork\User::isAdmin();

		$template = \Thurly\Tasks\Item\Task\Template::findOne(array(
		...
		if ($template)
		{
			...
			elseif ($_POST["mode"] == "load")
			{
				...
				$childCounts = \Thurly\Tasks\Internals\Helper\Task\Template\Dependence::getDirectChildCount($ids, array(
					'USER_ID' => $cUserId
				));
				...
				$ops = \Thurly\Tasks\Util\User::getAccessOperationsForEntity('task_template');
				$allowed = \Thurly\Tasks\Internals\Helper\Task\Template\Access::getAvailableOperations($ids, array(
					'USER_ID' => $cUserId
				));
