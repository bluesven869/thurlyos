...
use Thurly\Tasks\Helper\Grid;
use Thurly\Tasks\Helper\Filter;
...
class TasksTaskListComponent extends TasksBaseComponent
{
	...
	{
		$filter = Filter::getInstance(\Thurly\Tasks\Util\User::getId());
	...
	{
		$submodes = $this->filter->getListStateInstance()->getSubmodes();

		return $this->arParams['GROUP_ID'] == 0 && $submodes['VIEW_SUBMODE_WITH_GROUPS']['SELECTED'] == 'Y';
	}
	...
	protected function doPreAction()
	{
		...
		if ($this->needGroupBySubTasks())
		{
			...
			if (\Thurly\Main\Grid\Context::isInternalRequest() &&
				check_thurly_sessid() &&
				$_REQUEST['action'] == \Thurly\Main\Grid\Actions::GRID_GET_CHILD_ROWS)
			{
	...
	{
		if (!Thurly\Main\Grid\Context::isInternalRequest() || !check_thurly_sessid())
		{
			return false;
		}
		...
		$plan = new \Thurly\Tasks\Dispatcher\ToDo\Plan();
		$todo = array();
	...
	protected function doPostAction()
	{
		...
		$this->arParams['VIEW_STATE'] = $this->filter->getListStateInstance()->getState();

		if ($this->userId != $this->arParams['USER_ID'])
		{
			$users = \Thurly\Tasks\Util\User::getData(array($this->arParams['USER_ID']));
		...
		$oTimer = CTaskTimerManager::getInstance(\Thurly\Tasks\Util\User::getId());
	...
	protected function getOrder()
	{
		...
		if ($this->arParams['NEED_GROUP_BY_GROUPS'] == 'Y')
		{
		...
		if ($request->get('SORTF') != null &&
			in_array($request->get('SORTF'), \Thurly\Tasks\Ui\Controls\Column::getFieldsForSorting())
		)
		{
	...
	protected function mergeWithTags(array $items)
	{
		...
		$res = \Thurly\Tasks\Internals\Task\TagTable::getList(array(
			'select' => array(
				'TASK_ID', 'NAME'
			),
			'filter' => array(
