...
class TasksWidgetAccessComponent extends TasksBaseComponent
{
	protected function checkParameters()
	{
		...
		foreach($this->arParams['DATA'] as $k => $item)
		{
			...
			if($item instanceof \Thurly\Tasks\Item\Task\Template\Access)
			{
	...
	{
		$levels = \Thurly\Tasks\Util\User::getAccessLevelsForEntity($this->arParams['ENTITY_CODE']);
		if(!count($levels))
		{
	...
	protected function getMissingUserData()
	{
		...
		if(count($unKnownIds))
		{
			$users = \Thurly\Tasks\Util\User::getData($unKnownIds);
