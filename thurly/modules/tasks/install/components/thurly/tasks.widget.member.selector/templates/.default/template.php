...
<?if(!$helper->checkHasFatals()):?>
	...
	<div id="<?=$helper->getScopeId()?>" class="tasks task-form-field <?=$arParams['DISPLAY']?> <?=($arParams['READ_ONLY'] ? 'readonly' : '')?>" <?if($arParams['MAX_WIDTH'] > 0):?>style="max-width: <?=$arParams['MAX_WIDTH']?>px"<?endif?>>
	    ...
	    <span class="task-form-field-controls">
		    ...
		    <?else:?>
			    <?$add = $arParams['MAX'] > 1;?>
			    <a href="javascript:void(0);" class="js-id-tdp-mem-sel-is-open-form task-form-field-when-filled task-form-field-link <?if($add):?>add<?endif?>">
				    <?=Loc::getMessage($add ? 'TASKS_COMMON_ADD_MORE' : 'TASKS_COMMON_CHANGE')?>
			    </a>
			    <a href="javascript:void(0);" class="js-id-tdp-mem-sel-is-open-form task-form-field-when-empty task-form-field-link add">
