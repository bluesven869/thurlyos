...
$arResult['HELPER'] = $helper = new \Thurly\Tasks\UI\Component\TemplateHelper(null, $this, array(
	'RELATION' => array(
		'tasks_itemsetpicker',
		'tasks_util',
		'tasks_util_query',
...
$uUrl = \Thurly\Tasks\UI::convertActionPathToBarNotation(
	$helper->findParameterValue('PATH_TO_USER_PROFILE'),
	array('user_id' => 'ID')
);
$gUrl = \Thurly\Tasks\UI::convertActionPathToBarNotation(
...
foreach($arParams['DATA'] as $i => $item)
{
	...
	if($entityType == $uPref)
	{
		...
		{
			$formatted = \Thurly\Tasks\Util\User::formatName($item, false, $arParams["NAME_TEMPLATE"]);
			if($formatted != 'Noname')
			{
