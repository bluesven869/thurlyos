					...
					<?if(!empty($data['SE_CHECKLIST'])):?>
						...
						<p style="font-size:14px;">
							...
							<?foreach($data['SE_CHECKLIST'] as $item):?>
								<?if(\Thurly\Tasks\UI\Task\CheckList::checkIsSeparatorValue($item['TITLE'])):?>
						...
						<?foreach($arResult['AUX_DATA']['CHANGES'] as $key => $change):?>
							...
							switch ($key)
							{
								case 'TIME_ESTIMATE':
									...
									{
										$value = \Thurly\Tasks\UI::formatTimeAmount($change['TO_VALUE']);
									}
									break;
								...
								case "CREATED_BY":
									$value = htmlspecialcharsbx(\Thurly\Tasks\Util\User::formatName($data['SE_ORIGINATOR']));
									break;
								case "RESPONSIBLE_ID":
									$value = htmlspecialcharsbx(\Thurly\Tasks\Util\User::formatName(array_shift($data['SE_RESPONSIBLE'])));
									break;
								...
								case "TAGS":
									$value = htmlspecialcharsbx(\Thurly\Tasks\Ui\Task\Tag::formatTagString($data['SE_TAG']));
									break;
