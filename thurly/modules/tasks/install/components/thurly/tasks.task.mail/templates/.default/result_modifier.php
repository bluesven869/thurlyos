...
$sender['NAME_FORMATTED'] = \Thurly\Tasks\Util\User::formatName($sender, $siteId);
$sender['PERSONAL_GENDER'] = $sender['PERSONAL_GENDER'] == 'F' ? 'F' : 'M';

$receiver =& $arResult['DATA']['MEMBERS']['RECEIVER'];
$receiver['NAME_FORMATTED'] = \Thurly\Tasks\Util\User::formatName($receiver, $siteId);
...
{
	$data['DEADLINE'] = \Thurly\Tasks\Integration\SocialNetwork::formatDateTimeToGMT($data['DEADLINE'], $userId);
}
if((string) $data['START_DATE_PLAN'] != '')
{
	$data['START_DATE_PLAN'] = \Thurly\Tasks\Integration\SocialNetwork::formatDateTimeToGMT($data['START_DATE_PLAN'], $userId);
}
if((string) $data['END_DATE_PLAN'] != '')
{
	$data['END_DATE_PLAN'] = \Thurly\Tasks\Integration\SocialNetwork::formatDateTimeToGMT($data['END_DATE_PLAN'], $userId);
}
		...
		if($k == 'AUDITORS' || $k == 'ACCOMPLICES')
		{
			...
			if(is_array($v))
			{
				foreach($v as $mId)
				{
					...
					{
						$name = \Thurly\Tasks\Util\User::formatName($arResult['DATA']['USER'][$mId], $siteId);
						if((string) $name != '')
						{
