...
{
	$item['DISPLAY'] = htmlspecialcharsBack(\Thurly\Tasks\UI::convertBBCodeToHtmlSimple($item['TITLE']));

	// textParser returns escaped data, we want unescaped
	$item['TITLE'] = strip_tags(\Thurly\Tasks\UI::convertBBCodeToHtmlSimple($item['TITLE']));//htmlspecialcharsBack(\Thurly\Tasks\UI::convertBBCodeToHtmlSimple($item['TITLE']));
	...
	{
		$item['VALUE'] = 'n'.abs(Thurly\Tasks\Util::hashCode(rand(100, 999).rand(100, 999)));
	}
	...
	$isSeparator = \Thurly\Tasks\UI\Task\CheckList::checkIsSeparatorValue($item['TITLE']);
	if(!$isSeparator)
	{
		$total++;
	}
