...
)
{
	...
	if ($arParams['SHOW_SECTION_MANAGE'] === 'A')
	{
		...
		if ($isAccessToCounters)
		{
			if(\Thurly\Tasks\Integration\Intranet\User::isDirector($arParams['USER_ID']))
			{
	...
	if ($arResult['SHOW_SECTION_MANAGE'] === 'Y')
	{
		...
		if ($arEmployees = \Thurly\Tasks\Integration\Intranet\User::getSubordinate($arParams['USER_ID']))
		{
...
$arResult["DESTINATION"] = Thurly\Tasks\Integration\SocialNetwork::getLogDestination('TASKS', array(
	'USE_PROJECTS' => 'Y'
));
