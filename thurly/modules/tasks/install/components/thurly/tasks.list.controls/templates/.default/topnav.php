...
$taskListGlobalOpts = array(
	'enable_gantt_hint' => \Thurly\Main\Config\Option::get('tasks', 'task_list_enable_gantt_hint')
);
...
if ($arParams["SHOW_SECTIONS_BAR"] === "Y")
{
	...
	$menuItems[] = array(
		...
		"ID" => "view_all",
		'COUNTER' => $oCounters->get(\Thurly\Tasks\Internals\Counter\Name::TOTAL),
	...
	$counters = array(
		"view_role_responsible" => \Thurly\Tasks\Internals\Counter\Name::MY,
		"view_role_accomplice" => \Thurly\Tasks\Internals\Counter\Name::ACCOMPLICES,
		"view_role_auditor" => \Thurly\Tasks\Internals\Counter\Name::AUDITOR,
		"view_role_originator" => \Thurly\Tasks\Internals\Counter\Name::ORIGINATOR
	);

	$myPlace = $arParams['USER_ID'] == \Thurly\Tasks\Util\User::getId();
...
*/
	if ($arResult['SHOW_SECTION_PROJECTS'] == 'Y')
	{
		$menuItems[] = array(
			...
			'SUB_LINK'=>array(
				'CLASS'=>'',
				'URL'=> \Thurly\Main\Config\Option::get('socialnetwork', 'user_page', SITE_DIR.'company/personal/').
						'user/'.
						\Thurly\Tasks\Util\User::getId().'/groups/create/?firstRow=project'),
		);
	}
	...
	if (!\Thurly\Tasks\Integration\Extranet\User::isExtranet() && !intval($arParams['GROUP_ID']))
	{
		$menuItems[] = array(
	...
	if (\Thurly\Main\ModuleManager::isModuleInstalled("trash"))
	{
		$menuItems[] = array(
			"TEXT" => GetMessage("TASKS_PANEL_TAB_TRASH"),
			"URL" => CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_TRASH"], array()),
			"ID" => "view_trash",
	...
	$postfix = '';//$arParams["USER_ID"] === \Thurly\Tasks\Util\User::getId() ? "" : "_".$arParams["USER_ID"];
		...
		<div class="task-top-panel-righttop" id="task-new-item">
			...
			?><script>
				new BX.Tasks.QuickForm("task-new-item", {
					...
					canManageTask: <?=CUtil::PhpToJSObject(\Thurly\Tasks\Util\Restriction::canManageTask())?>,
					messages: {
