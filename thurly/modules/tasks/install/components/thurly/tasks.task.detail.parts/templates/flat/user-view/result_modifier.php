...
else
{
    ...
    if($needCurrentUser)
    {
        $formattedUsers[$currentUser] = \Thurly\Tasks\Util\User::extractPublicData($arResult['TEMPLATE_DATA']['USER']);
    }

	$arParams["PATH_TO_USER_PROFILE"] = \Thurly\Tasks\Integration\Socialnetwork\Task::addContextToURL($arParams["PATH_TO_USER_PROFILE"], $arResult['TEMPLATE_DATA']['TASK_ID']);

    foreach($formattedUsers as $i => $item)
    {
        $formattedUsers[$i]['AVATAR'] = \Thurly\Tasks\UI::getAvatar($item['PERSONAL_PHOTO'], 100, 100);
...
$arResult['TEMPLATE_DATA']['CAN_ADD_MAIL_USERS'] = \Thurly\Main\ModuleManager::isModuleInstalled("mail");