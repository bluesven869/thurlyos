...
(function(){
	...
	if(typeof BX.Tasks.Component.TasksWidgetReplication != 'undefined')
	{
		return;
	}
	...
	var timePicker = BX.Tasks.Util.Widget.extend({
		...
		methods: {
			construct: function()
			{
				...
				this.bindDelegateControl('display', 'click', this.openClock.bind(this));

				this.vars.formatDisplay = BX.date.convertThurlyFormat(BX.message('FORMAT_DATETIME').replace(BX.message('FORMAT_DATE'), '').replace(':SS', '').replace('/SS', '').trim());
				this.vars.formatValue = BX.date.convertThurlyFormat('HH:MI');
	 ...
	 */
	BX.Tasks.Component.TasksWidgetReplication = BX.Tasks.Component.extend({
		sys: {
			code: 'replication'
		},
		options: {
		...
		methods: {
			...
			getEndDate: function()
			{
				return this.subInstance('end-date-datepicker', function(){
					return new BX.Tasks.Util.DatePicker({
						scope: this.control('end-date-datepicker'),
						controlBind: 'class',
						displayFormat: 'system-short',
						defaultTime: {H: 0, M: 0, S:0}
					});
			...
			getTime: function()
			{
				return this.subInstance('creation-time', function(){
			...
			bindEvents: function()
			{
				...
				this.getStartDate().bindEvent('change', handler);
				this.getEndDate().bindEvent('change', handler);
				this.getTime().bindEvent('change', handler);
			...
			onUpdateHint: function()
			{
				...
				var params = {
					...
					'START_DATE': this.getStartDate().getValue(),
			...
			makeHintText: function (params)
			{
				...
				if (params.START_DATE != '')
				{
					...
					var shortStart = BX.date.format(BX.date.convertThurlyFormat(BX.message('FORMAT_DATE')), BX.parseDate(params.START_DATE, true), false, true);

					constraint += BX.message('TASKS_TTDP_REPLICATION_HINT_START_CONSTRAINT').replace('#DATETIME#', shortStart);
				}
				else
				{
				...
				{
					// start date to short format
					var shortEnd = BX.date.format(BX.date.convertThurlyFormat(BX.message('FORMAT_DATE')), BX.parseDate(params.END_DATE, true), false, true);

					constraint += BX.message('TASKS_TTDP_REPLICATION_HINT_END_CONSTRAINT').replace('#DATETIME#', shortEnd);
					endless = false;
				}
