...
use Thurly\Tasks\Integration\Report\Internals\TaskTable;
use Thurly\Tasks\Integration\Intranet\Department;
...
class TasksEmployeePlanComponent extends TasksBaseComponent
{
	...
	protected static function prepareQueryParametersUsers($filter, Collection $errors, array $params)
	{
		...
		if($params['DEPARTMENT_UF_EXISTS'])
		{
			...
			{
				$uParams = Thurly\Tasks\Internals\RunTime::apply($uParams, array(
					$constraint,
		...
		if(is_array($filter['MEMBER']))
		{
			...
			$user = \Thurly\Tasks\Util\Type::normalizeArrayOfUInteger($member['USER']);
			$department = \Thurly\Tasks\Util\Type::normalizeArrayOfUInteger($member['DEPARTMENT']);
	...
	protected static function prepareQueryParameters(array $filter = array())
	{
		...
		$departmentUFExists = \Thurly\Tasks\Util\Userfield\User::checkFieldExists($departmentUFCode);
	...
	protected static function getGridRegionData(array $userParameters = array(), array $taskParameters = array())
	{
		...
		$qUser = new \Thurly\Main\Entity\Query(UserTable::getEntity());
		$qTask = new \Thurly\Main\Entity\Query(TaskTable::getEntity());
		...
		if(!empty($ids)) // some users match the user filter, take their tasks
		{
			$taskParameters['runtime']['M'] = array(
				...
				'reference' => array(
					'=this.ID' => 'ref.TASK_ID',
					'=ref.TYPE' => array('?', 'A'),
					'@ref.USER_ID' => new \Thurly\Main\DB\SqlExpression(implode(', ', $ids)),
				),
				'join_type' => 'left'
			);
			...
			if(!$isSuperUser)
			{
				$myEmployees = array_flip(\Thurly\Tasks\Integration\Intranet\User::getSubordinateSubDepartments());
			}
	...
	private static function applyQueryParameters($query, array $parameters)
	{
		if(is_array($parameters['runtime']))
		{
			...
			{
				$isObject = is_subclass_of($v, '\Thurly\Main\Entity\Field');
	...
	protected static function correctDateRange($range, Collection $errors)
	{
		...
		if($from != '' && $to != '')
		{
			$fromTs =   \Thurly\Tasks\UI::parseDateTime($from);
			$toTs =     \Thurly\Tasks\UI::parseDateTime($to);
		...
		{
			// set $from + max length
			$to = \Thurly\Tasks\UI::formatDateTime(\Thurly\Tasks\UI::parseDateTime($from) + $limit);
		}
		elseif($from == '' && $to != '')
		{
			// set $to - max length
			$from = \Thurly\Tasks\UI::formatDateTime(\Thurly\Tasks\UI::parseDateTime($to) - $limit);
		}
		else
		{
			// set default
			// todo: we need a handy library to perform widely-used operations with dates in three formats:
			// todo: 1) timestamp(), 2) string site format, 3) \Thurly\Main\Type\DateTime
			$now = strtotime(date("Y-m-d 00:00:00", User::getTime()));

			$oneThird = floor($limit / 3);
			$from = \Thurly\Tasks\UI::formatDateTime($now - $oneThird);
			$to = \Thurly\Tasks\UI::formatDateTime($now + $oneThird*2);
		}
...
{
	final class TasksEmployeePlanComponentFilterStorage extends \Thurly\Tasks\Util\Type\ArrayOption
	{
			...
			return array(
				...
				'TASK' => array('VALUE' => array(
					...
					'DATE_RANGE' => array('VALUE' => array(
						'FROM' => array('VALUE' => '\Thurly\Tasks\UI::checkDateTime'),
						'TO' => array('VALUE' => '\Thurly\Tasks\UI::checkDateTime'),
					), 'DEFAULT' => array()),
				), 'DEFAULT' => array())
			);
		}
		...
		public function set(array $value)
		{
			...
			User::setOption(static::getFilterOptionName(), \Thurly\Tasks\Util\Type::serializeArray($value));
		}
		...
		public function get()
		{
			...
			$value = \Thurly\Tasks\Util\Type::unSerializeArray($this->fetchOptionValue());
