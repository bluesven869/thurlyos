...
{
	if ($field["VALUE"] !== false && !empty($field["VALUE"]) && $fieldId !== \Thurly\Tasks\Integration\Disk\UserField::getMainSysUFCode())
	{
...
if (is_array($arParams["TASK_URL_PARAMETERS"]) && !empty($arParams["TASK_URL_PARAMETERS"]))
{
	...
	{
		$arParams["PATH_TO_TASKS_TASK"] = \Thurly\Tasks\Util::replaceUrlParameters(
...
$arResult["TEMPLATE_DATA"]["TASK_TEMPLATES_PATH"] = CComponentEngine::makePathFromTemplate(
	...
	array("user_id" => \Thurly\Tasks\Util\User::getId())
);
...
{
	$arResult["TEMPLATE_DATA"]["FILES_IN_COMMENTS"] = \Thurly\Tasks\Integration\Forum\Task\Topic::getFileCount($taskData["FORUM_TOPIC_ID"], $taskData["FORUM_ID"]);
}
