...
use Thurly\Tasks\Integration\SocialNetwork;
use Thurly\Tasks\Integration\Forum\Task\Topic;
...
class TasksTaskComponent extends TasksBaseComponent
{
	...
	private function manageTemplates()
	{
		...
		// todo: move logic from \Thurly\Tasks\Manager\Task\Template::manageTaskReplication() here
	...
	private function createTemplate($taskId)
	{
		...
		{
			$task = new \Thurly\Tasks\Item\Task($taskId, $this->userId); // todo: use Task::getInstance($taskId, $this->userId) here, when ready
			if($task['REPLICATE'] != 'Y')
			{
			...
			// or else it was created above, inside \Thurly\Tasks\Manager\Task\Template::manageTaskReplication()
		}
	}
	...
	protected function getDataRequest()
	{
		...
		else
		{
			...
			if($parentId)
			{
				$parentTask = Thurly\Tasks\Item\Task::getInstance($parentId, $this->userId);
	...
	protected function getData()
	{
			...
			else
			{
				...
				try
				{
					if(intval($this->request['TEMPLATE'])) // copy from template?
					{
						...
						$requestParams = array();
						$wantedFields = array('UF_CRM_TASK', 'TITLE', 'TAGS', 'GROUP_ID');
						foreach($wantedFields as $field)
						{
							...
							if ($value)
							{
								if ($field == 'TAGS')
								{
									...
									foreach ($_value as $__v)
									{
										$requestParams[ 'SE_TAG' ][] = array('NAME'=>$__v);
									}
								}
								else
								{
						...
						$sourceData = $this->cloneDataFromTemplate(intval($this->request['TEMPLATE']));

						if(count($requestParams) > 0)
						{
							$sourceData['DATA'] = Task::mergeData($requestParams, $sourceData['DATA']);
						}
					...
					{
						$sourceData = $this->cloneDataFromTask(intval($this->request['COPY']));
				...
				catch(\Thurly\Tasks\AccessDeniedException $e)
				{
					$error = 'access';
				}
		...
		{
			$this->arResult['DATA']['STAGES'] = \Thurly\Tasks\Kanban\StagesTable::getStages(
				$data['DATA']['GROUP_ID'],
				true
			);
	...
	protected function getDataAux()
	{
		...
		$this->arResult['AUX_DATA']['HINT_STATE'] = \Thurly\Tasks\UI::getHintState();
		$this->arResult['AUX_DATA']['MAIL'] = array(
			//'FORWARD' => \Thurly\Tasks\Integration\Mail\Task::getReplyTo($this->userId, $this->arResult['DATA']['TASK']['ID'], 'dummy', SITE_ID)
		);
	}

	protected function getDataTemplates()
	{
		// todo: use \Thurly\Tasks\Item\Task\Template::find() here, and check rights
		$res = CTaskTemplates::GetList(
			...
			array(
				'USER_ID' => $this->userId,
				'USER_IS_ADMIN' => \Thurly\Tasks\Integration\SocialNetwork\User::isAdmin(),
			),
			array('ID', 'TITLE')
		);
...
//		$this->arResult['DATA']['TASK'][Task\Project::getCode(true)]['ID'] = $this->arResult['DATA']['TASK']['GROUP_ID']; // ?
	...
	protected function getCurrentUserData()
	{
		...
		$currentUser['IS_SUPER_USER'] = \Thurly\Tasks\Util\User::isSuper($this->userId);
		$roles = array(
			'ORIGINATOR' => false,
	...
	{
		return \Thurly\Tasks\Util\UserField\Task::getScheme($entityId);
	}
	...
	private function setResponsibles($users)
	{
		...
		{
			$this->responsibles = \Thurly\Tasks\Util\Type::normalizeArray($users);
		}
	}
	...
	private static function inviteUsers(array &$users, Collection $errors)
	{
		foreach($users as $i => $user)
		{
			if(is_array($user) && !intval($user['ID']))
			{
				...
				{
					$newId = \Thurly\Tasks\Integration\Mail\User::create($user);
					if($newId)
					{
						$users[$i]['ID'] = $newId;
					}
	...
	public static function getFiles($params)
	{
		...
		$APPLICATION->IncludeComponent(
			"thurly:disk.uf.comments.attached.objects",
			".default",
			array(
				"MAIN_ENTITY" => array(
					"ID" => $params["TASK_ID"]
			...
			return array(
				'INITIAL_TASK_DATA' => array(
					'VALUE' => array(
						...
						'TAGS' => array('VALUE' => StructureChecker::TYPE_ARRAY_OF_STRING, 'CAST' => function($value){
							if(!is_array($value))
							{
