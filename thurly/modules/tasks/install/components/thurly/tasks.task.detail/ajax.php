...
if (check_thurly_sessid() && ($loggedInUserId > 0))
{
	$action = '';
	...
	if ($action === 'render_task_log_last_row_with_date_change')
	{
		...
		if ($arData !== false)
		{
			...
			{
				$strDateFrom = \Thurly\Tasks\UI::formatDateTime($arData['FROM_VALUE'], '^'.\Thurly\Tasks\UI::getDateTimeFormat());
			}

			if ($arData['TO_VALUE'])
			{
				$strDateTo = \Thurly\Tasks\UI::formatDateTime($arData['TO_VALUE'], '^'.\Thurly\Tasks\UI::getDateTimeFormat());
			}
