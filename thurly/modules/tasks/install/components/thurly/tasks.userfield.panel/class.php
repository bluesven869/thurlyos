...
class TasksUserFieldPanelComponent extends TasksBaseComponent
{
	...
	protected function getData()
	{
		...
		if($this->ctrl)
		{
			if(!Restriction::canUse($this->ctrl->getEntityCode()))
			{
			...
			else
			{
				...
				foreach($fields as $ufCode => $ufDesc)
				{
					...
					{
						$fields[$ufCode]['VALUE'] = $this->arParams['DATA'][$ufCode];
					}

					$fields[$ufCode]['LABEL'] = (string) $ufDesc['EDIT_FORM_LABEL'] != '' ? $ufDesc['EDIT_FORM_LABEL'] : $ufDesc['FIELD_NAME_ORIG'];
				}
		...
		$this->arResult['COMPONENT_DATA']['RESTRICTION'] = array(
			'USE' => Restriction::canUse($this->ctrl->getEntityCode(), $this->userId),
			'MANAGE' => Restriction::canManage($this->ctrl->getEntityCode(), $this->userId),
			'CREATE_MANDATORY' => Restriction::canCreateMandatory($this->ctrl->getEntityCode(), $this->userId),
		);
	}
	...
	protected static function getItemControllerByEntity($entityCode, $entityId = 0)
	{
		...
		if($entityCode == 'TASK')
		{
			return new \Thurly\Tasks\Item\Task($entityId);
		}
		elseif($entityCode == 'TASK_TEMPLATE')
		{
			return new \Thurly\Tasks\Item\Task\Template($entityId);
		}
		else
		{
			return null;
	...
	public static function saveField($id = 0, array $data, array $parameters = array())
	{
		...
		if(!$ufController)
		{
			$result->addError('ILLEGAL_ARGUMENT.ENTITY_CODE', 'Unknown entity code');
		}
		...
		{
			$result->addError('ACTION_NOT_ALLOWED', 'Only admin can do field managing');
		}
		if($label == '')
		{
		...
		else // create a new one
		{
			...
			{
				$result->addError('ILLEGAL_ARGUMENT.USER_TYPE_ID', 'Unknown field type');
			}

			$freeName = static::getFreeFieldName($data['ENTITY_CODE'], $parameters['RELATED_ENTITIES']);
			if(!$freeName)
			{
				$result->addError('INTERNAL_ERROR', 'Cannot generate appropriate name');
			}
...
//					),
	...
	public static function deleteField($id, array $parameters = array())
	{
		...
		if(!intval($id))
		{
			$result->addError('ILLEGAL_ARGUMENT.ID', 'Illegal field id');
		}
		else
		{
			if(!is_array($parameters['RELATED_ENTITIES']))
			...
			if(!$data)
			{
				$result->addError('FIELD_NOT_FOUND', 'Field not found');
			}
			else
			{
...
//					}
	...
	public static function getFieldHTML($id, $entityCode, $entityId = 0, array $parameters = array())
	{
		...
		if($result->isSuccess())
		{
			...
			if($code == '')
			{
				$result->addError('ILLEGAL_ARGUMENT.ID', 'Unknown field id');
			}
			else
			{
	...
	public static function setState(array $state, $entityCode, $dropAll = false)
	{
		...
		else
		{
			...
			{
				if($dropAll && !\Thurly\Tasks\Util\User::isSuper())
				{
					$result->addError('ACTION_NOT_ALLOWED', 'Easier...');
				}
				else
				{
	...
	private static function addRelatedFields($relatedEntities, $field, $result)
	{
		foreach($relatedEntities as $relatedEntity)
		{
			...
			if($controller)
			{
				...
				{
					$result->addWarning('INTERNAL_ERROR', 'Some related fields were not created');
				}
			}
		}
	}
	...
	private static function getFieldUIHtml($fieldData, $preferDefault = false)
	{
		...
		ob_start();
		\Thurly\Tasks\Util\UserField\UI::showEdit($fieldData, array(
...
{
	final class TasksUserFieldPanelComponentState extends Thurly\Tasks\Util\Type\ArrayOption
	{
