...
ThurlyOS\UI::initLicensePopup('');
?>

<?if(!$arResult['COMPONENT_DATA']['RESTRICTION']['USE']):?>
	<div class="tasks-btn-restricted tasks-uf-panel-restricted">
		<?=Loc::getMessage('TASKS_TUFP_LICENSE_RESTRICTED');?> <a href="<?=ThurlyOS\UI::getLicenseUrl()?>" target="_blank"><?=Loc::getMessage('TASKS_TUFP_SHOW_DETAILS');?></a>
	</div>
<?else:?>
					...
					<div class="js-id-item-set-items js-id-uf-panel-items tasks-uf-panel-items not-empty">
						...
						<?foreach($arResult['DATA']['STATE'] as $id => $state):?>
							...
							if($state['D'])
							{
								...
								ob_start();
								\Thurly\Tasks\Util\UserField\UI::showEdit($uf, array(
									'PREFER_DEFAULT' => !intval($arParams['DATA']['ID'])
								), $this->__component);
								$html = ob_get_clean();

								// replace date icon, no ability to do it in other way
								$html = str_replace('/thurly/js/main/core/images/calendar-icon.gif', '/thurly/js/tasks/css/images/calendar.png', $html);
							}
							?>
