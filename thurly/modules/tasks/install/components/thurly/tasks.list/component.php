...
$loggedInUserId = (int) \Thurly\Tasks\Util\User::getId();
...
$oListState = CTaskListState::getInstance(\Thurly\Tasks\Util\User::getId());
...
if (!$arResult["USER"])
{
	$rsUser = CUser::GetByID(\Thurly\Tasks\Util\User::getId());
	$arResult["USER"] = $rsUser->GetNext();
}

$arParams["PATH_TO_TEMPLATES"] = str_replace("#user_id#", \Thurly\Tasks\Util\User::getId(), $arParams["PATH_TO_USER_TASKS_TEMPLATES"]);
// PATH_* block end

// filter

if ($taskType == "group" || $arParams["USER_ID"] == \Thurly\Tasks\Util\User::getId())
{
...
{
	if ($arParams["USER_ID"] == \Thurly\Tasks\Util\User::getId())
	{
...
if (isset($_GET['FORK_RECOUNT']) && ($_GET['FORK_RECOUNT'] == 1))
{
	CAgent::AddAgent(
