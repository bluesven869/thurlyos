...
Class tasks extends CModule
{
	...
	function InstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/db/".strtolower($DB->type)."/install.sql");
		}
		...
		RegisterModuleDependences('rest', 'onFindMethodDescription', 'tasks',
			'\\Thurly\\Tasks\\Dispatcher', 'restRegister');

		RegisterModuleDependences('forum', 'OnCommentTopicAdd', 'tasks',
			'\\Thurly\\Tasks\\Integration\\Forum\\Task\\Topic', 'onBeforeAdd');
		RegisterModuleDependences('forum', 'OnAfterCommentTopicAdd', 'tasks',
			'\\Thurly\\Tasks\\Integration\\Forum\\Task\\Topic', 'onAfterAdd');
		...
		RegisterModuleDependences(
			...
			'OnTaskExpired',
			'tasks',
			'\Thurly\Tasks\Internals\ViolationLogTable',
			'OnTaskExpired'
		);
		...
		RegisterModuleDependences('socialnetwork', 'OnSonetLogFavorites', 'tasks', '\\Thurly\\Tasks\\Integration\\Socialnetwork\\Task', 'OnSonetLogFavorites');
		...
		$eventManager->registerEventHandler('mail', 'onReplyReceivedTASKS_TASK', 'tasks', '\Thurly\Tasks\Integration\Mail\Task', 'onReplyReceived');
		$eventManager->registerEventHandler('mail', 'onForwardReceivedTASKS_TASK', 'tasks', '\Thurly\Tasks\Integration\Mail\Task', 'onForwardReceived');

		// user field update
		RegisterModuleDependences("main", "OnAfterUserTypeAdd", "tasks", "\\Thurly\\Tasks\\Util\\UserField", "onAfterUserTypeAdd");
		RegisterModuleDependences("main", "OnAfterUserTypeUpdate", "tasks", "\\Thurly\\Tasks\\Util\\UserField", "onAfterUserTypeUpdate");
		RegisterModuleDependences("main", "OnAfterUserTypeDelete", "tasks", "\\Thurly\\Tasks\\Util\\UserField", "onAfterUserTypeDelete");

		// disk entity controllers
		RegisterModuleDependences('disk', 'onBuildAdditionalConnectorList', 'tasks', '\Thurly\Tasks\Integration\Disk', 'onBuildConnectorList');

		$eventManager->registerEventHandler('socialnetwork', 'onLogIndexGetContent', 'tasks', '\Thurly\Tasks\Integration\Socialnetwork\Log', 'onIndexGetContent');

		// kanban
		$eventManager->registerEventHandler('socialnetwork', 'onSocNetGroupDelete', 'tasks', '\Thurly\Tasks\Kanban\StagesTable', 'onSocNetGroupDelete');
		$eventManager->registerEventHandler('socialnetwork', 'onSocNetGroupDelete', 'tasks', '\Thurly\Tasks\ProjectsTable', 'onSocNetGroupDelete');
		$eventManager->registerEventHandler('main', 'OnUserDelete', 'tasks', '\Thurly\Tasks\Kanban\StagesTable', 'onUserDelete');

		$eventManager->registerEventHandler('socialnetwork', 'onContentViewed', 'tasks', '\Thurly\Tasks\Integration\Socialnetwork\ContentViewHandler', 'onContentViewed');

		CAgent::AddAgent('\Thurly\Tasks\Util\AgentManager::sendReminder();','tasks', 'N', 60);
		CAgent::AddAgent('\Thurly\Tasks\Util\AgentManager::notificationThrottleRelease();','tasks', 'N', 300);
	...
	function UnInstallDB($arParams = array())
	{
		...
		if(!array_key_exists("savedata", $arParams) || $arParams["savedata"] != "Y")
		{
			...
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/db/".strtolower($DB->type)."/uninstall.sql");
		}
		...
		UnRegisterModuleDependences('socialnetwork', 'OnSonetLogFavorites', 'tasks', '\\Thurly\\Tasks\\Integration\\Socialnetwork\\Task', 'OnSonetLogFavorites');
		...
		// "mail" module integration
		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->unRegisterEventHandler('mail', 'onReplyReceivedTASKS_TASK', 'tasks', '\Thurly\Tasks\Integration\Mail\Task', 'onReplyReceived');
		$eventManager->unRegisterEventHandler('mail', 'onForwardReceivedTASKS_TASK', 'tasks', '\Thurly\Tasks\Integration\Mail\Task', 'onForwardReceived');

		// user field update
		UnRegisterModuleDependences("main", "OnAfterUserTypeAdd", "tasks", "\\Thurly\\Tasks\\Util\\UserField", "onAfterUserTypeAdd");
		UnRegisterModuleDependences("main", "OnAfterUserTypeUpdate", "tasks", "\\Thurly\\Tasks\\Util\\UserField", "onAfterUserTypeUpdate");
		UnRegisterModuleDependences("main", "OnAfterUserTypeDelete", "tasks", "\\Thurly\\Tasks\\Util\\UserField", "onAfterUserTypeDelete");

		// disk entity controllers
		UnRegisterModuleDependences('disk', 'onBuildAdditionalConnectorList', 'tasks', '\Thurly\Tasks\Integration\Disk', 'onBuildConnectorList');

		$eventManager->unRegisterEventHandler('socialnetwork', 'onLogIndexGetContent', 'tasks', '\Thurly\Tasks\Integration\Socialnetwork\Log', 'onIndexGetContent');

		// kanban
		$eventManager->unRegisterEventHandler('socialnetwork', 'onSocNetGroupDelete', 'tasks', '\Thurly\Tasks\Kanban\StagesTable', 'onSocNetGroupDelete');
		$eventManager->unRegisterEventHandler('socialnetwork', 'onSocNetGroupDelete', 'tasks', '\Thurly\Tasks\ProjectsTable', 'onSocNetGroupDelete');
		$eventManager->unRegisterEventHandler('main', 'OnUserDelete', 'tasks', '\Thurly\Tasks\Kanban\StagesTable', 'onUserDelete');

		$eventManager->unRegisterEventHandler('socialnetwork', 'onContentViewed', 'tasks', '\Thurly\Tasks\Integration\Socialnetwork\ContentViewHandler', 'onContentViewed');
	...
	function InstallEvents()
	{
		...
		if($ar["C"] <= 0)
		{
			include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/events/set_events.php");
		}
		return true;
	}
	...
	{
		global $DB;
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/events/del_events.php");
		return true;
	}
	...
	function InstallFiles($arParams = array())
	{
		...
		{
			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/admin",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/admin",
				false
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/components",
			...
			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/activities",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/activities",
				true,
				true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/public/js",
			...
			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/public/tools",
			...
			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/public/services",
	...
	{
		DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/admin/", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");

		DeleteDirFilesEx("/thurly/js/tasks/");//scripts
		return true;
	}
	...
	function DoInstall()
	{
		...
		if (!CBXFeatures::IsFeatureEditable('Tasks'))
		{
			...
			$APPLICATION->IncludeAdminFile(
				...
				$_SERVER['DOCUMENT_ROOT'] . '/thurly/modules/' . $this->MODULE_ID . '/install/step1.php');
		}
		elseif (!IsModuleInstalled("tasks"))
		{
			...
			$GLOBALS["errors"] = $this->errors;
			$APPLICATION->IncludeAdminFile(GetMessage("TASKS_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/step1.php");
		}
	}
	...
	function DoUninstall()
	{
		...
		if($step < 2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage("TASKS_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/unstep1.php");
		}
		elseif($step == 2)
		{
			...
			$GLOBALS["errors"] = $this->errors;
			$APPLICATION->IncludeAdminFile(GetMessage("TASKS_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/unstep2.php");
		}
	}
}
