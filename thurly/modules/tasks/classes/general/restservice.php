...
use Thurly\Tasks\Integration\Rest\UserField;
		...
		foreach (self::$allowedSpecialClasses as $className => $subclass)
		{
			...
			if(is_array($methods))
			{
				foreach($methods as $methodName => $methodDesc)
				{
					...
					$publicMethodName = implode('.', array('task', $subclass['publicParentClassName'], $subclass['publicClassName'], $methodName));
					// goes to "\Thurly\Tasks\Rest\Item\UserField::add()"
					$transitMethodName = implode(self::delimiter, array($className, $methodName));
	...
	public static function __callStatic($transitMethodName, $args)
	{
		...
		try
		{
			...
			list($returnValue, $dbResult) = $className::runRestMethod(
				(int) \Thurly\Tasks\Util\User::getId(),
				$methodName,
				$methodArgs,
	...
	private static function _init()
	{
		...
		foreach (self::$arAllowedClasses as $className)
		{
			...
			$specialClasses = array(
				'\\Thurly\\Tasks\\Integration\\Rest\\'.ucfirst($namespace).'\\UserField', // user field support
			);
	...
	private static function _parseReturnValue($className, $methodName, $returnValue, $params)
	{
		...
		)
		{
			...
			foreach ($returnValue as &$fileId)
				$parsedValue[] = '/thurly/components/thurly/tasks.task.detail/show_file.php?fid=' . (int) $fileId;
			unset($fileId);
		}
