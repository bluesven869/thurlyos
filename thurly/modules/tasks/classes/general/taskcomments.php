...
class CTaskComments
{
	...
	{
		\Thurly\Tasks\Integration\Forum\Task\Topic::onBeforeAdd($entityType, $entityId, $arPost, $arTopic);
	}
	...
	{
		return \Thurly\Tasks\Integration\Forum\Task\Topic::onAfterAdd($entityType, $entityId, $topicId);
	}
	...
	{
		return \Thurly\Tasks\Integration\Forum\Task\Comment::onAfterAdd($entityType, $entityId, $arData);
	}
	...
	{
		return \Thurly\Tasks\Integration\Forum\Task\Comment::onAfterUpdate($entityType, $entityId, $arData);
	}
	...
	public static function sendAddMessage($messageData, $taskData, $fromUser, $toUsers, array $eventData = array())
	{
		...
		$messageCropped = \Thurly\Tasks\Util::trim(self::cropMessage(CTextParser::clearAllTags($message)));
		...
		CTaskNotifications::SendMessageEx($taskData["ID"], $fromUser, $toUsers, array(
			...
			'TASK_DATA' => $taskData,
			'TASK_URL' => array(
				'PARAMETERS' => \Thurly\Tasks\Integration\Forum\Comment::getUrlParameters($messageData['ID']),
				'HASH' => \Thurly\Tasks\Integration\Forum\Comment::makeUrlHash($messageData['ID'])
			)
		));
	}
