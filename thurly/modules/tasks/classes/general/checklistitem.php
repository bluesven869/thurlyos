...
bool(true)
bool(false) 
Got exception: 8; /var/www/sites/RAM/cpb24.bxram.bsr/html/thurly/modules/tasks/classes/general/checklistitem.php:282
*/
...
final class CTaskCheckListItem extends CTaskSubItemAbstract
{
	...
	public static function add(CTaskItemInterface $oTaskItem, $arFields)
	{
		...
		$curDatetime     = \Thurly\Tasks\UI::formatDateTime(\Thurly\Tasks\Util\User::getTime());
		...
		$addResult = \Thurly\Tasks\Internals\Task\CheckListTable::add($arFieldsToDb);
	...
	final protected function fetchListFromDb($taskData, $arOrder = array('SORT_INDEX' => 'asc', 'ID' => 'asc'))
	{
		...
		if ( ! $rc )
			throw new \Thurly\Main\SystemException();
	...
	public function update($arFields)
	{
		...
		if (isset($arFields['IS_COMPLETE']))
		{
			...
			$arFields['TOGGLED_DATE'] = new \Thurly\Main\Type\DateTime($curDatetime);
		}

		$result = \Thurly\Tasks\Internals\Task\CheckListTable::update($this->itemId, $arFields);
	...
	public function delete()
	{
		...
		$rc = \Thurly\Tasks\Internals\Task\CheckListTable::delete($this->itemId);
	...
	public function setSortIndex($sortIndex)
	{
		...
		$rc = \Thurly\Tasks\Internals\Task\CheckListTable::update($this->itemId, array(
	...
	private function moveItem($selectedItemId, $insertAfterItemId)
	{
		...
		foreach(\Thurly\Main\EventManager::getInstance()->findEventHandlers("tasks", "OnCheckListItemMoveItem") as $event)
		{
	...
	public static function deleteByTaskId($taskId)
	{
		...
		$list = \Thurly\Tasks\Internals\Task\CheckListTable::getList(array(
			"select" => array("ID"),
			"filter" => array(
				"=TASK_ID" => $taskId,
			),
		...
		{
			$result = \Thurly\Tasks\Internals\Task\CheckListTable::delete($item["ID"]);
			if (!$result->isSuccess())
			{
				throw new TasksException(
					'',
