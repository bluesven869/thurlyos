...
use \Thurly\Tasks\Task\DependenceTable;
use \Thurly\Tasks\Integration\Rest\Task\UserField;
use \Thurly\Tasks\Integration\Disk\Rest\Attachment;
use \Thurly\Tasks\Util\Type\DateTime;
use \Thurly\Tasks\Util\Calendar;
use \Thurly\Tasks\ActionFailedException;
use \Thurly\Tasks\ActionNotAllowedException;
use \Thurly\Tasks\ActionRestrictedException;
...
final class CTaskItem implements CTaskItemInterface, ArrayAccess
{
	...
	private static function pinInStage($taskId, $taskData = array())
	{
		if (empty($taskData))
		{
			\Thurly\Tasks\Kanban\StagesTable::pinInStage($taskId);
		}
		else
		{
			$newUsers = array();
			...
			if (!empty($newUsers))
			{
				\Thurly\Tasks\Kanban\StagesTable::pinInStage($taskId, $newUsers);
			}
		}
	}
	...
	public static function add($arNewTaskData, $executiveUserId, array $parameters = array())
	{
		...
		)
		{
			...
			if (isset($arNewTaskData['GROUP_ID']) && ($arNewTaskData['GROUP_ID'] > 0) && \Thurly\Tasks\Integration\Socialnetwork::includeModule())
			{
	...
	))
	{
		...
		if(is_array($data))
		{
			...
			if(intval($taskDupId))
			{
				...
				if($parameters['CLONE_FILES'] && !empty($files) && \Thurly\Main\Loader::includeModule('disk'))
				{
	...
	))
	{
		...
		if(!isset($arTemplate['CHECK_LIST']))
		{
			...
			$res = \Thurly\Tasks\Internals\Task\Template\CheckListTable::getList(array(
		...
		$arFields['CREATED_DATE'] = \Thurly\Tasks\UI::formatDateTime(\Thurly\Tasks\Util\User::getTime());
		...
		if ($arTemplate['DEADLINE_AFTER'])
		{
			...
			$arFields['DEADLINE'] = \Thurly\Tasks\UI::formatDateTime($deadline);
		}
	...
	protected function getChildTemplateData($templateId)
	{
		...
		// todo: use Item here!!!
		$userId = \Thurly\Tasks\Util\User::getAdminId(); // todo: deprecated
		$ufc = new \Thurly\Tasks\Util\UserField\Task(); // todo: deprecated
		$scheme = array();
		if($ufc)
		{
		...
		// get check lists
		$res = \Thurly\Tasks\Internals\Task\Template\CheckListTable::getListByTemplateDependency($templateId, array(
	 ...
	 *
	 * @throws \Thurly\Main\SystemException, TasksException
	 *
	...
	public static function fetchListArray($userId, $arOrder, $arFilter, $arParams = array(), $arSelect = array(), array $arGroup = array())
	{
		...
		try
		{
			...
			if(is_array($arTasksIDs) && !empty($arTasksIDs))
			{
				...
				{
					$res = \Thurly\Tasks\Internals\Task\ParameterTable::getList(array('filter' => array(
						'TASK_ID' => $arTasksIDs
					)));
	...
	private function proceedAction($actionId, $arActionArguments = null)
	{
		...
		if ($actionId == self::ACTION_ADD_FAVORITE || $actionId == self::ACTION_DELETE_FAVORITE || $actionId == self::ACTION_TOGGLE_FAVORITE)
		{
			...
			// in case of a ticket came, get all child tasks IDs here, pass ID list to \Thurly\Tasks\Integration\Socialnetwork\Task::toggleFavorites()
			...
			{
				$result = ($res instanceof \Thurly\Main\Entity\AddResult);
				$add = $result;
			}
			else
			{
			...
			if($tellSocnet)
			{
				\Thurly\Tasks\Integration\Socialnetwork\Task::toggleFavorites(array(
					'TASK_ID' => $this->getId(),
					'USER_ID' => $this->executiveUserId,
	...
	public function addProjectDependence($parentId, $linkType = DependenceTable::LINK_TYPE_FINISH_START)
	{
		...
		if(!\Thurly\Tasks\Util\Restriction::checkCanCreateDependence())
		{
		...
		if($this->isActionAllowed(self::ACTION_CHANGE_DEADLINE))
		{
			...
			if($parentTask->isActionAllowed(self::ACTION_CHANGE_DEADLINE))
			{
				...
				{
					$scheduler = \Thurly\Tasks\Processor\Task\Scheduler::getInstance($this->executiveUserId);
	 ...
	 *
	 * @return \Thurly\Tasks\Util\Type\DateTime
	 */
	...
	{
		throw new \Thurly\Main\NotAllowedException('Manual managing of task data is not allowed');
	}
