...
use \Thurly\Tasks\ElapsedTimeTable;
...
final class CTaskElapsedItem extends CTaskSubItemAbstract
{
	...
	public static function getList(array $order = array(), array $filter = array(), array $select = array(), array $params = array())
	{
		...
		$res = \Thurly\Tasks\Integration\Rest\ElapsedTimeTable::getList($parameters, array(
		...
		/** @noinspection PhpUnusedParameterInspection */ $navigation)
	{
		...
		if (isset($arMethodMetaInfo['staticMethod']) && $arMethodMetaInfo['staticMethod'])
		{
			...
			{
				if (count($argsParsed) > 0 && $argsParsed[0] !== 0)
				{
					$taskId = $argsParsed[0];
					$order = $argsParsed[1];
					$filter = $argsParsed[2];
				...
				else
				{
					...
					$elapsedItems = self::getList($order, $filter);
	...
	public static function getManifest()
	{
		...
		$arWritableKeys = 		array('SECONDS', 'COMMENT_TEXT', 'SOURCE', 'CREATED_DATE', 'DATE_START', 'DATE_STOP', 'USER_ID');
