...
class CTaskTags
{
	...
	function Add($arFields, $effectiveUserId = null)
	{
		...
		{
			$result = \Thurly\Tasks\TagTable::add(array(
	...
	{
		$tasks = array();
		$list = \Thurly\Tasks\TagTable::getList(array(
		...
		{
			\Thurly\Tasks\TagTable::update($primary, array("NAME" => $NEW_NAME));
		}
	...
	function Delete($arFilter)
	{
		...
		if ($arFilter)
		{
			...
			while ($item = $list->fetch())
			{
				$result = \Thurly\Tasks\TagTable::delete($item);
			}
			return $result;
		}
	}
