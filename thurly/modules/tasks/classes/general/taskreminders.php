...
class CTaskReminders
{
		...
		/** @noinspection PhpUnusedParameterInspection */ $ID = false)
	{
		...
		if (!is_set($arFields, "REMIND_DATE") || !($arFields["REMIND_DATE"] = \Thurly\Tasks\UI::checkDateTime($arFields["REMIND_DATE"])))
		{
	...
	function Add($arFields)
	{
		...
		{
			$addResult = \Thurly\Tasks\Internals\Task\ReminderTable::add(array(
				"USER_ID" => $arFields["USER_ID"],
				"TASK_ID" => $arFields["TASK_ID"],
				"REMIND_DATE" => Thurly\Main\Type\DateTime::createFromUserTime($arFields['REMIND_DATE']),
				"TYPE" => $arFields["TYPE"],
	...
	{
		return self::Delete(array("=REMIND_DATE" => new \Thurly\Main\Type\DateTime($REMIND_DATE)));
	}
	...
	{
		$result = false;
		$list = \Thurly\Tasks\Internals\Task\ReminderTable::getList(array(
			"select" => array("ID"),
			"filter" => $arFilter,
		));
		while ($item = $list->fetch())
		{
			$result = \Thurly\Tasks\Internals\Task\ReminderTable::delete($item);
		}
		return $result;
	}
	...
	function SendAgent()
	{
		...
		$arFilter = array(
			...
			"<=REMIND_DATE" => ((string) new \Thurly\Main\Type\DateTime())
		);
