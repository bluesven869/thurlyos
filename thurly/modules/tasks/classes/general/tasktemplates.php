...
use \Thurly\Tasks\Template;
use Thurly\Tasks\Internals\Task\Template\DependenceTable;
...
class CTaskTemplates
{
	...
	function CheckFields(&$arFields, $ID = false, $arParams = array())
	{
		...
		*/
		...
		{
			$params = \Thurly\Tasks\Util\Type::unSerializeArray($arFields['REPLICATE_PARAMS']);
	...
	function Add($arFields, $arParams = array())
	{
		...
		if ($this->CheckFields($arFields, false, $arParamsForCheckFields))
		{
			if ($USER_FIELD_MANAGER->CheckFields("TASKS_TASK_TEMPLATE", 0, $arFields, $arParamsForCheckFields['USER_ID']))
			{
				...
				// periodic tasks
				// todo: use \Thurly\Tasks\Util\Replicator\Task\FromTemplate::reInstallAgent() here
				...
				if (intval($arFields['BASE_TEMPLATE_ID']))
				{
					...
					catch(\Thurly\Tasks\DB\Tree\LinkExistsException $e)
					{
					}
				}

				// need to create rights...
				\Thurly\Tasks\Item\Access\Task\Template::grantAccessLevel($ID, 'U'.$arFields['CREATED_BY'], 'full', array(
					'CHECK_RIGHTS' => false,
				));
	...
	function Update($ID, $arFields, $arParams = array())
	{
        ...
        else
        {
	        ...
	        if(!$userID)
	        {
		        $userID = \Thurly\Tasks\Util\User::getAdminId(); // compatibility
	        }
        }
		...
		if ($ID > 0)
		{
			...
			if ($arTemplate = $rsTemplate->Fetch())
			{
				...
				if ($DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__))
				{
					...
					{
						// drop checklists
						$clRes = \Thurly\Tasks\Internals\Task\Template\CheckListTable::getListByTemplateDependency($ID, array('select' => array('ID')));
						while($clItem = $clRes->fetch())
						{
							\Thurly\Tasks\Internals\Task\Template\CheckListTable::delete($clItem['ID']);
						}
						...
						catch(\Thurly\Tasks\DB\Tree\TargetNodeNotFoundException $e) // had no children actually
						{
						}

						// drop rights
						\Thurly\Tasks\Item\Access\Task\Template::revokeAll($ID, array('CHECK_RIGHTS' => false));
					}

					// drop syslog records
					\Thurly\Tasks\Item\SystemLog::deleteByEntity($ID, 1);
	...
	public static function GetList($arOrder, $arFilter, $arNavParams = array(), $arParams = array(), $arSelect = array())
	{
		...
		$temporalTableName = \Thurly\Tasks\DB\Helper::getTemporaryTableNameSql();
		...
		foreach ($arSelect as $field)
		{
			...
			{
				$arSqlSelect[$field] = \Thurly\Tasks\DB\Helper::wrapColumnWithFunction($arFields[$field]['FIELD'], $arFields[$field]['WRAP'])." AS ".$field;
			}
		}
	...
	private static function getAccessSql($arParams)
	{
		...
		if (isset($arParams['USER_ID']))
		{
			...
			if(!$isAdmin)
			{
				$mixin = \Thurly\Tasks\Internals\RunTime\Task\Template::getAccessCheckSql(array(
	...
	public static function GetCount($includeSubtemplates = false)
	{
		...
		if (intval(\Thurly\Tasks\Util\User::getId()))
		{
			$strSql = "
				SELECT COUNT(*) AS CNT 
				...
				WHERE 
					CREATED_BY = ".intval(\Thurly\Tasks\Util\User::getId())."
	...
	{
		// todo: use \Thurly\Tasks\Item\Task\Template\Field\ReplicateParams::createValueStructure() here
		...
		if(trim($params['TIME']) != '')
		{
			$time = \Thurly\Tasks\UI::parseTimeAmount($params['TIME'], 'HH:MI');
		}
		$params['TIME'] = \Thurly\Tasks\UI::formatTimeAmount($time, 'HH:MI');
