...
//		$allowedUserTypes = array('disk_file', 'crm');
		$allowedUserTypes = array('disk_file', 'crm', 'string', 'date', 'datetime', 'enumeration', 'double', 'integer', 'boolean', 'file',
		...
		foreach(self::$arUFId as $ufId)
		{
			...
			if (is_array($arUserFields) && count($arUserFields) > 0)
			{
				...
				{
					if (isset($field['FIELD_NAME']) && \Thurly\Tasks\Util\UserField::isUFKey($field['FIELD_NAME'])
	...
	public static function beforeViewDataQuery(&$select, &$filter, &$group, &$order, &$limit, &$options, &$runtime = null)
	{
		...
		if (\Thurly\Main\Application::getConnection() instanceof \Thurly\Main\DB\MssqlConnection)
		{
			foreach ($select as $k => $v)
			{
				if (substr($k, -9) == '_DURATION')
				{
					...
					$subQuery = new \Thurly\Main\Entity\Query(\Thurly\Tasks\Internals\Task\ElapsedTimeTable::getEntity());
					$subQuery->addSelect('TASK_ID');
					$subQuery->addSelect(new \Thurly\Main\Entity\ExpressionField(
						'DURATION', 'ROUND(SUM(%s)/60, 0)', 'SECONDS'
					));

					$subEntity = \Thurly\Main\Entity\Base::getInstanceByQuery($subQuery);
	...
	public static function appendDateTimeUserFieldsAsShort(\Thurly\Main\Entity\Base $entity)
	{
		...
		{
			if ($field instanceof Thurly\Main\Entity\ExpressionField)
			{
				$arUF = self::detectUserField($field);
				if ($arUF['isUF'])
				{
	...
	public static function getCurrentVersion()
	{
		...
		include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/tasks/install/version.php");
