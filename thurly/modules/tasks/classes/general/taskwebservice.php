...
class CTasksWebService extends IWebService
{
	...
	function GetList($listName)
	{
		...
		$data->setAttribute('Author', $this->__makeUser(\Thurly\Tasks\Util\User::getId()));
	...
	function GetListItemChangesSinceToken($listName, $viewFields = '', $query = '', $rowLimit = 0, $changeToken = '')
	{
		...
		if ($changeToken)
		{
			$thurlyTimestamp = $changeToken + CTasksTools::getTimeZoneOffset();
			$arFilter['>CHANGED_DATE'] = ConvertTimeStamp($thurlyTimestamp, 'FULL');
		...
		$arFilter['MEMBER'] = \Thurly\Tasks\Util\User::getId();
			...
			if ($bUpdate)
			{
				...
				elseif ($arData['_command'] == 'New' || $arData['_command'] == 'Update')
				{
					...
					$arFields = array(
						...
						'CHANGED_BY' => $userId,
						'CHANGED_DATE' => \Thurly\Tasks\UI::formatDateTime(time()),
						'SITE_ID' => SITE_ID,
						'TITLE' => $arData['Title'],
					...
					if ($bUpdate)
					{
						...
						if ($arData['_command'] == 'New')
						{
							...
							$arFields["CREATED_DATE"] = \Thurly\Tasks\UI::formatDateTime(time());
	...
	function GetWebServiceDesc()
	{
		...
		$wsdesc->wsname = "thurly.webservice.tasks";
