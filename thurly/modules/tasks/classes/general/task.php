...
use Thurly\Main\DB\MssqlConnection;
use Thurly\Main\DB\MysqlCommonConnection;
use Thurly\Main\DB\OracleConnection;

use \Thurly\Tasks\Util\Calendar;
use \Thurly\Tasks\Internals\Counter;
use \Thurly\Tasks\Internals\Task\FavoriteTable;
use \Thurly\Tasks\Internals\Task\ProjectDependenceTable;
use \Thurly\Tasks\Internals\Task\SortingTable;
use \Thurly\Tasks\Kanban\TaskStageTable;
use \Thurly\Tasks\Util\Type;

use Thurly\Tasks\Util\Replicator;
...
class CTasks
{
	...
	function CheckFields(&$arFields, $ID = false, $effectiveUserId = null)
	{
		...
		{
			$result = \Thurly\Tasks\Internals\Helper\Task\Dependence::canAttach($ID, $arFields["PARENT_ID"]);
	...
	public function Add($arFields, $arParams = array())
	{
		...
		if ($this->CheckFields($arFields, false, $effectiveUserId))
		{
			...
			if($arParams['CLONE_DISK_FILE_ATTACHMENT'] === true || $arParams['CLONE_DISK_FILE_ATTACHMENT'] === 'Y')
			{
				...
				{
					$arFields['UF_TASK_WEBDAV_FILES'] = \Thurly\Tasks\Integration\Disk::cloneFileAttachment($arFields['UF_TASK_WEBDAV_FILES'], $effectiveUserId);
				}
			}

			if ($USER_FIELD_MANAGER->CheckFields("TASKS_TASK", 0, $arFields, $effectiveUserId))
			{
				$nowDateTimeString = \Thurly\Tasks\UI::formatDateTime(User::getTime());
				...
				{
					$scheduler = \Thurly\Tasks\Processor\Task\Scheduler::getInstance($effectiveUserId);
				...
				if ($ID)
				{
					...
					if ($arTask = $rsTask->Fetch())
					{
						...
						if ($parentId)
						{
							\Thurly\Tasks\Internals\Helper\Task\Dependence::attachNew($ID, $parentId);
						}
...
//						CTaskCountersProcessor::onAfterTaskAdd($arFields);
						\Thurly\Tasks\Internals\Counter::onAfterTaskAdd($arFields);
						...
						if ($shiftResult !== null)
						{
							if ($parentId)
							{
								...
								{
									$scheduler = \Thurly\Tasks\Processor\Task\Scheduler::getInstance($effectiveUserId);
									$shiftResult = $scheduler->processEntity(0, $arFields, array('MODE' => 'BEFORE_ATTACH'));
								}
							}
						...
						$DB->Query(
							'UPDATE b_tasks SET SEARCH_INDEX = \''.
							$DB->ForSql(\Thurly\Tasks\Manager\Task::prepareSearchIndex($arTask)).
							'\' WHERE ID = '.
							$ID
						);
					}
	...
	public function Update($ID, $arFields, $arParams = array('CORRECT_DATE_PLAN_DEPENDENT_TASKS' => true, 'CORRECT_DATE_PLAN' => true, 'THROTTLE_MESSAGES' => false))
	{
		...
		if ($arTask = $rsTask->Fetch())
		{
			...
			{
				$ufCheck = true;
				$hasUfs = \Thurly\Tasks\Util\UserField::checkContainsUFKeys($arFields);
				if($hasUfs)
				{
				...
				if ($ufCheck)
				{
					...
					$arFields["CHANGED_DATE"] = \Thurly\Tasks\UI::formatDateTime($time);
					...
					)
					{
						...
						$arFields["STATUS_CHANGED_DATE"] = \Thurly\Tasks\UI::formatDateTime($time);

						if ($arFields["STATUS"] == 5 || $arFields["STATUS"] == 4)
						{
							$arFields["CLOSED_BY"] = $userID;
							$arFields["CLOSED_DATE"] = \Thurly\Tasks\UI::formatDateTime($time);
						}
						else
						{
							...
							{
								$arFields["DATE_START"] = \Thurly\Tasks\UI::formatDateTime($time);
							}
						}
					}
					...
					if($arParams['CORRECT_DATE_PLAN'])
					{
						...
						if($parentChanged)
						{
							...
							$scheduler = \Thurly\Tasks\Processor\Task\Scheduler::getInstance($userID);
						...
						if($parentChanged || $datesChanged || $followDatesChanged)
						{
							...
							if($shiftResult->isSuccess())
							{
								...
								{
									// will be saved...
									$arFields['START_DATE_PLAN'] = ((isset($arFields['START_DATE_PLAN']) && $shiftData['START_DATE_PLAN'] == null)? false : $shiftData['START_DATE_PLAN']);
									$arFields['END_DATE_PLAN'] = ((isset($arFields['END_DATE_PLAN']) && $shiftData['END_DATE_PLAN'] == null)? false : $shiftData['END_DATE_PLAN']);
									$arFields['DURATION_PLAN_SECONDS'] = $shiftData['DURATION_PLAN_SECONDS'];
					...
					$arFields['SEARCH_INDEX'] = \Thurly\Tasks\Manager\Task::prepareSearchIndex($arTask);
					...
					if ($result)
					{
						...
						if(array_key_exists('PARENT_ID', $arFields))
						{
							...
							\Thurly\Tasks\Internals\Helper\Task\Dependence::attach($ID, intval($arFields['PARENT_ID']));
						}
						...
						if(array_key_exists('STATUS', $arFields) && $arFields['STATUS'] == 5)
						{
							...
							{
								$closer = \Thurly\Tasks\Processor\Task\AutoCloser::getInstance($userID);
...
//						CTaskCountersProcessor::onAfterTaskUpdate($arTask, $arFields);
						\Thurly\Tasks\Internals\Counter::onAfterTaskUpdate($arTask, $arFields);
						static::clearCache();
						...
						if ($updatePins)
						{
							\Thurly\Tasks\Kanban\StagesTable::pinInStage($ID, array(
	...
	public static function Delete($ID, $arParams = array())
	{
		...
		if ($arTask = $rsTask->Fetch())
		{
			...
			$list = \Thurly\Tasks\Internals\Task\ViewedTable::getList(array(
			...
			{
				\Thurly\Tasks\Internals\Task\ViewedTable::delete($item);
			}

			CTaskReminders::DeleteByTaskID($ID);

			// delete parameters
			$list = \Thurly\Tasks\Internals\Task\ParameterTable::getList(array(
				"select" => array("ID"),
				"filter" => array(
					"=TASK_ID" => $ID,
				),
			...
			while ($item = $list->fetch())
			{
				\Thurly\Tasks\Internals\Task\ParameterTable::delete($item);
			}

			// by default, CTasks::Delete() should not delete the entire sub-tree, so we need to delete only node itself
			\Thurly\Tasks\Internals\Helper\Task\Dependence::delete($ID);
			...
			if($arTask['PARENT_ID'] && $arTask['START_DATE_PLAN'] && $arTask['END_DATE_PLAN'])
			{
				...
				$scheduler = \Thurly\Tasks\Processor\Task\Scheduler::getInstance($actorUserId);
			...
			$strUpdate = $DB->PrepareUpdate(
				...
				array(
					...
					'CHANGED_DATE' => \Thurly\Tasks\UI::formatDateTime(User::getTime())
				),
				"tasks"
			);
			...
			if ($DB->Query($strSql, false, "File: ".__FILE__."<br>Line: ".__LINE__))
			{
				...
				\Thurly\Tasks\Integration\Forum\Task\Topic::delete($arTask["FORUM_TOPIC_ID"]);
...
//				CTaskCountersProcessor::onAfterTaskDelete($arTask);
				\Thurly\Tasks\Internals\Counter::onAfterTaskDelete($arTask);
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case 'META:PARENT_ID_OR_NULL':
					if((array)$val)
					{
						$arSqlSearch[] = '(T.PARENT_ID IN('.join(',', (array)$val).') OR T.PARENT_ID IS NULL)';
					}
					break;
				...
				case 'TITLE':
					...
					break;
				case 'SEARCH_INDEX':
					$arSqlSearch[] = CTasks::FilterCreate($sAliasPrefix."T.SEARCH_INDEX", $val, "fulltext", $bFullJoin, $cOperationType);
					break;
				...
				case "STATUS":
					$arSqlSearch[] = CTasks::FilterCreate("
						CASE
							WHEN
								".$sAliasPrefix."T.DEADLINE < DATE_ADD(".$DB->CurrentTimeFunction().", INTERVAL ".
														  Counter::getDeadlineTimeLimit()." SECOND)
								AND ".$sAliasPrefix."T.DEADLINE >= ".$DB->CurrentTimeFunction()."
								AND ".$sAliasPrefix."T.STATUS != '4'
								AND ".$sAliasPrefix."T.STATUS != '5'
								AND (
									".$sAliasPrefix."T.STATUS != '7'
									OR ".$sAliasPrefix."T.RESPONSIBLE_ID != ".intval($userID)."
								)
							THEN
								'-3'
							WHEN
				...
				case 'STAGES_ID':
					$arSqlSearch[] = CTasks::FilterCreate($sAliasPrefix."STG.STAGE_ID", $val, "number", $bFullJoin, $cOperationType, false);
				break;
	...
	{
		return \Thurly\Tasks\Integration\Intranet\Department::getSubordinateIds($userID, true);
	}
	 ...
	 * @return mixed
	 * @deprecated
	 * @see \Thurly\Tasks\Integration\SocialNetwork\Group::getIdsByAllowedAction
	 */
	...
	public static function GetList($arOrder=array(), $arFilter=array(), $arSelect = array(), $arParams = array(), array $arGroup = array())
	{
		...
		if(!\Thurly\Tasks\Integration\Forum::isInstalled())
		{
...
//					$needle = 'STATUS';
		...
		if (is_array($arParams) && array_key_exists("NAV_PARAMS", $arParams) && is_array($arParams["NAV_PARAMS"]))
		{
			...
			{
				$res_cnt = $DB->Query("SELECT COUNT(DISTINCT T.ID) as C " . $strFrom);
...
//				CTaskCountersProcessorHomeostasis::onTaskGetList($arFilter, $totalTasksCount);
	...
	private static function makePossibleForwardedFilter($filter)
	{
		...
		if(is_array($filter) && !empty($filter))
		{
			...
			$filter = \Thurly\Tasks\Internals\DataBase\Helper\Common::parseFilter($filter);
			...
			$result = \Thurly\Tasks\Internals\DataBase\Helper\Common::makeFilter($result);
		}
	...
	private static function appendJoinRights($sql, $arParams)
	{
		...
		$access = \Thurly\Tasks\Internals\RunTime\Task::getAccessCheckSql($arParams);
	...
	{
		$arDepsIDs = \Thurly\Tasks\Integration\Intranet\Department::getSubordinateIds($arParams['USER_ID'], true);
	...
	{
		$arTask['SE_TAG'] = $tags;
		\Thurly\Tasks\Integration\Search\Task::index($arTask);
	}
	...
	public static function __updateViewed($TASK_ID, $USER_ID, $onTaskAdd = false)
	{
		...
		if ($item = $list->fetch())
		{
			\Thurly\Tasks\Internals\Task\ViewedTable::update($item, array(
				"VIEWED_DATE" => new \Thurly\Main\Type\DateTime(),
			));
		}
		else
		{
			\Thurly\Tasks\Internals\Task\ViewedTable::add(array(
				"TASK_ID" => $TASK_ID,
				"USER_ID" => $USER_ID,
				"VIEWED_DATE" => new \Thurly\Main\Type\DateTime(),
			));
		...
		\Thurly\Tasks\Internals\Counter::onAfterTaskViewedFirstTime($TASK_ID, $USER_ID, $onTaskAdd);
	...
	public static function OnBeforeUserDelete($inUserID)
	{
		...
		$templates = array();
		$res = \Thurly\Tasks\TemplateTable::getList(array('filter' => array(
			'LOGIC' => 'OR',
	...
	public static function EmitPullWithTagPrefix($arRecipients, $tagPrefix, $cmd, $arParams)
	{
		...
		*/
		...
		foreach ($arRecipients as $userId)
		{
			...
			//\Thurly\Pull\Event::add($userId, $arEventData);
