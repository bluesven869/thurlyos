...
class CTaskElapsedTime
{
	...
	function Add($arFields, $arParams = array())
	{
		...
		elseif ($userId = \Thurly\Tasks\Util\User::getId())
		...
		if ($this->CheckFields($arFields))
		{
			...
			if ($arFields['CREATED_DATE'])
				$createdDate = Thurly\Main\Type\DateTime::createFromUserTime($arFields['CREATED_DATE']);
			else
				$createdDate = new Thurly\Main\Type\DateTime();

			$addResult = \Thurly\Tasks\Internals\Task\ElapsedTimeTable::add(array(
				"CREATED_DATE" => $createdDate,
				"DATE_START" => Thurly\Main\Type\DateTime::createFromUserTime($arFields['DATE_START']),
				"DATE_STOP" => Thurly\Main\Type\DateTime::createFromUserTime($arFields['DATE_STOP']),
	...
	function Update($ID, $arFields, $arParams = array())
	{
		...
		if ($this->CheckFields($arFields, $ID))
		{
			...
			if (array_key_exists("CREATED_DATE", $arFields))
				$update["CREATED_DATE"] = Thurly\Main\Type\DateTime::createFromUserTime($arFields['CREATED_DATE']);
			if (array_key_exists("DATE_START", $arFields))
				$update["DATE_START"] = Thurly\Main\Type\DateTime::createFromUserTime($arFields['DATE_START']);
			if (array_key_exists("DATE_STOP", $arFields))
				$update["DATE_STOP"] = Thurly\Main\Type\DateTime::createFromUserTime($arFields['DATE_STOP']);
			...
			$updateResult = \Thurly\Tasks\Internals\Task\ElapsedTimeTable::update($ID, $update);
	...
	public static function Delete($ID, $arParams = array())
	{
		...
		$deleteResult = \Thurly\Tasks\ElapsedTimeTable::delete($ID);
	...
	function CanCurrentUserAdd($task)
	{
		...
		elseif (
			\Thurly\Tasks\Util\User::isAdmin()
