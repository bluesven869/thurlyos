...
class CTaskPlannerMaintance
{
	...
	public static function OnPlannerInit($params)
	{
		...
		CJSCore::RegisterExt('tasks_planner_handler', array(
			'js'   => '/thurly/js/tasks/core_planner_handler.js',
			'css'  => '/thurly/js/tasks/css/tasks.css',
		...
		$arResult = array(
			...
			'STYLES' => array('/thurly/js/tasks/css/tasks.css'),
			'SCRIPTS' => array('CJSTask', 'taskQuickPopups', 'tasks_planner_handler', '/thurly/js/tasks/task-iframe-popup.js')
		);
	...
	protected static function getTimemanCloseDayData($arParams)
	{
		if(CModule::IncludeModule('timeman'))
		{
			...
			$thurlyTimestampDateStart = $unixTsDateStart + CTasksTools::getTimeZoneOffset();
			$dateStartAsString        = ConvertTimeStamp($thurlyTimestampDateStart, 'FULL');
	...
	public static function plannerActions($arActions, $site_id = SITE_ID)
	{
		...
		self::$USER_ID = \Thurly\Tasks\Util\User::getId(); // todo: need to remove this, use $userId instead, to be able to manage other users planner
		...
		if (strlen($arActions['name']) > 0)
		{
			...
			try
			{
				$prevOccurAsUserId = \Thurly\Tasks\Util\User::getOccurAsId(); // null or positive integer
				\Thurly\Tasks\Util\User::setOccurAsId(self::$USER_ID);
				...
				), \Thurly\Tasks\Util\User::getAdminId()); // todo: why admin?
				$ID = $task->getId();

				\Thurly\Tasks\Util\User::setOccurAsId($prevOccurAsUserId);
			}
			catch(TasksException $e) // todo: when refactor exceptions, replace with somewhat like \Thurly\Tasks\Exception
			{
			}
	...
	private static function getTasks($arIDs = array(), $bOpened = false)
	{
		...
		if (count($arIDs) > 0)
		{
			$pathTemplate = \Thurly\Tasks\Integration\Socialnetwork\UI\Task::getActionPath();
			...
			if(!empty($taskIds))
			{
				...
				foreach($tasks as $id => $data)
				{
					...
					$res[] = array(
						...
						'URL'                 => \Thurly\Tasks\UI\Task::makeActionUrl($pathTemplate, $data['ID'], 'view', $USER_ID)
					);
				}
			}
		}
	...
	public static function getCurrentTasksList()
	{
		...
		{
			$items = array();
			$res = \Thurly\Tasks\Internals\TaskTable::getList(array(
	...
	public static function OnAfterTMDayStart()
	{
		...
		if(count($list) > 0)
		{
			...
			self::setCurrentTasksList($newList);
			$CACHE_MANAGER->ClearByTag('tasks_user_'.\Thurly\Tasks\Util\User::getId());
		}
	}
