...
use Thurly\Tasks\Internals\Notification\Task\ThrottleTable;
use Thurly\Tasks\Util\AgentManager;
...
class CTaskNotifications
{
		...
		if (sizeof($arRecipientsIDs) && ($effectiveUserId !== false))
		{
			...
			)
			{
				...
				foreach ($arRecipientsIDs as $userId)
				{
					...
					{
						// Make thurly timestamp for given user
						$thurlyTsDeadline = $utsDeadline + $tzOffset;

						$deadlineAsString = \Thurly\Tasks\UI::formatDateTime($thurlyTsDeadline, '^'.\Thurly\Tasks\UI::getDateTimeFormat());
					...
					switch ($key)
					{
						...
						case "END_DATE_PLAN":
							...
							foreach ($arRecipientsIDs as $userId)
							{
								...
								{
									// Make thurly timestamps for given user
									$thurlyTsFromValue = $utsFromValue + $tzOffset;
									$thurlyTsToValue   = $utsToValue + $tzOffset;

									$description = '';

									if ($utsFromValue > 360000)		// is correct timestamp?
									{
										$fromValueAsString = \Thurly\Tasks\UI::formatDateTime($thurlyTsFromValue, '^'.\Thurly\Tasks\UI::getDateTimeFormat());
									...
									{
										$toValueAsString = \Thurly\Tasks\UI::formatDateTime($thurlyTsToValue, '^'.\Thurly\Tasks\UI::getDateTimeFormat());
	...
	public static function getSonetLogFilter($taskId, $crm)
	{
		...
		elseif (\Thurly\Main\Loader::includeModule("crm"))
		{
	...
	private static function flushNotificationBuffer($doGrouping = true)
	{
		if(is_array(self::$buffer) && !empty(self::$buffer))
		{
			...
			$sites = \Thurly\Tasks\Util\Site::getPair();
			$byUser = array();
			$mailed = array();
			foreach(self::$buffer as $i => $message)
			{
				if(is_array($message['TO_USER_IDS']))
				{
					foreach($message['TO_USER_IDS'] as $userId)
					{
						...
						if(\Thurly\Tasks\Integration\Mail\User::isEmail($users[$userId])) // must send message to email users separately
						{
							if(!isset($mailed[$i]))
							{
								$mMessage = $message;
			...
			foreach($byUser as $userId => $messages)
			{
						...
						foreach($deepGrouping as $type => $byAuthor)
						{
							foreach($byAuthor as $authorId => $byEvent)
							{
								foreach($byEvent as $event => $taskIds)
								{
									...
									\Thurly\Tasks\Integration\Im::notifyAdd($arMessageFields);
								}
							}
						}
					}
				...
				foreach($unGroupped as $taskId => $message)
				{
					...
					if((string) $message['MESSAGE']['PUSH'] != '')
					{
						...
						// see /mobile/install/components/thurly/mobile.rtc/templates/.default/script.js for handling details
	...
	public static function OnAnswerNotify($module, $tag, $text, $arNotify)
	{
		if ($module == "tasks" && (string) $text != '')
		{
			...
			if($tagData['ENTITY'] == 'COMMENT')
			{
				...
				{
					throw new \Thurly\Main\SystemException(); // this will break json and make notify window glow red :)
				}
				else
				{
	...
	{
		return \Thurly\Tasks\Integration\SocialNetwork\UI\Task::formatFeedEntry($arFields, $arParams);
	}
	...
	public static function getNotificationPath($arUser, $taskID, $bUseServerName = true, $arSites = array())
	{
		...
		if (is_array($arTask) && !empty($arTask))
		{
			...
			{
				$arSites = \Thurly\Tasks\Util\Site::getPair();
			}

			// we have extranet and the current user is an extranet user
			$bExtranet = 	\Thurly\Tasks\Integration\Extranet\User::isExtranet($arUser["ID"]);
			// task is in a group
			...
			if(!isset($siteCache[$siteID]))
			{
				if((string) $siteID != '')
				{
					$siteCache[$siteID] = \Thurly\Main\SiteTable::getList(array(
	...
	private static function getSocNetGroupSiteList($id)
	{
		...
		$thurlyosInstalled = \Thurly\Main\ModuleManager::isModuleInstalled('thurlyos');
		$extranetInstalled = \Thurly\Main\ModuleManager::isModuleInstalled('extranet');
				...
				if (
					...
					|| (
						...
						&& (
							...
							|| $item['LID'] == \Thurly\Main\Config\Option::get("extranet", "extranet_site") // extranet uninstalled with 'Save data' option
						)
					)
				)
				{
	...
	private static function getTaskBaseByTaskId($taskId)
	{
		...
		{
			$item = CTasks::getList(array(), array('ID' => $taskId), array('ID', 'GROUP_ID'), array('USER_ID' => \Thurly\Tasks\Util\User::getAdminId()))->fetch();
	...
	private static function notifyByMail(array $message, array $site)
	{
		...
		if (!\Thurly\Tasks\Integration\Mail::isInstalled())
		{
			return false;
		}
		...
		{
			$site = \Thurly\Tasks\Util\Site::get(SITE_ID);
		}
		...
		if($entityCode == 'TASK')
		{
			if($entityOperation == 'ADD' || $entityOperation == 'UPDATE')
			{
				...
				$threadMessageId = \Thurly\Tasks\Integration\Mail::formatThreadId('TASK_'.$taskId, $siteId);
			}

			if($entityOperation == 'UPDATE')
			{
				$prevFields = $message["EVENT_DATA"]['arChanges'];
				$subjPrefix = \Thurly\Tasks\Integration\Mail::getSubjectPrefix();
			}
		...
		elseif($entityCode == 'COMMENT')
		{
			if($entityOperation == 'ADD')
			{
				...
				$threadMessageId = \Thurly\Tasks\Integration\Mail::formatThreadId('TASK_COMMENT_'.$commentId, $siteId);
				$subjPrefix = \Thurly\Tasks\Integration\Mail::getSubjectPrefix();
			}
		...
		// email letter data
		$pathToTask = \Thurly\Tasks\Integration\Mail\Task::getDefaultPublicPath($taskId);

		$users = static::getUsers(array_merge(array($authorId), $message["TO_USER_IDS"]));
		foreach($users as $i => $user)
		{
			$users[$i]['NAME_FORMATTED'] = \Thurly\Tasks\Util\User::formatName($users[$i], $siteId);
		}

		$receiversData = \Thurly\Tasks\Integration\Mail\User::getData($message["TO_USER_IDS"], $siteId);
		...
		foreach ($receiversData as $userId => $arUser)
		{
			...
			$replyTo = \Thurly\Tasks\Integration\Mail\Task::getReplyTo(
				$userId,
				$taskId,
				$pathToTask,
				$siteId
			...
			if ($replyTo != '')
			{
				...
				$e = array(
					...
					"=In-Reply-To" => \Thurly\Tasks\Integration\Mail::formatThreadId('TASK_'.$taskId, $siteId),
					"EMAIL_FROM" => $authorName.' <'.\Thurly\Tasks\Integration\Mail::getDefaultEmailFrom($siteId).'>',
					"EMAIL_TO" => (!empty($nameFormatted) ? ''.$nameFormatted.' <'.$email.'>' : $email),

					"TASK_ID" => $taskId,
					"TASK_COMMENT_ID" => $commentId,
					"TASK_TITLE" => $taskTitle,
					"TASK_PREVIOUS_FIELDS" => \Thurly\Tasks\Util\Type::serializeArray($prevFields),

					"RECIPIENT_ID" => $userId,
					"USER_ID" => \Thurly\Tasks\Util\User::getAdminId(),
