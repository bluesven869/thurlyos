...
class CTaskLog
{
	...
	public static function getTrackedFields()
	{
		...
		if (!$fields) {
			...
			foreach ($ufs as $code => $desc) {
				...
				$title = '';
				if ($code != \Thurly\Tasks\Integration\Disk\UserField::getMainSysUFCode()) {
	...
	)
	{
		if ((string)$arFields['CREATED_DATE'] == '') {
			$arFields['CREATED_DATE'] = \Thurly\Tasks\Util\Type\DateTime::getCurrentTimeString();
		}
	...
	public function Add($arFields)
	{
		if ($this->CheckFields($arFields))
		{
			...
			$addResult = \Thurly\Tasks\Internals\Task\LogTable::add(array(
	...
	public static function DeleteByTaskId($in_taskId)
	{
		...
		$list = \Thurly\Tasks\Internals\Task\LogTable::getList(array(
			"select" => array("ID"),
			"filter" => array(
				"=TASK_ID" => $taskId,
			),
		...
		while ($item = $list->fetch()) {
			$result = \Thurly\Tasks\Internals\Task\LogTable::delete($item);
		}
	}
}