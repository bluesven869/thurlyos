...
$arParams['PLACEMENT'] = isset($arParams['PLACEMENT']) ? ToUpper($arParams['PLACEMENT']) : \Thurly\Rest\PlacementTable::PLACEMENT_DEFAULT;
$arParams['PLACEMENT_OPTIONS'] = isset($arParams['PLACEMENT_OPTIONS']) ? $arParams['PLACEMENT_OPTIONS'] : '';

if($arParams['PLACEMENT'] === \Thurly\Rest\PlacementTable::PLACEMENT_DEFAULT && empty($arParams['PLACEMENT_OPTIONS']))
{
	$requestOptions = $_GET;
	if($arParams['POPUP'])
	{
	...
	$deniedParam = \Thurly\Main\HttpRequest::getSystemParameters();
	$deniedParam[] = '_r';
...
$arApp = \Thurly\Rest\AppTable::getByClientId($arParams['CODE']);

if(
	is_array($arApp) && $arApp['ACTIVE'] == \Thurly\Rest\AppTable::ACTIVE
...
)
{
	...
	if($arParams['PLACEMENT'] !== \Thurly\Rest\PlacementTable::PLACEMENT_DEFAULT)
	{
		$filter = array();
		...
		$dbRes = \Thurly\Rest\PlacementTable::getList(array(
			'filter' => $filter,
	...
	if (
		(
			$bHasAccess
			|| $arParams['PLACEMENT'] === \Thurly\Rest\Api\UserFieldType::PLACEMENT_UF_TYPE
		)
		&& strlen($arResult['APP_NAME']) > 0)
	{
		...
		$arResult['APP_INSTALLED'] = $arApp['INSTALLED'] == \Thurly\Rest\AppTable::INSTALLED;
	...
	// this is a first run for the application and the current user
		...
		if($bHasAccess)
		{
			$arResult['AUTH'] = \Thurly\Rest\OAuth\Auth::get(
				$arApp['CLIENT_ID'],
				$arApp['SCOPE'],
				array(),
				$USER->GetID()
		...
		elseif($arParams['PLACEMENT'] === \Thurly\Rest\Api\UserFieldType::PLACEMENT_UF_TYPE)
		{
			$arResult['AUTH'] = array();
		}
			...
			if($arResult['AUTH']['error'])
			{
				...
				if($arResult['AUTH']['error'])
				{
					...
					else
					{
						...
						$arApp = \Thurly\Rest\AppTable::getByClientId($arParams['ID']);
					}
				}
			}
		}
		...
		)
		{
			...
			$arApp['DATE_FINISH'] = $arResult['AUTH']['date_finish'] ? \Thurly\Main\Type\Date::createFromTimestamp($arResult['AUTH']['date_finish']) : '';

			\Thurly\Rest\AppTable::setSkipRemoteUpdate(true);
			$result = \Thurly\Rest\AppTable::update($arApp['ID'], array(
				'STATUS' => $arApp['STATUS'],
				'DATE_FINISH' => $arApp['DATE_FINISH'],
			));
			\Thurly\Rest\AppTable::setSkipRemoteUpdate(false);
			if(
				$result->isSuccess()
				&& $arApp['STATUS'] === \Thurly\Rest\AppTable::STATUS_PAID
			)
			{
				\Thurly\Rest\AppTable::callAppPaymentEvent($arApp['ID']);
			}
		}

		$arResult['DETAIL_URL'] = str_replace("#code#", $arApp['CODE'], $arParams['DETAIL_URL']);

		$arResult['APP_STATUS'] = \Thurly\Rest\AppTable::getAppStatusInfo($arApp, $arResult['DETAIL_URL']);

		$arResult['APP_NEED_REINSTALL'] = $arApp['STATUS'] == \Thurly\Rest\AppTable::STATUS_PAID && !isset($arApp['SHARED_KEY']);

		$arResult['APP_SID'] = md5(uniqid(rand(), true));

		$arResult['IS_ADMIN'] = \CRestUtil::isAdmin();
		$arResult['REST_PATH'] = \Thurly\Main\Config\Option::get("rest", "server_path", "/rest");
		...
		// additional actions
		if($arResult['APP_STATUS']['PAYMENT_ALLOW'] == 'Y' && isset($_REQUEST['action']) && in_array($_REQUEST['action'], array('access_refresh', 'set_option', 'set_installed')) && check_thurly_sessid())
		{
				...
				case 'set_option':
					if($arResult['IS_ADMIN'])
					{
						if(is_array($_REQUEST['options']))
						{
							...
							{
								$arResult['APP_OPTIONS'][$opt['name']] = \Thurly\Main\Text\Encoding::convertEncoding($opt['value'], 'utf-8', LANG_CHARSET);
							}
						}

						\Thurly\Main\Config\Option::set("rest", "options_".$arResult['APP_ID'], serialize($arResult['APP_OPTIONS']));

						echo \Thurly\Main\Web\Json::encode($arResult['APP_OPTIONS']);
					}
					else
					{
	...
	// set installation finish flag
				case 'set_installed':
					...
					{
						\Thurly\Rest\AppTable::setSkipRemoteUpdate(true);
						$updateResult = \Thurly\Rest\AppTable::update(
							$arParams['ID'],
							array(
						...
						\Thurly\Rest\AppTable::install($arParams['ID']);
	...
	// get and parse application URL
		...
		elseif($arParams['PLACEMENT'] !== \Thurly\Rest\PlacementTable::PLACEMENT_DEFAULT)
		{
			$arResult['APP_URL'] = $placementHandlerInfo['PLACEMENT_HANDLER'];
		}
		elseif(
			(
				$arResult['APP_STATUS']['STATUS'] == \Thurly\Rest\AppTable::STATUS_DEMO
				|| $arResult['APP_STATUS']['STATUS'] == \Thurly\Rest\AppTable::STATUS_TRIAL
			)
		...
		$serverPort = \Thurly\Main\Context::getCurrent()->getServer()->getServerPort();
		...
		$updateVersion = intval(\Thurly\Rest\Marketplace\Client::getAvailableUpdate($arApp["CODE"]));
		if(
