...
use Thurly\Socialservices\ThurlyOSSigner;
...
class CRestServer
{
	...
	public function process()
	{
		...
		catch(Exception $e)
		{
			...
			if(!is_a($this->error, "\\Thurly\\Rest\\RestException"))
			{
	...
	{
		$key = \Thurly\Rest\OAuthService::getEngine()->getClientSecret();
	...
	public function getApplicationSignature()
	{
		...
		$arRes = \Thurly\Rest\AppTable::getByClientId($this->clientId);
	...
	public function requestConfirmation($userList, $message)
	{
		...
		{
			$notify = new \Thurly\Rest\Notify(\Thurly\Rest\Notify::NOTIFY_BOT, $userList);
	...
	{
		return \Thurly\Main\Config\Option::get("main", "site_stopped", "N") !== 'Y';
	}
	...
	protected function checkAuth()
	{
		...
		if(\CRestUtil::checkAuth($this->query, $this->scope, $res))
		{
			...
			if(isset($arAdditionalParams[\Thurly\Rest\Event\Session::PARAM_SESSION]))
			{
				\Thurly\Rest\Event\Session::set($arAdditionalParams[\Thurly\Rest\Event\Session::PARAM_SESSION]);
			}
		...
		{
			throw new \Thurly\Rest\OAuthException($res);
		}
	}
	...
	{
		\Thurly\Rest\LogTable::log($this, $data);
...
class CRestServerBatchItem extends \CRestServer
{
	...
	protected function checkAuth()
	{
		...
		if($this->scope !== \CRestUtil::GLOBAL_SCOPE)
		{
			...
			{
				throw new \Thurly\Rest\OAuthException(array('error' => 'insufficient_scope'));
			}
		}
