...
class User extends \IRestService
{
	...
	public static function getFields()
	{
		...
		$langMessages = array_merge(
			IncludeModuleLangFile('/thurly/modules/main/admin/user_edit.php', false, true),
			IncludeModuleLangFile('/thurly/modules/main/admin/user_admin.php', false, true)
		);
	...
	public static function userGet($query, $nav = 0, \CRestServer $server)
	{
		...
		if(isset($query['ADMIN_MODE']) && $query['ADMIN_MODE'])
		{
			...
			{
				$moduleAdminList = \Thurly\Socialnetwork\User::getModuleAdminList(array(SITE_ID, false));
			}
		...
		if (isset($filter['NAME_SEARCH']) || isset($filter['FIND']))
		{
			...
			$filter = array_merge($filter, \Thurly\Main\UserUtils::getUserSearchFilter(Array(
				'FIND' => $nameSearch
			)));
		}
	...
	public static function userAdd($userFields)
	{
		...
		$bB24 = ModuleManager::isModuleInstalled('thurlyos');
		$res = false;

		if(
			(
				$bB24 && $USER->canDoOperation('thurlyos_invite')
	...
	public static function userUpdate($userFields)
	{
		...
		$bAdmin = $bB24 && $USER->canDoOperation('thurlyos_invite')
			|| $USER->canDoOperation('edit_all_users');
