...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/catalog/export_setup_templ.php');
...
$strAllowExportPath = COption::GetOptionString("catalog", "export_default_path", "/thurly/catalog_export/");
...
$aMenu = array(
	array(
		...
		"LINK"=>"/thurly/admin/cat_export_setup.php?lang=".LANGUAGE_ID,
		"ICON"=>"btn_list",
	)
);
...
?><tr>
	...
	echo GetIBlockDropDownListEx(
		...
		"ClearSelected(); changeIblockSites(0); BX('id_ifr').src='/thurly/tools/catalog_export/yandex_util.php?IBLOCK_ID=0&'+'".thurly_sessid_get()."';",
		"ClearSelected(); changeIblockSites(this[this.selectedIndex].value); BX('id_ifr').src='/thurly/tools/catalog_export/yandex_util.php?IBLOCK_ID='+this[this.selectedIndex].value+'&'+'".thurly_sessid_get()."';",
...
<tr>
	...
	function buildMenu()
	{
		...
		for (i in Tree[0])
		{
			...
			else
			{
				...
				imgSpace = '<img src="/thurly/images/catalog/load/plus.gif" width="13" height="13" id="img_' + i + '" OnClick="collapse(' + i + ')">';
			}
	...
	function collapse(node)
	{
		if (!BX('table_' + node))
		{
			...
			BX('img_' + node).src = '/thurly/images/catalog/load/minus.gif';
		}
		else
		{
			var tbl = BX('table_' + node);
			tbl.parentNode.removeChild(tbl);
			BX('img_' + node).src = '/thurly/images/catalog/load/plus.gif';
		}
		BX.adminPanel.modifyFormElements('yandex_setup_form');
	}
	</script>
	<iframe src="/thurly/tools/catalog_export/yandex_util.php?IBLOCK_ID=<?=intval($IBLOCK_ID)?>&<? echo thurly_sessid_get(); ?>" id="id_ifr" name="ifr" style="display:none"></iframe>
	</td>
</tr>
		...
		function showDetailPopup()
		{
			if (!obDetailWindow)
			{
				...
				var obDetailWindow = new BX.CAdminDialog({
					'content_url': '/thurly/tools/catalog_export/yandex_detail.php?lang=<?=LANGUAGE_ID?>&bxpublic=Y&IBLOCK_ID=' + s[s.selectedIndex].value,
					'content_post': 'XML_DATA='+BX.util.urlencode(dat.value)+'&'+'<?echo thurly_sessid_get(); ?>',
...
<tr>
	...
	<td width="60%">
		<b><? echo htmlspecialcharsbx(COption::GetOptionString("catalog", "export_default_path", "/thurly/catalog_export/"));?></b><input type="text" name="SETUP_FILE_NAME" value="<?=htmlspecialcharsbx($SETUP_FILE_NAME); ?>" size="50">
	</td>
</tr>
<?
	if ($ACTION=="EXPORT_SETUP" || $ACTION == 'EXPORT_EDIT' || $ACTION == 'EXPORT_COPY')
