...
use Thurly\Main,
	Thurly\Iblock,
	Thurly\Catalog,
	Thurly\Main\Localization\Loc as Loc,
...
CThurlyComponent::includeComponentClass("thurly:sale.bestsellers");
...
class CatalogBigdataProductsComponent extends CSaleBestsellersComponent
{
	...
	protected function extractDataFromCache()
	{
		...
		return !($this->startResultCache(false, $this->getAdditionalCacheId(), '/'.$this->getSiteId().'/thurly/catalog.bigdata.products/common'));
	}
	...
	protected function getServiceRequestParamsByType($type)
	{
		$a = array(
			...
			'aid' => \Thurly\Main\Analytics\Counter::getAccountId(),
		...
		if ($type == 'bestsell')
		{
			...
			$a['type'] = 'order';
			$a['domain'] = Thurly\Main\Context::getCurrent()->getServer()->getHttpHost();
		}
		...
		else
		{
			$iblockList = array();
			/* catalog */
			$iblockIterator = \Thurly\Catalog\CatalogIblockTable::getList(array(
			...
			/* iblock */
			$iblockIterator = \Thurly\Iblock\IblockSiteTable::getList(array(
		...
		if (!$showByIds)
		{
			...
			try
			{
				if (!$this->extractDataFromCache())
				{
					...
					{
						$this->clearResultCache($this->getAdditionalCacheId(), '/'.$this->getSiteId().'/thurly/catalog.bigdata.products/common');
					}
	...
	protected function getInjectedJs($items, $uniqId)
	{
		...
		$jsCookiePrefix = CUtil::JSEscape(COption::GetOptionString("main", "cookie_name", "THURLY_SM"));
