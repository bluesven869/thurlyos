	...
	class CIBlockCMLExportPackage extends \CIBlockCMLExport
	{
		...
		function ExportProperties(&$PROPERTY_MAP)
		{
			...
			while($arProp = $rsProp->Fetch())
			{
				...
				if($arProp["PROPERTY_TYPE"] == \Thurly\Iblock\PropertyTable::TYPE_LIST)
					$isPropertyEnum = true;

				if($arProp["PROPERTY_TYPE"] == \Thurly\Iblock\PropertyTable::TYPE_STRING && $arProp['USER_TYPE'] == "directory")
				{
					$isPropertyEnum = true;
					$isExternal = true;
				}
				...
				if($arProp["PROPERTY_TYPE"] == \Thurly\Iblock\PropertyTable::TYPE_LIST || $isExternal)
					$typeValue = GetMessage("IBLOCK_XML2_TYPE_LIST");
				elseif($arProp["PROPERTY_TYPE"] ==  \Thurly\Iblock\PropertyTable::TYPE_NUMBER)
					$typeValue = GetMessage("IBLOCK_XML2_TYPE_NUMBER");
				elseif($arProp["PROPERTY_TYPE"] == \Thurly\Iblock\PropertyTable::TYPE_STRING)
				{
...
class ManagerExport
{
	...
	static public function createSchema($typeId)
	{
		...
		{
			throw new \Thurly\Main\NotSupportedException("Schame type: '$typeId' is not supported in current context");
		}
	}
	...
	static public function createExport($typeId)
	{
		...
		{
			throw new \Thurly\Main\NotSupportedException("Export type: '$typeId' is not supported in current context");
		}
	}
}
			...
			if($_SESSION["BX_CML2_EXPORT"]["step"] === $manager::STEP_OFFERS_MIXED)
			{
				...
				$schema->initExport(
					...
					array(
						...
						'CATALOG_TYPE' => array(\Thurly\Catalog\ProductTable::TYPE_PRODUCT,
							\Thurly\Catalog\ProductTable::TYPE_SET
						)
					)
				);
				$schema->exportOffers();
