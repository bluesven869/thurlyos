	...
	public function onPrepareComponentParams($params)
	{
		...
		if(Main\Loader::includeModule('catalog'))
		{
			...
			else
			{
				...
				{
					$filter['=Thurly\Catalog\SubscribeAccessTable:SUBSCRIBE.TOKEN'] =
						$_SESSION['SUBSCRIBE_PRODUCT']['TOKEN'];
					$filter['=Thurly\Catalog\SubscribeAccessTable:SUBSCRIBE.USER_CONTACT'] =
			...
			foreach($offersList[$itemId] as $offersId => &$offers)
			{
				...
				foreach($offers['PROPERTIES'] as $propertiesCode => $properties)
				{
					...
					if(!in_array($properties['VALUE'],$propertyValue[$propertiesCode]))
					{
						...
						$valueId = ($properties['PROPERTY_TYPE'] == \Thurly\Iblock\PropertyTable::TYPE_LIST
