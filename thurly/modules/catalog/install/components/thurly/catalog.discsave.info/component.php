...
if ($onlySaleDiscount && class_exists('\Thurly\Sale\Discount\Preset\Manager'))
{
	$manager = \Thurly\Sale\Discount\Preset\Manager::getInstance();
...
if ($onlySaleDiscount && $cumulativePreset)
{
	...
	\Thurly\Main\Type\Collection::normalizeArrayValuesByInt($userGroups);

	$cumulativeDiscounts = \Thurly\Sale\Internals\DiscountTable::getList(
		array(
			'filter' => array(
				'=ACTIVE' => 'Y',
				'=PRESET_ID' => \Sale\Handlers\DiscountPreset\Cumulative::className(),
				'=\Thurly\Sale\Internals\DiscountGroupTable:DISCOUNT.ACTIVE' => 'Y',
				'@\Thurly\Sale\Internals\DiscountGroupTable:DISCOUNT.GROUP_ID' => $userGroups,
			),
			'data_doubling' => false
		)
	);
	...
	$cumulativeCalculator = new \Thurly\Sale\Discount\CumulativeCalculator($arFields['USER_ID'], $arFields['SITE_ID']);
	$arResult = array();
