...
class catalog extends CModule
{
	...
	function DoInstall()
	{
		...
		$APPLICATION->IncludeAdminFile(Loc::getMessage("CATALOG_INSTALL_TITLE"), $_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/step1.php");
	}
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/admin", $_SERVER['DOCUMENT_ROOT']."/thurly/admin");
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/components", $_SERVER['DOCUMENT_ROOT']."/thurly/components", true, true);
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/images", $_SERVER['DOCUMENT_ROOT']."/thurly/images/catalog", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/install/js", $_SERVER["DOCUMENT_ROOT"]."/thurly/js", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/install/panel", $_SERVER["DOCUMENT_ROOT"]."/thurly/panel", true, true);
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/themes", $_SERVER['DOCUMENT_ROOT']."/thurly/themes", true, true);
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/tools", $_SERVER['DOCUMENT_ROOT']."/thurly/tools", true, true);

			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/public/catalog_import", $_SERVER['DOCUMENT_ROOT']."/thurly/php_interface/include/catalog_import");
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/load_import/cron_frame.php", $_SERVER['DOCUMENT_ROOT']."/thurly/php_interface/include/catalog_import/cron_frame.php");
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/public/catalog_export", $_SERVER['DOCUMENT_ROOT']."/thurly/php_interface/include/catalog_export");
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/load/cron_frame.php", $_SERVER['DOCUMENT_ROOT']."/thurly/php_interface/include/catalog_export/cron_frame.php");
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/public/catalog_export/froogle_util.php", $_SERVER['DOCUMENT_ROOT']."/thurly/tools/catalog_export/froogle_util.php");
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/public/catalog_export/yandex_util.php", $_SERVER['DOCUMENT_ROOT']."/thurly/tools/catalog_export/yandex_util.php");
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/public/catalog_export/yandex_detail.php", $_SERVER['DOCUMENT_ROOT']."/thurly/tools/catalog_export/yandex_detail.php");

			CheckDirPath($_SERVER['DOCUMENT_ROOT']."/thurly/catalog_export/");
		}
	...
	function InstallDB()
	{
		...
		$thurlyos = ModuleManager::isModuleInstalled('thurlyos');

		if(!$DB->Query("SELECT 'x' FROM b_catalog_group", true))
			$errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/db/".strtolower($DB->type)."/install.sql");
		...
		\Thurly\Main\Config\Option::set('catalog', 'subscribe_repeated_notify', 'Y');

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->registerEventHandler('sale', 'onBuildCouponProviders', 'catalog', '\Thurly\Catalog\DiscountCouponTable', 'couponManager');
		$eventManager->registerEventHandler('sale', 'onBuildDiscountProviders', 'catalog', '\Thurly\Catalog\Discount\DiscountManager', 'catalogDiscountManager');
		$eventManager->registerEventHandler('sale', 'onExtendOrderData', 'catalog', '\Thurly\Catalog\Discount\DiscountManager', 'extendOrderData');
		$eventManager->registerEventHandler('currency', 'onAfterUpdateCurrencyBaseRate', 'catalog', '\Thurly\Catalog\Product\Price', 'handlerAfterUpdateCurrencyBaseRate');

		$eventManager->registerEventHandlerCompatible('main', 'onUserDelete', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onUserDelete');
		$eventManager->registerEventHandlerCompatible('iblock', 'OnIBlockElementDelete', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onIblockElementDelete');
		$eventManager->registerEventHandlerCompatible('catalog', 'OnProductUpdate', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onProductUpdate');
		$eventManager->registerEventHandlerCompatible('catalog', 'OnProductSetAvailableUpdate', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onProductSetAvailableUpdate');
		$eventManager->registerEventHandlerCompatible('catalog', 'onAddContactType', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onAddContactType');
		$eventManager->registerEventHandler('sale', 'OnSaleOrderSaved', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onSaleOrderSaved');
		...
		RegisterModuleDependences('iblock', 'OnIBlockElementAdd', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerIblockElementAdd');
		RegisterModuleDependences('iblock', 'OnAfterIBlockElementAdd', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerAfterIblockElementAdd');
		RegisterModuleDependences('iblock', 'OnIBlockElementUpdate', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerIblockElementUpdate');
		RegisterModuleDependences('iblock', 'OnAfterIBlockElementUpdate', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerAfterIblockElementUpdate');
		RegisterModuleDependences('iblock', 'OnIBlockElementDelete', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerIblockElementDelete');
		RegisterModuleDependences('iblock', 'OnAfterIBlockElementDelete', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerAfterIblockElementDelete');
		RegisterModuleDependences('iblock', 'OnIBlockElementSetPropertyValues', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerIblockElementSetPropertyValues');
		RegisterModuleDependences('iblock', 'OnAfterIBlockElementSetPropertyValues', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerAfterIBlockElementSetPropertyValues');

		RegisterModuleDependences('perfmon', 'OnGetTableSchema', 'catalog', 'catalog', 'getTableSchema');

		if (!$thurlyos)
		{
			CAgent::AddAgent('\Thurly\Catalog\CatalogViewedProductTable::clearAgent();', 'catalog', 'N', (int)COption::GetOptionString("catalog", "viewed_period") * 24 * 3600);
		}
	...
	{
		global $DB;
		include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/install/events/set_events.php");
		return true;
	}
	...
	function DoUnInstall()
	{
		...
		if ($step < 2)
		{
			$APPLICATION->IncludeAdminFile(Loc::getMessage("CATALOG_INSTALL_TITLE"), $_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/unstep1.php");
		}
		elseif ($step == 2)
		{
			$errors = false;
			...
			$APPLICATION->IncludeAdminFile(Loc::getMessage("CATALOG_INSTALL_TITLE"), $_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/unstep2.php");
		}
	}
	...
	function UnInstallFiles()
	{
		if ($_ENV["COMPUTERNAME"]!='BX')
		{
			DeleteDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/admin", $_SERVER['DOCUMENT_ROOT']."/thurly/admin");
			DeleteDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/themes/.default/", $_SERVER['DOCUMENT_ROOT']."/thurly/themes/.default");//css
			DeleteDirFilesEx("/thurly/themes/.default/icons/catalog/");//icons
			DeleteDirFilesEx("/thurly/tools/catalog/"); // scripts
			DeleteDirFilesEx("/thurly/js/catalog/");//javascript
			DeleteDirFilesEx("/thurly/panel/catalog/");//panel
		}
		return true;
	}
	...
	function UnInstallDB($arParams = array())
	{
		...
		{
			$errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT']."/thurly/modules/catalog/install/db/".strtolower($DB->type)."/uninstall.sql");
			if (!empty($errors))
			{
				$APPLICATION->ThrowException(implode("", $errors));
				return false;
		...
		UnRegisterModuleDependences('iblock', 'OnIBlockElementAdd', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerIblockElementAdd');
		UnRegisterModuleDependences('iblock', 'OnAfterIBlockElementAdd', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerAfterIblockElementAdd');
		UnRegisterModuleDependences('iblock', 'OnIBlockElementUpdate', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerIblockElementUpdate');
		UnRegisterModuleDependences('iblock', 'OnAfterIBlockElementUpdate', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerAfterIblockElementUpdate');
		UnRegisterModuleDependences('iblock', 'OnIBlockElementDelete', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerIblockElementDelete');
		UnRegisterModuleDependences('iblock', 'OnAfterIBlockElementDelete', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerAfterIblockElementDelete');
		UnRegisterModuleDependences('iblock', 'OnIBlockElementSetPropertyValues', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerIblockElementSetPropertyValues');
		UnRegisterModuleDependences('iblock', 'OnAfterIBlockElementSetPropertyValues', 'catalog', '\Thurly\Catalog\Product\Sku', 'handlerAfterIBlockElementSetPropertyValues');

		UnRegisterModuleDependences('perfmon', 'OnGetTableSchema', 'catalog', 'catalog', 'getTableSchema');

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->unRegisterEventHandler('sale', 'onBuildCouponProviders', 'catalog', '\Thurly\Catalog\DiscountCouponTable', 'couponManager');
		$eventManager->unRegisterEventHandler('sale', 'onBuildDiscountProviders', 'catalog', '\Thurly\Catalog\Discount\DiscountManager', 'catalogDiscountManager');
		$eventManager->unRegisterEventHandler('sale', 'onExtendOrderData', 'catalog', '\Thurly\Catalog\Discount\DiscountManager', 'extendOrderData');
		$eventManager->unRegisterEventHandler('currency', 'onAfterUpdateCurrencyBaseRate', 'catalog', '\Thurly\Catalog\Product\Price', 'handlerAfterUpdateCurrencyBaseRate');

		$eventManager->unRegisterEventHandler('main', 'onUserDelete', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onUserDelete');
		$eventManager->unRegisterEventHandler('iblock', 'OnIBlockElementDelete', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onIblockElementDelete');
		$eventManager->unRegisterEventHandler('catalog', 'OnProductUpdate', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onProductUpdate');
		$eventManager->unRegisterEventHandler('catalog', 'OnProductSetAvailableUpdate', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onProductSetAvailableUpdate');
		$eventManager->unRegisterEventHandler('catalog', 'onAddContactType', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onAddContactType');
		$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderSaved', 'catalog', '\Thurly\Catalog\SubscribeTable', 'onSaleOrderSaved');
	...
	{
		global $DB;
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/install/events/del_events.php");
		return true;
	}
