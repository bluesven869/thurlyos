			...
			if (!$bVarsFromForm)
			{
				if (empty($_POST['apply']))
				{
					if (!empty($returnUrl))
						...
						LocalRedirect('/thurly/admin/cat_discount_admin.php?lang='.LANGUAGE_ID.GetFilterParams('filter_', false));
				}
				else
				{
					LocalRedirect('/thurly/admin/cat_discount_edit.php?lang='.LANGUAGE_ID.'&ID='.$ID.GetFilterParams('filter_', false).'&'.$tabControl->ActiveTabParam());
				}
			}
		}
	}
...
if (0 < $ID)
{
	...
	else
	{
		...
		if (!isset($arDiscount['VERSION']) || (int)$arDiscount['VERSION'] != CCatalogDiscount::CURRENT_FORMAT)
		{
			...
			{
				$strLangPath = $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/catalog/lang/';
				$strDefLang = false;
				$arLangList = array();
				$by="def";
				$order="desc";
				...
				{
					$arFields = array(
						"MESSAGE" => str_replace("#LINK#", '/thurly/admin/cat_discount_convert.php', $arMess['BT_MOD_CAT_DSC_CONV_INVITE'][$strDefLang]),
					...
					foreach ($arMess['BT_MOD_CAT_DSC_CONV_INVITE'] as $strLangID => $strMess)
					{
						if (empty($strMess))
							continue;
						$arLangMess[$strLangID] = str_replace("#LINK#", '/thurly/admin/cat_discount_convert.php', $strMess);
					}
					if (!empty($arLangMess))
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/cat_discount_admin.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false)
	)
);
...
if ($ID > 0)
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/cat_discount_coupon.php?lang=".LANGUAGE_ID."&set_filter=Y&filter_discount_id=".$ID
	);
	...
	if (!$readOnly)
	{
		...
		$aMenu[] = array(
			...
			"ICON" => "btn_new",
			"LINK" => "/thurly/admin/cat_discount_edit.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false)
		);
		if (!$boolCopy)
		{
			$aMenu[] = array(
				"TEXT"=>GetMessage("BT_CAT_DISCOUNT_EDIT_CONT_NAME_COPY"),
				"LINK"=>"/thurly/admin/cat_discount_edit.php?ID=".$ID."&action=copy&lang=".LANGUAGE_ID.GetFilterParams("filter_", false),
				"ICON"=>"btn_copy",
			);
			...
			$aMenu[] = array(
				...
				"ICON" => "btn_delete",
				"LINK" => "javascript:if(confirm('".GetMessageJS("CDEN_DELETE_DISCOUNT_CONFIRM")."')) window.location='/thurly/admin/cat_discount_admin.php?action=delete&ID[]=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."#tb';",
				"WARNING" => "Y"
			);
		}
	}
...
$tabControl->Begin(array(
	"FORM_ACTION" => '/thurly/admin/cat_discount_edit.php?lang='.urlencode(LANGUAGE_ID),
));
...
$tabControl->BeginNextFormTab();
	...
	$strSubElementAjaxPath = '/thurly/admin/cat_subcoupons_admin.php?lang='.LANGUAGE_ID.'&find_discount_id='.$intDiscountID.'&TMP_ID='.urlencode($strSubTMP_ID);
	if (0 < $intDiscountID && $boolShowCoupons && !$boolCopy)
	{
		?><tr id="tr_COUPONS"><td colspan="2"><?
		require($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/catalog/admin/templates/discount_coupon_list.php');
		?></td></tr><?
	}
	else
	{
...
$arButtonsParams = array(
	"disabled" => $readOnly,
	"back_url" => "/thurly/admin/cat_discount_admin.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false)
);
