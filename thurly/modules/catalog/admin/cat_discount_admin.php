...
while ($arDiscount = $dbResultList->Fetch())
{
	...
	$row->AddViewField("ID", '<a href="/thurly/admin/cat_discount_edit.php?lang='.LANGUAGE_ID.'&ID='.$arDiscount["ID"].'">'.$arDiscount["ID"].'</a>');
	...
	$arActions[] = array(
		...
		"ACTION" => $lAdmin->ActionRedirect("/thurly/admin/cat_discount_edit.php?ID=".$arDiscount['ID']."&lang=".LANGUAGE_ID.GetFilterParams("filter_", false).""),
		"DEFAULT" => true
	);
	...
	if (!$bReadOnly)
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect("/thurly/admin/cat_discount_edit.php?ID=".$arDiscount['ID'].'&action=copy&lang='.LANGUAGE_ID)
		);
...
if ($arSelectFieldsMap['CREATED_BY'] || $arSelectFieldsMap['MODIFIED_BY'])
{
	if (!empty($arUserID))
	{
		...
		while ($oneUser = $userIterator->fetch())
		{
			...
			if ($canViewUserList)
				$arUserList[$oneUser['ID']] = '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$oneUser['ID'].'">'.CUser::FormatName($nameFormat, $oneUser).'</a>';
			else
