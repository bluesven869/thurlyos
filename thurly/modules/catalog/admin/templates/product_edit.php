...
{
	IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/catalog/templates/product_edit.php');
	...
	?>
		<tr>
			...
			<td width="60%"><?
				...
				<?else:
					echo GetMessage("C2IT_MEASURE_NO_MEASURE"); ?> <a href="/thurly/admin/cat_measure_list.php?lang=<? echo LANGUAGE_ID; ?>"><? echo GetMessage("C2IT_MEASURES"); ?></a><br><?
				endif;?>
			</td>
		</tr>
	<?
...
<table border="0" cellspacing="0" cellpadding="0" width="100%" class="internal">
	...
	if ($bNoAvailGroups)
	{
		?>
		<tr>
			<td colspan="3"><? echo GetMessage("C2IT_NO_USER_GROUPS1")?> <a href="/thurly/admin/settings.php?mid=catalog&lang=<? echo LANGUAGE_ID; ?>"><?echo GetMessage("C2IT_NO_USER_GROUPS2")?></a>.</td>
		</tr>
		<?
	}
	?>
...
<?endif;
	...
	else
	{
		...
		$discountUrl = '/thurly/admin/cat_discount_edit.php?ID=';
		if (Main\ModuleManager::isModuleInstalled('sale') && (string)Main\Config\Option::get('sale', 'use_sale_discount_only') == 'Y')
		{
			...
			$discountUrl = '/thurly/admin/sale_discount_edit.php?ID=';
		}
	...
	if ($storeCount > 0)
	{
		...
		foreach ($stores as $storeIndex => $row)
		{
			...
			{
				$storeId = $row['ID'];
				$address = ('' != $row['ADDRESS'] ? htmlspecialcharsbx($row['ADDRESS']) : '<a href="/thurly/admin/cat_store_edit.php?ID='.$row['ID'].'&lang='.LANGUAGE_ID.'">'.GetMessage("C2IT_EDIT").'</a>');
				$storeUrl = '<a href="/thurly/admin/cat_store_edit.php?ID='.$row['ID'].'&lang='.LANGUAGE_ID.'">'.$storeUrl.'</a>';
			}
			?><tr>
	...
	else
	{
		if ($bStore)
		{
			?><b><? echo GetMessage("C2IT_STORE_NO_STORE"); ?> <a href="/thurly/admin/cat_store_list.php?lang=<? echo LANGUAGE_ID; ?>"><? echo GetMessage("C2IT_STORE"); ?></a></b><br><?
		}
	}
	if (!$bUseStoreControl)
		...
		<script type="text/javascript">
			function getDataSubscriptions() {
				BX.ajax({
					method: 'POST',
					dataType: 'json',
					url: '/thurly/tools/catalog/subscription_card_product.php',
					data: {
		...
		<div id="subscription-data">
			...
			<table border="0" cellspacing="0" cellpadding="5" width="100%" class="edit-table" id="subscription-data-table">
				...
				<tr>
					...
					<td width="60%">
						<a href="/thurly/admin/cat_subscription_list.php?ITEM_ID=<?=htmlspecialcharsbx($PRODUCT_ID)?>
