...
while ($row = $iterator->fetch())
{
	...
	$arSiteLinkList[$row['LID']] = '<a href="/thurly/admin/site_edit.php?lang='.LANGUAGE_ID.'&LID='.$row['LID'].'" title="'.GetMessage('BT_CAT_DISCOUNT_ADM_MESS_SITE_ID').'">'.$row['LID'].'</a>';
}
unset($row, $iterator);
...
while($arRes = $rsDiscSaves->Fetch())
{
	...
	$row->AddViewField("ID", '<a href="/thurly/admin/cat_discsave_edit.php?lang='.LANGUAGE_ID.'&ID='.$arRes["ID"].'">'.$arRes["ID"].'</a>');
	...
	else
	{
		if ($arSelectFieldsMap['SITE_ID'])
			$row->AddViewField('SITE_ID',$arSiteLinkList[$arRes['SITE_ID']]);
		if ($arSelectFieldsMap['NAME'])
			$row->AddViewField("NAME", '<a href="/thurly/admin/cat_discsave_edit.php?lang='.LANGUAGE_ID.'&ID='.$arRes["ID"].'">'.htmlspecialcharsEx($arRes['NAME']).'</a>');
		if ($arSelectFieldsMap['ACTIVE'])
	...
	$arActions[] = array(
		...
		"ACTION" => $lAdmin->ActionRedirect("/thurly/admin/cat_discsave_edit.php?ID=".$arRes['ID'].'&lang='.LANGUAGE_ID)
	);
	if (!$bReadOnly)
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect("/thurly/admin/cat_discsave_edit.php?ID=".$arRes['ID'].'&action=copy&lang='.LANGUAGE_ID)
		);
...
if (!$bReadOnly)
{
	$aContext = array(
		array(
			...
			"LINK" => "/thurly/admin/cat_discsave_edit.php?lang=".LANGUAGE_ID,
