...
if (!$bReadOnly && 'POST' == $_SERVER['REQUEST_METHOD'] && (strlen($save)>0 || strlen($apply)>0) && check_thurly_sessid())
{
	$arGroupID = array();
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/cat_group_admin.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false)
	)
);
...
if ($ID > 0 && !$bReadOnly)
{
	if (CBXFeatures::IsFeatureEnabled('CatMultiPrice'))
	{
		...
		$aMenu[] = array(
			...
			"ICON" => "btn_new",
			"LINK" => "/thurly/admin/cat_group_edit.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false)
		);
	}
	...
	if (CBXFeatures::IsFeatureEnabled('CatMultiPrice') || !$boolRealBase)
	{
		$aMenu[] = array(
			...
			"ICON" => "btn_delete",
			"LINK" => "javascript:if(confirm('".GetMessage("CGEN_DELETE_GROUP_CONFIRM")."')) window.location='/thurly/admin/cat_group_admin.php?action=delete&ID[]=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."#tb';",
			"WARNING" => "Y"
		);
	}
}
...
$tabControl->Buttons(
		array(
				"disabled" => $bReadOnly,
				"back_url" => "/thurly/admin/cat_group_admin.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false)
			)
	);
