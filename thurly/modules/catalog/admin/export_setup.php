...
setTimeout('DoNext()', 2000);
				...
				{
					$strSetupFileName = '';
					$strRedirect = '/thurly/admin/cat_export_setup.php?lang='.urlencode(LANGUAGE_ID).'&success_export=Y';
...
foreach ($arReportsList as $strReportFile => $arReportParams)
{
	if ($bCanEdit && !empty($arReportParams["FILE_SETUP"]))
	{
		$arContextMenu[] = array(
			...
			"ACTION"=>"window.location='".addslashes($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&ACTION=EXPORT_SETUP"."&".thurly_sessid_get())."';"
		);
	}
	...
	while ($arProfile = $rsProfiles->Fetch())
	{
		...
		if ($bCanEdit)
		{
			if ($boolNeedEdit)
			{
				$strProfileLink = '<a href="'.$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($strReportFile)."&amp;ACTION=EXPORT_EDIT&amp;PROFILE_ID=".$arProfile["ID"]."&amp;".thurly_sessid_get().'" title="'.GetMessage("CES_EDIT_PROPFILE_DESCR").'"><i>'.GetMessage("CES_DEFAULT").'</i></a><br /><i>('.GetMessage('CES_NEED_EDIT').')</i>';
			}
			else
			{
				$strProfileLink = '<a href="'.('Y' == $arProfile["IN_MENU"] ? '/thurly/admin/cat_exec_exp.php' : $APPLICATION->GetCurPage()).'?lang='.LANGUAGE_ID."&amp;ACT_FILE=".urlencode($strReportFile)."&amp;ACTION=EXPORT&amp;PROFILE_ID=".$arProfile["ID"]."&amp;".thurly_sessid_get().'" title="'.GetMessage("export_setup_begin").'"><i>'.GetMessage("CES_DEFAULT").'</i></a>';
			}
		}
		else
		{
		...
		if ($bCanExec)
		{
			$arActions[] = array(
				...
				"ACTION"=>$lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&".thurly_sessid_get()."&ACTION=EXPORT&PROFILE_ID=".$arProfile['ID']),
			);
			$arActions[] = array(
				"TEXT" => GetMessage('CES_ADD_PROFILE'),
				"TITLE" => GetMessage('CES_ADD_PROFILE_DESCR'),
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&ACTION=EXPORT_SETUP"."&".thurly_sessid_get()),
			);
		}
	...
	if (!$boolExist)
	{
		...
		if ($bCanEdit)
		{
			$strProfileLink = '<a href="'.$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($strReportFile)."&amp;".thurly_sessid_get()."&amp;ACTION=EXPORT&amp;PROFILE_ID=0".'" title="'.GetMessage("export_setup_begin").'"><i>'.GetMessage("CES_DEFAULT").'</i></a>';
		}
		$row->AddViewField('PROFILE', $strProfileLink);
		...
		if ($bCanExec)
		{
			$arActions[] = array(
				...
				"TEXT" => GetMessage("CES_RUN_EXPORT"),
				"TITLE" => GetMessage("CES_RUN_EXPORT_DESCR"),
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&".thurly_sessid_get()."&ACTION=EXPORT&PROFILE_ID=0"),
			);
			$arActions[] = array(
				"TEXT" => GetMessage('CES_ADD_PROFILE'),
				"TITLE" => GetMessage('CES_ADD_PROFILE_DESCR'),
	...
	while ($arProfile = $rsProfiles->Fetch())
	{
		...
		if ($bCanExec)
		{
			if ($boolNeedEdit)
			{
				$strProfileLink = '<a href="'.$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($strReportFile)."&amp;ACTION=EXPORT_EDIT&amp;PROFILE_ID=".$arProfile["ID"]."&amp;".thurly_sessid_get().'" title="'.GetMessage("CES_EDIT_PROPFILE_DESCR").'">'.htmlspecialcharsbx($arProfile["NAME"]).'</a>'.
					'<br /><i>('.GetMessage('CES_NEED_EDIT').')</i>';
			}
			else
			{
				$strProfileLink = '<a href="'.('Y' == $arProfile["IN_MENU"] ? "/thurly/admin/cat_exec_exp.php" : $APPLICATION->GetCurPage())."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($strReportFile)."&amp;ACTION=EXPORT&amp;PROFILE_ID=".$arProfile["ID"]."&amp;".thurly_sessid_get().'" title="'.GetMessage("export_setup_begin").'">'.htmlspecialcharsbx($arProfile["NAME"]).'</a>';
			}
		}
		else
		{
		...
		if ($bCanExec)
			$arActions[] = array(
				...
				"TEXT" => GetMessage("CES_RUN_EXPORT"),
				"TITLE" => GetMessage("CES_RUN_EXPORT_DESCR"),
				"ACTION" => $lAdmin->ActionRedirect(('Y' == $arProfile["IN_MENU"] ? "/thurly/admin/cat_exec_exp.php" : $APPLICATION->GetCurPage())."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&".thurly_sessid_get()."&ACTION=EXPORT&PROFILE_ID=".$arProfile["ID"]),
			);
		...
		if ($bCanEdit)
		{
			...
			$arActions[] = array(
				...
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".urlencode(LANGUAGE_ID)."&ACT_FILE=".urlencode($strReportFile)."&ACTION=EXPORT_EDIT&PROFILE_ID=".$arProfile['ID']."&".thurly_sessid_get()),
			);
			$arActions[] = array(
				"TEXT" => GetMessage("CES_COPY_PROFILE"),
				"TITLE" => GetMessage("CES_COPY_PROPFILE_DESCR"),
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".urlencode(LANGUAGE_ID)."&ACT_FILE=".urlencode($strReportFile)."&ACTION=EXPORT_COPY&PROFILE_ID=".$arProfile['ID']."&".thurly_sessid_get()),
			);
		}
...
echo BeginNote();
	...
	<?else:?>
		...
		<?=GetMessage("CES_NOTES11_EXT", array('#FILE#' => '/thurly/php_interface/include/catalog_export/cron_frame.php'));?><br>
		<?=GetMessage("CES_NOTES12_EXT");?><br>
		<?=GetMessage('CES_NOTES13_EXT', array('#FOLDER#' => '/thurly/modules/catalog/load/'));
	endif;
