...
if ($_SERVER["REQUEST_METHOD"] == "POST" && strlen($_REQUEST["Update"]) > 0 && !$bReadOnly && check_thurly_sessid())
{
	...
	if ($errorMessage == '')
	{
		...
		if (strlen($_REQUEST["apply"]) <= 0)
			LocalRedirect("/thurly/admin/cat_store_list.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false));
		else
			LocalRedirect("/thurly/admin/cat_store_edit.php?lang=".LANGUAGE_ID."&ID=".$id."&".GetFilterParams("filter_", false));
	}
	else
	{
		$bVarsFromForm = true;
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/cat_store_list.php?lang=".LANG."&".GetFilterParams("filter_", false)
	)
);
...
if ($id > 0 && !$bReadOnly)
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_new",
		"LINK" => "/thurly/admin/cat_store_edit.php?lang=".LANG."&".GetFilterParams("filter_", false)
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("STORE_DELETE"),
		"ICON" => "btn_delete",
		"LINK" => "javascript:if(confirm('".GetMessage("STORE_DELETE_CONFIRM")."')) window.location='/thurly/admin/cat_store_list.php?action=delete&ID[]=".$id."&lang=".LANG."&".thurly_sessid_get()."#tb';",
		"WARNING" => "Y"
	);
}
...
<form enctype="multipart/form-data" method="POST" action="<?echo $APPLICATION->GetCurPage()?>?" name="store_edit">
	...
	<tr>
		<td align="left" colspan="2">
			<a href="/thurly/admin/userfield_edit.php?lang=<?=LANGUAGE_ID;?>&amp;ENTITY_ID=<?=$entityId;?>&amp;back_url=<?echo urlencode($APPLICATION->GetCurPageParam('', array('bxpublic'))."&tabControl_active_tab=user_fields_tab")?>"><?=GetMessage("STORE_E_USER_FIELDS_ADD_HREF");?></a>
		</td>
	</tr>
	<?if ($id > 0):?>
	<tr>
	...
	$tabControl->Buttons(
		array(
			"disabled" => $bReadOnly,
			"back_url" => "/thurly/admin/cat_store_list.php?lang=".LANG."&".GetFilterParams("filter_", false)
		)
	);
	$tabControl->End();
	?>
