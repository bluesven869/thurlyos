...
if ($_SERVER["REQUEST_METHOD"] == "POST" && check_thurly_sessid() && strlen($_REQUEST["Update"]) > 0 && !$bReadOnly)
{
	...
	if (strlen($errorMessage) == 0 && $ID > 0 && $res = CCatalogContractor::update($ID, $arFields))
	{
		...
		if (strlen($_REQUEST["apply"])<=0)
			LocalRedirect("/thurly/admin/cat_contractor_list.php?lang=".LANG."&".GetFilterParams("filter_", false));
		else
			LocalRedirect("/thurly/admin/cat_contractor_edit.php?lang=".LANG."&ID=".$ID."&".GetFilterParams("filter_", false));
	}
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/cat_contractor_list.php?lang=".LANG."&".GetFilterParams("filter_", false)
	)
);
...
if ($ID > 0 && !$bReadOnly)
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_new",
		"LINK" => "/thurly/admin/cat_contractor_edit.php?lang=".LANG."&".GetFilterParams("filter_", false)
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("CONTRACTOR_DELETE"),
		"ICON" => "btn_delete",
		"LINK" => "javascript:if(confirm('".GetMessage("CONTRACTOR_DELETE_CONFIRM")."')) window.location='/thurly/admin/cat_contractor_list.php?action=delete&ID[]=".$ID."&lang=".LANG."&".thurly_sessid_get()."#tb';",
		"WARNING" => "Y"
	);
}
...
<?CAdminMessage::ShowMessage($errorMessage);?>
	...
	<form enctype="multipart/form-data" method="POST" action="<?echo $APPLICATION->GetCurPage()?>?" name="contractor_edit">
		...
		$tabControl->Buttons(
			array(
				"disabled" => $bReadOnly,
				"back_url" => "/thurly/admin/cat_contractor_list.php?lang=".LANG."&".GetFilterParams("filter_", false)
			)
		);
		$tabControl->End();
		?>
