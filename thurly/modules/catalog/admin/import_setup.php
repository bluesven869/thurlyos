...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/catalog/prolog.php");
...
/////////////////////////////////////////////////////////////////////
if (($bCanEdit || $bCanExec) && check_thurly_sessid())
{
		...
		if (strlen($strErrorMessage)<=0)
		{
			...
			if ($bCanExec && $_REQUEST["ACTION"]=="IMPORT")
			{
					...
					if ($bDefaultProfile || $boolNeedEdit)
					{
						if (strlen($arReportsList[$strActFileName]["FILE_SETUP"]) > 0)
						{
							...
							if ($FINITE !== true)
							{
								...
								include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/prolog_admin_after.php");

								echo $ob;

								include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/epilog_admin.php");
								die();
							}
							ob_end_clean();
						}
				...
				if (!$bAllDataLoaded)
				{
					...
					$fullUrl = $APPLICATION->GetCurPage().'?lang='.LANGUAGE_ID.'&'.$urlParams.'&'.thurly_sessid_get();
					...
					include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/epilog_admin_after.php");
					die();
				}
				else
				{
				...
				if (strlen($strErrorMessage)<=0)
				{
					...
					LocalRedirect("/thurly/admin/cat_import_setup.php?lang=".LANGUAGE_ID."&success_import=Y&message_sess_id=".$randVal);
				}
			}
			//////////////////////////////////////////////
			// MENU
			...
			elseif ($USER->CanDoOperation('edit_php') && $_REQUEST["ACTION"]=="CRON")
			{
				...
				if (strlen($strErrorMessage)<=0)
				{
					...
					$cfg_data = "";
					if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/crontab/crontab.cfg"))
					{
						$cfg_file_size = filesize($_SERVER["DOCUMENT_ROOT"]."/thurly/crontab/crontab.cfg");
						$fp = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/crontab/crontab.cfg", "rb");
					...
					if (strlen($strErrorMessage)<=0)
					{
						CheckDirPath($_SERVER["DOCUMENT_ROOT"]."/thurly/crontab/");
						$cfg_data = preg_replace("#[\r\n]{2,}#im", "\n", $cfg_data);
						$fp = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/crontab/crontab.cfg", "wb");
						fwrite($fp, $cfg_data);
						fclose($fp);
						...
						{
							$arRetval = array();
							@exec("crontab ".$_SERVER["DOCUMENT_ROOT"]."/thurly/crontab/crontab.cfg", $arRetval, $return_var);
...
foreach($arReportsList as $strReportFile => $arReportParams)
{
	if ($bCanEdit && !empty($arReportParams["FILE_SETUP"]))
	{
		$arContextMenu[] = array(
			...
			"ACTION"=>"window.location='".addslashes($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&ACTION=IMPORT_SETUP"."&".thurly_sessid_get())."';"
		);
	}
	...
	while ($arProfile = $rsProfiles->Fetch())
	{
		...
		if ($bCanEdit)
		{
			if ($boolNeedEdit)
			{
				$strProfileLink = '<a href="'.$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($strReportFile)."&amp;ACTION=IMPORT_EDIT&amp;PROFILE_ID=".$arProfile["ID"]."&amp;".thurly_sessid_get().'" title="'.GetMessage("CES_EDIT_PROPFILE_DESCR").'"><i>'.GetMessage("CES_DEFAULT").'</i></a><br /><i>('.GetMessage('CES_NEED_EDIT').')</i>';
			}
			else
			{
				$strProfileLink = '<a href="'.('Y' == $arProfile["IN_MENU"] ? '/thurly/admin/cat_exec_imp.php' : $APPLICATION->GetCurPage()).'?lang='.LANGUAGE_ID."&amp;ACT_FILE=".urlencode($strReportFile)."&amp;ACTION=IMPORT&amp;PROFILE_ID=".$arProfile["ID"]."&amp;".thurly_sessid_get().'" title="'.GetMessage("import_setup_begin").'"><i>'.GetMessage("CES_DEFAULT").'</i></a>';
			}
		}
		else
		{
		...
		if (0 < $arProfile['MODIFIED_BY'])
		{
			if (!isset($arUserList[$arProfile['MODIFIED_BY']]))
			{
				...
				if ($arOneUser = $rsUsers->Fetch())
				{
					...
					$arUserList[$arOneUser['ID']] = '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$arProfile['MODIFIED_BY'].'">'.CUser::FormatName($strNameFormat, $arOneUser).'</a>';
				}
			}
			if (isset($arUserList[$arProfile['MODIFIED_BY']]))
		...
		if ($bCanExec)
		{
			$arActions[] = array(
				...
				"ACTION"=>$lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&".thurly_sessid_get()."&ACTION=IMPORT&PROFILE_ID=".$arProfile['ID']),
			);
			$arActions[] = array(
				"TEXT" => GetMessage('CES_ADD_PROFILE'),
				"TITLE" => GetMessage('CES_ADD_PROFILE_DESCR'),
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&ACTION=IMPORT_SETUP"."&".thurly_sessid_get()),
			);
		}
		...
		if ($bCanEdit)
		{
			if ('Y' == $arProfile["IN_MENU"])
			{
				$arActions[] = array(
					...
					"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&".thurly_sessid_get()."&ACTION=MENU&PROFILE_ID=".$arProfile['ID']),
				);
			}
			else
			{
		...
		if (empty($arReportParams["FILE_SETUP"]) && 'Y' != $arProfile["NEED_EDIT"] && $USER->CanDoOperation('edit_php'))
		{
			if ('Y' == $arProfile["IN_AGENT"])
			{
				$arActions[] = array(
					...
					"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($ReportFile)."&".thurly_sessid_get()."&ACTION=AGENT&PROFILE_ID=".$ar_prof_res["ID"]),
				);
			}
			else
			{
				$arActions[] = array(
					...
					"ACTION" => "ShowAgentForm('".$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".$strReportFile."&amp;".thurly_sessid_get()."&amp;ACTION=AGENT&amp;PROFILE_ID=".$arProfile["ID"]."');",
				);
			}
			...
			if ('Y' == $arProfile['IN_CRON'])
			{
				$arActions[] = array(
					...
					"ACTION" => ($bWindowsHosting ? '' : "ShowCronForm('".$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".$strReportFile."&amp;".thurly_sessid_get()."&amp;ACTION=CRON&amp;PROFILE_ID=".$arProfile["ID"]."', false);"),
				);
			}
			else
			{
				$arActions[] = array(
					...
					"ACTION" => ($bWindowsHosting ? '' : "ShowCronForm('".$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".$strReportFile."&amp;".thurly_sessid_get()."&amp;ACTION=CRON&amp;PROFILE_ID=".$arProfile["ID"]."', true);"),
				);
			}
		}
	...
	if (!$boolExist)
	{
		...
		if ($bCanEdit)
		{
			$strProfileLink = '<a href="'.$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($strReportFile)."&amp;".thurly_sessid_get()."&amp;ACTION=IMPORT&amp;PROFILE_ID=0".'" title="'.GetMessage("export_setup_begin").'"><i>'.GetMessage("CES_DEFAULT").'</i></a>';
		}
		$row->AddViewField('PROFILE', $strProfileLink);
		...
		if ($bCanExec)
		{
			$arActions[] = array(
				...
				"TEXT" => GetMessage("CES_RUN_IMPORT"),
				"TITLE" => GetMessage("CES_RUN_IMPORT_DESCR"),
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&".thurly_sessid_get()."&ACTION=IMPORT&PROFILE_ID=0"),
			);
			$arActions[] = array(
				"TEXT" => GetMessage('CES_ADD_PROFILE'),
				"TITLE" => GetMessage('CES_ADD_PROFILE_DESCR'),
		...
		if ($bCanEdit)
		{
			$arActions[] = array(
				"TEXT" => GetMessage("CES_TO_LEFT_MENU"),
				"TITLE" => GetMessage("CES_TO_LEFT_MENU_DESCR"),
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&".thurly_sessid_get()."&ACTION=MENU&PROFILE_ID=0"),
			);
		}
		...
		if (empty($arReportParams["FILE_SETUP"]) && $USER->CanDoOperation('edit_php'))
		{
			$arActions[] = array(
				"TEXT" => GetMessage("CES_TO_AGENT"),
				"TITLE" => GetMessage("CES_TO_AGENT_DESCR"),
				"ACTION" => "ShowAgentForm('".$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".$strReportFile."&amp;".thurly_sessid_get()."&amp;ACTION=AGENT&amp;PROFILE_ID=0');",
			);
			$arActions[] = array(
				"DISABLED" => $bWindowsHosting,
				"TEXT" => GetMessage("CES_TO_CRON"),
				"TITLE" => GetMessage("CES_TO_CRON_DESCR"),
				"ACTION" => ($bWindowsHosting ? '' : "ShowCronForm('".$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".$strReportFile."&amp;".thurly_sessid_get()."&amp;ACTION=CRON&amp;PROFILE_ID=0', true);"),
			);
		}
	...
	while ($arProfile = $rsProfiles->Fetch())
	{
		...
		if ($bCanExec)
		{
			if ($boolNeedEdit)
			{
				$strProfileLink = '<a href="'.$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($strReportFile)."&amp;ACTION=IMPORT_EDIT&amp;PROFILE_ID=".$arProfile["ID"]."&amp;".thurly_sessid_get().'" title="'.GetMessage("CES_EDIT_PROPFILE_DESCR").'">'.htmlspecialcharsbx($arProfile["NAME"]).'</a>'.
					'<br /><i>('.GetMessage('CES_NEED_EDIT').')</i>';
			}
			else
			{
				$strProfileLink = '<a href="'.('Y' == $arProfile["IN_MENU"] ? "/thurly/admin/cat_exec_imp.php" : $APPLICATION->GetCurPage())."?lang=".LANGUAGE_ID."&amp;ACT_FILE=".urlencode($strReportFile)."&amp;ACTION=IMPORT&amp;PROFILE_ID=".$arProfile["ID"]."&amp;".thurly_sessid_get().'" title="'.GetMessage("export_setup_begin").'">'.htmlspecialcharsbx($arProfile["NAME"]).'</a>';
			}
		}
		else
		{
		...
		if (0 < $arProfile['CREATED_BY'])
		{
			if (!isset($arUserList[$arProfile['CREATED_BY']]))
			{
				...
				{
					$arOneUser['ID'] = (int)$arOneUser['ID'];
					$arUserList[$arOneUser['ID']] = '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$arProfile['CREATED_BY'].'">'.CUser::FormatName($strNameFormat, $arOneUser).'</a>';
				}
			}
			if (isset($arUserList[$arProfile['CREATED_BY']]))
		...
		if ($bCanExec)
			$arActions[] = array(
				...
				"TEXT" => GetMessage("CES_RUN_IMPORT"),
				"TITLE" => GetMessage("CES_RUN_IMPORT_DESCR"),
				"ACTION" => $lAdmin->ActionRedirect(('Y' == $arProfile["IN_MENU"] ? "/thurly/admin/cat_exec_imp.php" : $APPLICATION->GetCurPage())."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&".thurly_sessid_get()."&ACTION=IMPORT&PROFILE_ID=".$arProfile["ID"]),
			);
		...
		if ($bCanEdit)
		{
			...
			$arActions[] = array(
				...
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".urlencode(LANGUAGE_ID)."&ACT_FILE=".urlencode($strReportFile)."&ACTION=IMPORT_EDIT&PROFILE_ID=".$arProfile['ID']."&".thurly_sessid_get()),
			);
			$arActions[] = array(
				"TEXT" => GetMessage("CES_COPY_PROFILE"),
				"TITLE" => GetMessage("CES_COPY_PROPFILE_DESCR"),
				"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".urlencode(LANGUAGE_ID)."&ACT_FILE=".urlencode($strReportFile)."&ACTION=IMPORT_COPY&PROFILE_ID=".$arProfile['ID']."&".thurly_sessid_get()),
			);
		}
		...
		if ($USER->CanDoOperation('edit_php') && 'Y' != $arProfile["NEED_EDIT"])
		{
			if ('Y' == $arProfile["IN_AGENT"])
			{
				$arActions[] = array(
					"TEXT" => GetMessage("CES_TO_AGENT_DEL"),
					"TITLE" => GetMessage("CES_TO_AGENT_DESCR_DEL"),
					"ACTION" => $lAdmin->ActionRedirect($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&".thurly_sessid_get()."&ACTION=AGENT&PROFILE_ID=".$arProfile["ID"]),
				);
			}
			else
			{
		...
		if($bCanEdit)
		{
			...
			$arActions[] = array(
				...
				"ACTION" => "if(confirm('".GetMessage("CES_DELETE_PROFILE_CONF")."')) window.location='".$APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID."&ACT_FILE=".urlencode($strReportFile)."&".thurly_sessid_get()."&ACTION=DEL_PROFILE&PROFILE_ID=".$arProfile["ID"]."';",
			);
		}
		$row->AddActions($arActions);
	}
...
echo BeginNote();
	...
	<?else:?>
		...
		<b><?echo $_SERVER["DOCUMENT_ROOT"];?>/thurly/crontab/crontab.cfg</b>
		<?echo GetMessage("CES_NOTES4");?><br>
		<?echo GetMessage("CES_NOTES5");?><br>
		<b>crontab <?echo $_SERVER["DOCUMENT_ROOT"];?>/thurly/crontab/crontab.cfg</b><br>
		...
		<?=GetMessage("CES_NOTES11_EXT", array('#FILE#' => '/thurly/php_interface/include/catalog_import/cron_frame.php'));?><br>
		<?=GetMessage("CES_NOTES12_EXT");?><br>
		<?=GetMessage('CES_NOTES13_EXT', array('#FOLDER#' => '/thurly/modules/catalog/load_import/'));
	endif;
