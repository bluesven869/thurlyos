...
use Thurly\Currency\CurrencyTable;
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && strlen($_REQUEST["Update"]) > 0 && !$bReadOnly && check_thurly_sessid())
{
	if (!$_REQUEST["cancellation"] && ($_REQUEST["save_document"] || $_REQUEST["save_and_conduct"]))
	{
		...
		if ($_REQUEST["save_document"] && $docId)
			LocalRedirect("/thurly/admin/cat_store_document_edit.php?lang=".LANGUAGE_ID."&ID=".$docId.GetFilterParams("filter_", false));
	}
	...
	if ($_REQUEST["save_and_conduct"] || $_REQUEST["cancellation"])
	{
		...
		{
			LocalRedirect("/thurly/admin/cat_store_document_list.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
		}
	}
}
ClearVars();
...
$APPLICATION->SetAdditionalCSS('/thurly/panel/catalog/catalog_store_docs.css');
if ($bVarsFromForm)
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/cat_store_document_list.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false)
	)
);
					...
					<?if(isset($requiredFields["CONTRACTOR"])):?>
						<tr class="adm-detail-required-field">
							...
							<td width="60%" class="adm-detail-content-cell-r">
								...
								<?else:?>
									<a href="/thurly/admin/cat_contractor_edit.php?lang=<? echo urlencode(LANGUAGE_ID); ?>"><?echo GetMessage("CAT_DOC_CONTRACTOR_ADD")?></a>
								<?endif;?>
							</td>
						</tr>
					<?endif;?>
...
$tabControl->Buttons(
	array(
		...
		"btnApply" => false,
		"btnCancel" => false,
		"back_url" => "/thurly/admin/cat_store_document_list.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false),
	)
);
...
if (typeof showTotalSum === 'undefined')
{
	...
	function makeProductSearchDialog(params)
	{
		...
		var popup = new BX.CDialog({
			content_url: '/thurly/admin/cat_product_search_dialog.php?lang='+lang+'&LID='+site_id+'&caller=' + caller + '&func_name='+callback+'&STORE_FROM_ID='+store_id,
	...
	{
		var dateURL = '<?=thurly_sessid_get()?>&BARCODE_AJAX=Y&BARCODE='+barcode+'&lang=<? echo LANGUAGE_ID; ?>';

		BX.showWait();
		BX.ajax.post('/thurly/admin/cat_store_product_search.php', dateURL, fSearchProductResult);
	}
