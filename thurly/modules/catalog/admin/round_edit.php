...
if (!$readOnly && $ruleId > 0)
{
	if (!$copy)
	{
		...
		$contextMenuItems[] = array(
			...
			'LINK' => "javascript:if (confirm('".CUtil::JSEscape(Loc::getMessage('PRICE_ROUND_EDIT_CONTEXT_DELETE_CONFIRM'))."')) window.location='/thurly/admin/cat_round_list.php?lang=".LANGUAGE_ID."&ID=".$ruleId."&action=delete&".thurly_sessid_get()."';",
			'WARNING' => 'Y',
		);
	}
}
