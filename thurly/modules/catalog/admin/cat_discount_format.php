...
if ('POST' == $_SERVER['REQUEST_METHOD'] && (isset($_REQUEST["Convert"]) && 'Y' == $_REQUEST["Convert"]) && (isset($_REQUEST["format"]) && 'Y' == $_REQUEST["format"]) && check_thurly_sessid())
{
		...
		if (!empty($arErrors) && is_array($arErrors))
		{
			...
			foreach ($arErrors as &$arOneError)
			{
				$strDetail .= '<li>'.str_replace(
					...
					array(
						'/thurly/admin/cat_discount_edit.php?lang='.LANGUAGE_ID.'&ID='.$arOneError['ID'],
...
else
{
	...
	function DoNext(converted, last_id, maxMessage, maxMessagePerStep, sess)
	{
		var arParams = {
			'Convert': 'Y',
	...
	$intCountFormat = (int)$intCountFormat;
	if (0 >= $intCountFormat)
	{
		CAdminMessage::ShowMessage(array(
			"MESSAGE"=>GetMessage("CAT_DISC_CONVERT_COMPLETE"),
