...
while ($arRes = $dbResultList->Fetch())
{
	...
	$row->AddViewField("ID", '<a href="/thurly/admin/cat_group_edit.php?lang='.LANGUAGE_ID.'&ID='.$arRes["ID"].'&'.GetFilterParams("filter_").'">'.$arRes["ID"].'</a>');
	...
	{
		if ($arSelectFieldsMap['NAME'])
			$row->AddViewField("NAME", '<a href="/thurly/admin/cat_group_edit.php?lang='.LANGUAGE_ID.'&ID='.$arRes["ID"].'&'.GetFilterParams("filter_").'">'.htmlspecialcharsbx($arRes['NAME']).'</a>');
		if ($arSelectFieldsMap['SORT'])
	...
	$arActions[] = array(
		...
		"ACTION" => $lAdmin->ActionRedirect("/thurly/admin/cat_group_edit.php?ID=".$arRes['ID']."&lang=".LANGUAGE_ID."&".GetFilterParams("filter_").""),
		"DEFAULT" => true
	);
