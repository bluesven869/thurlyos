...
if($_REQUEST["OKEI"] == "Y")
{
	...
	if(!is_array($arMeasureClassifier))
		LocalRedirect("/thurly/admin/cat_measure_list.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false));
...
<script type="text/javascript">
	function makeMeasureTable()
	{
		...
		window['<?=$sTableID?>'].GetAdminList("/thurly/admin/cat_measure_edit.php?OKEI=Y&main_section=" + mainSectionId + "&sub_section=" + subSectionId + '&lang=<? echo LANGUAGE_ID;?>');
	}
</script>
<?
if($_SERVER["REQUEST_METHOD"] == "POST" && strlen($_REQUEST["Update"]) > 0 && !$bReadOnly && check_thurly_sessid())
{
	...
	if(strlen($errorMessage) == 0 && $ID > 0 && $res = CCatalogMeasure::update($ID, $arFields))
	{
		...
		if(strlen($_REQUEST["apply"])<=0)
			LocalRedirect("/thurly/admin/cat_measure_list.php?lang=".LANG."&".GetFilterParams("filter_", false));
		else
			LocalRedirect("/thurly/admin/cat_measure_edit.php?lang=".LANG."&ID=".$ID."&".GetFilterParams("filter_", false));
	}
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/cat_measure_list.php?lang=".LANG."&".GetFilterParams("filter_", false)
	)
);
...
if($ID > 0 && !$bReadOnly)
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_new",
		"LINK" => "/thurly/admin/cat_measure_edit.php?lang=".LANG."&".GetFilterParams("filter_", false)
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("CAT_MEASURE_DELETE"),
		"ICON" => "btn_delete",
		"LINK" => "javascript:if(confirm('".GetMessage("CAT_MEASURE_DELETE_CONFIRM")."')) window.location='/thurly/admin/cat_measure_list.php?action=delete&ID[]=".$ID."&lang=".LANG."&".thurly_sessid_get()."#tb';",
		"WARNING" => "Y"
	);
}
...
if(!$classifierMode)
{
	$tabControl->Buttons(
		array(
			"disabled" => $bReadOnly,
			"back_url" => "/thurly/admin/cat_measure_list.php?lang=".LANG."&".GetFilterParams("filter_", false)
		)
	);
}
$tabControl->End();
