...
if (!$bReadOnly && $_SERVER['REQUEST_METHOD']=="POST" && !empty($_POST['Update']) && check_thurly_sessid())
{
	$DB->StartTransaction();
	...
	else
	{
		...
		if (empty($_POST['apply']))
			LocalRedirect("/thurly/admin/cat_discount_coupon.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
		else
			LocalRedirect("/thurly/admin/cat_discount_coupon_edit.php?lang=".LANGUAGE_ID."&ID=".$ID.GetFilterParams("filter_", false));
	}
}
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/cat_discount_coupon.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false)
	)
);
...
if ($ID > 0 && !$bReadOnly)
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_new",
		"LINK" => "/thurly/admin/cat_discount_coupon_edit.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false)
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("CDEN_DELETE_DISCOUNT"),
		"ICON" => "btn_delete",
		"LINK" => "javascript:if(confirm('".GetMessage("CDEN_DELETE_DISCOUNT_CONFIRM")."')) window.location='/thurly/admin/cat_discount_coupon.php?action=delete&ID[]=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."#tb';",
		"WARNING" => "Y"
	);
}
...
$tabControl->Begin(array(
	"FORM_ACTION" => '/thurly/admin/cat_discount_coupon_edit.php?lang='.LANGUAGE_ID,
));
...
$tabControl->BeginNextFormTab();
	...
	else
	{
		...
		?><tr id="tr_DISCOUNT_ID" class="adm-detail-required-field">
			<td width="40%"><? echo $tabControl->GetCustomLabelHTML(); ?></td>
			<td width="60%">&nbsp;<a href="/thurly/admin/cat_discount_edit.php?lang=<? echo LANGUAGE_ID; ?>&return_url=<? echo urlencode($APPLICATION->GetCurPage()."?lang=".LANGUAGE_ID); ?>"><? echo GetMessage('DSC_ADD_DISCOUNT'); ?></a></td>
		</tr><?
		$tabControl->EndCustomField('DISCOUNT_ID');
	}
	...
	$arButtonsParams = array(
		"disabled" => $bReadOnly,
		"back_url" => "/thurly/admin/cat_discount_coupon.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false)
	);
	...
	if (!!obCouponValue && !!obCouponBtn)
	{
		BX.bind(obCouponBtn, 'click', function(){
			var url,
				data;
			BX.showWait();
			url = '/thurly/tools/catalog/generate_coupon.php';
			data = {
							...
							obCouponErr = obCouponCell.insertBefore(BX.create(
								'IMG',
								{
									props: {
										...
										src: '/thurly/panel/main/images_old/icon_warn.gif'
									},
									style: {
										marginRight: '10px',
