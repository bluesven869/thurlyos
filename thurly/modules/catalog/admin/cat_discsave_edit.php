...
$APPLICATION->AddHeadScript('/thurly/js/catalog/tbl_edit.js');
...
)
{
	...
	if (!$bVarsFromForm)
	{
		...
		{
			if (!empty($apply))
				LocalRedirect("/thurly/admin/cat_discsave_edit.php?ID=".$ID."&mess=ok&lang=".urlencode(LANGUAGE_ID)."&".$tabControl->ActiveTabParam());
			else
				LocalRedirect("/thurly/admin/cat_discsave_admin.php?lang=".urlencode(LANGUAGE_ID));
		}
		else
		{
			$bVarsFromForm = true;
...
$aMenu = array(
	array(
		...
		"LINK"=>"/thurly/admin/cat_discsave_admin.php?lang=".urlencode(LANGUAGE_ID),
		"ICON"=>"btn_list",
	)
);
...
if (!$bReadOnly)
{
	if($ID > 0)
	{
		...
		$aMenu[] = array(
			...
			"LINK"=>"/thurly/admin/cat_discsave_edit.php?lang=".urlencode(LANGUAGE_ID),
			"ICON"=>"btn_new",
		);
		if (!$boolCopy)
		{
			$aMenu[] = array(
				...
				"LINK"=>"/thurly/admin/cat_discsave_edit.php?ID=".$ID."&action=copy&lang=".urlencode(LANGUAGE_ID),
				"ICON"=>"btn_copy",
			);
			$aMenu[] = array(
				"TEXT"=>GetMessage("BT_CAT_DISC_SAVE_EDIT_CONT_NAME_DELETE"),
				"TITLE"=>GetMessage("BT_CAT_DISC_SAVE_EDIT_CONT_TITLE_DELETE"),
				"LINK"=>"javascript:if(confirm('".GetMessage("BT_CAT_DISC_SAVE_EDIT_CONT_CONF_DELETE")."'))window.location='/thurly/admin/cat_discsave_admin.php?ID=".$ID."&action=delete&lang=".urlencode(LANGUAGE_ID)."&".thurly_sessid_get()."';",
				"ICON"=>"btn_delete",
			);
		}
	}
...
$tabControl->Begin(array(
	"FORM_ACTION" => '/thurly/admin/cat_discsave_edit.php?lang='.urlencode(LANGUAGE_ID),
));
...
$arButtonsParams = array(
	'disabled' => $bReadOnly,
	'back_url' => '/thurly/admin/cat_discsave_admin.php?lang='.urlencode(LANGUAGE_ID)
);
