...
if ($_SERVER['REQUEST_METHOD'] == "POST" && strlen($Update) > 0 && !$bReadOnly && check_thurly_sessid())
{
	$arFields = array(
		"NAME" => $NAME,
	...
	{
		if (empty($apply))
			LocalRedirect("/thurly/admin/cat_extra.php?lang=".LANGUAGE_ID);
		else
			LocalRedirect("/thurly/admin/cat_extra_edit.php?lang=".LANGUAGE_ID."&ID=".$ID);
	}
	else
	{
		$bVarsFromForm = true;
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/cat_extra.php?lang=".LANGUAGE_ID
	)
);
...
if ($ID > 0 && !$bReadOnly)
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_new",
		"LINK" => "/thurly/admin/cat_extra_edit.php?lang=".LANGUAGE_ID
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("CEEN_DELETE_DISCOUNT"),
		"ICON" => "btn_delete",
		"LINK" => "javascript:if(confirm('".GetMessageJS("CEEN_DELETE_DISCOUNT_CONFIRM")."')) window.location='/thurly/admin/cat_extra.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."#tb';",
		"WARNING" => "Y"
	);
}
...
$tabControl->Buttons(
	array(
		"disabled" => $bReadOnly,
		"back_url" => "/thurly/admin/cat_extra.php?lang=".LANGUAGE_ID
	)
);
