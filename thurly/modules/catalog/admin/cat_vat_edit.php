...
if ('POST' == $_SERVER['REQUEST_METHOD'] && strlen($Update)>0 && !$bReadOnly && check_thurly_sessid())
{
	$DB->StartTransaction();
	...
	if ($res)
	{
		$DB->Commit();
		if (strlen($apply)<=0)
			LocalRedirect("/thurly/admin/cat_vat_admin.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false));
		else
			LocalRedirect("/thurly/admin/cat_vat_edit.php?lang=".LANGUAGE_ID."&ID=".$ID."&".GetFilterParams("filter_", false));
	}
	else
	{
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/cat_vat_admin.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false)
	)
);
...
if ($ID > 0 && !$bReadOnly)
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_new",
		"LINK" => "/thurly/admin/cat_vat_edit.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false)
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("CVAT_DELETE"),
		"ICON" => "btn_delete",
		"LINK" => "javascript:if(confirm('".GetMessageJS("CVAT_DELETE_CONFIRM")."')) window.location='/thurly/admin/cat_vat_admin.php?action=delete&ID[]=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."#tb';",
		"WARNING" => "Y"
	);
}
...
$tabControl->Buttons(
	array(
		"disabled" => $bReadOnly,
		"back_url" => "/thurly/admin/cat_vat_admin.php?lang=".LANGUAGE_ID."&".GetFilterParams("filter_", false)
	)
);
$tabControl->End();
?>
