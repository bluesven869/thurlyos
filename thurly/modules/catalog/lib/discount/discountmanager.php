<?php
namespace Thurly\Catalog\Discount;

use Thurly\Main,
	Thurly\Main\Localization\Loc,
	Thurly\Main\Loader,
	Thurly\Main\Type\Collection,
	Thurly\Catalog,
	Thurly\Iblock,
	Thurly\Sale;
...
class DiscountManager
{
	...
	public static function getEditUrl($discount)
	{
		if (empty(self::$editUrlTemplate))
		{
			self::$editUrlTemplate = array(
				Catalog\DiscountTable::TYPE_DISCOUNT => '/thurly/admin/cat_discount_edit.php?lang='.LANGUAGE_ID.'&ID=',
				Catalog\DiscountTable::TYPE_DISCOUNT_SAVE => '/thurly/admin/cat_discsave_edit.php?lang='.LANGUAGE_ID.'&ID='
			);
		}
		$result = '';
	...
	public static function calculateApplyCoupons(array $couponsList, array $basket, array $params)
	{
		...
		if (empty($basket))
			return $result;
		$filteredBasket = array_filter($basket, '\Thurly\Catalog\Discount\DiscountManager::basketFilter');
		if (empty($filteredBasket))
			return $result;
		$filteredBasket = array_filter($filteredBasket, '\Thurly\Catalog\Discount\DiscountManager::lastDiscountFilter');
		if (empty($filteredBasket))
			return $result;
	...
	)
	{
		...
		$result = array();
		$basket = array_filter($basket, '\Thurly\Catalog\Discount\DiscountManager::basketFilter');
		if (!empty($basket))
		{
			$priceTypes = array();
			$loadPriceId = array();
	...
	public static function extendOrderData(Main\Event $event)
	{
		...
		if ($process)
		{
			...
			$basket = array_filter($orderData['BASKET_ITEMS'], '\Thurly\Catalog\Discount\DiscountManager::basketFilter');
			if (!empty($basket))
			{
				foreach ($basket as $basketCode => $basketItem)
				{
	...
	protected static function createSaleAction(&$discount, $params)
	{
		...
		$action = '\Thurly\Catalog\Discount\DiscountManager::applyDiscount('.$params['BASKET_ITEM'].', '.var_export($data, true).');';
