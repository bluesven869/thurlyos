...
//			"AVAILABLE_QUANTITY" => $arCatalogProduct["QUANTITY"],
	...
	public static function DeductProduct($arParams)
	{
		...
		{
			if ($arParams["BASKET_ITEM"] instanceof \Thurly\Sale\BasketItem)
			{
	...
	/**
	 * @param \Thurly\Sale\BasketItem $basketItem
	 * @param string $reserved
	 * @param array $basketStoreData
	 * @param null $quantity
	 * @return \Thurly\Sale\Result
	 */
	public static function tryShipmentProduct(\Thurly\Sale\BasketItem $basketItem, $reserved = 'N', array $basketStoreData = array(), $quantity = null)
	{
		...
		if (empty($arProduct))
		{
			$result->addError( new \Thurly\Sale\ResultError(Loc::getMessage("DDCT_DEDUCTION_PRODUCT_NOT_FOUND_ERROR", self::GetProductCatalogInfo($productId)), "DDCT_DEDUCTION_PRODUCT_NOT_FOUND_ERROR") );
			return $result;
		}
		...
		if ($strUseStoreControl == "Y")
		{
			...
			if (!empty($basketStoreData))
			{
				...
				foreach ($basketStoreData as $storeId => $basketStore)
				{
					...
					{
						$result->addError( new \Thurly\Sale\ResultError(Loc::getMessage("DDCT_DEDUCTION_STORE_ERROR", self::GetProductCatalogInfo($productId)), "DDCT_DEDUCTION_STORE_ERROR") );
					...
					else
					{
						...
						if ($arProp = $rsProps->Fetch())
						{
							...
							{
								$result->addError( new \Thurly\Sale\ResultError(
									Loc::getMessage(
										"DDCT_DEDUCTION_QUANTITY_STORE_ERROR",
...
//				$arRes["RESULT"] = true;
						...
						foreach ($arSet["ITEMS"] as $k1 => $item)
						{
							if ($item["ITEM_ID"] == $arProduct["ID"])
							{
								...
								if (!empty($proxyCatalogSkuData[$item["ITEM_ID"]]) && strpos($arProduct["XML_ID"], '#') === false)
								{
									...
									{
										$parentIterator = \Thurly\Iblock\ElementTable::getList(array(
	...
	protected static function getProductPoolQuantityByBasketItem(\Thurly\Sale\BasketItem $basketItem)
	{
		...
		return \Thurly\Sale\Provider::getReservationPoolItem($order->getInternalId(), $basketItem);
	}
	 ...
	 */
	protected static function canProductAutoShip(\Thurly\Sale\BasketItem $basketItem)
	{
	 ...
	 * @return array|bool
	 */
	protected static function getProductStoreData(\Thurly\Sale\BasketItem $basketItem, $quantity)
	{
	 ...
	 * @return array|bool
	 */
	protected static function getProductOneStoreData(\Thurly\Sale\BasketItem $basketItem, $quantity)
	{
		$productStoreData = array();
