...
class CAllCatalogDiscount
{
	...
	private static function getDiscountsFromApplyResult(array $calcResults, \Thurly\Sale\BasketItem $basketItem)
	{
	...
	private static function getReformattedDiscounts(array $finalDiscountList, array $calcResults, $siteId, $isRenewal = false)
	{
		...
		foreach($finalDiscountList as $discount)
		{
			...
			{
				$actionConfiguration = \Thurly\Sale\Discount\Actions::getActionConfiguration($discount);
			}

			if(!$actionConfiguration || $actionConfiguration['VALUE_TYPE'] === \Thurly\Sale\Discount\Actions::VALUE_TYPE_SUMM)
			{
				continue;
			}
			...
			if($discount['USE_COUPONS'] === 'Y')
			{
				foreach($calcResults['COUPON_LIST'] as $coupon)
				{
					...
					if($coupon['TYPE'] == \Thurly\Sale\Internals\DiscountCouponTable::TYPE_BASKET_ROW)
					{
						$reformattedDiscount['COUPON_ONE_TIME'] = 'Y';
					}
					elseif($coupon['TYPE'] == \Thurly\Sale\Internals\DiscountCouponTable::TYPE_ONE_ORDER)
					{
						$reformattedDiscount['COUPON_ONE_TIME'] = 'O';
					}
					elseif($coupon['TYPE'] == \Thurly\Sale\Internals\DiscountCouponTable::TYPE_MULTI_ORDER)
					{
	...
	public static function GetDiscount($intProductID, $intIBlockID, $arCatalogGroups = array(), $arUserGroups = array(), $strRenewal = "N", $siteID = false, $arDiscountCoupons = false, $boolSKU = true, $boolGetIDS = false)
	{
		...
		if (self::$useSaleDiscount && Loader::includeModule('sale'))
		{
			...
			if ($arCatalogGroups !== array(-1))
			{
				...
				{
					$priceRow = \Thurly\Catalog\Discount\DiscountManager::getPriceDataByProductId($intProductID, $catalogGroup);
					if (!$priceRow)
						continue;
			...
			if (!empty($arDiscountIDs))
			{
				...
				if ($arDiscountCoupons === false)
				{
					if (self::$existCouponsManager && Loader::includeModule('sale'))
					{
						...
						{
							$orderCoupons = array_filter($arDiscountCoupons, '\Thurly\Sale\DiscountCouponsManager::filterOrderCoupons');
	...
	public static function ExtendBasketItems(&$arBasket, $arExtend)
	{
		...
		if ($boolFields || $boolProps || $boolPrice)
		{
			...
			if($boolPrice && empty($productPriceIds) && $basketItemIds)
			{
				...
				$res = \Thurly\Sale\Basket::getList($basketFilter);
			...
			if (!empty($arIDS))
			{
				...
				if($boolPrice)
				{
					$priceList = \Thurly\Catalog\PriceTable::getList(array(
						'select' => array(
							'PRODUCT_ID',
							'CATALOG_GROUP_ID',
						),
