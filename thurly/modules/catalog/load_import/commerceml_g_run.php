...
if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/php_interface/include/1c_mutator.php"))
	include($_SERVER["DOCUMENT_ROOT"]."/thurly/php_interface/include/1c_mutator.php");
	...
	switch ($name)
	{
		case $nameUTF["Catalog"]:
			...
			if ($arIBlock = $dbIBlockList->Fetch())
			{
				...
				\Thurly\Iblock\PropertyIndex\Manager::markAsInvalid($arIBlock["ID"]);
			}
			elseif ($USER->IsAdmin())
			{
				$bUpdate = false;
