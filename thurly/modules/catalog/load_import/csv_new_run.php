...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/catalog/import_setup_templ.php');
		...
		do
		{
			...
			if ('' == $strErrorR)
			{
				...
				if ($previousProductId != $PRODUCT_ID)
				{
					...
					$ipropValues = new \Thurly\Iblock\InheritedProperty\ElementValues($IBLOCK_ID, $previousProductId);
					...
					if ($updateFacet)
					{
						...
						\Thurly\Iblock\PropertyIndex\Manager::updateElementIndex($IBLOCK_ID, $previousProductId);
					}
					$updateFacet = false;
					$previousProductId = $PRODUCT_ID;
				}
	...
	if ($PRODUCT_ID > 0)
	{
		...
		if ($updateFacet)
		{
			if (isset($newProducts[$PRODUCT_ID]))
				CCatalogSKU::ClearCache();
			\Thurly\Iblock\PropertyIndex\Manager::updateElementIndex($IBLOCK_ID, $PRODUCT_ID);
		}
		$updateFacet = false;
	}
