...
if ($_SERVER['REQUEST_METHOD'] == 'GET' && !empty($_REQUEST['RestoreDefaults']) && !$bReadOnly && check_thurly_sessid())
{
	$strValTmp = '';
	if (!$USER->IsAdmin())
...
$arAllOptions = array(
	array("export_default_path", Loc::getMessage("CAT_EXPORT_DEFAULT_PATH"), "/thurly/catalog_export/", array("text", 30)),
...
$strWarning = "";
$strOK = "";
if ($_SERVER['REQUEST_METHOD'] == "POST" && !empty($_POST['Update']) && !$bReadOnly && check_thurly_sessid())
{
	...
	{
		CAgent::RemoveAgent('\Thurly\Catalog\CatalogViewedProductTable::clearAgent();', 'catalog');
		CAgent::AddAgent('\Thurly\Catalog\CatalogViewedProductTable::clearAgent();', 'catalog', 'N', (int)Option::get('catalog', 'viewed_period') * 24 * 3600);
	}
	...
	if ($oldSimpleSearch != $newSimpleSearch)
	{
		if ($newSimpleSearch == 'Y')
			UnRegisterModuleDependences('search', 'BeforeIndex', 'catalog', '\Thurly\Catalog\Product\Search', 'onBeforeIndex');
		else
			RegisterModuleDependences('search', 'BeforeIndex', 'catalog', '\Thurly\Catalog\Product\Search', 'onBeforeIndex');
	}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['agent_start']) && !$bReadOnly && check_thurly_sessid())
{
	CAgent::RemoveAgent('CCatalog::PreGenerateXML("yandex");', 'catalog');
...
function RestoreDefaults()
{
	if (confirm('<?echo CUtil::JSEscape(Loc::getMessage("CAT_OPTIONS_BTN_HINT_RESTORE_DEFAULT_WARNING"));?>'))
		window.location = "<? echo $APPLICATION->GetCurPage(); ?>?RestoreDefaults=Y&lang=<? echo LANGUAGE_ID; ?>&mid=<? echo urlencode($mid); ?>&<? echo thurly_sessid_get(); ?>";
}
</script>
...
while ($arIBlock = $rsIBlocks->Fetch())
{
	...
	if (!isset($arIBlockSitesList[$arIBlock['ID']]))
	{
		...
		while ($arIBlockSite = $rsIBlockSites->Fetch())
		{
			...
			$arWithLinks[] = '<a href="/thurly/admin/site_edit.php?LID='.urlencode($arIBlockSite['LID']).'&lang='.LANGUAGE_ID.'" title="'.Loc::getMessage("CO_SITE_ALT").'">'.htmlspecialcharsbx($arIBlockSite["LID"]).'</a>';
...
$readOnly = false;
require($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/catalog/tools/iblock_catalog_list.php'); */
...
<table width="100%" cellspacing="0" cellpadding="0" border="0" class="internal">
	...
	foreach ($arIBlockFullInfo as &$res)
	{
		?>
		<tr>
			<td>[<a title="<? echo Loc::getMessage("CO_IB_TYPE_ALT"); ?>" href="/thurly/admin/iblock_admin.php?type=<? echo urlencode($res["IBLOCK_TYPE_ID"]); ?>&lang=<? echo LANGUAGE_ID; ?>&admin=Y"><? echo $res["IBLOCK_TYPE_ID"]; ?></a>]
...
if ($USER->IsAdmin())
{
	...
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/admin/group_rights2.php");
...
<script type="text/javascript">
	function catClearQuantity(el, action)
	{
		...
		BX.ajax.post(
			'/thurly/admin/cat_quantity_control.php?lang=<? echo LANGUAGE_ID; ?>',
			dateURL,
...
?><a href="/thurly/admin/event_log.php?lang=<? echo LANGUAGE_ID; ?>&set_filter=Y<? echo CCatalogEvent::GetYandexAgentFilter(); ?>"><? echo Loc::getMessage('CAT_AGENT_EVENT_LOG_SHOW_ERROR')?></a>
</td></tr>
<?
$systemTabControl->BeginNextTab();
?><tr><td align="left"><?
	...
	if ($strUseStoreControl === 'N' && $catalogCount > 0)
	{
		...
		if (!empty($userListID))
		{
			...
			while ($arOneUser = $userIterator->fetch())
			{
				...
				if ($canViewUserList)
					$arUserList[$arOneUser['ID']] = '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$arOneUser['ID'].'">'.CUser::FormatName($strNameFormat, $arOneUser).'</a>';
				else
...
function showDiscountReindex()
{
	...
	obDiscount = new BX.CAdminDialog({
		'content_url': '/thurly/admin/cat_discount_convert.php?lang=<? echo LANGUAGE_ID; ?>&format=Y',
		'content_post': params,
		'draggable': true,
		'resizable': true,
		'buttons': [obBtn]
...
function showSetsAvailableReindex()
{
	...
	obWindow = new BX.CAdminDialog({
		'content_url': '/thurly/tools/catalog/sets_available.php?lang=<? echo LANGUAGE_ID; ?>',
		'content_post': params,
		'draggable': true,
		'resizable': true,
		'buttons': [obBtn]
...
function showCatalogReindex()
{
	...
	obWindow = new BX.CAdminDialog({
		'content_url': '/thurly/tools/catalog/catalog_reindex.php?lang=<? echo LANGUAGE_ID; ?>',
		'content_post': params,
		'draggable': true,
		'resizable': true,
		'buttons': [obBtn]
...
function showProductSettings()
{
	...
	obWindow = new BX.CAdminDialog({
		'content_url': '/thurly/tools/catalog/product_settings.php?lang=<? echo LANGUAGE_ID; ?>',
		'content_post': params,
		'draggable': true,
		'resizable': true,
		'buttons': [obBtn]
