...
function BXComponents2Taskbar()
{
	...
	BXComponents2Taskbar.prototype.FetchArray = function (clear_cache)
	{
		...
		if (window.arComp2Elements)
			...
			CHttpRequest.Send('/thurly/admin/fileman_load_components2.php?lang='+BXLang+'&site='+BXSite+'&load_tree=Y'+(clear_cache === true ? '&clear_comp2_cache=Y' : ''));
	}
	...
	BXComponents2Taskbar.prototype.LoadComp2Params = function (arProps, calbackFunc, calbackObj, calbackParams, method, data)
	{
		...
		var url = '/thurly/admin/fileman_load_comp2_params.php?lang=' + BXLang + '&site=' + BXSite + '&cname=' + elementName+'&stid='+((this.pMainObj.templateID) ? this.pMainObj.templateID : '')+"&loadhelp="+loadHelp+'&tname=' + templName + '&bxsender=fileman_html_editor&bxeditor=' + this.pMainObj.name;
	...
	BXComponents2Taskbar.prototype.loadTemplateParams = function(componentName, templateName, calbackFunc, calbackObj, calbackParams, method, data)
	{
		...
		if (method == 'POST' && data)
			_CHttpRequest.Post('/thurly/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite + '&cname='+componentName+'&tname='+templateName+'&mode=params&stid='+((this.pMainObj.templateID) ? this.pMainObj.templateID : ''),data);
		else
			_CHttpRequest.Send('/thurly/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite+'&cname='+componentName+'&tname='+templateName+'&mode=params&stid='+((this.pMainObj.templateID) ? this.pMainObj.templateID : ''));
	}
...
C2Parser.prototype =
{
	...
	GetRenderedContent : function(P)
	{
		...
		var
			url = '/thurly/admin/fileman_comp2_render.php?sessid=' + BX.thurly_sessid() + '&site=' + BXSite,
	...
	if (_oFunc.name.toUpperCase() == '$APPLICATION->INCLUDECOMPONENT')
	{
		...
		//try{
			var
				...
				icon = (comProps.icon) ? comProps.icon : '/thurly/images/fileman/htmledit2/component.gif',
...
function loadComp2TemplateLists(siteTemplate,calbackFunc, calbackParams,method,data)
{
	...
	if (method == 'POST' && data)
		_CHttpRequest.Post('/thurly/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite+'&mode=list&stid='+siteTemplate,data);
	else
		_CHttpRequest.Send('/thurly/admin/fileman_load_templates.php?lang='+BXLang+'&site='+BXSite+'&mode=list&stid='+siteTemplate);
}
