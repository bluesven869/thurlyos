...
class CThurlyPlayer extends CThurlyComponent
{
	...
	protected function findCorrectFile($path, &$type = null, $warning = false)
	{
		...
		$uri = new \Thurly\Main\Web\Uri($path);
		if ($rewriteCondition === '')
		{
			if(\Thurly\Main\Loader::includeModule('disk') && method_exists(\Thurly\Disk\Driver::getInstance()->getUrlManager(), 'getUrlToDownloadByExternalLink'))
			{
				$extLinksAccessPoints = \CUrlRewriter::GetList(array('ID' => 'thurly:disk.external.link'));
		...
		if ($rewriteCondition)
		{
			...
			if ($hash != '')
			{
				...
				$ext = \Thurly\Disk\ExternalLink::load(array(
					'HASH' => $hash,
				));
				if ($ext)
				{
		...
		else // relative path
		{
			...
			{
				if(\Thurly\Main\Loader::includeModule('clouds'))
				{
	...
	{
		require_once($_SERVER['DOCUMENT_ROOT']."/thurly/modules/main/classes/general/xml.php");
		$path = Rel2Abs("/", $path);
	...
	public function executeComponent()
	{
		...
		else
		{
			...
			\Thurly\Main\Page\Asset::getInstance()->addJs($this->__path.'/videojs/video.js');
			...
			\Thurly\Main\Page\Asset::getInstance()->addJs($this->__path.'/videojs/videojs-playlist-dev.js');
			...
			\Thurly\Main\Page\Asset::getInstance()->addJs($this->__path.'/js/fileman_player.js');
			...
			if ($this->arResult['VIMEO'])
			{
				...
				\Thurly\Main\Page\Asset::getInstance()->addJs($this->__path.'/videojs/vimeo.js');
			...
			if ($this->arResult['YOUTUBE'])
			{
				...
				\Thurly\Main\Page\Asset::getInstance()->addJs($this->__path.'/videojs/youtube.js');
			...
			if ($this->arResult['STREAM'])
			{
				...
				\Thurly\Main\Page\Asset::getInstance()->addJs($this->__path.'/videojs/videojs-contrib-hls.js');
			...
			if ($this->arResult['WMV'] && count ($this->arResult['TRACKS']) > 1)
			{
				...
				\Thurly\Main\Page\Asset::getInstance()->addJs($this->__path.'/wmvplayer/silverlight.js');
				\Thurly\Main\Page\Asset::getInstance()->addJs($this->__path.'/wmvplayer/wmvplayer.js');
				\Thurly\Main\Page\Asset::getInstance()->addJs($this->__path.'/videojs/wmv.js');
	...
	{
		\Thurly\Main\Page\Asset::getInstance()->addJs($this->__path . '/mediaplayer/jwplayer.js');
		...
		if($this->arParams['SKIN'] != '' && $this->arParams['SKIN'] != 'default')
		{
			...
			if ($skinExt == 'css')
			{
				...
				if ($this->arResult['SKIN_CSS'] != '')
				{
					...
					{
						\Thurly\Main\Page\Asset::getInstance()->addJs($this->arResult['SKIN_JS']);
