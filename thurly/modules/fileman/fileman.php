...
// AddEventHandler("fileman", "OnBeforeHTMLEditorScriptsGet", "thurly_tabs");
// function thurly_tabs($editorName, $arEditorParams){return array("JS" => array('thurly_tabs.js'));}
...
class CFileMan
{
	public static function OnPanelCreate()
	{
		...
		{
			// New page
			$href = "/thurly/admin/fileman_".$editor_type."_edit.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&path=".UrlEncode($APPLICATION->GetCurDir())."&new=Y&templateID=".urlencode(SITE_TEMPLATE_ID)."&back_url=".UrlEncode($REQUEST_URI);
			...
			//New folder
			$href = "/thurly/admin/fileman_newfolder.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&path=". UrlEncode($APPLICATION->GetCurDir())."&back_url=".UrlEncode($REQUEST_URI);
		...
		{
			$href = "/thurly/admin/fileman_".$editor_type."_edit.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&templateID=".urlencode(SITE_TEMPLATE_ID).$full_src."&path=".UrlEncode(isset($_SERVER["REAL_FILE_PATH"]) && $_SERVER["REAL_FILE_PATH"]<>""? $_SERVER["REAL_FILE_PATH"] : $cur_page)."&back_url=".UrlEncode($REQUEST_URI);
		...
		if ($sect_permission>="W")
		{
			$href = "/thurly/admin/fileman_folder.php?lang=".LANGUAGE_ID."&site=".SITE_ID."&path=".UrlEncode($APPLICATION->GetCurDir())."&back_url=".UrlEncode($REQUEST_URI);
		...
		//Sticker buttons
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/fileman/classes/general/sticker.php");
	...
	{
		@include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/fileman/install/version.php");
			...
			BX.ready(
				function()
				{
					window.changeType_<?= $name?> = function(bSave)
					{
						...
						if (bSave !== false)
						{
							BX.ajax.get('/thurly/admin/fileman_manage_settings.php?<?= thurly_sessid_get()?>&target=text_type&edname=<?= $name?>&key=<?= $key?>&type=' + curType);
						}
						<?endif;?>
					};
	...
	public static function ShowHTMLEditControl($name, $content, $arParams = Array())
	{
		...
		if(!$bFirstUsed)
		{
			...
			for($i = 0, $c = count($arJS); $i < $c; $i++)
			{
				...
				if(file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/admin/htmleditor2/'.$arJS[$i]))
					$arr[] = $arJS[$i];
			}
			?>
			<script type="text/javascript" src="/thurly/admin/fileman_js.php?lang=<?=LANGUAGE_ID?>&v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/fileman/lang/'.LANGUAGE_ID.'/admin/fileman_js.php')?>"></script>
			<script type="text/javascript" src="/thurly/admin/fileman_common_js.php?s=<?=$str_taskbars?>"></script>
			<?
			for($i = 0, $l = count($arr); $i < $l; $i++)
			{
				$script_filename = $arr[$i];
				?><script type="text/javascript" src="/thurly/admin/htmleditor2/<?=$script_filename?>?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/thurly/admin/htmleditor2/'.$script_filename)?>"></script><?
			}
			?>
			<script type="text/javascript" src="/thurly/js/main/popup_menu.js?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/thurly/js/main/popup_menu.js')?>"></script>
			<?
			for($i = 0, $l = count($arCSS); $i < $l; $i++) // Additional CSS files from event OnBeforeHtmlEditorScriptGet
			{
				...
				if(!file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/admin/htmleditor2/'.$arCSS[$i]))
					continue;
				?><link rel="stylesheet" type="text/css" href="/thurly/admin/htmleditor2/<?=$arCSS[$i]?>?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/thurly/admin/htmleditor2/'.$arCSS[$i])?>"/><?
			}
						...
						<table class="dim100x100">
							<tr>
								...
								<td id="<?= $name?>_taskBarSet2" class="bxedtaskbarset" style="width:0%; display: none;">
									<table>
										<tr><td class="bx-move-col-v" rowSpan="3"><img src="/thurly/images/1.gif" /></td><td style="height:26px;"></td></tr>
							...
							<tr style="height:0%; display: none;">
								<td id="<?= $name?>_taskBarSet3" colspan="2">
									<table>
										<tr><td class="bx-move-col-h"><img src="/thurly/images/1.gif" /></td></tr>
		...
		<script>
			BX.loadCSS('/thurly/admin/htmleditor2/editor.css');
	...
	public static function ParseFileContent($filesrc, $bCheckProlog = false)
	{
		...
		if ($bCheckProlog)
		{
			...
			if (strlen($prolog) > 0 &&
				...
				strpos($prolog, 'thurly/header.php') === false &&
			...
			if (strlen($epilog) > 0 &&
				strpos($epilog, 'thurly/footer.php') === false &&
