...
BXNode.prototype.GetEditorCode = function()
{
	...
	else if(this.type == 'element')
	{
		...
		{
			//anchor
			res = '<img src="/thurly/images/fileman/htmledit2/anchor.gif" width="20" height="20" __bxtagname="anchor" __bxcontainer="'+bxhtmlspecialchars(BXSerialize(this.arAttributes))+'" />';
		}
		else
		{
			res = "<" + this.text;
	...
	if(arScripts.length>0)
	{
		...
		for(i=0; i<arScripts.length; i++)
		{
			...
			if((arRes = __CheckForComponent(arScript[2]))
				...
				newstr += str.substr(plast, arScript[0]-plast) + '<img src="/thurly/images/fileman/htmledit2/php.gif" border="0" __bxtagname="php" __bxcontainer="' + bxhtmlspecialchars(BXSerialize({'code':arScript[2]})) + '" />';
			plast = arScript[1];
		}
...
function BXParseContent(pMainObj, arParams)
{
	...
	for(var i=0; i<allLinks.length; i++)
	{
		...
		{
			pImg = pMainObj.createEditorElement('IMG', {'src': '/thurly/images/fileman/htmledit2/anchor.gif', 'width': '20', 'height': '20', '__bxtagname': 'anchor', '__bxcontainer': BXSerialize({'name':pNode.getAttribute("name", 2)})});
