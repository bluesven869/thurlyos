...
function BXButtonSeparator()
{
	BXButtonSeparator.prototype._Create = function ()
	{
		...
		this.pWnd = this.CreateElement("IMG", {'src': '/thurly/images/fileman/htmledit2/separator.gif', 'width': '2', 'height': '20'});
	...
	BXButtonSeparator.prototype.onToolbarChangeDirection = function (bVertical)
	{
		if(bVertical)
		{
			this.pWnd.src='/thurly/images/fileman/htmledit2/separator-v.gif';
			this.pWnd.style.width = "20px";
			this.pWnd.style.height = "2px";
		}
		else
		{
			this.pWnd.src='/thurly/images/fileman/htmledit2/separator.gif';
...
function _BXColorPicker()
{
	_BXColorPicker.prototype._Create = function ()
	{
		...
		this.pIcon = this.pMainObj.CreateElement("IMG", {'src': (this.icon?this.icon:'/thurly/images/fileman/htmledit2/bgcolor.gif'), 'width': '20', 'height': '20', 'alt': this.title});
		...
		for(i=0; i<BXConst.arColor.length/iColumnCount; i++)
		{
			...
			for(j=0; j<iColumnCount; j++)
			{
				...
				c1.innerHTML = '<img src="/thurly/images/1.gif" width="1" height="1"/>';
...
function _BXTAlignPicker()
{
	...
	_BXTAlignPicker.prototype._Create = function ()
	{
		...
		this.pIcon = this.pMainObj.CreateElement("IMG", {'src': '/thurly/images/fileman/htmledit2/talign-tl.gif', 'width': '20', 'height': '20', 'alt': this.title});
		this.pIcon.className = 'bxedtbutton';
		cell.appendChild(this.pIcon);
		addCustomElementEvent(this.pIcon, 'mouseover', this.onMouseOver, this);
		addCustomElementEvent(this.pIcon, 'mouseout', this.onMouseOut, this);
		...
		for(i=0; i<3; i++)
		{
			...
			for(j=0; j<3; j++)
			{
				...
				{
					c1 = c1.appendChild(BXPopupWindow.CreateElement("IMG", {"src": "/thurly/images/fileman/htmledit2/1.gif", "border": 0, "alt": "", "title": ""}));
					c1.className = 'bxedtbutton';
				}
				else
				{
					c1 = c1.appendChild(BXPopupWindow.CreateElement("IMG", {"src": "/thurly/images/fileman/htmledit2/talign-"+this.arIcon[i*3+j]+".gif", "border": 0, "alt": this.arIconName[i*3 + j], "title": this.arIconName[i*3 + j]}));
					c1.className = 'bxedtbutton';
					if(this.type == 'image')
					{
						if(j==1)
	...
	_BXTAlignPicker.prototype.SetValue = function(valH, valV)
	{
		...
		this.pIcon.src = "/thurly/images/fileman/htmledit2/talign-"+this.arIcon[i*3+j]+".gif";
...
function BXCombo(pMainObj, id, pHandler)
{
	...
	BXCombo.prototype.Show = function()
	{
		...
		c2.appendChild(this.pMainObj.CreateElement('IMG', {'src':'/thurly/images/fileman/htmledit2/combo_arr.gif', 'border':'0', 'width':'11', 'height':'17'}));
...
function BXDialog()
{
	BXDialog.prototype._Create = function ()
	{
		...
		if(window.showModalDialog)
			window.showModalDialog('/thurly/admin/fileman_editor_dialog.php?lang='+BXLang+'&site='+BXSite+'&name='+this.name+'&not_use_default='+this.not_use_default, this.params, "dialogWidth:"+this.width+"px;dialogHeight:"+this.height+"px;help:no;scroll:no;status:no;center:yes;");
		else
		{
			this.pWnd = window.open('/thurly/admin/fileman_editor_dialog.php?lang='+BXLang+'&site='+BXSite+'&name='+this.name+'&not_use_default='+this.not_use_default, 'BXDialog'+Math.random.toString().substring(2), 'height='+this.height+',width='+this.width+',toolbar=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,modal=yes,alwaysRaised=yes,dialog=yes');
