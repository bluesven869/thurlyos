...
{
	var sURL = '/thurly/admin/fileman_get_xml.php?op=sitetemplateparams&lang='+BXLang+'&site='+BXSite+'&templateID='+templateID;
...
return;
	...
	*/
	...
	if(this.pEditorDocument.selection)
	{
		...
		this.executeCommand("FontName", "thurlytemp");
		...
		{
			if(arB[i].face && arB[i].face=='thurlytemp')
			{
	...
	BXContextMenu.prototype._Create = function ()
	{
		...
		for(i=0; i<arMenuItems.length; i++)
		{
			if(arMenuItems[i] == 'separator')
			{
				...
				c.innerHTML = '<img src="/thurly/images/1.gif" width="1" height="1"/>';
			}
			else
			{
				r = t.insertRow(-1);
