...
$APPLICATION->AddHeadScript('/thurly/js/fileman/medialib/core_admin.js');
...
if (isset($_REQUEST['type']) && intVal($_REQUEST['type']) > 0 ) // && check_thurly_sessid() http://jabber.bx/view.php?id=28997 commit
{
...
if ($bCols && $exParams['arCountPerm']['new_item'] > 0)
{
	...
	$aContext[] = Array(
		...
		"ICON" => "btn_mass_upl",
		"LINK" => "fileman_medialib_upload.php?lang=".LANGUAGE_ID."&type=".$curType['id']."&".thurly_sessid_get(),
...
if (($bCols && $exParams['arCountPerm']['access'] > 0) || CMedialib::CanDoOperation('medialib_access', 0))
{
	$aContext[] = Array(
		...
		//"ICON" => "btn_access",
		"LINK" => "fileman_medialib_access.php?lang=".LANGUAGE_ID."&".thurly_sessid_get(),
...
if ($USER->CanDoOperation('fileman_view_all_settings'))
{
	$aContext[] = Array(
		...
		"LINK" => "/thurly/admin/settings.php?mid=fileman&tabControl_active_tab=edit5&lang=".LANGUAGE_ID."&".thurly_sessid_get(),
...
BX.ready(function()
	{
		BX.loadScript([
			"/thurly/js/fileman/medialib/common.js?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/thurly/js/fileman/medialib/common.js')?>",
			"/thurly/js/fileman/medialib/core_admin.js?v=<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/thurly/js/fileman/medialib/core_admin.js')?>"
		],
			...
			if (btn_mass_upload)
				btn_mass_upload.onclick = function()
				{
					...
					if (!col_id || !window.oBXMLAdmin.oCollections[col_id])
						col_id = '';
					window.location = "fileman_medialib_upload.php?lang=<?= LANGUAGE_ID ?>&type=<?= $curType['id'] ?>&<?= thurly_sessid_get() ?>&col_id=" + col_id;
					return false;
				};
		}, 50);}
		);
...
<div class="ml-cont">
	...
	<div class="ml-search-res-cont" id="ml_search_res_cont_par">
		<div class="ml-coll-title mlcolllevel-0" title="<?= GetMessage('ML_SEARCH_RESULT')?>">
			<img class="ml-col-icon ml-col-icon-closed" src="/thurly/images/1.gif" id="ml_srch_res_flip"/>
