...
if (!check_thurly_sessid())
	die('<!--BX_ML_DUBLICATE_ACTION_REQUEST'.thurly_sessid().'-->');
...
elseif ($action == 'edit_collection')
{
	...
	$id = CMedialib::EditCollection(array(
		'id' => intVal($_POST['id']),
		'name' => \Thurly\Main\Text\Encoding::convertEncoding(rawurldecode($_POST['name']), 'UTF-8', SITE_CHARSET),
		'desc' => \Thurly\Main\Text\Encoding::convertEncoding(rawurldecode($_POST['desc']), 'UTF-8', SITE_CHARSET),
		'keywords' => \Thurly\Main\Text\Encoding::convertEncoding(rawurldecode($_POST['keywords']), 'UTF-8', SITE_CHARSET),
		'parent' =>$parent,
		'site' => $_GET['site'],
		'type' => $_POST['type']
	));
