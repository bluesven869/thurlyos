...
//check folder access
if (!$USER->CanDoFileOperation('fm_create_new_folder',$arPath))
	$strWarning = '<img src="/thurly/images/fileman/deny.gif" width="28" height="28" border="0" align="left" alt="">'.GetMessage("ACCESS_DENIED");
else if(!$io->DirectoryExists($abs_path))
	$strWarning = GetMessage("FILEMAN_FOLDER_NOT_FOUND");
else
{
	if($REQUEST_METHOD=="POST" && strlen($save)>0 && check_thurly_sessid())
	{
			...
			else
			{
				...
				if(strlen($strWarning)<=0)
				{
					...
					else
					{
						...
						{
							if($toedit=="Y")
								LocalRedirect("/thurly/admin/fileman_html_edit.php?".$addUrl."&site=".$site."&template=".Urlencode($template)."&path=".UrlEncode($pathto)."&filename=index.php&new=Y".(strlen($back_url) <=0 ?"":"&back_url=".UrlEncode($back_url)).(strlen($gotonewpage)<=0?"":"&gotonewpage=".UrlEncode($gotonewpage)).(strlen($backnewurl)<=0?"":"&backnewurl=".UrlEncode($backnewurl)));
							else
					...
					elseif (strlen($apply)<=0 && $strNotice == '')
					{
						...
						else
						{
							...
							LocalRedirect("/thurly/admin/fileman_admin.php?".$addUrl."&site=".$site."&path=".UrlEncode($arPathtoParsed["PREV"]));
						}
					}
				}
			}
