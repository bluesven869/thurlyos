...
if(($extended=="Y" || $extended=="N") && $extended != ${COption::GetOptionString("main", "cookie_name", "THURLY_SM")."_menumode"})
	setcookie(COption::GetOptionString("main", "cookie_name", "THURLY_SM")."_menumode", $extended, time()+60*60*24*30*60 ,'/');
else
	$extended = ${COption::GetOptionString("main", "cookie_name", "THURLY_SM")."_menumode"};
...
{
	if($REQUEST_METHOD=="POST" && strlen($save)>0 && is_array($ids) && check_thurly_sessid())
	{
		$sMenuTemplateTmp = "";
	...
	if($REQUEST_METHOD=="POST" && strlen($save)>0 && strlen($name)<=0 && check_thurly_sessid())
	{
		$strWarning = GetMessage("FILEMAN_MENU_EDIT_ENTER_TYPE");
	}
	elseif(strlen($new)>0 && strlen($name)>0 && $io->FileExists($abs_path) && check_thurly_sessid())
	{
		...
		if($REQUEST_METHOD=="POST" && strlen($save) >0 && is_array($ids) && check_thurly_sessid())
		{
			...
			if(strlen($apply)<=0)
			{
				if(strlen($back_url)>0)
					...
					LocalRedirect("/thurly/admin/fileman_admin.php?".$addUrl."&site=".$site."&path=".UrlEncode($path));
			}
			else
				LocalRedirect("/thurly/admin/fileman_menu_edit.php?".$addUrl."&site=".$site."&path=".UrlEncode($path)."&name=".$name);
		}
	}
}
...
if ($bEdit && !$only_edit)
{
	...
	$aMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".GetMessage("FILEMAN_DEL_CONF")."')) window.location='fileman_admin.php?path=".UrlEncode($path)."&action=delete&ID[]=".UrlEncode(".".$name.".menu.php")."&".$addUrl."&".thurly_sessid_get()."#tb';",
		"WARNING" => "Y"
	);
}
...
?>
	...
	$tabControl->Buttons(
		array(
			"disabled" => false,
			"back_url" => ((strlen($back_url)>0 && strpos($back_url, "/thurly/admin/fileman_menu_edit.php")!==0) ? htmlspecialcharsex($back_url) : "/thurly/admin/fileman_admin.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($arParsedPath["FULL"]))
		)
	);
	?>
