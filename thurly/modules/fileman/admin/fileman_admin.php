...
{
	include($_SERVER["DOCUMENT_ROOT"]."/thurly/admin/fileman_access.php");
	die();
}
...
if($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_GET["fu_action"]) > 0 && check_thurly_sessid())
{
...
if (isset($_GET['check_for_file']) && $_GET['check_for_file'] == 'Y' && $io->FileExists($absPath))
	die('<script>top.location="'."fileman_file_view.php?path=".urlencode($path).'&'.thurly_sessid_get().'&'.$addUrl.'"</script>');
...
while($Elem = $db_DirContent->NavNext(true, "f_"))
{
	...
	else
	{
		...
		{
			$showFieldIcon = "<IMG SRC=\"/thurly/images/fileman/types/".$curFileType.".gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=0 ALT=\"\">";
	...
	if($logical == 'Y')
	{
		...
		else
		{
			...
			{
				$showFieldIcon = "<IMG SRC=\"/thurly/images/fileman/types/".$curFileType.".gif\" WIDTH=\"16\" HEIGHT=\"16\" BORDER=0 ALT=\"\"  title=\"".htmlspecialcharsbx($fpath)."\">";
...
function GALCallBack(url)
{
	...
	{
		tbl_fileman_admin_filter.OnSet('tbl_fileman_admin', '/thurly/admin/fileman_admin.php?lang=' + lang + '&site=' + site + '&path=' + path + '&');
		return false;
	};

	delFilterBut.onclick = function()
	{
		tbl_fileman_admin_filter.OnClear('tbl_fileman_admin', '/thurly/admin/fileman_admin.php?lang=' + lang + '&site=' + site + '&path=' + path + '&');
		return false;
	};
}
</script>
