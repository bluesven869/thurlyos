...
if($REQUEST_METHOD=="POST" && $_POST['saveperm'] == 'Y' && check_thurly_sessid())
{
	//Clear all
...
<tr>
	...
	<table class="edit-table" id="bxst_access_table">
		...
		<tr>
			<td class="field-name" width="50%"><label for="TASKS_<?= $group_id?>"><?= htmlspecialcharsex($arGroupIndex[$group_id])." [<a title=\"".GetMessage("FM_ST_EDIT_GROUP_TITLE")."\" href=\"/thurly/admin/group_edit.php?ID=".$group_id."&amp;lang=".LANGUAGE_ID."\">".$group_id."</a>]"?>:</label></td>
			<td  width="50%">
		...
		<?= BeginNote();?>
		<?= GetMessage("FM_ST_ACCESS_NOTE", array('#LINK_BEGIN#' => '<a href="/thurly/admin/settings.php?lang='.LANGUAGE_ID.'&mid=fileman&tabControl_active_tab=edit3&'.thurly_sessid_get().'">', '#LINK_END#' => '</a>'));?>
		<?= EndNote();?>
...
$tabControl->Buttons(
	array(
		"disabled" => false,
		"back_url" => "/thurly/admin/?lang=".LANGUAGE_ID."&".thurly_sessid_get()
	)
);
?>
