...
else
{
	...
	if($REQUEST_METHOD=="POST" && strlen($save)>0 && strlen($propeditmore)<=0 && check_thurly_sessid())
	{
		...
		else
		{
			if(strlen($apply)<=0)
			{
				...
				LocalRedirect("/thurly/admin/fileman_folder.php?".$addUrl."&site=".$site."&path=".UrlEncode($path));
		}
	}
}
	...
	<tr>
		<td align="center" colspan="2">
			<table border="0" cellspacing="1" cellpadding="2" width="100%" class="internal">
				...
				<tr>
					<td>
						[<a href="/thurly/admin/group_edit.php?ID=<?=$g_ID?>&lang=<?=LANGUAGE_ID?>"><?=$g_ID?></a>]&nbsp;<?echo $g_NAME?>:
					</td>
					<td>
						<?
						// Inherit access level
