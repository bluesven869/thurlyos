...
if($REQUEST_METHOD=="POST" && strlen($saveperm) > 0 && check_thurly_sessid()) // TODO: access
{
	$arTaskPerm = array();
...
<tr>
	...
	<table class="internal">
		<tr class="heading">
			...
			<tr valign="top">
				<td>
					[<a href="/thurly/admin/group_edit.php?ID=<?= $arGroup['ID']?>&lang=<?=LANGUAGE_ID?>"><?= $arGroup['ID']?></a>]&nbsp;<?= htmlspecialcharsex($arGroup['NAME'])?>:
				</td>
				<td>
...
$tabControl->Buttons(
	array(
		"disabled" => false,
		"back_url" => "fileman_medialib_admin.php?".$addUrl."&lang=".LANGUAGE_ID."&".thurly_sessid_get()
	)
);
?>
...
<script>
function colsOnChange(pSel){window.location = "/thurly/admin/fileman_medialib_access.php?col_id=" + pSel.value + "&lang=<?= LANGUAGE_ID?>&<?= thurly_sessid_get()?>";}
</script>
<?
