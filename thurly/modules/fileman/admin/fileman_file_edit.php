		...
		if(strlen($strWarning) <= 0)
		{
			...
			if (strlen($strWarning) <= 0)
			{
				if(strlen($apply) <= 0)
				{
					...
					LocalRedirect("/thurly/admin/fileman_admin.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($arParsedPath["PREV"]));
				}
				else
				{
					LocalRedirect("/thurly/admin/fileman_file_edit.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($path).($full_src=="Y"?"&full_src=Y":""));
				}
			}
...
$tabControl->BeginNextTab();?>
	...
	<?if(!$bEdit):?>
		...
		<tr>
			...
			<td width="70%">
				<select name="template" onchange="window.location='/thurly/admin/fileman_file_edit.php?lang=<?= LANG?>&site=<?=Urlencode($site)?>&path=<?= UrlEncode($path)?><? echo ($full_src=="Y" ? "&full_src=Y" : "")?>&new=y&template='+escape(this[this.selectedIndex].value)">
					<?for($i = 0, $l = count($arTemplates); $i < $l; $i++):?>
...
$tabControl->Buttons(
	array(
		"disabled" => $only_read,
		"back_url" => (strlen($back_url)>0 && strpos($back_url, "/thurly/admin/fileman_file_edit.php")!==0) ? htmlspecialcharsbx($back_url) : "/thurly/admin/fileman_admin.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($arParsedPath["PREV"])
	)
);
