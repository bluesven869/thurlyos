...
function OnLoad()
{
	...
	if(pElement && bxTag) /* Link selected*/
	{
		...
		else if (href.indexOf("://") !== -1 || href.substr(0, 'www.'.length) == 'www.' || href.indexOf("&goto=") !== -1)
		{
			...
			if(href.substr(0, '/thurly/redirect.php'.length) == '/thurly/redirect.php')
			{
				BX("bx_fixstat").checked = true;
				ChangeFixStat();
				var sParams = href.substring('/thurly/redirect.php'.length);
	...
	switch(BX('bx_link_type').value)
	{
		...
		case 't2':
			...
			if(BX("bx_fixstat").checked)
				href = '/thurly/redirect.php?event1=' + escape(BX("event1").value) + '&event2=' + escape(BX("event2").value) + '&event3=' + escape(BX("event3").value) + '&goto=' + escape(href);
			break;
		case 't3':
	...
	<table border="0" width="100%" height="100%" style="vertical-align: middle">
		...
		<tr>
			<td align="center" valign="top">
				<img style="vertical-align: middle;" src="/thurly/themes/.default/images/wait.gif" />
	...
	if (!compareObj(Settings.arTaskbarSettings, window.temp_arTaskbarSettings))
	{
		...
		if(temp_arTaskbarSettings['BXSnippetsTaskbar'].show)
		{
			if (window.BXSnippetsTaskbar_need_preload)
				arScripts.push("/thurly/admin/htmleditor2/snippets.js");
		}
		...
		if (temp_arTaskbarSettings['BXComponents2Taskbar'].show)
		{
			if (window.BXComponents2Taskbar_need_preload)
				arScripts.push("/thurly/admin/htmleditor2/components2.js");
	...
	BX.ajax.post(manage_snippets_path + '&target=add',
		...
		function()	{setTimeout(function(){
			...
			var __arEl =
			{
				...
				thumb:thumb,
				isGroup:false,
				icon:'/thurly/images/fileman/htmledit2/snippet.gif',
