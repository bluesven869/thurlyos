...
$l = count($arr);
for($i = 0; $i < $l; $i++)
	@$files_mod_str .= filemtime($_SERVER['DOCUMENT_ROOT'].'/thurly/admin/htmleditor2/'.$arr[$i]);
...
if (!isset($_SERVER['HTTP_IF_NONE_MATCH']) || $_SERVER['HTTP_IF_NONE_MATCH'] != '"'.$files_mod_hash.'"')
{
	...
	for($i = 0; $i < count($arr); $i++)
	{
		...
		$script_content = GetFileContent($_SERVER['DOCUMENT_ROOT'].'/thurly/admin/htmleditor2/'.$script_filename);
