	...
	if($REQUEST_METHOD=="POST" && strlen($save) > 0 && check_thurly_sessid())
	{
		...
		if(strlen($strWarning) <= 0)
		{
			if (!empty($_POST["apply"]))
				LocalRedirect("/thurly/admin/fileman_file_upload.php?".$addUrl."&site=".$site."&path=".UrlEncode($path));
			else
...
<?if(strlen($strWarning) <= 0 || $bCan):?>
	...
	$tabControl->Buttons(
		array(
			"disabled" => false,
			"back_url" => "/thurly/admin/fileman_admin.php?".$addUrl."&site=".$site."&path=".UrlEncode($path)
		)
	);
	$tabControl->End();
	?>
