...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/fileman/admin/fileman_html_edit.php");
...
if(strlen($back_url)>0 && strpos($back_url, "/thurly/admin/fileman_file_edit.php")!==0)
	$url = "/".ltrim($back_url, "/");
else
	$url = "/thurly/admin/fileman_admin.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($arParsedPath["PREV"]);
		...
		if(strlen($strWarning) <= 0)
		{
			...
			else
			{
				...
				if(strlen($strWarning)<=0 && strlen($apply)<=0 && strlen($apply2)<=0)
					...
					$localRedirectUrl = "/thurly/admin/fileman_html_edit.php?".$addUrl."&site=".Urlencode($site)."&path=".UrlEncode($path)."&back_url=".UrlEncode($back_url)."&fullscreen=".($bFullScreen?"Y":"N")."&tabControl_active_tab=".urlencode($tabControl_active_tab);
			}
...
if($bEdit)
{
	...
	if($USER->CanDoFileOperation('fm_delete_file', $arPath))
	{
		...
		$aMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('".GetMessage("FILEMAN_FILE_DELETE_CONFIRM")."')) window.location='/thurly/admin/fileman_admin.php?ID=".urlencode($id)."&action=delete&".$addUrl."&site=".urlencode($site)."&path=".urlencode($folder_path)."&".thurly_sessid_get()."';",
...
?>
	<?if(!$bEdit):?>
		...
		function templateOnChange(_this)
		{
			...
			window.location='/thurly/admin/fileman_html_edit.php?lang=<?= LANG?><?=$logic?>&site=<?=Urlencode($site)?>&path=<?= UrlEncode($folderPath)?>&new=y&template='+encodeURIComponent(_this[_this.selectedIndex].value)+_name+_title;
		}
		</script>
		...
		<tr>
			...
			<td style="padding: 0 0 3px!important;">
				<table id='jserror_name' style="visibility:hidden"><tr><td valign="top">
							<IMG src="/thurly/themes/.default/images/icon_warn.gif" title="<?=GetMessage("FILEMAN_NAME_ERR");?>">
...
{
	window.location='/thurly/admin/fileman_html_edit.php?lang=<?= LANG?>&site=<?=Urlencode($site)?>&path=<?= UrlEncode($path)?>&new=y&template='+id;
