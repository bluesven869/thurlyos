...
class filemanMedialibUpload
{
	...
	function handleFile($hash, $file, &$package, &$upload)
	{
		...
		{
			$upload["redirectUrlPart"] = "action=redirect&".thurly_sessid_get()."&first_id=".$res["ID"].
...
$action = (array_key_exists("action", $_REQUEST) && check_thurly_sessid() ? $_REQUEST["action"] : "");
...
elseif($action == 'redirect')  //Redirect after files uploading
{
	...
	$APPLICATION->SetAdditionalCSS('/thurly/js/fileman/medialib/medialib_admin.css');
	...
	$aContext[] = Array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/fileman_medialib_admin.php?lang=".LANGUAGE_ID."&".thurly_sessid_get()
	);
	...
	if ($len > 0)
	{
		...
		<form name="ml_mu_form" action="/thurly/admin/fileman_medialib_upload.php?action=postsave" method="POST">
		...
		for($i = 0; $i < $len; $i++)
		{
			...
			<tr>
				...
				<?else:?>
					<img src="/thurly/images/1.gif" class="ml-item-thumb ml-item-no-thumb" />
				<?endif;?>
				</td>
			</tr>
			<tr>
		...
		<input type="button" title="<?= GetMessage('admin_lib_edit_cancel_title')?>" onclick="window.location='/thurly/admin/fileman_medialib_admin.php?lang=<?= LANGUAGE_ID?>';" name="cancel" value="<?= GetMessage('admin_lib_edit_cancel')?>" />
		</form>
		<script>
		...
		function addCollToItem(ItemId, id, pSel)
		{
			...
			var
				...
				pDel = pDiv.appendChild(jsUtils.CreateElement("IMG", {src: '/thurly/images/1.gif', className: 'mu-col-del', id: 'mu_it_' + id, title: '<?= GetMessage('ML_DEL_COL2ITEM')?>'}));
...
elseif($action == 'postsave')
{
	...
	LocalRedirect("/thurly/admin/fileman_medialib_admin.php?lang=".LANGUAGE_ID."&".thurly_sessid_get());
}

// ***************************** Show upploader  **************************
$APPLICATION->SetAdditionalCSS('/thurly/js/fileman/medialib/medialib_admin.css');
$APPLICATION->AddHeadScript('/thurly/js/fileman/medialib/medialib_admin.js');
...
$menu = new CAdminContextMenu(array(
	array(
		"TEXT" => GetMessage("FM_ML"),
		"LINK" => "/thurly/admin/fileman_medialib_admin.php?lang=".LANGUAGE_ID,
		"ICON" => "btn_list",
	)
));
$menu->Show();
...
<form id="<?=$uploaderID?>_form" name="<?=$uploaderID?>_form" action="<?=$APPLICATION->GetCurPageParam("type_ml=".urlencode($_GET["type"])."&".thurly_sessid_get(), array("type_ml", "sessid"))?>" method="POST" enctype="multipart/form-data" class="bxiu-photo-form">
	<input type="hidden" name="action" id="id" value="uploadhtml5" />
	<input type="hidden" name="sessid" id="sessid" value="<?= thurly_sessid()?>" />
	...
	<div class="bxu-main-block" id="bxuDropzone<?=$uploaderID?>">
		<div class="bxu-start-block">
			<div class="bxu-start-block-spacer-div">
				<img class="bxu-start-block-spacer-img" src="/thurly/images/fileman/medialib/uploader/upl-start-spacer.png"/>
				<input type="file" id="bxuUploaderStartField<?=$uploaderID?>" multiple="multiple" />
			</div>
			<div class="bxu-start-block-cont">
				<img src="/thurly/images/fileman/medialib/uploader/start-img.png" class="bxu-start-block-img" alt=""/>
...
<span class="bxu-item-block">
	<span class="bxu-item-block-top">
		<img src="/thurly/images/fileman/medialib/uploader/upl-spacer-img.png" class="bxu-spacer"/>
