...
<script>
	...
	BX.loadCSS('/thurly/components/thurly/map.google.view/settings/settings.css');
...
function saveYandexKey(domain, input)
{
	...
	var data = 'key_type=yandex&domain=' + domain + '&key=' + value + '&<?echo thurly_sessid_get()?>';
	ShowWaitWindow();
	CHttpRequest.Post('/thurly/admin/settings.php?lang=<?echo LANGUAGE_ID?>&mid=fileman&save_map_key=Y', data);
}
</script>
<?
...
<script>
	...
	jsUtils.loadCSSFile('/thurly/components/thurly/map.yandex.view/settings/settings.css');
...
<script>
jsUtils.loadCSSFile("/thurly/components/thurly/player/js/skin_selector.css");
jsUtils.loadJSFile("/thurly/components/thurly/player/js/prop_skin_selector.js", function()
{
...
<table class="bx-video-prop-tbl">
	...
	<? if ($controlMode != "iblock_element_admin"): ?>
		...
		<tr id="bx_video_new_path_row_<?= $id?>" <?if (strlen($path) > 0){ echo 'style="display: none;"'; }?>>
			...
			<img src="/thurly/images/1.gif" class="spacer" />
...
<?
	...
	{
		return "/thurly/components/thurly/player/mediaplayer/skins";
	}
