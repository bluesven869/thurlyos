...
class CHTMLEditor
{
	...
	private function Init($arParams)
	{
		...
		$basePath = '/thurly/js/fileman/html_editor/';
		...
		$arParams["actionUrl"] = $arParams["bbCode"] ? '/thurly/tools/html_editor_action.php' : '/thurly/admin/fileman_html_editor_action.php';
	...
	function Show($arParams)
	{
		...
		if ($arParams["initConponentParams"])
		{
			CComponentParamsManager::Init(array(
				'requestUrl' => '/thurly/admin/fileman_component_params.php'
			));
		}
	}
	...
	function SafeJsonEncode($data = array())
	{
		try
		{
			$json = \Thurly\Main\Web\Json::encode($data, JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT|JSON_PARTIAL_OUTPUT_ON_ERROR);
		}
		catch (Exception $e)
		{
			$json = '{}';
	...
	{
		$mess_lang = \Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/fileman/classes/general/html_editor_js.php');
	...
	public static function ShowResponse($reqId = false, $Res = false)
	{
		if ($Res !== false)
		{
			...
			{
				?>
				<script>top.BXHtmlEditorAjaxResponse['<?= $reqId?>'] = <?= \Thurly\Main\Web\Json::encode($Res)?>;</script>
			<?
			}
		}
	}
	...
	public static function GetVideoOembed($url = '')
	{
		...
		$metaData = \Thurly\Main\UrlPreview\UrlPreview::fetchVideoMetaData($url);
