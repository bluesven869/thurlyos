<?
$module_id = "fileman";
$dicsRelPath = '/thurly/modules/fileman/dictionaries';
...
if ($REQUEST_METHOD=="GET" && $USER->CanDoOperation('fileman_edit_all_settings') && strlen($RestoreDefaults)>0 && check_thurly_sessid())
{
...
if($REQUEST_METHOD == "POST" && strlen($Update)>0 && $USER->CanDoOperation('fileman_edit_all_settings') && check_thurly_sessid())
{
	...
	if (isset($_POST['user_dics_path']) && $_POST['user_dics_path']!='')
		...
		COption::SetOptionString($module_id, "user_dics_path","/thurly/modules/fileman/u_dics");
	...
	COption::GetOptionString($module_id, "use_editor_3", "N") == "Y")
{
	...
	{
		$l_id = $_GET['load_dic'];
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/tar_gz.php");
...
table.bxopt-tbl td.bxopt-but-icon .bxed-button{width: 20px; height: 20px; background-image: url(/thurly/images/fileman/htmledit2/_global_iconkit.gif); background-repeat: norepeat;}
...
table.bxopt-list td.bx-listbutton{background:url("/thurly/images/fileman/htmledit2/_global_iconkit.gif") no-repeat scroll -63px -59px #E0DEDA !important; border-left:1px solid #A5ACB2 !important; font-size:1pt !important; height:18px !important; width:14px !important;}
...
function _GtFMess()
{
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/fileman/lang/'.LANGUAGE_ID.'/admin/fileman_js.php'))
		include($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/fileman/lang/'.LANGUAGE_ID.'/admin/fileman_js.php');
	else
		include($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/fileman/lang/en/admin/fileman_js.php');
	return $MESS;
}
...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/fileman/admin/fileman_js.php');
$l = count($aMess);
for($i = 0; $i < $l; $i++)
...
BXButtonConfig.prototype =
{
	Init: function()
	{
		...
		// Load buttons
		BX.loadScript('/thurly/admin/htmleditor2/toolbarbuttons.js', function()
			{
	...
	AddButton: function(typeCode, but, ind, toolbarId)
	{
		...
		else
		{
			...
			{
				var pIcon = iconCell.appendChild(BX.create("IMG", {props: {id: "bx_btn_" + but[1].id, src: "/thurly/images/1.gif", className: "bxed-button"}}));
...
<? if ($useEditor3):?>
	...
	<tr>
		...
		<td>
			<input type="text" name="user_dics_path" style="width: 100%" value="<?= htmlspecialcharsbx(COption::GetOptionString($module_id, "user_dics_path", "/thurly/modules/fileman/u_dics"))?>">
		</td>
	</tr>
	<tr>
...
?>
	...
	<tr>
		...
		<img src="/thurly/images/1.gif" style="width: 110px; height: 1px;" />
		</td>
	</tr>
...
<script>
	function RestoreDefaults()
	{
		if(confirm('<?= AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>'))
			window.location = "<?= $APPLICATION->GetCurPage()?>?RestoreDefaults=Y&lang=<?echo LANG?>&mid=<?echo urlencode($mid)?>&<?=thurly_sessid_get()?>";
	}
</script>
