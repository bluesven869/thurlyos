...
class pull extends CModule
{
	...
	function DoInstall()
	{
		$this->InstallFiles();
		$this->InstallDB();
		$GLOBALS['APPLICATION']->IncludeAdminFile(GetMessage("PULL_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/pull/install/step1.php");
	}
	...
	function InstallDB()
	{
		...
		if(!$DB->Query("SELECT 'x' FROM b_pull_stack", true))
			$this->errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT']."/thurly/modules/pull/install/db/".strtolower($DB->type)."/install.sql");
		...
		RegisterModuleDependences("main", "OnAfterEpilog", "pull", "\Thurly\Pull\Event", "onAfterEpilog");
		...
		RegisterModuleDependences("socialnetwork", "OnSonetLogCounterClear", "pull", "\Thurly\Pull\MobileCounter", "onSonetLogCounterClear");

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->registerEventHandler('rest', 'OnRestServiceBuildDescription', 'pull', '\Thurly\Pull\Rest', 'onRestServiceBuildDescription');
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/pull/install/js", $_SERVER["DOCUMENT_ROOT"]."/thurly/js", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/pull/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", true, true);
		}
		return true;
	}
	...
	function DoUninstall()
	{
		...
		if($step<2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage("PULL_UNINSTALL_TITLE"), $DOCUMENT_ROOT."/thurly/modules/pull/install/unstep1.php");
		}
		elseif($step==2)
		{
			$this->UnInstallDB(array("savedata" => $_REQUEST["savedata"]));
			$this->UnInstallFiles();
			$APPLICATION->IncludeAdminFile(GetMessage("PULL_UNINSTALL_TITLE"), $DOCUMENT_ROOT."/thurly/modules/pull/install/unstep2.php");
		}
	}
	...
	function UnInstallDB($arParams = Array())
	{
		...
		if (!$arParams['savedata'])
			$this->errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT']."/thurly/modules/pull/install/db/".strtolower($DB->type)."/uninstall.sql");
		...
		UnRegisterModuleDependences("main", "OnAfterEpilog", "pull", "\Thurly\Pull\Event", "onAfterEpilog");
		...
		UnRegisterModuleDependences("socialnetwork", "OnSonetLogCounterClear", "pull", "\Thurly\Pull\MobileCounter", "onSonetLogCounterClear");

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->unRegisterEventHandler('rest', 'OnRestServiceBuildDescription', 'pull', '\Thurly\Pull\Rest', 'onRestServiceBuildDescription');
