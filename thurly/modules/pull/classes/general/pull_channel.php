...
class CPullChannel
{
	...
	public static function Get($userId, $cache = true, $reOpen = false, $channelType = self::TYPE_PRIVATE)
	{
		...
		else
		{
			...
			{
				self::Send($arResult['CHANNEL_ID'], \Thurly\Pull\Common::jsonEncode(Array(
					'module_id' => 'pull',
					'command' => 'reopen',
					'expiry' => 1,
					'params' => Array(),
	...
	public static function Add($userId, $channelId = null, $channelType = self::TYPE_PRIVATE)
	{
		...
		if ($result > 0)
		{
			...
			{
				self::Send($channelId, \Thurly\Pull\Common::jsonEncode(Array(
					'module_id' => 'pull',
					'command' => 'open',
					'expiry' => 1,
					'params' => Array(),
			...
			$orm = \Thurly\Main\UserTable::getList(array(
				...
				'runtime' => Array(
					new \Thurly\Main\Entity\ReferenceField(
						'CHANNEL',
						'\Thurly\Pull\Model\ChannelTable',
						array(
							"=ref.USER_ID" => "this.ID",
							"=ref.CHANNEL_TYPE" => new \Thurly\Main\DB\SqlExpression('?s', 'private'),
						),
			...
			if ($isImInstalled)
			{
				foreach ($arOnline as $userId)
				{
					$arSend[$userId] = Array(
						...
						'color' => $users[$userId]['COLOR']? \Thurly\Im\Color::getColor($users[$userId]['COLOR']): '',
						'idle' => $users[$userId]['IDLE'] instanceof \Thurly\Main\Type\DateTime? $users[$userId]['IDLE']: false,
						'mobile_last_date' => $users[$userId]['MOBILE_LAST_DATE'] instanceof \Thurly\Main\Type\DateTime? $users[$userId]['MOBILE_LAST_DATE']: false,
						'last_activity_date' => new \Thurly\Main\Type\DateTime(),
					);
				}
			}
			else
	...
	public static function GetConfig($userId, $cache = true, $reopen = false, $mobile = false)
	{
		...
		$pullPath = ($nginxStatus? (CMain::IsHTTPS()? CPullOptions::GetListenSecureUrl($arChannels): CPullOptions::GetListenUrl($arChannels)): '/thurly/components/thurly/pull.request/ajax.php?UPDATE_STATE');
