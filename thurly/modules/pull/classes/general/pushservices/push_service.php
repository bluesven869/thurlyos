		...
		foreach ($appMessages as $appID => $arMessages)
		{
			...
			foreach ($arMessages as $token => $messages)
			{
				...
				while ($mess < $messCount)
				{
					...
					$text = \Thurly\Main\Text\Encoding::convertEncoding($messageArray["MESSAGE"], SITE_CHARSET, "utf-8");
					$title = \Thurly\Main\Text\Encoding::convertEncoding($messageArray["TITLE"], SITE_CHARSET, "utf-8");
					$message->setSound('');
					...
					{
						$messageArray["ADVANCED_PARAMS"] = \Thurly\Main\Text\Encoding::convertEncoding($messageArray["ADVANCED_PARAMS"], SITE_CHARSET, "UTF-8");
