...
/**
 * Class CPullPush
 * @deprecated use Thurly\Pull\Model\PushTable
 * @see \Thurly\Pull\Model\PushTable
 */
class CPullPush
{
	/**
	 * @deprecated use Thurly\Pull\Model\PushTable::getList
	 * @see Thurly\Pull\Model\PushTable::getList
	 * @param array $arOrder
	...
	public static function GetList($arOrder = array(), $arFilter = array(), $arSelect = array(), $arNavStartParams = Array())
	{
		...
		$res = \Thurly\Pull\Model\PushTable::getList($params);
	...
	/**
	 * @deprecated use Thurly\Pull\Model\PushTable::add
	 * @see Thurly\Pull\Model\PushTable::add
	...
	{
		$result = \Thurly\Pull\Model\PushTable::add($arFields);
	...
	/**
	 * @deprecated use Thurly\Pull\Model\PushTable::update
	 * @see Thurly\Pull\Model\PushTable::update
	 * @param $ID
	 * @param array $arFields
	 * @return int
	 * @throws Exception
	...
	{
		$result = \Thurly\Pull\Model\PushTable::update($ID, $arFields);
		return $result->getId();
	}

	/**
	 * @deprecated use Thurly\Pull\Model\PushTable::delete
	 * @see Thurly\Pull\Model\PushTable::delete
	 * @param bool $ID
	 * @return bool
	 * @throws Exception
	 */
	...
	{
		$result = \Thurly\Pull\Model\PushTable::delete(intval($ID));
...
class CPushManager
{
	...
	public static function DeleteFromQueueByTag($userId, $tag, $appId = 'ThurlyOS')
	{
		global $DB;
		...
		\Thurly\Pull\Push::add($userId, Array(
			'module_id' => 'pull',
			'push' => Array(
	...
	public function AddQueue($arParams)
	{
		...
		foreach ($groupMode[self::SEND_DEFERRED] as $userId)
		{
			...
			{
				$arAdd['ADVANCED_PARAMS'] = Thurly\Main\Web\Json::encode($arFields['ADVANCED_PARAMS']);
			}
			if (is_array($arFields['PARAMS']))
			{
				$arAdd['PARAMS'] = Thurly\Main\Web\Json::encode($arFields['PARAMS']);
			}
			else
			{
	...
	private static function prepareSend($userId, $fields, $type = self::SEND_IMMEDIATELY)
	{
		...
		if ($type != self::SEND_DEFERRED)
		{
			if (is_array($fields['PARAMS']))
			{
				...
				$result['PARAMS'] = Thurly\Main\Web\Json::encode($fields['PARAMS']);
			}
		...
		{
			$result['BADGE'] = \Thurly\Pull\MobileCounter::get($result['USER_ID']);
		}
	...
	public static function GetDeviceInfo($userId, $appId = 'ThurlyOS')
	{
		$result = Array();
		if (!is_array($userId))
		{
		...
		$imInclude = \Thurly\Main\Loader::includeModule('im');

		$query = new \Thurly\Main\Entity\Query(\Thurly\Main\UserTable::getEntity());

		$sago = Thurly\Main\Application::getConnection()->getSqlHelper()->addSecondsToDateTime('-300');
		$query->registerRuntimeField('', new \Thurly\Main\Entity\ExpressionField('IS_ONLINE_CUSTOM', 'CASE WHEN LAST_ACTIVITY_DATE > ' . $sago . ' THEN \'Y\' ELSE \'N\' END'));
		$query->addSelect('ID')->addSelect('ACTIVE')->addSelect('EMAIL')->addSelect('IS_ONLINE_CUSTOM');

		if ($imInclude)
		{
			$query->registerRuntimeField('', new \Thurly\Main\Entity\ReferenceField('im', 'Thurly\Im\Model\StatusTable', array('=this.ID' => 'ref.USER_ID')));
			$query->addSelect('im.IDLE', 'IDLE')->addSelect('im.MOBILE_LAST_DATE', 'MOBILE_LAST_DATE');
		}

		$query->registerRuntimeField('', new \Thurly\Main\Entity\ReferenceField('push', 'Thurly\Pull\Model\PushTable', array('=this.ID' => 'ref.USER_ID')));
		$query->registerRuntimeField('', new \Thurly\Main\Entity\ExpressionField('HAS_MOBILE', 'CASE WHEN main_user_push.USER_ID > 0 THEN \'Y\' ELSE \'N\' END'));
		...
		while ($user = $queryResult->fetch())
		{
			...
			if (!\Thurly\Pull\Push::getStatus($user['ID']))
			{
			...
			if ($imInclude)
			{
				$mobileLastDate = $user['MOBILE_LAST_DATE'] instanceof \Thurly\Main\Type\DateTime? $user['MOBILE_LAST_DATE']->getTimestamp(): 0;
			...
			{
				if (!\Thurly\Pull\PushSmartfilter::getStatus())
				{
	...
	public function sendBatch($batch)
	{
		...
		if (strlen($key) > 0 && strlen($batch) > 0)
		{
			...
			$httpClient = new \Thurly\Main\Web\HttpClient(array("waitResponse" => false));
	...
	public static function DeleteFromQueueBySubTag($userId, $tag, $appId = 'ThurlyOS')
	{
		global $DB;
		if (strlen($tag) <= 0 || intval($userId) == 0)
		{
	...
	public static function SendAgent()
	{
		...
		while ($arRes = $dbRes->Fetch())
		{
			if ($arRes['BADGE'] == '')
			{
				$arRes['BADGE'] = \Thurly\Pull\MobileCounter::get($arRes['USER_ID']);
			}

			try
			{
				$arRes['PARAMS'] = $arRes['PARAMS'] ? Thurly\Main\Web\Json::decode($arRes['PARAMS']) : "";
			}
			catch (Exception $e)
			{
				$arRes['PARAMS'] = "";
			...
			if (is_array($arRes['PARAMS']))
			{
				...
				$arRes['PARAMS'] = Thurly\Main\Web\Json::encode($arRes['PARAMS']);
			}
			try
			{
				$arRes['ADVANCED_PARAMS'] = strlen($arRes['ADVANCED_PARAMS']) > 0 ? Thurly\Main\Web\Json::decode($arRes['ADVANCED_PARAMS']) : Array();
			}
			catch (Exception $e)
			{
	...
	public function sendBadges($userId = null, $appId = 'ThurlyOS')
	{
		return \Thurly\Pull\MobileCounter::send($userId, $appId);
	}
}
?>