...
;(function (window)
{
					...
					if (params.command == 'invite')
					{
						if (this.callInit)
						{
							BX.ajax({
								...
								dataType: 'json',
								timeout: 30,
								data: {'COMMAND': 'busy', 'USER_ID' : params.senderId, 'sessid': BX.thurly_sessid()}
							});
						}
						else
						{
	...
	YourCompanyPrefix.webrtc.prototype.setLocalAndSend = function(userId, desc)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'COMMAND': 'signaling', 'USER_ID' : userId, 'PEER': JSON.stringify( desc ), 'sessid': BX.thurly_sessid()}
		});
	...
	YourCompanyPrefix.webrtc.prototype.onIceCandidate = function (userId, candidates)
	{
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'COMMAND': 'signaling', 'USER_ID' : userId, 'PEER': JSON.stringify(candidates), 'sessid': BX.thurly_sessid()}
		});
	}
	...
	YourCompanyPrefix.webrtc.prototype.peerConnectionReconnect = function (userId)
	{
		...
		BX.ajax({
			...
			dataType: 'json',
			timeout: 30,
			data: {'COMMAND': 'reconnect', 'USER_ID' : userId, 'sessid': BX.thurly_sessid()},
	...
	YourCompanyPrefix.webrtc.prototype.callCommand = function(command, async)
	{
		...
		BX.ajax({
			...
			timeout: 30,
			async: async != false,
			data: {'COMMAND': command, 'USER_ID': this.callUserId, 'sessid': BX.thurly_sessid()}
		});
	};
