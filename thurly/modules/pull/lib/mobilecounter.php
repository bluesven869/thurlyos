...
class MobileCounter
{
	...
	public static function getTypes()
	{
		...
		$event = new \Thurly\Main\Event("pull", "onGetMobileCounterTypes");
		$event->send();
	...
	public static function get($userId = null)
	{
		...
		{
			if (\Thurly\Main\Loader::includeModule('im')) // TODO remove IM include!
			{
				$siteId = \Thurly\Im\User::getInstance($userId)->isExtranet()? 'ex': 's1';
			}
			else
			{
				$siteId = 's1';
		...
		{
			$siteId = \Thurly\Main\Context::getCurrent()->getSite();
			if (!$siteId)
			{
				$siteId = 's1';
			}
		...
		$event = new \Thurly\Main\Event("pull", "onGetMobileCounter", array(
			'USER_ID' => $userId,
			'SITE_ID' => $siteId
		));
		$event->send();
