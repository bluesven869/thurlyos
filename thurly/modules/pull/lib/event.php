...
class Event
{
	...
	private static function addPush($users, $parameters)
	{
		...
		if (!empty($parameters['push']['type']))
		{
			...
			{
				if (!\Thurly\Pull\Push::getConfigTypeStatus($parameters['module_id'], $parameters['push']['type'], $userId))
				{
	...
	private static function executeEvent($parameters)
	{
		...
		{
			\Thurly\Main\Loader::includeModule($parameters['event']['paramsCallback']['module_id']);
			if (
	...
	private static function executePushEvent($parameters)
	{
		...
		{
			\Thurly\Main\Loader::includeModule($parameters['push']['pushParamsCallback']['module_id']);
	...
	public static function getUserIds($channels)
	{
		...
		$result = Array();
		$orm = \Thurly\Pull\Model\ChannelTable::getList(array(
	...
	private static function prepareParameters($parameters)
	{
		...
		if (isset($parameters['paramsCallback']))
		{
			...
			\Thurly\Main\Loader::includeModule($parameters['paramsCallback']['module_id']);
	...
	private static function preparePushParameters($parameters)
	{
		...
		if (isset($parameters['pushParamsCallback']))
		{
			...
			\Thurly\Main\Loader::includeModule($parameters['pushParamsCallback']['module_id']);
