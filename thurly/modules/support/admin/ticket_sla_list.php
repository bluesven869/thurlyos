...
$EDIT_URL = "/thurly/admin/ticket_sla_edit.php";
...
foreach ($fetchedRows as $arRes)
{
	...
	if (isset($slaSiteList[$arRes['ID']]))
	{
		...
		foreach($arrSITE as $sid)
		{
			if ($sid!="ALL")
			{
				$str_SITE .= ($str_SITE == "" ? "" : " / ").'<a title="'.GetMessage("MAIN_ADMIN_MENU_EDIT").'" href="/thurly/admin/site_edit.php?LID='.$sid.'&lang='.LANG.'">'.$sid.'</a>';
			}
	...
	if (isset($slaGroupList[$arRes['ID']]))
	{
		...
		foreach($arG as $gid)
		{
			$str .= '[<a title="'.GetMessage("MAIN_ADMIN_MENU_EDIT").'" href="/thurly/admin/group_edit.php?ID='.$gid.'&lang='.LANG.'">'.$gid.'</a>] '.htmlspecialcharsbx($slaGroupNames[$gid]).'<br>';
		}
	}
	...
	{
		$str = '[<a title="'.GetMessage("SUP_USER_PROFILE").'" href="/thurly/admin/user_edit.php?lang='.LANG.'&ID='.$arRes['RESPONSIBLE_USER_ID'].'">'.$arRes['RESPONSIBLE_USER_ID'].'</a>] ('.$slaResponsiblesInfo[$arRes['RESPONSIBLE_USER_ID']]['LOGIN'].') '.$slaResponsiblesInfo[$arRes['RESPONSIBLE_USER_ID']]['NAME'];
	}
	...
	{
		$str = '<a href="/thurly/admin/ticket_timetable_edit.php?ID='.intval($arRes['TIMETABLE_ID']).'&lang='.LANG.'">'.htmlspecialcharsbx($slaTimeTableList[$arRes['TIMETABLE_ID']]).'</a>';
	}
...
<tr valign="top"> 
	<td valign="top"><?=GetMessage("SUP_SITE")?>:<br><img src="/thurly/images/support/mouse.gif" width="44" height="21" border=0 alt=""></td>
	<td><?
