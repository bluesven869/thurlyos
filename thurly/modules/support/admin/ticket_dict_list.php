...
while($arRes = $rsData->Fetch())
{
	...
	if(array_key_exists($arRes["ID"], $arSiteArrayForAllDict))
	{
		...
		foreach($arrSITE as $sid)
		{
			...
			$str_SITE .= ($str_SITE == "" ? "" : " / ") . '<a title="' . GetMessage("MAIN_ADMIN_MENU_EDIT") . '" href="/thurly/admin/site_edit.php?LID=' . $sidS . '&lang=' . LANG . '">' . $sidS . '</a>';
		}
	}
...
foreach($arRows as $k => $v)
{
	...
	if ($rUserID > 0 && array_key_exists($rUserID, $arRespUsersProp)):
		...
		$str = '[<a title="' . GetMessage("SUP_USER_PROFILE") . '" href="/thurly/admin/user_edit.php?lang=' . LANG . '&ID=' . $rUserID . '">' .
	...
	array(
		...
		"MENU" => Array(
			Array(
				...
				"ACTION"	=> "window.location='/thurly/admin/ticket_dict_edit.php?lang=".LANG."&find_type=C';"
			),
			Array(
				"TEXT"	=> GetMessage("SUP_ADD_CRITICALITY"),
				"ACTION"	=> "window.location='/thurly/admin/ticket_dict_edit.php?lang=".LANG."&find_type=K';"
			),
			Array(
				"TEXT"	=> GetMessage("SUP_ADD_STATUS"),
				"ACTION"	=> "window.location='/thurly/admin/ticket_dict_edit.php?lang=".LANG."&find_type=S';"
			),
			Array(
				"TEXT"	=> GetMessage("SUP_ADD_MARK"),
				"ACTION"	=> "window.location='/thurly/admin/ticket_dict_edit.php?lang=".LANG."&find_type=M';"
			),

			Array(
				"TEXT"	=> GetMessage("SUP_ADD_FUA"),
				"ACTION"	=> "window.location='/thurly/admin/ticket_dict_edit.php?lang=".LANG."&find_type=F';"
			),

			Array(
				"TEXT"	=> GetMessage("SUP_ADD_SOURCE"),
				"ACTION"	=> "window.location='/thurly/admin/ticket_dict_edit.php?lang=".LANG."&find_type=SR';"
			),

			Array(
				"TEXT"	=> GetMessage("SUP_ADD_DIFFICULTY"),
				"ACTION"	=> "window.location='/thurly/admin/ticket_dict_edit.php?lang=".LANG."&find_type=D';"
			),
...
<tr valign="top"> 
	<td valign="top"><?=GetMessage("SUP_F_SITE")?>:<br><img src="/thurly/images/support/mouse.gif" width="44" height="21" border=0 alt=""></td>
	<td><?
	$ref = array();
	$ref_id = array();
