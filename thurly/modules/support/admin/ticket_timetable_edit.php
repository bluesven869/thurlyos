	...
	<!--
		...
		function Copy(i, j)
		{
			...
			if(nextFieldF==null)
			{
				...
				'<img src="/thurly/images/support/copy.gif" style="vertical-align:middle" width="15" height="15" border=0 hspace="2" alt="<? echo GetMessage("MAIN_ADMIN_MENU_COPY"); ?>"></a>';*/
				...
				oCellF.innerHTML = '<input id="MINUTE_FROM_'+i+'_'+j +'" type="text" title="" size="4" value="'+vF+'" name="ArrShedule['+i+'][CUSTOM_TIME]['+j+'][MINUTE_FROM]">' +
						...
						'<img src="/thurly/images/support/copy.gif" style="vertical-align:middle" width="15" height="15" border=0 hspace="2" alt="<? echo GetMessage("SUP_ADMIN_ROW_COPY"); ?>">' +
						'<a href="javascript: DeleteTabRow(\'' + oRow.id +'\')"><img src="/thurly/images/support/cross.png" style="vertical-align:middle" width="15" height="15" border=0 hspace="2" alt="<? echo GetMessage("SUP_ADMIN_ROW_DELETE"); ?>"></a>' +
						'</a>';
		...
		<?
			...
			foreach($arShedule[$i]["CUSTOM_TIME"] as $k => $v)
			{
				...
				<td' . $styleFirstRow . ' align="left">'.
					...
					'<a title="' . GetMessage("MAIN_ADMIN_MENU_COPY") . '" href="javascript: Copy(' . $i . ',' . $j . ')"><img src="/thurly/images/support/copy.gif" style="vertical-align:middle" width="15" height="15" border=0 hspace="2" alt="' . GetMessage("SUP_ADMIN_ROW_COPY") . '"></a>' .
					($j > 0 ? '<a href="javascript: DeleteTabRow(\'' . $trID . '\')"><img src="/thurly/images/support/cross.png" style="vertical-align:middle" width="15" height="15" border=0 hspace="2" alt="' . GetMessage("SUP_ADMIN_ROW_DELETE") . '"></a>' : '') .
				'</td></tr>';
				$styleFirstRow = "";
			}
...
class CSupportPage
{
	...
	static function GetPost()
	{
		...
		if(check_thurly_sessid() && $_SERVER["REQUEST_METHOD"] == "POST" )
		{
			// Get data from POST
	...
	static function Save()
	{
		...
		if($presSave || $presApply)
		{
			...
			if(self::$id > 0)
			{
				if(!$presApply)
				{
					LocalRedirect("/thurly/admin/" . self::LIST_URL . "?lang=". LANG . GetFilterParams("filter_", false));
				}
				return true;
			}
		}
		...
		if(!self::$notSaved)
		{
			$aContext[] = 	array(
				...
				"LINK" =>	"javascript:if(confirm('" . GetMessage("SUP_CONFIRM_DEL_MESSAGE") . "'))window.location='" . self::LIST_URL . "?lang=" . LANG .
							"&action=delete&ID=" . self::$timeTableFields->ID . "&" . thurly_sessid_get() . urlencode(GetFilterParams("filter_", false)) . "';",
			);
	...
	/*static function ShowClock($arrREQ, $jsonON = false)
	{
		...
		if($jsonON)	
		{
			$res = array( 
				...
				'<nobr>&nbsp;-&nbsp;</nobr>',
				$clock[2],
				'<a title="' . GetMessage("MAIN_ADMIN_MENU_COPY") . '" href="javascript: Copy(' . $i . ',' . $j . ')"><img src="/thurly/images/support/copy.gif" width="15" height="15" border=0 hspace="2" alt="' . GetMessage("MAIN_ADMIN_MENU_COPY") . '"></a>'
			);
