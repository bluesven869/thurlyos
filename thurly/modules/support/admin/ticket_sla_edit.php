...
$LIST_URL = "/thurly/admin/ticket_sla_list.php";
...
if ((strlen($save)>0 || strlen($apply)>0) && $bAdmin=="Y" && $_SERVER["REQUEST_METHOD"]=="POST" && check_thurly_sessid())
{
...
if(intval($ID)>0)
{
	...
	if ($ID>1 && $bAdmin=="Y")
	{
		...
		"ICON"	=> "btn_delete",
			...
			"LINK"	=> "javascript:if(confirm('".GetMessage("SUP_DELETE_RECORD_CONFIRM")."'))window.location='".$LIST_URL."?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		);
	}
}
...
?>
	...
	<tr valign="top"> 
		...
		if (intval($str_CREATED_USER_ID)>0) :
			...
			echo "&nbsp;&nbsp;[<a title=\"".GetMessage("MAIN_ADMIN_MENU_EDIT")."\" href=\"/thurly/admin/user_edit.php?ID=".$str_CREATED_USER_ID."\">".$str_CREATED_USER_ID."</a>]&nbsp;(".htmlspecialcharsbx($arUser["LOGIN"]).") ".htmlspecialcharsbx($arUser["NAME"])." ".htmlspecialcharsbx($arUser["LAST_NAME"]);
		endif;
		if (intval($str_CREATED_GUEST_ID)>0 && IsModuleInstalled("statistic")) :
			echo "&nbsp;[<a href=\"/thurly/admin/guest_list.php?find_id=".$str_CREATED_GUEST_ID."&find_id_exact_match=Y&set_filter=Y\">".$str_CREATED_GUEST_ID."</a>]";
		endif;
		?></td>
	</tr>
	<?endif;?>
	...
	<tr valign="top"> 
		...
		if (intval($str_MODIFIED_USER_ID)>0) :
			...
			echo "&nbsp;&nbsp;[<a title=\"".GetMessage("MAIN_ADMIN_MENU_EDIT")."\" href=\"/thurly/admin/user_edit.php?ID=".$str_MODIFIED_USER_ID."\">".$str_MODIFIED_USER_ID."</a>]&nbsp;(".htmlspecialcharsbx($arUser["LOGIN"]).") ".htmlspecialcharsbx($arUser["NAME"])." ".htmlspecialcharsbx($arUser["LAST_NAME"]);
		endif;
		if (intval($str_MODIFIED_GUEST_ID)>0 && IsModuleInstalled("statistic")) :
			echo "&nbsp;[<a href=\"/thurly/admin/guest_list.php?find_id=".$str_MODIFIED_GUEST_ID."&find_id_exact_match=Y&set_filter=Y\">".$str_MODIFIED_GUEST_ID."</a>]";
		endif;
		?></td>
	</tr>
	<?endif;?>
		...
		<td>
			...
			while(list($sid, $arrS) = each($arrSites)):
				...
				<div class="adm-list-item">
					...
					<div class="adm-list-label"><label for="<?=htmlspecialcharsbx($sid)?>"><?echo '[<a title="'.GetMessage("MAIN_ADMIN_MENU_EDIT").'" href="/thurly/admin/site_edit.php?LID='.htmlspecialcharsbx($sid).'&lang='.LANGUAGE_ID.'">'.htmlspecialcharsex($sid).'</a>]&nbsp;'.htmlspecialcharsex($arrS["NAME"])?></label></div>
				</div>
				<?
			endwhile;
			?></div>
		...
		<td width="60%">
			...
			while($ar = $rs->Fetch()):
					...
					in_array(CTicket::GetAdminRoleID(),$arRoles))
				{
					...
					" [<a title=\"".GetMessage("MAIN_ADMIN_MENU_EDIT")."\" href=\"/thurly/admin/group_edit.php?ID=".intval($ar["ID"])."&lang=".LANGUAGE_ID."\">".
					intval($ar["ID"]).
					"</a>]</div></div>";
				}
			endwhile;
...
$tabControl->BeginNextTab();?>
	...
	function Copy(i, j)
	{
		...
		if(counter.value==j)
		{
			...
			oCell.innerHTML = '<a title="<?=GetMessage("MAIN_ADMIN_MENU_COPY")?>" href="javascript: Copy('+i+', '+j+')"><img src="/thurly/images/support/copy.gif" width="15" height="15" border=0 hspace="2" alt="<?=GetMessage("MAIN_ADMIN_MENU_COPY")?>"></a>';
					...
					<td align="center" nowrap>
						<table border="0" cellspacing="0" cellpadding="0" width="100%" id="table<?=$i?>">
							...
							<tr>
								...
								<td><a title="<?=GetMessage("MAIN_ADMIN_MENU_COPY")?>" href="javascript: Copy(<?=$i?>, <?=$j?>)"><img src="/thurly/images/support/copy.gif" width="15" height="15" border=0 hspace="2" alt="<?=GetMessage("MAIN_ADMIN_MENU_COPY")?>"></a></td>
							</tr>
							<?
									$j++;
								endforeach;
					...
					foreach($arCategory as $ar)
					{
						...
						?><div class="adm-list-item" id="category_<?=$ar["ID"]?>"><?
							echo "<div class=\"adm-list-control\">".InputType("checkbox", "arCATEGORIES[]", $ar["ID"], $arCATEGORIES, false, "", "", "category_chbox_".$idR)."</div>".
								...
								" [<a title=\"".GetMessage("MAIN_ADMIN_MENU_EDIT")."\" href=\"/thurly/admin/ticket_dict_edit.php?ID=".intval($ar["ID"])."&find_type=C&lang=".LANGUAGE_ID."\">".
								intval($ar["ID"]).
								"</a>]</div>";
						?></div><?
					}?>
