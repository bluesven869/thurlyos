...
function fill_all_values($sid, $type, $mess=false, $site=false)
{
	...
	while ($zr = $z->Fetch())
	{
		$arrTickets[$sid][$zr["ID"]]["NAME"] = "[<a title='".GetMessage("MAIN_ADMIN_MENU_EDIT")."' href='/thurly/admin/ticket_dict_edit.php?ID=".$zr["ID"]."'>".$zr["ID"]."</a>] ".htmlspecialcharsbx($zr["NAME"]);
...
while ($arTicket = $rsTickets->Fetch())
{
	...
	if (intval($arTicket["RESPONSIBLE_USER_ID"])>0)
	{
		...
		//$R_NAME = "[<a href='/thurly/admin/user_edit.php?ID=".$arTicket["RESPONSIBLE_USER_ID"]."'>".$arTicket["RESPONSIBLE_USER_ID"]."</a>] (".htmlspecialcharsbx($arUser["LOGIN"]).") ".htmlspecialcharsbx($arUser["NAME"])." ".htmlspecialcharsbx($arUser["LAST_NAME"]);
	}
	else
	{
...
if(count($arUsersID) > 0)
{
	...
	while($ar = $rs->Fetch())
	{
		$arrT["RESPONSIBLE"][$ar["ID"]]["NAME"] = "[<a href='/thurly/admin/user_edit.php?ID=" . $ar["ID"] . "'>" . $ar["ID"] . "</a>] " .
...
<table border="0" cellspacing="1" cellpadding="3" width="100%" class="list-table">
	...
	<tr valign="top" class="head">
		...
		<td align="right">&nbsp;<?if (intval($OPEN_TICKETS_RED)>0):?><a href="/thurly/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_lamp[]=red&find_lamp[]=yellow&set_filter=Y<?=$tlist_filter_pass?>"><?echo intval($OPEN_TICKETS_RED)?></a><?endif;?></td>

		<td align="right">&nbsp;<?if(intval($OPEN_TICKETS_GREEN)>0):?><a href="/thurly/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_lamp[]=green&find_lamp[]=green_s&set_filter=Y<?=$tlist_filter_pass?>"><?echo intval($OPEN_TICKETS_GREEN)?></a><?endif;?></td>



		<td align="right">&nbsp;<?if(intval($CLOSE_TICKETS)>0):?><a href="/thurly/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_close=Y&set_filter=Y<?=$tlist_filter_pass?>"><?=$CLOSE_TICKETS?></a><?endif;?></td>
		...
		<td align="right">&nbsp;<?
			if (intval($CLOSE_OVERDUE_MESSAGES)>0):
				?><a title="<?=GetMessage("SUP_OVERDUE_MESSAGES")?>" href="/thurly/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_overdue_messages1=1&find_close=Y&set_filter=Y<?=$tlist_filter_pass?>" ><?=intval($CLOSE_OVERDUE_MESSAGES)?></a><?
			endif;
			?></td>
		<?endif;?>

		<td align="right">&nbsp;<?if(intval($OPEN_TICKETS)>0):?><a href="/thurly/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_close=N&set_filter=Y<?=$tlist_filter_pass?>"><?=$OPEN_TICKETS?></a><?endif;?></td>
		...
		<td align="right">&nbsp;<?
			if(intval($OPEN_OVERDUE_MESSAGES)>0):
				?><a title="<?=GetMessage("SUP_OVERDUE_MESSAGES")?>" href="/thurly/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_overdue_messages1=1&find_close=N&set_filter=Y<?=$tlist_filter_pass?>" ><?=intval($OPEN_OVERDUE_MESSAGES)?></a><?
			endif;
			?></td>
		<?endif;?>


		<td align="right">&nbsp;<?if (intval($OPEN_TICKETS+$CLOSE_TICKETS)>0):?><a href="/thurly/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&del_filter=Y<?=$tlist_filter_pass?>"><?=$OPEN_TICKETS+$CLOSE_TICKETS?></a><?endif;?></td>
		...
		<td align="right">&nbsp;<?
			if (intval($OPEN_OVERDUE_MESSAGES+$CLOSE_OVERDUE_MESSAGES)>0):
				?><a title="<?=GetMessage("SUP_OVERDUE_MESSAGES")?>" href="/thurly/admin/ticket_list.php?lang=<?=LANGUAGE_ID?>&find_overdue_messages1=1&set_filter=Y<?=$tlist_filter_pass?>" ><?=intval($OPEN_OVERDUE_MESSAGES+$CLOSE_OVERDUE_MESSAGES)?></a><?
			endif;
			?></td>
		<?endif;?>
	</tr>
...
<tr>
	...
	<td><?
		if ($bAdmin=="Y" || $bDemo=="Y")	:
			...
			?>[<a href="/thurly/admin/user_edit.php?ID=<?=$USER->GetID()?>"><?=$USER->GetID()?></a>] (<?=htmlspecialcharsEx($USER->GetLogin())?>) <?=htmlspecialcharsEx($USER->GetFullName())?><?
		endif;
		?></td>
</tr>
