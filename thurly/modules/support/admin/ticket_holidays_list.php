...
{    
	$row =& $lAdmin->AddRow($f_ID, $arBlog, "/thurly/admin/ticket_holidays_edit.php?ID=".$f_ID."&lang=".LANGUAGE_ID, GetMessage("SUP_UPDATE_ALT"));
	$row->AddField("NAME", '<a href="/thurly/admin/ticket_holidays_edit.php?ID=' . $f_ID . '&lang=' . LANGUAGE_ID . '" title="' . GetMessage("SUP_UPDATE_ALT").'">'.$f_NAME.'</a>');
	...
	while($arR = $rs->Fetch())
	{
		$SLA .= '<a href="/thurly/admin/ticket_sla_edit.php?ID=' . intval($arR["SLA_ID"]) . '&lang=' . LANGUAGE_ID . '">' . htmlspecialcharsbx($arR["NAME"]) . '</a><br/>';
	}
