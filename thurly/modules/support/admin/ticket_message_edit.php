...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/support/admin/ticket_message_edit.php");
...
if ($arTicket = $rsTicket->Fetch())
{
	...
	if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $bAdmin=="Y" && $ID>0 && $TICKET_ID>0 && check_thurly_sessid())
	{
...
$aMenu[] = array(
	...
	"LINK"	=> "/thurly/admin/ticket_edit.php?lang=".LANGUAGE_ID."&ID=".$TICKET_ID
	);
...
if (intval($arTicket["MESSAGES"])>1)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("SUP_DELETE_MESSAGE_CONFIRM")."')) window.location='/thurly/admin/ticket_edit.php?ID=".$TICKET_ID."&mdel_id=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get(). "&set_default=Y';",
		"WARNING"=>"Y"
		);
}
...
?>
	...
	<tr valign="top">
		...
		if (strlen($str_CREATED_MODULE_NAME)<=0 || $str_CREATED_MODULE_NAME=="support") :
			?>[<a title="<?=GetMessage("SUP_USER_PROFILE")?>" href="/thurly/admin/user_edit.php?lang=<?=LANG?>&ID=<?=$str_CREATED_USER_ID?>"><?echo $str_CREATED_USER_ID?></a>] (<?=$str_CREATED_LOGIN?>) <?=$str_CREATED_NAME?><?
			if (intval($str_CREATED_GUEST_ID)>0 && CModule::IncludeModule("statistic")) :
				echo " [<a title='".GetMessage("SUP_GUEST_ID")."'  href='/thurly/admin/guest_list.php?lang=".LANG."&find_id=". $str_CREATED_GUEST_ID."&find_id_exact_match=Y&set_filter=Y' >".$str_CREATED_GUEST_ID."</a>]";
			endif;
		else :
	...
	<tr valign="middle">
		...
		<td align="left"><?=$str_TIMESTAMP_X?>&nbsp;&nbsp;&nbsp;<?
			?>[<a title="<?=GetMessage("SUP_USER_PROFILE")?>" href="/thurly/admin/user_edit.php?lang=<?=LANG?>&ID=<?echo $str_MODIFIED_USER_ID?>"><?=$str_MODIFIED_USER_ID?></a>] (<?=$str_MODIFIED_LOGIN?>) <?=$str_MODIFIED_NAME?><?
		...
		<td><table cellspacing=0 cellpadding=0 border=0>
				...
				<tr>
					<td><? 
						...
						?>)&nbsp;&nbsp;[&nbsp;<a href="/thurly/tools/ticket_show_file.php?hash=<?echo $arFile["HASH"]?>&lang=<?=LANG?>&action=download"><?echo GetMessage("SUP_DOWNLOAD")?></a>&nbsp;]&nbsp;&nbsp;<?=GetMessage("SUP_DELETE")?>:<input type="checkbox" name="EXIST_FILE_<?=$arFile["ID"]?>_del" value="Y"></td>
				</tr>
				<tr>
