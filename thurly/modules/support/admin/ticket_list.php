...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/support/admin/ticket_list.php");
...
if (strlen($tf)<=0)
{
	$tf = ${COption::GetOptionString("main", "cookie_name", "THURLY_SM")."_TICKET_FILTER"};
}
if (strlen($tf)<=0)
{
	$tf = "none";
...
{
	$lamp = "/thurly/images/support/$f_LAMP.gif";
	...
	{
		$ID_HTML .= '<img src="/thurly/images/1.gif" width="1" height="3"><br>
	...
	if (COption::GetOptionString('support', "SHOW_COMMENTS_IN_TICKET_LIST") == 'Y')
	{
		if (($bADS) && strlen(trim($f_SUPPORT_COMMENTS))>0)
			$TITLE_HTML .= '<br><img src="/thurly/images/1.gif" width="1" height="5" border="0" alt=""><br>[&nbsp;'.TxtToHTML($f_SUPPORT_COMMENTS, true, 30).'&nbsp;]';
	}
	...
	if ($bADS)
	{
		...
		if (intval($f_OWNER_USER_ID)>0)
			$DATE_CREATE_HTML .= '[<a title="'.GetMessage("SUP_USER_PROFILE").'" href="/thurly/admin/user_edit.php?lang='.LANG.'&ID='.$f_OWNER_USER_ID.'">'.$f_OWNER_USER_ID.'</a>]  ('.$f_OWNER_LOGIN.') '.$f_OWNER_NAME;
	...
	{
		if ($bADS)
			$TIMESTAMP_X_HTML .= '[<a title="'.GetMessage("SUP_USER_PROFILE").'" href="/thurly/admin/user_edit.php?lang='.LANG.'&ID='.$f_MODIFIED_USER_ID.'">'.$f_MODIFIED_USER_ID.'</a>] ('.$f_MODIFIED_LOGIN.') '.$f_MODIFIED_NAME;
		else
	...
	*/
	...
	if (strlen($f_DATE_CLOSE)<=0)
	{
		$arActions[] = array(
			...
			"ACTION"=>$lAdmin->ActionAjaxReload("/thurly/admin/ticket_list.php?ID=".$f_ID."&action=close&lang=".LANGUAGE_ID."&".thurly_sessid_get()),
			);
	}
	else
	{
		$arActions[] = array(
			...
			"ACTION"=>$lAdmin->ActionAjaxReload("/thurly/admin/ticket_list.php?ID=".$f_ID."&action=open&lang=".LANGUAGE_ID."&".thurly_sessid_get())
			);
	}
	...
	if ($bSupportTeam=="Y" || $bAdmin=="Y")
	{
		if (strlen($f_IS_SPAM)>0)
		{
			$arActions[] = array(
				...
				//"LINK"	=> "/thurly/admin/ticket_edit.php?ID=".$ID."&action=unmark_spam&lang=".LANGUAGE_ID."&".thurly_sessid_get(),
				"ACTION"=>$lAdmin->ActionAjaxReload("/thurly/admin/ticket_list.php?ID=".$f_ID."&action=unmark_spam&lang=".LANGUAGE_ID."&".thurly_sessid_get())
				);
		}
		...
		if ($f_IS_SPAM!="N")
		{
			$arActions[] = array(
				...
				//"LINK"	=> "/thurly/admin/ticket_edit.php?ID=".$ID."&action=maybe_spam&lang=".LANGUAGE_ID."&".thurly_sessid_get()
				"ACTION"=>$lAdmin->ActionAjaxReload("/thurly/admin/ticket_list.php?ID=".$f_ID."&action=maybe_spam&lang=".LANGUAGE_ID."&".thurly_sessid_get())
				);
		}
		...
		if ($f_IS_SPAM!="Y" && $bAdmin=="Y")
		{
			$arActions[] = array(
				...
				//"LINK"	=> "/thurly/admin/ticket_edit.php?ID=".$ID."&action=mark_spam&lang=".LANGUAGE_ID."&".thurly_sessid_get()
				"ACTION"=>$lAdmin->ActionAjaxReload("/thurly/admin/ticket_list.php?ID=".$f_ID."&action=mark_spam&lang=".LANGUAGE_ID."&".thurly_sessid_get())
				);
		}
		...
		if ($bAdmin=="Y" || $bDemo=="Y")
		{
			$arActions[] = array(
				...
				//"LINK"	=> "javascript:if(confirm('".GetMessage("SUP_MARK_AS_SPAM_DELETE_CONFIRM")."')) window.location='/thurly/admin/ticket_list.php?ARR_TICKET[]=".$ID."&action=mark_spam_delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."&set_default=Y';",
				//"WARNING"=>"Y",
			...
			$arActions[] = array(
				...
				//"LINK"	=> "javascript:if(confirm('".GetMessage("SUP_DELETE_TICKET_CONFIRM")."')) window.location='/thurly/admin/ticket_list.php?ARR_TICKET[]=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."&set_default=Y';",
				//"WARNING"=>"Y",
