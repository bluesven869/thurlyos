...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/support/prolog.php");
...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/support/include.php");
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/support/admin/ticket_edit.php");
...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && check_thurly_sessid())
{
	$arFILES = array();
...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/ticket_list.php?lang=".LANGUAGE_ID,
		"ICON" => "btn_list",
	),
);
...
if(intval($ID)>0)
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_new",
		"LINK"	=> "/thurly/admin/ticket_edit.php?lang=".LANGUAGE_ID,
		);
	...
	if (strlen($str_DATE_CLOSE)<=0)
	{
		$aMenu[] = array(
			...
			"LINK"	=> "/thurly/admin/ticket_edit.php?ID=".$ID."&action=close&lang=".LANGUAGE_ID."&".thurly_sessid_get()
			);
	}
	else
	{
		$aMenu[] = array(
			...
			"LINK"	=> "/thurly/admin/ticket_edit.php?ID=".$ID."&action=open&lang=".LANGUAGE_ID."&".thurly_sessid_get()
			);
	}
	...
	if ($bSupportTeam=="Y" || $bAdmin=="Y")
	{
		...
		if (strlen($str_IS_SPAM)>0)
		{
			$arSpamMenu[] = array(
				...
				"ACTION"	=> "window.location='/thurly/admin/ticket_edit.php?ID=".$ID."&action=unmark_spam&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';"
				);
		}
		...
		if ($str_IS_SPAM!="N")
		{
			$arSpamMenu[] = array(
				...
				"ACTION"	=> "window.location='/thurly/admin/ticket_edit.php?ID=".$ID."&action=maybe_spam&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';"
				);
		}
		...
		if ($str_IS_SPAM!="Y" && $bAdmin=="Y")
		{
			$arSpamMenu[] = array(
				...
				"ACTION"	=> "window.location='/thurly/admin/ticket_edit.php?ID=".$ID."&action=mark_spam&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';"
				);
		}
		...
		if ($bAdmin=="Y" || $bDemo=="Y")
		{
			$arSpamMenu[] = array(
				...
				"ACTION"	=> "javascript:if(confirm('".GetMessage("SUP_MARK_AS_SPAM_DELETE_CONFIRM")."')) window.location='/thurly/admin/ticket_list.php?ID=".$ID."&action=mark_spam_delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
				);
			...
			$aMenu[] = array(
				...
				"ICON" => "btn_delete",
				"LINK"	=> "javascript:if(confirm('".GetMessage("SUP_DELETE_TICKET_CONFIRM")."')) window.location='/thurly/admin/ticket_list.php?ID=".$ID."&action=delete&redirectafter=Y&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
				);
		}
...
?>
	...
	<tr>
		...
		<td width="80%" nowrap><?
			...
			/*if (intval($str_OWNER_USER_ID)>0)
			{
				$owner_name = "[<a title=\"".GetMessage("SUP_USER_PROFILE")."\" href=\"/thurly/admin/user_edit.php?lang=".LANGUAGE_ID."&ID=".$str_OWNER_USER_ID."\">".$str_OWNER_USER_ID."</a>] (".$str_OWNER_LOGIN.") ".$str_OWNER_NAME;
			}
			*/
			if(intval($str_OWNER_USER_ID)>0)
			{
			...
			if ($ID>0):
				?><br>[&nbsp;<a href="/thurly/admin/ticket_list.php?set_filter=Y&lang=<?=LANGUAGE_ID?>&<?=implode("&",$arAuthorFilter)?>"><?=GetMessage("SUP_AUTHOR_TICKETS")?></a>&nbsp;]<?
			endif;
			?>
		</td>
	</tr>
	...
	<tr>
		...
		if ($bAdmin=="Y" || $bDemo=="Y" || $bSupportTeam=="Y")
		{
			if (intval($str_OWNER_USER_ID)>0)
			{
				/*?>[<a title="<?echo GetMessage("SUP_USER_PROFILE")?>" href="/thurly/admin/user_edit.php?lang=<?echo LANG?>&ID=<?=$str_OWNER_USER_ID?>"><?echo $str_OWNER_USER_ID?></a>]  (<?=$str_OWNER_LOGIN?>) <?=$str_OWNER_NAME?> */
			...
			{
				/*echo " [<a title='".GetMessage("SUP_GUEST_ID")."'  href='/thurly/admin/guest_list.php?lang=".LANG."&find_id=".$str_OWNER_GUEST_ID."&find_id_exact_match=Y&set_filter=Y'>".$str_OWNER_GUEST_ID."</a>]";*/
	...
	<tr valign="middle">
		...
		if (strlen($str_CREATED_MODULE_NAME)<=0 || $str_CREATED_MODULE_NAME=="support")
		{
			...
			if ($bAdmin=="Y" || $bDemo=="Y" || $bSupportTeam=="Y")
			{
				...
				/*?>[<a title="<?=GetMessage("SUP_USER_PROFILE")?>" href="/thurly/admin/user_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$str_CREATED_USER_ID?>"><?echo $str_CREATED_USER_ID?></a>] (<?=$str_CREATED_LOGIN?>) <?=$str_CREATED_NAME?> <?=$arrSUPPORT_TEAM[$str_CREATED_USER_ID]?><?*/

				if ($uid <= 0 && intval($str_CREATED_GUEST_ID) > 0 && CModule::IncludeModule("statistic"))
				{
					/*echo " [<a title='".GetMessage("SUP_GUEST_ID")."'  href='/thurly/admin/guest_list.php?lang=".LANG."&find_id=". $str_CREATED_GUEST_ID."&find_id_exact_match=Y&set_filter=Y' class='tablebodylink'>".$str_CREATED_GUEST_ID."</a>]";*/
	...
	<tr valign="middle">
		...
		if (strlen($str_MODIFIED_MODULE_NAME)<=0 || $str_MODIFIED_MODULE_NAME=="support")
		{
			...
			if ($bAdmin=="Y" || $bDemo=="Y" || $bSupportTeam=="Y")
			{
				...
				/*?>[<a title="<?=GetMessage("SUP_USER_PROFILE")?>" href="/thurly/admin/user_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?echo $str_MODIFIED_USER_ID?>"><?=$str_MODIFIED_USER_ID?></a>] (<?=$str_MODIFIED_BY_LOGIN?>) <?=$str_MODIFIED_BY_NAME?> <?=$arrSUPPORT_TEAM[$str_MODIFIED_USER_ID]?><?*/
				...
				if($uid <= 0 && intval($str_MODIFIED_GUEST_ID)>0 && CModule::IncludeModule("statistic"))
				{
					...
					/*echo " [<a title='".GetMessage("SUP_GUEST_ID")."'  href='/thurly/admin/guest_list.php?lang=".LANG."&find_id=".$str_MODIFIED_GUEST_ID."&find_id_exact_match=Y&set_filter=Y' >".$str_MODIFIED_GUEST_ID."</a>]";*/
				}
				elseif($uid > 0)
				{
					echo $arStrUsers["arUsers"][$uid]["HTML_NAME"] . " " . $arrSUPPORT_TEAM[$uid];
	...
	<?if ($ID>0 && IsModuleInstalled("sale")): 
		...
		if ($saleModulePermissions > "D"):?>
			<tr>
				...
				<td width="80%" nowrap>[ <a href="/thurly/admin/sale_order.php?lang=<?=LANGUAGE_ID?>&set_filter=Y&filter_user_id=<?=$str_CREATED_USER_ID?>" target="_blank"><?=GetMessage("SUP_SALE_ORDER_LIST")?></a> ]</td>
			</tr>
	<?  endif;
	endif;?>
												...
												if ($bAdmin=="Y" || $bDemo=="Y") :
													...
													if (intval($arM["SOURCE_ID"])>0 && IsModuleInstalled("mail") && intval($arM["EXTERNAL_ID"])>0)
													{
														...
														if (strtolower($arM["SOURCE_SID"])=="email")
														{
															...
															echo '<a title="' . GetMessage("SUP_VIEW_SOURCE_ALT") . '" href="/thurly/admin/mail_message_view.php?lang=' . LANGUAGE_ID . '&ID=' . $arM["EXTERNAL_ID"] . '">' . GetMessage("SUP_VIEW_SOURCE") . '</a>';
															$bSep = true;
														}
													...
													if ($messages>1)
													{
														echo ' | ' . '<a title="' . GetMessage("SUP_DELETE_ALT") . '" ' .
															...
															'window.location=\'' . urlencode(urlencode($TICKET_EDIT_URL)) . '?lang=' . LANGUAGE_ID . '&ID=' . $ID . '&mdel_id=' . $arM["ID"] . '&' . thurly_sessid_get() . '\'">' . GetMessage("SUP_DELETE") . '</a>';
													}
											...
											<td  style="padding:4px;text-align:left;<? echo $backcolor; ?>"><?
												...
												if($arM["IS_LOG"] != "Y")
												{
													...
													if($bAdmin=="Y" || $bDemo=="Y" || $bSupportTeam=="Y")
													{
														/*if($oUID > 0)
														{
															?>[<a title="<?=GetMessage("SUP_USER_PROFILE")?>" href="/thurly/admin/user_edit.php?lang=<?echo LANG?>&ID=<?echo $arM["OWNER_USER_ID"]; ?>"><?echo $arM["OWNER_USER_ID"]; ?></a>] (<? echo $arM["OWNER_LOGIN"]; ?>) <? echo $arM["OWNER_USER_NAME"]; ?>
														...
														if($oUID <= 0 && $oGID > 0 && CModule::IncludeModule("statistic"))
														{
															...
															<?
															//echo " [<a title='".GetMessage("SUP_GUEST_ID")."'  href='/thurly/admin/guest_list.php?lang=" . LANG . "&find_id=" . $arM["OWNER_GUEST_ID"] . "&find_id_exact_match=Y&set_filter=Y'>" . $arM["OWNER_GUEST_ID"] . "</a>]";
														}
														elseif($oUID > 0)
														{
												...
												else
												{
													...
													{
														/*?>[<a title="<?=GetMessage("SUP_USER_PROFILE")?>" href="/thurly/admin/user_edit.php?lang=<?=LANGUAGE_ID?>&ID=<? echo $arM["CREATED_USER_ID"]; ?>"><? echo $arM["CREATED_USER_ID"]; ?></a>] (<? echo $arM["CREATED_LOGIN"]; ?>) <? echo $arM["CREATED_USER_NAME"]; ?> <? echo $arrSUPPORT_TEAM[$arM["CREATED_USER_ID"]]; ?><?*/

														if ($cUID <= 0 && $cGID > 0 && CModule::IncludeModule("statistic"))
														{
															echo $arStrUsersM["arGuests"][$cGID]["HTML_NAME"];
															echo ($cUID > 0 ? " " . $arrSUPPORT_TEAM[$cUID] : "");
															/*echo " [<a title='".GetMessage("SUP_GUEST_ID")."'  href='/thurly/admin/guest_list.php?lang=".LANG."&find_id=". $arM["CREATED_GUEST_ID"]."&find_id_exact_match=Y&set_filter=Y'>".$arM["CREATED_GUEST_ID"]."</a>]";*/
														}
														elseif($cUID > 0)
														{
														...
														<table cellspacing=0 cellpadding=0 style="width:0%;">
															<tr>
																<td <?=$headerbackcolor?> valign="top" style="width:0%;" nowrap>
																	<img src="/thurly/images/support/paperclip.gif" width="16" height="16" border="0" alt="">
																</td>
																<td <?=$headerbackcolor?> style="width:0%;text-align:left;" nowrap><?
																	...
																	foreach ($ALL_TICKET_FILES[$arM["ID"]] as $arFile)
																	{
																		if(in_array(strtolower(GetFileExtension($arFile["NAME"])), $aImg)):
																			?><a title="<?=GetMessage("SUP_VIEW_ALT")?>" target="_blank" href="/thurly/tools/ticket_show_file.php?hash=<?echo $arFile["HASH"]?>&lang=<?=LANGUAGE_ID?>"><?echo htmlspecialcharsbx($arFile["NAME"])?></a>
																			<?else:?>
																...
																<td <?=$headerbackcolor?> style="width:0%;" nowrap><?
																	foreach ($ALL_TICKET_FILES[$arM["ID"]] as $arFile)
																	{
																		...
																		?>&nbsp;[<a title="<?=$alt?>" href="/thurly/tools/ticket_show_file.php?hash=<?echo $arFile["HASH"]?>&lang=<?=LANGUAGE_ID?>&action=download"><?echo GetMessage("SUP_DOWNLOAD")?></a>]<br><?
																	}
																	?></td>
															</tr>
														</table>
...
?>
	...
	document.getElementById("online_frame").src = "/thurly/admin/ticket_online.php?TICKET_ID=<?=$ID?>&OWNER_USER_ID=<?=intval($str_OWNER_USER_ID)?>&lang=<?=LANGUAGE_ID?>&mode="+mode+"&ONLINE_AUTO_REFRESH=<?=$ONLINE_AUTO_REFRESH?>";
...
<input type="button" name="FindUser" id="FindUser" OnClick="window.open('/thurly/admin/user_search.php?lang=<?=LANGUAGE_ID?>&FN=form1&FC=MESSAGE_AUTHOR_USER_ID', '', 'scrollbars=yes,resizable=yes,width=760,height=560,top='+Math.floor((screen.height - 560)/2-14)+',left='+Math.floor((screen.width - 760)/2-5));" value="..."-->
...
<?if (strlen($str_DATE_CLOSE)<=0):?>
	...
	<?require($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/support/admin/ticket_message_js.php");?>
		...
		<td align="right">
			<table cellspacing=0 cellpadding=0 width="100%" border=0>
				<tr>
					...
					<td style="padding-top:13px;" width="100%" align="left"><a target="online_frame" href="/thurly/admin/ticket_online.php?TICKET_ID=<?=$ID?>&OWNER_USER_ID=<?=intval($str_OWNER_USER_ID)?>&lang=<?=LANGUAGE_ID?>&ONLINE_AUTO_REFRESH=<?=$ONLINE_AUTO_REFRESH?>"><img src="/thurly/images/support/refresh.gif" width="14" height="14" border="0" alt="<?=GetMessage("SUP_REFRESH_ALT")?>"></a></td>
				</tr>
				<tr>
					<td colspan="2" width="100%" nowrap><iframe name="online_frame" id="online_frame" frameborder="0" style="width:100%; border:0; height:300px;" src="/thurly/admin/ticket_online.php?TICKET_ID=<?=$ID?>&OWNER_USER_ID=<?=intval($str_OWNER_USER_ID)?>&lang=<?=LANGUAGE_ID?>&mode=<?=$default_mode?>&ONLINE_AUTO_REFRESH=<?=$ONLINE_AUTO_REFRESH?>"></iframe></td>
				</tr>
			</table></td>
		<?endif;?>
	...
	<tr valign="top">
		...
		<td style="padding:0px" nowrap id="edit_9">
			...
			<table cellspacing=0 cellpadding=0 border=0 id="files_table">
				...
				<tr>
					<td><?
						?><a title="<?=GetMessage("SUP_VIEW_ALT")?>" target="_blank" href="/thurly/tools/ticket_show_file.php?hash=<?echo $arFile["HASH"]?>&lang=<?=LANG?>"><?echo htmlspecialcharsbx($arFile["NAME"])?></a> (<?
						/*$a = array("b", "kb", "mb", "gb");
						$pos = 0;
						$size = $arFile["FILE_SIZE"];
						...
						echo round($size,2)." ".$a[$pos];*/
						echo CFile::FormatSize($arFile["FILE_SIZE"]);
						?>)&nbsp;&nbsp;[&nbsp;<a href="/thurly/tools/ticket_show_file.php?hash=<?echo $arFile["HASH"]?>&lang=<?=LANG?>&action=download"><?echo GetMessage("SUP_DOWNLOAD")?></a>&nbsp;]&nbsp;&nbsp;<input type="checkbox" name="ATTACH_FILE[]" value="<?=$arFile["ID"]?>" checked>
					</td>
				</tr>
					<?
					}
...
?>
	...
	<tr valign="middle">
		...
		<td id="edit_19"><?
			...
			?><?if ($can_select_responsible=="Y"):?>&nbsp;&nbsp;<a title="<?=GetMessage("SUP_RESPONSIBLE_SELECT_BY_CATEGORY_ALT")?>" id="icon_1" href="javascript:SetResponsible('CATEGORY_ID')"><img src="/thurly/images/support/resp.gif" width="16" height="16" border="0" alt="<?=GetMessage("SUP_RESPONSIBLE_SELECT_BY_CATEGORY_ALT")?>"></a><?endif;?></td>
	</tr>
	<?endif;?>
	...
	<tr valign="middle">
		...
		<td id="edit_21" width="80%" nowrap><?
			...
			?><?if ($can_select_responsible=="Y"):?>&nbsp;&nbsp;<a id="icon_2" title="<?=GetMessage("SUP_RESPONSIBLE_SELECT_BY_SLA_ALT")?>" href="javascript:SetResponsible('SLA_ID')"><img src="/thurly/images/support/resp.gif" width="16" height="16" border="0" alt="<?=GetMessage("SUP_RESPONSIBLE_SELECT_BY_SLA_ALT")?>"></a><?endif;?></td>
	</tr>
