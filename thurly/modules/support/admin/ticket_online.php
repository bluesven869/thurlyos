	...
	<tr>
		<td valign="top" width="16%"><?
			if ($is_support=="Y"):
				...
				?><img src="/thurly/images/support/sup_write.gif" width="15" height="13" border="0" alt="<?=GetMessage("SUP_EDIT_MODE_ALT")?>"><?
				else:
				?><img src="/thurly/images/support/sup_view.gif" width="17" height="13" border="0" alt="<?=GetMessage("SUP_VIEW_MODE_ALT")?>"><?
				endif;
			elseif ($is_support=="N"):
				?><img src="/thurly/images/support/client.gif" width="11" height="13" border="0" alt="<?=GetMessage("SUP_CLIENT_MODE_ALT")?>"><?
			endif;
			?></font></td>

		<td width="84%" valign="top"><?
			if (strlen(trim($ar["USER_NAME"]))>0):
				?><a title="<?=GetMessage("SUP_USER_PROFILE")?>" target="_blank" href="/thurly/admin/user_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$ar["USER_ID"]?>"><?=$ar["USER_NAME"]?></a><?
			else:
				?><a title="<?=GetMessage("SUP_USER_PROFILE")?>" target="_blank" href="/thurly/admin/user_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$ar["USER_ID"]?>"><?=$ar["USER_LOGIN"]?></a>
				<?
			endif;
			?></td>
	</tr>
...
if (strlen($lamp)>0) 
{
	...
	//$lamp = "/thurly/images/support/$lamp.gif";?>
