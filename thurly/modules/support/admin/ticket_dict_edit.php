...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $bAdmin=="Y" && check_thurly_sessid())
{
	$arFields = array(
	...
	if ($bOK)
	{
		if (strlen($save)>0) LocalRedirect("/thurly/admin/ticket_dict_list.php?lang=".LANGUAGE_ID. "&find_type=".symbolsAndNumbers($_REQUEST['C_TYPE']));
		elseif ($new) LocalRedirect("/thurly/admin/ticket_dict_edit.php?ID=".$ID. "&lang=".LANGUAGE_ID."&find_type=".symbolsAndNumbers($_REQUEST['C_TYPE'])."&tabControl_active_tab=".urlencode($tabControl_active_tab));
	}
	else
	{
	...
	if (CheckFields())
	{
		...
		if (!$message)
		{
			...
			if (strlen($save)>0) LocalRedirect("/thurly/admin/ticket_dict_list.php?lang=".LANGUAGE_ID. "&find_type=".$str_C_TYPE."&set_filter=Y");
			elseif ($new=="Y") LocalRedirect("/thurly/admin/ticket_dict_edit.php?ID=".$ID. "&lang=".LANGUAGE_ID."&find_type=".$str_C_TYPE."&set_filter=Y&tabControl_active_tab=".urlencode($tabControl_active_tab));
		}
	}
	else
	{
...
$aMenu = array(
	array(
		...
		"LINK"	=> "/thurly/admin/ticket_dict_list.php?lang=".LANGUAGE_ID."&find_type=".symbolsAndNumbers($find_type)
	)
);
...
if(intval($ID)>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "/thurly/admin/ticket_dict_edit.php?lang=".LANGUAGE_ID."&find_type=".symbolsAndNumbers($find_type)
		);
	...
	if ($bAdmin=="Y")
	{
		$aMenu[] = array(
			...
			"LINK"	=> "javascript:if(confirm('".GetMessage("SUP_DELETE_RECORD_CONFIRM")."')) window.location='/thurly/admin/ticket_dict_list.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&find_type=".symbolsAndNumbers($find_type) ."&". thurly_sessid_get()."';",
			);
	}
}
...
<?
$tabControl->Buttons(Array("disabled" => $bAdmin!="Y","back_url" =>"/thurly/admin/ticket_dict_list.php?lang=".LANGUAGE_ID. "&find_type=".$str_C_TYPE));
$tabControl->End();
?>
