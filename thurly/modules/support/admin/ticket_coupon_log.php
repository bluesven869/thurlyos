...
$LIST_URL = '/thurly/admin/ticket_coupon_list.php';
...
while ($arCouponLog = $rsData->GetNext())
{
	...
	if ($bStatIncluded)
	{
		///thurly/admin/guest_list.php?lang=ru&set_filter=Y&find_user_exact_match=N&find_id=33&find_id_exact_match=N&find_id_exact_match=Y&find_country_exact_match=N
		$row->AddViewField('SESSION_ID', '<a href="/thurly/admin/session_list.php?lang='.LANGUAGE_ID.'&amp;set_filter=Y&amp;find_id='.$arCouponLog['SESSION_ID'].'">'.$arCouponLog['SESSION_ID'].'</a>');
		$row->AddViewField('GUEST_ID', '<a href="/thurly/admin/guest_list.php?lang='.LANGUAGE_ID.'&amp;set_filter=Y&amp;find_id_exact_match=Y&amp;find_id='.$arCouponLog['GUEST_ID'].'">'.$arCouponLog['SESSION_ID'].'</a>');
	}
}
