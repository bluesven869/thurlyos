	...
	*****************************************************************/
	...
	public static function Set_getFilesLinks($arFiles, $lID)
	{
		...
		if(is_array($arFiles) && count($arFiles) > 0)
		{
			...
			{
				$fl .= (CMain::IsHTTPS()? "https" : "http")."://" . $_SERVER["HTTP_HOST"] . "/thurly/tools/ticket_show_file.php?hash=" . $arFile["HASH"] . "&action=download&lang=" . $lID . "\n";
			}
			if (strlen($fl) > 0) $fl .= "\n";
		}
		return $fl;
	...
	public static function Set_sendMails($nf, $v, $arFields)
	{
		...
		IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/support/classes/general/messages.php", $v->arrSite["LANGUAGE_ID"]);
					
		$mf = self::GetCSupportTableFields("EventFields");
		
		$mf->ADMIN_EDIT_URL = "/thurly/admin/ticket_edit.php";
				...
				********************************************/
				...
				if($zr = $z->Fetch())
				{
					...
					if(CModule::IncludeModule("statistic"))
					{
						...
						if(strlen($v->T_EVENT3) <= 0) $v->T_EVENT3 = "http://" . $_SERVER["HTTP_HOST"] . "/thurly/admin/ticket_edit.php?ID=" . $f->ID . "&lang=" . $v->arrSite["LANGUAGE_ID"];
	...
	***********************************************/
	...
	public static function GetUsersPropertiesArray($arUserIDs = array(), $arGuestIDs = array())
	{
		...
		foreach($arUserIDs as $k => $v)
		{
			...
			$arResUsers[$v]["HTML_NAME"] = "[<a title=\"" . GetMessage("SUP_USER_PROFILE") . "\" href=\"/thurly/admin/user_edit.php?lang=" . LANGUAGE_ID . "&ID=" . $v . "\">" . $v."</a>] (" . htmlspecialcharsbx($arResUsers[$v]['LOGIN'])  . ") " . $name;
		...
		foreach($arGuestIDs as $k => $v)
		{
			...
			$arResGuests[$k]["HTML_NAME"] = "[<a title=\"" . GetMessage("SUP_USER_PROFILE") . "\" href=\"/thurly/admin/user_edit.php?lang=" . LANGUAGE_ID . "&ID=" . $v . "\">" . $v."</a>] " . $name .
				" [<a title='" . GetMessage("SUP_GUEST_ID") . "'  href='/thurly/admin/guest_list.php?lang=" . LANG . "&find_id=" . $k . "&find_id_exact_match=Y&set_filter=Y'>" . $k . "</a>]";
