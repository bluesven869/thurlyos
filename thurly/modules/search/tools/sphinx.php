...
class CSearchSphinx extends CSearchFullText
{
	...
	public function connect($connectionIndex, $indexName = "", $ignoreErrors = false)
	{
		...
		if (!preg_match("/^[a-zA-Z0-9_]+\$/", $indexName))
		{
			if ($ignoreErrors)
				...
				throw new \Thurly\Main\Db\ConnectionException('Sphinx connect error', GetMessage("SEARCH_SPHINX_CONN_ERROR_INDEX_NAME"));
			return false;
		}
		...
		if (!$this->canConnect())
		{
			if ($ignoreErrors)
				...
				throw new \Thurly\Main\Db\ConnectionException('Sphinx connect error', GetMessage("SEARCH_SPHINX_CONN_EXT_IS_MISSING"));
			return false;
		}
		...
		if (!$this->db)
		{
			if ($ignoreErrors)
				...
				throw new \Thurly\Main\Db\ConnectionException('Sphinx connect error', GetMessage("SEARCH_SPHINX_CONN_ERROR", array("#ERRSTR#" => $error)));
			return false;
		}
	...
	public function recodeTo($text)
	{
		...
		{
			$error = "";
			$result = \Thurly\Main\Text\Encoding::convertEncoding($text, SITE_CHARSET, "UTF-8", $error);
	...
	public function recodeFrom($text)
	{
		...
		{
			$error = "";
			$result = \Thurly\Main\Text\Encoding::convertEncoding($text, "UTF-8", SITE_CHARSET, $error);
			if (!$result && !empty($error))
				#$this->ThrowException($error, "ERR_CHAR_BX_CONVERT");
				return $text;
	...
	public function replace($ID, $arFields)
	{
		...
		{
			throw new \Thurly\Main\Db\SqlQueryException('Sphinx select error', $this->getError(), $sql);
		}
	}
...
class CSearchSphinxFormatter extends CSearchFormatter
{
	...
	public function buildExcerpts($str)
	{
		...
		{
			throw new \Thurly\Main\Db\SqlQueryException('Sphinx select error', $this->sphinx->getError(), $sql);
		}
	}
}
