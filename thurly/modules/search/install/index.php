...
Class search extends CModule
{
	...
	function InstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/db/".strtolower($DB->type)."/install.sql");
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/db/".strtolower($DB->type)."/stat_install.sql");
		}
	...
	function UnInstallDB($arParams = array())
	{
		...
		if(is_object($DB))
		{
			...
			{
				$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/db/".strtolower($DB->type)."/uninstall.sql");
			}

			if(!array_key_exists("savestat", $arParams) || ($arParams["savestat"] != "Y"))
			{
				$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/db/".strtolower($DB->type)."/stat_uninstall.sql");
			}
		}
	...
	function InstallFiles($arParams = array())
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/js", $_SERVER["DOCUMENT_ROOT"]."/thurly/js/search", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/images", $_SERVER["DOCUMENT_ROOT"]."/thurly/images/search", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", True, True);
		}
		...
		if(array_key_exists("public_dir", $arParams) && strlen($arParams["public_dir"]))
		{
			...
			{
				$source = $_SERVER['DOCUMENT_ROOT']."/thurly/modules/search/install/public/";
	...
	function UnInstallFiles()
	{
		if($_ENV["COMPUTERNAME"]!='BX')
		{
			DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/admin/", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
			DeleteDirFilesEx("/thurly/images/search/");//images
			DeleteDirFilesEx("/thurly/js/search/");//javascript
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if($step < 2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage("SEARCH_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/step1.php");
		}
		elseif($step == 2)
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("SEARCH_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/step2.php");
		}
	}
	...
	function DoUninstall()
	{
		...
		if($step < 2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage("SEARCH_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/unstep1.php");
		}
		elseif($step == 2)
		{
			...
			$this->UnInstallFiles();
			$GLOBALS["errors"] = $this->errors;
			$APPLICATION->IncludeAdminFile(GetMessage("SEARCH_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/search/install/unstep2.php");
		}
	}
