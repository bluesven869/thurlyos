...
use \Thurly\Main\Event,
	...
	\Thurly\Main\EventResult,
	\Thurly\Main\Config\Option,
	\Thurly\Main\ModuleManager,
	\Thurly\Main\Text\Encoding,
	\Thurly\Main\Web\HttpClient,
	\Thurly\Main\Localization\Loc;
use \Thurly\ImOpenLines\Network,
	\Thurly\ImOpenLines\LiveChatManager;
use	\Thurly\ImConnector\Connectors\Viber,
	\Thurly\ImConnector\Connectors\Instagram,
	\Thurly\ImConnector\Connectors\BotFramework;
...
final class Output
{
	...
	private function query($command, $result)
	{
		...
		else
		{
			...
			$httpClient->setHeader('User-Agent', 'Thurly Connector Client');
	...
	public static function infoConnectorsLine($lineId)
	{
		...
		if(Loader::includeModule(Library::MODULE_ID_OPEN_LINES))
		{
			...
			if(!empty($resultLiveChat) || !empty($resultNetwork) || !empty($resultCustom))
			{
