...
class timeman extends CModule
{
	...
	function InstallDB()
	{
		...
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/fields.php");
		...
		RegisterModuleDependences('rest', 'OnRestServiceBuildDescription', 'timeman', '\Thurly\Timeman\Rest', 'onRestServiceBuildDescription');
	...
	function UnInstallDB($arParams = array())
	{
		...
		UnRegisterModuleDependences('rest', 'OnRestServiceBuildDescription', 'timeman', '\Thurly\Timeman\Rest', 'onRestServiceBuildDescription');
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/components",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/components",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/admin",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/admin",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/js",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/js",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/themes",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/themes",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/tools",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/tools",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/images",
	...
	function DoInstall()
	{
		...
		if (!CBXFeatures::IsFeatureEditable('timeman'))
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("TIMEMAN_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/step1.php");
		}
		else
		{
	...
	function DoUninstall()
	{
		...
		if($USER->IsAdmin())
		{
			...
			if($step < 2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("TIMEMAN_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/unstep1.php");
			}
			elseif($step == 2)
			{
				...
				$GLOBALS["errors"] = $this->errors;
				$APPLICATION->IncludeAdminFile(GetMessage("TIMEMAN_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/unstep2.php");
			}
		}
	}
