...
if(check_thurly_sessid() && $USER->IsAuthorized())
{
			...
			else
			{
				...
				switch($action)
				{
					...
					case "add_comment_entry":
						...
						if ($entry && CModule::IncludeModule("forum"))
						{
							...
							if ($bCanAddComment)
							{
								...
								ob_start();
								$APPLICATION->IncludeComponent('thurly:timeman.topic.reviews', '', array('ENTRY_ID' => $ID), null, array('HIDE_ICONS' => 'Y'));
					...
					case 'close':
						...
						{
							ob_start();
							$APPLICATION->IncludeComponent('thurly:timeman.topic.reviews', '', array('ENTRY_ID' => $arInfo['ID']), null, array('HIDE_ICONS' => 'Y'));
							$res['COMMENTS'] = trim(ob_get_contents());
							ob_end_clean();
					...
					case 'open':
						...
						{
							if(!\CThurlyOSBusinessTools::isUserUnlimited($USER->GetID()))
							{
								$res = false;

								ob_start();

								$assetCollector = new \Thurly\Main\UserField\AssetCollector();

								$assetCollector->startAssetCollection();
								$APPLICATION->IncludeComponent("thurly:thurlyos.business.tools.info", "", array('SHOW_TITLE' => 'N'));

								$asset = $assetCollector->getCollectedAssets();

								$errorData = ob_get_clean().implode('', $asset);

								$error = \Thurly\Main\Web\Json::encode(array(
						...
						if (count($arAccessUsers['READ']) > 0)
						{
							...
							if ($arRes = $dbRes->Fetch())
							{
								...
								)
								{
									...
									ob_start();
									$APPLICATION->IncludeComponent('thurly:timeman.topic.reviews', '', array('ENTRY_ID' => $arInfo['INFO']['ID']), null, array('HIDE_ICONS' => 'Y'));
									$res['COMMENTS'] = trim(ob_get_contents());
									ob_end_clean();
								}
							}
