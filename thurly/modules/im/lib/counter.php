...
class Counter
{
	...
	public static function get($userId)
	{
		...
		";
		$counters = \Thurly\Main\Application::getInstance()->getConnection()->query($query)->fetchAll();
	...
	public static function countingLostCountersAgent($notifyRelationId = 0, $chatRelationId = 0)
	{
		...
		if ($notifyStartId >= 0)
		{
			$query = "
				...
				HAVING CNT > 0
			";
			$cursor = \Thurly\Main\Application::getInstance()->getConnection()->query($query);
			...
			while ($row = $cursor->fetch())
			{
				...
				\Thurly\Im\Model\RelationTable::update($row['ID'], Array(
		...
		if ($chatRelationId >= 0)
		{
			...
			while ($row = $cursor->fetch())
			{
				...
				\Thurly\Im\Model\RelationTable::update($row['ID'], $update);
			}
		}

		if ($foundNotify || $foundChat)
		{
			return '\Thurly\Im\Counter::countingLostCountersAgent('.($foundNotify? $notifyRelationId: -1).', '.($foundChat? $chatRelationId: -1).');';
		}
		else
		{
			return '';
	...
	public static function onGetMobileCounterTypes(\Thurly\Main\Event $event)
	{
	...
	public static function onGetMobileCounter(\Thurly\Main\Event $event)
	{
