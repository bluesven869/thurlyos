...
class Department
{
	public static function getColleagues($userId = null, $options = array())
	{
		...
		$departments = \Thurly\Im\User::getInstance($userId)->getDepartments();
		...
		if ($options['USER_DATA'] == 'Y')
		{
			...
			{
				$result[] = \Thurly\Im\User::getInstance($userId)->getArray($getOptions);
			}
		}
		else
		{
	...
	public static function getDepartmentYouManage($userId = null, $options = array())
	{
		...
		$list = \Thurly\Im\Integration\Intranet\Department::getList();
		...
		foreach ($list as $key => $department)
		{
			...
			if ($options['USER_DATA'] == 'Y')
			{
				$userData = \Thurly\Im\User::getInstance($department['MANAGER_USER_ID']);
	...
	public static function getManagers($ids = null, $options = array())
	{
		...
		foreach ($list as $department)
		{
			...
			if ($options['USER_DATA'] == 'Y')
			{
				$managers[$department['ID']][] = \Thurly\Im\User::getInstance($department['MANAGER_USER_ID'])->getArray($userOptions);
			}
			else
			{
	...
	public static function getEmployees($ids = null, $options = array())
	{
		...
		foreach ($list as $departmentId => $users)
		{
			...
			foreach ($users as $employeeId)
			{
				if ($options['USER_DATA'] == 'Y')
				{
					$employees[$departmentId][] = \Thurly\Im\User::getInstance($employeeId)->getArray($userOptions);
				}
				else
				{
