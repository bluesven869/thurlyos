...
class LastSearch
{
	...
	public static function add($dialogId, $userId = null)
	{
		...
		if (!\Thurly\Im\Common::isDialogId($dialogId))
		{
			return false;
		}

		if (\Thurly\Im\Common::isChatId($dialogId))
		{
		...
		{
			$relations = \Thurly\Im\Dialog::getRelation($userId, $dialogId);
			if (!$relations[$userId])
			{
				return false;
			}
		...
		$orm = \Thurly\Im\Model\LastSearchTable::getList(Array(
		...
		$result = \Thurly\Im\Model\LastSearchTable::add(Array(
			'USER_ID' => $userId,
			'DIALOG_ID' => $dialogId,
			'ITEM_CID' => $chatId,
			'ITEM_RID' => $relationId,
		...
		{
			\Thurly\Im\Model\LastSearchTable::delete($id);
		}
	...
	public static function delete($dialogId, $userId = null)
	{
		...
		\Thurly\Im\Model\LastSearchTable::delete($row['ID']);
	...
	public static function get($userId = null, $options = array())
	{
		...
		$cachePath = self::CACHE_PATH.\Thurly\Im\Common::getCacheUserPostfix($userId);
