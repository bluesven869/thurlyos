...
class MessageParamHandler extends \Thurly\Replica\Client\BaseHandler
{
	protected $tableName = "b_im_message_param";
	protected $moduleId = "im";
	protected $className = "\\Thurly\\Im\\Model\\MessageParamTable";
	...
	public function beforeLogFormat(array &$record)
	{
		...
		$url = \Thurly\Disk\Driver::getInstance()->getUrlManager()->getUrlExternalLink(array(
	...
	public function afterInsertTrigger(array $newRecord)
	{
		...
		if ($newRecord['PARAM_NAME'] === 'LIKE' && $newRecord["PARAM_VALUE"])
		{
			...
			$result = \Thurly\IM\Model\ChatTable::getList(Array(
				'filter'=>Array(
					'=ID' => $message['CHAT_ID']
				)
			));
			...
			{
				$message['MESSAGE'] = str_replace('<br />', ' ', \Thurly\Im\Text::parse($message['MESSAGE']));
			...
			{
				\Thurly\Pull\Event::add($rel['USER_ID'], Array(
					'module_id' => 'im',
					'command' => 'messageLike',
