...
class MessageHandler extends \Thurly\Replica\Client\BaseHandler
{
	...
	protected $className = "\\Thurly\\Im\\Model\\MessageTable";
	...
	public function afterInsertTrigger(array $newRecord)
	{
		...
		if ($arFields['MESSAGE_TYPE'] == IM_MESSAGE_PRIVATE)
		{
			...
			if (\CModule::IncludeModule('pull'))
			{
				...
				\Thurly\Pull\Event::add($arFields['TO_USER_ID'], $pullMessageTo);
				\Thurly\Pull\Event::add($arFields['FROM_USER_ID'], $pullMessage);
		...
		else if ($arFields['MESSAGE_TYPE'] == IM_MESSAGE_CHAT || $arFields['MESSAGE_TYPE'] == IM_MESSAGE_OPEN || $arFields['MESSAGE_TYPE'] == IM_MESSAGE_OPEN_LINE)
		{
			...
			{
				if ($relation["EXTERNAL_AUTH_ID"] == \Thurly\Im\Bot::EXTERNAL_AUTH_ID)
				{
					continue;
				}
			...
			if (\CModule::IncludeModule('pull'))
			{
				...
				\Thurly\Pull\Event::add($pullUsers, $pullMessage);
	...
	public function afterUpdateTrigger(array $oldRecord, array $newRecord)
	{
		...
		$arPullMessage = Array(
			...
			'text' => \Thurly\Im\Text::parse($arFields['MESSAGE']),
		...
		\Thurly\Pull\Event::add(array_keys($relations), $p=Array(
			'module_id' => 'im',
	...
	public function beforeLogFormat(array &$record)
	{
		...
		if (\Thurly\Im\User::getInstance($record['AUTHOR_ID'])->isBot())
		{
			$record['MESSAGE'] = "[b]".\Thurly\Im\User::getInstance($record['AUTHOR_ID'])->getFullName()."[/b] \n ".$record['MESSAGE'];
			$record['AUTHOR_ID'] = 0;
		}
	}
}
