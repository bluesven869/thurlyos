...
class Recent
{
	...
	public static function get($userId = null, $options = Array())
	{
		...
		$cachePath = '/bx/imc/recent'.\Thurly\Im\Common::getCacheUserPostfix($userId);
		...
		else
		{
			...
			$runtime = Array(
				new \Thurly\Main\Entity\ReferenceField(
					'ATTACH',
					'\Thurly\Im\Model\MessageParamTable',
					array("=ref.MESSAGE_ID" => "this.ITEM_MID", "ref.PARAM_NAME" => new \Thurly\Main\DB\SqlExpression("?s", "ATTACH")),
					array("join_type"=>"LEFT")
				),
				new \Thurly\Main\Entity\ReferenceField(
					'FILE',
					'\Thurly\Im\Model\MessageParamTable',
					array("=ref.MESSAGE_ID" => "this.ITEM_MID", "ref.PARAM_NAME" => new \Thurly\Main\DB\SqlExpression("?s", "FILE_ID")),
					array("join_type"=>"LEFT")
				)
			);
			if ($isOperator)
			{
				$runtime[] = new \Thurly\Main\Entity\ReferenceField(
					'LINES',
					'\Thurly\ImOpenlines\Model\SessionTable',
					array(">this.ITEM_OLID" => new \Thurly\Main\DB\SqlExpression("0"), "=ref.ID" => "this.ITEM_OLID"),
					array("join_type"=>"LEFT")
				);
			}

			$orm = \Thurly\Im\Model\RecentTable::getList(Array(
				'select' => $select,
		...
		\Thurly\Main\Type\Collection::sortByColumn(
			...
			Array(
				...
				{
					return $row['DATE'] instanceof \Thurly\Main\Type\DateTime? $row['DATE']->getTimeStamp(): 0;
				},
			)
		);
			...
			foreach ($result as $index => $item)
			{
				foreach ($item as $key => $value)
				{
					...
					else if (is_array($value))
					{
						...
						{
							if ($subValue instanceof \Thurly\Main\Type\DateTime)
							{
							...
							{
								$value[$subKey] = \Thurly\Im\Common::getPublicDomain().$subValue;
							}
	...
	public static function pin($dialogId, $action, $userId = null)
	{
		...
		$element = \Thurly\Im\Model\RecentTable::getList(Array(
