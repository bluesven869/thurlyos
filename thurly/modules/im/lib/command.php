...
class Command
{
	...
	public static function register(array $fields)
	{
		...
		if ($botId > 0 && (!\Thurly\Im\User::getInstance($botId)->isExists() || !\Thurly\Im\User::getInstance($botId)->isBot()))
		{
			$botId = 0;
		}
		...
		$result = \Thurly\Im\Model\CommandTable::add(Array(
			'BOT_ID' => $botId,
			'MODULE_ID' => $moduleId,
			'COMMAND' => $command,
			'COMMON' => $common,
		...
		if ($moduleId == 'rest')
		{
			foreach ($langSet as $lang)
			{
				...
				try
				{
					\Thurly\Im\Model\CommandLangTable::add(array(
	...
	public static function unRegister(array $command)
	{
		...
		\Thurly\Im\Model\CommandTable::delete($commandId);

		$orm = \Thurly\Im\Model\CommandLangTable::getList(Array(
			'filter' => Array('=COMMAND_ID' => $commandId)
		));
		while ($row = $orm->fetch())
		{
			\Thurly\Im\Model\CommandLangTable::delete($row['ID']);
		}
	...
	public static function update(array $command, array $updateFields)
	{
		...
		if (!empty($update))
		{
			\Thurly\Im\Model\CommandTable::update($commandId, $update);
	...
	public static function onCommandAdd($messageId, $messageFields)
	{
		...
		$messageFields['DIALOG_ID'] = \Thurly\Im\Command::getDialogId($messageFields);
			...
			if ($params["METHOD_COMMAND_ADD"] && class_exists($params["CLASS"]) && method_exists($params["CLASS"], $params["METHOD_COMMAND_ADD"]))
			{
				if ($params['BOT_ID'] > 0)
				{
					\Thurly\Im\Model\BotTable::update($params['BOT_ID'], array(
						"COUNT_COMMAND" => new \Thurly\Main\DB\SqlExpression("?# + 1", "COUNT_COMMAND")
					));
				}
		...
		foreach(\Thurly\Main\EventManager::getInstance()->findEventHandlers("im", "onImCommandAdd") as $event)
		{
	...
	public static function addMessage(array $access, array $messageFields)
	{
		...
		if (Common::isChatId($messageFields['DIALOG_ID']))
		{
			...
			else
			{
				...
				if ($botId > 0)
				{
					$ar['MESSAGE'] = Loc::getMessage("COMMAND_BOT_ANSWER", Array("#BOT_NAME#" => "[USER=".$botId."]".\Thurly\Im\User::getInstance($botId)->getFullName()."[/USER]\n ")).$ar['MESSAGE'];
				}
				else
				{
	...
	private static function findCommands($fields)
	{
		...
		$isExtranet = \Thurly\Im\User::getInstance($messageFields['FROM_USER_ID'])->isExtranet();
	...
	public static function getListCache($lang = LANGUAGE_ID)
	{
		...
		else
		{
			$loadRestLang = false;
			$result = Array();
			$orm = \Thurly\Im\Model\CommandTable::getList();
			while ($row = $orm->fetch())
			{
				...
				if ($row['BOT_ID'] > 0)
				{
					$row['CATEGORY'] = \Thurly\Im\User::getInstance($row['BOT_ID'])->getFullName();
				}
				...
				{
					if (\Thurly\Main\Loader::includeModule($row['MODULE_ID']) && class_exists($row["CLASS"]) && method_exists($row["CLASS"], $row["METHOD_LANG_GET"]))
					{
				...
				else
				{
					...
					if ($row['MODULE_ID'] == 'rest')
					{
						...
						{
							$res = \CThurlyOSApp::getList(array(), array('APP_ID' => $row['APP_ID']));
							if ($app = $res->fetch())
							{
			...
			{
				$langSet = Array();
				$orm = \Thurly\Im\Model\CommandLangTable::getList();
				while ($row = $orm->fetch())
				{
				...
				$langAlter = \Thurly\Im\Bot::getDefaultLanguage();
			...
			if (!empty($result))
			{
				\Thurly\Main\Type\Collection::sortByColumn(
					$result,
