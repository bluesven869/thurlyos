...
class Bot
{
	...
	public static function register(array $fields)
	{
		...
		{
			$botFields['UF_DEPARTMENT'] = Array(\Thurly\Im\Bot\Department::getId());
		}
		else
		{
		...
		$result = \Thurly\Im\Model\BotTable::add(Array(
			'BOT_ID' => $botId,
		...
		if ($result->isSuccess())
		{
			if (\Thurly\Main\Loader::includeModule('pull'))
			{
				...
				{
					$message = '';
					if ($installType == self::INSTALL_TYPE_USER && \Thurly\Im\User::getInstance($userId)->isExists())
					{
						$userName = \Thurly\Im\User::getInstance($userId)->getFullName();
						$userName = '[USER='.$userId.']'.$userName.'[/USER]';
						$userGender = \Thurly\Im\User::getInstance($userId)->getGender();
					...
					$attach->AddBot(Array(
						...
						"AVATAR" => \Thurly\Im\User::getInstance($botId)->getAvatar(),
						"BOT_ID" => $botId,
					));
					$attach->addMessage(\Thurly\Im\User::getInstance($botId)->getWorkPosition());
	...
	public static function unRegister(array $bot)
	{
		...
		\Thurly\Im\Model\BotTable::delete($botId);

		$orm = \Thurly\Im\Model\BotChatTable::getList(Array(
			'filter' => Array('=BOT_ID' => $botId)
		));
		if ($row = $orm->fetch())
		{
			\Thurly\Im\Model\BotChatTable::delete($row['ID']);
		}
		...
		if (\Thurly\Main\Loader::includeModule($bots[$botId]['MODULE_ID']) && $bots[$botId]["METHOD_BOT_DELETE"] && class_exists($bots[$botId]["CLASS"]) && method_exists($bots[$botId]["CLASS"], $bots[$botId]["METHOD_BOT_DELETE"]))
		{
			call_user_func_array(array($bots[$botId]["CLASS"], $bots[$botId]["METHOD_BOT_DELETE"]), Array($botId));
		}

		foreach(\Thurly\Main\EventManager::getInstance()->findEventHandlers("im", "onImBotDelete") as $event)
		{
			ExecuteModuleEventEx($event, Array($bots[$botId], $botId));
		}

		$orm = \Thurly\Im\Model\CommandTable::getList(Array(
			'filter' => Array('=BOT_ID' => $botId)
		));
		while ($row = $orm->fetch())
		{
			\Thurly\Im\Command::unRegister(Array('COMMAND_ID' => $row['ID'], 'FORCE' => 'Y'));
		}

		$orm = \Thurly\Im\Model\AppTable::getList(Array(
			'filter' => Array('=BOT_ID' => $botId)
		));
		while ($row = $orm->fetch())
		{
			\Thurly\Im\App::unRegister(Array('ID' => $row['ID'], 'FORCE' => 'Y'));
		}
	...
	public static function update(array $bot, array $updateFields)
	{
		...
		if (!\Thurly\Im\User::getInstance($botId)->isExists() || !\Thurly\Im\User::getInstance($botId)->isBot())
			return false;
		...
		if (!empty($update))
		{
			\Thurly\Im\Model\BotTable::update($botId, $update);
	...
	public static function onMessageAdd($messageId, $messageFields)
	{
		...
		{
			$messageFields['DIALOG_ID'] = \Thurly\Im\Bot::getDialogId($messageFields);
			...
			{
				if (!$params['MODULE_ID'] || !\Thurly\Main\Loader::includeModule($params['MODULE_ID']))
					continue;
				...
				{
					\Thurly\Im\Model\BotTable::update($params['BOT_ID'], array(
						"COUNT_MESSAGE" => new \Thurly\Main\DB\SqlExpression("?# + 1", "COUNT_MESSAGE")
					));
			...
			foreach(\Thurly\Main\EventManager::getInstance()->findEventHandlers("im", "onImBotMessageAdd") as $event)
			{
				ExecuteModuleEventEx($event, Array($botExecModule, $messageId, $messageFields));
			}

			if ($messageFields['CHAT_ENTITY_TYPE'] == 'LINES' && trim($messageFields['MESSAGE']) == '0' && \Thurly\Main\Loader::includeModule('imopenlines'))
			{
				$chat = new \Thurly\Imopenlines\Chat($messageFields['TO_CHAT_ID']);
				$chat->endBotSession();
			}
		}
	...
	public static function onMessageUpdate($messageId, $messageFields)
	{
		...
		if (!empty($botExecModule))
		{
			...
			foreach(\Thurly\Main\EventManager::getInstance()->findEventHandlers("im", "onImBotMessageUpdate") as $event)
			{
				ExecuteModuleEventEx($event, Array($botExecModule, $messageId, $messageFields));
			}
		}
	...
	public static function onMessageDelete($messageId, $messageFields)
	{
		...
		if (!empty($botExecModule))
		{
			...
			foreach(\Thurly\Main\EventManager::getInstance()->findEventHandlers("im", "onImBotMessageDelete") as $event)
			{
				ExecuteModuleEventEx($event, Array($botExecModule, $messageId, $messageFields));
			}
		}
	...
	public static function onJoinChat($dialogId, $joinFields)
	{
		...
		if (!\Thurly\Main\Loader::includeModule($bot['MODULE_ID']))
			return false;

		if ($joinFields['CHAT_TYPE'] == IM_MESSAGE_PRIVATE)
		{
			$updateCounter = array("COUNT_USER" => new \Thurly\Main\DB\SqlExpression("?# + 1", "COUNT_USER"));
		}
		else
		{
			$updateCounter = array("COUNT_CHAT" => new \Thurly\Main\DB\SqlExpression("?# + 1", "COUNT_CHAT"));
		}
		\Thurly\Im\Model\BotTable::update($joinFields['BOT_ID'], $updateCounter);
		...
		else if (strlen($bot["TEXT_PRIVATE_WELCOME_MESSAGE"]) > 0 && $joinFields['CHAT_TYPE'] == IM_MESSAGE_PRIVATE && $joinFields['FROM_USER_ID'] != $joinFields['BOT_ID'])
		{
			...
			{
				\Thurly\Im\Bot::startWriting(Array('BOT_ID' => $joinFields['BOT_ID']), $dialogId);
			}

			$userName = \Thurly\Im\User::getInstance($joinFields['USER_ID'])->getName();
			\Thurly\Im\Bot::addMessage(Array('BOT_ID' => $joinFields['BOT_ID']), Array(
				'DIALOG_ID' => $dialogId,
		...
		foreach(\Thurly\Main\EventManager::getInstance()->findEventHandlers("im", "onImBotJoinChat") as $event)
		{
	...
	public static function addMessage(array $bot, array $messageFields)
	{
		...
		else
		{
			if ($isPrivateSystem)
			{
				...
				if ($botId > 0)
				{
					$messageFields['MESSAGE'] = Loc::getMessage("BOT_MESSAGE_FROM", Array("#BOT_NAME#" => "[USER=".$botId."]".\Thurly\Im\User::getInstance($botId)->getFullName()."[/USER][BR]")).$messageFields['MESSAGE'];
				}
			}
			else
			{
	...
	public static function updateMessage(array $bot, array $messageFields)
	{
		...
		if (isset($messageFields['KEYBOARD']))
		{
			...
			else if ($messageFields['KEYBOARD'] instanceof \Thurly\Im\Bot\Keyboard)
			{
		...
		if (isset($messageFields['MENU']))
		{
			...
			else if ($messageFields['MENU'] instanceof \Thurly\Im\Bot\ContextMenu)
			{
	...
	public static function getListCache($type = self::LIST_ALL)
	{
		...
		{
			$result = Array();
			$orm = \Thurly\Im\Model\BotTable::getList();
			while ($row = $orm->fetch())
			{
	...
	public static function changeChatMembers($chatId, $botId, $append = true)
	{
		...
		if ($append)
		{
			...
			\Thurly\Im\Model\BotChatTable::add(Array(
				'BOT_ID' => $botId,
				'CHAT_ID' => $chatId
			));
		}
	...
	{
		$orm = \Thurly\Im\Model\BotTokenTable::getList(Array(
			'filter' => array(
				'<DATE_EXPIRE' => new \Thurly\Main\Type\DateTime(),
			),
			'select' => array('ID'),
			'limit' => 1
		));
		...
		return "\\Thurly\\Im\\Bot::deleteExpiredTokenAgent();";
	}
}