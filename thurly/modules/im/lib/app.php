...
class App
{
	...
	public static function register(array $fields)
	{
		...
		$result = \Thurly\Im\Model\AppTable::add(Array(
			'HASH' => $hash,
			'BOT_ID' => $botId,
			'MODULE_ID' => $moduleId,
			'CODE' => $code,
		...
		if ($moduleId == 'rest')
		{
			foreach ($langSet as $lang)
			{
				...
				try
				{
					\Thurly\Im\Model\AppLangTable::add(array(
						'APP_ID' => $appId,
	...
	public static function unRegister(array $app)
	{
		...
		\Thurly\Im\Model\AppTable::delete($appId);

		$orm = \Thurly\Im\Model\AppLangTable::getList(Array(
			'filter' => Array('=APP_ID' => $appId)
		));
		while ($row = $orm->fetch())
		{
			\Thurly\Im\Model\AppLangTable::delete($row['ID']);
		}
	...
	public static function update(array $app, array $updateFields)
	{
		...
		if (!empty($update))
		{
			\Thurly\Im\Model\AppTable::update($appId, $update);
	...
	public static function getListCache($lang = LANGUAGE_ID)
	{
		...
		else
		{
			$loadRestLang = false;
			$result = Array();
			$orm = \Thurly\Im\Model\AppTable::getList();
			while ($row = $orm->fetch())
			{
				if ($row['ICON_FILE_ID'])
				{
			...
			{
				$langSet = Array();
				$orm = \Thurly\Im\Model\AppLangTable::getList();
				while ($row = $orm->fetch())
				{
	...
	public static function getListForJs($lang = LANGUAGE_ID)
	{
		...
		$isExtranet = $userId && \Thurly\Im\User::getInstance($userId)->isExtranet();
		$isConnector = $userId && \Thurly\Im\User::getInstance($userId)->isConnector();
		...
		foreach ($apps as $app)
		{
			...
			$botData = \Thurly\Im\Bot::getCache($app['BOT_ID']);
			$result[] = Array(
				'id' => $app['ID'],
				'botId' => $app['BOT_ID'],
