...
class TextareaIcon
{
	...
	public static function register(array $fields)
	{
		...
		$result = \Thurly\Im\Model\TextareaIconTable::add(Array(
			'HASH' => $hash,
			'BOT_ID' => $botId,
			'MODULE_ID' => $moduleId,
			'ICON_CODE' => $icon,
		...
		if ($moduleId == 'rest')
		{
			foreach ($langSet as $lang)
			{
				...
				try
				{
					\Thurly\Im\Model\TextareaIconLangTable::add(array(
						'ICON_ID' => $iconId,
	...
	public static function unRegister(array $icon)
	{
		...
		\Thurly\Im\Model\TextareaIconTable::delete($iconId);

		$orm = \Thurly\Im\Model\TextareaIconLangTable::getList(Array(
			'filter' => Array('=ICON_ID' => $iconId)
		));
		while ($row = $orm->fetch())
		{
			\Thurly\Im\Model\TextareaIconLangTable::delete($row['ID']);
		}
	...
	public static function update(array $icon, array $updateFields)
	{
		...
		if (!empty($update))
		{
			\Thurly\Im\Model\TextareaIconTable::update($iconId, $update);
	...
	public static function getListCache($lang = LANGUAGE_ID)
	{
		...
		else
		{
			$loadRestLang = false;
			$result = Array();
			$orm = \Thurly\Im\Model\TextareaIconTable::getList();
			while ($row = $orm->fetch())
			{
				$row['ICON_ID'] = $row['ID'];
			...
			{
				$langSet = Array();
				$orm = \Thurly\Im\Model\TextareaIconLangTable::getList();
				while ($row = $orm->fetch())
				{
	...
	public static function getListForJs($lang = LANGUAGE_ID)
	{
		...
		if ($userId && \Thurly\Im\User::getInstance($userId)->isExtranet())
		{
			return Array();
		}
		...
		{
			$botData = \Thurly\Im\Bot::getCache($icon['BOT_ID']);
			$result[] = Array(
				'id' => $icon['ICON_ID'],
				'botId' => $icon['BOT_ID'],
