...
class User
{
	...
	public function getAvatar()
	{
		...
		return $fields && $fields['avatar'] != '/thurly/js/im/images/blank.gif'? $fields['avatar']: '';
	}
	...
	public static function uploadAvatar($avatarUrl = '')
	{
		...
		$orm = \Thurly\Im\Model\ExternalAvatarTable::getList(Array(
	...
	public static function getList($params)
	{
		...
		$intranetInstalled = \Thurly\Main\Loader::includeModule('intranet');
		$voximplantInstalled = \Thurly\Main\Loader::includeModule('voximplant');
		...
		$orm = \Thurly\Main\UserTable::getList($ormParams);
		...
		while ($user = $orm->fetch())
		{
			...
			{
				$color = \Thurly\Im\Color::getColor($user['COLOR']);
			}
			if (!$color)
			{
			...
			$users[$user["ID"]] = Array(
				...
				'BIRTHDAY' => $user['PERSONAL_BIRTHDAY'] instanceof \Thurly\Main\Type\Date? $user['PERSONAL_BIRTHDAY']->format('d-m'): false,
				...
				'NETWORK' => $user['EXTERNAL_AUTH_ID'] == \CIMContactList::NETWORK_AUTH_ID || $user['EXTERNAL_AUTH_ID'] == \Thurly\Im\Bot::EXTERNAL_AUTH_ID && $bots[$user["ID"]]['TYPE'] == \Thurly\Im\Bot::TYPE_NETWORK,
				'BOT' => $user['EXTERNAL_AUTH_ID'] == \Thurly\Im\Bot::EXTERNAL_AUTH_ID,
				'PROFILE' => \CIMContactList::GetUserPath($user["ID"]),
				'EXTERNAL_AUTH_ID' => $user['EXTERNAL_AUTH_ID']? $user['EXTERNAL_AUTH_ID']: 'default',
				'STATUS' => $user['STATUS'],
				'IDLE' => $user['IDLE'] instanceof \Thurly\Main\Type\DateTime? $user['IDLE']: false,
				'LAST_ACTIVITY_DATE' => $user['MOBILE_LAST_DATE'] instanceof \Thurly\Main\Type\DateTime? $user['MOBILE_LAST_DATE']: false,
				'MOBILE_LAST_DATE' => $user['LAST_ACTIVITY_DATE'] instanceof \Thurly\Main\Type\DateTime? $user['LAST_ACTIVITY_DATE']: false,
		...
		if ($params['JSON'])
		{
			...
			foreach ($users as $key => $value)
			{
				...
				{
					$users[$key] = \Thurly\Im\Common::getPublicDomain().$value;
				}
	...
	public static function getListFilter($params)
	{
		...
		{
			$filter = \Thurly\Main\UserUtils::getUserSearchFilter(Array('FIND' => $params['FILTER']['SEARCH']));
			if (empty($filter))
			{
				return null;
			}
		...
		$groups = \Thurly\Im\Integration\Socialnetwork\Extranet::getGroup(Array('CURRENT_USER' => $params['CURRENT_USER']));
		if (is_array($groups))
		{
