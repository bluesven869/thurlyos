...
class im extends CModule
{
	...
	function DoInstall()
	{
		...
		$GLOBALS['APPLICATION']->IncludeAdminFile(GetMessage("IM_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/im/install/step1.php");
	}
	...
	function InstallDB()
	{
		...
		else
		{
			...
			if(!$DB->Query("SELECT 'x' FROM b_im_chat", true))
				$this->errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT']."/thurly/modules/im/install/db/".strtolower($DB->type)."/install.sql");
		}
		...
		CAgent::AddAgent("\\Thurly\\Im\\Bot::deleteExpiredTokenAgent();", "im", "N", 86400);
		CAgent::AddAgent("\\Thurly\\Im\\Disk\\NoRelationPermission::cleaningAgent();", "im", "N", 3600);

		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->registerEventHandler('pull', 'onGetMobileCounter', 'im', '\Thurly\Im\Counter', 'onGetMobileCounter');
		$eventManager->registerEventHandler('pull', 'onGetMobileCounterTypes', 'im', '\Thurly\Im\Counter', 'onGetMobileCounterTypes');
		...
		{
			\Thurly\Im\Model\MessageTable::getEntity()->enableFullTextIndex("MESSAGE");
		}

		if (CIMConvert::ConvertCount() > 0)
		{
			CAdminNotify::Add(Array(
				"MESSAGE" => GetMessage("IM_CONVERT_MESSAGE", Array("#A_TAG_START#" => '<a href="/thurly/admin/im_convert.php?lang='.LANGUAGE_ID.'">', "#A_TAG_END#" => "</a>")),
				"TAG" => "IM_CONVERT",
				"MODULE_ID" => "IM",
				"ENABLE_CLOSE" => "Y"
			));
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/im/install/js", $_SERVER["DOCUMENT_ROOT"]."/thurly/js", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/im/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", true, true);
			CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/im/install/activities', $_SERVER['DOCUMENT_ROOT'].'/thurly/activities', true, true);
			CopyDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/im/install/admin', $_SERVER['DOCUMENT_ROOT'].'/thurly/admin', true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/im/install/templates", $_SERVER["DOCUMENT_ROOT"]."/thurly/templates", True, True);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/im/install/public", $_SERVER["DOCUMENT_ROOT"]."/", True, True);
	...
	function InstallEvents()
	{
		...
		$ar = $rs->Fetch();
		if($ar["CNT"] <= 0)
			include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/im/install/events/set_events.php");
	...
	{
		if (
			file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/intranet/install/templates/pub/")
			&& !file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/templates/pub/")
		)
		{
			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/intranet/install/templates/pub/",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/templates/pub/",
				$rewrite = true,
				$recursive = true,
	...
	function DoUninstall()
	{
		...
		if($step<2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage("IM_UNINSTALL_TITLE"), $DOCUMENT_ROOT."/thurly/modules/im/install/unstep1.php");
		}
		elseif($step==2)
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("IM_UNINSTALL_TITLE"), $DOCUMENT_ROOT."/thurly/modules/im/install/unstep2.php");
		}
	}
	...
	function UnInstallDB($arParams = Array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT']."/thurly/modules/im/install/db/".strtolower($DB->type)."/uninstall.sql");
		...
		CAgent::RemoveAgent("\\Thurly\\Im\\Bot::deleteExpiredTokenAgent();", "im");
		CAgent::RemoveAgent("\\Thurly\\Im\\Disk\\NoRelationPermission::cleaningAgent();", "im");
		...
		$eventManager = \Thurly\Main\EventManager::getInstance();
		$eventManager->unRegisterEventHandler('pull', 'onGetMobileCounter', 'im', '\Thurly\Im\Counter', 'onGetMobileCounter');
		$eventManager->unRegisterEventHandler('pull', 'onGetMobileCounterTypes', 'im', '\Thurly\Im\Counter', 'onGetMobileCounterTypes');
	...
	function UnInstallFiles($arParams = array())
	{
		if($_ENV["COMPUTERNAME"]!='BX')
		{
			...
			DeleteDirFilesEx('/thurly/templates/desktop_app/');
		}
	...
	function UnInstallEvents()
	{
		...
		include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/im/install/events/del_events.php");
