...
BX.ImMobile = function(params)
{
	...
	this.errorMessage = '';
	this.isAdmin = false;
	this.thurlyNetwork = false;
	this.thurlyNetwork2 = false;
	this.thurlyOpenLines = false;
	this.thurlyos = true;
	this.thurlyIntranet = true;
	this.thurlyosnet = false;
	this.thurlyXmpp = false;
	this.ppStatus = true;
...
BX.ImMobile.prototype.initParams = function(params)
{
	...
	this.thurlyNetwork = params.thurlyNetwork || false;
	this.thurlyNetwork2 = params.thurlyNetwork2 || false;
	this.thurlyOpenLines = params.thurlyOpenLines || false;
	this.thurlyos = params.thurlyos || false;
	this.thurlyIntranet = params.thurlyIntranet || false;
	this.thurlyosnet = params.thurlyosnet || false;
	this.thurlyXmpp = params.thurlyXmpp || false;
	...
	if (this.openChatFlag)
	{
		...
		if (this.chat[chatId] && this.chat[chatId].type != 'call')
		{
			...
			else if (this.chat[chatId].type == 'lines')
			{
				...
				if (this.chat[chatId].owner > 0)
				{
					...
					{
						items.push({ image: "/thurly/templates/mobile_app/images/im/icon-forward.png", name: BX.message('IM_OL_INVITE_TRANSFER'), action:BX.delegate(function() {  this.linesTransfer(chatId); }, this)});
					}
				...
				if (this.chat[chatId].owner == this.BXIM.userId)
				{
					...
					items.push({ image: "/thurly/templates/mobile_app/images/im/important.png", name: BX.message('IM_M_OL_CLOSE'), action:BX.delegate(function() {  this.linesCloseDialog(); }, this)});
				}
				if (session.crmLink)
				{
					items.push({image: "/thurly/templates/mobile_app/images/im/work.png", name: BX.message('IM_M_OL_GOTO_CRM'), action:BX.delegate(function() {
		...
		else
		{
			app.menuCreate({
				...
				items: [
					{ image: "/thurly/templates/mobile_app/images/im/icon-call.png",  name: BX.message('IM_AUDIO_CALL'), action:BX.delegate(function() {
	...
	else if (this.currentTab)
	{
		...
		if (this.BXIM.userId != userId && this.users[userId] && !this.users[userId].bot && !this.users[userId].network)
		{
			if (sheetButtons.length > 1)
			{
				...
				menuItems.push({ image: "/thurly/templates/mobile_app/images/im/icon-call.png",  name: BX.message('IM_AUDIO_CALL'), action:BX.delegate(function() {
					callSheet.show();
				}, this)});
			}
			else
			...
			menuItems.push({ image: "/thurly/templates/mobile_app/images/im/icon-video.png",  name: BX.message('IM_VIDEO_CALL_LIST'), action:BX.delegate(function() {
	...
	app.openTable({
		...
		callback: BX.delegate(function (data)
		{
			...
			if (!isChat)
			{
				...
				data = {'IM_CHAT_ADD': 'Y', 'USERS': JSON.stringify(arUsers), 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()};
			}
			else
			{
				data = {'IM_CHAT_EXTEND': 'Y', 'CHAT_ID': dialogId.substr(4), 'USERS': JSON.stringify(arUsers), 'IM_AJAX_CALL': 'Y', 'sessid': BX.thurly_sessid()};
			}
			if (!data)
				return false;
	...
	app.openTable({
		...
		callback: BX.delegate(function (data)
		{
			...
			BX.ajax({
				...
				dataType: 'json',
				timeout: 60,
				data: {'COMMAND': 'transfer', 'CHAT_ID' : chatId, 'TRANSFER_ID': user_id, 'IM_OPEN_LINES' : 'Y', 'IM_AJAX_CALL' : 'Y', 'sessid': BX.thurly_sessid()},
