...
class CIMMessage
{
	...
	public static function UpdateMessageOut($id, $messageOut)
	{
		...
		\Thurly\Im\Model\MessageTable::update($id, array(
	...
	function GetLastMessage($toUserId, $fromUserId = false, $loadUserData = false, $bTimeZone = true, $limit = true)
	{
		...
		else
		{
			...
			if ($arRes = $dbRes->Fetch())
			{
				...
				$lastRead = \Thurly\Main\Type\DateTime::createFromTimestamp($arRes['LAST_READ']);
	...
	function GetLastSendMessage($arParams)
	{
		...
		foreach ($arMessages as $key => $value)
		{
			$value['text'] = \Thurly\Im\Text::parse($value['text']);
		...
		if ($arRes = $dbRes->Fetch())
		{
			...
			if ($relation)
			{
				if (CModule::IncludeModule("pull"))
				{
					...
					\Thurly\Pull\Event::add($fromUserId, Array(
						'module_id' => 'im',
	...
	public static function SetLastId($chatId, $userId, $lastId = null)
	{
		...
		if ($update)
		{
			if ($relation['STATUS'] == IM_STATUS_READ)
			{
				$relation["LAST_READ"] = $update["LAST_READ"] = new Thurly\Main\Type\DateTime();
			...
			if ($relation['STATUS'] == IM_STATUS_READ && $relation['MESSAGE_TYPE'] != IM_MESSAGE_OPEN_LINE)
			{
				...
				while ($row = $orm->fetch())
				{
					...
					\Thurly\Im\Counter::clearCache($row['USER_ID']);
				}
			}
		}
		...
		if ($updateCounters)
		{
			\Thurly\Im\Counter::clearCache($userId);
		}
			...
			if ($relation['STATUS'] == IM_STATUS_NOTIFY && $relation['MESSAGE_TYPE'] != IM_MESSAGE_OPEN_LINE)
			{
				...
				if ($relations)
				{
					...
					foreach ($relations as $relationUserId)
					{
						...
						\Thurly\Im\Counter::clearCache($relationUserId);
					}
				}
			}
		}
	...
	public static function GetFormatMessage($arParams)
	{
		...
		if (isset($arParams['TO_CHAT_ID']))
		{
			...
			if (!empty($arUsers['users']) && $arParams['EXTRA_PARAMS']['CONTEXT'] == 'LIVECHAT' && CModule::IncludeModule('imopenlines'))
			{
				...
				{
					$arUsers['users'][$userId] = \Thurly\ImOpenLines\Connector::getOperatorName($arParams['EXTRA_PARAMS']['LINE_ID'], $userId, true);
				}
			}
		}
		...
		return Array(
			...
			'lines' => isset($arChat['lines'])? $arChat['lines']: Array(),
			'userInChat' => isset($arChat['userInChat'])? $arChat['userInChat']: Array(),
			...
			'message' => Array(
				...
				'date' => \Thurly\Main\Type\DateTime::createFromTimestamp($arParams['DATE_CREATE']),
				'text' => \Thurly\Im\Text::parse($arParams['MESSAGE']),
	...
	public static function GetChatId($fromUserId, $toUserId)
	{
		...
		if ($chatId <= 0)
		{
			$result = \Thurly\IM\Model\ChatTable::add(Array('TYPE' => IM_MESSAGE_PRIVATE, 'AUTHOR_ID' => $fromUserId));
			$chatId = $result->getId();
			if ($chatId > 0)
			{
				...
				if (\Thurly\Im\User::getInstance($fromUserId)->isExists() && !\Thurly\Im\User::getInstance($fromUserId)->isBot())
				{
					...
					\Thurly\Im\Bot::onJoinChat($fromUserId, $botJoinFields);
				}
				else if (\Thurly\Im\User::getInstance($toUserId)->isExists() && !\Thurly\Im\User::getInstance($toUserId)->isBot())
				{
					$botJoinFields['BOT_ID'] = $fromUserId;
					$botJoinFields['USER_ID'] = $toUserId;
					$botJoinFields['TO_USER_ID'] = $toUserId;
					$botJoinFields['FROM_USER_ID'] = $fromUserId;
					\Thurly\Im\Bot::onJoinChat($toUserId, $botJoinFields);
				}
			}
		}
