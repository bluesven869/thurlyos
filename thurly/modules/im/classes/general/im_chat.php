...
class CIMChat
{
	...
	function GetLastMessage($toChatId, $fromUserId = false, $loadExtraData = false, $bTimeZone = true, $limit = true)
	{
		...
		else if ($chatData['TYPE'] == IM_MESSAGE_OPEN_LINE && \Thurly\Main\Loader::includeModule('imopenlines') && \Thurly\ImOpenLines\Config::canJoin($toChatId))
		{
			$strSql = "
				SELECT
					M.ID,
			...
			\Thurly\Im\Disk\NoRelationPermission::add($toChatId, $fromUserId);
		}
		...
		if ($strSql)
		{
			...
			{
				if ($arRes['CHAT_ENTITY_TYPE'] != 'LIVECHAT' && \Thurly\Im\User::getInstance($fromUserId)->isConnector())
				{
					return false;
				}
				$arRes['CHAT_TYPE'] = trim($arRes['CHAT_TYPE']);
				...
				$arMessages[$arRes['ID']] = Array(
					...
					'date' => \Thurly\Main\Type\DateTime::createFromTimestamp($arRes['DATE_CREATE']),
					'text' => \Thurly\Im\Text::parse($arRes['MESSAGE'])
				);
				...
				foreach ($relations as $relation)
				{
					$readedList['chat'.$arRes['CHAT_ID']][$relation['USER_ID']] = Array(
						...
						'date' => \Thurly\Main\Type\DateTime::createFromTimestamp($relation['LAST_READ']),
					);
				}
			}
		}
		...
		else if ($chatData['ENTITY_TYPE'] == 'LIVECHAT' && $chatData['ENTITY_ID'] && CModule::IncludeModule('imopenlines'))
		{
			...
			{
				$arResult['users'][$userId] = \Thurly\ImOpenLines\Connector::getOperatorName($lineId, $userId, true);
			}
		}
	...
	function GetLastMessageLimit($chatId, $messageStartId, $messageEndId = 0, $loadExtraData = false, $bTimeZone = true, $order = 'DESC')
	{
		...
		{
			\Thurly\Im\Disk\NoRelationPermission::add($chatId, $this->user_id);
		}
	...
	public static function InstallGeneralChat($agentMode = false)
	{
		...
		if ($row = $dbRes->Fetch())
		{
			...
			if ($userCount > 50)
			{
				...
				{
					$perms = Array();
					$admins = \CThurlyOS::getAllAdminId();
	...
	public static function GetChatData($arParams = Array())
	{
		...
		while ($arRes = $dbRes->GetNext(true, false))
		{
			...
			{
				$avatar = '/thurly/js/im/images/blank.gif';
				...
				$arChat[$arRes["CHAT_ID"]] = Array(
					...
					'mute_list' => array(),
					'date_create' => $arRes["CHAT_DATE_CREATE"]? \Thurly\Main\Type\DateTime::createFromTimestamp($arRes["CHAT_DATE_CREATE"]): false,
					'type' => $chatType,
		...
		if ($arRes = $dbRes->Fetch())
		{
			...
			if ($relation)
			{
				if (CModule::IncludeModule("pull"))
				{
					...
					\Thurly\Pull\Event::add($this->user_id, Array(
						'module_id' => 'im',
					...
					{
						\Thurly\Pull\Event::add(array_keys($arRelation), $pushMessage);
		...
		if (!$bLoadMessage || ($bLoadMessage && intval($count) > 0))
		{
			...
			if ($bGroupByChat)
			{
				foreach ($arMessages as $key => $value)
				{
					...
					{
						$arMessages[$key]['text'] = \Thurly\Im\Text::parse($value['text']);
	...
	public function SetOwner($chatId, $userId, $checkPermission = true)
	{
		...
		if (CModule::IncludeModule('pull'))
		{
			\Thurly\Pull\Event::add(array_keys($arRelation), Array(
				'module_id' => 'im',
				'command' => 'chatOwner',
				'params' => Array(
					'chatId' => $chatId,
	...
	{
		\Thurly\Im\Model\ChatTable::update($chatId, Array(
		...
		if (CModule::IncludeModule('pull'))
		{
			...
			\Thurly\Pull\Event::add(array_keys($arRelation), Array(
				...
				'params' => Array(
					'chatId' => $chatId,
					'description' => \Thurly\Im\Text::parse($description)
				),
				'extra' => Array(
					'im_revision' => IM_REVISION,
					'im_revision_mobile' => IM_REVISION_MOBILE,
	...
	public function SetManagers($chatId, $users, $checkPermission = true)
	{
		...
		if (CModule::IncludeModule('pull'))
		{
			\Thurly\Pull\Event::add(array_keys($relation), Array(
				'module_id' => 'im',
	...
	public function SetColor($chatId, $color)
	{
		...
		if ($arRes = $dbRes->Fetch())
		{
			...
			if (CModule::IncludeModule("pull"))
			{
				...
				\Thurly\Pull\Event::add(array_keys($ar), $arPushMessage);
				if ($arRes['CHAT_TYPE'] == IM_MESSAGE_OPEN || $arRes['CHAT_TYPE'] == IM_MESSAGE_OPEN_LINE)
				{
	...
	public function SetAvatarId($chatId, $fileId)
	{
		...
		if ($fileId > 0)
		{
			$orm = \Thurly\Main\FileTable::getById($fileId);
			$file = $orm->fetch();
			if (!$file)
				return false;
	...
	public function Rename($chatId, $title, $checkPermission = true, $sendMessage = true)
	{
		...
		if ($arRes = $dbRes->Fetch())
		{
			...
			if (CModule::IncludeModule("pull"))
			{
				...
				\Thurly\Pull\Event::add(array_keys($ar), $pushMessage);
				if ($arRes['CHAT_TYPE'] == IM_MESSAGE_OPEN || $arRes['CHAT_TYPE'] == IM_MESSAGE_OPEN_LINE)
				{
					CPullWatch::AddToStack('IM_PUBLIC_'.$chatId, $pushMessage);
				}
	...
	public function Add($arParams)
	{
		...
		if ($chatTitle == "")
		{
			if (IM\Color::isEnabled())
			{
				$chatTitle = GetMessage('IM_CHAT_NAME_FORMAT', Array(
					'#COLOR#' => \Thurly\Im\Color::getName($chatColorCode),
		...
		if ($chatId > 0)
		{
			...
			{
				$attach = new CIMMessageParamAttach(null, Thurly\Im\Color::getColor($chatColorCode));
	...
	public function AddUser($chatId, $userId, $hideHistory = null, $skipMessage = false, $skipRecent = false)
	{
		...
		if ($chatEntityType == 'LINES')
		{
			...
			{
				if (
					!\Thurly\Im\User::getInstance($userId)->isConnector() &&
					(\Thurly\Im\User::getInstance($userId)->isExtranet() || \Thurly\Im\User::getInstance($userId)->isNetwork())
				)
				{
					unset($arUserId[$id]);
				}
		...
		foreach ($arUserId as $userId)
		{
			...
			if ($chatEntityType != 'LINES' && $arRes['CHAT_TYPE'] != IM_MESSAGE_PRIVATE && \Thurly\Im\User::getInstance($userId)->isExtranet())
			{
		...
		if (CModule::IncludeModule("pull"))
		{
			...
			\Thurly\Pull\Event::add(array_merge(array_keys($arRelation), array_keys($arUsers)), $pushMessage);
	...
	{
		return \Thurly\Im\Chat::mute($chatId, $mute, $this->user_id);
	}
	...
	public function DeleteUser($chatId, $userId, $checkPermission = true, $skipMessage = false, $skipRecent = false)
	{
		...
		\Thurly\Im\LastSearch::delete('chat'.$chatId, $userId);
		...
		if ($pullLoad)
		{
			\Thurly\Pull\Event::add(array_keys($arOldRelation), $pushMessage);
		}
	...
	{
		$url = $addBlankPicture? '/thurly/js/im/images/blank.gif': '';
	...
	public static function GetSonetGroupChatId($groupId)
	{
		...
		$chatData = \Thurly\Socialnetwork\Integration\Im\Chat\Workgroup::getChatData(Array(
			'group_id' => $groupId,
		...
		{
			$chatId = \Thurly\Socialnetwork\Integration\Im\Chat\Workgroup::createChat(Array(
				'group_id' => $groupId
			));
		}
