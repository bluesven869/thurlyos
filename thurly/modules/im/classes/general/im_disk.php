...
class CIMDisk
{
	...
	public static function GetStorage()
	{
		...
		{
			$storageModel = \Thurly\Disk\Storage::loadById($storageId);
	...
	{
		$post = \Thurly\Main\Context::getCurrent()->getRequest()->getPostList()->toArray();
		...
		/** @var $fileModel \Thurly\Disk\File */
		$fileModel = \Thurly\Disk\File::getById($fileId);
		...
		foreach ($chatRelation as $relation)
		{
			...
			\Thurly\Disk\Driver::getInstance()->getRecentlyUsedManager()->push($relation['USER_ID'], $fileId);
		}
		...
		if (CModule::IncludeModule('pull'))
		{
			...
			\Thurly\Pull\Event::add(array_keys($chatRelation), $pullMessage);
		...
		if (CModule::IncludeModule('imopenlines'))
		{
			if ($chat['ENTITY_TYPE'] == 'LINES' && $file['fileMessageHidden'] == 'N')
			{
				...
				if ($connectorId == 'livechat')
				{
					...
					{
						\Thurly\Im\Model\MessageParamTable::add(array(
						...
						$event = new \Thurly\Main\Event("imopenlines", "OnLivechatUploadFile", Array('FILES' => Array($file['fileId'])));
						$event->send();
					}
				}
			}
			else if ($chat['ENTITY_TYPE'] == 'LIVECHAT' && $file['fileMessageHidden'] == 'N')
			{
				...
				$session = new \Thurly\Imopenlines\Session();
				...
				if ($result)
				{
					...
					if ($uploadResult)
					{
						...
						$event = new \Thurly\Main\Event("imopenlines", "OnLivechatUploadFile", Array('FILES' => $uploadResult['DISK_ID']));
						$event->send();
					}
				}
			}
	...
	public static function UploadFileUnRegister($chatId, $files, $messages)
	{
		...
		if (CModule::IncludeModule('pull'))
		{
			...
			$orm = \Thurly\Im\Model\ChatTable::getById($result['CHAT_ID']);
			$chat = $orm->fetch();
			if ($chat['TYPE'] == IM_MESSAGE_OPEN || $chat['TYPE'] == IM_MESSAGE_OPEN_LINE)
			{
				CPullWatch::AddToStack('IM_PUBLIC_'.$chat['ID'], $pullMessage);
		...
		foreach ($files as $fileId)
		{
			...
			if ($newFile)
			{
				...
				foreach ($chatRelation as $relation)
				{
					...
					\Thurly\Disk\Driver::getInstance()->getRecentlyUsedManager()->push($relation['USER_ID'], $newFile->getId());
				}
			}
			else
			{
	...
	public static function UploadFileFromMain($chatId, $files)
	{
		...
		foreach ($files as $fileId)
		{
			...
			$newFile = $folderModel->addFile(array(
				...
				'FILE_ID' => $fileId,
				'SIZE' => $file['FILE_SIZE'],
				'CREATED_BY' => \Thurly\Disk\SystemUser::SYSTEM_USER_ID,
			), Array(), true);
			if ($newFile)
			{
	...
	public static function SaveToLocalDisk($fileId)
	{
		...
		$fileModel = \Thurly\Disk\File::getById($fileId, array('STORAGE'));
		if (!$fileModel)
			return false;
	...
	public static function GetHistoryFiles($chatId, $historyPage = 1)
	{
		...
		/*
		 * See details \Thurly\Im\Disk\ProxyType\Im::getSecurityContextByUser
		 */
		$securityContext = new \Thurly\Disk\Security\DiskSecurityContext(self::GetUserId());
		...
		$parameters = \Thurly\Disk\Driver::getInstance()->getRightsManager()->addRightsCheck($securityContext, $parameters, array('ID', 'CREATED_BY'));

		$fileCollection = \Thurly\Disk\File::getModelList($parameters);
	...
	public static function GetMaxFileId($chatId)
	{
		...
		$result = \Thurly\Disk\Internals\ObjectTable::getList(array(
			'select' => array('MAX_ID'),
			'filter' => array(
				'=PARENT_ID' => $folderModel->getId(),
				'=TYPE' => \Thurly\Disk\Internals\ObjectTable::TYPE_FILE
			),
			'runtime' => array(
				'MAX_ID' => array(
					'data_type' => 'integer',
	...
	public static function GetFiles($chatId, $fileId = false, $checkPermission = true)
	{
		...
		{
			$securityContext = \Thurly\Disk\Driver::getInstance()->getFakeSecurityContext();
		}
	...
	public static function GetFileParams($chatId, $fileModel)
	{
		...
		if ($fileModel instanceof \Thurly\Disk\File)
		{
		}
		else if (intval($fileModel) > 0)
		{
			$fileModel = \Thurly\Disk\File::getById($fileModel);
		}
		else
		{
			return false;
		...
		/** @var \Thurly\Disk\File $fileModel */
		$isImage = \Thurly\Disk\TypeFile::isImage($fileModel->getName());
		$imageParams = null;
		if ($isImage)
		{
	...
	public static function Enabled()
	{
		...
		if (!\Thurly\Disk\Driver::isSuccessfullyConverted())
			return false;
	...
	public static function GetFolderModel($chatId)
	{
		...
		if ($folderId > 0)
		{
			$folderModel = \Thurly\Disk\Folder::getById($folderId);
	...
	public static function GetLocalDiskMolel()
	{
		...
		$storageModel = \Thurly\Disk\Driver::getInstance()->getStorageByUserId(self::GetUserId());
		if (!$storageModel)
		{
			return false;
		}
		$folderModel = \Thurly\Disk\Folder::load(array(
			'STORAGE_ID' => $storageModel->getId(),
			'PARENT_ID' => $storageModel->getRootObjectId(),
			'TYPE' => \Thurly\Disk\Internals\ObjectTable::TYPE_FOLDER,
			'CODE' => 'IM_SAVED',
		));
		if (!$folderModel)
		{
			...
			if (!$folderModel)
			{
				if ($storageModel->getErrorByCode(\Thurly\Disk\Folder::ERROR_NON_UNIQUE_NAME))
				{
					$badFileModel = \Thurly\Disk\File::load(array(
                        'STORAGE_ID' => $storageModel->getId(),
                        'TYPE' => \Thurly\Disk\Internals\ObjectTable::TYPE_FILE,
                        'NAME' => $folderName,
                    ));
                    if($badFileModel)
                    {
                        $badFileModel->delete(\Thurly\Disk\SystemUser::SYSTEM_USER_ID);
	...
	public static function GetPublicPath($type, \Thurly\Disk\File $fileModel)
	{
		...
		$isShow = in_array($type, Array(self::PATH_TYPE_SHOW, self::PATH_TYPE_PREVIEW)) && \Thurly\Disk\TypeFile::isImage($fileModel->getName());
		...
		$url = Array(
			'default' => '/thurly/components/thurly/im.messenger/'.($isShow? 'show.file.php?': 'download.file.php?')
		);
	...
	public static function RemoveTmpFileAgent()
	{
		...
		$fileModels = \Thurly\Disk\File::getModelList(Array(
			'filter' => Array(
		...
		{
			$fileModel->delete(\Thurly\Disk\SystemUser::SYSTEM_USER_ID);
		}
	...
	public static function EnabledExternalLink()
	{
		...
		return \Thurly\Disk\Configuration::isEnabledExternalLink();
	}
	...
	public static function SetEnabledExternalLink($flag = true)
	{
		...
		\Thurly\Main\Config\Option::set('disk', 'disk_allow_use_external_link', $flag? 'Y': 'N');
