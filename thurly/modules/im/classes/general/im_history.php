...
class CIMHistory
{
	...
	function SearchMessage($searchText, $toUserId, $fromUserId = false, $bTimeZone = true)
	{
		...
		{
			$op = "*%";
			if (\Thurly\Main\Search\Content::canUseFulltextSearch($searchText))
			{
				if (\Thurly\Im\Model\MessageTable::getEntity()->fullTextIndexEnabled("MESSAGE"))
				{
					$op = "*";
				}
				else
			...
			$orm = \Thurly\Im\Model\MessageTable::getList(Array(
				'select' => Array(
	...
	function SearchDateMessage($searchDate, $toUserId, $fromUserId = false, $bTimeZone = true)
	{
		...
		$sqlHelper = Thurly\Main\Application::getInstance()->getConnection()->getSqlHelper();

		try
		{
			$dateStart = \Thurly\Main\Type\DateTime::createFromUserTime($searchDate);
	...
	function GetMoreChatMessage($pageId, $chatId, $bTimeZone = true)
	{
		...
		else if ($chatData['TYPE'] == IM_MESSAGE_OPEN_LINE && \Thurly\Main\Loader::includeModule('imopenlines') && \Thurly\ImOpenLines\Config::canJoin($chatId))
		{
			$strCountSql ="
			SELECT COUNT(M.ID) as CNT
			FROM b_im_message M
		...
		if ($cnt > 0 && ceil($cnt/20) >= $iNumPage)
		{
			...
			while ($arRes = $dbRes->Fetch())
			{
				if ($arRes['CHAT_ENTITY_TYPE'] != 'LIVECHAT' && \Thurly\Im\User::getInstance($this->user_id)->isConnector())
				{
					return false;
				}
				$arMessages[$arRes['ID']] = Array(
	...
	public function GetMessagesByDate($chatId, $date, $messageStart = 0, $timezone = true)
	{
		...
		{
			$checkDate = new \Thurly\Main\Type\DateTime($date, "Y-m-d H:i:s");
			$dateSql = $checkDate->format("Y-m-d H:i:s");
			if ($dateSql != $date)
			{
				return false;
		...
		while ($message = $result->Fetch())
		{
			...
			$arMessages[$message['ID']] = Array(
				...
				'date' => \Thurly\Main\Type\DateTime::createFromTimestamp($message['DATE_CREATE']),
				'system' => $message['AUTHOR_ID'] > 0? 'N': 'Y',
				'text' => \Thurly\Im\Text::parse($message['MESSAGE'])
			);
	...
	public function GetRelatedMessages($messageId, $previous = 10, $next = 10, $timezone = true, $textParser = true)
	{
		...
		foreach ($messages as $mess)
		{
			...
			$arMessages[$mess['ID']] = Array(
				...
				'recipientId' => $recipientId,
				'date' => \Thurly\Main\Type\DateTime::createFromTimestamp($mess['DATE_CREATE']),
				'system' => $mess['AUTHOR_ID'] > 0? 'N': 'Y',
				'text' => $textParser? \Thurly\Im\Text::parse($mess['MESSAGE']): $mess['MESSAGE']
			);
	...
	private function GetPreviousMessages($messageId, $chatId = null, $dateCreate = null, $limit = 10, $timezone = true)
	{
		...
		{
			$message = \Thurly\Im\Model\MessageTable::getList(Array('select' => Array('DATE_CREATE', 'CHAT_ID'), 'filter' => Array('=ID' => $messageId)))->fetch();
			if (!$message)
				return false;
