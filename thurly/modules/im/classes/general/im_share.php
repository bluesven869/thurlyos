...
class CIMShare
{
	...
	public function Chat($messageId)
	{
		...
		if (!\Thurly\Im\User::getInstance($message['AUTHOR_ID'])->isExtranet())
		{
		...
		if ($message['MESSAGE_TYPE'] == IM_MESSAGE_PRIVATE)
		{
			...
			$relations = \Thurly\Im\Chat::getRelation($parentChatId);
			...
			$chatLink = '[USER='.$dialogId.']'.Thurly\Im\User::getInstance($dialogId)->getFullName(false)."[/USER]";
		}
		else
		{
	...
	public function Task($messageId, $date = '')
	{
		...
		$task = new \Thurly\Tasks\Item\Task(0, $this->user_id);
		...
		if (
			...
			&& !\Thurly\Im\User::getInstance($message['AUTHOR_ID'])->isExtranet()
			&& !\Thurly\Im\User::getInstance($message['AUTHOR_ID'])->isBot()
		)
		{
		...
		{
			$chat = \Thurly\Im\Model\ChatTable::getById($message['CHAT_ID'])->fetch();
		...
		if ($date > 0)
		{
			$task['DEADLINE'] = Thurly\Main\Type\DateTime::createFromTimestamp($date);
		}
		else
		{
			$results = \Thurly\Main\Text\DateConverter::decode(CTextParser::clearAllTags($message['MESSAGE']), 1000);
			if (!empty($results))
			{
	...
	public function Calendar($messageId, $date = '')
	{
		...
		$dateFrom = new Thurly\Main\Type\DateTime();
		$dateFrom->add('1 DAY');

		$date = intval($date);
		if ($date > 0)
		{
			$dateFrom = Thurly\Main\Type\DateTime::createFromTimestamp($date);
		}
		else
		{
			$results = \Thurly\Main\Text\DateConverter::decode(CTextParser::clearAllTags($message['MESSAGE']), 1000);
	...
	private function PrepareText($quoteMessage)
	{
		...
		{
			$chat = \Thurly\Im\Model\ChatTable::getById($quoteMessage['CHAT_ID'])->fetch();
			if ($chat)
			{
		...
		$userName = \Thurly\Im\User::getInstance($quoteMessage['AUTHOR_ID'])->getFullName(false);
		$result .= "[B]".$userName."[/B]\n";
