...
class CIMNotify
{
	...
	public static function GetFormatNotify($arFields)
	{
		...
		$arNotify = Array(
			...
			'date' => \Thurly\Main\Type\DateTime::createFromTimestamp($arFields['DATE_CREATE']),
	...
	public function MarkNotifyRead($id = 0, $setThisAndHigher = false, $appId = 'ThurlyOS')
	{
		global $DB;
		...
		if (CModule::IncludeModule("pull"))
		{
			...
			\Thurly\Pull\MobileCounter::send($this->user_id, $appId);
		}
	...
	public function MarkNotifyReadBySubTag($subTagList = array())
	{
		...
		if (!empty($messages))
		{
			...
			{
				\Thurly\Im\Counter::clearCache($users[$chatId]);
				CIMMessenger::SpeedFileDelete($users[$chatId], IM_SPEED_NOTIFY);

				if ($isLoadPull)
				{
					\Thurly\Pull\Event::add($users[$chatId], Array(
						'module_id' => 'im',
						'command' => 'readNotifyList',
						'params' => Array(
							'chatId' => $chatId,
	...
	public function MarkNotifyUnRead($id = 0, $setThisAndHigher = false, $appId = 'ThurlyOS')
	{
		global $DB;
	...
	public function Answer($id, $text)
	{
		...
		$mention = \Thurly\Im\User::getInstance($arRes['AUTHOR_ID']);
		if ($mention->isExists())
		{
	...
	public static function Delete($ID)
	{
		...
		\Thurly\Im\Model\MessageTable::delete($ID);
		...
		\Thurly\Im\Counter::clearCache($arRes['RELATION_USER_ID']);
		...
		if (CModule::IncludeModule("pull"))
		{
			\Thurly\Pull\Event::add($arRes['RELATION_USER_ID'], Array(
				'module_id' => 'im',
	...
	public static function DeleteByTag($notifyTag, $authorId = false)
	{
		...
		if ($pullActive)
		{
			\Thurly\Pull\Event::add(array_keys($arUsers), Array(
				'module_id' => 'im',
				'command' => 'deleteNotifies',
				'params' => Array(
					'id' => $messages
	...
	public static function ConfirmBySubTag($notifySubTag, $resultMessages)
	{
		...
		if (count($messages) > 0)
		{
			foreach ($messages as $messageId => $message)
			{
				...
				if ($pullActive)
				{
					\Thurly\Pull\Event::add($messages[$messageId]['USER_ID'], Array(
						'module_id' => 'im',
						'command' => 'confirmNotify',
						'params' => Array(
							'id' => $messageId,
	...
	{
		return \Thurly\Im\Notify::getCounterByChatId($chatId);
	}

	public static function GetCounters($chatIds)
	{
		return \Thurly\Im\Notify::getCountersByChatId($chatIds);
	}
}
?>