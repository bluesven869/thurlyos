...
class CIMEvent
{
	...
	public static function GetMessageRatingVote($arParams, $bForMail = false)
	{
		...
		foreach(\Thurly\Main\EventManager::getInstance()->findEventHandlers("im", "OnGetMessageRatingVote") as $event)
		{
	...
	public static function OnAfterUserAdd($arParams)
	{
		...
		{
			if (
				!\Thurly\Im\User::getInstance($arParams["ID"])->isBot()
				&& !\Thurly\Im\User::getInstance($arParams["ID"])->isConnector()
				&& !\Thurly\Im\User::getInstance($arParams["ID"])->isNetwork()
			)
			{
			...
			if (\Thurly\Im\User::getInstance($arParams["ID"])->isBot())
				return true;
		...
		if ($commonChatId > 0 && (isset($arParams['ACTIVE']) || isset($arParams['UF_DEPARTMENT'])))
		{
			...
			else
			{
				...
				if ($commonChatId)
				{
					...
					if (!\Thurly\Im\User::getInstance($arParams["ID"])->isActive())
						return true;
	...
	public static function OnUserDelete($ID)
	{
		...
		\Thurly\Im\Bot::unRegister(Array('BOT_ID' => $ID));
...
class DesktopApplication extends Thurly\Main\Authentication\Application
{
	protected $validUrls = array(
		...
		"/thurly/components/thurly/imopenlines.iframe.quick/ajax.php",
	);
