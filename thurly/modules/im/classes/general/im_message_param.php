<?
use Thurly\Im as IM;
use Thurly\Main\UrlPreview as UrlPreview;
...
class CIMMessageParam
{
	public static function Set($messageId, $params = Array())
	{
		...
		foreach($params as $k1 => $v1)
		{
			...
			if(strlen($name))
			{
				if(is_object($v1) && $v1 instanceof \Thurly\Im\Bot\Keyboard)
				{
					$v1 = array($v1);
				}
				else if(is_object($v1) && $v1 instanceof \Thurly\Im\Bot\ContextMenu)
				{
					$v1 = array($v1);
				}
				...
				else if(is_object($v1) && $v1 instanceof \Thurly\Main\Type\DateTime)
				{
					$v1 = array($v1->getTimestamp());
				}
				else if(is_array($v1) && \Thurly\Main\Type\Collection::isAssociative($v1))
				{
					$v1 = array($v1);
				}
				else if (!is_array($v1))
				...
				else
				{
					foreach($v1 as $v2)
					{
						if (is_array($v2))
						{
							$value = \Thurly\Main\Web\Json::encode($v2);
						...
						else if(is_object($v2) && ($v2 instanceof \Thurly\Im\Bot\Keyboard || $v2 instanceof \Thurly\Im\Bot\ContextMenu))
						{
							$value = $v2->getJson();
							if(strlen($value))
							{
	...
	public static function UpdateTimestamp($messageId, $chatId = 0)
	{
		...
		if ($chatId <= 0)
		{
			$message = \Thurly\Im\Model\MessageTable::getById($messageId)->fetch();
			if ($message)
			{
	...
	public static function Get($messageId, $paramName = false, $withDefault = false)
	{
		...
		while($ar = $messageParameters->fetch())
		{
			if (strlen($ar["PARAM_JSON"]))
			{
				$value = \Thurly\Main\Web\Json::decode($ar["PARAM_JSON"]);
			}
			else
			{
	...
	public static function PrepareValues($values, $withDefault = false)
	{
		...
		foreach($values as $key => $value)
		{
			...
			{
				if (is_object($value[0]) && $value[0] instanceof \Thurly\Main\Type\DateTime)
				{
					$arValues[$key] = $value[0];
				}
				else
				{
					$arValues[$key] = \Thurly\Main\Type\DateTime::createFromTimestamp(intval($value[0]));
				}
			}
...
class CIMMessageParamAttach
{
	...
	function __construct($id = null, $color = null)
	{
		...
		else if ($color != self::CHAT)
		{
			...
			{
				$color = Thurly\Im\Color::getRandomColor();
			}
	...
	{
		return \Thurly\Im\Text::parse($message, Array('SAFE' => 'N'));
	}
	...
	public static function GetAttachByJson($array)
	{
		...
		foreach ($blocks as $data)
		{
			if (isset($data['USER']))
			{
				if (is_array($data['USER']) && !\Thurly\Main\Type\Collection::isAssociative($data['USER']))
				{
			...
			{
				if (is_array($data['LINK']) && !\Thurly\Main\Type\Collection::isAssociative($data['LINK']))
				{
			...
			{
				if (is_array($data['RICH_LINK']) && !\Thurly\Main\Type\Collection::isAssociative($data['RICH_LINK']))
				{
			...
			{
				if (is_array($data['IMAGE']) && \Thurly\Main\Type\Collection::isAssociative($data['IMAGE']))
				{
			...
			{
				if (is_array($data['FILE']) && \Thurly\Main\Type\Collection::isAssociative($data['FILE']))
				{
	...
	public static function PrepareAttach($attach)
	{
		...
		$isCollection = true;
		if(\Thurly\Main\Type\Collection::isAssociative($attach))
		{
			$isCollection = false;
			$attach = array($attach);
		}
	...
	{
		$result = \Thurly\Main\Web\Json::encode($this->result);
