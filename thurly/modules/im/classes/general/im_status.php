...
class CIMStatus
{
				...
				'params' => Array(
					'users' => Array(
						$userId => Array(
							...
							'color' => $status['COLOR']? \Thurly\Im\Color::getColor($status['COLOR']): '',
							'idle' => $status['IDLE'] instanceof \Thurly\Main\Type\DateTime? date('c', $status['IDLE']->getTimestamp()): false,
							'mobile_last_date' => $status['MOBILE_LAST_DATE'] instanceof \Thurly\Main\Type\DateTime? date('c', $status['MOBILE_LAST_DATE']->getTimestamp()): false,
	...
	public static function GetList($params = Array())
	{
		...
		if ($loadFromDb)
		{
			...
			while ($user = $orm->fetch())
			{
				...
				$user['IDLE'] = $user['IDLE'] instanceof \Thurly\Main\Type\DateTime? $user['IDLE']: false;
				$user['MOBILE_LAST_DATE'] = $user['MOBILE_LAST_DATE'] instanceof \Thurly\Main\Type\DateTime? $user['MOBILE_LAST_DATE']: false;
				$user['LAST_ACTIVITY_DATE'] = $user['LAST_ACTIVITY_DATE'] instanceof \Thurly\Main\Type\DateTime? $user['LAST_ACTIVITY_DATE']: false;
	...
	public static function OnUserOnlineStatusGetCustomStatus($userId, $lastseen, $now, $mode)
	{
		...
		/** @var \Thurly\Main\Type\DateTime $mobileLastDate */
		...
		/** @var \Thurly\Main\Type\DateTime $idleDate */
