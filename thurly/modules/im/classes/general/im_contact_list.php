...
class CAllIMContactList
{
	...
	function GetList($arParams = Array())
	{
		...
		$groups = \Thurly\Im\Integration\Socialnetwork\Extranet::getGroup(Array('CURRENT_USER' => $this->user_id));
		if (is_array($groups))
		{
		...
		foreach (\Thurly\Main\UserTable::getExternalUserTypes() as $authId)
		{
			if ($authId == \Thurly\Im\Bot::EXTERNAL_AUTH_ID)
				continue;
			...
			else
			{
				...
				while ($arUser = $orm->fetch())
				{
					...
					if (!$skipUser)
					{
						...
						$arUser['PERSONAL_BIRTHDAY'] = $arUser['PERSONAL_BIRTHDAY'] instanceof \Thurly\Main\Type\Date? $arUser['PERSONAL_BIRTHDAY']->format('d-m'): false;
						$arUser['IDLE'] = $arUser['IDLE'] instanceof \Thurly\Main\Type\DateTime? $arUser['IDLE']: false;
						$arUser['MOBILE_LAST_DATE'] = $arUser['MOBILE_LAST_DATE'] instanceof \Thurly\Main\Type\DateTime? $arUser['MOBILE_LAST_DATE']: false;
						$arUser['LAST_ACTIVITY_DATE'] = $arUser['LAST_ACTIVITY_DATE'] instanceof \Thurly\Main\Type\DateTime? $arUser['LAST_ACTIVITY_DATE']: false;
						...
						$arUsers[$arUser["ID"]] = Array(
							...
							'color' => $color,
							'avatar' => empty($arFileTmp['src'])? '/thurly/js/im/images/blank.gif': $arFileTmp['src'],
							...
							'network' => $arUser['EXTERNAL_AUTH_ID'] == self::NETWORK_AUTH_ID || $arUser['EXTERNAL_AUTH_ID'] == \Thurly\Im\Bot::EXTERNAL_AUTH_ID && $bots[$arUser["ID"]]['TYPE'] == \Thurly\Im\Bot::TYPE_NETWORK,
							'bot' => $arUser['EXTERNAL_AUTH_ID'] == \Thurly\Im\Bot::EXTERNAL_AUTH_ID,
	...
	function SearchUsers($searchText)
	{
		...
		$filter = \Thurly\Main\UserUtils::getUserSearchFilter(Array('FIND'=> $searchText));
		$filter["=ACTIVE"] = "Y";
			...
			if ($result)
			{
				foreach ($result as $arLine)
				{
					...
					$arUsers[$id] = Array(
						...
						'avatar' => empty($arLine['LINE_AVATAR'])? '/thurly/js/im/images/blank.gif': $arLine['LINE_AVATAR'],
						'birthday' => false,
						'gender' => 'M',
						'phone_device' => false,
						'tz_offset' => 0,
				...
				if ($result)
				{
					...
					foreach ($result as $arUser)
					{
						...
						$arUsers[$id] = Array(
							...
							'color' => IM\Color::getColor('GRAY'),
							'avatar' => empty($arUser['PERSONAL_PHOTO'])? '/thurly/js/im/images/blank.gif': $arUser['PERSONAL_PHOTO'],
							'birthday' => false,
							'gender' => $arUser['PERSONAL_GENDER'] == 'F'? 'F': 'M',
							'phone_device' => false,
							'tz_offset' => 0,
							...
							'network' => true,
							'bot' => false,
							'profile' => 'https://www.thurlyos.net/id'.$arUser['NETWORK_USER_ID'],
							'select' => 'Y',
					...
					if (!empty($arUserIds))
					{
						$result = \Thurly\Main\UserTable::getList(Array(
							'select' => Array('XML_ID'),
							'filter' => Array(
								'=XML_ID' => array_keys($arIntersectUserIds),
								'=EXTERNAL_AUTH_ID' => \Thurly\Socialservices\Network::EXTERNAL_AUTH_ID
							),
						));
	...
	static function GetUserData($arParams = Array())
	{
			...
			if($obCache->InitCache($cache_ttl, $cache_id, $cache_dir))
			{
				...
				foreach ($arCacheResult['users'] as $userId => $value)
				{
					...
					if ($getHrPhoto && !isset($arCacheResult['hrphoto']))
					{
						...
						$arCacheResult['hrphoto'][$userId] = empty($arPhotoHrTmp['src'])? '/thurly/js/im/images/hidef-avatar-v3.png': $arPhotoHrTmp['src']; // TODO REMOVE DEFAULT
					}
				}
		...
		$query = new \Thurly\Main\Entity\Query(\Thurly\Main\UserTable::getEntity());

		$query->registerRuntimeField('', new \Thurly\Main\Entity\ReferenceField('ref', 'Thurly\Im\Model\StatusTable', array('=this.ID' => 'ref.USER_ID')));
		...
		while ($arUser = $resultQuery->fetch())
		{
			...
			$arUsers[$arUser["ID"]] = Array(
				...
				'work_position' => $arUser['WORK_POSITION'],
				'color' => $color,
				'avatar' => empty($arPhotoTmp['src'])? '/thurly/js/im/images/blank.gif': $arPhotoTmp['src'],
			...
			if ($getHrPhoto)
			{
				...
				$arHrPhoto[$arUser["ID"]] = empty($arPhotoHrTmp['src'])? '/thurly/js/im/images/hidef-avatar-v3.png': $arPhotoHrTmp['src']; // TODO REMOVE DEFAULT
			}
	...
	public static function InRecent($userId, $type, $itemId)
	{
		...
		$messageId = false;
		$result = \Thurly\Im\Model\RecentTable::getList(Array(
			'filter' => Array(
				'=USER_ID' => $userId,
				'=ITEM_TYPE' => $type,
				'=ITEM_ID' => $itemId,
	...
	public static function SetRecentForNewUser($userId)
	{
		...
		$colleagues = \Thurly\Im\Department::getColleagues($userId);
	...
	public static function SetRecent($arParams)
	{
		...
		$connection = \Thurly\Main\Application::getInstance()->getConnection();
		...
		$merge = $connection->getSqlHelper()->prepareMerge(
			...
			array(
				...
				'ITEM_RID' => $relationId,
				'ITEM_OLID' => $sessionId,
				'DATE_UPDATE' => new \Thurly\Main\Type\DateTime(),
			),
			array(
				'ITEM_MID' => $messageId,
				'ITEM_CID' => $chatId,
		...
		{
			$event = new \Thurly\Main\Event("im", "OnAfterRecentAdd", array(
				"user_id" => $userId,
			));
			$event->send();
		}
	...
	public static function DeleteRecent($entityId, $isChat = false, $userId = false)
	{
		...
		if (!$rs->Fetch())
		{
			$event = new \Thurly\Main\Event("im", "OnAfterRecentDelete", array(
				"user_id" => $userId,
			));
			$event->send();
		}
	...
	public static function DialogHide($dialogId, $userId = null)
	{
		...
		$pullInclude = \Thurly\Main\Loader::includeModule("pull");
	...
	public static function GetRecentList($arParams = Array())
	{
		...
		$isOperator = \Thurly\Im\Integration\Imopenlines\User::isOperator();
			...
			while ($arRes = $dbRes->GetNext(true, false))
			{
				...
				$item = Array(
					...
					'MESSAGE' => Array(
						...
						'date' => \Thurly\Main\Type\DateTime::createFromTimestamp($arRes['M_DATE_CREATE']),
						'text' => \Thurly\Im\Text::parse($arRes['M_MESSAGE'], Array('CUT_STRIKE' => 'Y', 'SMILES' => 'N', 'SAFE' => 'N')),
				...
				if ($arRes['ITEM_TYPE'] == IM_MESSAGE_PRIVATE)
				{
					...
					$arRes['PERSONAL_BIRTHDAY'] = $arRes['PERSONAL_BIRTHDAY']? \Thurly\Main\Type\DateTime::createFromTimestamp($arRes['PERSONAL_BIRTHDAY'])->format('d-m'): false;
					$arRes['IDLE'] = $arRes['IDLE']? \Thurly\Main\Type\DateTime::createFromTimestamp($arRes['IDLE']): false;
					$arRes['MOBILE_LAST_DATE'] = $arRes['MOBILE_LAST_DATE']? \Thurly\Main\Type\DateTime::createFromTimestamp($arRes['MOBILE_LAST_DATE']): false;
					$arRes['LAST_ACTIVITY_DATE'] = $arRes['LAST_ACTIVITY_DATE']? \Thurly\Main\Type\DateTime::createFromTimestamp($arRes['LAST_ACTIVITY_DATE']): false;
					...
					$item['USER'] = Array(
						...
						'extranet' => false,
						'network' => $arRes['EXTERNAL_AUTH_ID'] == self::NETWORK_AUTH_ID || $arRes['EXTERNAL_AUTH_ID'] == \Thurly\Im\Bot::EXTERNAL_AUTH_ID && $bots[$arRes["ITEM_ID"]]['TYPE'] == \Thurly\Im\Bot::TYPE_NETWORK,
						'bot' => $arRes['EXTERNAL_AUTH_ID'] == \Thurly\Im\Bot::EXTERNAL_AUTH_ID,
	...
	public static function formatAbsentResult($userId)
	{
		...
		if (method_exists('\Thurly\Intranet\UserAbsence', 'isAbsentOnVacation'))
		{
			$result = \Thurly\Intranet\UserAbsence::isAbsentOnVacation($userId, true);
		}
		else
		{
			$result = \Thurly\Intranet\UserAbsence::isAbsent($userId, true);
		}

		if ($result)
		{
			$result = \Thurly\Main\Type\DateTime::createFromTimestamp($result['DATE_TO_TS']+86400);
		}
		else
		{
			$result = false;
