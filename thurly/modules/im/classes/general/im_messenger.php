...
class CIMMessenger
{
	...
	*/
	public static function Add($arFields)
	{
		...
		{
			$results = \Thurly\Im\Text::getDateConverterParams($arFields['MESSAGE']);
		...
		if ($arFields['MESSAGE_TYPE'] == IM_MESSAGE_PRIVATE)
		{
			...
			if ($chatId > 0)
			{
				...
				{
					$arParams['DATE_CREATE'] = new Thurly\Main\Type\DateTime($arFields['MESSAGE_DATE']);
				}
				$arFiles = Array();
				...
				$arParams['MESSAGE'] = \Thurly\Im\Text::prepareBeforeSave($arParams['MESSAGE']);
				...
				IM\Model\ChatTable::update($chatId, Array(
					'MESSAGE_COUNT' => new \Thurly\Main\DB\SqlExpression('?# + 1', 'MESSAGE_COUNT'),
				...
				if ($chatData['PARENT_MID'])
				{
					...
					CIMMessageParam::set($chatData['PARENT_MID'], Array(
						...
						'CHAT_LAST_DATE' => new \Thurly\Main\Type\DateTime()
					));
				...
				if (!$bConvert)
				{
					$relations = \Thurly\Im\Chat::getRelation($chatId, Array(
						'REAL_COUNTERS' => 'Y',
						'USER_DATA' => 'Y',
					));
					foreach ($relations as $id => $relation)
					{
						...
						else
						{
							...
							IM\Model\RelationTable::update($relation["ID"], array(
								...
								"LAST_READ" => new Thurly\Main\Type\DateTime(),
							));
						}
						\Thurly\Im\Counter::clearCache($relation['USER_ID']);
					}
					...
					if (CModule::IncludeModule("pull"))
					{
						...
						\Thurly\Pull\Event::add($arParams['FROM_USER_ID'], $pullMessageFrom);

						if ($arParams['FROM_USER_ID'] != $arParams['TO_USER_ID'])
						{
							$pullMessageTo['params']['counter'] = $relations[$arParams['TO_USER_ID']]['COUNTER'];
							\Thurly\Pull\Event::add($arParams['TO_USER_ID'], $pullMessageTo);
							...
							if ($arFields['PUSH'] != 'N')
							{
								...
								\Thurly\Pull\Push::add($arParams['TO_USER_ID'], $pushParams);
							}
						}
					...
					$result = \Thurly\Im\Command::onCommandAdd(intval($messageID), $arFields);
					if (!$result)
					{
						\Thurly\Im\Bot::onMessageAdd(intval($messageID), $arFields);
					}
				}
		...
		else if ($arFields['MESSAGE_TYPE'] == IM_MESSAGE_CHAT || $arFields['MESSAGE_TYPE'] == IM_MESSAGE_OPEN || $arFields['MESSAGE_TYPE'] == IM_MESSAGE_OPEN_LINE)
		{
			...
			if ($chatId > 0)
			{
				...
				foreach ($relations as $id => $relation)
				{
					...
					if ($relation['USER_DATA']["EXTERNAL_AUTH_ID"] == \Thurly\Im\Bot::EXTERNAL_AUTH_ID)
					{
		...
		else if ($arFields['MESSAGE_TYPE'] == IM_MESSAGE_SYSTEM)
		{
			...
			$orm = \Thurly\Main\UserTable::getById($arFields['TO_USER_ID']);
			$userData = $orm->fetch();
			...
			if ($chatId > 0)
			{
				...
				\Thurly\Im\Counter::clearCache($arFields['TO_USER_ID']);
				...
				if (CModule::IncludeModule("pull"))
				{
					\Thurly\Pull\Push::add($arFields['TO_USER_ID'], Array(
					...
					if ($sendToSite)
					{
						\Thurly\Pull\Event::add($arFields['TO_USER_ID'], Array(
							'module_id' => 'im',
							'command' => 'notify',
	...
	public static function CheckPossibilityUpdateMessage($type, $id, $userId = null)
	{
		...
		if (CModule::IncludeModule('pull') && CPullOptions::GetNginxStatus())
		{
			...
			if ($message !== false)
			{
				...
				else if (\Thurly\Im\User::getInstance($userId)->isBot() && ($message['AUTHOR_ID'] == 0 || $message['PARAMS']['IS_DELETED'] == 'Y'))
				{}
			...
			if ($enableCheck)
			{
				if ($message['CHAT_ENTITY_TYPE'] == 'LINES' && CModule::IncludeModule('imopenlines'))
				{
					...
					{
						if (!($type == IM_CHECK_UPDATE && in_array($connectorType, \Thurly\ImOpenlines\Connector::getListCanUpdateOwnMessage()))
							&& !($type == IM_CHECK_DELETE && in_array($connectorType, \Thurly\ImOpenlines\Connector::getListCanDeleteOwnMessage())))
						{
							return false;
						}
					}
					...
					{
						if ($type == IM_CHECK_UPDATE || !in_array($connectorType, \Thurly\ImOpenlines\Connector::getListCanDeleteMessage()))
						{
							return false;
						}
					}
	...
	public static function Update($id, $text, $urlPreview = true, $editFlag = true, $userId = null, $byEvent = false)
	{
		...
		if ($urlPreview)
		{
			$results = \Thurly\Im\Text::getDateConverterParams($text);
			foreach ($results as $result)
			{
		...
		$arFields['DATE_MODIFY'] = new \Thurly\Main\Type\DateTime();

		$pullMessage = \Thurly\Im\Text::parse($arFields['MESSAGE']);
		...
		\Thurly\Pull\Event::add(array_keys($relations), Array(
			'module_id' => 'im',
		...
		\Thurly\Im\Bot::onMessageUpdate(intval($id), $arFields);
	...
	public static function Share($id, $type, $date = '')
	{
		...
		if (\Thurly\Im\User::getInstance($chat->user_id)->isExtranet())
		{
			return false;
		}
	...
	public static function Delete($id, $userId = null, $completeDelete = false, $byEvent = false)
	{
		...
		\Thurly\Im\Bot::onMessageDelete(intval($id), $arFields);
		...
		if ($completeDelete)
		{
			...
			\Thurly\Im\Model\MessageTable::delete($message['ID']);

			$relationCounters = \Thurly\Im\Chat::getRelation($message['CHAT_ID'], Array(
			...
			{
				if (
					$relation['USER_DATA']["EXTERNAL_AUTH_ID"] == \Thurly\Im\Bot::EXTERNAL_AUTH_ID
					|| $relation['USER_DATA']['ACTIVE'] == 'N'
				)
				{
					continue;
				}
				\Thurly\Im\Model\RelationTable::update($relation['ID'], Array('COUNTER' => $relation['COUNTER']));
				\Thurly\Im\Counter::clearCache($relation['USER_ID']);
			}

			$result = \Thurly\Im\Model\RecentTable::getList(Array('filter' => Array('=ITEM_MID' => $message['ID'])))->fetchAll();
			if (!empty($result))
			{
				$message = \Thurly\Im\Model\MessageTable::getList(Array(
				...
				if ($message)
				{
					...
					{
						\Thurly\Im\Model\RecentTable::update(Array(
	...
	public static function LinesSessionVote($dialogId, $messageId, $action, $userId = null)
	{
		...
		if (!\Thurly\Im\User::getInstance($userId)->isConnector() && !\Thurly\Im\User::getInstance($dialogId)->isBot())
			return false;
		...
		if ($chat['ENTITY_TYPE'] == 'LIVECHAT')
		{
			...
			{
				\Thurly\ImOpenlines\Session::voteAsUser(intval($message['PARAMS']['IMOL_VOTE']), $action);
			}
		}
		else
		{
	...
	{
		return COption::GetOptionString('thurlyos', 'network', 'N') == 'Y';
	}
	...
	public static function CheckNetwork2()
	{
		...
		$network = new \Thurly\Socialservices\Network();
	...
	{
		if(!\Thurly\Main\Loader::includeModule('voximplant') || !\Thurly\Main\Loader::includeModule('pull'))
			return false;
	...
	{
		if(!\Thurly\Main\Loader::includeModule('voximplant'))
			return false;

		$userPermissions = \Thurly\Voximplant\Security\Permissions::createWithCurrentUser();
		return $userPermissions->canPerform(
			\Thurly\Voximplant\Security\Permissions::ENTITY_CALL,
			\Thurly\Voximplant\Security\Permissions::ACTION_PERFORM,
			\Thurly\Voximplant\Security\Permissions::PERMISSION_CALL_CRM
		);
	}
	...
	public static function CanUserCallUserNumber()
	{
		...
		return $userPermissions->canPerform(
			\Thurly\Voximplant\Security\Permissions::ENTITY_CALL,
			\Thurly\Voximplant\Security\Permissions::ACTION_PERFORM,
			\Thurly\Voximplant\Security\Permissions::PERMISSION_CALL_USERS
		);
	}
	...
	public static function CanUserCallAnyNumber()
	{
		...
		return $userPermissions->canPerform(
			\Thurly\Voximplant\Security\Permissions::ENTITY_CALL,
			\Thurly\Voximplant\Security\Permissions::ACTION_PERFORM,
			\Thurly\Voximplant\Security\Permissions::PERMISSION_ANY
		);
	}
	...
	public static function CanUserPerformCalls()
	{
		...
		$userPermissions = \Thurly\Voximplant\Security\Permissions::createWithCurrentUser();
		return $userPermissions->canPerform(\Thurly\Voximplant\Security\Permissions::ENTITY_CALL, \Thurly\Voximplant\Security\Permissions::ACTION_PERFORM);
	}
	...
	public static function CanInterceptCall()
	{
		...
		return \Thurly\Voximplant\Limits::canInterceptCall();
	}
	...
	public static function GetCallCardRestApps()
	{
		...
		$result = array();
		if(!defined('Thurly\Voximplant\Rest\Helper::PLACEMENT_CALL_CARD'))
			return array();

		$placementId = Thurly\Voximplant\Rest\Helper::PLACEMENT_CALL_CARD;
		$cursor = \Thurly\Rest\PlacementTable::getHandlersList($placementId);
	...
	public static function SetDesktopStatusOnline($userId = null, $cache = true)
	{
		...
		if ($cache && $userId == $USER->GetId())
		{
			...
			if (CModule::IncludeModule("pull"))
			{
				\Thurly\Pull\Event::add($userId, Array(
					'module_id' => 'im',
					'expiry' => 3600,
	...
	public static function GetTemplateJS($arParams, $arTemplate)
	{
		...
		if(!$phoneCanInterceptCall && \Thurly\Main\Loader::includeModule('voximplant'))
		{
			\Thurly\Voximplant\Ui\Helper::initLicensePopups();
		}
		...
		if (CModule::IncludeModule('imopenlines'))
		{
			$crmPath['LEAD'] = \Thurly\ImOpenLines\Crm::getLink('LEAD');
			$crmPath['CONTACT'] = \Thurly\ImOpenLines\Crm::getLink('CONTACT');
			$crmPath['COMPANY'] = \Thurly\ImOpenLines\Crm::getLink('COMPANY');
			$crmPath['DEAL'] = \Thurly\ImOpenLines\Crm::getLink('DEAL');

			$businessUsers = \Thurly\Imopenlines\Limit::getLicenseUsersLimit();

			$olConfig['canDeleteMessage'] = str_replace('.', '_', \Thurly\Imopenlines\Connector::getListCanDeleteMessage());
			$olConfig['canDeleteOwnMessage'] = str_replace('.', '_', \Thurly\Imopenlines\Connector::getListCanDeleteOwnMessage());
			$olConfig['canUpdateOwnMessage'] = str_replace('.', '_', \Thurly\Imopenlines\Connector::getListCanUpdateOwnMessage());

			$olConfig['queue'] = Array();
			foreach (\Thurly\ImOpenLines\Config::getQueueList($USER->GetID()) as $config)
			{
		...
		$sJS = "
			BX.ready(function() {
				BXIM = new BX.IM(BX('bx-notifier-panel'), {
					...
					'colors': ".(IM\Color::isEnabled()? \Thurly\Im\Common::objectEncode(IM\Color::getSafeColorNames()): 'false').",
					'colorsHex': ".\Thurly\Im\Common::objectEncode(IM\Color::getSafeColors()).",
					'mailCount': ".$arTemplate["MAIL_COUNTER"].",
					'notifyCount': ".$arTemplate["NOTIFY_COUNTER"].",
					'messageCount': ".$arTemplate["MESSAGE_COUNTER"].",
					'counters': ".(empty($arTemplate['COUNTERS'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['COUNTERS'])).",
					'ppStatus': ".$ppStatus.",
					...
					'thurlyNetwork': ".(CIMMessenger::CheckNetwork()? 'true': 'false').",
					'thurlyNetwork2': ".(CIMMessenger::CheckNetwork2()? 'true': 'false').",
					'thurlyos': ".(IsModuleInstalled('thurlyos')? 'true': 'false').",
					'thurlyosnet': ".(IsModuleInstalled('b24network')? 'true': 'false').",
					'thurlyIntranet': ".(IsModuleInstalled('intranet')? 'true': 'false').",
					'thurlyXmpp': ".(IsModuleInstalled('xmpp')? 'true': 'false').",
					'thurlyMobile': ".(IsModuleInstalled('mobile')? 'true': 'false').",
					'thurlyOpenLines': ".(IsModuleInstalled('imopenlines')? 'true': 'false').",
					...
					'tooltipShowed': ".\Thurly\Im\Common::objectEncode(CUserOptions::GetOption('im', 'tooltipShowed', array())).",

					'bot': ".(empty($arTemplate['BOT'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate["BOT"])).",
					'textareaIcon': ".(empty($arTemplate['TEXTAREA_ICON'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate["TEXTAREA_ICON"])).",
					'command': ".(empty($arTemplate['COMMAND'])? '[]': \Thurly\Im\Common::objectEncode($arTemplate["COMMAND"])).",

					'smile': ".\Thurly\Im\Common::objectEncode($arTemplate["SMILE"]).",
					'smileSet': ".\Thurly\Im\Common::objectEncode($arTemplate["SMILE_SET"]).",
					'settings': ".\Thurly\Im\Common::objectEncode($arTemplate['SETTINGS']).",
					'settingsNotifyBlocked': ".(empty($arTemplate['SETTINGS_NOTIFY_BLOCKED'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['SETTINGS_NOTIFY_BLOCKED'])).",

					'notify': ".(empty($arTemplate['NOTIFY']['notify'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['NOTIFY']['notify'])).",
					'unreadNotify' : ".(empty($arTemplate['NOTIFY']['unreadNotify'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['NOTIFY']['unreadNotify'])).",
					'flashNotify' : ".(empty($arTemplate['NOTIFY']['flashNotify'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['NOTIFY']['flashNotify'])).",
					'countNotify' : ".intval($arTemplate['NOTIFY']['countNotify']).",
					'loadNotify' : ".($arTemplate['NOTIFY']['loadNotify']? 'true': 'false').",

					'recent': ".(empty($arTemplate['RECENT']) && $arTemplate['RECENT'] !== false? '[]': \Thurly\Im\Common::objectEncode($arTemplate['RECENT'])).",
					'users': ".(empty($arTemplate['CONTACT_LIST']['users'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['CONTACT_LIST']['users'])).",
					'businessUsers': ".($businessUsers === false? false: empty($businessUsers)? '{}': \Thurly\Im\Common::objectEncode($businessUsers)).",
					'groups': ".(empty($arTemplate['CONTACT_LIST']['groups'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['CONTACT_LIST']['groups'])).",
					'userInGroup': ".(empty($arTemplate['CONTACT_LIST']['userInGroup'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['CONTACT_LIST']['userInGroup'])).",
					'chat': ".(empty($arTemplate['CHAT']['chat'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['CHAT']['chat'])).",
					'userInChat': ".(empty($arTemplate['CHAT']['userInChat'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['CHAT']['userInChat'])).",
					'userChatBlockStatus': ".(empty($arTemplate['CHAT']['userChatBlockStatus'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['CHAT']['userChatBlockStatus'])).",
					'userChatOptions': ".\Thurly\Im\Common::objectEncode(CIMChat::GetChatOptions()).",
					'message' : ".(empty($arTemplate['MESSAGE']['message'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['MESSAGE']['message'])).",
					'files' : ".(empty($arTemplate['MESSAGE']['files'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['MESSAGE']['files'])).",
					'showMessage' : ".(empty($arTemplate['MESSAGE']['usersMessage'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['MESSAGE']['usersMessage'])).",
					'unreadMessage' : ".(empty($arTemplate['MESSAGE']['unreadMessage'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['MESSAGE']['unreadMessage'])).",
					'flashMessage' : ".(empty($arTemplate['MESSAGE']['flashMessage'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['MESSAGE']['flashMessage'])).",
					'history' : {},
					...
					'webrtc': {
						...
						'phoneCallCardRestApps': ".\Thurly\Im\Common::objectEncode(self::GetCallCardRestApps()).",
						'phoneLines': ".\Thurly\Im\Common::objectEncode(self::GetTelephonyLines()).",
						'phoneDefaultLineId': '".self::GetDefaultTelephonyLine()."',
						'availableLines': ".\Thurly\Im\Common::objectEncode(self::GetTelephonyAvailableLines())."
					},
					'openlines': ".\Thurly\Im\Common::objectEncode($olConfig).",
					...
					'path' : {'lf' : '".CUtil::JSEscape($pathToLf)."', 'profile' : '".CUtil::JSEscape($arTemplate['PATH_TO_USER_PROFILE'])."', 'profileTemplate' : '".CUtil::JSEscape($arTemplate['PATH_TO_USER_PROFILE_TEMPLATE'])."', 'mail' : '".CUtil::JSEscape($arTemplate['PATH_TO_USER_MAIL'])."', 'im': '".CUtil::JSEscape($pathToIm)."', 'call': '".CUtil::JSEscape($pathToCall)."', 'file': '".CUtil::JSEscape($pathToFile)."', 'crm' : ".\Thurly\Im\Common::objectEncode($crmPath)."}
				});
			});
		";
	...
	public static function GetMobileTemplateJS($arParams, $arTemplate)
	{
		...
		$phoneEnabled = self::CheckPhoneStatus() && CModule::IncludeModule('mobileapp') && \Thurly\MobileApp\Mobile::getInstance()->isWebRtcSupported();
		if ($phoneEnabled && CModule::IncludeModule('voximplant'))
		{
			$phoneSipAvailable = CVoxImplantConfig::GetModeStatus(CVoxImplantConfig::MODE_SIP);
			$phoneDeviceActive = CVoxImplantUser::GetPhoneActive($USER->GetId());
		...
		$sJS = "
			BX.ready(function() {
				BXIM = new BX.ImMobile({
					...
					'smile': ".(empty($arTemplate['SMILE'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate["SMILE"])).",
					'smileSet': ".(empty($arTemplate['SMILE_SET'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate["SMILE_SET"])).",
					'settings': ".(empty($arTemplate['SETTINGS'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['SETTINGS'])).",
					'settingsNotifyBlocked': ".(empty($arTemplate['SETTINGS_NOTIFY_BLOCKED'])? '{}': \Thurly\Im\Common::objectEncode($arTemplate['SETTINGS_NOTIFY_BLOCKED'])).",
	...
	public static function StartWriting($dialogId, $userId = false, $userName = "", $byEvent = false, $linesSilentMode = false)
	{
		...
		if ($userId > 0 && strlen($dialogId) > 0 && CModule::IncludeModule("pull"))
		{
			...
			if (substr($dialogId, 0, 4) == 'chat')
			{
				$orm = \Thurly\Im\Model\ChatTable::getById(substr($dialogId, 4));
				$chat = $orm->fetch();
	...
	public static function SendMention($params)
	{
		...
		$userName = \Thurly\Im\User::getInstance($params['FROM_USER_ID'])->getFullName(false);
		$userGender = \Thurly\Im\User::getInstance($params['FROM_USER_ID'])->getGender();
		...
		{
			$orm = \Thurly\Im\Model\ChatTable::getById($params['CHAT_ID']);
			$chat = $orm->fetch();
			if (!$chat)
			{
				return false;
		...
		if (strlen($pushText1) > 0)
		{
			foreach ($forUsers as $userId)
			{
				...
				\Thurly\Pull\Push::add($userId, $pushParams);
			}
		}
	...
	public static function PrepareParamsForPull($params)
	{
		...
		foreach ($params as $key => $value)
		{
			...
			elseif ($key == 'KEYBOARD')
			{
				if (is_object($value) && $value instanceof \Thurly\Im\Bot\Keyboard)
				{
			...
			elseif ($key == 'MENU')
			{
				if (is_object($value) && $value instanceof \Thurly\Im\Bot\ContextMenu)
				{
					$params[$key] = $value->getArray();
				}
			}
	...
	public static function PreparePushForChat($params)
	{
		...
		$avatarUser = \Thurly\Im\User::getInstance($params['FROM_USER_ID'])->getAvatar();
		if ($avatarUser && strpos($avatarUser, 'http') !== 0)
		{
			$avatarUser = \Thurly\Im\Common::getPublicDomain().$avatarUser;
		}
		$avatarChat = \CIMChat::GetAvatarImage($params['CHAT_AVATAR'], 100, false);
		if ($avatarChat && strpos($avatarChat, 'http') !== 0)
		{
			$avatarChat = \Thurly\Im\Common::getPublicDomain().$avatarChat;
		}
		...
		$advancedParams = array(
			...
			"userFirstName" => (string)\Thurly\Im\User::getInstance($params['FROM_USER_ID'])->getName(false),
			"userLastName" => (string)\Thurly\Im\User::getInstance($params['FROM_USER_ID'])->getLastName(false),
	...
	public static function PreparePushForPrivate($params)
	{
		...
		else
		{
			...
			$advancedParams = array(
				...
				"userFirstName" => (string)\Thurly\Im\User::getInstance($params['FROM_USER_ID'])->getName(false),
				"userLastName" => (string)\Thurly\Im\User::getInstance($params['FROM_USER_ID'])->getLastName(false),
				"userColor" => (string)\Thurly\Im\User::getInstance($params['FROM_USER_ID'])->getColor(),
				"userExtranet" => \Thurly\Im\User::getInstance($params['FROM_USER_ID'])->isExtranet(),
				"messageId" => (int)$params['MESSAGE_ID'],
				"messageDate" => date('c', time()),
				"messageAuthorId" => (int)$params['FROM_USER_ID'],
				"messageText" => (string)$senderMessage,
	...
	{
		return \Thurly\Im\Common::getCacheUserPostfix($id);
	}
	...
	{
		global $USER;
		return $USER->IsAdmin() || CModule::IncludeModule('thurlyos') && CThurlyOS::IsPortalAdmin($USER->GetId());
	}
	...
	private static function GetEventByCounterGroup($events, $maxUserInGroup = 100)
	{
		...
		\Thurly\Main\Type\Collection::sortByColumn($groups, Array('count' => SORT_DESC));
		...
		\Thurly\Main\Type\Collection::sortByColumn($finalGroup, Array('count' => SORT_ASC));
