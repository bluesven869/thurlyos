...
class CIMRestService extends IRestService
{
	public static function OnRestServiceBuildDescription()
	{
		return array(
			...
			'imbot' => Array(
				...
				'imbot.chat.leave' => array(__CLASS__, 'botLeave'),
				...
				CRestUtil::EVENTS => array(
					'OnImBotMessageAdd' => array('im', 'onImBotMessageAdd', array(__CLASS__, 'onBotMessageAdd'), array("category" => \Thurly\Rest\Sqs::CATEGORY_BOT, "sendRefreshToken" => true)),
					'OnImBotJoinChat' => array('im', 'onImBotJoinChat', array(__CLASS__, 'onBotJoinChat'), array("category" => \Thurly\Rest\Sqs::CATEGORY_BOT)),
					'OnImBotDelete' => array('im', 'onImBotDelete', array(__CLASS__, 'onBotDelete'), array("category" => \Thurly\Rest\Sqs::CATEGORY_BOT)),
					'OnImCommandAdd' => array('im', 'onImCommandAdd', array(__CLASS__, 'onCommandAdd'), array("category" => \Thurly\Rest\Sqs::CATEGORY_BOT, "sendRefreshToken" => true)),
				),
			)
		);
	}
	...
	public static function OnRestAppDelete($arParams)
	{
		...
		$result = \Thurly\Rest\AppTable::getList(array('filter' =>array('=ID' => $arParams['APP_ID'])));
		if ($result = $result->fetch())
		{
			$bots = \Thurly\Im\Bot::getListCache();
			foreach ($bots as $bot)
			{
				if ($bot['APP_ID'] == $result['CLIENT_ID'])
				{
					\Thurly\Im\Bot::unRegister(Array('BOT_ID' => $bot['BOT_ID']));
				}
			}
		}
	}
	...
	public static function userGet($arParams, $n, CRestServer $server)
	{
		...
		if (isset($arParams['ID']))
		{
			...
			if ($userId <= 0)
			{
				throw new Thurly\Rest\RestException("User ID can't be empty", "ID_EMPTY", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
		...
		if (!$user->isExists())
		{
			throw new Thurly\Rest\RestException("User is not exists", "USER_NOT_EXISTS", CRestServer::STATUS_WRONG_REQUEST);
		}

		$currentUserId = \Thurly\Im\User::getInstance()->getId();
		$isExtranet = \Thurly\Im\User::getInstance()->isExtranet();

		$extranetUsers = Array($currentUserId => $currentUserId);
		if ($isExtranet)
		{
			$groups = \Thurly\Im\Integration\Socialnetwork\Extranet::getGroup(Array('CURRENT_USER' => $currentUserId));
			if (is_array($groups))
			{
			...
			if (!isset($extranetUsers[$userId]))
			{
				throw new Thurly\Rest\RestException("You can request only users who consist of your extranet group", "ACCESS_DENIED", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
	...
	public static function userList($arParams, $offset, CRestServer $server)
	{
		...
		if (empty($users))
		{
			throw new Thurly\Rest\RestException("A wrong format for the USERS field is passed", "INVALID_FORMAT", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		foreach ($users as $userId)
		{
			...
			{
				$result[$userId] = \Thurly\Im\User::getInstance($userId)->getArray(Array('JSON' => 'Y'));
			}
		}
	...
	public static function searchUser($arParams, $offset, CRestServer $server)
	{
		...
		{
			throw new Thurly\Rest\RestException("Too short a search phrase.", "FIND_SHORT", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		$filter = \Thurly\Im\User::getListFilter($params);
		if (is_null($filter))
		{
			throw new Thurly\Rest\RestException("Too short a search phrase.", "FIND_SHORT", CRestServer::STATUS_WRONG_REQUEST);
		}

		$counter = \Thurly\Main\UserTable::getList(array(
			'filter' => $filter,
			'select' => array("CNT" => new \Thurly\Main\Entity\ExpressionField('CNT', 'COUNT(1)')),
		))->fetch();
		...
		if ($counter && $counter["CNT"] > 0)
		{
			...
			$params['LIMIT'] = intval($arParams['LIMIT']) <= 50? $arParams['LIMIT']: 10;

			$result = \Thurly\Im\User::getList($params);
	...
	public static function searchLastGet($arParams, $n, CRestServer $server)
	{
		...
		return \Thurly\Im\LastSearch::get(null, $config);
	}
	...
	public static function searchLastAdd($arParams, $n, CRestServer $server)
	{
		...
		{
			throw new Thurly\Rest\RestException("Dialog ID can't be empty", "DIALOG_ID_EMPTY", CRestServer::STATUS_WRONG_REQUEST);
		}

		return \Thurly\Im\LastSearch::add($arParams['DIALOG_ID'])? true: false;
	}
	...
	public static function searchLastDelete($arParams, $n, CRestServer $server)
	{
		...
		return \Thurly\Im\LastSearch::delete($arParams['DIALOG_ID']);
	}
	...
	public static function recentGet($arParams, $n, CRestServer $server)
	{
		...
		return \Thurly\Im\Recent::get(null, $config);
	}
	...
	public static function recentPin($arParams, $n, CRestServer $server)
	{
		...
		return \Thurly\Im\Recent::pin($arParams['DIALOG_ID'], $arParams['ACTION'] != 'N');
	}
	...
	public static function recentHide($arParams, $n, CRestServer $server)
	{
		...
		return \Thurly\Im\Recent::hide($arParams['DIALOG_ID']);
	}
	...
	public static function departmentList($arParams, $n, CRestServer $server)
	{
		...
		{
			throw new Thurly\Rest\RestException("A wrong format for the ID field is passed", "INVALID_FORMAT", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (isset($arParams['ID']))
		{
			$list = \Thurly\Im\Department::getStructure($options);
			...
			foreach ($ids as $id)
			{
				...
				if ($list[$id]['MANAGER_USER_DATA'] instanceof \Thurly\Im\User)
				{
		...
		{
			$result = \Thurly\Im\Department::getStructure($options);

			foreach ($result as $id => $department)
			{
				if ($department['MANAGER_USER_DATA'] instanceof \Thurly\Im\User)
				{
	...
	public static function departmentGet($arParams, $n, CRestServer $server)
	{
		...
		if ($id <= 0)
		{
			throw new Thurly\Rest\RestException("Department ID can't be empty", "ID_EMPTY", CRestServer::STATUS_WRONG_REQUEST);
		}

		$list = \Thurly\Im\Department::getStructure();
		if (!isset($list[$id]))
		{
			throw new Thurly\Rest\RestException("Department is not exists", "DEPARTMENT_NOT_EXISTS", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function departmentManagersGet($arParams, $n, CRestServer $server)
	{
		...
		return \Thurly\Im\Department::getManagers(empty($ids)? null: $ids, array('JSON' => 'Y', 'USER_DATA' => $withUserData? 'Y': 'N'));
	}
	...
	public static function departmentEmployeesGet($arParams, $n, CRestServer $server)
	{
		...
		return \Thurly\Im\Department::getEmployees($ids, array('JSON' => 'Y', 'USER_DATA' => $withUserData? 'Y': 'N'));
	}
	...
	public static function departmentColleaguesGet($arParams, $offset, CRestServer $server)
	{
		...
		$result = \Thurly\Im\Department::getColleagues(null, array('JSON' => 'Y', 'USER_DATA' => $withUserData? 'Y': 'N', 'LIST' => $params));
	...
	public static function chatCreate($arParams, $n, CRestServer $server)
	{
		...
		{
			throw new Thurly\Rest\RestException("Please select users before creating a new chat", "USERS_EMPTY", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (!$chatId)
		{
			throw new Thurly\Rest\RestException("Chat can't be created", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function chatSetOwner($arParams, $n, CRestServer $server)
	{
		...
		{
			throw new Thurly\Rest\RestException("Chat ID can't be empty", "CHAT_ID_EMPTY", CRestServer::STATUS_WRONG_REQUEST);
		}

		if ($arParams['USER_ID'] <= 0)
		{
			throw new Thurly\Rest\RestException("User ID can't be empty", "USER_ID_EMPTY", CRestServer::STATUS_WRONG_REQUEST);
		}

		if (CIMChat::GetGeneralChatId() == $arParams['CHAT_ID'])
		{
			throw new Thurly\Rest\RestException("Action unavailable", "ACCESS_ERROR", CRestServer::STATUS_FORBIDDEN);
		}
		...
		if (!$result)
		{
			throw new Thurly\Rest\RestException("Change owner can only owner or user isn't member in chat", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function chatSetManager($arParams, $n, CRestServer $server)
	{
		...
		if (!$result)
		{
			throw new Thurly\Rest\RestException("Change manger can only owner or user isn't member in chat", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function chatUpdateColor($arParams, $n, CRestServer $server)
	{
		...
		if (!Thurly\Im\Color::isSafeColor($arParams['COLOR']))
		{
			throw new Thurly\Rest\RestException("This color currently unavailable", "WRONG_COLOR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (!$result)
		{
			throw new Thurly\Rest\RestException("This color currently set or chat isn't exists", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function chatUpdateTitle($arParams, $n, CRestServer $server)
	{
		...
		{
			throw new Thurly\Rest\RestException("Title can't be empty", "TITLE_EMPTY", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (!$result)
		{
			throw new Thurly\Rest\RestException("This title currently set or chat isn't exists", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function chatUpdateAvatar($arParams, $n, CRestServer $server)
	{
		...
		if (!$arParams['AVATAR'] || strpos($arParams['AVATAR']['type'], "image/") !== 0)
		{
			throw new Thurly\Rest\RestException("Avatar incorrect", "AVATAR_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (!$result)
		{
			throw new Thurly\Rest\RestException("Chat isn't exists", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function chatUserAdd($arParams, $n, CRestServer $server)
	{
		...
		if (!$result)
		{
			throw new Thurly\Rest\RestException("You don't have access or user already member in chat", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function chatUserDelete($arParams, $n, CRestServer $server)
	{
		...
		if (!$result)
		{
			throw new Thurly\Rest\RestException("You don't have access or user isn't member in chat", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function chatMute($arParams, $n, CRestServer $server)
	{
		...
		return \Thurly\Im\Chat::mute($arParams['CHAT_ID'], $arParams['ACTION'] != 'N');
	}
	...
	public static function botLeave($arParams, $n, CRestServer $server)
	{
		...
		if (!$clientId)
		{
			...
			{
				throw new \Thurly\Rest\AccessException("Client ID not specified");
			}
		}
		...
		if (!$botFound)
		{
			throw new \Thurly\Rest\RestException("Bot not found", "BOT_ID_ERROR", \CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	{
		if ($server->getAuthType() == \Thurly\Rest\SessionAuth\Auth::AUTH_TYPE)
		{
			throw new \Thurly\Rest\RestException("Access for this method not allowed by session authorization.", "WRONG_AUTH_TYPE", \CRestServer::STATUS_FORBIDDEN);
		}
		$result = Array();
		...
		foreach ($list as $botId => $botData)
		{
			...
			$result[$botId] = Array(
				'ID' => $botId,
				'NAME' => \Thurly\Im\User::getInstance($botId)->getFullName(),
				'CODE' => $botData['CODE'],
	...
	public static function messageAdd($arParams, $n, CRestServer $server)
	{
		...
		{
			throw new Thurly\Rest\RestException("Message can't be empty", "MESSAGE_EMPTY", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (isset($arParams['USER_ID']))
		{
			...
			if (!Thurly\Im\User::getInstance($arParams['USER_ID'])->isExists())
			{
				throw new Thurly\Rest\RestException("User not found", "USER_NOT_FOUND", CRestServer::STATUS_WRONG_REQUEST);
			}
		...
		else if (isset($arParams['CHAT_ID']))
		{
			...
			if (isset($arParams['SYSTEM']) && $arParams['SYSTEM'] == 'Y')
			{
				...
				if ($clientId)
				{
					$result = \Thurly\Rest\AppTable::getList(
						array(
							'filter' => array(
								'=CLIENT_ID' => $clientId
							),
			...
			else
			{
				...
				{
					throw new Thurly\Rest\RestException("You don't have access or user isn't member in chat", "ACCESS_ERROR", CRestServer::STATUS_WRONG_REQUEST);
				}
			}
		...
		{
			throw new Thurly\Rest\RestException("Incorrect params", "PARAMS_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if ($attach)
		{
			...
			{
				throw new Thurly\Rest\RestException("You have exceeded the maximum allowable size of attach", "ATTACH_OVERSIZE", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
		else if ($arParams['ATTACH'])
		{
			throw new Thurly\Rest\RestException("Incorrect attach params", "ATTACH_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (isset($arParams['KEYBOARD']) && !empty($arParams['KEYBOARD']))
		{
			...
			{
				throw new Thurly\Rest\RestException("Incorrect keyboard params", "KEYBOARD_ERROR", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
		...
		if (isset($arParams['MENU']) && !empty($arParams['MENU']))
		{
			...
			$menu = \Thurly\Im\Bot\ContextMenu::getByJson($menu);
			if ($menu)
			{
			...
			{
				throw new Thurly\Rest\RestException("Incorrect menu params", "MENU_ERROR", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
		...
		if (!$id)
		{
			throw new Thurly\Rest\RestException("Message isn't added", "PARAMS_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function messageDelete($arParams, $n, CRestServer $server)
	{
		...
		if ($arParams['ID'] <= 0)
		{
			throw new Thurly\Rest\RestException("Message ID can't be empty", "MESSAGE_ID_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}

		$res = CIMMessenger::Delete($arParams['ID']);
		if (!$res)
		{
			throw new Thurly\Rest\RestException("Time has expired for modification or you don't have access", "CANT_EDIT_MESSAGE", CRestServer::STATUS_FORBIDDEN);
		}
		...
		if (isset($arParams['KEYBOARD']))
		{
			...
			else
			{
				...
				if ($keyboard)
				{
					...
					{
						throw new Thurly\Rest\RestException("You have exceeded the maximum allowable size of keyboard", "KEYBOARD_OVERSIZE", CRestServer::STATUS_WRONG_REQUEST);
					}
				}
		...
		if (isset($arParams['MENU']))
		{
			...
			else
			{
				...
				if ($menu)
				{
					...
					{
						throw new Thurly\Rest\RestException("You have exceeded the maximum allowable size of menu", "MENU_OVERSIZE", CRestServer::STATUS_WRONG_REQUEST);
					}
				}
	...
	public static function messageLike($arParams, $n, CRestServer $server)
	{
		...
		if ($result === false)
		{
			throw new Thurly\Rest\RestException("Action completed without changes", "WITHOUT_CHANGES", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function notifyAdd($arParams, $n, CRestServer $server)
	{
		...
		if ($arParams['TYPE'] == 'SYSTEM')
		{
			...
			if ($clientId)
			{
				$result = \Thurly\Rest\AppTable::getList(array('filter' => array('=CLIENT_ID' => $clientId)));
				$result = $result->fetch();
	...
	public static function counterGet($arParams, $n, CRestServer $server)
	{
		...
		return \Thurly\Im\Counter::get($USER->GetID());
	}
	...
	public static function botRegister($arParams, $n, CRestServer $server)
	{
		...
		$dbRes = \Thurly\Rest\AppTable::getList(array('filter' => array('=CLIENT_ID' => $clientId)));
		$arApp = $dbRes->fetch();
		...
		if (isset($arParams['EVENT_MESSAGE_ADD']) && !empty($arParams['EVENT_MESSAGE_ADD']))
		{
			...
			try
			{
				Thurly\Rest\EventTable::checkCallback($arParams['EVENT_MESSAGE_ADD'], $arApp);
			}
			catch(Exception $e)
			{
				throw new Thurly\Rest\RestException($e->getMessage(), "EVENT_MESSAGE_ADD_ERROR", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
		else
		{
			throw new Thurly\Rest\RestException("Handler for \"Message add\" event isn't specified", "EVENT_MESSAGE_ADD_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (isset($arParams['EVENT_WELCOME_MESSAGE']) && !empty($arParams['EVENT_WELCOME_MESSAGE']))
		{
			...
			try
			{
				Thurly\Rest\EventTable::checkCallback($arParams['EVENT_WELCOME_MESSAGE'], $arApp);
			}
			catch(Exception $e)
			{
				throw new Thurly\Rest\RestException($e->getMessage(), "EVENT_WELCOME_MESSAGE_ERROR", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
		else
		{
			throw new Thurly\Rest\RestException("Handler for \"Welcome message\" event isn't specified", "EVENT_WELCOME_MESSAGE_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		if (isset($arParams['EVENT_BOT_DELETE']) && !empty($arParams['EVENT_BOT_DELETE']))
		{
			...
			try
			{
				Thurly\Rest\EventTable::checkCallback($arParams['EVENT_BOT_DELETE'], $arApp);
			}
			catch(Exception $e)
			{
				throw new Thurly\Rest\RestException($e->getMessage(), "EVENT_BOT_DELETE_ERROR", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
		else
		{
			throw new Thurly\Rest\RestException("Handler for \"Bot delete\" event isn't specified", "EVENT_BOT_DELETE_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}

		if (!isset($arParams['CODE']) || empty($arParams['CODE']))
		{
			throw new Thurly\Rest\RestException("Bot code isn't specified", "CODE_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}

		$counter = \Thurly\Im\Model\BotTable::getCount(array('=APP_ID' => $clientId));
		if ($counter >= 5)
		{
			throw new Thurly\Rest\RestException("Has reached the maximum number of bots for application (max: 5)", "MAX_COUNT_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		{
			throw new Thurly\Rest\RestException("Bot name isn't specified", "NAME_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		{
			$birthday = new \Thurly\Main\Type\DateTime($arParams['PROPERTIES']['PERSONAL_BIRTHDAY'].' 19:45:00', 'Y-m-d H:i:s');
		...
		{
			throw new Thurly\Rest\RestException("Bot can't be created", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function botUnRegister($arParams, $n, CRestServer $server)
	{
		...
		{
			throw new Thurly\Rest\RestException("Bot was installed by another rest application", "APP_ID_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}

		$result = \Thurly\Im\Bot::unRegister(Array('BOT_ID' => $arParams['BOT_ID']));
		if (!$result)
		{
			throw new Thurly\Rest\RestException("Bot can't be deleted", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function botUpdate($arParams, $n, CRestServer $server)
	{
		...
		if (isset($arParams['FIELDS']['EVENT_MESSAGE_ADD']) && !empty($arParams['FIELDS']['EVENT_MESSAGE_ADD']))
		{
			...
			try
			{
				Thurly\Rest\EventTable::checkCallback($arParams['FIELDS']['EVENT_MESSAGE_ADD'], $arApp);
			}
			catch(Exception $e)
			{
				throw new Thurly\Rest\RestException($e->getMessage(), "EVENT_MESSAGE_ADD_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		...
		if (isset($arParams['FIELDS']['EVENT_WELCOME_MESSAGE']) && !empty($arParams['FIELDS']['EVENT_WELCOME_MESSAGE']))
		{
			...
			try
			{
				Thurly\Rest\EventTable::checkCallback($arParams['FIELDS']['EVENT_WELCOME_MESSAGE'], $arApp);
			}
			catch(Exception $e)
			{
				throw new Thurly\Rest\RestException($e->getMessage(), "EVENT_WELCOME_MESSAGE_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		...
		if (isset($arParams['FIELDS']['EVENT_BOT_DELETE']) && !empty($arParams['FIELDS']['EVENT_BOT_DELETE']))
		{
			...
			try
			{
				Thurly\Rest\EventTable::checkCallback($arParams['FIELDS']['EVENT_BOT_DELETE'], $arApp);
			}
			catch(Exception $e)
			{
				throw new Thurly\Rest\RestException($e->getMessage(), "EVENT_BOT_DELETE_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		...
		{
			$birthday = new \Thurly\Main\Type\DateTime($arParams['FIELDS']['PROPERTIES']['PERSONAL_BIRTHDAY'].' 19:45:00', 'Y-m-d H:i:s');
			$birthday = $birthday->format(\Thurly\Main\Type\Date::convertFormatToPhp(\CSite::GetDateFormat('SHORT')));
		...
		if (empty($updateFields))
		{
			...
			{
				throw new Thurly\Rest\RestException("Update fields can't be empty", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
		else
		{
			$result = \Thurly\Im\Bot::update(Array('BOT_ID' => $arParams['BOT_ID']), $updateFields);
			if (!$result)
			{
				throw new Thurly\Rest\RestException("Bot can't be updated", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
	...
	public static function botMessageAdd($arParams, $n, CRestServer $server)
	{
		...
		$id = \Thurly\Im\Bot::addMessage(array('BOT_ID' => $arParams['BOT_ID']), $arMessageFields);
		if (!$id)
		{
			throw new Thurly\Rest\RestException("Message isn't added", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function botSendTyping($arParams, $n, CRestServer $server)
	{
		...
		\Thurly\Im\Bot::startWriting(Array('BOT_ID' => $arParams['BOT_ID']), $arParams['DIALOG_ID']);
	...
	public static function onCommandAdd($arParams, $arHandler)
	{
		...
		$bot = \Thurly\Im\Bot::getListCache();
		...
		foreach ($arParams[0] as $commandData)
		{
			if ($commandData['APP_ID'] == $arHandler['APP_CODE'] && $commandData['BOT_ID'] > 0)
			{
				...
				if ($commandData['CONTEXT'] != 'KEYBOARD')
				{
					...
					{
						\Thurly\Im\Bot::startWriting(Array('BOT_ID' => $commandData['BOT_ID']), $arParams[2]['DIALOG_ID']);
					}
				}
			}
		}
		...
		$arParams[2]['LANGUAGE'] = \Thurly\Im\Bot::getDefaultLanguage();

		if ($arParams[2]['FROM_USER_ID'] > 0)
		{
			$fromUser = \Thurly\Im\User::getInstance($arParams[2]['FROM_USER_ID'])->getFields();
	...
	public static function onBotMessageAdd($arParams, $arHandler)
	{
		...
		foreach ($arParams[0] as $botData)
		{
			if ($botData['APP_ID'] == $arHandler['APP_CODE'])
			{
				...
				{
					\Thurly\Im\Bot::startWriting(Array('BOT_ID' => $botData['BOT_ID']), $arParams[2]['DIALOG_ID']);
				}
			}
		}
	...
	public static function onBotJoinChat($arParams, $arHandler)
	{
		...
		$params['LANGUAGE'] = \Thurly\Im\Bot::getDefaultLanguage();

		if ($params['USER_ID'] > 0)
		{
			$fromUser = \Thurly\Im\User::getInstance($params['USER_ID'])->getFields();
		...
		\Thurly\Im\Bot::startWriting(Array('BOT_ID' => $params['BOT_ID']), $params['DIALOG_ID']);
	...
	public static function commandRegister($arParams, $n, CRestServer $server)
	{
		...
		if (isset($arParams['EVENT_COMMAND_ADD']) && !empty($arParams['EVENT_COMMAND_ADD']))
		{
			...
			try
			{
				Thurly\Rest\EventTable::checkCallback($arParams['EVENT_COMMAND_ADD'], $arApp);
			}
			catch(Exception $e)
			{
				throw new Thurly\Rest\RestException($e->getMessage(), "EVENT_COMMAND_ADD_ERROR", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
		else
		{
			throw new Thurly\Rest\RestException("Handler for \"Command add\" event isn't specified", "EVENT_COMMAND_ADD_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}

		if (!isset($arParams['COMMAND']) || empty($arParams['COMMAND']))
		{
			throw new Thurly\Rest\RestException("Command isn't specified", "COMMAND_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		{
			throw new Thurly\Rest\RestException("Lang set can't be empty", "LANG_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		{
			throw new Thurly\Rest\RestException("Command can't be created", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function commandUnRegister($arParams, $n, CRestServer $server)
	{
		...
		$commands = \Thurly\Im\Command::getListCache();
		if (!isset($commands[$arParams['COMMAND_ID']]))
		{
			throw new Thurly\Rest\RestException("Command not found", "COMMAND_ID_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		if ($commands[$arParams['COMMAND_ID']]['APP_ID'] != $clientId)
		{
			throw new Thurly\Rest\RestException("Command was installed by another rest application", "APP_ID_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}

		$result = \Thurly\Im\Command::unRegister(Array('COMMAND_ID' => $arParams['COMMAND_ID']));
		if (!$result)
		{
			throw new Thurly\Rest\RestException("Command can't be deleted", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function commandUpdate($arParams, $n, CRestServer $server)
	{
		...
		$bots = \Thurly\Im\Command::getListCache();
		...
		if (isset($arParams['FIELDS']['EVENT_COMMAND_ADD']) && !empty($arParams['FIELDS']['EVENT_COMMAND_ADD']))
		{
			...
			try
			{
				Thurly\Rest\EventTable::checkCallback($arParams['FIELDS']['EVENT_COMMAND_ADD'], $arApp);
			}
			catch(Exception $e)
			{
				throw new Thurly\Rest\RestException($e->getMessage(), "EVENT_COMMAND_ADD_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		...
		{
			$result = \Thurly\Im\Command::update(Array('COMMAND_ID' => $arParams['COMMAND_ID']), $updateFields);
			if (!$result)
			{
				throw new Thurly\Rest\RestException("Command can't be updated", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
			}
		}
	...
	public static function commandAnswer($arParams, $n, CRestServer $server)
	{
		...
		if ($arParams['MESSAGE_ID'] <= 0)
		{
			throw new Thurly\Rest\RestException("Message ID can't be empty", "MESSAGE_ID_EMPTY", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		$id = \Thurly\Im\Command::addMessage(Array(
	...
	public static function appRegister($arParams, $n, CRestServer $server)
	{
		...
		if (!isset($arParams['CODE']) || empty($arParams['CODE']))
		{
			throw new Thurly\Rest\RestException("App code isn't specified", "CODE_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		else if (isset($arParams['IFRAME']) && !empty($arParams['IFRAME']))
		{
			...
			{
				throw new Thurly\Rest\RestException("Iframe params must be HTTPS site", "IFRAME_HTTPS", CRestServer::STATUS_WRONG_REQUEST);
			}
			...
			{
				throw new Thurly\Rest\RestException("Hash can't be empty", "HASH_ERROR", CRestServer::STATUS_WRONG_REQUEST);
			}
		...
		if (!$iframe && !$js)
		{
			throw new Thurly\Rest\RestException("Iframe or JS method isn't specified", "PARAMS_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		...
		$id = \Thurly\Im\App::register(Array(
			'APP_ID' => $clientId,
			'BOT_ID' => $arParams['BOT_ID'],
			'CODE' => $arParams['CODE'],
			'ICON_ID' => $iconId,
		...
		if (!$id)
		{
			throw new Thurly\Rest\RestException("App can't be registered".\var_export(Array(
			'APP_ID' => $clientId,
			'BOT_ID' => $arParams['BOT_ID'],
			'CODE' => $arParams['CODE'],
			'ICON_ID' => $iconId,
	...
	public static function appUnRegister($arParams, $n, CRestServer $server)
	{
		...
		$apps = \Thurly\Im\App::getListCache();
		if (!isset($apps[$arParams['APP_ID']]))
		{
			throw new Thurly\Rest\RestException("App not found", "CHAT_APP_ID_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}
		if ($apps[$arParams['APP_ID']]['APP_ID'] != $clientId)
		{
			throw new Thurly\Rest\RestException("App was installed by another rest application", "APP_ID_ERROR", CRestServer::STATUS_WRONG_REQUEST);
		}

		$result = \Thurly\Im\App::unRegister(Array('ID' => $arParams['APP_ID']));
		if (!$result)
		{
			throw new Thurly\Rest\RestException("App can't be deleted", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
		}
	...
	public static function appUpdate($arParams, $n, CRestServer $server)
	{
		...
		if (!empty($updateFields))
		{
			$result = \Thurly\Im\App::update(Array('ID' => $arParams['APP_ID']), $updateFields);
			if (!$result)
			{
				throw new Thurly\Rest\RestException("Command can't be updated", "WRONG_REQUEST", CRestServer::STATUS_WRONG_REQUEST);
			}
	...
	{
		$session = \Thurly\Rest\Event\Session::get();
		if(!$session)
		{
			return Array();
		}
		$auth = \Thurly\Rest\Event\Sender::getAuth(
			$appId,
			$userId,
	...
	private static function bindEvent($appId, $thurlyEventModule, $thurlyEventName, $restEventName, $restEventHandler)
	{
		$res = \Thurly\Rest\EventTable::getList(array(
			'filter' => array(
		...
		if($result->isSuccess())
		{
			\Thurly\Rest\Event\Sender::bind($thurlyEventModule, $thurlyEventName);
		}

		return true;
	}

	private static function unbindEvent($appId, $appCode, $thurlyEventModule, $thurlyEventName, $restEventName, $skipCheck = false)
	{
		if (!$skipCheck)
		{
			$res = \Thurly\Im\Model\BotTable::getList(array(
				'filter' => array(
					'=APP_ID' => $appCode,
				),
				'select' => array('BOT_ID')
		...
		{
			$eventFound = true;
			\Thurly\Rest\EventTable::delete($handler['ID']);
		}
		if ($eventFound)
		{
			\Thurly\Rest\Event\Sender::unbind($thurlyEventModule, $thurlyEventName);
		}
