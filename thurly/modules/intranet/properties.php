...
<input type="text" name="<?echo htmlspecialcharsbx($strHTMLControlName["VALUE"])?>" id="<?echo $name_x?>" value="<?echo intval($value['VALUE']) > 0 ? intval($value['VALUE']) : ''?>" size="3" class="typeinput" />&nbsp;&nbsp;<?
		$APPLICATION->IncludeComponent('thurly:intranet.user.search', '', array(
			'INPUT_NAME' => $name_x,
			'MULTIPLE' => 'N',
			'SHOW_BUTTON' => 'Y',
	...
	if (document.getElementById('<?echo $name_x?>'))
	{
		...
		if (value_<?=$name_x?> > 0)
		{
			if (old_value != value_<?=$name_x?>)
			{
				...
				{
					document.getElementById("hiddenframe<?=$name_x?>").src='/thurly/admin/get_user.php?ID=' + value_<?=$name_x?>+'&strName=<?=$name_x?>&lang=<? echo LANG.(defined("ADMIN_SECTION") && ADMIN_SECTION===true?"&admin_section=Y":"")?>';
				}
				else
				{
					DV_<?=$name_x?>.innerHTML = '[<a title="<?echo CUtil::JSEscape(GetMessage("MAIN_EDIT_USER_PROFILE"))?>" class="tablebodylink" href="/thurly/admin/user_edit.php?ID=<?echo $USER->GetID()?>&lang=<?echo LANG?>"><?echo $USER->GetID()?></a>] (<?echo CUtil::JSEscape(htmlspecialcharsbx($USER->GetLogin()))?>) <? echo CUtil::JSEscape(htmlspecialcharsbx($USER->GetFirstName().' '.$USER->GetLastName()))?>';
				}
			}
...
{
	protected static $lastUsed = array();
	...
	{
		if(count(static::$lastUsed) > 0)
		{
			\Thurly\Main\FinderDestTable::merge(array(
				"CONTEXT" => \CUserTypeEmployeeDisplay::SELECTOR_CONTEXT,
				"CODE" => static::$lastUsed,
			));

			static::$lastUsed = array();
		}
...
class CUserTypeEmployeeDisplay extends \Thurly\Main\UserField\TypeBase
{
	...
	public static function getPublicEdit($arUserField, $arAdditionalParameters = array())
	{
		...
		$selectorName = $arUserField['FIELD_NAME'].\Thurly\Main\Security\Random::getString(5);
...
?>
		<div id="cont_<?=$selectorName?>">
			...
			<div id="value_<?=$selectorName?>" style="display: none;"><input type="hidden" name="<?=\Thurly\Main\Text\HtmlFilter::encode($fieldName)?>" value=""></div>
...
<?
		...
		$APPLICATION->IncludeComponent(
			...
			array(
				'ID' => $selectorName,
				'BIND_ID' => 'add_user_'.$selectorName,
	...
	public static function getPublicView($arUserField, $arAdditionalParameters = array())
	{
		...
		{
			$dbRes = \Thurly\Main\UserTable::getList(array(
				'filter' => array(
					'@ID' => $value
				),
			));
			...
			foreach($resultList as $res)
			{
				...
				$userRow = '
<a class="uf-employee-wrap" href="'.\Thurly\Main\Text\HtmlFilter::encode(str_replace('#user_id#', $res['ID'], $pathToUser)).'" target="_blank">
	...
	<span class="uf-employee-data">
		<span class="uf-employee-name">'.\Thurly\Main\Text\HtmlFilter::encode($res['NAME']).'</span>
		<span class="uf-employee-position">'.\Thurly\Main\Text\HtmlFilter::encode($res['WORK_POSITION']).'</span>
	</span>
</a>
';
...
class CIBlockPropertyEmployee extends CIEmployeeProperty
{
	...
	function GetPublicFilterHTML($arProperty, $strHTMLControlName)
	{
		...
		<script type="text/javascript" src="/thurly/components/thurly/intranet.user.selector.new/templates/.default/users.js"></script>
		<script type="text/javascript">BX.loadCSS('/thurly/components/thurly/intranet.user.selector.new/templates/.default/style.css');</script>
