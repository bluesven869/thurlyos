...
class CUserCMLImport
{
	...
	function CheckUserFields()
	{
		...
		if (count($arUserTypeList) > 0)
		{
			...
			$arUserFields = array();
			require($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/intranet/install/1c_intranet/user.php');
	...
	function CheckIBlockFields()
	{
		...
		*/
		...
		require_once($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/intranet/install/1c_intranet/iblock.php');
	...
	function LoadUser($arXMLElement, &$counter)
	{
		...
		{
			$counter['ERR']++;
			$fp = fopen($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/intranet/cml2-import-user.log', 'a');
			fwrite($fp, "==============================================================\r\n");
		...
		elseif (is_array($arStateHistory) && count($arStateHistory) > 0)
		{
			...
			foreach ($arStateHistory as $date => $arState)
			{
				...
				{
					$fp = fopen($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/intranet/cml2-import-state.log', 'a');
					fwrite($fp, "==============================================================\r\n");
