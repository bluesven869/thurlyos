...
*/
	...
	*/
	public static function OnBeforeUserUpdate(&$fields)
	{
		...
		$res = CUser::getList(($by="id"), ($order="asc"), array("ID_EQUAL_EXACT" => $userId), array("SELECT" => array("UF_DEPARTMENT"), "FIELDS" => array("ID", "ACTIVE")));
		if ($user = $res->fetch())
		{
	...
	*/
	public static function OnAfterUserUpdate(&$fields)
	{
		if(array_key_exists('UF_DEPARTMENT', $fields))
		{
			...
			{
				\Thurly\Intranet\Internals\UserSubordinationTable::delayReInitialization();
				\Thurly\Intranet\Internals\UserToDepartmentTable::delayReInitialization();
	...
	{
		\Thurly\Intranet\Internals\UserSubordinationTable::reInitialize();
		\Thurly\Intranet\Internals\UserToDepartmentTable::deleteByUserId($user);
	}
...
*/
	function ClearSingleUserCache($arFields)
	{
		...
		{
			\Thurly\Intranet\Composite\CacheProvider::deleteUserCache(intval($arFields['ID']));
		}
...
*/
	public static function OnCreatePanel()
	{
		...
		if($USER->IsAdmin())
		{
			...
			$arMenu = Array(
				Array(
					"ACTION" => "jsUtils.Redirect([], '".CUtil::JSEscape("/thurly/admin/wizard_install.php?lang=".LANGUAGE_ID."&wizardName=thurly:portal&wizardSiteID=".SITE_ID."&".thurly_sessid_get())."');",
					"ICON" => "wizard",
			...
			if(IsModuleInstalled('extranet'))
			{
				...
				$arMenu[] = Array(
					"ACTION" => "jsUtils.Redirect([], '".CUtil::JSEscape("/thurly/admin/wizard_install.php?lang=".LANGUAGE_ID."&wizardName=thurly:extranet&".thurly_sessid_get())."');",
					"ICON" => "wizard",
			...
			if(COption::GetOptionString("main", "wizard_clear_exec", "N", SITE_ID) <> "Y")
			{
				...
				$arMenu[] = Array(
					"ACTION" => "jsUtils.Redirect([], '".CUtil::JSEscape("/thurly/admin/wizard_install.php?lang=".LANGUAGE_ID."&wizardSiteID=".SITE_ID."&wizardName=thurly:portal_clear&".thurly_sessid_get())."');",
					"ICON" => "wizard-clear",
			...
			$arButton = array(
				"HREF" => "/thurly/admin/wizard_install.php?lang=".LANGUAGE_ID."&wizardName=thurly:portal&wizardSiteID=".SITE_ID."&".thurly_sessid_get(),
				"ID" => "portal_wizard",
	...
	{
		global $APPLICATION;
		$APPLICATION->SetAdditionalCSS("/thurly/themes/.default/intranet_sonet_log.css");

		$arResult = array(
			"EVENT" => $arFields,
			"ENTITY" => CIntranetEventHandlers::GetEntity_News($arFields, $bMail),
			"URL" => "",
			"CACHED_CSS_PATH" => "/thurly/themes/.default/intranet_sonet_log.css"
		);
	...
	{
		if (!isset($params["APP_ID"]) || !\Thurly\Main\Loader::includeModule("rest"))
			return;
		...
		if ($appInfo = $dbRes->fetch())
		{
			if (
				...
				|| $appInfo["CODE"] === \CRestUtil::THURLY_1C_APP_CODE
			)
			{
				return;
			}
