...
use Thurly\Socialnetwork;
use Thurly\Main\UserTable;
use Thurly\Main\FinderDestTable;
...
class CIntranetInviteDialog
{
	...
	public static function ShowInviteDialogLink($arParams = array())
	{
		...
		return "B24.ThurlyOSInviteDialog.ShowForm(".CUtil::PhpToJSObject($arParams).")";
	}
		...
		if (!$strError)
		{
			...
			else
			{
				...
				else
				{
					...
					elseif (IsModuleInstalled("thurlyos"))
					{
						$event->SendImmediate("THURLY24_USER_INVITATION", $siteIdByDepartmentId, array(
	...
	public static function RegisterNewUser($SITE_ID, $arFields, &$arError)
	{
		...
		)
		{
			...
			CUserOptions::SetOption((IsModuleInstalled("thurlyos")? "thurlyos" : "intranet"), "invite_message_text", $arFields["MESSAGE_TEXT"]);
		}
		else
		{
			$messageText = GetMessage("BX24_INVITE_DIALOG_INVITE_MESSAGE_TEXT");
		}
	...
	public static function checkUsersCount($cnt)
	{
		if (CModule::IncludeModule("thurlyos"))
		{
			...
			$currentUserCount = CThurlyOS::ActiveUserCount();
	...
	public static function TransferEmailUser($userId, $arParams = array())
	{
		...
		if ($obUser->Update($userId, $arFields))
		{
			...
			else
			{
				...
				{
					$event->SendImmediate("THURLY24_USER_INVITATION", $arParams["SITE_ID"], array(
						"EMAIL_FROM" => $USER->GetEmail(),
						"EMAIL_TO" => $arUser["EMAIL"],
	...
	public static function getInviteLink($arUser, $siteId)
	{
		...
		return CHTTP::URN2URI("/thurly/tools/intranet_invite_dialog.php?user_id=".$arUser["ID"]."&checkword=".urlencode($arUser["CONFIRM_CODE"]), $serverName);
	}

	public static function getInviteMessageText()
	{
		return (
			($userMessageText = \Thurly\Main\Config\Option::get(ModuleManager::isModuleInstalled("thurlyos") ? "thurlyos" : "intranet", "invite_message_text"))
			&& (
				!Loader::includeModule('thurlyos')
				|| (
					CThurlyOS::IsLicensePaid()
