...
class CIntranetUtils
{
	...
	*/
	public static function GetStsSyncURL($arSectionParams, $type = 'calendar', $employees = false)
	{
		...
		return \Thurly\WebService\StsSync::getUrl($type, $type_script, $arSectionParams['LINK_URL'], $arSectionParams['PREFIX'], $arSectionParams['NAME'], $GUID);
	}
	...
	public static function getDepartmentEmployees($arDepartments, $bRecursive = false, $bSkipSelf = false, $onlyActive = 'Y', $arSelect = null)
	{
		...
		return \Thurly\Intranet\Util::getDepartmentEmployees(array(
	...
	public static function notifyMailDomain($type, $sid, $user_id, $cnt = 0)
	{
		...
		{
			$dbServices = \Thurly\Mail\MailServicesTable::getList(array(
	...
	public static function LoadCustomMessages()
	{
		...
		$organizationType = \Thurly\Main\Config\Option::get("intranet", "organization_type");
		if($organizationType <> '' && !preg_match('/[^a-z0-9_-]/', $organizationType))
		{
			\Thurly\Main\Localization\Loc::loadCustomMessages(__DIR__."/../../organization/".$organizationType.".php");
		}
	}
	...
	public static function getB24Host()
	{
		...
		{
			case 'la':
				return 'www.thurlyos.es';
			case 'br':
				return 'www.thurlyos.com.br';
			case 'tc':
			case 'sc':
				return 'www.thurlyos.cn';
			case 'ru':
			case 'de':
			case 'ua':
				return 'www.thurlyos.'.LANGUAGE_ID;
			default:
	...
	public static function getHostName()
	{
		...
		if (is_null($host))
		{
			...
			$host = isModuleInstalled('thurlyos') ? BX24_HOST_NAME
	...
	protected static function getB24Referral($source)
	{
		...
		if (isModuleInstalled('thurlyos'))
		{
			$pid = (int) COption::getOptionInt('thurlyos', 'partner_id', 0);
