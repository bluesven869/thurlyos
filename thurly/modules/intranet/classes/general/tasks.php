	...
	function PrepareUserForPrint($value, $nameTemplate = false, $bShowLogin = true, $bShowTooltip = false, $arTooltipParams = false)
	{
		...
		foreach ($valueTmp as $val)
		{
			...
			if ($arUser = $dbUser->GetNext())
			{
				...
				if ($bShowTooltip)
				{
					$arReturn[] = $GLOBALS["APPLICATION"]->IncludeComponent("thurly:main.user.link",
						'',
						array(
							"ID" => $val,
							"HTML_ID" => "tasks_".$cnt,
...
//			}
			...
			{
				require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/intranet/classes/general/tasks_wf_template.php");
			...
			if ($arResult = $dbResult->Fetch())
			{
				$arOldTasksWFs = array();
				$arOldTasksSTs = array();
				require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/intranet/classes/general/tasks_wf_template1.php");
