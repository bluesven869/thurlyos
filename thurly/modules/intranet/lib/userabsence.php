...
class UserAbsence
{
	...
	{
		$iblockId = \Thurly\Main\Config\Option::get('intranet', 'iblock_absence', 0);
		...
		else
		{
			...
			foreach ($absenceData as $userId => $record)
			{
				foreach ($record as $index => $data)
				{
					...
					$dateFrom = new \Thurly\Main\Type\DateTime($data['DATE_FROM']);
					$dateTo = new \Thurly\Main\Type\DateTime($data['DATE_TO']);
