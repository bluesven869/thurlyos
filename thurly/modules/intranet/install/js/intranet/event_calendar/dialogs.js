	...
	if (this.arConfig.bSocNet && oEvent.IS_MEETING)
	{
		if (oEvent.HOST && !oEvent.bSuperposed && !this.bReadOnly)
		{
			...
			if (oEvent.STATUS == 'Q')
			{
				cConf.appendChild(BX.create('A', {
					...
					html: '<img src="/thurly/images/1.gif" class="bxec-vd-g-status-y"/>' + EC_MESS.ConfirmEncY
				}));
				cConf.appendChild(document.createTextNode(' | '));
			}
			else if(oEvent.STATUS == 'Y')
			{
				cConf.appendChild(BX.create('IMG', {props: {src: '/thurly/images/1.gif', className: 'bxec-iconkit bxec-g-status-y1', title: EC_MESS['GuestStatus_Y']}}));
			...
			cConf.appendChild(BX.create('A', {
				...
				html: '<img src="/thurly/images/1.gif" class="bxec-vd-g-status-n"/>' + EC_MESS.ConfirmEncN
			}));
		}
		else
		{
		...
		if (oEvent.GUESTS && oEvent.GUESTS.length > 0)
		{
			...
			for (i = 0; i < l; i++)
			{
				...
				statImg = '<img src="/thurly/images/1.gif" title="' + EC_MESS['GuestStatus_' + status] + '" class="bxec-iconkit bxec-g-status ' + cn + '" align="top">';
...
JCEC.prototype.SPD_GetGroup = function(id, title)
{
	...
	c_t.innerHTML = '<img class="bxec-iconkit bxec-spd-big-plus" src="/thurly/images/1.gif"/>' + BX.util.htmlspecialchars(title);
	if (id == 'SOCNET_USERS')
	{
...
JCEC.prototype.SPD_DisplayCalendars = function(oGroup, Item, bDefCheck)
{
	...
	c_t.innerHTML = '<img class="bxec-iconkit bxec-spd-small-plus" src="/thurly/images/1.gif"/>' + Item.NAME;
	if (Item.bDeletable)
	{
		var _this = this;
		var c = r.insertCell(-1);
...
JCEC.prototype.ExD_DisplayConnection = function(con, ind)
{
	var
		...
		pStatus = pTitle.appendChild(BX.create("IMG", {props: {src: "/thurly/images/1.gif", className: 'bxec-dav-item-status'}})),
