...
BX.namespace("BX.Intranet.ThurlyOS");

BX.Intranet.ThurlyOS.ThemePicker = function(options)
{
	...
	this.newThemeDialog = new BX.Intranet.ThurlyOS.NewThemeDialog(this);
	...
	if (this.isVideo)
	{
		...
		BX.addCustomEvent("SidePanel.Slider:onFrameFocus", this.handleWindowFocus.bind(this));
...
BX.Intranet.ThurlyOS.ThemePicker.prototype =
{
	...
	{
		this.hideLoader();
		this.getThemeListDialog().setContent(BX.message("THURLY24_THEME_UNKNOWN_ERROR"));
	...
	applyThemeAssets: function(assets)
	{
		...
		{
			BX.removeClass(document.body, "thurlyos-" + appliedBaseThemeId + "-theme");
			BX.addClass(document.body, "thurlyos-" + baseThemeId + "-theme");
		}
	},
						...
						theme.removable
							...
							BX.create("div", {
								attrs: {
									...
									"data-theme-id": theme.id,
									title: BX.message("THURLY24_THEME_REMOVE_THEME")
								},
								events: {
	...
	createDefaultLabel: function()
	{
		return BX.create("div", {
			...
			text: BX.message("THURLY24_THEME_DEFAULT_THEME")
		});
	},
	...
	/**
	 *
	 * @returns {BX.Intranet.ThurlyOS.ThemePickerCheckboxButton}
	 */
	...
	getThemeListDialog: function()
	{
		...
		this.popup = new BX.PopupWindow("thurlyos-theme-list-dialog", null, {
			width: 800,
			height: 430,
			titleBar: BX.message("THURLY24_THEME_DIALOG_TITLE"),
			...
			buttons: [
				new BX.PopupWindowButton({
					id: "save-button",
					text: BX.message("THURLY24_THEME_DIALOG_SAVE_BUTTON"),
				...
				new BX.PopupWindowButtonLink({
					id: "cancel-button",
					text: BX.message("THURLY24_THEME_DIALOG_CANCEL_BUTTON"),
				...
				new BX.PopupWindowButtonLink({
					id: "create-button",
					text: BX.message("THURLY24_THEME_DIALOG_NEW_THEME"),
				...
				this.canSetDefaultTheme() ? [new BX.Intranet.ThurlyOS.ThemePickerCheckboxButton({ id: "checkbox" })] : []
			)
		});
 ...
 * @constructor
 */
BX.Intranet.ThurlyOS.ThemePickerCheckboxButton = function(params)
{
	...
	this.buttonNode = BX.create("div", {
		...
		children: [
			...
			BX.create("label", {
				...
				text: BX.message("THURLY24_THEME_SET_AS_DEFAULT")
			})
		]
	});
...
BX.Intranet.ThurlyOS.ThemePickerCheckboxButton.prototype = {
	__proto__: BX.PopupWindowButton.prototype,
	constructor: BX.Intranet.ThurlyOS.ThemePickerCheckboxButton,
...
/**
 *
 * @param {BX.Intranet.ThurlyOS.ThemePicker} themePicker
 * @constructor
 */
BX.Intranet.ThurlyOS.NewThemeDialog = function(themePicker)
{
...
BX.Intranet.ThurlyOS.NewThemeDialog.prototype =
{
	show: function()
	{
	...
	/**
	 *
	 * @returns {BX.Intranet.ThurlyOS.ThemePicker|*}
	 */
	...
	getPopup: function()
	{
		...
		this.popup = new BX.PopupWindow("thurlyos-new-theme-dialog", null, {
			width: 500,
			height: 400,
			className: "theme-dialog-popup-window-container",
			titleBar: BX.message("THURLY24_THEME_CREATE_YOUR_OWN_THEME"),
			closeByEsc: true,
			bindOnResize: false,
			closeIcon: true,
			draggable: true,
			...
			buttons: [
				new BX.PopupWindowButton({
					...
					text: BX.message("THURLY24_THEME_DIALOG_CREATE_BUTTON"),
					className: "popup-window-button-accept",
					events: {
		...
		BX.ajax.submitAjax(form, {
			...
			onsuccess: function(response) {
				...
				else
				{
					createButton.removeClassName("popup-window-button-wait");
					BX.removeClass(form, "theme-dialog-form-disabled");
					this.showError(response.error || BX.message("THURLY24_THEME_UNKNOWN_ERROR"));
				}
			}.bind(this),
			...
			onfailure: function() {
				createButton.removeClassName("popup-window-button-wait");
				BX.removeClass(form, "theme-dialog-form-disabled");
				this.showError(BX.message("THURLY24_THEME_UNKNOWN_ERROR"));
			}.bind(this)
		});
	},
		...
		return BX.create("form", {
			...
			children: [
				...
				BX.create("div", {
					...
					children: [
						this.createField(BX.message("THURLY24_THEME_THEME_BG_IMAGE"), this.getBgImageField()),
						this.createField(BX.message("THURLY24_THEME_THEME_BG_COLOR"), this.getBgColorField()),
						this.createField(BX.message("THURLY24_THEME_THEME_TEXT_COLOR"), this.getTextColorField())
					]
				})
			]
		});
						...
						BX.create("div", {
							...
							children: [
								BX.create("span", {
									...
									text: BX.message("THURLY24_THEME_UPLOAD_BG_IMAGE")
								}),
								BX.create("span", {
									attrs: {
										className: "theme-dialog-field-file-add-info"
									},
									text: BX.message("THURLY24_THEME_DRAG_BG_IMAGE")
								})
							]
						})
					]
	...
	validateBgImage: function(image)
	{
		...
		{
			return BX.message("THURLY24_THEME_WRONG_FILE_TYPE");
		}

		if (image.size > this.getThemePicker().getMaxUploadSize())
		{
			var limit = this.getThemePicker().getMaxUploadSize() / 1024 / 1024;
			return BX.message("THURLY24_THEME_FILE_SIZE_EXCEEDED").replace("#LIMIT#", limit.toFixed(0) + "Mb");
		}
	...
	validateForm: function()
	{
		...
		{
			return BX.message("THURLY24_THEME_WRONG_BG_COLOR");
		}

		if (!bgImage && !BX.type.isNotEmptyString(bgColor))
		{
			return BX.message("THURLY24_THEME_EMPTY_FORM_DATA");
		}
		...
		return BX.create("div", {
			...
			children: [
				BX.create("div", {
					...
					text: BX.message("THURLY24_THEME_THEME_LIGHT_COLOR"),
					events: {
				...
				BX.create("div", {
					...
					text: BX.message("THURLY24_THEME_THEME_DARK_COLOR"),
					events: {
						click: this.handleSwitcherClick.bind(this)
					}
				}),
