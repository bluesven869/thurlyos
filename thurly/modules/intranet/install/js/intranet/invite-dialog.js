...
BX.InviteDialog.Init = function(arParams)
{
	...
	var nodeCopyToClipboardButton = document.querySelector("[data-role='copy-to-clipboard-node']");

	if (nodeCopyToClipboardButton)
	{
		BX.bind(nodeCopyToClipboardButton, 'click', function(){
			BX.clipboard.copy(BX('allow_register_url').value);
		});

	//	BX.clipboard.bindCopyClick(nodeCopyToClipboardButton, {text: BX('allow_register_url').value});
	}
};
				...
				BX.create("tr", {
					children: [
						BX.create("td", {
							...
							events: {
								"click" : function(){
									B24.ThurlyOSInviteDialog.ShowForm();
								}
							}
						})
					]
...
BX.InviteDialog.bindInviteDialogChangeTab = function(oBlock)
{
	...
	BX.bind(oBlock, "click", function(e)
	{
		...
		if (action.length > 0)
		{
			...
			if(windowObj)
			{
				windowObj.B24.ThurlyOSInviteDialog.popup.setTitleBar(windowObj.BX.message('BX24_INVITE_TITLE_' + (action == 'invite' || action == 'self' ? 'INVITE' : 'ADD')));
			}
		}
...
BX.InviteDialog.onInviteDialogClose = function(bCloseDialog)
{
	...
	if (
		bCloseDialog 
		&& B24.ThurlyOSInviteDialog.popup != null
	)
	{
		B24.ThurlyOSInviteDialog.popup.close();
	}
