...
$diskEnabled =
	\Thurly\Main\Config\Option::get('disk', 'successfully_converted', false) === 'Y' &&
...
if($diskEnabled)
{
	...
	{
		$storage = \Thurly\Disk\Storage::loadById($arGadgetParams['STORAGE_ID'], array('ROOT_OBJECT'));
	}
	elseif($emptyIblockId)
	{
		$storage = \Thurly\Disk\Driver::getInstance()->getStorageByCommonId('shared_files_' . SITE_ID);
	}
	elseif($arGadgetParams['IBLOCK_ID'])
	{
		$storage = \Thurly\Disk\Storage::load(array(
			'MODULE_ID' => \Thurly\Disk\Driver::INTERNAL_MODULE_ID,
			'ENTITY_TYPE' => \Thurly\Disk\ProxyType\Common::className(),
