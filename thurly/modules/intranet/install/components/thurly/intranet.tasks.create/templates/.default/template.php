...
else
{
	...
	function __ShowTextFieldTmp($name, $multiple, $key, $value, $isInSecurity)
	{
		...
		else
		{
			...
			if ($name == "DETAIL_TEXT")
			{
				...
				"ul.bx-subtasklist {list-style-image: url(/thurly/images/fileman/light_htmledit/check_off.gif);}" + 
				"ul.bx-subtasklist li{padding-left: 5px;}" +
				"ul.bx-subtasklist li.checked{ list-style-image: url(/thurly/images/fileman/light_htmledit/check_on.gif);}" +
				"\n";
				</script>
				<?
				...
				{
					if ($LheId == 'LHETaskId')
						return array("JS" => array('/thurly/js/fileman/light_editor/task_checkbox.js'));
				}
				$ar = array(
					'id' => 'LHETaskId',
					'height' => '200px',
	...
	function __ShowUserFieldTmp($name, $multiple, $key, $value, $form_name, $arParams)
	{
		...
		$GLOBALS["APPLICATION"]->IncludeComponent('thurly:intranet.user.selector', '', array(
			'INPUT_NAME' => $name,
		...
		<table class="intranet-view-form data-table" cellspacing="0" cellpadding="0">
			...
			<tr>
				...
				<td valign="top" align="left" width="70%">
					<?if ($USER->IsAdmin() && strlen($arResult["DocumentState"]["ID"]) > 0):?><a href="/thurly/admin/bizproc_log.php?ID=<?= $arResult["DocumentState"]["ID"] ?>"><?endif;?><?= strlen($arResult["DocumentState"]["STATE_TITLE"]) > 0 ? $arResult["DocumentState"]["STATE_TITLE"] : $arResult["DocumentState"]["STATE_NAME"] ?><?if ($USER->IsAdmin() && strlen($arResult["DocumentState"]["ID"]) > 0):?></a><?endif;?>
					<script language="JavaScript">
