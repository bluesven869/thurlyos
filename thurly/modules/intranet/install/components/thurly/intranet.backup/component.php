	...
	if (isset($_REQUEST['action']))
	{
		...
		switch ($_REQUEST['action'])
		{
			...
			case "delete":
				...
				{
					if (!unlink($f))
						echo \Thurly\Main\Web\Json::encode(array("error" => GetMessage('DUMP_DELETE_ERROR', array('#FILE#' => $f))));
			...
			case "restore":
				...
				{
					echo \Thurly\Main\Web\Json::encode(array("error" => GetMessage('MAIN_DUMP_ERR_COPY_FILE', array('#FILE#' => "restore.php"))));
				}
				else
				{
					$name = $path.'/'.$_REQUEST['f_id'];
			...
			case "rename":
				if (preg_match('#^[a-z0-9\-\._]+$#i',$_REQUEST['name']))
				{
					...
					while(file_exists($_SERVER["DOCUMENT_ROOT"].$path.'/'.$ID))
					{
						...
						{
							echo \Thurly\Main\Web\Json::encode(array("error" => GetMessage("MAIN_DUMP_ERR_FILE_RENAME").htmlspecialcharsbx($ID)));
							break;
						}
					...
					echo \Thurly\Main\Web\Json::encode(array("error" => GetMessage("MAIN_DUMP_ERR_NAME")));
				break;
		}
		CMain::FinalActions();
		die();
...
$gridObj = new \Thurly\Main\Grid\Options("backup_grid");
