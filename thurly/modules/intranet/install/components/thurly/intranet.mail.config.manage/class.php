...
class CIntranetMailConfigManageComponent extends CThurlyComponent
{
	...
	public function executeComponent()
	{
		...
		\CThurlyComponent::includeComponentClass('thurly:intranet.mail.config');
	...
	private function executeManagePage()
	{
		...
		$res = \Thurly\Mail\BlacklistTable::getList(array(
		...
		$filterOption = new \Thurly\Main\UI\Filter\Options($this->arResult['FILTER_ID']);
		$filterData   = $filterOption->getFilter($this->arResult['FILTER']);

		$gridOptions = new \Thurly\Main\Grid\Options($this->arResult['GRID_ID']);

		$navData = $gridOptions->getNavParams(array('nPageSize' => 50));
		$nav = new \Thurly\Main\UI\PageNavigation('nav-mail-manage');
		...
		if ('user' == $this->arResult['MODE'])
		{
			$runtime = array(
				new \Thurly\Main\Entity\ReferenceField(
					'MAILBOX', 'Thurly\Mail\Mailbox',
					array(
						'=this.ID'    => 'ref.USER_ID',
						'=ref.LID'    => new \Thurly\Main\DB\SqlExpression('?s', SITE_ID),
						'=ref.ACTIVE' => new \Thurly\Main\DB\SqlExpression('?s', 'Y'),
					)
				),
			);
			...
			if (!empty($filterData['FILTER_APPLIED']))
			{
				...
				{
					$runtime[] = new \Thurly\Main\Entity\ExpressionField('LOWER_NAME', 'LOWER(%s)', array('NAME'));
					$runtime[] = new \Thurly\Main\Entity\ExpressionField('LOWER_LAST_NAME', 'LOWER(%s)', array('LAST_NAME'));
					$runtime[] = new \Thurly\Main\Entity\ExpressionField('LOWER_LOGIN', 'LOWER(%s)', array('LOGIN'));
			...
			$dbUsers = \Thurly\Main\UserTable::getList(array(
				'runtime'     => $runtime,
				'select'      => array(
					'ID', 'LOGIN', 'NAME', 'SECOND_NAME', 'LAST_NAME', 'PERSONAL_PHOTO', 'WORK_POSITION',
					new \Thurly\Main\Entity\ExpressionField('MAILBOX_ID', 'MAX(%s)', array('MAILBOX.ID')),
				),
				'filter'      => $filter,
				'group'       => array('ID'),
				'order'       => $order,
		...
		$this->arParams['CURRENT_USER'] = \Thurly\Main\UserTable::getList(array(
