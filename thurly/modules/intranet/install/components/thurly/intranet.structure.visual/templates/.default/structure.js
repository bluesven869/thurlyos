...
BX.IntranetVSBlock.prototype.Init = function(tplParts)
{
	...
	if (tplParts.department_employee_images)
	{
		...
		while (!!emp)
		{
			...
			{
				this.setTooltip(this.employees_index[emp.getAttribute('data-user')].ID, emp);
	...
	{
		this.setTooltip(this.head, tplParts.department_head);
...
BX.IntranetVSBlock.prototype.setTooltip = function(user_id, user_node)
{
	if (!!user_id && !!this.config.USE_USER_LINK && !!user_node)
	{
		if (!user_node.id)
			user_node.id = 'employee_' + user_id + '_' + Math.random();

		BX.tooltip(user_id, user_node.id, this.config.URL);
	}
}
...
BX.IntranetVSBlock.prototype._showNext = function (data) {
		...
		this.shadow = document.body.appendChild(BX.create('DIV', {
			...
			style: {
				...
				padding: (obPos.height + 30) + "px 20px 20px 20px"
			},
			html: data
		}));
...
BX.IntranetVSUser = {
	...
	onbxdragstart: function()
	{
		...
		BX.IntranetVSUser.mirror = document.body.appendChild(BX.create('DIV', {
			...
			html: '<span' + (this.__bxemployee.PHOTO ? ' style="background: url(\''+this.__bxemployee.PHOTO+'\') no-repeat scroll center center transparent;"' : '') + ' class="structure-avatar"></span><div class="structure-employee-name">'+this.__bxemployee.NAME +'</div><div class="structure-employee-post">'+BX.util.htmlspecialchars(this.__bxemployee.POSITION)+'</div>'
		}));
