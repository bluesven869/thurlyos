...
else
{
	...
	if (strlen($arResult["FatalError"]) <= 0)
	{
		if (isset($_GET['bx_task_action_request']) && $_GET['bx_task_action_request'] == 'Y')
		{
			...
			include($_SERVER['DOCUMENT_ROOT']."/thurly/components/thurly/intranet.tasks/action_ajax.php");
			die();
		}

		if (isset($_GET['action']) && strlen($_GET['action']) > 0 && check_thurly_sessid())
		{
		...
		while ($arTask = $dbTasksList->Fetch())
		{
			...
			if (count($arTask["DocumentState"]["AllowableEvents"]) > 0)
			{
				...
				foreach ($arTask["DocumentState"]["AllowableEvents"] as $e)
				{
					...
					$p11 .= "action_task_id=".$arTask["ID"]."&".thurly_sessid_get()."&action=".$e["NAME"]."&wf=".$arTask["DocumentState"]["ID"];
			...
			if ($arTask["CurrentUserCanDeleteTask"])
			{
				...
				$p1 .= ((StrPos($p1, "?") !== false) ? "&" : "?");
				$p1 .= "del_task_id=".$arTask["ID"]."&".thurly_sessid_get()."&action=delete_task";
