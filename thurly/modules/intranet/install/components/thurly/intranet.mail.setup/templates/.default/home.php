...
{
	if ($limitedLicense = !in_array(CThurlyOS::getLicenseType(), array('company', 'nfr', 'edu', 'demo')))
...
<? $showB24Block = IsModuleInstalled('thurlyos') && !empty($b24Settings) && !empty($arParams['CR_DOMAINS']); ?>
<? $showDomainBlock = IsModuleInstalled('thurlyos') || !empty($domainSettings) || in_array(LANGUAGE_ID, array('ru', 'ua')); ?>
			...
			<td class="mail-block mail-block-imap<? if ($defaultBlock == 'imap'): ?> mail-block-active<? endif ?>" data-block="mail-set-third">
				...
				<div id="imap_block_descr_nomailbox" class="mail-block-text" style="<? if ($imapMailbox): ?>display: none; <? endif ?><? if (!$showB24Block && !$showDomainBlock): ?>min-height: 0; <? endif ?>">
					...
					<? if ($showB24Block || $showDomainBlock): ?>
						<img class="mail-block-icon-list" src="/thurly/components/thurly/intranet.mail.setup/templates/.default/images/<?=getMessage('INTR_MAIL_IMAP_DESCR_ICONS') ?>"/>
						<img class="mail-block-icon-list-colorless" src="/thurly/components/thurly/intranet.mail.setup/templates/.default/images/<?=getMessage('INTR_MAIL_IMAP_DESCR_ICONS_CL') ?>"/>
					<? endif ?>
				</div>
			</td>
			<? if ($showB24Block): ?>
