...
class CIntranetMailSetupComponent extends CThurlyComponent
{
	...
	private function executeDomainPage()
	{
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['act']))
		{
				...
				if (empty($errors))
				{
					...
					if (empty($errors))
					{
						...
						if (empty($errors))
						{
							if ($serviceId)
							{
								$result = \Thurly\Mail\MailServicesTable::update($serviceId, array(
			...
			{
				$result = \Thurly\Mail\MailServicesTable::delete($serviceId);
