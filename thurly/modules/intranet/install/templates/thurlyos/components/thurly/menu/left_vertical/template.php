...
<div class="menu-items-block" id="bx-left-menu">
			...
			foreach ($arResult["ITEMS"][$status] as $item)
			{
				if ($item["PERMISSION"] > "D")
				{
					...
					<li id="bx_left_menu_<?=$id?>"
						...
						class="menu-item-block<? if ($isCompositeMode === false && $counter > 0 && strlen($counterId) > 0):?> menu-item-with-index<?endif ?><? if (IsModuleInstalled("thurlyos") && $item["PARAMS"]["menu_item_id"] == "menu_live_feed"):?> menu-item-live-feed<?endif ?>"
					>
						<span
							class="menu-fav-editable-btn menu-favorites-btn"
							onclick="BX.ThurlyOS.LeftMenuClass.openMenuPopup(this, '<?=CUtil::JSEscape($item["PARAMS"]["menu_item_id"])?>')">
						...
						<a class="menu-item-link"
							...
							target="_blank"
							<?endif?>
							onclick="if (BX.ThurlyOS.LeftMenuClass.isEditMode()) return false;
	...
	<div class="menu-favorites-settings-btn" id="menu-favorites-settings-btn">
		...
		<span class="menu-favorites-btn-done" onclick="BX.ThurlyOS.LeftMenuClass.applyEditMode();"><?=GetMessage("MENU_EDIT_READY_FULL")?></span>
	</div>

	<? if (CModule::IncludeModule("thurlyos") && CThurlyOS::isInvitingUsersAllowed() && CModule::IncludeModule("intranet")):?>
	<div class="menu-invite-employees">
		<span
			class="menu-invite-employees-text"
			onclick="<?=CIntranetInviteDialog::ShowInviteDialogLink()?>"><?=GetMessage("THURLY24_INVITE_ACTION")?></span>
	</div>
	<? endif ?>
...
$arJSParams = array(
	...
	"isThurlyOS" => IsModuleInstalled("thurlyos") ? "Y" : "N",
	"siteId" => SITE_ID,
	"siteDir" => SITE_DIR,
...
if (\Thurly\Main\ModuleManager::isModuleInstalled("thurlyos") && \Thurly\Main\ModuleManager::isModuleInstalled("landing"))
{
	$APPLICATION->includeComponent("thurly:spotlight", "", array(
...
<script>
	...
	BX.ThurlyOS.LeftMenuClass.init(<?=CUtil::PhpToJSObject($arJSParams)?>);
</script>
...
<script>
if (!BX.ThurlyOS.LeftMenuClass.initPagetitleStar())
{
	BX.ready(function() {
		BX.ThurlyOS.LeftMenuClass.initPagetitleStar()
	});
}
