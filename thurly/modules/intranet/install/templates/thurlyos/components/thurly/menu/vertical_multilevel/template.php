...
foreach($arResult["TITLE_ITEMS"] as $title => $arTitleItem)
{
	...
	{
		$hideOption = CUserOptions::GetOption("thurlyos", $arTitleItem["PARAMS"]["class"]);
					...
					if (is_array($arResult["SORT_ITEMS"][$title][$status]))
					{
						foreach($arResult["SORT_ITEMS"][$title][$status] as $arItem)
						{
							if ($arItem["PERMISSION"] > "D")
							{
								...
								if ($couterId == "bp_tasks" && IsModuleInstalled("thurlyos"))
								{
									$showMenuItem = CUserOptions::GetOption("thurlyos", "show_bp_in_menu", false);
									if ($showMenuItem === false && $counter > 0)
									{
										CUserOptions::SetOption("thurlyos", "show_bp_in_menu", true);
										$showMenuItem = true;
									}
								...
								<li <?if ($title!= "menu-favorites" && in_array($arItem["PARAMS"]["menu_item_id"],$arResult["ALL_FAVOURITE_ITEMS_ID"])):?>style="display:none; " <?endif?>
									...
									<?endif?>
									class="menu-item-block <?if ($isCompositeMode === false && $arItem["SELECTED"]):?> menu-item-active<?endif?><?if($isCompositeMode === false && $counter > 0 && strlen($couterId) > 0 && (!$arItem["SELECTED"] || ($arItem["SELECTED"] && $couterId == "bp_tasks"))):?> menu-item-with-index<?endif?><?if ((IsModuleInstalled("thurlyos") && $arItem["PARAMS"]["menu_item_id"] == "menu_live_feed") || $arItem["PARAMS"]["menu_item_id"] == "menu_all_groups"):?> menu-item-live-feed<?endif?>">
									<?if (!((IsModuleInstalled("thurlyos") && $arItem["PARAMS"]["menu_item_id"] == "menu_live_feed") || $arItem["PARAMS"]["menu_item_id"] == "menu_all_groups")):?>
...
$arJSParams = array(
	...
	"isAdmin" => (IsModuleInstalled("thurlyos") && $GLOBALS['USER']->CanDoOperation('thurlyos_config') || !IsModuleInstalled("thurlyos") && $GLOBALS['USER']->IsAdmin()) ? "Y" : "N",
...
<script>
	...
	if (!BX.ThurlyOS.MenuClass.highlight(path))
	{
		BX.ready(function() {
			BX.ThurlyOS.MenuClass.highlight(path);
		});
	}
	<?endif?>

	BX.ready(function() {
		window.B24menuItemsObj = new BX.ThurlyOS.MenuClass(<?=CUtil::PhpToJSObject($arJSParams)?>);
	});
</script>
