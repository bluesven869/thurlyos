...
$arFilterValues = $APPLICATION->IncludeComponent("thurly:intranet.structure.selector", "sections", $arSelectorParams, $component);
		...
		<div class="department-profile-events-cont">
			...
			<a class="department-profile-events-item department-profile-remove" onclick="
				if (confirm('<?=CUtil::JSEscape(GetMessage('ISV_confirm_delete_department'))?>'))
					BX.ajax.loadJSON(
						'<?="/thurly/tools/intranet_structure.php"?>?action=delete_department&dpt_id=<?=$arStructureParams["UF_DEPARTMENT_ID"]?>&<?=thurly_sessid_get()?>',
						function(result)
						{
