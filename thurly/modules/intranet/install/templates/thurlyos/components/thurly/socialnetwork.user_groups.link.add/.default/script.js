...
BX.ThurlyOS.SonetGroupSlider = function()
{
	this.actualSearchString = '';
	this.minSearchStringLength = 2;
};

BX.ThurlyOS.SonetGroupSlider.prototype.init = function (params) {
...
(function(){
ThurlySGFilterDestinationSelectorManager = {
	controls: {},
	...
	onSelect: function(item, type, search, bUndeleted, name, state)
	{
		...
		var control = ThurlySGFilterDestinationSelectorManager.controls[name];
		if(control)
		{
...
ThurlySGFilterDestinationSelector = function ()
{
	this.id = "";
	this.filterId = "";
	this.settings = {};
	...
	ThurlySGFilterDestinationSelector.create = function(id, settings)
	{
		var self = new ThurlySGFilterDestinationSelector(id, settings);
		self.initialize(id, settings);
		BX.onCustomEvent(window, 'BX.SonetGroupList.Filter:create', [ id ]);
		return self;
	};

	ThurlySGFilterDestinationSelector.prototype.getSetting = function(name, defaultval)
	{
		return this.settings.hasOwnProperty(name) ? this.settings[name] : defaultval;
	};

	ThurlySGFilterDestinationSelector.prototype.getSearchInput = function()
	{
		return this.control ? this.control.getLabelNode() : null;
	};

	ThurlySGFilterDestinationSelector.prototype.initialize = function(id, settings)
	{
		this.id = id;
	...
	ThurlySGFilterDestinationSelector.prototype.open = function()
	{
		var name = this.id;
	...
	ThurlySGFilterDestinationSelector.prototype.close = function()
	{
	...
	ThurlySGFilterDestinationSelector.prototype.onCustomEntitySelectorOpen = function(control)
	{
		...
		else
		{
			...
			ThurlySGFilterDestinationSelectorManager.controls[this.id] = this.control;
	...
	ThurlySGFilterDestinationSelector.prototype.onCustomEntitySelectorClose = function(control)
	{
		if(
	...
	ThurlySGFilterDestinationSelector.prototype.onGetStopBlur = function(event, result)
	{
	...
	ThurlySGFilterDestinationSelector.prototype.onCustomEntityRemove = function(control)
	{
	...
	ThurlySGFilterDestinationSelector.prototype.onBeforeSwitchTabFocus = function(ob)
	{
		if(this.id === ob.id)
		{
			ob.blockFocus = true;
	...
	ThurlySGFilterDestinationSelector.prototype.onBeforeSelectItemFocus = function(ob)
	{
		if(this.id === ob.id)
		{
			ob.blockFocus = true;
	...
	ThurlySGFilterDestinationSelector.prototype.onBeforeInitDialog = function(params)
	{
		if (
			typeof params.id == 'undefined'
			|| params.id != this.id
