...
foreach ($arFields as $GROUP_ID => $arGroupFields):
	...
	if ($GROUP_ID == "MAIN"):?>
		...
		$extmailConfigPath = \Thurly\Main\Config\Option::get('intranet', $arParams['ID'] == $USER->getId() ? 'path_mail_config' : 'path_mail_manage');
		if (
			!$arResult["User"]["IS_EMAIL"]
			&& (
				...
				|| (
					...
					&& (
						...
						|| $USER->isAdmin()
						|| $USER->canDoOperation('thurlyos_config')
					)
				)
			)
		) { ?>
		<tr>
			...
			<td class="content-edit-form-field-input">
				...
				<? if ($arParams['ID'] == $USER->getId() || $USER->isAdmin() || $USER->canDoOperation('thurlyos_config')): ?>
					<? if (!empty($extmailConfigPath)): ?><a href="<?=htmlspecialcharsbx($extmailConfigPath) ?>"><?=getMessage('ISL_EXTMAIL_EDIT') ?></a><? endif ?>
				<? endif ?>
			</td>
			<td class="content-edit-form-field-error">&nbsp;</td>
		</tr>
...
<?endif;?>
				...
				if(CModule::IncludeModule("socialservices")):
					...
					<td class="content-edit-form-field-input" colspan="<?=$bNetwork?3:2?>">
						<div class="bx-sonet-profile-field-socserv">
							<?
							$APPLICATION->IncludeComponent("thurly:socserv.auth.split", $bNetwork ? "network" : "twitpost", array(
									"SHOW_PROFILES" => "Y",
									"CAN_DELETE" => "Y",
									"USER_ID" => $arParams['ID']
								),
						...
						<label style="display: inline-block;">
							<input
								...
								<?if (
									...
									|| $arResult["IS_THURLY24"] && $arResult["ADMIN_RIGHTS_RESTRICTED"]
									&& (
		...
		<?if ($bThurlyOS && $FIELD == "PERSONAL_PHOTO" && in_array('UF_PHONE_INNER', $arFields['MAIN']) && !($arResult['User']["UF_PHONE_INNER"] && $arResult['User']["PERSONAL_MOBILE"] &&  $arResult['User']["WORK_PHONE"])): // for phones link?>
	<tr>
		<td class="content-edit-form-field-name"></td>
		...
		<?endif;?>
		<?if ($bThurlyOS && $FIELD == "UF_SKYPE" && count($arSocialFields) > 0 && $sonetLinkAddShow):?>
	<tr>
		<td class="content-edit-form-field-name"></td>
