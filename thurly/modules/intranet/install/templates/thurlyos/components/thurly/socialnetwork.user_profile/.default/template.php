...
$bThurlyOS = IsModuleInstalled("thurlyos");
$bNetwork = $bThurlyOS && COption::GetOptionString('thurlyos', 'network', 'N') == 'Y';
...
<div class="user-profile-block-wrap">
	<div class="user-profile-block-wrap-l">
		...
		<?if (
			...
			&& !in_array($arUser["EXTERNAL_AUTH_ID"], \Thurly\Socialnetwork\ComponentHelper::checkPredefinedAuthIdList(array('email')))
		):
			...
			if (!in_array($arUser["EXTERNAL_AUTH_ID"], \Thurly\Socialnetwork\ComponentHelper::checkPredefinedAuthIdList(array('bot', 'imconnector')))):
	...
	<div class="user-profile-block-wrap-r">
		...
		<div class="user-profile-events">
			...
			<? if ($arUser['ACTIVITY_STATUS'] == 'email') : ?>
					...
					<? if (isModuleInstalled('thurlyos') && $USER->canDoOperation('edit_all_users')) : ?>
			...
			<? else : ?>
					...
					<? if ($arUser["ACTIVITY_STATUS"] == "invited" && ($USER->CanDoOperation('thurlyos_invite') && CModule::IncludeModule('thurlyos') || !IsModuleInstalled("thurlyos") && $USER->CanDoOperation('edit_all_users'))):
					...
					<?if ($arUser["ACTIVITY_STATUS"] == "extranet" && IsModuleInstalled('thurlyos') && $USER->CanDoOperation("edit_all_users")):?>
			...
			<div class="user-profile-events-cont"><?
				if (
					file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/components/thurly/socialnetwork.admin.set")
					&& $arResult["SHOW_SONET_ADMIN"]

				):
					?><?
					$APPLICATION->IncludeComponent(
						"thurly:socialnetwork.admin.set",
						"",
						Array(
							"PROCESS_ONLY" => "Y"
						),
...
<!-- // otp -->
			...
			if(
				...
				&& $USER->GetID() == $arUser["ID"]
				&& \Thurly\Main\Config\Option::get('socialservices', 'network_last_update_check', 0) > 0
			)
			{
?>
				<tr>
