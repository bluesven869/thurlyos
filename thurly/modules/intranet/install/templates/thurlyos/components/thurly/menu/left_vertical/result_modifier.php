...
if (!function_exists("recalculateCrmCounter"))
{
	...
	{
		if (preg_match("~^crm_~i", $counterId) && \Thurly\Main\Loader::includeModule("crm"))
		{
			\Thurly\Crm\Counter\EntityCounterManager::prepareValue($counterId);
		}
	}
}
...
if (
	...
	&& $GLOBALS['USER']->CanDoOperation('thurlyos_config')
	|| !IsModuleInstalled("thurlyos")
...
{
	if (!(CThurlyOS::IsLicensePaid() || CThurlyOS::IsDemoLicense() || CThurlyOS::IsNfrLicense()))
	{
...
if (!$arResult["IS_EXTRANET"] && $GLOBALS["USER"]->isAuthorized())
{
	...
	$cacheId = "thurlyos_group_list_".$userId."_".SITE_ID."_".isModuleInstalled("extranet");
	$cacheDir = "/bx/thurlyos_group_list/".$userId;
	$cache = new CPHPCache;
