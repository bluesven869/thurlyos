...
if (StrLen($errorMessage) <= 0)
{
	...
	if (IsModuleInstalled("extranet"))
	{
		$arGroups[] = array(
			...
			"DESCRIPTION" => GetMessage("SONET_GROUP_DESCRIPTION_NEW_4"),
			"=DATE_CREATE" => $GLOBALS["DB"]->CurrentTimeFunction(),
			"=DATE_UPDATE" => $GLOBALS["DB"]->CurrentTimeFunction(),
			"ACTIVE" => "Y",
			"VISIBLE" => "N",
...
if (StrLen($errorMessage) <= 0)
{
	foreach ($arGroupsId as $ind => $val)
	{
		...
		{
			$groupStorage = \Thurly\Disk\Driver::getInstance()->addGroupStorage($val);
			if($groupStorage)
			{
				$errorCollection = new Thurly\Disk\Internals\Error\ErrorCollection;
				\Thurly\Disk\Sharing::connectGroupToSelfUserStorage(1, $groupStorage, $errorCollection);
			}
		}
	}
}
