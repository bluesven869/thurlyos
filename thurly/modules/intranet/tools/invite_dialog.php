...
?><div style="width: 542px; min-height: 200px; padding: 5px; overflow-y: auto; margin: 5px;">
	...
	if (
		(
			...
			&& !CThurlyOS::isInvitingUsersAllowed()
		)
		|| (
	...
	else
	{
		...
		{
			$bMailInstalled = true;
			$dbService = \Thurly\Mail\MailServicesTable::getList(array(
				'filter' => array(
					'ACTIVE' => 'Y',
					'=SITE_ID' => $SITE_ID
				),
			...
			while ($arService = $dbService->fetch())
			{
				$arMailServices[$arService['ID']] = array(
					...
					'name' => $arService['NAME'],
					'link' => $arService['LINK'],
					'icon' => \Thurly\Mail\MailServicesTable::getIconSrc($arService['NAME'], $arService['ICON']),
		...
		)
		{
			...
			elseif($strAction == "add")
			{
				...
				if ($bMailInstalled)
				{
					...
					{
						require_once($_SERVER['DOCUMENT_ROOT']."/thurly/components/thurly/intranet.mail.setup/helper.php");
	...
	if (!$strError)
	{
		...
		$GLOBALS["APPLICATION"]->AddHeadScript("/thurly/js/intranet/invite-dialog.js");
		$GLOBALS["APPLICATION"]->SetAdditionalCSS("/thurly/components/thurly/main.post.form/templates/.default/style.css");
	...
	?>
		<div class="popup-window-tabs-box" id="intranet-dialog-tabs"><?
			...
			<span class="popup-window-tab<?=(IsModuleInstalled("thurlyos")  ? " popup-window-tab-selected" : "")?>" id="intranet-dialog-tab-self" data-action="self">
				<?=GetMessage('BX24_INVITE_DIALOG_TAB_SELF_TITLE')?>
			</span>
			<?endif?>

			<span class="popup-window-tab<?=(!IsModuleInstalled("thurlyos")  ? " popup-window-tab-selected" : "")?>" id="intranet-dialog-tab-invite" data-action="invite">
			...
			if (IsModuleInstalled("thurlyos"))
			{
				$isUserAdmin = CThurlyOS::IsPortalAdmin($USER->GetID());
			...
			?>
				<div class="popup-window-tab-content<?=(IsModuleInstalled("thurlyos") ? " popup-window-tab-content-selected" : "")?>" id="intranet-dialog-tab-content-self" data-user-type="employee">
						...
						<div style="margin-right: 10px;">
							...
							<div <?if ($registerSettings["REGISTER"] != "Y"):?>style="display: none"<?endif?> id="intranet-dialog-tab-content-self-block">
								...
								<div class = "invite-dialog-inv-text-bold" style="margin-left: 25px">
									...
									<input type="text" class="invite-dialog-inv-form-inp" value="<?=\Thurly\Main\Text\Converter::getHtmlConverter()->encode($registerUrl)?>" id="allow_register_url" readonly="readonly"/>
								...
								<div id="intranet-dialog-tab-content-self-hidden-block" style="display: none; margin-left:27px">
									...
									<div style="color: rgba(128, 128, 128, 0.87); font-size: 11px; margin-top: 10px;"><?=GetMessage("BX24_INVITE_DIALOG_REGISTER_INVITE_ADD_INFO_N", array("#PATH_CONFIGS#" => CThurlyOS::PATH_CONFIGS))?></div>
									<?endif?>
								</div>
							</div>
						</div>
			...
			?>
				<div class="popup-window-tab-content<?=(!IsModuleInstalled("thurlyos") ? " popup-window-tab-content-selected" : "")?>" id="intranet-dialog-tab-content-invite" data-user-type="employee"><?
					...
					?><form method="POST" action="<?echo BX_ROOT."/tools/intranet_invite_dialog.php"?>" id="INVITE_DIALOG_FORM"><?
						...
						elseif (
							!$messageTextDisabled
							&& $userMessage = CUserOptions::GetOption((IsModuleInstalled("thurlyos") ? "thurlyos" : "intranet"), "invite_message_text"))
						{
		...
		<script type="text/javascript">
			...
			if(windowObj)
			{
				var popup = windowObj.B24.ThurlyOSInviteDialog.popup;
				<?
				if($ID_INVITED):
					?>
