...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["Update"].$_POST["Apply"].$_POST["RestoreDefaults"] <> '' && check_thurly_sessid())
{
...
window.networkRegister = (function(){
	...
	return function(s)
	{
		if(BX.type.isString(s))
		{
			...
			BX.ajax.loadJSON('/thurly/tools/oauth/socserv.ajax.php', {
		...
		else if(!!s && !!s.client_id)
		{
			...
			if(!document.forms.socserv_settings['thurlyosnet_domain' + currentSite])
			{
				document.forms.socserv_settings.appendChild(BX.create('input', {attrs: {
					'type': 'hidden',
					'name': 'thurlyosnet_domain' + currentSite
				}}));

				var el = document.forms.socserv_settings['thurlyosnet_id' + currentSite];

				BX.findPreviousSibling(el.parentNode.parentNode).cells[1].innerHTML = BX.util.htmlspecialchars(domainUrl);
			}

			document.forms.socserv_settings['thurlyosnet_id' + currentSite].value = s.client_id;
			document.forms.socserv_settings['thurlyosnet_secret' + currentSite].value = s.client_secret;
			document.forms.socserv_settings['thurlyosnet_domain' + currentSite].value = domainUrl;

			BX('AUTH_SERVICES'+currentSite+'ThurlyOSNet').checked = true;

			BX.focus(document.forms.socserv_settings['thurlyosnet_id' + currentSite]);
...
?>
				<tr>
					...
					<td>&nbsp;</td>
					<td><a href="javascript:void(0)" onclick="MoveRowUp(this)"><img src="/thurly/images/socialservices/up.gif" width="16" height="16" alt="<?echo GetMessage("soc_serv_opt_up")?>" border="0"></a></td>
					<td><a href="javascript:void(0)" onclick="MoveRowDown(this)"><img src="/thurly/images/socialservices/down.gif" width="16" height="16" alt="<?echo GetMessage("soc_serv_opt_down")?>" border="0"></a></td>
				</tr>
<?
	endforeach;
?>
