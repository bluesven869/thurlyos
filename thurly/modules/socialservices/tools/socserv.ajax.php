...
?>
BX.loadCSS('/thurly/js/socialservices/css/ss.css');
BX.loadScript('/thurly/js/socialservices/ss_timeman.js?<?=$t?>', function(){
...
<?
	...
	elseif($_REQUEST['action'] == "registernetwork")
	{
		...
		{
			$res = CSocServThurlyOSNet::registerSite($domain);
		}
		else
		{
	...
	{
		\Thurly\Socialservices\Network::setAdminPopupSession();

		Header('Content-Type: application/json');
		echo \Thurly\Main\Web\Json::encode(array("result" => 1));
	}
}
