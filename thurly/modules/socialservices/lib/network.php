...
class Network
{
	...
	public function isEnabled()
	{
		...
		{
			if(method_exists('CThurlyOS', 'isEmailConfirmed') && !\CThurlyOS::isEmailConfirmed())
			{
				return false;
			}
		}
		...
		{
			if(is_object($USER) && $USER->GetID() > 0 && \Thurly\Replica\Client\User::getGuid($USER->GetID()) === false)
			{
				return false;
			}
		}
	...
	public function setEnable($enable = true)
	{
		...
		$query = \CThurlyOSNetPortalTransport::init();
		if (!$query)
		{
	...
	{
		$result = \Thurly\Main\UserTable::getById($userId);
		$user = $result->fetch();
	...
	/**
	 * Prepares and shows popup offerring current user to attach thurlyos.net account
	 */
	...
	public static function initAdminPopup(array $params = array())
	{
		...
		\CJSCore::registerExt("socialservices_admin", array(
			'js' => "/thurly/js/socialservices/ss_admin.js",
			'css' => "/thurly/js/socialservices/css/ss_admin.css",
