...
$needAuthorize = false;
$accessToken = '';
$redirectURI = \CHTTP::URN2URI('/thurly/tools/oauth/thurlyos.php');
$savedPortalURI = CUserOptions::GetOption('socialservices', 'thurlyos_task_planer_gadget_portal', '');
$requestCode = CUserOptions::GetOption('socialservices', 'thurlyos_task_planer_gadget_code', '');

if($savedPortalURI !== $portalURI || $savedPortalURI == '')
{
	$needAuthorize = true;
	CUserOptions::SetOption('socialservices', 'thurlyos_task_planer_gadget_portal', $portalURI, false);
}
if(!preg_match('|^http[s]?|', $portalURI))
	$portalURI = 'https://'.$portalURI;
$objThurlyOAuth = new CSocServThurlyOAuth($clientId, $clientSecret, $portalURI, $redirectURI, $USER->GetID());
$objThurlyOAuth->addScope(array('task', 'calendar'));

$arTasks = $arEvents = array();
if($requestCode <> '')
{
	$accessToken = $objThurlyOAuth->getAccessToken($requestCode);
	$objThurlyOAuth->getEntityOAuth()->saveDataDB();

	CUserOptions::SetOption('socialservices', 'thurlyos_task_planer_gadget_code', '', false);
}
else
{
	$accessToken = $objThurlyOAuth->getStorageToken();
}

if($accessToken != '' && $domain != '' && !$needAuthorize)
{
	$obApp = new CThurlyPHPAppTransport($accessToken, $portalURI);
	...
	else
	{
		...
		<div class="bx-gadgets-content-padding-rl bx-gadgets-content-padding-t">
			...
			https://&nbsp;<input type="text" id="portal" name="portal" value="<?=\Thurly\Main\Text\HtmlFilter::encode($domain)?>">
