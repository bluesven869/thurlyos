...
$twitNum = 100;
$dbSocservUser = CSocServAuthDB::GetList(array("PERSONAL_PHOTO" => "DESC"), array('USER_ID' => $userID, "!EXTERNAL_AUTH_ID" => 'ThurlyOSOAuth'));
//***************************************
...
while($arUser = $dbSocservUser->Fetch())
{
	...
	if($arUser["CAN_DELETE"] != 'N' && $arParams["ALLOW_DELETE"] != 'N')
	{
		...
		$arService['DELETE_LINK'] = $APPLICATION->GetCurPageParam("action=delete&user_id=".$arUser["ID"]."&".thurly_sessid_get(), array("action", "user_id"));
	}
...
if ($_SERVER["REQUEST_METHOD"] == "GET" && $_REQUEST["action"] == "delete" && isset($_REQUEST["user_id"]) && intval($_REQUEST["user_id"] > 0) && check_thurly_sessid())
{
	$userId = intval($_REQUEST["user_id"]);
	if(in_array($userId, $arResult["ALLOW_DELETE_ID"]))
	{
		$result = \Thurly\Socialservices\UserTable::delete($userId);
		if (!$result->isSuccess())
		{
