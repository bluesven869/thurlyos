...
if(!empty($arResult["AUTH_SERVICES"]))
{
	...
	<?
	$APPLICATION->IncludeComponent("thurly:socserv.auth.form", "split",
		array(
			...
			foreach($arResult["DB_SOCSERV_USER"] as $key => $arUser)
			{
				...
				<tr class="soc-serv-personal">
					...
					<td class="split-item-actions">
						...
						<a class="split-delete-item" href="?action=delete&user_id=<?=$arUser["ID"]."&".thurly_sessid_get()?>" onclick="return confirm('<?=GetMessage("SS_PROFILE_DELETE_CONFIRM")?>')" title=<?=GetMessage("SS_DELETE")?>></a>
						<?endif;?>
					</td>
				</tr>
				<?
