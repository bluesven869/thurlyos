...
class CSocServThurlyOAuth extends CSocServAuth
{
	const ID = "ThurlyOSOAuth";

	/** @var CThurlyOAuthInterface null  */
	...
	public function getEntityOAuth($code = false)
	{
		if(!$this->entityOAuth)
		{
			$this->entityOAuth = new CThurlyOAuthInterface($this->appID, $this->appSecret, $this->portalURI);
		}
	...
	public function getStorageToken()
	{
		...
		if($userId > 0)
		{
			$dbSocservUser = CSocServAuthDB::GetList(array(), array('USER_ID' => $userId, 'XML_ID' => $this->appID, "EXTERNAL_AUTH_ID" => "ThurlyOSOAuth", 'PERSONAL_WWW' => $this->portalURI), false, false, array("OATOKEN", "REFRESH_TOKEN", "OATOKEN_EXPIRES", "OASECRET"));
	...
	public function Authorize()
	{
		...
		{
			$redirect_uri = \CHTTP::URN2URI('/thurly/tools/oauth/thurlyos.php');
			$userId = intval($_REQUEST['uid']);
			$appID = trim(COption::GetOptionString("socialservices", "thurlyos_gadget_appid", ''));
			$appSecret = trim(COption::GetOptionString("socialservices", "thurlyos_gadget_appsecret", ''));
			$portalURI = $_REQUEST['domain'];
			if(strpos($portalURI, "http://") === false && strpos($portalURI, "https://") === false)
				$portalURI = "https://".$portalURI;
			$gAuth = new CThurlyOAuthInterface($appID, $appSecret, $portalURI, $_REQUEST["code"]);
	...
	public function gadgetAuthorize()
	{
		...
		if((isset($_REQUEST["code"]) && $_REQUEST["code"] <> '') && CSocServAuthManager::CheckUniqueKey())
		{
			CUserOptions::SetOption('socialservices', 'thurlyos_task_planer_gadget_code', $_REQUEST["code"]);
		}
...
class CThurlyOAuthInterface extends CSocServOAuthTransport
{
	const SERVICE_ID = 'ThurlyOSOAuth';
	...
	public function GetAccessToken($redirect_uri)
	{
		...
		$arResult = \Thurly\Main\Web\Json::decode($result);
	...
	public function getNewAccessToken($refreshToken, $userId = 0, $save = false, $scope = array())
	{
		...
		if(isset($arResult["access_token"]) && $arResult["access_token"] <> '')
		{
			...
			{
				CUserOptions::SetOption('socialservices', 'thurlyos_task_planer_gadget_token', $this->access_token, false, $userId);
				CUserOptions::SetOption('socialservices', 'thurlyos_task_planer_gadget_token_expire', $this->accessTokenExpires + time(), false, $userId);
				CUserOptions::SetOption('socialservices', 'thurlyos_task_planer_gadget_refresh_token', $this->refresh_token, false, $userId);
			}
	...
	public function saveDataDB()
	{
		...
		$dbSocUser = CSocServAuthDB::GetList(array(), array('XML_ID' => $this->appID, 'PERSONAL_WWW' => $this->portalURI, 'EXTERNAL_AUTH_ID' => "ThurlyOSOAuth"), false, false, array("ID"));
...
class CThurlyPHPAppTransport
{
	...
	{
		return \Thurly\Main\Web\Json::decode($result);
	}
	...
	public function callSigned($methodName, $additionalParams = '')
	{
		if($this->signatureKey)
		{
			...
			{
				$signer = new Thurly\Socialservices\ThurlyOSSigner();
