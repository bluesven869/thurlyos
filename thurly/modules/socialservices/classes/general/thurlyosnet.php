...
{
	$defaultValue = \Thurly\Main\Config\Option::get('socialservices', 'network_url', 'https://www.thurlyos.net');
	define('B24NETWORK_URL', $defaultValue);
}

class CSocServThurlyOSNet extends CSocServAuth
{
	const ID = "ThurlyOSNet";
	...
	{
		return array(
			array("thurlyosnet_domain", Loc::getMessage("socserv_b24net_domain"), "", array("statictext")),
			array("thurlyosnet_id", Loc::getMessage("socserv_b24net_id"), "", array("text", 40)),
			array("thurlyosnet_secret", Loc::getMessage("socserv_b24net_secret"), "", array("text", 40)),
	...
	{
		return self::GetOption('thurlyosnet_id') !== '' && self::GetOption('thurlyosnet_secret') !== '';
	}
	...
	public function getFormHtml($arParams)
	{
		...
		return $arParams["FOR_INTRANET"]
			...
			: '<a href="javascript:void(0)" onclick="BX.util.popup(\''.htmlspecialcharsbx(CUtil::JSEscape($url)).'\', 800, 600)" class="bx-ss-button thurlyosnet-button thurlyosnet-button-'.LANGUAGE_ID.'"></a><span class="bx-spacer"></span><span>'.$phrase.'</span>';
	}
	...
	public function getEntityOAuth($code = false)
	{
		if(!$this->entityOAuth)
		{
			$this->entityOAuth = new CThurlyOSNetOAuthInterface();
		}
	...
	public function Authorize($skipCheck = false)
	{
		...
		{
			$redirect_uri = \CHTTP::URN2URI('/thurly/tools/oauth/thurlyosnet.php');
			$bProcessState = true;
			$bAdmin = false;
			...
			if($this->getEntityOAuth()->GetAccessToken($redirect_uri) !== false)
			{
				...
				if($authError !== true && !IsModuleInstalled('thurlyos'))
				{
		...
		if($bSuccess && !self::GetOption("thurlyosnet_domain"))
		{
			$request = \Thurly\Main\Context::getCurrent()->getRequest();
			self::SetOption("thurlyosnet_domain", ($request->isHttps() ? "https://" : "http://").$request->getHttpHost());
		}

		$aRemove = array_merge(array("auth_service_error", "auth_service_id", "code", "error_reason", "error", "error_description", "check_key", "current_fieldset", "checkword"), \Thurly\Main\HttpRequest::getSystemParameters());
...
<?
	...
	public static function registerSite($domain)
	{
		...
		{
			$query = new \Thurly\Main\Web\HttpClient();
			$result = $query->get(B24NETWORK_URL.'/client.php?action=register&redirect_uri='.urlencode($domain.'/thurly/tools/oauth/thurlyosnet.php').'&key='.urlencode(LICENSE_KEY));
...
class CThurlyOSNetOAuthInterface
{
	...
	public function __construct($appID = false, $appSecret = false, $code = false)
	{
		if($appID === false)
		{
			$appID = trim(CSocServThurlyOSNet::GetOption("thurlyosnet_id"));
		}

		if($appSecret === false)
		{
			$appSecret = trim(CSocServThurlyOSNet::GetOption("thurlyosnet_secret"));
		}
	...
	public function GetCurrentUser()
	{
		...
		{
			$ob = new CThurlyOSNetTransport($this->access_token);
			$res = $ob->getProfile();
...
class CThurlyOSNetTransport
{
	...
	{
		$ob = new CThurlyOSNetOAuthInterface();
	...
	public function call($methodName, $additionalParams = null)
	{
		...
		$result = $http->post(
			CThurlyOSNetOAuthInterface::NET_URL.self::SERVICE_URL.$methodName,
			$request
		);
...
class CThurlyOSNetPortalTransport extends CThurlyOSNetTransport
{
	...
	public static function init()
	{
		...
		if(!$result)
		{
			$interface = new CThurlyOSNetOAuthInterface();
