...
use Thurly\Socialservices\ContactTable;
...
class CSocServAuthManager
{
	...
	protected static function AppyUserSettings($suffix)
	{
		...
		if(is_array($arServices))
		{
			...
			\Thurly\Main\Type\Collection::sortByColumn($arAuthServices, "__sort");
		}
	...
	{
		if($service_id === 'ThurlyOSOAuth')
		{
			CSocServThurlyOAuth::gadgetAuthorize();
		}
		...
		if(isset(self::$arAuthServices[$service_id]))
		{
			...
			if(
				...
				|| (
					$service_id == CSocServThurlyOSNet::ID
	...
	{
		if(!isset($_SESSION["UNIQUE_KEY"]))
			$_SESSION["UNIQUE_KEY"] = md5(thurly_sessid_get().uniqid(rand(), true));
	}
	...
	public static function PostIntoBuzz($arUserTwit, $lastTwitId, $arSiteId=array())
	{
		if(isset($arUserTwit['statuses']) && !empty($arUserTwit['statuses']))
		{
			foreach($arUserTwit['statuses'] as $userTwit)
			{
				...
				if(IsModuleInstalled('thurlyos') && defined('BX24_HOST_NAME'))
				{
	...
	public static function PostIntoBuzzAsBlog($userTwit, $arSiteId=array(), $userLogin = '')
	{
		...
		if(!CModule::IncludeModule("blog") || !CModule::IncludeModule("socialnetwork"))
			return;
		$arParams = array();
		if((IsModuleInstalled('thurlyos') && defined('BX24_HOST_NAME')) && $userLogin != '')
		{
...
/*
		...
		)
		{
			...
			$controllerUrlList = array(
				'de' => 'https://www.thurlyos.de/controller',
				'ua' => 'https://www.thurlyos.ua/controller',
				'ru' => 'https://www.thurlyos.ru/controller',
				'eu' => 'https://www.thurlyos.eu/controller',
				'la' => 'https://www.thurlyos.es/controller',
				'br' => 'https://www.thurlyos.com.br/controller',
				'in' => 'https://www.thurlyos.in/controller',
				'cn' => 'https://www.thurlyos.cn/controller',
				'kz' => 'https://www.thurlyos.kz/controller',
				'by' => 'https://www.thurlyos.by/controller',
				'fr' => 'https://www.thurlyos.fr/controller',
				'pl' => 'https://www.thurlyos.pl/controller',
			);

			$lang = \CThurlyOS::getLicensePrefix();
