...
class CGoogleOAuthInterface extends CSocServOAuthTransport
{
	...
	const REDIRECT_URI = "/thurly/tools/oauth/google.php";
	...
	{
		$savedScope = \Thurly\Main\Config\Option::get('socialservices', 'saved_scope_'.static::SERVICE_ID, '');
	...
	protected function saveScope()
	{
		...
		\Thurly\Main\Config\Option::set('socialservices', 'saved_scope_'.static::SERVICE_ID, serialize($scope));
	}
	...
	public function GetAccessToken($redirect_uri = false)
	{
		...
		try
		{
			$this->arResult = \Thurly\Main\Web\Json::decode($result);
		}
	...
	public function GetCurrentUser()
	{
		...
		$result = \Thurly\Main\Web\Json::decode($result);
	...
	public function getNewAccessToken($refreshToken = false, $userId = 0, $save = false)
	{
		...
		if(isset($this->arResult["access_token"]) && $this->arResult["access_token"] <> '')
		{
			...
			{
				$dbSocservUser = \Thurly\Socialservices\UserTable::getList(array(
					'filter' => array(
				...
				if($arOauth = $dbSocservUser->Fetch())
				{
					\Thurly\Socialservices\UserTable::update($arOauth["ID"], array(
						"OATOKEN" => $this->access_token,
						"OATOKEN_EXPIRES" => $this->accessTokenExpires)
					);
				}
