...
if (is_array($ARR_RESULT) && count($ARR_RESULT)>0 && strlen($delete)>0 && (check_thurly_sessid() || defined("FORM_NOT_CHECK_SESSID"))) 
{
	foreach($ARR_RESULT as $rid) CFormResult::Delete($rid);
}

$del_id = intval($del_id);
if ($del_id>0 && (check_thurly_sessid() || defined("FORM_NOT_CHECK_SESSID"))) CFormResult::Delete($del_id);
...
if (strlen($save)>0 && $REQUEST_METHOD=="POST" && (check_thurly_sessid() || defined("FORM_NOT_CHECK_SESSID")))
{
	// обновляем записи
