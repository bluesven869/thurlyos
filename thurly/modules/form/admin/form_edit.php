...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/form/prolog.php");
...
// copying
if ($copy_id > 0 && check_thurly_sessid() && $F_RIGHT >= 30)
{
	$new_id = CForm::Copy($copy_id);
	if (strlen($strError)<=0 && intval($new_id)>0)
	{
		LocalRedirect("/thurly/admin/form_edit.php?ID=".$new_id."&lang=".LANGUAGE_ID);
	}
}

// cleaning
if ($reset_id > 0 && check_thurly_sessid() && $F_RIGHT >= 30)
{
	CForm::Reset($reset_id);
	LocalRedirect("/thurly/admin/form_edit.php?ID=".$reset_id."&lang=".LANGUAGE_ID);
}
...
while ($wr=$w->Fetch())
{
	$arGroups[] = array(
		"ID" => $wr["ID"],
		"NAME" => "[<a title=\"".GetMessage("FORM_GROUP_EDIT")."\" href=\"/thurly/admin/group_edit.php?ID=".intval($wr["ID"])."&lang=".LANGUAGE_ID."\">".intval($wr["ID"])."</a>] ".htmlspecialcharsbx($wr["NAME"]),
	);
}
...
if ((strlen($_REQUEST['save'])>0 || strlen($_REQUEST['apply'])>0) && $_SERVER['REQUEST_METHOD']=="POST" && ($F_RIGHT>=30 || $ID<=0) && check_thurly_sessid())
{
	...
	if ($res>0)
	{
		...
		if (strlen($strError)<=0)
		{
			if (strlen($_REQUEST['save'])>0)
			{
				...
				else LocalRedirect("/thurly/admin/form_list.php?lang=".LANGUAGE_ID);

			}
			else LocalRedirect("/thurly/admin/form_edit.php?ID=".$ID."&lang=".LANGUAGE_ID."&".$tabControl->ActiveTabParam().(!empty($_REQUEST["back_url"]) ? "&back_url=".urlencode($_REQUEST["back_url"]) : ""));
...
<?
	...
	$aMenu = $ID > 0 ? array() : array(
		array(
			...
			"ICON"	=> "btn_list",
			"LINK"	=> "/thurly/admin/form_list.php?lang=".LANGUAGE_ID
		)
	);
	...
	if ($ID>0 && (CForm::IsAdmin() || $F_RIGHT>=30))
	{
		...
		if (CForm::IsAdmin())
		{
			...
			$aMenu[] = array(
				...
				"ICON"	=> "btn_copy",
				"LINK"	=> "form_edit.php?copy_id=".$ID."&ID=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get(),
				);
		}
		...
		if ($F_RIGHT>=30)
		{
			$aMenu[] = array(
				...
				"ICON"	=> "btn_delete",
				"LINK"	=> "javascript:if(confirm('".GetMessage("FORM_CONFIRM_DELETE_RESULTS")."'))window.location='form_edit.php?ID=".$ID. "&reset_id=".$ID."&".thurly_sessid_get()."&lang=".LANGUAGE_ID."';",
				);
		}
		...
		if (CForm::IsAdmin())
		{
			$aMenu[] = array(
				...
				"LINK"	=> "javascript:if(confirm('".GetMessage("FORM_CONFIRM_DELETE")."'))window.location='form_list.php?action=delete&ID=".$ID. "&".thurly_sessid_get()."&lang=".LANGUAGE_ID."';",
				);
		}
	}
	...
	<tr>
		...
		while(list($sid, $arrS) = each($arrSites)):
			...
			<div class="adm-list-item">
				...
				<div class="adm-list-label"><label for="<?=htmlspecialcharsbx($sid)?>"><?echo "[<a class=tablebodylink href='/thurly/admin/site_edit.php?LID=".htmlspecialcharsbx($sid)."&lang=".LANGUAGE_ID."'>".htmlspecialcharsbx($sid)."</a>]&nbsp;".htmlspecialcharsbx($arrS["NAME"])?></label></div>
			</div>
			<?
		endwhile;
		?></div></td>
...
?>
	<tr>
		...
		<td>
			...
			[<a href="/thurly/admin/message_admin.php?find_type_id=FORM_FILLING_<?=$str_SID?>&set_filter=Y"><?echo GetMessage("FORM_VIEW_TEMPLATE_LIST")?></a>]
		</td>
	</tr>
<?
	}
...
</script><script src="/thurly/js/form/form_info.js?<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/thurly/js/form/form_info.js')?>"></script><script>
var arrInputObjects = [];
...
</script><script src="/thurly/js/form/form_taskbar.js?<?=@filemtime($_SERVER['DOCUMENT_ROOT'].'/thurly/js/form/form_taskbar.js')?>"></script>
	<tr>
...
function GenerateMailTemplate()
{
	...
	var url = '/thurly/admin/form_mail.php?lang=<?=LANGUAGE_ID?>&<?=thurly_sessid_get()?>&WEB_FORM_ID=<?=intval($ID)?>';
...
function _processData(arReturn)
{
	...
	if (arReturn.TEMPLATES && arReturn.TEMPLATES.length > 0)
	{
		...
		for (var i=0; i<arReturn.TEMPLATES.length; i++)
		{
			...
			obCell.innerHTML += '[<a class="tablebodylink" href="/thurly/admin/message_edit.php?ID=' + arReturn.TEMPLATES[i].ID + '&lang=<?=LANGUAGE_ID?>">' + arReturn.TEMPLATES[i].ID + '</a>]&nbsp;';
...
function DeleteMailTemplate(template_id)
{
	...
	if (confirm('<?echo CUtil::JSEscape(GetMessage('FORM_CONFIRM_DEL_MAIL_TEMPLATE'))?>'))
	{
		...
		//var url = 'message_admin.php?action=delete&ID=' + template_id + '&lang=<?echo LANGUAGE_ID?>&<?=thurly_sessid_get()?>';
		var url = '/thurly/admin/form_mail.php?action=delete&ID=' + template_id + '&lang=<?echo LANGUAGE_ID?>&<?=thurly_sessid_get()?>&WEB_FORM_ID=<?=intval($ID)?>';
...
<?
	...
	<tr>
		...
		?><?if ($RESULT_ID>0) :?>&nbsp;[&nbsp;<a href="/thurly/admin/form_result_view.php?lang=<?=LANGUAGE_ID?>&WEB_FORM_ID=<?=$ID?>&RESULT_ID=<?=$RESULT_ID?>"><?=GetMessage("FORM_PREVIEW")?></a>&nbsp;]<?endif;?></td>
	</tr>
<?
...
?>
	<tr>
		...
		?><?if ($RESULT_ID>0) :?>&nbsp;[&nbsp;<a href="/thurly/admin/form_result_edit.php?lang=<?=LANGUAGE_ID?>&WEB_FORM_ID=<?=$ID?>&RESULT_ID=<?=$RESULT_ID?>"><?=GetMessage("FORM_PREVIEW")?></a>&nbsp;]<?endif;?></td>
	</tr>
	<?endif;?>
	<?if ($ID>0):?>
	<tr>
...
.action-edit {background: scroll transparent url(/thurly/images/form/options_buttons.gif) no-repeat 0 0; }
.action-delete {background: scroll transparent url(/thurly/images/form/options_buttons.gif) no-repeat -29px 0; }
</style>
<script type="text/javascript">
...
function CRMSave(wnd, data_old, form)
{
	...
	if (data.URL)
	{
		...
		if (!res)
		{
			var proto = data.URL.match(/\.thurlyos\./) ? 'https' : 'http';
	...
	else
	{
		...
		BX.ajax({
			method: 'POST',
			dataType: 'json',
			url: '/thurly/admin/settings.php?mid=form&saveCrm=Y&ajax=Y&<?=thurly_sessid_get()?>',
			data: query_str,
			onsuccess: CRMRedraw
		});
...
function loadCrmFields(ID, cb, additional)
{
	...
	if (ID > 0)
	{
		...
		BX.ajax.loadJSON('/thurly/admin/form_crm.php?action=check&ID='+ID+'&<?=thurly_sessid_get();?>', additional, function(res)
		{
			BX.cleanNode(s);
...
<?
	endforeach;
?>
	</select>&nbsp;&nbsp;<a href="/thurly/admin/settings.php?lang=<?=LANGUAGE_ID?>&amp;mid=form&amp;tabControl_active_tab=edit_crm"><?=GetMessage('FORM_CRM_GOTOLIST')?></a></td>
</tr>
