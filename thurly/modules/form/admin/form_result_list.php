...
$HELP_FILE_ACCESS = $APPLICATION->GetFileAccessPermission("/thurly/modules/form/help/".LANGUAGE_ID."/index.php");
$FORM_RIGHT = $APPLICATION->GetGroupRight("form");
$MAIN_RIGHT = $APPLICATION->GetGroupRight("main");
$WEB_FORM_NAME = $arForm["SID"];
##########
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/form/include.php");
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/form/admin/form_result_list.php");
...
if ($lAdmin->EditAction() && /*$FORM_RIGHT>="W"*/ $F_RIGHT >= 20 && check_thurly_sessid())
{
...
if(($arID = $lAdmin->GroupAction()) && /*$FORM_RIGHT>="W"*/ $F_RIGHT >= 20 && check_thurly_sessid())
{
		...
		foreach ($arrAnswers[$f_ID] as $arFieldValues)
		{
			...
			if (CModule::IncludeModule("statistic")):
				if (intval($f_STAT_GUEST_ID)>0) :
					$row->AddViewField("STAT_GUEST_ID", " [<a title='".GetMessage("FORM_GUEST_TITLE")."' href='/thurly/admin/guest_list.php?lang=".LANGUAGE_ID."&find_id=". $f_STAT_GUEST_ID."&set_filter=Y'>".$f_STAT_GUEST_ID."</a>]");
				endif;
				if (intval($f_STAT_SESSION_ID)>0) :
					$row->AddViewField("STAT_SESSION_ID", " (<a title='".GetMessage("FORM_SESSION_TITLE")."' href='/thurly/admin/session_list.php?lang=".LANGUAGE_ID."&find_id=". $f_STAT_SESSION_ID."&set_filter=Y'>".$f_STAT_SESSION_ID."</a>)");
				endif;
			endif;
			...
			{
				$row->AddViewField('SENT_TO_CRM', $f_SENT_TO_CRM == 'N' ? '<img src="/thurly/images/form/red.gif" height="14" width="14" border="0" id="crm_sent_'.$f_ID.'" />' : '<img src="/thurly/images/form/green.gif" height="14" width="14" border="0" id="crm_sent_'.$f_ID.'" />');
			}
			...
			foreach ($arFieldValues as $arrA)
			{
				...
				if (intval($arrA["USER_FILE_ID"])>0)
				{
					if ($arrA["USER_FILE_IS_IMAGE"]=="Y") :
						...
						$txt.="<a title=\"".GetMessage("FORM_VIEW_FILE")."\" href=\"/thurly/tools/form_show_file.php?rid=$f_ID&hash=$arrA[USER_FILE_HASH]&lang=".LANGUAGE_ID."\">".htmlspecialcharsbx($arrA["USER_FILE_NAME"])."</a><br>(";
						$txt.= CFile::FormatSize($arrA["USER_FILE_SIZE"]);
						$txt.=")<br>[&nbsp;<a title=\"".str_replace("#FILE_NAME#", $arrA["USER_FILE_NAME"], GetMessage("FORM_DOWNLOAD_FILE"))."\" href=\"/thurly/tools/form_show_file.php?rid=$f_ID&hash=$arrA[USER_FILE_HASH]&lang=".LANGUAGE_ID."&action=download\">".GetMessage("FORM_DOWNLOAD")."</a>&nbsp;]";
					endif;
				}
...
{
	BX.ajax.loadJSON('/thurly/admin/form_crm.php?action=add_lead&FORM_ID=<?=$WEB_FORM_ID?>&RESULT_ID=' + parseInt(RESULT_ID) + '&<?=thurly_sessid_get()?>', function(result){
		if (result.result == 'ok')
		{
			var img = BX('crm_sent_' + RESULT_ID);
			if (img)
				img.src = '/thurly/images/form/green.gif';
		...
		else
		{
			var img = BX('crm_sent_' + RESULT_ID);
			if (img)
				img.src = '/thurly/images/form/red.gif';
