...
if ($FORM_RIGHT=="W" && $reset_id>0 && check_thurly_sessid()) CForm::Reset($reset_id);

$copy_id = intval($makecopy_id);
if ($FORM_RIGHT=="W" && $copy_id>0 && check_thurly_sessid())
{
	CForm::Copy($copy_id);
...
if ($lAdmin->EditAction() && $FORM_RIGHT>="W" && check_thurly_sessid())
{
...
if(($arID = $lAdmin->GroupAction()) && $FORM_RIGHT=="W" && check_thurly_sessid())
{
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if (is_array($arrSITE))
	{
		foreach($arrSITE as $sid)
			$txt.= "<a href='/thurly/admin/site_edit.php?LID=".htmlspecialcharsbx($sid, ENT_QUOTES)."&lang=".LANGUAGE_ID."'>".htmlspecialcharsbx($sid)."</a>,";
	}
	else
		$txt="&nbsp;";
		$txt=trim($txt,",");
	...
	if (CForm::IsAdmin())
		$arActions[] = array("ICON"=>"copy", "TITLE"=>GetMessage("FORM_COPY_ALT"),"ACTION"=>$lAdmin->ActionRedirect("form_list.php?lang=".LANGUAGE_ID."&amp;makecopy_id=$f_ID&".thurly_sessid_get()),"TEXT"=>GetMessage("FORM_COPY"));
	if ($F_RIGHT>=30)
	{
		$arActions[] = array("SEPARATOR"=>true);
		$arActions[] = array("TITLE"=>GetMessage("FORM_DELETE_RESULTS_ALT"),"ACTION"=>"javascript:if(confirm('".CUtil::JSEscape(GetMessage("FORM_CONFIRM_DELETE_RESULTS"))."')) window.location='?lang=".LANGUAGE_ID."&reset_id=".$f_ID."&".thurly_sessid_get()."'", "TEXT"=>GetMessage("FORM_DELETE_RESULTS"));
	}
	if (CForm::IsAdmin())
		$arActions[] = array("ICON"=>"delete", "TITLE"=>GetMessage("FORM_DELETE_ALT"),"ACTION"=>"javascript:if(confirm('".CUtil::JSEscape(GetMessage("FORM_CONFIRM_DELETE"))."')) window.location='?lang=".LANGUAGE_ID."&action=delete&ID=$f_ID&".thurly_sessid_get()."'","TEXT"=>GetMessage("FORM_DELETE"));
...
<tr>
	<td><?=GetMessage("FORM_F_SITE")?><br><img src="/thurly/images/form/mouse.gif" width="44" height="21" border=0 alt=""></td>
	<td><?
	$ref = array();
	$ref_id = array();
