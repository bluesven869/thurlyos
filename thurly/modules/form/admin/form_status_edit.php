...
// copying
if (intval($copy_id)>0 && check_thurly_sessid() && $F_RIGHT >= 30)
{
...
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $F_RIGHT >= 30 && check_thurly_sessid())
{
	$arFields = array(
		"FORM_ID"				=> $WEB_FORM_ID,
		"C_SORT"				=> $C_SORT,
...
if ($F_RIGHT>=30 && $ID>0)
{
	...
	$aMenu[] = array(
		...
		"LINK"	=> "form_status_edit.php?ID=".$ID."&amp;copy_id=".$ID."&lang=".LANGUAGE_ID."&WEB_FORM_ID=".$WEB_FORM_ID."&".thurly_sessid_get()
		);
	...
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("FORM_CONFIRM_DELETE")."'))window.location='form_status_list.php?action=delete&ID=".$ID."&WEB_FORM_ID=".$WEB_FORM_ID."&".thurly_sessid_get()."&lang=".LANGUAGE_ID."';",
		"WARNING"=>"Y"
		);
...
function GenerateMailTemplate()
{
	...
	var url = '/thurly/admin/form_status_mail.php?lang=<?=LANGUAGE_ID?>&<?=thurly_sessid_get()?>&WEB_FORM_ID=<?=$WEB_FORM_ID;?>&STATUS_ID=<?=intval($ID)?>';
...
function DeleteMailTemplate(template_id)
{
	...
	if (confirm('<?echo CUtil::JSEscape(GetMessage('FORM_CONFIRM_DEL_MAIL_TEMPLATE'))?>'))
	{
		...
		var url = '/thurly/admin/form_status_mail.php?action=delete&ID=' + template_id + '&lang=<?echo LANGUAGE_ID?>&<?=thurly_sessid_get()?>&WEB_FORM_ID=<?=intval($ID)?>';
			...
			?>
				<table cellpadding="0" cellspacing="0">
					<tr>
						...
						if (count($arrMAIL)>0):
							?>&nbsp;&nbsp;&nbsp;[&nbsp;<a href="/thurly/admin/message_admin.php?find_type_id=FORM_STATUS_CHANGE_<?=$arForm['SID']?>_<?=$ID?>&set_filter=Y"><?echo GetMessage("FORM_VIEW_TEMPLATE_LIST")?></a>&nbsp;]<?
						endif;
						?></td>
					</tr>
				</table>
...
?>
	...
	?>
	<tr>
		<td><?=GetMessage('FORM_MOVE_RIGHTS')." [MOVE]";?><br><img src="/thurly/images/form/mouse.gif" width="44" height="21" border=0 alt=""></td>
		<td><?
		echo SelectBoxMFromArray("arPERMISSION_MOVE[]", array("REFERENCE" => $arr_ref, "REFERENCE_ID" => $arr_ref_id), $arPERMISSION_MOVE, "", false, 8, "");
		?></td>
	</tr>
	<tr>
		<td><?echo GetMessage('FORM_VIEW_RIGHTS')." [VIEW]"?><br><img src="/thurly/images/form/mouse.gif" width="44" height="21" border=0 alt=""></td>
		<td><?
		echo SelectBoxMFromArray("arPERMISSION_VIEW[]", array("REFERENCE" => $arr_ref, "REFERENCE_ID" => $arr_ref_id), $arPERMISSION_VIEW, "", false, 8, "");
		?></td>
	</tr>
	<tr>
		<td><?echo GetMessage('FORM_EDIT_RIGHTS')." [EDIT]"?><br><img src="/thurly/images/form/mouse.gif" width="44" height="21" border=0 alt=""></td>
		<td><?
		echo SelectBoxMFromArray("arPERMISSION_EDIT[]", array("REFERENCE" => $arr_ref, "REFERENCE_ID" => $arr_ref_id), $arPERMISSION_EDIT, "", false, 8, "");
		?></td>
	</tr>
	<tr>
		<td><?echo GetMessage('FORM_DELETE_RIGHTS')." [DELETE]"?><br><img src="/thurly/images/form/mouse.gif" width="44" height="21" border=0 alt=""></td>
		<td><?
