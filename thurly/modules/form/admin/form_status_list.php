...
$cp_id = intval($cp_id);
if ($cp_id>0 && check_thurly_sessid() && $F_RIGHT >= 30)
{
...
if ($lAdmin->EditAction() && $FORM_RIGHT>="W" && $F_RIGHT>=30 && check_thurly_sessid())
{
...
if(($arID = $lAdmin->GroupAction()) && $FORM_RIGHT=="W" && $F_RIGHT>=30 && check_thurly_sessid())
{
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if ($F_RIGHT>=30)
	{
		$arActions[] = array(
			...
			"ACTION"=>$lAdmin->ActionRedirect("form_status_list.php?lang=".LANGUAGE_ID."&cp_id=$f_ID&WEB_FORM_ID=$WEB_FORM_ID&".thurly_sessid_get()),
			"TEXT"=>GetMessage("FORM_CP")
		);
		$arActions[] = array(
			"ICON"=>"delete",
			"TITLE"=>GetMessage("FORM_STATUS_DELETE_ALT"),
			"ACTION"=>"javascript:if(confirm('".GetMessage("FORM_DELETE_STATUS_CONFIRM")."')) ".$lAdmin->ActionRedirect("?lang=".LANGUAGE_ID."&action=delete&ID=$f_ID&WEB_FORM_ID=$WEB_FORM_ID&".thurly_sessid_get()),
...
$aMenu = array(
	array(
		...
		"LINK"	=>	"/thurly/admin/form_status_edit.php?lang=".LANGUAGE_ID."&WEB_FORM_ID=".$WEB_FORM_ID,
	),
);
