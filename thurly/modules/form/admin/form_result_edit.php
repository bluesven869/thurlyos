...
if ($RESULT_ID > 0)
{
	...
	if (!($arrResult=$q->Fetch()))
	{
		...
		echo "<p><a href='/thurly/admin/form_result_list.php?lang=".LANGUAGE_ID."&WEB_FORM_ID=".$WEB_FORM_ID."'>".$title."</a></p>";
...
if ($old_module_version=="Y")
{
	...
	if ($F_RIGHT>=30 && $_SERVER['REQUEST_METHOD']=="GET" && strlen($_REQUEST['save'])>0 && check_thurly_sessid())
	{
...
if ($old_module_version != 'Y' && $_SERVER['REQUEST_METHOD'] == "POST" && intval($WEB_FORM_ID)>0 && (strlen($_REQUEST['save'])>0 || strlen($_REQUEST['apply'])>0) && check_thurly_sessid())
{
	$arrVALUES = $_REQUEST;
	...
	if (strlen($error)<=0)
	{
		...
		{
			if (strlen($_REQUEST['apply'])>0) LocalRedirect("/thurly/admin/form_result_edit.php?lang=".LANGUAGE_ID."&WEB_FORM_ID=".$WEB_FORM_ID."&RESULT_ID=".$RESULT_ID);
			else LocalRedirect("/thurly/admin/form_result_list.php?lang=".LANGUAGE_ID."&WEB_FORM_ID=".$WEB_FORM_ID);
		}
	}
	else $strError .= $error;
}
...
if ($can_add)
{
	$aMenu[] = array(
		...
		"LINK"		=> "/thurly/admin/form_result_edit.php?lang=".LANGUAGE_ID."&WEB_FORM_ID=".$WEB_FORM_ID
		);
}
...
if ($can_view)
{
	$aMenu[] = array(
		...
		"LINK"		=> "/thurly/admin/form_result_view.php?lang=".LANGUAGE_ID."&WEB_FORM_ID=".$WEB_FORM_ID."&RESULT_ID=".$RESULT_ID
		);
}
*/
...
if ($can_delete)
{
	$aMenu[] = array(
		...
		"LINK"	=> "javascript:if(confirm('".GetMessage("FORM_CONFIRM_DELETE")."'))window.location='form_result_list.php?action=delete&ID=".$RESULT_ID."&WEB_FORM_ID=".$WEB_FORM_ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		"WARNING"=>"Y"
		);
}
	...
	if ($old_module_version=="Y"):
		...
		<table cellspacing="0" cellpadding="2">
			...
			<tr>
				...
				<td>[<a title="<?=GetMessage("FORM_GUEST_ALT")?>" href="/thurly/admin/guest_list.php?lang=<?=LANGUAGE_ID?>&find_id=<?=$arrResult["STAT_GUEST_ID"]?>&set_filter=Y"><?=$arrResult["STAT_GUEST_ID"]?></a>]</td>
			</tr>
			<?endif;?>
			<?if (intval($arrResult["STAT_SESSION_ID"])>0):?>
			<tr>
				<td><b><?=GetMessage("FORM_SESSION")?></b></td>
				<td>[<a title="<?=GetMessage("FORM_SESSION_ALT")?>" href="/thurly/admin/session_list.php?lang=<?=LANGUAGE_ID?>&find_id=<?=$arrResult["STAT_SESSION_ID"]?>&set_filter=Y"><?=$arrResult["STAT_SESSION_ID"]?></a>]</td>
			</tr>
			<?endif;?>
			<?endif;?>
			<?endif;?>
...
?>
<form name="form1" action="/thurly/admin/form_result_edit.php?lang=<?echo LANG?>&WEB_FORM_ID=<?echo $WEB_FORM_ID?><?if($RESULT_ID>0): echo '&RESULT_ID='.$RESULT_ID; endif;?>" method="POST" enctype="multipart/form-data">
	...
	if ($EDIT_STATUS == 'Y' && $bResultStatusChangeAccess)
	{
		...
		while ($arStatus = $dbStatusList->Fetch())
		{
			$arStatus['REFERENCE'] = str_replace(
				...
				'[<a href="/thurly/admin/form_status_edit.php?lang='.LANG.'&WEB_FORM_ID='.$WEB_FORM_ID.'&ID='.$arStatus['REFERENCE_ID'].'">'.$arStatus['REFERENCE_ID'].'</a>]',
...
?>
	<tr>
		...
		<td>[<a href="/thurly/admin/form_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$WEB_FORM_ID?>"><?=$WEB_FORM_ID?></a>]&nbsp;<a href="/thurly/admin/form_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$WEB_FORM_ID?>"><?=htmlspecialcharsbx($arForm["NAME"])?> (<?=htmlspecialcharsbx($arForm["SID"])?>)</a></td>
	</tr>
	<tr>
...
?>
			[<a title="<?echo GetMessage('FORM_RESULT_EDIT_USER')?>" href='/thurly/admin/user_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$arUser["ID"]?>'><?=$arUser['ID']?></a>] <a title="<?echo GetMessage('FORM_RESULT_EDIT_USER')?>" href='/thurly/admin/user_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$arUser["ID"]?>'><?=htmlspecialcharsbx($arUser["NAME"])?> <?=htmlspecialcharsbx($arUser["LAST_NAME"])?> (<?=htmlspecialcharsbx($arUser['LOGIN'])?>)</a><?if($arrResult["RESULT_USER_AUTH"] == "N"): ?>&nbsp;<?echo GetMessage('FORM_RESULT_EDIT_USER_NOTAUTH')?><?endif;?>
<?
	else:
?>
...
?>
	<tr>
		...
		<td>[<a title="<?=GetMessage("FORM_GUEST_ALT")?>" href="/thurly/admin/guest_list.php?lang=<?=LANGUAGE_ID?>&find_id=<?=$arrResult["STAT_GUEST_ID"]?>&find_id_exact_match=Y&set_filter=Y"><?=$arrResult["STAT_GUEST_ID"]?></a>]</td>
	</tr>
	<tr>
		<td><?=GetMessage("FORM_SESSION")?></td>
		<td>[<a href="/thurly/admin/session_list.php?lang=<?=LANGUAGE_ID?>&find_id=<?=$arrResult["STAT_SESSION_ID"]?>&find_id_exact_match=Y&set_filter=Y"><?=$arrResult["STAT_SESSION_ID"]?></a>]</td>
	</tr>
<?
		}
	endif; // RESULT_ID > 0
				...
				switch ($arAnswer["FIELD_TYPE"])
				{
					...
					case "file":
						...
						if ($arFile = CFormResult::GetFileByAnswerID($RESULT_ID, $arAnswer["ID"]))
						{
							if (intval($arFile["USER_FILE_ID"])>0)
							{
								echo "<a title=\"".GetMessage("FORM_VIEW_FILE")."\" target=\"_blank\" class=\"tablebodylink\" href=\"/thurly/tools/form_show_file.php?rid=".$RESULT_ID."&hash=".$arFile["USER_FILE_HASH"]."&lang=".LANGUAGE_ID."\">".htmlspecialcharsbx($arFile["USER_FILE_NAME"])."</a>&nbsp;(";
								echo CFile::FormatSize($arFile["USER_FILE_SIZE"]);
								echo ")&nbsp;&nbsp;[&nbsp;<a title=\"".str_replace("#FILE_NAME#", $arFile["USER_FILE_NAME"], GetMessage("FORM_DOWNLOAD_FILE"))."\" class=\"tablebodylink\" href=\"/thurly/tools/form_show_file.php?rid=".$RESULT_ID."&hash=".$arFile["USER_FILE_HASH"]."&lang=".LANGUAGE_ID."&action=download\">".GetMessage("FORM_DOWNLOAD")."</a>&nbsp;]<br />";
