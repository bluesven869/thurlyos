	<?
$module_id = "form";
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$module_id."/include.php");
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$module_id."/options.php");
$old_module_version = CForm::IsOldVersion();
$FORM_RIGHT = $APPLICATION->GetGroupRight($module_id);
if ($FORM_RIGHT>="R") :

if ($REQUEST_METHOD=="GET" && CForm::IsAdmin() && strlen($RestoreDefaults)>0 && check_thurly_sessid())
{
	...
	while($zr = $z->Fetch())
	{
		$ar = array();
		$ar["ID"] = intval($zr["ID"]);
		$ar["NAME"] = htmlspecialcharsbx($zr["NAME"])." [<a title=\"".GetMessage("MAIN_USER_GROUP_TITLE")."\" href=\"/thurly/admin/group_edit.php?ID=".intval($zr["ID"])."&lang=".LANGUAGE_ID."\">".intval($zr["ID"])."</a>]";
...
if($REQUEST_METHOD=="POST" && strlen($Update)>0 && CForm::IsAdmin() && check_thurly_sessid())
{
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $_REQUEST['saveCrm'] && CForm::IsAdmin() && check_thurly_sessid())
{
	if ($_REQUEST['ajax'])
	...
	if ($_REQUEST['ajax'])
	{
		...
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/epilog_admin_after.php");
		exit();
	}
}
...
<?
$tabControl->Begin();
?><form method="POST" action="<?echo $APPLICATION->GetCurPage()?>?mid=<?=htmlspecialcharsbx($mid)?>&lang=<?=LANGUAGE_ID?>"><?=thurly_sessid_post()?><?
...
function CRMRedraw(data)
{
	...
	for (var i = 0; i < data.length; i++)
	{
		...
		if (!!data[i].LOGIN && !!data[i].PASSWORD)
		{
			...
			BX.ajax.loadJSON('/thurly/admin/form_crm.php?action=check&reload=Y&ID=' + BX.util.urlencode(data[i].ID) + '&LOGIN=' + BX.util.urlencode(data[i].LOGIN) + '&PASSWORD=' + BX.util.urlencode(data[i].PASSWORD) + '&<?=thurly_sessid_get()?>', BX.delegate(function(data) {
					BX.cleanNode(this);
...
function CRMSave(wnd, data_old, form)
{
	...
	else
	{
		...
		BX.ajax({
			method: 'POST',
			dataType: 'json',
			url: '<?=CUtil::JSEscape($APPLICATION->GetCurPageParam('saveCrm=Y&ajax=Y&'.thurly_sessid_get()))?>',
			data: query_str,
			onsuccess: CRMRedraw
		});
...
function CRMCheck(ID)
{
	...
	BX.ajax.loadJSON('/thurly/admin/form_crm.php?action=check&ID='+ID+'&reload=Y&<?=thurly_sessid_get();?>', function(res)
	{
		if (!!res)
		{
			if (res.result == 'ok')
...
function RestoreDefaults()
{
	if(confirm('<?echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>'))
		window.location = "<?echo $APPLICATION->GetCurPage()?>?RestoreDefaults=Y&lang=<?=LANGUAGE_ID?>&mid=<?echo urlencode($mid)?>&<?=thurly_sessid_get()?>";
}
</script>
