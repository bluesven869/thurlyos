...
class CAllForm extends CForm_old
{
	...
	public static function SetMailTemplate($WEB_FORM_ID, $ADD_NEW_TEMPLATE="Y", $old_SID="", $bReturnFullInfo = false)
	{
		...
		if ($arrForm = $q->Fetch())
		{
			...
			{
				IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/form/admin/form_mail.php", $arLang["LID"]);
			...
			if ($ADD_NEW_TEMPLATE=="Y")
			{
				...
				if (is_array($arrFormSite) && count($arrFormSite)>0)
				{
					...
					{
						IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/form/admin/form_mail.php", $arrSiteLang[$sid]);
...
http://#SERVER_NAME#/thurly/admin/form_result_view.php?lang=".$arrSiteLang[$sid]."&WEB_FORM_ID=#RS_FORM_ID#&RESULT_ID=#RS_RESULT_ID#
	...
	public static function GetByID_admin($WEB_FORM_ID, $current_section = false)
	{
		...
		if ($arForm = $dbForm->Fetch())
		{
			...
			$arForm['ADMIN_MENU'][] = array(
				...
				"LINK"	=> "/thurly/admin/form_edit.php?lang=".LANGUAGE_ID."&ID=".$WEB_FORM_ID,
			...
			$arForm['ADMIN_MENU'][] = array(
				...
				"LINK"	=> "/thurly/admin/form_result_list.php?lang=".LANGUAGE_ID."&WEB_FORM_ID=".$WEB_FORM_ID,
