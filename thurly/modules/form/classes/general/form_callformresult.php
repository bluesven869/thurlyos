			...
			if ($zr = $z->Fetch())
			{
				...
				if ($IN_EVENT3===false)
				{
					$event3 = strlen($zr["STAT_EVENT3"])<=0
						...
						).$_SERVER["HTTP_HOST"]."/thurly/admin/form_result_list.php?lang=".LANGUAGE_ID."&WEB_FORM_ID=".$WEB_FORM_ID."&find_id=".$RESULT_ID."&find_id_exact_match=Y&set_filter=Y"
						: $zr["STAT_EVENT3"];
				}
		...
		if ($arrResult)
		{
			...
			if ($arrFORM = $z->Fetch())
			{
				...
				foreach($arrTemplates as $arrTemplate)
				{
					...
					$OLD_MESS = $MESS;
					$MESS = array();
					IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/form/admin/form_mail.php", $arrSites[$arrTemplate["SITE_ID"]]["LANGUAGE_ID"]);
										...
										case 'image':
											...
											if ($USER_FILE_EXIST)
											{
												...
												if ($fr = $f->Fetch())
												{
													...
													$url = ($APPLICATION->IsHTTPS() ? "https://" : "http://").$_SERVER["HTTP_HOST"]. "/thurly/tools/form_show_file.php?rid=".$RESULT_ID. "&hash=".$arrA["USER_FILE_HASH"]."&lang=".LANGUAGE_ID;
									...
									switch ($arrA['FIELD_TYPE'])
									{
										...
										case 'image':
											...
											if ($USER_FILE_EXIST)
											{
												...
												{
													$file_size = CFile::FormatSize($fr["FILE_SIZE"]);
													$url = ($APPLICATION->IsHTTPS() ? "https://" : "http://").$_SERVER["HTTP_HOST"]. "/thurly/tools/form_show_file.php?rid=".$RESULT_ID. "&hash=".$arrA["USER_FILE_HASH"]."&action=download&lang=".LANGUAGE_ID;
