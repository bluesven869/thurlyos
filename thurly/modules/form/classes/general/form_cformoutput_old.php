	...
	function Init($arParams, $admin = false)
	{
		...
		if ($this->WEB_FORM_ID > 0)
		{
			...
			if ($this->isAccessForm())
			{
				...
				if (!$this->comp2 || $this->arParams["COMPONENT"]["componentName"] != "thurly:form.result.list" || $this->isAccessFormResultList())
				{
					if ($this->RESULT_ID)
					{
						if ($this->isAccessFormResult($arrResult))
						{
							...
							if (
								...
								$this->comp2 && // for components2 - check whether there's editing or viewing and check rights for it
									(
										$this->arParams["COMPONENT"]["componentName"] == "thurly:form.result.edit" && !$this->isAccessFormResultEdit()
										||
										$this->arParams["COMPONENT"]["componentName"] == "thurly:form.result.view" && !$this->isAccessFormResultView()
									)
							)
							{
							...
							else
							{
								...
								if ($this->arResult)
								{
									if ($this->comp2 && $this->arParams["COMPONENT"]["componentName"] == "thurly:form.result.view")
									{
...
/*****************************************/
	...
	function Out()
	{
		...
		if(!file_exists($_SERVER["DOCUMENT_ROOT"].$path))
		{
			...
			if(!file_exists($_SERVER["DOCUMENT_ROOT"].$path))
			{
				$path = "/thurly/modules/form/install/templates/".$rel_path;
		...
		if ($USER->IsAdmin())
		{
			$APPLICATION->AddPanelButton(array(
				"SORT" => 100,
				"MAIN_SORT" => 1000,
				"HREF" => "/thurly/admin/form_edit.php?lang=".LANGUAGE_ID."&amp;ID=".$this->WEB_FORM_ID. "&amp;tabControl_active_tab=edit5&back_url=".urlencode($back_url),
				"SRC"  => "/thurly/images/form/edit_templ.gif",
			...
			$APPLICATION->AddPanelButton(array(
				...
				"MAIN_SORT" => 1000,
				"HREF" => $href,
				"SRC"  => "/thurly/images/form/edit_default_templ.gif",
			...
			if($APPLICATION->GetShowIncludeAreas())
			{
				if ($this->arForm["USE_DEFAULT_TEMPLATE"] == "N")
				{
					...
					$arIcons[] =
							Array(
								"URL" => "/thurly/admin/form_edit.php?lang=".LANGUAGE_ID."&amp;ID=".$this->WEB_FORM_ID. "&amp;tabControl_active_tab=edit5&back_url=".urlencode($back_url),
								"ICON" => 'form-edit-tpl',
								"ALT" => GetMessage("FORM_PUBLIC_ICON_EDIT_TPL")
								);
		...
		if (is_array($arrResultAnswer))
		{
			...
			foreach ($arrResultAnswer as $key => $arrA)
			{
				...
				if (intval($arrA["USER_FILE_ID"])>0)
				{
					...
					{
						$file_link = "/thurly/tools/form_show_file.php?rid=".$this->RESULT_ID."&hash=".$arrA["USER_FILE_HASH"]."&lang=".LANGUAGE_ID;
