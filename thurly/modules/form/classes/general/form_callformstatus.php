	...
	function SetMailTemplate($WEB_FORM_ID, $STATUS_ID, $ADD_NEW_TEMPLATE="Y", $old_SID="", $bReturnFullInfo = false)
	{
		...
		if ($arrForm = $q->Fetch())
		{
			...
			if ($arrStatus = $dbRes->Fetch())
			{
				...
				{
					IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/form/admin/form_status_mail.php", $arLang["LID"]);
				...
				if ($ADD_NEW_TEMPLATE=="Y")
				{
					...
					if (is_array($arrFormSite) && count($arrFormSite)>0)
					{
						...
						{
							IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/form/admin/form_status_mail.php", $arrSiteLang[$sid]);
