...
use Thurly\Security\Filter;

class CSecurityFilter
{
	const DEFAULT_REQUEST_ORDER = "GP";
	private $doBlock = false;
	/** @var \Thurly\Security\Filter\Request $requestFilter */
	private $requestFilter = null;
	/** @var \Thurly\Security\Filter\Server $serverFilter */
	private $serverFilter = null;
	/** @var \Thurly\Main\Context $context */
	private $context = null;
	private $splittingChar = '';
	protected $defaultAuditors = array(
		array('type' => 'XSS', 'class' => 'Thurly\Security\Filter\Auditor\Xss'),
		array('type' => 'SQL', 'class' => 'Thurly\Security\Filter\Auditor\Sql'),
		array('type' => 'PHP', 'class' => 'Thurly\Security\Filter\Auditor\Path')
	);
	...
	public function __construct($customOptions = array(), $char = "")
	{
		...
		$this->context = \Thurly\Main\Application::getInstance()->getContext();
	...
	public static function OnAdminInformerInsertItems()
	{
		...
		$setupLink = '/thurly/admin/security_filter.php?lang='.LANGUAGE_ID;
		$WAFAIParams = array(
		...
		try
		{
			if (self::IsActive())
			{
				...
				$eventLink = '/thurly/admin/event_log.php?set_filter=Y&find_type=audit_type_id&find_audit_type[]=SECURITY_FILTER_SQL&find_audit_type[]=SECURITY_FILTER_XSS&find_audit_type[]=SECURITY_FILTER_XSS2&find_audit_type[]=SECURITY_FILTER_PHP&mod=security&find_timestamp_x_1='.$timestampX.'&lang='.LANGUAGE_ID;
...
';
	...
	{
		$wafConfig = \Thurly\Main\Config\Configuration::getValue("waf");
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="/thurly/themes/.default/404.css" />
	</head>
	<body>
