...
use Thurly\Security\Mfa\OtpException;

/**
 * @deprecated use \Thurly\Security\Mfa\Otp
 */
class CSecurityUser
{
	const BX_SECURITY_SYNC_WINDOW = 15000;

	/** @var \Thurly\Security\Mfa\Otp[]*/
	...
	{
		\Thurly\Security\Mfa\UserTable::delete($userId);
		return true;
	}
	...
	{
		$otpRecheckAgent = 'Thurly\Security\Mfa\OtpEvents::onRecheckDeactivate();';
		if($pActive)
		{
			if(!CSecurityUser::isActive())
			{
				...
				$f = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/security/options_user_settings.php", "w");
				fwrite($f, "<?include(\$_SERVER[\"DOCUMENT_ROOT\"].\"/thurly/modules/security/options_user_settings_1.php\");?>");
				fclose($f);
		...
		else
		{
			if(CSecurityUser::isActive())
			{
				...
				CAgent::RemoveAgent($otpRecheckAgent, "security");
				unlink($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/security/options_user_settings.php");
