	...
	extends CSecurityBaseTest
{
	...
	protected static function formatRecommendation(array $weakUsers)
	{
		...
		{
			$result .= sprintf(
				'<br><a href="/thurly/admin/user_edit.php?ID=%d" target="_blank">%s<a/>',
				$id, $login
			);
		}
	...
	protected static function getPasswordDictionary()
	{
		...
		if (is_null($passwords))
			$passwords = file($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/security/data/passwordlist.txt', FILE_IGNORE_NEW_LINES | FILE_SKIP_EMPTY_LINES);
