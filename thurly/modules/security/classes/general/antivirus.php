...
class CSecurityAntiVirus
{
	...
	var $replace = 1;//
	var $replacement = "<!-- deleted by thurly Antivirus -->"; //на что заменяем, если заменяем..
	...
	{
		if(strpos($this->atributes, 'src="/thurly/') !== false)
			return 1;
		...
		if(strpos($this->body, 'document\.write(\'<link href="/thurly/templates/') !== false)
			return 11;
		...
		if(preg_match('/window\.location\s*=\s*[\'"]\/thurly\/admin\/iblock_bizproc_workflow_edit.php/', $this->body))
			return 36;

		if(preg_match('/window\.parent\.location\.href\s*=\s*[\'"]\/thurly\/admin\/sale_order_new.php/', $this->body))
			return 43;
	...
	function CreateTrace()
	{
		...
		$fn = $_SERVER["DOCUMENT_ROOT"]."/thurly/cache/virus.db/".$cache_id.".vir";
		if(!file_exists($fn))
		{
			CheckDirPath($fn);
			$f = fopen($fn, "wb");
