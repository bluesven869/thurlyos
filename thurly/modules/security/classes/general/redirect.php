		...
		if(!$bSkipCheck && preg_match("/^(http|https|ftp):\\/\\//i", $url_l))
		{
			...
			if(!$good)
			{
				...
				)
				{
					...
					{
						$url_c = \Thurly\Main\Text\Encoding::convertEncoding($url, "UTF-8", LANG_CHARSET);
					}
					else
					{
						$url_c = $url;
...
<table class="error-404" border="0" cellpadding="0" cellspacing="0" align="center">
	...
	public static function SetActive($bActive = false)
	{
		if($bActive)
		{
			...
			{
				COption::SetOptionString("security", "redirect_sid", Thurly\Main\Security\Random::getString(32));
