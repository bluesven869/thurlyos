...
class CFileChecker
{
	...
	public static function GetList()
	{
		...
		if ($handle = @opendir($_SERVER["DOCUMENT_ROOT"]."/thurly/modules"))
		{
			while (($file = readdir($handle)) !== false)
			{
				...
				if (!is_file($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$file))
					continue;
...
class CFileCheckerLog
{
	...
	var $serverFileLogPath = "/thurly/modules";
	...
	public static function StartDownload($ts)
	{
		...
		$serverFileLog = "/thurly/modules/serverfilelog-".$ts.".dat";
	...
	public static function StopDownload($ts)
	{
		...
		$streamFileName = "/thurly/modules/serverfilelog-".$ts.".dat2";
		if (function_exists("gzcompress"))
...
if (in_array($_REQUEST["fcajax"], array("cl", "vf", "df")))
{
	...
	elseif ($_REQUEST["fcajax"] == "df")
	{
		...
		unlink($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/serverfilelog-".$_REQUEST["df"].".dat");
		echo $_REQUEST["df"];
	}

	die();
}
if ($_REQUEST["fcdld"] == "Y" && check_thurly_sessid() && $canCollect)
{
...
$scriptName = "/thurly/modules/security/admin/security_file_verifier.php";
$isSubscribed = CFileCheckerSubscriber::IsSubscribed($scriptName);
$okMessage = "";
$errorMessage = "";

if ($_SERVER["REQUEST_METHOD"] == "POST" && $tabStep == 2 && check_thurly_sessid())
{
	if ($isSubscribed)
	{
		$errorMessageTmp = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $tabStep == 3 && check_thurly_sessid())
{
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $tabStep == 4 && check_thurly_sessid())
{
	...
	elseif ($_REQUEST["action"] == "verify" && $canVerify)
	{
		...
		if (isset($_FILES["crc_file"]) && is_uploaded_file($_FILES["crc_file"]["tmp_name"]))
		{
			...
			if(function_exists("gzcompress"))
			{
				$hFile = fopen($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/serverfilelog-1.dat", "wb");
			...
			{
				copy($_FILES["crc_file"]["tmp_name"], $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/serverfilelog-1.dat");
			}
			...
			if ($ts > 0)
			{
				@unlink($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/serverfilelog-".$ts.".dat");
				copy($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/serverfilelog-1.dat", $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/serverfilelog-".$ts.".dat");
				unlink($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/serverfilelog-1.dat");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $tabStep == 5 && check_thurly_sessid())
{
			...
			<td colspan="2">
				...
				<!--
					function CFDeleteLog(ts)
					{
						...
						BX.ajax.post(
							"/thurly/admin/security_file_verifier.php",
							"fcajax=df&df=" + ts + "&<?= thurly_sessid_get() ?>",
							function(result)
							{
						...
						foreach ($arFiles as $arFile)
						{
							...
							<tr onclick="CFTrClick('<?= $arFile["TIMESTAMP_X"] ?>')" id="cf_tr_select_dfile_<?= $arFile["TIMESTAMP_X"] ?>">
								...
								if (($arFile["REGION"] & BX_FILE_CHECKER_REGION_KERNEL) != 0)
									echo GetMessage("MFC1_R_KERNEL")." ( /thurly/modules )<br />";
								if (($arFile["REGION"] & BX_FILE_CHECKER_REGION_ROOT) != 0)
									echo GetMessage("MFC1_R_SYSTEM")." ( /thurly )<br />";
...
?>
	<tr class="adm-detail-required-field">
		...
		<td width="70%">
			<input type="checkbox" name="checker_region_kernel" id="id_checker_region_kernel" <?if(COption::GetOptionString("security", "checker_region_kernel")=="Y") echo "checked";?> value="Y"><label for="id_checker_region_kernel"><?= GetMessage("MFC1_R_KERNEL") ?> ( /thurly/modules )</label><br />
			<input type="checkbox" name="checker_region_root" id="id_checker_region_root" <?if(COption::GetOptionString("security", "checker_region_root")=="Y") echo "checked";?> value="Y"><label for="id_checker_region_root"><?= GetMessage("MFC1_R_SYSTEM") ?> ( /thurly )</label><br />
			<?if (BX_PERSONAL_ROOT != BX_ROOT):?>
	...
	<!--
		...
		function __FCCollectData(ts, completed, startPoint)
		{
			...
			<?if ($_REQUEST["action"] == "verify"):?>
				data = "fcajax=vf&df=<?= intval($_REQUEST["cf_select_dfile"]) ?>&pwd=<?= urlencode($_REQUEST['checker_pwd']) ?>&tm=<?= intval($_REQUEST['checker_time']) ?>&<?= thurly_sessid_get() ?>&ts=" + ts + "&completed=" + completed + "&startpoint=" + startPoint + "&updRand=" + updRand;
			<?elseif ($_REQUEST["action"] == "collect"):?>
				data = "fcajax=cl&region=<?= intval($region) ?>&exts=<?= urlencode($_REQUEST['checker_exts']) ?>&pwd=<?= urlencode($_REQUEST['checker_pwd']) ?>&tm=<?= intval($_REQUEST['checker_time']) ?>&<?= thurly_sessid_get() ?>&ts=" + ts + "&completed=" + completed + "&startpoint=" + startPoint + "&updRand=" + updRand;
			<?endif;?>
			BX.ajax.post("/thurly/admin/security_file_verifier.php", data, callback);
		}
		...
		function __FCLoadCollectDataResult(result)
		{
			...
			if (arData[0] == "FIN")
			{
				...
				<?elseif ($_REQUEST["action"] == "collect"):?>
					...
					__FCSuccessAdd("<?= GetMessage("MFC1_J_DWL_PROMT1") ?> <a href='/thurly/admin/security_file_verifier.php?fcdld=Y&ts=" + arData[1] + "&<?= thurly_sessid_get() ?>'><?= GetMessage("MFC1_J_DWL_PROMT2") ?></a>.");
				<?endif;?>
