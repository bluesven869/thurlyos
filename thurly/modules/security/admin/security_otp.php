...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/security/options_user_settings_1.php");
...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["save"].$_REQUEST["apply"].$_REQUEST["otp_siteb"] !="" && $canWrite && check_thurly_sessid())
{
	...
	if ($_POST['otp_default_type'])
		Thurly\Security\Mfa\Otp::setDefaultType($_POST['otp_default_type']);

	if (is_numeric($_POST['otp_mandatory_skip_days']))
		Thurly\Security\Mfa\Otp::setSkipMandatoryDays($_POST['otp_mandatory_skip_days']);

	Thurly\Security\Mfa\Otp::setMandatoryUsing($_POST['otp_mandatory_using'] === 'Y');

	if (is_array($_POST['otp_mandatory_rights']))
		Thurly\Security\Mfa\Otp::setMandatoryRights($_POST['otp_mandatory_rights']);

	if($_REQUEST["save"] != "" && $_GET["return_url"] != "")
		LocalRedirect($_GET["return_url"]);
	else
		LocalRedirect("/thurly/admin/security_otp.php?lang=".LANGUAGE_ID.$returnUrl."&".$tabControl->ActiveTabParam());
}

$availableTypes = \Thurly\Security\Mfa\Otp::getAvailableTypes();
$availableTypesDescription = \Thurly\Security\Mfa\Otp::getTypesDescription();
$defaultType = \Thurly\Security\Mfa\Otp::getDefaultType();
$targetRights = \Thurly\Security\Mfa\Otp::getMandatoryRights();
$access = new CAccess();
$targetRightsNames = $access->GetNames($targetRights);

CJSCore::Init(array('access'));
$APPLICATION->AddHeadScript('/thurly/js/security/admin/page/otp.js');
	...
	<td colspan="2">
		<div style=" padding: 20px; margin-top: 20px">
			...
			<div style="float: left; margin-right: 20px">
				<div style="-webkit-box-sizing: border-box; -moz-box-sizing: border-box; box-sizing: border-box; border: 2px solid #e0e3e5; border-radius: 2px; height: 156px; width: 156px; background: white url(/thurly/images/security/etoken_pass.png?v2) no-repeat center center;"></div>
			</div>
			<div>
			...
			if (in_array(LANGUAGE_ID, array('en', 'ru', 'de'), true))
				...
				$imageLanguage = \Thurly\Main\Localization\Loc::getDefaultLang(LANGUAGE_ID);
			?>
			...
			<div style="float: left; margin-right: 20px">
				<div style="-webkit-box-sizing: border-box; -moz-box-sizing: border-box; box-sizing: border-box; border: 2px solid #e0e3e5; border-radius: 2px; padding: 5px 10px; background: white; height: 150px;">
					<div style="float: left; background: url(/thurly/images/security/<?=$imageLanguage?>_login_step0.png) no-repeat top right; width: 220px; height: 120px; padding-top: 20px;" ><?=getMessage('SEC_OTP_DESCRIPTION_USING_STEP_0')?></div>
					<div style="float: left; background: url(/thurly/images/security/<?=$imageLanguage?>_login_step1.png) no-repeat top right; width: 220px; height: 120px; padding-top: 20px; margin-left:20px;"><?=getMessage('SEC_OTP_DESCRIPTION_USING_STEP_1')?></div>
				</div>
			</div>
			<div>
...
?>
	...
	<script id="settings" type="application/json"><?=\Thurly\Main\Web\Json::encode(array(
