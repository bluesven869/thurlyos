...
if($_SERVER['REQUEST_METHOD'] == "POST" && check_thurly_sessid() && $_POST["action"].$_POST["save"].$_POST["apply"] != "")
{
	$result = "error";
...
CUtil::InitJSCore();
$APPLICATION->AddHeadScript('/thurly/js/security/admin/page/scanner.js');
...
?>
	...
	<script id="scanner_messages" type="application/json"><?=\Thurly\Main\Web\Json::encode(IncludeModuleLangFile(__FILE__, false, true))?></script>
	<script id="scanner_results" type="application/json"><?=\Thurly\Main\Web\Json::encode($lastResults)?></script>
<?endif;?>
<?
