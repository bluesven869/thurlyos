...
if($_SERVER["REQUEST_METHOD"] == "POST" && $_REQUEST["save"].$_REQUEST["apply"] !="" && $canWrite && check_thurly_sessid())
{
	...
	else
	{
		...
		else
		{
			...
			{
				if($_REQUEST["apply"] != "")
					LocalRedirect("/thurly/admin/security_iprule_edit.php?ID=".$ID."&lang=".LANG."&".$tabControl->ActiveTabParam());
				else
					LocalRedirect("/thurly/admin/security_iprule_list.php?lang=".LANG);
			}
			else
			{
				if($e = $APPLICATION->GetException())
...
CUtil::InitJSCore();
$APPLICATION->AddHeadScript('/thurly/js/security/admin/interface.js');
...
if($ID > 0)
{
	...
	$aMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".GetMessage("SEC_IP_EDIT_MENU_DELETE_CONF")."'))window.location='security_iprule_list.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."';",
		"ICON" => "btn_delete",
	);
}
