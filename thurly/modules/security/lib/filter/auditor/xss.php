	...
	extends Base
{
	...
	protected function getWhiteList()
	{
		...
		return array(
			...
			array(
				'store_match' => '#(<script)(\\s+type="text/javascript"\\s+src="/thurly/components/thurly/player/wmvplayer/(silverlight|wmvplayer).js"[\\s/]*></script>)#s',
				'store_replacement' => '<'.$safeReplacement.'\\2',
				'restore_match' => '#<'.$safeReplacement.'#',
				'restore_replacement' => '<script',
			),
			array(
				'store_match' => '#(<script)(\\s+type\\s*=\\s*"text/javascript"\\s*>\\s*new\\s+jeroenwijering\\.Player\\(\\s*document\\.getElementById\\(\\s*"[a-zA-Z0-9_]+"\\s*\\)\\s*,\\s*"/thurly/components/thurly/player/wmvplayer/wmvplayer.xaml"\\s*,\\s*{\\s*(?:[a-zA-Z0-9_]+:\\s+"[a-zA-Z0-9/.]*?"[,\\s]*)*}\\);</script>)#s',
				'store_replacement' => '<'.$safeReplacement.'\\2',
				'restore_match' => '#<'.$safeReplacement.'#',
				'restore_replacement' => '<script',
			)
