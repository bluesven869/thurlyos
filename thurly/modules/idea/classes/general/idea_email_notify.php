...
Class CIdeaManagmentEmailNotify
{
	...
	public static function GetList($order = Array(), $arFilter = Array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
			...
			foreach($arFilter as $dkey => $val)
			{
				...
				if ($key["FIELD"] == "ID" && is_array($val))
				{
					...
					foreach ($val as $v)
					{
						...
						{
							$res[] = array("=SUBSCRIBE_TYPE" => \Thurly\Idea\NotifyEmailTable::SUBSCRIBE_TYPE_ALL);
						}
						else if (strpos($v, self::SUBSCRIBE_IDEA_COMMENT) === 0)
						{
							$res[] = array(
								"=ENTITY_TYPE" => \Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_IDEA,
		...
		{
			$select = array_intersect($arSelectFields, array_keys(\Thurly\Idea\NotifyEmailTable::getMap()));
			...
			if (in_array("ID", $arSelectFields))
			{
				...
				$runtime[] = new ExpressionField(
					...
					Application::getConnection()->getSqlHelper()->getConcatFunction(
						"CASE ".
							"WHEN %s='".\Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_IDEA."' AND %s='' THEN '".self::SUBSCRIBE_ALL."' ".
							"WHEN %s='".\Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_IDEA."' THEN '".self::SUBSCRIBE_IDEA_COMMENT."' ".
							"WHEN %s='".\Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_CATEGORY."' AND %s='' THEN '".self::SUBSCRIBE_ALL_IDEA."' ".
	...
	public function Send()
	{
		...
		if ($arNotification["TYPE"] == "IDEA") // (COMMENT, IDEA)
		{
			$filter = array(
				"LOGIC" => "OR",
				array(
					"=ENTITY_TYPE" => \Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_CATEGORY,
					"=ENTITY_CODE" => '',
				)
			);
			if (!is_null($category))
		...
		else
		{
			$filter = array(
				...
				array(
					"=SUBSCRIBE_TYPE" => \Thurly\Idea\NotifyEmailTable::SUBSCRIBE_TYPE_ALL,
					"=ENTITY_TYPE" => \Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_CATEGORY,
					"=ENTITY_CODE" => ''
				)
			);
