					...
					<div class="idea-post-meta">
						<div class="idea-post-meta-util">
							<?if(strLen($arResult["urlToHide"])>0):?>
								<a href="javascript:void(0)" onclick="if(confirm('<?=GetMessage("BLOG_MES_HIDE_POST_CONFIRM")?>')) window.location='<?=$arResult["urlToHide"]."&".thurly_sessid_get()?>'"><span><?=GetMessage("BLOG_MES_HIDE")?></span></a>
							<?elseif(strLen($arResult["urlToShow"])>0):?>
								<a href="javascript:void(0)" onclick="if(confirm('<?=GetMessage("IDEA_MES_SHOW_POST_CONFIRM")?>')) window.location='<?=$arResult["urlToShow"]."&".thurly_sessid_get()?>'"><span><?=GetMessage("IDEA_MES_SHOW")?></span></a>
							<?endif;?>
							...
							<?if(strLen($arResult["urlToDelete"])>0):?>
								<a href="javascript:void(0)" onclick="if(confirm('<?=GetMessage("BLOG_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$arResult["urlToDelete"]."&".thurly_sessid_get()?>'"><span><?=GetMessage("BLOG_BLOG_BLOG_DELETE")?></span></a>
							<?endif;?>
						</div>
