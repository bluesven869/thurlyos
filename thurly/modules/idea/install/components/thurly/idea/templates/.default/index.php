 ...
 * @var array $arParams
 * @var array $arResult
 * @param CThurlyComponentTemplate $this
 */
...
if($USER->IsAuthorized())
{
	...
	if (array_key_exists("action", $_REQUEST) && $_REQUEST["action"] == "subscribe" && check_thurly_sessid())
	{
	...
	else
	{
		...
		if ($subscribes !== false && empty($subscribes))
		{
			...
			array_unshift($arResult["ACTIONS"]["MENU"]["MENU"], array(
				...
				"ONCLICK" => "top.window.location.href='".CUtil::JSEscape($APPLICATION->GetCurPageParam("action=subscribe&".thurly_sessid_get(), array("action", "sessid")))."';",
			));
		}
	}
}
