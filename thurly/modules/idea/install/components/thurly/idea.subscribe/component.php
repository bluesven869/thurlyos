...
$arParams['ACTION'] = (array_key_exists("ACTION", $_REQUEST) && check_thurly_sessid() ? $_REQUEST["ACTION"] : (array_key_exists('ACTION', $arParams) ? $arParams["ACTION"] : false));
//ACTION PROCESSING
if($arParams['ACTION'])
{
	switch ($arParams['ACTION'])
	{
		...
		case "DELETE":
			...
			{
				$notifyEmail = new \Thurly\Idea\NotifyEmail();
				if ($_REQUEST["ENTITY_TYPE"] == \Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_CATEGORY)
...
if($arResult["USER_ID"]>0)
{
	...
	$db_res = \Thurly\Idea\NotifyEmailTable::getList(
		array(
			...
			'runtime' => array(new \Thurly\Main\Entity\ExpressionField(
						'RUNTIME_ID',
							\Thurly\Main\Application::getConnection()->getSqlHelper()->getConcatFunction(
							"CASE ".
								"WHEN %s='".\Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_IDEA."' AND %s='' THEN '".CIdeaManagmentEmailNotify::SUBSCRIBE_ALL."' ".
								"WHEN %s='".\Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_IDEA."' THEN '".CIdeaManagmentEmailNotify::SUBSCRIBE_IDEA_COMMENT."' ".
								"WHEN %s='".\Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_CATEGORY."' AND %s='' THEN '".CIdeaManagmentEmailNotify::SUBSCRIBE_ALL_IDEA."' ".
								"WHEN %s='".\Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_CATEGORY."' THEN '".\Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_CATEGORY."' ".
								"ELSE 'UNK' END",
							"%s"),
	...
	while($r = $oIdeaSubscribe->GetNext())
	{
		...
		else if ($r["ENTITY_TYPE"] == \Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_CATEGORY)
		...
		else if ($r["ENTITY_TYPE"] == \Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_IDEA)
		{
			$arResult["IDEA"][$r["ID"]] = $r + array(
	...
	foreach($arResult["IDEA"] as $res)
	{
		...
		if ($res["ENTITY_TYPE"] == \Thurly\Idea\NotifyEmailTable::ENTITY_TYPE_CATEGORY)
		{
			$data = array(
				"NAME" => $res["TITLE"],
				"STATUS" => ($res["SUBSCRIBE_TYPE"] == \Thurly\Idea\NotifyEmailTable::SUBSCRIBE_TYPE_NEW_IDEAS ? GetMessage("IDEA_SUBSCRIBE_NEW_IDEAS") : ""),
			);
		}
		else
		{
				...
				array(
					...
					"ONCLICK" => "window.location.href='".
						CUtil::JSEscape($APPLICATION->GetCurPageParam(
							"ACTION=DELETE&ENTITY_TYPE=".$res["ENTITY_TYPE"]."&ENTITY_CODE=".$res["ENTITY_CODE"]."&".thurly_sessid_get(),
