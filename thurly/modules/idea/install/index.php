...
Class idea extends CModule
{
	...
	function InstallDB()
	{
		...
		if(!$DB->Query("SELECT 'x' FROM b_idea_email_subscribe", true))
			$this->errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT']."/thurly/modules/".$this->MODULE_ID."/install/db/".ToLower($DBType)."/install.sql");
	...
	function InstallUserFields()
	{
		...
		$messages = array();
		$db_res = \Thurly\Main\Localization\LanguageTable::getList(array('order'=>array('SORT'=>'ASC')));
	...
	function UnInstallDB($arParams = array())
	{
		...
		if(array_key_exists("savedata", $arParams) && $arParams["savedata"] != "Y")
		{
			...
			$this->errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT']."/thurly/modules/".$this->MODULE_ID."/install/db/".ToLower($DBType)."/uninstall.sql");
		}
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", true, true);
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		{
			//Installation error
			$APPLICATION->IncludeAdminFile(GetMessage("ERR_IDEA_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/step2.php");
		}
		else
		{
			if($step<2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("IDEA_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/step1.php");
			}
			elseif($step == 2)
			{
				...
				$APPLICATION->IncludeAdminFile(GetMessage("IDEA_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/".$this->MODULE_ID."/install/step2.php");
			}
		}
	}
	...
	function DoUninstall()
	{
		...
		if($step < 2)
		{
			$GLOBALS["APPLICATION"]->IncludeAdminFile(GetMessage("FORUM_DELETE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/idea/install/unstep1.php");
		}
		else
		{
			...
			$GLOBALS["APPLICATION"]->IncludeAdminFile(GetMessage("FORUM_DELETE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/idea/install/unstep2.php");
		}
	}
}