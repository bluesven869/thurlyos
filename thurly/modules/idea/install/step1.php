<?if(!check_thurly_sessid() || !CModule::IncludeModule("iblock") || !CModule::IncludeModule("blog"))
	return;

IncludeModuleLangFile(__FILE__);

//Include Idea API
include_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/idea/include.php");
...
Class CIdeaManagmentInstall
{
	...
	private function CopyPublucFiles()
	{
			...
			$source = $_SERVER['DOCUMENT_ROOT']."/thurly/modules/idea/install/public/idea/";
...
if(strlen($_REQUEST["idea_install"]) > 0)
{
	if($_REQUEST["demo"])
	{
		...
		if($CIdeaInstaller->HaveError())
			...
			LocalRedirect('/thurly/admin/module_admin.php?step=2&lang='.LANGUAGE_ID.'&id=idea&install=Y&'.thurly_sessid_get());
	}
	else
		LocalRedirect('/thurly/admin/module_admin.php?step=2&lang='.LANGUAGE_ID.'&id=idea&install=Y&'.thurly_sessid_get());
}
