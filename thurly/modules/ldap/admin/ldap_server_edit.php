...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/ldap/include.php");
...
if($_SERVER['REQUEST_METHOD'] == "POST" && (strlen($save)>0 || strlen($apply)>0) && $MOD_RIGHT=="W" && check_thurly_sessid())
{
	$bPostback = true;
	$arGroups = false;
...
if($ID>0)
{
	...
	if ($MOD_RIGHT=="W")
	{
		$aMenu[] = array(
			...
			"LINK"=>"javascript:if(confirm('".GetMessage("LDAP_EDIT_DEL_CONF")."'))window.location='ldap_server_admin.php?action=delete&ID=".$ID."&lang=".LANG."&".thurly_sessid_get()."';",
		);
	}
