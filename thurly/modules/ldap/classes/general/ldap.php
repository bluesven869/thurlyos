...
class CLDAP
{
	...
	/**
	 * Returns value of ldap user field mapped to thurly field.
	 * @param string $fieldName Name of user field in Thurly system.
	 * @param array $arLdapUser User params received from ldap.
	 * @return mixed.
	 */
	function getLdapValueByThurlyFieldName($fieldName, $arLdapUser)
	{
	...
	function GetUserFields($arLdapUser, &$departmentCache=FALSE)
	{
		...
		foreach($this->arFields["FIELD_MAP"] as $userField=>$attr)
			$arFields[$userField] = $this->getLdapValueByThurlyFieldName($userField, $arLdapUser);
	...
	function SetUser($arLdapUser, $bAddNew = true)
	{
		...
		else
		{
			...
			if($thurlyUserId <= 0 && $ldapUserID <= 0)
			{
				if($bAddNew)
				{
			...
			{
				$ID = ($ldapUserID > 1 ? $ldapUserID : $thurlyUserId);
				self::PrepareUserPhoto($ID,$arLdapUser);
		...
		if ($ID > 0)
		{
			...
			if(!empty($arGroupMaps))
			{
				...
				$arGroupMaps = $this->GetGroupMaps();
				$arUserThurlyGroups = $USER->GetUserGroup($ID);
				$arUserThurlyGroupsNew = array();

				$prevGroups = $arUserThurlyGroups;
				sort($prevGroups);
				...
				foreach($arGroupMaps as $fromLdapGroup=>$arToUserGroups)
				{
					...
					{
						if (($k = array_search($toUserGroup, $arUserThurlyGroups)) !== false)
						{
							unset($arUserThurlyGroups[$k]);
						}

						// If there is such a group among user's
						if (in_array($fromLdapGroup, $arUserLdapGroups))
						{
							$arUserThurlyGroupsNew[] = $toUserGroup;
						}
					}
				}
				$arUserThurlyGroups = array_merge($arUserThurlyGroups, array_unique($arUserThurlyGroupsNew));
				sort($arUserThurlyGroups);

				if($arUserThurlyGroups <> $prevGroups)
				{
					$USER->SetUserGroup($ID, $arUserThurlyGroups);
				}
			}
		}
