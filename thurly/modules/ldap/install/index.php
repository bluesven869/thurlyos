...
Class ldap extends CModule
{
	...
	function InstallDB($arParams = array())
	{
		...
		if ($this->CheckLDAP())
		{
			...
			{
				$errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT']."/thurly/modules/ldap/install/db/".$DBType."/install.sql");
			}
	...
	function UnInstallDB($arParams = array())
	{
		...
		{
			$errors = $DB->RunSQLBatch($_SERVER['DOCUMENT_ROOT']."/thurly/modules/ldap/install/db/".$DBType."/uninstall.sql");
			if (!is_array($errors))
	...
	function InstallEvents()
	{
		...
		{
			$lid = $arLang["LID"];
			IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/thurly/modules/ldap/install/events.php", $lid);
	...
	function InstallFiles($arParams = array())
	{
		...
		if ($this->CheckLDAP())
		{
			...
			{
				CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/ldap/install/images", $_SERVER['DOCUMENT_ROOT']."/thurly/images/ldap");
				CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/ldap/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
				CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/ldap/install/themes", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes", true, true);
			}
		}
	...
	{
		DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/ldap/install/admin/", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
		
		DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/ldap/install/themes/.default/", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes/.default");//css
		DeleteDirFilesEx("/thurly/themes/.default/icons/ldap/");//icons
		DeleteDirFilesEx("/thurly/images/ldap/");//images
	...
	function DoInstall()
	{
		...
		$APPLICATION->IncludeAdminFile(GetMessage("LDAP_INSTALL_TITLE"), $DOCUMENT_ROOT."/thurly/modules/ldap/install/step1.php");
	}
	...
	function DoUninstall()
	{
		...
		$step = IntVal($step);
		if($step<2)
			$APPLICATION->IncludeAdminFile(GetMessage("LDAP_UNINSTALL_TITLE"), $DOCUMENT_ROOT."/thurly/modules/ldap/install/unstep1.php");
		elseif($step==2)
		{
			$APPLICATION->ResetException();
			...
			$APPLICATION->IncludeAdminFile(GetMessage("LDAP_UNINSTALL_TITLE"), $DOCUMENT_ROOT."/thurly/modules/ldap/install/unstep2.php");
		}
	}
}
?>