...
##############################################
# Thurly: SiteManager                        #
# Copyright (c) 2002-2012 Thurly             #
# http://www.thurlysoft.com                  #
# mailto:admin@thurlysoft.com                #
##############################################

global $MESS;
include(GetLangFileName($GLOBALS["DOCUMENT_ROOT"]."/thurly/modules/ldap/lang/", "/options.php"));
...
$arAllOptions =	Array(
		...
		GetMessage("LDAP_THURLYVM_BLOCK"),
		Array("thurlyvm_auth_support", GetMessage("LDAP_THURLYVM_SUPPORT"), "N", Array("checkbox")),
		Array("thurlyvm_auth_net", GetMessage('LDAP_THURLYVM_NET'), "", Array("text", 40)),
	);
...
if($MOD_RIGHT>="W"):
	...
	{
		if($_POST['thurlyvm_auth_net'] && !preg_match("#(\d{1,3}\.){3,3}(\d{1,3})/(\d{1,3}\.){3,3}(\d{1,3})#",$_POST['thurlyvm_auth_net']) && !preg_match("#(\d{1,3}\.){3,3}(\d{1,3})/(\d{1,3})#",$_POST['thurlyvm_auth_net']))
		...
		if ($_POST['thurlyvm_auth_support'] == 'Y')
			CLdapUtil::SetThurlyVMAuthSupport();
		else
			CLdapUtil::UnSetThurlyVMAuthSupport();
	}

endif; //if($MOD_RIGHT>="W"):

$arAllOptions[] = Array("thurlyvm_auth_hint", "", BeginNote().GetMessage("LDAP_THURLYVM_HINT").EndNote(), Array("statichtml", ""));
