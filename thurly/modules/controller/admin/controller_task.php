...
while ($arRes = $rsData->Fetch())
{
	...
	if ($USER->CanDoOperation("controller_log_view"))
	{
		$arActions[] = array(
			...
			"ACTION" => $lAdmin->ActionRedirect("/thurly/admin/controller_log_admin.php?lang=".urlencode(LANGUAGE_ID)."&set_filter=Y&find_task_id=".urlencode($arRes["ID"])),
		);
	}
...
<script>
	function Start(cnt, executed)
	{
		ShowWaitWindow();
		BX.ajax.post(
			'controller_task.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&act=process&cnt=' + cnt + '&executed=' + executed,
			null,
			function (result)
			{
