...
$US_LICENSE_KEY = "";
if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/license_key.php"))
	include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/license_key.php");
...
define("US_SHARED_KERNEL_PATH", COption::GetOptionString("controller", "shared_kernel_path", "/thurly/clients"));
...
$arMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/controller_update.php?refresh=Y&lang=".LANGUAGE_ID,
		"ICON"=>"btn_update",
	),
	array("SEPARATOR" => "Y"),
	array(
		"TEXT" => GetMessage("SUP_SETTINGS"),
		"LINK" => "/thurly/admin/settings.php?lang=".LANGUAGE_ID."&mid=controller&back_url_settings=%2Fthurly%2Fadmin%2Fcontroller_update.php%3Flang%3D".LANGUAGE_ID."",
	),
	/*
	array("SEPARATOR" => "Y"),
	array(
			...
			if ($arUpdateList)
			{
				...
				if ($newLicenceSigned != "Y")
				{
					...
					function ShowLicence()
					{
						...
						txt += '<iframe name="license_text" src="http://www.thurlysoft.ru/license-<?= ((LANGUAGE_ID == "ru") ? "ru" : "en") ?>.htm" style="width:450px; height:250px; display:block;"></iframe>';
						txt += '</td></tr><tr><td>';
					...
					function AgreeLicence()
					{
						...
						CHttpRequest.Send('/thurly/admin/controller_update_act.php?query_type=licence&<?= thurly_sessid_get() ?>');
					}
				...
				if (strlen($strLicenseKeyTmp) <= 0 || strtolower($strLicenseKeyTmp) == "demo" || $bLicenseNotFound)
				{
					...
					function LicenceKeyFormSubmit()
					{
						...
						CHttpRequest.Send('/thurly/admin/controller_update_act.php?query_type=key&<?= thurly_sessid_get() ?>&NEW_LICENSE_KEY=' + escape(document.licence_key_form.NEW_LICENSE_KEY.value));
					}
					//-->
					</SCRIPT>
					<?
				...
				else
				{
					if (isset($arUpdateList["CLIENT"]) && !isset($arUpdateList["UPDATE_SYSTEM"]) && count($arUpdateList["CLIENT"]) > 0 && $arUpdateList["CLIENT"][0]["@"]["RESERVED"] == "Y")
					{
						...
						function ActivateFormSubmit()
						{
							...
							CHttpRequest.Send('/thurly/admin/controller_update_act.php?query_type=activate&<?= thurly_sessid_get() ?>&' + param);
						}
					...
					else
					{
						if ($arUpdateList !== false && isset($arUpdateList["UPDATE_SYSTEM"]))
						{
							...
							function UpdateUpdate()
							{
								...
								CHttpRequest.Send('/thurly/admin/controller_update_act.php?query_type=updateupdate&<?= thurly_sessid_get() ?>');
							}
							//-->
							</SCRIPT>
							<?
					...
					&& ($arUpdateList["CLIENT"][0]["@"]["ENC_TYPE"] == "F" || $arUpdateList["CLIENT"][0]["@"]["ENC_TYPE"] == "E"))
				{
					...
					function RegisterSystem()
					{
						...
						CHttpRequest.Send('/thurly/admin/controller_update_act.php?query_type=register&<?= thurly_sessid_get() ?>');
					}
					//-->
					</SCRIPT>
					<?
				...
				if ($arUpdateList !== false && defined("ENCODE") && ENCODE=="Y" && isset($arUpdateList["CLIENT"]) && !isset($arUpdateList["UPDATE_SYSTEM"]) && ($arUpdateList["CLIENT"][0]["@"]["ENC_TYPE"] == "F"))
				{
					...
					function __LoadSources()
					{
						...
						if (requestedModules.length > 0)
							CHttpRequest.Send('/thurly/admin/controller_update_act.php?query_type=sources&<?= thurly_sessid_get() ?>&requested_modules=' + requestedModules);
						else
				...
				function __InstallUpdates()
				{
					...
					CHttpRequest.Send('/thurly/admin/controller_update_call.php?' + aStrParams + "&<?= thurly_sessid_get() ?>&query_type=" + param);
				}
	...
	<tr>
		<td colspan="2">
			...
			function ActivateCoupon()
			{
				...
				if (param.length > 0)
				{
					CHttpRequest.Send('/thurly/admin/controller_update_act.php?query_type=coupon&<?= thurly_sessid_get() ?>&COUPON=' + escape(param));
				}
				else
				{
					document.getElementById("id_coupon_btn").disabled = false;
			...
			function SwithStability()
			{
				...
				CHttpRequest.Send('/thurly/admin/controller_update_act.php?query_type=stability&<?= thurly_sessid_get() ?>&STABILITY=' + escape("<?= $stableVersionsOnly ?>"));
			}
			//-->
			</SCRIPT>
			...
			function SubscribeMail()
			{
				...
				if (param.length > 0)
				{
					CHttpRequest.Send('/thurly/admin/controller_update_act.php?query_type=mail&<?= thurly_sessid_get() ?>&EMAIL=' + escape(param));
				}
				else
				{
					CloseWaitWindow();
