...
if ($ID > 0 && $USER->CanDoOperation("controller_group_manage"))
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_delete",
		"LINK" => "javascript:if(confirm('".GetMessage("CTRLR_GR_ED_LINK_DEL_CONFIRM")."'))window.location='controller_group_admin.php?action=delete&ID=".$ID."&lang=".LANG."&".thurly_sessid_get()."';",
	);
}
...
<form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?lang=<?=LANG?>&ID=<?=$ID?>" name="form1">
	...
	)
	{
		...
		{
			echo "<tr "."colspan=\"2\"><td align=\"left\"><a href=\"/thurly/admin/userfield_edit.php?lang=".LANGUAGE_ID."&ENTITY_ID=".urlencode($ENTITY_ID)."&back_url=".urlencode($APPLICATION->GetCurPageParam("", array("bxpublic"))."&tabControl_active_tab=user_fields_tab")."\">".GetMessage("USER_TYPE_EDIT_TAB_HREF")."</a></td></tr>";
		}
