...
if ($ID > 0)
{
	...
	$grantList = \Thurly\Controller\AuthGrantTable::getActiveForControllerMember($ID, $USER->GetID(), $USER->GetUserGroupArray());
	while ($currentUserAuthGrant = $grantList->fetch())
	{
	...
	else
	{
		...
		if ($ID > 0)
		{
			...
			if (
				...
				&& array_key_exists($_REQUEST['AUTH']['GRANTEE_USER_ID'], \Thurly\Controller\AuthGrantTable::getGranteeUserList($USER->GetID()))
			)
			{
				\Thurly\Controller\AuthGrantTable::add(array(
			...
			)
			{
				...
				{
					\Thurly\Controller\AuthGrantTable::update($id, array("ACTIVE" => "N"));
				}
			}
		}
...
if (strlen($_REQUEST['MEMBER_ID']) > 0)
	...
	$str_MEMBER_ID = "m".\Thurly\Main\Security\Random::getString(31);
...
if ($ID > 0)
{
	...
	if ($str_DISCONNECTED != "Y")
	{
		if ($USER->CanDoOperation("controller_member_disconnect"))
		{
			$aMenu[] = array(
				...
				"LINK" => "javascript:if(confirm('".GetMessage("CTRLR_MEM_EDIT_TOOLBAR_DISCN_CONFIRM")."'))window.location='controller_member_edit.php?unregister=Y&ID=".$ID."&lang=".LANG."&".thurly_sessid_get()."';",
			);
		}
	}
	...
	if ($USER->CanDoOperation("controller_member_delete"))
	{
		$aMenu[] = array(
			...
			"ICON" => "btn_delete",
			"LINK" => "javascript:if(confirm('".GetMessage("CTRLR_MEM_EDIT_TOOLBAR_DELETE_CONFIRM")."'))window.location='controller_member_admin.php?action=delete&ID=".$ID."&lang=".LANG."&".thurly_sessid_get()."';",
		);
	}
}
...
if ($bUnregisterError):
	?>
	<input type="button" value="<? echo GetMessage("CTRLR_MEM_EDIT_MARK_DISCN") ?>" onclick="window.location='controller_member_edit.php?unregister=Y&anywhere=Y&ID=<?=$ID?>&lang=<?=LANG?>&<?=thurly_sessid_get()?>'">
	<?
endif;
...
<form method="POST" action="<? echo $APPLICATION->GetCurPage() ?>?lang=<?=LANG?><? echo $ID > 0? '&amp;ID='.$ID: '' ?><? echo $_REQUEST["reconnect_id"] > 0? '&amp;reconnect_id='.intval($_REQUEST["reconnect_id"]): '' ?>" name="form1" id="form1">
	...
	<? if ($ID > 0): ?>
		...
		<script>
			function UpdateCounters()
			{
				BX.showWait();
				BX.ajax.loadJSON(
					'/thurly/admin/controller_member_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$ID?>',
					{
						countersupdate: 'Y',
	...
	)
	{
		...
		{
			echo "<tr><td colspan=\"2\" align=\"left\"><a href=\"/thurly/admin/userfield_edit.php?lang=".LANG."&ENTITY_ID=".urlencode($ENTITY_ID)."&back_url=".urlencode($APPLICATION->GetCurPageParam("", array("bxpublic"))."&tabControl_active_tab=user_fields_tab")."\">".GetMessage("USER_TYPE_EDIT_TAB_HREF")."</a></td></tr>";
		}
		...
		if ($ID > 0 && $USER->CanDoOperation("controller_member_grant_auth"))
		{
			...
			<tr>
				...
				<td><select name="AUTH[GRANTEE_USER_ID]">
						...
						<?
						foreach (\Thurly\Controller\AuthGrantTable::getGranteeUserList($USER->GetID()) as $userId => $userTitle):
							?>
			...
			<?
			$grantList = \Thurly\Controller\AuthGrantTable::getList(array(
				"select" => array(
					"ID",
					"TIMESTAMP_X",
					"GRANTED_BY",
