...
if(strlen($_REQUEST['domain_name'])>0)
{
	...
	if(!count($arError) && ($arParams["REGISTER_IMMEDIATE"] != "N"))
	{
		$ar_member = array(
			...
			'SECRET_ID' => "m".\Thurly\Main\Security\Random::getString(31),
	...
	$path_kernel = $_SERVER['DOCUMENT_ROOT'].COption::GetOptionString("controller", "shared_kernel_path", "/thurly/clients");
	if(!count($arError))
	{
		//create kernel link
		$result = @symlink($path_kernel, $path_to.'thurly');
	}

	if(!count($arError))
	{
		// adding dbconn.php
		$filename = str_replace("//", "/", $path_to."thurly_personal/php_interface/dbconn.php");
		$str = '<?
