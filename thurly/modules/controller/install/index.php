...
Class controller extends CModule
{
	...
	function InstallDB()
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/db/".strtolower($DB->type)."/install.sql");
		}
	...
	function UnInstallDB($arParams = array())
	{
		...
		if (!array_key_exists("savedata", $arParams) || ($arParams["savedata"] != "Y"))
		{
			if ($DB->Query("SELECT 'x' FROM b_controller_member", true))
				$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/db/".strtolower($DB->type)."/uninstall.sql");
		}
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin", false);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/images", $_SERVER["DOCUMENT_ROOT"]."/thurly/images/controller", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/themes", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", True, True);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/activities", $_SERVER["DOCUMENT_ROOT"]."/thurly/activities", True, True);
			if (IsModuleInstalled('bizproc'))
			{
				CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/bizproc/templates", $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/bizproc/templates", true, true);
				$b = "";
				$o = "";
				$langs = CLanguage::GetList($b, $o);
				while ($lang = $langs->Fetch())
				{
					CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/lang/".$lang["LID"]."/install/bizproc/templates", $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/bizproc/lang/".$lang["LID"]."/templates", true, true);
				}
			}
		}
		return true;
	...
	function UnInstallFiles()
	{
		if ($_ENV["COMPUTERNAME"] != 'BX')
		{
			DeleteDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/controller/install/admin/', $_SERVER['DOCUMENT_ROOT'].'/thurly/admin');
			DeleteDirFiles($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/controller/install/themes/.default/', $_SERVER['DOCUMENT_ROOT'].'/thurly/themes/.default');//css
			DeleteDirFilesEx('/thurly/themes/.default/icons/controller/');//icons
			DeleteDirFilesEx('/thurly/images/controller/');//images
		}
		return true;
	}
	...
	function InstallEvents()
	{
		...
		if ($ar["C"] <= 0)
		{
			include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/events/set_events.php");
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if (!CBXFeatures::IsFeatureEditable("Controller"))
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("CTRL_INST_STEP1"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/step.php");
		}
		else
		{
			$this->InstallDB();
	...
	function DoUninstall()
	{
		...
		if ($RIGHT >= "W")
		{
			...
			if ($step < 2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("CTRL_INST_STEP1_UN"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/unstep1.php");
			}
			elseif ($step == 2)
			{
				...
				$APPLICATION->IncludeAdminFile(GetMessage("CTRL_INST_STEP1_UN"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/controller/install/unstep.php");
			}
		}
	}
