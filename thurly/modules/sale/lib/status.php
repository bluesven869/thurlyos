...
abstract class StatusBase
{
	...
	public static function canGroupDoOperations($groupId, $fromStatus, array $operations)
	{
		...
		$result = StatusTable::getList(array(
			'select' => array(
				'NAME' => 'Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.TASK.Thurly\Main\TaskOperation:TASK.OPERATION.NAME',
			),
			'filter' => array(
				'=ID' => $fromStatus,
				'=Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.GROUP_ID' => $groupId,
				'=Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.TASK.Thurly\Main\TaskOperation:TASK.OPERATION.NAME' => $operations,
			),
		));
		...
		if (in_array('1', $groupId, true) || \CMain::GetUserRight('sale', $groupId) >= 'W') // Admin
		{
			if (!array_key_exists($cacheKey, static::$cacheAllowStatuses))
			{
				$result = StatusTable::getList(array(
					...
					'filter' => array('=TYPE' => static::TYPE, '=Thurly\Sale\Internals\StatusLangTable:STATUS.LID' => LANGUAGE_ID),
					'order'  => array('SORT'),
				));
			...
			if (!array_key_exists($cacheKey, static::$cacheAllowStatuses))
			{
				if (StatusTable::getList(array( // check if group can change from status
											 ...
											 'filter' => array(
												 ...
												 '=TYPE' => static::TYPE,
												 '=Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.GROUP_ID' => $groupId,
												 '=Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.TASK.Thurly\Main\TaskOperation:TASK.OPERATION.NAME' => 'sale_status_from',
											 ),
											 'limit' => 1,
										 ))->fetch())
				{
					$result = StatusTable::getList(array(
													   ...
													   'filter' => array(
														   ...
														   '=Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.GROUP_ID' => $groupId,
														   '=Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.TASK.Thurly\Main\TaskOperation:TASK.OPERATION.NAME' => 'sale_status_to',
													   ),
													   'order' => array('SORT'),
												   ));
	...
	public static function getAllStatusesNames($lang = null)
	{
		...
		if ($lang !== null)
		{
			$filter['filter']['=Thurly\Sale\Internals\StatusLangTable:STATUS.LID'] = $lang;
		}
		elseif (defined("LANGUAGE_ID"))
		{
			$filter['filter']['=Thurly\Sale\Internals\StatusLangTable:STATUS.LID'] = LANGUAGE_ID;
		}
	...
	protected static function getStatusesByGroupId($groupId, array $operations = array())
	{
		...
		$filter = array(
			'select' => array(
				'ID',
				'OPERATION' => 'Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.TASK.Thurly\Main\TaskOperation:TASK.OPERATION.NAME',
			),
			'filter' => array(
				'=TYPE' => static::TYPE,
				'=Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.GROUP_ID' => $groupId,
		...
		if (!empty($operations))
		{
			$filter['filter']['=Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.TASK.Thurly\Main\TaskOperation:TASK.OPERATION.NAME'] = $operations;
		}
