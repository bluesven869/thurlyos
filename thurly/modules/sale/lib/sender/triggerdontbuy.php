...
class TriggerDontBuy extends \Thurly\Sender\TriggerConnectorClosed
{
	...
	public function filter()
	{
		...
		{
			$filter = array(
				'<MAX_DATE_INSERT' => $dateTo->format(\Thurly\Main\UserFieldTable::MULTIPLE_DATETIME_FORMAT),
			);
		}
		else
		{
			$filter = array(
				'>MAX_DATE_INSERT' => $dateFrom->format(\Thurly\Main\UserFieldTable::MULTIPLE_DATETIME_FORMAT),
				'<MAX_DATE_INSERT' => $dateTo->format(\Thurly\Main\UserFieldTable::MULTIPLE_DATETIME_FORMAT),
			);
		}
		$filter = $filter + array(
		...
		$userListDb = \Thurly\Sale\Internals\OrderTable::getList(array(
			'select' => array('BUYER_USER_ID' => 'USER.ID', 'EMAIL' => 'USER.EMAIL', 'BUYER_USER_NAME' => 'USER.NAME'),
			'filter' => $filter,
			'runtime' => array(
				new \Thurly\Main\Entity\ExpressionField('MAX_DATE_INSERT', 'MAX(%s)', 'DATE_INSERT'),
			),
