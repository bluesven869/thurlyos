...
class TriggerBasketForgotten extends \Thurly\Sender\TriggerConnectorClosed
{
	...
	public function filter()
	{
		...
		$dateTo = new \Thurly\Main\Type\DateTime;
		...
		{
			$filter = array(
				'<MIN_DATE_INSERT' => $dateTo->format(\Thurly\Main\UserFieldTable::MULTIPLE_DATETIME_FORMAT),
			);
		}
		else
		{
			$filter = array(
				'>MIN_DATE_INSERT' => $dateFrom->format(\Thurly\Main\UserFieldTable::MULTIPLE_DATETIME_FORMAT),
				'<MIN_DATE_INSERT' => $dateTo->format(\Thurly\Main\UserFieldTable::MULTIPLE_DATETIME_FORMAT),
			);
		}
		$filter = $filter + array(
		...
		$userListDb = \Thurly\Sale\Internals\BasketTable::getList(array(
			'select' => array('USER_ID' => 'FUSER.USER_ID', 'EMAIL' => 'FUSER.USER.EMAIL', 'FUSER_USER_NAME' => 'FUSER.USER.NAME'),
			'filter' => $filter,
			'runtime' => array(
				new \Thurly\Main\Entity\ExpressionField('MIN_DATE_INSERT', 'MIN(%s)', 'DATE_INSERT'),
			),
