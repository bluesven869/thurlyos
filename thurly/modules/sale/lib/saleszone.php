...
class SalesZone
{
	const CONN_ENTITY_NAME = 		'Thurly\Sale\Location\SiteLocation';
	const LOCATION_ENTITY_NAME = 	'Thurly\Sale\Location\Location';
	...
	private static function checkLocationIsInLinkedPart($locationId, $siteId)
	{
		...
		$node = \Thurly\Sale\Location\LocationTable::getList(array(
	...
	public static function getSelectedIds($siteId)
	{
		...
		if((string) $siteId != '' && \Thurly\Sale\Location\SiteLocationTable::checkLinkUsageAny($siteId) && !empty($types))
		{
			$result = array();

			$sql = \Thurly\Sale\Location\SiteLocationTable::getConnectedLocationsSql(
				$siteId,
	...
	/**
	 * Returns filter for using in queries such as in thurly/modules/sale/install/components/thurly/sale.ajax.locations/search.php
