...
namespace Thurly\Sale\TradingPlatform\Ebay;

use Thurly\Main\Localization\Loc;
use Thurly\Main\SystemException;
use Thurly\Sale\TradingPlatform\MapTable;
use Thurly\Sale\TradingPlatform\TabHandler;
use Thurly\Sale\TradingPlatform\Ebay\MapHelper;
 ...
 * @package Thurly\Sale\TradingPlatform\Ebay
 */
class CatalogSectionTabHandler extends TabHandler
{
	...
	public function Action($arArgs)
	{
		...
		if(
			...
			&& isset($_POST["SALE"]["EBAY"]["THURLY_CATEGORY_PROPS"])
			&& is_array($_POST["SALE"]["EBAY"]["THURLY_CATEGORY_PROPS"])
		)
		{
			$propsRes = self::saveCategoryPropsMap($arArgs["IBLOCK"]["ID"], $ebayCategoryId, $_POST["SALE"]["EBAY"]["EBAY_CATEGORY_VARIATIONS"], $_POST["SALE"]["EBAY"]["THURLY_CATEGORY_PROPS"]);
		}

		return $catRes && $propsRes;
	}

	protected function saveCategoryPropsMap($iblockId, $ebayCategoryId, array $ebayCatVar, array $thurlyCatProps)
	{
		$result = true;
		...
		{
			if(!isset($thurlyCatProps[$key]) || strlen($ebayCategoryVariation) <=0 )
				continue;

			$fields = array(
				"ENTITY_ID" => $mapEntityId,
				"VALUE_EXTERNAL" => $ebayCategoryVariation,
				"VALUE_INTERNAL" => $thurlyCatProps[$key]
			);
	...
	protected function saveCategoryMap($thurlyCategoryId, $iblockId, array $ebayCategoriesIds, $params = array())
	{
		$result = true;
		...
		foreach($ebayCategoriesIds as $ebayCategoryId)
		{
			...
			$fields = array(
				...
				"VALUE_INTERNAL" => $thurlyCategoryId
			);
	...
	public function Check($arArgs)
	{
		...
		while($var = $categoriesVarResult->fetch())
		{
			if($var['REQUIRED'] == 'Y')
			{
				...
				||!isset($_POST["SALE"]["EBAY"]["THURLY_CATEGORY_PROPS"][$variationIdx])
				|| strlen($_POST["SALE"]["EBAY"]["THURLY_CATEGORY_PROPS"][$variationIdx]) <=0 )
				{
					$result = false;
					break;
				}
	...
	public function ShowTabSection($divName, $arArgs, $bVarsFromForm)
	{
		...
		$catMapRes = \Thurly\Sale\TradingPlatform\MapTable::getList(array(
			"filter" => array(
				"ENTITY_ID" => \Thurly\Sale\TradingPlatform\Ebay\MapHelper::getCategoryEntityId($arArgs["IBLOCK"]["ID"]),
		...
		$params = array(
			...
			"THURLY_CATEGORY_PROPS_SN" => "SALE[EBAY][THURLY_CATEGORY_PROPS]",
			"THURLY_CATEGORY_ID"=> $arArgs["ID"],
		...
		if(isset($_POST["SALE"]["EBAY"]["EBAY_CATEGORY_VARIATIONS"]) && isset($_POST["SALE"]["EBAY"]["THURLY_CATEGORY_PROPS"]))
		{
			$params["VARIATIONS_VALUES"] = array();

			for($i=0, $l=count($_POST["SALE"]["EBAY"]["EBAY_CATEGORY_VARIATIONS"]); $i<$l; $i++)
				$params["VARIATIONS_VALUES"][$_POST["SALE"]["EBAY"]["EBAY_CATEGORY_VARIATIONS"][$i]] = $_POST["SALE"]["EBAY"]["THURLY_CATEGORY_PROPS"][$i];
		}

		ob_start();
		$APPLICATION->IncludeComponent(
			"thurly:sale.ebay.categories",
			".default",
			$params
		);
		...
		if(isset($siteSettings["API"]["AUTH_TOKEN"]) && strlen($siteSettings["API"]["AUTH_TOKEN"]) > 0)
			$policy = new \Thurly\Sale\TradingPlatform\Ebay\Policy($siteSettings["API"]["AUTH_TOKEN"], $arArgs["IBLOCK"]["LID"]);
		else
		...
		if($policy)
			$names = $policy->getPoliciesNames(\Thurly\Sale\TradingPlatform\Ebay\Policy::TYPE_RETURN);
		else
			$names = array();
		...
		if($policy)
			$names = $policy->getPoliciesNames(\Thurly\Sale\TradingPlatform\Ebay\Policy::TYPE_SHIPPING);
		else
			$names = array();
		...
		if($policy)
			$names = $policy->getPoliciesNames(\Thurly\Sale\TradingPlatform\Ebay\Policy::TYPE_PAYMENT);
		else
			$names = array();
