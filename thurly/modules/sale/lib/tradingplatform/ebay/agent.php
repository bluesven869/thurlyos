...
class Agent
{
	...
	public static function start($feedType, $siteId, $startPosition="", $once = false)
	{
		...
		{
			$result = 'Thurly\Sale\TradingPlatform\Ebay\Agent::start("'.$feedType.'","'.$siteId.'","'.$e->getEndPosition().'",, '.($once ? 'true' : 'false').');';
		}
		catch(\Exception $e)
		{
		...
		if(strlen($result) <=0 && !$once)
			$result = 'Thurly\Sale\TradingPlatform\Ebay\Agent::start("'.$feedType.'","'.$siteId.'");';
	...
	{
		return 'Thurly\Sale\TradingPlatform\Ebay\Agent::start("'.$feedType.'","'.$siteId.'", "", '.($once ? 'true' : 'false').');';
	}
