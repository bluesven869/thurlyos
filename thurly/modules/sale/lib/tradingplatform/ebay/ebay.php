...
class Ebay extends Platform
{
	...
	{
		return defined('SALE_EBAY_SERVICE_HOST') ? SALE_EBAY_SERVICE_HOST : 'www.1c-thurly.ru';
	}
	...
	public function setActive()
	{
		...
		RegisterModuleDependences("sale", "OnSaleDeductOrder", "sale", '\Thurly\Sale\TradingPlatform\Ebay\Helper', "onSaleDeductOrder", 100);
	...
	public function unsetActive()
	{
		...
		UnRegisterModuleDependences("sale", "OnSaleDeductOrder", "sale", '\Thurly\Sale\TradingPlatform\Ebay\Helper', "onSaleDeductOrder", 100);
	...
	{
		RegisterModuleDependences('main', 'OnEventLogGetAuditTypes', 'sale', '\Thurly\Sale\TradingPlatform\Ebay\Helper', 'OnEventLogGetAuditTypes');

		$tptAddRes =  \Thurly\Sale\TradingPlatformTable::add(array(
			"CODE" => $this->getCode(),
			"ACTIVE" => "N",
			"NAME" => Loc::getMessage("SALE_EBAY_NAME"),
			"DESCRIPTION" => Loc::getMessage("SALE_EBAY_DESCRIPTION"),
			"CATALOG_SECTION_TAB_CLASS_NAME" => '\Thurly\Sale\TradingPlatform\Ebay\CatalogSectionTabHandler',
			"CLASS" => '\Thurly\Sale\TradingPlatform\Ebay\Ebay'
		));

		$ebay = Ebay::getInstance();
		$catMapEntRes = \Thurly\Sale\TradingPlatform\MapEntityTable::add(array(
	...
	public static function onAfterUpdateShipment(\Thurly\Main\Event $event, array $additional)
	{
		$data = array();
		$ebay = self::getInstance();
		$settings = $ebay->getSettings();
		...
		if(!empty($data))
		{
			$ebayFeed = \Thurly\Sale\TradingPlatform\Ebay\Feed\Manager::createFeed("SHIPMENT", $additional["SITE_ID"]);
			$ebayFeed->setSourceData(array($data));
			$ebayFeed->processData();
			\Thurly\Sale\TradingPlatform\Ebay\Agent::add('SHIPMENT', $additional["SITE_ID"], 1, true);
		}
	}
