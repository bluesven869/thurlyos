...
use Thurly\Sale\TradingPlatform\Ebay\CategoryVariationTable;
...
class Product extends DataConverter
{
	...
	{
		$this->ebayCategories = $this->thurlyToEbayCategories($data["IBLOCK_ID"], $data["CATEGORIES"]);
		...
		$this->thurlyCategories = $data["CATEGORIES"];
	...
	protected function getItemDataOffers($data)
	{
		...
		if(is_array($this->variationsVector) && !empty($this->variationsVector))
		{
			...
			foreach($this->variationsVector as $ebayAttributeName => $thurlyPropId)
	...
	protected function getAttributesItem($attributesList, $data)
	{
		...
		foreach($attributesList as $ebayCategoryAttrId => $thurlyAttr)
		{
			$value = $this->getThurlyItemPropValue($thurlyAttr, $data["PROPERTIES"]);
	...
	protected function getThurlyItemPropValue($propId, array $props)
	{
		$result = false;
	...
	protected function getAttributesList($iblockId, array $ebayCategories)
	{
		...
		{
			$mapEntityId = \Thurly\Sale\TradingPlatform\Ebay\MapHelper::getCategoryVariationEntityId($iblockId, $category);
	...
	protected function thurlyToEbayCategories($iblockId, array $thurlyCategories)
	{
		$categories = $this->getEbayCategoriesParams($iblockId, $thurlyCategories);
		$result = array();
	...
	protected static function getEbayCategoriesParams($iblockId, array $thurlyCategories = array())
	{
		static $entitiesIds = array();

		if(empty($entitiesIds[$iblockId]))
		{
			$res = \Thurly\Sale\TradingPlatform\Ebay\MapHelper::getCategoryEntityId($iblockId);
		...
		if(!isset($params[$iblockId]))
		{
			...
			$catRes = \Thurly\Sale\TradingPlatform\MapTable::getList(array(
				'filter' => array(
		...
		if(!empty($thurlyCategories))
		{
			foreach($thurlyCategories as $catId)
			{
