...
use \Thurly\Sale\TradingPlatform\Sftp;
use \Thurly\Main\ArgumentNullException;
use \Thurly\Sale\TradingPlatform\Logger;
use \Thurly\Sale\TradingPlatform\Ebay\Ebay;
use \Thurly\Sale\TradingPlatform\Ebay\Feed\QueueTable;
use \Thurly\Sale\TradingPlatform\Ebay\Feed\ResultsTable;
...
class SftpQueue extends DataProcessor
{
	...
	public function __construct(array $params)
	{
		...
		$this->path = \Thurly\Sale\TradingPlatform\Ebay\Helper::getSftpPath()."/".$this->feedType;
	}
	...
	protected function flushData()
	{
		...
		while($feedData = $feedDataRes->fetch())
		{
			...
			if(strtolower(SITE_CHARSET) != 'utf-8')
				$feedData["DATA"] = \Thurly\Main\Text\Encoding::convertEncoding($feedData["DATA"], SITE_CHARSET, 'UTF-8');
	...
	public function sendData()
	{
		...
		$zipFile = new \Thurly\Main\IO\File($tmpFile);
		$zipFile->rename($this->path."/zip/".$this->feedType."_".$this->fileNameSalt.".zip");
		$this->sendDataSftp();

		$checkResultsInterval = 5; //min.
		\Thurly\Sale\TradingPlatform\Ebay\Agent::add('RESULTS', $this->siteId, $checkResultsInterval, true);
	...
	protected function packData($xmlFile)
	{
		...
		if($oArchiver->Pack($xmlFile))
			\Thurly\Main\IO\File::deleteFile($xmlFile);
	...
	{
		$directory = new \Thurly\Main\IO\Directory($this->path."/zip");
		...
		$sftp = \Thurly\Sale\TradingPlatform\Ebay\Helper::getSftp($this->siteId);
		...
		for($i = 0; $i < count($filesToSend); $i++)
		{
			...
			if((!($directoryEntry instanceof \Thurly\Main\IO\File)) || GetFileExtension($localPath) != "zip")
				continue;
