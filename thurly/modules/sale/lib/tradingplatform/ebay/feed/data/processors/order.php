...
use Thurly\Sale\Internals\SiteCurrencyTable;
...
class Order extends DataProcessor
{
	...
	public function process($data)
	{
		...
		\Thurly\Sale\TradingPlatform\Ebay\Agent::add('ORDER_ACK', $this->siteId, 1, true);
		return true;
	}
...
//			|| empty($orderEbay["PaymentClearedTime"])
		...
		if(strtolower(SITE_CHARSET) != 'utf-8')
			$orderEbay = \Thurly\Main\Text\Encoding::convertEncodingArray($orderEbay, 'UTF-8', SITE_CHARSET);
		*/
		...
		foreach($transactionsArray as $transaction)
		{
			...
			if(!$basket)
			{
				$basket = \Thurly\Sale\Basket::create($this->siteId);
		...
		else
		{
			...
			Ebay::log(
				...
				Loc::getMessage(
					...
					array("#ORDER_ID#" => $thurlyOrderId)
				),
				$this->siteId
			);

			\CSaleMobileOrderPush::send("ORDER_CREATED", array("ORDER_ID" => $thurlyOrderId));

			$res = OrderTable::add(array(
				"ORDER_ID" => $thurlyOrderId,
				"TRADING_PLATFORM_ID" => $ebay->getId(),
				"EXTERNAL_ORDER_ID" => $orderEbay["ExtendedOrderID"],
				"PARAMS" => array(
		...
		// send confirmation
		if($thurlyOrderId > 0 && !empty($orderLineItemsIds))
		{
			$ebayFeed = \Thurly\Sale\TradingPlatform\Ebay\Feed\Manager::createFeed("ORDER_ACK", $this->siteId);
			$sourceData = array();
