...
use Thurly\Sale\Internals\OrderPropsTable;
...
class Helper
{
	...
	public static function getProductById($productId, $quantity, $siteId)
	{
		...
		{
			throw new \Thurly\Main\SystemException("Can't include module \"Catalog\"!");
		}
	 ...
	 */
	public static function getThurlyStatuses($siteId)
	{
		$result = array(
	 ...
	 * @param int $thurlyCategoryId Iblock section Id.
	 * @param string $value Selected value.
	 * @return string Html element <select>....
	 */
	public static function getThurlyCategoryPropsHtml($selectName, $iblockId, $thurlyCategoryId = 0, $value = '')
	{
		$catProps = self::getPropertiesList($iblockId, $thurlyCategoryId);
		...
		if ($arOffers)
		{
			$catProps2 = self::getPropertiesList($arOffers["IBLOCK_ID"], $thurlyCategoryId);
	...
	public static function onAfterUpdateShipment(\Thurly\Main\Event $event)
	{
		...
		$dbRes = OrderTable::getList(array(
			...
			'runtime' => array(
				'SHIPMENT' => array(
					'data_type' => 'Thurly\Sale\Internals\ShipmentTable',
					'reference' => array(
		...
		{
			if (class_exists($platformOrder['TRADING_PLATFORM_CLASS']) && is_subclass_of($platformOrder['TRADING_PLATFORM_CLASS'], '\Thurly\Sale\TradingPlatform\Platform'))
			{
