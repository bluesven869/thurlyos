...
class Order
{
	...
	public static function create(array $params)
	{
		...
		$result = new Result();
		$currencyList = \Thurly\Currency\CurrencyManager::getCurrencyList();
		...
		$order = \Thurly\Sale\Order::create($params['SITE_ID'], \CSaleUser::GetAnonymousUserID(), $currency);
		...
		$fUserId = $order->getUserId() > 0 ? \Thurly\Sale\Fuser::getIdByUserId($order->getUserId()) : null;
		$isStartField = $order->isStartField();
		$basket = \Thurly\Sale\Basket::create($params['SITE_ID']);
		...
		foreach ($params['CART_ITEMS'] as $itemKey => $item)
		{
			$basketItemFields = array(
				...
				'QUANTITY' => $item['count'],
				'PRODUCT_PROVIDER_CLASS' => '\Thurly\Catalog\Product\CatalogProvider'
			);
			...
			{
				/** @var \Thurly\Sale\BasketItemBase $item */
		 ...
		 * 	todo: check if we still need this
			if($arProduct["CURRENCY"] != $currency && \Thurly\Main\Loader::includeModule('currency'))
			{
...
//
//			if ($discount instanceof \Thurly\Sale\Discount)
//				$discount->setBasketItemData($basketCode, $providerData[$basketCode]);
	...
	public static function createShipment(\Thurly\Sale\Order &$order, $deliveryId = 0, $deliveryPrice = false)
	{
		...
		{
			/** @var \Thurly\Sale\Delivery\Services\Base $dlvSrv */
			if($dlvSrv = \Thurly\Sale\Delivery\Services\Manager::getObjectById($deliveryId))
	...
	public static function createPayment(\Thurly\Sale\Order &$order, $paySystemId = 0)
	{
