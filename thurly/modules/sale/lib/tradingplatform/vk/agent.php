...
use Thurly\Sale\TradingPlatform\Vk\Feed\Manager;
...
//		all OK - create new agent with null start position
	...
	{
		return 'Thurly\Sale\TradingPlatform\Vk\Agent::start("' . $feedType . '","' . $exportId . '","' . $startPosition . '",' . ($once ? 'true' : 'false') . ',' . $execNumber . ');';
	}
...
//		agent not exist - CREATE
	...
	{
		return 'Thurly\Sale\TradingPlatform\Vk\Agent::start("' . $feedType . '","' . $exportId . '",%true%';
	}
	...
	{
		return 'Thurly\Sale\TradingPlatform\Vk\Agent::start("' . $feedType . '","' . $exportId . '","_%"%';
	}
}