...
class DiscountCouponsManager
{
	...
	public static function get($extMode = true, $filter = array(), $show = false, $final = false)
	{
		...
		$validCoupons = (
			$show
			? self::$coupons
			: array_filter(self::$coupons, '\Thurly\Sale\DiscountCouponsManager::filterFreezeCoupons')
		);
	...
	public static function getOrderedCoupons($extMode = true, $filter = array())
	{
		...
		$result = array_filter(self::$coupons, '\Thurly\Sale\DiscountCouponsManager::filterFreezeCoupons');
		if (empty($result))
			return $result;
		$result = array_filter($result, '\Thurly\Sale\DiscountCouponsManager::filterFreezeOrderedCoupons');
		if (empty($result))
			return $result;
	...
	protected static function saveToStorage()
	{
		if (self::isSuccess())
		{
			...
			if (!empty(self::$coupons))
			{
				$couponsList = array_filter(self::$coupons, '\Thurly\Sale\DiscountCouponsManager::clearSavedCoupons');
				if (!empty($couponsList))
