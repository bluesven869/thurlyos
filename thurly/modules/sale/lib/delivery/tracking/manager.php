...
use Thurly\Sale\PropertyValueCollection;
...
class Manager
{
	...
	/**
	 * @param string $className Class name delivered from \Thurly\Sale\Delivery\Tracking\Base
	 * @param array $params
	 * @return Base
	 * @throws ArgumentNullException
	 * @throws SystemException
	...
	protected function createTrackingObject($className, array $params, Services\Base $deliveryService)
	{
		...
		if(get_parent_class($className) != 'Thurly\Sale\Delivery\Tracking\Base')
			throw new SystemException($className.' is not inherited from \"\Thurly\Sale\Delivery\Tracking\Base\"!');
	...
	public static function startRefreshingStatuses()
	{
		...
		return '\Thurly\Sale\Delivery\Tracking\Manager::startRefreshingStatuses();';
	}
	...
	protected function updateStatuses()
	{
		...
		$lastChage = \Thurly\Main\Type\DateTime::createFromTimestamp(time()-self::$activeStatusLiveTime);
		$lastUpdate = \Thurly\Main\Type\DateTime::createFromTimestamp(time()-$checkPeriod*60*60);
	...
	protected function sendStatusChangedMail($params)
	{
		...
		$res = ShipmentTable::getList(array(
			...
			'runtime' => array(
				'SITE' => array(
					'data_type' => 'Thurly\Main\SiteTable',
					'reference' => array(
	...
	protected function initClassNames()
	{
		...
		$classes = array(
			'\Thurly\Sale\Delivery\Tracking\RusPost' => 'lib/delivery/tracking/rus_post.php',
		);
	...
	public function updateShipment($shipmentId, StatusResult $params)
	{
		...
		$updParams['TRACKING_LAST_CHECK'] = new \Thurly\Main\Type\DateTime();

		if(intval($params->lastChangeTimestamp) > 0)
		{
			$updParams['TRACKING_LAST_CHANGE'] = \Thurly\Main\Type\DateTime::createFromTimestamp(
