...
use Thurly\Sale\Location\Comparator;
...
use Thurly\Sale\Location\LocationTable;
use Thurly\Sale\Location\ExternalServiceTable;

class ExternalLocationMap
{
	//Dlivery idtifyer, stored in \Thurly\Sale\Location\ExternalServiceTable : CODE
	...
	public static function importFromCsv($path)
	{
		...
		$content = \Thurly\Main\IO\File::getFileContents($path);
	...
	public static function exportToCsv($path)
	{
		...
		return \Thurly\Main\IO\File::putFileContents($path, $content);
	}
	 ...
	 * @return \Thurly\Main\Entity\AddResult|\Thurly\Main\Entity\Result|\Thurly\Main\Entity\UpdateResult
	 * @throws ArgumentNullException
	 * @throws \Thurly\Main\ArgumentException
	 * @throws \Exception
	 */
	public static function setExternalLocation2($srvId, $locationId, $xmlId, $updateExist = false)
	{
		...
		if(!empty($locCache[$srvId][$locationId.'##'.$xmlId]))
		{
			...
			{
				$result = new \Thurly\Main\Entity\UpdateResult();
				...
				VALUES(
					...
					'".$sqlHelper->forSql(
							preg_replace(
									...
									'',
									\Thurly\Sale\Location\Comparator::flatten($loc['NAME_UPPER']))
					)."'
				)
			");
	...
	public static function getLocationIdByNames($name, $city, $subregion, $region, $country = '', $exactOnly = false)
	{
		...
		if(!$exactOnly)
			$searchNames = array_merge($searchNames, \Thurly\Sale\Location\Comparator::getLocalityNamesArray($nameNorm['NAME'], $nameNorm['TYPE']));

		$searchNames = array_map(array('\Thurly\Sale\Location\Comparator', 'flatten'), $searchNames);
