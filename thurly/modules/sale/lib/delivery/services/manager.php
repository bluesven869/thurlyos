...
use Thurly\Sale\Internals\Pool;
...
use Thurly\Main\Entity\EventResult;
...
class Manager
{
	...
	public static function getActiveList($calculatingOnly = false, $restrictedIds = null)
	{
		...
		if(!$isDataPrepared || !empty($unPreparedRestrictedIds))
		{
			...
			{
				if(!is_subclass_of($service["CLASS_NAME"], 'Thurly\Sale\Delivery\Services\Base'))
					continue;
	 ...
	 * It's better to use \Thurly\Sale\Delivery\Services\Manager::getPooledObject for performance purposes
	 */
	public static function createObject(array $srvParams)
	{
		...
		if(class_exists($srvParams['CLASS_NAME']))
		{
			...
			if($service && !($service instanceof Base))
				$errorMsg = "Can't create delivery object. Class ".$srvParams['CLASS_NAME'].' is not the instance of Thurly\Sale\DeliveryService';
		}
		else
		{
	...
	protected static function initHandlers()
	{
		...
		self::$handlersDirectories = array(
			...
			'SYSTEM' => '/thurly/modules/sale/handlers/delivery/'
		);

		$result = array(
			'\Thurly\Sale\Delivery\Services\Group' => 'lib/delivery/services/group.php',
			'\Thurly\Sale\Delivery\Services\Automatic' => 'lib/delivery/services/automatic.php',
			'\Thurly\Sale\Delivery\Services\Configurable' => 'lib/delivery/services/configurable.php',
			'\Thurly\Sale\Delivery\Services\AutomaticProfile' => 'lib/delivery/services/automatic_profile.php',
			'\Thurly\Sale\Delivery\Services\EmptyDeliveryService' => 'lib/delivery/services/emptydeliveryservice.php'
		);

		\Thurly\Main\Loader::registerAutoLoadClasses('sale', $result);
		 ...
		 *			\Thurly\Main\EventResult::SUCCESS,
		 *			array(
		 *					'\Sale\Handlers\Delivery\SimpleHandler' => '/thurly/modules/sale/handlers/delivery/simple/handler.php'
		 *			),
		 *			'sale'
		 *		);
		 *	}
		...
		if (is_array($resultList) && !empty($resultList))
		{
			...
			{
				\Thurly\Main\Loader::registerAutoLoadClasses(null, $customClasses);
		...
		if(!empty($handlers))
		{
			\Thurly\Main\Loader::registerAutoLoadClasses(null, $handlers);
	 ...
	 * @return \Thurly\Sale\Delivery\CalculationResult
	 * @throws ArgumentNullException
	 */
	...
	{
		return	'Thurly\Sale\Delivery\DeliveryLocation';
	}

	/**
	 * Adds delivery service
	 * @param array $fields
	 * @return \Thurly\Main\Entity\AddResult
	 * @throws SystemException
	 * @throws \Exception
	 */
	 ...
	 * @param int $id
	 * @param array $fields
	 * @return \Thurly\Main\Entity\UpdateResult
	 * @throws SystemException
	 * @throws \Exception
	 */
	...
	{
		self::initHandlers();
		$res = \Thurly\Sale\Delivery\Services\Table::update($id, $fields);
	...
	public static function delete($id)
	{
		...
		$res = \Thurly\Sale\Delivery\Services\Table::delete($id);
	 ...
	 * 	'\Thurly\Sale\Delivery\Services\Manager',
	...
	{
		self::initHandlers();
		return \Thurly\Sale\Delivery\Services\EmptyDeliveryService::getEmptyDeliveryServiceId();
	}
	 ...
	 * inheritance of Thurly\Sale\Delivery\Services\Base.
	...
	public static function isDeliveryServiceClassValid($class)
	{
		...
		if(!is_subclass_of($class, 'Thurly\Sale\Delivery\Services\Base'))
			return false;
	...
	/**
	 * @deprecated  use \Thurly\Sale\Delivery\Services\Manager::calculateDeliveryPrice()
	 */
	...
	public static function getRestrictionObject($className)
	{
		...
		if(!($restriction instanceof Restrictions\Base))
			throw new SystemException('Object must be the instance of Thurly\Sale\Delivery\Restrictions\Base');
	...
	/**
	 * @deprecated use \Thurly\Sale\Delivery\Services\Manager::getRestrictedObjectsList()
	 */
	...
	/**
	 * @deprecated use \Thurly\Sale\Delivery\Services\Manager::isServiceExist()
	 */
	...
	/**
	 * @deprecated use \Thurly\Sale\Delivery\Services\Manager::getActiveList()
	 */
	...
	/**
	 * @deprecated use \Thurly\Sale\Delivery\Services\Manager::getRestrictedList()
	 */
	...
	/**
	 * @deprecated use \Thurly\Sale\Delivery\Services\Manager::createObject()
	 */
	...
	/**
	 * @deprecated use \Thurly\Sale\Delivery\Services\Manager::getObjectById()
	 */
	...
	/**
	 * @deprecated use \Thurly\Sale\Delivery\Services\Manager::getServiceByCode()
	 */
	...
	/**
	 * @deprecated use \Thurly\Sale\Delivery\Services\Manager::getHandlersList()
	 */
