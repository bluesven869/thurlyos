...
abstract class Base
{
	...
	public function __construct(array $initParams)
	{
		...
		if(!is_array($initParams["CONFIG"]))
			throw new \Thurly\Main\ArgumentTypeException("CONFIG", "array");
		...
		if(isset($initParams["EXTRA_SERVICES"]))
			$this->extraServices = new \Thurly\Sale\Delivery\ExtraServices\Manager($initParams["EXTRA_SERVICES"], $this->currency);
		elseif($this->id > 0)
			$this->extraServices = new \Thurly\Sale\Delivery\ExtraServices\Manager($this->id, $this->currency);
		else
			$this->extraServices = new \Thurly\Sale\Delivery\ExtraServices\Manager(array(), $this->currency);
	}
	 ...
	 */
	public function calculate(\Thurly\Sale\Shipment $shipment = null, $extraServices = array()) // null for compability with old configurable services api
	{
		if($shipment && !$shipment->getCollection())
		{
			$result = new Delivery\CalculationResult();
			$result->addError(new Error('\Thurly\Sale\Delivery\Services\Base::calculate() can\'t calculate empty shipment!'));
			return $result;
		}
	 ...
	 * @return float|int
	 */
	protected static function calculateShipmentPrice(\Thurly\Sale\Shipment $shipment)
	{
		$result = 0;
	 ...
	 */
	protected function calculateConcrete(\Thurly\Sale\Shipment $shipment)
	{
	...
	public function prepareFieldsForSaving(array $fields)
	{
		...
		foreach($structure as $key1 => $rParams)
		{
			foreach($rParams["ITEMS"] as $key2 => $iParams)
			{
				...
				$errors = \Thurly\Sale\Internals\Input\Manager::getError($iParams, $fields["CONFIG"][$key1][$key2]);
	...
	public function getEmbeddedExtraServicesList()
	{
		...
		return array(
			"ZAPALECH" => array(
				...
				"RIGHTS" => "YYY",
				"ACTIVE" => "Y",
				"CLASS_NAME" => '\Thurly\Sale\Delivery\ExtraServices\Checkbox',
	...
	*/
	...
	/**
	 * @return string Class name inherited from \Thurly\Sale\Delivery\Tracking\Base
	 */
	...
	/**
	 * @param string $class Class name inherited from \Thurly\Sale\Delivery\Tracking\Base
	 */
