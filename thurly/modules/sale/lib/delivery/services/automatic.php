...
use Thurly\Sale\Delivery\Helper;
...
use Thurly\Sale\Delivery\Inputs\MultiControlString;
...
class Automatic extends Base
{
	...
	protected static function convertNewOrderToOld(\Thurly\Sale\Shipment $shipment)
	{
	...
	public static function initHandlers()
	{
		...
		$arPathList = array( // list of valid services include files paths (security)
			\Thurly\Main\Config\Option::get('sale', 'delivery_handles_custom_path', BX_PERSONAL_ROOT.'/php_interface/include/sale_delivery/'),
			"/thurly/modules/sale/delivery/",
		);
		...
		foreach ($arPathList as $basePath)
		{
			if (file_exists($_SERVER["DOCUMENT_ROOT"].$basePath) && is_dir($_SERVER["DOCUMENT_ROOT"].$basePath))
			{
				...
				while(($filename = readdir($handle)) !== false)
				{
					...
					{
						if(\Thurly\Main\IO\Path::getExtension($filename) == 'php')
						{
	...
	public static function convertNewServiceToOld($service, $sid = false)
	{
		...
		{
			$restrictions = \Thurly\Sale\Delivery\Restrictions\Manager::getRestrictionsList($service["ID"]);

			foreach($restrictions as $restriction)
			{
				if($restriction["CLASS_NAME"] == '\Thurly\Sale\Delivery\Restrictions\BySite' && !empty($restriction["PARAMS"]["SITE_ID"]))
				{
			...
			foreach(Manager::getByParentId($service["ID"]) as $profile)
			{
				...
				foreach($restrictions as $restriction)
				{
					switch($restriction["CLASS_NAME"])
					{
						...
						case '\Thurly\Sale\Delivery\Restrictions\ByDimensions':
	 ...
	 */
	public function calculateProfile($profileId, array $profileConfig, \Thurly\Sale\Shipment $shipment)
	{
		static $result = array();
		$oldOrder = self::convertNewOrderToOld($shipment);
	...
	public function processAdditionalInfoShipmentEdit(Shipment $shipment, array $requestData)
	{
		...
		if(isset($this->handlerInitParams["PROCESS_ADD_INFO_SHIPMENT_EDIT"]) && is_callable($this->handlerInitParams["PROCESS_ADD_INFO_SHIPMENT_EDIT"]))
		{
			...
			if($result && get_class($result) != 'Thurly\Sale\Shipment')
				throw new SystemException('PROCESS_ADD_INFO_SHIPMENT_EDIT return value myst be of type "Thurly\Sale\Result" !');
		}
