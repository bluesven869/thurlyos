...
Loader::registerAutoLoadClasses('sale',
	array(
		...
		'\Thurly\Sale\Helpers\Admin\Blocks\OrderFinanceInfo' => 'lib/helpers/admin/blocks/orderfinanceinfo.php',
		'\Thurly\Sale\Helpers\Admin\Blocks\OrderAdditional' => 'lib/helpers/admin/blocks/orderadditional.php',
		'\Thurly\Sale\Helpers\Admin\Blocks\OrderShipment' => 'lib/helpers/admin/blocks/ordershipment.php',
		'\Thurly\Sale\Helpers\Admin\Blocks\OrderPayment' => 'lib/helpers/admin/blocks/orderpayment.php',
		'\Thurly\Sale\Helpers\Admin\Blocks\OrderStatus' => 'lib/helpers/admin/blocks/orderstatus.php',
		'\Thurly\Sale\Helpers\Admin\Blocks\OrderBasket' => 'lib/helpers/admin/blocks/orderbasket.php',
		'\Thurly\Sale\Helpers\Admin\Blocks\OrderBuyer' => 'lib/helpers/admin/blocks/orderbuyer.php',
		'\Thurly\Sale\Helpers\Admin\Blocks\OrderInfo' => 'lib/helpers/admin/blocks/orderinfo.php',
		'\Thurly\Sale\Helpers\Admin\Blocks\OrderMarker' => 'lib/helpers/admin/blocks/ordermarker.php',
));

/**
 * Class OrderEdit
 * Helper class for order administration.
 * @package Thurly\Sale\Helpers\Admin
 */
class OrderEdit
{
	...
	{
		Asset::getInstance()->addJs("/thurly/js/sale/admin/order_edit.js");
		Asset::getInstance()->addJs("/thurly/js/sale/admin/order_ajaxer.js");
	...
	protected static function getUserId($order, $formData, $createUserIfNeed, \Thurly\Sale\Result &$result)
	{
	...
	public static function createOrderFromForm(array $formData, $creatorUserId, $createUserIfNeed = true, array $files = array(), Result &$opResult)
	{
		...
		if(isset($formData["PRODUCT"]) && is_array($formData["PRODUCT"]) && !empty($formData["PRODUCT"]))
		{
			...
			$basket = \Thurly\Sale\Basket::create($formData["SITE_ID"]);
			...
			\Thurly\Sale\ProviderBase::setUsingTrustData(true);
			...
			foreach($formData["PRODUCT"] as $basketCode => $productData)
			{
				...
				else
				{
					...
					{
						/** @var \Thurly\Sale\BasketPropertiesCollection $property */
	 ...
	 * @param bool $createUserIfNeed
	 * @param array $files
	 * @param \Thurly\Sale\Result $result
	 * @return \Thurly\Sale\Order
	 * @throws SystemException
	 * @throws \Thurly\Main\ObjectNotFoundException
	 */
	public static function editOrderByFormData(array $formData, Order $order, $userId, $createUserIfNeed = true, array $files = array(), \Thurly\Sale\Result &$result)
	{
		/** @var \Thurly\Sale\Result $res */
		...
		/** @var  \Thurly\Sale\BasketItem  $item */
...
//					return $result;
	 ...
	 * @return \Thurly\Sale\Result
	 * @throws SystemException
	 * @deprecated use \Thurly\Sale\Helpers\Admin\OrderEdit::fillBasketItems()
	 */
	...
	public static function setProviderTrustData(BasketItem $item, Order $order, array $data)
	{
		...
		if ($item->isBundleParent())
		{
			...
			{
				/** @var \Thurly\Sale\BasketItem $bundleItem */
	...
	protected static function setBasketItemFields(\Thurly\Sale\BasketItem &$item, array $fields = array())
	{
	...
	public static function getDiscountsApplyResult(\Thurly\Sale\Order $order, $needRecalculate = false)
	{
		static $calcResults = null;

		if ($order instanceof \Thurly\Sale\Archive\Order)
		{
			/** @var \Thurly\Sale\Archive\Order $order*/
	...
	public static function getOrderedDiscounts(\Thurly\Sale\Order $order, $needRecalculate = true)
	{
	...
	public static function getCouponList(\Thurly\Sale\Order $order = null, $needRecalculate = true)
	{
		$result = array();
		$discounts = array();
	...
	public static function getLockingMessage($orderId)
	{
		$intLockUserID = 0;
		$strLockTime = '';
		$r = \Thurly\Sale\Order::getLockedStatus($orderId);
		...
		if ($r->isSuccess())
		{
			...
			if (array_key_exists('DATE_LOCK', $lockResult) && $lockResult['DATE_LOCK'] instanceof \Thurly\Main\Type\DateTime)
				$strLockTime = $lockResult['DATE_LOCK']->toString();
		}

		$strLockUserInfo = $intLockUserID;

		$userIterator = \Thurly\Main\UserTable::getList(array(
