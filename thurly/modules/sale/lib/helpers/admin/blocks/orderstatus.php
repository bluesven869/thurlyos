...
use Thurly\Sale\Internals\StatusLangTable;
use Thurly\Sale\Internals\StatusTable;
use Thurly\Sale\Order;
use Thurly\Main\Localization\Loc;
use Thurly\Sale\TradingPlatform\OrderTable;
...
class OrderStatus
{
	public static function getEdit(Order $order, \CUser $user, $showCancel, $showSaveButton)
	{
		...
		$orderLocked = \Thurly\Sale\Order::isLocked($order->getId());

		$allowCancel = false;

		if($showCancel)
		{
			$allowedStatusesCancel = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($user->GetID(), array('cancel'));
				...
				<tbody>
					<tr>
						...
						<td class="adm-detail-content-cell-r">
							<div>'.
								 $data["DATE_INSERT"].
								'&nbsp;<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='. $data["CREATOR_USER_ID"].'">'.
					...
					<tr>
						...
						<td class="adm-detail-content-cell-r"><div>'.
								htmlspecialcharsbx(
									\Thurly\Sale\Helpers\Admin\OrderEdit::getSiteName($order->getSiteId())
								).
							'</div></td>
					</tr>';
		...
		if(!empty($data["AFFILIATE_NAME"]))
		{
			...
			{
				$data["AFFILIATE_NAME"] = '<a href="/thurly/admin/sale_affiliate_edit.php?lang='.LANGUAGE_ID.'&ID='.$data['AFFILIATE_ID'].'">'.
						$data["AFFILIATE_NAME"].
					'</a>';
			}
			...
			$text = '
				<div class="adm-s-select-popup-element-selected" id="sale-adm-status-cancel-blocktext">
					<div class="adm-s-select-popup-element-selected-bad">
						...
						<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='. $order->getField("EMP_CANCELED_ID").'">'.
	...
	public static function getStatusesList($userId, $orderStatus = false)
	{
		if($orderStatus === false)
			$orderStatus = \Thurly\Sale\OrderStatus::getInitialStatus();

		$result = \Thurly\Sale\OrderStatus::getAllowedUserStatuses($userId, $orderStatus);
