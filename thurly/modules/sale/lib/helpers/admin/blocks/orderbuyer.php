...
use Thurly\Sale\OrderUserProperties;
use Thurly\Sale\Order;
use Thurly\Sale\OrderTable;
...
class OrderBuyer
{
	public static function getEdit(Order $order, $showProfiles = false, $profileId = 0)
	{
		...
		if($showProfiles)
		{
			$result .=	\Thurly\Sale\Helpers\Admin\OrderEdit::makeSelectHtml(
							"BUYER_PROFILE_ID",
	...
	{
		\Thurly\Main\Page\Asset::getInstance()->addJs("/thurly/js/sale/admin/order_buyer.js");
		\Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/sale/input.js');
		\Thurly\Sale\PropertyValueCollection::initJs();
	...
	public static function 	prepareData(Order $order)
	{
		...
		{
			$strBuyerProfileUrl = '/thurly/admin/sale_buyers_profile.php?USER_ID='.intval($order->getUserId()).'&lang='.LANGUAGE_ID;
		}
		else
		{
			$strBuyerProfileUrl = '/thurly/admin/user_edit.php?ID='.$order->getUserId().'&lang='.LANGUAGE_ID;
		}
	...
	public static function getOrderPropertiesByJS(Order $order)
	{
		...
		if (!empty($result['properties']))
		{
			...
			{
				if (strpos($typeData['CLASS'], 'Thurly\\Sale\\Internals\\Input') !== false)
					$baseTypes[] = $typeName;
			}
