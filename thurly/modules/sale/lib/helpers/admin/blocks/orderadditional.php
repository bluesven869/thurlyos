...
use Thurly\Sale\Services\Company\Manager;
...
class OrderAdditional
{
	public static function getEdit($collection, $formName, $formPrefix, array $post = array())
	{
		...
		if(get_class($collection) == 'Thurly\Sale\Order')
			$orderLocked = \Thurly\Sale\Order::isLocked($collection->getId());
		else
			$orderLocked = false;
		...
		$data['COMPANIES'] = Manager::getListWithRestrictions($collection, \Thurly\Sale\Services\Company\Restrictions\Manager::MODE_MANAGER);
					...
					<tr>
						...
						<td class="adm-detail-content-cell-r">
							<div class="adm-s-order-person-choose">
								<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='. $data["RESPONSIBLE_ID"].'" id="order_additional_info_responsible">'.
	...
	{
		\Thurly\Main\Page\Asset::getInstance()->addJs("/thurly/js/sale/admin/order_additional_info.js");
	...
	protected static function prepareData($collection)
	{
		...
		else
		{
			if (intval($collection->getField('EMP_RESPONSIBLE_ID')) > 0)
				$data['EMP_RESPONSIBLE'] = \Thurly\Sale\Helpers\Admin\OrderEdit::getUserName($collection->getField('EMP_RESPONSIBLE_ID'));
		...
		{
			$data['RESPONSIBLE'] = \Thurly\Sale\Helpers\Admin\OrderEdit::getUserName($collection->getField('RESPONSIBLE_ID'));
