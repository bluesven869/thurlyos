...
use Thurly\Sale\Internals\CompanyTable;
...
class OrderPayment
{
	...
	public static function getPaySystemParams($paySystemId)
	{
		...
		if (!isset($result[$paySystemId]))
		{
			...
			{
				$result[$paySystemId]["LOGOTIP_PATH"] = '/thurly/images/sale/nopaysystem.gif';
				$result[$paySystemId]["LOGOTIP_SHORT_PATH"] = '/thurly/images/sale/nopaysystem.gif';
			}
		}
	...
	/**
	 * @param \Thurly\Sale\Payment $payment
	 * @param int $index
	...
	{
		Asset::getInstance()->addJs("/thurly/js/sale/admin/order_payment.js");
	...
	private static function getEditTemplate($data, $index, $post = array())
	{
		...
		{
			/** @var \Thurly\Sale\PaySystem\Service $service */
		...
		else
		{
			...
			{
				$companies = str_replace("#URL#", "/thurly/admin/sale_company_edit.php?lang=".$lang, Loc::getMessage('SALE_ORDER_PAYMENT_ADD_COMPANY'));
			}
		}
										...
										<tbody>
											<tr>
												...
												<td class="adm-detail-content-cell-r tal" id="PAYMENT_CHANGE_USER_INFO_'.$index.'">
													'.$data['DATE_PAID'].'
													<a href="/thurly/admin/user_edit.php?lang='.$lang.'&ID='.$data['EMP_PAID_ID'].'">'.htmlspecialcharsbx($data['EMP_PAID_ID_NAME']).' '.htmlspecialcharsbx($data['EMP_PAID_ID_LAST_NAME']).'</a>
												</td>
											</tr>
										</tbody>
									</table>
	...
	private static function getViewTemplate($data, $index, $form)
	{
		...
		if ($isAllowEdit && !$data['ORDER_LOCKED'])
			$sectionEdit = '<div class="adm-bus-pay-section-action" id="SECTION_'.$index.'_EDIT"><a href="/thurly/admin/sale_order_payment_edit.php?order_id='.$data['ORDER_ID'].'&payment_id='.$data['ID'].'&backurl='.urlencode($_SERVER['REQUEST_URI']).'">'.Loc::getMessage('SALE_ORDER_PAYMENT_EDIT').'</a></div>';
	...
	private static function getImgPathList()
	{
		...
		$logotypes = array('/thurly/images/sale/nopaysystem.gif');
		foreach ($paySystems as $paySystem)
		{
