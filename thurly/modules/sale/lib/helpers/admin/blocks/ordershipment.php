...
use Thurly\Sale\Cashbox\Internals\CashboxTable;
...
class OrderShipment
{
	...
	public static function getEditTemplate($data, $index, $formType, $post)
	{
		...
		$map = '';
		$extraServiceHTML = '';
		$extraServiceManager = new \Thurly\Sale\Delivery\ExtraServices\Manager($data['DELIVERY_ID']);
		...
		else
		{
			...
			{
				$companies = str_replace("#URL#", "/thurly/admin/sale_company_edit.php?lang=".$lang, Loc::getMessage('SALE_ORDER_SHIPMENT_ADD_COMPANY'));
			}
		}
		...
		if (isset($items[$data['DELIVERY_ID']]['LOGOTIP']['MAIN']))
			...
			$logo = '/thurly/images/sale/logo-default-d.gif';
			...
			<table class="adm-detail-content-table edit-table" border="0" width="100%" cellpadding="0" cellspacing="0">
				<tbody>
					...
					<tr>
						...
						<td class="adm-detail-content-cell-r">
							<div>'.Loc::getMessage('SALE_ORDER_SHIPMENT_MODIFY_BY').': <span style="color: #66878F" id="order_additional_info_date_responsible">'.htmlspecialcharsbx($data['DATE_DEDUCTED']).'</span>  <a href="/thurly/admin/user_edit.php?lang='.$lang.'&ID='.$data['EMP_DEDUCTED_ID'].'" id="order_additional_info_emp_responsible">'.htmlspecialcharsbx($data['EMP_DEDUCTED_ID_LAST_NAME']).' '.htmlspecialcharsbx($data['EMP_DEDUCTED_ID_NAME']).'</a></div>
						</td>
					</tr>
					' : '').'
				</tbody>
			...
			<tbody>
				...
				'<tr>
					...
					<td class="adm-detail-content-cell-r tal">'.
						'<span id="sale-order-shipment-tracking-status-'.$index.'">'.(intval($data['TRACKING_STATUS']) >= 0  ? \Thurly\Sale\Delivery\Tracking\Manager::getStatusName($data['TRACKING_STATUS']) : '-').'</span>'.
			...
			foreach($data['DELIVERY_ADDITIONAL_INFO_EDIT'] as $name => $params)
			{
				$result .='
					...
					<td class="adm-detail-content-cell-r" width="60%">
						'.\Thurly\Sale\Internals\Input\Manager::getEditHtml('SHIPMENT['.$index.'][ADDITIONAL]['.$name.']', $params).'
					</td>';
			}
	...
	private static function getDeliveryServiceInfoById($id)
	{
		...
		if ($id > 0)
		{
			$resService = \Thurly\Sale\Delivery\Services\Table::getList(array(
				'filter' => array('ID' => $id),
	...
	{
		static $result = null;
		$logoPath ='/thurly/images/sale/logo-default-d.gif';
	...
	public static function getExtraServiceEditControl($extraService, $index, $view = false, Shipment $shipment = null)
	{
		...
		/**
		 * @var  $itemId
		 * @var \Thurly\Sale\Delivery\ExtraServices\Base $item
		 */
	...
	public static function getEdit($shipment, $index = 0, $formType = '', $dataForRecovery = array())
	{
		...
		$data['COMPANIES'] = Company\Manager::getListWithRestrictions($shipment, \Thurly\Sale\Services\Company\Restrictions\Manager::MODE_MANAGER);
	...
	protected static function getStoresList($deliveryId, $storeId)
	{
		...
		$storesIds = \Thurly\Sale\Delivery\ExtraServices\Manager::getStoresList($deliveryId);
	...
	public static function getMap($deliveryId, $index, $storeId = 0, $formType = "")
	{
		...
		if ($stores)
		{
			...
			$map .= '<link rel="stylesheet" type="text/css" href="/thurly/components/thurly/sale.store.choose/templates/.default/style.css">';
		}
	...
	{
		Asset::getInstance()->addJs("/thurly/js/sale/admin/order_shipment.js");
		$message = array(
	...
	/**
	 * @param \Thurly\Sale\Shipment $shipment
	 * @param int $index
	 * @param string $formType
	 * @return string
	 */
	...
	public static function getViewTemplate($data, $index, $formType)
	{
		...
		$service = null;
		$extraServiceHTML = '';
		$mainLogoPath =  '/thurly/images/sale/logo-default-d.gif';
		$shortLogoPath =  '/thurly/images/sale/logo-default-d.gif';
		...
		if (in_array($data["STATUS_ID"], $allowedOrderStatusesUpdate) && !$data['ORDER_LOCKED'] && $formType != 'archive')
			$sectionEdit = '<div class="adm-bus-pay-section-action" id="SHIPMENT_SECTION_'.$index.'_EDIT"><a href="/thurly/admin/sale_order_shipment_edit.php?order_id='.$data['ORDER_ID'].'&shipment_id='.$data['ID'].'&backurl='.urlencode($backUrl).'">'.Loc::getMessage('SALE_ORDER_SHIPMENT_BLOCK_SHIPMENT_EDIT').'</a></div>';
		...
		if(is_array($data['DELIVERY_ADDITIONAL_INFO_VIEW']) && !empty($data['DELIVERY_ADDITIONAL_INFO_VIEW']))
		{
			...
			foreach($data['DELIVERY_ADDITIONAL_INFO_VIEW'] as $name => $params)
			{
				$result .='
					<td class="adm-detail-content-cell-l" width="40%">'.$params['LABEL'].':</td>
					<td class="adm-detail-content-cell-r" width="60%">
						'.\Thurly\Sale\Internals\Input\Manager::getViewHtml($params).'
					</td>';
			}
	...
	protected static function prepareData($error = false, $needRecalculate = true)
	{
		...
		/** @var \Thurly\Sale\Delivery\Services\Base $delivery */
		...
		if(!is_null($delivery))
		{
			...
			{
				$trackingManager = \Thurly\Sale\Delivery\Tracking\Manager::getInstance();
			...
			if(intval($request['REQUEST_ID']) > 0)
			{
				$fields['DELIVERY_REQUEST_NAME'] = Loc::getMessage(
					...
					array(
						...
						'#REQUEST_DATE#' => $request['REQUEST_DATE']->format(\Thurly\Main\Type\Date::getFormat())
				));
				$fields['DELIVERY_REQUEST_LINK'] = '/thurly/admin/sale_delivery_request_view.php?lang='.LANGUAGE_ID.'&ID='.intval($request['REQUEST_ID']);
			}
	...
	public static function updateData(Order &$order, array $shipments)
	{
		...
		foreach ($shipments as $item)
		{
			...
			/** @var \Thurly\Sale\BasketItem $product */
			...
			if($deliveryService && !empty($item['ADDITIONAL']))
			{
				...
				if($modifiedShipment && get_class($modifiedShipment) == 'Thurly\Sale\Shipment')
	 ...
	 * @throws Main\ArgumentNullException
	 */
	public static function calculateDeliveryPrice(\Thurly\Sale\Shipment $shipment)
	{
	...
	public static function setShipmentByDefaultValues(&$shipment)
	{
		...
		$systemShipment = $shipmentCollection->getSystemShipment();
		$systemShipmentItemCollection = $systemShipment->getShipmentItemCollection();
		/** @var \Thurly\Sale\ShipmentItemCollection $shipmentItemCollection */
		$shipmentItemCollection = $shipment->getShipmentItemCollection();

		/** @var \Thurly\Sale\ShipmentItem $systemShipmentItem */
