...
class OrderBasket
{
	...
	public static function getProductsData(array $productsIds, $siteId, array $fields = array(), $userId = 0)
	{
		...
		{
			$noCachedData = \Thurly\Sale\Helpers\Admin\Product::getData($noCachedProductIds, $siteId, array_keys($fields));
	...
	{
		$arRes = array();
		$bUseHLIblock = \Thurly\Main\Loader::includeModule('highloadblock');
		$filter = array(
		...
		$rsProps = \Thurly\Iblock\PropertyTable::getList(array(
			'filter' => $filter,
	...
	protected function getVisibleColumns($idPrefix)
	{
		...
		if(is_array($arTmpColumns) && !empty($arTmpColumns))
		{
			...
			if(!empty($iBlockProps))
			{
				$dbRes = \Thurly\Iblock\PropertyTable::getList(array(
					'filter' => array(
						'LOGIC' => 'OR',
						'=CODE' => $iBlockProps,
						'=ID' => $iBlockPropsInt
	 ...
	 * @return array
	 * @throws Main\LoaderException
	 * @deprecated use \Thurly\Sale\Helpers\Admin\Product::getData() instead it.
	 */
			...
			foreach($items as $item)
			{
				...
				if(!Provider::isExistsTrustData($this->order->getSiteId(), 'sale', $item->getProductId()))
				{
					...
					if($basketItem)
					{
						...
						{
							\Thurly\Sale\Helpers\Admin\OrderEdit::setProviderTrustData($item, $this->order, $providerData[$basketCode]);
				...
				elseif($item->isBundleParent())
				{
					...
					if($children = $item->getBundleCollection())
					{
						/** @var \Thurly\Sale\BasketItem $child */
						foreach($children->getBasketItems() as $child)
						{
