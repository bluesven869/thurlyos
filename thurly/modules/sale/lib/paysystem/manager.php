...
use Thurly\Sale\Internals\EntityCollection;
use Thurly\Sale\Internals\PaymentTable;
use Thurly\Sale\Internals\PaySystemActionTable;
use Thurly\Sale\Internals\PaySystemRestHandlersTable;
 ...
 * @package Thurly\Sale\Payment
 */
final class Manager
{
	const EVENT_ON_GET_HANDLER_DESC = 'OnSaleGetHandlerDescription';
	const CACHE_ID = "THURLY_SALE_INNER_PS_ID";
	const TTL = 31536000;
	/**
	 * @var array
	 */
	private static $handlerDirectories = array(
		...
		'SYSTEM' => '/thurly/modules/sale/handlers/paysystem/',
		'SYSTEM_OLD' => '/thurly/modules/sale/payment/'
	);
	...
	public static function searchByRequest(Request $request)
	{
		...
		while ($item = $items->fetch())
		{
			...
			foreach (self::getHandlerDirectories() as $type => $path)
			{
				...
				{
					$className = '\Thurly\Sale\PaySystem\RestHandler';
				}
					...
					if ($item->isFile())
					{
						...
						if (strpos($item->getName(), '.description') !== false)
						{
							...
							if (!isset($result[$group][$handlerName]))
							{
								...
								{
									if ((IsModuleInstalled('thurlyos') && $data['DOMAIN'] === 'BOX') ||
										(!IsModuleInstalled('thurlyos') && $data['DOMAIN'] === 'CLOUD')
									)
									{
										continue(2);
									}
	...
	private static function createInnerPaySystem()
	{
		...
		$imagePath = Application::getDocumentRoot().'/thurly/images/sale/sale_payments/inner.png';
		...
		if ($path !== null)
		{
			if (File::isFileExists($documentRoot.$path.'/handler.php'))
			{
				...
				if (class_exists($className))
				{
					...
					if (array_key_exists('Thurly\Sale\PaySystem\IPayable', $interfaces))
		...
		if ($result->isSuccess())
		{
			...
			if ($restrictionList)
			{
				...
				foreach ($restrictionList as $restriction)
				{
					...
					if (is_subclass_of($className, '\Thurly\Sale\Services\Base\Restriction'))
					{
	...
	public static function getDataRefundablePage()
	{
		...
		foreach ($paySystemList as $handler => $data)
		{
			...
			if (is_subclass_of($classHandler, '\Thurly\Sale\PaySystem\ServiceHandler'))
			{
