 ...
 */
class CashboxThurly extends Cashbox
{
	...
	{
		return Localization\Loc::getMessage('SALE_CASHBOX_THURLY_TITLE');
	}
	...
	protected static function extractCheckData(array $data)
	{
		...
		{
			$errorMsg = Localization\Loc::getMessage('SALE_CASHBOX_THURLY_ERR'.$data['code']);
			if (!$errorMsg)
	...
	public static function getSettings($modelId = 0)
	{
		...
		if (isset($kkmList[$modelId]))
		{
			...
			if (isset($defaultSettings['PAYMENT_TYPE']))
			{
				$settings['PAYMENT_TYPE'] = array(
					'LABEL' => Localization\Loc::getMessage('SALE_CASHBOX_THURLY_SETTINGS_P_TYPE'),
					'REQUIRED' => 'Y',
					'ITEMS' => array()
				);
				...
				foreach ($systemPaymentType as $type)
				{
					$settings['PAYMENT_TYPE']['ITEMS'][$type] = array(
						'TYPE' => 'STRING',
						'LABEL' => Localization\Loc::getMessage('SALE_CASHBOX_THURLY_SETTINGS_P_TYPE_LABEL_'.ToUpper($type)),
			...
			$settings['VAT'] = array(
				'LABEL' => Localization\Loc::getMessage('SALE_CASHBOX_THURLY_SETTINGS_VAT'),
				'REQUIRED' => 'Y',
				'ITEMS' => array(
					'NOT_VAT' => array(
						'TYPE' => 'STRING',
						'LABEL' => Localization\Loc::getMessage('SALE_CASHBOX_THURLY_SETTINGS_VAT_LABEL_NOT_VAT'),
						'VALUE' => 4
					)
				)
			);
		...
		$settings['Z_REPORT'] = array(
			'LABEL' => Localization\Loc::getMessage('SALE_CASHBOX_THURLY_SETTINGS_Z_REPORT'),
			'ITEMS' => array(
				'TIME' => array(
					'TYPE' => 'DELIVERY_MULTI_CONTROL_STRING',
					'LABEL' => Localization\Loc::getMessage('SALE_CASHBOX_THURLY_SETTINGS_Z_REPORT_LABEL'),
					'ITEMS' => array(
						'H' => array(
							'TYPE' => 'ENUM',
							'LABEL' => Localization\Loc::getMessage('SALE_CASHBOX_THURLY_SETTINGS_Z_REPORT_LABEL_H'),
							'VALUE' => 23,
							'OPTIONS' => $hours
						),
						'M' => array(
							'TYPE' => 'ENUM',
							'LABEL' => Localization\Loc::getMessage('SALE_CASHBOX_THURLY_SETTINGS_Z_REPORT_LABEL_M'),
							'VALUE' => 30,
							'OPTIONS' => $minutes
						),
					)
