...
namespace Thurly\Sale\Internals\Input;
	...
	static function initJs()
	{
		...
		if (! $done)
		{
			...
			\CJSCore::RegisterExt('input', array(
				'js'   => '/thurly/js/sale/input.js',
				'lang' => '/thurly/modules/sale/lang/'.LANGUAGE_ID.'/lib/internals/input.php',
			));
			\CJSCore::Init(array('input'));

			print('<div style="display:none">');
			$GLOBALS['APPLICATION']->IncludeComponent("thurly:sale.location.selector.".\Thurly\Sale\Location\Admin\LocationHelper::getWidgetAppearance(), "", array(
				"ID" => '',
				"CODE" => '',
...
class Location extends Base
{
	public static function getViewHtmlSingle(array $input, $value)
	{
		...
		try
		{
			$result = \Thurly\Sale\Location\LocationTable::getPathToNodeByCode($value, array(
	...
	public static function getEditHtmlSingle($name, array $input, $value)
	{
		...
		$GLOBALS['APPLICATION']->IncludeComponent(
			'thurly:sale.location.selector.'.($filterMode ? 'search' : \Thurly\Sale\Location\Admin\Helper::getWidgetAppearance()),
			'',
			$parameters,
			false
		);
	...
	public static function getErrorSingle(array $input, $value)
	{
		return \Thurly\Sale\Location\LocationTable::getByCode($value)->fetch()
			? array()
			: array('INVALID' => Loc::getMessage('INPUT_INVALID_ERROR'));
	}
}
