...
class DeliveryPaySystemTable extends \Thurly\Main\Entity\DataManager
{
	...
	public static function getMap()
	{
		return array(
			...
			'DELIVERY' => array(
				'data_type' => '\Thurly\Sale\Delivery\Services\Table',
				'reference' => array(
			...
			'PAYSYSTEM' => array(
				'data_type' => '\Thurly\Sale\Internals\PaySystemActionTable',
				'reference' => array(
	...
	{
		return array(
			new \Thurly\Main\Entity\Validator\Length(1, 1),
		);
	}
	 ...
	 * @return \Thurly\Main\Entity\Result
	...
	protected static function isValidParent($parentId, $entityType)
	{
		...
		if($entityType == self::ENTITY_TYPE_DELIVERY)
		{
			...
			if($activeDeliveryData[$parentId]['PARENT_CLASS_NAME'] == '\Thurly\Sale\Delivery\Services\Group')
				return false;
		}
	...
	protected static function actualizeDeliveriesRestrictionByPS()
	{
		...
		$restrictions = array();
		$dbR = \Thurly\Sale\Internals\ServiceRestrictionTable::getList(array(
			'filter' => array(
		...
		if(!empty($notNeedRestriction))
		{
			...
			foreach($notNeedRestriction as $deliveryId)
				$sql .= " ".($sql == "" ? "WHERE CLASS_NAME='".$sqlHelper->forSql('\Thurly\Sale\Delivery\Restrictions\ByPaySystem')."' AND (" : "OR " )."SERVICE_ID=".$sqlHelper->forSql($deliveryId)." AND SERVICE_TYPE=".Restrictions\Manager::SERVICE_TYPE_SHIPMENT;

			$sql = "DELETE FROM ".\Thurly\Sale\Internals\ServiceRestrictionTable::getTableName().$sql.")";
		...
		if(!empty($needRestriction))
		{
			...
			foreach($needRestriction as $deliveryId)
				$sql .= ($sql == "" ? " " : ", ")."(".$sqlHelper->forSql($deliveryId).", '".$sqlHelper->forSql('\Thurly\Sale\Delivery\Restrictions\ByPaySystem')."', ".Restrictions\Manager::SERVICE_TYPE_SHIPMENT.")";

			$sql = "INSERT INTO ".\Thurly\Sale\Internals\ServiceRestrictionTable::getTableName()."(SERVICE_ID, CLASS_NAME, SERVICE_TYPE) VALUES".$sql;
			$con->queryExecute($sql);
		}
	}
	...
	protected static function actualizePaySystemRestrictionByDelivery()
	{
		...
		$dbR = \Thurly\Sale\Internals\ServiceRestrictionTable::getList(array(
			'filter' => array(
				'=CLASS_NAME' => '\Thurly\Sale\Services\PaySystem\Restrictions\Delivery'
			),
			'select' => array('SERVICE_ID')
		));
		...
		if(!empty($notNeedRestriction))
		{
			...
			foreach($notNeedRestriction as $deliveryId)
				$sql .= " ".($sql == "" ? "WHERE CLASS_NAME='".$sqlHelper->forSql('\Thurly\Sale\Services\PaySystem\Restrictions\Delivery')."' AND (" : "OR " )."SERVICE_ID=".$sqlHelper->forSql($deliveryId)." AND SERVICE_TYPE=".Manager::SERVICE_TYPE_PAYMENT;
		...
		if(!empty($needRestriction))
		{
			...
			foreach($needRestriction as $deliveryId)
				$sql .= ($sql == "" ? " " : ", ")."(".$sqlHelper->forSql($deliveryId).", '".$sqlHelper->forSql('\Thurly\Sale\Services\PaySystem\Restrictions\Delivery')."', ".Manager::SERVICE_TYPE_PAYMENT.")";
	...
	protected static function excludeDeliveryByParent(array $dlvIds)
	{
		...
		foreach($dlvIds as $id)
		{
			if(intval($activeDeliveryData[$id]['PARENT_ID']) <= 0)
				$result[] = $id;
			elseif($activeDeliveryData[$id]['PARENT_CLASS_NAME'] == '\Thurly\Sale\Delivery\Services\Group')
				$result[] = $id;
	...
	protected static function getDeliveryParentId($deliveryId)
	{
		...
		if($activeData[$parentId]['CLASS_NAME'] == '\Thurly\Sale\Delivery\Services\Group')
			return 0;
	...
	protected static function getEntityItemsFullList($entityType)
	{
		...
		if($entityType == self::ENTITY_TYPE_DELIVERY)
		{
			\Thurly\Sale\Delivery\Services\Manager::getActiveList();
