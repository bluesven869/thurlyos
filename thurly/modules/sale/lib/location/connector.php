...
abstract class Connector extends Entity\DataManager
{
	...
	*/
	...
	/** Method should return a name of the "target" entity plus namespace but without trailing 'Table'. For example, for delivery this should be 'Thurly\Sale\Delivery\Delivery'
	*
			...
			if($connType == self::LINK_CODE) // entity connected by CODE
			{
				$query
					->registerRuntimeField(
						'G',
						array(
							'data_type' => '\Thurly\Sale\Location\GroupTable',
							'reference' => array(
					...
					->registerRuntimeField(
						'LG',
						array(
							'data_type' => '\Thurly\Sale\Location\GroupLocation',
							'reference' => array(
	...
	* @throws NotImplementedException
	*
	* @return \Thurly\Main\DB\Result list of locations
	*/
	...
	public static function checkConnectionExists($entityPrimary, $locationPrimary, array $behaviour = array('LOCATION_LINK_TYPE' => 'ID'))
	{
		...
		if(!intval($node['ID']))
		{
			throw new \Thurly\Sale\Location\Tree\NodeNotFoundException(false, array('INFO' => array($field => $locationPrimary)));
		}
	...
	*/
	protected static function queryPage($sql, $limit = 0, $offset = 0)
	{
		...
		// todo: Luke, use Thurly\Main\DB\Paginator here
	...
	*/
	...
	public static function normalizeLocationList($denormalizedList = array())
	{
		...
		$query = new Entity\Query('Thurly\Sale\Location\Location');
