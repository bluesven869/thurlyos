...
class Finder
{
	...
	{
		$langsFromDb = array();
		$res = \Thurly\Main\Localization\LanguageTable::getList(array('select' => array('ID')));
		...
		if($filterByPhrase)
		{
			$sql = "
				select ".($dbConnection->getType() != 'mysql' ? '' : 'distinct')/*fix this in more clever way later*/." 
					".$sqlSelect.(\Thurly\Sale\Location\DB\Helper::needSelectFieldsInOrderByWhenDistinct() ? ', A.RELEVANCY' : '')."
