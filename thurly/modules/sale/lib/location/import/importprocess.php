...
namespace Thurly\Sale\Location\Import;
...
use Thurly\Sale\Location\DB\BlockInserter;
use Thurly\Sale\Location\Util\CSVReader;
...
final class ImportProcess extends Location\Util\Process
{
	...
	protected function stageProcessFiles()
	{
		...
		$this->hitData['HANDLES']['LOCATION'] = new BlockInserter(array(
			'entityName' => '\Thurly\Sale\Location\LocationTable',
		...
		$this->hitData['HANDLES']['NAME'] = new BlockInserter(array(
			'entityName' => '\Thurly\Sale\Location\Name\LocationTable',
		...
		$this->hitData['HANDLES']['EXTERNAL'] = new BlockInserter(array(
			'entityName' => '\Thurly\Sale\Location\ExternalTable',
	...
	public function getStatistics($type = 'TOTAL')
	{
		if(empty($this->stat))
		{
			$types = \Thurly\Sale\Location\Admin\TypeHelper::getTypes(array('LANGUAGE_ID' => $this->getLanguageId()));
	...
	public static function getSiteLanguages()
	{
		...
		if($langs == null)
		{
			...
			$res = \Thurly\Main\SiteTable::getList(array('filter' => array('ACTIVE' => 'Y'), 'select' => array('LANGUAGE_ID'), 'group' => array('LANGUAGE_ID')));
			while($item = $res->fetch())
			{
	...
	{
		$wasCreated = false;
		if((string) $this->data['LOCAL_PATH'] != '' && \Thurly\Main\IO\Directory::isDirectoryExists($this->data['LOCAL_PATH']))
		{
		...
		else
		{
			...
			if(!\Thurly\Main\IO\Directory::isDirectoryExists($localPath))
				\Thurly\Main\IO\Directory::createDirectory($localPath);
		}
	...
	{
		if(!\Thurly\Main\IO\Directory::isDirectoryExists($path))
		{
			\Thurly\Main\IO\Directory::createDirectory($path);
		}
	}
	...
	protected static function deleteDirectory($path)
	{
		if(\Thurly\Main\IO\Directory::isDirectoryExists($path))
		{
			\Thurly\Main\IO\Directory::deleteDirectory($path);
		}
	}
	...
	public static function importFile(&$descriptior)
	{
		...
		if($descriptior['STEP'] == 'import')
		{
			...
			{
				$res = \Thurly\Sale\Location\LocationTable::getList(array('select' => array('CNT')))->fetch();
