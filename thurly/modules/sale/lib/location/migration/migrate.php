...
namespace Thurly\Sale\Location\Migration;
...
class CUpdaterLocationPro extends \CUpdater implements \Serializable
{
	...
	{
		$dbConnection = \Thurly\Main\HttpApplication::getConnection();

		$agentName = '\Thurly\Sale\Location\Migration\CUpdaterLocationPro::updateDBSchemaRestoreLegacyIndexes();';
				...
				\CAdminNotify::Add(
					array(
						"MESSAGE" => Loc::getMessage('SALE_LOCATION_MIGRATION_PLZ_MIGRATE_NOTIFIER', array(
							'#ANCHOR_MIGRATE#' => '<a href="/thurly/admin/sale_location_migration.php">',
							'#ANCHOR_END#' => '</a>'
						)),
	...
	{
		/** @var  \Thurly\Sale\Location\Connector $class */
	...
	{
		$this->convertEntityLocationLinks('\Thurly\Sale\Location\GroupLocation');
	}

	public function convertDeliveryLocationLinks()
	{
		$this->convertEntityLocationLinks('\Thurly\Sale\Delivery\DeliveryLocation');
	}

	public function convertTaxRateLocationLinks()
	{
		$this->convertEntityLocationLinks('\Thurly\Sale\Tax\RateLocation');
	}
	...
	public static function createBaseTypes()
	{
		...
		$langs = array();
		$res = \Thurly\Main\Localization\LanguageTable::getList();
		while($item = $res->Fetch())
		{
			$MESS = array();
			@include($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/lang/'.$item['LID'].'/lib/location/migration/migrate.php');
		...
		foreach($types as $code => &$type)
		{
			...
			{
				$type['NAME'][$lid] = \Thurly\Sale\Location\Admin\NameHelper::getTranslatedName($type['NAME'], $lid);
			}
