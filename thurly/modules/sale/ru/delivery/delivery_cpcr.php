...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/delivery/delivery_cpcr.php');
	...
	function Calculate($profile, $arConfig, $arOrder, $STEP)
	{
		...
		else
		{
			...
			*/
			...
			$arProfiles = array(
				'SIMPLE' => '"ГЕПАРД-ЭКСПРЕСС"',
				'ECONOM' => '"ПЕЛИКАН-СТАНДАРТ"',
				'SIMPLE13' => '"ГЕПАРД-ЭКСПРЕСС 13"',
				'SIMPLE18' => '"ГЕПАРД-ЭКСПРЕСС 18"',
				'BIZON' => '"БИЗОН-КАРГО"',
				'COLIBRI' => '"КОЛИБРИ-ДОКУМЕНТ"',
				'PELICAN' => '"ПЕЛИКАН-ОНЛАЙН"',
				'FRAXT' => '"ФРАХТ"',
				);
			$arTmpResult = array();
	...
	public function getAdminMessage()
	{
		return array(
			'MESSAGE' => GetMessage(
				...
				array(
					'#A1#' => '<a href="/thurly/admin/sale_delivery_service_edit.php?lang='.LANGUAGE_ID.'&PARENT_ID=0&CLASS_NAME=%5CSale%5CHandlers%5CDelivery%5CSpsrHandler">',
					'#A2#' => '</a>'
				)
			),
			"TYPE" => "ERROR",
