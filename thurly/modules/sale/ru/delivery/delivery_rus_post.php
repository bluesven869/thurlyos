...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/delivery/delivery_rus_post.php');
...
class CDeliveryRusPost
{
	 ...
	 * using file /thurly/modules/sale/delivery/rus_post/zip_zones.csv created
	...
	private static function calculatePackPrice($arPackage, $profile, $arConfig, $arLocationTo)
	{
		...
		if(strlen($code) <= 0)
			throw new \Thurly\Main\SystemException(GetMessage("SALE_DH_RP_ERROR_LOCATION_NOT_FOUND"));

		$zoneTo = self::getConfValue($arConfig, 'REG_'.$code);
		$basePrice = floatval(self::getConfValue($arConfig, 'ZONE_RATE_MAIN_'.$zoneTo));

		if($basePrice <=0)
			throw new \Thurly\Main\SystemException(GetMessage("SALE_DH_RP_CALCULATE_ERROR"));
