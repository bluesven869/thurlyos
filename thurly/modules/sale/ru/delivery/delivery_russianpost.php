...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/delivery/delivery_russianpost.php');
...
class CDeliveryRUSSIANPOST
{
	function Init()
	{
		if (\Thurly\Main\Loader::includeModule('currency') && $arCurrency = CCurrency::GetByID('RUR'))
		{
			$base_currency = 'RUR';
		}
		else
		...
		return array(
			...
			"TRACKING_CLASS_NAME" => '\Thurly\Sale\Delivery\Tracking\RusPost',
	...
	protected static function sendRequestData($data, $method)
	{
		...
		$method = (DELIVERY_RUSSIANPOST_SERVER_METHOD == $method ? \Thurly\Main\Web\HttpClient::HTTP_GET : \Thurly\Main\Web\HttpClient::HTTP_POST);
		...
		if (!$reqResult && !empty($errors))
		{
			foreach($errors as $errorCode => $errMes)
				$result->addError(new \Thurly\Main\Entity\EntityError($errorCode.": ".$errMes));
		}
		else
		{
			...
			if ($status != 200)
			{
				$result->addError(new \Thurly\Main\Entity\EntityError('HTTP error code: %d', $status));
			}
			else
			{
	...
	public function getAdminMessage()
	{
		return array(
			'MESSAGE' => GetMessage(
				...
				array(
					'#A1#' => '<a href="/thurly/admin/sale_delivery_service_edit.php?lang='.LANGUAGE_ID.'&PARENT_ID=0&CLASS_NAME=%5CSale%5CHandlers%5CDelivery%5CAdditionalHandler&SERVICE_TYPE=RUSPOST">',
					'#A2#' => '</a>'
				)
			),
			"TYPE" => "ERROR",
