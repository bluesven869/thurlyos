...
if ($saleModulePermissions != "W")
{
	$allowedStatusesView = \Thurly\Sale\OrderStatus::getStatusesGroupCanDoOperations($userGroups, array('view'));
	...
	else
	{
		foreach($arStatus1 as $status_code => $arStatus)
		{
			...
			)
			{
				...
				$resOrder = \Thurly\Sale\Internals\OrderTable::getList(array(
					'select' => array(
						new \Thurly\Main\Entity\ExpressionField('SUM', 'SUM(%s)', 'PRICE'),
						new \Thurly\Main\Entity\ExpressionField('COUNT', 'COUNT(%s)', 'ID')
					),
					'filter' => $arFilter
				));
		...
		for($i = 0; $i < $countTab; $i++)
		{
			?><div id="<?=$aTabs[$i]["DIV"]?>_content" style="display: <?=($i==0 ? "block" : "none")?>;" class="bx-gadgets-tab-container"><?
				...
				elseif ($i == 1)
				{
					if (count($arOrders) > 0)
					{
						?><table class="bx-gadgets-table">
							<tbody>
								...
								{
									?><tr>
										<td align="left" style="vertical-align: top;"><a href="/thurly/admin/sale_order_view.php?ID=<?=$arOrder["ID"]?>&lang=<?=LANGUAGE_ID?>"><?=GetMessage("GD_ORDERS_ORDER_1")?><?=$arOrder["ID"]?></a><br><?=$arOrder["DATE_INSERT"]?></td>
										<td align="left" style="vertical-align: top;"><? echo htmlspecialcharsex(CurrencyFormat($arOrder["PRICE"], $arOrder["CURRENCY"])); ?></td>
										<td align="left" style="vertical-align: top;"><a href="/thurly/admin/user_edit.php?ID=<?=$arOrder["USER_ID"]?>&lang=<?=LANGUAGE_ID?>"><?=htmlspecialcharsbx($arOrder["USER_NAME"])." ".htmlspecialcharsbx($arOrder["USER_LAST_NAME"])?></a></td>
					...
					?><div class="bx-gadgets-content-padding-rl bx-gadgets-content-padding-t"><a href="/thurly/admin/sale_order.php?lang=<?=LANGUAGE_ID?>"><?=GetMessage("GD_ORDERS_ALL_ORDERS")?></a></div><?
				}
			?></div><?
		}
	?></div>
