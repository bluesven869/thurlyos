...
if ($install_public == "Y" && !empty($public_dir))
{
	...
	{
		CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/install/public/".$site['LANGUAGE_ID'], $site['ABS_DOC_ROOT'].$site["DIR"].$public_dir, $bReWritePublicFiles, true);

		if(!file_exists($_SERVER["DOCUMENT_ROOT"]."/personal/.left.menu.php"))
		{
			IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/install/public_menu.php", $site['LANGUAGE_ID']);
