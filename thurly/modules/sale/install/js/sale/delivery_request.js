...
(function(window) {
	...
	BX.Sale.Delivery.Request = {
		...
		ajaxUrl: "/thurly/admin/sale_delivery_request_ajax.php",
		timeout: 120,
		...
		showShipmentContent: function(requestId, shipmentId)
		{
			var storeForm = new BX.CDialog({
				...
				'content_post': 'requestId='+requestId+'&shipmentId='+shipmentId+"&action=getShipmentContent&lang="+BX.message('LANGUAGE_ID')+"&sessid="+BX.thurly_sessid(),
				'width': 500,
				'height':550
			});
				...
				onsuccess: function(result)
				{
					...
					if(result)
					{
						if(result.RESULT != 'ERROR')
						{
							...
							{
								window.location.href = BX.Sale.Delivery.Request.ajaxUrl+'?action=downloadFile&fileName='+encodeURI(result.FILE_NAME)+'&filePath='+encodeURI(result.FILE_PATH)+'&sessid='+BX.thurly_sessid();
							}
