...
if(strlen($result["ERROR"]) <= 0 && $saleModulePermissions >= "W" && check_thurly_sessid())
{
	...
	switch ($action)
	{
		case "locations_map":
			...
			if(get_parent_class($class) != 'Thurly\Sale\Location\Comparator\Mapper')
			{
				$result["ERROR"] = 'Error! Location class "'.$class.'" is not instance of \Thurly\Sale\Location\Comparator\Mapper!';
				break;
			}

			/** @var \Thurly\Sale\Location\Comparator\Mapper $locationMapper */
