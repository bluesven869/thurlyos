...
class CSaleDeliveryRequestComponent extends CThurlyComponent
{
	public function checkParams($params)
	{
		if(!isset($params["SHIPMENTS_ERRORS"]))
			throw new \Thurly\Main\ArgumentNullException('params["SHIPMENTS_ERRORS"]');

		if(!isset($params["DELIVERY_ID"]))
			throw new \Thurly\Main\ArgumentNullException('params["DELIVERY_ID"]');

		if(!isset($params["DELIVERY_REQUESTS"]))
			throw new \Thurly\Main\ArgumentNullException('params["DELIVERY_REQUESTS"]');

		if(!isset($params["SHIPMENTS_COUNT"]))
			throw new \Thurly\Main\ArgumentNullException('params["SHIPMENTS_COUNT"]');

		if(!isset($params["WEIGHT"]))
			throw new \Thurly\Main\ArgumentNullException('params["WEIGHT"]');

		if(!isset($params["ACTION"]))
			throw new \Thurly\Main\ArgumentNullException('params["ACTION"]');
