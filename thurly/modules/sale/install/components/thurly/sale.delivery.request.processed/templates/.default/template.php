...
<div class="admin-delivery-request-box admin-delivery-request-accept">
			...
			<div class="admin-delivery-request-confirm green">
				<span class="admin-delivery-request-confirm-text">
					<?if($arParams['ACTION'] == 'CREATE'):?>
						<?=Loc::getMessage('SALE_CSDRPT_DELIVERY_REQUEST_CREATED', array('#REQUEST_ID#' => '<a href="/thurly/admin/sale_delivery_request_view.php?ID='.$id.'&lang='.LANGUAGE_ID.'" title="'.Loc::getMessage('SALE_CSDRPT_DELIVERY_REQUEST_TITLE', array('#REQUEST_ID#' => $id)).'">'.$id.'</a>'))?>
					 <?else:?>
						<?=Loc::getMessage('SALE_CSDRPT_DELIVERY_REQUEST_UPD', array('#REQUEST_ID#' => '<a href="/thurly/admin/sale_delivery_request_view.php?ID='.$id.'&lang='.LANGUAGE_ID.'" title="'.Loc::getMessage('SALE_CSDRPT_DELIVERY_REQUEST_TITLE', array('#REQUEST_ID#' => $id)).'">'.$id.'</a>'))?>
					 <?endif;?>
				</span>
	...
	<?if($arParams['SHIPMENTS_ERRORS'] > 0):?>
		<div class="admin-delivery-request-confirm red">
			<span class="admin-delivery-request-confirm-text"><?=Loc::getMessage('SALE_CSDRPT_SHIPMENTS_FAIL_AMOUNT')?>: <a href="/thurly/admin/sale_order_shipment.php?lang=<?=LANGUAGE_ID?>&filter_delivery_id=<?=$arResult['DELIVERY']['ID']?>&filter_is_delivery_request_failed=Y&set_filter=Y" title="<?=Loc::getMessage('SALE_CSDRPT_SHIPMENTS_FAIL_AMOUNT_TITLE')?>"><?=$arParams['SHIPMENTS_ERRORS']?></a></span>
		</div>
	<?endif;?>
