...
if ($_SERVER["REQUEST_METHOD"] == "POST" && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_thurly_sessid()))
{
...
if ($_REQUEST["CurrentStep"] == 7 || ($_SERVER["REQUEST_METHOD"] == "POST" && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_thurly_sessid())))
{
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && strlen($_REQUEST["backButton"]) > 0 && ($arParams["DELIVERY_NO_SESSION"] == "N" || check_thurly_sessid()))
{
...
if (!$USER->IsAuthorized())
{
	$arResult["USER_LOGIN"] = ((strlen($arResult["POST"]["USER_LOGIN"]) > 0) ? $arResult["POST"]["USER_LOGIN"] : htmlspecialcharsbx(${COption::GetOptionString("main", "cookie_name", "THURLY_SM")."_LOGIN"}));
	...
	if ($arResult["CurrentStep"] > 0 && $arResult["CurrentStep"] <= 6)
	{
		...
		if (strlen($arResult["ERROR_MESSAGE"]) <= 0 && $arResult["CurrentStep"] > 5)
		{
			...
			if (strlen($arResult["ERROR_MESSAGE"]) <= 0)
			{
				...
				\Thurly\Sale\Notify::setNotifyDisable(true);
			...
			\Thurly\Sale\Notify::setNotifyDisable(false);
			if (strlen($arResult["ERROR_MESSAGE"]) <= 0)
			{
				...
				while ($arUserPropsValues = $dbUserPropsValues->GetNext())
				{
					...
					elseif ($arUserPropsValues["PROP_TYPE"] == "LOCATION")
					{
						if ($arLocation = CSaleLocation::GetByID($arUserPropsValues["VALUE"], LANGUAGE_ID))
						{
							...
							{
								$locationName = \Thurly\Sale\Location\Admin\LocationHelper::getLocationStringById($arLocation['ID']);
							}
							else
							{
								/*
		...
		while ($arProperties = $dbProperties->GetNext())
		{
			...
			elseif ($arProperties["TYPE"] == "LOCATION")
			{
				...
				*/
				...
				{
					if(intval($arVal['ID']))
						$locationName = \Thurly\Sale\Location\Admin\LocationHelper::getLocationStringById($arVal['ID']);
				}
				else
				{
