...
use Thurly\Main,
	Thurly\Main\Config,
	Thurly\Main\Localization,
	Thurly\Highloadblock as HL,
	Thurly\Main\Loader,
	Thurly\Sale,
	Thurly\Iblock,
	Thurly\Main\Data,
	Thurly\Sale\Location,
	Thurly\Sale\Cashbox\CheckManager;

if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die();

class CThurlyPersonalOrderDetailComponent extends CThurlyComponent
{
	...
	protected function checkAuthorized()
	{
		...
		if ($access = $request->get('access'))
		{
			...
			if (
				...
				\Thurly\Sale\Helpers\Order::isAllowGuestView($this->order)
			)
			{
	...
	protected function obtainDeliveryStore(&$cached)
	{
		...
		foreach ($this->dbResult['SHIPMENT'] as $shipment)
		{
			...
			if (!empty($shipment["DELIVERY"]) && count($shipment["DELIVERY"]["STORE"]) > 0 && $this->useCatalog)
			{
				...
				$resultStore = Thurly\Catalog\StoreTable::getList(
					array(
						'order' => array(
							"SORT" => "DESC",
							"ID" => "DESC"),
	...
	protected function obtainDataOrder()
	{
		...
		foreach ($shipmentCollection as $shipment)
		{
			...
			/** @var \Thurly\Sale\ShipmentItem $item */
	...
	protected function obtainDataPaySystem()
	{
		...
		foreach ($this->dbResult['PAYMENT'] as &$payment)
		{
			...
			{
				$payment["PAY_SYSTEM"] = \Thurly\Sale\PaySystem\Manager::getById($payment["PAY_SYSTEM_ID"]);
			...
			if ($payment["PAID"] != "Y" && $this->dbResult["CANCELED"] != "Y" &&  $this->dbResult["ALLOW_PAY"] != "N")
			{
				...
				$payment['ERROR'] = '';
				$service = new \Thurly\Sale\PaySystem\Service($payment["PAY_SYSTEM"]);
				if ($service)
				{
					$payment["CAN_REPAY"] = "Y";
	...
	protected function adaptCachedReferences($cached)
	{
		...
		foreach ($this->dbResult['SHIPMENT'] as $shipment)
		{
			...
			$shipment['DELIVERY']['STORE'] = \Thurly\Sale\Delivery\ExtraServices\Manager::getStoresList($shipment["DELIVERY_ID"]);
	...
	protected function obtainDataReferences()
	{
		if ($this->startCache(array('spo-shared')))
		{
			try
			{
				...
				$dbDelivery = \Thurly\Sale\Delivery\Services\Table::getList();
	...
	protected function formatResultDeliverySystem()
	{
		...
		foreach ($arResult['SHIPMENT'] as &$shipment)
		{
			if (!empty($shipment["DELIVERY_ID"]))
			{
				...
				{
					$shipment["DELIVERY"]["SRC_LOGOTIP"] = '/thurly/images/sale/logo-default-d.gif';
				}
			}
		...
		if(self::isNonemptyArray($arResult['BASKET']))
		{
			foreach ($arResult["BASKET"] as $k => $arBasket)
			{
				...
				if (doubleval($arBasket["DISCOUNT_PRICE"]))
				{
					...
					$arBasket["DISCOUNT_PRICE_PERCENT_FORMATED"] = Thurly\Sale\PriceMaths::roundPrecision($arBasket["DISCOUNT_PRICE_PERCENT"])."%";
