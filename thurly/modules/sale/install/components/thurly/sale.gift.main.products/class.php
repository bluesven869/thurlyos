...
use Thurly\Sale\Internals\DiscountTable;

if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die();

class CSaleGiftMainProductsComponent extends CThurlyComponent
{
	...
	private function getSectionIdByElement($elementId, $iblockId)
	{
		...
		$itemIterator = \Thurly\Iblock\ElementTable::getList(array(
	...
	private function getDataMainProducts()
	{
		...
		$query = new \Thurly\Main\Entity\Query(\Thurly\Sale\Discount\Gift\RelatedDataTable::getEntity());
		...
		while($row = $dbResult->fetch())
		{
			...
			list($productElementIds, $productSectionIds) = Thurly\Sale\Discount\Gift\RelatedDataTable::getProductsData($discounts[$row['ID']]);
	...
	private function getState($elementIds, $sectionIds)
	{
		sort($elementIds);
		sort($sectionIds);
		$sign = new \Thurly\Main\Security\Sign\Signer;
