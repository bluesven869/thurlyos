...
// obtain cached data
$cacheNeeded = intval($arParams['CACHE_TIME']) > 0 && $arParams['CACHE_TYPE'] != 'N' && \Thurly\Main\Config\Option::get("main", "component_cache_on", "Y") == "Y";
$cachedData = array();
...
if($cacheNeeded)
{
	...
	if($currentCache->startDataCache(intval($arParams['CACHE_TIME']), implode('|', array_merge($arParams, array(SITE_ID, LANGUAGE_ID))), $cacheDir))
	{
		...
		$CACHE_MANAGER->RegisterTag(\Thurly\Sale\Location\Admin\LocationHelper::LOCATION_LINK_DATA_CACHE_TAG);
