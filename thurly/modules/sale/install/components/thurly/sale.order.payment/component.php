...
if ($arOrder)
{
	/** @var \Thurly\Sale\Payment|null $paymentItem */
	$paymentItem = null;
	...
	if ($order)
	{
		...
		if (!empty($hash) && ($order->getHash() !== $hash || !\Thurly\Sale\Helpers\Order::isAllowGuestView($order)))
		{
			LocalRedirect('/');
			return;
		}
		...
		if ($paymentCollection)
		{
			if ($paymentId)
			{
				$data = \Thurly\Sale\PaySystem\Manager::getIdsByPayment($paymentId);
			...
			if ($paymentItem !== null)
			{
				$service = \Thurly\Sale\PaySystem\Manager::getObjectById($paymentItem->getPaymentSystemId());
				if ($service)
				{
