...
class PersonalProfileList extends CThurlyComponent
{
	...
	public function executeComponent()
	{
		...
		if ($deleteElementId > 0 && check_thurly_sessid())
		{
		...
		while($arUserProps = $dbUserProps->GetNext())
		{
			...
			$personTypeList = Thurly\Sale\PersonType::load(SITE_ID, $arUserProps["PERSON_TYPE_ID"]);
			...
			{
				$arResultTmp["URL_TO_DETELE"] = htmlspecialcharsbx($APPLICATION->GetCurPage()."?del_id=".$arUserProps["ID"]."&".thurly_sessid_get());
			}
			else
			{
				$arResultTmp["URL_TO_DETELE"] = CComponentEngine::MakePathFromTemplate($this->arParams["PATH_TO_DELETE"], Array("ID" => $arUserProps["ID"]))."&".thurly_sessid_get();
			}
