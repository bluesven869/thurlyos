...
if(isset($arResult["ERROR"]) <= 0 && $USER->IsAdmin() && check_thurly_sessid())
{
	...
	switch ($action)
	{
		...
		case "get_variation_values":
			...
			$variationRes = Thurly\Sale\TradingPlatform\Ebay\CategoryVariationTable::getById($variationId);
		...
		case "set_category_property_link":
			...
			$thurlyCategoryId = isset($_REQUEST['thurlyCategoryId']) ? trim($_REQUEST['thurlyCategoryId']): '';
			$properyId = isset($_REQUEST['properyId']) ? trim($_REQUEST['properyId']): '';

			CIBlockSectionPropertyLink::Add($thurlyCategoryId, $properyId);
			break;
		...
		case "delete_category_map":
			...
			$thurlyCategoryId = isset($_REQUEST['thurlyCategoryId']) ? intval($_REQUEST['thurlyCategoryId']) : 0;
