...
class CEbayCategoriesLink extends CThurlyComponent
{
	public function onPrepareComponentParams($arParams)
	{
		$requiredParams = array( "CATEGORY_INPUT_NAME", "TOP_CATEGORY_SELECT_NAME",  "THURLY_CATEGORY_ID", "IBLOCK_ID", "THURLY_CATEGORY_PROPS_SN");
	...
	protected function getMappedEbayCategoryId($thurlyCategoryId)
	{
		$catMapRes = \Thurly\Sale\TradingPlatform\MapTable::getList(array(
			"filter" => array(
				"ENTITY_ID" => \Thurly\Sale\TradingPlatform\Ebay\MapHelper::getCategoryEntityId($this->arParams["IBLOCK_ID"]),
	...
	{
		$categoriesRes = \Thurly\Sale\TradingPlatform\Ebay\CategoryTable::getList( array(
	...
	{
		$categories = Thurly\Sale\TradingPlatform\Ebay\CategoryTable::getCategoryParents($categoryId);
	...
	protected function getVariationsValues($iblockId, $ebayCategoryId, $ebayCategoryVariations)
	{
		...
		$catVarEntId = \Thurly\Sale\TradingPlatform\Ebay\MapHelper::getCategoryVariationEntityId($iblockId, $ebayCategoryId);

		$catMapVarRes = \Thurly\Sale\TradingPlatform\MapTable::getList(array(
			"filter" => array(
	...
	public function executeComponent()
	{
		...
		$this->arResult["THURLY_CATEGORY_ID"] = isset($this->arParams["THURLY_CATEGORY_ID"]) ? intval($this->arParams["THURLY_CATEGORY_ID"]) : 0;
		...
		if(isset($this->arParams["EBAY_CATEGORY_ID"]))
			...
			$this->arResult["EBAY_CATEGORY_ID"] = $this->getMappedEbayCategoryId($this->arResult["THURLY_CATEGORY_ID"]);
		...
		if(strlen($this->arResult["EBAY_CATEGORY_ID"]) > 0)
		{
			...
			$this->arResult["EBAY_CATEGORY_VARIATIONS"] = \Thurly\Sale\TradingPlatform\Ebay\Helper::getEbayCategoryVariations($this->arResult["EBAY_CATEGORY_ID"], $this->arResult["SITE_ID"]);
		...
		$this->arResult["CATEGORY_PROPS"] = \CIBlockSectionPropertyLink::GetArray($this->arParams["IBLOCK_ID"], $this->arParams["THURLY_CATEGORY_ID"]);
		...
		if(is_array($arOffers) && !empty($arOffers))
		{
			...
			$this->arResult["CATEGORY_OFFERS_PROPS"] = \CIBlockSectionPropertyLink::GetArray($arOffers["IBLOCK_ID"], $this->arParams["THURLY_CATEGORY_ID"]);
