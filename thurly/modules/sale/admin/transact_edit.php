...
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $saleModulePermissions >= "U" && check_thurly_sessid())
{
	...
	if (strlen($errorMessage) <= 0)
		LocalRedirect("/thurly/admin/sale_transact_admin.php?lang=".LANG.GetFilterParams("filter_", false));
	else
		$bVarsFromForm = true;
}
...
$aMenu = array(
		array(
				...
				"LINK" => "/thurly/admin/sale_transact_admin.php?lang=".LANG.GetFilterParams("filter_"),
				"ICON" => "btn_list"
			)
	);
...
?>
	<tr class="adm-detail-required-field">
		...
		<td width="60%"><?
			$user_name = "";
			if ($ID > 0)
				$user_name = "[<a title=\"".GetMessage("STE_USER_PROFILE")."\" href=\"/thurly/admin/user_edit.php?lang=".LANGUAGE_ID."&ID=".$str_USER_ID."\">".$str_USER_ID."</a>] (".$str_USER_LOGIN.") ".$str_USER_NAME." ".$str_USER_LAST_NAME;
...
<input type="submit" name="save" value="<?= GetMessage("STE_SAVE") ?>" <?if ($saleModulePermissions < "U") echo "disabled" ?>>&nbsp;<input type="button" value="<?echo GetMessage("STE_CANCEL1")?>" OnClick="window.location='/thurly/admin/sale_transact_admin.php?lang=<?= LANG ?><?= GetFilterParams("filter_") ?>'">
