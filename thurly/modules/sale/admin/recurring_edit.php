...
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $saleModulePermissions >= "U" && check_thurly_sessid())
{
	...
	if (strlen($errorMessage) <= 0)
	{
		...
		{
			if (strlen($apply)<=0)
				LocalRedirect("/thurly/admin/sale_recurring_admin.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
		}
	}
	else
	{
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/sale_recurring_admin.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	)
);
...
if ($ID > 0 && $saleModulePermissions >= "U")
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_new",
		"LINK" => "/thurly/admin/sale_recurring_edit.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	);
	...
	if ($saleModulePermissions >= "W")
	{
		$aMenu[] = array(
			...
			"ICON" => "btn_delete",
			"LINK" => "javascript:if(confirm('".GetMessageJS("SREN_DELETE_RECURR_CONFIRM")."')) window.location='/thurly/admin/sale_recurring_admin.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."#tb';",
			"WARNING" => "Y"
		);
	}
}
	...
	<tr class="adm-detail-required-field">
		...
		<td width="60%"><?
			...
			if ($ID>0 && $str_USER_ID>0)
				$user_name = "[<a title=\"".GetMessage("SRE_USER_PROFILE")."\" href=\"/thurly/admin/user_edit.php?lang=".LANGUAGE_ID."&ID=".$str_USER_ID."\">".$str_USER_ID."</a>] (".$str_USER_LOGIN.") ".$str_USER_NAME." ".$str_USER_LAST_NAME;
			...
			<?if ($saleModulePermissions >= "W"):?>
				...
				function makeProductSearchDialog(params)
				{
					...
					var popup = new BX.CDialog({
						content_url: '/thurly/tools/sale/product_search_dialog.php?lang='+lang+'&LID='+site_id+'&caller=' + caller + '&func_name='+callback+'&subscribe=Y&STORE_FROM_ID='+store_id,
...
$tabControl->Buttons(
	array(
		...
		"back_url" => "/thurly/admin/sale_recurring_admin.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	)
);
