...
function canShowDiscountInCatalog(array $discount)
{
	...
	)
	{
		...
		if (!$actionConfiguration ||
			$actionConfiguration['VALUE_TYPE'] === \Thurly\Sale\Discount\Actions::VALUE_TYPE_SUMM)
		{
			return false;
		}
...
if ($selectFieldsMap['CREATED_BY'] || $selectFieldsMap['MODIFIED_BY'])
{
	if (!empty($arUserID))
	{
		...
		while ($arOneUser = $userIterator->fetch())
		{
			...
			$userList[$arOneUser['ID']] = '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$arOneUser['ID'].'">'.CUser::FormatName($nameFormat, $arOneUser).'</a>';
		}
