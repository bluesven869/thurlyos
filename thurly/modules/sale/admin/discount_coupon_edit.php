...
if (!$subWindow && !$readOnly && $couponID > 0)
{
	if (!$copy)
	{
		...
		$contextMenuItems[] = array(
			...
			'LINK' => "javascript:if(confirm('".CUtil::JSEscape(Loc::getMessage('BX_SALE_DISCOUNT_COUPON_CONTEXT_DELETE_CONFIRM'))."')) window.location='/thurly/admin/sale_discount_coupons.php?lang=".LANGUAGE_ID."&ID=".$couponID."&action=delete&".thurly_sessid_get()."';",
			'WARNING' => 'Y',
		);
	}
}
...
BX.ready(function(){
	...
	if (!!obCouponValue && !!obCouponBtn)
	{
		BX.bind(obCouponBtn, 'click', function(){
			...
			BX.showWait();
			url = '/thurly/tools/sale/generate_coupon.php';
			data = {
