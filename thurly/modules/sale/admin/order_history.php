...
if (!isset($saleOrder) || !($saleOrder instanceof \Thurly\Sale\Order))
...
{
	$arFilterHistory['@TYPE'] = \Thurly\Sale\OrderHistory::getManagerLogItems();
}
...
while ($arChangeRecord = $dbRecords->Fetch())
{
	...
	$datetime = new \Thurly\Main\Type\DateTime($arChangeRecord["DATE_CREATE"]);
	$datetime->format(\Thurly\Main\Type\DateTime::getFormat());
...
$oFilter->Buttons(
	array(
		...
		"url" => $APPLICATION->GetCurPageParam(), //"/thurly/admin/sale_order_history.php?lang=".LANGUAGE_ID,
