...
if($arResult["ERROR"] === '' && $saleModulePermissions >= "W" && check_thurly_sessid())
{
	...
	switch ($action)
	{
		...
		case "save_restriction":
			...
			if(!$cashboxId)
				throw new \Thurly\Main\ArgumentNullException("cashboxId");
		...
		case "reload_settings":
			...
			{
				ob_start();
				require_once($_SERVER['DOCUMENT_ROOT']."/thurly/modules/sale/admin/cashbox_settings.php");
		...
		case "reload_ofd_settings":
			...
			ob_start();
			require_once($_SERVER['DOCUMENT_ROOT']."/thurly/modules/sale/admin/cashbox_ofd_settings.php");
			$arResult["HTML"] = ob_get_contents();
			ob_end_clean();
		...
		case "get_order_entities":
			...
			if ($orderId > 0)
			{
				...
				$userId = $USER->GetID();
				$userCompanyList = \Thurly\Sale\Services\Company\Manager::getUserCompanyList($userId);
...
function getRestrictionHtml($cashboxId)
{
	...
	ob_start();
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/admin/cashbox_restrictions_list.php");
