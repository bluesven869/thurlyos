...
use Thurly\Sale\DiscountCouponsManager;
use Thurly\Sale;
...
if ($crmMode)
{
	...
	echo '<link rel="stylesheet" type="text/css" href="/thurly/themes/.default/sale.css" />';
}
...
if (CSaleLocation::isLocationProEnabled())
{
	if (isset($_REQUEST['ACT']))
	{
		...
		elseif($_REQUEST['ACT'] == 'GET_ZIP_BY_LOC')
		{
			...
			{
				$item = \Thurly\Sale\Location\LocationTable::getList(
					array(
						'filter' => array(
	...
	&& empty($_POST['dontsave']))
{
	$ID = intval($ID);
	$recalcOrder = "N";
	$isOrderConverted = \Thurly\Main\Config\Option::get("main", "~sale_converted_15", 'Y');
...
//							}
	...
	if ('' == $errorMessage AND $ID > 0)
	{
		...
		{
			CSaleOrder::UnLock($ID);
			LocalRedirect("/thurly/admin/sale_order.php?lang=".LANGUAGE_ID."&LID=".urlencode($LID).GetFilterParams("filter_", false));
		}

		if (isset($apply) AND strlen($apply) > 0)
			LocalRedirect("/thurly/admin/sale_order_new.php?lang=".LANGUAGE_ID."&ID=".$ID."&LID=".urlencode($LID).GetFilterParams("filter_", false));
	}
	if ('' != $errorMessage)
		$bVarsFromForm = true;
}
...
if (!empty($dontsave))
{
	...
	LocalRedirect("/thurly/admin/sale_order.php?lang=".LANGUAGE_ID."&LID=".CUtil::JSEscape($LID));
}
...
)
{
	...
	*/
	if (isset($userId) && isset($buyerType) && (!isset($profileDefault) || $profileDefault == ""))
	{
		...
		$viewedIterator = \Thurly\Catalog\CatalogViewedProductTable::getList(
			array(
	...
	if (isset($userId) AND isset($buyerType) AND isset($profileDefault))
	{
		...
		if (strlen($strPropsList)>0)
		{
			...
			<script type="text/javascript">
				...
				{
					BX.ajax.post('/thurly/admin/sale_order_new.php', '<?=thurly_sessid_get()?>&ORDER_AJAX=Y&locid=' + locationID + '&propID=<?=$buyerType?>&LID=<?=CUtil::JSEscape($LID)?>&location=' + arProps[locationID], fLocationResult);
				}
				else
		...
		<div id="select_lid">
			<form action="" name="select_lid">
				...
				<script type="text/javascript">
					...
					{
						BX.showWait();
						BX.ajax.post("/thurly/admin/sale_order_new.php", "<?=thurly_sessid_get()?>&ORDER_AJAX=Y&lang=<?=LANGUAGE_ID?>&LID=" + el.value, fLidChangeResult);
					}
...
if ($boolLocked)
{
	...
	if ($arOneUser = $rsUsers->Fetch())
	{
		...
		$strLockUserInfo = '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$intLockUserID.'">'.$strLockUser.'</a>';
	}
...
if (0 < $ID)
{
	$aMenu = array(
		array(
			...
			"LINK" => "/thurly/admin/sale_order_new.php?lang=".LANGUAGE_ID."&ID=".$ID."&dontsave=Y&LID=".CUtil::JSEscape($LID).GetFilterParams("filter_")
		)
	);
}
else
{
	$aMenu = array(
		array(
			"ICON" => "btn_list",
			"TEXT" => GetMessage("SOE_TO_LIST"),
			"LINK" => "/thurly/admin/sale_order.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
		)
	);
}
...
if ($boolLocked && $saleModulePermissions >= 'W')
{
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/sale_order_new.php?ID=".$ID."&unlock=Y&lang=".LANGUAGE_ID.GetFilterParams("filter_"),
	);
}
...
if ($bUserCanViewOrder && $ID > 0)
{
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/sale_order_view.php?ID=".$ID."&lang=".LANGUAGE_ID.GetFilterParams("filter_")
	);
}
...
if ($ID > 0)
{
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/sale_order_print.php?ID=".$ID."&lang=".LANGUAGE_ID.GetFilterParams("filter_")
	);
}
if (!$boolLocked && ($saleModulePermissions == "W" || $str_PAYED["PAYED"] != "Y" && $bUserCanDeleteOrder) && 0 < $ID)
{
	$aMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".GetMessageJS("NEWO_CONFIRM_DEL_MESSAGE")."')) window.location='sale_order.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get().urlencode(GetFilterParams("filter_"))."'",
		"WARNING" => "Y"
	);
}
...
function getLocation(country_id, region_id, city_id, arParams, site_id, admin_section)
{
	...
	var url = '/thurly/components/thurly/sale.ajax.locations/templates/.default/ajax.php';
	...
	<td id="buyer_type_change" colspan="2">
		...
		function fButtonCurrent(el)
		{
			...
			else if (el == 'btnBuyerExist' || el == 'btnBuyerExistRemote')
			{
				...
				if (el == 'btnBuyerExist')
					window.open('/thurly/admin/user_search.php?lang=<? echo LANGUAGE_ID; ?>&FN=order_edit_info_form&FC=user_id', '', 'scrollbars=yes,resizable=yes,width=840,height=500,top='+Math.floor((screen.height - 560)/2-14)+',left='+Math.floor((screen.width - 840)/2-5));
			}
		}
		...
		function fBuyerChangeType(el)
		{
			...
			BX.showWait();
			BX.ajax.post('/thurly/admin/sale_order_new.php', '<?=thurly_sessid_get()?>&ORDER_AJAX=Y&paysystemid=' + orderPaySystemID + '&ID=' + orderID + '&LID=<?=CUtil::JSEscape($LID)?>&buyertypechange=' + el.value + '&userId=' + userId, fBuyerChangeTypeResult);
		}
		...
		function fUserGetProfile(el)
		{
			...
			if (userId != "" && buyerType != "")
			{
				...
				BX.ajax.post(
					'/thurly/admin/sale_order_new.php',
					postData,
		...
		{
			BX.ajax({
				url: '/thurly/admin/sale_order_new.php',
				method: 'POST',
				data : '<?=thurly_sessid_get()?>&ORDER_AJAX=Y&LID=<?=CUtil::JSEscape($LID)?>&userId=' + userId + '&buyerType=' + buyerType + '&profileDefault=' + profileDefault,
				dataType: 'html',
				timeout: 30,
				async: true,
				processData: true,
...
<script type="text/javascript">
	...
	function getParamsByProductId(arParams, iblockID)
	{
		var productId = arParams['id'],
			...
			dateURL = '<?=thurly_sessid_get()?>&ORDER_AJAX=Y&get_product_params=Y&ID=<?=$ID?>&LID=<?=CUtil::JSEscape($LID)?>&productId=' + productId + '&quantity=' + productQuantity + '&userId=' + userId + '&userColumns=' + strUserColumns;
...
//			prOld = parseFloat(BX('PRODUCT[' + i + '][PRICE_DEFAULT]').value);
		...
		dateURL = '<?=thurly_sessid_get()?>&ORDER_AJAX=Y'
			+ '&id=<?=$ID?>'
		...
		BX.showWait();
		BX.ajax.post('/thurly/admin/sale_order_new.php', dateURL, fRecalProductResult);
	}
	...
	function fGetRelatedOrderProps()
	{
		...
		BX.ajax({
			url: '/thurly/admin/sale_order_new.php',
			method: 'POST',
			data : '<?=thurly_sessid_get()?>&ORDER_AJAX=Y&get_props=Y&id=<?=$ID?>&userId=' + userId + '&delivery_id=' + deliveryId + '&paysystem_id=' + paymentId + '&CURRENCY=<?=$str_CURRENCY?>&LID=<?=CUtil::JSEscape($LID)?>',
			dataType: 'html',
			timeout: 30,
			async: true,
			processData: true,
	...
	*/
	function fGetMoreBasket(showAll)
	{
		...
		BX.ajax.post('/thurly/admin/sale_order_new.php', '<?=thurly_sessid_get()?>&ORDER_AJAX=Y&showAll='+showAll+'&arProduct='+productData+'&getmorebasket=Y&CURRENCY=<?=$str_CURRENCY?>&LID=<?=CUtil::JSEscape($LID)?>&userId=' + userId, fGetMoreBasketResult);
	}
	...
	*/
	function fGetMoreViewed(showAll) {
		...
		BX.ajax.post('/thurly/admin/sale_order_new.php', '<?=thurly_sessid_get()?>&ORDER_AJAX=Y&showAll='+showAll+'&arProduct='+productData+'&getmoreviewed=Y&CURRENCY=<?=$str_CURRENCY?>&LID=<?=CUtil::JSEscape($LID)?>&userId=' + userId, fGetMoreViewedResult);
	}
	...
	*/
	function fAddToBasketMoreProduct(type, productId)
	{
		...
		BX.ajax.post(
			'/thurly/admin/sale_order_new.php',
			'<?=thurly_sessid_get()?>&ORDER_AJAX=Y&get_product_params=Y&ID=<?=$ID?>&LID=<?=CUtil::JSEscape($LID)?>&type=' + type + '&productId=' + productId + '&userId=' + userId + '&userColumns=' + strUserColumns,
	...
	function fCheckBarcode(basketItemId, storeId, isMultiBarcode, barcodeId)
	{
		...
		else
		{
			if (isNewProduct)
			{
				...
				BX.ajax.post(
					'/thurly/admin/sale_order_new.php',
					'<?=thurly_sessid_get()?>&ORDER_AJAX=Y&id=<?=$ID?>&LID=<?=CUtil::JSEscape($LID)?>&checkBarcode=Y&productId=' + productId + '&barcode=' + barcode.value + '&storeId=' + realStoreId + '&productProvider=' + productProvider + '&moduleName=' + moduleName + '&barcodeMulti=' + barcodeMulti,
					function (res)
					{
						var result = eval( '('+res+')' );
			...
			else
			{
				...
				BX.ajax.post(
					'/thurly/admin/sale_order_new.php',
					'<?=thurly_sessid_get()?>&ORDER_AJAX=Y&id=<?=$ID?>&LID=<?=CUtil::JSEscape($LID)?>&checkBarcode=Y&basketItemId=' + basketItemId + '&barcode=' + barcode.value + '&storeId=' + realStoreId,
					function (res)
					{
						var result = eval( '('+res+')' );
		...
		window['orderNewTableSettings'].SetButtons([
			{
				...
				'action': function() {
					...
					BX.showWait();
					BX.ajax.post('/thurly/admin/sale_order_new.php', '<?=thurly_sessid_get()?>&ORDER_AJAX=Y&change_columns=Y&cols=' + sCols + '&ids=' + IDs + '&ID=<?=$ID?>', changeColumns);
...
if (!defined('BX_PUBLIC_MODE') || BX_PUBLIC_MODE != 1)
{
	$tabControl->Buttons(
		array(
			...
			"back_url" => "/thurly/admin/sale_order_new.php?lang=".LANGUAGE_ID."&ID=".$ID."&dontsave=Y&LID=".CUtil::JSEscape($LID).GetFilterParams("filter_"))
	);
}
			...
			if (arSKU[i]["ID"] == selectedSkuId)
			{
				...
				if (arSKU[i]["CAN_BUY"] == "Y")
				{
					...
					else
					{
						...
						BX('popup-params-product').value = "/thurly/admin/sale_order_new.php?lang=<?=LANGUAGE_ID?>&user_id="+arSKU[i]["USER_ID"]+"&LID="+arSKU[i]["LID"]+"&product["+arSKU[i]["ID"]+"]=1";
					}
				}
				else
				{
