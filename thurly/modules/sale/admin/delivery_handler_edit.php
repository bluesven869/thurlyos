...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_REQUEST["Update"]) && check_thurly_sessid())
{
	...
	if ($arHandlersData)
	{
		...
		if (strlen($errorsList) <= 0)
		{
			if (strlen($_REQUEST["apply"]) > 0)
				...
				LocalRedirect('/thurly/admin/sale_delivery_handlers.php?lang='.LANG);
...
$parentTabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/sale_delivery_handlers.php?lang=".LANG,
			)
	);
