...
if(!isset($settings[$SITE_ID]))
	LocalRedirect("/thurly/admin/sale_ebay_general.php?lang=".LANG."&SITE_ID=".$SITE_ID."&back_url=".urlencode($APPLICATION->GetCurPageParam()));

$siteSettings = $settings[$SITE_ID];
$details = new \Thurly\Sale\TradingPlatform\Ebay\Api\Details($SITE_ID);
unset ($settings);
...
if(isset($siteSettings["API"]["AUTH_TOKEN"]) && strlen($siteSettings["API"]["AUTH_TOKEN"]) > 0)
	$policy = new \Thurly\Sale\TradingPlatform\Ebay\Policy($siteSettings["API"]["AUTH_TOKEN"], $SITE_ID);
