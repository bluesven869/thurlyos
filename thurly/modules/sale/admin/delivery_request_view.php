...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/sale_delivery_request_list.php?lang=".LANGUAGE_ID,
		"ICON" => "btn_list"
	)
);
...
if ($ID > 0)
{
	...
	if($deliveryRequest)
	{
		...
		$rTypesMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('".Loc::getMessage('SALE_DELIVERY_REQ_VIEW_DEL_CONFIRM')."')){ window.location=\"/thurly/admin/sale_delivery_request_list.php?lang=".LANGUAGE_ID."&action=delete&ID=".$ID."&".thurly_sessid_get()."\"};"
		);
...
?>
	...
	<tr>
		...
		<td>
			<a href="/thurly/admin/sale_delivery_service_edit.php?lang=ru&ID=<?=$fields["DELIVERY_ID"]?>"><?=htmlspecialcharsbx($deliveryName)?></a>
		</td>
	</tr>
<!--
	<tr>
...
<?$tabControl->BeginNextTab();
	?><tr><td><?
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/admin/delivery_request_shipment.php");
	?></td></tr>
<?$tabControl->End();?>
</form>
