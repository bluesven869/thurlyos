...
function GetRealPath2Report($rep_name)
{
	...
	$rep_file_name = $_SERVER["DOCUMENT_ROOT"]."/thurly/admin/reports/".$rep_name;
	if (!file_exists($rep_file_name))
	{
		$rep_file_name = $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/reports/".$rep_name;
		if (!file_exists($rep_file_name))
		{
			return "";
		}
...
if (CModule::IncludeModule("sale"))
{
	...
	{
		$order = \Thurly\Sale\Order::load($ORDER_ID);
		...
		if (isset($_REQUEST['SHIPMENT_ID']) && intval($_REQUEST['SHIPMENT_ID']) > 0)
		{
			...
			{
				ShowError(\Thurly\Main\Localization\Loc::getMessage("SALE_PRINT_DENIED_PRINT_PERMISSION"));
				exit;
			}
		...
		$shipmentRes = \Thurly\Sale\Shipment::getList(array(
		...
		else
		{
			...
			$db_basket = \Thurly\Sale\Internals\BasketTable::getList($params);
