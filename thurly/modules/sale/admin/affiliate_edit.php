...
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $saleModulePermissions>="W" && check_thurly_sessid())
{
	if (StrLen($SITE_ID) <= 0)
	...
	{
		if (strlen($apply) <= 0)
			LocalRedirect("/thurly/admin/sale_affiliate.php?lang=".LANG.GetFilterParams("filter_", false));
		else
			LocalRedirect("/thurly/admin/sale_affiliate_edit.php?lang=".LANG."&ID=".$ID.GetFilterParams("filter_", false));
	}
	else
	{
		$bVarsFromForm = true;
...
$aMenu = array(
		array(
				...
				"LINK" => "/thurly/admin/sale_affiliate.php?lang=".LANG.GetFilterParams("filter_"),
				"ICON" => "btn_list"
			)
	);
...
if ($ID > 0)
{
	...
	$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/sale_affiliate_edit.php?lang=".LANG.GetFilterParams("filter_"),
			"ICON" => "btn_new"
		);
	...
	if ($saleModulePermissions >= "W")
	{
		$aMenu[] = array(
				...
				"LINK" => "javascript:if(confirm('".GetMessage("SAE_AFF_DELETE_CONF")."')) window.location='/thurly/admin/sale_affiliate.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."#tb';",
				"WARNING" => "Y",
				"ICON" => "btn_delete"
			);
	}
...
?>
		...
		<td>
			...
			if ($str_USER_ID > 0)
			{
				...
				if ($arUser = $dbUser->Fetch())
					$userName = "[<a class=\"tablebodylink\" title=\"".GetMessage("SAE_PROFILE")."\" href=\"/thurly/admin/user_edit.php?lang=".LANGUAGE_ID."&ID=".$str_USER_ID."\">".$str_USER_ID."</a>] (".htmlspecialcharsex($arUser["LOGIN"]).") ".htmlspecialcharsex($arUser["NAME"])." ".htmlspecialcharsex($arUser["LAST_NAME"]);
			}
	...
	<tr>
		...
		<td valign="top">
			...
			<input type="button" class="button" name="FindAffiliate" OnClick="window.open('/thurly/admin/sale_affiliate_search.php?func_name=SetAffiliateID', '', 'scrollbars=yes,resizable=yes,width=600,height=500,top='+Math.floor((screen.height - 500)/2-14)+',left='+Math.floor((screen.width - 400)/2-5));" value="...">
...
$tabControl->Buttons(
	array(
		...
		"back_url" => "/thurly/admin/sale_affiliate_plan.php?lang=".LANG.GetFilterParams("filter_")
	)
);
?>
