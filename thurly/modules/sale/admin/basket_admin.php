...
if (isset($_REQUEST['action']))
{
	if($_REQUEST['action'] == "order_basket")
	{
		...
		if($fuserID > 0)
		{
			...
			$url = "/thurly/admin/sale_order_create.php?lang=".LANG."&SITE_ID=".$siteID."&USER_ID=".$userID."&FUSER_ID=".$fuserID."&ABANDONED=Y";
...
while ($arBasket = $dbResultList->Fetch())
{
	...
	{
		$fieldValue = "[<a href=\"/thurly/admin/user_edit.php?ID=".$arBasket["USER_ID"]."&lang=".LANG."\" title=\"".GetMessage("SB_USER_INFO")."\">".$arBasket["USER_ID"]."</a>] ";
		...
		<td>
			...
			function getProductData(arParams)
			{
				var productId = arParams['id'],
					dateURL = '<?=thurly_sessid_get()?>&ORDER_AJAX=Y&get_product_params=Y&LID=<?=CUtil::JSEscape($LID)?>&productId=' + productId;
