...
if (!$errorMessage)
{
	...
	if (($arID = $lReports->GroupAction()) && $saleModulePermissions >= 'W')
	{
		if ($_REQUEST['action_target'] === 'on' || $_REQUEST['action_target'] === 'selected')
		{
			...
			$res = Thurly\Report\ReportTable::getList(array(
				'select' => array('ID'),
	...
	while ($arRes = $dbRepList->NavNext(false))
	{
		...
		$createdDateStr = ($arRes['CREATED_DATE'] instanceof \Thurly\Main\Type\DateTime || $arRes['CREATED_DATE'] instanceof \Thurly\Main\Type\Date) ? ConvertTimeStamp($arRes['CREATED_DATE']->getTimestamp(), 'SHORT') : '';
