...
use Thurly\Main\Localization\Loc,
	Thurly\Sale\DiscountCouponsManager,
	Thurly\Sale\Helpers\Admin\OrderEdit,
...
if($isSavingOperation || $needFieldsRestore)
{
	...
	{
		$result->addError(new \Thurly\Main\Error(Loc::getMessage('SALE_OK_ORDER_CREATE_ERROR')));
	}
}
elseif($createWithProducts)
{
	...
	else
	{
		if(isset($_GET['FUSER_ID']) && intval($_GET['FUSER_ID']) > 0)
			...
			$fuserId = \Thurly\Sale\Fuser::getIdByUserId($_GET["USER_ID"]);
		...
		if(intval($fuserId) > 0)
		{
			...
			Thurly\Main\Loader::includeModule('catalog');
			$basketFilter = array(
				'filter' => array(
					'LID' => $_GET['SITE_ID'],
			...
			$resBasketDataList = \Thurly\Sale\Basket::getList($basketFilter);
	...
	$res = new \Thurly\Sale\Result();
	$order = OrderEdit::createOrderFromForm($formData, $USER->GetID(), false, array(), $res);
	$userProfiles = \Thurly\Sale\Helpers\Admin\Blocks\OrderBuyer::getUserProfiles($_GET['USER_ID']);

	//Just get first available profile
	if($order && !empty($userProfiles))
	{
		$propCollection = $order->getPropertyCollection();
		$ptList = \Thurly\Sale\Helpers\Admin\Blocks\OrderBuyer::getBuyerTypesList($order->getSiteId());
		$ptIndex = 0;
...
elseif($isRestoringOrderOperation) // Restore order from archive
{
	...
	$allowedStatusUpdate = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($USER->GetID(), array('update'));

	if (!in_array($archivedOrder->getField("STATUS_ID"), $allowedStatusUpdate))
	{
		LocalRedirect("/thurly/admin/sale_order_archive.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
	}
	...
	$userProfiles = \Thurly\Sale\Helpers\Admin\Blocks\OrderBuyer::getUserProfiles($order->getUserId());
	...
	/** @var \Thurly\Sale\Shipment $archivedShipment */
...
{
	/** @var \Thurly\Sale\Order $originalOrder */
	$originalOrder = Thurly\Sale\Order::load($ID);
	if ($originalOrder)
	{
		$order = \Thurly\Sale\Order::create($originalOrder->getSiteId(), $originalOrder->getUserId(), $originalOrder->getCurrency());
		$order->setPersonTypeId($originalOrder->getPersonTypeId());
		$userProfiles = \Thurly\Sale\Helpers\Admin\Blocks\OrderBuyer::getUserProfiles($originalOrder->getUserId());
		...
		/** @var \Thurly\Sale\PropertyValue $prop */
		...
		$basket = \Thurly\Sale\Basket::create($originalOrder->getSiteId());
		$basket->setFUserId($originalBasket->getFUserId());

		/** @var \Thurly\Sale\BasketItem $originalBasketItem */
...
if(!$order)
{
	$order = \Thurly\Sale\Order::create($siteId);
...
if ($isRestoringOrderOperation)
{
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/sale_order_archive.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	);
}
else
{
		...
		foreach ($blocksOrder as $blockCode)
		{
			...
			switch ($blockCode)
			{
				...
				case "additional":
					...
					break;
				case "statusorder":
					echo Blocks\OrderStatus::getEditSimple($USER->GetID(), 'STATUS_ID', \Thurly\Sale\OrderStatus::getInitialStatus());
					break;
				default:
...
$tabControl->Buttons(
	array(
		"back_url" => "/thurly/admin/sale_order_create.php?lang=".LANGUAGE_ID."&SITE_ID=".$siteId.GetFilterParams("filter_"))
);
