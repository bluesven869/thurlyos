...
if(isset($_POST["EBAY_SETTINGS"]) && is_array($_POST["EBAY_SETTINGS"]))
{
	...
	$_POST["EBAY_SETTINGS"]["FEEDS"] = \Thurly\Sale\TradingPlatform\Ebay\Agent::update($site, $_POST["EBAY_SETTINGS"]["FEEDS"]);
	...
	else
	{
		$errorMsg .= Loc::getMessage(
			...
			array(
				'#A1#' => '<a href="/thurly/admin/sale_ebay_wizard.php?lang='.LANGUAGE_ID.'&STEP=1&SITE_ID='.$site.'">',
				'#A2#' => '</a>',
				'#S#' => $siteList[$site]
			)
		);
...
$res = \Thurly\Sale\TradingPlatform\Ebay\CategoryTable::getList(array(
	...
	"runtime" => array(
		...
		new \Thurly\Main\Entity\ExpressionField('LAST_UPDATE', 'MAX(LAST_UPDATE)')
	)
));
...
$res = \Thurly\Sale\TradingPlatform\Ebay\CategoryVariationTable::getList(array(
	"select" => array("CNT"),
	"runtime" => array(
		new \Thurly\Main\Entity\ExpressionField('CNT', 'COUNT(1)'),
	)
...
$res = \Thurly\Sale\TradingPlatform\Ebay\Feed\ResultsTable::getList(array(
	"select" => array("FEED_TYPE", "MAX_UPLOAD_TIME"),
	"group" => array("FEED_TYPE"),
	"runtime" => array(
		new \Thurly\Main\Entity\ExpressionField('MAX_UPLOAD_TIME', 'MAX(UPLOAD_TIME)'),
	)
));
