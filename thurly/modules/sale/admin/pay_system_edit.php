...
Asset::getInstance()->addJs("/thurly/js/sale/pay_system.js");

\Thurly\Sale\Delivery\Restrictions\Manager::getClassesList();
	...
	&& check_thurly_sessid())
{
	...
	if ($errorMessage === '')
	{
		...
		if (\Thurly\Main\IO\File::isFileExists($documentRoot.$path.'/handler.php'))
		{
			...
			if (is_subclass_of($className, '\Thurly\Sale\PaySystem\IPrePayable'))
				$fields['HAVE_PREPAY'] = 'Y';

			if (is_subclass_of($className, '\Thurly\Sale\PaySystem\ServiceHandler'))
				$fields['HAVE_RESULT_RECEIVE'] = 'Y';

			if (is_subclass_of($className, '\Thurly\Sale\PaySystem\IPayable'))
				$fields['HAVE_PRICE'] = 'Y';
		}
		else
		{
			if (\Thurly\Main\IO\File::isFileExists($documentRoot.$actionFile."/pre_payment.php"))
				$fields["HAVE_PREPAY"] = "Y";
			if (\Thurly\Main\IO\File::isFileExists($documentRoot.$actionFile."/result.php"))
				$fields["HAVE_RESULT"] = "Y";
			if (\Thurly\Main\IO\File::isFileExists($documentRoot.$actionFile."/action.php"))
				$fields["HAVE_ACTION"] = "Y";
			if (\Thurly\Main\IO\File::isFileExists($documentRoot.$actionFile."/payment.php"))
				$fields["HAVE_PAYMENT"] = "Y";
			if (\Thurly\Main\IO\File::isFileExists($documentRoot.$actionFile."/result_rec.php"))
				$fields["HAVE_RESULT_RECEIVE"] = "Y";
		}
		...
		elseif ($id <= 0)
		{
			$image = '/thurly/images/sale/sale_payments/'.$request->get('ACTION_FILE').'.png';
			if (\Thurly\Main\IO\File::isFileExists($documentRoot.$image))
			{
			...
			else
			{
				...
				if ($id > 0)
				{
					...
					if ($currency)
					{
						...
						$saveResult = \Thurly\Sale\Services\PaySystem\Restrictions\Currency::save($params);
...
if ($id > 0)
{
	$dbRes = \Thurly\Sale\PaySystem\Manager::getList(
		array(
			'filter' => array('ID' => $id),
...
require_once($documentRoot."/thurly/modules/sale/prolog.php");
...
{
	ob_start();
	require_once($documentRoot."/thurly/modules/sale/admin/pay_system_restrictions_list.php");
	$restrictionsHtml = ob_get_contents();
	ob_end_clean();
}

require($documentRoot."/thurly/modules/main/include/prolog_admin_after.php");
...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/sale_pay_system.php?lang=".$context->getLanguage().GetFilterParams("filter_"),
		"ICON" => "btn_list"
	)
);
...
if ($id > 0 && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/sale_pay_system_edit.php?lang=".$context->getLanguage().GetFilterParams("filter_"),
			"ICON" => "btn_new"
		);

	$aMenu[] = array(
			"TEXT" => Loc::getMessage("SPSN_DELETE_PAYSYS"),
			"LINK" => "javascript:if(confirm('".Loc::getMessage("SPSN_DELETE_PAYSYS_CONFIRM")."')) window.location='/thurly/admin/sale_pay_system.php?action=delete&ID[]=".$id."&lang=".$context->getLanguage()."&".thurly_sessid_get()."#tb';",
			"WARNING" => "Y",
			"ICON" => "btn_delete"
		);
}
	...
	<tr class="adm-detail-required-field">
		...
		<td width="60%" valign="top">
			<select name="ACTION_FILE" onchange='BX.Sale.PaySystem.getHandlerOptions(this)'>
				<?
					$handlerList = Thurly\Sale\PaySystem\Manager::getHandlerList();
					...
					if (\Thurly\Main\IO\File::isFileExists($documentRoot.$pathToHandler.'/.description.php'))
	...
	<?
		...
		if ($handlerModeList):?>
			<tr>
				...
				<td width="60%" valign="top">
				<?
					if (!class_exists('\Thurly\Sale\Internals\Input\Enum'))
						require $documentRoot.'/thurly/modules/sale/lib/internals/input.php';

					if (class_exists($className))
						echo Thurly\Sale\Internals\Input\Enum::getEditHtml('PS_MODE', array('OPTIONS' => $handlerModeList), $psMode);
				?>
				</td>
			</tr>
		<?endif;?>
	...
	<?
		if ($paySystem && array_key_exists('ACTION_FILE', $paySystem))
		{
			if ($paySystem['ACTION_FILE'] == 'yandex')
			{
				...
				{
					$pathToReturnPage = $documentRoot.'/thurly/modules/sale/handlers/paysystem/'.$handlerName.'/settings/refund.php';
			...
			{
				$pathToReturnPage = $documentRoot.'/thurly/modules/sale/admin/yandexinvoice_settings.php';
				if (IO\File::isFileExists($pathToReturnPage)):?>
					<tr>
						<td colspan="2" align="center" class="heading" style="padding-top: 10px">
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/sale_pay_system.php?lang=".$context->getLanguage().GetFilterParams("filter_")
			)
	);
$tabControl->End();
?>
...
<?
require($documentRoot."/thurly/modules/main/include/epilog_admin.php");
