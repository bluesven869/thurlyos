...
if (($ids = $lAdmin->GroupAction()) && $saleModulePermissions >= "W")
{
	...
	foreach ($ids as $id)
	{
		...
		{
			case "delete":
				$dbRes = \Thurly\Sale\Internals\PaymentTable::getList(array('filter' => array('PAY_SYSTEM_ID' => $id)));
				if ($dbRes->fetch())
				{
				...
				$result = \Thurly\Sale\PaySystem\Manager::delete($id);
				if (!$result->isSuccess())
				{
			...
			case "deactivate":
				...
				$result = \Thurly\Sale\Internals\PaySystemActionTable::update($id, $arFields);
				if (!$result->isSuccess())
				{
					if ($result->getErrorMessages())
						$lAdmin->AddGroupError(join(', ', $result->getErrorMessages()), $id);
...
$dbRes = \Thurly\Sale\Internals\PaySystemActionTable::getList($params);
...
while ($paySystem = $dbRes->fetch())
{
	if (!empty($filter_person_type) && !in_array("NOT_REF", $filter_person_type))
	{
		...
		$dbRestriction = \Thurly\Sale\Internals\ServiceRestrictionTable::getList(array(
			'filter' => array(
...
while ($arCCard = $dbRes->NavNext(true, "f_"))
{
	...
	$dbRestriction = \Thurly\Sale\Internals\ServiceRestrictionTable::getList(array(
	 ...
	 {
		 $ptRes = \Thurly\Sale\PersonTypeTable::getList(array('select' => array('NAME'), 'filter' => array('ID' => $restriction['PARAMS']['PERSON_TYPE_ID'])));
	...
	$dbRestriction = \Thurly\Sale\Internals\ServiceRestrictionTable::getList(array(
		...
		'filter' => array(
			'SERVICE_ID' => $f_ID,
			'SERVICE_TYPE' => \Thurly\Sale\Services\PaySystem\Restrictions\Manager::SERVICE_TYPE_PAYMENT,
			'=CLASS_NAME' => '\Thurly\Sale\Services\PaySystem\Restrictions\Site'
		)
	));
	...
	if ($restriction = $dbRestriction->fetch())
	{
		$siteRes = \Thurly\Main\SiteTable::getList(array('select' => array('NAME', 'LID'), 'filter' => array('LID' => $restriction['PARAMS']['SITE_ID'], 'LANGUAGE_ID' => $context->getLanguage())));
	...
	$description = \Thurly\Sale\PaySystem\Manager::getHandlerDescription($f_ACTION_FILE);
