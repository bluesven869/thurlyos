...
$errorMessage = "";
$successMessage = "";
if ($_SERVER["REQUEST_METHOD"] == "POST" && $do_create_link == "Y" && $saleModulePermissions >= "W" && check_thurly_sessid())
{
	...
	function __CrmSaleQuery($crmUrlScheme, $crmUrlHost, $crmUrlPort, $crmLogin, $crmPassword, $head, $body, &$errorMessage)
	{
		...
		if (empty($errorMessage))
		{
			$buffer  = "POST /thurly/components/thurly/crm.config.external_sale.edit/bus.php HTTP/1.0\r\n";
	...
	if (empty($errorMessage))
	{
		if ($createNewSaleUser)
		{
			...
			{
				$PERM = array();
				@include($_SERVER["DOCUMENT_ROOT"]."/thurly/.access.php");
				return $PERM;
			}
			...
			$arFPerms[$groupId.""] = "R";
			$APPLICATION->SetFileAccessPermission(array(SITE_ID, "/thurly/admin"), $arFPerms);
		}
...
if ($_SERVER["REQUEST_METHOD"] == "GET" && $_REQUEST["clear_crm_stat"] == "Y" && $saleModulePermissions >= "W" && check_thurly_sessid())
{
...
<div class="crm-admin-wrap">
		...
		foreach ($arCrmIntegration as $crmUrl => $arCrm)
		{
			...
			<div class="crm-admin-statistics-block">
				<div class="crm-admin-stat-top">
					...
					<a class="crm-admin-reset" href="/thurly/admin/sale_crm.php?clear_crm_stat=Y&clear_crm_stat_url=<?= urlencode($crmUrl) ?>&<?= thurly_sessid_get() ?>"><?= GetMessage("SPTEN_SCRM_CLEAR_STAT") ?></a>
				</div>
	...
	<div class="crm-admin-buttons" id="id_new_crm_btns">
		...
		<span class="crm-admin-button-wrap">
			<a target="_blank" href="https://www.thurlyos.<? if (LANGUAGE_ID == "ru") echo "ru"; elseif (LANGUAGE_ID == "de") echo "de"; else echo "com"; ?>/" class="adm-btn adm-btn-green"><?= GetMessage("SPTEN_SCRM_REG_BTN_24") ?></a>
