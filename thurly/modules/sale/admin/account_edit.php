...
if ($_SERVER['REQUEST_METHOD']=="POST" && strlen($Update)>0 && $saleModulePermissions>="U" && check_thurly_sessid())
{
	if ($ID <= 0)
	{
	...
	{
		if (strlen($apply) <= 0)
			LocalRedirect("/thurly/admin/sale_account_admin.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
	}
	else
	{
		$bVarsFromForm = true;
...
$aMenu = array(
		array(
				...
				"LINK" => "/thurly/admin/sale_account_admin.php?lang=".LANGUAGE_ID.GetFilterParams("filter_"),
				"ICON"	=> "btn_list",
...
if ($ID > 0 && $saleModulePermissions >= "U")
{
	...
	$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/sale_account_edit.php?lang=".LANGUAGE_ID.GetFilterParams("filter_"),
			"ICON"	=> "btn_new",
	...
	if ($saleModulePermissions >= "W")
	{
		$aMenu[] = array(
				...
				"LINK" => "javascript:if(confirm('".GetMessage("SAEN_DELETE_ACCOUNT_CONFIRM")."')) window.location='/thurly/admin/sale_account_admin.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."#tb';",
				"WARNING" => "Y",
				"ICON"	=> "btn_delete"
			);
	}
	...
	<tr class="adm-detail-required-field">
		...
		<td width="60%">
			<?if ($ID > 0):?>
				...
				[<a title="<?echo GetMessage("SAE_USER_PROFILE")?>" href="/thurly/admin/user_edit.php?lang=<?=LANGUAGE_ID?>&ID=<?=$str_USER_ID?>"><?=$str_USER_ID?></a>] (<?=$str_USER_LOGIN?>) <?=$str_USER_NAME?> <?=$str_USER_LAST_NAME?>
			<?else:?>
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/sale_account_admin.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
			)
	);
