...
while ($arTransact = $dbTransactList->NavNext(true, "f_"))
{
	...
	$fieldValue  = "[<a href=\"/thurly/admin/sale_affiliate_edit.php?ID=".$f_AFFILIATE_ID."&lang=".LANG."\" title=\"".GetMessage("SAT2_AFF_PROFILE")."\">".$f_AFFILIATE_ID."</a>] ";
	...
	if (in_array("DESCR", $arVisibleColumns))
	{
		...
		if (IntVal($arTransact["EMPLOYEE_ID"]) > 0)
		{
			...
			$fieldValue .= "[<a href=\"/thurly/admin/user_edit.php?ID=".$arTransact["EMPLOYEE_ID"]."&lang=".LANG."\" title=\"".GetMessage("SAT2_USER_PROFILE")."\">".$arTransact["EMPLOYEE_ID"]."</a>] ";
