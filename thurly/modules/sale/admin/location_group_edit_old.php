...
$strError = "";
$bInitVars = false;
if ((strlen($save)>0 || strlen($apply)>0) && $REQUEST_METHOD=="POST" && $saleModulePermissions=="W" && check_thurly_sessid())
{
	$SORT = IntVal($SORT);
	if ($SORT<=0) $SORT = 100;
...
$aMenu = array(
		array(
				...
				"ICON" => "btn_list",
				"LINK" => "/thurly/admin/sale_location_group_admin.php?lang=".LANG.GetFilterParams("filter_")
			)
	);
...
if ($ID > 0 && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
			...
			"ICON" => "btn_new",
			"LINK" => "/thurly/admin/sale_location_group_edit.php?lang=".LANG.GetFilterParams("filter_")
		);

	$aMenu[] = array(
			"TEXT" => GetMessage("SLGEN_DELETE_LGROUP"),
			"ICON" => "btn_delete",
			"LINK" => "javascript:if(confirm('".GetMessage("SLGEN_DELETE_LGROUP_CONFIRM")."')) window.location='/thurly/admin/sale_location_group_admin.php?action=delete&ID[]=".$ID."&lang=".LANG."&".thurly_sessid_get()."#tb';",
		);
}
...
?>
	...
	<tr class="adm-detail-required-field">
		<td valign="top">
			<?echo GetMessage("SALE_LOCATIONS")?>:<br><img src="/thurly/images/sale/mouse.gif" width="44" height="21" border="0" alt="">
		</td>
		<td valign="top">
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/sale_location_group_admin.php?lang=".LANG.GetFilterParams("filter_")
			)
	);
?>
