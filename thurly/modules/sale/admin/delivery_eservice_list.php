<?
namespace Thurly\Sale\Delivery\AdminPage\DeliveryExtraServiceEdit
{
	...
	if(isset($_REQUEST["action"]) && $_REQUEST["action"] == "delete_extra_service" && isset($_REQUEST["ES_ID"]) && $saleModulePermissions == "W" && check_thurly_sessid())
	{
	...
	$res = \Thurly\Sale\Delivery\ExtraServices\Table::getList(array(
		'filter' => array(
			'=DELIVERY_ID' => $ID,
			...
			$arActions[] = array(
				...
				"ACTION"=> "javascript:if(confirm('".Loc::getMessage("SALE_ESDL_CONFIRM_DEL_MESSAGE")."')) window.location='".
					$APPLICATION->GetCurPageParam(
						"action=delete_extra_service&ES_ID=".$record['ID']."&".thurly_sessid_get().'&back_url='.urlencode($_REQUEST["back_url"]),
	...
	if ($saleModulePermissions == "W")
	{
		...
		/** @var  \Thurly\Sale\Delivery\ExtraServices\Base $esClass */
		foreach(ExtraServices\Manager::getClassesList() as $esClass)
		{
			if($esClass == '\Thurly\Sale\Delivery\ExtraServices\String')
				continue;
