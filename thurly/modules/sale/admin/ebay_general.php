...
if($ebay->isActive())
{
	...
	if(!empty($arPersonTypes))
	{
		...
		$thurlyStatuses = Helper::getThurlyStatuses($SITE_ID);
...
elseif($ebay->isActive())
{
			...
			<div id="SALE_EBAY_IBLOCK_CHOOSE">
				<?for($i = 0; $i < count($siteSettings["IBLOCK_ID"]); $i++):?>
					<div  style="padding-top: 10px;">
						...
						?>&nbsp;
						<?=Helper::getThurlyCategoryPropsHtml("EBAY_SETTINGS[MORE_PHOTO_PROP][".$siteSettings["IBLOCK_ID"][$i]."]", $siteSettings["IBLOCK_ID"][$i],0, $siteSettings["MORE_PHOTO_PROP"][$siteSettings["IBLOCK_ID"][$i]])?>
					</div>
				<?endfor;?>
			</div>
	...
	<tr>
		...
		<td width="60%"><b><?=Loc::getMessage("SALE_EBAY_STATUSES_THURLY")?>:</b></td>
	</tr>
	<?foreach(\Thurly\Sale\TradingPlatform\Ebay\Helper::getEbayOrderStatuses() as $ebayStatus):?>
		<tr>
			<td width="40%">[<?=$ebayStatus?>]:</td>
			<td width="60%"><?=Helper::getSelectHtml("EBAY_SETTINGS[STATUS_MAP][".$ebayStatus."]", $thurlyStatuses, $siteSettings["STATUS_MAP"][$ebayStatus])?></td>
		</tr>
	<?endforeach;?>
	<tr>
		<td width="40%" class = "adm-detail-valign-top"><?=Loc::getMessage("SALE_EBAY_ORDER_IS_READY")?>:</td>
		<td width="60%"><?=Helper::getSelectHtml("EBAY_SETTINGS[ORDER_READY_MAP]", $thurlyStatuses, $siteSettings["ORDER_READY_MAP"])?>
	...
	$res = \Thurly\Catalog\CatalogIblockTable::getList(array(
		'select' => array('IBLOCK_ID', 'SITE_ID' => 'IBLOCK_SITE.SITE_ID'),
		'filter' => array('SITE_ID' => $SITE_ID),
		'runtime' => array(
			'IBLOCK_SITE' => array(
				'data_type' => 'Thurly\Iblock\IblockSiteTable',
				'reference' => array(
	...
	while($ib = $res->fetch())
	{
		$entityId = \Thurly\Sale\TradingPlatform\Ebay\MapHelper::getCategoryEntityId($ib["IBLOCK_ID"]);
		...
		$mapRes = \Thurly\Sale\TradingPlatform\MapTable::getList(array(
			'select' => array(
				...
				'CATEGORY_THURLY_NAME' => 'CATEGORY_THURLY.NAME',
				'IBLOCK_ID' => 'CATEGORY_THURLY.IBLOCK_ID'
			),
			'filter' => array(
				'=ENTITY_ID' => $entityId
			),
			'order' => array(
				'CATEGORY_THURLY_NAME' => 'ASC',
				'CATEGORY_EBAY_NAME' => 'ASC'
			),
			'runtime' => array(
				new \Thurly\Main\Entity\ReferenceField('CATEGORY_EBAY', '\Thurly\Sale\TradingPlatform\Ebay\CategoryTable',
					array('=this.VALUE_EXTERNAL' => 'ref.CATEGORY_ID')
				),
				new \Thurly\Main\Entity\ReferenceField('CATEGORY_THURLY', '\Thurly\Iblock\SectionTable',
					array(
	...
	<tr class="heading"><td><?=Loc::getMessage("SALE_EBAY_CAT_THURLY_NAME")?></td><td><?=Loc::getMessage("SALE_EBAY_CAT_EBAY_NAME")?></td></tr>
	<?
	if(!empty($maps))
	{
		foreach($maps as $map)
		{
			...
			<td>
				<a
					href="/thurly/admin/cat_section_edit.php?IBLOCK_ID=<?=$map["IBLOCK_ID"]?>&type=catalog&ID=<?=$map["VALUE_INTERNAL"]?>&lang=<?=LANGUAGE_ID?>&find_section_section=0&form_section_2_active_tab=SALE_TRADING_PLATFORM_edit_trading_platforms"
					title="<?=Loc::getMessage("SALE_EBAY_CAT_SETT_EDIT")?>"
				>
					<?=htmlspecialcharsbx($map["CATEGORY_THURLY_NAME"])?></a> [<?=htmlspecialcharsbx($map["VALUE_INTERNAL"])?>]
			</td>
			<td>
...
{
	$res = Thurly\Sale\TradingPlatform\Ebay\Helper::checkEnveronment();
