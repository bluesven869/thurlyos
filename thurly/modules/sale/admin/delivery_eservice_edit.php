...
if($saleModulePermissions == "W" && check_thurly_sessid())
{
...
if($DELIVERY_ID > 0)
{
	$deliveryService = \Thurly\Sale\Delivery\Services\Manager::getObjectById($DELIVERY_ID);

	if($deliveryService && \Thurly\Main\Loader::includeModule('currency'))
	{
...
$aMenu = array(
	array(
		...
		"LINK" => isset($_GET["back_url"]) ? $_GET["back_url"] : "/thurly/admin/sale_delivery_service_list.php?lang=".LANGUAGE_ID."&ID=".$DELIVERY_ID,
		"ICON" => "btn_list"
	)
);
...
if ($ID > 0 && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/sale_delivery_eservice_edit.php?lang=".LANGUAGE_ID."&DELIVERY_ID=".$DELIVERY_ID.(isset($_REQUEST["back_url"]) ? "&back_url=".urlencode($_REQUEST["back_url"]) : ""),
		"ICON" => "btn_new"
	);
...
$tabControl->Buttons(
	array(
		...
		"back_url" => isset($_REQUEST["back_url"]) ? $_REQUEST["back_url"] : ("/thurly/admin/sale_delivery_service_edit.php?lang=".LANGUAGE_ID.'&ID='.(isset($_REQUEST["DELIVERY_ID"]) ? $_REQUEST["DELIVERY_ID"] :0))
	)
);
