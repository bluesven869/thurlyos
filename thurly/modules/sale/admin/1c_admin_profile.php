...
include_once($GLOBALS["DOCUMENT_ROOT"]."/thurly/modules/sale/include.php");
...
if($REQUEST_METHOD=="POST" && strlen($Update)>0 && $salePermissions>="W" && check_thurly_sessid())
{
...
function ActionFileChange(ind, type)
{
	...
	window.frames["hidden_action_frame_" + ind].location.replace('/thurly/admin/sale_options_get.php?lang=<?= htmlspecialcharsbx($lang) ?>&type='+type+'&divInd='+ind);
}
