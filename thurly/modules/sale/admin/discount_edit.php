	...
	switch ($_POST['AJAX_ACTION'])
	{
		case 'getUserName':
			...
			echo Main\Web\Json::encode(array(
				'userId' => $userId,
				'name' => \Thurly\Sale\Helpers\Admin\OrderEdit::getUserName($userId),
			));
			CMain::FinalActions();
			die;
...
)
{
	...
	if (empty($errors))
	{
		if (empty($_POST['apply']))
			LocalRedirect("/thurly/admin/sale_discount.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
		else
			LocalRedirect("/thurly/admin/sale_discount_edit.php?lang=".LANGUAGE_ID."&ID=".$discountID.'&'.$control->ActiveTabParam());
	}
}
...
{
	LocalRedirect('/thurly/admin/sale_discount_preset_detail.php?DISCOUNT_ID='.$arDiscount['ID'].'&from_list=order&lang='.LANGUAGE_ID);
}
...
$contextMenuItems = array(
	array(
		...
		"LINK" => "/thurly/admin/sale_discount.php?lang=".LANGUAGE_ID.GetFilterParams("filter_"),
		"ICON" => "btn_list"
	)
);
...
if ($discountID > 0 && $saleModulePermissions == 'W')
{
	if (!$copy)
	{
		...
		$contextMenuItems[] = array(
			...
			"LINK" => "/thurly/admin/sale_discount_edit.php?lang=".LANGUAGE_ID.GetFilterParams("filter_"),
			"ICON" => "btn_new"
		);
		$contextMenuItems[] = array(
			"TEXT"=>GetMessage("BT_SALE_DISCOUNT_EDIT_MESS_COPY_DISCOUNT"),
			"LINK"=>'/thurly/admin/sale_discount_edit.php?lang='.LANGUAGE_ID.'&ID='.$discountID.'&action=copy&'.GetFilterParams('filter_'),
			"ICON"=>"btn_copy",
		);
		$contextMenuItems[] = array(
			"TEXT" => GetMessage("BT_SALE_DISCOUNT_EDIT_MESS_DELETE_DISCOUNT"),
			"LINK" => "javascript:if(confirm('".GetMessageJS("BT_SALE_DISCOUNT_EDIT_MESS_DELETE_DISCOUNT_CONFIRM")."')) window.location='/thurly/admin/sale_discount.php?lang=".LANGUAGE_ID."&ID=".$discountID."&action=delete&".thurly_sessid_get()."';",
			"WARNING" => "Y",
			"ICON" => "btn_delete"
		);
	}
...
$hintPath = '/thurly/images/sale/discount/';
...
$control->Begin(array(
	"FORM_ACTION" => '/thurly/admin/sale_discount_edit.php?lang='.LANGUAGE_ID,
));
...
$control->BeginNextFormTab();
	...
	$couponsAjaxPath = '/thurly/tools/sale/discount_coupon_list.php?lang='.LANGUAGE_ID.'&find_discount_id='.$discountID;
	if ($discountID > 0 && !$copy)
	{
		?><tr id="tr_COUPONS"><td colspan="2"><?
		require($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/tools/discount_coupon_list.php');
		?></td></tr><?
	}
	else
	{
...
$control->Buttons(
	array(
		...
		"back_url" => "/thurly/admin/sale_discount.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	)
);
$control->Show();
?>
