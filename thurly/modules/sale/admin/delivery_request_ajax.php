...
if(empty($arResult["ERRORS"]) && $saleModulePermissions >= "W" && check_thurly_sessid())
{
	...
	switch ($action)
	{
		...
		case "actionShipmentExecute":
					...
					foreach($requestInputs as $name => $params)
					{
						$content .=
							'<tr>
								...
								<td>'.\Thurly\Sale\Internals\Input\Manager::getEditHtml("requestInputs[".$name."]", $params, (isset($params[$name]) ? $params[$name] : null)).'</td>
							</tr>';
					}
			...
			if(empty($requestInputs))
			{
				...
				if($res instanceof Requests\ResultFile && $res->isSuccess())
				{
					...
					$res = \Thurly\Main\IO\File::putFileContents($filePath, $fileContent);
		...
		case "downloadFile":
			...
			try
			{
				$filePath = \CTempFile::GetAbsoluteRoot().\Thurly\Main\IO\Path::normalize($filePath.$fileName);
			}
			...
			if(!\Thurly\Main\IO\File::isFileExists($filePath))
			{
				die();
			}

			$file = new \Thurly\Main\IO\File($filePath);
