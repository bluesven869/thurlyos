...
if ($request->isPost() && $request->getPost("update") && check_thurly_sessid() && $saleModulePermissions == 'W')
{
	...
	if (empty($errorMessage))
	{
		...
		if ($id > 0)
		{
			\Thurly\Sale\Internals\CompanyGroupTable::deleteByCompanyId($id);
			\Thurly\Sale\Internals\CompanyResponsibleGroupTable::deleteByCompanyId($id);
		}
		...
		if ($id > 0)
		{
			$fields['DATE_MODIFY'] = new \Thurly\Main\Type\DateTime();
		...
		else
		{
			...
			$fields['DATE_CREATE'] = new \Thurly\Main\Type\DateTime();
		...
		if ($result && $result->isSuccess())
		{
			...
			if ($groups = $request->getPost('GROUPS'))
			{
				...
				{
					$r = \Thurly\Sale\Internals\CompanyGroupTable::add(array(
																		   'COMPANY_ID' => $id,
																		   'GROUP_ID' => $groupId,
																	   ));
			...
			if ($responsibleGroups = $request->getPost('RESPONSIBLE_GROUPS'))
			{
				...
				{
					$r = \Thurly\Sale\Internals\CompanyResponsibleGroupTable::add(array(
																		   'COMPANY_ID' => $id,
																		   'GROUP_ID' => $groupId,
																	   ));
			...
			if (strlen($request->getPost("apply")) == 0)
				LocalRedirect("/thurly/admin/sale_company.php?lang=".$lang."&".GetFilterParams("filter_", false));
			else
				LocalRedirect("/thurly/admin/sale_company_edit.php?lang=".$lang."&ID=".$id."&".GetFilterParams("filter_", false));
		}
		else
		{
...
Main\Page\Asset::getInstance()->addJs("/thurly/js/sale/company.js");
...
if ($saleModulePermissions >= 'W'):?>
	<tr>
		...
		<td>
			<?$APPLICATION->IncludeComponent("thurly:sale.location.selector.".\Thurly\Sale\Location\Admin\LocationHelper::getWidgetAppearance(), "", array(
					"ID" => "",
...
<?
	...
	try
	{
		$res = \Thurly\Sale\Location\LocationTable::getPathToNodeByCode(
				$fields['LOCATION_ID'],
				array(
...
if ($id > 0)
{
	$resCompayGroup = \Thurly\Sale\Internals\CompanyGroupTable::getList(array(
...
if ($id > 0)
{
	$resCompayGroup = \Thurly\Sale\Internals\CompanyResponsibleGroupTable::getList(array(
		'filter' => array('=COMPANY_ID' => $id),
		'select' => array('GROUP_ID')
	));
	while($companyGroup = $resCompayGroup->fetch())
...
if ($id > 0):
	...
	ob_start();
	require_once($documentRoot."/thurly/modules/sale/admin/company_rules_list.php");
