<?
namespace Thurly\Sale\AdminPage;
...
use Thurly\Sale\Cashbox\Internals\CashboxCheckTable;
...
/**
 * Class AjaxProcessor
 * @package Thurly\Sale\AdminPage
 * Class helper for processing ajax requests
 */
class AjaxProcessor
{
	protected $userId;
	/** @var \Thurly\Sale\Result $result*/
	protected $result;
	protected $request;
	...
	protected function addProductToBasketAction()
	{
		...
		if(empty($productParams))
		{
			...
			$providerData = \Thurly\Sale\Helpers\Admin\Product::getProviderData($productParams, $siteId, $userId);
	...
	protected function changeResponsibleUserAction()
	{
		...
		$dateResponsible = new \Thurly\Main\Type\DateTime();
	...
	protected function updatePaymentStatusAction()
	{
		...
		if ($this->request['method'] == 'save')
		{
			...
			if (!$hasErrors)
			{
				...
				try
				{
					$fields['PAY_VOUCHER_DATE'] = new \Thurly\Main\Type\Date($fields['PAY_VOUCHER_DATE']);
				}
	...
	protected function changeDeliveryServiceAction()
	{
		...
		/** @var  \Thurly\Sale\Delivery\Services\Base $service */
	...
	protected function getDemandedFields(array $demandedFields, array $incomingFields, \Thurly\Sale\Order $order = null)
	{
		$result = array();
			...
			switch($demandedField)
			{
				...
				case "PROPERTIES":
					...
					if($userId > 0)
					{
						...
						$result["PROPERTIES"] = \Thurly\Sale\Helpers\Admin\Blocks\OrderBuyer::getProfileParams($userId, $profileId);
					}
					else
					{
						...
						/** @val \Thurly\Sale\PropertyValue $prop */
						foreach(\Thurly\Sale\PropertyValue::loadForOrder($porder) as $prop)
						{
							$p = $prop->getProperty();
				...
				case "BUYER_PROFILES_LIST":
					...
					if(intval($personTypeId)<=0)
						throw new \Thurly\Main\ArgumentNullException("personTypeId");

					$result["BUYER_PROFILES_LIST"] = \Thurly\Sale\Helpers\Admin\Blocks\OrderBuyer::getBuyerProfilesList($userId, $personTypeId);
					break;

				case "BUYER_PROFILES_DATA":

					$result["BUYER_PROFILES_DATA"] = \Thurly\Sale\Helpers\Admin\Blocks\OrderBuyer::getUserProfiles($userId, $personTypeId);
					break;
				...
				case "PROPERTIES_ARRAY":
					...
					if(!$order)
						throw new \Thurly\Main\SystemException("Can't init order");
				...
				case "COUPONS":
					if(!$userId)
						throw new \Thurly\Main\ArgumentNullException("userId");
				...
				default:
					throw new \Thurly\Main\SystemException("Field: \"".$demandedField."\" is unknown!");
			}
		}
	...
	protected function refreshTrackingStatusAction()
	{
		...
		if($result->isSuccess())
		{
			...
			$this->addResultData(
				...
				\Thurly\Main\Type\DateTime::createFromTimestamp(
		...
		$cashboxData = Cashbox\Internals\CashboxTable::getList(
			array(
				'filter' => array(
					'=ID' => $cashboxId,
					'USE_OFFLINE' => 'N',
					'!%HANDLER' => '\\Thurly\\Sale\\Cashbox\\Cashbox1C'
				)
			)
		);
