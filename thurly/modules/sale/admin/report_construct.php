...
if ($fEditMode || $fCopyMode)
{
	...
	if (!($arRepParams = Thurly\Report\ReportTable::getById($repID)->fetch()))
	{
...
$arParams = array(
	...
	'PATH_TO_REPORT_LIST' => '/thurly/admin/sale_report.php?lang='.LANG,
	'PATH_TO_REPORT_CONSTRUCT' => '/thurly/admin/sale_report_construct.php',
	'PATH_TO_REPORT_VIEW' => '/thurly/admin/sale_report_view.php?lang='.LANG.'&ID=#report_id#',
	'USE_CHART' => true
);
		...
		<div class="adm-detail-content-item-block">
			...
			<tr>
					...
					<div id="report-filter-value-control-examples-custom" style="display: none">
						...
						<span name="report-filter-value-control-Thurly\Sale\Internals\Order:USER.PERSON_TYPE_ID">
							<select class="report-filter-select sale-report-site-dependent" name="value" tid="PersonType">
								<option value=""><?=GetMessage('REPORT_IGNORE_FILTER_VALUE')?></option>
								<? $siteId = CBaseSaleReportHelper::getDefaultSiteId(); ?>
								<? foreach(CBaseSaleReportHelper::getPersonTypes() as $kID => $v): ?>
						...
						<span name="report-filter-value-control-\Thurly\Sale\Internals\StatusLang">
							<select class="report-filter-select" name="value">
								<option value=""><?=GetMessage('REPORT_IGNORE_FILTER_VALUE')?></option>
							...
							<script type="text/javascript">
								...
								function onChangeSiteFilter()
								{
									...
									if (siteSelect && siteSelect.value && filterContainer)
									{
										...
										{
											BX.showWait();
											url = '/thurly/admin/sale_report_construct.php?<?=thurly_sessid_get()?>'+
						...
						<style type="text/css">
							...
							.report-filter-compare-\\Thurly\\Main\\User {display: none;}
							.report-filter-compare-\\Thurly\\Main\\Group {display: none;}
						...
						<span name="report-filter-value-control-\Thurly\Main\User" callback="RTFilter_chooseSALEUSER">
						...
						<span name="report-filter-value-control-\Thurly\Main\Group" callback="RTFilter_chooseGroup">
							<a class="report-select-popup-link" caller="true" style="cursor: pointer;"><?=GetMessage('REPORT_CHOOSE')?></a>
							<input type="hidden" name="value" />
						</span>
						<script type="text/javascript">
