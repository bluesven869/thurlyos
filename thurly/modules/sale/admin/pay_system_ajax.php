...
if(strlen($arResult["ERROR"]) <= 0 && $saleModulePermissions >= "W" && check_thurly_sessid())
{
	...
	switch ($action)
	{
		case "get_restriction_params_html":
			...
			if ($className == '\Thurly\Sale\Services\PaySystem\Restrictions\Price')
		...
		case "save_restriction":
			...
			/** @var \Thurly\Sale\Services\Base\Restriction $className */
			$className = ($request->get('className') !== null) ? trim($request->get('className')): '';
			$params = ($request->get('params') !== null) ? $request->get('params') : array();
			...
			if(!class_exists($className) || !(is_subclass_of($className, '\Thurly\Sale\Services\Base\Restriction')))
				throw new \Thurly\Main\ArgumentNullException("className");

			if(!$paySystemId)
				throw new \Thurly\Main\ArgumentNullException("paySystemId");
		...
		case 'getHandlerDescription':
			...
			$data = \Thurly\Sale\PaySystem\Manager::getHandlerDescription($handler);
			...
			$businessValueControl = new \Thurly\Sale\Helpers\Admin\BusinessValueControl('PAYSYSTEM');

			$tariff = \Thurly\Sale\PaySystem\Manager::getTariff($handler);
			if (!$tariff)
			...
			$className = \Thurly\Sale\PaySystem\Manager::getClassNameFromPath($handler);
			...
			if (class_exists($className))
			{
				...
				if ($modeList)
					$arResult["PAYMENT_MODE"] = Thurly\Sale\Internals\Input\Enum::getEditHtml('PS_MODE', array('OPTIONS' => $modeList));
			}
			...
			if (IO\File::isFileExists($_SERVER['DOCUMENT_ROOT'].$path.'/.description.php'))
			{
				...
				if ($paySystemId <= 0)
				{
					...
					if (IO\File::isFileExists($_SERVER['DOCUMENT_ROOT'].'/thurly/images/sale/sale_payments/'.$handler.'.png'))
					{
						$arResult['LOGOTIP']['NAME'] = $handler.'.png';
						$arResult['LOGOTIP']['PATH'] = '/thurly/images/sale/sale_payments/'.$handler.'.png';
					}
				}
			}
			ob_start();
		...
		case 'checkHttps':
			...
			$response = @$http->get('https://'.$_SERVER['SERVER_NAME'].'/thurly/tools/sale_ps_result.php');
...
function getRestrictionHtml($paySystemId)
{
	...
	ob_start();
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/admin/pay_system_restrictions_list.php");
