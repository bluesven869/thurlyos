...
if(isset($_REQUEST["action"]) && $_REQUEST["action"] == "delete_request_shipment" && isset($_REQUEST["SHIPMENT_ID"]) && $saleModulePermissions >= "W" && check_thurly_sessid())
{
...
while ($record = $dbRecords->Fetch())
{
	...
	$row->AddField(
		'ORDER_NUMBER',
		'<a href="/thurly/admin/sale_order_view.php?ID='.$record['ORDER_ID'].'&lang='.LANGUAGE_ID.'">'.$record['ORDER_NUMBER'].'</a>'
	);
	$row->AddField(
		'SHIPMENT_ID',
	...
	if(intval($record['DELIVERY_ID']) >0 && $delivery = \Thurly\Sale\Delivery\Services\Manager::getObjectById($record['DELIVERY_ID']))
	{
		$deliveryServiceName = $delivery->getNameWithParent();
		$row->AddField(
			'DELIVERY_ID',
			'<a href="/thurly/admin/sale_delivery_service_edit.php?lang='.LANGUAGE_ID.'&ID='.$record['DELIVERY_ID'].'&PARENT_ID='.$delivery->getParentId().'">'.htmlspecialcharsbx($deliveryServiceName).'</a>'
		);
	}
	else
	{
		...
		$arActions[] = array(
			...
			"ACTION"=> "javascript:if(confirm('".Loc::getMessage('SALE_DELIVERY_REQ_DRS_DEL_CONFIRM')."')){ window.location='".
				$APPLICATION->GetCurPageParam(
					"action=delete_request_shipment&SHIPMENT_ID=".$record["SHIPMENT_ID"]."&RS_ID=".$record['ID']."&".thurly_sessid_get().'&back_url='.urlencode($_REQUEST["back_url"]),
