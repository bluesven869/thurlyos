...
use Thurly\Sale\Location\Admin\ExternalServiceHelper;
use Thurly\Sale\Location\Admin\SearchHelper;
...
CJSCore::Init();
$APPLICATION->AddHeadScript('/thurly/js/sale/core_ui_widget.js');
$APPLICATION->AddHeadScript('/thurly/js/sale/core_ui_etc.js');
$APPLICATION->AddHeadScript('/thurly/js/sale/core_ui_dynamiclist.js');
...
try
{
	...
	$formSubmitted = ($actionSave || $actionApply || $actionSaveAndAdd) && check_thurly_sessid();
...
<?//temporal code?>
<?if(!CSaleLocation::locationProCheckEnabled())require($DOCUMENT_ROOT."/thurly/modules/main/include/epilog_admin.php");?>
				...
				<td width="60%">
					...
					<?elseif($code == 'PARENT_ID'):?>
						...
						<div style="max-width: 500px">
							...
							<?$APPLICATION->IncludeComponent("thurly:sale.location.selector.".Helper::getWidgetAppearance(), "", array(
								"ID" => $value,
								"CODE" => "",
