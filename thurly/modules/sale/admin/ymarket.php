...
if (isset($_REQUEST["https_check"]) && $_REQUEST["https_check"] == "Y" && check_thurly_sessid())
{
	$ob = new CHTTP();
	$ob->http_timeout = 10;
...
else if($REQUEST_METHOD=="POST" && check_thurly_sessid())
{
...
if(CSaleYMHandler::isActive())
{
	?>
		...
		<table width="100%">
			<tr>
				...
				<td align="right">
					<img alt="eBay logo" src="/thurly/images/sale/yandex-market-logo.png" style="width: 100px; height: 67px;">
				</td>
			</tr>
		</table>
	<?
	...
	foreach(\Thurly\Sale\Delivery\Services\Manager::getActiveList() as $id => $fields)
	{
		if(!$fields["CLASS_NAME"]::canHasProfiles())
			if(!$fields["CLASS_NAME"]::canHasChildren())
				if($delivery = \Thurly\Sale\Delivery\Services\Manager::createObject($fields))
					$activeListNames[$id] = $delivery->getNameWithParent();
	}

	$siteSetts = CSaleYMHandler::getSettingsBySiteId($SITE_ID, false);

	$dlvFilteredIds = \Thurly\Sale\Delivery\Services\Manager::checkServiceRestriction(
		array_keys($activeListNames),
		$SITE_ID,
		'\Thurly\Sale\Delivery\Restrictions\BySite'
	);
				...
				<input
					...
					onclick="
						var checkHTTPS = function(){
							BX.showWait();
							BX.ajax.post('<?=$APPLICATION->GetCurPage()?>', '<?=CUtil::JSEscape(thurly_sessid_get())."&https_check=Y"?>', function (result){
								BX.closeWait();
								var res = eval( '('+result+')' );
...
function makeSelectorFromPaySystems($psTypeYandex, $psIdValue, $personTypeId, $siteId)
{
	...
	if($allPaySystems === null)
	{
		...
		$dbRes = \Thurly\Sale\PaySystem\Manager::getList(array(
	...
	if(!isset($paySystems[$siteId]))
	{
		...
		$dbRes = \Thurly\Sale\Internals\ServiceRestrictionTable::getList(array(
			'filter' => array(
				'=SERVICE_ID' => array_keys($allPaySystems),
				'=SERVICE_TYPE' => Restrictions\Manager::SERVICE_TYPE_PAYMENT,
				'=CLASS_NAME' => array(
					'\Thurly\Sale\Services\PaySystem\Restrictions\Site',
					'\Thurly\Sale\Services\PaySystem\Restrictions\PersonType'
				)
			)
		));

		/** @var \Thurly\Sale\Services\Base\Restriction $restriction */
		$rstParams = array();
		...
		{
			if(!empty($rstParams[$psId]['\Thurly\Sale\Services\PaySystem\Restrictions\Site']['SITE_ID']))
				if(!in_array($siteId, $rstParams[$psId]['\Thurly\Sale\Services\PaySystem\Restrictions\Site']['SITE_ID']))
					continue;

			if(!empty($rstParams[$psId]['\Thurly\Sale\Services\PaySystem\Restrictions\PersonType']['PERSON_TYPE_ID']))
				if(!in_array($personTypeId, $rstParams[$psId]['\Thurly\Sale\Services\PaySystem\Restrictions\PersonType']['PERSON_TYPE_ID']))
					continue;
