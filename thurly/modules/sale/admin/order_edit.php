...
$boolLocked = \Thurly\Sale\Order::isLocked($ID);
...
{
	$lockStatusRes = \Thurly\Sale\Order::getLockedStatus($ID);
	...
	if(isset($lockStatusData['LOCK_STATUS'])
			&&
			(	$lockStatusData['LOCK_STATUS'] != \Thurly\Sale\Order::SALE_ORDER_LOCK_STATUS_RED
					|| !isset($_REQUEST['target'])
			)
	)
	{
		$res = \Thurly\Sale\Order::unlock($ID);

		if($res->isSuccess())
			\Thurly\Sale\DiscountCouponsManager::clearByOrder($ID);
	}
...
$order = Thurly\Sale\Order::load($_REQUEST["ID"]);
if(!$order)
	LocalRedirect("/thurly/admin/sale_order.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
...
if(!in_array($order->getField("STATUS_ID"), $allowedStatusesUpdate))
	LocalRedirect("/thurly/admin/sale_order_view.php?ID=".$ID."&lang=".LANGUAGE_ID.GetFilterParams("filter_"));
	...
	if(($isSavingOperation || $isRefreshDataAndSaveOperation))
	{
		...
		if($res)
		{
			if (!$customTabber->Check())
			{
				...
				$result->addError(new \Thurly\Main\Entity\EntityError($errorMessage));
			}
			...
			if(!$res)
				$result->addError(new \Thurly\Main\Entity\EntityError("Can't save coupons!"));

			$discount = $order->getDiscount();

			if ($isRefreshDataAndSaveOperation)
			{
				\Thurly\Sale\DiscountCouponsManager::clearApply(true);
				\Thurly\Sale\DiscountCouponsManager::useSavedCouponsForApply(true);
				$discount->setOrderRefresh(true);
				$discount->setApplyResult(array());
				/** @var \Thurly\Sale\Basket $basket */
				if (!($basket = $order->getBasket()))
					throw new \Thurly\Main\ObjectNotFoundException('Entity "Basket" not found');
			...
			if($result->isSuccess())
			{
				...
				else
				{
					...
					if (!$customTabber->Action())
					{
						...
						$result->addError(new \Thurly\Main\Error($errorMessage));
					}
					...
					if(isset($_POST["save"]))
					{
						if (\Thurly\Sale\Order::isLocked($ID))
							\Thurly\Sale\Order::unlock($ID);

						LocalRedirect("/thurly/admin/sale_order_edit.php?lang=".LANGUAGE_ID."&unlock=Y&target=list&ID=".$ID.GetFilterParams("filter_", false));
					}
					else
						LocalRedirect("/thurly/admin/sale_order_edit.php?lang=".LANGUAGE_ID."&ID=".$order->getId().GetFilterParams("filter_", false));
				}
			}
		}
		else
		{
			$result->addError(new \Thurly\Main\Error(Loc::getMessage('SOE_ORDER_UPDATE_ERROR')));
		}
	}
}
...
\Thurly\Main\Page\Asset::getInstance()->addJs("/thurly/js/sale/admin/order_edit.js");
...
$aMenu[] = array(
	...
	"LINK" => "/thurly/admin/sale_order_edit.php?lang=".LANGUAGE_ID."&unlock=Y&target=list&ID=".$ID.GetFilterParams("filter_")
);
...
if ($boolLocked && $saleModulePermissions >= 'W')
{
	$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/sale_order_edit.php?ID=".$ID."&unlock=Y&lang=".LANGUAGE_ID.GetFilterParams("filter_"),
	);
}
		...
		if ($handle = opendir($dir))
		{
			while (($file = readdir($handle)) !== false)
			{
				...
				if (is_file($dir.$file) && ToUpper(substr($file, -4)) == ".PHP")
				{
					...
					$arReports[] = array(
						"TEXT" => $rep_title,
						"ONCLICK" => "window.open('/thurly/admin/sale_order_print_new.php?&ORDER_ID=".$ID."&doc=".substr($file, 0, strlen($file) - 4)."&".thurly_sessid_get()."', '_blank');"
					);
				}
			}
		}
		...
		foreach ($blocksOrder as $blockCode)
		{
			...
			switch ($blockCode)
			{
				...
				case "delivery":
					...
					if($defTails)
					{
						\Thurly\Main\Page\Asset::getInstance()->addJs("/thurly/js/sale/admin/order_shipment_basket.js");
						echo '<div id="sale-adm-order-shipments-content"><img src="/thurly/images/sale/admin-loader.gif"/></div>';
					}
					else
					{
	...
	<td>
		<?if($defTails):?>
			<div style="position:relative; vertical-align:top" id="sale-adm-order-analysis-content">
				<img src="/thurly/images/sale/admin-loader.gif"/>
			</div>
		<?else:?>
...
$tabControl->Buttons(
	array(
		...
		"back_url" => "/thurly/admin/sale_order_edit.php?lang=".LANGUAGE_ID."&unlock=Y&target=list&ID=".$ID.GetFilterParams("filter_"))
);
