...
/**************************** SAVE PROFILE ***********************************/
/*****************************************************************************/
if ($_SERVER['REQUEST_METHOD'] == "POST" && $saleModulePermissions >= "U" && check_thurly_sessid() && !empty($arProfile))
{
	...
	while ($arOrderProps = $dbProperties->Fetch())
	{
		...
		if ($arOrderProps['TYPE'] == "LOCATION")
		{
			...
			$locationResult = Thurly\Sale\Location\LocationTable::getList(
				array(
	...
	if (isset($_REQUEST["save"]) && strlen($_REQUEST["save"]) > 0 && empty($arErrors))
		LocalRedirect("/thurly/admin/sale_buyers_profile.php?lang=".LANGUAGE_ID."&USER_ID=".$USER_ID);
	elseif (isset($_REQUEST["apply"]) && strlen($_REQUEST["apply"]) > 0 && empty($arErrors))
		LocalRedirect("/thurly/admin/sale_buyers_profile_edit.php?id=".$ID."&lang=".LANGUAGE_ID);
}
...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/sale_buyers_profile.php?USER_ID=".$USER_ID."&lang=".LANGUAGE_ID
	)
);
...
if(!empty($arProfile) && !empty($arUser))
{
	...
	$tabControl->AddViewField("CODE_USER", GetMessage("BUYER_PE_USER").":", "[<a href=\"/thurly/admin/user_edit.php?ID=".$arUser["ID"]."&lang=".LANGUAGE_ID."\">".$arUser["ID"]."</a>] (".htmlspecialcharsEx($arUser["LOGIN"]).") ".htmlspecialcharsEx($userFIO));
	...
	$userPropertyValues = Thurly\Sale\OrderUserProperties::getProfileValues((int)$ID);
	$curVal = "";
	...
	$tabControl->Buttons(array("back_url"=>"/thurly/admin/sale_buyers_profile.php?lang=".LANGUAGE_ID."&USER_ID=".$USER_ID));
	$tabControl->Show();
}
