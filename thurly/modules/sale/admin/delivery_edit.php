...
if ((strlen($save)>0 || strlen($apply)>0) && $_SERVER['REQUEST_METHOD']=="POST" && $saleModulePermissions=="W" && check_thurly_sessid())
{
	$store = "";
...
$aMenu = array(
		array(
				...
				"LINK" => "/thurly/admin/sale_delivery.php?lang=".LANGUAGE_ID.GetFilterParams("filter_"),
				"ICON" => "btn_list"
			)
	);
...
if ($ID > 0 && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/sale_delivery_edit.php?lang=".LANGUAGE_ID.GetFilterParams("filter_"),
			"ICON" => "btn_new"
		);

	$aMenu[] = array(
			"TEXT" => GetMessage("SDEN_DELETE_DELIVERY"),
			"LINK" => "javascript:if(confirm('".GetMessage("SDEN_DELETE_DELIVERY_CONFIRM")."')) window.location='/thurly/admin/sale_delivery.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get()."#tb';",
			"ICON" => "btn_delete"
		);
}
	...
	<?if($lpEnabled):?>
		...
		<tr class="adm-detail-required-field">
			<td colspan="2">
				...
				<?$APPLICATION->IncludeComponent("thurly:sale.location.selector.system", "", array(
						"ENTITY_PRIMARY" => $ID,
...
$tabControl->Buttons(
	array(
		...
		"back_url" => "/thurly/admin/sale_delivery.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	)
);
