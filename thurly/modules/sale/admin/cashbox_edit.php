...
\Thurly\Sale\Cashbox\Cashbox::init();
	...
	if ($errorMessage === '')
	{
		...
		if ($result->isSuccess())
		{
			if (strlen($request->getPost("apply")) == 0)
				LocalRedirect("/thurly/admin/sale_cashbox_list.php?lang=".$lang."&".GetFilterParams("filter_", false));
			else
				LocalRedirect("/thurly/admin/sale_cashbox_edit.php?lang=".$lang."&ID=".$id."&".GetFilterParams("filter_", false));
		}
		else
		{
...
Page\Asset::getInstance()->addJs("/thurly/js/sale/cashbox.js");
...
{
	$res = \Thurly\Sale\Cashbox\Internals\CashboxTable::getList(array('filter' => array('ID' => $id)));
	$cashbox = $res->fetch();
}
...
{
	ob_start();
	require_once($documentRoot."/thurly/modules/sale/admin/cashbox_restrictions_list.php");
...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/sale_cashbox_list.php?lang=".$context->getLanguage().GetFilterParams("filter_"),
		"ICON" => "btn_list"
	)
);
...
if ($id > 0 && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('".Loc::getMessage("SPSN_DELETE_CASHBOX_CONFIRM")."')) window.location='/thurly/admin/sale_cashbox_list.php?action=delete&ID[]=".$id."&lang=".$context->getLanguage()."&".thurly_sessid_get()."#tb';",
			"WARNING" => "Y",
			"ICON" => "btn_delete"
		);
}
	...
	<tr class="adm-detail-required-field">
		...
		<td width="60%" valign="top">
			...
			<select name="HANDLER" id="HANDLER" onchange="BX.Sale.Cashbox.reloadSettings()" <?=$disabled;?>>
				<?
					$handlerList = Thurly\Sale\Cashbox\Cashbox::getHandlerList();
				?>
				<option value=""><?=Loc::getMessage("SALE_CASHBOX_NO_HANDLER") ?></option>
				<?
				foreach ($handlerList as $handler => $path)
				{
					if ($handler === '\Thurly\Sale\Cashbox\Cashbox1C' && $cashbox['ID'] != Cashbox\Cashbox1C::getId())
						continue;
	...
	<tr id="tr_OFD">
		...
		<td width="60%">
			<select name="OFD" id="OFD" onchange="BX.Sale.Cashbox.reloadOfdSettings()">
				<?
				$ofdList = Thurly\Sale\Cashbox\Ofd::getHandlerList();
...
ob_start();
require_once($documentRoot."/thurly/modules/sale/admin/cashbox_settings.php");
...
ob_start();
require_once($documentRoot."/thurly/modules/sale/admin/cashbox_ofd_settings.php");
...
$tabControl->Buttons(
	array(
		...
		"back_url" => "/thurly/admin/sale_cashbox_list.php?lang=".$context->getLanguage().GetFilterParams("filter_")
	)
);
