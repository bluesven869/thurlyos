...
\Thurly\Main\Loader::registerAutoLoadClasses('sale',
							array(
								'\Thurly\Sale\Helpers\Admin\Blocks\OrderShipmentStatus' => 'lib/helpers/admin/blocks/ordershipmentstatus.php',
							));
...
if (isset($_REQUEST["ORDER_AJAX"]) AND $_REQUEST["ORDER_AJAX"] == "Y" AND check_thurly_sessid())
{
	CUtil::DecodeUriComponent($_REQUEST);
	$type = $_REQUEST["type"];

	$order = CSaleOrder::getById($ID);
	$allowIds = \Thurly\Main\Config\Option::get("sale", "p2p_status_list", "");
	if(strlen($allowIds))
	...
	*/
	if (isset($type) && $type != "")
	{
		...
		{
			$viewedIterator = \Thurly\Catalog\CatalogViewedProductTable::getList(array(
...
elseif ($saleModulePermissions >= "U" && check_thurly_sessid() && !array_key_exists('dontsave', $_REQUEST))
{
...
$aMenu = array(
	array(
		...
		"LINK" => "/thurly/admin/sale_order_detail.php?ID=".$ID."&dontsave=Y&lang=".LANGUAGE_ID.GetFilterParams("filter_"),
		"ICON"=>"btn_list",
	)
);
...
if ($boolLocked && $saleModulePermissions >= 'W')
{
	$aMenu[] = array(
		...
		"LINK" => "/thurly/admin/sale_order_detail.php?ID=".$ID."&unlock=Y&lang=".LANGUAGE_ID.GetFilterParams("filter_"),
	);
}
...
if ($bUserCanEditOrder)
{
	if (!$boolLocked)
	{
		$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/sale_order_edit.php?ID=".$ID."&lang=".LANGUAGE_ID.GetFilterParams("filter_"),
			"ICON"=>"btn_edit",
		);
	}
	$aMenu[] = array(
		"TEXT" => GetMessage("SOD_TO_NEW_ORDER"),
		"LINK" => "/thurly/admin/sale_order_edit.php?lang=".LANGUAGE_ID."&SITE_ID=".$arOrder["LID"],
		"ICON"=>"btn_edit",
	);
}
...
if (!$boolLocked && ($saleModulePermissions == "W" || $arOrder["PAYED"] != "Y" && $bUserCanDeleteOrder))
{
	$aMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".GetMessageJS("SODN_CONFIRM_DEL_MESSAGE")."')) window.location='sale_order.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get().urlencode(GetFilterParams("filter_"))."'",
		"WARNING" => "Y",
		"ICON"=>"btn_delete",
	);
}
...
<style type="text/css">
	.bx-adm-bigdata-icon-medium-inner{
		...
		width:24px;
		height: 24px;
		background: url('/thurly/panel/main/images/icons-sprite-7.png') no-repeat center -2176px;
		float: left;
		margin-right: 10px;
	}
</style>
		...
		$tabControl->BeginNextFormTab();
									...
									<div id="editStatusDIV">
										...
										<script type="text/javascript">
											function fChangeStatus()
											{
												...
												BX.showWait();
												BX.ajax.post('/thurly/admin/sale_order_detail.php', '<?=CUtil::JSEscape(thurly_sessid_get())?>&ORDER_AJAX=Y&save_order_data=Y&change_status=Y&STATUS_ID='+BX('STATUS_ID').value+'&ID=<?=$ID?>', fChangeStatusResult);
					...
					<tr>
						<td valign="top">
							...
							<script type="text/javascript">
								...
								{
									BX.showWait();
									BX.ajax.post('/thurly/admin/sale_order_detail.php', '<?=CUtil::JSEscape(thurly_sessid_get())?>&ORDER_AJAX=Y&save_order_data=Y&'+'&change_cancel=Y&CANCELED=N&ID=<?=$ID?>', fCancelCancelOrderResult);
								}
									...
									formCancelOrder.setButtons([
										new BX.PopupWindowButton({
											...
											events : {
												...
												{
													BX.showWait();
													BX.ajax.post('/thurly/admin/sale_order_detail.php', '<?=CUtil::JSEscape(thurly_sessid_get())?>&ORDER_AJAX=Y&save_order_data=Y&'+'&change_cancel=Y&CANCELED=Y&REASON_CANCELED='+BX('FORM_REASON_CANCELED').value+'&ID=<?=$ID?>', fChangeCancelResult);
			...
			$tabControl->AddSection("order_user", GetMessage("P_ORDER_USER_ACC"));
				...
				?>
					<tr>
						...
						{
							$strBuyerProfileUrl = '/thurly/admin/sale_buyers_profile.php?USER_ID='.$arOrder["USER_ID"].'&lang='.LANGUAGE_ID;
						}
						else
						{
							$strBuyerProfileUrl = '/thurly/admin/user_edit.php?ID='.$arOrder["USER_ID"].'&lang='.LANGUAGE_ID;
						}
					...
					foreach ($arOrderPropsValue as $arOrderProps)
					{
						...
						<tr>
							...
							elseif ($arOrderProps["TYPE"] == "LOCATION")
							{
								...
								{
									$locationString = \Thurly\Sale\Location\Admin\LocationHelper::getLocationStringById($arOrderProps['VALUE']);
			...
			$tabControl->AddSection("order_delivery", GetMessage("P_ORDER_DELIVERY_TITLE"));
				...
				$tabControl->BeginCustomField("ORDER_DELIVERY", GetMessage("P_ORDER_DELIVERY"));
					...
					$depList = \Thurly\Sale\Internals\OrderDeliveryReqTable::getList(array(
												...
												click : function()
												{
													...
													if(actionId && actionId.value !== "")
													{
														BX.ajax({
															'method': 'POST',
															'dataType': 'json',
															'url': '/thurly/admin/sale_order_detail.php',
															'data':  '<?=CUtil::JSEscape(thurly_sessid_get())?>&ORDER_AJAX=Y&DELIVERY_ACTION='+actionId.value+'&ID=<?=$ID?>',
							...
							<script type="text/javascript">
								...
								function fEditTrackingNumber(el, type)
								{
									if (type == 'change')
									{
										...
										BX.ajax.post('/thurly/admin/sale_order_detail.php', '<?=CUtil::JSEscape(thurly_sessid_get())?>&ORDER_AJAX=Y&change=Y&tracking_number='+BX.util.urlencode(el.value)+'&ID=<?=$ID?>', fEditTrackingNumberResult);
						...
						<td>
							...
							if (!$boolLocked)
							{
								if (!$crmMode && $arPaySys["PSA_HAVE_RESULT"] == "Y" || strlen($arPaySys["PSA_RESULT_FILE"]) > 0)
								{
									?>
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<a href="/thurly/admin/sale_order_detail.php?ID=<?= $ID ?>&action=ps_update&lang=<? echo LANGUAGE_ID; ?><?echo GetFilterParams("filter_")?>&<?= thurly_sessid_get() ?>"><?echo GetMessage("P_ORDER_PS_STATUS_UPDATE") ?> &gt;&gt;</a>
									<?
								}
							}
							?>
				...
				elseif (!$crmMode && $arPaySys["PSA_HAVE_RESULT"] == "Y" || strlen($arPaySys["PSA_RESULT_FILE"]) > 0)
				{
					...
					<tr>
						...
						if (!$boolLocked)
						{
							?><a href="/thurly/admin/sale_order_detail.php?ID=<?= $ID ?>&action=ps_update&lang=<? echo LANGUAGE_ID; ?><?= GetFilterParams("filter_") ?>&<?= thurly_sessid_get() ?>"><?= GetMessage("P_ORDER_PS_STATUS_UPDATE") ?> &gt;&gt;</a><?
						}
						else
						{
					...
					<tr>
						<td valign="top">
							...
							<script type="text/javascript">
								...
								{
									BX.showWait();
									BX.ajax.post('/thurly/admin/sale_order_detail.php', '<?=CUtil::JSEscape(thurly_sessid_get())?>&ORDER_AJAX=Y&save_order_data=Y&'+'&change_marked=Y&MARKED=N&ID=<?=$ID?>', fCancelMarkOrderResult);
								}
									...
									formMarkOrder.setButtons([
										new BX.PopupWindowButton({
											...
											events : {
												...
												{
													BX.showWait();
													BX.ajax.post('/thurly/admin/sale_order_detail.php', '<?=CUtil::JSEscape(thurly_sessid_get())?>&ORDER_AJAX=Y&save_order_data=Y&'+'&change_marked=Y&MARKED=Y&REASON_MARKED='+BX.util.urlencode(BX('FORM_REASON_MARKED').value)+'&ID=<?=$ID?>', fChangeMarkResult);
													formMarkOrder.close();
												}
											}
										}),
						...
						<script type="text/javascript">
							...
							function fEditComment(el, type)
							{
								if (type == 'change')
								{
									...
									BX.ajax.post('/thurly/admin/sale_order_detail.php', '<?=CUtil::JSEscape(thurly_sessid_get())?>&ORDER_AJAX=Y&change=Y&comment='+BX.util.urlencode(el.value)+'&ID=<?=$ID?>', fEditCommentResult);
									...
									formUndoDeductOrder.setButtons([
										new BX.PopupWindowButton({
											...
											events : {
												...
												{
													BX.showWait();
													var urlparams = '<?=CUtil::JSEscape(thurly_sessid_get())?>&ORDER_AJAX=Y&save_order_data=Y&change_deduct=Y&UNDO_DEDUCT=Y&REASON_UNDO_DEDUCTED='+BX('FORM_REASON_UNDO_DEDUCT').value+'&ID=<?=$ID?>';
													BX.ajax.post('/thurly/admin/sale_order_detail.php', urlparams, fUndoDeductOrderResult);
						...
						<script type="text/javascript">
								...
								*/
								function fGetMoreProduct(type)
								{
									...
									BX.ajax.post('/thurly/admin/sale_order_detail.php', '<?=CUtil::JSEscape(thurly_sessid_get())?>&ORDER_AJAX=Y&type='+type+'&arProduct='+productData+'&currency='+currency+'&LID='+lid+'&userId='+userId+'&fUserId='+fUserId+'&ID=<?=$ID?>', fGetMoreProductResult);
								}
			...
			function addHtml(lastPropCode, arSKU, type)
			{
				...
				for (var i = 0; i < arSKU.length; i++)
				{
					if (arSKU[i]["ID"] == selectedSkuId)
					{
						...
						{
							BX('popup-params-product').value = "/thurly/admin/sale_order_new.php?lang=<? echo LANGUAGE_ID; ?>&user_id="+arSKU[i]["USER_ID"]+"&LID="+arSKU[i]["LID"]+"&product["+arSKU[i]["ID"]+"]=1";
