...
$dbRes = \Thurly\Sale\Delivery\Services\Table::getList(array('select' => array('ID', 'NAME', 'PARENT_ID', 'CLASS_NAME'), 'order' => array('SORT' => 'ASC')));
...
$allowedStatusesView = \Thurly\Sale\DeliveryStatus::getStatusesUserCanDoOperations($USER->GetID(), array('view'));
$allowedStatusesUpdate = \Thurly\Sale\DeliveryStatus::getStatusesUserCanDoOperations($USER->GetID(), array('update'));
...
if($arID = $lAdmin->GroupAction())
{
	...
	$filter['=STATUS.Thurly\Sale\Internals\StatusLangTable:STATUS.LID'] = $lang;
	$filter['=SYSTEM'] = 'N';
	...
	foreach ($shipments as $orderId => $ids)
	{
		...
		/** @var \Thurly\Sale\Order $currentOrder */
		$currentOrder = \Thurly\Sale\Order::load($orderId);
		if (!$currentOrder)
			continue;
...
$select = array(
	'*',
	'STATUS_NAME' => 'STATUS.Thurly\Sale\Internals\StatusLangTable:STATUS.NAME',
	'ORDER.CURRENCY',
...
$arFilter['=STATUS.Thurly\Sale\Internals\StatusLangTable:STATUS.LID'] = $lang;
$arFilter['!=SYSTEM'] = 'Y';
...
{
	$runtimeFields[] = new \Thurly\Main\Entity\ReferenceField(
		'DELIVERY_REQUEST_SHIPMENT',
		\Thurly\Main\Entity\Base::getInstance('\Thurly\Sale\Delivery\Requests\ShipmentTable'),
...
if ($usePageNavigation)
{
	$countQuery = new \Thurly\Main\Entity\Query(ShipmentTable::getEntity());
...
while ($shipment = $dbResultList->Fetch())
{
	...
	$row->AddField("ORDER_USER_NAME", "<a href='/thurly/admin/user_edit.php?ID=".$shipment['ORDER_USER_ID']."&lang=".$lang."'>".htmlspecialcharsbx($shipment['ORDER_USER_NAME'])." ".htmlspecialcharsbx($shipment['ORDER_USER_LAST_NAME'])."</a>");
	...
	if(in_array("DELIVERY_REQUEST_ID", $visibleHeaders))
		$row->AddField("DELIVERY_REQUEST_ID", intval($shipment["DELIVERY_REQUEST_ID"]) > 0 ? '<a href="/thurly/admin/sale_delivery_request_view.php?lang='.LANGUAGE_ID.'&ID='.$shipment["DELIVERY_REQUEST_ID"].'">'.$shipment["DELIVERY_REQUEST_ID"].'</a>' : '');
...
<?
	...
	$res = \Thurly\Sale\Internals\CompanyTable::getList($params);
...
<?
	$statusesList = \Thurly\Sale\DeliveryStatus::getStatusesUserCanDoOperations(
		$USER->GetID(),
		array('view')
	);

	$allStatusNames = \Thurly\Sale\DeliveryStatus::getAllStatusesNames();
?>
<tr>
	<td valign="top"><?echo GetMessage("SALE_ORDER_SHIPMENT_STATUS")?>:<br /><img src="/thurly/images/sale/mouse.gif" width="44" height="21" border="0" alt=""></td>
	<td valign="top">
...
<tr>
	<td><?echo \Thurly\Main\Localization\Loc::getMessage("SALE_SHIPMENT_F_USER_ID");?>:</td>
	<td>
		<?echo FindUserID("filter_user_id", $filter_user_id, "", "find_form");?>
	</td>
</tr>
<tr>
	<td><?echo \Thurly\Main\Localization\Loc::getMessage("SALE_SHIPMENT_F_USER_LOGIN");?>:</td>
	<td>
		<input type="text" name="filter_user_login" value="<?echo htmlspecialcharsbx($filter_user_login)?>" size="40">
	</td>
</tr>
<tr>
	<td><?echo \Thurly\Main\Localization\Loc::getMessage("SALE_SHIPMENT_F_USER_EMAIL");?>:</td>
	<td>
		<input type="text" name="filter_user_email" value="<?echo htmlspecialcharsbx($filter_user_email)?>" size="40">
	</td>
</tr>
<tr>
	<td><?=\Thurly\Main\Localization\Loc::getMessage('SALE_ORDER_DELIVERY_REQ_DELIVERY_ERROR')?>:</td>
	<td>
		<select name="filter_is_delivery_request_failed" class="adm-select">
			<option value="">(<?=\Thurly\Main\Localization\Loc::getMessage('SALE_ORDER_ALL')?>)</option>
			<option value="Y"><?=\Thurly\Main\Localization\Loc::getMessage('SHIPMENT_ORDER_YES')?></option>
			<option value="N"><?=\Thurly\Main\Localization\Loc::getMessage('SHIPMENT_ORDER_NO')?></option>
		</select>
	</td>
</tr>
	<tr>
		<td><?=\Thurly\Main\Localization\Loc::getMessage('SALE_ORDER_DELIVERY_REQ_IS_SENT')?>:</td>
		<td>
			<select name="filter_is_delivery_request_sent" class="adm-select">
				<option value="">(<?=\Thurly\Main\Localization\Loc::getMessage('SALE_ORDER_ALL')?>)</option>
				<option value="Y"><?=\Thurly\Main\Localization\Loc::getMessage('SALE_ORDER_DELIVERY_REQ_IS_SENT_Y')?></option>
				<option value="N"><?=\Thurly\Main\Localization\Loc::getMessage('SALE_ORDER_DELIVERY_REQ_IS_SENT_N')?></option>
			</select>
		</td>
	</tr>
<?
