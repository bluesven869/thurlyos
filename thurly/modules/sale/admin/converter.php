...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["is_ajax"] == "Y")
{
	...
	switch ($ajax_step)
	{
		...
		case 1:	
			...
			$end = microtime(true);
			file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'alter b_sale_order = '.($end-$start)."\n", FILE_APPEND);
		...
		case 2:	
			...
			file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'alter b_sale_order_props_value = '.($end-$start)."\n", FILE_APPEND);
		...
		case 3:	
			...
			file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'alter b_sale_order_props = '.($end-$start)."\n", FILE_APPEND);
		...
		case 10:	
			...
			break;
		case 11:
			if (!CopyDirFiles($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/distr', $_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale', true, true, true))
				$error .= "<br>".str_replace(array('#FROM#', '#TO#'), array($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/distr', $_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale'),Loc::getMessage('SALE_CONVERTER_COPY_FILES_ERROR'));
		...
		case 12:
			...
			{				
				if($DB->TableExists("b_sale_delivery") && !$DB->TableExists("b_sale_delivery_old"))
				{
					...
					file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'CSaleDelivery::convertToNew = '.(microtime(true)-$start)."\n", FILE_APPEND);
				}
				...
				if($DB->TableExists("b_sale_delivery_handler") && !$DB->TableExists("b_sale_delivery_handler_old"))
				{
					...
					file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'CSaleDeliveryHandler::convertToNew = '.(microtime(true)-$start)."\n", FILE_APPEND);
				}			
				...
				if($DB->TableExists("b_sale_delivery2paysystem"))
				{
					...
					file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'CSaleDelivery::convertPSRelations= '.(microtime(true)-$start)."\n", FILE_APPEND);
				}							
			}
			catch(Exception $e)
			{							
		...
		case 13:
			...
			try
			{
				require_once $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/lib/compatible/compatible.php';
				require_once $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/lib/internals/status_grouptask.php';
				require_once $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/general/status.php';
			...
			try
			{
				require_once $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/lib/status.php';
				require_once $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/lib/internals/status.php';
				require_once $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/lib/internals/status_lang.php';
				...
				while ($row = $result->Fetch())
				{
					...
					Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/lib/status.php', $languageId);
			...
			try
			{
				...
				{
					require_once $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/lib/compatible/compatible.php';
					require_once $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/lib/internals/orderprops.php';
					require_once $_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/general/order_props.php';
			...
			$end = microtime(true);
			file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'Migrate statuses and properties = '.($end-$start)."\n", FILE_APPEND);
			...
			break;
		case 14:
			$res = \Thurly\Sale\Internals\PaySystemInner::add();
			if ($res !== false)
			{
			...
			break;		
		case 15:
			if (!CopyDirFiles($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/admin', $_SERVER["DOCUMENT_ROOT"].'/thurly/admin', true, true))
				$error .= "<br>".str_replace(array('#FROM#', '#TO#'), array($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/admin', $_SERVER["DOCUMENT_ROOT"].'/thurly/admin'),Loc::getMessage('SALE_CONVERTER_COPY_FILES_ERROR'));
				
			if (!CopyDirFiles($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/components', $_SERVER["DOCUMENT_ROOT"].'/thurly/components', true, true))
				$error .= "<br>".str_replace(array('#FROM#', '#TO#'), array($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/components', $_SERVER["DOCUMENT_ROOT"].'/thurly/admin'),Loc::getMessage('SALE_CONVERTER_COPY_FILES_ERROR'));
				
			if (!CopyDirFiles($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/images', $_SERVER["DOCUMENT_ROOT"].'/thurly/images/sale', true, true))
				$error .= "<br>".str_replace(array('#FROM#', '#TO#'), array($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/images', $_SERVER["DOCUMENT_ROOT"].'/thurly/images/sale'),Loc::getMessage('SALE_CONVERTER_COPY_FILES_ERROR'));
			
			if (!CopyDirFiles($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/js', $_SERVER["DOCUMENT_ROOT"].'/thurly/js', true, true))
				$error .= "<br>".str_replace(array('#FROM#', '#TO#'), array($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/images', $_SERVER["DOCUMENT_ROOT"].'/thurly/js'),Loc::getMessage('SALE_CONVERTER_COPY_FILES_ERROR'));
				
			if (!CopyDirFiles($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/themes', $_SERVER["DOCUMENT_ROOT"].'/thurly/themes', true, true))
				$error .= "<br>".str_replace(array('#FROM#', '#TO#'), array($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/themes', $_SERVER["DOCUMENT_ROOT"].'/thurly/themes'),Loc::getMessage('SALE_CONVERTER_COPY_FILES_ERROR'));
				
			if (!CopyDirFiles($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/tools', $_SERVER["DOCUMENT_ROOT"].'/thurly/tools', true, true))
				$error .= "<br>".str_replace(array('#FROM#', '#TO#'), array($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale/install/tools', $_SERVER["DOCUMENT_ROOT"].'/thurly/tools'),Loc::getMessage('SALE_CONVERTER_COPY_FILES_ERROR'));
			...
			$arToDelete = array(
				...
				"modules/sale/install/components/thurly/sale.ajax.locations/.description.php",
				"components/thurly/sale.ajax.locations/.description.php",
				...
				"modules/sale/install/components/thurly/sale.location.selector.system/templates/.default/lang/ru/result_modifier.php",
				"components/thurly/sale.location.selector.system/templates/.default/lang/ru/result_modifier.php",
				...
				"modules/sale/install/components/thurly/sale.location.selector.system/templates/.default/lang/de/result_modifier.php",
				"components/thurly/sale.location.selector.system/templates/.default/lang/de/result_modifier.php",
				"modules/sale/install/components/thurly/sale.location.selector.system/templates/.default/lang/en/result_modifier.php",
				"components/thurly/sale.location.selector.system/templates/.default/lang/en/result_modifier.php",
				...
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/lang/de/result_modifier.php",
				"components/thurly/sale.location.import/templates/.default/lang/de/result_modifier.php",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/lang/de/template.php",
				"components/thurly/sale.location.import/templates/.default/lang/de/template.php",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/lang/en/result_modifier.php",
				"components/thurly/sale.location.import/templates/.default/lang/en/result_modifier.php",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/lang/en/template.php",
				"components/thurly/sale.location.import/templates/.default/lang/en/template.php",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/lang/ru/result_modifier.php",
				"components/thurly/sale.location.import/templates/.default/lang/ru/result_modifier.php",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/lang/ru/template.php",
				"components/thurly/sale.location.import/templates/.default/lang/ru/template.php",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/lang/ua/result_modifier.php",
				"components/thurly/sale.location.import/templates/.default/lang/ua/result_modifier.php",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/lang/ua/template.php",
				"components/thurly/sale.location.import/templates/.default/lang/ua/template.php",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/result_modifier.php",
				"components/thurly/sale.location.import/templates/.default/result_modifier.php",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/script.js",
				"components/thurly/sale.location.import/templates/.default/script.js",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/style.css",
				"components/thurly/sale.location.import/templates/.default/style.css",
				"modules/sale/install/components/thurly/sale.location.import/templates/.default/template.php",
				"components/thurly/sale.location.import/templates/.default/template.php",
				...
				"modules/sale/install/components/thurly/sale.discount.coupon.mail/lang/ua/.description.php",
				"components/thurly/sale.discount.coupon.mail/lang/ua/.description.php",
				"modules/sale/install/components/thurly/sale.discount.coupon.mail/lang/ua/.parameters.php",
				"components/thurly/sale.discount.coupon.mail/lang/ua/.parameters.php",
				"modules/sale/install/components/thurly/sale.discount.coupon.mail/lang/ua/class.php",
				"components/thurly/sale.discount.coupon.mail/lang/ua/class.php",
			...
			foreach($arToDelete as $file)
				DeleteDirFilesEx($_SERVER["DOCUMENT_ROOT"]."/thurly/".$file);
			...
			RegisterModuleDependences('conversion', 'OnGetRateClasses' , 'sale', '\Thurly\Sale\Conversion\Handlers', 'OnGetRateClasses' );
			RegisterModuleDependences('sale', 'OnBeforeBasketAdd', 'sale', '\Thurly\Sale\Conversion\Handlers', 'OnBeforeBasketAdd');
			RegisterModuleDependences('sale', 'OnBasketAdd', 'sale', '\Thurly\Sale\Conversion\Handlers', 'OnBasketAdd');
			RegisterModuleDependences('sale', 'OnOrderAdd', 'sale', '\Thurly\Sale\Conversion\Handlers', 'OnOrderAdd');
			RegisterModuleDependences('sale', 'OnSalePayOrder', 'sale', '\Thurly\Sale\Conversion\Handlers', 'OnSalePayOrder');
			...
			$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderPaid', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSalePayOrder');
			$eventManager->unRegisterEventHandler('sale', 'OnSaleBeforeOrderDelete', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onBeforeOrderDelete');
			$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderDeleted', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onOrderDelete');
			$eventManager->unRegisterEventHandler('sale', 'OnSaleShipmentDelivery', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSaleDeliveryOrder');
			
			$eventManager->unRegisterEventHandler('sale', 'OnSaleBeforeOrderCanceled', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSaleBeforeCancelOrder');
			$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderCanceled', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSaleCancelOrder');
			$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderPaidSendMail', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSaleOrderPaidSendMail');
			$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderCancelSendEmail', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSaleOrderCancelSendEmail');

			$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderSaved', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onOrderSave');

			$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderSaved', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onOrderNewSendEmail');

			$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderSaved', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onOrderAdd');
			

			$eventManager->registerEventHandler('sale', 'OnSaleOrderPaid', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSalePayOrder');
			$eventManager->registerEventHandler('sale', 'OnSaleBeforeOrderDelete', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onBeforeOrderDelete');
			$eventManager->registerEventHandler('sale', 'OnSaleOrderDeleted', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onOrderDelete');
			$eventManager->registerEventHandler('sale', 'OnSaleShipmentDelivery', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSaleDeliveryOrder');
			
			$eventManager->registerEventHandler('sale', 'OnSaleBeforeOrderCanceled', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSaleBeforeCancelOrder');
			$eventManager->registerEventHandler('sale', 'OnSaleOrderCanceled', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSaleCancelOrder');
			$eventManager->registerEventHandler('sale', 'OnSaleOrderPaidSendMail', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSaleOrderPaidSendMail');
			$eventManager->registerEventHandler('sale', 'OnSaleOrderCancelSendEmail', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onSaleOrderCancelSendEmail');
			
			$eventManager->registerEventHandler('sale', 'OnSaleOrderSaved', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onOrderAdd');

			$eventManager->registerEventHandler('sale', 'OnSaleOrderSaved', 'sale', '\Thurly\Sale\Compatible\EventCompatibility', 'onOrderSave');
		...
		case 16:
			...
			$end = microtime(true);			
			file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'insert into b_sale_order_payment = '.($end-$start)."\n", FILE_APPEND);
			...
			break;
		case 17:
			$id = \Thurly\Sale\Delivery\Services\EmptyDeliveryService::getEmptyDeliveryServiceId();
			...
			if ($id <= 0)
			{
				...
				$res = \Thurly\Sale\Delivery\Services\Table::add($fields);
				$id = $res->getId();
				...
				$rstrPM = new \Thurly\Sale\Delivery\Restrictions\ByPublicMode();
				$rstrPM->save($fields);
			}
			...
			$end = microtime(true);			
			file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'insert into b_sale_order_delivery = '.($end-$start)."\n", FILE_APPEND);
		...
		case 18:
			...
			$end = microtime(true);			
			file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'insert into b_sale_order_dlv_basket = '.($end-$start)."\n", FILE_APPEND);
		...
		case 19:
			...
			$end = microtime(true);			
			file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'update b_sale_store_barcode = '.($end-$start)."\n", FILE_APPEND);
		...
		case 20:
			...
			$end = microtime(true);			
			file_put_contents($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/sale_convert.txt', 'update b_sale_order = '.($end-$start)."\n", FILE_APPEND);
		...
		case 21:
			if($DB->TableExists('b_report'))
			{
				...
				$replaces = array(
					'sale_SaleOrder' => array(
						'entity' => 'Thurly\\Sale\\Internals\\Order',
						'pattern' => array(
							'/^DATE_INS$/',
							'/^BUYER$/',
							'/^BUYER\./'
					...
					'sale_User' => array(
						'entity' => 'Thurly\\Main\\User',
						'pattern' => array(
							'/^Thurly\\\Sale\\\Order$/',
							'/^Thurly\\\Sale\\\Order:/',
							'/^Thurly\\\Sale\\\Internals\\\Order:BUYER$/',
							'/^Thurly\\\Sale\\\Internals\\\Order:BUYER\./',
							'/^Thurly\\\Sale\\\Internals\\\Order:USER\.DATE_INS$/',
						),
						'replacement' => array(
							'Thurly\\Sale\\Internals\\Order',
							'Thurly\\Sale\\Internals\\Order:',
							'Thurly\\Sale\\Internals\\Order:USER',
							'Thurly\\Sale\\Internals\\Order:USER.',
							'Thurly\\Sale\\Internals\\Order:USER.DATE_INSERT_FORMAT'
						)
					),
					'sale_SaleBasket' => array(
						'entity' => 'Thurly\\Sale\\Internals\\Basket',
						'pattern' => array(
							'/^ORDER\.DATE_INS$/'
						),
						'replacement' => array(
			...
			Thurly\Main\Config\Option::set("main", "~sale_converted_15", 'Y');
		...
		case 22:
			...
			if (empty($error))
			{
				...
				if (\Thurly\Main\ModuleManager::isModuleInstalled('catalog') && \Thurly\Main\Config\Option::get('sale', 'basket_discount_converted'))
				{
					$countQuery = new \Thurly\Main\Entity\Query(\Thurly\Sale\Internals\OrderTable::getEntity());
					$countQuery->addSelect(new \Thurly\Main\Entity\ExpressionField('CNT', 'COUNT(1)'));
					...
					if ((int)$totalCount['CNT'] > 0)
					{
						...
						if (empty($adminNotify))
						{
							$langMess = array();
							$langList = array();
							$languageIterator = \Thurly\Main\Localization\LanguageTable::getList(array(
								'select' => array('ID'),
								'filter' => array('=ACTIVE' => 'Y')
							));
							...
							foreach ($langList as &$oneLanguage)
							{
								...
								$langMess[$oneLanguage] = str_replace(
									'#LINK#',
									'/thurly/admin/settings.php?lang='.$oneLanguage.'&mid=sale',
									$mess[$messID]
								);
							}
...
<div id="result">
	<div style = "width : 100px; float: left;" id="img_wrapper">
		<img src='/thurly/images/sale/magaz.png'>
	</div>
...
switch ($step)
{
	...
	case 4:
		if(!CSaleLocation::isLocationProMigrated())
		{
			$migrator = new \Thurly\Sale\Location\Migration\CUpdaterLocationPro();
