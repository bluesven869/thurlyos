...
{
	require($DOCUMENT_ROOT."/thurly/modules/main/include/prolog_admin_after.php");
...
if(Thurly\Main\Loader::includeModule("catalog"))
	$catalogSubscribeEnabled = class_exists('\Thurly\Catalog\SubscribeTable');
...
if(isset($_REQUEST["reorder"]) && IntVal($_REQUEST["reorder"]) > 0)
{
	...
	$dbBasketList = \Thurly\Sale\Internals\BasketTable::getList(array(
	...
	LocalRedirect("/thurly/admin/sale_order_create.php?USER_ID=".CUtil::JSEscape($ID)."&lang=".LANG."&SITE_ID=".CUtil::JSEscape($lid).CUtil::JSEscape($urlProduct));
}
...
//basket
if (isset($_REQUEST['apply']) && isset($_REQUEST['action']) && $saleModulePermissions >= "W" && check_thurly_sessid())
{
	$arID = array();
	$arFields = array();
	...
	if($arFuserItems = CSaleUser::GetList(array("USER_ID" => $ID)))
	{
		...
		else
		{
			...
			$dbBasketEl = \Thurly\Sale\Internals\BasketTable::getList(array(
				...
				case "order_basket":
					if (count($arID[$LID]) > 0)
					{
						...
						if (strlen($basketError) <= 0)
						{
							...
							echo "window.parent.location.href = '/thurly/admin/sale_order_create.php?USER_ID=".CUtil::JSEscape($ID)."&lang=".LANG."&SITE_ID=".CUtil::JSEscape($LID).CUtil::JSEscape($urlProduct)."';";
							echo "</script>";
							exit;
						}
					}
...
if(!empty($arUser))
{
	...
	$orderStatusNames = \Thurly\Sale\OrderStatus::getAllStatusesNames(LANGUAGE_ID);
	...
	while ($arOrderMain = $dbOrderList->Fetch())
	{
		...
		$basketCount = 0;
		$dbBasketCount = \Thurly\Sale\Internals\BasketTable::getList(array(
			                                                          'filter' => array(
	...
	while ($arProfList = $dbProfileList->GetNext())
	{
		...
		$row->AddField("NAME", "[".$arProfList["ID"]."] <a href=\"/thurly/admin/sale_buyers_profile_edit.php?id=".$arProfList["ID"]."&lang=".LANG."\">".$arProfList["NAME"]."</a>");
...
//			                                           "DATE_ALLOW_DELIVERY"
	...
	$dbOrderList = new CAdminResult(\Thurly\Sale\Internals\OrderTable::getList($getListParams), $sTableID_tab3);
...
//	$dbOrderList->NavStart();
	...
	while ($arOrder = $dbOrderList->Fetch())
	{
		...
		$status_id = "<a title=\"".GetMessage('BUYERS_ORDER_DETAIL_PAGE')."\" href=\"/thurly/admin/sale_order_view.php?ID=".$arOrder["ID"]."&lang=".LANG."\">".GetMessage('BUYERS_PREF').$arOrder["ID"]."</a>";
		...
		while($payment = $res->fetch())
		{
			...
			$payed .= "[<a href='/thurly/admin/sale_order_payment_edit.php?order_id=".$arOrder['ID']."&payment_id=".$payment["ID"]."&lang=".LANGUAGE_ID."'>".$payment["ID"]."</a>], ".
				htmlspecialcharsbx($payment["PAY_SYSTEM_NAME"]).", ".
				($payment["PAID"] == "Y" ? \Thurly\Main\Localization\Loc::getMessage("SOB_PAYMENTS_PAID") :  \Thurly\Main\Localization\Loc::getMessage("SOB_PAYMENTS_UNPAID")).", ".
				(strlen($payment["PS_STATUS"]) > 0 ? \Thurly\Main\Localization\Loc::getMessage("SOB_PAYMENTS_STATUS").": ".htmlspecialcharsbx($payment["PS_STATUS"]).", " : "").
		...
		$dbRes = \Thurly\Sale\Internals\StatusTable::getList(array(
		...
		{
			$shipment["ID_LINKED"] = '[<a href="/thurly/admin/sale_order_shipment_edit.php?order_id='.$arOrder['ID'].'&shipment_id='.$shipment["ID"].'&lang='.LANGUAGE_ID.'">'.$shipment["ID"].'</a>]';


			if (strval($allowDelivery) != "")
				$allowDelivery .= "<hr>";

			$allowDelivery .= "[<a href='/thurly/admin/sale_order_shipment_edit.php?order_id=".$arOrder['ID']."&shipment_id=".$shipment["ID"]."&lang=".LANGUAGE_ID."'>".$shipment["ID"]."</a>], ".
				...
				($shipment["ALLOW_DELIVERY"] == "Y" ? \Thurly\Main\Localization\Loc::getMessage("SOB_SHIPMENTS_ALLOW_DELIVERY") : \Thurly\Main\Localization\Loc::getMessage("SOB_SHIPMENTS_NOT_ALLOW_DELIVERY")).", ".
				($shipment["CANCELED"] == "Y" ? \Thurly\Main\Localization\Loc::getMessage("SOB_SHIPMENTS_CANCELED").", " : "").
				($shipment["DEDUCTED"] == "Y" ? \Thurly\Main\Localization\Loc::getMessage("SOB_SHIPMENTS_DEDUCTED").", " : "").
				($shipment["MARKED"] == "Y" ? \Thurly\Main\Localization\Loc::getMessage("SOB_SHIPMENTS_MARKED").", " : "").
				(strlen($shipment["TRACKING_NUMBER"]) > 0 ? htmlspecialcharsbx($shipment["TRACKING_NUMBER"]).", " : "");

			if(strlen($shipment["STATUS_ID"]) > 0)
				$allowDelivery .= $shipmentStatuses[$shipment["STATUS_ID"]] ? htmlspecialcharsbx($shipmentStatuses[$shipment["STATUS_ID"]]) : \Thurly\Main\Localization\Loc::getMessage("SOB_SHIPMENTS_STATUS").": ".$shipment["STATUS_ID"];
		...
		$arActions[] = array("ICON"=>"edit", "TEXT"=>GetMessage("BUYER_PD_REORDER"),"ACTION"=>$lAdmin_tab3->ActionRedirect("/thurly/admin/sale_buyers_profile.php?USER_ID=".$ID."&lang=".LANG."&reorder=".$arOrder["ID"]."&lid=".$arOrder["LID"]));
	...
	if(\Thurly\Main\Loader::includeModule("catalog"))
	{
		$viewedQuery = new \Thurly\Main\Entity\Query(\Thurly\Catalog\CatalogViewedProductTable::getEntity());
	...
	Thurly\Main\Type\Collection::sortByColumn($arViewsData, $arViewSort);
	...
	foreach ($arViewsData as $arViews)
	{
		...
		if (count($arResult["SKU_ELEMENTS"]) > 0):
			...
			$linkOrder = 'BX.adminPanel.Redirect([], \'/thurly/admin/sale_order_create.php?USER_ID='.$ID.'&lang='.LANG.'&SITE_ID='.$arViews["SITE_ID"].'&product['.$arViews["PRODUCT_ID"].']=1\', event);';
	...
	if($catalogSubscribeEnabled)
	{
		...
		$nav = new Thurly\Main\UI\AdminPageNavigation('buyers-subscription-list');
		$queryObject = Thurly\Catalog\SubscribeTable::getList(array(
			'select' => $select,
			'filter' => $filter,
			'order' => $queryOrder,
			'count_total'=>true,
		...
		$contactType = Thurly\Catalog\SubscribeTable::getContactTypes();
		...
		if (!empty($listUserData))
		{
			...
			while($user = $userQuery->fetch())
			{
				if(is_array($listUserData[$user['ID']]))
				{
					...
					{
						$userString='<a href="/thurly/admin/user_edit.php?ID='.$user['ID'].'&lang='.LANGUAGE_ID.'" target="_blank">'.
	...
	$arMenu = array(
		array(
			...
			"LINK" => "/thurly/admin/sale_buyers.php?lang=".LANGUAGE_ID.GetFilterParams("filter_"),
			"ICON" => "btn_list",
		),
		array(
			"TEXT"=>GetMessage("BUYER_NEW_ORDER"),
			"LINK" => "/thurly/admin/sale_order_create.php?lang=".LANGUAGE_ID.$siteLID,
			...
			<td colspan="2">
				...
				<tr>
					...
					<td class="adm-detail-content-cell-r">
						<div><a href="/thurly/admin/user_edit.php?ID=<?=$u_ID?>&lang=<?=LANG?>"><?=$u_LOGIN?></a></div>
					</td>
				</tr>
				<?if(strlen($userFIO) > 0):?>
					<tr>
		...
		<tr>
			<td colspan="2">
				...
				<script>
						...
						{
							t_stat_list_tab5.GetAdminList('/thurly/admin/sale_buyers_profile.php?USER_ID=<?=$ID?>&lang=<?=LANGUAGE_ID?>&action=viewed_apply&viewed_id='+product_id+'&viewed_lid='+lid);
							t_stat_list_tab4.GetAdminList('/thurly/admin/sale_buyers_profile.php?USER_ID=<?=$ID?>&lang=<?=LANGUAGE_ID?>');
						}
								...
								if (arSKU[i]["ID"] == selectedSkuId)
								{
									...
									if (arSKU[i]["CAN_BUY"] == "Y")
									{
										...
										else
										{
											...
											BX('viewed_url_action').value = '/thurly/admin/sale_order_create.php?USER_ID='+arSKU[i]["USER_ID"]+'&lang=<?=LANG?>&SITE_ID='+arSKU[i]["LID"]+'&product['+arSKU[i]["ID"]+']=1';
