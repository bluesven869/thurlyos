...
use Thurly\Sale\TradingPlatform\Vk;
...
\Thurly\Main\Page\Asset::getInstance()->addJs("/thurly/js/sale/vk_admin.js", true);
...
//		logger my throw exception
	...
	{
		$response = Thurly\Main\Web\Json::decode($responseStr);
	...
	<tr class="adm-detail-required-field">
		<td>
			<?= ShowJSHint(Loc::getMessage("SALE_VK_SETTINGS_EXPORT_SETTING_MANUAL", array(
				"#A1" => "<a href=/thurly/admin/sale_vk_manual.php?lang=" . LANGUAGE_ID . ">",
				"#A2" => '</a>',
			))); ?>
	...
	<? if ($vk->isActive() && $vk->isActiveById($exportId)): ?>
		...
		<tr>
			<td colspan="2"><input type="hidden" name="VK[LOG_LEVEL]"
								   value="<?= \Thurly\Sale\TradingPlatform\Logger::LOG_LEVEL_DEBUG ?>"></td>
		</tr>
	...
	<tr>
		<td>
			...
			<input type="button" style="margin-right:10px" id="vk_export_button__cancel"
				   onclick="window.location='/thurly/admin/sale_vk_export_list.php?lang=<?= LANGUAGE_ID ?>'"
				   name="cancel"
