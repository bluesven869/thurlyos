...
if(($ids = $lAdmin->GroupAction()) && $saleModulePermissions >= "W" && check_thurly_sessid())
{
	foreach ($ids as $id)
	{
		if(intval($id) <= 0)
...
$nav = new \Thurly\Main\UI\AdminPageNavigation("nav-delivery-requests");
...
while($fields = $resRequestList->fetch())
{
	...
	if($delivery = \Thurly\Sale\Delivery\Services\Manager::getObjectById($fields['DELIVERY_ID']))
	...
	$row->AddViewField(
		'DELIVERY_ID',
		'<a href="/thurly/admin/sale_delivery_service_edit.php?lang='.LANGUAGE_ID.'&ID='.$fields['DELIVERY_ID'].'&PARENT_ID='.$delivery->getParentId().'">'.htmlspecialcharsbx($deliveryServiceName).'</a>'
	);
...
//	$row->AddViewField("STATUS", $fields['STATUS']);
	...
	if(in_array("ORDERS_NUMBER", $arVisibleColumns) || in_array("SHIPMENTS_NUMBER", $arVisibleColumns))
	{
		...
		while($req = $reqRes->fetch())
		{
			...
			$orderNumbers .= '<a href="/thurly/admin/sale_order_view.php?ID='.$req['ORDER_ID'].'&lang='.LANGUAGE_ID.'">'.
				$req['ORDER_NUMBER'].'</a>';
		}
