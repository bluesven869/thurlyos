...
if (!empty($filter_product_id))
{
	if ((int)($filter_product_id) > 0)
	{
		$runtimeFields["ARCHIVED_BASKET_ITEMS"] = array(
			'select' => "PRODUCT_ID",
			'data_type' => '\Thurly\Sale\Internals\BasketArchiveTable',
			'reference' => array(
...
$nav = new \Thurly\Main\UI\AdminPageNavigation("nav-archive");

$orderIterator = \Thurly\Sale\Internals\OrderArchiveTable::getList(array(
	'filter' => $arFilterTmp,
...
if (!empty($orderList) && is_array($orderList))
{
	...
	{
		$dbItemsList = \Thurly\Sale\Internals\BasketArchiveTable::getList(array(
			'order' => array('ID' => 'ASC'),
	...
	foreach ($orderList as $id => $arOrder)
	{
		...
		$idTmp = '<table>
						<tr>
							<td valign="top"></td>
							<td><a href="/thurly/admin/sale_order_archive_view.php?ID=##ID##"><b>'.Loc::getMessage("SO_ORDER_ID_PREF").'##ID##</b></a></td>
						</tr>
					</table>';
		...
		{
			$arActions[] = array("ICON"=>"copy", "TEXT"=>Loc::getMessage("SOA_RESTORE_ORDER"), "ACTION"=>$lAdmin->ActionRedirect("sale_order_create.php?restoreID=".$arOrder['ID']."&lang=".LANGUAGE_ID."&".'SITE_ID='.$arOrder['LID'].'&'.thurly_sessid_get().GetFilterParams("filter_")));
		}

		$arActions[] = array("ICON"=>"view", "TEXT"=>Loc::getMessage("SALE_DETAIL_DESCR"), "ACTION"=>$lAdmin->ActionRedirect("sale_order_archive_view.php?ID=".$arOrder['ID']."&lang=".LANGUAGE_ID."&".'SITE_ID='.$arOrder['LID'].'&'.thurly_sessid_get()), "DEFAULT"=>true);
		if ($saleModulePermissions == "W")
		{
