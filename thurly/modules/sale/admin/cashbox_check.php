...
use Thurly\Sale\Cashbox\Internals;
...
elseif($set_filter!="Y" && $del_filter != "Y")
{
	...
	$filter[">=DATE_CREATE"] = new \Thurly\Main\Type\Date();
}
...
if ($usePageNavigation)
{
	$countQuery = new \Thurly\Main\Entity\Query(Internals\CashboxCheckTable::getEntity());
...
{
	$linkId = '[<a href="/thurly/admin/sale_order_payment_edit.php?order_id='.$payment['ORDER_ID'].'&payment_id='.$payment["ID"].'&lang='.LANGUAGE_ID.'">'.$payment["ID"].'</a>]';
...
{
	$linkId = '[<a href="/thurly/admin/sale_order_shipment_edit.php?order_id='.$shipment['ORDER_ID'].'&shipment_id='.$shipment["ID"].'&lang='.LANGUAGE_ID.'">'.$shipment["ID"].'</a>]';
...
while ($check = $dbResultList->Fetch())
{
	...
	{
		$cashbox = \Thurly\Sale\Cashbox\Manager::getObjectById($check['CASHBOX_ID']);
