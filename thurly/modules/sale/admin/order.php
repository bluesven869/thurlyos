...
$dbProps = \Thurly\Sale\Internals\OrderPropsTable::getList(array(
...
if(!empty($filter_product_id) || !empty($filter_product_xml_id))
{
	...
	if (strval(trim($filter_product_xml_id)) != "")
	{
		...
		/** @var \Thurly\Main\DB\Connection $connection */
		$connection = \Thurly\Main\Application::getConnection();
		/** @var \Thurly\Main\DB\SqlHelper $sqlHelper */
...
$allowedStatusesView = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($intUserID, array('view'));
...
$dbRes = \Thurly\Sale\Services\Company\Manager::getList(array(
...
if($lAdmin->EditAction() && $saleModulePermissions >= "U")
{
	foreach ($FIELDS as $ID => $arFields)
	{
		...
		/** @var \Thurly\Sale\Order $editOrder */
		$editOrder = \Thurly\Sale\Order::load($ID);
		...
		if($editOrder)
		{
			...
			{
				$statusesList = \Thurly\Sale\OrderStatus::getAllowedUserStatuses(
					$USER->GetID(),
					\Thurly\Sale\OrderStatus::getInitialStatus()
				);
...
$bShowBasketProps = ((string)\Thurly\Main\Config\Option::get('sale', 'show_basket_props_in_order_list') == 'Y');
...
foreach ($arOrderPropsCode as $key => $value)
{
	...
	)
	{
		...
		$runtimeFields['PROP_'.$propIterator] = array(
			'data_type' => 'Thurly\Sale\Internals\OrderPropsValueTable',
			'reference' => array(
...
if(($arID = $lAdmin->GroupAction()) && $saleModulePermissions >= "P")
{
	...
	$dbOrderList = \Thurly\Sale\Internals\OrderTable::getList(array(
	...
	foreach ($arID as $ID)
	{
		...
		{
			/** @var \Thurly\Sale\Order $saleOrder */
			if(!($saleOrder = \Thurly\Sale\Order::load($ID)))
			{
		...
		else
		{
			...
			{
				case "delete":
					$allowedStatusesDelete = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($intUserID, array('delete'));
					...
					$res = \Thurly\Sale\Order::delete($ID);
					if(!$res->isSuccess())
				...
				case "cancel":
					$allowedStatusesCancel = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($intUserID, array('cancel'));
				...
				case "update_payment_status":
					...
					foreach ($saleOrder->getPaymentCollection() as $payment)
					{
						...
						$allowedStatusesView = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($USER->GetID(), array('view'));
				...
				case "paid_n":
					...
					foreach ($saleOrder->getPaymentCollection() as $payment)
					{
						...
						$allowedStatusesUpdate = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($USER->GetID(), array('update'));
					...
					if(substr($_REQUEST['action'], 0, strlen("status_")) == "status_")
					{
						...
						if(strlen($statusID) > 0)
						{
							$resStatus = StatusTable::getList(array(
								'select' => array('ID', 'NAME' => 'Thurly\Sale\Internals\StatusLangTable:STATUS.NAME'),
								'filter' => array(
									'=Thurly\Sale\Internals\StatusLangTable:STATUS.LID' => LANGUAGE_ID,
									'=ID' => $statusID
								),
							));
							...
							{
								$allowedStatusesTo = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($intUserID, array('to'));
								if(in_array($statusID, $allowedStatusesTo))
								{
									if($arAffectedOrders[$ID]["STATUS_ID"] != $statusID)
									{
										$saleOrder = \Thurly\Sale\Order::load($ID);
...
while ($arSite = $dbSite->Fetch())
{
	...
	if(strlen($serverName[$arSite["LID"]]) <= 0)
	{
		if(defined("SITE_SERVER_NAME") && strlen(SITE_SERVER_NAME) > 0)
			...
			$serverName[$arSite["LID"]] = \Thurly\Main\Config\Option::get("main", "server_name", "");
	}

	$WEIGHT_UNIT[$arSite["LID"]] = htmlspecialcharsbx(\Thurly\Main\Config\Option::get('sale', 'weight_unit', "", $arSite["LID"]));
	$WEIGHT_KOEF[$arSite["LID"]] = htmlspecialcharsbx(\Thurly\Main\Config\Option::get('sale', 'weight_koef', 1, $arSite["LID"]));
}
...
if(array_key_exists('=SOURCE.TRADING_PLATFORM_ID', $arFilterTmp) || in_array('SOURCE_NAME', $arSelectFields))
{
	$runtimeFields['SOURCE'] = array(
		'data_type' => 'Thurly\Sale\TradingPlatform\OrderTable',
		'reference' => array(
			'ref.ORDER_ID' => 'this.ID',
		),
		'join_type' => 'left'
...
/** @var \Thurly\Main\Entity\Event $event */
$event = new \Thurly\Main\Event('sale', Sale\EventActions::EVENT_ON_ADMIN_ORDER_LIST, $getListParams);
$event->send();

if ($event->getResults())
{
	/** @var \Thurly\Main\EventResult $eventResult */
	foreach($event->getResults() as $eventResult)
	{
		if($eventResult->getType() == \Thurly\Main\EventResult::ERROR)
		{
			$errorMsg = new Sale\ResultError(\Thurly\Main\Localization\Loc::getMessage('SALE_EVENT_ON_ADMIN_ORDER_LIST_ERROR'), 'SALE_EVENT_ON_ADMIN_ORDER_LIST_ERROR');
		...
		elseif($eventResult->getType() == \Thurly\Main\EventResult::SUCCESS)
		{
...
$dbOrderList = new CAdminResult(\Thurly\Sale\Internals\OrderTable::getList($getListParams), $sTableID);
...
{
	/*
	 * $dbProp = \Thurly\Sale\Internals\BasketPropertyTable::getList(array(
	 ...
	 */
	if(\Thurly\Main\Analytics\Catalog::isOn() || $bNeedBasket)
	{
		$basketItemIds = array();
		$dbItemsList = \Thurly\Sale\Internals\BasketTable::getList(array(
						   'order' => array('ID' => 'ASC'),
	...
	if ($bShowBasketProps && !empty($basketItemIds))
	{
		...
		{
			$resProp = \Thurly\Sale\Internals\BasketPropertyTable::getList(array(
				'order' => array("SORT" => "ASC", "ID" => "ASC"),
				'filter' => array(
					"BASKET_ID" => $basketIds,
	...
	foreach ($orderList as $orderId => $arOrder)
	{
		...
		$lamp = "/thurly/images/sale/".$arOrder['LOCK_STATUS'].".gif";
		...
		$idTmp .=" href='/thurly/admin/sale_order_view.php?ID=".$orderId.GetFilterParams("filter_")."&lang=".LANGUAGE_ID."' title='".Loc::getMessage("SALE_DETAIL_DESCR")."'>##ID##</a></b></td>";
		$idTmp .= "</tr>";
		...
		if(in_array("PERSON_TYPE", $arVisibleColumns))
		{
			...
			$fieldValue = "[";
			if($saleModulePermissions >= "W")
				$fieldValue .= '<a href="/thurly/admin/sale_person_type.php?lang='.LANGUAGE_ID.'">';
		...
		if(in_array("STATUS", $arVisibleColumns))
		{
			if($row->bEditMode == true)
			{
				if($saleModulePermissions < "W")
				{
					$allowedStatusesFrom = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($intUserID, array('from'));
				...
				if(in_array("STATUS", $arVisibleColumns))
				{
						...
						|| empty($LOCAL_STATUS_CACHE[$arOrder["STATUS_ID"]]))
					{
						$arStatus =  StatusTable::getList(array(
							'select' => array(
								'NAME' => 'Thurly\Sale\Internals\StatusLangTable:STATUS.NAME',
								'COLOR'
							),
							'filter' => array(
					...
					$fieldValue .= "[";
					if($saleModulePermissions >= "W")
						$fieldValue .= '<a href="/thurly/admin/sale_status_edit.php?ID='.$arOrder["STATUS_ID"].'&lang='.LANGUAGE_ID.'">';
		...
		if(in_array("STATUS_ID", $arVisibleColumns))
		{
				...
				|| $row->bEditMode && !$isStatusAllowed)
			{
				...
				if(in_array("STATUS_ID", $arVisibleColumns))
				{
					...
					if($saleModulePermissions >= "W")
						$fieldValueTmp .= '<a href="/thurly/admin/sale_status_edit.php?ID='.$arOrder["STATUS_ID"].'&lang='.LANGUAGE_ID.'">';
		...
		{
			$payments = array();
			/** @var \Thurly\Main\DB\Result $res */
			$res = \Thurly\Sale\Internals\PaymentTable::getList(array(
				'order' => array('ID' => 'ASC'),
				'filter' => array('ORDER_ID' => $arOrder['ID'])
			));
			while($payment = $res->fetch())
			{
				$payment["ID_LINKED"] = '[<a href="/thurly/admin/sale_order_payment_edit.php?order_id='.$arOrder['ID'].'&payment_id='.$payment["ID"].'&lang='.LANGUAGE_ID.'">'.$payment["ID"].'</a>]';
				$payments[] = $payment;
			}
			unset($payment, $res);
			...
			if(in_array("PAY_SYSTEM", $arVisibleColumns))
			{
				foreach($payments as $payment)
				{
					...
					if($saleModulePermissions >= "W")
						$tmp .= '<a href="/thurly/admin/sale_pay_system_edit.php?ID='.$payment["PAY_SYSTEM_ID"].'&lang='.LANGUAGE_ID.'">';
		...
		if($bNeedProps)
		{
			/** @var \Thurly\Sale\Order $propOrder */
			$propOrder = \Thurly\Sale\Order::load($arOrder["ID"]);

			/** @var Thurly\Sale\PropertyValue  $property */
		...
		{
			$shipments = array();
			$res = \Thurly\Sale\Internals\ShipmentTable::getList(array(
				'order' => array('ID' => 'ASC'),
			...
			{
				$shipment["ID_LINKED"] = '[<a href="/thurly/admin/sale_order_shipment_edit.php?order_id='.$arOrder['ID'].'&shipment_id='.$shipment["ID"].'&lang='.LANGUAGE_ID.'"  title="'.Loc::getMessage('SALE_O_SHIPMENT_ID_TITLE', array('#SHIPMENT_ID#' => $shipment["ID"])).'">'.$shipment["ID"].'</a>]';
				$shipments[] = $shipment;
			}
			...
			if(in_array("DELIVERY", $arVisibleColumns))
			{
				foreach($shipments as $shipment)
				{
					...
					if($saleModulePermissions >= "W")
						$tmp .= '<a href="/thurly/admin/sale_delivery_service_edit.php?ID='.$shipment["DELIVERY_ID"].'&lang='.LANGUAGE_ID.'">';
		...
		if(in_array("COMPANY_ID", $arVisibleColumns) && intval($arOrder["COMPANY_ID"]) > 0)
		{
			...
			$fieldValue = "[";
			if($saleModulePermissions >= "W")
				$fieldValue .= '<a href="/thurly/admin/sale_company_edit.php?lang='.LANGUAGE_ID.'&ID='.$companyId.'">';
		...
		$arActions[] = array("ICON"=>"copy", "TEXT"=>Loc::getMessage("SOA_ORDER_COPY"), "ACTION"=>$lAdmin->ActionRedirect("sale_order_create.php?ID=".$orderId."&lang=".LANGUAGE_ID."&".'SITE_ID='.$arOrder['LID'].'&'.thurly_sessid_get().GetFilterParams("filter_")));
...
$runtime = array(
	new \Thurly\Main\Entity\ExpressionField('SUM', 'SUM(sale_internals_order.PRICE)')
);
...
if($saleModulePermissions == "W")
{
	$dbOrderList = \Thurly\Sale\Internals\OrderTable::getList($getListParamsSum);
	...
	$runtime = array(
			new \Thurly\Main\Entity\ExpressionField('SUM', 'SUM(%s * %s)', array('BASKET.PRICE', 'BASKET.QUANTITY')),
			new \Thurly\Main\Entity\ExpressionField('COUNT', 'COUNT(sale_internals_order.ID)')
	);
...
elseif ($saleModulePermissions < "W")
{
	...
	$dbOrderList = \Thurly\Sale\Internals\OrderTable::getList($orderFilter);
...
?>
	...
	<div class="adm-c-bigdatabar-container">
		...
		<div class="adm-c-bigdatabar-content">
			...
			<div class="adm-c-bigdatabar-line">
				<? $installed = (time()-Thurly\Main\Config\Option::get('main', 'rcm_component_usage', 0)<3600*24);?>
				<? if($installed): ?>
				...
				<? $available = \Thurly\Main\Analytics\Catalog::isOn(); ?>
				<? if($available): ?>
...
if($saleModulePermissions >= 'P')
{
	$allowedStatusesUpdate = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($intUserID, array('update'));
}
...
\Thurly\Main\Page\Asset::getInstance()->addString('<style>.adm-filter-item-center, .adm-filter-content {overflow: visible !important;}</style>');
...
?>
	...
	<tr>
		<td valign="top"><?echo Loc::getMessage("SALE_F_STATUS")?>:<br /><img src="/thurly/images/sale/mouse.gif" width="44" height="21" border="0" alt=""></td>
		<td valign="top">
			<select name="filter_status[]" multiple size="3">
				<?
				$statusesList = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations(
					$USER->GetID(),
					array('view')
				);

				$allStatusNames = \Thurly\Sale\OrderStatus::getAllStatusesNames();
	...
	<tr>
		...
		<td>
			...
			<select name="filter_pay_system[]" multiple size="3">
				<option value=""><?echo GetMessage("SALE_F_ALL")?></option>
				<?
				$res = \Thurly\Sale\PaySystem\Manager::getList(array(
					'select' => array('ID', 'NAME'),
				...
				if ($paySystemList):
					$dbRestRes = Sale\Services\PaySystem\Restrictions\Manager::getList(array(
						...
						'filter' => array(
							'=CLASS_NAME' => '\Thurly\Sale\Services\PaySystem\Restrictions\PersonType',
		...
		<td>
			...
			function makeProductSearchDialog(params)
			{
				...
				var popup = new BX.CDialog({
					content_url: '/thurly/tools/sale/product_search_dialog.php?lang='+lang+'&LID='+site_id+'&caller=' + caller + '&func_name='+callback+'&STORE_FROM_ID='+store_id,
	...
	<tr>
		...
		<td>
			...
			<input type="button" class="button" name="FindAffiliate" OnClick="window.open('/thurly/admin/sale_affiliate_search.php?func_name=SetAffiliateID', '', 'scrollbars=yes,resizable=yes,width=800,height=500,top='+Math.floor((screen.height - 500)/2-14)+',left='+Math.floor((screen.width - 400)/2-5));" value="...">
			...
			function ChangeAffiliateName()
			{
				if(affiliateID != document.find_form.filter_affiliate_id.value)
				{
					...
					if(affiliateID != '' && !isNaN(parseInt(affiliateID, 10)))
					{
						...
						window.frames["hiddenframe_affiliate"].location.replace('/thurly/admin/sale_affiliate_get.php?ID=' + affiliateID + '&func_name=SetAffiliateName');
					}
					else
	...
	<?
		...
		$dbRes = Thurly\Sale\TradingPlatformTable::getList(array(
			'select' => array('ID', 'NAME')
		));
			...
			<tr>
				...
				<td valign="top" style="overflow: visible; ">
					...
					if($value["TYPE"] == "ENUM")
					{
						...
						$inputParams["OPTIONS"] = $inputParams["OPTIONS"] + \Thurly\Sale\PropertyValue::loadOptions($value["ID"]);
					}

					echo \Thurly\Sale\Internals\Input\Manager::getFilterEditHtml(
						"filter_prop_".$key,
						$inputParams,
						${"filter_prop_".$key}
					);
...
<?$spotlight = new \Thurly\Main\UI\Spotlight("DELIVERY_REQUESTS_ADDED");?>
