...
$orderRes = \Thurly\Sale\Internals\OrderTable::getList($orderParams);
...
if (empty($errorMsg))
{
	$allowedStatusesOrderView = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($USER->GetID(), array('view'));
...
if (empty($errorMsg) && $_SERVER["REQUEST_METHOD"] == "GET" && check_thurly_sessid())
{
	$doc = (string)$_GET['doc'];
	if (strlen($doc) == 0)
	{
	...
	else
	{
		if (isset($_GET['SHIPMENT_ID']) && intval($_GET['SHIPMENT_ID']) > 0)
		{
			...
			$shipmentRes = \Thurly\Sale\Internals\ShipmentTable::getList($shipmentParams);
			if ($shipment = $shipmentRes->fetch())
			{
				$allowedStatusesDeliveryView = \Thurly\Sale\DeliveryStatus::getStatusesUserCanDoOperations($USER->GetID(), array('view'));
				...
				if (strval($errorMsg) === '')
				{
					...
					$items = \Thurly\Sale\Internals\ShipmentItemTable::getList($params);
					$quantity = array();
					$basketIds = array();
					...
					LocalRedirect('/thurly/admin/sale_print.php?PROPS_ENABLE=Y&doc='.CUtil::JSEscape($doc).'&ORDER_ID='.$orderId.'&'.$urlParams.'&SHIPMENT_ID='.$shipmentId);
				}
			}
			else
			{
		...
		elseif ($allowOrderView)
		{
			...
			LocalRedirect('/thurly/admin/sale_print.php?PROPS_ENABLE=Y&doc='.CUtil::JSEscape($doc).'&ORDER_ID='.$orderId.'&'.$urlParams);
		}
		else
		{
			$errorMsg .= GetMessage('SOP_ERROR_ACCESS');
