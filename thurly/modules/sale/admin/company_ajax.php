...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/lib/services/company/inputs.php");

$saleModulePermissions = $APPLICATION->GetGroupRight("sale");

if($arResult["ERROR"] === '' && $saleModulePermissions >= "W" && check_thurly_sessid())
{
	...
	switch ($action)
	{
		case "get_rule_params_html":
			...
			if ($className == '\Thurly\Sale\Services\Company\Restrictions\Price')
			...
			foreach ($paramsStructure as $name => $param)
			{
				$paramsField .= "<tr>".
					...
					"<td>".\Thurly\Sale\Internals\Input\Manager::getEditHtml("RULE[".$name."]", $param, (isset($params[$name]) ? $params[$name] : null))."</td>".
					"</tr>";
			}
		...
		case "save_rule":
			...
			if(!$companyId)
				throw new \Thurly\Main\ArgumentNullException("companyId");
		...
		case "delete_rule":
			...
			if(!$ruleId)
				throw new \Thurly\Main\ArgumentNullException('ruleId');
...
function getRuleHtml($companyId)
{
	...
	ob_start();
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/admin/company_rules_list.php");
