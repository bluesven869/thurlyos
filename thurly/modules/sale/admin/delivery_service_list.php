...
\Thurly\Main\Page\Asset::getInstance()->addJs("/thurly/js/sale/delivery.js");
...
if(!empty($_REQUEST["SHOW_GROUPS"]) && $_REQUEST["SHOW_GROUPS"] == 'Y')
{
	unset($del_filter);
	$set_filter='Y';
	$filter_class_name = '\Thurly\Sale\Delivery\Services\Group';
}
...
{
	$handlersList = \Thurly\Sale\Delivery\Services\Manager::getHandlersList();
	...
	/** @var \Thurly\Sale\Delivery\Services\Base $handlerClass */
...
if (($arID = $lAdmin->GroupAction()) && $saleModulePermissions >= "W")
{
	if ($_REQUEST['action_target']=='selected')
	{
		...
		$dbResultList = \Thurly\Sale\Delivery\Services\Table::getList($params);
	...
	foreach ($arID as $ID)
	{
		...
		{
			case "delete":
				$res = \Thurly\Sale\Delivery\Services\Manager::delete($ID);
			...
			case "deactivate":
				...
				$res = \Thurly\Sale\Delivery\Services\Manager::update($ID, $arFields);
				...
				{
					\Thurly\Sale\Delivery\Services\Manager::setChildrenFieldsValues(
						$ID,
						$arFields
					);
				}
...
$db = \Thurly\Main\SiteTable::getList(
	array(
...
if(strlen($filter_site) > 0 || in_array('SITES', $arVisibleColumns))
{
	$glParams['runtime'] = array(
		'RESTRICTION_BY_SITE' => array(
			'data_type' => 'Thurly\Sale\Internals\ServiceRestrictionTable',
			'reference' => array(
				'ref.SERVICE_ID' => 'this.ID',
				'ref.SERVICE_TYPE' => array('?', \Thurly\Sale\Delivery\Restrictions\Manager::SERVICE_TYPE_SHIPMENT),
				'ref.CLASS_NAME' => array('?', '\Thurly\Sale\Delivery\Restrictions\BySite')
			),
			'join_type' => 'left'
		)
	);
...
$dbResultList = \Thurly\Sale\Delivery\Services\Table::getList($glParams);
...
while ($service = $dbResultList->NavNext(true, "f_"))
{
	...
	if($f_PARENT_ID > 0)
	{
		$res = \Thurly\Sale\Delivery\Services\Table::getById($f_PARENT_ID);
...
if ($saleModulePermissions == "W")
{
	...
	if(isset($filter["=CLASS_NAME"]) && $filter["=CLASS_NAME"] == '\Thurly\Sale\Delivery\Services\Group')
	{
		$aContext[] = array(
			"TEXT" => Loc::getMessage("SALE_SDL_ADD_NEW"),
			"TITLE" => Loc::getMessage("SALE_SDL_ADD_NEW_ALT"),
			"LINK" => "sale_delivery_service_edit.php?lang=".LANG."&CLASS_NAME=".urlencode('\Thurly\Sale\Delivery\Services\Group')."&back_url=".$backUrl,
			"ICON" => "btn_new"
		);

		$aContext[] = array(
			"TEXT" => Loc::getMessage("SALE_SDL_TO_LIST"),
			"LINK" => isset($_GET["back_url"]) ? $_GET["back_url"] : "/thurly/admin/sale_delivery_service_list.php?lang=".LANGUAGE_ID.
	...
	{
		$classNamesList = \Thurly\Sale\Delivery\Services\Manager::getHandlersList();
		...
		if(\Thurly\Sale\Delivery\Services\EmptyDeliveryService::getEmptyDeliveryServiceId() > 0)
			$classesToExclude[] = '\Thurly\Sale\Delivery\Services\EmptyDeliveryService';

		$menu = array();

		/** @var \Thurly\Sale\Delivery\Services\Base $class */
	...
	<tr>
		...
		<td>
			<select name="filter_class_name">
				<option value=""></option>
				<?foreach(\Thurly\Sale\Delivery\Services\Manager::getHandlersList() as $className):?>
