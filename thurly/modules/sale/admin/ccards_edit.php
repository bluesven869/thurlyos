...
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $saleModulePermissions=="W" && check_thurly_sessid())
{
	...
	if (strlen($errorMessage) <= 0)
	{
		...
		{
			if (strlen($apply)<=0)
				LocalRedirect("/thurly/admin/sale_ccards_admin.php?lang=".LANG.GetFilterParams("filter_", false));
		}
	}
	else
	{
...
$aMenu = array(
		array(
				...
				"LINK" => "/thurly/admin/sale_ccards_admin.php?lang=".LANG.GetFilterParams("filter_"),
				"ICON" => "btn_list"
			)
	);
...
if ($ID > 0 && $saleModulePermissions >= "U")
{
	...
	$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/sale_ccards_edit.php?lang=".LANG.GetFilterParams("filter_"),
			"ICON" => "btn_new"
		);
	...
	if ($saleModulePermissions >= "W")
	{
		$aMenu[] = array(
				...
				"LINK" => "javascript:if(confirm('".GetMessage("SCEN_DELETE_CCARD_CONFIRM")."')) window.location='/thurly/admin/sale_ccards_admin.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."#tb';",
				"WARNING" => "Y",
				"ICON" => "btn_delete"
			);
	}
...
?>
	...
	<tr class="adm-detail-required-field">
		...
		<td width="60%"><?
			$user_name = "";
			if ($ID>0)
				$user_name = "[<a title=\"".GetMessage("SCE_USER_PROFILE")."\" href=\"/thurly/admin/user_edit.php?lang=".LANGUAGE_ID."&ID=".$str_USER_ID."\">".$str_USER_ID."</a>] (".$str_USER_LOGIN.") ".$str_USER_NAME." ".$str_USER_LAST_NAME;
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/sale_account_admin.php?lang=".LANG.GetFilterParams("filter_")
			)
	);
?>
