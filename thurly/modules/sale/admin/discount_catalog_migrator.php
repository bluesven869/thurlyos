...
if (!$USER->IsAdmin() || !\Thurly\Main\Loader::includeModule('catalog') || !\Thurly\Main\Loader::includeModule('sale'))
{
	$APPLICATION->AuthForm(GetMessage('ACCESS_DENIED'));
}

final class DiscountCatalogMigratorLogger extends \Thurly\Main\Diag\FileExceptionHandlerLog
{
	const MAX_LOG_SIZE = 10000000;
	const DEFAULT_LOG_FILE = "thurly/modules/sale_migrator.log";
...
final class DiscountCatalogMigrator
{
	...
	public function __construct(array $options = array())
	{
		...
		$this->connection = \Thurly\Main\Application::getInstance()->getConnection();
		...
		\Thurly\Sale\Discount\Preset\Manager::getInstance()->registerAutoLoader();
	}
	...
	public function revert()
	{
		...
		$query = Catalog\DiscountTable::getList(array(
			...
			'runtime' => array(
				new \Thurly\Main\Entity\ExpressionField('FSALE_ID', 'CASE WHEN SALE_ID IS NOT NULL THEN 1 ELSE 0 END')
			),
...
//		$this->recalculatePriority();
	...
	private function migrateDiscounts(\Thurly\Main\DB\Result $discountIterator)
	{
		global $APPLICATION;
		...
		foreach ($discountIterator as $discount)
		{
			...
			if (\CSaleDiscount::checkFields('ADD', $rawFields))
			{
				...
				$addResult = \Thurly\Sale\Internals\DiscountTable::add($newData);
			...
			else
			{
				...
				$this->stepErrors[] = GetMessage(
					...
					array(
						'#URL#' => str_replace('#ID#', $discount['ID'], '/thurly/admin/cat_discount_edit.php?ID=#ID#&lang='.LANGUAGE_ID),
	...
	private function getActionsAndConditionsForCumulativeDiscount(array $catalogRow)
	{
		...
		$state = new \Thurly\Sale\Discount\Preset\State($catalogRow);
	...
	private function getRanges($cumulativeDiscountId)
	{
		...
		\Thurly\Main\Type\Collection::sortByColumn($matrix, 'sum');
	...
	protected function fillShortDescription()
	{
		...
		$discountIterator = \Thurly\Sale\Internals\DiscountTable::getList(array(
		...
		while($discount = $discountIterator->fetch())
		{
			...
			{
				\Thurly\Sale\Internals\DiscountTable::update($discount['ID'], array(
	...
	protected function getAllUserGroups()
	{
		...
		if($groups === null)
		{
			$groupsIterator = \Thurly\Main\GroupTable::getList(array(
				'select' => array('ID'),
			));
	...
	protected function moveUserGroupsByDiscount(array $catalogRow, $saleDiscountId, $typeDiscount = self::TYPE_GENERAL)
	{
		...
		\Thurly\Sale\Internals\DiscountGroupTable::updateByDiscount(
			$saleDiscountId,
			$groupIds,
			'Y',
			false
	...
	protected function rewriteUnpack(array $catalogRow)
	{
		...
		{
			$f = function($row){
				$arProduct = \Thurly\Sale\Compatible\DiscountCompatibility::reformatProductRowToOldCatalog($row);
				return ' . $unpackCode . ';
			};
	...
	protected function rewriteApplication(array $catalogRow)
	{
		return 'function (&$arOrder)
		{
			...
			\Thurly\Sale\Discount\Actions::applyToBasket($arOrder, array(
				\'VALUE\' => -' . $catalogRow['VALUE'] . ',
...
class TimeExecutionException extends \Thurly\Main\SystemException
{}
...
else
{
	...
	$isMysql = $connection instanceof \Thurly\Main\DB\MysqlCommonConnection;
		...
		array(
			...
			'CONTENT' => '
				<div style="width: 180px; float: left;">
					<img width="160" height="160" src="/thurly/images/sale/discount/wizard.png">
				</div>
				<div>' .
	...
	<style type="text/css">
		.bx-discount-catalog-help-btn {
			background:url("/thurly/panel/main/images/bx-admin-sprite.png") no-repeat 4px -88px;
			color:#e3ecee;
			cursor:pointer;
	...
	function DoNext(processedSummary, options)
	{
		...
		queryString += '&<?php echo thurly_sessid_get(); ?>';
