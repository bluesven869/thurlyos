...
foreach(\Thurly\Sale\Delivery\Services\Manager::getActiveList(true) as $deliveryId => $deliveryFields)
{
	$name = $deliveryFields["NAME"]." [".$deliveryId."]";
	$sites = \Thurly\Sale\Delivery\Restrictions\Manager::getSitesByServiceId($deliveryId);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && (isset($_POST["apply"]) || isset($_POST["save"])) && thurly_sessid())
{
	// validate property
	...
	if (! $errors && ($_POST['save'] || $_POST['apply']) && $saleModulePermissions == 'W' && check_thurly_sessid())
	{
		// save uploaded files
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/sale_order_props.php?lang=".LANG.GetFilterParams("filter_")
	)
);
...
if ($propertyId && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".Loc::getMessage('SOPEN_DELETE_PROPS_CONFIRM')."')) window.location='/thurly/admin/sale_order_props.php?action=delete&ID[]=".$propertyId."&lang=".LANG."&".thurly_sessid_get()."#tb';",
		"ICON" => "btn_delete",
	);
}
...
<form method="POST" action="<?=$APPLICATION->GetCurPage()?>?lang=<?=LANG;?>&PERSON_TYPE_ID=<?=$personTypeId;?><?=GetFilterParams("filter_", false);?>" name="form1" id="form1" enctype="multipart/form-data">
	...
	$tabControl->Buttons(array(
		...
		'back_url' => '/thurly/admin/sale_order_props.php?lang='.LANG.GetFilterParams('filter_'),
	));
	$tabControl->End();
	?>
</form>
