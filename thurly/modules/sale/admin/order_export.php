...
$strPath2Export1 = "/thurly/modules/sale/export/";
	...
	if (strlen($EXPORT_FORMAT) > 0)
	{
		if (array_key_exists($EXPORT_FORMAT, $arAvailableExports))
		{
			...
			if (StrLen($exportFilePath) > 0)
			{
				...
				{
					$filter_date_update_from_DAYS_TO_BACK = \Thurly\Main\Config\Option::get("sale", "order_list_date", 30);
					$filter_date_update_from = GetTime(time()-86400 * \Thurly\Main\Config\Option::get("sale", "order_list_date", 30));
				...
				$aliasFieldsList = \Thurly\Sale\Compatible\OrderCompatibility::getAliasFields();
				$alreadyUsedFields = \Thurly\Sale\Order::getAllFields();
...
if (strlen($errorMessage) > 0)
{
	...
	require($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/epilog_admin_before.php");
}
