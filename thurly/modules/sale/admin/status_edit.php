...
use	Thurly\Sale\Internals\StatusTable,
	Thurly\Sale\Internals\StatusLangTable,
	Thurly\Sale\Internals\StatusGroupTaskTable,
	Thurly\Main\Loader,
	Thurly\Main\TaskTable,
	Thurly\Main\GroupTable,
	Thurly\Main\Localization\Loc,
	Thurly\Main\Localization\LanguageTable;
...
// A D D / U P D A T E /////////////////////////////////////////////////////////////////////////////////////////////////
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !$readOnly && check_thurly_sessid() && ($_POST['save'] || $_POST['apply']))
{
	$errors = array();
	$statusType = $_REQUEST['TYPE'] == \Thurly\Sale\OrderStatus::TYPE ? \Thurly\Sale\OrderStatus::TYPE : \Thurly\Sale\DeliveryStatus::TYPE;
	$lockedStatusList = array(
		\Thurly\Sale\OrderStatus::TYPE => array(
			\Thurly\Sale\OrderStatus::getInitialStatus(),
			\Thurly\Sale\OrderStatus::getFinalStatus(),
		),
		\Thurly\Sale\DeliveryStatus::TYPE => array(
			\Thurly\Sale\DeliveryStatus::getInitialStatus(),
			\Thurly\Sale\DeliveryStatus::getFinalStatus(),
		),
	);
	...
	if ($statusId)
	{
										...
										))->fetch())
		{
			if ($statusData['TYPE'] != $statusType)
			{
				...
				if ($statusData['TYPE'] == \Thurly\Sale\OrderStatus::TYPE)
				{
					$checkStatus = \Thurly\Sale\Internals\OrderTable::getList($checkFilter)->fetch();
				...
				{
					$checkStatus = \Thurly\Sale\Internals\ShipmentTable::getList($checkFilter)->fetch();
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/sale_status.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	)
);
...
if ($statusId && !$readOnly)
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_new",
		"LINK" => "/thurly/admin/sale_status_edit.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	);
	$aMenu[] = array(
		"TEXT" => Loc::getMessage("SSEN_DELETE_STATUS"),
		"ICON" => "btn_delete",
		"LINK" => "javascript:if(confirm('".GetMessageJS("SSEN_DELETE_STATUS_CONFIRM")."')) window.location='/thurly/admin/sale_status.php?action=delete&ID[]=".urlencode($statusId)."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."#tb';",
	);
}
...
<form method="POST" action="<?echo $APPLICATION->GetCurPage()?>?" name="fform">
	...
	<tr>
		...
		<td>
			...
			<a id="status-template-link"
				<?if ($statusId):?>
					href="/thurly/admin/message_admin.php?lang=<?=LANGUAGE_ID?>&find_event_type=SALE_STATUS_CHANGED_<?=$statusId?>" target="_blank"
				<?else:?>
	...
	<tr>
		...
		<td>
			<a href="/thurly/admin/task_admin.php?lang=<?=LANGUAGE_ID?>&set_filter=Y&find_module_id=sale&find_binding=status" target="_blank">
	...
	$tabControl->Buttons(array(
		"disabled" => $readOnly,
		"back_url" => "/thurly/admin/sale_status.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	));
	$tabControl->End();
	?>
