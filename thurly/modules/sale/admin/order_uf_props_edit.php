...
$strError = "";
$bInitVars = false;
if ((strlen($save)>0 || strlen($apply)>0) && $_SERVER['REQUEST_METHOD'] == "POST" && $saleModulePermissions=="W" && strlen($propeditmore)<=0 && check_thurly_sessid())
{
	$PERSON_TYPE_ID = IntVal($PERSON_TYPE_ID);
	if ($PERSON_TYPE_ID<=0)
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/sale_order_props.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	)
);
...
if ($ID > 0 && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".GetMessage("SOPEN_DELETE_PROPS_CONFIRM")."')) window.location='/thurly/admin/sale_order_props.php?action=delete&ID[]=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."#tb';",
		"ICON" => "btn_delete",
	);
}
...
$tabControl->BeginNextTab();
	...
	<tr>
		...
		<td width="60%">
			<?
				foreach(\Thurly\Sale\Internals\Input\Manager::getTypes() as $k => $v)
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/sale_order_props.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
			)
	);
?>
