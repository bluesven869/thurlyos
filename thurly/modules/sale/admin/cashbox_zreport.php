...
CUtil::InitJSCore();
Page\Asset::getInstance()->addJs("/thurly/js/sale/admin/cashbox_zreport.js");
...
else
{
	$cashboxData = Internals\CashboxTable::getList(
		array(
			'limit' => 1,
			'filter' => array('USE_OFFLINE' => 'N', '%HANDLER' => '\\Thurly\\Sale\\Cashbox\\CashboxThurly')
		)
	);
}
...
if (!empty($cashboxList))
{
	...
	if (empty($blockData['CURRENCY']))
	{
		$blockData['CURRENCY'] = \Thurly\Currency\CurrencyManager::getBaseCurrency();
	}
...
if ($usePageNavigation)
{
	$countQuery = new \Thurly\Main\Entity\Query(Internals\CashboxZReportTable::getEntity());
