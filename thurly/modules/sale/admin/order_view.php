...
if(!empty($_REQUEST["ID"]) && intval($_REQUEST["ID"]) > 0)
	$saleOrder = Thurly\Sale\Order::load($_REQUEST["ID"]);
...
if($saleOrder)
{
	...
	$allowedStatusesDelete = \Thurly\Sale\OrderStatus::getStatusesUserCanDoOperations($USER->GetID(), array('delete'));
...
$aMenu[] = array(
	...
	"LINK" => "/thurly/admin/sale_order_view.php?unlock=Y&target=list&ID=".$ID."&lang=".LANGUAGE_ID.GetFilterParams("filter_")
);
...
if ($boolLocked && $saleModulePermissions >= 'W')
{
	$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/sale_order_view.php?ID=".$ID."&unlock=Y&lang=".LANGUAGE_ID.GetFilterParams("filter_"),
	);
}
...
if ($isAllowUpdate)
{
	$actionMenu[] = array(
		...
		"LINK" => '/thurly/admin/sale_order_create.php?lang='.LANGUAGE_ID."&SITE_ID=".$saleOrder->getSiteId()."&ID=".$ID."&".thurly_sessid_get().GetFilterParams("filter_")
	);
}
...
if($isAllowDelete)
{
	$actionMenu[] = array(
		...
		"LINK" => "javascript:if(confirm('".GetMessageJS("SALE_CONFIRM_ARCHIVE_MESSAGE")."')) window.location='sale_order.php?lang=".LANGUAGE_ID."&SITE_ID=".$saleOrder->getSiteId()."&ID=".$ID."&action=archive&".thurly_sessid_get().urlencode(GetFilterParams("filter_"))."'",
		"WARNING" => "Y"
	);
	...
	if(!$boolLocked)
	{
		$actionMenu[] = array(
			...
			"LINK" => "javascript:if(confirm('".GetMessageJS("SALE_OVIEW_DEL_MESSAGE")."')) window.location='sale_order.php?ID=".$ID."&action=delete&lang=".LANGUAGE_ID."&".thurly_sessid_get().urlencode(GetFilterParams("filter_"))."'",
			"WARNING" => "Y"
		);
	}
}
