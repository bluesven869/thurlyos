...
if (($ids = $lAdmin->GroupAction()) && $saleModulePermissions >= "W")
{
	...
	{
		$ids = array();
		$dbRes = \Thurly\Sale\Cashbox\Internals\CashboxTable::getList(
			array(
				'select' => array('ID'),
				'filter' => $filter,
	...
	foreach ($ids as $id)
	{
		...
		{
			case "delete":
				if ($id == \Thurly\Sale\Cashbox\Cashbox1C::getId())
				{
...
if ($usePageNavigation)
{
	$countQuery = new \Thurly\Main\Entity\Query(\Thurly\Sale\Cashbox\Internals\CashboxTable::getEntity());
...
$dbResultList = new CAdminResult(\Thurly\Sale\Cashbox\Internals\CashboxTable::getList($params), $tableId);
