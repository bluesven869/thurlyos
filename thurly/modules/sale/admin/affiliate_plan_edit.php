...
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $saleModulePermissions>="W" && check_thurly_sessid())
{
	...
	{
		if (strlen($apply) <= 0)
			LocalRedirect("/thurly/admin/sale_affiliate_plan.php?lang=".LANG.GetFilterParams("filter_", false));
	}
	else
	{
		$bVarsFromForm = true;
...
$aMenu = array(
		array(
				...
				"LINK" => "/thurly/admin/sale_affiliate_plan.php?lang=".LANG.GetFilterParams("filter_"),
				"ICON" => "btn_list"
			)
	);
...
if ($ID > 0)
{
	...
	$aMenu[] = array(
			...
			"LINK" => "/thurly/admin/sale_affiliate_plan_edit.php?lang=".LANG.GetFilterParams("filter_"),
			"ICON" => "btn_new"
		);
	...
	if ($saleModulePermissions >= "W")
	{
		$aMenu[] = array(
				...
				"LINK" => "javascript:if(confirm('".GetMessage("SAPE1_DELETE_CONF")."')) window.location='/thurly/admin/sale_affiliate_plan.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."#tb';",
				"WARNING" => "Y",
				"ICON" => "btn_delete"
			);
	}
