...
if ($REQUEST_METHOD=="POST" && strlen($Update)>0 && $saleModulePermissions>="W" && check_thurly_sessid())
{
	...
	else
	{
		...
		{
			LocalRedirect("/thurly/admin/sale_order_props_group_edit.php?ID=$ID&lang=".LANG.GetFilterParams("filter_", false));
		}
	}
	...
	{
		if (strlen($apply) <= 0)
			LocalRedirect("/thurly/admin/sale_order_props_group.php?lang=".LANG.GetFilterParams("filter_", false));
	}
	else
	{
		$bVarsFromForm = true;
...
$aMenu = array(
	array(
		...
		"ICON" => "btn_list",
		"LINK" => "/thurly/admin/sale_order_props_group.php?lang=".LANG.GetFilterParams("filter_")
	)
);
...
if ($ID > 0 && $saleModulePermissions >= "W")
{
	...
	$aMenu[] = array(
		...
		"ICON" => "btn_new",
		"LINK" => "/thurly/admin/sale_order_props_group_edit.php?lang=".LANG.GetFilterParams("filter_")
	);

	$aMenu[] = array(
		"TEXT" => GetMessage("SOPGEN_DELETE_PROPS_GRP"), 
		"LINK" => "javascript:if(confirm('".GetMessage("SOPGEN_DELETE_PROPS_GRP_CONFIRM")."')) window.location='/thurly/admin/sale_order_props_group.php?ID=".$ID."&action=delete&lang=".LANG."&".thurly_sessid_get()."#tb';",
		"ICON" => "btn_delete",
	);
}
...
$tabControl->Buttons(
		array(
				...
				"back_url" => "/thurly/admin/sale_order_props_group.php?lang=".LANG.GetFilterParams("filter_")
			)
	);
?>
