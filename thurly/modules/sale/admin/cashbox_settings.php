<?
namespace Thurly\Sale\Cashbox\AdminPage\Settings
{
	...
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/lib/cashbox/inputs/file.php");
			...
			if ($settings)
			{
				foreach ($settings as $group => $block)
				{
					...
					foreach ($block['ITEMS'] as $code => $item)
					{
						...
						if ($handler === '\Thurly\Sale\Cashbox\CashboxThurly' && $group === 'PAYMENT_TYPE')
						{
