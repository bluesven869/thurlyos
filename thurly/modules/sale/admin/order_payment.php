...
if ($usePageNavigation)
{
	$countQuery = new \Thurly\Main\Entity\Query(PaymentTable::getEntity());
...
while ($payment = $dbResultList->Fetch())
{
	...
	$row->AddField("ORDER_USER_NAME", "<a href='/thurly/admin/user_edit.php?ID=".$payment['ORDER_USER_ID']."&lang=".$lang."'>".$userName."</a>");
...
<?
	...
	$res = \Thurly\Sale\PaySystem\Manager::getList($params);
...
<tr>
	...
	<td>
		<select multiple name="filter_pay_system_id[]">
			<option value="NOT_REF">(<?=GetMessage("PAYMENT_ORDER_PAID_ALL");?>)</option>
			<?
			$ptRes = \Thurly\Sale\Internals\PersonTypeTable::getList(array(
				'order' => array("SORT"=>"ASC", "NAME"=>"ASC")
			));
			...
			foreach ($paySystems as $paySystem):
				$dbRestRes = \Thurly\Sale\Services\PaySystem\Restrictions\Manager::getList(array(
					'select' => array('PARAMS'),
					'filter' => array(
...
<tr>
	<td><?echo \Thurly\Main\Localization\Loc::getMessage("SALE_PAYMENT_F_USER_ID");?>:</td>
	<td>
		<?echo FindUserID("filter_user_id", $filter_user_id, "", "find_form");?>
	</td>
</tr>
<tr>
	<td><?echo \Thurly\Main\Localization\Loc::getMessage("SALE_PAYMENT_F_USER_LOGIN");?>:</td>
	<td>
		<input type="text" name="filter_user_login" value="<?echo htmlspecialcharsbx($filter_user_login)?>" size="40">
	</td>
</tr>
<tr>
	<td><?echo \Thurly\Main\Localization\Loc::getMessage("SALE_PAYMENT_F_USER_EMAIL");?>:</td>
	<td>
