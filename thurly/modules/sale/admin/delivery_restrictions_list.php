<?
namespace Thurly\Sale\Delivery\AdminPage\DeliveryRestrictions
{
	...
	$res = \Thurly\Sale\Internals\ServiceRestrictionTable::getList(array(
		'filter' => array(
			'=SERVICE_ID' => $ID,
	...
	while ($record = $dbRecords->Fetch())
	{
		...
		if(!is_subclass_of($record['CLASS_NAME'], 'Thurly\Sale\Delivery\Restrictions\Base'))
			continue;
