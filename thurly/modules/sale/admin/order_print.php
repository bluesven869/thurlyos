...
$allowedStatusesView = \Thurly\Sale\OrderStatus::getStatusesGroupCanDoOperations($USER->GetUserGroupArray(), array('view'));
if(in_array($str_STATUS_ID, $allowedStatusesView))
{
	$bUserCanViewOrder = true;
}

$allowedStatusesUpdate = \Thurly\Sale\OrderStatus::getStatusesGroupCanDoOperations($USER->GetUserGroupArray(), array('update'));
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && strlen($Print)>0 && check_thurly_sessid() && $bUserCanViewOrder)
{
	if(count($REPORT_ID) > 0)
	{
		...
		{
			?>
			window.open('/thurly/admin/sale_print.php?PROPS_ENABLE=<?=$PROPS_ENABLE?>&doc=<?echo CUtil::JSEscape($REPORT_ID[$i]) ?>&ORDER_ID=<?echo $ID ?>&<?=$urlParams?>', '_blank');
			<?
		}
		?>
		</script>
...
else
{
	...
	?>
		...
		<tr>
			<td align="right" valign="top"><?echo GetMessage("SALE_PR_SHABLON")?>:<br><img src="/thurly/images/sale/mouse.gif" width="44" height="21" border="0" alt=""></td>
			<td>
				<select size="5" multiple name="REPORT_ID[]">
					...
					$arReports = array();
					if (file_exists($_SERVER["DOCUMENT_ROOT"]."/thurly/admin/reports/"))
					{
						if ($handle = opendir($_SERVER["DOCUMENT_ROOT"]."/thurly/admin/reports/"))
						{
							while (($file = readdir($handle)) !== false)
							{
								...
								if (is_file($_SERVER["DOCUMENT_ROOT"]."/thurly/admin/reports/".$file) && ToUpper(substr($file, -4))==".PHP")
								{
									$rep_title = $file;
									$file_contents = file_get_contents($_SERVER["DOCUMENT_ROOT"]."/thurly/admin/reports/".$file);
					...
					if ($handle = opendir($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/reports/"))
					{
						while (($file = readdir($handle)) !== false)
						{
							...
							if (is_file($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/reports/".$file)
							...
							)
							{
								...
								{
									$file_contents = file_get_contents($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/reports/".$file);
								}
