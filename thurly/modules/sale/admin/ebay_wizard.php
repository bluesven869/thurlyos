...
namespace
{
	...
	use \Thurly\Sale\TradingPlatform\Ebay\Wizard;
	...
	if(!$ebay->isActive())
		LocalRedirect("/thurly/admin/sale_ebay_general.php?lang=".LANG."&back_url=".urlencode($APPLICATION->GetCurPageParam()));
	...
	if($step >= 1) // Till now we will save smth.
	{
		...
		)
		{
			...
			$className = '\Thurly\Sale\TradingPlatform\Ebay\Wizard\\'.$stepClassesList[$step-1];
	...
	$wizardStepClassName = '\Thurly\Sale\TradingPlatform\Ebay\Wizard\\'.$stepClassesList[$step];
	$wizardStep = new $wizardStepClassName($siteId, $settings, $cleanCache);
	\Thurly\Main\Page\Asset::getInstance()->addJs("/thurly/js/sale/ebay_admin.js", true);
			...
			<tr><td style="vertical-align: top;">
				...
				<div id="result">
					<table width="100%">
						<tr>
							<td width="120px">
								<img alt="eBay logo" src="/thurly/images/sale/ebay/logo.png" style="width: 100px; height: 67px;">
				...
				<div class="adm-sale-ebay-wizard-state">
					...
					<div class="adm-sale-ebay-wizard-state-body">
						...
						<?foreach($stepClassesList as $classStep => $class):?>
							<?$fullClassName = '\Thurly\Sale\TradingPlatform\Ebay\Wizard\\'.$class;?>
...
namespace Thurly\Sale\TradingPlatform\Ebay\Wizard
{

	use Thurly\Main\Config\Option;
	use Thurly\Sale\TradingPlatform\Ebay\Api\Caller;
	use Thurly\Sale\TradingPlatform\Ebay\Ebay;
	use Thurly\Sale\TradingPlatform\Helper;
	...
	abstract class Step
	{
		...
		{
			$ebay = \Thurly\Sale\TradingPlatform\Ebay\Ebay::getInstance();
			$settings = $ebay->getSettings();
		...
		{
			return '<img src="/thurly/images/sale/'.($isGreen ? 'green.gif' : 'red.gif').'" hspace="4">';
		}
		...
		protected static function getUserInfo($siteId, array $ebaySettings)
		{
			...
			else
			{
				...
				$caller = new Caller(array(
					"URL" => \Thurly\Sale\TradingPlatform\Ebay\Ebay::getApiUrl()
				));
		...
		protected static function getPolicyInfo($siteId, $ebaySettings)
		{
			...
			else
			{
				...
				$policy = new \Thurly\Sale\TradingPlatform\Ebay\Policy($ebaySettings[$siteId]["API"]["AUTH_TOKEN"], $siteId);
	...
	class StepSite extends Step
	{
		...
		public function getHtml()
		{
			...
			{
				$dbRes = \Thurly\Main\SiteTable::getById($this->siteId);
	...
	class StepPersonType extends Step
	{
		...
		protected function createStatusMapHtml()
		{
			...
			foreach(\Thurly\Sale\TradingPlatform\Ebay\Helper::getEbayOrderStatuses() as $ebayStatus)
			{
	...
	class StepEbayDefaultPolicies extends Step
	{
		...
		public function getHtml()
		{
			$policy = new \Thurly\Sale\TradingPlatform\Ebay\Policy($this->ebaySettings[$this->siteId]["API"]["AUTH_TOKEN"], $this->siteId);
			$hiddenPolicyFields = "";
			...
			foreach($policy->getPoliciesNames(\Thurly\Sale\TradingPlatform\Ebay\Policy::TYPE_RETURN) as $policyId => $policyName)
			{
			...
			foreach($policy->getPoliciesNames(\Thurly\Sale\TradingPlatform\Ebay\Policy::TYPE_SHIPPING) as $policyId => $policyName)
			{
			...
			foreach($policy->getPoliciesNames(\Thurly\Sale\TradingPlatform\Ebay\Policy::TYPE_PAYMENT) as $policyId => $policyName)
			{
	...
	class StepPaymentMapping extends Step
	{
		...
		public function getHtml()
		{
			...
			$result = '';
			$details = new \Thurly\Sale\TradingPlatform\Ebay\Api\Details($this->siteId);
	...
	class StepImportEbayCategories extends Step
	{
		...
		protected static function getCategoriesCount()
		{
			...
			$res = \Thurly\Sale\TradingPlatform\Ebay\CategoryTable::getList(array(
				"select" => array("CNT"),
				"runtime" => array(
					new \Thurly\Main\Entity\ExpressionField('CNT', 'COUNT(1)')
				)
			));
		...
		public function getHtml()
		{
			...
			for($i = 0; $i < count($this->ebaySettings[$this->siteId]["IBLOCK_ID"]); $i++)
			{
				$result .= '<div  style="padding-top: 10px;">'.
					...
					'&nbsp;'.
					Helper::getThurlyCategoryPropsHtml(
	...
	class StepCategoriesMap extends Step
	{
		...
		protected static function getMappedCount($siteId, $ebaySettings)
		{
			...
			{
				$entityId = \Thurly\Sale\TradingPlatform\Ebay\MapHelper::getCategoryEntityId($ib);
				$mapRes = \Thurly\Sale\TradingPlatform\MapTable::getList(array(
					'filter' => array(
						'=ENTITY_ID' => $entityId
					),
					"select" => array("CNT"),
	...
	class StepStartExchange extends Step
	{
		...
		public function getHtml()
		{
			...
			if(!$exchangeStarted)
			{
				...
				$defaultFeedIntervals = \Thurly\Sale\TradingPlatform\Helper::getDefaultFeedIntervals();
		...
		public function save()
		{
			...
			{
				$this->ebaySettings[$this->siteId]["FEEDS"] = \Thurly\Sale\TradingPlatform\Ebay\Agent::update(
					$this->siteId,
