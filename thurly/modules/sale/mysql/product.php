...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/general/product.php");
...
class CSaleViewedProduct extends CAllSaleViewedProduct
{
	...
	*/
	public function Add($arFields)
	{
		...
		if (\Thurly\Main\Loader::includeModule('statistic') && isset($_SESSION['SESS_SEARCHER_ID']) && (int)$_SESSION['SESS_SEARCHER_ID'] > 0)
			return false;

		if ((string)\Thurly\Main\Config\Option::get('sale', 'viewed_capability') == 'Y')
		{
			if (\Thurly\Main\Loader::includeModule('catalog'))
			{
				return \Thurly\Catalog\CatalogViewedProductTable::refresh($arFields["PRODUCT_ID"], CSaleBasket::GetBasketUserID(), $arFields["LID"]);
			}
		}
