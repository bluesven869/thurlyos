<?
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/general/basket.php");
...
class CSaleBasket extends CAllSaleBasket
{
	...
	*/
	...
	function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		{
			$result = \Thurly\Sale\Compatible\BasketCompatibility::getList($arOrder, $arFilter, $arGroupBy, $arNavStartParams, $arSelectFields);
			if ($result instanceof \Thurly\Sale\Compatible\CDBResult)
				$result->addFetchAdapter(new \Thurly\Sale\Compatible\BasketFetchAdapter());
			return $result;
		}
...
//			return false;
		...
		if ($isOrderConverted != 'N')
		{
			...
			$result = \Thurly\Sale\Compatible\BasketCompatibility::add($arFields);
			if (!$result->isSuccess())
			{
	...
	function Delete($ID)
	{
		...
		if ($isOrderConverted != 'N')
		{
			...
			$r = \Thurly\Sale\Compatible\BasketCompatibility::delete($ID);
			if (!$r->isSuccess(true))
			{
	...
	function DeleteAll($FUSER_ID = 0, $bIncOrdered = false)
	{
		...
		while ($arBasket = $dbBasket->Fetch())
		{
			...
			{
				/** @var \Thurly\Sale\Result $r */
				$r = \Thurly\Sale\Compatible\BasketCompatibility::delete($arBasket["ID"]);
				if (!$r->isSuccess(true))
				{
					foreach($r->getErrorMessages() as $error)
					{
