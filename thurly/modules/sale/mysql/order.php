<?
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/general/order.php");
...
class CSaleOrder extends CAllSaleOrder
{
	function Add($arFields)
	{
		...
		if ($isOrderConverted != 'N')
		{
			if (!empty($arFields1))
			{
				$arFields1 = \Thurly\Sale\Compatible\OrderCompatibility::backRawField(\Thurly\Sale\Compatible\OrderCompatibility::ENTITY_ORDER, $arFields1);
			}

			$result = \Thurly\Sale\Compatible\OrderCompatibility::add(array_merge($arFields, $arFields1));
	...
	function Update($ID, $arFields, $bDateUpdate = true)
	{
		...
		if ($isOrderConverted != 'N')
		{
			...
			$result = \Thurly\Sale\Compatible\OrderCompatibility::update($ID, array_merge($arFields, $arFields1), $bDateUpdate);
			if (!$result->isSuccess())
			{
				foreach($result->getErrorMessages() as $error)
				{
	...
	public static function GetList($arOrder = array("ID"=>"DESC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array(), $arOptions = array())
	{
		...
		if ($isOrderConverted != 'N')
		{
			$result = \Thurly\Sale\Compatible\OrderCompatibility::getList($arOrder, $arFilter, $arGroupBy, $arNavStartParams, $arSelectFields, $callback);
			if ($result instanceof \Thurly\Sale\Compatible\CDBResult)
				$result->addFetchAdapter(new \Thurly\Sale\Compatible\OrderFetchAdapter());
			return $result;
		}
