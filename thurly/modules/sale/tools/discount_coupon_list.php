...
while ($coupon = $couponIterator->Fetch())
{
	...
	$urlEdit = '/thurly/admin/sale_discount_coupon_edit.php?ID='.$coupon['ID'].'&DISCOUNT_ID='.$discountID.'&lang='.LANGUAGE_ID.'&bxpublic=Y';
...
if (!empty($rowList) && ($selectFieldsMap['CREATED_BY'] || $selectFieldsMap['MODIFIED_BY'] || $selectFieldsMap['USER_ID']))
{
	if (!empty($userIDs))
	{
		...
		while ($oneUser = $userIterator->fetch())
		{
			...
			if ($canViewUserList)
				$userList[$oneUser['ID']] = '<a href="/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&ID='.$oneUser['ID'].'">'.CUser::FormatName($nameFormat, $oneUser).'</a>';
			else
...
if (!isset($_REQUEST["mode"]) || ($_REQUEST["mode"] != 'excel' && $_REQUEST["mode"] != 'subsettings'))
{
	...
	function ShowNewCoupons(id, multi)
	{
		...
		(new BX.CAdminDialog({
			'content_url': '/thurly/admin/sale_discount_coupon_edit.php',
