...
class CAllSaleOrder
{
		...
		$deliveryId, $paySystemId, $arOptions, &$arErrors, &$arWarnings)
	{
		...
		$arOrder["PRICE"] = \Thurly\Sale\PriceMaths::roundPrecision($arOrder["PRICE"]);
		$arOrder["TAX_VALUE"] = \Thurly\Sale\PriceMaths::roundPrecision($arOrder["TAX_VALUE"]);
	...
	public static function CalculateOrderPrices($arBasketItems)
	{
		...
		foreach ($arResult['BASKET_ITEMS'] as &$arItem)
		{
			if (!CSaleBasketHelper::isSetItem($arItem))
			{
				...
				if ($arItem["VAT_RATE"] > 0)
				{
					...
					$arItem["VAT_VALUE"] = \Thurly\Sale\PriceMaths::roundPrecision($v / $arItem["QUANTITY"]);
					$arResult["VAT_SUM"] += $v;
		...
		$arResult['ORDER_PRICE'] = \Thurly\Sale\PriceMaths::roundPrecision($arResult['ORDER_PRICE']);
		$arResult['VAT_SUM'] = \Thurly\Sale\PriceMaths::roundPrecision($arResult['VAT_SUM']);
		unset($arItem);
	...
	*/
	...
	function CheckFields($ACTION, &$arFields, $ID = 0)
	{
		...
		{
			if (!($delivery = \Thurly\Sale\Delivery\Services\Table::getById($arFields["DELIVERY_ID"])))
			{
	...
	function Delete($ID)
	{
		...
		if ($arOrder)
		{
			...
			if ($isOrderConverted != 'N')
			{
				...
				$r = \Thurly\Sale\Compatible\OrderCompatibility::delete($ID);
	...
	function GetByID($ID)
	{
		...
		else
		{
			...
			if ($isOrderConverted != 'N')
			{
				$db_res = \Thurly\Sale\Compatible\OrderCompatibility::getById($ID);
			}
			else
			{
				$strSql =
			...
			if ($res = $db_res->Fetch())
			{
				if ($isOrderConverted != 'N')
				{
					...
					foreach ($dataKeys as $key)
					{
						...
						if (!empty($res[$key])
							&& (($res[$key] instanceof \Thurly\Main\Type\DateTime)
								|| ($res[$key] instanceof \Thurly\Main\Type\Date)))
						{
							/** @var \Thurly\Main\Type\Date|\Thurly\Main\Type\DateTime $dateObject */
							$dateObject = $res[$key];
					...
					$res = \Thurly\Sale\Compatible\OrderFetchAdapter::convertRowData($res);
				}
	...
	function PayOrder($ID, $val, $bWithdraw = True, $bPay = True, $recurringID = 0, $arAdditionalFields = array())
	{
		...
		if ($isOrderConverted != 'N')
		{
			$errorMessage = "";
			/** @var \Thurly\Sale\Result $r */
			$r = \Thurly\Sale\Compatible\OrderCompatibility::pay($ID, $arFields, $bWithdraw, $bPay);
	...
	function DeliverOrder($ID, $val, $recurringID = 0, $arAdditionalFields = array())
	{
		...
		else
		{
			...
			/** @var \Thurly\Sale\Result $r */
			$r = \Thurly\Sale\Compatible\OrderCompatibility::allowDelivery($ID, ($val == "Y" ? true : false));
			if (!$r->isSuccess(true))
			{
				foreach($r->getErrorMessages() as $error)
				{
	...
	function ReserveOrder($ID, $val)
	{
		...
		{
			/** @var \Thurly\Sale\Result $r */
			$r = \Thurly\Sale\Compatible\OrderCompatibility::reserve($ID, $val);
	...
	function CancelOrder($ID, $val, $description = "")
	{
		...
		if ($isOrderConverted != 'N')
		{
			$r = \Thurly\Sale\Compatible\OrderCompatibility::cancel($ID, $val, $description);
			if ($r->isSuccess(true))
			{
				$res = true;
			}
	...
	function Lock($ID)
	{
		...
		$arFields = array(
			"DATE_LOCK" => new \Thurly\Main\Type\DateTime(),
...
//		return CSaleOrder::Update($ID, $arFields, false);
	...
	*/
	...
	public static function setAccountNumberById($id)
	{
		...
		if ($isCustomAlgorithm)
		{
			if ($isOrderConverted != 'N')
			{
				...
				{
					/** @var \Thurly\Sale\Result $r */
					$r = \Thurly\Sale\Internals\OrderTable::update($id, array("ACCOUNT_NUMBER" => $value));
	...
	protected static function generateCustomAccountNumber($id)
	{
		...
		$type = \Thurly\Main\Config\Option::get("sale", "account_number_template", "");
	...
	protected static function generateAccountNumber($id)
	{
		...
		$type = \Thurly\Main\Config\Option::get("sale", "account_number_template", "");
		$param = \Thurly\Main\Config\Option::get("sale", "account_number_data", "");
		$isOrderConverted = \Thurly\Main\Config\Option::get("main", "~sale_converted_15", 'Y');
	...
	protected static function setIdAsAccountNumber($id)
	{
		...
		if ($isOrderConverted != 'N')
		{
			...
			for ($i = 1; $i <= 10; $i++)
			{
				...
				{
					/** @var \Thurly\Sale\Result $r */
					$r = \Thurly\Sale\Internals\OrderTable::update($id, array("ACCOUNT_NUMBER" => $accountNumber));
					$res = $r->isSuccess(true);
				}
				catch (\Thurly\Main\DB\SqlQueryException $exception)
				{
	...
	*/
	public static function ClearProductReservedQuantity()
	{
		...
		if ($days_ago > 0)
		{
			...
			$res = \Thurly\Sale\Internals\OrderTable::getList($filter);
