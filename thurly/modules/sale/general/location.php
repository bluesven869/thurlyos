...
class CAllSaleLocation
{
	...
	const SELF_ENTITY_NAME = 			'Thurly\Sale\Location\Location';
	const NAME_ENTITY_NAME = 			'Thurly\Sale\Location\Name\Location';
	const DEFAULT_SITE_ENTITY_NAME = 	'Thurly\Sale\Location\DefaultSite';
	...
	public static function proxySaleAjaxLocationsComponent($parameters = array(), $additionalParams = array(), $template = '', $hideIcons = true, $wrapNewComponentWith = false)
	{
		...
		if(self::isLocationProEnabled())
		{
			...
			if(!strlen($template))
				$appearance = \Thurly\Sale\Location\Admin\Helper::getWidgetAppearance();
			else
			{
				$appearance = 'steps';
			...
			$GLOBALS["APPLICATION"]->IncludeComponent(
				"thurly:sale.location.selector.".$appearance,
				"",
				$parametersProxed,
				null,
	...
	public static function tryTranslateIDToCode($id)
	{
		...
		if($id != '' && $id === (string) intval($id))
		{
			...
			$location = \Thurly\Sale\Location\LocationTable::getList(array('filter' => array('=ID' => $id), 'select' => array('ID', 'CODE')))->fetch();
	...
	public static function GetLocationString($locationId, $siteId = SITE_ID, $langId = LANGUAGE_ID)
	{
		...
		if(!\Thurly\Sale\SalesZone::checkLocationId($locationId, $siteId))
			$locationId = 0;
		...
		{
			if(!\Thurly\Sale\SalesZone::checkCountryId($arCountry["ID"], $siteId))
				continue;
		...
		if ($countryId > 0 || count($arCountryList) <= 0)
		{
			...
			{
				if(!\Thurly\Sale\SalesZone::checkRegionId($arRegion["ID"], $siteId))
					continue;
		...
		)
		{
			...
			{
				if(!\Thurly\Sale\SalesZone::checkCityId($arCity["CITY_ID"], $siteId))
					continue;
		...
		if(self::isLocationProMigrated())
		{
			...
			try
			{
				if(!$regionId)
				{
					...
					$res = \Thurly\Sale\Location\TypeTable::getList(array('filter' => array('=CODE' => 'REGION'), 'select' => array('ID')))->fetch();
				...
				$res = \Thurly\Sale\Location\LocationTable::getList($params);
	...
	public static function getFieldMapForGetList($arFilter)
	{
		...
		if(self::isLocationProMigrated())
		{
			$tableDefault = \Thurly\Sale\Location\DefaultSiteTable::getTableName();
...
WHERE ".$strWhere;
	...
	protected static function AddLocationUnattached($typeCode, $names = array())
	{
		...
		$id = false;
		$res = \Thurly\Sale\Location\LocationTable::add($fields);
		if($res->isSuccess())
		{
			$id = $res->getId();
			$uRes = \Thurly\Sale\Location\LocationTable::update($id, array('CODE' => $id));
	...
	protected static function RebindLocationTriplet($fields = array())
	{
		...
		{
			$uRes = \Thurly\Sale\Location\LocationTable::update($region, array('PARENT_ID' => $country));
		}
		if($city)
		{
			if($region) // set region as PARENT_ID for city
			{
				$uRes = \Thurly\Sale\Location\LocationTable::update($city, array('PARENT_ID' => $region));
			}
			elseif($country) // set country as PARENT_ID for city
			{
				$uRes = \Thurly\Sale\Location\LocationTable::update($city, array('PARENT_ID' => $country));
			}
		}
		...
		if(intval($fields['SORT']))
		{
			...
			if($loc2Update)
			{
				$uRes = \Thurly\Sale\Location\LocationTable::update($loc2Update, array('SORT' => $fields['SORT']));
			}
		}
