...
use \Thurly\Main\Type\RandomSequence;
use \Thurly\Sale;
...
class CSaleOrderLoader
{
	...
	public function getSiteId()
	{
		...
		if($siteId === null)
		{
			$siteId = \Thurly\Main\Config\Option::get("sale", "1C_SITE_NEW_ORDERS");

			if($siteId == "")
				$siteId = \Thurly\Main\Application::getInstance()->getContext()->getSite();
		}
		return $siteId;
	}

	function deleteDocumentShipment(\Thurly\Sale\Shipment $shipment)
	{
	...
	function deleteDocumentPayment(\Thurly\Sale\Payment $payment)
	{
		if($payment->isPaid())
			$payment->setPaid('N');
	...
	public function afterShippedCompatible1C(\Thurly\Sale\Order $order)
	{
	...
	public function setMarkedOrder(\Thurly\Sale\Order $order)
	{
	...
	public function unMarkedOrder(\Thurly\Sale\Order $order)
	{
		$manyReason = array();
	...
	public function isMarkedOrder(\Thurly\Sale\Order $order)
	{
		return 	(
	...
	public function createPaymentCompatible1C(\Thurly\Sale\Order $order, $arDocument)
	{
		if( !$order->isCanceled())
		{
			if(
			...
			)
			{
				...
				$paySystem = \Thurly\Sale\PaySystem\Manager::getObjectById($paymentServiceId);
				$payment = $paymentCollection->createItem($paySystem);

				$date = new Thurly\Main\Type\Date(CDatabase::FormatDate(str_replace("T", " ", $arDocument["TRAITS"][GetMessage("CC_BSC1_1C_PAYED_DATE")]), "YYYY-MM-DD HH:MI:SS", CLang::GetDateFormat("FULL", LANG)));
	...
	public function beforePaidCompatible1C(\Thurly\Sale\Order $order)
	{
		if( !$order->isCanceled())
		{
	...
	public function afterPaidCompatible1C(\Thurly\Sale\Order $order)
	{
		if( !$order->isCanceled())
		{
			$paymentCompatible1CId = $this->getPaymentCompatible1CByOrder($order);
	...
	public function getPaymentCompatible1CByOrder(\Thurly\Sale\Order $order)
	{
	...
	public function createEntityCompatible1C(\Thurly\Sale\Order $order, $arDocument)
	{
	...
	public function updateEntityCompatible1C(\Thurly\Sale\Order $order, $arDocument)
	{
	...
	public static function onBeforeUpdateOrderWithoutShipmentsPayments(Thurly\Main\Event $event)
	{
		$parameters = $event->getParameters();
		/** @var Thurly\Sale\Order $order */
		$order = $parameters['ENTITY'];
		if (!$order instanceof Thurly\Sale\Order)
		{
			return new Thurly\Main\EventResult(
					Thurly\Main\EventResult::ERROR,
					new Thurly\Sale\ResultError(Thurly\Main\Localization\Loc::getMessage('SALE_EVENT_COMPATIBILITY_ORDER_SAVE_WRONG_ORDER'), 'SALE_EVENT_COMPATIBILITY_ORDER_SAVE_WRONG_ORDER'),
					'sale'
			);
		}
		...
		return new Thurly\Main\EventResult( Thurly\Main\EventResult::SUCCESS, null, 'sale');
	}

	public static function onBeforeSaveOrderWithoutShipmentsPayments(Thurly\Main\Event $event)
	{
		$parameters = $event->getParameters();
		...
		$serviceId = \Thurly\Main\Config\Option::get('sale', '1C_IMPORT_DEFAULT_SHIPMENT_SERVICE', 0);
		if(intval($serviceId)<=0)
		{
			$serviceId = Thurly\Sale\Delivery\Services\EmptyDeliveryService::getEmptyDeliveryServiceId();
			if($serviceId<=0)
			{
	...
	function getDocumentId($type,$code,$xmlId1CDocument)
	{
		...
		if((strlen($code) <= 0 ||  $result['ID'] === false) && strlen($xmlId1CDocument) > 0)//try to search document from 1C // !==false fix for accountNumberPrefix
		{
			...
			{
				case 'Shipment':
					if($shimpentCode1C = \Thurly\Sale\Internals\ShipmentTable::getList(array(
					...
					break;
				case 'Payment':
					if($paymentCode1C = \Thurly\Sale\Internals\PaymentTable::getList(array(
							'select' => array('ID', 'ID_1C','ORDER_ID'),
							'filter' => array('ID_1C' => $xmlId1CDocument),
							'order' => array('ID' => 'DESC')
					))->fetch())
					...
					break;
				case 'Order':
					if($OrderCode1C = \Thurly\Sale\Internals\OrderTable::getList(array(
	...
	{
		$accountNumberPrefix = \Thurly\Main\Config\Option::get("sale", "1C_SALE_ACCOUNT_NUMBER_SHOP_PREFIX", "");
		if(strlen($orderCode)>0)
		{
			if(is_numeric($orderCode))
			{
				if (!$orderByID = \Thurly\Sale\Internals\OrderTable::getById($orderCode)->fetch())
				{
					if (!$orderBy1CID = \Thurly\Sale\Internals\OrderTable::getList(array(
							'select' => array('ID'),
					...
					{
						if (!$orderByAccountNumber = \Thurly\Sale\Internals\OrderTable::getList(array(
								'select' => array('ID'),
	...
	function addBasketForShipment($arDocument, $newBasketItems, \Thurly\Sale\Shipment $shipment)
	{
		...
		if($orderInfo = $dbOrder->Fetch())
		{
			if($arOrder["VERSION_1C"] != $orderInfo["VERSION_1C"] || (strlen($orderInfo["VERSION_1C"]) <= 0 || strlen($arOrder["VERSION_1C"]) <= 0)) // skip update if the same version
			{
						...
						&& $orderInfo["STATUS_ID"] != "F")
				{
					...
					{
						/** @var Thurly\Sale\BasketItem $basketItem */
					...
					$isUsed = \Thurly\Sale\Compatible\DiscountCompatibility::isUsed();
					\Thurly\Sale\Compatible\DiscountCompatibility::stopUsageCompatible();
					...
					if($isUsed === true)
					{
						\Thurly\Sale\Compatible\DiscountCompatibility::revertUsageCompatible();
					}
				}
				else
				{
			...
			if(\Thurly\Main\Config\Option::get("sale", "1C_CHANGE_STATUS_FROM_1C", "") && strlen($arOrder["TRAITS"][GetMessage("CC_BSC1_1C_STATUS_ID")])>0)
			{
	...
	function Paid(\Thurly\Sale\Payment $payment, $arDocument)
	{
	...
	function Ship(\Thurly\Sale\Shipment $shipment, $arDocument)
	{
		/** @var Thurly\Sale\Order $order */
		...
		if(strlen($arDocument["DATE"])>1)
		{
			...
			$date = new Thurly\Main\Type\Date(CDatabase::FormatDate(str_replace("T", " ", $arDocument["DATE"]), "YYYY-MM-DD HH:MI:SS", CLang::GetDateFormat("FULL", LANG)));
	...
	function addProduct($arDocument, $documentBasketItems, $documentShipmentItems, \Thurly\Sale\Shipment $shipment)
	{
		...
		foreach($documentBasketItems as $item)
		{
			foreach($item as $itemID=>$arItem)
			{
				...
				/** @var Thurly\Sale\BasketPropertiesCollection $propertyCollection */
	...
	function updatePaymentFromDocument(array $arDocument, \Thurly\Sale\Payment $payment)
	{
	...
	function addPaymentFromDocumentByOrder(array $arDocument, \Thurly\Sale\Order $order)
	{
		...
		$paymentServiceId =	intval(\Thurly\Main\Config\Option::get('sale', self::getOptionNameByType(), 0));
		if($paymentServiceId<=0)
			$paymentServiceId = \Thurly\Sale\PaySystem\Manager::getInnerPaySystemId();

		if ($paySystem = \Thurly\Sale\PaySystem\Manager::getObjectById($paymentServiceId))
		{
			/** @var Thurly\Sale\Payment $payment */
			$paymentCollection = $order->getPaymentCollection();
			$payment = $paymentCollection->createItem($paySystem);
	...
	function addShipmentFromDocumentByOrder(array $arDocument, \Thurly\Sale\Order $order)
	{
		...
		$service = \Thurly\Sale\Delivery\Services\Manager::getObjectById($serviceId);
	...
	function updateShipmentQuantityFromDocument(array $arDocument, \Thurly\Sale\Shipment $shipment)
	{
				...
				foreach ($arDocument['items'] as $k=>$item)
				{
					...
					)
					{
						...
						elseif($externalQuantity > $shipmentItemQuantity)
						{
							...
							elseif(\Thurly\Main\Config\Option::get("sale", "1C_IMPORT_UPDATE_BASKET_QUANTITY", 'Y')=='Y')
							{
	...
	function deleteShipmentItemsByDocument($arDocument, \Thurly\Sale\Shipment $shipment)
	{
		if ($this->checkVersion1C($shipment)) // skip update if the same version
		{
			/** @var \Thurly\Sale\BasketItem $basketItems */
			$basket = $shipment->getCollection()->getOrder()->getBasket();
			foreach($basket as $basketItem)
			{
				foreach($arDocument['items'] as $item)
	...
	function updateShipmentNeedQuantity($externalQuantity, \Thurly\Sale\BasketItem $basketItem, \Thurly\Sale\Shipment $shipment, $shipmentItemFields)
	{
	...
	function updateShipmentNeedQuantityFromBasketItems( $needQuantityWithoutSystem, $needQuantity, \Thurly\Sale\BasketItem $basketItem, \Thurly\Sale\Shipment $shipment)
	{
	...
	function updateShipmentItemQuantity($needQuantity, \Thurly\Sale\BasketItem $basketItem, \Thurly\Sale\Shipment $shipment)
	{
	...
	function setShipmentItemQuantity($needQuantity, \Thurly\Sale\BasketItem $basketItem, \Thurly\Sale\Shipment $shipment)
	{
	...
	function updateShipmentNeedQuantityFromSystem($Quantity, \Thurly\Sale\BasketItem $basketItem, \Thurly\Sale\Shipment $shipment)
	{
	...
	function updateBasketNeedExternalQuantity($externalQuantity, $availableQuantityByProducts, \Thurly\Sale\BasketItem $basketItem, \Thurly\Sale\Shipment $shipment)
	{
	...
	function checkConditionForShipmentByBasket(\Thurly\Sale\BasketItem $basketItem, \Thurly\Sale\Shipment $shipment)
	{
    ...
    protected static function getDefaultPaySystem()
    {
        ...
        if (!is_set($id))
            $id =  (int)\Thurly\Main\Config\Option::get('sale', '1C_IMPORT_DEFAULT_PS_ORDER_PAID', 0);

        if (!is_set($innerPsId))
            $innerPsId = (int)Thurly\Sale\PaySystem\Manager::getInnerPaySystemId();
        ...
        {
            $r = \Thurly\Sale\Internals\PaySystemActionTable::getList(array(
                'select' => array('ID'),
	...
	function checkPSOnStatusPaymentOrder()
	{
		...
		if (!is_set($psOnStatusPaymentOrder))
			$psOnStatusPaymentOrder =  \Thurly\Main\Config\Option::get('sale', '1C_IMPORT_DEFAULT_PS_ORDER_PAID', 0);

		if (!is_set($innerPsId))
			$innerPsId = Thurly\Sale\PaySystem\Manager::getInnerPaySystemId();
	...
	function checkVersion1C(\Thurly\Sale\Shipment $shipment)
	{
	...
	function getAvailableSystemQuentityProduct(\Thurly\Sale\BasketItem $basketItem, \Thurly\Sale\Shipment $shipment)
	{
		$systemQuantity = 0;
	...
	function getAvailableQuentityProduct(\Thurly\Sale\BasketItem $basketItem, \Thurly\Sale\Shipment $shipment)
	{
		$quantity = 0;
		$order = $basketItem->getCollection()->getOrder();
		foreach($order->getShipmentCollection() as $shipment)
	...
	function existsBasketItem(\Thurly\Sale\BasketItem $basketItem, array $item = array())
	{
	...
	function nodeHandler(CDataXML $dataXml)
	{
 		...
 		)
		{
			...
			\Thurly\Main\Config\Option::set('sale', 'onec_exchange_type', 'container');
			\Thurly\Main\Config\Option::set('sale', 'onec_exchange_last_time', time());
		}
		elseif(!empty($value[GetMessage("CC_BSC1_DOCUMENT")]))
		{
			...
			if(!empty($arDocument))
			{
				...
				{
					if(\Thurly\Main\Config\Option::get('catalog', 'default_use_store_control', 'N')=='Y' || \Thurly\Main\Config\Option::get('catalog', 'enable_reservation', 'N')=='Y')
						$this->strErrorDocument .= "\n".GetMessage("CC_BSC1_USE_STORE_SALE");
					else
					{
						if(\Thurly\Main\Config\Option::get("main", "~sale_converted_15", 'N') <> 'Y')
							$this->strErrorDocument .= "\n".GetMessage("CC_BSC1_CONVERT_SALE");
						else
						{
							if(\Thurly\Main\Config\Option::get("sale", "allow_deduction_on_delivery", "N") == 'Y')
									...
									switch($arDocument['OPERATION_TYPE'])
									{
										case 'order_operation':
											...
											if(strlen($arDocument["XML_1C_DOCUMENT_ID"])>0)
											{
												...
												if(intval($arDocument["ID"])>0)
												{
													...
													if(strlen($this->strErrorDocument)<=0)
													{
														$order = \Thurly\Sale\Order::load($arDocument["ID"]);
												...
												elseif(\Thurly\Main\Config\Option::get("sale", "1C_IMPORT_NEW_ORDERS", "Y") == "Y")
												{
													...
													{
														$order = \Thurly\Sale\Order::load($arOrder["ID"]);
														if(strlen($this->strErrorDocument)<=0)
														{
															...
															if(strlen($arDocument["DATE"])>0)
																$order->setField('DATE_INSERT', new Thurly\Main\Type\DateTime(CDatabase::FormatDate($arDocument["DATE"]." ".$arDocument["TIME"], "YYYY-MM-DD HH:MI:SS", CLang::GetDateFormat("FULL", LANG))));
															$r = $order->save();
															if(!$r->isSuccess())
																$this->strErrorDocument .= array_shift($r->getErrors())->getMessage();
														}
										...
										case 'pay_system_a_operation':
											...
											if(strlen($arDocument["XML_1C_DOCUMENT_ID"])>0)
											{
												...
												{
													if($order = \Thurly\Sale\Order::load($arDocument['ORDER_ID']))
													{
														if (!$order->isCanceled())
														{
															if ($order->getField("STATUS_ID") != "F")
															{
																...
																else
																{
																	...
																	elseif (\Thurly\Main\Config\Option::get("sale", "1C_IMPORT_NEW_PAYMENT", "Y") == 'Y') // create new payment (ofline 1C))
																	{
																		$this->beforePaidCompatible1C($order);
										...
										case 'shipment_operation':
											...
											if(strlen($arDocument["XML_1C_DOCUMENT_ID"])>0)
											{
													...
													if($order = \Thurly\Sale\Order::load($arDocument['ORDER_ID']))
													{
														if ($order->getField("STATUS_ID") != "F")
														{
															...
															else
															{
																...
																elseif(\Thurly\Main\Config\Option::get("sale", "1C_IMPORT_NEW_SHIPMENT", 'Y')=='Y')
																{
												...
												elseif(\Thurly\Main\Config\Option::get("sale", "1C_IMPORT_NEW_ORDER_NEW_SHIPMENT", "Y") == 'Y') // create new shipment (ofline 1C))
												{
													if($arDocument["CANCELED"] != "true")
													{
														/** @var Thurly\Sale\Order $order */
					...
					\Thurly\Main\Config\Option::set('sale', 'onec_exchange_type', 'partial');
					\Thurly\Main\Config\Option::set('sale', 'onec_exchange_last_time', time());
				}
				else
				{
					...
					\Thurly\Main\Config\Option::set('sale', 'onec_exchange_type', 'default');
					\Thurly\Main\Config\Option::set('sale', 'onec_exchange_last_time', time());
				}
			}
	...
	function collectDocumentInfo($value)
	{
		...
		)
		{
			...
			if($bNeedFull)
			{
				IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT']."/thurly/modules/sale/general/export.php");
				$arOrder["DATE"] = $value["#"][GetMessage("CC_BSC1_1C_DATE")][0]["#"];
