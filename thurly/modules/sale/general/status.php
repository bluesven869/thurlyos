...
use	Thurly\Sale\Internals\StatusTable,
	...
	Thurly\Sale\Internals\OrderTable,
	Thurly\Sale\Internals\OrderArchiveTable,
	Thurly\Sale\Compatible,
	Thurly\Main\TaskTable,
	Thurly\Main\OperationTable,
	Thurly\Main\TaskOperationTable,
	Thurly\Main\Application,
	Thurly\Main\SystemException,
	Thurly\Main\Entity\Result,
...
class CSaleStatus
{
	...
	{
		$allowTypes = array(
			\Thurly\Sale\OrderStatus::TYPE,
			\Thurly\Sale\DeliveryStatus::TYPE,
		);
		...
		$filter = array(
			'select' => array(
				...
				'NOTIFY',
				'TYPE',
				'LID' => 'Thurly\Sale\Internals\StatusLangTable:STATUS.LID',
				'NAME' => 'Thurly\Sale\Internals\StatusLangTable:STATUS.NAME',
				'DESCRIPTION' => 'Thurly\Sale\Internals\StatusLangTable:STATUS.DESCRIPTION'
			),
			'filter' => array(
				'=ID' => $statusId,
				'=Thurly\Sale\Internals\StatusLangTable:STATUS.LID' => $languageId,
			),
			'limit'  => 1,
		);
	...
	static function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$query->addAliases(array(
			...
			'NAME'        => 'Thurly\Sale\Internals\StatusLangTable:STATUS.NAME',
			'DESCRIPTION' => 'Thurly\Sale\Internals\StatusLangTable:STATUS.DESCRIPTION',
			'GROUP_ID'    => 'Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.GROUP_ID',
		));

		$taskIdName = 'Thurly\Sale\Internals\StatusGroupTaskTable:STATUS.TASK_ID';
	...
	/*
	 * For modern api see: Thurly\Sale\OrderStatus and Thurly\Sale\DeliveryStatus
	 */
	...
	function CreateMailTemplate($ID)
	{
		...
		{
			\Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/general/status.php", $arSiteList["LANGUAGE_ID"]);
			...
			if (!($arEventType = $dbEventType->Fetch()))
			{
				...
				if ($statusData['TYPE'] == \Thurly\Sale\DeliveryStatus::TYPE)
				{
					$eventTypeName = Loc::getMessage("SKGS_CHANGING_SHIPMENT_STATUS_TO", null, $arSiteList["LANGUAGE_ID"])." \"".$arStatusLang["NAME"]."\"";

					$str .= "#SHIPMENT_ID# - ".\Thurly\Main\Localization\Loc::getMessage("SKGS_SHIPMENT_ID", null, $arSiteList["LANGUAGE_ID"])."\n";
					$str .= "#SHIPMENT_DATE# - ".\Thurly\Main\Localization\Loc::getMessage("SKGS_SHIPMENT_DATE", null, $arSiteList["LANGUAGE_ID"])."\n";
					$str .= "#SHIPMENT_STATUS# - ".\Thurly\Main\Localization\Loc::getMessage("SKGS_SHIPMENT_STATUS", null, $arSiteList["LANGUAGE_ID"])."\n";
				}
...
final class CSaleStatusAdapter implements Compatible\FetchAdapter
{
	...
	public static function getTasksOperations()
	{
		$result = TaskTable::getList(array(
			'select' => array(
				'TASK' => 'ID',
				'OPERATION' => 'Thurly\Main\TaskOperationTable:TASK.OPERATION_ID',
			),
			'filter' => array(
				'=MODULE_ID' => 'sale',
				'=BINDING' => 'status',
			...
			'order'  => array(
				'Thurly\Main\TaskOperationTable:TASK.OPERATION_ID' => 'ASC',
			),
		));
