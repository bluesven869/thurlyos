...
abstract class CBaseSaleReportHelper extends CReportHelper
{
	...
	{
		$res = \Thurly\Report\ReportTable::getList(
			array(
			...
			if (is_string($title) && strlen($title) > 0)
			{
				...
				if ($title === $titleMsg && is_string($row['SETTINGS']) && strlen($row['SETTINGS']) > 0)
				{
					...
					if (is_array($settings))
					{
						...
						if ($needUpdate)
						{
							Thurly\Report\ReportTable::update($id, array('SETTINGS' => serialize($settings)));
						}
					}
				}
			}
	...
	private static function localUpdate()
	{
		...
		{
			if (\Thurly\Main\Config\Option::get('sale', $updateCode, 'N') === 'Y'
	...
	public static function init()
	{
		...
		if (!self::$fInit)
		{
			...
			$result = Thurly\Main\SiteTable::getList(array('select' => array('LID', 'DEF', 'NAME')));
			$i = 0;
			...
			$statusEntity = Entity\Base::getInstance('\Thurly\Sale\Internals\StatusLang');
			if ($statusEntity instanceof \Thurly\Main\Entity\Base)
			{
			...
			$shipmentEntity = Entity\Base::getInstance('\Thurly\Sale\Internals\Shipment');
			if ($shipmentEntity instanceof \Thurly\Main\Entity\Base)
			{
			...
			$result = Thurly\Sale\Internals\StatusLangTable::getList(array(
			...
			$result = Thurly\Sale\Internals\PersonTypeTable::getList(array(
			...
			$result = \Thurly\Sale\PaySystem\Manager::getList(array(
			...
			$result = \Thurly\Sale\Delivery\Services\Table::getList(array(
				'select' => array('ID', 'NAME')/*,
				'filter' => array('=ACTIVE', 'Y')*/
			));
			while ($row = $result->fetch())
			...
			$result = Thurly\Iblock\IblockSiteTable::getList();
			...
			$result = Thurly\Catalog\StoreTable::getList(array('select' => array('ID', 'TITLE')));
			while ($row = $result->fetch())
			{
		...
		$reports = array(
			'12.0.0' => array(
				...
				array(
					...
					'mark_default' => 5,
					'settings' => unserialize('a:7:{s:6:"entity";s:16:"Thurly\Main\User";s:6:"period";a:2:{s:4:"type";s:5:"month";'.
							...
							's:14:"COUNT_DISTINCT";}i:3;a:3:{s:4:"name";s:35:"Thurly\Sale\Internals\Order:USER.ID";s:5:"alias";'.
							's:14:"xxxxxxxxxxxxxx";s:4:"aggr";s:14:"COUNT_DISTINCT";}i:4;a:3:{s:4:"name";'.
							's:45:"Thurly\Sale\Internals\Order:USER.DISCOUNT_ALL";s:5:"alias";s:15:"xxxxxxxxxxxxxxx";'.
							's:4:"aggr";s:3:"SUM";}i:9;a:3:{s:4:"name";s:38:"Thurly\Sale\Internals\Order:USER.PRICE";'.
							's:5:"alias";s:13:"xxxxxxxxxxxxx";s:4:"aggr";s:3:"AVG";}i:5;a:3:{s:4:"name";'.
							's:38:"Thurly\Sale\Internals\Order:USER.PRICE";s:5:"alias";s:16:"xxxxxxxxxxxxxxxx";s:4:"aggr";'.
							's:3:"SUM";}i:6;a:3:{s:4:"name";s:48:"Thurly\Sale\Internals\Order:USER.SUM_PAID_FORREP";'.
				...
				array(
					...
					'mark_default' => 7,
					'settings' => unserialize('a:10:{s:6:"entity";s:29:"Thurly\\Sale\\Internals\\Product";'.
			...
			'12.5.0' => array(
				array(
					...
					'description' => '',
					'mark_default' => 9,
					'settings' => unserialize('a:11:{s:6:"entity";s:29:"Thurly\Sale\Internals\Product";s:6:"period";'.
				...
				array(
					...
					'description' => '',
					'mark_default' => 10,
					'settings' => unserialize('a:12:{s:6:"entity";s:29:"Thurly\Sale\Internals\Product";s:6:"period";'.
				...
				array(
					...
					'description' => '',
					'mark_default' => 11,
					'settings' => unserialize('a:11:{s:6:"entity";s:28:"Thurly\Sale\Internals\Basket";s:6:"period";'.
	...
	public static function getCurrentVersion()
	{
		...
		include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/install/version.php");
...
class CSaleReportSaleOrderHelper extends CBaseSaleReportHelper
{
	...
	{
		return 'Thurly\Sale\Internals\Order';
	}
	...
	public static function getCompareVariations()
	{
		return array_merge(parent::getCompareVariations(), array(
			...
			'\Thurly\Main\User' => array(
				'EQUAL'
			),
			'\Thurly\Main\Group' => array(
				'EQUAL'
			)
		));
	}
	...
	{
		if ($field->getRefEntityName() == '\Thurly\Main\User')
		{
			// USER
		...
		else if ($field->getRefEntityName() == '\Thurly\Main\Group')
		{
			// GROUP
			if ($filterElement['value'])
			{
	...
	public static function formatResultValue($k, &$v, &$row, &$cInfo, $total, &$customChartValue = null)
	{
		...
		/** @var Thurly\Main\Entity\Field[] $cInfo */
		...
		if (empty($elem['aggr']))
		{
			...
			if ($field->getEntity()->getName() == 'User')
			{
							...
							true))
				{
					...
					$href = array('pattern' => '/thurly/admin/sale_buyers_profile.php?USER_ID=#'.$userDef.'.ID#&lang='.LANG);
				}
			}
			else if ($field->getEntity()->getName() == 'Order')
			{
				if ($elem['name'] === 'ID')
				{
					$href = array('pattern' => '/thurly/admin/sale_order_view.php?ID=#ID#&lang='.LANG);
				}
			}
			else if ($field->getEntity()->getName() === 'Payment')
			{
				...
				{
					$href = array('pattern' => '/thurly/admin/sale_order_payment_edit.php?order_id=#Payment:ORDER.ORDER_ID#&payment_id=#Payment:ORDER.ID#&lang='.LANG);
				}
				else if ($elem['name'] === 'Payment:ORDER.PAY_SYSTEM_ID'
					|| $elem['name'] === 'Payment:ORDER.PAY_SYSTEM_NAME')
				{
					$href = array('pattern' => '/thurly/admin/sale_pay_system_edit.php?ID=#Payment:ORDER.PAY_SYSTEM_ID#&lang='.LANG);
				}
			}
			else if ($field->getEntity()->getName() === 'Shipment')
			{
				...
				{
					$href = array('pattern' => '/thurly/admin/sale_order_shipment_edit.php?order_id=#Shipment:REPS_ORDER.ORDER_ID#&shipment_id=#Shipment:REPS_ORDER.ID#&lang='.LANG);
				}
				else if ($elem['name'] === 'Shipment:REPS_ORDER.DELIVERY_ID'
					|| $elem['name'] === 'Shipment:REPS_ORDER.DELIVERY_NAME')
				{
					$href = array('pattern' => '/thurly/admin/sale_delivery_service_edit.php?ID=#Shipment:REPS_ORDER.DELIVERY_ID#&lang='.LANG);
				}
			}
		}
...
class CSaleReportUserHelper extends CBaseSaleReportHelper
{
	...
	public static function getColumnList()
	{
		...
		return array(
			...
			'EMAIL',
			'PERSONAL_PHONE',
			'Thurly\Sale\Internals\Order:USER' => array(
				'DATE_INSERT_SHORT',
				'DATE_UPDATE_SHORT',
				'STATUS' => array(
					'STATUS_ID',
	...
	public static function getCompareVariations()
	{
		return array_merge(parent::getCompareVariations(), array(
			// Order
			'\Thurly\Sale\Internals\StatusLang' => array(
				'EQUAL',
				'NOT_EQUAL'
			),
			'Thurly\Sale\Internals\Order:USER.PERSON_TYPE_ID' => array(
				'EQUAL',
				'NOT_EQUAL'
			),
			// User
	...
	{
		if (strncasecmp($fElem['name'], 'Thurly\Sale\Internals\Order:USER', 23) === 0)
		{
			return false;
		}
	...
	{
		if (strncasecmp($elem['name'], 'Thurly\Sale\Internals\Order:USER', 23) === 0)
		{
			return false;
		}
	...
	private static function filterBySiteNeeded(&$select, &$filter)
	{
		...
		if (!$bResult)
		{
			if (self::fieldInFilter($filter, 'Thurly\Sale\Internals\Order:USER.')) $bResult = true;
		}
	...
	public static function beforeViewDataQuery(&$select, &$filter, &$group, &$order, &$limit, &$options, &$runtime = null)
	{
		...
		if (self::filterBySiteNeeded($select, $filter))
		{
			if ($filter['LOGIC'] === 'AND')
			{
				$filter[] = array(
					'LOGIC' => 'OR',
					array('=Thurly\Sale\Internals\Order:USER.LID' => $DB->ForSql(self::getDefaultSiteId())),
					array('=Thurly\Sale\Internals\Order:USER.LID' => null)
				);
			}
			else
			{
	...
	public static function formatResultValue($k, &$v, &$row, &$cInfo, $total, &$customChartValue = null)
	{
		...
		foreach (self::$monetaryFields as $monField)
		{
			if ($cInfo['field']->getEntity()->getDataClass() === 'Thurly\\Sale\\Internals\\OrderTable'
	...
	public static function getDefaultElemHref($elem, $fList)
	{
		...
		if (empty($elem['aggr']))
		{
			...
			if ($field->getEntity()->getName() == 'User')
			{
				...
				{
					$href = array('pattern' => '/thurly/admin/sale_buyers_profile.php?USER_ID=#ID#&lang='.LANG);
				}
				else if ($elem['name'] == 'EMAIL')
				{
					$href = array('pattern' => 'mailto:#EMAIL#');
				}
				else if ($elem['name'] == 'Thurly\Sale\Internals\Order:USER.EMP_STATUS_BY.SHORT_NAME')
				{
					$href = array('pattern' => '/thurly/admin/sale_buyers_profile.php?USER_ID=#Thurly\Sale\Internals\Order:USER.EMP_STATUS_BY.ID#&lang='.LANG);
				}
				else if ($elem['name'] == 'Thurly\Sale\Internals\Order:USER.EMP_STATUS_BY.EMAIL')
				{
					$href = array('pattern' => 'mailto:#Thurly\Sale\Internals\Order:USER.EMP_STATUS_BY.EMAIL#');
				}
				else if ($elem['name'] == 'Thurly\Sale\Internals\Order:USER.EMP_CANCELED_BY.SHORT_NAME')
				{
					$href = array('pattern' => '/thurly/admin/sale_buyers_profile.php?USER_ID=#Thurly\Sale\Internals\Order:USER.EMP_CANCELED_BY.ID#&lang='.LANG);
				}
				else if ($elem['name'] == 'Thurly\Sale\Internals\Order:USER.EMP_CANCELED_BY.EMAIL')
				{
					$href = array('pattern' => 'mailto:#Thurly\Sale\Internals\Order:USER.EMP_CANCELED_BY.EMAIL#');
				}
			}
			elseif ($field->getEntity()->getName() == 'Order')
			{
				if ($elem['name'] == 'Thurly\Sale\Internals\Order:USER.ID')
				{
					$href = array('pattern' => '/thurly/admin/sale_order_view.php?ID=#Thurly\Sale\Internals\Order:USER.ID#&lang='.LANG);
				}
			}
		}
...
class CSaleReportSaleBasketHelper extends CBaseSaleReportHelper
{
	...
	public static function getEntityName()
	{
		return 'Thurly\Sale\Internals\Basket';
	}
	...
	public static function getDefaultElemHref($elem, $fList)
	{
		...
		if (empty($elem['aggr']))
		{
			...
			elseif ($field->getEntity()->getName() == 'Basket')
			{
				...
				{
					$href = array('pattern' => '/thurly/admin/cat_product_edit.php?IBLOCK_ID='.
						'#PRODUCT.IBLOCK.IBLOCK.ID#&type=#PRODUCT.IBLOCK.IBLOCK.IBLOCK_TYPE_ID#&ID='.
						'#PRODUCT_ID#&lang='.LANG.'&WF=Y');
				}
				elseif ($elem['name'] == 'ORDER_ID')
				{
					$href = array('pattern' => '/thurly/admin/sale_order_view.php?ID=#ORDER_ID#&lang='.LANG);
				}
			}
			else if ($field->getEntity()->getName() == 'Order')
			{
				...
				{
					$href = array('pattern' => '/thurly/admin/sale_order_view.php?ID=#ORDER.ID#&lang='.LANG);
				}
			}
			else if ($field->getEntity()->getName() === 'Payment')
			{
				...
				{
					$href = array('pattern' => '/thurly/admin/sale_order_payment_edit.php?order_id=#ORDER.Payment:ORDER.ORDER_ID#&payment_id=#ORDER.Payment:ORDER.ID#&lang='.LANG);
				}
				else if ($elem['name'] === 'ORDER.Payment:ORDER.PAY_SYSTEM_ID'
					|| $elem['name'] === 'ORDER.Payment:ORDER.PAY_SYSTEM_NAME')
				{
					$href = array('pattern' => '/thurly/admin/sale_pay_system_edit.php?ID=#ORDER.Payment:ORDER.PAY_SYSTEM_ID#&lang='.LANG);
				}
			}
			else if ($field->getEntity()->getName() === 'Shipment')
			{
				...
				{
					$href = array('pattern' => '/thurly/admin/sale_order_shipment_edit.php?order_id=#ORDER.Shipment:REPS_ORDER.ORDER_ID#&shipment_id=#ORDER.Shipment:REPS_ORDER.ID#&lang='.LANG);
				}
				else if ($elem['name'] === 'ORDER.Shipment:REPS_ORDER.DELIVERY_ID'
					|| $elem['name'] === 'ORDER.Shipment:REPS_ORDER.DELIVERY_NAME')
				{
					$href = array('pattern' => '/thurly/admin/sale_delivery_service_edit.php?ID=#ORDER.Shipment:REPS_ORDER.DELIVERY_ID#&lang='.LANG);
				}
			}
		}
...
class CSaleReportSaleProductHelper extends CBaseSaleReportHelper
{
	...
	public static function getEntityName()
	{
		return 'Thurly\Sale\Internals\Product';
	}
