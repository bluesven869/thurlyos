...
class CSaleYMHandler
{
	 ...
	 * @return \Thurly\Main\Entity\UpdateResult|bool
	 */
	...
	public static function getSettings($cached = true)
	{
		...
		{
			$settingsRes = Thurly\Sale\TradingPlatformTable::getList(array(
	...
	public static function saveSettings($arSettings)
	{
		...
		if(!empty($settings))
		{
			...
			$result = Thurly\Sale\TradingPlatformTable::update(
	...
	protected function processCartRequest($arPostData)
	{
		...
		$res = \Thurly\Sale\TradingPlatform\YMarket\Order::create(array(
		...
		foreach($deliveryObjs as $delivery)
		{
			...
			/** @var \Thurly\Sale\Shipment $clonedShipment*/
	...
	protected function makeAdditionalOrderProps($address, $buyer, $psId, $deliveryId, $locationId)
	{
		...
		while ($arOrderProps = $dbOrderProps->Fetch())
		{
			...
			elseif(strlen($this->orderProps["LOCATION"]) > 0 && $arOrderProps["CODE"] == $this->orderProps["LOCATION"])
			{
				if($locationId > 0)
				{
					$dbRes = \Thurly\Sale\Location\LocationTable::getById($locationId);
	...
	protected function processOrderAcceptRequest($arPostData)
	{
		...
		$dbRes = \Thurly\Sale\TradingPlatform\OrderTable::getList(array(
			"filter" => array(
		...
		if($orderId <= 0)
		{
			...
			$dbRes = \Thurly\Iblock\ElementTable::getList(array(
				'filter' => array(
					'=ID' => $productIds
				),
				'select' => array(
			...
			$r = new \Thurly\Sale\Result();
			$r->addWarning(new \Thurly\Main\Error(GetMessage('SALE_YMH_MARK_BUYER_WAITING'), 'YMARKET_BUYER_INFO_WAITING'));
			\Thurly\Sale\EntityMarker::addMarker($order, $order, $r);
			...
			$res = \Thurly\Sale\TradingPlatform\OrderTable::add(array(
				"ORDER_ID" => $res->getId(),
				"TRADING_PLATFORM_ID" => YMarket\YandexMarket::getInstance()->getId(),
				"EXTERNAL_ORDER_ID" => $arPostData["order"]["id"]
			));
	...
	protected function checkBasketPrice(array $items, \Thurly\Sale\Order $order)
	{
		$yandexBasketPrice = 0;
	...
	protected function setBasketOldPrice(array $items, \Thurly\Sale\Order $order)
	{
		$basket = $order->getBasket();
	...
	protected function processOrderStatusRequest($arPostData)
	{
		...
		{
			$dbOrder = \Thurly\Sale\Internals\OrderTable::getList(array(
	...
	public function sendStatus($orderId, $status, $substatus = false)
	{
		...
		if(!$bResult)
		{
			...
			{				
				$r = new \Thurly\Sale\Result();
				$r->addWarning(new \Thurly\Main\Error($message, 'YMARKET_STATUS_CHANGE_ERROR'));
				\Thurly\Sale\EntityMarker::addMarker($order, $order, $r);
				$order->setField('MARKED', 'Y');
				$order->save();
			}
	...
	public function getOrderInfo($orderId)
	{
		...
		$res = \Thurly\Sale\Internals\OrderTable::getList(array(
			'filter' => array(
				'=ID' => $orderId,
	...
	/**
	 * @param \Thurly\Main\Event $params
	 */
	public static function onSaleStatusOrderChange(Thurly\Main\Event $params)
	{
		/** @var \Thurly\Sale\Order $order */
	 ...
	 * @param \Thurly\Main\Event $params
	 */
	public static function onSaleOrderCanceled(Thurly\Main\Event $params)
	{
		global $USER;
	 ...
	 * @param \Thurly\Main\Event $params
	 */
	public static function onSaleShipmentDelivery(Thurly\Main\Event $params)
	{
	 ...
	 * @param \Thurly\Main\Event $params
	 */
	public static function onSaleOrderPaid(Thurly\Main\Event $params)
	{
		/** @var \Thurly\Sale\Order $order */
		$order = $params->getParameter("ENTITY");
	 ...
	 * @param \Thurly\Main\Event $params
	 */
	public static function onShipmentDeducted(Thurly\Main\Event $params)
	{
		/** @var \Thurly\Sale\Shipment $shipment */
		$shipment = $params->getParameter("ENTITY");
	...
	protected function mapYandexStatusToOrder($order, $yandexStatus, $cancelReason="")
	{
		...
		$result = false;
		$thurlyStatus = $settings["STATUS_IN"][$yandexStatus];
		...
		switch($thurlyStatus)
		{
			...
			/* statuses */
			default:
				if(CSaleStatus::GetByID($thurlyStatus))
				{
					$result = CSaleOrder::StatusOrder($order["ID"], $thurlyStatus);
				}
				break;
		}
		...
		$this->log(
			...
			$order["ID"],
			($result ? GetMessage("SALE_YMH_LOG_TYPE_ORDER_STATUS_CHANGE_OK") : GetMessage("SALE_YMH_LOG_TYPE_ORDER_STATUS_CHANGE_ERROR"))." (".$thurlyStatus.")"
		);
	...
	public static function install()
	{
		...
		if(empty($settings))
		{
			$res =  Thurly\Sale\TradingPlatformTable::add(array(
	...
	public static function unInstall($deleteRecord = true)
	{
		...
		{
			if($deleteRecord)
				Thurly\Sale\TradingPlatformTable::delete(static::TRADING_PLATFORM_CODE);
			else
	...
	protected function sendEmailNewOrder($newOrderId, $buyer)
	{
		...
		$strOrderList = "";
		$baseLangCurrency = Thurly\Sale\Internals\SiteCurrencyTable::getSiteCurrency($this->siteId);
	...
	public static function takeOutOrdersToCorrespondentTable()
	{
		...
		$conn = \Thurly\Main\Application::getConnection();
		$helper = $conn->getSqlHelper();

		$correspondence = $conn->query(
			'SELECT ID
				FROM '.$helper->quote(\Thurly\Sale\TradingPlatform\OrderTable::getTableName()).'
		...
		$sql = 'INSERT INTO '.\Thurly\Sale\TradingPlatform\OrderTable::getTableName().' (ORDER_ID, EXTERNAL_ORDER_ID, TRADING_PLATFORM_ID)
				SELECT ID, '.$right.', '.$platformId.'
					FROM '.\Thurly\Sale\Internals\OrderTable::getTableName().'
	...
	{
		if(\Thurly\Main\Config\Option::get("sale", 'YANDEX_MARKET_DELIVERY_PS_IDS_CONVERTED', 'N') == 'Y')
			return '';
		...
		if(!empty($settings['SETTINGS']) && is_array($settings['SETTINGS']))
		{
			...
			foreach($settings['SETTINGS'] as $siteId => $siteSettings)
			{
				if(!empty($siteSettings['DELIVERIES']) && is_array($siteSettings['DELIVERIES']))
				{
					...
					{
						$newId = \Thurly\Sale\Delivery\Services\Manager::getIdByCode($oldId);
						$message .= $oldId.'->'.$newId.', ';
			...
			if(!empty($message))
			{
				...
				$res = Thurly\Sale\TradingPlatformTable::update(
					YMarket\YandexMarket::getInstance()->getId(),
					array("SETTINGS" => $settings['SETTINGS'])
				);

				if($res->isSuccess())
					\Thurly\Main\Config\Option::set("sale", 'YANDEX_MARKET_DELIVERY_PS_IDS_CONVERTED', 'Y');
			}
		}
	...
	public function loadOrderByYandexOrderId($yandexOrderId)
	{
		...
		$list = \Thurly\Sale\Order::loadByFilter($filter);
