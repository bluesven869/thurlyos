...
use Thurly\Main,
	...
	Thurly\Sale;
use Thurly\Sale\Discount\CumulativeCalculator;
...
class CSaleCondCtrlOrderFields extends CSaleCondCtrlComplex
{
	...
	public static function GetControls($strControlID = false)
	{
		...
		$iterator = Sale\Delivery\Services\Table::getList(array(
			...
			'filter' => array('=CLASS_NAME' => '\Thurly\Sale\Delivery\Services\Group')
		));
		...
		{
			if ($delivery['CLASS_NAME'] == '\Thurly\Sale\Delivery\Services\Group')
				continue;
...
class CSaleCondCtrlPastOrder extends CSaleCondCtrlOrderFields
{
	...
	{
		$orderData = \Thurly\Sale\Internals\OrderTable::getList(
			array(
				'select' => array('ID'),
				'filter' => array(
					'=CANCELED' => 'N',
		...
		return \Thurly\Sale\Order::load($orderData['ID']);
	}
