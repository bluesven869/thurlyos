 ...
 * @see \Thurly\Sale\Discount\Actions
 */
class CSaleDiscountActionApply
{
	 ...
	 *
	 * @deprecated deprecated since sale 16.0.10
	 * @see \Thurly\Sale\Discount\Actions::isManualMode
	 *
	 ...
	 *
	 * @deprecated deprecated since sale 16.0.10
	 * @see \Thurly\Sale\Discount\Actions::setUseMode
	 *
	 * @param array &$order			Order data.
	 * @return void
	 */
	 ...
	 *
	 * @deprecated deprecated since sale 16.0.10
	 * @see \Thurly\Sale\Discount\Actions::filterBasketForAction
	 *
	 ...
	 *
	 * @deprecated deprecated since sale 16.0.10
	 * @see \Thurly\Sale\Discount\Actions::applyToDelivery
	 *
	 * @param array &$order				Order data.
	 ...
	 *
	 * @deprecated deprecated since sale 16.0.10
	 * @see \Thurly\Sale\Discount\Actions::applyToBasket
	 *
	 * @param array &$order			Order data.
	 ...
	 *
	 * @deprecated deprecated since sale 16.0.10
	 * @see \Thurly\Sale\Discount\Actions::applySimpleGift
	 *
	 * @param array &$order				Order data.
...
class CSaleCumulativeAction extends CGlobalCondCtrl
{
	...
	public static function Generate($oneCondition, $params, $control, $subs = false)
	{
		...
		/** @see \Thurly\Sale\Discount\Actions::applyCumulativeToBasket() */
		return "\\Thurly\\Sale\\Discount\\Actions::applyCumulativeToBasket({$params['ORDER']}, {$rangesAsString}, {$configurationAsString}, {$filterCode})";
	}
...
class CSaleActionGiftCtrlGroup extends CSaleActionCtrlGroup
{
	...
	public static function Generate($arOneCondition, $arParams, $arControl, $arSubs = false)
	{
		...
		{
			$mxResult = '\Thurly\Sale\Discount\Actions::applySimpleGift(' . $arParams['ORDER'] . ', ' . implode('; ',$arSubs) . ');';
		}
		return $mxResult;
	}
...
class CSaleActionCtrlDelivery extends CSaleActionCtrl
{
	...
	public static function Generate($arOneCondition, $arParams, $arControl, $arSubs = false)
	{
		...
		if (!$boolError)
		{
			...
			$mxResult = '\Thurly\Sale\Discount\Actions::applyToDelivery('.$arParams['ORDER'].', '.var_export($actionParams, true).')';
			unset($actionParams);
		}
		...
		$arAtomList = array(
			...
			'GiftValue' => array(
				'JS' => array(
					...
					'name' => 'gifts_value',
					'type' => 'multiDialog',
					'popup_url' =>  '/thurly/tools/sale/product_search_dialog.php',
					'popup_params' => array(
						'lang' => LANGUAGE_ID,
						'caller' => 'discount'
					),
	...
	public static function Generate($arOneCondition, $arParams, $arControl, $arSubs = false)
	{
		...
		if (!$boolError)
		{
			...
			if (!empty($arSubs))
			{
				...
				$mxResult .= '};';
				$mxResult .= '\Thurly\Sale\Discount\Actions::applyToBasket('.$arParams['ORDER'].', '.var_export($discountParams, true).', '.$filter.');';
				unset($filter);
			}
			else
			{
				$mxResult = '\Thurly\Sale\Discount\Actions::applyToBasket('.$arParams['ORDER'].', '.var_export($discountParams, true).', "");';
			}
...
//				'EXECUTE_MODULE' => 'all', //we can't set "all" and can't set sale. So, below we control it
		...
		$additionalControls = CSaleCondCtrlBasketFields::GetControls(false);
