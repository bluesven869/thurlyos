...
class CAllSaleOrderChange
{
	...
	public static function deleteOld($days, $limit = null)
	{
		...
		$expired = new \Thurly\Main\Type\DateTime();
	...
	public static function MakeRecordFromField($field, $arFields, $entityName = "", $entity = null)
	{
		foreach (CSaleOrderChangeFormat::$operationTypes as $code => $arInfo)
		{
			...
			if (in_array($field, $arInfo["TRIGGER_FIELDS"]))
			{
				...
				if ($entity !== null)
				{
					/** @var \Thurly\Sale\Internals\Fields $fields */
...
class CSaleOrderChangeFormat
{
				...
				if ($param == "DELIVERY_ID")
				{
					if (!array_key_exists('DELIVERY_NAME', $data) && strval($data['DELIVERY_NAME']) != '')
					{
						...
						elseif (intval($value) > 0)
						{
							...
							{
								$arDelivery = \Thurly\Sale\Delivery\Services\Manager::getById($value);
							}
							else
							{
	...
	public static function FormatShipmentStatusChanged($arData)
	{
		...
		{
			$status = \Thurly\Sale\Helpers\Admin\Blocks\OrderShipmentStatus::getShipmentStatusList($arData['STATUS_ID']);
	...
	public static function FormatMarkerSuccess($data)
	{
		...
		{
			$data['ENTITY_NAME'] = \Thurly\Main\Localization\Loc::getMessage('SOC_MARKER_'. $data['ENTITY_TYPE'] .'_INFO');
		}
