...
class CAllSaleBasket
{
	...
	public static function GetProductProvider($arBasketItem)
	{
		...
		{
			return '\Thurly\Catalog\Product\CatalogProviderCompatibility';
		}
	...
	public static function DoGetUserShoppingCart($siteId, $userId, $shoppingCart, &$arErrors, $arCoupons = array(), $orderId = 0, $enableCustomCurrency = false)
	{
		...
		foreach ($shoppingCart as $itemIndex => $arShoppingCartItem)
		{
			...
			if ($arShoppingCartItem["CAN_BUY"] == "Y")
			{
				...
				$arShoppingCartItem["PRICE"] = \Thurly\Sale\PriceMaths::roundPrecision($arShoppingCartItem["PRICE"]);
				...
				if ($arShoppingCartItem["VAT_RATE"] > 0)
					$arShoppingCartItem["VAT_VALUE"] = \Thurly\Sale\PriceMaths::roundPrecision(($arShoppingCartItem["PRICE"] / ($arShoppingCartItem["VAT_RATE"] + 1)) * $arShoppingCartItem["VAT_RATE"]);
		...
		foreach ($arShoppingCart as &$arItem)
		{
			...
			if (array_key_exists("ID", $arItem) && (int)$arItem["ID"] > 0)
			{
				...
				if ($isOrderConverted != 'N')
				{
					...
					$r = \Thurly\Sale\Compatible\BasketCompatibility::update($arItem["ID"], $fields);
		...
		if (!array_key_exists('IGNORE_CALLBACK_FUNC', $arFields) || 'Y' != $arFields['IGNORE_CALLBACK_FUNC'])
		{
			...
			)
			{
				...
				if ($productProvider = CSaleBasket::GetProductProvider(array("MODULE" => $arFields["MODULE"], "PRODUCT_PROVIDER_CLASS" => $arFields["PRODUCT_PROVIDER_CLASS"])))
				{
					...
					elseif (get_parent_class($productProvider) == 'Thurly\Sale\SaleProviderBase')
					{
...
//						$productProvider::getProviderData();
	...
	function Update($ID, $arFields)
	{
		...
		else
		{
			...
			{
				/** @var \Thurly\Sale\Result $r */
				$r = \Thurly\Sale\Compatible\BasketCompatibility::update($ID, $arFields);
				if (!$r->isSuccess())
				{
					foreach($r->getErrorMessages() as $error)
					{
	...
	public static function OrderBasket($orderID, $fuserID = 0, $strLang = SITE_ID, $arDiscounts = False)
	{
		...
		while ($arBasket = $dbBasketList->Fetch())
		{
			...
			if ($arBasket["DELAY"] == "N" && $arBasket["CAN_BUY"] == "Y")
			{
				...
				if (!empty($arFields))
				{
					...
					{
						$found = true;
						if (!\Thurly\Sale\Compatible\DiscountCompatibility::isInited())
							\Thurly\Sale\Compatible\DiscountCompatibility::init();
						if (\Thurly\Sale\Compatible\DiscountCompatibility::isInited())
							\Thurly\Sale\Compatible\DiscountCompatibility::setRepeatSave(true);
					}
		...
		if ($isOrderConverted != 'N' && $found)
		{
			...
			{
				if (\Thurly\Sale\Compatible\DiscountCompatibility::isInited())
					\Thurly\Sale\Compatible\DiscountCompatibility::setRepeatSave(false);
	...
	function OrderCanceled($orderID, $bCancel)
	{
		...
		if ($isOrderConverted != 'N')
		{
			\Thurly\Sale\Compatible\OrderCompatibility::cancel($orderID, $bCancel?'Y':'N');
		}
		else
		{
			$arOrder = CSaleOrder::GetByID($orderID);
	...
	function OrderDeduction($orderID, $bUndoDeduction = false, $recurringID = 0, $bAutoDeduction = true, $arStoreBarcodeOrderFormData  = array())
	{
		...
		if ($isOrderConverted != 'N')
		{
			$ship = !$bUndoDeduction;
			/** @var \Thurly\Sale\Result $r */
			$r = \Thurly\Sale\Compatible\OrderCompatibility::shipment($orderID, $ship, $arStoreBarcodeOrderFormData);
			if (!$r->isSuccess(true))
			{
				foreach($r->getErrorMessages() as $error)
				{
