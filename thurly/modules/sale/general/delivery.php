...
use Thurly\Sale\Location\Admin\LocationHelper as Helper;
...
class CAllSaleDelivery
{
	const CONN_ENTITY_NAME = 'Thurly\Sale\Delivery\DeliveryLocation';
	...
	static function DoProcessOrder(&$arOrder, $deliveryCode, &$arErrors)
	{
		...
		{
			$service = \Thurly\Sale\Delivery\Services\Manager::getObjectById(
				CSaleDeliveryHandler::getIdFromNewSid($deliveryCode)
			);
		}
		else
		{
			$service = \Thurly\Sale\Delivery\Services\Manager::getObjectByCode($deliveryCode);
		}
	...
	/**
	 * @deprecated Use \Thurly\Sale\Delivery\Services\Manager
	 */
	...
	/**
	 * @deprecated Use \Thurly\Sale\Delivery\Services\Table::getById().
	 */
	...
	public static function Update($oldId, $arFields, $arOptions = array())
	{
		...
		$dbRes = Thurly\Sale\Delivery\Services\Table::getList(array(
			'filter' => array(
				"CODE" => $oldId,
				"=CLASS_NAME" => '\Thurly\Sale\Delivery\Services\Configurable'
			)
		));
		...
		$fields = array_intersect_key($arFields, Thurly\Sale\Delivery\Services\Table::getMap());
		...
		if(!empty($fields))
		{
			...
			$res = \Thurly\Sale\Delivery\Services\Manager::update($newId, $fields);
		...
		if(isset($arFields["WEIGHT_FROM"]) || isset($arFields["WEIGHT_TO"]))
		{
			...
			if($restrict = $rstrRes->fetch())
			{
				...
				{
					$rres = \Thurly\Sale\Internals\ServiceRestrictionTable::delete($restrict["ID"]);
				}
				else
				{
		...
		if(isset($arFields["STORE"]))
		{
			...
			if($stores)
				\Thurly\Sale\Delivery\ExtraServices\Manager::saveStores($newId, $stores);
		}
	...
	public static function Delete($ID)
	{
		...
		try
		{
			$res = \Thurly\Sale\Delivery\Services\Manager::delete($newId);
		}
	...
	{
		$serviceFields = \Thurly\Sale\Delivery\Services\Table::getMap();
		...
		switch($restriction["CLASS_NAME"])
		{
			...
			case '\Thurly\Sale\Delivery\Restrictions\ByLocation':
				...
				if($fieldInFilter && strlen($value) > 0 && $restriction['SERVICE_ID'] > 0)
				{
					try
					{
						$result = \Thurly\Sale\Delivery\DeliveryLocationTable::checkConnectionExists(
					...
					catch(\Thurly\Sale\Location\Tree\NodeNotFoundException $e)
					{
						$result = false;
					}
					...
					try
					{
						return \Thurly\Sale\Delivery\DeliveryLocationTable::checkConnectionExists(
							intval($restriction['SERVICE_ID']),
							$value,
							array(
	...
	public static function GetList($arOrder = array("SORT" => "ASC", "NAME" => "ASC"), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array('*'))
	{
		...
		$services = array();
		$params['filter']['=CLASS_NAME'] = '\Thurly\Sale\Delivery\Services\Configurable';
		...
		while($service = $dbRes->fetch())
		{
			...
			{
				$service["CLASS_NAME"] = '\Thurly\Sale\Delivery\Services\Configurable';
				$tmpSrv = \Thurly\Sale\Delivery\Services\Manager::getPooledObject($service);
			...
			{
				$stores = \Thurly\Sale\Delivery\ExtraServices\Manager::getStoresList($service["ID"]);
	...
	static function Add($arFields, $arOptions = array())
	{
		...
		$fields["PARENT_ID"] = 0;
		$fields["CLASS_NAME"] = '\Thurly\Sale\Delivery\Services\Configurable';
		$fields["CONFIG"] = array(
			"MAIN" => array(
		...
		{
			\Thurly\Sale\Delivery\Services\Manager::update($newId, array('CODE' => $newId));
		}
		...
		if(isset($arFields["LOCATIONS"]) && is_array($arFields["LOCATIONS"]))
		{
			...
			\Thurly\Sale\Internals\ServiceRestrictionTable::add(array(
				"SERVICE_ID" => $newId,
				"SERVICE_TYPE" => \Thurly\Sale\Services\Base\RestrictionManager::SERVICE_TYPE_SHIPMENT,
				"CLASS_NAME" => '\Thurly\Sale\Delivery\Restrictions\ByLocation',
				"SORT" => 100
			));
		}
	...
	protected static function createD2LTable()
	{
		...
		{
			$result->addError(new \Thurly\Main\Error('Wrong connection type!'));
			return $result;
		}
	...
	public static function convertToNew($renameTable = false)
	{
		...
		while($delivery = $deliveryRes->fetch())
		{
			...
			if(intval($newId) <= 0)
			{
				$result->addError( new \Thurly\Main\Entity\EntityError("Can't convert old delivery id: ".$delivery["CODE"]));
				continue;
			}
	...
	public static function convertPSRelations()
	{
		...
		$query = new \Thurly\Main\Entity\Query(DeliveryPaySystemTable::getEntity());
	 ...
	 * @throws \Thurly\Main\ArgumentNullException
	 */
	public static function convertOrderNewToOld(\Thurly\Sale\Shipment $shipment)
	{
		/** @var \Thurly\Sale\ShipmentCollection $shipmentCollection */
		$shipmentCollection = $shipment->getCollection();
		/** @var \Thurly\Sale\Order $newOrder */
		...
		/** @var \Thurly\Sale\ShipmentItem $shipmentItem */
	...
	public static function convertOrderOldToNew(array $oldOrder)
	{
		...
		$newOrder = \Thurly\Sale\Order::create($siteId, null, $oldOrder["CURRENCY"]);
		...
		$basket = \Thurly\Sale\Basket::create($siteId);
		$settableFields = array_flip(\Thurly\Sale\BasketItemBase::getSettableFields());
		...
		if (!empty($oldOrder["ITEMS"]) && is_array($oldOrder["ITEMS"]))
		{
			foreach($oldOrder["ITEMS"] as $oldBasketItem)
			{
				...
				$newBasketItem = \Thurly\Sale\BasketItem::create($basket, $oldBasketItem['MODULE'], $oldBasketItem['PRODUCT_ID']);
	...
	public static function createNoDeliveryServiceAgent()
	{
		...
		if ($id <= 0)
		{
			Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/lib/delivery/helper.php', 'ru');

			$fields = array();
			$fields["NAME"] = \Thurly\Main\Localization\Loc::getMessage('SALE_DELIVERY_HELPER_NO_DELIVERY_SERVICE');
			...
			$fields = array(
				'SORT' => 100,
				'SERVICE_ID' => $id,
				'SERVICE_TYPE' => \Thurly\Sale\Delivery\Restrictions\Manager::SERVICE_TYPE_SHIPMENT,
				'PARAMS' => array(
					'PUBLIC_SHOW' => 'N'
				)
			);
	...
	public static function getIdByCode($code)
	{
		if(CSaleDeliveryHandler::isSidNew($code))
			...
			$id = \Thurly\Sale\Delivery\Services\Manager::getIdByCode($code);
	...
	public static function getCodeById($id)
	{
		...
		$code = \Thurly\Sale\Delivery\Services\Manager::getCodeById($id);
