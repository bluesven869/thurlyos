...
class CAllSaleAffiliate
{
	...
	function GetAffiliate($affiliateID = 0)
	{
		...
		if ($affiliateID <= 0)
		{
			$cookieName = COption::GetOptionString("main", "cookie_name", "THURLY_SM");
	...
	function SetAffiliatePlan($affiliate, $dateFrom = false, $dateTo = false)
	{
		...
		{
			$dbOrders = \Thurly\Sale\Internals\OrderTable::getList(
				array(
					'filter' => array(
						"=ALLOW_DELIVERY" => "Y",
					...
					'runtime' => array(
						new \Thurly\Main\Entity\ExpressionField('BASKET_QUANTITY', 'SUM(%s)', array('BASKET.QUANTITY'))
					),
		...
		else
		{
			$dbOrders = \Thurly\Sale\Internals\OrderTable::getList(
				array(
					...
					'runtime' => array(
						new \Thurly\Main\Entity\ExpressionField('ORDER_SUM_PRICE', 'SUM(%s)', array('PRICE'))
					),
			...
			$dbOrders = \Thurly\Sale\Internals\OrderTable::getList(
				array(
					...
					'runtime' => array(
						new \Thurly\Main\Entity\ExpressionField('ORDER_PRICE_DELIVERY', 'SUM(%s)', array('PRICE_DELIVERY'))
					),
			...
			$dbOrders = \Thurly\Sale\Internals\OrderTable::getList(
				array(
					...
					'runtime' => array(
						new \Thurly\Main\Entity\ExpressionField('ORDER_TAX_VALUE', 'SUM(%s)', array('TAX_VALUE'))
					),
		...
		while ($arOrder = $dbOrders->fetch())
		{
			...
			if ($realRateType == "P")
			{
				if ($arOrder["CURRENCY"] != $affiliateCurrency)
				{
					...
					$affiliateSum += \Thurly\Sale\PriceMaths::roundPrecision((($arOrder["BASKET_PRICE"] * $arOrder["BASKET_QUANTITY"]) * $GLOBALS["SALE_CONVERT_CURRENCY_CACHE"][$arOrder["CURRENCY"]."-".$affiliateCurrency] * $realRate) / 100);
				}
				else
				{
					$affiliateSum += \Thurly\Sale\PriceMaths::roundPrecision((($arOrder["BASKET_PRICE"] * $arOrder["BASKET_QUANTITY"]) * $realRate) / 100);
				}
			}
			else
			{
