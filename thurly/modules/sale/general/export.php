...
use Thurly\Sale\BusinessValueConsumer1C;
...
class CSaleExport
{
	...
	public static function getCashBoxOneCId()
    {
        ...
        {
			$cashBoxOneCId = \Thurly\Sale\Cashbox\Cashbox1C::getId();
        }
	...
	{
		$delivery = array();
		$dbDeliveryList = \Thurly\Sale\Delivery\Services\Table::GetList();
		while($service = $dbDeliveryList->fetch())
		{
		    $deliveryObj = Thurly\Sale\Delivery\Services\Manager::createObject($service);
			...
			elseif ($arOrderPropVals["PROP_TYPE"] == "LOCATION")
			{
				...
				if(CSaleLocation::isLocationProEnabled())
				{
					if(intval($arVal['ID']))
					{
						try
						{
							$res = \Thurly\Sale\Location\LocationTable::getPathToNode($arVal['ID'], array('select' => array('LNAME' => 'NAME.NAME', 'TYPE_ID'), 'filter' => array('=NAME.LANGUAGE_ID' => LANGUAGE_ID)));
							$types = \Thurly\Sale\Location\Admin\TypeHelper::getTypeCodeIdMapCached();
							$path = array();
	...
	static function prepareSalePropertyRekv(\Thurly\Sale\IBusinessValueProvider $entity, $agentParams, $arProp, $locationStreetPropertyValue = '')
	{
	    if(!($entity instanceof \Thurly\Sale\Order))
        {
			/** @var \Thurly\Sale\PaymentCollection|\Thurly\Sale\ShipmentCollection $collection */
					...
					else
					{
					    switch($v["TYPE"])
					    {
					        ...
					        default:
                                ...
                                {
                                    $agent["REKV"][$k] = Thurly\Sale\BusinessValue::getValueFromProvider($providerInstance, $codeKey, BusinessValueConsumer1C::CONSUMER_KEY);
                                }
					    }
					}
				}
                        ...
                        switch($v["TYPE"])
                        {
                            ...
                            default:
                                if (($codeKey = $systemCodes1C[$k])
                                    ...
                                    && is_set($providerInstance))
                                {
                                    $agent[$k] = Thurly\Sale\BusinessValue::getValueFromProvider($providerInstance, $codeKey, BusinessValueConsumer1C::CONSUMER_KEY);
                                }
                        }
					}
	 ...
	 * @return array
	 */
	function getSaleProperties(Thurly\Sale\Order $order, $arOrder, $agentParams, $bExportFromCrm, $bCrmModuleIncluded, $paySystems, $delivery, array $options = array())
	{
	...
	static function getPayment($arOrder)
	{
		...
		{
			$innerPS = \Thurly\Sale\PaySystem\Manager::getInnerPaySystemId();
			$limit = 1;
		}

		$resPayment = \Thurly\Sale\Internals\PaymentTable::getList($PaymentParam);
	...
	static function getShipment($arOrder)
	{
		...
		$resShipment = \Thurly\Sale\Internals\ShipmentTable::getList($ShipmentParams);
...
<<?=CSaleExport::getTagName("SALE_EXPORT_COM_INFORMATION")?> <?=self::getCmrXmlRootNameParams()?>><?
        ...
        $dbOrderList = \Thurly\Sale\Internals\OrderTable::getList($filter);
   ...
   static function getShipmentsStoreList(Thurly\Sale\Order $order)
    {
        $result = array();
	...
	static function getXmlBasketItems($type, $arOrder, $arFilter, $arSelect=array(), $arShipment=array())
	{
		...
		$dbBasket = \Thurly\Sale\Internals\BasketTable::getList(array(
			'select' => $select,
			'filter' => $arFilter,
		...
		{
			if(strval($arBasket['TYPE'])!='' && $arBasket['TYPE']== \Thurly\Sale\BasketItem::TYPE_SET)
			    continue;
	...
	static function OutputXmlDocumentsByType($typeDocument, $xmlResult, $arOrder, $documents, \Thurly\Sale\Order $order=null, $agentParams, $arProp, $locationStreetPropertyValue)
	{
	...
	<<?=CSaleExport::getTagName("SALE_EXPORT_DOCUMENT")?>><?
		...
		<<?=CSaleExport::getTagName("SALE_EXPORT_HOZ_OPERATION")?>><?=CSaleExport::getTagName("SALE_EXPORT_ITEM_PAYMENT_".\Thurly\Sale\PaySystem\Manager::getPsType($document['PAY_SYSTEM_ID']))?></<?=CSaleExport::getTagName("SALE_EXPORT_HOZ_OPERATION")?>>
		<?		break;
				case 'Shipment':?>
		...
		<?	switch($typeDocument)
			{
				case 'Payment':
                    ...
                    {
                        $checks = \Thurly\Sale\Cashbox\CheckManager::getPrintableChecks(array($cashBoxOneCId), array($document['ORDER_ID']));
	...
	static function ExportContragents($arOrder = array(), $arProp = array(), $agent = array(), $arOptions = array())
	{
		...
		if ($bExportFromCrm): ?>
                ...
                <?else:
                    ...
                    \Thurly\Sale\Exchange\Entity\UserImportBase::updateEmptyXmlId($arOrder["USER_ID"], $xmlId);
                ?>
    ...
    public static function getFormatDate($value)
    {
        ...
        /** @var \Thurly\Main\Type\DateTime $time */
        $time = new Thurly\Main\Type\DateTime($setValue);
        if(self::getVersionSchema() >= self::PARTIAL_VERSION )
            $format = 'Y-m-d\TH:i:s';
        else
            $format = 'd.m.Y H:i:s';
	...
	/**
	 * @param \Thurly\Sale\IBusinessValueProvider $entity
	 * @return array
	 */
	static protected function getProvidersInstanceByEntity(\Thurly\Sale\IBusinessValueProvider $entity)
	{
	 ...
	 * @return array
	 */
	protected function getProvidersInstanceByOrder(Thurly\Sale\Order $order)
    {
    ...
    static protected function getProviderInstanceByProviderCode(\Thurly\Sale\IBusinessValueProvider $entity, $providerCode)
	{
	...
	/** @deprecated */
	private static function logError($itemId, $message, Thurly\Main\Result $result = null)
	{
		if ($result)
	...
	public static function migrateToBusinessValues()
	{
		...
		Thurly\Main\Application::getConnection()->query('DELETE FROM b_sale_bizval_code_1c');

		$result = Thurly\Main\Application::getConnection()->query('SELECT * FROM b_sale_export');
		...
		while ($row = $result->fetch())
		{
			...
			{
				$r = Thurly\Sale\Internals\BusinessValuePersonDomainTable::add(array(
					'PERSON_TYPE_ID' => $personTypeId,
					'DOMAIN'         => $domain,
				));
    ...
    {
        $r = new \Thurly\Main\Result();

        $res = \Thurly\Sale\Internals\BusinessValueCode1CTable::getList(array(
                'select'=>array('CODE_INDEX', 'PERSON_TYPE_ID'),
                'filter'=>array('PERSON_TYPE_ID'=>$typeId)
        ));
        while($row=$res->fetch())
        {
			$r = \Thurly\Sale\Internals\BusinessValueCode1CTable::delete(array(
			...
			if($r->isSuccess())
            {
                $r = \Thurly\Sale\Internals\BusinessValueTable::delete(array(
	...
	public static function getTagName($name)
	{
		...
		{
			$lang[static::$lid] = \Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/sale/general/export.php', static::$lid);
		}
		...
		{
			$value = \Thurly\Main\Localization\Loc::getMessage($name);
		}
