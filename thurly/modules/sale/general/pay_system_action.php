<?
use \Thurly\Sale\Internals\PaySystemActionTable;
use \Thurly\Sale\Internals\ServiceRestrictionTable;
use \Thurly\Sale\Services\PaySystem\Restrictions\Manager;
...
class CAllSalePaySystemAction
{
	...
	function GetByID($id)
	{
		...
		$dbRes = \Thurly\Sale\Internals\PaySystemActionTable::getById($id);
		if ($res = $dbRes->fetch())
			return $res;
	...
	function Delete($id)
	{
		...
		$result = \Thurly\Sale\Internals\PaySystemActionTable::delete($id);
	...
	function GetParamValue($key, $defaultValue = null)
	{
		...
		{
			throw new \Thurly\Main\SystemException('SALE_CORRESPONDENCE or SALE_INPUT_PARAMS were defined in superglobal variable!');
		}
		...
		if(!array_key_exists($key, $GLOBALS["SALE_CORRESPONDENCE"]))
		{
			...
			throw new \Thurly\Main\SystemException($message, self::GET_PARAM_VALUE);
		}
	...
	function InitParamArrays($arOrder, $orderID = 0, $psParams = "", $relatedData = array(), $payment = array())
	{
		...
		{
			$payment = \Thurly\Sale\Internals\PaymentTable::getRow(
				array(
					'select' => array('*'),
					'filter' => array('ORDER_ID' => $orderID, '!PAY_SYSTEM_ID' => \Thurly\Sale\PaySystem\Manager::getInnerPaySystemId())
				)
			);
		}
		...
		$shipment = \Thurly\Sale\Internals\ShipmentTable::getRow(
			array(
		...
		if ($payment['COMPANY_ID'] > 0)
		{
			...
			$userFieldsList = $USER_FIELD_MANAGER->GetUserFields(\Thurly\Sale\Internals\CompanyTable::getUfId(), null, LANGUAGE_ID);
			foreach ($userFieldsList as $key => $userField)
			{
				$value = $USER_FIELD_MANAGER->GetUserFieldValue(\Thurly\Sale\Internals\CompanyTable::getUfId(), $key, $payment['COMPANY_ID']);
				$GLOBALS["SALE_INPUT_PARAMS"]["COMPANY"][$key] = $value;
				$GLOBALS["SALE_INPUT_PARAMS"]["COMPANY"]["~".$key] = $value;
			}

			$companyFieldList = \Thurly\Sale\Internals\CompanyTable::getRowById($payment['COMPANY_ID']);
	...
	{
		if (\Thurly\Main\Config\Option::get('main', '~sale_paysystem_converted') == 'Y')
		{
			...
			$dbRes = \Thurly\Sale\Internals\PaySystemActionTable::getList(array(
					'select' => $select,
					'filter' => $filter,
					'group' => $groupBy
			));
	...
	{
		if (isset($filter['PERSON_TYPE_ID']) && $restriction['CLASS_NAME'] == '\Thurly\Sale\Services\PaySystem\Restrictions\PersonType')
		{
	...
	{
		$consumers = \Thurly\Sale\BusinessValue::getConsumers();
		$params = array();
		...
		if (is_array($consumers[$consumer]['CODES']) && $consumers[$consumer]['CODES'])
		{
			...
			{
				$map = \Thurly\Sale\BusinessValue::getMapping($key, $consumer, $personTypeId);
				if ($map)
				{
				...
				if ($fields['PARAMS'])
				{
					...
					if (!isset($params['BX_PAY_SYSTEM_ID']))
					{
						...
						if (!isset($consumers['PAYSYSTEM_'.$result->getId()]))
							\Thurly\Sale\BusinessValue::addConsumer('PAYSYSTEM_'.$result->getId(), \Thurly\Sale\PaySystem\Manager::getHandlerDescription($fields['ACTION_FILE']));
						else
							\Thurly\Sale\BusinessValue::changeConsumer('PAYSYSTEM_'.$result->getId(), \Thurly\Sale\PaySystem\Manager::getHandlerDescription($fields['ACTION_FILE']));
					}
					$params = self::prepareParamsForBusVal($result->getId(), $fields);
					foreach ($params as $item)
						\Thurly\Sale\BusinessValue::setMapping($item['CODE'], $item['CONSUMER'], $item['PERSON_TYPE_ID'], $item['MAP']);
				}
		...
		if (\Thurly\Main\Config\Option::get('main', '~sale_paysystem_converted') == 'Y')
		{
			...
			if ($result->isSuccess())
			{
				...
				if ($fields['PERSON_TYPE_ID'])
				{
					...
					\Thurly\Sale\Services\PaySystem\Restrictions\PersonType::save($fields, $restrictionId);
				}
	...
	public static function convertPsBusVal()
	{
		...
		\Thurly\Main\Config\Option::set('main', '~sale_paysystem_converted', 'Y');
		...
		$dbRes = \Thurly\Sale\Internals\PaySystemActionTable::getList();
		...
		foreach ($generalBusVal as $code => $param)
		{
			...
			\Thurly\Sale\BusinessValue::setMapping($code, null, null, array('PROVIDER_KEY' => $type, 'PROVIDER_VALUE' => $value));
		}
			...
			foreach ($actions as $items)
			{
				...
				foreach ($items as $item)
				{
					...
					if ($item['PARAMS'])
					{
						...
						if ($itemParams)
						{
							foreach ($itemParams as $code => $param)
							{
								...
								{
									if (\Thurly\Sale\BusinessValue::isSetMapping($code, $case['PS'], $case['PT']))
									{
								...
								\Thurly\Sale\BusinessValue::setMapping($code, $pS, $pT, array('PROVIDER_KEY' => $type, 'PROVIDER_VALUE' => $value), true);
							}
						}
					}
					...
					if (!isset($mustDeleted[$firstItem['ID']][$item['ID']]))
					{
						...
						$itemId = $item['ID'];
						unset($item['ID']);
						\Thurly\Sale\Internals\PaySystemActionTable::update($itemId, $item);
					}
				}
			}
		}
...
//		\Thurly\Main\Config\Option::set('main', '~sale_paysystem_converted', 'Y');
		...
		$DB->Query('
			...
			SELECT bsd2p.PAYSYSTEM_ID, 100, \'Thurly\\\\Sale\\\\Services\\\\PaySystem\\\\Restrictions\\\\Delivery\', \'a:0:{}\', 1 FROM b_sale_delivery2paysystem bsd2p GROUP BY bsd2p.PAYSYSTEM_ID'
		);
	...
	{
		return array(
			'/thurly/modules/sale/payment/yandex_3x' => 'yandex',
			'/thurly/modules/sale/payment/webmoney_web' => 'webmoney',
			'/thurly/modules/sale/payment/assist' => 'assist',
			'/thurly/modules/sale/payment/qiwi' => 'qiwi',
			'/thurly/modules/sale/payment/paymaster' => 'paymaster',
			'/thurly/modules/sale/payment/paypal' => 'paypal',
			'/thurly/modules/sale/payment/roboxchange' => 'roboxchange',
			'/thurly/modules/sale/payment/sberbank_new' => 'sberbank',
			'/thurly/modules/sale/payment/bill' => 'bill',
			'/thurly/modules/sale/payment/bill_en' => 'billen',
			'/thurly/modules/sale/payment/bill_de' => 'billde',
			'/thurly/modules/sale/payment/bill_ua' => 'billua',
			'/thurly/modules/sale/payment/bill_la' => 'billla',
			'/thurly/modules/sale/payment/liqpay' => 'liqpay',
			'/thurly/modules/sale/payment/payment_forward_calc' => 'cashondeliverycalc',
			'/thurly/modules/sale/payment/payment_forward' => 'cashondelivery',
			'/thurly/modules/sale/payment/cash' => 'cash',
			'INNER_BUDGET' => 'inner'
		);
	}
