...
class CAllSaleOrderUserPropsValue
{
	...
	{
		$tableAlias = \Thurly\Main\HttpApplication::getConnection()->getSqlHelper()->forSql($tableAlias);
		$propTableAlias = \Thurly\Main\HttpApplication::getConnection()->getSqlHelper()->forSql($propTableAlias);
		...
		if(CSaleLocation::isLocationProMigrated())
			return "
				CASE
					...
					THEN
						CAST(L.ID as ".\Thurly\Sale\Location\DB\Helper::getSqlForDataType('char', 255).")
...
//	{
//		$query = new \Thurly\Sale\Compatible\OrderQueryLocation(Thurly\Sale\Internals\UserPropsValueTable::getEntity());
