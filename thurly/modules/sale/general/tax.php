...
class CAllSaleTax
{
	...
	public static function calculateTax(&$arOrder, $arOptions, &$arErrors)
	{
				...
				else
				{
					...
					while ($arTaxRate = $dbTaxRate->GetNext())
					{
						if (!in_array(intval($arTaxRate["TAX_ID"]), $arOrder["TAX_EXEMPT"]))
						{
							...
							{
								$arTaxRate["VALUE"] = \Thurly\Sale\PriceMaths::roundPrecision(CCurrencyRates::ConvertCurrency($arTaxRate["VALUE"], $arTaxRate["CURRENCY"], $arOrder["CURRENCY"]));
		...
		$arOrder["TAX_PRICE"] = \Thurly\Sale\PriceMaths::roundPrecision($arOrder["TAX_PRICE"]);
	}
	...
	public static function calculateDeliveryTax(&$arOrder, $arOptions, &$arErrors)
	{
		...
		if (!$arOrder["USE_VAT"] || $arOrder["USE_VAT"] != "Y")
		{
			...
			if (!array_key_exists("TAX_LIST", $arOrder))
			{
				...
				while ($arTaxRate = $dbTaxRate->GetNext())
				{
					if (is_array($arOrder["TAX_EXEMPT"]) && !in_array(intval($arTaxRate["TAX_ID"]), $arOrder["TAX_EXEMPT"]))
					{
						if ($arTaxRate["IS_PERCENT"] != "Y")
						{
							$arTaxRate["VALUE"] = \Thurly\Sale\PriceMaths::roundPrecision(CurrencyRates::ConvertCurrency($arTaxRate["VALUE"], $arTaxRate["CURRENCY"], $arOrder["CURRENCY"]));
							$arTaxRate["CURRENCY"] = $arOrder["CURRENCY"];
						}
						$arOrder["TAX_LIST"][] = $arTaxRate;
					}
			...
			if (count($arOrder["TAX_LIST"]) > 0)
			{
				...
				foreach ($arOrder["TAX_LIST"] as &$arTax)
				{
					$arTax["VALUE_MONEY"] += \Thurly\Sale\PriceMaths::roundPrecision($arTax["TAX_VAL"]);
					$arTax['VALUE_MONEY_FORMATED'] = SaleFormatCurrency($arTax["VALUE_MONEY"], $arOrder["CURRENCY"]);
		...
		$arOrder["TAX_PRICE"] = \Thurly\Sale\PriceMaths::roundPrecision($arOrder["TAX_PRICE"]);
		$arOrder["VAT_SUM"] = \Thurly\Sale\PriceMaths::roundPrecision($arOrder["VAT_SUM"]);
		$arOrder["VAT_DELIVERY"] = \Thurly\Sale\PriceMaths::roundPrecision($arOrder["VAT_DELIVERY"]);
	...
	static function DoSaveOrderTax($orderId, $taxList, &$arErrors)
	{
		...
		if (is_array($taxList))
		{
			foreach ($taxList as $itemData)
			{
				...
				if ($orderId > 0)
				{
					\Thurly\Sale\OrderHistory::addLog('TAX', $orderId, $isNew ? 'TAX_ADD' : 'TAX_UPDATE', $taxId, null, array(
						"NAME" => $itemData["NAME"],
						"CODE" => $itemData["CODE"]
					), \Thurly\Sale\OrderHistory::SALE_ORDER_HISTORY_LOG_LEVEL_1);
		...
		foreach ($idList as $code => $id)
		{
			...
			if ($orderId > 0)
			{
				\Thurly\Sale\OrderHistory::addLog('TAX', $orderId, 'TAX_DELETED', $id, null, array(), \Thurly\Sale\OrderHistory::SALE_ORDER_HISTORY_LOG_LEVEL_1);
			}
		}
		...
		if (!empty($duplicateList))
		{
			...
			{
				CSaleOrderTax::Delete($id);
				\Thurly\Sale\OrderHistory::addLog('TAX', $orderId, 'TAX_DUPLICATE_DELETED', $id, null, array(), \Thurly\Sale\OrderHistory::SALE_ORDER_HISTORY_LOG_LEVEL_1);
			}
		}

		if ($isChanged)
		{
			\Thurly\Sale\OrderHistory::addAction(
				'TAX',
				$orderId,
				"TAX_SAVED"
			);
