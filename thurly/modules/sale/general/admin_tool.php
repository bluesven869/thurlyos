...
function fGetUserName($USER_ID)
{
	...
	if (intval($USER_ID) > 0)
	{
		...
		if (count($arUser) > 1)
		{
			$user = "<a href='javascript:void(0);' onClick=\"window.open('/thurly/admin/user_search.php?lang=".LANGUAGE_ID."&FN=order_edit_info_form&FC=user_id', '', 'scrollbars=yes,resizable=yes,width=840,height=500,top='+Math.floor((screen.height - 840)/2-14)+',left='+Math.floor((screen.width - 760)/2-5));\">";
			...
			foreach ($arProducts as $arProduct)
			{
				...
				if (!$crmMode)
				{
					...
					else
					{
						...
						$url = "/thurly/admin/sale_order_new.php?lang=".LANGUAGE_ID."&user_id=".$USER_ID."&LID=".$LID."&product[".$arProduct["PRODUCT_ID"]."]=".$cntProd;
			...
			while($arProduct = $dbProduct->Fetch())
			{
				...
				if (!$crmMode)
				{
					...
					else
					{
						...
						$url = "/thurly/admin/sale_order_new.php?lang=".LANGUAGE_ID."&user_id=".$USER_ID."&LID=".$LID."&product[".$item["PRODUCT_ID"]."]=".$cntProd;
...
function showImageOrDownloadLink($fileId, $orderId = 0, $arSize = array("WIDTH" => 90, "HEIGHT" => 90))
{
	...
	if ($arFile)
	{
		...
		if ($is_image)
			...
			$resultHTML = "<a href=\"sale_order_detail.php?ID=".$orderId."&download=Y&file_id=".$arFile["ID"]."&".thurly_sessid_get()."\">".$arFile["ORIGINAL_NAME"]."</a>";
	}
