...
Thurly\Main\Loader::registerAutoLoadClasses('sale', array(
	"\\Thurly\\Sale\\Delivery\\Services\\Automatic" => "lib/delivery/services/automatic.php",
	"\\Thurly\\Sale\\Delivery\\Services\\NewToAutomatic" => "lib/delivery/services/new_to_automatic.php",
));
...
class CAllSaleDeliveryHandler
{
	...
	{
		\Thurly\Sale\Delivery\Services\Automatic::initHandlers();
	}
	...
	{
		return \Thurly\Sale\Delivery\Services\Automatic::getRegisteredHandlers("SID");
	}
	...
	{
		$result = array_intersect_key($oldFilter, Thurly\Sale\Delivery\Services\Table::getMap());

		$result[] = array(
			'LOGIC' => 'OR',
			'=CLASS_NAME' => array(
				'\\Thurly\\Sale\\Delivery\\Services\\Automatic',
			...
			array(
				...
				'!=CLASS_NAME' => array(
					...
					'\\Thurly\\Sale\\Delivery\\Services\\Configurable',
					'\\Thurly\\Sale\\Delivery\\Services\\EmptyDeliveryService'
				)
		));
	...
	protected static function checkRestrictionFilter(array $restriction, array $filter)
	{
		...
		{
			case '\Thurly\Sale\Delivery\Restrictions\BySite':
			...
			case '\Thurly\Sale\Delivery\Restrictions\ByWeight':
			...
			case '\Thurly\Sale\Delivery\Restrictions\ByPrice':
			...
			case '\Thurly\Sale\Delivery\Restrictions\ByLocation':
			case '\Thurly\Sale\Delivery\Restrictions\ByPaySystem':
			default:
				break;
		}
	...
	public static function GetList($arSort = array("SORT" => "ASC"), $arFilter = array())
	{
		...
		$params = array(
			'order' => array_intersect_key($arSort, Thurly\Sale\Delivery\Services\Table::getMap()),
			'filter' => self::convertFilterOldToNew($arFilter));

		$services = array();

		$dbRes = \Thurly\Sale\Delivery\Services\Table::getList($params);

		while($service = $dbRes->fetch())
		{
			$dbRstrRes = \Thurly\Sale\Internals\ServiceRestrictionTable::getList(array(
				'filter' => array(
			...
			while($restr = $dbRstrRes->fetch())
			{
				...
				if($restr["CLASS_NAME"] == '\Thurly\Sale\Delivery\Restrictions\BySite' && !empty($restr["PARAMS"]["SITE_ID"]))
				{
			...
			{
				$srv = \Thurly\Sale\Delivery\Services\Automatic::convertNewServiceToOld($service);
			}
			else
			{
				\Thurly\Sale\Delivery\Services\Manager::getHandlersList();

				if(get_parent_class($service['CLASS_NAME']) == 'Thurly\Sale\Delivery\Services\Base')
					if($service['CLASS_NAME']::canHasProfiles())
						continue;

				$srv = \Thurly\Sale\Delivery\Services\NewToAutomatic::convertNewServiceToOld($service);
			}
	...
	{
		$res = \Thurly\Sale\Delivery\Services\Table::getList(array(
			'filter' => array(
				'CODE' => $sid,
				'=CLASS_NAME' => '\Thurly\Sale\Delivery\Services\Automatic'
			)
		));

		while($handler = $res->fetch())
		{
			$rstrRes = \Thurly\Sale\Internals\ServiceRestrictionTable::getList(array(
				'filter' =>array(
					"=SERVICE_ID" => $handler["ID"],
					"=CLASS_NAME" => '\Thurly\Sale\Delivery\Restrictions\BySite',
					"=SERVICE_TYPE" => \Thurly\Sale\Services\Base\RestrictionManager::SERVICE_TYPE_SHIPMENT,
				)
			));
	...
	protected static function saveRestrictionBySiteId($deliveryId, $siteId, $update)
	{
		...
		if($update)
		{
			...
			if($restrict = $rstrRes->fetch())
				$rres = \Thurly\Sale\Internals\ServiceRestrictionTable::update($restrict["ID"], $rfields);
			else
				$rres = \Thurly\Sale\Internals\ServiceRestrictionTable::add($rfields);
		}
		else
		{
			$rres = \Thurly\Sale\Internals\ServiceRestrictionTable::add($rfields);
	...
	protected static function saveRestrictionByWeight($deliveryId, array $weightParams, $update)
	{
		$rfields = array(
			"SERVICE_ID" => $deliveryId,
			"CLASS_NAME" => '\Thurly\Sale\Delivery\Restrictions\ByWeight',
			"SERVICE_TYPE" => \Thurly\Sale\Services\Base\RestrictionManager::SERVICE_TYPE_SHIPMENT,
			"PARAMS" => array(
	...
	protected static function saveRestrictionByPublicShow($deliveryId, $publicShow, $update)
	{
		$rfields = array(
			"SERVICE_ID" => $deliveryId,
			"CLASS_NAME" => '\Thurly\Sale\Delivery\Restrictions\ByPublicMode',
			"SERVICE_TYPE" => \Thurly\Sale\Services\Base\RestrictionManager::SERVICE_TYPE_SHIPMENT,
			"PARAMS" => array(
	...
	protected static function saveRestrictionByPrice($deliveryId, array $priceParams, $currency, $update)
	{
		$rfields = array(
			"SERVICE_ID" => $deliveryId,
			"SERVICE_TYPE" => \Thurly\Sale\Services\Base\RestrictionManager::SERVICE_TYPE_SHIPMENT,
			"CLASS_NAME" => '\Thurly\Sale\Delivery\Restrictions\ByPrice',
			"PARAMS" => array(
	...
	protected static function saveRestrictionByDimensions($deliveryId, array $params, $update)
	{
		$rfields = array(
			"SERVICE_ID" => $deliveryId,
			"SERVICE_TYPE" => \Thurly\Sale\Services\Base\RestrictionManager::SERVICE_TYPE_SHIPMENT,
			"CLASS_NAME" => '\Thurly\Sale\Delivery\Restrictions\ByDimensions',
			"PARAMS" => array(
	...
	protected static function saveRestrictionByMaxSize($deliveryId, $maxSize, $update)
	{
		$rfields = array(
			"SERVICE_ID" => $deliveryId,
			"SERVICE_TYPE" => \Thurly\Sale\Services\Base\RestrictionManager::SERVICE_TYPE_SHIPMENT,
			"CLASS_NAME" => '\Thurly\Sale\Delivery\Restrictions\ByMaxSize',
			"PARAMS" => array(
				"MAX_SIZE" => $maxSize,
			)
		);
	...
	public static function Set($code, $arData, $siteId = false)
	{
		...
		$update = intval($id) > 0;
		$fields = array_intersect_key($arData, Thurly\Sale\Delivery\Services\Table::getMap());
		...
		if(!$update) //add new
		{
			...
			if(!isset($arData["CLASS_NAME"]))
				$fields["CLASS_NAME"] = '\Thurly\Sale\Delivery\Services\Automatic';
			else
		...
		if($update)
			$res = \Thurly\Sale\Delivery\Services\Manager::update($id, $fields);
		else
			$res = \Thurly\Sale\Delivery\Services\Manager::add($fields);

		if(!$res->isSuccess())
		{
			throw new \Thurly\Main\SystemException(implode("\n", $res->getErrorMessages()));
		}
			...
			foreach($arData["PROFILES"] as $profileCode => $profileData)
			{
				...
				self::Set($code.":".$profileCode,
					array(
						...
						"CLASS_NAME" => '\Thurly\Sale\Delivery\Services\AutomaticProfile',
		...
		elseif($update)
		{
			\Thurly\Sale\Delivery\Restrictions\Manager::deleteByDeliveryIdClassName($id, '\Thurly\Sale\Delivery\Restrictions\BySite');
		}
		...
		elseif($update)
		{
			\Thurly\Sale\Delivery\Restrictions\Manager::deleteByDeliveryIdClassName($id, '\Thurly\Sale\Delivery\Restrictions\ByWeight');
		}
		...
		elseif($update)
		{
			\Thurly\Sale\Delivery\Restrictions\Manager::deleteByDeliveryIdClassName($id, '\Thurly\Sale\Delivery\Restrictions\ByPrice');
		}
		...
		elseif($update)
		{
			\Thurly\Sale\Delivery\Restrictions\Manager::deleteByDeliveryIdClassName($id, '\Thurly\Sale\Delivery\Restrictions\ByDimensions');
		}
		...
		elseif($update)
		{
			\Thurly\Sale\Delivery\Restrictions\Manager::deleteByDeliveryIdClassName($id, '\Thurly\Sale\Delivery\Restrictions\ByMaxSize');
		}
	...
	public static function Reset($sid)
	{
		...
		{
			while($service = $dbRes->fetch())
				\Thurly\Sale\Delivery\Services\Manager::delete($service["ID"]);
		}
	...
	{
		$serviceRes = \Thurly\Sale\Delivery\Services\Table::getList(
			array(
				'filter' => array("=CLASS_NAME" => '\Thurly\Sale\Delivery\Services\Automatic'),
				'select' => array("CODE")
			));
	...
	public static function execOldEventWithNewParams(Thurly\Main\Event $params)
	{
		/** @var \Thurly\Sale\Shipment $shipment*/
		if(!$shipment = $params->getParameter("SHIPMENT"))
			return new \Thurly\Main\EventResult(\Thurly\Main\EventResult::ERROR, null, 'sale');
		...
		/** @var \Thurly\Sale\Delivery\Services\Base $deliverySrv */
		if(!$deliverySrv = \Thurly\Sale\Delivery\Services\Manager::getObjectById($deliveryId))
			return new \Thurly\Main\EventResult(\Thurly\Main\EventResult::ERROR, null, 'sale');

		if(get_class($deliverySrv) != 'Thurly\Sale\Delivery\Services\AutomaticProfile')
			return new \Thurly\Main\EventResult(\Thurly\Main\EventResult::ERROR, null, 'sale');
		...
		/** @var \Thurly\Sale\Delivery\CalculationResult $result*/
		if(!$result = $params->getParameter("RESULT"))
			throw new \Thurly\Main\ArgumentNullException("params[RESULT]");
		...
		$oldOrder = \Thurly\Sale\Compatible\OrderCompatibility::convertOrderToArray($order);
		...
		if($oldResult["RESULT"] == "ERROR")
		{
			if($oldResult["TEXT"] != $errorMessage)
				$result->addError(new \Thurly\Main\Entity\EntityError($oldResult["TEXT"]));
		}
	...
	public static function convertToNew($renameTable = false)
	{
		...
		while($delivery = $deliveryRes->fetch())
		{
			...
			{
				//$result->addError( new \Thurly\Main\Entity\EntityError("Can't find delivery HID. ID: \"".$delivery["ID"]."\""));
				continue;
			}
			...
			if(!isset($handlers[$delivery["HID"]]))
			{
				...
				//$result->addError( new \Thurly\Main\Entity\EntityError("Can't find delivery handler for registered HID: \"".$delivery["HID"]."\""));
				continue;
			}
			...
			{
				//$result->addError( new \Thurly\Main\Entity\EntityError("Can't receive info about profiles. Delivery HID: \"".$delivery["HID"]."\""));
			...
			{
				$result->addError(
					new \Thurly\Main\Entity\EntityError(
			...
			foreach($delivery["PROFILES"] as $profileName => $profileData)
			{
				...
				{
					$result->addError( new \Thurly\Main\Entity\EntityError("Cant determine id for profile code: ".$fullSid));
				}
			}
			...
			$d2pRes = \Thurly\Sale\Internals\DeliveryPaySystemTable::getList(array(
				'filter' => array(
					'DELIVERY_ID' => $ids
				),
			...
			{
				$res = \Thurly\Sale\Internals\ServiceRestrictionTable::add(array(
					"SERVICE_ID" => $d2p["DELIVERY_ID"],
					"SERVICE_TYPE" => \Thurly\Sale\Services\Base\RestrictionManager::SERVICE_TYPE_SHIPMENT,
					"CLASS_NAME" => '\Thurly\Sale\Delivery\Restrictions\ByPaySystem',
					"SORT" => 100
				));
	...
	{
		\Thurly\Sale\Delivery\Services\Manager::getHandlersList();
		$initedHandlersH = \Thurly\Sale\Delivery\Services\Automatic::getRegisteredHandlers("HANDLER");
		$initedHandlersS = \Thurly\Sale\Delivery\Services\Automatic::getRegisteredHandlers("SID");
		$filter = array('=CLASS_NAME' => '\Thurly\Sale\Delivery\Services\Automatic');
		...
		while($params = $res->fetch())
		{
			...
			\Thurly\Sale\Delivery\Services\Manager::update($params["ID"], array("CONFIG" => $config));
		}
