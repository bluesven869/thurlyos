...
use
	Thurly\Sale\Internals\OrderPropsTable,
	Thurly\Sale\Compatible\OrderQuery,
	Thurly\Sale\Compatible\FetchAdapter,
	Thurly\Main\Entity,
...
class CSaleOrderProps
{
	...
	function GetList($arOrder = array(), $arFilter = array(), $arGroupBy = false, $arNavStartParams = false, $arSelectFields = array())
	{
		...
		$query = new \Thurly\Sale\Compatible\OrderQueryLocation(OrderPropsTable::getEntity());
		...
		$query->addAliases(array(
			...
			'PAYSYSTEM_ID'         => 'Thurly\Sale\Internals\OrderPropsRelationTable:lPROPERTY.PROPERTY_ID',
			'DELIVERY_ID'          => 'Thurly\Sale\Internals\OrderPropsRelationTable:lPROPERTY.PROPERTY_ID',
		));
		...
		if (isset($arFilter['RELATED']))
		{
			...
			if (is_array($arFilter['RELATED']))
			{
				...
				if ($arFilter['RELATED']['PAYSYSTEM_ID'])
					$relationFilter []= array(
						'=Thurly\Sale\Internals\OrderPropsRelationTable:lPROPERTY.ENTITY_TYPE' => 'P',
						'=Thurly\Sale\Internals\OrderPropsRelationTable:lPROPERTY.ENTITY_ID' => $arFilter['RELATED']['PAYSYSTEM_ID'],
					);
				...
				if ($arFilter['RELATED']['DELIVERY_ID'])
				{
					...
					$relationFilter []= array(
						'=Thurly\Sale\Internals\OrderPropsRelationTable:lPROPERTY.ENTITY_TYPE' => 'D',
						'=Thurly\Sale\Internals\OrderPropsRelationTable:lPROPERTY.ENTITY_ID' => \CSaleDelivery::getIdByCode($arFilter['RELATED']['DELIVERY_ID']),
					);
				}
				...
				if ($arFilter['RELATED']['TYPE'] == 'WITH_NOT_RELATED' && $relationFilter)
				{
					$relationFilter = array(
						'LOGIC' => 'OR',
						$relationFilter,
						array('=Thurly\Sale\Internals\OrderPropsRelationTable:lPROPERTY.PROPERTY_ID' => null),
					);
				}
			...
			{
				$query->addFilter('=Thurly\Sale\Internals\OrderPropsRelationTable:lPROPERTY.PROPERTY_ID', null);
		...
		{
			$result = new \Thurly\Sale\Compatible\CDBResult;
				...
				if(strlen($value))
				{
					...
					if(intval($value))
					{
						try
						{
							...
							$res = \Thurly\Sale\Location\LocationTable::getPathToNode($value, array('select' => array('LNAME' => 'NAME.NAME', 'TYPE_ID'), 'filter' => array('=NAME.LANGUAGE_ID' => LANGUAGE_ID)));
	...
	static function convertOldToNew(array $property)
	{
		...
		switch ($property['TYPE'])
		{
			...
			case 'LOCATION':
				...
				{
					$res = \Thurly\Sale\Location\LocationTable::getList(array('filter' => array('=ID' => intval($property['DEFAULT_VALUE'])), 'select' => array('CODE')))->fetch();
