...
class CSaleDeliveryHelper
{
	...
	public static function getRegionsList($countryId = 0, $bFlip = false)
	{
		...
		{
			$types = array();
			$res = \Thurly\Sale\Location\TypeTable::getList(array(
			...
			$dbRegionList = \Thurly\Sale\Location\LocationTable::getList(array(
				'filter' => $filter,
		...
		if(empty($arResult))
		{
			...
			if($actionId == "REQUEST_SELF" && isset($arResult["TRACKING_NUMBER"]))
			{
				...
				if ($bUserCanEditOrder)
				{
					if($dep)
					{
						\Thurly\Sale\Internals\OrderDeliveryReqTable::update(
							$dep["ID"],
							array(
								"DATE_REQUEST" => $dt
							)
					...
					{
						\Thurly\Sale\Internals\OrderDeliveryReqTable::add(
							array(
								"ORDER_ID" => $orderId,
								"DATE_REQUEST" => $dt
							)
