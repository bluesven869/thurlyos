...
class CAllSaleRecurring
{
	...
	function NextPayment($ID)
	{
		...
		if ($arProduct["WITHOUT_ORDER"] == "Y" || $arRecur["SUCCESS_PAYMENT"] == "Y")
		{
			...
			$productPrice = \Thurly\Sale\PriceMaths::roundPrecision(CCurrencyRates::ConvertCurrency($arProduct["PRICE"], $arProduct["CURRENCY"], $baseSiteCurrency));
			...
			if ($arOrder["DELIVERY_ID"] > 0)
			{
				...
				{
					$deliveryPriceTmp = \Thurly\Sale\PriceMaths::roundPrecision(CCurrencyRates::ConvertCurrency($arDelivery["PRICE"], $arDelivery["CURRENCY"], $baseSiteCurrency));
				...
				if ($deliveryID <= 0)
				{
					$deliveryID = \Thurly\Sale\Delivery\Services\EmptyDeliveryService::getEmptyDeliveryServiceId();
			...
			if ($arDiscount = $dbDiscount->Fetch())
			{
				...
				{
					$discountProduct = \Thurly\Sale\PriceMaths::roundPrecision($productPrice * $arDiscount["DISCOUNT_VALUE"] / 100);
					$discount = \Thurly\Sale\PriceMaths::roundPrecision($discountProduct * DoubleVal($arProduct["QUANTITY"])); // Changed by Sigurd, 2007-08-16
				...
				else
				{
					...
					$discountValue = \Thurly\Sale\PriceMaths::roundPrecision($discountValue);

					$discountProduct = \Thurly\Sale\PriceMaths::roundPrecision(1.0 * $discountValue / DoubleVal($arProduct["QUANTITY"]));// Changed by Sigurd, 2007-08-16
					$discount = \Thurly\Sale\PriceMaths::roundPrecision($curDiscount * DoubleVal($arProduct["QUANTITY"]));
					$discountPrice = $productPrice - $discountProduct;
				}
			}
			...
			if (!empty($arTaxList))
			{
				...
				{
					$arTaxList[0]["VALUE_MONEY"] = \Thurly\Sale\PriceMaths::roundPrecision((($discountPrice / ($vatRate +1)) * $vatRate) * DoubleVal($arProduct["QUANTITY"]));
