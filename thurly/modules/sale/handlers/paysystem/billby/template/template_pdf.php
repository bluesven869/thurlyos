...
if ($params['BILLBY_HEADER'])
{
	...
	if ($dateValue instanceof \Thurly\Main\Type\Date || $dateValue instanceof \Thurly\Main\Type\DateTime)
	{
...
if ($currencyPrecision <= 0)
	$currencyPrecision = 2;
$salePrecision = (int)Thurly\Main\Config\Option::get('sale', 'value_precision', 2);
if ($salePrecision <= 0)
	$salePrecision = 2;
...
if ($params['BILLBY_SIGN_SHOW'] == 'Y')
{
	if ($params['BILLBY_PATH_TO_STAMP'])
	{
		...
		if ($filePath != '' && !$blank && \Thurly\Main\IO\File::isFileExists($filePath))
		{
