...
<html>
<head>
<title><?=Loc::getMessage('SALE_HPS_BILLFR_TITLE')?></title>
...
foreach ($columnList as $column)
{
	...
	{
		$arCols[$column] = array(
			'NAME' => $params['BILLFR_COLUMN_'.$column.'_TITLE'],
...
if ($params['BASKET_ITEMS'])
{
		...
		foreach ($arCols as $columnId => $col)
		{
			...
			switch ($columnId)
			{
				...
				case 'QUANTITY':
					...
					break;
				case 'MEASURE':
					$data = $basketItem["MEASURE_NAME"] ? htmlspecialcharsbx($basketItem["MEASURE_NAME"]) : Loc::getMessage('SALE_HPS_BILLFR_MEASURE');
					break;
				case 'PRICE':
	...
	if ($params['BILLFR_TOTAL_SHOW'] == 'Y')
	{
		...
		if ($params['SUM'] - $sum > $eps)
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = Loc::getMessage('SALE_HPS_BILLFR_SUB_TOTAL').":";
		...
		if (!empty($vats))
		{
			...
			foreach ($vats as $vatRate => $vatSum)
			{
				...
				$arCells[$n][$arColumnKeys[$columnCount-2]] = sprintf(Loc::getMessage('SALE_HPS_BILLFR_TAX')." (%s%%):", roundEx($vatRate * 100, SALE_VALUE_PRECISION));
		...
		if ($params['SUM_PAID'])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = Loc::getMessage('SALE_HPS_BILLFR_SUM_PAID').":";
		...
		if ($params['DISCOUNT_PRICE'])
		{
			...
			$arCells[$n][$arColumnKeys[$columnCount-2]] = Loc::getMessage('SALE_HPS_BILLFR_DISCOUNT').":";
		...
		$arCells[$n][$arColumnKeys[$columnCount-2]] = Loc::getMessage('SALE_HPS_BILLFR_TOTAL').":";
