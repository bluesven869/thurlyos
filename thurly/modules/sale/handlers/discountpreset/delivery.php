...
use Thurly\Sale\Discount\Preset\SelectProductPreset;
...
class Delivery extends SelectProductPreset
{
	 ...
	 * @param $siteId
	 *
	 * @return \Thurly\Sale\Delivery\Services\Base[]
	 */
	protected function getDeliverySystems($siteId)
	{
				...
				'RESTRICTION_BY_SITE' => array(
					...
					'reference' => array(
						...
						'ref.SERVICE_TYPE' => array(
							'?',
							\Thurly\Sale\Delivery\Restrictions\Manager::SERVICE_TYPE_SHIPMENT,
						),
		...
		$result = array();
		$dbResultList = \Thurly\Sale\Delivery\Services\Table::getList($parameters);
		while($service = $dbResultList->fetch())
		{
			...
			$deliveryService = \Thurly\Sale\Delivery\Services\Manager::createObject($service);
