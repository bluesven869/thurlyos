...
use Thurly\Main\Error,
	Thurly\Main\Loader,
	Thurly\Main\IO\File,
	Thurly\Sale\Shipment,
	Thurly\Main\Page\Asset,
	Thurly\Main\Config\Option,
	Thurly\Main\SystemException,
	Thurly\Main\Localization\Loc,
	Thurly\Main\ArgumentNullException,
	Thurly\Sale\Delivery\Services\Base,
	Thurly\Sale\Delivery\ExtraServices,
	Thurly\Sale\Location\ExternalTable,
	Thurly\Sale\Location\LocationTable,
	Thurly\Sale\Delivery\Services\Manager,
	Thurly\Sale\Delivery\ExtraServices\Table,
	Thurly\Sale\Location\Admin\LocationHelper,
	Sale\Handlers\Delivery\Additional\Location,
	Sale\Handlers\Delivery\Additional\RestClient,
	Thurly\Sale\Internals\ServiceRestrictionTable,
...
class AdditionalHandler extends Base
{
	...
	public function __construct(array $initParams)
	{
		...
		if($initParams['CONFIG']['MAIN']['SERVICE_TYPE'] == "RUSPOST")
			$this->setTrackingClass('\Thurly\Sale\Delivery\Tracking\RusPost');
	...
	public function execAdminAction()
	{
		...
		Asset::getInstance()->addJs("/thurly/js/main/core/core.js");
		Asset::getInstance()->addJs("/thurly/js/sale/additional_delivery.js");
		Asset::getInstance()->addString('<link rel="stylesheet" type="text/css" href="/thurly/css/sale/additional_delivery.css">');
		return $result;
	}
	...
	public static function install()
	{
		...
		if(!file_exists($_SERVER["DOCUMENT_ROOT"].'/thurly/css/sale/additional_delivery.css'))
		{
			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/handlers/delivery/additional/install/css",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/css/sale", true, true
			);
			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/handlers/delivery/additional/install/js",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/js/sale", true, true
			);
			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/handlers/delivery/additional/install/tools",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/tools/sale", true, true
			);
		}
		...
		{
			$DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/handlers/delivery/additional/install/db/".$con->getType()."/install.sql");
		}
	...
	public static function unInstall()
	{
		...
		if(file_exists($_SERVER["DOCUMENT_ROOT"].'/thurly/css/sale/additional_delivery.css'))
		{
			DeleteDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/handlers/delivery/additional/install/css",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/css/sale"
			);
			DeleteDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/handlers/delivery/additional/install/js",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/js/sale"
			);
			DeleteDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/handlers/delivery/additional/install/tools",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/tools/sale"
			);
		}
		...
		if(!$con->isTableExists('b_sale_hdale'))
		{
			$DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/sale/handlers/delivery/additional/install/db/".$con->getType()."/uninstall.sql");
		}
	...
	public static function onSaleDeliveryTrackingClassNamesBuildList()
	{
		return new \Thurly\Main\EventResult(
			...
			array(
				'\Sale\Handlers\Delivery\AdditionalTracking' => '/thurly/modules/sale/handlers/delivery/additional/tracking.php'
			),
			'sale'
		);
	}
	...
	public static function onSaleDeliveryExtraServicesClassNamesBuildList()
	{
		return new \Thurly\Main\EventResult(
			\Thurly\Main\EventResult::SUCCESS,
			array(
				'\Sale\Handlers\Delivery\Additional\ExtraServices\Insurance' => '/thurly/modules/sale/handlers/delivery/additional/extra_services/insurance.php',
				'\Sale\Handlers\Delivery\Additional\ExtraServices\Lift' => '/thurly/modules/sale/handlers/delivery/additional/extra_services/lift.php'
			),
			'sale'
		);
	}
	...
	public static function getShipmentParams(Shipment $shipment, $serviceType)
	{
		...
		//Extra services
		$esList = \Thurly\Sale\Delivery\ExtraServices\Manager::getExtraServicesList($shipment->getDeliveryId(), false);
