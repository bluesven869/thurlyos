...
class Calculator
{
	...
	{
		return new \Thurly\Main\Web\HttpClient(array(
			"version" => "1.1",
			"socketTimeout" => 20,
			"streamTimeout" => 20,
			"redirect" => true,
	...
	protected static function sendRequest($request)
	{
		...
		if (!$httpRes && !empty($errors))
		{
			...
			$result->addError(new \Thurly\Main\Error(\Thurly\Main\Localization\Loc::getMessage('SALE_DLV_SRV_SPSR_ERROR_HTTP_PUBLIC')));
