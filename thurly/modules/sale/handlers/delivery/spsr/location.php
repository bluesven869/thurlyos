...
use Thurly\Sale\Location\Comparator\Mapper;
use Thurly\Sale\Location\Comparator\TmpTable;
use Thurly\Sale\Location\Comparator\MapResult;
...
final class Location extends Mapper
{
	...
	const CSV_FILE_PATH = '/thurly/modules/sale/handlers/delivery/spsr/location.csv';
	...
	protected function mapByNames($startId = 0, $timeout = 0)
	{
		...
		\Thurly\Sale\Location\Comparator::setVariants(Replacement::getVariants());
		...
		switch($stage)
		{
			case 'start':
				...
				$res = \Thurly\Sale\Location\LocationTable::getList(array(
					'runtime' => array(new \Thurly\Main\Entity\ExpressionField('MAX', 'MAX(ID)')),
					'select' => array('MAX')
				));
