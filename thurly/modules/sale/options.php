...
use Thurly\Sale\Cashbox;
use Thurly\Sale\SalesZone;
use Thurly\Sale;
...
IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/main/options.php');
IncludeModuleLangFile(__FILE__);

Main\Page\Asset::getInstance()->addJs('/thurly/js/sale/options.js');
$APPLICATION->SetAdditionalCSS("/thurly/themes/.default/sale.css");
...
if ($_SERVER['REQUEST_METHOD'] == "GET" && strlen($RestoreDefaults)>0 && $SALE_RIGHT=="W" && check_thurly_sessid())
{
	$bWasUpdated = true;
...
$arAllOptions =
	array(
		...
		Array("measurement_path", GetMessage("SMO_MEASUREMENT_PATH"), "/thurly/modules/sale/measurements.php", Array("text", 40)),
...
$strWarning = "";
if ($_SERVER['REQUEST_METHOD'] == "POST" && strlen($Update) > 0 && $SALE_RIGHT == "W" && check_thurly_sessid())
{
	...
	else
	{
			...
			if (!empty($_REQUEST["ADDRESS_dif_settings"]))
			{
				for ($i = 0; $i < $siteCount; $i++)
				{
					if($lMigrated)
					{
						try
						{
							\Thurly\Sale\SalesZone::saveSelectedTypes(array(
		...
		$rsAgents = CAgent::GetList(array("ID"=>"DESC"), array(
			"MODULE_ID" => "sale",
			"NAME" => "\\Thurly\\Sale\\Basket::deleteOldAgent(%",
		));
		...
		if ($delete_after > 0)
			CAgent::AddAgent("\\Thurly\\Sale\\Basket::deleteOldAgent(".$delete_after.");", "sale", "N", 8*60*60, "", "Y");
		...
		if ($oldExpirationProcessingEvents != $EXPIRATION_PROCESSING_EVENTS)
		{
			...
			if ($EXPIRATION_PROCESSING_EVENTS == "Y")
			{
				...
				$eventManager->registerEventHandlerCompatible('sale', 'OnBeforeBasketAdd', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onBeforeBasketAdd');
				$eventManager->registerEventHandlerCompatible('sale', 'OnBasketAdd', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onBasketAdd');
				$eventManager->registerEventHandlerCompatible('sale', 'OnOrderAdd', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onOrderAdd');
				$eventManager->registerEventHandlerCompatible('sale', 'OnSalePayOrder', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onSalePayOrder');

				$eventManager->unRegisterEventHandler('sale', 'OnSaleBasketItemSaved', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onSaleBasketItemSaved');
				$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderSaved', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onSaleOrderSaved');
				$eventManager->unRegisterEventHandler('sale', 'OnSaleOrderPaid', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onSaleOrderPaid');
			}
			else
			{
				Sale\Compatible\EventCompatibility::unRegisterEvents();

				$eventManager->unRegisterEventHandler('sale', 'OnBeforeBasketAdd', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onBeforeBasketAdd');
				$eventManager->unRegisterEventHandler('sale', 'OnBasketAdd', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onBasketAdd');
				$eventManager->unRegisterEventHandler('sale', 'OnOrderAdd', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onOrderAdd');
				$eventManager->unRegisterEventHandler('sale', 'OnSalePayOrder', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onSalePayOrder');

				$eventManager->registerEventHandler('sale', 'OnSaleBasketItemSaved', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onSaleBasketItemSaved');
				$eventManager->registerEventHandler('sale', 'OnSaleOrderSaved', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onSaleOrderSaved');
				$eventManager->registerEventHandler('sale', 'OnSaleOrderPaid', 'sale', '\Thurly\Sale\Internals\ConversionHandlers', 'onSaleOrderPaid');
			}
		}
		...
		$agentData = CAgent::GetList(array("ID"=>"DESC"), array(
			"MODULE_ID" => "sale",
			"NAME" => "\\Thurly\\Sale\\Product2ProductTable::addProductsByAgent(%",
		));
		...
		if ($SALE_P2P_ALLOW_COLLECT_DATA == "Y")
		{
			if (!$agent)
			{
				...
				CAgent::AddAgent("Thurly\\Sale\\Product2ProductTable::addProductsByAgent($limit);", "sale", "N", 60, "", "Y");
			}
		}
		else
		{
		...
		$rsAgents = CAgent::GetList(array("ID"=>"DESC"), array(
			"MODULE_ID" => "sale",
			"NAME" => "\\Thurly\\Sale\\Product2ProductTable::deleteOldProducts(%",
		));
		while($arAgent = $rsAgents->Fetch())
		{
			CAgent::Delete($arAgent["ID"]);
		...
		CAgent::AddAgent("Thurly\\Sale\\Product2ProductTable::deleteOldProducts(".$p2p_del_exp.");", "sale", "N", 24 * 3600 * $p2p_del_period, "", "Y");
		...
		$agentsList = CAgent::GetList(array("ID"=>"DESC"), array(
			"MODULE_ID" => "sale",
			"NAME" => "\\Thurly\\Sale\\Archive\\Manager::archiveOnAgent(%",
		));
		...
		{
			CAgent::AddAgent("\\Thurly\\Sale\\Archive\\Manager::archiveOnAgent(".$archiveLimit.",".$archiveTimeLimit.");", "sale", "N", 24*60*60, "", "Y");
		}
		...
		$agentsList = CAgent::GetList(array("ID"=>"DESC"), array(
			"MODULE_ID" => "sale",
			"NAME" => "\\Thurly\\Sale\\OrderHistory::deleteOldAgent(%",
		));
		while($agent = $agentsList->Fetch())
		{
			CAgent::Delete($agent["ID"]);
		...
		{
			CAgent::AddAgent("\\Thurly\\Sale\\OrderHistory::deleteOldAgent(\"$orderChangesCleanerDays\",\"$orderChangesCleanerLimit\");", "sale", "N", 60, "", "Y");
		}

		ob_start();
		require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/admin/group_rights.php");
		ob_end_clean();
		...
		$agentName = '\Thurly\Sale\Delivery\Tracking\Manager::startRefreshingStatuses();';
...
<?
	...
	<tr>
		...
		<td>
			<?
			$val = \Thurly\Main\Config\Option::get("sale", "allow_pay_status", Sale\OrderStatus::getInitialStatus());
			?>
	...
	<tr>
		...
		<td>
			<?
			$val = \Thurly\Main\Config\Option::get("sale", "allow_guest_order_view", 'N');
			?>
	...
	<tr class="sale_allow_guest_order_view" <?=($val === "Y") ? "" : "style=\"display:none\""?>>
		...
		<td>
			<?
			$guestStatuses = \Thurly\Main\Config\Option::get("sale", "allow_guest_order_view_status", "");
	...
	<?
	$paths = unserialize(\Thurly\Main\Config\Option::get("sale", "allow_guest_order_view_paths"));
	...
	<tr>
		...
		<td>
			<?$isSearch = Thurly\Sale\Location\Admin\Helper::getWidgetAppearance() == 'search';?>
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					<?if($lpEnabled):?>
						...
						<?$APPLICATION->IncludeComponent("thurly:sale.location.selector.".\Thurly\Sale\Location\Admin\Helper::getWidgetAppearance(), "", array(
							"ID" => "",
							"CODE" => $location,
			...
			<tr>
				...
				<td class="adm-detail-content-cell-r">
					...
					<?if($lpEnabled):?>
						...
						<?/*<a href="<?=\Thurly\Sale\Location\Admin\SiteLocationHelper::getListUrl();?>"><?=GetMessage('SMO_LOCATION_SALES_ZONE_SELECT')?></a>*/?>

					<?else:?>

						<?
						$sales_zone_countries = \Thurly\Sale\SalesZone::getCountriesIds($siteList[$i]["ID"]);
						$sales_zone_regions = \Thurly\Sale\SalesZone::getRegionsIds($siteList[$i]["ID"]);
						$sales_zone_cities = \Thurly\Sale\SalesZone::getCitiesIds($siteList[$i]["ID"]);
						?>
							...
							<td>
								...
								<select id="sales_zone_regions_<?=htmlspecialcharsbx($siteList[$i]["ID"])?>" name="sales_zone_regions[<?=htmlspecialcharsbx($siteList[$i]["ID"])?>][]" multiple size="10" class="sale-options-location-mselect">
									...
									<?if(!in_array("", $sales_zone_countries)):?>
										<?$arRegions = \Thurly\Sale\SalesZone::getRegions($sales_zone_countries, LANGUAGE_ID);?>
...
<?$tabControl->BeginNextTab();?>
	...
	$dbRes = Sale\Internals\StatusTable::getList(array(
		'select' => array('ID', 'Thurly\Sale\Internals\StatusLangTable:STATUS.NAME'),
		'filter' => array(
			'=Thurly\Sale\Internals\StatusLangTable:STATUS.LID' => $context->getLanguage(),
			'=TYPE' => 'D'
		),
		'order' => array('SORT' => 'ASC')
	));
	...
	$trackingStatuses = \Thurly\Sale\Delivery\Tracking\Manager::getStatusesList();
...
<script type="text/javascript">
	...
	function showDiscountReindex()
	{
		...
		obDiscount = new BX.CAdminDialog({
			'content_url': '/thurly/tools/sale/discount_reindex.php?lang=<? echo LANGUAGE_ID; ?>',
			'content_post': params,
			'draggable': true,
			'resizable': true,
			'buttons': [obBtn]
	...
	function showBasketDiscountConvert()
	{
		...
		obDiscount = new BX.CAdminDialog({
			'content_url': '/thurly/tools/sale/basket_discount_convert.php?lang=<? echo LANGUAGE_ID; ?>',
			'content_post': params,
			'draggable': true,
			'resizable': true,
			'buttons': [obBtn]
