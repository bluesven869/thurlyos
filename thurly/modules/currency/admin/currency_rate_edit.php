...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/prolog.php");
$CURRENCY_RIGHT = $APPLICATION->GetGroupRight("currency");
if ($CURRENCY_RIGHT=="D") $APPLICATION->AuthForm(GetMessage("ACCESS_DENIED"));
Loader::includeModule('currency');
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/currencies_rates.php");
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && !empty($_POST['Update']) && $CURRENCY_RIGHT=="W" && check_thurly_sessid())
{
	$arFields = array(
	...
	else
	{
		if (empty($_POST['apply']))
		{
			if (!empty($return_url))
				...
				LocalRedirect("/thurly/admin/currencies_rates.php?lang=".LANGUAGE_ID.GetFilterParams("filter_", false));
		}
		LocalRedirect("/thurly/admin/currency_rate_edit.php?lang=".LANGUAGE_ID."&ID=".$ID."&".GetFilterParams("filter_", false));
	}
}
...
$aContext = array(
	array(
		...
		"LINK" => "/thurly/admin/currencies_rates.php?lang=".LANGUAGE_ID,
...
if ($ID > 0)
{
	$aContext[] = 	array(
		...
		"LINK" => "/thurly/admin/currency_rate_edit.php?lang=".LANGUAGE_ID,
	...
	if ($CURRENCY_RIGHT == "W")
	{
		$aContext[] = 	array(
			...
			"LINK" => "javascript:if(confirm('".GetMessage("CONFIRM_DEL_MESSAGE")."'))window.location='/thurly/admin/currencies_rates.php?action=delete&ID=".$ID."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		);
	}
}
...
if (empty($currencyList))
	LocalRedirect("/thurly/admin/currency_edit.php?lang=".LANGUAGE_ID);
...
$tabControl->Buttons(
	array(
		...
		"back_url" =>"/thurly/admin/currencies_rates.php?lang=".LANGUAGE_ID.GetFilterParams("filter_")
	)
);
$tabControl->End();?>
</form>
...
function getCurrencyRate()
{
	...
	url = '/thurly/tools/currency/get_rate.php';
	ajaxData = {
