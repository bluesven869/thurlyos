...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $CURRENCY_RIGHT=="W" && !empty($_POST['Update']) && check_thurly_sessid())
{
	...
	else
	{
		...
		if (empty($_POST['apply']))
			LocalRedirect('/thurly/admin/currencies.php?lang='.LANGUAGE_ID);

		LocalRedirect('/thurly/admin/currency_edit.php?ID='.$ID.'&lang='.LANGUAGE_ID.'&'.$tabControl->ActiveTabParam());
	}
}
...
if ($ID != '')
{
	...
	if ($CURRENCY_RIGHT == "W" && $currency['BASE'] != 'Y')
	{
		$aContext[] = array(
			...
			"ONCLICK" => "javascript:if(confirm('".GetMessageJS("CONFIRM_DEL_MESSAGE")."'))window.location='currencies.php?action=delete&ID[]=".CUtil::JSEscape($ID)."&lang=".LANGUAGE_ID."&".thurly_sessid_get()."';",
		);
	}
}
...
$tabControl->EndTab();
$tabControl->Buttons(array("disabled" => $CURRENCY_RIGHT < "W", "back_url" =>"/thurly/admin/currencies.php?lang=".LANGUAGE_ID));
$tabControl->End();?>
</form>
<?
echo BeginNote();
