...
use Thurly\Currency\CurrencyClassifier;
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && $currencyRights == "W" && (!empty($_POST['save']) || !empty($_POST['apply'])) && check_thurly_sessid())
{
	...
	else
	{
		...
		else
		{
			...
			if ($_POST['apply'])
				LocalRedirect('/thurly/admin/currency_edit.php?ID=' . $currencyId . '&lang=' . LANGUAGE_ID . '&' . $tabControl->ActiveTabParam());
			else
...
<form method="post" action="<?$APPLICATION->GetCurPage()?>" name="admin_currency_classifier">
	...
	<?$tabControl->BeginNextTab();
		...
		<?}
	$tabControl->EndTab();
	$tabControl->Buttons(array("disabled" => $currencyRights < "W", "back_url" =>"/thurly/admin/currencies.php?lang=".LANGUAGE_ID));
	$tabControl->End();?>
</form>
