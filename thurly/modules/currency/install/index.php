...
class currency extends CModule
{
	...
	function DoInstall()
	{
		...
		$APPLICATION->IncludeAdminFile(Loc::getMessage("CURRENCY_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/step1.php");
	}
	...
	function DoUninstall()
	{
		...
		if ($step<2)
		{
			$APPLICATION->IncludeAdminFile(Loc::getMessage("CURRENCY_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/unstep1.php");
		}
		elseif ($step==2)
		{
			...
			$GLOBALS["errors"] = $this->errors;
			$APPLICATION->IncludeAdminFile(Loc::getMessage("CURRENCY_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/unstep2.php");
		}
	}
	...
	function InstallDB()
	{
		...
		if (!$DB->Query("SELECT COUNT(CURRENCY) FROM b_catalog_currency", true)):
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/db/".strtolower($DB->type)."/install.sql");
		endif;
		...
		$eventManager->registerEventHandlerCompatible('iblock', 'OnIBlockPropertyBuildList', 'currency',
			'\Thurly\Currency\Integration\IblockMoneyProperty', 'getUserTypeDescription');
		$eventManager->registerEventHandlerCompatible('main', 'OnUserTypeBuildList', 'currency',
			'\Thurly\Currency\UserField\Money', 'getUserTypeDescription');
	...
	function UnInstallDB($arParams = array())
	{
		...
		if (Loader::includeModule('currency'))
			\Thurly\Currency\CurrencyManager::clearCurrencyCache();
		if (!isset($arParams["savedata"]) || $arParams["savedata"] != "Y")
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/db/".strtolower($DB->type)."/uninstall.sql");
			if($this->errors !== false)
			{
				$APPLICATION->ThrowException(implode('', $this->errors));
				return false;
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin", true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/images", $_SERVER["DOCUMENT_ROOT"]."/thurly/images/currency", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/themes", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/js", $_SERVER["DOCUMENT_ROOT"]."/thurly/js", true, true);
			CopyDirFiles($_SERVER['DOCUMENT_ROOT']."/thurly/modules/currency/install/tools", $_SERVER['DOCUMENT_ROOT']."/thurly/tools", true, true);
		}
		return true;
	}
	...
	{
		DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
		DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/currency/install/themes/.default/", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes/.default");
		DeleteDirFilesEx("/thurly/themes/.default/icons/currency/");
		DeleteDirFilesEx("/thurly/images/currency/");
		DeleteDirFilesEx("/thurly/js/currency/");
		DeleteDirFilesEx("/thurly/tools/currency/"); // scripts
	...
	protected function installCurrencies()
	{
		...
		$currencyIterator = \Thurly\Currency\CurrencyTable::getList(array(
		...
		$baseCurrency = '';
		if ($thurlyos && Loader::includeModule('thurlyos'))
		{
			$thurlyosZone = CThurlyOS::getCurrentAreaConfig();
			if (!empty($thurlyosZone) && is_array($thurlyosZone))
				$baseCurrency = $thurlyosZone['CURRENCY'];
		}
		if ($baseCurrency == '')
		{
			$languageID = '';
		...
		foreach ($addCurrency as $fields)
		{
			...
			$currencyResult = \Thurly\Currency\CurrencyTable::add($fields);
		...
		if (!empty($currencyList))
		{
			...
			{
				$messList = Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/currency/install_lang.php', $existLanguage['ID']);
				foreach($currencyList as $oneCurrency)
				{
					...
					$resultCurrencyLang = \Thurly\Currency\CurrencyLangTable::add($fields);
			...
			if (!$thurlyos)
			{
				...
				CAgent::AddAgent('\Thurly\Currency\CurrencyManager::currencyBaseRateAgent();', 'currency', 'Y', 86400, '', 'Y', $checkDate->toString(), 100, false, true);
				unset($checkDate);
			}
			\Thurly\Currency\CurrencyManager::clearCurrencyCache();
		}
