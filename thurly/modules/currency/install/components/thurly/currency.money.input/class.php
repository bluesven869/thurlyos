...
class CCurrencyMoneyInputComponent extends \CThurlyComponent
{
	...
	{
		$params['CONTROL_ID'] = !empty($params['CONTROL_ID']) ? trim($params['CONTROL_ID']) : 'bxme_'.(\Thurly\Main\Security\Random::getString(5));
		$params['FIELD_NAME'] = !empty($params['FIELD_NAME']) ? trim($params['FIELD_NAME']) : 'money_'.(\Thurly\Main\Security\Random::getString(5));
	...
	{
		$this->currencyList = \Thurly\Currency\Helpers\Editor::getListCurrency();
	}
