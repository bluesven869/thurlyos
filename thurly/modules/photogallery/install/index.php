...
Class photogallery extends CModule
{
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/photogallery/install/components", $_SERVER["DOCUMENT_ROOT"]."/thurly/components", true, true);
		}
		return true;
	}
	...
	function DoInstall()
	{
		...
		if (IsModuleInstalled("iblock"))
		{
			...
			if ($step < 2)
				$APPLICATION->IncludeAdminFile(GetMessage("PHOTO_INSTALL"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/photogallery/install/step1.php");
			elseif($step == 2)
				$APPLICATION->IncludeAdminFile(GetMessage("PHOTO_INSTALL"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/photogallery/install/step2.php");
			elseif ($step == 3)
			{
				$this->InstallDB();
				$this->InstallEvents();
