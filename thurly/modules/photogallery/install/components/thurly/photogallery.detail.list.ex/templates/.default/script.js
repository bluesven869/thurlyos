...
window.BXPhotoSlider.prototype = {
	...
	CreateSceleton: function()
	{
		...
		this.pImageWait = this.pImgCell.appendChild(BX.create("IMG", {props: {src: "/thurly/images/1.gif", className: "photo-image-wait"}, style: {display: 'none'}}));
		this.pImage = this.pImgCell.appendChild(BX.create("IMG", {props: {src: "/thurly/images/1.gif"}}));
		...
		// Rating with stars - filled by component thurly:iblock.vote with 'ajax' template
		if (this.useRatings)
		{
		...
		this.pComWaiter = pLeftCont.appendChild(BX.create("IMG", {props:{className: "photo-wait-comments", src: "/thurly/images/1.gif"}, style: {display: "none"}})); // Waiter
	...
	EditItem: function(e)
	{
		...
		var _this = this;
		var url = this.actionUrl + (this.actionUrl.indexOf('?') == -1 ? '?' : '&') + 'photo_list_action=edit&sessid=' + BX.thurly_sessid();
...
BXTopSlider.prototype = {
	...
	BuildItems: function()
	{
		...
		for (i = 0; i < this.pObj.itemsCount; i++)
		{
			...
			pImg = pDiv.appendChild(BX.create("IMG", {props:{src: '/thurly/images/1.gif'}, style: {width: this.thumbSize + 'px', height: this.thumbSize + 'px'}}));
