...
CJSCore::Init(array('jquery2', 'fullscreen'));
//\Thurly\Main\Page\Asset::getInstance()->addJs($this->GetFolder().'/jquery.facedetection.js');
\Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/faceid/WebPhotoMaker/WebPhotoMaker.js');
\Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/faceid/WebPhotoMaker/smoother.js');
\Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/faceid/WebPhotoMaker/fpsmeter.min.js');
...
<script type="text/javascript">
	BX.message(<?=\Thurly\Main\Web\Json::encode($jsMessages)?>);
	window.FACEID_AGREEMENT = <?=$arResult['HAS_AGREEMENT']?'true':'false'?>;

	if (window.FACEID_AGREEMENT)
    {
		BX.ready(function(){
			BXFaceIdStart(<?=\Thurly\Main\Web\Json::encode(array(
				'socnet_enabled' => \Thurly\Main\Config\Option::get('faceid', 'ftracker_socnet_enabled', 'Y') == 'Y'
			))?>);
		});
    }
</script>
...
<div class="tracker-agreement-shadow">
	<div class="tracker-agreement-popup">
		...
		<?=\Thurly\Faceid\AgreementTable::getAgreementText(true)?>
		<div class="tracker-agreement-popup-button">
			<div id="faceid-agreement-accept" class="webform-small-button webform-small-button-accept tracker-agreement-popup-button-item"><?=Loc::getMessage('FACEID_TRACKER_CMP_AGR_BUTTON')?></div>
		</div>
	</div>
</div>
