...
// check permissions
$accessAllowed = false;
$acceptedAgreement = \Thurly\Faceid\AgreementTable::checkUser($USER->getId());

if (empty($USER) || !$USER->GetID())
{
	echo \Thurly\Main\Web\Json::encode(array('error' => array(
...
if (!\Thurly\Main\Config\Option::get('faceid', 'user_index_enabled', 0))
{
	echo \Thurly\Main\Web\Json::encode(array('error' => array(
...
$expReasons = \Thurly\Faceid\UsersTable::getExpiredReasonList();
...
if (!empty($_POST['action']))
{
	...
	elseif ($_POST['action'] == 'identify')
	{
		...
		$response = \Thurly\FaceId\FaceId::identifyUser($fileContent);
		...
		if (!empty($response['success']) && !empty($response['result']['found']))
		{
			...
			$userFace = \Thurly\Faceid\UsersTable::getById($faceId)->fetch();
			if (!empty($userFace))
			{
				$user = \Thurly\Main\UserTable::getByPrimary($userFace['USER_ID'], array(
				...
				if (!empty($user))
				{
					...
					if (\Thurly\Faceid\UsersTable::checkTimemanEnabled($user['ID']))
					{
						...
						elseif ($currentStatus == 'EXPIRED')
						{
							...
							$outputVisitor['workday_expired_reasons_string'] = \Thurly\Main\Web\Json::encode($expReasons);

							// get day of expiration
							$timemanInfo = $tmUser->GetCurrentInfo();
							$startDate = new \Thurly\Main\Type\DateTime($timemanInfo['DATE_START']);
	...
	{
		$user = \Thurly\Main\UserTable::getByPrimary($_POST['id'], array(
			'select' => array('ID', 'LOGIN', 'NAME', 'LAST_NAME', 'PERSONAL_PHOTO', 'WORK_POSITION')
		))->fetch();
	...
	{
		$actionStatus = 'INFO';
		$portalInfo['agreementText'] = \Thurly\Faceid\AgreementTable::getAgreementText();
	}
	...
	$portalInfo['logo24'] = \COption::GetOptionString("thurlyos", "logo24show", "Y") !== "N";
	...
	echo \Thurly\Main\Web\Json::encode(array(
		...
		'action' => $actionStatus,
		'status' => array(
			'balance' => \Thurly\Main\Config\Option::get('faceid', 'balance', '1000'),
		),
		'portal' => $portalInfo,
		'error' => $errorResponse
	), JSON_FORCE_OBJECT);
...
function faceidSaveWorkdayActionLog($userId, $action, $imageBinaryContent)
{
	...
	return \Thurly\Faceid\TrackingWorkdayTable::add(array(
		'USER_ID' => $userId,
		'ACTION' => $action,
...
{
	$title = \COption::GetOptionString("thurlyos", "site_title", "");
	if (empty($title))
	{
