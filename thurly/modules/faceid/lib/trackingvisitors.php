...
class TrackingVisitorsTable extends Main\Entity\DataManager
{
	...
	public static function getCrmInfoByFace($faceId)
	{
		...
		{
			// try contacts
			$result = \Thurly\Crm\ContactTable::getList(array(
		...
		{
			// try leads
			$result = \Thurly\Crm\LeadTable::getList(array(
	...
	public static function createCrmLead($visitor, $leadTitle)
	{
		if (!is_array($visitor))
		{
			$visitor = \Thurly\Faceid\TrackingVisitorsTable::getRowById($visitor);
		}
		...
		// get lead source
		$leadSource = \Thurly\Main\Config\Option::get('faceid', 'ftracker_lead_source');
	...
	public static function toJson($visitor, $confidence = 0, $returnAsArray = false)
	{
		...
		{
			$visitor['CRM'] = \Thurly\Faceid\TrackingVisitorsTable::getCrmInfoByFace($visitor['FACE_ID']);
		}
		...
		return $returnAsArray ? $jsonResult : \Thurly\Main\Web\Json::encode($jsonResult);
	}
}