...
if ($REQUEST_METHOD == "POST" && isset($_REQUEST['save_type']) && $_REQUEST['save_type'] == 'Y' && check_thurly_sessid())
{
...
$GLOBALS['APPLICATION']->AddHeadScript("/thurly/js/calendar/cal-controlls.js");
?>
	<table id="bxcal_type_tbl" style="width: 650px;">
		<?
		$actionUrl = '/thurly/admin/settings.php?mid=calendar&lang='.LANG;
		$arXML_ID = array();
...
function delType(xml_id)
{
	...
	{
		BX.ajax.post('<?= $APPLICATION->GetCurPage()?>?mid=<?= urlencode($mid)?>&lang=<?=LANGUAGE_ID?>&save_type=Y&del_type=Y&type_xml_id=' + xml_id, {sessid: BX.thurly_sessid()}, function()
		{
