...
namespace Thurly\Calendar\Sync;
...
final class GoogleApiPush
{
	...
	public static function renewWatchChannels()
	{
		...
		$result = $DB->Query("SELECT * FROM b_calendar_push WHERE " .
			...
			" <= '" .
			\Thurly\Main\Type\DateTime::createFromTimestamp(strtotime('+1 day')) .
		...
		if (count($pushRows) < 4)
		{
			...
			if ($row)
			{
				$nextAgentDate = \Thurly\Main\Type\DateTime::createFromTimestamp(strtotime($row['EXPIRES']) - (60*60*20))->format(\Thurly\Main\Type\Date::convertFormatToPhp(FORMAT_DATETIME));
			}
			else
			{
				$nextAgentDate = \Thurly\Main\Type\DateTime::createFromTimestamp(strtotime('now') + (60*60*20))->format(\Thurly\Main\Type\Date::convertFormatToPhp(FORMAT_DATETIME));
			}

			\CAgent::RemoveAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::renewWatchChannels();", "calendar");
			\CAgent::AddAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::renewWatchChannels();", "calendar", "N", 900,"", "Y", $nextAgentDate);
			return false;
		}

		return "\\Thurly\\Calendar\\Sync\\GoogleApiPush::renewWatchChannels();";
	}
	...
	public static function createWatchChannels($start = 0)
	{
		...
		$lastId = $start;
		\Thurly\Main\Loader::includeModule('dav');
		$davConnections = \CDavConnection::GetList(
			array("ID" => "ASC"),
			array('ACCOUNT_TYPE' => 'google_api_oauth', '>ID' => $start),
			false,
		...
		$connections = $pushConnectionIds = array();
		...
		if ($lastId == $start)
		{
			\CAgent::RemoveAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::createWatchChannels(" . $start . ");", "calendar");
			\CAgent::RemoveAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::createWatchChannels(0);", "calendar");
			\CAgent::AddAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::createWatchChannels(0);", "calendar", "N", 3600, "", "Y", Type\DateTime::createFromTimestamp(strtotime('+1 hour'))->format(Type\Date::convertFormatToPhp(FORMAT_DATETIME)));
			return null;
		}
		else
		{
			return "\\Thurly\\Calendar\\Sync\\GoogleApiPush::createWatchChannels(" . $lastId . ");";
		}
	}
	...
	public static function clearPushChannels()
	{
		...
		if ($hasRows)
			return "\\Thurly\\Calendar\\Sync\\GoogleApiPush::clearPushChannels();";
		else
			return null;
	}
	...
	public static function processPush()
	{
		...
		if (!empty($pushRows))
		{
			...
			foreach($pushRows as $row)
			{
				...
				if ($row['ENTITY_TYPE'] == 'CONNECTION')
				{
					if (!empty($connections[$row['ID']]))
					{
						$resynced = \CCalendarSync::syncConnection($connections[$row['ID']]);
					}
				}
				elseif ($row['ENTITY_TYPE'] == 'SECTION')
				{
					if (!empty($sections[$row['ID']]))
					{
						$eventsSyncToken = \CCalendarSync::syncCalendarEvents($sections[$row['ID']]);
						if ($eventsSyncToken)
						{
							\CCalendarSect::Edit(array(
								'arFields' => array(
									'ID' 		 => $sections[$row['ID']]['ID'],
				...
				if (($resynced && $row['ENTITY_TYPE'] == 'CONNECTION') || ($eventsSyncToken && $row['ENTITY_TYPE'] == 'SECTION'))
				{
					PushTable::Update(
						array(
							'ENTITY_TYPE' => $row['ENTITY_TYPE'],
		...
		return "\\Thurly\\Calendar\\Sync\\GoogleApiPush::processPush();";
	}
}