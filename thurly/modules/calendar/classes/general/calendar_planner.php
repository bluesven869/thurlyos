...
class CCalendarPlanner
{
	...
	public static function InitJsCore($config = array(), $initialParams)
	{
		...
		$APPLICATION->AddHeadScript('/thurly/js/calendar/planner.js');
		$APPLICATION->SetAdditionalCSS("/thurly/js/calendar/planner.css");

		$mess_lang = \Thurly\Main\Localization\Loc::loadLanguageFile(__FILE__);
		?>
		...
		<script type="text/javascript">
			...
			{
				new CalendarPlanner(
					<?=\Thurly\Main\Web\Json::encode($config, false);?>,
					<?=\Thurly\Main\Web\Json::encode($initialParams);?>
				);
			});
		</script>
		<?
