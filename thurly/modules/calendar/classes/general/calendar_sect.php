...
class CCalendarSect
{
	...
	public static function GetList($params = array())
	{
		...
		if (($checkPermissions || $getPermissions) && count($arSectionIds) > 0 && $userId > 0)
		{
			...
			foreach($arResult as $sect)
			{
				...
				$bManager = false;
				if(\Thurly\Main\Loader::includeModule('intranet') && $sect['CAL_TYPE'] == 'user' && $settings['dep_manager_sub'])
				{
					if(!$userId)
	...
	public static function CanDo($operation, $sectId = 0, $userId = false)
	{
		...
		if ((CCalendar::GetType() == 'group' || CCalendar::GetType() == 'user' || CCalendar::IsThurlyOS()) && CCalendar::IsSocNet() && CCalendar::IsSocnetAdmin())
			return true;
