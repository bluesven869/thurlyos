<?
use \Thurly\Calendar\Ui\CalendarFilter;
...
class CCalendarRequest
{
	...
	public static function Process($action = '', CCalendar $calendar)
	{
		...
		self::$request = \Thurly\Main\Context::getCurrent()->getRequest()->toArray();
		...
		{
			// Check the access
			if (!CCalendarType::CanDo('calendar_type_view', CCalendar::GetType()) || !check_thurly_sessid())
			{
		...
		if ($newId)
		{
			...
			if ($arFields['IS_MEETING'])
			{
				\Thurly\Main\FinderDestTable::merge(array(
					"CONTEXT" => "CALENDAR",
					"CODE" => \Thurly\Main\FinderDestTable::convertRights($arAccessCodes, array('U'.CCalendar::GetUserId()))
				));
			}
		}
...
//	}
	...
	public static function GetEditSlider()
	{
		...
		if (!$entryId || !empty($entry) && CCalendarSceleton::CheckThurlyOSLimits(array('id' => $jsId)))
		{
			$APPLICATION->IncludeComponent("thurly:calendar.edit.slider", "", array(
				'id' => $jsId,
				'event' => $entry,
				'formType' => $formType,
				'type' => CCalendar::GetType(),
	...
	public static function GetViewSlider()
	{
		...
		if ($event)
		{
			$APPLICATION->IncludeComponent("thurly:calendar.view.slider", "", array(
				'id' => $jsId,
				'event' => $event,
				'type' => CCalendar::GetType(),
				'sectionName' => $_REQUEST['section_name'],
	...
	public static function getSettingsSlider()
	{
		...
		$APPLICATION->IncludeComponent("thurly:calendar.settings.slider", "", array(
			'id' => $jsId,
