...
use Thurly\Calendar\PushTable;
use Thurly\Calendar\Sync\GoogleApiSync;
...
class CCalendar
{
	...
	function Init($params)
	{
		...
		self::$showTasks = (self::$type == 'user' || self::$type == 'group')
			...
			&& \Thurly\Main\Loader::includeModule('tasks')
		...
		self::$outerUrl = $APPLICATION->GetCurPageParam('', array("action", "bx_event_calendar_request", "clear_cache", "thurly_include_areas", "thurly_show_mode", "back_url_admin", "SEF_APPLICATION_CUR_PAGE_URL", "EVENT_ID", "EVENT_DATE", "CHOOSE_MR"), false);
	...
	public function Show($params = array())
	{
		...
		$JSConfig = Array(
			...
			'weekDays' => CCalendarSceleton::GetWeekDaysEx(self::GetWeekStart()),
			...
			'locationFeatureEnabled' => !self::IsThurlyOS() || \Thurly\ThurlyOS\Feature::isFeatureEnabled("calendar_location")
		);
		...
		if(self::$type == 'user' && self::$userId == self::$ownerId)
		{
			...
			$JSConfig['filterId'] = \Thurly\Calendar\Ui\CalendarFilter::getFilterId(self::$type, self::$ownerId, self::$userId);
		}
		...
		if ($bCalDAV)
			self::InitCalDavParams($JSConfig, $bGoogleApi);

		if ($bGoogleApi)
			self::InitGoogleApiParams($JSConfig);
		...
		$entries = self::GetEventList(array(
			'type' => self::$type,
			'section' => $arSectionIds,
			'fromLimit' => self::Date(mktime(0, 0, 0, $init_month - 1, 1, $init_year), false),
			'toLimit' => self::Date(mktime(0, 0, 0, $init_month + 2, 0, $init_year), false),
			'additonalMeetingsId' => $additonalMeetingsId
		), $arAttendees);

		if ($startupEvent && is_array($startupEvent))
			$entries[] = $startupEvent;
		...
		if (count($arDisplayedNowSPSections) > 0)
		{
			$arSuperposedEvents = CCalendarEvent::GetList(
				array(
					'arFilter' => array(
						"FROM_LIMIT" => self::Date(mktime(0, 0, 0, $init_month - 1, 1, $init_year), false),
						"TO_LIMIT" => self::Date(mktime(0, 0, 0, $init_month + 2, 1, $init_year), false),
		...
		if (self::$showTasks)
		{
			...
			if (!in_array('tasks', $hiddenSections))
			{
				$tasksEntries = self::getTaskList(
					array(
						'type' => self::$type,
						'ownerId' => self::$ownerId
					)
...
//	}
	...
	public static function DeleteSection($id)
	{
		...
		{
			\Thurly\Calendar\Sync\GoogleApiPush::stopChannel($row);
		}
	...
	{
		$bExtranet = \Thurly\Main\Loader::includeModule('extranet');
		// It's extranet user
		...
		foreach($arOpt as $opt)
		{
			...
			elseif (isset($settings[$opt]))
			{
				...
				elseif ($opt == 'sync_by_push' && self::isGoogleApiEnabled())
				{
					...
					{
						// start push agents
						\CAgent::RemoveAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::clearPushChannels();", "calendar");
						\CAgent::AddAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::createWatchChannels(0);", "calendar", "N", 60);
						\CAgent::AddAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::processPush();", "calendar", "N", 180);
						\CAgent::AddAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::renewWatchChannels();", "calendar", "N", 14400);
					}
					else
					{
						// start clear push channels agent
						\CAgent::AddAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::clearPushChannels();", "calendar", "N", 60);
						\CAgent::RemoveAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::processPush();", "calendar");
						\CAgent::RemoveAgent("\\Thurly\\Calendar\\Sync\\GoogleApiPush::renewWatchChannels();", "calendar");
					}
				}
	...
	public static function IsThurlyOS()
	{
		return \Thurly\Main\ModuleManager::isModuleInstalled('thurlyos');
	}
	...
	public static function UpdateUFRights($files, $rights, $ufEntity = array())
	{
		...
		if (sizeof($files) <= 0)
			return false;
		if (!\Thurly\Main\Loader::includeModule('iblock') || !\Thurly\Main\Loader::includeModule('webdav'))
			return false;
	...
	public static function GetUserAvatarSrc($user = array(), $params = array())
	{
		...
		if ($avatar_src === false)
			$avatar_src = '/thurly/images/1.gif';
		return $avatar_src;
	}
	...
	{
		\Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/admin/task_description.php");
		...
		else
		{
			...
			elseif($locNew['room_id'])
			{
				$roomEventId = CCalendarLocation::reserveRoom(array(
					...
					'room_event_id' => $locNew['room_event_id'],
					'dateFrom' => $params['dateFrom'],
					'dateTo' => $params['dateTo'],
				...
				if ($roomEventId)
					$locNew = 'calendar_'.$locNew['room_id'].'_'.$roomEventId;
				else
					$locNew = '';
			}
			else
	...
	public static function RemoveConnection($connection = array())
	{
		...
		{
			\Thurly\Calendar\Sync\GoogleApiPush::stopChannel($row, $connection['ENTITY_ID']);
		}
		...
		foreach ($res as $sect)
		{
			if ($connection['del_calendars'] == 'Y') // Delete all callendars from this connection
			{
				...
				if ($row = $pushChannelResult->fetch())
				{
					\Thurly\Calendar\Sync\GoogleApiPush::stopChannel($row, $sect['OWNER_ID']);
				}
	...
	public static function GetSocNetDestination($user_id = false, $selected = array())
	{
		...
		if (\Thurly\Main\Loader::includeModule('extranet') && !CExtranet::IsIntranetUser())
		{
	 ...
	 * @return null
	 */
	public static function OnDavCalendarSync(\Thurly\Main\Event $event)
	{
		...
		static $arAgentsMap = array(
				'android' => 'android', // Android/iOS CardDavThurlyOS
	...
	public static function OnExchangeCalendarSync(\Thurly\Main\Event $event)
	{
	...
	public static function IsSocNet()
	{
		...
		{
			\Thurly\Main\Loader::includeModule("socialnetwork");
	...
	public static function GetSettings($params = array())
	{
		...
		if (self::$settings['forum_id'] == "")
		{
			...
			if (self::$settings['forum_id'] == "" && \Thurly\Main\Loader::includeModule("forum"))
			{
			...
			if (!$USER->CanDoOperation('edit_php'))
			{
				...
				if (\Thurly\Main\Loader::includeModule("socialnetwork"))
				{
					...
					if(!empty($group['CLOSED']) && $group['CLOSED'] === 'Y' &&
						\Thurly\Main\Config\Option::get('socialnetwork', 'work_with_closed_groups', 'N') === 'N')
					{
	...
	public static function IsExchangeEnabled($userId = false)
	{
		...
		if (!IsModuleInstalled('dav') || COption::GetOptionString("dav", "agent_calendar") != "Y")
			$res = false;
		elseif (!\Thurly\Main\Loader::includeModule('dav'))
			$res = false;
		elseif ($userId === false)
	...
	{
		return is_null(\Thurly\Main\Config\Configuration::getValue("calendar_integration")) || \Thurly\Main\Config\Configuration::getValue("calendar_integration") === self::INTEGRATION_GOOGLE_API;
	}
	...
	public static function IsCalDAVEnabled()
	{
		if (!IsModuleInstalled('dav') || COption::GetOptionString("dav", "agent_calendar_caldav") != "Y")
			return false;
		return \Thurly\Main\Loader::includeModule('dav') && CDavGroupdavClientCalendar::IsCalDAVEnabled();
	}
	...
	public static function IsExtranetEnabled()
	{
		if (!isset(self::$bExtranet))
			self::$bExtranet = \Thurly\Main\Loader::includeModule('extranet') && CExtranet::IsExtranetSite();
		return self::$bExtranet;
	}

	public static function IsAbsenceEnabled()
	{
		return !self::IsThurlyOS()
		|| COption::GetOptionString("thurlyos",  "absence_limits_enabled", "") != "Y"
		|| \Thurly\ThurlyOS\Feature::isFeatureEnabled("absence");
	}
	...
	public static function InitCalDavParams(&$JSConfig, $isGoogleApiEnabled = false)
	{
		global $USER;
		...
		if (self::$type == 'user')
		{
			...
			$JSConfig['caldav_link_one'] = CCalendar::GetServerPath()."/thurly/groupdav.php/".SITE_ID."/".$login."/calendar/#CALENDAR_ID#/";

			$arConnections = array();
			$res = CDavConnection::GetList(array("ID" => "DESC"), array("ENTITY_TYPE" => "user","ENTITY_ID" => self::$ownerId), false, false);

			if (self::IsPersonal() && !$isGoogleApiEnabled)
			{
				$googleCalDavStatus = CCalendarSync::GetGoogleCalendarConnection();
			}
			...
			while($arCon = $res->Fetch())
			{
				if (($arCon['ACCOUNT_TYPE'] == 'caldav_google_oauth' && !$isGoogleApiEnabled) || $arCon['ACCOUNT_TYPE'] == 'caldav')
				{
					$arConnections[] = array(
						'id' => $arCon['ID'],
		...
		{
			$JSConfig['caldav_link_one'] = CCalendar::GetServerPath()."/thurly/groupdav.php/".SITE_ID."/group-".self::$ownerId."/calendar/#CALENDAR_ID#/";
		}
	...
	{
		if (!\Thurly\Main\Loader::includeModule('socialservices') || !\Thurly\Main\Loader::includeModule('dav'))
		{
			return;
		}
		...
		if (self::IsPersonal())
		{
			...
			if ($googleApiStatus['googleCalendarPrimaryId'] || $arConnections)
			{
				$serverPath = 'https://www.googleapis.com/calendar/v3';
				$addConnection = true;
				...
				if ($addConnection && !$arConnections)
				{
					...
					if ($conId)
					{
						...
						if ($pushOptionEnabled || CCalendar::IsThurlyOS())
						{
	...
	public static function getTaskList($params = array())
	{
		...
		$mgrResult = \Thurly\Tasks\Manager\Task::getList(
			\Thurly\Tasks\Util\User::getId(),
			array(
