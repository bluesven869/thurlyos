...
class calendar extends CModule
{
	...
	function InstallDB()
	{
		...
		$eventManager->registerEventHandler('socialnetwork', 'onLogIndexGetContent', 'calendar', '\Thurly\Calendar\Integration\Socialnetwork\Log', 'onIndexGetContent');
		...
		if ($pushOptionEnabled || \Thurly\Main\ModuleManager::isModuleInstalled('thurlyos'))
		{
	...
	function UnInstallDB($arParams = array())
	{
		...
		if ((true == array_key_exists("savedata", $arParams)) && ($arParams["savedata"] != 'Y'))
		{
			...
			$errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/'.$this->MODULE_ID.'/install/db/'.strtolower($DB->type).'/uninstall.sql');
		...
		$eventManager->unRegisterEventHandler('socialnetwork', 'onLogIndexGetContent', 'calendar', '\Thurly\Calendar\Integration\Socialnetwork\Log', 'onIndexGetContent');
	...
	function InstallEvents()
	{
		...
		if($ar["C"] <= 0)
			include($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/events.php");
	...
	function InstallFiles()
	{
		...
		{
			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/tools",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/tools",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/components",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/components",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/admin",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/admin",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/js",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/js",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/images",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/images",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/activities",
				$_SERVER["DOCUMENT_ROOT"]."/thurly/activities",
				true, true
			);

			CopyDirFiles(
				$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/services",
	...
	function DoInstall()
	{
		...
		if (!IsModuleInstalled("calendar"))
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("CAL_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/step1.php");
		}
	}
	...
	function DoUninstall()
	{
		...
		if($USER->IsAdmin())
		{
			...
			if($step < 2)
			{
				$APPLICATION->IncludeAdminFile(GetMessage("CAL_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/unstep1.php");
			}
			elseif($step == 2)
			{
				...
				$GLOBALS["errors"] = $this->errors;
				$APPLICATION->IncludeAdminFile(GetMessage("CAL_UNINSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/install/unstep2.php");
			}
		}
	}
