function JCEC(params) // Javascript Class Event Calendar
{
	...
	this.DATETIME_FORMAT = BX.date.convertThurlyFormat(BX.message("FORMAT_DATETIME"));
	if ((this.DATETIME_FORMAT.substr(0, this.DATE_FORMAT.length) == this.DATE_FORMAT))
		...
		this.TIME_FORMAT = BX.date.convertThurlyFormat(this.bAMPM ? 'H:MI:SS T' : 'HH:MI:SS');
			...
			menu.push({
				...
				onclick: function(){
					...
					if (!window.jsOutlookUtils)
						BX.loadScript('/thurly/js/calendar/outlook.js', function(){try{eval(el.OUTLOOK_JS);}catch(e){}});
					else
