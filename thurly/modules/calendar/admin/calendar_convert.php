...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/include.php");
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/calendar/prolog.php");
...
class CCalendarConvert
{
	...
	function ParseParams($lastPath = false, $lastSite = false)
	{
		...
		?><script>top.location = "/thurly/admin/calendar_convert.php?lang=<?= LANG?>&content_parsed=Y";</script><?
	}
	...
	function ParseFile($file)
	{
		...
		if ($bIsDir)
		{
			// Skip 'thurly' and 'upload' folders
			if ($file == $docRoot.'/thurly' || $file == $docRoot.'/upload' || $file == $docRoot.'/images')
				return;
		...
		else
		{
			...
			{
				if (preg_match('/includecomponent\([\n\t\r\s]*("|\')thurly:'.$p.'/i'.BX_UTF_PCRE_MODIFIER, $fileContent))
				{
	...
	function FetchParams($content)
	{
		...
		$arComponents = array('thurly:intranet.event_calendar', 'thurly:socialnetwork','thurly:socialnetwork_user','thurly:socialnetwork_group');
		...
		if (count($arPHP) > 0)
		{
			...
			foreach($arPHP as $code)
			{
				...
				if ($arRes && in_array($arRes['COMPONENT_NAME'], $arComponents))
				{
					...
					if ($arRes['COMPONENT_NAME'] == 'thurly:intranet.event_calendar')
					{
...
if (CModule::IncludeModule("intranet") && CModule::IncludeModule("calendar"))
{
	...
	// 1. Fetch options
	if($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["convert"]) && $_POST["convert"] == 'Y' && check_thurly_sessid())
	{
	...
	if (isset($_POST["parse_public"]) && check_thurly_sessid())
	{
	...
	?>
	<form style="border: 2px solid #B8C1DD; padding: 10px; background: #F8F8F8;" method="post" name="calendar_form" action="/thurly/admin/calendar_convert.php" enctype="multipart/form-data" encoding="multipart/form-data">
		<?= thurly_sessid_post();?>
...
<?endif;?>
		...
		<input id="bxconv_go_to_settings" type="button" value="<?= GetMessage('CAL_GO_TO_SETTINGS')?>" onclick="window.location='/thurly/admin/settings.php?lang=<?= LANG?>&mid=calendar';" style="display: none;"/>
		<?endif;?>
	</form>
<script>
