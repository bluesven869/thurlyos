...
use Thurly\Disk\CrumbStorage;
use Thurly\Disk\Document\CloudImport;
...
class Controller extends Internals\Controller
{
				...
				array(
					...
					'runtime' => array(
						new ReferenceField('UG',
							'Thurly\Socialnetwork\UserToGroupTable',
	...
	protected function processActionUploadFile()
	{
		...
		if ($uploader === null)
			$uploader = new \Thurly\Main\UI\Uploader\Uploader(array(
				"events" => array(
	...
	protected function processActionGetBreadcrumbs($attachedId)
	{
		...
		array_unshift($crumbs, $file->getStorage()->getProxyType()->getTitleForCurrentUser());
	...
	protected function sendNeedAuth($authUrl)
	{
		$this->sendJsonResponse(array(
			...
			'authUrl' => $authUrl,
			'isThurlyOS' => ModuleManager::isModuleInstalled('thurlyos'),
		));
	}
}