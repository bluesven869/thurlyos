...
use Thurly\Main\NotSupportedException;
...
final class FileUserType
{
	...
	public static function checkFields($userField, $value, $userId = false)
	{
		...
		if($type == self::TYPE_ALREADY_ATTACHED)
		{
			...
			if(
				...
				!(
					is_subclass_of($connectorClass, 'Thurly\Disk\Uf\ISupportForeignConnector') ||
					in_array('Thurly\Disk\Uf\ISupportForeignConnector', class_implements($connectorClass)) //5.3.9
				)
			)
			{
				$errors[] = array(
	 ...
	 * In general UI disk.uf.file renders checkbox (@see \Thurly\Disk\Uf\UserFieldManager::getInputNameForAllowEditByEntityType),
