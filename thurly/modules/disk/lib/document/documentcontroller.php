...
class DocumentController extends Internals\Controller
{
	...
	{
		if(
			ThurlyOSManager::isEnabled() &&
			!ThurlyOSManager::isAccessEnabled('disk', $this->getUser()->getId()))
		{
	...
	protected function processActionShow()
	{
		...
		$thurlyHandler = new ThurlyHandler($this->getUser()->getId());
		$dataForView = $thurlyHandler->getDataForViewFile($fileData);
		if ($dataForView && !$thurlyHandler->getErrors())
		{
	...
	private function createEditSessionFromShowSession(ShowSession $showSession)
	{
		...
		/** @see \Thurly\Disk\Document\Office365Handler::shareFileToEdit() */
		/** @see \Thurly\Disk\Document\GoogleHandler::shareFileToEdit() */
