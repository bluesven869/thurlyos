...
abstract class BaseObject extends Internals\Model
{
	...
	protected function moveInSameStorage(Folder $folder, $movedBy)
	{
		...
		{
			Driver::getInstance()->cleanCacheTreeThurlyDisk(array_keys($subscribersAfterMove));
		}
	...
	/**
	 * Builds model from \Thurly\Main\Entity\Result.
	...
	public function restoreInternal($restoredBy)
	{
		...
		if($status)
		{
			//we have to delete links which are in trashcan. It's special case. @see \Thurly\Disk\BaseObject::markDeletedInternal.
