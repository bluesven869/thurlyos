...
namespace Thurly\Disk\ThurlyOSDisk\Legacy;

use Thurly\Disk\ThurlyOSDisk;
use Thurly\Disk\ThurlyOSDisk\Legacy\Exceptions\OldDiskVersionException;
...
use Thurly\Disk\Internals\Controller;
use Thurly\Disk\Internals\Error\Error;
use Thurly\Disk\Storage;
use Thurly\Disk\User;
use Thurly\Disk\ThurlyOSDisk\Legacy\Exceptions\AccessDeniedException;
use Thurly\Disk\ThurlyOSDisk\Legacy\Exceptions\BadStorageAfterMigrateException;
use Thurly\Main\ArgumentException;
use Thurly\Main\Event;
use Thurly\Main\EventResult;
use Thurly\Main\Loader;
use Thurly\Main\Localization\Loc;
use Thurly\Main\ObjectNotFoundException;
...
class StorageController extends Controller
{
	...
	protected function sendJsonResponse($response, $params = null)
	{
		...
		header("X-Thurly-Disk-API: " . self::VERSION);
	...
	protected function processActionUpload()
	{
		...
		$tmpFileManager = new ThurlyOSDisk\UploadFileManager();
		$tmpFileManager
		...
		if(!$tmpFileManager->upload($this->request->getPost('name'), $this->request->getFile('file')))
		{
			...
			if($this->errorCollection->getErrorByCode(ThurlyOSDisk\TmpFile::ERROR_CLOUD_APPEND_INVALID_CHUNK_SIZE))
			{
				$this->sendJsonResponse(array(
	...
	{
		//disables CThurlyOSCdn::bx24ReplaceStaticDomain(). Safe memory!
	...
	protected function generateNextPageUrl(ThurlyOSDisk\PageState $pageState = null)
	{
	...
	protected function processActionInitialize()
	{
		...
		$this->sendJsonSuccessResponse(array(
			...
			'isB24' => (bool)isModuleInstalled('thurlyos'),
			'isExtranetUser' => (bool)$isExtranetUser,

			'version'=> self::VERSION,
			'token_sid' => thurly_sessid(),

			'defaultChunkSize' => ThurlyOSDisk\UploadFileManager::DEFAULT_CHUNK_SIZE,
		));
	}
	...
	/**
	 * @return ThurlyOSDisk\PageState|null
	 */
	private function getPageStateFromRequest()
	{
		...
		return ThurlyOSDisk\PageState::createFromSignedString($pageStateString);
	}
