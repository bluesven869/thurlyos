...
class Task
{
	...
	/** @var int */
	private $ownerId = \Thurly\Disk\SystemUser::SYSTEM_USER_ID;
	...
	public function fixState()
	{
		if ($this->getId() > 0)
		{
			...
			$result = \Thurly\Disk\Internals\VolumeTable::update($this->getId(), $taskParams);

			return $result->isSuccess();
		}

		$result = \Thurly\Disk\Internals\VolumeTable::add(array(
	 ...
	 * @return boolean
	 */
	public function loadTaskById($filterId, $ownerId = \Thurly\Disk\SystemUser::SYSTEM_USER_ID)
	{
		$filter = array(
			'=ID' => $filterId,
		);
		if ($ownerId != \Thurly\Disk\SystemUser::SYSTEM_USER_ID)
		{
	...
	public function hasUserCanceled()
	{
		...
		{
			$param = \Thurly\Disk\Internals\VolumeTable::getByPrimary($this->getId(), array('select' => array('AGENT_LOCK')))->fetch();
			if ($param)
			{
	 ...
	 * @param \Thurly\Disk\BaseObject|\Thurly\Disk\Version $object Object to delete.
	 * @return array
	 */
	public function collectLogData($object)
	{
		$logData = array();
		/** \Thurly\Disk\File $object */
		if ($object instanceof \Thurly\Disk\File)
		{
			$crumbs = \Thurly\Disk\CrumbStorage::getInstance()->getByObject($object, false);
			$logData = array(
				'OBJECT_TYPE' => \Thurly\Disk\Internals\ObjectTable::TYPE_FILE,
		...
		/** \Thurly\Disk\Folder $object */
		elseif ($object instanceof \Thurly\Disk\Folder)
		{
			$crumbs = \Thurly\Disk\CrumbStorage::getInstance()->getByObject($object, false);
			$logData = array(
				'OBJECT_TYPE' => \Thurly\Disk\Internals\ObjectTable::TYPE_FOLDER,
				'STORAGE_ID' => $object->getStorageId(),
				'OBJECT_ID' => $object->getId(),
				'OBJECT_PARENT_ID' => $object->getParentId(),
				'OBJECT_NAME' => $object->getName(),
		...
		/** \Thurly\Disk\Version $object */
		elseif ($object instanceof \Thurly\Disk\Version)
		{
			...
			{
				$crumbs = \Thurly\Disk\CrumbStorage::getInstance()->getByObject($file, false);
	...
	public function log($data, $operation)
	{
		...
		$result = \Thurly\Disk\Internals\VolumeDeletedLogTable::add($data);
		return $result->isSuccess();
	}
}
