...
class File extends Volume\Base implements Volume\IVolumeIndicatorParent, Volume\IVolumeIndicatorLink
{
	...
	public static function getTitle(Volume\Fragment $fragment)
	{
		...
		{
			throw new ArgumentTypeException('Fragment must be subclass of '.\Thurly\Disk\File::className());
		}
	...
	public static function getParents(Volume\Fragment $fragment)
	{
		...
		// Im
		if (in_array($fragment->getEntityType(), \Thurly\Disk\Volume\Module\Im::getEntityType()))
		{
			$imFragment = \Thurly\Disk\Volume\Module\Im::getFragment(array(
				'INDICATOR_TYPE' => Volume\Folder::className(),
				//'FOLDER_ID' => $file->getParentId()
				'FOLDER_ID' => $fragment->getFolderId(),
			));
			//$imFragment->getSpecific();
			$parents[] = \Thurly\Disk\Volume\Module\Im::getTitle($imFragment);
		}
		elseif ($parent = \Thurly\Disk\Folder::loadById($file->getParentId()))
		{
			$parents = \Thurly\Disk\CrumbStorage::getInstance()->getByObject($parent, true);
		}
	...
	public static function getAttachedList(Volume\Fragment $fragment, $userId)
	{
		...
		if ($fragment->getAttachedCount() > 0)
		{
			...
			if($attachedObjects)
			{
				...
				{
					if ($attachedObject instanceof \Thurly\Disk\AttachedObject)
					{
						try
						{
