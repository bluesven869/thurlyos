...
abstract class Base implements \Thurly\Disk\Volume\IVolumeIndicator, IErrorable
{
	/** @var array */
	...
	/** @var double */
	protected $ownerId = \Thurly\Disk\SystemUser::SYSTEM_USER_ID;
	...
	public function resetMeasurementResult()
	{
		...
		{
			\Thurly\Disk\Internals\VolumeTable::update(
				$this->getFilterId(),
				array(
					'FILE_SIZE' => 0,
					'FILE_COUNT' => 0,
	 ...
	 * @param \Thurly\Main\Entity\Base $entity Leave only fields for this entity.
	 * @return array
	 */
	public function getFilter(array $defaultFilter = array(), $entity = null)
	{
		...
		if ($entity instanceof \Thurly\Main\Entity\Base)
		{
	...
	{
		return $this->ownerId > 0 ? $this->ownerId : \Thurly\Disk\SystemUser::SYSTEM_USER_ID;
	}
	...
	public function getCorrespondingFileList($additionalFilter = array())
	{
		...
		$filterToFileList['=TYPE'] = \Thurly\Disk\Internals\ObjectTable::TYPE_FILE;

		if ($this instanceof \Thurly\Disk\Volume\Storage\TrashCan)
		{
		...
		{
			$filterToFileList['=DELETED_TYPE'] = \Thurly\Disk\Internals\ObjectTable::DELETED_TYPE_NONE;
		}

		$filterToFileList = array_merge($filterToFileList, $additionalFilter);

		$fileList = \Thurly\Disk\File::getList(array(
			'select'  => array('ID'),
			'filter'  => $filterToFileList,
			'runtime' => array(
				'IS_REAL_OBJECT' => new \Thurly\Main\Entity\ExpressionField(
					'IS_REAL_OBJECT',
	...
	public function getCorrespondingFolderList($additionalFilter = array())
	{
		...
		$filterToFolderList['=TYPE'] = \Thurly\Disk\Internals\ObjectTable::TYPE_FOLDER;
		...
		if (!isset($additionalFilter['!DELETED_TYPE']))
		{
			$filterToFolderList['=DELETED_TYPE'] = \Thurly\Disk\Internals\ObjectTable::DELETED_TYPE_NONE;
		}

		$filterToFolderList = array_merge($filterToFolderList, $additionalFilter);

		$folderList = \Thurly\Disk\Folder::getList(array(
			'select'  => array('ID'),
	...
	public function getCorrespondingUnnecessaryVersionList($additionalFilter = array())
	{
		...
		$querySql = "
			...
			FROM 
				...
				LEFT JOIN b_disk_version head ON head.OBJECT_ID = files.ID AND head.FILE_ID = files.FILE_ID /* head */
			
				LEFT JOIN b_disk_attached_object attached
					ON attached.OBJECT_ID  = ver.OBJECT_ID
					AND (@headId := ifnull(head.id, (SELECT max(ID) FROM b_disk_version WHERE head.OBJECT_ID = files.ID))) > 0 /* head id */
					AND ifnull(attached.VERSION_ID, @headId) = ver.ID
					AND ifnull(attached.VERSION_ID, @headId) != @headId
			
				LEFT JOIN b_disk_external_link link
					ON link.OBJECT_ID = ver.OBJECT_ID
					AND ifnull(link.VERSION_ID, @headId) = ver.ID
					AND ifnull(link.VERSION_ID, @headId) != @headId
					AND ifnull(link.TYPE, -1) != ". \Thurly\Disk\Internals\ExternalLinkTable::TYPE_AUTO. "
	...
	{
		$directory = new \Thurly\Main\IO\Directory(__DIR__. '/'. $libraryPath);
	 ...
	 * @param \Thurly\Main\Error $error Error message object.
	 * @return void
	 */
	public function addError(\Thurly\Main\Error $error)
	{
