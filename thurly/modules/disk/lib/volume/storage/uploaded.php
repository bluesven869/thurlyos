...
use Thurly\Disk\Internals\StorageTable;
...
class Uploaded extends Volume\Storage\Storage implements Volume\IVolumeIndicatorLink, Volume\IVolumeTimeLimit
{
	...
	public function measure($collectData = array())
	{
		...
		foreach ($filterStorage as $storageId)
		{
			...
			*/
			...
			{
				$measureResult = \Thurly\Disk\Internals\VolumeTable::getList(array(
					'filter' => array(
			...
			*/
			...
			{
				\Thurly\Disk\Internals\VolumeTable::add(array(
					'INDICATOR_TYPE' => static::className(),
					'OWNER_ID' =>	$ownerId,
					'STORAGE_ID' =>	$storageId,
					'FOLDER_ID' =>	$folderId,
			...
			\Thurly\Disk\Internals\VolumeTable::clearTemporally();
		}
	...
	{
		$filter['=ENTITY_TYPE'] = \Thurly\Disk\ProxyType\User::className();
	...
	{
		$filter['=STORAGE.ENTITY_TYPE'] = \Thurly\Disk\ProxyType\User::className();
		$filter['=CODE'] = \Thurly\Disk\Folder::CODE_FOR_UPLOADED_FILES;

		$result = \Thurly\Disk\Folder::getList(array(
