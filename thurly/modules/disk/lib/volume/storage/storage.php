...
class Storage extends Volume\Base implements Volume\IVolumeIndicatorStorage, Volume\IVolumeIndicatorLink
{
	 ...
	 * @return self
	 */
	public function recalculatePercent($totalSizeIndicator = '\\Thurly\\Disk\\Volume\\Module\\Disk', $excludeSizeIndicator = '')
	{
	...
	public static function getTitle(Volume\Fragment $fragment)
	{
		...
		if ($fragment->getEntityType() == \Thurly\Disk\ProxyType\User::className())
		{
			$user = \Thurly\Disk\User::loadById($fragment->getEntityId());
			if ($user instanceof \Thurly\Disk\User && $user->getActive() !== 'Y')
			{
				// user fired
	...
	public static function getUrl(Volume\Fragment $fragment)
	{
		...
		if (in_array($storage->getEntityType(), \Thurly\Disk\Volume\Module\Im::getEntityType()))
		{
		...
		if (
			$testUrl == '' ||
			$testUrl == \Thurly\Disk\ProxyType\Base::SUFFIX_FOLDER_LIST ||
	...
	public static function getAvailableSpace(\Thurly\Disk\Storage $storage = null)
	{
		...
		{
			$diskSpace = (float)\Thurly\Main\Config\Option::get('main', 'disk_space', -1);
			if ($diskSpace > 0)
			{
				$diskSpace *= 1024 * 1024;
			}
