...
use Thurly\Main\ObjectPropertyException;
...
abstract class Module extends Volume\Base implements Volume\IVolumeIndicatorModule
{
		...
		$buildExternalSql = function(&$selectSql, &$fromSql, &$whereSql, &$columns, $subSelectSql = '', $subWhereSql = '')
		{
			...
			$fromSql .= "
				...
				(
					...
					WHERE
						files.TYPE = ".ObjectTable::TYPE_FILE."
						AND link.TYPE != ". \Thurly\Disk\Internals\ExternalLinkTable::TYPE_AUTO. "
						AND files.ID = files.REAL_OBJECT_ID
						{$subWhereSql}
				) CNT_LINK
					...
					(
						...
						FROM 
							...
							LEFT JOIN b_disk_attached_object  attached
								...
								AND (@headId := ifnull(head.id, (SELECT max(ID) FROM b_disk_version WHERE OBJECT_ID = files.ID))) > 0 /* head id */
	 ...
	 * @return self
	 */
	public function recalculatePercent($totalSizeIndicator = '\\Thurly\\Disk\\Volume\\Bfile', $excludeSizeIndicator = null)
	{
		...
		{
			throw new \Thurly\Main\ArgumentException('Wrong parameter totalSizeIndicator');
		}
	...
	{
		$isAvailable =
			\Thurly\Main\ModuleManager::isModuleInstalled(self::getModuleId()) &&
	...
	public function getUserTypeFieldList(
		$entityClass,
		$userTypeField = array(\CUserTypeFile::USER_TYPE_ID, \Thurly\Disk\Uf\FileUserType::USER_TYPE_ID, \Thurly\Disk\Uf\VersionUserType::USER_TYPE_ID)
	)
	{
		static $fields = array();
		...
		if (!isset($fields[$entityClass]))
		{
			...
			/** @var \Thurly\Main\Entity\DataManager $entityClass */
			...
			if (strlen($ufName) > 0 && count($userTypeField) > 0)
			{
				...
				$userFieldList = \Thurly\Main\UserFieldTable::getList(array(
					'filter' => $filter,
					'select' => array(
						'ID',
						'ENTITY_ID',
	...
	protected function prepareUserFieldQuery($entityClass, array $userField, array $relation = null)
	{
		...
		if ($userField['MULTIPLE'] == 'Y')
		{
			...
			if ($connection->isTableExists($utmEntityTableName))
			{
				switch ($ufType)
				{
					case \Thurly\Disk\Uf\FileUserType::USER_TYPE_ID:
					{
						$querySql = "
							SELECT
					...
					case \Thurly\Disk\Uf\VersionUserType::USER_TYPE_ID:
					{
						$querySql = "
							SELECT
								SUM(FILE_SIZE) as FILE_SIZE,
