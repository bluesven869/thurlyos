...
use Thurly\Disk\Integration\TransformerManager;
use Thurly\Disk\Internals\AttachedObjectTable;
use Thurly\Disk\Internals\EditSessionTable;
...
class File extends BaseObject
{
	 ...
	 * @return \Thurly\Disk\Internals\Model|static|null
	 * @throws \Thurly\Main\NotImplementedException
	 * @internal
	 */
	...
	public function getLock()
	{
		...
		$this->lock = ObjectLock::load(array('OBJECT_ID' => $this->id));
	 ...
	 * @param bool $disableJoin If set false the method attempts to join version with last version (@see \Thurly\Disk\File::SECONDS_TO_JOIN_VERSION).
	 * @return Version|null
	 ...
	 * @see \Thurly\Disk\File::addVersion().
	 ...
	 *
	 * The method is similar with (@see Thurly\Disk\File::addVersion()).
	 *
	...
	public function restore($restoredBy)
	{
		...
		if($status)
		{
			...
			//it's necessary to reset cache because in default way \Thurly\Disk\Driver::sendChangeStatusToSubscribers
			...
			Driver::getInstance()->cleanCacheTreeThurlyDisk(array_keys($subscribers));
		}
	...
	public function getView()
	{
		if(!$this->view)
		{
			...
			{
				$this->view = new View\Document($this->getName(), $this->getFileId(), $this->getViewId(), $this->getPreviewId());
			}
			elseif(TypeFile::isVideo($this))
			{
				$this->view = new View\Video($this->getName(), $this->getFileId(), $this->getViewId(), $this->getPreviewId());
			}
			else
			{
				$this->view = new View\Base($this->getName(), $this->getFileId(), $this->getViewId(), $this->getPreviewId());
			}
		}
