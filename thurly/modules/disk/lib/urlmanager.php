...
use Thurly\Disk\Internals\Diag;
...
use Thurly\Disk\Security\ParameterSigner;
use Thurly\Main\Config\Option;
use Thurly\Main\Context;
use Thurly\Main\InvalidOperationException;
...
class UrlManager implements IErrorable
{
	 ...
	 * @param string $typeFile Type of file (docx, pptx, xlsx). See in \Thurly\Disk\Document\BlankFileData (docx, pptx, xslx).
	 * @param string $service Service which will use to create file. See in subclasses \Thurly\Disk\Document\DocumentHandler::getCodeName, and event subscribers 'onDocumentHandlerBuildList'.
	 * @return string
	 */
	...
	public static function getUrlUfController($action, array $params = array())
	{
		...
		$params['ncc'] = 1;
		return '/thurly/tools/disk/uf.php?' . http_build_query($params);
	}
	...
	public static function getUrlFocusController($action, array $params = array(), $absolute = false)
	{
		...
		return $prefix . '/thurly/tools/disk/focus.php?' . http_build_query($params);
	}
	...
	{
		$params['action'] = $action;
		return '/thurly/tools/disk/document.php?' . http_build_query($params);
	}
	...
	public function getUrlForShowPreview(File $file, array $params = array(), $absolute = false)
	{
		...
		$params['ts'] = $file->getUpdateTime()->getTimestamp();
		return $this->getUrlDownloadController('showPreview', array_merge($params, array('fileId' => $file->getId(), 'filename' => $file->getView()->getPreviewName())), $absolute);
	}
	...
	public function getUrlDownloadController($action, array $params = array(), $absolute = false)
	{
		...
		{
			/** @noinspection PhpDynamicAsStaticMethodCallInspection */
			$accessPoints = \CUrlRewriter::getList(array('ID' => 'thurly:disk.services'));
