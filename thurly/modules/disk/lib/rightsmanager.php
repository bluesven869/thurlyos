...
use Thurly\Disk\Internals\Rights\SetupSession;
use Thurly\Disk\Internals\Rights\Table\TmpSimpleRightTable;
...
class RightsManager implements IErrorable
{
	...
	/**
	 * Rewrite if exists.
	 * @param \Thurly\Disk\BaseObject|BaseObject $object
	 * @param array                      $rights
	...
	private function insertRightsInternal(BaseObject $object, array $rights)
	{
		...
		foreach ($rights as $right)
		{
			...
			{
				$right['NEGATIVE'] = 0; //default value @see \Thurly\Disk\Internals\RightTable
			}
...
final class SimpleReBuilder
{
	...
	private function runByOnceObject()
	{
		...
		TmpSimpleRightTable::insertBatchBySessionId($items, $this->setupSession->getId());
	}
	...
	private function fillChildren()
	{
		...
		if(!$childrenRights)
		{
			TmpSimpleRightTable::fillDescendants($this->object->getId(), $this->setupSession->getId());
			return;
		}
