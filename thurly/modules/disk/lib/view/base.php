...
class Base
{
	...
	public function transformOnOpen(File $file)
	{
		if($this->isTransformationAllowedOnOpen($file->getSize()))
		{
			...
			{
				if(!\Thurly\Disk\Integration\TransformerManager::checkTransformationAttempts($file))
				{
					\Thurly\Disk\Integration\TransformerManager::transformToView($file);
	...
	public function isLastTransformationFailed()
	{
		if(Loader::includeModule('transformer'))
		{
			$info = \Thurly\Transformer\FileTransformer::getTransformationInfoByFile((int)$this->fileId);
			if(!$info || $info['status'] > \Thurly\Transformer\Command::STATUS_SUCCESS)
			{
				return true;
			}
		}
