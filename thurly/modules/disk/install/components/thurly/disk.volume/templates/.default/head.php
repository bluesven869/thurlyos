...
CJSCore::Init(array('disk', 'viewer'));
...
if ($isThurlyOSTemplate)
{
	...
	if ($arResult["ADMIN_MODE_ALLOW"] || $arResult["WORKER_COUNT"] > 0)
	{
		...
		<div id="bx-disk-volume-menu" class="pagetitle-container pagetitle-align-right-container">
			...
			<div id="bx-disk-volume-popupMenuOptions" class="webform-small-button webform-small-button-transparent task-list-toolbar-lightning webform-cogwheel">
	...
	{
		?>
		<a href="<?= $component->getActionUrl(array('reload' => 'Y', 'action' => $component::ACTION_DEFAULT)); ?>" class="webform-small-button webform-small-button-blue disk-volume-reload-link">
			<?= Loc::getMessage('DISK_VOLUME_MEASURE_DATA_REPEAT') ?>
		</a>
		<?
	}
...
if ($isThurlyOSTemplate && !$arResult['DISK_EMPTY'] && !$isQueueRunning && $arResult['DATA_COLLECTED'])
{
...
<script type="text/javascript">
	...
	BX(function () {
		...
		<? foreach ($arResult['SCAN_ACTION_LIST'] as $item): ?>
			BX.Disk.measureManager.addQueueItem(<?= \Thurly\Main\Web\Json::encode((object)$item) ?>);
		<? endforeach; ?>
		...
		<? if ($isQueueRunning): ?>
			...
			<? elseif ($arResult['RUN_QUEUE'] === 'continue'): ?>
				BX.ready( function() {
					...
					BX.Disk.measureManager.runQueue(<?= $arResult['QUEUE_STEP']['queueStep'] ?>, <?= \Thurly\Main\Web\Json::encode($arResult['QUEUE_STEP']) ?>);
				});
			<? endif; ?>
		<? endif; ?>
...
)
{
	...
	<div class="disk-volume-wrap">
		...
		else
		{
			...
			<div class="disk-volume-header-container <?=($arResult['Storage']['IS_EXTRANET'] ? 'disk-volume-header-container-extranet' : '')?>">
				...
				<div class="disk-volume-header-block">
					...
					<? else: ?>
						<div class="disk-volume-header-title"><?= Loc::getMessage("DISK_VOLUME_DISK") ?> <?= $arResult['Storage']['TITLE']; ?></div>
			...
			<? if(count($arResult['ACTION_MENU']) > 0):?>
				<span id="bx-disk-volume-popupMenuStorage" class="webform-small-button task-small-button-menu webform-small-button-transparent">
		...
		if (!$isTrashcan)
		{
			$plotIndicatorId = \Thurly\Disk\Volume\FileType::getIndicatorId();
