...
else if (strpos($this->__page, "show") === 0)
{
	...
	foreach ($arResult['FILES'] as &$file)
	{
		if ($this->__page == "show_inline" && array_key_exists("IMAGE", $file) &&
			...
			($fileModel = \Thurly\Disk\File::loadById($file["FILE_ID"])) && $fileModel
		)
		{
			...
			{
				$file["INLINE"] = array(
					"src" => \Thurly\Disk\Driver::getInstance()->getUrlManager()->getUrlExternalLink(
						array(
		...
		if ((list($replyTo, $url) = \Thurly\Mail\User::getReplyTo(
				$arParams["SITE_ID"],
