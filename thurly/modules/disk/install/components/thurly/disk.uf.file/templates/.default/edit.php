...
CJSCore::Init(array('file_dialog', 'ajax', 'dd', "core", "uploader", "canvas", "disk_external_loader", 'viewer'));
\Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/js/disk/css/legacy_uf_common.css');
...
foreach ($arResult['FILES'] as $file)
{
	if (array_key_exists("IMAGE", $file))
	{
		CFile::ScaleImage(
			$file["IMAGE"]["WIDTH"],
			$file["IMAGE"]["HEIGHT"],
			\Thurly\Disk\Uf\Controller::$previewParams,
...
?>
				<tr class="wd-inline-file" id="disk-edit-attach<?=$file['ID']?>" bx-attach-file-id="<?=\Thurly\Disk\Uf\FileUserType::NEW_FILE_PREFIX?><?=$file['FILE_ID']?>"<?
...
BX.ready(function(){
	...
	BX.Disk.UF.add({
		...
		urlSelect : '<?=CUtil::JSEscape('/thurly/tools/disk/uf.php?action=selectFile&SITE_ID=')?>' + BX.message('SITE_ID'),
		urlRenameFile : '<?=CUtil::JSEscape('/thurly/tools/disk/uf.php?action=renameFile')?>',
		urlDeleteFile : '<?=CUtil::JSEscape('/thurly/tools/disk/uf.php?action=deleteFile')?>',
		urlUpload : '<?= CUtil::JSUrlEscape($arResult['UPLOAD_FILE_URL']) ?>'
	});
});
BX.message({
	DISK_FOLDER_TOOLBAR_LABEL_LOCAL_BDISK_EDIT: '<?= CUtil::JSEscape(\Thurly\Disk\Document\LocalDocumentController::getName()) ?>',
...
<?
if(\Thurly\Disk\Integration\ThurlyOSManager::isEnabled() && \Thurly\Disk\Integration\ThurlyOSManager::isLicensePaid())
{
	\Thurly\Disk\Integration\ThurlyOSManager::initLicenseInfoPopupJS('disk_transformation_video_limit');
}
?>
