...
class CDiskUfFileComponent extends BaseComponent
{
	...
	protected function processActionDefault()
	{
		...
		if($this->arResult['ENABLED_MOD_ZIP'] && !$this->editMode)
		{
			...
			$this->arResult['DOWNLOAD_ARCHIVE_URL'] = $urlManager->getUrlUfController('downloadArchiveByEntity', array(
				...
				'signature' => \Thurly\Disk\Security\ParameterSigner::getEntityArchiveSignature(
	...
	private function loadFilesData()
	{
		...
		foreach($values as $id)
		{
			...
			{
				/** @var \Thurly\Disk\AttachedObject $attachedModel */
			...
			if(\Thurly\Disk\TypeFile::isImage($fileModel))
			{
			...
			if(\Thurly\Disk\TypeFile::isVideo($fileModel))
			{
				if($fileModel->getView()->isHtmlAvailable())
				{
					$maxWidth = \Thurly\Main\Config\Option::get('blog', 'image_max_width', 800);
