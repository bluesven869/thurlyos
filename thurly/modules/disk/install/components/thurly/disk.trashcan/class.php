...
class CDiskTrashCanComponent extends DiskComponent
{
	...
	protected function processActionDefault()
	{
		...
		$gridOptions = new Thurly\Disk\Internals\Grid\TrashCanOptions($this->storage);
		$gridId = $gridOptions->getGridId();
	...
	protected function findFolder()
	{
		...
		if(!$this->folder)
		{
			throw new \Thurly\Main\SystemException("Invalid folder.");
		}
		return $this;
	}
	...
	private function getGridData($gridId)
	{
		...
		{
			$parameters['filter']['DELETED_TYPE'] = ObjectTable::DELETED_TYPE_ROOT;
			$items = $this->folder->getDescendants($securityContext, $parameters, null);
		}
		else
		{
			$parameters['filter']['DELETED_TYPE'] = ObjectTable::DELETED_TYPE_CHILD;
			$items = $this->folder->getChildren($securityContext, $parameters);
		}
		...
		foreach ($items as $object)
		{
			...
			{
				$uri = \Thurly\Disk\Driver::getInstance()->getUrlManager()->encodeUrn($detailPageFile);
			}
			$iconClass = \Thurly\Disk\Ui\Icon::getIconClassByObject($object);
