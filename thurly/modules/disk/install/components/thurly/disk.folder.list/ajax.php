...
use Thurly\Disk\FileLink;
use Thurly\Disk\Folder;
use Thurly\Disk\FolderLink;
...
class DiskFolderListAjaxController extends \Thurly\Disk\Internals\Controller
{
	...
	protected function processActionShowCreateFolderWithSharingInCommon($storageId)
	{
		...
		if(!($storage->getProxyType() instanceof Thurly\Disk\ProxyType\Common) || !$storage->getRootObject()->canAdd($securityContext))
		{
	...
	protected function processActionShowRightsOnStorageDetail($storageId)
	{
		...
		if($storage->getProxyType() instanceof Thurly\Disk\ProxyType\User && !User::isCurrentUserAdmin())
		{
			$readOnlyAccessCodes['IU' . $storage->getEntityId()] = true;
			$readOnlyAccessCodes['U' . $storage->getEntityId()] = true;
		}
		elseif($storage->getProxyType() instanceof Thurly\Disk\ProxyType\Group)
		{
	...
	{
		/** @var \Thurly\Disk\File|\Thurly\Disk\Folder $object */
	...
	protected function processActionAppendSharing($objectId)
	{
		...
		if(!empty($entityToNewShared) && is_array($entityToNewShared))
		{
			...
			foreach($extendedRights as $entityId => $right)
			{
				switch($right['right'])
				{
					case \Thurly\Disk\RightsManager::TASK_READ:
					case \Thurly\Disk\RightsManager::TASK_ADD:
					case \Thurly\Disk\RightsManager::TASK_EDIT:
					case \Thurly\Disk\RightsManager::TASK_FULL:
						$newExtendedRightsReformat[$entityId] = $right['right'];
						break;
				}
			}
		...
		if(!empty($entityToNewShared) && is_array($entityToNewShared))
		{
			...
			if($needToOverwrite)
			{
				...
				foreach($needToOverwrite as $sharingRow)
				{
					...
					{
						/** @var \Thurly\Disk\Sharing $sharingModel */
	...
	protected function processActionCopyTo($objectId, $targetObjectId)
	{
		...
		/** @var \Thurly\Disk\Folder $targetObject */
	...
	protected function processActionShowSubFoldersToAdd($objectId)
	{
		...
		unset($subFolder);
		\Thurly\Main\Type\Collection::sortByColumn($subFolders, 'name');
	...
	{
		/** @var File|Folder $object */
		$object = \Thurly\Disk\BaseObject::loadById((int)$objectId, array('REAL_OBJECT.STORAGE'));
		...
		if(!$object->canShare($securityContext) && !$object->canChangeRights($securityContext))
		{
			...
			$entityList = array(
				array(
					'entityId' => Sharing::CODE_USER . $this->getUser()->getId(),
					'name' => $user->getFormattedName(),
					'right' => \Thurly\Disk\RightsManager::TASK_READ,
					'avatar' => $user->getAvatarSrc(),
					'type' => 'users',
				)
			);
	...
	protected function processActionGetUrlToDownloadArchive($folderId, array $objectIds)
	{
		...
		$this->sendJsonSuccessResponse(array(
			'downloadArchiveUrl' => \Thurly\Disk\Driver::getInstance()->getUrlManager()->getUrlDownloadController('downloadArchive', array(
				'fileId' => 0,
				'objectIds' => $fileIds,
				'signature' => \Thurly\Disk\Security\ParameterSigner::getArchiveSignature($fileIds),
			)),
		));
	}
 ...
 * @Internals
 */
final class DataSetOfRights extends \Thurly\Disk\Type\DataSet
{
