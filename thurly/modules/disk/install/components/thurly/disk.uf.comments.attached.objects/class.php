...
use Thurly\Disk\Document\GoogleHandler;
use Thurly\Disk\Document\LocalDocumentController;
use Thurly\Disk\Internals\BaseComponent;
use Thurly\Disk\Document\DocumentHandler;
use Thurly\Disk\Ui;
...
class CDiskUfCommentsAttachesComponent extends BaseComponent
{
	...
	protected function processActionDefault()
	{
		...
		{
			$documentServiceCode = \Thurly\Disk\UserConfiguration::getDocumentServiceCode();
			...
			{
				$defaultDocumentHandler = \Thurly\Disk\Driver::getInstance()
		...
		if($this->arResult['ENABLED_MOD_ZIP'])
		{
			...
			$this->arResult['DOWNLOAD_ARCHIVE_URL'] = \Thurly\Disk\Driver::getInstance()->getUrlManager()->getUrlUfController('downloadArchive', array(
				'attachedIds' => $this->arResult['ATTACHED_IDS'],
				'signature' => \Thurly\Disk\Security\ParameterSigner::getArchiveSignature($this->arResult['ATTACHED_IDS']), 
			));
		}
	...
	private function prepareObjectToResult(array $attachedObjects)
	{
		...
		foreach($attachedObjects as $attachedObject)
		{
			...
			if(\Thurly\Disk\TypeFile::isImage($file))
			{
		...
		\Thurly\Main\Type\Collection::sortByColumn($files, 'ID');
	...
	private function getAttachedObjectByForum(array $commentsData)
	{
		...
		if(!\Thurly\Main\Loader::includeModule($moduleId))
		{
			return array();
		}
