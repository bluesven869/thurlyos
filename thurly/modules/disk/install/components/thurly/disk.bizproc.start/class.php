...
class CDiskBizprocStartComponent extends BaseComponent
{
	...
	protected function prepareParams()
	{
		...
		$this->arResult['DOCUMENT_DATA'] = array(
			'DISK' => array(
				'DOCUMENT_TYPE' => \Thurly\Disk\BizProcDocument::generateDocumentComplexType($this->arParams['STORAGE_ID']),
				'DOCUMENT_ID' => \Thurly\Disk\BizProcDocument::getDocumentComplexId($this->arParams['DOCUMENT_ID']),
			),
			'WEBDAV' => array(
				'DOCUMENT_TYPE' => \Thurly\Disk\BizProcDocumentCompatible::generateDocumentComplexType($this->arParams['STORAGE_ID']),
				'DOCUMENT_ID' => \Thurly\Disk\BizProcDocumentCompatible::getDocumentComplexId($this->arParams['DOCUMENT_ID']),
			),
		);
	...
	protected function getData()
	{
			...
			while ($workflowTemplate = $workflowTemplateObject->getNext())
			{
				...
				if($nameModule == 'DISK')
				{
					...
					$this->arResult['TEMPLATES'][$workflowTemplate['ID']]['URL'] =
						htmlspecialcharsex($this->application->getCurPageParam(
							'workflow_template_id='.$workflowTemplate['ID'].'&'.thurly_sessid_get(),
				...
				else
				{
					...
					$this->arResult['TEMPLATES_OLD'][$workflowTemplate['ID']]['URL'] =
						htmlspecialcharsex($this->application->getCurPageParam(
							'workflow_template_id='.$workflowTemplate['ID'].'&old=1&'.thurly_sessid_get(),
							array('workflow_template_id', 'sessid')));
				}
			...
			&& (array_key_exists($this->arParams['TEMPLATE_ID'], $this->arResult['TEMPLATES']) || array_key_exists($this->arParams['TEMPLATE_ID'], $this->arResult['TEMPLATES_OLD'])))
		{
			...
			elseif ($this->request->isPost() && strlen($this->request->getPost('DoStartParamWorkflow')) > 0 && check_thurly_sessid())
			{
			...
			{
				$doStartParam = ($this->request->isPost() && strlen($this->request->getPost('DoStartParamWorkflow') && check_thurly_sessid()) > 0);
