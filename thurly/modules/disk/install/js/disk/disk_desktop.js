...
BX.Disk.Desktop.Settings = (function ()
{
	...
	Settings.prototype.buildFolderList = function (rootFolders, type)
	{
		...
		var html =
			'<ul id="bx-disk-sync-folder-' + type + '" class="bx-im-disk-list-block" ' + (type == 'shared'? 'style="display:none"' : '') + '>' +
				'<li class="bx-im-disk-list-item">' +
					'<input onclick="ThurlyDiskSettings.onClickSyncAllFolder(this);" name="sync-all-' + type + '" data-save=1 type="checkbox" ' + (allToSync? 'checked="checked"' : '') + ' class="bx-im-disk-checkbox sync-all-folder" id="bx-disk-sync-all-folder' + type + '"><label class="bx-im-disk-label" for="bx-disk-sync-all-folder' + type + '">' + BX.message('DISK_DESKTOP_JS_SETTINGS_SYNC_ALL') + '</label>' +
				'</li>'
		;
		for (var i = 0, length = rootFolders.length; i < length; i++)
		{
			...
			html +=
				'<li class="bx-im-disk-list-item">' +
					'<input onclick="ThurlyDiskSettings.onClickSyncFolder(this);" name="sync-' + name + '" data-save=1 type="checkbox" ' + (folder.isSynced? 'checked="checked"' : '') + ' class="bx-im-disk-checkbox" id="f-' + name + '"><label class="bx-im-disk-label" for="f-' + name + '">' + name + '</label>' +
				'</li>'
			;
		}
		...
		if(BX('bx-im-disk-folders-setting-block-wrap'))
		{
			BX('bx-im-disk-folders-setting-block-wrap').innerHTML =
				'<div class="bx-im-disk-btn-wrap">' +
					'<span id="bx-disk-header-sync-folder-own" onclick="ThurlyDiskSettings.onClickTab(\'own\', \'shared\');" class="bx-im-disk-btn bx-im-disk-btn-active"><span class="bx-disk-tab-btn-text">' + BX.message('DISK_DESKTOP_JS_SETTINGS_SYNC_TAB_OWNER') + '</span></span><span id="bx-disk-header-sync-folder-shared"  onclick="ThurlyDiskSettings.onClickTab(\'shared\', \'own\');" class="bx-im-disk-btn"><span class="bx-disk-tab-btn-text">' + BX.message('DISK_DESKTOP_JS_SETTINGS_SYNC_TAB_SHARED') + '</span></span>' +
				'</div>' +
				ownFolderHtml +
				sharedFolderHtml +
