...
<tr>
	...
	<td width="60%">
		...
		{
			$object = \Thurly\Disk\BaseObject::loadById($arCurrentValues['source_id']);
			if ($object)
			{
...
<tr>
	...
	<td width="60%">
		<select name="entity_type" onchange="BPDUA_changeEntityType(this.value);">
			...
			<option value="folder" <?if ($arCurrentValues['entity_type'] == 'folder') echo 'selected'?>><?=GetMessage('BPDCM_PD_ENTITY_TYPE_FOLDER')?></option>
		</select>
	</td>
</tr>
...
<tr id="id_entity_id_common" <?if ($arCurrentValues['entity_type'] != 'common') echo 'style="display: none"'?>>
	...
	<td width="60%">
		...
		<select name="entity_id_common">
			<option value=""><?= GetMessage("BPDCM_PD_LABEL_CHOOSE") ?></option>
			<?
			$iterator = \Thurly\Disk\Storage::getList(array(
				'select' => array('NAME', 'ENTITY_ID', 'SITE_ID'),
				'filter' => array('=ENTITY_TYPE' => \Thurly\Disk\ProxyType\Common::className()),
...
<tr id="id_entity_id_folder" <?if ($arCurrentValues['entity_type'] != 'folder') echo 'style="display: none"'?>>
	...
	<td width="60%">
		...
		{
			$folder = \Thurly\Disk\Folder::loadById($value);
			if ($folder)
			{
...
<script>
	...
	{
		var urlSelect = '/thurly/tools/disk/uf.php?action=selectFile&dialog2=Y&SITE_ID=' + BX.message('SITE_ID');
