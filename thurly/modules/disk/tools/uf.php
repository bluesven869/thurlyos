<?php
use Thurly\Disk\Uf\LocalDocumentController;
...
if(!empty($_GET['document_action']) && !empty($_GET['service']))
{
	...
	{
		$docController = new \Thurly\Disk\Uf\DocumentController();
		$docController
...
if($oauthToken && \Thurly\Main\Loader::includeModule('rest'))
{
	$authResult = null;
	if(\CRestUtil::checkAuth(
		$oauthToken,
		\Thurly\Disk\Driver::INTERNAL_MODULE_ID,
		$authResult
	))
	{
