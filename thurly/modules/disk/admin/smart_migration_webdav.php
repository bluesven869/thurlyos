...
use Thurly\Disk\Internals\FolderTable;
use Thurly\Disk\Internals\SharingTable;
use Thurly\Disk\Ui\Text;
use Thurly\Main\Application;
use Thurly\Disk\ProxyType;
use Thurly\Main\Config\Option;
use Thurly\Disk\RightsManager;
...
class SmartMigrationWebdavLogger extends \Thurly\Main\Diag\FileExceptionHandlerLog
{

	const MAX_LOG_SIZE = 10000000;
	const DEFAULT_LOG_FILE = "thurly/modules/disk_convertor.log";
...
class SmartMigrationWebdav
{
	...
	public function __construct(array $options = array())
	{
		...
		$this->isOracle = $this->connection instanceof \Thurly\Main\DB\OracleConnection;
		$this->isMysql = $this->connection instanceof \Thurly\Main\DB\MysqlCommonConnection;
		$this->isMssql = $this->connection instanceof \Thurly\Main\DB\MssqlConnection;
	...
	protected function moveGroupStorageFromIblock(array $iblock)
	{
		...
		{
			$classDocument = $sqlHelper->forSql(\Thurly\Disk\BizProcDocumentCompatible::className());
			if($this->isOracle)
			{
				$this->connection->queryExecute("
	...
	protected function migrateDataCommonStorages()
	{
		...
		foreach($this->getIblockIdsWithCommonFiles() as $iblock)
		{
			...
			if($this->runWorkWithBizproc)
			{
				$classDocument = $this->sqlHelper->forSql(\Thurly\Disk\BizProcDocumentCompatible::className());
	...
	protected function convertBadNames()
	{
		...
		if($this->isMysql)
		{
			$sql = "
				SELECT NAME, ID
				FROM b_disk_object
				WHERE NAME REGEXP CONCAT('[', x'01', '-', x'1F', '" . $sqlHelper->forSql(Thurly\Main\IO\Path::INVALID_FILENAME_CHARS) . "]')
			";
		}
		elseif($this->isOracle)
		{
			$sql = "
				SELECT NAME, ID
				FROM b_disk_object
				WHERE REGEXP_LIKE(NAME, '[" . $sqlHelper->forSql(Thurly\Main\IO\Path::INVALID_FILENAME_CHARS) . "]')
			";
		}
		elseif($this->isMssql)
		{
			$sql = "
				SELECT NAME, ID
				FROM b_disk_object
				WHERE PATINDEX('%[" . $sqlHelper->forSql(Thurly\Main\IO\Path::INVALID_FILENAME_CHARS) . "]%', NAME) != 0
			";
		}
		...
		while($query && $row = $query->fetch())
		{
			try
			{
				$this->connection->queryExecute("UPDATE b_disk_object SET NAME='".$sqlHelper->forSql(\Thurly\Disk\Ui\Text::correctFilename($row['NAME']))."' WHERE ID=".$row['ID']);
			}
			catch(Exception $e)
			{
				$this->connection->queryExecute("UPDATE b_disk_object SET NAME='".$sqlHelper->forSql(Thurly\Main\IO\Path::randomizeInvalidFilename($row['NAME']))."' WHERE ID=".$row['ID']);
			}
		}
	...
	protected function addMissedGroupStorages()
	{
		...
		while($row = $query->fetch())
		{
			\Thurly\Disk\Driver::getInstance()->addGroupStorage($row['ID']);
	...
	protected function fillObjectRightsMoveTasks()
	{
		...
		\Thurly\Main\Type\Collection::sortByColumn($tasks, 'TASK_ID');
	...
	protected function migrateVersion()
	{
		...
		$useGZipCompressionOption = \Thurly\Main\Config\Option::get("bizproc", "use_gzip_compression", "");

		$isThurlyOS = IsModuleInstalled('thurlyos');
		$bucket = null;
		if($isThurlyOS && $isCloud)
		{
		...
		while($version = $versionQuery->fetch())
		{
			...
			if(substr($fullPath, 0, 4) == "http")
			{
				...
				$contentType = \Thurly\Disk\TypeFile::getMimeTypeByFilename($filename);
			}
			else
			{
	...
	private function getContentType($fullPath, $filename)
	{
		...
		return \Thurly\Disk\TypeFile::normalizeMimeType($contentType, $filename);
	}
...
//		}
	...
	protected function migrateMetaFolders()
	{
		...
		$proxyType = $sqlHelper->forSql(Thurly\Disk\ProxyType\User::className());
		$names = array();
		...
		Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'] . '/thurly/modules/disk/lib/storage.php');
	...
	protected function getNamesDroppedSection()
	{
		...
		{
			\Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'] . '/thurly/modules/webdav/classes/general.php', $lang["LID"]);
			$names[$lang["LID"]] = \Thurly\Main\Localization\Loc::getMessage('WD_DOWNLOADED', null, $lang["LID"]);
		}
		unset($lang);
	...
	protected function getNamesSavedSection()
	{
		...
		{
			\Thurly\Main\Localization\Loc::loadLanguageFile($_SERVER['DOCUMENT_ROOT'] . '/thurly/modules/webdav/classes/general.php', $lang["LID"]);
			$names[$lang["LID"]] = \Thurly\Main\Localization\Loc::getMessage('WD_SAVED', null, $lang["LID"]);
		}
		unset($lang);
