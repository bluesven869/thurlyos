...
Class mail extends CModule
{
	...
	function InstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/db/".strtolower($DB->type)."/install.sql");
		}
		...
		else
		{
			...
			{
				$result = Thurly\Mail\MailServicesTable::getList(array('filter' => array('ACTIVE' => 'Y')));
				if ($result->fetch() === false)
				{
					...
					$result = \Thurly\Main\SiteTable::getList();
					while (($site = $result->fetch()) !== false)
					{
						...
						foreach ($mailServicesList as $serviceSort => $serviceName)
						{
							...
							Thurly\Mail\MailServicesTable::add($serviceSettings);
						}
					}
				}
				...
				$arGroup = array(
					...
					"TASKS_FILE" => array(
						Array("fm_folder_access_read", "/thurly/components/thurly/"),
						Array("fm_folder_access_read", "/thurly/rk.php"),
						Array("fm_folder_access_read", "/thurly/tools/"),
			...
			CAgent::addAgent('Thurly\Mail\Helper::resyncDomainUsersAgent();', 'mail', 'N', 60*60*24);

			return true;
		}
	}

	public function installThurlyOSMailService()
	{
		if (CModule::IncludeModule("mail"))
		{
			$result = \Thurly\Main\SiteTable::getList();
			while (($site = $result->fetch()) !== false)
			{
				...
				\Thurly\Mail\MailServicesTable::add(array(
					'SITE_ID'      => $site['LID'],
					'ACTIVE'       => 'Y',
					'NAME'         => 'thurlyos',
	...
	function UnInstallDB($arParams = array())
	{
		...
		{
			$this->errors = $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/db/".strtolower($DB->type)."/uninstall.sql");
		}
	...
	function InstallFiles($arParams = array())
	{
		...
		{
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/images", $_SERVER["DOCUMENT_ROOT"]."/thurly/images/mail", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/admin", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin", true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/tools", $_SERVER["DOCUMENT_ROOT"]."/thurly/tools", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/themes", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes", true, true);
			CopyDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/templates", $_SERVER["DOCUMENT_ROOT"]."/thurly/templates", true, true);
		}
		return true;
	}
	...
	{
		DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/admin/", $_SERVER["DOCUMENT_ROOT"]."/thurly/admin");
		DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/tools/", $_SERVER["DOCUMENT_ROOT"]."/thurly/tools");
		DeleteDirFiles($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/themes/.default/", $_SERVER["DOCUMENT_ROOT"]."/thurly/themes/.default");//css
		DeleteDirFilesEx("/thurly/themes/.default/icons/mail/");//icons
		DeleteDirFilesEx("/thurly/images/mail/");//images
		return true;
	}
	...
	function DoInstall()
	{
		...
		$APPLICATION->IncludeAdminFile(GetMessage("MAIL_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/step1.php");
	}
	...
	function DoUninstall()
	{
		...
		if($step < 2)
		{
			$APPLICATION->IncludeAdminFile(GetMessage("MAIL_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/unstep1.php");
		}
		elseif($step == 2)
		{
			...
			$APPLICATION->IncludeAdminFile(GetMessage("MAIL_INSTALL_TITLE"), $_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/install/unstep2.php");
		}
	}
}
?>