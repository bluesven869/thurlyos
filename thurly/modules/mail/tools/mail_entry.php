...
try
{
	...
	if (empty($hostname))
		throw new Thurly\Mail\ReceiverException('Empty \'hostname\' request parameter');

	$message = $request->getPostList()->getRaw('message');
	if (empty($message))
		throw new Thurly\Mail\ReceiverException('Empty \'message\' request parameter');

	try
	{
		$message = Thurly\Main\Web\Json::decode($message);
	}
	catch (Exception $e)
	{
		throw new Thurly\Mail\ReceiverException('Invalid \'message\' request parameter');
	}

	if (empty($message['rcpt_to']))
		throw new Thurly\Mail\ReceiverException('Empty recipients list');
	...
	if (empty($rcpt))
		throw new Thurly\Mail\ReceiverException('Invalid recipients list');
	...
	{
		$error  = null;
		$result = Thurly\Mail\User::onEmailReceived($to, $message, $error);
	...
	if (!$success)
	{
		if (count($rcptErrors) == count($rcpt))
			throw new Thurly\Mail\ReceiverException(join('; ', $rcptErrors));

		throw new Thurly\Main\SystemException(sprintf('Message processing failed (rcpt: %s)', join(', ', $rcpt)));
	}

	$response->setStatus('204 No Content');
}
catch (Thurly\Mail\ReceiverException $e)
{
