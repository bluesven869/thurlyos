...
$ms = Thurly\Mail\MailServicesTable::getById($ID)->fetch();
if ($ms)
{
	$str_ID         = $ms['ID'];
...
if ($REQUEST_METHOD == "POST" && (strlen($save) > 0 || strlen($apply) > 0) && $MOD_RIGHT == "W" && check_thurly_sessid())
{
	$ICON = $_FILES['ICON'];
	...
	if ($ID > 0)
	{
		$result = Thurly\Mail\MailServicesTable::update($ID, $arFields);
	}
	else
	{
		$arFields['SERVICE_TYPE'] = $TYPE;

		$result = Thurly\Mail\MailServicesTable::add($arFields);
...
if ($ID > 0)
{
	...
	if ($MOD_RIGHT == "W")
	{
		$aMenu[] = array(
			...
			"ICON" => "btn_delete",
			"LINK" => "javascript:if(confirm('".GetMessage("MAIL_MSERVICE_EDT_DELETE_CONFIRM")."'))window.location='mail_mailservice_admin.php?action=delete&ID=".$ID."&lang=".LANG."&".thurly_sessid_get()."';",
		);
	}
}
...
<? $tabControl->BeginNextTab(); ?>
	...
	<tr>
		...
		<td width="60%">
			<select name="SITE_ID">
				<? $result = Thurly\Main\SiteTable::getList(array('filter' => array('ACTIVE' => 'Y'), 'order' => array('SORT' => 'ASC'))); ?>
	...
	<tr>
		...
		<td>
			...
			<? if ($icon = Thurly\Mail\MailServicesTable::getIconSrc($str_NAME, $str_ICON)) { ?>
