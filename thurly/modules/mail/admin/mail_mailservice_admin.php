...
if ($MOD_RIGHT == "W" && $lAdmin->EditAction())
{
	foreach ($FIELDS as $ID => $arFields)
	{
		...
		$DB->StartTransaction();
		if (!Thurly\Mail\MailServicesTable::update($ID, $arFields))
		{
...
if ($MOD_RIGHT == "W" && $arID = $lAdmin->GroupAction())
{
	...
	{
		$rsData = Thurly\Mail\MailServicesTable::getList(array('filter' => array_filter($arFilter), 'order' => array(strtoupper($by) => $order)));
	...
	foreach ($arID as $ID)
	{
		...
		switch ($_REQUEST['action'])
		{
			case "delete":
				@set_time_limit(0);
				$DB->StartTransaction();
				if (!Thurly\Mail\MailServicesTable::delete($ID))
				{
					$DB->Rollback();
