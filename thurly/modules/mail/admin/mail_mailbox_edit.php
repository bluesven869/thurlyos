...
if($REQUEST_METHOD=="POST" && (strlen($save)>0 || strlen($save_ext)>0 || strlen($apply)>0) && $MOD_RIGHT=="W" && check_thurly_sessid())
{
	$arFields = array(
		'ACTIVE'          => $ACTIVE,
		'LID'             => $LID,
...
if ($ID > 0)
{
	...
	if ($MOD_RIGHT=="W")
	{
		$aMenu[] = array(
			...
			"ICON" => "btn_delete",
			"LINK" => "javascript:if(confirm('".GetMessage("MAIL_MBOX_EDT_DEL_CONFIRM")."'))window.location='mail_mailbox_admin.php?action=delete&ID=".$ID."&lang=".LANG."&".thurly_sessid_get()."';",
		);
	}
}
...
<?$tabControl->BeginNextTab();?>
	...
	<tr>
		...
		<td width="60%">
			...
			<? if ($ID > 0) { ?>
			<? $defSite = $str_LID; ?>
			<? $result = Thurly\Main\SiteTable::getList(array('filter' => array('LID' => $str_LID), 'order' => array('SORT' => 'ASC'))); ?>
			<? $site = $result->fetch(); ?>
			[<?=$str_LID; ?>] <?=htmlspecialcharsbx($site['NAME']); ?>
			<? } else { ?>
			<select id="mailbox_site_id" name="LID"<? if ($mailbox_type == 'user') { ?> onchange="changeServicesList();"<? } ?>>
			<? $result = Thurly\Main\SiteTable::getList(array('order' => array('SORT' => 'ASC'))); ?>
