...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/prolog.php");
...
if($MOD_RIGHT=="W" && $arID = $lAdmin->GroupAction())
{
	if($_REQUEST['action_target']=='selected')
	{
		...
		$rsData = Thurly\Mail\MailMessageTable::getList(array(
			'select' => array('ID'),
...
$nav = new Thurly\Main\UI\AdminPageNavigation('nav-mail-message');

$messageList = Thurly\Mail\MailMessageTable::getList(array(
	'select'      => array(
...
$lAdmin->setNavigation($nav, Thurly\Main\Localization\Loc::getMessage("MAIL_MSG_ADM_NAVIGATION"));
	...
	if ($MOD_RIGHT=="W")
	{
		...
		if ($arRes['SPAM'] != "Y")
		{
			$arActions[] = array(
				...
				"ACTION"=>$lAdmin->ActionAjaxReload($APPLICATION->GetCurPage()."?action=mark_as_spam&ID=".$arRes['ID']."&lang=".LANG."&".thurly_sessid_get())
			);
		}
		...
		if ($arRes['SPAM'] != "N")
		{
			$arActions[] = array(
				...
				"ACTION"=>$lAdmin->ActionAjaxReload($APPLICATION->GetCurPage()."?action=mark_as_notspam&ID=".$arRes['ID']."&lang=".LANG."&".thurly_sessid_get())
			);
		}
		...
		if ($arRes['NEW_MESSAGE'] == "Y")
		{
			$arActions[] = array(
				...
				"ACTION"=>$lAdmin->ActionAjaxReload($APPLICATION->GetCurPage()."?action=mark_as_read&ID=".$arRes['ID']."&lang=".LANG."&".thurly_sessid_get())
			);
		}
		else
		{
			$arActions[] = array(
				...
				"ACTION"=>$lAdmin->ActionAjaxReload($APPLICATION->GetCurPage()."?action=mark_as_unread&ID=".$arRes['ID']."&lang=".LANG."&".thurly_sessid_get())
			);
		}
