...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/mail/include.php");
$err_mess = "File: ".__FILE__."<br>Line: ";

ClearVars("str_");

$ID = IntVal($ID);
if($_SERVER["REQUEST_METHOD"]=="POST" && $_REQUEST["save_form"]=="Y" && $MOD_RIGHT>="W" && check_thurly_sessid())
{
	...
	{
		$md5Path = md5("/thurly/admin/mail_message_admin.php");
	...
	LocalRedirect("/thurly/admin/mail_message_admin.php?lang=".LANG);
}
...
<?$tabControl->BeginNextTab();?>
	...
	<tr>
		...
		function hideshowhdr()
		{
			...
			<td width="20%" nowrap><a href="javascript:void(0)" onclick="hideshowhdr()" title="<?echo GetMessage("MMV_SHOW_HEADER_TITLE")?>"><?echo GetMessage("MMV_SHOW_HEADER")?></a><?if(COption::GetOptionString("mail", "save_src", B_MAIL_SAVE_SRC)=="Y" && $dbr_arr["FULL_TEXT"]!=''):?> | <a href="/thurly/admin/mail_message_view.php?lang=<?=LANGUAGE_ID?>&amp;ID=<?=intval($ID)?>&amp;show=original" target="_blank" title="<?echo GetMessage("MMV_SHOW_ORIG_TITLE")?>"><?echo GetMessage("MMV_SHOW_ORIG")?></a><?endif?></td>
			</tr></table>
		</td>
	</tr>
