...
class CMailDomain2
{
	...
	public static function addUser($token, $domain, $login, $password, &$error)
	{
		...
		if ($result !== false)
		{
			\Thurly\Mail\Internals\DomainEmailTable::add(array(
				'DOMAIN' => $domain,
				'LOGIN'  => $login,
			));
	...
	public static function getDomainUsers($token, $domain, &$error, $resync = false)
	{
		...
		$res = \Thurly\Mail\Internals\DomainEmailTable::getList(array(
			'filter' => array(
				'=DOMAIN' => $domain,
			),
		));
		...
		if ($resync || empty($users))
		{
			...
			if (!$error)
			{
				...
				{
					\Thurly\Mail\Internals\DomainEmailTable::delete(array(
						'DOMAIN' => $domain,
						'LOGIN'  => $login,
					));
				}
