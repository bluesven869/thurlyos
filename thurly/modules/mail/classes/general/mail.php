...
class CMail
{
	...
	public static function option($name, $value = null)
	{
		...
		if (!is_scalar($name))
			throw new \Thurly\Main\ArgumentTypeException('name');
...
class CAllMailBox
{
		...
		if (in_array(strtolower($arFields['SERVER_TYPE']), array('imap', 'controller', 'domain', 'crdomain')))
		{
			if (is_set($arFields, 'SERVICE_ID'))
			{
				if (!empty($arFields['LID']) || $ID)
				{
					...
					$result = Thurly\Mail\MailServicesTable::getList(array(
	...
	function Add($arFields)
	{
		...
		if (in_array($arFields['SERVER_TYPE'], array('imap', 'controller', 'domain', 'crdomain')) && $imapSync)
		{
			if ($arFields['USER_ID'] > 0)
				CAgent::addAgent(sprintf('Thurly\Mail\Helper::syncMailboxAgent(%u);', $ID), 'mail', 'N', 3600*24);
			elseif ($arFields['SERVER_TYPE'] == 'imap' && $arFields['PERIOD_CHECK'] > 0)
				CAgent::addAgent(sprintf('Thurly\Mail\Helper::syncMailboxAgent(%u);', $ID), 'mail', 'N', (int) $arFields['PERIOD_CHECK']*60);
		}
	...
	function Update($ID, $arFields)
	{
		...
		if (!empty($mbox))
		{
			...
			if ($userChanged || $siteChanged)
			{
				if ($mbox['ACTIVE'] == 'Y')
				{
					...
					{
						\Thurly\Main\Config\Option::delete('mail', array('name' => 'last_mail_check', 'site_id' => $mbox['LID']));
						\Thurly\Main\Config\Option::delete('mail', array('name' => 'last_mail_sync', 'site_id' => $mbox['LID']));
						\Thurly\Main\Config\Option::delete('mail', array('name' => 'last_mail_check_success', 'site_id' => $mbox['LID']));
					}
				}
				...
				if ($newActive == 'Y')
				{
					...
					{
						\Thurly\Main\Config\Option::set('mail', 'last_mail_check', false, $newSiteId);
						\Thurly\Main\Config\Option::set('mail', 'last_mail_sync', false, $newSiteId);
						\Thurly\Main\Config\Option::delete('mail', array('name' => 'last_mail_check_success', 'site_id' => $newSiteId));
					}
				}
			}
		...
		CAgent::removeAgent(sprintf('Thurly\Mail\Helper::syncMailboxAgent(%u);', $ID), 'mail');
		...
		if (in_array($serverType, array('imap', 'controller', 'domain', 'crdomain')) && $imapSync)
		{
			...
			elseif ($serverType == 'imap' && $periodCheck > 0)
				CAgent::addAgent(sprintf('Thurly\Mail\Helper::syncMailboxAgent(%u);', $ID), 'mail', 'N', (int) $periodCheck*60);
		}
	...
	function Delete($ID)
	{
		...
		Thurly\Main\Loader::includeModule('mail');
		$db_msg = Thurly\Mail\MailMessageTable::getList(array(
			'select' => array('ID'),
		...
		if ($message_id = \CMailMessage::add($arFields))
		{
			\CMailLog::addMessage(array(
				...
				'MESSAGE'     => sprintf(
					...
					\Thurly\Main\Config\Option::get('mail', 'spam_check', B_MAIL_CHECK_SPAM) == 'Y'
						? sprintf(' [%.3f]', $arFields['SPAM_RATING']) : ''
				),
			));

			$atchCnt = 0;
			if (\Thurly\Main\Config\Option::get('mail', 'save_attachments', B_MAIL_SAVE_ATTACHMENTS) == 'Y')
			{
			...
			if ($message_body_html)
			{
				...
				$arFields['BODY_BB'] = \Thurly\Mail\Message::parseMessage($msg);
...
class CAllMailUtil
{
	public static function convertCharset($str, $from, $to)
	{
		...
		if ($from != $to)
		{
			...
			if ($result = Thurly\Main\Text\Encoding::convertEncoding($str, $from, $to, $error))
				$str = $result;
			else
