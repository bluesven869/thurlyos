...
class Mobile
{
	...
	private function __construct()
	{
		...
		{
			$userAgent = \Thurly\Main\Context::getCurrent()->getServer()->get("HTTP_USER_AGENT");
	...
	protected function _Init()
	{
		...
		$GLOBALS["THURLY_PLATFORM"] = self::$platform;
		$GLOBALS["THURLY_API_VERSION"] = self::$apiVersion;
	...
	/**
	 * @return \Thurly\MobileApp\Mobile
	 */
...
JSCODE;
		...
		if(strpos($userAgent, "WKWebView/ThurlyMobile") === false)
		{
			if (self::getInstance()->getBXScriptSupported())
			{
				 ...
				 */
				$pgJsFile = "/thurly/js/mobileapp/__deviceload__/cordova.js?mod=1";
				$APPLICATION->AddHeadString("<script type=\"text/javascript\" src=\"" . $pgJsFile . "\"></script>", false, true);

			}
			else
			{
				$pgJsFile = "/thurly/js/mobileapp/" . self::$platform . "-cordova-" . self::$pgVersion . ".js";
				$APPLICATION->AddHeadString("<script type=\"text/javascript\" src=\"" . \CUtil::GetAdditionalFileURL($pgJsFile) . "\"></script>", false, true);
			}
		}


		$APPLICATION->AddHeadString("<script type=\"text/javascript\" src=\"" . \CUtil::GetAdditionalFileURL("/thurly/js/mobileapp/thurly_mobile.js") . "\"></script>", false, true);
		$APPLICATION->AddHeadString("<script type=\"text/javascript\" src=\"" . \CUtil::GetAdditionalFileURL("/thurly/js/mobileapp/mobile_lib.js") . "\"></script>", false, true);
