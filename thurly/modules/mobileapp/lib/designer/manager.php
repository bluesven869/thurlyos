...
class Manager
{
	...
	public static function copyFromTemplate($folder, $appCode, $useOffline = false, $templateCode = "simple")
	{
		...
		$offlineTemplate = Application::getDocumentRoot() . "/thurly/modules/mobileapp/templates_app/offline/";
		$templatePath = Application::getDocumentRoot() . "/thurly/modules/mobileapp/templates_app/".$templateCode."/";
		...
		if($directory->isExists())
		{
			if (!Directory::isDirectoryExists($appFolderPath))
			{
				...
				{
					/**
					 * @var $entry \Thurly\Main\IO\FileSystemEntry
					 */
	...
	public static function bindTemplate($templateId, $folder, $createNew)
	{
		...
		{
			CopyDirFiles(
				Application::getDocumentRoot() . "/thurly/modules/mobileapp/templates/default_app/",
				Application::getDocumentRoot() . "/thurly/templates/" . $templateId, True, True
			);

			File::putFileContents(
				Application::getDocumentRoot() . "/thurly/templates/" . $templateId . "/description.php",
				str_replace(Array("#mobile_template_name#"), Array($templateId), File::getFileContents(Application::getDocumentRoot() . "/thurly/templates/" . $templateId . "/description.php"))
			);
	 ...
	 * $value = "value1";
	 * $resultArray = \Thurly\MobileApp\Designer\Manager\nameSpaceToArray($namespace,$value));
	 *
	 * <b>Result:</b>
	 * array(
	 *  "depth0"=>array(
	...
	private static function addVirtualParams(&$structuredConfig, $platform)
	{
		if($structuredConfig["offline"] && !empty($structuredConfig["offline"]["file_list"]))
		{
			...
			$offlineParams["file_list"]["thurly_mobile_core.js"] = Tools::getMobileJSCorePath();
