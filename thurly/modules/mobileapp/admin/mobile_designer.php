...
{
	$map = new \Thurly\MobileApp\Designer\ConfigMap();
	...
	$result = \Thurly\Mobileapp\Designer\AppTable::getList(
		array(
	...
	else
	{
		switch ($action)
		{
			...
			case "save":
				...
				$status = \Thurly\MobileApp\Designer\Manager::updateConfig($app, $platform, $params);
				break;
			case "removePlatform":
				$code = $_REQUEST["code"];
				$platform = $_REQUEST["platform"];
				$status = Thurly\MobileApp\Designer\Manager::removeConfig($code, $platform);
				break;
			case "createPlatform":
				$status = \Thurly\MobileApp\Designer\Manager::addConfig($_REQUEST["code"], $_REQUEST["platform"], array());
				break;
			case "createApp":
				$code = $_REQUEST["code"];
				...
				if (!\Thurly\Main\Application::isUtfMode())
				{
					$fields = \Thurly\Main\Text\Encoding::convertEncodingArray($fields, "UTF-8", SITE_CHARSET);
					$code = \Thurly\Main\Text\Encoding::convertEncoding($code, "UTF-8", SITE_CHARSET);
					$templateName = \Thurly\Main\Text\Encoding::convertEncoding($templateName, "UTF-8", SITE_CHARSET);
				}
				$initConfig = array();
				if($useOffline)
				{
				...
				$result = \Thurly\MobileApp\Designer\Manager::createApp($code, $fields, $initConfig); //creating global config inside

				if($result == \Thurly\MobileApp\Designer\Manager::IS_ALREADY_EXISTS)
				{
				...
				$status = ($result == \Thurly\MobileApp\Designer\Manager::SUCCESS);

				if ($status)
				{
					\Thurly\MobileApp\Designer\Manager::copyFromTemplate($fields["FOLDER"], $code, $useOffline, $appTemplateName);

					if ($bindTemplate)
					{
						$templateId = $_REQUEST["template_id"];
						\Thurly\MobileApp\Designer\Manager::bindTemplate($templateId, $fields["FOLDER"], $createTemplate);
					}
				}
			...
			case "removeApp":
				$code = $_REQUEST["code"];
				$status = Thurly\MobileApp\Designer\Manager::removeApp($code);
				$data["code"] = $code;
				break;
			case "getFiles":
				$code = $_REQUEST["code"];
				$files = \Thurly\MobileApp\Designer\Manager::getAppFiles($code);
				$data = array(
					"status" => true,
					"files" => $files
				);
...
<?
AddEventHandler("mobileapp", "onDesignerFileUploaded", Array("Thurly\\MobileApp\\Designer\\Manager", "registerFileInApp"));
AddEventHandler("mobileapp", "onDesignerFileRemoved", Array("Thurly\\MobileApp\\Designer\\Manager", "unregisterFileInApp"));

$APPLICATION->IncludeComponent
(
	"thurly:mobileapp.colorpicker",
	"",
	Array(
		"SHOW_BUTTON" => "Y",
		"ID" => 'picker',
...
$GLOBALS['APPLICATION']->IncludeComponent('thurly:mobileapp.designer.file.input', 'drag_n_drop', $componentParams, false);
...
<script>
	BX.ready(function ()
	{
		...
		window.designer = new BX.Mobile.Designer({
			...
			platforms:<?=Json::encode(\Thurly\MobileApp\Designer\ConfigTable::getSupportedPlatforms())?>
		});
