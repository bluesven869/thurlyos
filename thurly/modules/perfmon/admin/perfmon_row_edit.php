...
if ($_SERVER["REQUEST_METHOD"] === "POST" && check_thurly_sessid() && $isAdmin && $_REQUEST["action"] === "unserialize")
{
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && check_thurly_sessid() && $isAdmin && $_REQUEST["action"] === "serialize")
{
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/prolog_admin_js.php");
	CUtil::JSPostUnescape();
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && check_thurly_sessid() && $isAdmin)
{
	CTimeZone::Disable();
	...
	<script>
		...
		function editAsSerialize(button, Field, mark)
		{
			...
			if (textArea && markHidden)
			{
				...
				var url = 'perfmon_row_edit.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&action=' + action;
				BX.showWait();
				BX.ajax.post(
					url,
					{data: textArea.value},
