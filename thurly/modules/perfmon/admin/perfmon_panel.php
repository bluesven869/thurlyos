...
elseif(isset($_REQUEST["test"]) && $_REQUEST["test"] === "cluster")
{
	...
	)
	{
		...
		if(strlen($_GET["server_url"]) > 0)
			...
			$server_url = '/thurly/admin/perfmon_panel.php?test=Y&show_page_exec_time=Y&show_sql_stat=N';
...
elseif(isset($_REQUEST["test"]))
{
	...
	/** @global CDatabase $DB */
	/** @global CUser $USER */
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/perfmon/include.php");
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/perfmon/prolog.php");
	require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/include/prolog_admin_js.php");

	IncludeModuleLangFile(__FILE__);

	if(check_thurly_sessid() && $APPLICATION->GetGroupRight("perfmon") >= "W")
	{
...
if($REQUEST_METHOD == "POST" && ($calc.$total_calc!="") && $RIGHT >= "W" && check_thurly_sessid())
{
...
if($bOptimal)
{
	if(COption::GetOptionString("perfmon", "thurly_optimal", "-") !== "Y")
		COption::SetOptionString("perfmon", "thurly_optimal", "Y");
}
else
{
	if(COption::GetOptionString("perfmon", "thurly_optimal", "-") !== "N")
		COption::SetOptionString("perfmon", "thurly_optimal", "N");
}
...
$aTabs = array(
	...
	array(
		"DIV" => "thurly",
		"TAB" => GetMessage("PERFMON_PANEL_THURLY_NAME").(
			COption::GetOptionString("perfmon", "thurly_optimal", "-") === "Y"?
			" (".GetMessage("PERFMON_PANEL_MARK_PHP_IS_GOOD").")":
			" (".GetMessage("PERFMON_PANEL_MARK_PHP_IS_NO_GOOD").")"
		),
		"ICON" => "main_user_edit",
		"TITLE" => GetMessage("PERFMON_PANEL_THURLY_TITLE"),
	),
	array(
		"DIV" => "dev",
...
function StopTest()
{
	...
	CHttpRequest.Send('perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=monitor&action=stop');
}

function ShowThurly()
{
	tabControl.SelectTab('thurly');
...
function ShowDev()
{
	...
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=monitor';
	if(duration > 0)
	{
...
function PageRate()
{
	...
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=Y';
	if(page_rate_count == 0)
		url += '&last=Y';
...
function CPUMeasure()
{
	...
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=cpu';
	CHttpRequest.Send(url);
}
function FilesMeasure()
{
	...
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=files';
	CHttpRequest.Send(url);
}
function MailMeasure()
{
	...
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=mail';
	CHttpRequest.Send(url);
}
function SessionMeasure()
{
	...
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=session';
	if(session_count == 0)
		url += '&last=Y';
...
function PHPMeasure()
{
	...
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=php';
	CHttpRequest.Send(url);
}
function DBInsertMeasure()
{
	...
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=db_insert';
	CHttpRequest.Send(url);
}
function DBReadMeasure()
{
	...
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=db_read';
	CHttpRequest.Send(url);
}
function DBUpdateMeasure()
{
	...
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=db_update';
	CHttpRequest.Send(url);
}
...
{
	var url = 'perfmon_panel.php?lang=<?echo LANGUAGE_ID?>&<?echo thurly_sessid_get()?>&test=cluster';
...
<?$tabControl->BeginNextTab();?>
	<tr>
		...
		<table border="0" cellpadding="0" cellspacing="0" class="internal" width="100%">
		<tr class="heading">
			<td width="40%" align="center"><?echo GetMessage("PERFMON_PANEL_THURLY_PARAMETER")?></td>
			<td width="30%" align="center"><?echo GetMessage("PERFMON_PANEL_THURLY_VALUE")?></td>
			<td width="30%" align="center"><?echo GetMessage("PERFMON_PANEL_THURLY_RECOMMENDATION")?></td>
		</tr>
		<tr>
		...
		<tr>
			...
			<?if($statistic_path):?>
				...
				<td><a href="settings.php?lang=<?echo LANGUAGE_ID?>&amp;mid=statistic&amp;tabControl_active_tab=edit2&amp;back_url_settings=<?echo urlencode($APPLICATION->GetCurPageParam("tabControl_active_tab=thurly", array("tabControl_active_tab")))?>"><?echo GetMessage("PERFMON_PANEL_STAT_SAVE_PATH_REC")?></a></td>
			<?else:?>
		...
		<tr>
			...
			<?if($adv_banners_fix_shows):?>
				...
				<td><a href="settings.php?lang=<?echo LANGUAGE_ID?>&amp;mid=advertising&amp;back_url_settings=<?echo urlencode($APPLICATION->GetCurPageParam("tabControl_active_tab=thurly", array("tabControl_active_tab")))?>"><?echo GetMessage("PERFMON_PANEL_ADV_BANNERS_FIX_SHOWS_REC")?></a></td>
			<?else:?>
		...
		<tr>
			...
			<?else:?>
				...
				<td><a href="settings.php?lang=<?echo LANGUAGE_ID?>&amp;mid=search&amp;back_url_settings=<?echo urlencode($APPLICATION->GetCurPageParam("tabControl_active_tab=thurly", array("tabControl_active_tab")))?>"><?echo GetMessage("PERFMON_PANEL_SEARCH_STEM_REC")?></a></td>
			<?endif?>
		</tr>
		<?endif?>
		<tr>
			...
			switch(\Thurly\Main\Data\Cache::getCacheEngineType())
			{
			...
			case "cacheenginememcachecluster":
				...
				break;
			default:
				$cache_type = \Thurly\Main\Data\Cache::getCacheEngineType();
				break;
			}
			?>
			...
			<?else:?>
				...
				<td><?echo GetMessage("PERFMON_PANEL_MANAGED_CACHE_REC", array(
					"#file#" => (
						...
						"<a href=\"".'/thurly/admin/fileman_file_edit.php?lang='.LANGUAGE_ID.'&amp;full_src=Y&amp;path='.urlencode(BX_PERSONAL_ROOT.'/php_interface/dbconn.php').'&amp;back_url='.urlencode('/thurly/admin/security_panel.php?lang='.LANGUAGE_ID)."\">dbconn.php</a>":
						"dbconn.php"
					),
				));?>
				</td>
...
<?$tabControl->BeginNextTab();
	...
	<tr>
		...
		<td width="<?echo $width?>" align="center" class="adm-detail-valign-top">
			...
			<img class="graph" src="/thurly/admin/perfmon_cluster_graph.php?rand=<?=rand()?>&amp;find_data_type=PAGES_PER_SECOND&amp;width=<?echo $width?>&amp;height=<?echo $height?>&amp;lang=<?echo LANGUAGE_ID?>" width="<?echo $width?>" height="<?echo $height?>">
			</div>
			<br />
			<div id="img_PAGE_EXEC_TIME">
			<?echo GetMessage("PERFMON_PANEL_CLUSTER_PAGE_TIME")?><br>
			<img class="graph" src="/thurly/admin/perfmon_cluster_graph.php?rand=<?=rand()?>&amp;find_data_type=PAGE_EXEC_TIME&amp;width=<?echo $width?>&amp;height=<?echo $height?>&amp;lang=<?echo LANGUAGE_ID?>" width="<?echo $width?>" height="<?echo $height?>">
			</div>
		</td>
	</tr>
<?$tabControl->End();?>
