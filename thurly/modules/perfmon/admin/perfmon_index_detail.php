...
if($_SERVER["REQUEST_METHOD"] === "POST" && check_thurly_sessid() && $isAdmin)
{
	if(isset($_REQUEST["create_index"]) && isset($_REQUEST["ddl"]))
	{
		...
		if(is_object($res))
		{
			...
			LocalRedirect("/thurly/admin/perfmon_index_detail.php?ID=".$ID."&lang=".LANGUAGE_ID."&".$tabControl->ActiveTabParam());
		}
		else
		{
	...
	elseif(isset($_REQUEST["ban_index"]))
	{
		...
		LocalRedirect("/thurly/admin/perfmon_index_list.php?lang=".LANGUAGE_ID);
	}
}
