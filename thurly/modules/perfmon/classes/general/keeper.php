...
class CPerfomanceKeeper
{
	...
	function setDebugModeOn()
	{
		...
		define("PERFMON_STARTED", $DB->ShowSqlStat."|".\Thurly\Main\Data\Cache::getShowCacheStat()."|".$APPLICATION->ShowIncludeStat);

		$DB->ShowSqlStat = true;
		$application = \Thurly\Main\HttpApplication::getInstance();
		$application->getConnection()->startTracker();

		\Thurly\Main\Data\Cache::setShowCacheStat(COption::GetOptionString("perfmon", "cache_log") === "Y");
	...
	function restoreDebugMode()
	{
		...
		\Thurly\Main\Data\Cache::setShowCacheStat($toRestore[1]);
	...
	function writeToDatabase()
	{
		...
		$cache_count = array();
		/** @var \Thurly\Main\Diag\CacheTracker $arCacheDebug */
		$arCacheDebug = null;
		if ($cache_log)
		{
		...
		if ($_SERVER["SCRIPT_NAME"] == "/thurly/urlrewrite.php" && isset($_SERVER["REAL_FILE_PATH"]))
	...
	function findCaller($trace, &$module_id, &$comp_id)
	{
		...
		foreach ($trace as $i => $arCallInfo)
		{
			if (array_key_exists("file", $arCallInfo))
			{
				...
				{
					$match = array();
					if (preg_match("#.*/thurly/modules/(.+?)/#", $file, $match))
					{
						$module_id = $match[1];
					}
				}
				...
				if (
					!$comp_id
					&& preg_match("#.*/(?:thurly|install)/components/(.+?)/(.+?)/#", $file, $match)
				)
				{
