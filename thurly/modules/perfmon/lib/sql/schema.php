<?php
namespace Thurly\Perfmon\Sql;

use Thurly\Main\NotSupportedException;

//Sample usage:
/*
\Thurly\Main\Loader::includeModule('perfmon');
$dir = new \Thurly\Main\IO\Directory("/opt/php03.cp1251.www/mercurial/thurly/modules");
	...
	if ($child->isDirectory() && $child->getName()!=='xxx')
	{
		...
		foreach (array("mysql"=>";", "mssql"=>"GO", "oracle"=>"/") as $db=>$delimiter)
		{
			...
			if (!\Thurly\Main\IO\File::isFileExists($path))
				continue;
			//echo "<br>$path<br>";
			$sql = \Thurly\Main\IO\File::getFileContents($path);
			$s = new \Thurly\Perfmon\Sql\Schema;
