		...
		if(CModule::IncludeModule("iblock"))
		{
			...
			elseif($serviceID == "disk")
			{
				if($serviceStage == "disk" && CModule::IncludeModule('disk'))
				{
					...
					{
						$dbDisk = Thurly\Disk\Storage::getList(array("filter"=>array("ENTITY_ID" => $entityId)));
						if ($arDisk = $dbDisk->Fetch())
						{
							$storage = \Thurly\Disk\Storage::loadById($arDisk["ID"]);
							if($storage)
							{
								$userId = \Thurly\Disk\SystemUser::SYSTEM_USER_ID;
								$fakeSecurityContext = \Thurly\Disk\Driver::getInstance()->getFakeSecurityContext();
								foreach($storage->getChildren($fakeSecurityContext, array('filter' => array('MIXED_SHOW_DELETED' => true))) as $child)
								{
									if($child instanceof \Thurly\Disk\Folder)
									{
										$res = $child->deleteTree($userId);
									}
									elseif($child instanceof \Thurly\Disk\File)
									{
