...
$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH."/xml/".LANGUAGE_ID."/sales_files.xml";
$iblockCode = "sales_files_".WIZARD_SITE_ID; 
$iblockType = "library";
...
{
	$rsIBlock = CIBlock::GetList(array(), array("CODE" => "sales_files", "TYPE" => $iblockType));
	if ($arIBlock = $rsIBlock->Fetch())
	{
		$iblockID = $arIBlock["ID"]; 
	}
...
if($iblockID == false)
{
	$iblockID = WizardServices::ImportIBlockFromXML(
		...
		$permissions = Array(
			...
			WIZARD_MARKETING_AND_SALES_GROUP => "W",
		)
	);
	...
	if(CModule::IncludeModule("bizproc") && false)
	{
			...
			function __wd_replace_user_and_groups(&$val, $key)
			{
				if ($key == "MailText")
				{
					$val = str_replace(
						...
						"/docs/sale/webdav_bizproc_view/{=Document:ID}/", 
						$val);
				}
				return true;
			}
		...
		if($handle = opendir($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/bizproc/templates'))
		{
			while(false !== ($file = readdir($handle)))
			{
				if(!is_file($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/bizproc/templates/'.$file))
					continue;
	
				$arFields = false;
				include($_SERVER['DOCUMENT_ROOT'].'/thurly/modules/bizproc/templates/'.$file);
...
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/docs/sale/index.php", Array("SALES_FILES_IBLOCK_ID" => $iblockID));
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/docs/index.php", Array("SALES_FILES_IBLOCK_ID" => $iblockID));
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/m/docs/index.php", Array("SALES_FILES_IBLOCK_ID" => $iblockID));
?>
