...
if (WIZARD_B24_TO_CP)
{
	$commonStorage = \Thurly\Disk\Driver::getInstance()->getStorageByCommonId('shared_files_'.WIZARD_SITE_ID);
	if ($commonStorage)
	{
...
//Common storage
$dbDisk = Thurly\Disk\Storage::getList(array("filter"=>array("ENTITY_ID" => "shared_files_".WIZARD_SITE_ID)));
if (!$dbDisk->Fetch() && !WIZARD_B24_TO_CP)
{
	...
	if($commonStorage)
	{
		...
		//Create new folder \Thurly\Disk\Folder $folder
...
//directors storage
$dbDisk = Thurly\Disk\Storage::getList(array("filter"=>array("ENTITY_ID" => "directors_files_".WIZARD_SITE_ID)));
if (!$dbDisk->Fetch())
{
...
//sales storage
$dbDisk = Thurly\Disk\Storage::getList(array("filter"=>array("ENTITY_ID" => "sales_files_".WIZARD_SITE_ID)));
if (!$dbDisk->Fetch())
{
