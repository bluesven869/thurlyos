...
$thurlyTemplateDir = $_SERVER["DOCUMENT_ROOT"].BX_PERSONAL_ROOT."/templates/";


if (WIZARD_TEMPLATE_ID === "thurlyos")
{
	$bTemplateDir = $thurlyTemplateDir.WIZARD_TEMPLATE_ID;
	BXClearCache(true, "/bx/user_slmenu/");
}
else
	$bTemplateDir = $thurlyTemplateDir.WIZARD_TEMPLATE_ID.'_'.WIZARD_THEME_ID;
...
CopyDirFiles(
	$_SERVER["DOCUMENT_ROOT"]."/thurly/modules/intranet/install/templates/login/",
	$thurlyTemplateDir."login/",
	$rewrite = true,
	$recursive = true,
	$delete_after_copy = false
);
...
elseif (WIZARD_TEMPLATE_ID=="thurlyos")
{
	CheckDirPath(WIZARD_SITE_PATH."/include/");
	___writeToAreasFile(WIZARD_SITE_PATH."/include/company_name.php", COption::GetOptionString("main", "site_name", "Compamy Name", WIZARD_SITE_ID));
}
else
{
	CWizardUtil::ReplaceMacros(
		$thurlyTemplateDir.WIZARD_TEMPLATE_ID.'_'.WIZARD_THEME_ID.'/include_areas/company_name.php',
		array(
	...
	CWizardUtil::ReplaceMacros(
		$thurlyTemplateDir.WIZARD_TEMPLATE_ID.'/footer.php',
		array(
...
// for b24	
	if (
		WIZARD_TEMPLATE_ID === "thurlyos" 
		&& $current_template !== "thurlyos" 
	...
	)
	{
		...
		else
		{
			...
			preg_match('/\$APPLICATION->IncludeComponent\(\"thurly:socialnetwork_user\"?.*\)[\n\s\t]*\);/si', $contents, $matches);//preg_match('<\?/\$APPLICATION->IncludeComponent\(\"thurly:socialnetwork_user\"?.*\)[\n\s\t]*\);/si', $contents, $matches); 
	...
	elseif (WIZARD_TEMPLATE_ID !== "thurlyos" && $current_template === "thurlyos"  && WIZARD_FIRST_INSTAL == "Y")
	{	
		CopyDirFiles(
			WIZARD_SITE_PATH."index.php",
			WIZARD_SITE_PATH."index_b24.php",
