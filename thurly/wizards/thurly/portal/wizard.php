...
//b24 to cp		
	...
	function ShowStep()
	{
		...
		if(file_exists($_SERVER["DOCUMENT_ROOT"].'/thurly/modules/ldap/include.php'))
		{
...
class SelectTemplateStep extends CWizardStep
{
	...
	function OnPostForm()
	{
		...
		if ($wizard->IsNextButtonClick())
		{
			$templatesPath = WizardServices::GetTemplatesPath("/thurly/modules/intranet/install");
	...
	function ShowStep()
	{
		...
		$arTemplateOrder = array("thurlyos", "light", "bright", "modern", "classic");
...
class PortalFeaturesSettingsStep extends CWizardStep
{
	...
	function InitStep()
	{
		...
		if ($wizard->GetVar("templateID") === "thurlyos")
			$this->SetPrevStep("select_template");
		else
	...
	function ShowStep()
	{
		...
		if(($_SERVER["PHP_SELF"] == "/index.php" && CBXFeatures::IsFeatureEnabled("Extranet")) || COption::GetOptionString("main", "wizard_install_extranet", "N", "") == "Y")
		{
			...
			$this->content .=  "<br clear=\"all\"><div style='text-align: right; margin-top: 40px; padding-top: 18px; border-top: 1px solid #e5e5e5;'>
				<input style='padding: 0 21px 0 11px' class='wizard-next-button' value='".GetMessage("wizard_extranet_config")."' onclick='document.location.href=\"/thurly/admin/wizard_install.php?".thurly_sessid_get()."&lang=".LANGUAGE_ID."&wizardName=thurly:extranet\"'/>
...
//b24 to cp
		...
		define("WIZARD_FIRST_INSTAL", $firstStep);
		//Copy index page
		if (WIZARD_FIRST_INSTAL !== "Y" && $wizard->GetVar("templateID") === "thurlyos"  || $b24ToCp && $wizard->GetVar("templateID") === "thurlyos")
		{
			CopyDirFiles(
