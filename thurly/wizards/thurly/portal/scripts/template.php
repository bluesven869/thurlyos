...
class WizardTemplate extends CWizardTemplate
{
	function GetLayout()
	{
		...
		$wizardPath = "/thurly/images/main/wizard_sol";
		...
		elseif(GetMessage("WIZARD_TITLE_SOL")!="")
			...
			$title = EDITION == "H" ? GetMessage("WIZARD_TITLE") : GetMessage("WIZARD_TITLE_CP");
		...
		{
			$box = EDITION == "H" ? "box.png" : "box_cp.png";
			if (file_exists($_SERVER["DOCUMENT_ROOT"].$wizardPathCustom."/images/".LANGUAGE_ID."/".$box))
				$boxImage = '<img src="'.$wizardPathCustom.'/images/'.LANGUAGE_ID.'/'.$box.'" alt="" />';
			elseif (file_exists($_SERVER["DOCUMENT_ROOT"].$wizardPathCustom."/images/en/".$box))
				$boxImage = '<img src="'.$wizardPathCustom.'/images/en/'.$box.'" alt="" />';
		}
		...
		$jsCode = "";
		$jsCode = file_get_contents($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/install/wizard_sol/script.js");
