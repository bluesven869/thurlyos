...
AddEventHandler("blog", "BlogImageSize", "ResizeMobileLogImages", 100, $_SERVER["DOCUMENT_ROOT"].SITE_TEMPLATE_PATH."/components/thurly/socialnetwork.blog.post/mobile/functions.php");
...
)
{
	function LocalRedirectHandler(&$url)
	{
		...
		)
		{
			...
			{
				ob_start();
				?><?$GLOBALS["APPLICATION"]->IncludeComponent("thurly:mobile.socialnetwork.log.ex", ".default", array(
	...
	$APPLICATION->IncludeComponent("thurly:socialnetwork.blog.post.edit", "mobile_empty", array(
			...
			"GROUP_ID" => (IsModuleInstalled("thurlyos") ? $GLOBAL_BLOG_GROUP[SITE_ID] : false),
			"MOBILE" => "Y"
		),
		false,
		Array("HIDE_ICONS" => "Y")
...
?><?$APPLICATION->IncludeComponent("thurly:mobile.socialnetwork.log.ex", ".default", array(
