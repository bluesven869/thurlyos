...
if (in_array(WIZARD_TEMPLATE_ID, array("bright_extranet", "classic_extranet", "modern_extranet")))
{
	...
	CopyDirFiles(
		...
		$thurlyTemplateDir.WIZARD_TEMPLATE_ID,
		$rewrite = true,
		$recursive = true
	);

	$logo = (intval($logo) ? CFile::GetPath($logo) : '/thurly/templates/'.WIZARD_TEMPLATE_ID.'/images/default_logo.gif');

	CWizardUtil::ReplaceMacros(
		$thurlyTemplateDir.WIZARD_TEMPLATE_ID.'/include_areas/company_name.php',
		array(
	...
	CWizardUtil::ReplaceMacros(
		$thurlyTemplateDir.WIZARD_TEMPLATE_ID.'/header.php',
		array(
			"SITE_DIR" => WIZARD_SITE_DIR,
		)
	);
...
if ($arSite = $obSite->Fetch())
{
	...
	if (WIZARD_TEMPLATE_ID === "thurlyos" && ($current_template !== "thurlyos" || WIZARD_B24_TO_CP))
	{
		CopyDirFiles(
		...
		{	
			...
			$sonet_user = preg_match('/\$APPLICATION->IncludeComponent\(\"thurly:socialnetwork_user\".*?\);/si', $contents, $matches);
			...
			$contents_b24_new = preg_replace('/\$APPLICATION->IncludeComponent\(\"thurly:socialnetwork_user\"[^;]+;/si', $sonet_replace, $contents_b24);
	...
	elseif ($current_template === "thurlyos" && WIZARD_TEMPLATE_ID !== "thurlyos")
	{
		CopyDirFiles(
			WIZARD_SITE_PATH."index.php",
			WIZARD_SITE_PATH."index_b24.php",
