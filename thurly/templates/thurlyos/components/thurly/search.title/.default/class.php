...
final class CB24SearchTitle
{
	final public static function getUsers($searchString = false)
	{
		...
		$userPageURLTemplate = \Thurly\Main\Config\Option::get('socialnetwork', 'user_page', SITE_DIR.'company/personal/', SITE_ID).'user/#user_id#/';
		...
		if (\Thurly\Main\ModuleManager::isModuleInstalled('replica'))
		{
			$arExternalAuthId[] = 'replica';
		}
		if (\Thurly\Main\ModuleManager::isModuleInstalled('im'))
		{
			$arExternalAuthId[] = 'bot';
		}
		if (\Thurly\Main\ModuleManager::isModuleInstalled('imconnector'))
		{
			$arExternalAuthId[] = 'imconnector';
		}
		if (\Thurly\Main\ModuleManager::isModuleInstalled('mail'))
		{
	...
	final public static function getSonetGroups($searchString = false)
	{
		...
		{
			$extranetIncluded = \Thurly\Main\Loader::includeModule('extranet');
		...
		$groupPageURLTemplate = \Thurly\Main\Config\Option::get('socialnetwork', 'workgroups_page', SITE_DIR.'workgroups/', SITE_ID).'group/#group_id#/';
		...
		if ($extranetUser)
		{
			...
			$res = \Thurly\Socialnetwork\UserToGroupTable::getList(array(
				'filter' => array(
					'USER_ID' => $USER->getId(),
					'@ROLE' => \Thurly\Socialnetwork\UserToGroupTable::getRolesMember()
				),
	...
	final public static function getMenuItems($searchString = false)
	{
		...
		$arMenuResult = $APPLICATION->IncludeComponent(
			...
			"left_vertical",
			array(
				"ROOT_MENU_TYPE" => isModuleInstalled("thurlyos") ? "superleft" : "top",
				"MENU_CACHE_TYPE" => "Y",
	...
	final public static function customSearch($searchString, $arParams, &$arResult)
	{
		...
		{
			$bSocialnetworkIncluded = \Thurly\Main\Loader::includeModule('socialnetwork');
		}

		if ($bExtranetSite === null)
		{
			$bExtranetSite = (\Thurly\Main\Loader::includeModule('extranet') && CExtranet::IsExtranetSite());
		}
