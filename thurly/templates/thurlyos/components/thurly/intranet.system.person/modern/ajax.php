...
if ($_SERVER["REQUEST_METHOD"]=="POST" && isset($_POST["active"]) && in_array($_POST["active"], array("D", "Y", "N")) && check_thurly_sessid())
{
	$userId = intval($_POST["user_id"]);

	$res = false;
	$canEdit = ($USER->CanDoOperation('edit_own_profile') || $USER->IsAdmin()) ? "Y" : "N";
	$CurrentUserPerms = CSocNetUserPerms::InitUserPerms($USER->GetID(), $userId, CSocNetUser::IsCurrentUserModuleAdmin($_POST["site_id"], (CModule::IncludeModule("thurlyos") && CThurlyOS::IsPortalAdmin($USER->GetID()) ? false : true)));
