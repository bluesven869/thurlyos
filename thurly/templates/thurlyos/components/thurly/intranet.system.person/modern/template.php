...
function user_action_menu<?=$user_action_menu_number?> (button, number, user_id, is_extranet) {
	...
	var popupUserMenuItems = [
		<?if ($arUser["ACTIVITY_STATUS"] == "active" || $arUser["ACTIVITY_STATUS"] == "extranet"):?>
			...
			<?endif?>
		<?elseif ($arUser["ACTIVITY_STATUS"] == "inactive" && (!IsModuleInstalled("thurlyos") && $USER->CanDoOperation('edit_all_users') || $USER->CanDoOperation('thurlyos_invite') && CModule::IncludeModule('thurlyos'))):?>
