<?
use Thurly\Socialnetwork\Item\WorkgroupFavorites;
	...
	switch ($_POST["action"])
	{
		...
		*/
		case "add_item_to_all":
			if (
				!(
					...
					(\Thurly\Main\Loader::includeModule("thurlyos") && \CThurlyOS::IsPortalAdmin($GLOBALS["USER"]->getID()))
				)
			)
				break;
		...
		case "set_preset":
			...
			else
			{
				...
				switch ($_POST["preset"])
				{
					...
					case "crm":
						$firstPageUrl = $_POST["siteDir"]."crm/start/";
						break;
				...
				if($_POST['mode'] === 'global' && IsModuleInstalled('thurlyos'))
				{
					$_SESSION['B24_SHOW_RENAME_POPUP_HINT'] = 1;
		...
		case "save_custom_preset":
			...
			if (isset($_POST["firstItemLink"]))
			{
				COption::SetOptionString("intranet", "left_menu_first_page", $_POST["firstItemLink"], false, $siteID);
			}
