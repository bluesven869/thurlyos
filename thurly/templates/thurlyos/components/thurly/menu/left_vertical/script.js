BX.namespace("BX.ThurlyOS.LeftMenuClass");

BX.ThurlyOS.LeftMenuClass = {
	init: function (params)
	{
		...
		this.isThurlyOS = params.isThurlyOS === "Y";
			...
			onsuccess: BX.proxy(function (json)
			{
				...
				else
				{
					if (json.hasOwnProperty("itemId"))
					{
						...
						BX.onCustomEvent("BX.ThurlyOS.LeftMenuClass:onMenuItemAdded", [itemInfo, this]);
		...
		BX.ajax({
			...
			onsuccess: BX.proxy(function (json)
			{
				...
				else
				{
					if (json.hasOwnProperty("itemId"))
					{
						BX.onCustomEvent("BX.ThurlyOS.LeftMenuClass:onMenuItemDeleted", [json, this]);
	...
	{
		BX.onCustomEvent("BX.ThurlyOS.LeftMenuClass:onDragStart");
	...
	toggle: function (flag)
	{
		...
		/* we have to modify thurlyos template */
		...
		BX.onCustomEvent("BX.ThurlyOS.LeftMenuClass:onMenuToggle", [flag, this]);
		...
		BX.ready(function ()
		{
			...
			BX.PopupWindowManager.create("menu-preset-popup", null, {
						...
						events: {
							click: BX.proxy(function ()
							{
								...
								BX.ajax({
									method: 'POST',
									dataType: 'json',
									url: this.ajaxPath + "?analyticsLabel=" + currentPreset,
									data: {
										sessid: BX.thurly_sessid(),
										site_id: this.siteId,
						...
						events: {
							click: BX.proxy(function ()
							{
								BX.ajax({
									method: 'POST',
									dataType: 'json',
									url: this.ajaxPath + "?analyticsLabel=later"
								});
								BX.proxy_context.popupWindow.close();
							}, this)
						}
				...
				events: {
					onPopupClose: function ()
					{
						if (mode == 'global' && !!BX.ThurlyOS && !!BX.ThurlyOS.renamePortal)
						{
							BX.ThurlyOS.renamePortal.showNotify();
						}
					}
				}
			}).show();
