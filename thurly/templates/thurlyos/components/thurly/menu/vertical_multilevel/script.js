...
BX.ThurlyOS.MenuClass = (function()
{
		...
		for (var j=0; j<this.arTitles.length; j++)
		{
			...
			if (!isEditMode)
			{
				...
				for (var i=0; i<liObj.length; i++)
				{
					...
					if (((this.isThurlyOS && this.arTitles[j] == "menu-favorites") || this.arTitles[j] == "menu-groups") && liObj[i].id == "empty_li_"+this.arTitles[j])
						continue;

					if ((this.isThurlyOS && liObj[i].id == "menu_live_feed") || liObj[i].id == "menu_all_groups")
					{
	...
	MenuClass.prototype.menuItemDragMove = function(x, y)
	{
		...
		if (this.isThurlyOS && (dragElement.getAttribute("data-title-item") == "menu-favorites" || dragElement.getAttribute("data-title-item") == "menu-groups") && y<this.objHeight)
			y = this.objHeight;
		else if (y < 0)
			y = 0;
