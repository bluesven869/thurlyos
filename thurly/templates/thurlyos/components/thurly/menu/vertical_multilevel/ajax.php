...
if ($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["action"])>0 && check_thurly_sessid())
{
	...
	$userOption = CUserOptions::GetOption("thurlyos", $optionName);
	...
	switch ($_POST["action"])
	{
		case "get_app_rigths":
			if (CModule::IncludeModule("thurlyos"))
			{
				...
				$resRights = CThurlyOSAppRights::GetAppRights($app_id);
				$arJsonData["rights"] = $resRights;
			}
			break;
		case "set_app_rights":
			if (CModule::IncludeModule("thurlyos") && $GLOBALS['USER']->CanDoOperation('thurlyos_config'))
			{
				if (isset($_POST["app_id"]) && intval($_POST["app_id"]))
					$app_id = intval($_POST["app_id"]);

				$resRights = CThurlyOSAppRights::SetAppRights($app_id, $_POST["rights"]);
			}
			break;
		case "add_favorite":
			if (
			...
			)
			{
				...
				CUserOptions::SetOption("thurlyos", $optionName, $userOption);

				$favoriteItemsUserAdded = CUserOptions::GetOption("thurlyos", $optionNameUserFavoriteItems);
				...
				CUserOptions::SetOption("thurlyos", $optionNameUserFavoriteItems, $favoriteItemsUserAdded);
			}
			break;
		case "delete_favorite":
			...
			foreach ($arStatus as $status)
			{
				if (isset($userOption["menu-favorites"][$status]) && in_array($menuItemID, $userOption["menu-favorites"][$status]))
				{
					...
					if (empty($userOption))
						CUserOptions::DeleteOption("thurlyos", $optionName);
					else
						CUserOptions::SetOption("thurlyos", $optionName, $userOption);
					break;
				}
		...
		case "add_favorite_admin":
			...
			{
				$adminOption = COption::GetOptionString("thurlyos", "admin_menu_items");
				if ($adminOption)
				...
				if (is_array($adminOption) && !in_array($menuItemID, $adminOption))
				{
					...
					COption::SetOptionString("thurlyos", "admin_menu_items", $adminOption, false, SITE_ID);
				}
			}
			else
			{
		...
		case "sort_sections":
			...
			{
				CUserOptions::SetOption("thurlyos", $optionSectionsName, $_POST["all_sections"]);
			}
			else
			{
				$error = "Y";
