...
if(!function_exists("CheckMenuAdminItems"))
{
	function CheckMenuAdminItems(&$adminFavoriteOption, $arMenuItemsId)
	{
		if (is_array($adminFavoriteOption) && !empty($adminFavoriteOption))
		{
			...
			if ($isChanged && $GLOBALS['USER']->CanDoOperation('thurlyos_config'))
			{
				COption::SetOptionString("thurlyos", "admin_menu_items", serialize($adminFavoriteOption), false, SITE_ID);
			}
		}
	}
}
if(!function_exists("CheckMenuUserItems"))
{
	function CheckMenuUserItems(&$userMenuOption, $arMenuItemsId)
	{
		if (is_array($userMenuOption) && !empty($userMenuOption))
		{
			...
			if ($isChanged)
			{
				CUserOptions::SetOption("thurlyos", "user_menu_items_".SITE_ID, $userMenuOption);
			}
		}
	}
}
if(!function_exists("CheckFavouriteItemsUserAdded"))
{
	function CheckFavouriteItemsUserAdded(&$favoriteItemsUserAdded, $userMenuOption, $arMenuItemsId)
	{
		if (is_array($favoriteItemsUserAdded))
		{
			...
			if ($isChanged)
			{
				CUserOptions::SetOption("thurlyos", "user_added_favorite_items_".SITE_ID, $favoriteItemsUserAdded);
			}
		}
	}
}
if(!function_exists("CheckMenuUserSections"))
{
	function CheckMenuUserSections(&$userMenuSectionsOption, $arTitles)
	{
		if (is_array($userMenuSectionsOption))
		{
			...
			if ($isChanged)
			{
				CUserOptions::SetOption("thurlyos", "user_menu_sections_".SITE_ID, $userMenuSectionsOption);
			}
		}
	}
}
...
//sections order
$userMenuSectionsOption = CUserOptions::GetOption("thurlyos", "user_menu_sections_".SITE_ID);

//admin option (items were added to favorite by admin)
$adminFavoriteOption = COption::GetOptionString("thurlyos", "admin_menu_items");
if ($adminFavoriteOption)
	$adminFavoriteOption = unserialize($adminFavoriteOption);

//user menu option
$userMenuOption = CUserOptions::GetOption("thurlyos", $optionNameUserMenu);

//user added items to favorite (not admin)
$favoriteItemsUserAdded = CUserOptions::GetOption("thurlyos", "user_added_favorite_items_".SITE_ID);
...
// sort sections
if (is_array($userMenuSectionsOption) && !empty($userMenuSectionsOption))
{
	CheckMenuUserSections($userMenuSectionsOption, array_keys($arResultTitleItemsNew));

	$arResultTitleItemsNew2 = array();
	foreach($userMenuSectionsOption as $key=>$section)
	{
		$arResultTitleItemsNew2[$section] = $arResultTitleItemsNew[$section];
	}
	$arResultTitleItemsNew = $arResultTitleItemsNew2;
}
//-- sort sections
...
if ($userMenuOption)
{
	...
	if ($isChanged)
		CUserOptions::SetOption("thurlyos", $optionNameUserMenu, $userMenuOption);
}
