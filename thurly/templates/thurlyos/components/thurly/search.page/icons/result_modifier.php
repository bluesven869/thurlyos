...
if (IsModuleInstalled('socialnetwork'))
{
	...
	{
		if ($key === "iblock_library")
			$key = "socialnetwork_library";
		$newDropDown[$key] = $value;
	}
	$arResult["DROPDOWN"] = $newDropDown;
}
	...
	foreach($arResult["SEARCH"] as $i=>$arItem)
	{
		if($arItem["MODULE_ID"] ===  "intranet" && substr($arItem["ITEM_ID"], 0, 1) === "U")
		{
			...
			if($arUser)
			{
				...
				if (IsModuleInstalled("thurlyos") && !empty($arUser["CONFIRM_CODE"]))
				{
