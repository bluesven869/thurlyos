...
<?endif;?>
<div class="pb-popup-mobile" data-role="mobile-log-bp-wf" data-rendered="" data-tasks="<?=htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($jsTasks))?>" data-workflow-id="<?=htmlspecialcharsbx($arParams["WORKFLOW_ID"])?>">
		...
		<span class="wf_status bp-status" style="display: none">
			...
			<? if ($task['IS_INLINE'] == 'Y'):
				foreach ($task['BUTTONS'] as $control):
					...
					$props = htmlspecialcharsbx(\Thurly\Main\Web\Json::encode(array(
						'TASK_ID' => $task['ID'],
						'WORKFLOW_ID' => $arParams["WORKFLOW_ID"],
