...
{
	$config = \Thurly\Crm\Conversion\QuoteConversionConfig::load();
	if($config === null)
	{
		$config = \Thurly\Crm\Conversion\QuoteConversionConfig::getDefault();
	}
...
$arFields['CONTACT_IDS'] = \Thurly\Crm\Binding\QuoteContactTable::getQuoteContactIDs($arResult['ELEMENT_ID']);
...
$arResult['FIELDS']['tab_1'][] = array(
	...
	'value' => isset($arResult['ELEMENT']['LEAD_TITLE'])
		? ($CCrmQuote->cPerms->HavePerm('LEAD', BX_CRM_PERM_NONE, 'READ')
			...
			'<script type="text/javascript">BX.tooltip("LEAD_'.$arResult['ELEMENT']['~LEAD_ID'].'", "balloon_'.$arResult['GRID_ID'].'_L_'.$arResult['ELEMENT']['LEAD_ID'].'", "/thurly/components/thurly/crm.lead.show/card.ajax.php", "crm_balloon_lead", true);</script>'
		) : '',
	'type' => 'custom',
	'isTactile' => true
);
	...
	'value' => isset($arResult['ELEMENT']['DEAL_TITLE'])
		? (!CCrmDeal::CheckReadPermission($arResult['ELEMENT']['~DEAL_ID'], $userPermissions)
			...
			'<a href="'.$arResult['PATH_TO_DEAL_SHOW'].'" id="balloon_'.$arResult['GRID_ID'].'_D_'.$arResult['ELEMENT']['DEAL_ID'].'">'.$arResult['ELEMENT']['DEAL_TITLE'].'</a>'.
				'<script type="text/javascript">BX.tooltip("DEAL_'.$arResult['ELEMENT']['~DEAL_ID'].'", "balloon_'.$arResult['GRID_ID'].'_D_'.$arResult['ELEMENT']['DEAL_ID'].'", "/thurly/components/thurly/crm.deal.show/card.ajax.php", "crm_balloon_deal", true);</script>'
		) : '',
	'type' => 'custom',
	'isTactile' => true
);
...
if(CCrmCompany::CheckReadPermission(0, $userPermissions) || CCrmContact::CheckReadPermission(0, $userPermissions))
{
	...
	$contactBindings = \Thurly\Crm\Binding\QuoteContactTable::getQuoteBindings($arParams['ELEMENT_ID']);
	...
	$requisteBindings = Thurly\Crm\Requisite\EntityLink::getByEntity(CCrmOwnerType::Quote, $arParams['ELEMENT_ID']);
...
$arResult['USER_FIELD_COUNT'] = $CCrmUserType->AddFields(
	...
	array(
		'FILE_URL_TEMPLATE' =>
			"/thurly/components/thurly/crm.quote.show/show_file.php?ownerId=#owner_id#&fieldName=#field_name#&fileId=#file_id#",
		'IS_TACTILE' => true
	)
);
// <-- ADDITIONAL SECTION
...
include_once($_SERVER['DOCUMENT_ROOT'].'/thurly/components/thurly/crm.quote/include/nav.php');
?>
