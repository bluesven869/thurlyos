...
elseif(COption::GetOptionString("sale", "secure_1c_exchange", "N") == "Y" && !check_thurly_sessid())
{
...
else
{
				...
				function getImporter($loader, $params=array())
				{
					...
					switch($class)
					{
						case 'ImportOneCPackageSale':
							if(CModule::IncludeModule('CRM'))
							{
								\Thurly\Sale\Exchange\ImportOneCPackageCRM::configuration();
								$loader->importer = \Thurly\Sale\Exchange\ImportOneCPackageCRM::getInstance();
							}
							else
							{
								\Thurly\Sale\Exchange\ImportOneCPackageSale::configuration();
								$loader->importer = \Thurly\Sale\Exchange\ImportOneCPackageSale::getInstance();
							}
							break;
                        case 'ImportOneCContragent':
							$loader->importerContragent = new \Thurly\Sale\Exchange\ImportOneCContragent();
							break;
						default;
	...
	elseif($_GET["mode"] == "info")
	{
		...
		<<?=GetMessage("CC_BSC1_DI_GENERAL")?>>
            ...
            <?
            $deliveryList = \Thurly\Sale\Delivery\Services\Manager::getActiveList(true);
