...
for($i=0;$i<112;$i++) 
{
	...
	echo ($i%16==0 && $i >= 16) ? "</tr>" : "";
	echo ($i%16==0) ? "<tr>" : "";
	echo '<td bgcolor="'.$t_curCOL.'" onmousedown="alterfont(\''.$t_curCOL.'\', \'COLOR\')"><img src="/thurly/images/1.gif" border="0" width="15" height="15"></td>';
}
?>
</tr>
</table>
...
foreach($arResult["Smiles"] as $arSmiles)
{
	...
	?>
	<td onmousedown="emoticon('<?=$arSmiles["TYPE"]?>')" ><img src="/thurly/images/blog/smile/<?=$arSmiles["IMAGE"]?>" width="<?=$arSmiles["IMAGE_WIDTH"]?>" height="<?=$arSmiles["IMAGE_HEIGHT"]?>" title="<?=$arSmiles["LANG_NAME"]?>"style="cursor:pointer"></td>
	<?
	$i++;
}
?>
...
{
	win = window.open('<?=$APPLICATION->GetCurPageParam("image_upload=Y&".thurly_sessid_get())?>',null,'height=150,width=400');
}
...
blogVars = {'templatePath' : '/thurly/components/thurly/blog/templates/blog.post.edit/.default/'};
...
function ShowVideoInput()
{
	...
	for (var i = 0; i < scripts.length; i++)
	{
		...
		if (thisScript.src && sSrc != 'thurly/js/main/utils.js' && sSrc != 'thurly/js/main/admin_tools.js' &&
			sSrc != '/thurly/js/main/utils.js' && sSrc != '/thurly/js/main/admin_tools.js') 
		{
