...
if($action == 'DELETE')
{
	...
	if ($isSpam && !$isOutgoing && \CModule::includeModule('mail'))
	{
		...
		while ($mailbox = $res->fetch())
		{
			...
			{
				$blacklistCommunications = \CCrmActivity::getCommunications($ID);
				break;
			}
		}
	}
	...
	{
		if (!empty($blacklistCommunications))
		{
			$blacklist = array();
			foreach ($blacklistCommunications as $item)
			{
			...
			if (!empty($blacklist))
			{
				$existsEntries = \Thurly\Mail\BlacklistTable::getList(array(
					'select' => array('ITEM_VALUE'),
					'filter' => array(
						'MAILBOX_ID'  => $mailbox['ID'],
						'ITEM_TYPE'   => \Thurly\Mail\Blacklist\ItemType::EMAIL,
				...
				if (!empty($blacklist))
				{
					...
					{
						\Thurly\Mail\BlacklistTable::add(array(
							'SITE_ID'    => SITE_ID,
							'MAILBOX_ID' => $mailbox['ID'],
							'ITEM_TYPE'  => \Thurly\Mail\Blacklist\ItemType::EMAIL,
							'ITEM_VALUE' => $item,
...
elseif($action == 'COMPLETE')
{
	...
	if(CCrmActivity::Complete($ID, $completed, array('REGISTER_SONET_EVENT' => true)))
	{
		...
		__CrmActivityEditorEndResonse($responseData);
	}
	else
	{
...
elseif($action == 'POSTPONE')
{
	...
	if($offset <= 0)
	{
		__CrmActivityEditorEndResonse(array('ERROR' => 'Invalid offset'));
	}

	$arUpdatedFields = CCrmActivity::Postpone($ID, $offset, array('FIELDS' => $arActivity));
	if(!empty($arUpdatedFields))
	{
		__CrmActivityEditorEndResonse(array('ITEM_ID' => $ID, 'FIELDS' => $arUpdatedFields));
	}
	else
	{
		__CrmActivityEditorEndResonse(array('ERROR' => 'Postpone denied.'));
	}
}
...
elseif($action == 'SAVE_ACTIVITY')
{
	...
	if(!$disableStorageEdit)
	{
		...
		elseif($storageTypeID === CCrmActivityStorageType::WebDav || $storageTypeID === CCrmActivityStorageType::Disk)
		{
			...
			{
				$arFields['STORAGE_ELEMENT_IDS'] = Thurly\Crm\Integration\StorageManager::filterFiles($arFileIDs, $storageTypeID, $userID);
			}
		}
	}
...
elseif($action == 'SAVE_EMAIL')
{
	...
	$rawData = (array) \Thurly\Main\Application::getInstance()->getContext()->getRequest()->getPostList()->getRaw('DATA');
	...
	foreach (array('to', 'cc', 'bcc') as $field)
	{
		if (!empty($rawData[$field]) && is_array($rawData[$field]))
		{
			foreach ($rawData[$field] as $item)
			{
				try
				{
					$item = \Thurly\Main\Web\Json::decode($item);
	...
	foreach ($commData as &$commDatum)
	{
		...
		if (isset($commDatum['isEmail']) && $commDatum['isEmail'] == 'Y' && strtolower(trim($commValue)))
		{
			$newEntityTypeId = \Thurly\Crm\Settings\ActivitySettings::getCurrent()->getOutgoingEmailOwnerTypeId();
			if (\CCrmOwnerType::Contact == $newEntityTypeId && \CCrmContact::checkCreatePermission())
			{
				$contactFields = array(
					...
					'FM'             => array(
						'EMAIL' => array(
							'n1' => array(
								'VALUE_TYPE' => 'WORK',
								'VALUE'      => strtolower(trim($commValue))
							)
						)
					),
				);
				...
				if ($contactFields['NAME'] == '' && $contactFields['LAST_NAME'] == '')
					$contactFields['NAME'] = strtolower(trim($commValue));
	...
	if ($ownerBinded)
	{
		if (!\CCrmActivity::checkUpdatePermission($ownerTypeID, $ownerID))
		{
			...
			if (\CCrmOwnerType::Contact == $ownerTypeID)
				...
				$errorMsg = getMessage('CRM_LEAD_UPDATE_PERMISSION_DENIED', array('#TITLE#' => $entityTitle));
			else if (\CCrmOwnerType::Deal == $ownerTypeID)
				$errorMsg = getMessage('CRM_DEAL_UPDATE_PERMISSION_DENIED', array('#TITLE#' => $entityTitle));
	...
	else
	{
		...
		if (!$ownerBinded)
		{
			__CrmActivityEditorEndResonse(array(
				'ERROR' => getMessage(
					empty($arBindings)
	...
	if ($from == '')
	{
		...
		if ($from == '')
			__CrmActivityEditorEndResonse(array('ERROR' => getMessage('CRM_ACTIVITY_EMAIL_EMPTY_FROM_FIELD')));
	...
	else
	{
		...
		if (!check_email($fromEmail, true))
			__CrmActivityEditorEndResonse(array('ERROR' => getMessage('CRM_ACTIVITY_INVALID_EMAIL', array('#VALUE#' => $from))));
					...
					? array_map(
						function ($item)
						{
							...
							return ltrim($item, join(array(
								'n', // \Thurly\Disk\Uf\FileUserType::NEW_FILE_PREFIX
							)));
						},
						$data['__diskfiles']
					) : array()
	...
	if (isset($arFields['STORAGE_ELEMENT_IDS']) && !empty($arFields['STORAGE_ELEMENT_IDS']))
	{
		...
		{
			$arRawFiles[$item] = \Thurly\Crm\Integration\StorageManager::makeFileArray($item, $storageTypeID);

			if (\CCrmContentType::Html == $contentType)
			{
				$fileInfo = \Thurly\Crm\Integration\StorageManager::getFileInfo(
	...
	\CCrmActivity::update($ID, array(
		...
		'URN'         => $urn,
		'SETTINGS'    => array(
			'IS_BATCH_EMAIL'  => !empty($data['batch']),
	...
	$rcptBcc = array();
	foreach ($to as $item)
		$rcpt[] = \Thurly\Main\Mail\Mail::encodeHeaderFrom($item, SITE_CHARSET);
	foreach ($cc as $item)
		$rcptCc[] = \Thurly\Main\Mail\Mail::encodeHeaderFrom($item, SITE_CHARSET);
	foreach ($bcc as $item)
		$rcptBcc[] = \Thurly\Main\Mail\Mail::encodeHeaderFrom($item, SITE_CHARSET);
	...
	if (empty($data['batch']))
	{
		...
		$sendResult = \Thurly\Main\Mail\Mail::send(array_merge(
			$outgoingParams,
			array(
				'TRACK_READ' => array(
					'MODULE_ID' => 'crm',
	...
	else
	{
		$postingData = array(
			...
			'TO_FIELD'      => join(', ', array_diff($cc, $rawCc)),
			'BCC_FIELD'     => implode(',', $to), // no cc or bcc for batch
			'SUBJECT'       => $outgoingSubject,
			'BODY_TYPE'     => 'html',
			'BODY'          => $outgoingBody,
			'DIRECT_SEND'   => 'Y',
	...
	if (!empty($userImap['need_sync']) || !empty($crmImap['need_sync']))
	{
		...
		$outgoing = new \Thurly\Mail\DummyMail(array_merge(
			$outgoingParams,
			array(
...
elseif($action == 'SEARCH_COMMUNICATIONS')
{
	...
	{
		if(!\Thurly\Crm\Security\EntityAuthorization::checkReadPermission(
	...
	if ($_REQUEST['soc_net_log_dest'] == 'search_email_comms')
	{
		...
		foreach ($data as $key => $item)
		{
			...
			foreach ($item['communications'] as $subItem)
			{
				...
				$response[strtoupper($itemType)][$itemId] = array(
					'id'         => $itemId,
		...
		if (count($words) > 0)
		{
			...
			if (\Thurly\Main\Config\Option::get('socialnetwork', 'email_users_all', 'N') != 'Y')
			{
				$res = \Thurly\Main\FinderDestTable::getList(array(
					'order'  => array(),
					'filter' => array(
				...
				$res = \Thurly\Main\UserTable::getList(array(
					'runtime' => array(
						new \Thurly\Main\Entity\ReferenceField(
							'FINDER_DEST', 'Thurly\Main\FinderDest',
							array(
								'=this.ID'     => 'ref.CODE_USER_ID',
								'=ref.USER_ID' => new \Thurly\Main\DB\SqlExpression('?i', $curUser->getId()),
							)
						),
						new \Thurly\Main\Entity\ExpressionField(
							'MAX_LAST_USE_DATE',
		...
		__CrmActivityEditorEndResonse($response);
	}
	else
	{
	...
	__CrmActivityEditorEndResonse(
		array(
			'DATA' => array(
				...
				'INFO' => \Thurly\Crm\Integration\StorageManager::getFileInfo(
					$elementID,
					\Thurly\Crm\Integration\StorageType::WebDav
				)
			)
		)
	);
...
elseif($action == 'PREPARE_MAIL_TEMPLATE')
{
	...
	$files = !empty($files) && is_array($files) && \CModule::includeModule('disk')
		? \Thurly\Disk\Uf\FileUserType::getItemsInfo($files) : array();

	foreach ($files as $k => $item)
	{
		if (strpos($item['id'], 'n') !== 0)
		{
...
elseif($action == 'UPDATE_DOCS')
{
	...
	foreach ($docsItems as $item)
	{
		...
		{
			$errors[] = __CrmActivityEditorEndResonse(array('ERROR' => 'Invalid parameters!'));
			continue;
		}
	...
	if (!empty($errors))
		__CrmActivityEditorEndResonse(array('ERROR' => $errors));
