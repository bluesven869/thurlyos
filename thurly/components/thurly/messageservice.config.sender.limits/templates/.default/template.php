...
?>
<div class="messageservice-view-progress messageservice-view-progress-show"><?=GetMessage('MESSAGESERVICE_CONFIG_SENDER_LIMIT_TIP')?></div>
<?
...
<script>
	BX.ready(function()
	{
		BX.message(<?=\Thurly\Main\Web\Json::encode(array(
		...
		var retryTime = <?=\Thurly\Main\Web\Json::encode($arResult['RETRY_TIME'])?>;
		...
		BX.bind(saveNode, 'click', function()
		{
			...
			BX.ajax({
				method: 'POST',
				dataType: 'json',
				url: '/thurly/components/thurly/messageservice.config.sender.limits/ajax.php',
				data: {
