...
CThurlyComponent::includeComponentClass("thurly:sale.location.selector.search");

Loc::loadMessages(__FILE__);

class CThurlyLocationSelectorSystemComponent extends CThurlyLocationSelectorSearchComponent
{
	const ID_BLOCK_LEN = 			90;
	const HUGE_TAIL_LEN = 			30;
	const PAGE_SIZE = 				10;
	const LOCATION_ENTITY_NAME = 	'\Thurly\Sale\Location\LocationTable';
	 ...
	 *
	 * @return Thurly\Main\DB\ArrayResult $result
	 */
	...
	protected function obtainDataConnectors()
	{
		...
		if($this->useGroups)
		{
			...
			if($this->groupsFromRequest !== false)
				$res = self::getEntityListByListOfPrimary('Thurly\Sale\Location\GroupTable', $this->groupsFromRequest, $parameters, $linkFld);
	...
	protected function checkParameters()
	{
		...
		else
		{
			...
			{
				$a = new $class();
				$isInstace = ($a instanceof \Thurly\Sale\Location\Connector);
			}
			catch(\Exception $e)
			{
			}
