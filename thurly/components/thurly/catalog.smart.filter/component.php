...
/** @var CThurlyCatalogSmartFilter $this */
/** @var array $arParams */
/** @var array $arResult */
	...
	if(!empty($arResult["ITEMS"]))
	{
		if ($this->facet->isValid())
		{
			...
			while ($row = $res->fetch())
			{
				...
				if (\Thurly\Iblock\PropertyIndex\Storage::isPropertyId($facetId))
				{
					$PID = \Thurly\Iblock\PropertyIndex\Storage::facetIdToPropertyId($facetId);
				...
				{
					$priceId = \Thurly\Iblock\PropertyIndex\Storage::facetIdToPriceId($facetId);
	...
	if ($arResult["FACET_FILTER"])
	{
		if (!$this->facet->isEmptyWhere())
		{
			...
			while ($row = $res->fetch())
			{
				...
				if (\Thurly\Iblock\PropertyIndex\Storage::isPropertyId($facetId))
				{
					$pp = \Thurly\Iblock\PropertyIndex\Storage::facetIdToPropertyId($facetId);
...
$uri->deleteParams(\Thurly\Main\HttpRequest::getSystemParameters());
$pageURL = $uri->GetUri();
...
if ($arParams["XML_EXPORT"] === "Y" && $_REQUEST["mode"] === "xml")
{
	...
	$error = "";
	echo \Thurly\Main\Text\Encoding::convertEncoding($xml, LANG_CHARSET, "utf-8", $error);
	CMain::FinalActions();
	die();
}
