...
if ($bSearchInstalled && (strlen($arResult["~q"]) > 0 || strlen($arResult["~tags"]) > 0))
{
	...
	if ($obSearch->errorno == 0)
	{
		...
		while ($arSearch = $obSearch->GetNext())
		{
			...
			$arImage = CSocNetTools::InitImage($arGroup["IMAGE_ID"], 100, "/thurly/images/socialnetwork/nopic_group_100.gif", 100, $arSearch["URL"], true);
...
else
{
	...
	while ($arGroup = $dbGroups->GetNext())
	{
		...
		$arImage = CSocNetTools::InitImage($arGroup["IMAGE_ID"], 100, "/thurly/images/socialnetwork/nopic_group_100.gif", 100, $arGroup["URL"], true);
