...
global $APPLICATION;
$APPLICATION->AddHeadScript('/thurly/js/crm/interface_form.js');
...
foreach($arResult['FIELDS']['tab_1'] as $field):
	...
	if (strpos($field['id'], 'UF_') !== false):
		...
		if($field['id'] == 'CONTACT_COMPANY_ID' || $field['type'] == 'section')
			continue;
		elseif ($field['id'] == 'CONTACT_COMMENTS')
			$field['value'] = str_replace(":'COMMENTS'", ":'CONTACT_COMMENTS'", $field['value']);
		$arFields[] = $field;
	endif;
endforeach;
$arResult['FIELDS']['tab_1'] = array_merge($arFields, $arrAddRequiredFields, $arrAddFields);

$bChecked = !isset($_POST['CONVERT_CONTACT']) || $_POST['CONVERT_CONTACT'] == 'Y';
?>
...
<tr  style="background-color: #ededed;">
	<td class="bx-padding" colspan="2" style="background:none; text-align: center; font-weight: bold;">
			<input type="radio" value="Y" id="CONVERT_CONTACT_Y" name="CONVERT_CONTACT" <?=($bChecked ? 'checked' : '')?>
			onclick="var checked = this.checked; BX('CONTACT_form').style.display = checked ? '' : 'none'; BX('CONTACT_choise').style.display = !checked ? '' : 'none';" /><label for="CONVERT_CONTACT_Y" style="padding-right: 70px;"><?=GetMessage('CRM_FIELD_CONVERT_CONTACT')?></label>
			<input type="radio" value="N" id="CONVERT_CONTACT_N" name="CONVERT_CONTACT" <?=(!$bChecked ? 'checked' : '')?>
			onclick="var checked = this.checked; BX('CONTACT_form').style.display = !checked ? '' : 'none'; BX('CONTACT_choise').style.display = checked ? '' : 'none';"/><label for="CONVERT_CONTACT_N"><?=GetMessage('CRM_FIELD_CONVERT_CONTACT_LIST')?></label>
	</td>
</tr>
<tr id="CONTACT_choise" style="display:<?=(!$bChecked ? '' : 'none')?>">
	<td class="bx-field-name bx-padding" style="background:none"><span class="required">*</span><?=GetMessage('CRM_FIELD_CONVERT_CONTACT_CHOISE')?>:</td>
	<td class="bx-field-value" style="background:none">
	<?
$GLOBALS["APPLICATION"]->IncludeComponent('thurly:crm.entity.selector', 
	'', 
	array(
		'ENTITY_TYPE' => 'CONTACT',
		'INPUT_NAME' => 'CONTACT_CONTACT_ID',
...
<div class="bx-edit-table" id="CONTACT_form" style="display:<?=($bChecked ? 'block' : 'none')?>">
<table cellpadding="0" cellspacing="0" border="0" class="bx-edit-table" id="tab_convert_contact_edit_table">
<?
$i = 0;
...
foreach($arResult['FIELDS']['tab_1'] as $field):
	...
	if ($sFirstAddField == $field["id"]):
		...
		?>
		<tr id="tr_contact_add_fields"><td colspan="2" class="bx-add-fields-section"><a href="javascript:showAdditionalFields('contact')" id="contact_a"><?=GetMessage('CRM_ADDITIONAL_FIELDS')?></a></td></tr>
		<?
	endif;		
...
<?
		...
		case 'intranet_user_search':
			...
			if(!empty($params)):
				$params['SEARCH_INPUT_NAME'] = 'CONTACT_'.$params['SEARCH_INPUT_NAME'];
				$params['INPUT_NAME'] = 'CONTACT_'.$params['INPUT_NAME'];
		...
		case 'address':
			...
			$addressLabels = Thurly\Crm\EntityAddress::getLabels();
					...
					<td class="crm-offer-info-right">
						<div class="crm-offer-info-data-wrap"><?
							if(is_array($itemLocality)):
								$searchInputID = "CONTACT_{$arParams['FORM_ID']}_{$itemName}";
								...
								if(isset($item['IS_MULTILINE']) && $item['IS_MULTILINE']):
									?><textarea class="bx-crm-edit-text-area" name="CONTACT_<?=htmlspecialcharsEx($itemName)?>"><?=$itemValue?></textarea><?
								else:
									?><input class="crm-offer-item-inp" name="CONTACT_<?=htmlspecialcharsEx($itemName)?>" type="text" value="<?=htmlspecialcharsEx($itemValue)?>" /><?
								endif;
							endif;
						?></div>
					</td>
...
?><script type="text/javascript">
	BX.ready(
		function()
		{<?
			$defaultNameFormat = CSite::GetNameFormat(false);
			foreach($arUserSearchFields as &$arField):
				...
				if(isset($arField['USER'])):
					...
					$arUserData['name'] = CUser::FormatName(($nameFormat != '' ? $nameFormat : $defaultNameFormat), $arField['USER'], true, false);
				endif;
			?>
