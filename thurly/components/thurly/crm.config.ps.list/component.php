...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && check_thurly_sessid() && isset($_POST['action_button_'.$arResult['GRID_ID']]))
{
...
elseif ($_SERVER['REQUEST_METHOD'] == 'GET' && check_thurly_sessid() && isset($_GET['action_'.$arResult['GRID_ID']]))
{
...
$dbPaySystems = \Thurly\Sale\PaySystem\Manager::getList(
	array(
		'filter' => array("!ID" => \Thurly\Sale\PaySystem\Manager::getInnerPaySystemId())
	)
);

while ($paySystem = $dbPaySystems->fetch())
{
	$pt2psList = \Thurly\Sale\PaySystem\Manager::getPersonTypeIdList($paySystem['ID']);
	...
	$tmpPS['PATH_TO_PS_DELETE'] =
		CHTTP::urlAddParams(
			...
			array('action_'.$arResult['GRID_ID'] => 'delete', 'ID' => $paySystem['ID'], 'sessid' => thurly_sessid())
		);
