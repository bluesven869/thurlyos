...
if(!function_exists('__CrmPSListEndResonse'))
{
	function __CrmPSListEndResonse($result)
	{
...
{
	__CrmPSListEndResonse(array('ERROR' => 'Could not include crm module.'));
}
...
{
	__CrmPSListEndResonse(array('ERROR' => 'Access denied.'));
}
...
elseif ($action === 'CONVERT_PS_REQUISITES')
{
	...
	{
		__CrmPSListEndResonse(array('ERROR' => 'Could not include sale module.'));
	}
	...
	{
		__CrmPSListEndResonse(array('ERROR' => $e->getMessage()));
	}

	$progressData = $converter::getProgressData();

	if (!is_array($progressData))
		__CrmPSListEndResonse(array('ERROR' => 'Invalid progress data.'));
