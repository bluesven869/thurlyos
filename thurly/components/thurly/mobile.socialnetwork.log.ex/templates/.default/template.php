...
use \Thurly\Main\Data\AppCacheManifest;
use \Thurly\Main\Page\Asset;
use \Thurly\Main\Loader;
use \Thurly\Main\UI;

$APPLICATION->AddHeadScript("/thurly/components/thurly/mobile.socialnetwork.log.ex/templates/.default/script_attached.js");
$APPLICATION->AddHeadScript("/thurly/components/thurly/rating.vote/templates/mobile_like/script_attached.js");
$APPLICATION->AddHeadScript("/thurly/components/thurly/rating.vote/templates/mobile_comment_like/script_attached.js");
$APPLICATION->AddHeadScript(SITE_TEMPLATE_PATH."/components/thurly/voting.current/.userfield/script.js");
if (CModule::IncludeModule("vote") && class_exists("\\Thurly\\Vote\\UF\\Manager"))
{
	$APPLICATION->AddHeadScript("/thurly/components/thurly/voting.uf/templates/.default/script.js");
	$APPLICATION->AddHeadString('<link href="'.CUtil::GetAdditionalFileURL('/thurly/components/thurly/voting.uf/templates/.default/style.css').'" type="text/css" rel="stylesheet" />');
}
else if (IsModuleInstalled("vote"))
{
	$APPLICATION->AddHeadString('<link href="'.CUtil::GetAdditionalFileURL(SITE_TEMPLATE_PATH.'/components/thurly/voting.current/.userfield/style.css').'" type="text/css" rel="stylesheet" />');
}
...
else
{
	...
	if ($arResult["RELOAD"])
	{
		if ($arResult["RELOAD_JSON"])
		{
			...
			Asset::getInstance()->startTarget('livefeed_ajax', \Thurly\Main\Page\AssetMode::STANDARD);
		}
		?><script>
	...
	if ($arParams["EMPTY_PAGE"] == "Y")
	{
		...
		AppCacheManifest::getInstance()->addAdditionalParam("MobileAPIVersion", CMobile::getApiVersion());
		AppCacheManifest::getInstance()->addAdditionalParam("MobilePlatform", CMobile::getPlatform());
		AppCacheManifest::getInstance()->addAdditionalParam("version", "v5");
	...
	)
	{
		...
		foreach ($arResult["Events"] as $arEvent)
		{
			...
			else
			{
				...
				$APPLICATION->IncludeComponent(
					"thurly:mobile.socialnetwork.log.entry",
					"",
					$arComponentParams,
					$component,
					Array("HIDE_ICONS" => "Y")
	...
	)
	{
		$APPLICATION->RestartBuffer();
		$res = array(
			'ERROR_MESSAGE' => \Thurly\Main\Localization\Loc::getMessage('MOBILE_LOG_ERROR_ENTRY_NOT_FOUND')
		);
	...
	if ($arResult["AJAX_CALL"])
	{
		...
		?><script>
			...
			{
				?>
				url_next = '<?=$APPLICATION->GetCurPageParam($strParams, array("LAST_LOG_TS", "AJAX_CALL", "RELOAD", "PAGEN_".$arResult["PAGE_NAVNUM"], "pagesize"));?>';
				<?
			}
			else
			{
	...
	if ($arParams["NEW_LOG_ID"] <= 0)
	{
		...
		?><script type="text/javascript">
			...
			var url_next = '<?=$APPLICATION->GetCurPageParam($strParams, array("LAST_LOG_TS", "AJAX_CALL", "RELOAD", "PAGEN_".$arResult["PAGE_NAVNUM"], "pplogid", "pagesize"));?>';
			<?
			if (
		...
		if ($arResult["RELOAD"])
		{
			...
			if ($arResult["RELOAD_JSON"])
			{
				...
				AddEventHandler("main", "OnEndBufferContent", function($staticContent) use($strText, $arResult)
				{
					...
					$resources = \Thurly\Main\Page\Asset::getInstance()->getAssetInfo('livefeed_ajax', \Thurly\Main\Page\AssetMode::STANDARD);
					$files1 = array(
