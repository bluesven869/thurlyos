...
elseif (!$this->__component->__parent || strpos($this->__component->__parent->__name, "photogallery") === false):
	$GLOBALS['APPLICATION']->SetAdditionalCSS('/thurly/components/thurly/photogallery/templates/.default/style.css');
	$GLOBALS['APPLICATION']->SetAdditionalCSS('/thurly/components/thurly/photogallery/templates/.default/themes/gray/style.css');
endif;
...
else
{
	...
	if ($GLOBALS['USER']->IsAuthorized())
	{
		...
		if ($_REQUEST["template"] && check_thurly_sessid() && $arTemplateParams["template"] != $_REQUEST["template"])
		{
...
if (typeof(phpVars) != "object")
	var phpVars = {};
phpVars.thurly_sessid = '<?=thurly_sessid()?>';
</script>
<noindex>
				...
				<ul class="photo-controls photo-control-photo-templates">
					<li class="photo-control-photo-template-square<?=($arParams["TEMPLATE"] == "square" ? " photo-control-photo-template-square-active" : "")?>">
						<a rel="nofollow" href="<?=$APPLICATION->GetCurPageParam(
							"template=square".($GLOBALS["USER"]->IsAuthorized() ? "&".thurly_sessid_get() : ""),
					...
					<li class="photo-control-photo-template-rectangle<?=($arParams["TEMPLATE"] == "rectangle" ? " photo-control-photo-template-rectangle-active" : "")?>">
						<a rel="nofollow" href="<?=$APPLICATION->GetCurPageParam(
							"template=rectangle".($GLOBALS["USER"]->IsAuthorized() ? "&".thurly_sessid_get() : ""),
							array("template", "sessid"))?>" <?
					...
					<li class="photo-control-photo-template-default<?=($arParams["TEMPLATE"] == "default" ? " photo-control-photo-template-default-active" : "")?>">
						<a rel="nofollow" href="<?=$APPLICATION->GetCurPageParam(
							"template=default".($GLOBALS["USER"]->IsAuthorized() ? "&".thurly_sessid_get() : ""),
							array("template", "sessid"))?>" <?
...
<form action="<?=POST_FORM_ACTION_URI?>" method="post" id="photoForm" class="photo-form" onsubmit="return false;">
	<input type="hidden" name="sessid" value="<?=thurly_sessid()?>" />
