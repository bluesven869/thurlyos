...
if (!empty($arResult['ITEMS']))
{
	...
	foreach ($arResult['ITEMS'] as $key => $arItem)
	{
		...
		$arItem['LABEL_VALUE'] = $arParams['TEXT_LABEL_GIFT']?: Loc::getMessage('SGS_TPL_TEXT_LABEL_GIFT_DEFAULT');
		$arItem['LABEL'] = !empty($arItem['LABEL_VALUE']);
		// !Item Label Properties

		if(parse_url($arItem['DETAIL_PAGE_URL'], PHP_URL_FRAGMENT))
		{
			$arItem['DETAIL_PAGE_URL'] .= ',as_gift';
		}
		else
		{
			$arItem['DETAIL_PAGE_URL'] .= '#as_gift';
		...
		if ($arItem['CATALOG'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS']))
		{
			...
			\Thurly\Main\Type\Collection::sortByColumn($arItem['OFFERS'], $arSortFields);
