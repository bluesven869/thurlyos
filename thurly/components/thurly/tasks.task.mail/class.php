...
CThurlyComponent::includeComponentClass("thurly:tasks.task");
...
class TasksMailTaskComponent extends TasksTaskComponent
{
	...
	protected function processExecutionStart()
	{
		...
		$this->replaceUser = !\Thurly\Tasks\Util\User::getId() && \Thurly\Tasks\Integration\Forum::includeModule();
		if($this->replaceUser)
		{
			try
			{
			...
			catch(\Thurly\Tasks\Exception $e)
			{
				$this->replaceUser = false;
			...
			$GLOBALS['USER'] = new \Thurly\Forum\Comments\User($taskData['CREATED_BY']); // dont pass admin id here, its too far dangerous
		}
	...
	protected function processExecutionEnd()
	{
		...
		{
			\Thurly\Tasks\Integration\Mail::stopMailEventCompiler();
		}
	}
	...
	protected function checkParameters()
	{
		...
		static::tryParseNonNegativeIntegerParameter($this->arParams['RECIPIENT_ID'], \Thurly\Tasks\Util\User::getId()); // Bob

		$this->arParams['PREVIOUS_FIELDS'] = \Thurly\Tasks\Util\Type::unSerializeArray($this->arParams['~PREVIOUS_FIELDS']); // tilda is required, as "PREVIOUS_FIELDS" contains broken serialization
		...
		static::tryParseStringParameter($this->arParams["URL"], \Thurly\Tasks\Integration\Mail\Task::getDefaultPublicPath($this->arParams['ID']));
	}
	...
	protected function getDataAux()
	{
		...
		{
			$backUrl = \Thurly\Tasks\Integration\Mail\Task::getBackUrl(
		...
		{
			$fileData = \Thurly\Tasks\Integration\Disk::getAttachmentData($this->arResult['DATA']['TASK'][$ufCode]);
			foreach($fileData as $k => &$v)
			{
				if((string) $v['URL'] != '')
				{
					$v['URL'] = \Thurly\Tasks\Integration\Mail\Task::getBackUrl(
						intval($this->arParams["RECIPIENT_ID"]),
						$this->arResult['DATA']['TASK']["ID"],
						$v['URL'],
						$this->arResult['AUX_DATA']['SITE']['SITE_ID'],
