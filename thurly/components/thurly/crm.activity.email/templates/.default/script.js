...
;(function() {
	...
	BXCrmActivityEmailController.applyTemplate = function (id, ownerType, ownerId, callback)
	{
		...
		BX.ajax({
			'url': '/thurly/components/thurly/crm.activity.editor/ajax.php?action=prepare_mail_template&templateid='+id,
			'method': 'POST',
			'dataType': 'json',
			'data': {
	...
	var BXCrmActivityEmail = function (options)
	{
		...
		if ('edit' != this.ctrl.options.type)
		{
			...
			BX.bind(forwardLink, 'click', function ()
			{
				...
				window.location.href = '/thurly/components/thurly/crm.activity.planner/slider.php'
			...
			BX.bind(spamLink, 'click', this.delete.bind(this, true));
	...
	BXCrmActivityEmail.prototype.delete = function (spam)
	{
		var self = this;

		spam = spam === true;

		if (!window.confirm(BX.message(spam ? 'CRM_ACT_EMAIL_SPAM_CONFIRM' : 'CRM_ACT_EMAIL_DELETE_CONFIRM')))
			return false;
		...
		BX.ajax({
			'url': '/thurly/components/thurly/crm.activity.editor/ajax.php?id='+this.options.activityId+'&action=delete',
			'method': 'POST',
			'dataType': 'json',
			'data': data,
			onsuccess: function(data)
	...
	BXCrmActivityEmail.prototype.templateMenu = function (ownerType, ownerId, selector)
	{
		...
		var handler = function(event, item)
		{
			...
			self.ctrl.applyTemplate(item.__id, ownerType, ownerId, function (data)
			{
				...
				mailForm.getField('DATA[message]').setValue(data.BODY, {quote: true});
			});
