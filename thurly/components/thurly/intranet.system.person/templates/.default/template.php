...
<div class="bx-user-info-big">
		...
		<ul>
			...
			elseif ($arResult['CAN_EDIT_USER']):
				...
				<li class="bx-icon bx-icon-edit"><a href="javascript:<?echo $APPLICATION->GetPopupLink(
						array(
							'URL' => '/thurly/admin/user_edit.php?lang='.LANGUAGE_ID.'&bxpublic=Y&from_module=main&ID='.$arUser['ID'],
	...
	?><div class="bx-user-text<? if (!is_array($arParams['USER_PROPERTY']) || !in_array('PERSONAL_PHOTO', $arParams['USER_PROPERTY'])) { ?> no-photo<? } ?>">
		<div class="bx-user-name">
			<a href="<?=$arUser['DETAIL_URL']?>" id="system_person_<?=$arUser['ID']?>"><?=CUser::FormatName($arParams['NAME_TEMPLATE'], $arUser, $arParams["SHOW_LOGIN"] != 'N');?></a>
		</div>
		<script type="text/javascript">BX.tooltip(<?=$arUser["ID"]?>, "system_person_<?=$arUser['ID']?>", "<?=CUtil::JSEscape($arResult['Urls']['TooltipCall'])?>")</script>
