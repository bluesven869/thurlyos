			...
			if ($arDocumentFields[$key]["BaseType"] == "file")
			{
				...
				if (sizeof($ar) > 0)
				{
					...
					while ($file = $fileIterator->fetch())
					{
						if ($arRecord[$key] != '')
							$arRecord[$key] .= ' ';
						$arRecord[$key] .= '<a href="/thurly/tools/bizproc_show_file.php?bp_id=' . $arParams['BP_ID'] . '&iblock_id=' . $arParams['BLOCK_ID'] . '&f=' . urlencode($key) . '&i=' . $file['ID'] . '">' . htmlspecialcharsbx($file['ORIGINAL_NAME']) . '</a>';
					}
				}
			}
...
if (strlen($arResult["FatalErrorMessage"]) <= 0)
{
	...
	foreach ($arDocumentStates as $arDocumentState)
	{
		...
		foreach ($ar as $ar1)
		{
			$ar1["URL"] = $APPLICATION->GetCurPageParam("bizproc_id=".$arDocumentState["ID"]."&process_state_event=Y&bizproc_event=".htmlspecialcharsbx($ar1["NAME"])."&".thurly_sessid_get(), array("sessid", "stop_bizproc_id", "process_state_event", "bizproc_event", "bizproc_id"));
		...
		if ($arResult["AllowAdmin"] && strlen($arDocumentState["ID"]) > 0 && strlen($arDocumentState["WORKFLOW_STATUS"]) > 0)
			$arResult["BP"]["CancelUrl"] = $APPLICATION->GetCurPageParam("stop_bizproc_id=".$arDocumentState["ID"]."&".thurly_sessid_get(), array("sessid", "stop_bizproc_id"));
	}
}
