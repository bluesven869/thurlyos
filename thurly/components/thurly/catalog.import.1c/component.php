...
/** @global CUser $USER */
/** @global CDatabase $DB */
/** @var CThurlyCatalogImport1C $this */
/** @var array $arParams */
/** @var array $arResult */
...
)
{
	...
	if (
		isset($_GET["mode"])
		&& $_GET["mode"] !== "checkauth"
		&& check_thurly_sessid() //we got valid token from 1C
...
elseif ((!$arParams["SKIP_SOURCE_CHECK"]) && (!check_thurly_sessid()))
{
...
elseif ($_GET["mode"]=="deactivate")
{
	if ($_GET["timestamp"] > 0)
	{
		$rsImportedIBlocks = \Thurly\IBlock\IblockFieldTable::getList(array(
