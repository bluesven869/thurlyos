...
class CThurlyCatalogImport1C extends CThurlyComponent
{
	...
	public function cleanUpDirectory($directoryName)
	{
		...
		$directory = new \Thurly\Main\IO\Directory($directoryName);
		if ($directory->isExists())
		{
			if (defined("BX_CATALOG_IMPORT_1C_PRESERVE"))
			{
				$i = 0;
				while (\Thurly\Main\IO\Directory::isDirectoryExists($directory->getPath().$i))
				{
					$i++;
				}
			...
			else
			{
				foreach ($directory->getChildren() as $directoryEntry)
				{
					...
					{
						$emptyDirectory = true;
						$reportsDirectory = new \Thurly\Main\IO\Directory($directoryEntry->getPath());
