...
foreach($arParams['DATA'] as $item)
{
	...
	{
		$rule['DISPLAY'] = \Thurly\Tasks\Util\User::formatName($arResult['AUX_DATA']['USERS'][$item->getGroupId()]);
	}
	else
	{
	...
	$rule['VALUE'] = Thurly\Tasks\Util::hashCode(rand(100, 999).rand(100, 999));
	$rule['ITEM_SET_INVISIBLE'] = '';

	$data[] = $rule;
}

$arResult['TEMPLATE_DATA']['URL'] = \Thurly\Tasks\UI::convertActionPathToBarNotation(
