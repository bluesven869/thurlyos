...
else
{
	if ($_SERVER['REQUEST_METHOD'] == 'POST' && check_thurly_sessid() && $arResult["IS_EDIT_PERMITTED"])
	{
		...
		if(isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']) || isset($_POST['continue']))
		{
			...
			elseif(!$isEditMode)
			{
				$arFields['OPENED'] = \Thurly\Crm\Settings\CompanySettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N';
			}
...
$arResult['FIELDS'][] = array(
	...
	'value' => isset($arResult['ELEMENT']['OPENED'])
		? $arResult['ELEMENT']['OPENED'] : \Thurly\Crm\Settings\CompanySettings::getCurrent()->getOpenedFlag(),
);
...
if ($arParams["RESTRICTED_MODE"])
{
	$addressHtml = Thurly\Crm\Format\CompanyAddressFormatter::format(
		$arResult['ELEMENT'],
