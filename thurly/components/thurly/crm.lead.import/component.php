...
use Thurly\Crm\Integration\Channel\LeadImportTracker;
...
//Download sample
$filename = $_SERVER['DOCUMENT_ROOT'].'/thurly/components/thurly/crm.lead.import/sample.csv';
...
else if (isset($_REQUEST['import']) && isset($_SESSION['CRM_IMPORT_FILE']))
{
	...
	$dupChecker = new \Thurly\Crm\Integrity\LeadDuplicateChecker();
	$processedQty = 0;
	...
	foreach($arLeads as $arLead)
	{
			...
			&& ($enableDupCtrlByPerson || $enableDupCtrlByOrganization || $enableDupCtrlByPhone || $enableDupCtrlByEmail))
		{
			...
			$adapter = \Thurly\Crm\EntityAdapterFactory::create($arLead, CCrmOwnerType::Lead);
	...
	if($arResult['error'] > 0)
	{
		$arResult['errata_url'] = SITE_DIR.'thurly/components/thurly/crm.lead.import/show_file.php?name=errata';
	}

	if($enableDupFile && $arResult['duplicate'] > 0)
	{
		$arResult['duplicate_url'] = SITE_DIR.'thurly/components/thurly/crm.lead.import/show_file.php?name=duplicate';
	}
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_thurly_sessid())
{
	if (isset($_POST['next']))
	{
		...
		else if ($arResult['STEP'] == 3)
		{
			...
			ob_start();
			?>
				<div class="crm_import_entity"><?=GetMessage('CRM_IMPORT_FINISH')?>: <span id="crm_import_entity">0</span> <span id="crm_import_entity_progress"><img src="/thurly/components/thurly/crm.lead.import/templates/.default/images/wait.gif" align="absmiddle"></span></div>
