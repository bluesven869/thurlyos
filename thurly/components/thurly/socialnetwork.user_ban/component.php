...
else
{
	...
	/***********************  ACTIONS  *******************************/
	if ($_REQUEST["action"] == "clear_ban" && check_thurly_sessid() && IntVal($_REQUEST["eventID"]) > 0)
	{
		$errorMessage = "";
	...
	elseif ($_SERVER["REQUEST_METHOD"]=="POST" && strlen($_POST["delete"]) > 0 && check_thurly_sessid())
	{
		$errorMessage = "";
	...
	if ($dbBan)
	{
		...
		while ($arBan = $dbBan->GetNext())
		{
			...
			if (intval($arParams["THUMBNAIL_LIST_SIZE"]) > 0)
			{
				...
				$arImage = CSocNetTools::InitImage($arBan[$pref."_USER_PERSONAL_PHOTO"], $arParams["THUMBNAIL_LIST_SIZE"], "/thurly/images/socialnetwork/nopic_30x30.gif", 30, $pu, $canViewProfile);
			}
			else // old 
				$arImage = CSocNetTools::InitImage($arBan[$pref."_USER_PERSONAL_PHOTO"], 150, "/thurly/images/socialnetwork/nopic_user_150.gif", 150, $pu, $canViewProfile);
			...
			$arResult["Ban"]["List"][] = array(
				...
				"DELETE_FROM_BAN_LINK" => htmlspecialcharsbx($APPLICATION->GetCurUri("eventID=".$arBan["ID"]."&action=clear_ban&".thurly_sessid_get()."")),
