...
$arResult['AJAX_MODE'] = isset($arParams['AJAX_MODE']) ? $arParams['AJAX_MODE'] : 'Y';
		...
		elseif($arResult['CAN_EDIT'] && $action === 'edit' && isset($_POST['FIELDS']) && is_array($_POST['FIELDS']))
		{
			foreach($_POST['FIELDS'] as $ID => &$data)
			{
				if($ID === 0)
				{
					if(isset($data['NAME']))
					{
						Thurly\Crm\Category\DealCategory::setDefaultCategoryName(trim($data['NAME']));
					}
					continue;
				}
...
for($i = 0; $i < $arResult['ROWS_COUNT']; $i++)
{
	...
	$items[$i]['PATH_TO_DELETE'] =
		CHTTP::urlAddParams(
			...
			array('action_'.$arResult['GRID_ID'] => 'delete', 'ID' => $ID, 'sessid' => thurly_sessid())
		);
