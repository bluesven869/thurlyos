...
{
	AddEventHandler('crm', 'onCrmInvoiceListItemBuildMenu', array('\Thurly\Crm\CallList\CallList', 'handleOnCrmInvoiceListItemBuildMenu'));
}
...
else
{
	...
	$query = new Thurly\Main\Entity\Query(\Thurly\Crm\InvoiceTable::getEntity());
	$subQuery = new Thurly\Main\Entity\Query(\Thurly\Crm\InvoiceRecurTable::getEntity());
	...
	$invoiceFields = array_keys(\Thurly\Crm\InvoiceTable::getMap());
	...
	$recurringFields = \Thurly\Crm\InvoiceRecurTable::getFieldNames();
	...
	$query->registerRuntimeField('',
		new Thurly\Main\Entity\ReferenceField('recurring',
			Thurly\Main\Entity\Base::getInstanceByQuery($subQuery),
	...
	)
	{
		...
		$query->addFilter('@ID', new Thurly\Main\DB\SqlExpression($permissionSql));
	}
...
while($arInvoice = $obRes->GetNext())
{
	...
	$arInvoice['PATH_TO_INVOICE_DELETE'] =  CHTTP::urlAddParams(
		...
		array('action_'.$arResult['GRID_ID'] => 'delete', 'ID' => $entityID, 'sessid' => thurly_sessid())
	);
