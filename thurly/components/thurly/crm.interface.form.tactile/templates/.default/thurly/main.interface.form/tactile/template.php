...
foreach($arSections as &$arSection):
	...
	foreach($arSection['FIELDS'] as &$field):
		...
		elseif($field['type'] === 'multiple_address'):
			...
			{
				$addressTypeInfos = Thurly\Crm\EntityAddress::getClientTypeInfos();
			}
			...
			$currentAddressTypeID = Thurly\Crm\EntityAddress::Primary;
							...
							<span class="crm-offer-requisite-option-text">
								<?=htmlspecialcharsbx(
									isset($addressTypeDesc[$currentAddressTypeID])
										...
										: Thurly\Crm\EntityAddress::getTypeDescription($currentAddressTypeID)
								)?>
							</span>
						...
						<div class="crm-offer-requisite-form-wrap">
							...
							foreach($addressData as $addressTypeID => $addresFields):
								...
								$itemTitle = isset($addressTypeDesc[$addressTypeID]) ?
									...
									Thurly\Crm\EntityAddress::getTypeDescription($addressTypeID);
			...
			switch($field['type']):
					...
					case 'crm_entity_selector':
						...
						?></div>
							...
							{
								$serviceUrl = '/thurly/components/thurly/crm.contact.edit/ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get();
							...
							{
								$serviceUrl = '/thurly/components/thurly/crm.company.edit/ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get();
								...
								function()
								{
									...
									BX.CrmEntityEditor.create(
										'<?=CUtil::JSEscape($editorID.'_C')?>',
										{
											...
											'newDataInputId': '<?=CUtil::JSEscape($newDataInputID)?>',
											'entitySelectorId': entitySelectorId,
											'serviceUrl': '<?=CUtil::JSEscape('/thurly/components/thurly/crm.contact.edit/ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get())?>',
									...
									BX.CrmEntityEditor.create(
										'<?=CUtil::JSEscape($editorID).'_CO'?>',
										{
											...
											'newDataInputId': '<?=CUtil::JSEscape($newDataInputID)?>',
											'entitySelectorId': entitySelectorId,
											'serviceUrl': '<?=CUtil::JSEscape('/thurly/components/thurly/crm.company.edit/ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get())?>',
								...
								?><script type="text/javascript">
									BX.ready(
										function()
										{
											...
											BX.CrmClientPanelCommunication.callToFormat = <?=CCrmCallToUrl::GetFormat(CCrmCallToUrl::Thurly)?>;
