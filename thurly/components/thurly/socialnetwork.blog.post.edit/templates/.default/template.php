...
$APPLICATION->SetAdditionalCSS('/thurly/components/thurly/socialnetwork.log.ex/templates/.default/style.css');
$APPLICATION->SetAdditionalCSS('/thurly/components/thurly/socialnetwork.blog.blog/templates/.default/style.css');
...
HTML;
	...
	)
	{
		...
		{
			$dynamicArea = new \Thurly\Main\Page\FrameStatic("task_created");
	...
	//						,"Important"
			...
			?><div id="feed-add-post-content-message-add-ins"><?
								...
								<script type="text/javascript">
									...
									BX.ready(function(){
										BX.SocNetLogDestination.init({
											...
											pathToAjax : '/thurly/components/thurly/socialnetwork.blog.post.edit/post.ajax.php',
											extranetUser : false,
											bindMainPopup : {
				...
				if ($bCalendar)
				{
					...
					<div id="feed-add-post-content-calendar" style="display: none;">
						<?
						$APPLICATION->IncludeComponent("thurly:calendar.livefeed.edit", '',
							array(
								"EVENT_ID" => '',
				...
				if($bLists)
				{
					...
					<div id="feed-add-post-content-lists" style="display: none;">
						<?
						$APPLICATION->IncludeComponent("thurly:lists.live.feed", "",
							array(
				...
				if($bTasks)
				{
					?><div id="feed-add-post-content-tasks" style="display: none;"><div id="feed-add-post-content-tasks-container"><?
						...
						if ($taskSubmitted)
						{
							...
							$APPLICATION->IncludeComponent('thurly:tasks.task', '',
								$componentParameters,
								null,
								array("HIDE_ICONS" => "Y")
							);
				...
				{
					$dynamicArea = new \Thurly\Main\Page\FrameStatic("post-autosave");
					$dynamicArea->startDynamicArea();
					$as = new CAutoSave();
					$as->Init(false);
					$dynamicArea->finishDynamicArea();
		...
		if ($_POST["action"] == "SBPE_get_full_form")
		{
			...
			\Thurly\Main\Page\Asset::getInstance()->getJs();
			$CSSStrings = \Thurly\Main\Page\Asset::getInstance()->getCss();
			\Thurly\Main\Page\Asset::getInstance()->getStrings();

			$targetTypeList = array('JS'/*, 'CSS'*/);
			foreach($targetTypeList as $targetType)
			{
				$targetAssetList = \Thurly\Main\Page\Asset::getInstance()->getTargetList($targetType);

				foreach($targetAssetList as $targetAsset)
				{
					$assetInfo = \Thurly\Main\Page\Asset::getInstance()->getAssetInfo($targetAsset['NAME'], \Thurly\Main\Page\AssetMode::ALL);
