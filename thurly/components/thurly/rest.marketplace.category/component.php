...
$nav = new \Thurly\Main\UI\PageNavigation("nav-apps");
...
if (isset($_GET["q"]) && trim($_GET['q']))
{
	...
	$arCategory = \Thurly\Rest\Marketplace\Client::searchApp($q, $curPage);
...
else
{
	...
	{
		$tag = \Thurly\Rest\Marketplace\Client::getTagByPlacement($_GET['placement']);

		if(count($tag) > 0)
		{
			$arResult["CAT_NAME"] = GetMessage('MARKETPLACE_CAT_PLACEMENT');
			$arCategory = \Thurly\Rest\Marketplace\Client::getByTag($tag, $curPage);
		}
		else
		{
			$arCategory = array();
	...
	{
		$arCategory = \Thurly\Rest\Marketplace\Client::getCategory($arParams['CATEGORY'], $curPage);
