...
class CThurlyCrmConfigLocationList2Component extends CThurlyComponent
{
	...
	protected function performAction()
	{
		...
		$requestMethod = \Thurly\Main\Context::getCurrent()->getServer()->getRequestMethod();
		...
		if ($requestMethod === 'POST' && check_thurly_sessid() && isset($this->dbResult['REQUEST']['POST']['action_button_'.$gridId]))
		{
			// group delete
		...
		elseif ($requestMethod == 'GET' && check_thurly_sessid() && isset($this->dbResult['REQUEST']['GET']['action_'.$gridId]))
		{
			// single delete
		...
		while($arLoc = $dbRecordsList->GetNext())
		{
			...
			$arLoc['PATH_TO_LOCATIONS_DELETE'] =
				CHTTP::urlAddParams(
					...
					array('action_'.$this->dbResult['GRID_ID'] => 'delete', 'ID' => $arLoc['ID'], 'sessid' => thurly_sessid())
				);
