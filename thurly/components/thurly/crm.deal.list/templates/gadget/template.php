...
global $APPLICATION;
$APPLICATION->SetAdditionalCSS("/thurly/themes/.default/crm-gadget.css");
...
else
{
	foreach($arResult['DEAL'] as &$arDeal)
	{
		...
		$linkID = "ballon_{$arResult['GADGET_ID']}_D_{$arDeal['~ID']}";
		echo '<a id="', $linkID, '" href="', $arDeal['PATH_TO_DEAL_SHOW'], '" class="crm-gadg-link" title="', $arDeal['TITLE'], '">', $arDeal['TITLE'], '</a>';
		echo '<span class="crm-gadg-title-deadline"> &ndash; </span>';
		echo '<span class="crm-gadg-title-status">', $arDeal['DEAL_TYPE_NAME'], '</span>';
		echo '<script type="text/javascript">BX.tooltip("DEAL_', $arDeal['~ID'], '", "', $linkID,'", "/thurly/components/thurly/crm.deal.show/card.ajax.php", "crm_balloon_no_photo", true);</script>';
		echo '</div>';
