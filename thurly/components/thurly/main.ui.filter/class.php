...
use Thurly\Main\UI\Filter\Type;
use Thurly\Main\UI\Filter\FieldAdapter;
use Thurly\Main\UI\Filter\DateType;
use Thurly\Main\UI\Filter\NumberType;
use Thurly\Main\UI\Filter\Theme;
...
class CMainUiFilter extends CThurlyComponent
{
	...
	{
		if (!($this->options instanceof \Thurly\Main\UI\Filter\Options))
		{
			$this->options = new \Thurly\Main\UI\Filter\Options(
	...
	protected function checkRequiredParams()
	{
		...
		{
			$errors->add(array(new \Thurly\Main\Error(Loc::getMessage("MAIN_UI_FILTER__FILTER_ID_NOT_SET"))));
		}
	...
	{
		$file = new \Thurly\Main\IO\File($path);
		$config = array();
		...
		if ($file->isExists())
		{
			...
			if (!empty($jsonConfig))
			{
				$config = \Thurly\Main\Web\Json::decode($jsonConfig);
			}
		}
	...
	protected function getAbsoluteThemesPath()
	{
		...
		$absolutePath = \Thurly\Main\IO\Path::convertRelativeToAbsolute($relativeThemesPath);
		return $absolutePath;
	}
