...
function blogShowImagePopup(src)
{
	...
	{
		BX('blgimgppp').src = '/thurly/images/1.gif';
...
(function() {
	...
	BX.SBPostMenu.showMenu = function(params) {
		...
		BX.ajax({
			url: '/thurly/components/thurly/socialnetwork.blog.post/ajax.php',
			method: 'POST',
			dataType: 'json',
			data: {
				sessid : BX.thurly_sessid(),
			...
			onsuccess: function(postData) {
				if (
					...
					|| (
						postData.perms <= 'D' // \Thurly\Blog\Item\Permissions::DENY
						&& (
				...
				if(!BX.util.in_array(postType, ["DRAFT", "MODERATION"]))
				{
					...
					if (
						postData.perms >= 'W' // \Thurly\Blog\Item\Permissions::FULL
						|| (
							postData.perms >= 'P' // \Thurly\Blog\Item\Permissions::WRITE
					...
					if(postData.perms >= 'T') // \Thurly\Blog\Item\Permissions::MODERATE
					{
						menuItems.push({
					...
					if (postData.perms >= 'W') //  // \Thurly\Blog\Item\Permissions::FULL
					{
						menuItems.push({
...
(function() {
	...
	top.SBPImpPostCounter.prototype.get = function() {
		...
		if (this.node.getAttribute("inumpage") != "done") {
			...
			BX.ajax({
				url: "/thurly/components/thurly/socialnetwork.blog.blog/users.php",
				method: 'POST',
				dataType: 'json',
				data: {
					'ID' : this.postId,
