...
{
	$APPLICATION->AddHeadScript("/thurly/components/thurly/socialnetwork.log.ex/templates/.default/script.js");
}
...
?><script>
	BX.message({
		sonetBPSetPath: '<?=CUtil::JSEscape("/thurly/components/thurly/socialnetwork.log.ex/ajax.php")?>',
...
else
{
	if(!empty($arResult["Post"]))
	{
		...
		if (!in_array($arParams["TYPE"], array("DRAFT", "MODERATION")))
		{
			...
			{
				$APPLICATION->IncludeComponent(
					"thurly:socialnetwork.blog.post.comment",
					"",
					Array(
