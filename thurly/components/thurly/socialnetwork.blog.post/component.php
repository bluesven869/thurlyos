...
use Thurly\Socialnetwork\Livefeed;
...
if(IntVal($arParams["IMAGE_MAX_WIDTH"]) <= 0)
	$arParams["IMAGE_MAX_WIDTH"] = \Thurly\Blog\Util::getImageMaxWidth();
if(IntVal($arParams["IMAGE_MAX_HEIGHT"]) <= 0)
	$arParams["IMAGE_MAX_HEIGHT"] = \Thurly\Blog\Util::getImageMaxHeight();
...
$arPost = array();
$cacheTtl = 2592000;
$cacheId = 'blog_post_socnet_general_'.$arParams["ID"].'_'.LANGUAGE_ID.($arResult["TZ_OFFSET"] <> 0 ? "_".$arResult["TZ_OFFSET"] : "")."_".Thurly\Main\Context::getCurrent()->getCulture()->getDateTimeFormat();
...
else
{
	...
	$postItem = \Thurly\Blog\Item\Post::getById($arParams["ID"]);
...
)
{
	if(!empty($arPost))
	{
		...
		else
		{
			if ($bNoLogEntry)
			{
				$arPost["perms"] = $arResult["PostPerm"] = \Thurly\Blog\Item\Permissions::DENY;
			}
			elseif (
				CSocNetUser::IsCurrentUserModuleAdmin(SITE_ID, (!isset($arParams["MOBILE"]) || $arParams["MOBILE"] != "Y"))
				|| $APPLICATION->GetGroupRight("blog") >= "W"
			...
			else
			{
				...
				$arResult["ReadOnly"] = (
					$permsResult['PERM'] <= \Thurly\Blog\Item\Permissions::READ
		...
		if ($arResult["PostPerm"] > BLOG_PERMS_DENY)
		{
			...
			)
			{
				...
				$tmp = $_POST;
				$tmp['SPERM'] = $spermNew;
				\Thurly\Socialnetwork\ComponentHelper::processBlogPostNewMailUser($tmp, $arResult);
				$spermNew = $tmp['SPERM'];
				...
				elseif (!$canPublish)
				{
					...
					?><?=\Thurly\Main\Web\Json::encode($response)?><?
					CMain::finalActions();
					die;
				}
			...
			else
			{
						...
						if (count($arParams["POST_PROPERTY"]) > 0)
						{
							foreach ($arPostFields as $FIELD_NAME => $arPostField)
							{
								...
								)
								{
									...
									$arResult['URL_PREVIEW'] = \Thurly\Socialnetwork\ComponentHelper::getUrlPreviewContent($arPostField, array(
				...
				if($arPost["HAS_IMAGES"] != "N")
				{
					...
					while ($arImage = $res->Fetch())
					{
						...
						$arResult["images"][$arImage['ID']] = Array(
							"small" => "/thurly/components/thurly/blog/show_file.php?fid=".$arImage['ID']."&width=".$arParams["ATTACHED_IMAGE_MAX_WIDTH_SMALL"]."&height=".$arParams["ATTACHED_IMAGE_MAX_HEIGHT_SMALL"]."&type=square"
						);

						$arResult["images"][$arImage['ID']]["full"] = "/thurly/components/thurly/blog/show_file.php?fid=".$arImage['ID']."&width=".$arParams["ATTACHED_IMAGE_MAX_WIDTH_FULL"]."&height=".$arParams["ATTACHED_IMAGE_MAX_HEIGHT_FULL"];
					}
				}
							...
							elseif($type == "U")
							{
								if(in_array("US".$vv["ENTITY_ID"], $vv["ENTITY"]))
								{
									...
									if (
										...
										&& defined("THURLY24_PATH_COMPANY_STRUCTURE_VISUAL")
									)
									{
										$link = THURLY24_PATH_COMPANY_STRUCTURE_VISUAL;
									}
				...
				else
				{
					$arResult["Post"]["SPERM"]["U"][1] = Array(
						...
						"URL" => (
							!$arResult["bExtranetSite"]
							&& defined("THURLY24_PATH_COMPANY_STRUCTURE_VISUAL")
								? THURLY24_PATH_COMPANY_STRUCTURE_VISUAL
								: ""
						),
						"ID" => ""
					);
				...
				$arResult["Post"]["LIMITED_VIEW"] = \Thurly\Socialnetwork\ComponentHelper::getBlogPostLimitedViewStatus(array(
			...
			if($arParams["FROM_LOG"] != "Y")
			{
				if($arResult["PostPerm"]>=BLOG_PERMS_MODERATE)
				{
					$arResult["urlToHide"] = htmlspecialcharsex($APPLICATION->GetCurPageParam("hide=Y"."&".thurly_sessid_get(), Array("sessid", "success", "hide", "delete")));
				}
				...
				if($arResult["PostPerm"] >= BLOG_PERMS_FULL)
				{
					if(in_array($arParams["TYPE"], array("DRAFT", "MODERATION")))
					{
						...
						if(strpos($arResult["urlToDelete"], "?") === false)
							...
							$arResult["urlToDelete"] .= "&";
						$arResult["urlToDelete"] .= "delete_blog_post_id=#del_post_id#&ajax_blog_post_delete=Y"."&".thurly_sessid_get();
					}
					else
						$arResult["urlToDelete"] = htmlspecialcharsex($APPLICATION->GetCurPageParam("delete=Y"."&".thurly_sessid_get(), Array("sessid", "delete", "hide", "success")));
			...
			{
				if (
					($contentViewData = \Thurly\Socialnetwork\Item\UserContentView::getViewData(array(
