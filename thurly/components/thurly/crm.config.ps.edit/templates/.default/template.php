...
<form name="form_<?=$arResult["FORM_ID"]?>" id="form_<?=$arResult["FORM_ID"]?>" action="<?=POST_FORM_ACTION_URI?>" method="POST" enctype="multipart/form-data">
	...
	<table width="100%" class="bx-settings-table">
		<tbody id="PS_INFO">
			...
			<?if (isset($arResult['PS_MODE'])):?>
				<tr>
					<td width="40%" class="bx-field-name"><?=Loc::getMessage('CRM_PS_FIELD_PS_MODE')?>: </td>
					<td class="bx-field-value">
						<select name="PS_MODE">
	...
	<table cellspacing="0" class="bx-edit-tab" width="100%">
					<tr>
	...
	<div class="bx-buttons">
		...
		<input type="button" value="<?echo GetMessage("CRM_BUTTON_FORM_CANCEL")?>" name="cancel" onclick="window.location='<?=htmlspecialcharsbx(CUtil::addslashes($arResult['BACK_URL']))?>'" title="<?echo GetMessage("CRM_BUTTON_FORM_CANCEL_TITLE")?>" />
	</div>
</form>
<?
