...
BX.ready(function(){
			...
			switch(command)
			{
				...
				case 'createfile':
					...
					if('EditFile' in BXFileStorage)
					{
						...
						//fake!!!
						ThurlyDisk.onStartEditingFile(decodeURIComponent(params.name), {}, null, true, true, true);
				...
				case 'editfile':
					...
					if('EditFile' in BXFileStorage)
					{
						console.debug(params, params.name, decodeURIComponent(params.name));
						//fake!!!
						ThurlyDisk.onStartEditingFile(decodeURIComponent(params.name), {}, null, true, true);
						//BX.desktop.syncPause(true);
						BXFileStorage.EditFile(urlDownload, urlUpload, decodeURIComponent(params.name));
						//BX.desktop.syncPause(false);
					}
				...
				case 'viewfile':
					...
					if('ViewFile' in BXFileStorage)
					{
						console.debug(params, params.name, decodeURIComponent(params.name));
						//fake!!!
						ThurlyDisk.onStartViewingFile(decodeURIComponent(params.name), {}, null, true, true);
						//BX.desktop.syncPause(true);
	...
	BX.addCustomEvent(window, 'prepareSettingsView', function () {
		...
		ThurlyDiskSettings.prepareSettingsView();
	});
});
...
var ThurlyDisk = {
	...
	onStartPackage : function(numberOfFiles, packageSize, detailedData)
	{
		...
		console.debug(
			...
			"detailedData: ", detailedData,
			"packageSize: ", ThurlyDisk.formatSize(packageSize) + " (" + packageSize + ")"
		);
	},
	...
	onStartFile : function(path, snapshot, fileSize, isDownloaded)
	{
		...
		console.debug(
			...
			path + "  ",
			snapshot,
			ThurlyDisk.formatSize(fileSize) + " (" + fileSize + ")" + "  ",
	...
	onProgressFile : function(path, bytes, speed, partFileSize)
	{
		...
		console.debug(
			...
			"ProgressFile:  ",
			path + "  ",
			ThurlyDisk.formatSize(bytes) + " (" + bytes + ")  ",
			ThurlyDisk.formatSize(speed) + "/" + BX.message("disk_speed_seconds") + " (" + speed + ")"
		);
	},
	...
	onSaveSettings : function(settings)
	{
		...
		ThurlyDiskSettings.saveSettings(settings);
	},
	...
	updateChart : function()
	{
		BXFileStorage.GetStorageSize(BX.proxy(function(data){
			...
			this.layout.mySpace.innerHTML = ThurlyDisk.formatSize(this.mySpace);
			...
			if (this.chart)
			{
				...
				this.layout.freeSpace.innerHTML = ThurlyDisk.formatSize(this.freeSpace);
				this.layout.companySpace.innerHTML = ThurlyDisk.formatSize(companySpace);
				this.layout.usedSpace.innerHTML = ThurlyDisk.formatSize(usedSpace);
				this.layout.fullSpace.innerHTML = ThurlyDisk.formatSize(this.diskSpace);
			...
			onsuccess: BX.proxy(function(result) {
				if (result)
				{
					console.debug(
						...
						ThurlyDisk.formatSize(result.diskSpace) + " (" + result.diskSpace + ")  ",
						"freeSpace: ", ThurlyDisk.formatSize(result.freeSpace) + " (" + result.freeSpace + ")"
					);
	...
	logError : function(errorData, context, path)
	{
		...
		BX.ajax({
			method : "POST",
			dataType : "json",
			url : 'https://www.thurlyos.ru/bx24_disk_logging',
	...
	setProgress : function(currentFileNumber, numberOfFiles, percent, speed)
	{
		...
		if (speed)
		{
			this.layout.syncSpeed.innerHTML = " " + ThurlyDisk.formatSize(speed) + "/" + BX.message("disk_speed_seconds");
		}
...
//						li.style.opacity = state.opacity / 100;
	...
	setWorkingTabIcon : function(counter)
	{
		...
		if(icon)
		{
			icon.style.background = 'url("/thurly/components/thurly/disk.thurlyosdisk/templates/.default/images/icon-working.gif") no-repeat';
	...
	setPauseTabIcon : function(counter)
	{
		...
		if(icon)
		{
			icon.style.background = 'url("/thurly/components/thurly/disk.thurlyosdisk/templates/.default/images/icon-pause.png") no-repeat';
			icon.style.width = '19px';
			icon.style.marginTop = '8px';
		}
	},
			...
			BX.create("div", {
				...
				children: [
					...
					BX.create("span", {
						...
						html :  ThurlyDisk.formatSize(fileData.size)
					}),
					BX.create("span", {
	...
	showNotifySingleFile : function(fileData)
	{
		...
		var notify = {
			...
			userAvatar: user.avatar || '/thurly/components/thurly/disk.thurlyosdisk/templates/.default/images/disk_34x34.png',
			userName: user.name,
			type: "2"
		};
	...
	notifyProgressEdit : function(params)
	{
		...
		var notify = {
			...
			userAvatar: '/thurly/components/thurly/disk.thurlyosdisk/templates/.default/images/disk_34x34.png',
		...
		var messsageJs =
			...
			'BX.desktop.addCustomEvent("sub-BXFileStorageSyncStatusProgressFile", function(path, bytes, speed, fileSize){' +
				...
				'BX.adjust(BX("disk-speed"), {text: portion + "%" + " " + (!!speed? "(" + ThurlyDisk.formatSize(speed) + "/" + disk_speed_seconds + ")" : "")});' +
	...
	notifyProgressLaunchApp : function(params)
	{
		...
		var notifyHtml = BX.create("div", {attrs : {'data-notifyId' : notify.id, 'data-notifyType' : notify.type}, props : { className: "bx-notifier-item"}, children : [
			BX.create('span', {props : { className : "bx-notifier-item-content" }, children : [
				...
				BX.create("a", {attrs : {href : '#', 'data-notifyId' : notify.id, 'data-notifyType' : notify.type}, props : { className: "bx-notifier-item-delete"}}),
				BX.create('span', {props : { className : "bx-notifier-item-text" }, text: params.name}),
				BX.create('div', {style: {marginLeft: '50px'}, children: [BX.create('img', {props: {src: '/thurly/components/thurly/disk.thurlyosdisk/templates/.default/images/popups-alert-mov.gif'}})]}),
	...
	notifyProgressExtLink : function(params)
	{
		...
		var notifyHtml = BX.create("div", {attrs : {'data-notifyId' : notify.id, 'data-notifyType' : notify.type}, props : { className: "bx-notifier-item"}, children : [
			BX.create('span', {props : { className : "bx-notifier-item-content" }, children : [
				...
				BX.create("a", {attrs : {href : '#', 'data-notifyId' : notify.id, 'data-notifyType' : notify.type}, props : { className: "bx-notifier-item-delete"}}),
				(isFinish? null : BX.create('div', {style: {marginLeft: '50px'}, children: [BX.create('img', {props: {src: '/thurly/components/thurly/disk.thurlyosdisk/templates/.default/images/popups-alert-mov.gif'}})]})),
