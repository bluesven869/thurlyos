...
{
	$arParams["PATH_TO_CONPANY_DEPARTMENT"] = \Thurly\Main\Config\Option::get('main', 'TOOLTIP_PATH_TO_CONPANY_DEPARTMENT', SITE_DIR."company/structure.php?set_filter_structure=Y&structure_UF_DEPARTMENT=#ID#");
}
...
else
{
	...
	else
	{
		...
		else
		{
			...
			$arImage = CSocNetTools::InitImage($arResult["Group"]["IMAGE_ID"], $iSize, "/thurly/images/socialnetwork/nopic_group_100.gif", 100, "", false);
			...
			if (!$arParams["SHORT_FORM"])
			{
				...
				if ($dbOwners)
				{
					while ($arOwner = $dbOwners->GetNext())
					{
						...
						if (intval($arParams["THUMBNAIL_LIST_SIZE"]) > 0)
						{
							...
							$arImage = CSocNetTools::InitImage($arOwner["USER_PERSONAL_PHOTO"], $arParams["THUMBNAIL_LIST_SIZE"], "/thurly/images/socialnetwork/nopic_30x30.gif", 30, $pu, $canViewProfile);
						}
						else // old 
							$arImage = CSocNetTools::InitImage($arOwner["USER_PERSONAL_PHOTO"], 50, "/thurly/images/socialnetwork/nopic_user_50.gif", 50, $pu, $canViewProfile);
				...
				if ($dbModerators)
				{
					...
					while ($arModerators = $dbModerators->GetNext())
					{
						...
						if (intval($arParams["THUMBNAIL_LIST_SIZE"]) > 0)
						{
							...
							$arImage = CSocNetTools::InitImage($arModerators["USER_PERSONAL_PHOTO"], $arParams["THUMBNAIL_LIST_SIZE"], "/thurly/images/socialnetwork/nopic_30x30.gif", 30, $pu, $canViewProfile);
						}
						else // old 
							$arImage = CSocNetTools::InitImage($arModerators["USER_PERSONAL_PHOTO"], 50, "/thurly/images/socialnetwork/nopic_user_50.gif", 50, $pu, $canViewProfile);
				...
				if ($dbMembers)
				{
					...
					while ($arMembers = $dbMembers->GetNext())
					{
						...
						if (intval($arParams["THUMBNAIL_LIST_SIZE"]) > 0)
						{
							...
							$arImage = CSocNetTools::InitImage($arMembers["USER_PERSONAL_PHOTO"], $arParams["THUMBNAIL_LIST_SIZE"], "/thurly/images/socialnetwork/nopic_30x30.gif", 30, $pu, $canViewProfile);
						}
						else // old 
							$arImage = CSocNetTools::InitImage($arMembers["USER_PERSONAL_PHOTO"], 50, "/thurly/images/socialnetwork/nopic_user_50.gif", 50, $pu, $canViewProfile);					
				...
				if (
					...
					&& \Thurly\Main\Loader::includeModule('intranet')
				)
				{
