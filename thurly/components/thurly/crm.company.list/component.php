...
{
	AddEventHandler('crm', 'onCrmCompanyListItemBuildMenu', array('\Thurly\Crm\CallList\CallList', 'handleOnCrmCompanyListItemBuildMenu'));
}
...
if($actionData['ACTIVE'])
{
	...
	else//if ($actionData['METHOD'] == 'GET')
	{
		...
		elseif ($isMyCompanyMode && $actionData['NAME'] === 'set_def_mycompany' && isset($actionData['ID']))
		{
			...
			if ($defMyCompanyId > 0)
				Thurly\Crm\Requisite\EntityLink::setDefaultMyCompanyId($defMyCompanyId);
		}
...
if ($isMyCompanyMode)
	$defaultMyCompanyId = \Thurly\Crm\Requisite\EntityLink::getDefaultMyCompanyId();

//region Navigation data initialization
$pageNum = 0;
if ($isInExportMode && $isStExport)
{
	$pageSize = $arParams['COMPANY_COUNT'];
}
else
{
...
{
	$addressSort = array();
	$addressTypeID = \Thurly\Crm\EntityAddress::Primary;
	...
	{
		$addressTypeID = \Thurly\Crm\EntityAddress::Registered;
		foreach($arSort as $k => $v)
		{
	...
	if(!empty($addressSort))
	{
		...
		$navDbResult = \Thurly\Crm\CompanyAddress::getEntityList(
			$addressTypeID,
			$addressSort,
			$arFilter,
			false,
...
if (!$isInExportMode)
{
	...
	if(!$bInternal)
	{
		...
		$arResult['NEED_FOR_BUILD_TIMELINE'] = \Thurly\Crm\Agent\Timeline\CompanyTimelineBuildAgent::getInstance()->isEnabled();

		$agent = Thurly\Crm\Agent\Duplicate\CompanyDuplicateIndexRebuildAgent::getInstance();
