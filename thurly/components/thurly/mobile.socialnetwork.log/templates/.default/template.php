...
$APPLICATION->AddHeadString('<script src="'.CUtil::GetAdditionalFileURL("/thurly/components/thurly/mobile.socialnetwork.log/templates/.default/script_attached.js").'"></script>', true);
					...
					<div class="post-item-top-wrap">
						...
						)
						{
							$arResultVote = $APPLICATION->IncludeComponent(
								"thurly:rating.vote", "mobile_like",
								Array(
						...
						elseif (strlen($arEvent["EVENT_FORMATTED"]["MESSAGE"]) > 0)
						{
							...
							)
							{
								...
								?><div class="<?=$news_item_style?>"<?=$strOnClick?> id="post_block_check_cont_<?=$arEvent["EVENT"]["ID"]?>"><?
									?><div class="lenta-info-block <?=(in_array($arEvent["EVENT"]["EVENT_ID"], array("intranet_new_user", "thurlyos_new_user")) ? "lenta-block-new-employee" : "info-block-important")?>"><?
								...
								foreach($arEvent["COMMENTS"] as $arComment)
								{
									...
									)
									{
										...
										BX.bind(BX('post-comment-more'), 'click', function(e)
										{
											...
											BMAjaxWrapper.Wrap({
												'type': 'json',
												'method': 'POST',
												'url': '/thurly/components/thurly/mobile.socialnetwork.log/ajax.php',
												'data': get_data,
									...
									?><div class="post-comment-block<?=($bUnread ? " post-comment-new" : "")?>">
										...
										<div class="post-comment-cont"><?
											...
											{
												$APPLICATION->IncludeComponent(
													//"thurly:rating.vote", "mobile_comment_".$arParams["RATING_TYPE"],
													"thurly:rating.vote", "mobile_comment_like",
													Array(
