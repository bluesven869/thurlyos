...
require_once($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/components/thurly/webdav/functions.php");
...
/********************************************************************
				Action
********************************************************************/
if (!empty($arParams['ACTION']) && !empty($_REQUEST["ID"]) && check_thurly_sessid())
{
	$arError = array();
	$ID = $_REQUEST["ID"];
...
if ($db_res)
{
	...
	while ($res = $db_res->GetNext())
	{
		...
		$res["URL"] = array(
			...
			"RECOVER" => $APPLICATION->GetCurPageParam("ID=".$res["ID"]."&action=recover&".thurly_sessid_get(), array("ID", "action", "sessid", "result")),
			"DELETE" => $APPLICATION->GetCurPageParam("ID=".$res["ID"]."&action=delete&".thurly_sessid_get(), array("ID", "action", "sessid", "result")));
