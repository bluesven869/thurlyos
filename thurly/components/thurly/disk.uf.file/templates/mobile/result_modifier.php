...
if (!\Thurly\Main\Loader::includeModule('mobileapp'))
{
	return;
}
...
foreach ($arResult['FILES'] as $id => $file)
{
	...
	{
		$file["PREVIEW_URL"] = str_replace("/thurly/tools/disk/uf.php", SITE_DIR."mobile/ajax.php", $file["PREVIEW_URL"]);
	}

	if (isset($file["DOWNLOAD_URL"]))
	{
		$file["DOWNLOAD_URL"] = str_replace("/thurly/tools/disk/uf.php", SITE_DIR."mobile/ajax.php", $file["DOWNLOAD_URL"]);
		$file["DOWNLOAD_URL"] = $file["DOWNLOAD_URL"].(strpos($file["DOWNLOAD_URL"], "?") === false ? "?" : "&")."mobile_action=disk_uf_view&filename={$file['NAME']}";
	}

	if (isset($file["PATH"]))
	{
		$file["PATH"] = str_replace("/thurly/tools/disk/uf.php", SITE_DIR."mobile/ajax.php", $file["PATH"]);
	...
	elseif (array_key_exists("IMAGE", $file))
	{
		...
		$arParams["THUMB_SIZE"]["signature"] = \Thurly\Disk\Security\ParameterSigner::getImageSignature($file["ID"], $arParams["THUMB_SIZE"]["width"], $arParams["THUMB_SIZE"]["height"]);
		...
		$arSmallSize["signature"] = \Thurly\Disk\Security\ParameterSigner::getImageSignature($file["ID"], $arSmallSize["width"], $arSmallSize["height"]);
		...
		{
			$arDestinationSize["signature"] = \Thurly\Disk\Security\ParameterSigner::getImageSignature($file["ID"], $arDestinationSize["width"], $arDestinationSize["height"]);
		...
		if ($bNeedCreatePicture)
		{
			$arParams["SCREEN_SIZE"]["signature"] = \Thurly\Disk\Security\ParameterSigner::getImageSignature($file["ID"], $arParams["SCREEN_SIZE"]["width"], $arParams["SCREEN_SIZE"]["height"]);
		...
		if ($bNeedCreatePicture)
		{
			$arParams["PREVIEW_SIZE"]["signature"] = \Thurly\Disk\Security\ParameterSigner::getImageSignature($file["ID"], $arParams["PREVIEW_SIZE"]["width"], $arParams["PREVIEW_SIZE"]["height"]);
