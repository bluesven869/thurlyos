;(function(window) {
		...
		UF.prototype = {
			...
			selectFolder : function(tab, path, selected, folderByPath)
			{
				...
				var moveQuery = BX.delegate(function(id, targetFolderId, arProp, secPath){
					BX.Disk.ajax({
						method: 'POST',
						dataType: 'json',
						url: BX.Disk.addToLinkParam('/thurly/tools/disk/uf.php', 'action', 'moveUploadedFile'),
						data: {
							attachedId: id,
			...
			selectItem : function(element, itemID, name)
			{
				...
				libLink = libLink.replace("/index.php","") + '/element/upload/' + targetID +
					'/?use_light_view=Y&AJAX_CALL=Y&SIMPLE_UPLOAD=Y&IFRAME=Y&sessid='+BX.thurly_sessid()+
					'&SECTION_ID='+targetID+
	...
	var getBreadcrumbsByAttachedObject = function(attachedId) {
		return BX.Disk.ajaxPromise({
			url: BX.Disk.addToLinkParam('/thurly/tools/disk/uf.php', 'action', 'getBreadcrumbs'),
			method: 'POST',
			dataType: 'json',
			data: {
...
BX.Disk.UF.disableAutoCommentToAttachedObject = function(params)
{
	...
	BX.Disk.ajax({
		method: 'POST',
		dataType: 'json',
		url: BX.Disk.addToLinkParam('/thurly/tools/disk/uf.php', 'action', 'disableAutoCommentToAttachedObject'),
		data: {
			attachedId: attachedId
		},
		onsuccess: BX.delegate(function (response) {
...
BX.Disk.UF.enableAutoCommentToAttachedObject = function(params)
{
	...
	BX.Disk.ajax({
		method: 'POST',
		dataType: 'json',
		url: BX.Disk.addToLinkParam('/thurly/tools/disk/uf.php', 'action', 'enableAutoCommentToAttachedObject'),
		data: {
			attachedId: attachedId
		},
		onsuccess: BX.delegate(function (response) {
		...
		obElementViewer.openMenu('disk_open_menu_with_services', BX(targetElement), [
			...
			},
			{
				...
				onclick: BX.delegate(function (e)
				{
					...
					BX.adjust(targetElement, {text: this.getNameEditService('office365')});
					BX.PopupMenu.destroy('disk_open_menu_with_services');
