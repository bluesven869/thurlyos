...
if (!function_exists('__SLMGetLogRecord'))
{
	function __SLMGetLogRecord($logID, $arParams, $current_page_date)
	{
		...
		if (!array_key_exists("COMMENTS_PARAMS", $arEvent["FIELDS_FORMATTED"]))
		{
			...
			if (
				$arForumMetaData
				&& intval($arEvent["FIELDS_FORMATTED"]["EVENT"]["SOURCE_ID"]) > 0
			)
			{
...
if (!function_exists('__SLMGetLogCommentRecord'))
{
	function __SLMGetLogCommentRecord($arComments, $arParams)
	{
		...
		{
			$urlPreviewText = \Thurly\Socialnetwork\ComponentHelper::getUrlPreviewContent($arTmpCommentEvent["UF"]["UF_SONET_COM_URL_PRV"], array(
				"MOBILE" => "Y",
	...
	function __SLMAjaxGetComment($comment_id, $arParams, $bCheckRights = false)
	{
		if ($arComment = CSocNetLogComments::GetByID($comment_id))
		{
			...
			)
			{
				...
				{
					$urlPreviewTextMobile = \Thurly\Socialnetwork\ComponentHelper::getUrlPreviewContent($arComment["UF"]["UF_SONET_COM_URL_PRV"], array(
						"MOBILE" => "Y",
						"NAME_TEMPLATE" => $arParams["NAME_TEMPLATE"],
						"PATH_TO_USER" => $arParams["~PATH_TO_USER"]
					));

					$urlPreviewTextWeb = \Thurly\Socialnetwork\ComponentHelper::getUrlPreviewContent($arComment["UF"]["UF_SONET_COM_URL_PRV"], array(
						"MOBILE" => "N",
						"NAME_TEMPLATE" => $arParams["NAME_TEMPLATE"],
						"PATH_TO_USER" => $arParams["~PATH_TO_USER"]
					));
			...
			if (sizeof($arResult["VALUE"]) > 0)
			{
				...
				foreach ($arResult["VALUE"] as $fileID)
				{
					...
					if($arFile)
					{
						...
						$arFile["LINK"] = "/thurly/components/thurly/socialnetwork.log.ex/show_file.php?bp_fid=".$fileID;
