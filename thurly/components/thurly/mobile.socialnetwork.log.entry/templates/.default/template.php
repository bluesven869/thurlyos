	...
	)
	{
		...
		?><div class="<?=($item_class)?>" id="lenta_item_<?=$arEvent["EVENT"]["ID"]?>"><?
			...
			><?
					...
					if ($arResultVote)
					{
						...
						?><span id="rating_button_<?=intval($arEvent["EVENT"]["ID"])?>"><?
							?><div class="post-item-informers post-item-inform-likes<?=($arResultVote['USER_HAS_VOTED'] == 'Y' ? '-active' : '')?>" id="bx-ilike-button-<?=CUtil::JSEscape(htmlspecialcharsbx($arResultVote['VOTE_ID']))?>"><?
								...
								$like = COption::GetOptionString("main", "rating_text_like_y", GetMessage("MOBILE_LOG_LIKE_ACTION"));
								$like2 = str_replace('#LIKE#', $like, GetMessage("MOBILE_LOG_LIKE2_ACTION_PATTERN"));
				...
				{
					include($_SERVER["DOCUMENT_ROOT"]."/thurly/components/thurly/mobile.socialnetwork.log.entry/templates/.default/photo.php");
				}
				elseif (strlen($arEvent["EVENT_FORMATTED"]["MESSAGE"]) > 0)
				{
					...
					)
					{
						...
						?><div class="<?=$news_item_style?>"<?=$strOnClick?> id="post_block_check_cont_<?=$arEvent["EVENT"]["ID"]?>" bx-content-view-xml-id="<?=(!empty($arResult["CONTENT_ID"]) ? htmlspecialcharsBx($arResult["CONTENT_ID"]) : "")?>"><?
							?><div class="post-item-full-content post-item-copytext lenta-info-block <?=(in_array($arEvent["EVENT"]["EVENT_ID"], array("intranet_new_user", "thurlyos_new_user")) ? "lenta-block-new-employee" : "info-block-important")?>"><?
								if (in_array($arEvent["EVENT"]["EVENT_ID"], array("intranet_new_user", "thurlyos_new_user")))
								{
			...
			if (!$arParams["IS_LIST"])
			{
				...
				if (is_array($arEvent["COMMENTS"]))
				{
					foreach($arEvent["COMMENTS"] as $arComment)
					{
						...
						$records[$commentId] = array(
							...
							"POST_MESSAGE_TEXT" => CSocNetTextParser::closetags(htmlspecialcharsback((array_key_exists("EVENT_FORMATTED", $arComment) && array_key_exists("MESSAGE", $arComment["EVENT_FORMATTED"]) ? $arComment["EVENT_FORMATTED"]["MESSAGE"] : $arComment["EVENT"]["MESSAGE"])))
						);
					}
				}
				...
				if ($arEvent["CAN_ADD_COMMENTS"] == "Y")
				{
					...
					$APPLICATION->IncludeComponent("thurly:main.post.form",
						".default",
						array(
