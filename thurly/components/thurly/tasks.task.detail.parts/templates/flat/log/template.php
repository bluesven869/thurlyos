...
<table id="task-log-table" class="task-log-table">
	...
	<tr>
		...
		<td class="task-log-where-column">
			...
			if($record['FIELD'] == \Thurly\Tasks\Integration\Disk\UserField::getMainSysUFCode())
			{
			...
			<span class="task-log-where"><?=$fieldName?><?
			...
			{
				$link = \Thurly\Tasks\UI\Task::makeActionUrl($arParams["PATH_TO_TASKS_TASK"], $taskData["ID"], 'view');
				$link = \Thurly\Tasks\Integration\Forum\Comment::makeUrl($link, $record["TO_VALUE"]);
		...
		switch($record["FIELD"])
		{
			...
			case "CHECKLIST_ITEM_RENAME":
				...
				if(\Thurly\Tasks\UI\Task\CheckList::checkIsSeparatorValue($record["FROM_VALUE"]))
				{
				...
				if(\Thurly\Tasks\UI\Task\CheckList::checkIsSeparatorValue($record["TO_VALUE"]))
				{
