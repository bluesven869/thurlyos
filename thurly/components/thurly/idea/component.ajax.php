...
//Moderator actions
if($arResult["IDEA_MODERATOR"] && check_thurly_sessid())
{
...
if(empty($arAjaxResult))
{
	switch ($_REQUEST["ACTION"])
	{
		...
		case "SUBSCRIBE":
			...
			if(check_thurly_sessid() && $USER->IsAuthorized() && $IdeaId>0 && CIdeaManagment::getInstance()->Notification()->getEmailNotify()->add($IdeaId))
			{
		...
		case "UNSUBSCRIBE":
			...
			if(check_thurly_sessid() && $USER->IsAuthorized() && $IdeaId>0)
			{
