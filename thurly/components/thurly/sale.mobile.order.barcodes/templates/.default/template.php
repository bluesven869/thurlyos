...
<script type="text/javascript">
	...
	app.addButtons({
		...
		saveButton:
		{
			...
			callback: function()
			{
				BX.addCustomEvent('onThurlySaleMOBCheckingComplete', function() {
					app.onCustomEvent("onThurlySaleMOBResult", {
				...
				if(document.activeElement && document.activeElement.onchange)
					...
					BX.onCustomEvent("onThurlySaleMOBCheckingComplete");

			}
		}
	});

	BX.addCustomEvent('onThurlySaleMOBGetInfo', function (params){
	...
	BX.addCustomEvent('onOpenPageBefore', function (params){
		...
		{
			app.onCustomEvent("onThurlySaleMOBLoaded", {
				"productId" : "<?=$arParams["PRODUCT_DATA"]["ID"]?>"
			});
		}
	});
