...
class CrmInvoicePaymentClientComponent extends CThurlyComponent
{
	...
	protected function prepareData($request)
	{
		if (!isset($this->arParams['EXCLUDED_ACTION_LIST']))
		{
			$this->arParams['EXCLUDED_ACTION_LIST'] = array(
				'bill', 'billde', 'billen', 'billla', 'billua', 'billkz', 'billby', 'billbr', 'billfr'
			);
		}
	...
	protected function fillPaySystemsArray($payment)
	{
		...
		foreach ($paySystemList as $key => $paySystemElement)
		{
			...
			)
			{
				...
				{
					$paySystemElement["LOGOTIP"] = '/thurly/images/sale/sale_payments/'.$paySystemElement['ACTION_FILE'].'.png';
					if (!Thurly\Main\IO\File::isFileExists($_SERVER['DOCUMENT_ROOT'].$paySystemElement["LOGOTIP"]))
						$paySystemElement["LOGOTIP"] = '/thurly/images/sale/sale_payments/default.png';
				}
	...
	protected function processExecution()
	{
		...
		$order = \Thurly\Sale\Order::loadByAccountNumber($this->arParams['ACCOUNT_NUMBER']);
		if (!$order)
		{
			$this->errorCollection->setError(new Main\Error(Loc::getMessage('CIPC_WRONG_ACCOUNT_NUMBER')));
			return;
