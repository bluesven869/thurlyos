...
CCrmComponentHelper::RegisterScriptLink('/thurly/js/crm/interface_grid.js');
Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/crm/requisite.js');
...
foreach ($arResult['LIST_DATA'] as $sKey => $arRequisite)
{
	...
	{
		$row[$fieldName] = Thurly\Crm\Format\RequisiteAddressFormatter::format(
			$row,
			array(
				'SEPARATOR' => Thurly\Crm\Format\AddressSeparator::HtmlLineBreak,
				'NL2BR' => true,
	...
	$resultItem = array(
		...
		'columns' => array(
			...
			'CREATED_BY_ID' => intval($arRequisite['CREATED_BY_ID']) > 0 ?
				...
				).'" id="balloon_'.$arResult['FORM_ID'].'_'.$arResult['COMPONENT_ID'].'_CREATED_BY_ID_'.$arRequisite['ID'].'">'.
				CCrmViewHelper::GetFormattedUserName(
					$arRequisite['CREATED_BY_ID'],
					$arParams['NAME_TEMPLATE'],
					true
				).'</a><script type="text/javascript">BX.tooltip('.$arRequisite['CREATED_BY_ID'].', "balloon_'.
				$arResult['FORM_ID'].'_'.$arResult['COMPONENT_ID'].'_CREATED_BY_ID_'.$arRequisite['ID'].'", "");</script>'
				: '',
			'MODIFY_BY_ID' => intval($arRequisite['MODIFY_BY_ID']) > 0 ?
				'<a href="'.CComponentEngine::MakePathFromTemplate(
					$arParams['PATH_TO_USER_PROFILE'],
					array('user_id' => $arRequisite['MODIFY_BY_ID'])
				).'" id="balloon_'.$arResult['FORM_ID'].'_'.$arResult['COMPONENT_ID'].'_MODIFY_BY_ID_'.$arRequisite['ID'].'">'.
				CCrmViewHelper::GetFormattedUserName(
					$arRequisite['MODIFY_BY_ID'],
					$arParams['NAME_TEMPLATE'],
					true
				).'</a><script type="text/javascript">BX.tooltip('.$arRequisite['MODIFY_BY_ID'].', "balloon_'.
				$arResult['FORM_ID'].'_'.$arResult['COMPONENT_ID'].'_MODIFY_BY_ID_'.$arRequisite['ID'].'", "");</script>'
				: ''/*,
	...
	array(
		...
		'FOOTER' => array(
			array(
				...
				'service_url' => '/thurly/components/thurly/crm.requisite.list/list.ajax.php?'.thurly_sessid_get()
			),
...
<script type="text/javascript">
	...
	BX.Crm["<?= CUtil::JSEscape($requisiteGridEditorId) ?>"] = new BX.Crm.RequisiteGridEditorClass({
		...
		requisitePopupAjaxUrl: "/thurly/components/thurly/crm.requisite.edit/popup.ajax.php?&site=<?=SITE_ID?>&<?=thurly_sessid_get()?>",
		requisiteAjaxUrl: "/thurly/components/thurly/crm.requisite.edit/ajax.php?&site=<?=SITE_ID?>&<?=thurly_sessid_get()?>",
		messages: {
