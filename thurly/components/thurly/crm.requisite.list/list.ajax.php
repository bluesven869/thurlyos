...
if(!function_exists('__CrmRequisiteListEndResonse'))
{
	function __CrmRequisiteListEndResonse($result)
	{
		global $APPLICATION;
...
if ($action === 'GET_ROW_COUNT')
{
	...
	{
		__CrmRequisiteListEndResonse(array('DATA' => array('TEXT' => '')));
	}
	...
	{
		__CrmRequisiteListEndResonse(array('ERROR' => 'Incorrect entity type.'));
	}
	if ($entityTypeId === \CCrmOwnerType::Company)
	{
		if ($entityId <= 0 || !\CCrmCompany::Exists($entityId))
		{
			__CrmRequisiteListEndResonse(array('ERROR' => 'Company not found.'));
		}
		else
		{
			if (!\CCrmCompany::CheckReadPermission($entityId, $userPerms))
				__CrmRequisiteListEndResonse(array('ERROR' => 'Access denied.'));
		}
	}
	else if ($entityTypeId === \CCrmOwnerType::Contact)
	{
		...
		{
			__CrmRequisiteListEndResonse(array('ERROR' => 'Contact not found.'));
		}
		else
		{
