					...
					foreach($tab["fields"] as $field)
					{
						...
						else
						{
							...
							switch($field["type"])
							{
								...
								if ($field["type"] == "crm" && !\Thurly\Main\ModuleManager::isModuleInstalled("crm") ||
									$field["type"] == "disk_file" && !\Thurly\Main\ModuleManager::isModuleInstalled("disk"))
									continue;
									$val = is_array($val) ? $val : array();
									$className = ($field["type"] == "disk_file" ? "file" : $field["type"]);
									ob_start();
									$APPLICATION->IncludeComponent("thurly:system.field.view", $field["type"],
										array(
											"arUserField" => $val,
											"MOBILE" => "Y",
											"CAN_EDIT" => false,
								...
								case 'disk':
									...
									$className = ($field["type"] == "disk_file" ? "file" : $field["type"]);
									ob_start();
									$APPLICATION->IncludeComponent("thurly:system.field.edit", $field["type"],
										array(
											"arUserField" => $val,
											"MOBILE" => "Y",
											"CAN_EDIT" => true,
