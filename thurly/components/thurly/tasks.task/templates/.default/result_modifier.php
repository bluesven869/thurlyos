...
$request = \Thurly\Main\HttpApplication::getInstance()->getContext()->getRequest()->toArray();

$arParams['NAME_TEMPLATE'] = empty($arParams['NAME_TEMPLATE']) ? \Thurly\Tasks\Util\Site::getUserNameFormat() : str_replace(array("#NOBR#","#/NOBR#"), array("",""), $arParams["NAME_TEMPLATE"]);
...
$editorProps = array();
$ufCode = \Thurly\Tasks\Integration\Disk\UserField::getMainSysUFCode();
...
if(is_array($arParams['TASK_URL_PARAMETERS']) && !empty($arParams['TASK_URL_PARAMETERS']))
{
	...
	{
		$arParams['PATH_TO_TASKS_TASK'] = \Thurly\Tasks\Util::replaceUrlParameters($arParams['PATH_TO_TASKS_TASK'], $arParams['TASK_URL_PARAMETERS']);
	}
}
		...
		if(array_key_exists(TasksTaskFormState::O_CHOSEN, $state))
		{
			...
			if(array_key_exists($block, $taskData) && !empty($taskData[$block]) && $block != 'SE_ORIGINATOR')
			{
				...
				elseif($block == 'UF_CRM_TASK')
				{
					...
					$normalData = \Thurly\Tasks\Util\Type::normalizeArray($taskData[$block]);
			...
			else // "special" cases
			{
				if($block == 'SE_ORIGINATOR')
				{
					...
					$filled = $taskData['SE_ORIGINATOR']['ID'] != \Thurly\Tasks\Util\User::getId();
				}
				if($block == 'DATE_PLAN')
				{
                ...
                if($block == 'USER_FIELDS')
                {
                    ...
                    {
                        if(\Thurly\Tasks\Util\UserField::isUFKey($field) && !empty($value))
                        {
                            $filled = true;
                            break;
                        }
...
// etc
$arResult['TEMPLATE_DATA']['TAG_STRING'] = \Thurly\Tasks\UI\Task\Tag::formatTagString($arResult['DATA']['TASK']['SE_TAG']);
...
if(Type::isIterable($arResult['DATA']['EVENT_TASK']))
{
	...
	$arResult['DATA']['EVENT_TASK_SAFE'] = \Thurly\Tasks\Util::escape($arResult['DATA']['EVENT_TASK']);
}

// checklist pre-format
// todo: remove this when use object with array access instead of ['ITEMS']['DATA']
$code = \Thurly\Tasks\Manager\Task\CheckList::getCode(true);
...
// current user
$arResult['AUX_DATA']['USER']['DATA'] = \Thurly\Tasks\Util\User::extractPublicData($arResult['AUX_DATA']['USER']['DATA']);
...
$arResult['DATA']['TASK']['DEPENDS_ON'] = $dependsOn;

$params = array();
if(Thurly\Tasks\Util\Type::isIterable($taskData['SE_PARAMETER']))
{
...
$project = array();
if(count($taskData['SE_PROJECT']))
{
	$project = $taskData['SE_PROJECT'];
	$project['ENTITY_TYPE'] = \Thurly\Tasks\Integration\SocialNetwork::getGroupEntityPrefix();
