...
$diskUfCode = \Thurly\Tasks\Integration\Disk\UserField::getMainSysUFCode();
...
<div class="task-detail">
	<div class="task-detail-info">
		<div class="task-detail-header">
			...
			<? else:
				...
				<div class="task-detail-subtitle-status <?if($expired):?>task-detail-subtitle-status-delay-message<?endif?>"><?=Loc::getMessage('TASKS_TTV_SUB_TITLE', array(
					...
					))?> - <span id="task-detail-status-below-name"><?=\Thurly\Tasks\UI::toLowerCaseFirst(Loc::getMessage("TASKS_TASK_STATUS_".$taskData["REAL_STATUS"]))?><?if($expired):?></span>, <?=\Thurly\Tasks\UI::toLowerCaseFirst(Loc::getMessage('TASKS_STATUS_OVERDUE'))?><?endif?>
				</div>
			<? endif ?>
		</div>
		...
		<?if (isset($userFields[$diskUfCode]) && !empty($userFields[$diskUfCode]["VALUE"])):?>
			<div class="task-detail-files" id="task-detail-files">
				<?\Thurly\Tasks\Util\UserField\UI::showView($userFields[$diskUfCode], array(
		...
		<? if ($templateData["SHOW_USER_FIELDS"]): ?>
			<div class="task-detail-properties">
				...
				{
					if (empty($userField["VALUE"]) || $userField["FIELD_NAME"] === $diskUfCode || !\Thurly\Tasks\Util\UserField\UI::isSuitable($userField))
					{
						continue;
					}
					?>
					<tr>
						...
						<td class="task-detail-property-value">
							<?\Thurly\Tasks\Util\UserField\UI::showView($userField, array(
								"PUBLIC_MODE" => $arParams["PUBLIC_MODE"],
	...
	<div class="task-detail-buttons"><?
		$APPLICATION->IncludeComponent(
			"thurly:tasks.widget.buttons",
			"task",
			array(
				"GROUP_ID" => $arParams["GROUP_ID"],
				"PATH_TO_TASKS_TASK" => $arParams["PATH_TO_TASKS_TASK"],
				"PATH_TO_TASKS_TASK_COPY" => \Thurly\Tasks\UI\Task::makeCopyUrl($arParams["PATH_TO_TASKS_TASK"], $taskData["ID"]),
				"PATH_TO_TASKS_TASK_CREATE_SUBTASK" => \Thurly\Tasks\UI\Task::makeCreateSubtaskUrl($arParams["PATH_TO_TASKS_TASK"], $taskData["ID"]),
				"PATH_TO_USER_PROFILE" => $arParams["PATH_TO_USER_PROFILE"],
	...
	<? if (count($templateData["PREDECESSORS"])):?>
		...
		<div class="task-detail-list">
			...
			<?$APPLICATION->IncludeComponent(
				'thurly:tasks.widget.related.selector',
				'staticgrid',
				array(
				...
				<span class="task-switcher" id="task-time-switcher" data-id="time">
					<span class="task-switcher-text">
						<span class="task-switcher-text-inner">
							...
							<span class="task-switcher-text-counter" id="task-switcher-elapsed-time"><?=\Thurly\Tasks\UI::formatTimeAmount($templateData["ELAPSED"]['TIME'])?></span>
						</span>
					</span>
				</span>
...
$APPLICATION->IncludeComponent(
	...
	array(
		...
		"TASK_ID" => $taskData["ID"],
		"PUBLIC_MODE" => $arParams["PUBLIC_MODE"],
		"USER" => $arResult['DATA']['USER'][\Thurly\Tasks\Util\User::getId()],
		"TEMPLATE_DATA" => array(
			"DATA" => $arResult["DATA"],
...
<script>
	...
	new BX.Tasks.Component.TaskView({
		...
		paramsToLazyLoadTabs: <?=\Thurly\Main\Web\Json::encode($arResult['PARAMS_TO_LAZY_LOAD_TABS'])?>
	});
