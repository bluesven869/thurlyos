...
while($arEvent = $events->Fetch())
{
	...
	foreach ($arStatuses as $key => $arStatus)
	{
		...
		if($arResult['ENTITY'][$arStatus['ENTITY_ID']]['FINAL_SUCCESS_FIELD'] == $arStatus['STATUS_ID'])
		{
			$arResult['ENTITY'][$arStatus['ENTITY_ID']]['FINAL_SORT'] = $arStatus['SORT'];
		}

		if(!empty($settings))
		{
			$arResult['ROWS'][$arStatus['ENTITY_ID']][$arStatus['ID']]['COLOR'] = $settings[$arStatus['ENTITY_ID']][$arStatus['STATUS_ID']]['COLOR'];
		}
	}
}
