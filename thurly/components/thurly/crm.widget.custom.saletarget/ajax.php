...
$saleTargetWidget = \Thurly\Crm\Widget\Custom\SaleTarget::getInstance();
...
switch ($action)
{
	case 'GET_CONFIGURATION':
		...
		list($current, $totalCurrent) = \Thurly\Crm\Widget\Data\DealSaleTarget::getCurrentValues($data['configuration']);
	...
	case 'NOTIFY_ADMIN':
		...
		if ($userId && Thurly\Main\Loader::includeModule('im'))
		{
			...
			if (isset($admins[$userId]))
			{
				...
				\CIMNotify::Add(array(
					...
					'NOTIFY_MESSAGE' => \Thurly\Main\Localization\Loc::getMessage('CRM_WIDGET_SALETARGET_AJAX_NOTIFY_TEXT', array(
						'#URL#' => '/crm/start'
					))
				));
			}
