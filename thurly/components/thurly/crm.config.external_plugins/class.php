...
use \Thurly\Rest\APAuth\PermissionTable;
use \Thurly\Rest\APAuth\PasswordTable;
use \Thurly\Main\Config\Option;

class CrmConfigExternalPluginsComponent extends \CThurlyComponent
{
	protected $id = '';
	protected $uid = 0;
	protected $allowedCMS = array('1cthurly', 'wordpress', 'drupal7', 'magento2', 'joomla');
	...
	/**
	 * Make some actions.
	 * @param Thurly\Main\HttpRequest $request
	 * @return void
	 */
	protected function makeAction(Thurly\Main\HttpRequest $request)
	{
		$redirect = false;

		if ($this->id)
		{
			if (($request->get('enable') == 1 || $request->get('enable') == 0) && check_thurly_sessid())
			{
				$redirect = true;
