...
else
{
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					$arImage = CSocNetTools::InitImage($arResult["Group"]["IMAGE_ID"], 50, "/thurly/images/socialnetwork/nopic_group_50.gif", 50, $arResult["Urls"]["Group"], true, 'target="_blank"');
	...
	else
	{
		...
		if (StrLen($arResult["FatalError"]) <= 0)
		{
			...
			$arImage = CSocNetTools::InitImage($arResult["User"]["PERSONAL_PHOTO"], 50, "/thurly/images/socialnetwork/nopic_user_50.gif", 50, $arResult["CurrentUserPerms"]["Operations"]["viewprofile"] ? $arResult["Urls"]["User"] : "", true, 'target="_blank"');
	...
	if (StrLen($arResult["FatalError"]) <= 0)
	{
		...
		$dirPath = '/thurly/components/thurly/socialnetwork.messages_chat';
