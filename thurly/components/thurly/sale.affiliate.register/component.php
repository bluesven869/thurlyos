...
if (CModule::IncludeModule("sale"))
{
	...
	if ($arResult["AFFILIATE"] == "N")
	{
		...
		if (!$GLOBALS["USER"]->IsAuthorized())
		{
			...
			$arResult["DEFAULT_USER_LOGIN"] = ${COption::GetOptionString("main", "cookie_name", "THURLY_SM")."_LOGIN"};
		...
		else
		{
			...
			{
				$arResult["AGREEMENT_TEXT_FILE"] = "/thurly/components/thurly/sale.affiliate.register/agreement-".SITE_ID.".htm";
				if (!file_exists($_SERVER["DOCUMENT_ROOT"].$arResult["AGREEMENT_TEXT_FILE"]))
				{
					$arResult["AGREEMENT_TEXT_FILE"] = "/thurly/php_interface/agreement.htm";
					if (!file_exists($_SERVER["DOCUMENT_ROOT"].$arResult["AGREEMENT_TEXT_FILE"]))
					{
