	...
	<div id="form_comment_" >
		...
		<div class="blog-comment-fields">
			...
			function CustomizeLHEForBlogComments()
			{
				...
				LHEButtons['Image'].src = '/thurly/components/thurly/blog/templates/.default/images/bbcode/font_image_upload.gif';
				LHEButtons['Image'].name = '<?=GetMessage("BLOG_P_IMAGE_LINK")?>';

				LHEButtons['BlogInputVideo'] = {
					id : 'BlogInputVideo',
					src : '/thurly/components/thurly/blog/templates/.default/images/bbcode/font_video.gif',
						...
						obj: {
							Parse: function(sName, sContent, pLEditor)
							{
								sContent = sContent.replace(/\[VIDEO\s*?width=(\d+)\s*?height=(\d+)\s*\]((?:\s|\S)*?)\[\/VIDEO\]/ig, function(str, w, h, src)
								{
									...
									return '<img id="' + pLEditor.SetBxTag(false, {tag: "blogvideo", params: {value : src}}) + '" src="/thurly/images/1.gif" class="bxed-video" width=' + w + ' height=' + h + ' title="' + BX.message.Video + ": " + src + '" />';
								});
								return sContent;
							},
				...
				window.LHEDailogs['BlogVideo'] = function(pObj)
				{
					...
					return {
						...
						OnSave: function()
						{
							...
							{
								pLEditor.InsertHTML('<img id="' + pLEditor.SetBxTag(false, {tag: "blogvideo", params: {value : src}}) + '" src="/thurly/images/1.gif" class="bxed-video" width=' + w + ' height=' + h + ' title="' + BX.message.Video + ": " + src + '" />');
							}
						}
					};
				};
				...
				LHEButtons['CreateLinkNC'] = {
					id : 'CreateLinkNC',
					src : '/thurly/components/thurly/blog/templates/.default/images/bbcode/font_link.gif',
			...
			if(!empty($arResult["Smiles"]))
			{
				foreach($arResult["Smiles"] as $arSmile)
				{
					$arSmiles[] = array(
						...
						'path' => "/thurly/images/blog/smile/".$arSmile["IMAGE"],
			...
			if($comment["PUBLISH_STATUS"] == BLOG_PUBLISH_STATUS_PUBLISH || $comment["SHOW_SCREENNED"] == "Y" || $comment["ID"] == "preview")
			{
				...
				<div class="blog-comment-info">
					...
					if(strlen($comment["AVATAR_img"]) > 0)
						...
						echo '<img src="/thurly/components/thurly/blog/templates/.default/images/noavatar.gif" border="0">';
						?></div>
					...
					if(strlen($comment["urlToBlog"])>0)
					{
						...
						<?
						$GLOBALS["APPLICATION"]->IncludeComponent("thurly:main.user.link",
							'',
							array(
				...
				<div class="blog-comment-content">
					...
					if(strlen($comment["urlToShow"])>0)
					{
						?>
						<span class="blog-vert-separator"></span>
						<span class="blog-comment-show"><a href="<?=$comment["urlToShow"]."&".thurly_sessid_get()?>"><?=GetMessage("BPC_MES_SHOW")?></a></span>
						<?
					}
					if(strlen($comment["urlToHide"])>0)
					{
						?>
						<span class="blog-vert-separator"></span>
						<span class="blog-comment-show"><a href="<?=$comment["urlToHide"]."&".thurly_sessid_get()?>"><?=GetMessage("BPC_MES_HIDE")?></a></span>
						<?
					}
					if(strlen($comment["urlToDelete"])>0)
					{
						?>
						<span class="blog-vert-separator"></span>
						<span class="blog-comment-delete"><a href="javascript:if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$comment["urlToDelete"]."&".thurly_sessid_get()?>'"><?=GetMessage("BPC_MES_DELETE")?></a></span>
						<?
					}
