...
use Thurly\Disk\Ui\Icon;
use Thurly\Disk\Ui;
use Thurly\Disk\ZipNginx;
...
class CDiskExternalLinkComponent extends DiskComponent
{
	...
	/** @var \Thurly\Disk\ExternalLink */
	protected $externalLink;
	/** @var string */
	protected $hash;
	/** @var string */
	protected $downloadToken;
	/** @var \Thurly\Disk\Document\DocumentHandler  */
	...
	{
		if ($e instanceof \Thurly\Main\ObjectNotFoundException)
		{
			$this->showNotFoundPage();
		}
		else
	...
	protected function prepareParams()
	{
		...
		if(!$hash)
		{
			throw new \Thurly\Main\ArgumentException('Empty hash');
		}

		if(!\Thurly\Disk\ExternalLink::isValidValueForField('HASH', $hash, $this->errorCollection))
		{
			throw new \Thurly\Main\ArgumentException('Hash contains invalid character');
		}
	...
	protected function processActionDefault($path = '/')
	{
		...
		$server = \Thurly\Main\Application::getInstance()->getContext()->getServer();
		$this->arResult = array(
		...
		if($isFolder)
		{
			...
			if(!$targetFolder)
			{
				throw new \Thurly\Main\SystemException('Wrong path');
			}

			$this->arResult['FOLDER'] = $this->getResultByFolder();
			$this->arResult['ENABLED_MOD_ZIP'] = \Thurly\Disk\ZipNginx\Configuration::isEnabled();
	...
	{
		$data = Driver::getInstance()->getUrlManager()->resolvePathUnderRootObject($rootFolder, $path);
		if (!$data)
		{
			return null;
		}
	...
	protected function getBreadcrumbs($path, array $relativeItems)
	{
		...
		$uri = new \Thurly\Main\Web\Uri($this->request->getRequestUri());
		...
		foreach ($relativeItems as $i => $item)
		{
			...
			$uri->deleteParams(
				array_merge(
					\Thurly\Main\HttpRequest::getSystemParameters(),
					array('path')
				)
			);
			$uri->addParams(
	...
	private function getGridData(Folder $rootFolder, Folder $targetFolder, $path, $gridId)
	{
		...
		foreach ($cursor as $row)
		{
			...
			if ($isFolder)
			{
				...
				$actions[] = array(
					...
					"DEFAULT" => true,
					"ICONCLASS" => "show",
					"TEXT" => Loc::getMessage('DISK_EXTERNAL_OBJECT_ACT_OPEN'),
			...
			if ($isFile)
			{
				...
				$actions[] = array(
					...
					"DEFAULT" => true,
					"ICONCLASS" => "download",
					"TEXT" => Loc::getMessage('DISK_EXTERNAL_OBJECT_ACT_DOWNLOAD'),
			...
			if ($isFolder)
			{
				$dataAttributesForViewer = Ui\Viewer::getAttributesByExternalLink($this->externalLink, $object);
			...
			{
				$dataAttributesForViewer = Ui\Viewer::getAttributesByExternalLink($this->externalLink, $object, array(
					'urlForDownload' => $exportData['OPEN_URL'],
					'path' => $path,
					'token' => $this->downloadToken,
					'urlForShow' => $exportData['OPEN_URL'],
					'disableDocumentViewer' => $this->arResult['DISABLE_DOCUMENT_VIEWER'] || $this->externalLink->hasPassword(),
				));
				$nameSpecialChars = htmlspecialcharsbx($name);
				$columnName = "
					<table class=\"bx-disk-object-name\"><tr>
		...
		$grid['HEADERS'] = array(
			...
			array(
				'id' => 'NAME',
				'name' => Loc::getMessage('DISK_EXTERNAL_OBJECT_COLUMN_NAME'),
				'sort' => false,
				'default' => true,
			),
			array(
				'id' => 'CREATE_TIME',
				'name' => Loc::getMessage('DISK_EXTERNAL_OBJECT_COLUMN_CREATE_TIME'),
				'sort' => false,
				'default' => false,
			),
			array(
				'id' => 'UPDATE_TIME',
				'name' => Loc::getMessage('DISK_EXTERNAL_OBJECT_COLUMN_UPDATE_TIME'),
				'sort' => false,
				'order' => 'desc',
				'default' => true,
			),
			array(
				'id' => 'FORMATTED_SIZE',
				'name' => Loc::getMessage('DISK_EXTERNAL_OBJECT_COLUMN_FORMATTED_SIZE'),
				'sort' => false,
				'default' => true,
			),
		);
	...
	private function getResultByFile()
	{
		...
		elseif ($file->getView()->getData())
		{
			...
			$height = 520;
			$width = 720;
			if ($file->getView() instanceof \Thurly\Disk\View\Video && !$file->getView()->getPreviewData())
			{
				$height = 400;
				$width = 600;
			}
	...
	{
		$fileData = new \Thurly\Disk\Document\FileData();
		$fileData
			->setFile($file)
	...
	private function canMakePreview(File $file)
	{
		...
		return !$this->externalLink->hasPassword() && $this->defaultHandlerForView instanceof \Thurly\Disk\Document\GoogleViewerHandler;
	}
	...
	{
		$this->externalLink = \Thurly\Disk\ExternalLink::load(array('=HASH' => $this->hash), array('OBJECT'));

		if(!$this->externalLink || $this->externalLink->isExpired())
		{
			throw new \Thurly\Main\ObjectNotFoundException('Invalid external link');
		}
