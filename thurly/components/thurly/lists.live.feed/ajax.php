<?php
use Thurly\Lists\Internals\Error\Error;
use Thurly\Main\Localization\Loc;
use Thurly\Main\Loader;
use Thurly\Lists\Internals\Controller;
...
if (!Loader::IncludeModule('lists') || !\Thurly\Main\Application::getInstance()->getContext()->getRequest()->getQuery('action'))
{
	return;
}
...
class LiveFeedAjaxController extends Controller
{
	...
	/** @var  string */
	protected $iblockTypeId = 'thurly_processes';
	protected $listPerm;
	protected $formId;
	...
	protected function createHtmlSetConstants($templateId, $templateName)
	{
		...
		$this->getApplication()->includeComponent(
			'thurly:bizproc.workflow.setconstants',
			'',
	...
	{
		if($_POST["save"] != "Y" && $_POST["changePostFormTab"] != "lists" && !check_thurly_sessid())
			$this->errorCollection->add(array(new Error(Loc::getMessage('LISTS_SEAC_CONNECTION_MODULE_IBLOCK'))));
	...
	protected function createPreparedFields()
	{
		...
		foreach($this->lists['FIELDS'] as $fieldId => $field)
		{
			...
			elseif($field["PROPERTY_TYPE"] == "E")
			{
				...
				$randomGenerator = new Thurly\Main\Type\RandomSequence($fieldId);
				$randString = strtolower($randomGenerator->randString(6));
				$html = '';
				global $APPLICATION;
				ob_start();
				$APPLICATION->includeComponent('thurly:iblock.element.selector', '',
					array(
	...
	protected function createHtml()
	{
		...
		{
			case 'thurly_outgoing_doc':
				$blueDudeId = 1304135;
				break;
			case 'thurly_incoming_doc':
				$blueDudeId = 1304134;
				break;
			case 'thurly_cash':
				$blueDudeId = 1304133;
