...
else
{
	...
	?><script type="text/javascript"><?
		foreach($arResult['TASK'] as $arEvent):
			...
			?>BX.tooltip('<?=$arTask['ENTITY_TYPE']?>_<?=$arTask['ENTITY_ID']?>', "balloon_<?=$arResult['GRID_ID']?>_T_<?=$arTask['ID']?>", "/thurly/components/thurly/crm.<?=strtolower($arTask['ENTITY_TYPE'])?>.show/card.ajax.php", "crm_balloon<?=($arTask['ENTITY_TYPE'] == 'LEAD' || $arTask['ENTITY_TYPE'] == 'DEAL' || $arTask['ENTITY_TYPE'] == 'QUOTE' ? '_no_photo': '_'.strtolower($arTask['ENTITY_TYPE']))?>", true);<?
			endif;
		endforeach;
	?></script><?
}
