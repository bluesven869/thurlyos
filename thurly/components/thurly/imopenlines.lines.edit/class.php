...
use \Thurly\Main\Loader,
	...
	\Thurly\ImOpenlines\QuickAnswers;


class ImOpenLinesComponentLinesEdit extends CThurlyComponent
{
	...
	private function updateLine()
	{
		...
		if (!empty($post['CONFIG']['QUEUE']['U']))
		{
			...
			foreach ($post['CONFIG']['QUEUE']['U'] as $userCode)
			{
				...
				if (\Thurly\Im\User::getInstance($userId)->isExtranet())
					continue;
			...
			\Thurly\Main\FinderDestTable::merge(array(
				"CONTEXT" => "IMOPENLINES",
				"CODE" => \Thurly\Main\FinderDestTable::convertRights($arAccessCodes, array('U'.$GLOBALS["USER"]->GetId()))
			));
		}
	...
	private function showConfig()
	{
		...
		if ($configId == 0)
		{
			...
			if(!$this->userPermissions->canPerform(\Thurly\ImOpenlines\Security\Permissions::ENTITY_LINES, \Thurly\ImOpenlines\Security\Permissions::ACTION_MODIFY))
			{
				LocalRedirect($this->arResult['PATH_TO_LIST']);
				return false;
			}
		...
		$this->arResult['LANGUAGE_LIST'] = \Thurly\Intranet\Util::getLanguageList();
		if (!$config["LANGUAGE_ID"])
		{
			$context = \Thurly\Main\Context::getCurrent();
		...
		$this->arResult['CRM_SOURCES'] = \Thurly\Main\Loader::includeModule('crm')? CCrmStatus::GetStatusList('SOURCE'): Array();
		...
		{
			$list = \Thurly\Im\Bot::getListCache(\Thurly\Im\Bot::LIST_OPENLINE);
			foreach ($list as $botId => $botData)
			{
				$this->arResult['BOT_LIST'][$botId] = \Thurly\Im\User::getInstance($botId)->getFullName();
			}
		}
		...
		$usersLimit = \Thurly\Imopenlines\Limit::getLicenseUsersLimit();
		if ($usersLimit)
		{
	...
	protected function isCreateNewQuickAnswersStorageAllowed($config, $storageList = null)
	{
		...
		$configsWithTheSameQuickAnswersStorage = \Thurly\ImOpenLines\Model\ConfigTable::getCount(array('=QUICK_ANSWERS_IBLOCK_ID' => $config['QUICK_ANSWERS_IBLOCK_ID']));
