...
if($arResult['NEED_FOR_TRANSFER_PS_REQUISITES']):
	...
	<?=Thurly\Crm\Requisite\Conversion\PSRequisiteConverter::getIntroMessage(
		array(
...
$prefixLC = strtolower($arResult['GRID_ID']);
	...
	foreach($arResult['QUOTE'] as $sKey =>  $arQuote)
	{
		...
		if(!$isInternal && $arResult['CAN_CONVERT'])
		{
			...
			{
				$arSchemeDescriptions = \Thurly\Crm\Conversion\QuoteConversionScheme::getJavaScriptDescriptions(true);
				$arSchemeList = array();
			...
			'columns' => array(
				...
				'STATUS_ID' => CCrmViewHelper::RenderQuoteStatusControl(
					array(
						...
						'SERVICE_URL' => '/thurly/components/thurly/crm.quote.list/list.ajax.php'
					)
				),
	...
	array(
		...
		'EXTENSION' => array(
			...
			'CONFIG' => array(
				...
				'activityServiceUrl' => '',
				'taskCreateUrl'=> '',
				'serviceUrl' => '/thurly/components/thurly/crm.quote.list/list.ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
		...
		BX.ready(
			function()
			{
				BX.CrmQuoteConversionScheme.messages =
					<?=CUtil::PhpToJSObject(\Thurly\Crm\Conversion\QuoteConversionScheme::getJavaScriptDescriptions(false))?>;
				...
				{
					serviceUrl: "<?='/thurly/components/thurly/crm.quote.show/ajax.php?action=convert&'.thurly_sessid_get()?>",
		...
		BX.ready(
			function()
			{
				...
				var manager = BX.AutorunProcessManager.create("rebuildQuoteSearch",
					{
						serviceUrl: "<?='/thurly/components/thurly/crm.quote.list/list.ajax.php?'.thurly_sessid_get()?>",
		...
		BX.ready(
			function()
			{
				...
				var converter = BX.CrmPSRequisiteConverter.create(
					"psRqConverter",
					{
						serviceUrl: "<?=SITE_DIR?>thurly/components/thurly/crm.config.ps.list/list.ajax.php?&<?=thurly_sessid_get()?>"
					}
				);
