...
class CCrmConfigEmailtrackerAjax
{
	...
	public static function execute()
	{
		...
		if ($error === false)
		{
			if (\CModule::includeModule('thurlyos'))
				self::$limitedLicense = !in_array(\CThurlyOS::getLicenseType(), array('company', 'nfr', 'edu', 'demo'));
		...
		if ($error instanceof \Thurly\Main\ErrorCollection)
	...
	private static function executeCreateMailbox(&$error)
	{
		...
		if ($error === false)
		{
			...
			{
				$service = \Thurly\Mail\MailServicesTable::getList(array(
					'filter' => array(
						'=ID'          => $_REQUEST['SERVICE'],
						'ACTIVE'       => 'Y',
						'SERVICE_TYPE' => 'imap',
		...
		if ($error === false)
		{
				...
				if (!empty($_REQUEST['oauth']) && \CModule::includeModule('socialservices'))
				{
					...
					if ($service['NAME'] == 'gmail' && $service['SERVER'] == 'imap.gmail.com')
					{
						...
						if ($oauthClient->checkSettings())
						{
							...
							if (empty($response['email']))
							{
								...
								$error->setError(new \Thurly\Main\Error(getMessage('INTR_MAIL_IMAP_OAUTH_ACC'), 0));

								if (!empty($response['error']['message']))
									$error->setError(new \Thurly\Main\Error($response['error']['message'], -1));
							}
							else
							{
					...
					if ($service['NAME'] == 'outlook.com' && $service['SERVER'] == 'imap-mail.outlook.com')
					{
						...
						if ($oauthClient->checkSettings())
						{
							...
							if ($oauthEntity->getAccessToken())
							{
								$httpClient = new \Thurly\Main\Web\HttpClient();
			...
			if ($error === false)
			{
				$unseen = \Thurly\Mail\Helper::getImapUnseen($mbData, 'inbox', $error);
				if ($unseen !== false)
					$error = false;
			}
			...
			if ($error === false)
			{
				...
				if ($mbId > 0)
				{
					\Thurly\Main\Config\Option::set('mail', 'last_mail_check', time(), self::$currentSite['LID']);
					\Thurly\Main\Config\Option::set('mail', 'last_mail_check_success', $unseen >= 0 ? 'Y' : 'N', self::$currentSite['LID']);

					\Thurly\Mail\BlacklistTable::replace(
			...
			if ($error === false)
			{
				...
				$imapDirs = \Thurly\Mail\Helper::listImapDirs($mbData, $error);
				if ($imapDirs !== false)
				{
					$error = false;
				...
				if ($error === false)
				{
					...
					if ($result)
					{
						...
						{
							$filterFields['ACTION_DELETE_MESSAGE'] = 'Y';
							$filterFields['ACTION_STOP_EXEC'] = 'Y';
						}
	...
	private static function executeEditMailbox(&$error)
	{
		...
		if ($error === false)
		{
			...
			if ($error === false)
			{
				...
				$imapDirs = \Thurly\Mail\Helper::listImapDirs(array_merge($mailbox, $mbData), $error, $errors);
				if ($imapDirs !== false)
				{
					$error = false;
					...
					{
						$income = (array) \Thurly\Main\Text\Encoding::convertEncoding(
							$_REQUEST['imap_dirs']['income'], 'UTF-8', SITE_CHARSET
						);
						$mbData['OPTIONS']['imap']['income'] = array_intersect($income, $availableDirs);

						$outcome = (array) \Thurly\Main\Text\Encoding::convertEncoding(
				...
				{
					if ($errors instanceof \Thurly\Main\ErrorCollection)
						$error = $errors;
				}
			}
			...
			if ($error === false)
			{
				$unseen = \Thurly\Mail\Helper::getImapUnseen(array_merge($mailbox, $mbData), 'inbox', $error, $errors);
				if ($unseen === false)
				{
					if ($errors instanceof \Thurly\Main\ErrorCollection)
						$error = $errors;
	...
	private static function executeCheckMailbox(&$error)
	{
		...
		if ($error === false)
		{
			$unseen = \Thurly\Mail\Helper::getImapUnseen($mailbox, 'inbox', $error);
			if ($unseen !== false)
				$error = false;

			\Thurly\Main\Config\Option::set('mail', 'last_mail_check', time(), $mailbox['LID']);
			\Thurly\Main\Config\Option::set('mail', 'last_mail_check_success', $unseen >= 0 ? 'Y' : 'N', $mailbox['LID']);
		}
	...
	private static function executeEnableCrm(&$error)
	{
		...
		if ($error === false)
		{
			...
			if ($error === false)
			{
				$imapDirs = \Thurly\Mail\Helper::listImapDirs($mailbox, $error);
				if ($imapDirs !== false)
				{
					$error = false;
			...
			if ($error === false)
			{
				...
				if ($result)
				{
					$res = \Thurly\Mail\MailFilterTable::getList(array(
						'select' => array('ID'),
						'filter' => array(
	...
	private static function prepareError(\Thurly\Main\ErrorCollection $errors)
	{
		$messages = array();
		$details  = array();
		...
		if (count($messages) == 1 && reset($messages)->getCode() == \Thurly\Mail\Imap::ERR_AUTH)
		{
			$messages = array(
				new \Thurly\Main\Error(getMessage('INTR_MAIL_IMAP_AUTH_ERR_EXT'), \Thurly\Mail\Imap::ERR_AUTH)
			);
		}
	...
	private static function decodeJson($json)
	{
		...
		try
		{
			return \Thurly\Main\Web\Json::decode($json);
		}
		catch (Exception $e)
		{
			return null;
	...
	private static function returnJson($data)
	{
		...
		echo \Thurly\Main\Web\Json::encode($data);
		die;
	}
