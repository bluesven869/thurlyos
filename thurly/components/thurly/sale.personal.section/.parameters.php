...
$arComponentParameters = array(
	...
	"PARAMETERS" => array(		
		...
		'CUSTOM_PAGES' => array(
			...
			'TYPE' => 'CUSTOM',
			'JS_FILE' => "/thurly/components/thurly/sale.personal.section/settings/settings.js",
...
if ($arCurrentValues["SHOW_ACCOUNT_PAGE"] !== "N" && CBXFeatures::IsFeatureEnabled('SaleAccounts'))
{
	...
	if ($arCurrentValues["SHOW_ACCOUNT_PAY_COMPONENT"] !== "N")
	{
		...
		{
			$personTypeList = Thurly\Sale\PersonType::load($siteId);
			foreach ($personTypeList as $personTypeElement)
			{
				$personTypes[$personTypeElement["ID"]] = $personTypeElement['NAME'];
			}
			$baseCurrencyCode = Thurly\Sale\Internals\SiteCurrencyTable::getSiteCurrency($siteId);
		}
		...
		$paySystemManagerResult = Thurly\Sale\PaySystem\Manager::getList(array('select' => array('ID','NAME')));
...
if ($arCurrentValues["SHOW_ORDER_PAGE"] !== "N")
{
	...
	if(CModule::IncludeModule("sale"))
	{
		...
		$listStatusNames = Thurly\Sale\OrderStatus::getAllStatusesNames(LANGUAGE_ID);
