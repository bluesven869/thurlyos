...
if (CModule::IncludeModule("form"))
{
	...
	if ($del_id > 0 && check_thurly_sessid())
	{
		$GLOBALS['strError'] = '';
	...
	if ($_REQUEST["delete"] && check_thurly_sessid())
	{
		$ARR_RESULT = $_REQUEST["ARR_RESULT"];
		if (is_array($ARR_RESULT) && count($ARR_RESULT) > 0 && check_thurly_sessid())
		{
			$GLOBALS['strError'] = '';
	...
	if ($arParams["arFormInfo"] = $dbres->Fetch())
	{
		...
		if (strlen($_POST['save'])>0 && $_SERVER['REQUEST_METHOD']=="POST" && check_thurly_sessid())
		{
			// update results
		...
		$arResult["tf_name"] = COption::GetOptionString("main", "cookie_name", "THURLY_SM")."_FORM_RESULT_FILTER";
