...
if (!function_exists("__wd_get_root_section"))
{
	function __wd_get_root_section($IBLOCK_ID, $object, $object_id)
	{
		...
		/*		$res = CSocNetGroup::GetByID($arResult["VARIABLES"]["group_id"]);
			...
			if(\Thurly\Main\Config\Option::get('disk', 'successfully_converted', false) && CModule::includeModule('disk'))
			{
				\Thurly\Disk\Driver::getInstance()->addGroupStorage($arFields["SOCNET_GROUP_ID"]);
			}
...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/interface/admin_lib.php");
...
elseif ($_SERVER["REQUEST_METHOD"] == "POST")
{
	...
	include($_SERVER["DOCUMENT_ROOT"].BX_ROOT."/components/thurly/webdav.iblock.rights/action.php");
...
?>
	<tr>
		<td width="50%" align="right" valign="top">
			<a href="/thurly/admin/userfield_edit.php?ID=<?=$field["ID"]?>&back_url=<?=htmlspecialcharsbx($backUrl)?>"><?=htmlspecialcharsbx($name)?></a>:
		</td>
		<td width="50%">
...
?>
	<tr>
		<td colspan="2" align="center" valign="top">
			<a href="/thurly/admin/userfield_edit.php?ENTITY_ID=<?=htmlspecialcharsbx($UF_ENTITY)?>&back_url=<?=htmlspecialcharsbx($backUrl)?>"><?=GetMessage("IB_WDUF_ADD")?></a>
		</td>
	</tr>
<? } ?>
</table>
...
<?
	...
	$APPLICATION->IncludeComponent("thurly:webdav.iblock.rights", ".default", $arWDRights, null, array("HIDE_ICONS" => "Y"));
