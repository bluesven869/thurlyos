...
class CrmActivityVisitComponent extends \CThurlyComponent
{
	...
	public static function saveActivity($fields, $userId, $siteId)
	{
		...
		if($activityId > 0)
		{
			...
			\Thurly\Crm\Automation\Trigger\VisitTrigger::execute($bindings, array('ACTIVITY_ID' => $activityId));
		}
		else
		{
	...
	public static function saveRecognizeConsent($fields)
	{
		...
		$checkCursor = \Thurly\Faceid\AgreementTable::getList(array(
	...
	public static function recognizeFace($fields, $userId)
	{
		...
		$imageFile = new Thurly\Main\IO\File($uploadedFileDescriptor['tmp_name']);
		$imageFile->open('rb');
		...
		$recognizeResponse = \Thurly\FaceId\FaceId::identify($rawImage, 'vtracker');
	...
	protected function prepareDataForSocial()
	{
		...
		$rawImage = $imageFile->getContents();
		$recognizeResponse = \Thurly\FaceId\FaceId::identifyVk($rawImage);
	...
	protected static function updateRecognizePicture($entityTypeId, $entityId)
	{
		...
		$rawImage = $imageFile->getContents();
		$recognizeResponse = \Thurly\FaceId\FaceId::add($rawImage, 'vtracker');
