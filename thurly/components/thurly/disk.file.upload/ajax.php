...
class DiskFileUploadAjaxController extends \Thurly\Disk\Internals\Controller
{
	...
	protected function processActionUpload()
	{
		...
		{
			self::$uploader = new \Thurly\Main\UI\Uploader\Uploader(array(
				"events" => array(
					"onFileIsUploaded" => array($this, "processActionHandleFile")
				),
				"storage" => array(
					"cloud" => true,
					"moduleId" => \Thurly\Disk\Driver::INTERNAL_MODULE_ID
				)
			), "get");
		}
	...
	private function tryToRunWorkflow(File $file)
	{
		...
		{
			\Thurly\Disk\BizProcDocument::runAfterCreateWithInputParameters(
				$file->getStorageId(),
				$file->getId(),
				$workflowParameters
			);
		...
		{
			\Thurly\Disk\BizProcDocument::runAfterEditWithInputParameters(
				$file->getStorageId(),
				$file->getId(),
				$workflowParameters
			);
