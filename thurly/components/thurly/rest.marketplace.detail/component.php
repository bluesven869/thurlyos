...
$arApp = \Thurly\Rest\Marketplace\Client::getApp($arParams['APP'], $ver, $arResult['CHECK_HASH'], $arResult['INSTALL_HASH']);
...
if($arApp)
{
	...
	$dbApp = \Thurly\Rest\AppTable::getList(array(
	...
	if($ar)
	{
		...
		$arApp['APP_STATUS'] = \Thurly\Rest\AppTable::getAppStatusInfo($arApp, $APPLICATION->GetCurPageParam());
	...
	if ($arApp["ACTIVE"] == "Y")
	{
		$arApp["UPDATES"] = \Thurly\Rest\Marketplace\Client::getAvailableUpdate($arApp["CODE"]);
	}
...
if($request->isPost() && $request['install'] && check_thurly_sessid())
{
...
else
{
	if(is_array($arResult["APP"]["PRICE"]) && !empty($arResult["APP"]["PRICE"]) && $arResult['ADMIN'])
	{
		...
		foreach($arResult["APP"]["PRICE"] as $num => $price)
		{
			...
			{
				$arResult['BUY'][] = array(
					'LINK' => \Thurly\Rest\Marketplace\Client::getBuyLink($num, $arResult['APP']['CODE']),
