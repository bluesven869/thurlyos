	...
	if (!$serverParams['SECURE']): ?>
		<div id="bx-disk-network-drive-full" style="display:none;" class="bx-disk-network-drive-full-style">
			...
			<? if ($serverParams['AUTH_MODE'] == 'BASIC'): ?>
				<p><?= Loc::getMessage("DISK_NETWORK_DRIVE_REGISTERPATCH", array("#LINK#" => "/thurly/webdav/xp.reg"));?></p>
			<? endif;?>
		</div>
	<? endif;
}
...
{ ?>
	<div id="bx-disk-network-drive-full" style="display:none;" class="bx-disk-network-drive-full-style">
		...
		<? if ($serverParams['AUTH_MODE'] == 'BASIC'): ?>
			<p><?= Loc::getMessage("DISK_NETWORK_DRIVE_REGISTERPATCH", array("#LINK#" => "/thurly/webdav/vista.reg"));?></p>
		<? endif;?>
	</div>
<? }
...
<script>
	...
	function ShowImg(sImgPath, width, height, alt)
	{
		...
		alt = encodeURI(alt);
		window.open('/thurly/tools/imagepg.php?alt='+alt+'&img='+sImgPath,'','scrollbars='+scroll+',resizable=yes, width='+width+',height='+height+',left='+left+',top='+top);
	}
</script>