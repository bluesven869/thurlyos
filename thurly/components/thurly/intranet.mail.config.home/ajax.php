...
\Thurly\Main\Localization\Loc::loadMessages(__DIR__.'/class.php');
...
class CIntranetMailConfigHomeAjax
{
	...
	public static function execute()
	{
		...
		if ($error === false)
		{
			...
			{
				self::$crmAvailable = $USER->isAdmin() || $USER->canDoOperation('thurlyos_config')
	...
	private static function executeCreateMailbox(&$error)
	{
		...
		if ($error === false)
		{
			...
			else if ($services[$serviceId]['type'] == 'crdomain')
			{
				...
				if (!$USER->isAdmin() && !$USER->canDoOperation('thurlyos_config') && $services[$serviceId]['encryption'] != 'N')
					$error = CMail::getErrorMessage(CMail::ERR_API_OP_DENIED);
			...
			else if ($services[$serviceId]['type'] == 'imap')
			{
				...
				if ($error === false)
				{
					$unseen = \Thurly\Mail\Helper::getImapUnseen($mbData, 'inbox', $error, $errors);
			...
			if ($error === false)
			{
				if (self::$crmAvailable && $_REQUEST['crm_connect'] == 'Y')
				{
					...
					$imapDirs = \Thurly\Mail\Helper::listImapDirs($imapData, $error);
					if ($imapDirs !== false)
					{
						$error = false;
	...
	private static function executeCheck(&$error)
	{
		...
		if ($error === false)
		{
			...
			if (!empty($mailbox['OPTIONS']['flags']) && in_array('crm_connect', $mailbox['OPTIONS']['flags']))
			{
				...
				$unseen = \Thurly\Mail\Helper::getImapUnseen($imapData, 'inbox', $error);
				if ($unseen !== false)
					$error = false;
				else
					$unseen = -1;
	...
	private static function executeEnableCrm(&$error)
	{
		...
		if ($error === false)
		{
			...
			if ($error === false)
			{
				...
				$imapDirs = \Thurly\Mail\Helper::listImapDirs($imapData, $error, $errors);
				if ($imapDirs !== false)
				{
					$error = false;
	...
	private static function executeChangePassword(&$error)
	{
		...
		if ($error === false)
		{
			...
			else if ($mailbox['SERVER_TYPE'] == 'imap')
			{
				...
				$unseen = \Thurly\Mail\Helper::getImapUnseen($mailbox, 'inbox', $error, $errors);
