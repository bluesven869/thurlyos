...
foreach ($arParams['SERVICES'] as $id => $settings)
{
	...
	if ($settings['type'] != 'imap')
	{
		if ($settings['type'] == 'controller' && $settings['name'] == 'thurlyos')
		{
...
$isUserAdmin = $USER->isAdmin() || $USER->canDoOperation('thurlyos_config');

$showB24Block = \CIntranetMailConfigComponent::isFeatureAvailable('b24_service') >= 0;
$showDomainBlock = !empty($arParams['PATH_TO_MAIL_CFG_DOMAIN_COMPONENT']) && \CIntranetMailConfigComponent::isFeatureAvailable('domain_service') >= 0;

$b24Mailbox = $showB24Block && !empty($arParams['MAILBOX']) && $arParams['MAILBOX']['SERVICE_ID'] == $b24Settings['id'];
$domainMailbox = $showDomainBlock && !empty($arParams['MAILBOX']) && $arParams['MAILBOX']['SERVICE_ID'] == $domainSettings['id'];
$imapMailbox = !empty($arParams['MAILBOX']) && $arParams['MAILBOX']['SERVER_TYPE'] == 'imap';

$limitedLicense = false;
$limitedDomainBlock = \CIntranetMailConfigComponent::isFeatureAvailable('domain_service') < 1;

if (\CModule::includeModule('thurlyos'))
{
	$limitedLicense = !in_array(\CThurlyOS::getLicenseType(), array('company', 'nfr', 'edu', 'demo'));

	if ($limitedLicense || $limitedDomainBlock)
		\CThurlyOS::initLicenseInfoPopupJS();
}

$defaultBlock = 'imap';

if ($showB24Block && $b24Mailbox)
	$defaultBlock = 'thurlyos';
...
<div class="mail-set-wrap<? if ($showB24Block xor $showDomainBlock): ?> mail-set-wrap-small<? endif ?>"
	...
	<div class="mail-top-title-block"<? if (!$showB24Block && !$showDomainBlock): ?> style="margin-bottom: 20px; "<? endif ?>>
		...
		<? else: ?>
			<?=getMessage(isModuleInstalled('thurlyos') ? 'INTR_MAIL_HINT_B24_CRM' : 'INTR_MAIL_HINT_BOX_CRM') ?>
		<? endif ?>
	</div>
	<table class="mail-block-table" id="mail-block-table"<? if (!$showB24Block && !$showDomainBlock): ?> style="display: none; "<? endif ?>>
			...
			<td class="mail-block mail-block-imap<? if ($defaultBlock == 'imap'): ?> mail-block-active<? endif ?>" data-block="mail-set-third">
				...
				<div id="imap_block_descr_nomailbox" class="mail-block-text" style="<? if ($imapMailbox): ?>display: none; <? endif ?><? if (!$showB24Block && !$showDomainBlock): ?>min-height: 0; <? endif ?>">
					...
					<? if ($showB24Block || $showDomainBlock): ?>
						<img class="mail-block-icon-list" src="/thurly/components/thurly/intranet.mail.config.home/templates/.default/images/<?=getMessage('INTR_MAIL_IMAP_DESCR_ICONS') ?>"/>
						<img class="mail-block-icon-list-colorless" src="/thurly/components/thurly/intranet.mail.config.home/templates/.default/images/<?=getMessage('INTR_MAIL_IMAP_DESCR_ICONS_CL') ?>"/>
					<? endif ?>
				</div>
			</td>
			<? if ($showB24Block): ?>
				<td class="mail-block-space"></td>
				<td class="mail-block mail-block-b24<? if ($defaultBlock == 'thurlyos'): ?> mail-block-active<? endif ?>" data-block="mail-set-first">
					<div class="mail-block-title">
				...
				<td class="mail-block mail-block-own<? if ($defaultBlock == 'domain'): ?> mail-block-active<? endif ?>" data-block="mail-set-second">
					...
					<? if (empty($domainSettings) || !$isUserAdmin && !$domainAdded): ?>
						<div class="mail-block-text">
							<?=getMessage(isModuleInstalled('thurlyos') ? 'INTR_MAIL_DOMAIN_DESCR_B24' : 'INTR_MAIL_DOMAIN_DESCR_BOX') ?>
						</div>
					<? else: ?>
						<? if ($domainMailbox && $domainAdded): ?>
							<div id="domain_block_descr_mailbox" class="mail-block-text">
								<?=getMessage(isModuleInstalled('thurlyos') ? 'INTR_MAIL_DOMAIN_DESCR_B24_DOMAIN' : 'INTR_MAIL_DOMAIN_DESCR_BOX_DOMAIN', array('#DOMAIN#' => htmlspecialcharsbx($domainSettings['server']))) ?>
								<br><br>
						...
						<div id="domain_block_descr_nomailbox" class="mail-block-text"<? if ($domainMailbox && $domainAdded): ?> style="display: none; "<? endif ?>>
							<?=getMessage(
								isModuleInstalled('thurlyos') ? 'INTR_MAIL_DOMAIN_DESCR_B24_DOMAIN' : 'INTR_MAIL_DOMAIN_DESCR_BOX_DOMAIN',
		...
		<tr class="mail-block-bottom">
			...
			<? if ($showB24Block): ?>
				<td class="mail-block-space"></td>
				<td class="mail-block<? if ($defaultBlock == 'thurlyos'): ?> mail-block-active<? endif ?>"  data-block="mail-set-first">
					<div class="mail-block-footer">
	...
	<div class="mail-set-block-wrap" id="mail-set-block-wrap"<? if (!$showB24Block && !$showDomainBlock): ?> style="margin-top: 20px; "<? endif ?>>
		<div class="mail-set-block mail-set-block-active" id="mail-set-block">
			<? if ($showB24Block): ?>
				<div id="mail-set-first" class="mail-set-first-wrap"<? if ($defaultBlock == 'thurlyos'): ?> style="display: block;"<? endif ?>>
					...
					<div class="mail-set-second">
						...
						<? if ($isUserAdmin && empty($domainSettings)): ?>
									...
									var handleDomainForm = function(e)
									{
										...
										if (<? if (in_array(LANGUAGE_ID, array('ru', 'ua'))): ?>BX('select-delegate').checked<? else: ?>true<? endif ?>)
										{
											...
											if (form.elements['domain'].value.length > 0)
											{
												...
												BX.ajax({
													method: 'POST',
													url: '<?=CUtil::jsEscape($arParams['PATH_TO_MAIL_CFG_DOMAIN_COMPONENT']) ?>/ajax.php?siteid=<?=urlencode(SITE_ID) ?>&act=create',
													data: data,
													dataType: 'json',
													onsuccess: function(json)
													{
														...
														{
															window.location = '<?=CUtil::jsEscape($arParams['PATH_TO_MAIL_CFG_DOMAIN']) ?>#delegate';
														}
														else
														{
										...
										else if (BX('select-get').checked)
										{
											<? if ($arParams['REG_DOMAIN']): ?>
												...
												BX.ajax({
													method: 'POST',
													url: '<?=CUtil::jsEscape($arParams['PATH_TO_MAIL_CFG_DOMAIN_COMPONENT']) ?>/ajax.php?siteid=<?=urlencode(SITE_ID) ?>&act=get&domain=<?=CUtil::jsEscape($arParams['REG_DOMAIN']) ?>',
													data: '<?=thurly_sessid_get() ?>',
													dataType: 'json',
													onsuccess: function(json)
													{
														if (json.result == 'ok')
										...
										{
											window.location = '<?=CUtil::jsEscape($arParams['PATH_TO_MAIL_CFG_DOMAIN']) ?>#connect';
										}
							...
							<? else: ?>
								...
								<br><br><br>
								<div class="mail-set-footer">
									<a class="webform-button webform-button-accept" href="<?=$arParams['PATH_TO_MAIL_CFG_DOMAIN'] ?>">
...
<script type="text/javascript">
	...
	var setPost = {
		...
		show : function(ev)
		{
			...
			{
				if (<?=(empty($limitedDomainBlock) ? 'true' : "blockID != 'mail-set-second'") ?>)
			...
			else if(event.type == 'click')
			{
				...
				<? else: ?>
					...
					else
					{
						B24.licenseInfoPopup.show(
							'mail_domain_setup',
							'<?=CUtil::jsEscape(getMessage('INTR_MAIL_DOMAIN_TITLE')) ?>',
							'<?=CUtil::jsEscape(getMessage('INTR_MAIL_DOMAIN_LICENSE_DESCR')) ?>'
						);
					}
				<? endif ?>
			}
...
<script type="text/javascript">
	...
	(function() {
		...
		var enableCrm = function(data)
		{
			...
			BX.ajax({
				...
				onsuccess: function(json)
				{
					if (json.result != 'error')
					{
						window.location = '<?=CUtil::jsEscape($arParams['PATH_TO_MAIL_CONFIG']) ?>';
					}
					else
					{
						if (json.imap_dirs)
		...
		var disableCrm = function()
		{
			...
			BX.ajax({
				method: 'POST',
				url: '<?=$this->__component->getPath() ?>/ajax.php?siteid=<?=urlencode(SITE_ID) ?>&act=disablecrm',
				data: '<?=thurly_sessid_get() ?>',
				dataType: 'json',
				onsuccess: function(json)
				{
			...
			this.checkName = function(e)
			{
				...
				if (typeof cnResults[key] == 'undefined')
				{
					cnTimeout = setTimeout(function() {
						...
						cnAjax = BX.ajax({
							method: 'POST',
							url: '<?=$this->__component->getPath() ?>/ajax.php?siteid=<?=urlencode(SITE_ID) ?>&act=name',
							data: data,
							dataType: 'json',
			...
			this.enableCrm = function(data)
			{
				...
				BX.ajax({
					...
					onsuccess: function(json)
					{
						if (json.result != 'error')
						{
							window.location = '<?=CUtil::jsEscape($arParams['PATH_TO_MAIL_SUCCESS']) ?>';
						}
						else
						{
							if (json.imap_dirs)
				...
				var doSubmit = function()
				{
					...
					BX.ajax({
						...
						onsuccess: function(json) {
							if (json.result != 'error')
							{
								window.location = json.late_error
									? '<?=CUtil::jsEscape($arParams['PATH_TO_MAIL_CONFIG']) ?>'
									: '<?=CUtil::jsEscape($arParams['PATH_TO_MAIL_SUCCESS']) ?>';
							}
							else
							{
								<? if ($arParams['CRM_AVAILABLE']): ?>
		...
		function EditMailboxForm(createForm, setupForm, deleteForm, checkForm, disableCrmForm, enableCrmForm, configCrmForm, passwordForm, editForm)
		{
			...
			this.configCrm = function(formButton)
			{
				var configCrm = function(params)
				{
					...
					BX.ajax({
						method: 'POST',
						url: '<?=$this->__component->getPath() ?>/ajax.php?siteid=<?=urlencode(SITE_ID) ?>&act=configcrm',
						data: data,
						dataType: 'json',
						onsuccess: function(json)
						{
			...
			this.submitPassword = function()
			{
				...
				BX.ajax({
					method: 'POST',
					url: '<?=$this->__component->getPath() ?>/ajax.php?siteid=<?=urlencode(SITE_ID) ?>&act=password',
					data: data,
					dataType: 'json',
					onsuccess: function(json)
					{
