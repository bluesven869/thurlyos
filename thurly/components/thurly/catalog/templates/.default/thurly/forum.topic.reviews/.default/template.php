		...
		<?if ($arParams["SHOW_AVATAR"] == "Y") { ?>
			<div class="review-avatar">
				...
				if(isset($res["AVATAR"]["HTML"]) > 0)
					...
					echo '<img src="/thurly/components/thurly/forum.topic.reviews/templates/.default/images/noavatar.gif" border="0" />';
				?>
			</div>
		<? } ?>
...
<?
	...
	?><div class="reviews-message-img"><?
		?><?$GLOBALS["APPLICATION"]->IncludeComponent(
			"thurly:forum.interface", "show_file",
			Array(
				"FILE" => $arFile,
...
?>
		<div class="reviews-reply-field reviews-reply-field-captcha">
			...
			<div class="reviews-reply-field-captcha-image">
				<img src="/thurly/tools/captcha.php?captcha_code=<?=$arResult["CAPTCHA_CODE"]?>" alt="<?=GetMessage("F_CAPTCHA_TITLE")?>" />
			</div>
		</div>
<?
endif;
...
?>
			<div class="reviews-uploaded-file">
				...
				<label for="FILES_TO_UPLOAD_<?=$key?>"><?=$val["ORIGINAL_NAME"]?> (<?=$val["CONTENT_TYPE"]?>) <?=$sFileSize?>
					( <a href="/thurly/components/thurly/forum.interface/show_file.php?action=download&amp;fid=<?=$key?>"><?=GetMessage("F_DOWNLOAD")?></a> )
				</label>
			</div>
<?
	endforeach;
