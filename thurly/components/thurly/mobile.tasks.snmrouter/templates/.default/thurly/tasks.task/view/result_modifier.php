...
{
	$arResult["TEMPLATE_DATA"]["FILES_IN_COMMENTS"] = \Thurly\Tasks\Integration\Forum\Task\Comment::getFileCount($taskData["FORUM_TOPIC_ID"], $taskData["FORUM_ID"]);
}

$arResult['DATA']['TASK']['~DESCRIPTION'] = $arResult['DATA']['TASK']['DESCRIPTION'];
//$arResult['DATA']['TASK']['DESCRIPTION'] = \Thurly\Tasks\Util\UI::convertHtmlToSafeHtml($arResult['DATA']['TASK']['DESCRIPTION']);
if($arResult['DATA']['TASK']['DESCRIPTION_IN_BBCODE'] == 'Y')
{
	// convert to bbcode to html to show inside a document body
	$arResult['DATA']['TASK']['DESCRIPTION'] = \Thurly\Tasks\Util\UI::convertBBCodeToHtml($arResult['DATA']['TASK']['DESCRIPTION'], array(
