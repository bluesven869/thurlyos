...
\Thurly\Main\Page\Asset::getInstance()->addJs(SITE_TEMPLATE_PATH.'/log_mobile.js');
\Thurly\Main\Page\Asset::getInstance()->addJs($templateFolder.'/../.default/script.js');
...
<form name="<?=$arResult["FORM_ID"]?>" id="<?=$arResult["FORM_ID"]?>" action="<?=$url?>" method="POST">
	...
	<input type="hidden" name="back_url" value="<?=$url."&".http_build_query(array("save" => "Y", "sessid" => thurly_sessid())) ?>" />
						...
						"value" => "<div id=\"checkList".$task["ID"]."Container\" class=\"mobile-grid-field-tasks-checklist-container\">".(($checkList = function($list, &$ids)
							{
								...
								{
									$separator = \Thurly\Tasks\UI\Task\CheckList::checkIsSeparatorValue($item["TITLE"]);
									$ids[] = $item["ID"];
...
ob_start();
global $APPLICATION;
$APPLICATION->IncludeComponent("thurly:forum.comments", "", array(
