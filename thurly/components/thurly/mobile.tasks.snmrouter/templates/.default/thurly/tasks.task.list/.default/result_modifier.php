...
$arParams['GRID_ID'] = 'mobile_'.$arResult['GRID_ID'];
...
//dmp(array('$arResult["FIELDS"]' => $arResult["FIELDS"]));
...
foreach ($arResult["LIST"] as $task)
{
	...
	if ($task["ALLOW_TIME_TRACKING"] == "Y")
	{
		...
		$task["TIMETRACKING"] =
			...
			($timerTask['TASK_ID'] == $task['ID'] || $task["ACTION"]["START_TIME_TRACKING"]) ? "" : " disabled")." />".
	...
	$task["FAVORITES"] = "";
	if ($task["ACTION"]["FAVORITE.ADD"] || $task["ACTION"]["FAVORITE.DELETE"])
		$task["FAVORITES"] =
			'<label for="bx-task-favorites-'.$task["ID"].'" class="mobile-grid-field-favorites">'.
			'<input id="bx-task-favorites-'.$task["ID"].'" type="checkbox"'.($task["ACTION"]["FAVORITE.DELETE"] ? " checked=\"checked\"" : "") .' />'.
