...
if ($arParams['MEETING_ID'] > 0)
{
	...
	if ($arResult['ACCESS'])
	{
		if ($_REQUEST['DELETE'] == 'Y' && $arResult['ACCESS'] == CMeeting::ROLE_OWNER && check_thurly_sessid())
		{
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_REQUEST['save']) && $arResult['CAN_EDIT'] && check_thurly_sessid())
{
	...
	if ($bFromEditForm)
	{
		...
		$arFields['FILES'] = \Thurly\Main\UI\FileInputUtility::instance()->checkFiles(
			'MEETING_DESCRIPTION',
			$_REQUEST['FILES']
		);

		$deletedFileList = \Thurly\Main\UI\FileInputUtility::instance()->checkDeletedFiles('MEETING_DESCRIPTION');
				...
				if ($_REQUEST['AGENDA_TASK'][$key] && !$arResult['MEETING']['AGENDA'][$key]['TASK_ID'])
				{
					...
					if ($TASK_ID <= 0)
					{
						...
						$arTaskFields = array(
							...
							'TAGS' => array(),
							'STATUS' => array_key_exists($arFields['RESPONSIBLE'][0], $arEmplIDs) ? 2 : 1,
							'SITE_ID' => SITE_ID
						);
...
if ($_REQUEST['AGENDA_EX'] && check_thurly_sessid())
{
	$APPLICATION->RestartBuffer();
