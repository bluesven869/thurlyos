...
function checkMeetingRoom()
{
	if(!!document.forms.meeting_edit.PLACE_ID.value)
	{
		...
		BX.ajax.loadJSON('/thurly/tools/ajax_meeting.php', queryData, function(res){
...
<?
$APPLICATION->IncludeComponent('thurly:fileman.light_editor', '', array(
...
if (IsModuleInstalled('socialnetwork')):
	...
	$APPLICATION->IncludeComponent('thurly:socialnetwork.group.selector', '', array(
...
BX.ready(function() {
	BX.ajax.insertToNode(
		'<?=CUtil::JSEscape(str_replace('#MEETING_ID#', $arResult['MEETING']['PARENT_ID'], $arParams["MEETING_EDIT_URL_TPL"])).'?AGENDA_EX=Y&'.thurly_sessid_get()?>',
		BX('parent_agenda')
	);
});
</script>
