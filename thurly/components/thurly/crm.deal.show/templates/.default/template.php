		...
		function()
		{
			...
			var instantEditor = BX.CrmInstantEditor.create(
				'<?=CUtil::JSEscape($instantEditorID)?>',
				{
					...
					url: '/thurly/components/thurly/crm.deal.show/ajax.php?<?=thurly_sessid_get()?>',
			...
			if(BX(bpContainerId))
			{
				BX.CrmBizprocDispatcher.create(
					"<?=CUtil::JSEscape($bizprocDispatcherID)?>",
					{
						...
						entityTypeName: "<?=CCrmOwnerType::DealName?>",
						entityID: <?=$arResult['ELEMENT_ID']?>,
						serviceUrl: "/thurly/components/thurly/crm.deal.show/bizproc.php?deal_id=<?=$arResult['ELEMENT_ID']?>&post_form_uri=<?=urlencode($arResult['POST_FORM_URI'])?>&<?=thurly_sessid_get()?>",
						formID: "<?=CUtil::JSEscape($arResult['FORM_ID'])?>",
	...
	(function()
	{
		...
		BX.CrmFormTabLazyLoader.create(
			"<?=CUtil::JSEscape(strtolower($arResult['FORM_ID'])).'_livefeed'?>",
			{
				...
				serviceUrl: "/thurly/components/thurly/crm.entity.livefeed/lazyload.ajax.php?&site=<?=SITE_ID?>&<?=thurly_sessid_get()?>",
				formID: "<?=CUtil::JSEscape($arResult['FORM_ID'])?>",
				tabID: "tab_live_feed",
				params: params
			}
