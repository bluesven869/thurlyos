...
if(\Thurly\Main\Loader::includeModule('sale'))
{
	if(!empty($_REQUEST["search"]) && is_string($_REQUEST["search"]))
	{
		...
		$filter = \Thurly\Sale\SalesZone::makeSearchFilter("city", $arParams["siteId"]);
		...
		$filter = \Thurly\Sale\SalesZone::makeSearchFilter("region", $arParams["siteId"]);
		...
		$filter = \Thurly\Sale\SalesZone::makeSearchFilter("country", $arParams["siteId"]);
