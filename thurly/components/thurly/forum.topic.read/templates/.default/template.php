...
<?
	...
	if ($USER->IsAuthorized())
	{
		if (empty($arResult["USER"]["SUBSCRIBE"]))
		{
			...
			?><?=$APPLICATION->GetCurPageParam("TOPIC_SUBSCRIBE=Y&".thurly_sessid_get(), array("FORUM_SUBSCRIBE", "FORUM_SUBSCRIBE_TOPIC", "sessid"))?><?
		...
		else
		{
			...
			?><?=$APPLICATION->GetCurPageParam("TOPIC_UNSUBSCRIBE=Y&".thurly_sessid_get(), array("FORUM_UNSUBSCRIBE", "FORUM_UNSUBSCRIBE_TOPIC", "sessid"))?><?
...
function ShowFirstPost(oA)
{
	...
	BX.ajax.get(
			'/thurly/components/thurly/forum/templates/.default/user_settings.php',
			{'save': 'first_post', 'value' :(div.style.display == 'none' ? 'hide' : 'show'), 'sessid': '<?=thurly_sessid()?>'}
	);
	return false;
}
<?
