...
if ((strlen($action) > 0 || strlen($s_action) > 0) && ($_REQUEST["MESSAGE_MODE"] != "VIEW") && check_thurly_sessid())
{
	//*************************!Subscribe***************************************************
	...
	if (strlen($action) > 0 && $action != "SUBSCRIBE" && $action != "UNSUBSCRIBE" )
	{
		...
		switch ($action)
		{
			...
			case "FORUM_MESSAGE2SUPPORT":
				...
				{
					$url = "/thurly/admin/ticket_edit.php?ID=#result#&amp;lang=".LANGUAGE_ID;
				}
				break;
			case "DEL":
				$arFields = array("MID" => $message);
...
elseif ((strlen($action) > 0) && ($_REQUEST["MESSAGE_MODE"] != "VIEW") && !check_thurly_sessid())
{
	$bVarsFromForm = true;
