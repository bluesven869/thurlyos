...
if (check_thurly_sessid())
{
	...
	elseif ($action == 'FAVORITES')
	{
		...
		try
		{
			$res = \Thurly\Socialnetwork\Item\WorkgroupFavorites::set(array(
				'GROUP_ID' => $groupId,
				'USER_ID' => $USER->getId(),
				'VALUE' => $value
			));
	...
	elseif ($action == 'FAVORITES')
	{
		$groupItem = \Thurly\Socialnetwork\Item\Workgroup::getById($group["ID"]);
		...
		$groupSiteList = array();
		$resSite = \Thurly\Socialnetwork\WorkgroupSiteTable::getList(array(
			'filter' => array(
				'=GROUP_ID' => $group["ID"]
			),
