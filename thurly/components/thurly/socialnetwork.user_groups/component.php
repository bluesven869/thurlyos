...
if ($arResult["USE_UI_FILTER"])
{
	...
	$filterOption = new \Thurly\Main\UI\Filter\Options(
		$arParams["FILTER_ID"],
		Thurly\Socialnetwork\Integration\Main\UIFilter\Workgroup::getFilterPresetList(array(
...
if (StrLen($arResult["FatalError"]) <= 0)
{
	...
	$nav = new \Thurly\Main\UI\PageNavigation($arResult["NAV_ID"]);
	...
	else
	{
		...
		)
		{
			...
			if (!empty($arResult["filter_name"]))
			{
				if ($arResult["USE_UI_FILTER"])
				{
					$operation = \Thurly\Socialnetwork\WorkgroupTable::getEntity()->fullTextIndexEnabled("SEARCH_INDEX") ? '*' : '*%';
					$arGroupFilter[$operation."SEARCH_INDEX"] = \Thurly\Socialnetwork\Item\Workgroup::prepareToken($arResult["filter_name"]);
				}
				else
				{
					$arGroupFilter[] = array(
			...
			{
				if (isset($arResult["filter_archive"])) // thurlyos ui.filter
				{
		...
		if (!$bNoMyGroups)
		{
			...
			$arFilterTmp = array();
			$dbGroupTmp = \Thurly\Socialnetwork\WorkgroupTable::getList(array(
				'order' => array(
					'ID' => 'ASC'
				),
			...
			if (!empty($arFilterTmp))
			{
							...
							$query->registerRuntimeField(
								...
								new \Thurly\Main\Entity\ReferenceField('UG',
									...
									array(
										...
										'=ref.USER_ID' =>  new \Thurly\Main\DB\SqlExpression($USER->getId())
									),
				...
				)
				{
					$query->registerRuntimeField(
						'',
						new \Thurly\Main\Entity\ReferenceField('GF',
							\Thurly\Socialnetwork\WorkgroupFavoritesTable::getEntity(),
							array(
								'=ref.GROUP_ID' => 'this.ID'
							),
			...
			if ($dbGroup)
			{
				while ($arGroup = $dbGroup->fetch())
				{
					...
					$arImage = CSocNetTools::InitImage($arGroup["IMAGE_ID"], 150, "/thurly/images/socialnetwork/nopic_group_150.gif", 150, $pu, true);
	...
	)
	{
		...
		if (!empty($groupIdList))
		{
			...
			$res = \Thurly\Socialnetwork\WorkgroupFavoritesTable::getList(array(
				'filter' => array(
	...
	)
	{
		...
		$INTRANET_TOOLBAR->AddButton(array(
			'HREF' => "/thurly/admin/socnet_subject.php?lang=".LANGUAGE_ID,
