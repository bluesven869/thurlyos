...
use Thurly\Tasks\Dispatcher;
use Thurly\Tasks\Util\Calendar;
use Thurly\Tasks\Util;

Loc::loadMessages(__FILE__);

abstract class TasksBaseComponent extends CThurlyComponent
{
	///////////////////////////////////
	 ...
	 * @return \Thurly\Tasks\Util\Result
	 */
	...
	protected static function outputJSONResponce($result)
	{
		...
		print(\Thurly\Tasks\UI::toJSON($result));
	}
	...
	protected function getAuxData()
	{
		$this->arResult['AUX_DATA']['USER'] = array(
			'IS_SUPER' => \Thurly\Tasks\Util\User::isSuper($this->userId)
		);
	}
	...
	protected static function checkPermissions(array &$arParams, array &$arResult, Collection $errors, array $auxParams = array())
	{
		...
		$arResult['COMPONENT_DATA']['MODULES']['thurlyos'] = \Thurly\Main\ModuleManager::isModuleInstalled('thurlyos');
	...
	{
		return \Thurly\Tasks\Util\User::getId();
	}

	protected static function checkUserRestrictions($userId, Collection $errors)
	{
		if($userId == \Thurly\Tasks\Util\User::getId()) // if the effective user equals to the current user, check if authorized
		{
			if(!\Thurly\Tasks\Util\User::get()->isAuthorized())
			{
	...
	{
		return \Thurly\Tasks\Integration\SocialNetwork\Group::getData($groupIds);
	}
