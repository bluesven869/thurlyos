...
global $APPLICATION;
$cUserId = \Thurly\Tasks\Util\User::getId();
...
if (($arResult["ACTION"] == "edit" || $arResult["ACTION"] == "delete") && intval($arParams["TEMPLATE_ID"])) // check access rights and get data (better to rewrite the condition to: if(intval($arParams["TEMPLATE_ID"])) )
{
	...
	{
		$arData["ACCOMPLICES"] = \Thurly\Tasks\Util\Type::unSerializeArray($arData["~ACCOMPLICES"]);
		$arData["AUDITORS"] = \Thurly\Tasks\Util\Type::unSerializeArray($arData["~AUDITORS"]);
		$arData["RESPONSIBLES"] = \Thurly\Tasks\Util\Type::unSerializeArray($arData["~RESPONSIBLES"]);
		$arData["FILES"] = $arData["~FILES"] ? unserialize($arData["~FILES"]) : array();
		$arData["TAGS"] = $arData["~TAGS"] ? unserialize($arData["~TAGS"]) : "";
		$arData["DEPENDS_ON"] = Thurly\Tasks\Util\Type::unSerializeArray($arData["~DEPENDS_ON"]);
		...
		if(!empty($users))
		{
			$res = \Thurly\Main\UserTable::getList(array('filter' => array('ID' => $users), 'select' => array('ID')));
			$users = array();
			while($item = $res->fetch())
			{
				$users[$item['ID']] = true;
...
$arResult["USER_FIELDS"] = \Thurly\Tasks\Util\UserField\Task\Template::getScheme($arParams["TEMPLATE_ID"]);

//Form submitted
if($_SERVER["REQUEST_METHOD"] == "POST" && check_thurly_sessid() && ($arResult["ACTION"] == "create" || $arResult["ACTION"] == "edit") && !isset($_POST['controller_id']))
{
	...
	if(isset($_POST["save"]) || isset($_POST["apply"]))
	{
		...
		if (sizeof($arResult["ERRORS"]) == 0)
		{
			...
			try
			{
				\Thurly\Tasks\Internals\Task\Template\CheckListTable::updateForTemplate($templateID, $checkList2Store);
			}
...
elseif(check_thurly_sessid() && $arResult["ACTION"] == "delete") // came from external widget with GET
{
...
if($arData['TPARAM_TYPE'] != CTaskTemplates::TYPE_FOR_NEW_USER) // if this is not a "system" template (template that is used for newly-created user)
{
	if (!$arData["RESPONSIBLE_ID"])
	{
		if (($arData["CREATED_BY"] && $arData["CREATED_BY"] != $cUserId))
		{
			...
			$cUser = \Thurly\Tasks\Util\User::get();
			if($cUser)
			{
