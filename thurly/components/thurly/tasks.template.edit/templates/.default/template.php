...
//$GLOBALS['APPLICATION']->AddHeadScript("/thurly/components/thurly/tasks.task.edit/templates/.default/script.js");
	...
	<div class="webform task-webform">
		...
		<div class="webform-round-corners webform-additional-fields task-additional-fields">
			<div class="webform-content">
				...
				<?$diskUf = \Thurly\Tasks\Integration\Disk\UserField::getMainSysUFCode();?>

				<?if(\Thurly\Tasks\Integration\Disk::isInstalled()):?>

					<?foreach($arResult["USER_FIELDS"] as $field):?>
						<?if($field['FIELD_NAME'] == \Thurly\Tasks\Integration\Disk\UserField::getMainSysUFCode()):?>
							<?
							$APPLICATION->IncludeComponent(
							...
							<ol class="webform-field-upload-list" id="webform-field-upload-list">
								<?php if ($arData["FILES"]):?>
									...
									<?php while($file = $resFiles->GetNext()):?>
										<li class="saved"><a href="/thurly/components/thurly/tasks.task.detail/show_file.php?fid=<?php echo $file["ID"]?>&tid=<?php echo $arParams["TEMPLATE_ID"]?>" target="_blank" class="upload-file-name"><?php echo $file["FILE_NAME"]?></a><i></i><a href="" class="delete-file"></a><input type="hidden" name="FILES[]" value="<?php echo $file["ID"]?>" /></li>
									<?php endwhile?>
								<?php endif?>
							</ol>
									...
									<table cellspacing="0" class="task-to-tasks-layout">
										<tr class="task-template-relations">
											...
											<td class="task-base-template">
												<?
													$APPLICATION->IncludeComponent(
														"thurly:tasks.template.selector",
														"",
														array(
															"MULTIPLE" => "N",
															"NAME" => "BASE_TEMPLATE",
									...
									<div class="task-repeating<?php echo ($arData["REPLICATE"] == "Y" ? " selected" : "")?>" id="task-repeating">
										...
										<div class="task-repeating-timespan-details" id="task-repeating-timespan-details">
											<div class="task-repeating-timespan-details-inner">
												...
												<?if(!intval($arData['ID'])):?>
													<?$arData['REPLICATE_TIMEZONE_OFFSET'] = \Thurly\Tasks\Util\User::getTimeZoneOffset();?>
												<?endif?>
												...
												<div class="task-repeating-time">
												    ...
												    <span id="template-replication-timepicker">
										                ...
										                <div class="task-main-clock-monkeyfix">
											                <?$GLOBALS['APPLICATION']->IncludeComponent('thurly:main.clock', '', array(
								...
								<?foreach ($arResult["USER_FIELDS"] as $arUserField):?>
									...
									<tr>
										...
										<td class="task-property-value">
											<?\Thurly\Tasks\Util\UserField\UI::showEdit($arUserField);?>
										</td>
									</tr>
								<?php endforeach ?>
							</table>
