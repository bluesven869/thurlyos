var CrmWebFormList = function(params)
{
	...
	this.initSlider = function()
	{
		...
		this.slider.init({
			'condition': [this.detailPageUrlTemplate.replace('#id#', '(\\d+)').replace('#form_id#', '(\\d+)')],
	...
	this.slider = {
		...
		{
			if (!BX.ThurlyOS || !BX.ThurlyOS.PageSlider)
			{
				return;
			}
		...
		{
			e.preventDefault();
			BX.ThurlyOS.PageSlider.open(this.getAttribute('href'));
		},
		open: function (url, reloadAfterClosing)
		{
			...
			BX.ThurlyOS.PageSlider.open(url);
			if (reloadAfterClosing)
			{
				BX.addCustomEvent(
					BX.ThurlyOS.PageSlider.iframe.contentWindow,
					"BX.ThurlyOS.PageSlider:onClose",
					function () {
...
function CrmWebFormListItem(params)
{
	...
	this.nodeAdsButton = this.node.querySelector('[' + this.attributeAds + ']');
...
CrmWebFormListItem.prototype =
{
		...
		if(!this.popupSettings)
		{
			...
			for(var code in actionList)
			{
				...
				switch(popupItemType)
				{
					...
					case 'adsSend':
						popupItem.onclick = BX.proxy(function() {
							this.showAdsSend(item.adsType, item.text);
							this.popupSettings.close();
						}, this);
						break;
				}
	...
	{
		if (!this.caller.adsConfig.canEdit)
		{
			return;
		}
		...
		this.sendActionRequest('showAdsSend', function(data){
	...
	createAdsPopup: function (data)
	{
		...
		var templateNode = BX('crm-webform-list-template-ads-popup');

		data = data || {};
		this.scriptPopup = BX.PopupWindowManager.create(
			'crm_webform_list_ads_popup',
			null,
			{
				titleBar: data.title || this.caller.mess.dlgGetScriptTitle,
				contentColor: 'white',
	...
	bindControls: function ()
	{
		...
		BX.bind(this.nodeAdsButton, 'click', BX.proxy(function() {
			this.showAdsSend(
				this.nodeAdsButton.getAttribute(this.attributeAds),
				this.nodeAdsButton.getAttribute(this.attributeAdsName)
			);
		}, this));
	},
