...
function __UFEExcludeFromFriends(popup)
{
	if(confirm(BX.message('UFEExcludeFromFriendsConfirmTitle'))) 
	{
		if (actionUsers['Friends'].length > 0)
		{
			__UFEShowWait();
			BX.ajax({
				url: '/thurly/components/thurly/socialnetwork.user_friends.ex/ajax.php',
				method: 'POST',
				dataType: 'json',
				data: {'ACTION': 'EX', 'USER_ID': parseInt(BX.message('UFEUserId')), 'FRIEND_ID' : actionUsers['Friends'], 'sessid': BX.thurly_sessid(), 'site': BX.util.urlencode(BX.message('UFESiteId'))},
...
function __UFEBan(popup)
{
	if (actionUsers['Friends'].length > 0)
	{
		...
		BX.ajax({
			...
			method: 'POST',
			dataType: 'json',
			data: {'ACTION': 'BAN', 'USER_ID': parseInt(BX.message('UFEUserId')), 'FRIEND_ID' : actionUsers['Friends'], 'sessid': BX.thurly_sessid(), 'site': BX.util.urlencode(BX.message('UFESiteId'))},
			onsuccess: function(data) { __UFEProcessAJAXResponse(data, popup); }
		});
	}
	else
...
function __UFEUnBan(popup)
{
	if (actionUsers['Banned'].length > 0)
	{
		...
		BX.ajax({
			...
			method: 'POST',
			dataType: 'json',
			data: {'ACTION': 'UNBAN', 'USER_ID': parseInt(BX.message('UFEUserId')), 'FRIEND_ID' : actionUsers['Banned'], 'sessid': BX.thurly_sessid(), 'site': BX.util.urlencode(BX.message('UFESiteId'))},
			onsuccess: function(data) { __UFEProcessAJAXResponse(data, popup); }
		});
	}
	else
