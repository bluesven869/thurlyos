...
$cacheID = md5(serialize($arParams["COMMON_FILTER"]).\Thurly\Tasks\Util\User::getId()."user".$arParams["USER_ID"].$arParams['CHECK_TASK_IN']);
...
if($arParams['CHECK_TASK_IN'] == 'R' || $arParams['CHECK_TASK_IN'] == 'A')
{
	foreach($preDefinedFilterRole as $key=>$f)
	{
		...
		if (!array_key_exists($statusFilter, $sf))
		{
			$statusFilter = 0;
		}
