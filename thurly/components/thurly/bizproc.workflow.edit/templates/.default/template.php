...
Asset::getInstance()->addCss("/thurly/themes/.default/pubstyles.css");
Asset::getInstance()->addCss("/thurly/themes/.default/jspopup.css");
Asset::getInstance()->addCss("/thurly/themes/.default/calendar.css");
Asset::getInstance()->addJs('/thurly/js/main/utils.js');
Asset::getInstance()->addJs('/thurly/js/main/popup_menu.js');
Asset::getInstance()->addJs('/thurly/js/main/admin_tools.js');
CUtil::InitJSCore(array("window", "ajax", 'bp_selector'));
Asset::getInstance()->addJs('/thurly/js/main/public_tools.js');
Asset::getInstance()->addJs('/thurly/js/bizproc/bizproc.js');
...
function BCPProcessExport()
{
	...
	window.open('<?=CUtil::JSEscape($v)?><?if(strpos($v, "?")):?>&<?else:?>?<?endif?>export_template=Y&<?=thurly_sessid_get()?>');
}
...
function BCPSaveUserParams()
{
	...
	jsExtLoader.startPost('<?= CUtil::JSEscape($v) ?><?if(strpos($v, "?")):?>&<?else:?>?<?endif?><?=thurly_sessid_get()?>&saveajax=Y&saveuserparams=Y', data);
}
...
function BCPSaveTemplate(save)
{
	...
	jsExtLoader.onajaxfinish = BCPSaveTemplateComplete;
	jsExtLoader.startPost('<?=CUtil::JSEscape($v)?><?if(strpos($v, "?")):?>&<?else:?>?<?endif?><?=thurly_sessid_get()?>&saveajax=Y'+
		(save ? '': '&apply=Y'),
		data);
}
