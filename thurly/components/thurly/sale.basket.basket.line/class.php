...
class SaleBasketLineComponent extends CThurlyComponent
{
	...
	public function executeComponent()
	{
		...
		{
			$currentPage = strtolower(\Thurly\Main\Context::getCurrent()->getRequest()->getRequestedPage());
		...
		$this->bUseCatalog = \Thurly\Main\Loader::includeModule('catalog');
		...
		else
		{
			...
			{
				$this->arResult["TOTAL_PRICE"] = \Thurly\Sale\BasketComponentHelper::getFUserBasketPrice($this->currentFuser, SITE_ID);
			}

			$this->arResult["NUM_PRODUCTS"] = \Thurly\Sale\BasketComponentHelper::getFUserBasketQuantity($this->currentFuser, SITE_ID);
		}
	 ...
	 *
	 * @return \Thurly\Main\EventResult
	 */
	public function onSaleBasketItemEntitySaved(\Thurly\Main\Event $event)
	{
		return \Thurly\Sale\BasketComponentHelper::onSaleBasketItemEntitySaved($event);
	}
	 ...
	 * @return \Thurly\Main\EventResult
	 */
	public function onSaleBasketItemDeleted(\Thurly\Main\Event $event)
	{
		return \Thurly\Sale\BasketComponentHelper::onSaleBasketItemDeleted($event);
	}

	/**
	 * @param \Thurly\Sale\Basket $basket
	 *
	 * @return float
	 */
	protected static function getActualBasketPrice(\Thurly\Sale\Basket $basket)
	{
		$basketPrice = 0;

		/** @var \Thurly\Sale\Basket $basketOrderable */
	 ...
	 * @return float
	 */
	protected static function getActualBasketQuantity(\Thurly\Sale\Basket $basket)
	{
		$basketQuantity = 0;
