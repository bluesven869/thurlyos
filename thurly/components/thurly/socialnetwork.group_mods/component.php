	...
	else
	{
		...
		else
		{
			...
			if ($_SERVER["REQUEST_METHOD"]=="POST" && $arResult["CurrentUserPerms"]["UserCanModifyGroup"] 
				&& strlen($_POST["save"]) > 0 && check_thurly_sessid())
			{
				$errorMessage = "";
			...
			if ($dbRequests)
			{
				...
				while ($arRequests = $dbRequests->GetNext())
				{
					...
					if (intval($arParams["THUMBNAIL_LIST_SIZE"]) > 0)
					{
						...
						$arImage = CSocNetTools::InitImage($arRequests["USER_PERSONAL_PHOTO"], $arParams["THUMBNAIL_LIST_SIZE"], "/thurly/images/socialnetwork/nopic_30x30.gif", 30, $pu, $canViewProfile);			
					}
					else // old 				
					$arImage = CSocNetTools::InitImage($arRequests["USER_PERSONAL_PHOTO"], 150, "/thurly/images/socialnetwork/nopic_user_150.gif", 150, $pu, $canViewProfile);
			...
			if ($arResult["CurrentUserPerms"]["UserCanModifyGroup"])
			{
				$js = '/thurly/js/main/popup_menu.js';
				$GLOBALS['APPLICATION']->AddHeadString('<script type="text/javascript" src="'.$js.'?v='.filemtime($_SERVER['DOCUMENT_ROOT'].$js).'"></script>');
				$GLOBALS["APPLICATION"]->AddHeadString('<link rel="stylesheet" type="text/css" href="/thurly/themes/.default/pubstyles.css" />');
			}
		}
	}
}
