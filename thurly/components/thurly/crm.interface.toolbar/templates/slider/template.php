...
/** @var array $arParams */
global $APPLICATION;
CJSCore::RegisterExt('popup_menu', array('js' => array('/thurly/js/main/popup_menu.js')));

Thurly\Main\Page\Asset::getInstance()->addCss('/thurly/components/thurly/crm.interface.toolbar/templates/slider/style.css');
...
{
	?>
	<div class="webform-small-button webform-small-button-transparent task-list-toolbar-lightning webform-cogwheel">
					...
					BX.CrmParamBag.create(
						{
							...
							"moreButtonClassName": "webform-cogwheel"
						}
					)
				);
			}
...
foreach($items as $item)
{
	...
	elseif($type == 'toolbar-conv-scheme')
	{
		...
		{
			Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/crm/crm.js');
		}
		?><span class="webform-small-button-separate-wrap webform-small-button-transparent webform-small-button crm-add-button-wrapper">
			<span id="<?=htmlspecialcharsbx($labelID);?>" class="webform-small-button webform-small-button-blue">
				<?=htmlspecialcharsbx($schemeDescr)?>
			</span>
			<span id="<?=htmlspecialcharsbx($buttonID);?>" class="webform-small-button-right-part"></span>
		</span>
		<script type="text/javascript">
			BX.ready(
				function()
		...
		<script type="text/javascript">
			BX.ready(
				function()
				{
					...
					if (button)
					{
						var config = <?=\Thurly\Main\Web\Json::encode($item['DATA'])?>;
