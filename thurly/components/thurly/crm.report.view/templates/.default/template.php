...
<div id="report-chfilter-examples-custom" style="display: none;">
	...
	<div class="filter-field filter-field-company chfilter-field-\Thurly\Crm\Company" callback="crmCompanySelector">
	...
	<div class="filter-field filter-field-contact chfilter-field-\Thurly\Crm\Contact" callback="crmContactSelector">
		<label for="%ID%" class="filter-field-title">%TITLE% "%COMPARE%"</label>
		<span class="webform-field-textbox-inner">
			<input id="%ID%" type="text" class="webform-field-textbox" caller="true" />
			<input type="hidden" name="%NAME%" value=""/>
	...
	<div class="filter-field filter-field-eventType chfilter-field-CATEGORY_ID" callback="RTFilter_chooseBoolean">
		...
		<select id="%ID%" name="%NAME%" class="filter-dropdown" caller="true">
			<option value=""><?=GetMessage('CRM_REPORT_INCLUDE_ALL')?></option><?
			$categories = \Thurly\Crm\Category\DealCategory::getSelectListItems();
	...
	<div class="filter-field filter-field-eventType chfilter-field-STAGE_ID" callback="RTFilter_chooseBoolean">
		...
		<select id="%ID%" name="%NAME%" class="filter-dropdown" caller="true">
			<option value=""><?=GetMessage('CRM_REPORT_INCLUDE_ALL')?></option><?
			$categoryGroups = \Thurly\Crm\Category\DealCategory::getStageGroupInfos();
	...
	BX.ready(function()
	{
		window.setTimeout(
			function()
			{
				...
				// Deal
				i = 0; temp = []; deal = [];
				//temp[i++] = BX.findChildren(BX('report-rewrite-filter'), { 'class':'chfilter-field-DEAL_BY' }, true);
				temp[i++] = BX.findChildren(BX('report-rewrite-filter'), { 'class':'chfilter-field-INVOICE_UTS.DEAL_BY' }, true);
				...
				// Company
				i = 0; temp = []; company = [];
				temp[i++] = BX.findChildren(BX('report-rewrite-filter'), { 'class':'chfilter-field-\\Thurly\\Crm\\Company' }, true);
				...
				// Contact
				i = 0; temp = []; contact = [];
				temp[i++] = BX.findChildren(BX('report-rewrite-filter'), { 'class':'chfilter-field-\\Thurly\\Crm\\Contact' }, true);
