...
class TasksInterfaceFilterComponent extends TasksBaseComponent
{
	...
	protected function doPreAction()
	{
		...
		$this->arResult['USER_ID'] = \Thurly\Tasks\Util\User::getId();
		...
		if ($this->arParams['USE_GROUP_SELECTOR'] == 'Y')
		{
			...
			if (\Thurly\Main\Loader::includeModule('socialnetwork'))
			{
				...
				$res = \Thurly\Socialnetwork\WorkgroupViewTable::getList(
					array(
						'select' => array(
							'GROUP_ID',
			...
			if ($selectGroup !== null)
			{
				...
				if ($selectGroup > 0)
				{
					$path = \Thurly\Main\Config\Option::get('tasks', 'paths_task_group', null, SITE_ID);
