		...
		<?if($arResult["CAPTCHA_CODE"]):?>
			<tr>
				...
				<span id="popup_captcha_img">
					<img src="/thurly/tools/captcha.php?captcha_sid=<?echo $arResult["CAPTCHA_CODE"]?>" width="180" height="40" alt="CAPTCHA" /></td>
				</span>
			</tr>
			<tr>
			...
			events : {
				click : function() {
					...
					if (error == 'N')
					{
						...
						BX.ajax.post('/thurly/components/thurly/sale.notice.product/ajax.php', data, function(res) {
							BX.closeWait();
							...
							if (rs['ERRORS'].length > 0)
							{
								...
								if (useCaptha == 'Y')
								{
									BX.ajax.get('/thurly/components/thurly/sale.notice.product/ajax.php?reloadcaptha=Y', '', function(res) {
										BX('popup_captcha_sid').value = res;
										BX('popup_captcha_img').innerHTML = '<img src="/thurly/tools/captcha.php?captcha_sid='+res+'" width="180" height="40" alt="CAPTCHA" />';
									});
								}
							}
							else if (rs['STATUS'] == 'Y')
