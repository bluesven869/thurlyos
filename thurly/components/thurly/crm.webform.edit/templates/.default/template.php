...
\Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/main/core/core_dragdrop.js');
...
<div class="crm-webform-edit-wrapper">
	...
	<div id="CRM_WEBFORM_ADS_FORM">
		<?foreach ($arResult['ADS_FORM'] as $adsForm):?>
			<div>
				<span data-bx-ads-button="<?=htmlspecialcharsbx($adsForm['TYPE'])?>" class="webform-small-button webform-small-button-transparent">
	...
	<div class="crm-webform-edit-button-container">
		<?if($arResult['PERM_CAN_EDIT']):?>
			<?if($arResult['FORM']['IS_READONLY'] != 'Y'):?>
				<?if($arParams['IFRAME']):?>
					<button id="CRM_WEBFORM_SUBMIT_APPLY" name="submit_apply" value="Y" type="submit" class="webform-small-button webform-small-button-accept crm-webform-edit-button-save"><?=Loc::getMessage('CRM_WEBFORM_EDIT_BUTTON_APPLY')?></button>
				<?else:?>
			...
			<?endif;?>
		<?endif;?>
		<a id="CRM_WEBFORM_EDIT_TO_LIST_BOTTOM" href="<?=htmlspecialcharsbx($arResult['PATH_TO_WEB_FORM_LIST'])?>" class="webform-small-button webform-small-button-transparent crm-webform-edit-button-cancel"><?=Loc::getMessage('CRM_WEBFORM_EDIT_BACK_TO_LIST')?></a>
	</div>
</div>
