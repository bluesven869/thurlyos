...
use Thurly\Crm\WebForm\FieldSynchronizer;
use Thurly\Crm\WebForm\Entity;
use Thurly\Crm\WebForm\ResultEntity;
use Thurly\Crm\WebForm\EntityFieldProvider;
use Thurly\Crm\Category\DealCategory;
use Thurly\Crm\WebForm\ReCaptcha;
use Thurly\Crm\WebForm\ResponsibleQueue;
...
class CCrmWebFormEditComponent extends \CThurlyComponent
{
	protected $errors = array();

	/** @var  Form */
	protected $crmWebForm;
	protected $isItemWasAdded = false;
	...
	protected function processPostDependencies($dependencies)
	{
		...
		foreach($dependencies as $dependencyId => $dependency)
		{
			$dependencyList[] = array(
				...
				'IF_ACTION' => \Thurly\Crm\WebForm\Internals\FieldDependenceTable::ACTION_ENUM_CHANGE,
	...
	public function processPost()
	{
		...
		if (!$this->crmWebForm->getId())
		{
			$this->isItemWasAdded = true;
		}
	...
	protected function redirectTo($toList = true)
	{
		...
		if (!$toList && $this->arParams['IFRAME'])
		{
			$uri = new \Thurly\Main\Web\Uri($url);
			$urlAdd = array('IFRAME' => 'Y');
			if ($this->isItemWasAdded)
			{
				$urlAdd['itemWasAdded'] = 'Y';
			}
	...
	protected function setResultCallBackForm()
	{
		...
		$callbackNumbers = \Thurly\Crm\WebForm\Callback::getPhoneNumbers();
	...
	public function setResultAssignedBy()
	{
		...
		foreach ($list as $item)
		{
			$userFields = \Thurly\Main\UserTable::getRowById($item);
			if (!$userFields)
			{
				continue;
			}
	...
	public function prepareResult()
	{
		...
		$request = \Thurly\Main\Context::getCurrent()->getRequest();
		if($request->getRequestMethod() == "POST" && check_thurly_sessid())
		{
		...
		$this->arResult['IS_ITEM_WAS_ADDED'] = $this->request->get('itemWasAdded') == 'Y';
	}
