...
if ($arParams["FILTER_MY"] != "Y")
{
	if ($this->StartResultCache(false, false))
	{
		...
		while ($arGroup = $dbGroups->GetNext())
		{
			...
			$arImage = CSocNetTools::InitImage($arGroup["IMAGE_ID"], 50, "/thurly/images/socialnetwork/nopic_group_50.gif", 50, $arGroup["GROUP_URL"], true);
...
else
{
	...
	while ($arGroup = $dbGroups->GetNext())
	{
		...
		$arImage = CSocNetTools::InitImage($arGroup["GROUP_IMAGE_ID"], 50, "/thurly/images/socialnetwork/nopic_group_50.gif", 50, $arGroup["GROUP_URL"], true);
		$arGroup["IMAGE_FILE"] = $arImage["FILE"];
		$arGroup["IMAGE_IMG"] = $arImage["IMG"];
