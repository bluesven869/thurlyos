...
if(typeof BX.Crm.EntityDetailProgressControl === "undefined")
{
	...
	BX.Crm.EntityDetailProgressControl.prototype =
	{
		initialize: function(id, settings)
		{
			...
			if(this._entityTypeId === BX.CrmEntityType.enumeration.dealrecurring)
			{
			...
			for(var i = 0, l = this._stepInfos.length; i < l; i++)
			{
				...
				if (stepContainerText.scrollWidth > (stepContainer.clientWidth + 10)) {
					...
					stepContainer.style.maxWidth = stepContainerText.scrollWidth + "px"
				}
		...
		{
			for(var i = index, l = this._steps.length; i < l; i++)
			{
		...
		setCurrentStep: function(stepInfo)
		{
		...
		processStepSelect: function(step)
		{
			...
			{
				this.adjustSteps(step.getIndex(), step.getBackgroundColor());
				if(this._currentStepId !== stepInfo["id"])
				{
					this.setCurrentStep(stepInfo);
					this.save();
				}
