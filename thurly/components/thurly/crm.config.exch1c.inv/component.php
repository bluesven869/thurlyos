...
if($_SERVER['REQUEST_METHOD'] == "POST" && $_POST['save'] != "" && check_thurly_sessid())
{
	...
	$personTypes = \Thurly\Sale\BusinessValue::getPersonTypes();
	foreach ($personTypes as $personTypeId => $personType)
	{
		CSaleExport::deleteREKV($personType['ID']);
		CSaleExport::Delete($personType['ID']);
	}
	...
	foreach ($arPersonTypes as $personType => $personTypeId)
	{
		...
		if(IntVal($arExportProfile[$personTypeId])>0)
			$res = CSaleExport::Update($arExportProfile[$personTypeId], Array("PERSON_TYPE_ID" => $personTypeId, "VARS" => serialize($arParams)));
		else
			$res = CSaleExport::Add(Array("PERSON_TYPE_ID" => $personTypeId, "VARS" => serialize($arParams)));
	}
	// </editor-fold>
...
$dbOrderProps = CSaleOrderProps::GetList(
	...
	false,
	false,
	array("ID", "PERSON_TYPE_ID", "CODE", "NAME", "TYPE", "SORT")
);
