...
class CrmActivityCallListComponent extends \CThurlyComponent
{
	...
	public static function createCallList($request)
	{
		...
		if(!\Thurly\Crm\CallList\CallList::isAvailable())
		{
			$result->setData(array(
				'RESTRICTION' => array(
					'HEADER' => Thurly\Crm\CallList\CallList::getLicensePopupHeader(),
					'CONTENT' => \Thurly\Crm\CallList\CallList::getLicensePopupContent()
				)
			));
			return $result;
		}
		...
		try{
			...
			{
				$callList = \Thurly\Crm\CallList\CallList::createWithGridId($entityType, $gridId);
			}
			else
			{
				$filterParams = '';
				...
				$callList = \Thurly\Crm\CallList\CallList::createWithEntities($entityType, $entityIds);
			}
	...
	public static function addToCallList($request)
	{
		...
		try
		{
			$callList = \Thurly\Crm\CallList\CallList::createWithId($callListId, true);
		}
		catch (Main\SystemException $e)
		{
			$result->addError(new Main\Error($e->getMessage()));
	...
	public function prepareItems(\Thurly\Crm\CallList\CallList $callList)
	{
		$result = array();
		...
		$row = \Thurly\Crm\CallList\Internals\CallListItemTable::getList(array(
			'select' => array('CNT'),
	...
	protected function getRows(\Thurly\Crm\CallList\CallList $callList, $offset, $limit)
	{
		$result = array();
		$entityIds = array();
		$cursor = \Thurly\Crm\CallList\Internals\CallListItemTable::getList(array(
		...
		while ($row = $cursor->fetch())
		{
			...
			$records[$entityId] = array(
				'ENTITY_ID' => $entityId,
				'STATUS' => \Thurly\Crm\CallList\Item::getStatusName($row['STATUS_ID']),
		...
		$cursor = \Thurly\Crm\CallList\Internals\CallListCreatedTable::getList(array(
			'filter' => array(
				'=LIST_ID' => $callList->getId(),
	...
	{
		$callList = \Thurly\Crm\CallList\CallList::createWithId((int)$this->arParams['CALL_LIST_ID']);
		$this->arResult = array(
	...
	protected function prepareDataForEdit($params)
	{
		...
		if($callListId)
		{
			$callList = \Thurly\Crm\CallList\CallList::createWithId($callListId);
		...
		{
			$callList = \Thurly\Crm\CallList\CallList::createEmpty();
			$callList->persist();
	...
	protected function getRecordHtml($callId, $recordId)
	{
		...
		$APPLICATION->IncludeComponent(
			...
			Array(
				...
				"PLAYER_ID" => "thurly_vi_record_".$callId,
				"TYPE" => "audio/mp3",
			),
			false,
			Array("HIDE_ICONS" => "Y")
	...
	protected function prepareDataForView()
	{
		...
		{
			$webform = \Thurly\Crm\WebForm\Internals\FormTable::getById($result['CALL_LIST']['WEBFORM_ID'])->fetch();
			$result['CALL_LIST']['WEBFORM_SECURITY_CODE'] = $webform['SECURITY_CODE'];
			$result['CALL_LIST']['WEBFORM_NAME'] = $webform['NAME'];
			$result['CALL_LIST']['WEBFORM_URL'] = Thurly\Crm\WebForm\Manager::getEditUrl($result['CALL_LIST']['WEBFORM_ID']);
		}
		$started = false;
	...
	{
		$result = array();
		$cursor = \Thurly\Crm\ActivityTable::getList(array(
