...
if (!$curUser || !$curUser->IsAuthorized() || !check_thurly_sessid())
{
	die();
}
...
switch ($action)
{
	case 'RELOAD':
		...
		$APPLICATION->IncludeComponent('thurly:crm.activity.call_list',
			'',
			array(
				'ACTION' => 'RELOAD',
	...
	case 'GET_AVATAR':
		...
		CThurlyComponent::includeComponentClass('thurly:crm.activity.call_list');
	...
	case 'SET_ELEMENT_RANK':
		...
		$callList->persist();
		echo \Thurly\Main\Web\Json::encode($callList->toArray());
	...
	case 'SET_ELEMENT_STATUS':
		...
		$callList->persist();
		if($callList->getItemsCount(\Thurly\Crm\CallList\CallList::STATUS_IN_WORK) == 0)
		{
