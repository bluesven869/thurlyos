...
{
	$arParams['TEMPLATE_THEME'] = Main\ModuleManager::isModuleInstalled('thurly.eshop') ? 'site' : 'blue';
}
...
if (!empty($arParams['TEMPLATE_THEME']))
{
	if (!is_file(Main\Application::getDocumentRoot().'/thurly/css/main/themes/'.$arParams['TEMPLATE_THEME'].'/style.css'))
	{
...
$APPLICATION->SetAdditionalCSS('/thurly/css/main/themes/'.$arParams['TEMPLATE_THEME'].'/style.css', true);
...
else
{
	...
	<div style="display: none">
		...
		$APPLICATION->IncludeComponent(
			'thurly:sale.location.selector.steps',
			'.default',
			array(),
			false
		);
		$APPLICATION->IncludeComponent(
			'thurly:sale.location.selector.search',
			'.default',
			array(),
			false
		);
	...
	<script>
		...
		$city = \Thurly\Sale\Location\TypeTable::getList(array('filter' => array('=CODE' => 'CITY'), 'select' => array('ID')))->fetch();
		?>
