...
use Thurly\Main,
	...
	Thurly\Main\Web\Json,
	Thurly\Main\Localization\Loc,
	Thurly\Sale,
	Thurly\Sale\Order,
	Thurly\Sale\PersonType,
	Thurly\Sale\Shipment,
	Thurly\Sale\PaySystem,
	Thurly\Sale\Payment,
	Thurly\Sale\Delivery,
	Thurly\Sale\Location\LocationTable,
	Thurly\Sale\Result,
	Thurly\Sale\DiscountCouponsManager,
	Thurly\Sale\Services\Company,
	Thurly\Sale\Location\GeoIp;
...
class SaleOrderAjax extends \CThurlyComponent
{
	...
	public function onPrepareComponentParams($arParams)
	{
		...
		if (!isset($arParams['COMPATIBLE_MODE']) && $this->initComponentTemplate())
		{
			...
			if (
				$template instanceof CThurlyComponentTemplate
		...
		$this->arResult = array(
			...
			'bUsingVat' => false,
			'BASKET_ITEMS' => array(),
			'BASE_LANG_CURRENCY' => Thurly\Sale\Internals\SiteCurrencyTable::getSiteCurrency(SITE_ID),
	...
	protected function initProperties(Order $order, $isPersonTypeChanged)
	{
		...
		if($this->arParams['SPOT_LOCATION_BY_GEOIP'] === 'Y')
			$ipAddress = \Thurly\Main\Service\GeoIp\Manager::getRealIp();
	...
	protected function showAuthFormAction()
	{
		...
		$arResult['AUTH']['USER_LOGIN'] = strlen($request['USER_LOGIN']) > 0
			...
			: htmlspecialcharsbx(${Option::get('main', 'cookie_name', 'THURLY_SM').'_LOGIN'});
	...
	protected function checkSocServicesAuthForm()
	{
		...
		if (Thurly\Main\ModuleManager::isModuleInstalled("intranet") || Thurly\Main\ModuleManager::isModuleInstalled("rest"))
	...
	protected function autoRegisterUser()
	{
		...
		)
		{
			...
			if ($userProps['EMAIL'] != '')
			{
				$res = Thurly\Main\UserTable::getRow(array(
					'filter' => array(
	...
	public function getGridHeaders($productColumns)
	{
		...
		if (is_array($productColumns) && !empty($productColumns))
		{
			...
			{
				$iBlockList = array();
				$catalogIterator = Thurly\Catalog\CatalogIblockTable::getList(array(
				...
				if (!empty($iBlockList))
				{
					$propertyIterator = Thurly\Iblock\PropertyTable::getList(array(
	...
	protected function obtainBasket()
	{
		...
		foreach ($this->order->getBasket() as $basketItem)
		{
			...
			$arBasketItem["DISCOUNT_PRICE_PERCENT_FORMATED"] = Thurly\Sale\PriceMaths::roundPrecision($arBasketItem["DISCOUNT_PRICE_PERCENT"])."%";
		...
		$propertyIterator = Sale\Internals\OrderPropsTable::getList(array(
			...
			'runtime' => array(
				new \Thurly\Main\Entity\ReferenceField(
					'PERSON_TYPE_SITE',
					'Thurly\Sale\Internals\PersonTypeSiteTable',
			...
			while ($paySysAction = $paySysActionIterator->fetch())
			{
				$dbRestriction = Sale\Internals\ServiceRestrictionTable::getList(array(
					...
					'filter' => array(
						...
						'CLASS_NAME' => $helper->forSql('\Thurly\Sale\Services\PaySystem\Restrictions\PersonType'),
		...
		if (!empty($actualValues) && is_array($actualValues))
		{
			foreach ($actualValues as $key => $value)
			{
				...
				$APPLICATION->IncludeComponent(
					'thurly:sale.location.selector.'.$locationTemplate,
					'',
					$parameters,
					null,
					array('HIDE_ICONS' => 'Y')
	...
	public function executeComponent()
	{
		...
		$this->checkSession = $this->arParams["DELIVERY_NO_SESSION"] == "N" || check_thurly_sessid();
