...
$asset->addCss('/thurly/themes/.default/crm-entity-show.css');
$asset->addCss('/thurly/js/crm/css/crm.css');
...
$settings = array(
	...
	'serviceUrl' => '/thurly/components/thurly/crm.widget_panel/settings.php?'.thurly_sessid_get(),
	'listUrl' => $listUrl,
	'widgetUrl' => $widgetUrl,
	...
	'isAjaxMode' => \Thurly\Main\Page\Frame::isAjaxRequest()
);
...
<script type="text/javascript">
	BX.ready(
		function()
		{
			...
			BX.CrmWidgetManager.serviceUrl = "<?='/thurly/components/thurly/crm.widget_panel/ajax.php?'.thurly_sessid_get()?>";
...
<?*/?>				{
			...
			BX.CrmWidgetTypeSelector.infos.push({
				...
				logo: "<?=$templateFolder?>/images/view-saletarget.png",
				params:
				{
					rowHeight: 360,
					config: { typeName: "custom", customType: 'saletarget', configs: [ {
						name: "sale_target",
						title:'<?=GetMessageJS('CRM_WGT_SELECTOR_TYPE_CUSTOM_SALETARGET')?>',
			...
			BX.CrmWidgetPanel.isAjaxMode = <?=\Thurly\Main\Page\Frame::isAjaxRequest() ? 'true' : 'false'?>;
...
//Load custom widgets
$APPLICATION->IncludeComponent(
	'thurly:crm.widget.custom.saletarget',
	'',
	array(),
	$component,
	array('HIDE_ICONS' => 'Y')
