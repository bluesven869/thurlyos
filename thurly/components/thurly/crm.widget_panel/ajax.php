...
if($action == 'PREPARE_DATA')
{
	...
	$commonFilter = new Thurly\Crm\Widget\Filter($commonFilterConfig);
	if($commonFilter->isEmpty())
	{
		$commonFilter->setPeriodTypeID(Thurly\Crm\Widget\FilterPeriodType::LAST_DAYS_30);
	}

	if(isset($control['filter']) && is_array($control['filter']))
	{
		$filter = new Thurly\Crm\Widget\Filter($control['filter']);
		if($filter->isEmpty())
		{
			Thurly\Crm\Widget\Filter::merge($commonFilter, $filter);
		}
	}
	else
	{
	...
	$widget = Thurly\Crm\Widget\WidgetFactory::create($control, $filter);
