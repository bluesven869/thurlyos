...
for($i = 0; $i < $rowQty; $i++)
{
	...
	for($j = 0; $j < $cellQty; $j++)
	{
		...
		for($k = 0; $k < $controlQty; $k++)
		{
			...
			$widget = Thurly\Crm\Widget\WidgetFactory::create($control, $filter, $factoryOptions);
...
if($arParams['NOT_CALCULATE_DATA'] == false && !$enableDemo && CCrmPerms::IsAdmin())
{
	...
	{
		$entityBuilders = Thurly\Crm\Statistics\StatisticEntryManager::prepareBuilderData(CCrmOwnerType::ResolveID($entityType));
