...
if ($arResult['VAT_MODE'])
{
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST[$arResult['AJAX_PARAM_NAME']]) && check_thurly_sessid())
	{
...
foreach($allVats as $k => $v)
{
	...
	$arVat['PATH_TO_VAT_DELETE'] =
		CHTTP::urlAddParams(
			...
			array('action_'.$arResult['GRID_ID'] => 'delete', 'ID' => $k, 'sessid' => thurly_sessid())
		);
