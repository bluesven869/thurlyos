			...
			foreach ($arResult["RequestsOut"]["List"] as $arRequest)
			{
				...
				?><tr>
					<td class="invite-list-checkbox"><div class="invite-active-block"><?
						...
						?><input type="hidden" name="id_<?=$ind ?>" value="<?=$arRequest["ID"] ?>"><?
					?></div></td>
					<td class="invite-list-img"><div class="invite-active-block"><span class="invite-list-img-image" style="<?=(is_array($arRequest["USER_PERSONAL_PHOTO_IMG"]) && strlen($arRequest["USER_PERSONAL_PHOTO_IMG"]["src"]) > 0 ? "background: url('".$arRequest["USER_PERSONAL_PHOTO_IMG"]["src"]."') no-repeat center center;" : "")?>"></span></div></td>
					<td class="invite-list-name">
						<div class="invite-active-block"><?
						if ($arRequest["SHOW_PROFILE_LINK"])
						{
