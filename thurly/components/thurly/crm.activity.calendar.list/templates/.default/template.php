...
<?
	for ($i=0, $ic=sizeof($arResult['FILTER']); $i < $ic; $i++)
	{
		if ($arResult['FILTER'][$i]['type'] === 'user')
		{
			...
			ob_start();
			$APPLICATION->IncludeComponent('thurly:intranet.user.selector', 'minimized', array(
...
/*		$arActions[] =  array(
		...
		$arColumns = array(
			...
			'OWNER_ID' => $arCal['~OWNER_ID'] > 0 ?
				'<a href="'.$arCal['PATH_TO_USER_PROFILE'].'" id="balloon_'.$arResult['GRID_ID'].'_'.$arCal['ID'].'">'.$arCal['OWNER_ID'].'</a>'.
				'<script type="text/javascript">BX.tooltip('.$arCal['~OWNER_ID'].', "balloon_'.$arResult['GRID_ID'].'_'.$arCal['ID'].'", "");</script>'
				: ''
		);
		if ($arResult['ACTIVITY_ENTITY_LINK'] == 'Y')
		{
			...
			$arColumns['ENTITY_TITLE'] = !empty($arCal['ENTITY_TITLE'])?
				'<a href="'.$arCal['ENTITY_LINK'].'" id="balloon_'.$arResult['GRID_ID'].'_I_'.$arCal['REL_ID'].'">'.$arCal['ENTITY_TITLE'].'</a>'.
				'<script type="text/javascript">BX.tooltip('.$arCal['ENTITY_ID'].', "balloon_'.$arResult['GRID_ID'].'_I_'.$arCal['REL_ID'].'", "/thurly/components/thurly/crm.'.strtolower($arCal['ENTITY_TYPE']).'.show/card.ajax.php", "crm_balloon'.($arCal['ENTITY_TYPE'] == 'LEAD' || $arCal['ENTITY_TYPE'] == 'DEAL' || $arCal['ENTITY_TYPE'] == 'QUOTE' ? '_no_photo': '_'.strtolower($arCal['ENTITY_TYPE'])).'", true);</script>'
				: '';
		}
		else
		{
