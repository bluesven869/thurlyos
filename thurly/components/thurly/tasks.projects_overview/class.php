...
//		$filter['GROUP.CLOSED'] = 'N';

		//region GROUPS
		$nav = new \Thurly\Main\UI\PageNavigation("nav-projects");
...
//			}
	...
	private function getGridFilter()
	{
		...
		{
			$filter['*%GROUP.SEARCH_INDEX'] = trim($filterData['FIND']);
		}
...
//					dd($filterData);
	...
	private function getProjectTypes()
	{
		...
		if (
			empty($types) &&
			class_exists('\Thurly\Socialnetwork\Item\Workgroup') &&
			method_exists('\Thurly\Socialnetwork\Item\Workgroup', 'getTypes')
		)
		{
			$types = \Thurly\Socialnetwork\Item\Workgroup::getTypes(
				array(
	...
	private function getCounters(array $groupIds)
	{
		...
		$filterInstance = CTaskFilterCtrl::getInstance(
		...
		$groupsData = \Thurly\Tasks\Integration\SocialNetwork\Group::getData($groupIds);
		...
		{
			$counters[$groupId] = array(
				'ALL' => 0,
				'IN_WORK' => 0,
				'EXPIRED' => 0,
				'COMPLETE' => 0,
				'EFFECTIVE' => round(Effective::getByRange($dateFrom, $dateTo, null, $groupId))
			);
		}
