			...
			if (strlen($arResult["PAY_SYSTEM"]["ACTION_FILE"]) > 0)
			{
				...
				<tr>
					<td>
						...
						{
							$service = \Thurly\Sale\PaySystem\Manager::getObjectById($arResult['PAY_SYSTEM_ID']);
							...
							if ($service)
							{
								...
								if ($order === null)
								{
									$data = \Thurly\Sale\Internals\OrderTable::getRow(array(
										'select' => array('ID'),
										'filter' => array('ACCOUNT_NUMBER' => $orderId)
									));

									$order = \Thurly\Sale\Order::load($data['ID']);
								}
