...
else
{
	...
	function showComment(key, subject, error, comment, userName, userEmail)
	{
		...
		if ($arResult["use_captcha"]===true)
		{
			...
			BX.ajax.getCaptcha(function(data) {
				...
				BX("captcha").src = '/thurly/tools/captcha.php?captcha_code=' + data["captcha_sid"];
	...
	function ShowComment($comment, $tabCount=0, $tabSize=30, $canModerate=false, $User=Array(), $use_captcha=false, $bCanUserComment=false, $errorComment=false, $arParams = array())
	{
		...
		<tr>
			<td width="0%"><img src="/thurly/images/1.gif" height="1" width="<?=$tabWidth?>"></td>
				...
				<tr>
					...
					<tr>
						...
						if(strlen($comment["urlToDelete"])>0)
						{
							...
							?>
							<td align="right">
								<a href="javascript:if(confirm('<?=GetMessage("BPC_MES_DELETE_POST_CONFIRM")?>')) window.location='<?=$comment["urlToDelete"]."&".thurly_sessid_get()?>'" class="blog-post-delete"></a>
							</td>
							<?
						}
						?>
