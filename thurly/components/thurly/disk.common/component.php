...
if(empty($arParams['STORAGE']))
{
	...
	$arParams['STORAGE'] = \Thurly\Disk\Storage::loadById((int)$arParams['STORAGE_ID'], array('ROOT_OBJECT'));
}
if(empty($arParams['STORAGE']))
{
	//necessary
...
if ($arParams['SEF_MODE'] == 'Y')
{
	...
	$engine->setResolveCallback(array(\Thurly\Disk\Driver::getInstance()->getUrlManager(), "resolvePathComponentEngine"));
	...
	{
		/** @var \Thurly\Disk\Storage $arParams['STORAGE'] */
		$componentPage = 'folder_list';
		if($arParams['STORAGE'] instanceof \Thurly\Disk\Storage)
		{
			$arVariables = array(
