...
{
	$backUrl = CComponentEngine::MakePathFromTemplate($arResult["PATH_TO_FILE_VIEW"], array("FILE_PATH" => $_REQUEST["file"]));
	$backUrl .= '#tab-bp';
}
else
{
...
{
	$dbTask = \Thurly\Disk\BizProcDocument::getTaskServiceList($arParams["TASK_ID"], $arParams["USER_ID"]);
...
{
	$back_url = (isset($_REQUEST["back_url"])) ? urldecode($_REQUEST["back_url"]) : CComponentEngine::MakePathFromTemplate($arResult["PATH_TO_FOLDER_LIST"], array("PATH" => ""));
