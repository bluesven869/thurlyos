...
if($this->StartResultCache(false, array($USER->GetGroups(), $arNavigation, $arCache), false))
{
	...
	if(!empty($historyId) && check_thurly_sessid())
	{
		$this->AbortResultCache();
	...
	while($arHistory = $rsHistory->GetNext())
	{
		...
		$arHp = array('oldid' => $arHistory['ID'], 'sessid' => thurly_sessid());
		...
		if (CWikiUtils::IsDeleteable())
		{
			$arHp = array('oldid' => $arHistory['ID'], 'sessid' => thurly_sessid(), 'delete' => 1);
			if ($arParams['IN_COMPLEX'] == 'Y' && $arParams['SEF_MODE'] == 'N')
				$arHp[$arParams['OPER_VAR']] = 'history';
...
include_once($_SERVER['DOCUMENT_ROOT'].'/thurly/components/thurly/wiki/include/nav.php');
