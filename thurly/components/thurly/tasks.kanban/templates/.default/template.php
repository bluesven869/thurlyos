...
{
	$filterInstance = \Thurly\Tasks\Helper\Filter::getInstance($arParams["USER_ID"], $arParams["GROUP_ID"]);
...
<?if ($emptyKanban):?>
	<div class="tasks-kanban-start">
		<div class="tasks-kanban-start-wrapper">
			...
			<a href="<?= SITE_DIR?>workgroups/?new" class="webform-button webform-button-blue tasks-kanban-start-button"><?
...
if ($show)
{
	...
	?>>
		...
		<a href="https://helpdesk.thurlyos.<?= $popupDomain?>/open/<?= $popupUrlId?>/" target="_blank" data-helpId="<?= $popupUrlId?>"<?if (SITE_TEMPLATE_ID == 'thurlyos'){?> id="kanban-readmore"<?}?> class="tasks-kanban-popup-text-redmore">
...
//if (\Thurly\Intranet\Integration\Templates\ThurlyOS\ThemePicker::getInstance()->shouldShowHint()):
