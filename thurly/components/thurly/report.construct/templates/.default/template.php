...
<? if ($arParams['ACTION'] == 'delete'): ?>
	...
	<form style="float: left;" method="POST" action="<?=CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_REPORT_CONSTRUCT"], array("report_id" => $arParams['REPORT_ID'], 'action' => 'delete_confirmed'));?>">
		<?php echo thurly_sessid_post('csrf_token')?>
...
$GLOBALS['APPLICATION']->AddHeadScript('/thurly/js/report/construct.js');
...
<?php if ($arParams['USE_CHART']): ?>
	<style type="text/css">
		.report-chart-add-ycolumn {
			...
			margin-left: 4px;
			background: url("/thurly/js/report/css/images/sprites.png") -119px -3px no-repeat;
			cursor: pointer;
		}
		...
		.report-chart-span-minus {
			...
			width: 10px;
			height: 2px;
			background: url("/thurly/js/report/css/images/sprites.png") -122px 0 no-repeat;
			cursor: pointer;
		}
...
<div id="report-filter-value-control-examples" style="display: none">
	...
	<span name="report-filter-value-control-datetime" class="report-filter-calendar">
		<input type="text" class="reports-filter-input" name="value" /><img alt="img" class="reports-filt-calen-img" src="/thurly/js/main/core/images/calendar-icon.gif" />
	</span>
	...
	<span name="report-filter-value-control-\Thurly\Main\User" callback="RTFilter_chooseUser">
		<a href="" class="report-select-popup-link" caller="true"><?=GetMessage('REPORT_CHOOSE')?></a>
		<input type="hidden" name="value" />
	</span>
	<script type="text/javascript">
	...
	<span name="report-filter-value-control-\Thurly\Socialnetwork\Workgroup" callback="RTFilter_chooseGroup">
		<a href="" class="report-select-popup-link" caller="true"><?=GetMessage('REPORT_CHOOSE')?></a>
		<input type="hidden" name="value" />
	</span>
	<script type="text/javascript">
...
<script type="text/javascript">
	BX(function () {
		...
		BX.Report['<?=$jsClass?>'] = new BX.Report.ReportConstructClass({
			jsClass:'<?=$jsClass?>',
			sharingData: <?= Thurly\Main\Web\Json::encode($arResult['SHARING_DATA']) ?>,
