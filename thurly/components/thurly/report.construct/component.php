...
try
{
	...
	{
		$result = Thurly\Report\ReportTable::getById($arParams['REPORT_ID']);
		$report = $result->fetch();
		...
		$rightsManager = new Thurly\Report\RightsManager($userId);
	...
	if($arParams['ACTION'] == 'edit' || $arParams['ACTION'] == 'create' || $arParams['ACTION'] == 'copy')
	{
		...
		$entitySharing = Thurly\Report\Sharing::getEntityOfSharing($reportId);
		$entityList = array();
		foreach($entitySharing as $entity)
		{
			list($type, $id) = Thurly\Report\Sharing::parseEntityValue($entity['ENTITY']);
			$typeData = Thurly\Report\Sharing::getTypeData($type, $id);
			$entityList[] = array(
		...
		$destination = Thurly\Report\Sharing::getSocNetDestination($userId, $selected);
		$arResult['SHARING_DATA'] = array(
			'members' => $entityList,
			'maxAccess' => Thurly\Report\RightsManager::ACCESS_READ,
			'destination' => array(
				'items' => array(
	...
	if($isPost && isset($_POST['EXPORT_REPORT']))
	{
		...
		$rightsmanager = new Thurly\Report\RightsManager($USER->GetID());
		...
		$queryObject = Thurly\Report\ReportTable::getById($reportId);
	...
	if (!empty($_POST) && (!empty($_POST['report_select_columns']) || $arParams['ACTION'] == 'delete_confirmed'))
	{
		...
		// check csrf
		if (!check_thurly_sessid('csrf_token'))
		{
			throw new BXFormException(GetMessage('REPORT_CSRF'));
		}
		...
		if($ID)
		{
			$sharing = new Thurly\Report\Sharing($ID);
