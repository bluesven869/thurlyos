...
if (strlen($arResult["FatalErrorMessage"]) <= 0)
{
	...
	if ($deleteBlockId > 0 && $arResult["AdminAccess"] && check_thurly_sessid())
	{
		$db = CIBlock::GetList(
			array(),
...
if (strlen($arResult["FatalErrorMessage"]) <= 0)
{
	...
	while ($arBlock = $dbBlockList->GetNext())
	{
		...
		if (intval($arBlock["PICTURE"]) <= 0)
			$arBlock["PICTURE"] = "/thurly/images/bizproc/vd_bp.jpg";
		...
		if ($arResult["AdminAccess"])
		{
			...
			$arBlock["DELETE_URL"] .= "delete_block_id=".$arBlock["ID"]."&".thurly_sessid_get();
		}
