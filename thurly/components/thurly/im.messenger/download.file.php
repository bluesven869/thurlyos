...
if($_GET['action'] == 'showFile')
{
	if ($_GET['preview'] == 'Y')
	{
		$_GET['width'] = 500;
		$_GET['height'] = 500;
		$_GET['signature'] = \Thurly\Disk\Security\ParameterSigner::getImageSignature($_GET['fileId'], $_GET['width'], $_GET['height']);
...
class ImagePreviewSizeFilter implements Thurly\Main\Type\IRequestFilter
{
	/**
	 * @param array $values
	 * @return array
	...
	public function filter(array $values)
	{
		if($values['get']['action'] == 'showFile')
		{
			if($values['get']['preview'] == 'Y')
			{
				...
				$values['get']['signature'] = \Thurly\Disk\Security\ParameterSigner::getImageSignature(
...
\Thurly\Main\Application::getInstance()->getContext()->getRequest()->addFilter(new ImagePreviewSizeFilter);

if($_SERVER['REQUEST_METHOD'] == 'POST')
{
	LocalRedirect(\Thurly\Main\Application::getInstance()->getContext()->getRequest()->getRequestUri());
}
