...
{
	echo \Thurly\Im\Common::objectEncode(Array('ERROR' => GetMessage('IM_MODULE_NOT_INSTALLED')));
	CMain::FinalActions();
	die();
}
...
if (!$USER->IsJustAuthorized() && !check_thurly_sessid())
{
	if (\Thurly\Im\User::getInstance($USER->GetID())->isConnector())
	{
		echo \Thurly\Im\Common::objectEncode(Array(
			'ERROR' => '',
...
else if ($_POST['IM_FILE_REGISTER'] == 'Y')
{
	...
	if ($_POST['TEXT'])
	{
		...
		$ar['MESSAGE'] = \Thurly\Im\Text::parse($ar['MESSAGE']);
	}
	else
	{
		$ar['MESSAGE'] = '';
...
elseif ($_POST['IM_UPDATE_STATE'] == 'Y')
{
	...
	$arSend['TEXTAREA_ICON'] = \Thurly\Im\App::getListForJs();
	$arSend['COMMAND'] = \Thurly\Im\Command::getListForJs();

	echo \Thurly\Im\Common::objectEncode($arSend);
}
else if ($_POST['IM_UPDATE_STATE_LIGHT'] == 'Y')
{
	...
	{
		echo \Thurly\Im\Common::objectEncode($arResult);
	}
}
...
else if ($_POST['IM_SEND_MESSAGE'] == 'Y')
{
	...
	else if (substr($_POST['RECIPIENT_ID'], 0, 4) != 'chat' && !\Thurly\Im\User::getInstance($USER->GetID())->isConnector())
	{
		$ar = Array(
	...
	$arResult = Array(
		...
		'SEND_DATE' => new \Thurly\Main\Type\DateTime(),
		'SEND_MESSAGE' => \Thurly\Im\Text::parse($ar['MESSAGE']),
...
else if ($_POST['IM_BOT_COMMAND'] == 'Y')
{
	...
	$orm = \Thurly\Im\Model\MessageTable::getById($messageId);
	if($message = $orm->fetch())
	{
		$orm = \Thurly\Im\Model\ChatTable::getById($message['CHAT_ID']);
		$chat = $orm->fetch();
		...
		if (isset($relations[$userId]))
		{
			...
			$result = \Thurly\Im\Command::onCommandAdd($messageId, $messageFields);
			$errorMessage = '';
		}
	}
...
else if ($_POST['IM_EDIT_MESSAGE'] == 'Y')
{
	...
	else
	{
		...
		$arResult = Array(
			'ID' => $insertID,
			'MESSAGE' => \Thurly\Im\Text::parse($_POST['MESSAGE']),
...
else if ($_POST['IM_CONTACT_LIST_SEARCH'] == 'Y')
{
	...
	else if (!Thurly\Im\User::getInstance()->isExtranet() && CModule::IncludeModule('socialservices'))
	{
		...
		if ($enabled)
		{
			$query = CThurlyOSNetTransport::init();
			if(!$query)
			{
				$enabled = false;
			}
	...
	{
		echo \Thurly\Im\Common::objectEncode(Array('ERROR' => 'DISABLED_FUNCTION'));
	}
}
				...
				if (strlen($history['CALL_RECORD_HREF']) > 0)
				{
					...
					$APPLICATION->IncludeComponent(
						...
						Array(
							...
							"PLAYER_ID" => "thurly_vi_record_".$arResult['HISTORY_ID']
						),
						false,
						Array("HIDE_ICONS" => "Y")
					);
...
else if ($_POST['IM_SHARING'] == 'Y' && intval($_POST['USER_ID']) > 0)
{
	...
	{
		\Thurly\Pull\Event::add(intval($_POST['USER_ID']), Array(
			'module_id' => 'im',
...
{
	require($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/voximplant/ajax_hit.php");
}
else if (($_POST['IM_OPEN_LINES'] == 'Y' || $_POST['IM_OPEN_LINES_CLIENT'] == 'Y') && CModule::IncludeModule('imopenlines'))
{
	...
	require($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/imopenlines/handlers/ajax.php");
}
...
else if ($_POST['IM_GET_TEXTAREA_ICONS'] == 'Y')
{
	...
	echo \Thurly\Im\Common::objectEncode(Array(
		'TEXTAREA_ICON' => \Thurly\Im\App::getListForJs(),
		'ERROR' => ''
	));
}
...
else if ($_POST['IM_GET_MOBILE_CHAT_AVATAR'] == 'Y')
{
	...
	if ($_POST['CHAT_ID'] > 0)
	{
		...
		if ($arResult['CHAT'])
		{
			$avatar = $arResult['CHAT']['avatar'] == '/thurly/js/im/images/blank.gif'? '': $arResult['CHAT']['avatar'];
		}
	}
...
{
	echo \Thurly\Im\Common::objectEncode(Array('ERROR' => GetMessage('IM_UNKNOWN_ERROR')));
}
