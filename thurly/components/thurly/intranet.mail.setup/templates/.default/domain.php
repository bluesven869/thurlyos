...
<script type="text/javascript">
	...
	BX.bind(BX('check-confirm'), 'click', function()
	{
		...
		BX.ajax({
			method: 'POST',
			url: '<?=$this->__component->GetPath() ; ?>/ajax.php?page=domain&act=check',
...
<script type="text/javascript">
	...
	var RegDomain = {
		...
		check: function (e, callback)
		{
			...
			if (typeof RegDomain.checkResults[dname] == 'undefined')
			{
				...
				BX.ajax({
					method: 'POST',
					url: '<?=$this->__component->GetPath() ; ?>/ajax.php?page=domain&act=whois&domain='+dname,
					dataType: 'json',
					onsuccess: function (json)
					{
						...
						new BX.PopupWindowButton({
							...
							events: {
								click: function() {
									...
									BX.ajax({
										method: 'POST',
										url: '<?=$this->__component->GetPath() ; ?>/ajax.php?page=domain&act=get',
		...
		suggest: {
			request: function ()
			{
				...
				if (typeof RegDomain.suggestResults[dname] == 'undefined')
				{
					BX.ajax({
						method: 'POST',
						url: '<?=$this->__component->GetPath() ; ?>/ajax.php?page=domain&act=suggest&domain='+dname,
						dataType: 'json',
	...
	if (window.location.hash.substr(1) == 'get')
	{
		...
		BX.ajax({
			method: 'GET',
			url: '<?=$this->__component->GetPath() ; ?>/ajax.php?page=domain&act=initget',
			dataType: 'json',
			onsuccess: function() {},
			onfailure: function() {}
		});
