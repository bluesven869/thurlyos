...
CThurlyComponent::includeComponentClass("thurly:catalog.viewed.products");

class CSaleGiftBasketComponent extends CCatalogViewedProductsComponent
{
	/** @var \Thurly\Sale\Discount\Gift\Manager */
	protected $giftManager;
	/** @var  \Thurly\Sale\Basket */
	private $basket;
	/** @var array */
	private $productIds;
	...
	protected function initGiftManager()
	{
		...
		$this->giftManager = \Thurly\Sale\Discount\Gift\Manager::getInstance()->setUserId($userId);
	}
	...
	private function getBasket()
	{
		if($this->basket === null)
		{
			$basketStorage = \Thurly\Sale\Basket\Storage::getInstance(\Thurly\Sale\Fuser::getId(), SITE_ID);
	...
	private function fetchFirstIblockId()
	{
		...
		foreach($this->getBasket() as $item)
		{
			...
			{
				$element = \Thurly\Iblock\ElementTable::getRow(array(
	...
	private function hasProperty($catalogIblockId, $propertyName)
	{
		...
		return (bool)\Thurly\Iblock\PropertyTable::getRow(array(
			...
			'filter' => array(
				...
				'@PROPERTY_TYPE' => array(
					\Thurly\Iblock\PropertyTable::TYPE_FILE,
				),
				'=MULTIPLE' => 'N'
			),
		));
	...
	private function getProperties($offersIblockId)
	{
		...
		$propertyIterator = \Thurly\Iblock\PropertyTable::getList(array(
			...
			'filter' => array(
				...
				'!=ID' => $offers['SKU_PROPERTY_ID'],
				'@PROPERTY_TYPE' => array(
					\Thurly\Iblock\PropertyTable::TYPE_STRING,
					\Thurly\Iblock\PropertyTable::TYPE_LIST,
				),
				'=MULTIPLE' => 'N'
			),
		...
		while($property = $propertyIterator->fetch())
		{
			...
			if($property['PROPERTY_TYPE'] == \Thurly\Iblock\PropertyTable::TYPE_STRING && $property['USER_TYPE'] != 'directory')
			{
				continue;
			}
	 ...
	 * @return bool
	 */
	private function isExtendedCatalogProvider(\Thurly\Sale\BasketItem $item)
	{
		return
			...
			(
				...
				(
					...
					|| $item->getProvider() === "\Thurly\Catalog\Product\CatalogProvider"
					|| array_key_exists("CCatalogProductProvider", class_parents($item->getProvider()))
					|| array_key_exists("\Thurly\Catalog\Product\CatalogProvider", class_parents($item->getProvider()))
				)
			);
	}
}