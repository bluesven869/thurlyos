...
class CDiskFileViewComponent extends DiskComponent
{
	...
	protected function prepareParams()
	{
		...
		{
			throw new \Thurly\Main\ArgumentException('FILE_ID required');
		}
		$this->arParams['FILE_ID'] = (int)$this->arParams['FILE_ID'];
		if($this->arParams['FILE_ID'] <= 0)
		{
			throw new \Thurly\Main\ArgumentException('FILE_ID < 0');
		}
	...
	protected function processActionDefault()
	{
		...
		$this->arResult = array(
			...
			'FILE' => array(
				...
				'SHOW_PREVIEW_URL' => \Thurly\Disk\Driver::getInstance()->getUrlManager()->getUrlForShowFile($this->file, array('width' => $this->imageSize['width'], 'height' => $this->imageSize['height'],)),
				'SHOW_FILE_URL' => \Thurly\Disk\Driver::getInstance()->getUrlManager()->getUrlForShowFile($this->file),
		...
		if($this->arParams['STATUS_BIZPROC'])
		{
			$documentData = array(
				'DISK' => array(
					...
					'DOCUMENT_ID' => \Thurly\Disk\BizProcDocument::getDocumentComplexId($this->file->getId()),
				),
				'WEBDAV' => array(
					'DOCUMENT_TYPE' => \Thurly\Disk\BizProcDocumentCompatible::generateDocumentComplexType($this->storage->getId()),
					'DOCUMENT_ID' => \Thurly\Disk\BizProcDocumentCompatible::getDocumentComplexId($this->file->getId()),
				),
			);
	...
	protected function processActionShowBp()
	{
		...
		$urlStartBizproc .= (strpos($urlStartBizproc, "?") === false ? "?" : "&").'workflow_template_id=0&'.thurly_sessid_get();
	...
	{
		$this->file = \Thurly\Disk\File::loadById($this->arParams['FILE_ID'], array('REAL_OBJECT', 'CREATE_USER'));
	...
	/**
	 * @return \Thurly\Disk\ExternalLink|null
	 */
	...
	{
		$userFieldsObject = \Thurly\Disk\Driver::getInstance()->getUserFieldManager()->getFieldsForObject($this->file);
		if($userFieldsObject)
		{
