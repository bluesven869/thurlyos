...
<div>
	...
	<?= thurly_sessid_post();
		...
		if(!isset($_GET['log_workflow'])) { ?>
			<ul class="bizproc-list bizproc-document-states">
				<li class="bizproc-list-item bizproc-document-start bizproc-list-item-first">
					<a class="bizproc-main-btn" href="<?= $arResult['PATH_TO_START_BIZPROC'] ?>"><?= Loc::getMessage('DISK_FILE_VIEW_BIZPROC_NEW_BIZPROC_START') ?></a>
				</li>
			<?
					...
					<table class="bizproc-table-main" id="<?= $bizProcArray['ID'] ?>">
						<tr>
							...
							<th class="bizproc-field-value">
								...
								<? } ?>
								<a class="webform-small-button webform-button-transparent" href="?log_workflow=<?= $key ?>#tab-bp"><?= Loc::getMessage('DISK_FILE_VIEW_BIZPROC_LOG_BUTTON') ?></a>
							</th>
						</tr>
		...
		<?}else{ $sortBpLog = true; ?>
			<ul class="bizproc-list bizproc-document-states">
				<li class="bizproc-list-item bizproc-document-start bizproc-list-item-first">
					<a class="bizproc-main-btn" href="<? echo $arResult['PATH_TO_FILE_VIEW']."#tab-bp"; ?>"><?= Loc::getMessage('DISK_FILE_VIEW_BIZPROC_LIST_BIZPROC') ?></a>
				</li>
			<?
			$checkAction = true;
			...
			?>
				...
				<table class="bizproc-table-main" id="<?= $idWorkflow ?>">
					...
					<tr class="bizproc-item-row-last">
						<td colspan="2">
							<?
							$APPLICATION->IncludeComponent("thurly:bizproc.log", "", Array(
									"MODULE_ID" => \Thurly\Disk\Driver::INTERNAL_MODULE_ID,
									"ENTITY" => \Thurly\Disk\BizProcDocument::className(),
