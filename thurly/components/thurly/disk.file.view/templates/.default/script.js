...
BX.Disk.FileViewClass = (function ()
{
	...
	this.xpi_path_en =  "/thurly/webdav/ff_bx_integration@thurlysoft.com.xpi";
	this.xpi_path_ru =  "/thurly/webdav/ff_bx_integration@1c-thurly.ru.xpi";
	...
	{
		return this.Bind("ThurlyWebdavConfig", "");
	};
	...
	this.OpenDoc = function(doc)
	{
		...
		items.push(ext);
		doc = items.join('.');
		this.Bind("ThurlyWebdavOpenFile", doc);
	};
	...
	var FileViewClass = function (parameters)
	{
		...
		this.ajaxUrl = '/thurly/components/thurly/disk.file.view/ajax.php';
		...
		if(!!this.tabs)
		{
			BX.addCustomEvent(this.tabs, 'onChangeTab', BX.delegate(function(tabName){
				...
				if(tabName == 'history')
				{
					...
					BX.Disk.ajax({
						url: BX.Disk.addToLinkParam(this.getUrlToAjaxCurrentPage(), 'action', 'showVersion'),
						method: 'POST',
						dataType: 'html',
				...
				else if(tabName == 'bp')
				{
					...
					BX.Disk.ajax({
						url: BX.Disk.addToLinkParam(this.getUrlToAjaxCurrentPage(), 'action', 'showBp'),
						method: 'POST',
						dataType: 'html',
						scriptsRunFirst: true,
						data: {
	...
	FileViewClass.prototype.onBeforeElementShow = function(viewer, element, status)
	{
		...
		BX.PopupWindowManager.create('bx-disk-business-tools-info', null, {
			content: BX('bx-thurlyos-business-tools-info'),
			closeIcon: true,
	...
	{
		if(responseData && responseData.status === "restriction" && BX('bx-thurlyos-business-tools-info'))
		{
			setTimeout(function ()
			{
	...
	FileViewClass.prototype.onClickEditButton = function(e)
	{
		...
		//todo shit! or sheet?
		if(BX.CViewer.temporaryServiceEditDoc && !BX.CViewer.isLocalEditService(BX.CViewer.temporaryServiceEditDoc))
		{
