...
CJSCore::Init(array('viewer', 'disk', 'disk_tabs', 'disk_information_popups'));
$sortBpLog = false;
...
<div class="bx-disk-filepage-container">
	<div class="bx-disk-tab-container">
		<div id="<?= $component->getComponentId() ?>_header" class="disk-tabs-block">
			...
			<? if($arResult["SHOW_USER_FIELDS"]){ ?><span bx-disk-tab="ufprops" class="disk-tab"><?= Loc::getMessage('DISK_FILE_VIEW_TAB_USER_FIELDS') ?></span><? } ?>
					...
					<table class="bx-disk-filepage-table">
						<tr>
							...
							<?  } elseif($arResult['FILE']['SHOW_PREVIEW_IMAGE_URL']) { ?>
								<div id="bx-shared-preview-document-image" class="bx-shared-preview-document-image <?= $arResult['FILE']['ICON_CLASS'] ?>" style="position: relative">
									<a href="#" <?= $arResult['FILE']['PREVIEW_IMAGE_VIEWER_ATTRIBUTES'];?>>
							...
							<td class="bx-disk-filepage-fileinfoblock">
								...
								<hr class="bx-disk-delimiter">
								<?php if($arResult['EXTERNAL_LINK']['ENABLED']){ ?>
										...
										<td>
											<div class="bx-disk-sidebar-shared-outlink-container" style="padding-bottom: 0;">
												<div class="bx-disk-sidebar-shared-outlink-switcher-container">
													<div id="bx-disk-sidebar-shared-outlink" class="bx-disk-sidebar-shared-outlink-switcher-track <?= !empty($arResult['EXTERNAL_LINK']['LINK'])? 'on' : 'off'?>">
														<span id="bx-disk-sidebar-shared-outlink-label"><?= !empty($arResult['EXTERNAL_LINK']['LINK'])? Loc::getMessage('DISK_FILE_VIEW_EXT_LINK_ON') : Loc::getMessage('DISK_FILE_VIEW_EXT_LINK_OFF') ?></span>
														<div class="bx-disk-sidebar-shared-outlink-switcher-point"></div>
													</div>
												</div>
								...
								<?php } ?>
									...
									<? if(!empty($arResult['CAN_UPDATE']) && (!$arResult['FILE']['LOCK']['IS_LOCKED'] || $arResult['FILE']['LOCK']['IS_LOCKED_BY_SELF']) && \Thurly\Disk\Document\DocumentHandler::isEditable(GetFileExtension($arResult['FILE']['NAME'])))
									{
										?><a id="bx-disk-file-edit-btn" class="bx-disk-btn bx-disk-btn-big bx-disk-btn-lightgray" href=""><?= Loc::getMessage('DISK_FILE_VIEW_FILE_EDIT') ?></a><?
									}?>
									<? if(!empty($arResult['CAN_UPDATE']) && (!$arResult['FILE']['LOCK']['IS_LOCKED'] || $arResult['FILE']['LOCK']['IS_LOCKED_BY_SELF'])){?><a id="bx-disk-file-upload-btn" class="bx-disk-btn bx-disk-btn-big bx-disk-btn-lightgray" href="javascript:void(0);"><?= Loc::getMessage('DISK_FILE_VIEW_FILE_UPLOAD_VERSION') ?></a><? } ?>
							</td>
						</tr>
					</table>
		...
		<!--										<a href="" class="bx-disk-filepage-used-people-list-all-btn">--><?php //echo Loc::getMessage('DISK_FILE_VIEW_ENTITY_MEMBERS_NEXT_PAGE') ?><!-- <span></span></a>-->
			...
			<div bx-disk-tab="ufprops" class="disk-tab-content bx-disk-filepage-section">
				<div class="bx-disk-filepage-title"><span><?= Loc::getMessage('DISK_FILE_VIEW_TITLE_USER_FIELDS') ?></span><? if(!empty($arResult['CAN_UPDATE'])) {?><a id="bx-disk-link-edit-uf-values" style="float: right; margin-right: 20px;" href="#"><?= Loc::getMessage('DISK_FILE_VIEW_LINK_EDIT_USER_FIELDS')?></a><? } ?></div>
				<div id="bx-disk-file-uf-props-content">
					<div class="bx-disk-uf-show-content">
						<table>
							<tbody>
					...
					<div class="bx-disk-uf-edit-content"></div>
				</div>
			</div>
...
<div style="display:none;">
	<form id="parametersFormBp">bx-file-icon-container-big
	<div id="divStartBizProc" class="bx-disk-form-bizproc-start-div">
		<table class="bx-disk-form-bizproc-start-table">
			...
			<col class="bx-disk-col-table-right">bx-disk-filepage-previewblock
...
<? if(!empty($arResult['FILE']['LOCK']['IS_LOCKED'])) { ?>
	BX.tooltip(
		<?php echo $arResult['FILE']['LOCK']['CREATED_BY']?>,
		"lock-anchor-created-<?= $component->getComponentId() ?>"
	);
<? } ?>
...
)
{
	...
	<script type="text/javascript">
	BX.message({
		disk_restriction: <?= (!\Thurly\Disk\Integration\ThurlyOSManager::checkAccessEnabled('disk', $USER->getId())? 'true' : 'false') ?>
	});
	</script>
