			...
			if($arResult["PostPerm"]>=BLOG_PERMS_MODERATE)
			{
				...
				if (IntVal($_GET["del_id"]) > 0)
				{
					...
					else
					{
						...
						if (check_thurly_sessid() && CBlogPost::CanUserDeletePost($DEL_ID, $user_id))
						{
				...
				while($arPost = $dbPost->Fetch())
				{
					...
					{
						$arPost["urlToDelete"] = urlencode($APPLICATION->GetCurPageParam("del_id=".$arPost["ID"].'&'.thurly_sessid_get(), Array("del_id", "sessid", "show_id", "success")));
