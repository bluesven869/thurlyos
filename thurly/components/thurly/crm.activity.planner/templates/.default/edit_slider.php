...
/** @var \Thurly\Crm\Activity\Provider\Base $provider */
...
$pageAsset = \Thurly\Main\Page\Asset::getInstance();

switch ($activity['STORAGE_TYPE_ID'])
{
	case Integration\StorageType::WebDav:

		$pageAsset->addCss('/thurly/components/thurly/webdav/templates/.default/style.css');
		$pageAsset->addCss('/thurly/components/thurly/webdav.user.field/templates/.default/style.css');
		$pageAsset->addCss('/thurly/js/webdav/css/file_dialog.css');
	...
	case Integration\StorageType::Disk:
		...
		$pageAsset->addCss('/thurly/js/disk/css/legacy_uf_common.css');
		$storageValues = $activity['DISK_FILES'];
		break;

	default:
		$storageValues = $activity['FILES'];
		$pageAsset->addCss('/thurly/js/disk/css/disk.css');
		break;
}
$storageValues = htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($storageValues));
$storageProps = htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($storageProps));
$destinationEntities = htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($arResult['DESTINATION_ENTITIES']));
$communicationsData = htmlspecialcharsbx(\Thurly\Main\Web\Json::encode($arResult['COMMUNICATIONS_DATA']));

$APPLICATION->restartBuffer();

\CJSCore::init(array('crm_activity_planner'));
$pageAsset->addCss('/thurly/js/crm/css/crm.css');
$pageAsset->addCss('/thurly/themes/.default/crm-entity-show.css');
$pageAsset->addJs('/thurly/js/calendar/planner.js');
$pageAsset->addCss('/thurly/js/calendar/planner-style.css');
?><!DOCTYPE html>
<html>
