...
class CrmActivityPlannerComponent extends \CThurlyComponent
{
	...
	protected function getActivityAdditionalData($activityId, &$activity, $provider = null)
	{
		...
		if (count($activity['__communications']) < 10 && !empty($commType))
		{
			...
			$createdSince = new Thurly\Main\Type\Datetime();
			$createdSince->add('-30 days');

			$res = \Thurly\Crm\ActivityTable::getList(array(
				'runtime' => array(
	...
	protected function executeEditAction()
	{
		...
		if (!$error && !$isNew && !$provider::checkUpdatePermission($activity))
	...
	protected function executeViewAction()
	{
		...
		{
			$template .= '_slider';
			$this->arResult['IS_SLIDER_ENABLED'] = \Thurly\Crm\Settings\LayoutSettings::getCurrent()->isSliderEnabled();
		}
	...
	private function getTypeIcon($activity)
	{
		...
		if ($activity['PROVIDER_ID'] == 'CRM_EXTERNAL_CHANNEL')
		{
			if (
				...
				in_array($activity['ORIGINATOR_ID'], array('THURLY', 'WORDPRESS', 'DRUPAL', 'JOOMLA', 'MAGENTO'))
			)
			{
				return 'cmsplugins';
			}
		...
		if(!empty($activity['FILES']))
		{
			foreach($activity['FILES'] as $file)
			{
				$result[] = array(
					...
					'viewURL' => '/thurly/components/thurly/crm.activity.planner/show_file.php?activity_id='
	...
	public static function saveActivity($data, $userID, $siteID)
	{
		...
		if($isNew)
		{
			...
			{
				\Thurly\Crm\Automation\Trigger\CallTrigger::execute($arFields['BINDINGS'], $arFields);
			}
			//end region
		}
		else
	...
	private function getCrmEntityCommunications($entityTypeId, $entityId, $communicationType)
	{
		...
		else if (\CCrmOwnerType::Deal == $entityTypeId)
		{
			$entity = \CCrmDeal::getById($entityId);
			if (empty($entity))
				return $result($communications);

			$entityContactsIds = \Thurly\Crm\Binding\DealContactTable::getDealContactIds($entityId);
