...
use Thurly\Tasks\UI;
use Thurly\Tasks\Item\Converter\Task\Template\ToTemplate;
...
class TasksTaskTemplateComponent extends TasksBaseComponent
{
	...
	protected function getData()
	{
		...
		elseif(!$id)
		{
			if(intval($this->request['COPY'])) // copy from another template?
			{
				...
				/**
				 * todo: @see \Thurly\Tasks\Item\Converter::convert for todo remark
				 */
	...
	protected function getAuxData()
	{
		$this->arResult['AUX_DATA'] = array(
			...
			'HINT_STATE' => \Thurly\Tasks\UI::getHintState(),
		);
	...
	{
		/** @var \Thurly\Tasks\Dispatcher\ToDo\Plan $plan */
	...
	/**
	 * @param \Thurly\Tasks\Dispatcher\ToDo $operation
	 * @return int
	 */
	...
	protected function getTaskTemplateData(array $ids)
	{
		...
		if(!empty($ids))
		{
			...
			if(!empty($ids))
			{
				$items = \Thurly\Tasks\Item\Task\Template::find(
					array(
