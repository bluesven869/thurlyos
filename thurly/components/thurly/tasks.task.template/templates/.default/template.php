	...
	<?if($arResult['COMPONENT_DATA']['EVENT_TYPE'] == 'ADD' && !empty($arResult['DATA']['EVENT_TASK'])):?>
		<div class="task-message-label">
			...
			<?if($arResult['DATA']['EVENT_TASK']['ID'] != $arResult['DATA']['TASK']['ID']):?>
				<a href="<?=\Thurly\Tasks\UI\Task::makeActionUrl($arParams['PATH_TO_TASKS_TASK'], $arResult['DATA']['EVENT_TASK']['ID'], 'view');?>" target="_blank"><?=Loc::getMessage('TASKS_TASK_TEMPLATE_COMPONENT_TEMPLATE_VIEW_TASK');?></a>
			<?endif?>
		</div>
	<?endif?>
	*/?>
	...
	<?
	/** @var \Thurly\Tasks\Item\Task\Template $template */
	...
	<?else:?>
		...
		<input class="js-id-task-template-edit-csrf" type="hidden" name="sessid" value="<?=thurly_sessid()?>" />
		...
		$blocks['HEAD'] = array(
			'HTML' => \Thurly\Tasks\UI\Editor::getHTML(array(
				'ID' => $helper->getId(),
		...
		<div class="task-options-field-container">
			...
			if($arResult['COMPONENT_DATA']['MODULES']['thurlyos'])
			{
				$options[] = array(
					'CODE' => 'TPARAM_TYPE',
					'YES_VALUE' => '1',
