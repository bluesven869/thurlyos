...
<body style="height: 100%;margin: 0;padding: 0; background: #fff" id="workarea-content">
	...
	<?='BX.message('.\CUtil::PhpToJSObject(\Thurly\Main\Localization\Loc::loadLanguageFile(__FILE__)).');'?>
	BX.message({'PHONE_BASE_PATH': '<?=$this->GetFolder().'/base/'?>', 'PHONE_BASE_LANG': '<?=LANGUAGE_ID?>', 'DEFAULT_GUEST_NAME': '<?=htmlspecialcharsbx(\Thurly\ImOpenLines\LiveChat::getDefaultGuestName())?>'});
</script>
<?
