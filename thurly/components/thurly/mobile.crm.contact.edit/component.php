...
else
{
	...
	if($bizCardId)
	{
		...
		{
			$dbBizCard = Thurly\Bizcard\Model\CardTable::getById($bizCardId);
...
{
	if($_SERVER['REQUEST_METHOD'] == 'POST' && check_thurly_sessid() && $arResult["IS_EDIT_PERMITTED"])
	{
...
ob_start(); //phone
$APPLICATION->IncludeComponent($arParams["RESTRICTED_MODE"] ? 'thurly:crm.field_multi.view' : 'thurly:crm.field_multi.edit', 'mobile',
	array(
		'FM_MNEMONIC' => 'CONFM',
		'ENTITY_ID' => 'CONTACT',
...
if (CCrmCompany::CheckReadPermission(0, $userPermissions))
{
	...
	$companyIds = \Thurly\Crm\Binding\EntityBinding::prepareEntityIDs(CCrmOwnerType::Company, $companyBindings);
...
if ($arParams["RESTRICTED_MODE"])
{
	$addressHtml =  Thurly\Crm\Format\ContactAddressFormatter::format(
		$arResult['ELEMENT'],
		array('SEPARATOR' => Thurly\Crm\Format\AddressSeparator::HtmlLineBreak, 'NL2BR' => true)
	);
}
else
{
...
$arResult['FIELDS'][] = array(
	...
	'value' => isset($arResult['ELEMENT']['OPENED'])
		? $arResult['ELEMENT']['OPENED'] : \Thurly\Crm\Settings\ContactSettings::getCurrent()->getOpenedFlag()
);
