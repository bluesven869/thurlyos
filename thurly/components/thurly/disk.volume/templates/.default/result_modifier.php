...
\Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/main/public_tools.js');
\Thurly\Main\Page\Asset::getInstance()->addJs('/thurly/js/main/popup_menu.js');
...
if (!isset($diskTitleFormat) || !is_callable($diskTitleFormat))
{
	...
	$diskTitleFormat = function (&$row, &$storage) use ($arParams, $arResult, $component, $linkStorageReload)
	{
		...
		if ($entityType == \Thurly\Disk\ProxyType\User::className())
		{
			...
			if ($row['PICTURE'] == \Thurly\Disk\Ui\Avatar::getDefaultPerson())
			{
				unset($row['PICTURE']);
			}
		...
		elseif ($entityType == \Thurly\Disk\ProxyType\Group::className())
		{
			...
			if ($row['PICTURE'] == \Thurly\Disk\Ui\Avatar::getDefaultGroup())
			{
				unset($row['PICTURE']);
			}
...
if (!isset($folderTitleFormat) || !is_callable($folderTitleFormat))
{
	...
	$folderTitleFormat = function (&$row, &$storage) use ($arParams, $arResult, $component, $linkStorageReload, $linkFolderReload)
	{
		...
		if (in_array($entityType, \Thurly\Disk\Volume\Module\Im::getEntityType()))
		{
			...
			if ($specific['chat'])
			{
				...
				$titleFormat =
					...
					'<br><span class="disk-volume-folder-parent">'.
						Loc::getMessage('DISK_VOLUME_IM_AUTHOR').': '. $specific['chat']['owner_name'].
					'</span>'.
					'</div>';
...
if (!isset($fileTitleFormat) || !is_callable($fileTitleFormat))
{
	...
	$fileTitleFormat = function (&$row, &$storage) use ($arParams, $arResult, $component)
	{
		...
		if (
			...
			$fragment->getIndicatorType() == \Thurly\Disk\Volume\FileDeleted::className()
		)
		{
			$iconClass = 'bx-disk-file-icon';
			$dataAttributesForViewer = '';

			/** @var \Thurly\Disk\File $file */
			$file = $fragment->getFile();
			if ($file instanceof \Thurly\Disk\File)
			{
				$iconClass = \Thurly\Disk\Ui\Icon::getIconClassByObject($file);
				$dataAttributesForViewer = \Thurly\Disk\Ui\Viewer::getAttributesByObject($file);
			}
...
if (!isset($fileUsingCountTitle) || !is_callable($fileUsingCountTitle))
{
	...
	$fileUsingCountTitle = function (&$row) use ($arParams, $arResult, $component, $USER)
	{
		...
		)
		{
			...
			/** @var \Thurly\Disk\Storage $storage */
			...
			if ($row['USING_COUNT'] > 0)
			{
				...
				if ($file instanceof \Thurly\Disk\File)
				{
					...
					{
						/** @var \Thurly\Disk\Volume\File $indicator */
...
/** @var \Thurly\Disk\Storage $currentStorage */
$currentStorage = null;
...
if (!empty($arResult['Storage']['LIST'][0]))
{
	do
	{
		...
		if (!$fragment instanceof \Thurly\Disk\Volume\Fragment)
		{
			break;
		}
		...
		if (
			...
			$arResult['Storage']['PICTURE'] == \Thurly\Disk\Ui\Avatar::getDefaultPerson()
		)
		{
		...
		if (
			...
			isset($arResult['Storage']['PICTURE']) &&
			$arResult['Storage']['PICTURE'] == \Thurly\Disk\Ui\Avatar::getDefaultGroup()
		)
		{
			unset($arResult['Storage']['PICTURE']);
		}
...
switch ($arResult['ACTION'])
{
	case $component::ACTION_DISKS:
	{
		...
		foreach ($arResult[$resId]['LIST'] as &$row)
		{
			...
			// trashcan filter id
			$trashcanFilterId = -1;
			if ($indicator::getIndicatorId() == \Thurly\Disk\Volume\Storage\TrashCan::getIndicatorId())
			{
	...
	{
		$resId = $arResult['INDICATOR'];
		$isTrashcan = ($arResult['INDICATOR'] === \Thurly\Disk\Volume\FileDeleted::getIndicatorId());
