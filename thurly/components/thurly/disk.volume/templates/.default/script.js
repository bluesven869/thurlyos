...
BX.Disk.MeasureClass = (function ()
{
	...
	var MeasureClass = function (param)
	{
		...
		{
			throw new Error("BX.Disk.Measure: 'relUrl' parameter missing.");
		}

		this.amount = {};

		this.ajaxUrl = param.ajaxUrl || '/thurly/components/thurly/disk.volume/ajax.php';
		this.relUrl = param.relUrl;
		this.filterId = param.filterId;
		this.restrictStorageId = param.restrictStorageId;
	...
	MeasureClass.prototype.ajaxAuxParams = function ()
	{
		...
		if(!!this.restrictStorageId)
		{
			ret.restrictStorageId = this.restrictStorageId;
		}
		if(!!this.storageId)
		{
				...
				if(!!response.status && response.status === "success")
				{
					...
					if(!!totalUnnecessary || !!totalUnnecessaryFormat)
					{
						if (!!response.DROP_UNNECESSARY_VERSION_FORMAT)
						{
							if(!!totalUnnecessary)
							{
								if(response.DROP_UNNECESSARY_VERSION > 0)
								{
