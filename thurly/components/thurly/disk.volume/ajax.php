...
class DiskVolumeController extends \Thurly\Disk\Internals\Controller
{
	...
	{
		$this->relativePath = $this->request->get('relUrl');

		if (!is_null($this->request->get('restrictStorageId')) && (int)$this->request->get('restrictStorageId') > 0)
		{
			$this->component->arParams['STORAGE_ID'] = (int)$this->request->get('restrictStorageId');
		}

		if (!is_null($this->request->get('sefMode')))
		{
			$this->sefMode = (bool)($this->request->get('sefMode') === 'Y');
		}
	...
	protected function processBeforeAction($actionName)
	{
		...
		)
		{
			...
			$this->storage = \Thurly\Disk\Storage::loadById($this->storageId);
		...
		)
		{
			...
			$this->folder = \Thurly\Disk\Folder::loadById($this->folderId);
		...
		)
		{
			...
			$this->file = \Thurly\Disk\File::loadById($this->fileId);
			if (!$this->file instanceof \Thurly\Disk\File)
			{
	...
	protected function processActionSendNotification()
	{
		...
		$filterList = \Thurly\Disk\Internals\VolumeTable::getList(array(
			'filter' => array(
				'=ID' => $filterIds,
		...
		foreach ($filterList as $filter)
		{
			...
			if (!($fragment->getStorage() instanceof \Thurly\Disk\Storage))
			{
				continue;
			}
			...
			if ($indicator instanceof Volume\IVolumeIndicatorStorage)
			{
				...
				elseif ($indicator instanceof Volume\Storage\Group || $fragment->getStorage()->getEntityType() === ProxyType\Group::className())
				{
					...
					$groupTo = \Thurly\Socialnetwork\WorkgroupTable::getById($groupIdTo)->fetch();
			...
			if ($userIdTo > 0 && $fileSize > 0)
			{
				...
				\Thurly\Disk\Driver::getInstance()->sendNotify(
					$userIdTo,
					array(
	...
	protected function processActionMeasureStorage()
	{
		...
		do
		{
			...
			*/
			...
			if ($this->filterId > 0)
			{
				\Thurly\Disk\Internals\VolumeTable::update($this->filterId, array('COLLECTED' => 1));
			}
		}
		while(false);
	...
	private function getSecurityContextByObject($object)
	{
		...
		if (!($securityContextCache[$object->getStorageId()] instanceof \Thurly\Disk\Security\SecurityContext))
		{
			if (\Thurly\Disk\User::isCurrentUserAdmin())
			{
				$securityContextCache[$object->getStorageId()] = new \Thurly\Disk\Security\FakeSecurityContext($this->getUser());
			}
			else
			{
