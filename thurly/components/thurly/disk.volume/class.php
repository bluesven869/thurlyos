...
use Thurly\Disk\Internals\VolumeTable;
...
class CDiskVolumeComponent extends BaseComponent
{
	...
	public function isAdminModeAllowed()
	{
		...
		{
			$isUserAdmin = \Thurly\Disk\User::isCurrentUserAdmin();
		}
		...
		$filter = array(
			...
			array(
				...
				array(
					...
					'<CREATE_TIME' => new \Thurly\Main\Type\DateTime(date('Y-m-d H:i:s', strtotime('-1 days')), 'Y-m-d H:i:s'),
				),
			),
		);
		if (!$this->isAdminMode())
	...
	protected function processBeforeAction($actionName)
	{
		...
		if ($this->indicatorId !== '')
		{
			...
			catch (\Thurly\Main\ObjectException $ex)
			{
		...
		{
			$this->arResult['WORKER_COUNT'] = \Thurly\Disk\Volume\Cleaner::checkRestoreWorkers($this->getUser()->getId());
		}
		else
		{
			$this->arResult['WORKER_COUNT'] = \Thurly\Disk\Volume\Cleaner::countWorker($this->getUser()->getId());
		}
	...
	protected function processActionDefault()
	{
		try
		{
			...
			if ($this->isOnlySpecificStorage() || !$this->isAdminMode())
			{
				...
				catch (\Thurly\Main\SystemException $exception)
				{
	...
	private function getFilterDefinition($action)
	{
		...
		elseif ($action === self::ACTION_FILES)
		{
			...
			$fileTypes = \Thurly\Disk\TypeFile::getListOfValues();
			$items = array();
			foreach ($fileTypes as $fileTypeId)
			{
				switch ($fileTypeId)
				{
					case \Thurly\Disk\TypeFile::PDF:
					case \Thurly\Disk\TypeFile::KNOWN:
					case \Thurly\Disk\TypeFile::UNKNOWN:
						break;
					default:
						$items[$fileTypeId] = \Thurly\Disk\TypeFile::getName($fileTypeId);
				}
			}
			$result['TYPE_FILE'] =
				array(
	...
	private function getFilter($action)
	{
		...
		$filterOptions = new \Thurly\Main\UI\Filter\Options($this->arResult['FILTER_ID'], $filterPresetList);
	...
	protected function processActionDisks()
	{
		...
		$gridOptions = new \Thurly\Main\Grid\Options($this->arResult['GRID_ID'], $this->arResult['FILTER_PRESETS']);
		...
		try
		{
			...
			if ($cursor instanceof \Thurly\Main\DB\Result && $cursor->getSelectedRowsCount() > 0)
			{
				$this->arResult['DATA_COLLECTED'] = true;
	...
	protected function processActionStorage()
	{
		...
		// im storage
		$isImStorage = in_array($this->storage->getEntityType(), \Thurly\Disk\Volume\Module\Im::getEntityType());
		if ($isImStorage === true)
		{
			$imIndicator = new \Thurly\Disk\Volume\Module\Im();
		...
		if ($this->reload)
		{
			try
			{
				$row = \Thurly\Disk\Internals\VolumeTable::getByPrimary($this->filterId, array('select' => array('INDICATOR_TYPE')))->fetch();
	...
	protected function processActionFiles()
	{
		...
		if (isset($filter['TYPE']) && in_array(\Thurly\Disk\TypeFile::DOCUMENT, $filter['TYPE']))
		{
			$filter['TYPE'][] = \Thurly\Disk\TypeFile::PDF;
			$filter['TYPE'][] = \Thurly\Disk\TypeFile::KNOWN;
		}
	...
	public function purify($indicatorType, $filter = array(), $filterId = -1)
	{
		...
		{
			/** @var \Thurly\Disk\Volume\IVolumeIndicator $indicator */
	...
	public function measure($indicatorType, $filter = array(), $filterId = -1)
	{
		...
		if ($indicator instanceof \Thurly\Disk\Volume\IVolumeIndicatorModule)
		{
		...
		if ($indicator instanceof \Thurly\Disk\Volume\IVolumeTimeLimit)
		{
	 ...
	 *
	 * @return String Status code STATUS_SUCCESS | STATUS_ERROR | STATUS_TIMEOUT.
	 * @throws \Thurly\Main\ObjectException
	 */
	 ...
	 *
	 * @return \Thurly\Main\DB\Result|null
	 * @throws \Thurly\Main\ObjectException
	 */
	private function loadMeasurementResult($resId, $indicatorId, $filter = array(), $order = array(), $pagination = array(), $recalculatePercent = false)
	{
		...
		if ($indicator->getTotalCount() > 0 || $indicator instanceof \Thurly\Disk\Volume\File)
		{
			$this->arResult['INDICATOR_COLLECTED'][$indicatorId] = true;
			...
			foreach ($result as $row)
			{
				...
				// count workers
				if (\Thurly\Disk\Volume\Task::isRunningMode($row['AGENT_LOCK']))
				{
					$this->arResult[$resId]['WORKER_COUNT'] ++;
					if (\Thurly\Disk\Volume\Task::isRunningMode($row['DROP_UNNECESSARY_VERSION']))
					{
						$this->arResult[$resId]['WORKER_COUNT_DROP_UNNECESSARY_VERSION'] ++;
					}
					if (\Thurly\Disk\Volume\Task::isRunningMode($row['DROP_TRASHCAN']))
					{
						$this->arResult[$resId]['WORKER_COUNT_DROP_TRASHCAN'] ++;
					}
					if (\Thurly\Disk\Volume\Task::isRunningMode($row['EMPTY_FOLDER']))
					{
						$this->arResult[$resId]['WORKER_COUNT_EMPTY_FOLDER'] ++;
					}
					if (\Thurly\Disk\Volume\Task::isRunningMode($row['DROP_FOLDER']))
					{
	...
	{
		// current user storageId
		$userStorageList = \Thurly\Disk\Storage::getList(array(
			'filter' => array(
				'ENTITY_TYPE' => \Thurly\Disk\ProxyType\User::className(),
	...
	private function loadStorage()
	{
		...
		$fragment = new Volume\Fragment(array('INDICATOR_TYPE' => \Thurly\Disk\Volume\Storage\Storage::className(), 'STORAGE_ID' => $this->storageId));

		$this->storage = $fragment->getStorage();
		if (!$this->storage instanceof \Thurly\Disk\Storage)
		{
		...
		return ($this->storage instanceof \Thurly\Disk\Storage);
	}
	...
	private function loadFolder()
	{
		...
		$this->folder = \Thurly\Disk\Folder::getById($this->folderId);
		if (!$this->folder instanceof \Thurly\Disk\Folder)
		{
			throw new Main\SystemException("Could not find folder {$this->folderId}");
		}

		return ($this->folder instanceof \Thurly\Disk\Folder);
	}
	...
	private function getBreadcrumb()
	{
		if (!$this->folder instanceof \Thurly\Disk\Folder)
		{
			throw new Main\ArgumentTypeException("Fragment must be subclass of ".\Thurly\Disk\Folder::className());
		}

		$crumbs = array();

		$parents = \Thurly\Disk\CrumbStorage::getInstance()->getByObject($this->folder);
	...
	public function getActionUrl($params = array())
	{
		...
		return $this->arParams['RELATIVE_PATH'].$path.(count($params) > 0 ? '?'.http_build_query($params) : '');
	}
	...
	public function getWorkerProgressBar()
	{
		...
		return \Thurly\Main\Update\Stepper::getHtml(
			$res,
	 ...
	 *
	 * @return \Thurly\Disk\Volume\IVolumeIndicator
	 * @throws \Thurly\Main\ArgumentTypeException
	 * @throws \Thurly\Main\ObjectException
	 */
	public function getIndicatorResult(&$row)
	{
		if (!$row['_INDICATOR'] instanceof \Thurly\Disk\Volume\IVolumeIndicator)
		{
			if (!is_array($row) || empty($row['INDICATOR_TYPE']))
			{
				throw new \Thurly\Main\ArgumentTypeException('Parameter must contains an indicator class name.');
			}

			// IM folder
			if (
				$row['INDICATOR_TYPE'] === \Thurly\Disk\Volume\Folder::className() &&
				in_array($row['ENTITY_TYPE'], \Thurly\Disk\Volume\Module\Im::getEntityType())
			)
			{
				$row['_INDICATOR'] = $this->getIndicator(\Thurly\Disk\Volume\Module\Im::className());
			}
			else
			{
	 ...
	 * @param array $row Result row.
	 *
	 * @return \Thurly\Disk\Volume\Fragment
	 * @throws \Thurly\Main\ArgumentTypeException
	 * @throws \Thurly\Main\ObjectException
	 */
	public function getFragmentResult(&$row)
	{
		if (!$row['_FRAGMENT'] instanceof \Thurly\Disk\Volume\Fragment)
		{
			/** @var \Thurly\Disk\Volume\IVolumeIndicator $indicator */
	...
	public function decorateResult(&$row)
	{
		try
		{
			...
			/** \Thurly\Disk\Volume\Fragment $fragment */
			...
			// Folder | File
			if ($indicator instanceof \Thurly\Disk\Volume\IVolumeIndicatorParent)
			{
				/** @var \Thurly\Disk\Volume\IVolumeIndicatorParent $indicator */
				$row['PARENTS'] = $indicator::getParents($fragment);

				/** @var \Thurly\Disk\Volume\IVolumeIndicator $indicator */
				/** @var \Thurly\Main\Type\DateTime $dateCreate */
			...
			if ($indicator instanceof \Thurly\Disk\Volume\IVolumeIndicatorLink)
			{
				/** @var \Thurly\Disk\Volume\IVolumeIndicatorLink $indicator */
		...
		catch (\Thurly\Main\SystemException $ex)
		{
		}
	}
	...
	public function decorateResultActionUrl(&$row)
	{
		try
		{
			...
			if (
				$indicator instanceof \Thurly\Disk\Volume\Folder ||
			...
			elseif
			(
				$indicator instanceof \Thurly\Disk\Volume\Storage\TrashCan
			)
			{
				$param = array(
			...
			elseif
			(
				$indicator instanceof \Thurly\Disk\Volume\IVolumeIndicatorStorage ||
			...
			elseif ($indicator instanceof \Thurly\Disk\Volume\IVolumeIndicatorModule)
			{
				/** @var \Thurly\Disk\Volume\IVolumeIndicatorModule $indicator */
				$row['SPECIFIC'] = $this->getFragmentResult($row)->getSpecific();

				if (!$indicator instanceof \Thurly\Disk\Volume\Module\Disk)
				{
					/** @var \Thurly\Disk\Storage[] $storageList */
	...
	/**
	 * @param array $row Result row.
	 * @param \Thurly\Disk\Storage $storage |null
	...
	public function decorateStorageIcon(&$row, &$storage, $width = 50, $height = 50)
	{
		...
		if ($entityType ==  \Thurly\Disk\ProxyType\User::className())
		{
			$row['STYLE'] = 'User';

			/*
			$userId = $storage->getEntityId();
			$user = \Thurly\Main\UserTable::getByPrimary($userId, array('select' => array('PERSONAL_PHOTO')))->fetch();
			if ($user && (int)$user['PERSONAL_PHOTO'] > 0)
			{
				$row['PICTURE'] = \Thurly\Disk\Ui\Avatar::getSrc($user['PERSONAL_PHOTO'], $width, $height);
			}
			*/
			if ($storage instanceof \Thurly\Disk\Storage)
			{
				$proxyType = $storage->getProxyType();
				if ($proxyType instanceof \Thurly\Disk\ProxyType\User)
				{
					$user = $proxyType->getUser();
					if ($user instanceof \Thurly\Disk\User)
					{
		...
		elseif ($entityType == \Thurly\Disk\ProxyType\Group::className())
		{
			$row['STYLE'] = 'Group';

			/*
			$groupId = $storage->getEntityId();
			$group = \Thurly\Socialnetwork\WorkgroupTable::getByPrimary($groupId, array('select' => array('IMAGE_ID')))->fetch();
			if ($group && (int)$group['IMAGE_ID'] > 0)
			{
				$row['PICTURE'] = \Thurly\Disk\Ui\Avatar::getSrc($group['IMAGE_ID'], $width, $height);
			}
			*/
			if ($storage instanceof \Thurly\Disk\Storage)
			{
				$proxyType = $storage->getProxyType();
				if ($proxyType instanceof \Thurly\Disk\ProxyType\Group)
				{
		...
		elseif ($entityType == \Thurly\Disk\ProxyType\Common::className())
		{
			$row['STYLE'] = 'Common';
		}
		elseif (in_array($entityType, \Thurly\Disk\Volume\Module\Im::getEntityType()))
		{
			$row['STYLE'] = 'Im';
		}
	}
	...
	{
		$snippet = new \Thurly\Main\Grid\Panel\Snippet();
		...
		$applyButton = $snippet->getApplyButton(
			array(
				'ONCHANGE' => array(
					array(
						'ACTION' => \Thurly\Main\Grid\Panel\Actions::CALLBACK,
						'DATA' => array(
							array(
			...
			case self::ACTION_DISKS:
			{
				if ($this->isAdminMode())
				{
					$actionList[] = array(
						...
						'ONCHANGE' => array(
							array(
								'ACTION' => \Thurly\Main\Grid\Panel\Actions::RESET_CONTROLS
							)
						)
					);
				}
		...
		$groupActions = array(
			'GROUPS' => array(
				array(
					'ITEMS' => array(
						array(
							"TYPE" => \Thurly\Main\Grid\Panel\Types::DROPDOWN,
							"ID" => "action_button",
							"NAME" => "action_button",
							"ITEMS" => $actionList
						),
	 ...
	 *
	 * @param string $action Component action command.
	 * @param \Thurly\Disk\Storage $storage Current storage.
	 *
	 * @return array
	 */
	public function getGridMenuGroupActionsStorage($action, &$storage)
	{
		...
		if ($storage instanceof \Thurly\Disk\Storage)
		{
			$isImStorage = in_array($storage->getEntityType(), \Thurly\Disk\Volume\Module\Im::getEntityType());
		}
	...
	public function getMenuItemDiskClearance(&$row, &$storage)
	{
		...
		try
		{
			...
			{
				case \Thurly\Disk\Volume\Storage\Storage::getIndicatorId():
				case \Thurly\Disk\Volume\Storage\Common::getIndicatorId():
				case \Thurly\Disk\Volume\Storage\Group::getIndicatorId():
				case \Thurly\Disk\Volume\Storage\User::getIndicatorId():
				{
				...
				case \Thurly\Disk\Volume\Storage\TrashCan::getIndicatorId():
				{
					$action = array(
						"text" => Loc::getMessage('DISK_VOLUME_ACTION_SAFE_CLEAR'),
						'onclick' => 'BX.Disk.measureManager.openConfirm('.\CUtil::PhpToJSObject(array(
	...
	public function getMenuItemDiskSendNotification(&$row, &$storage)
	{
		...
		try
		{
			...
			switch ($entityType)
			{
				case \Thurly\Disk\ProxyType\User::className():
				case \Thurly\Disk\ProxyType\Group::className():
				{
					$action = array(
	...
	public function getFolderActionMenu(&$row, &$storage)
	{
		...
		try
		{
			/** @var \Thurly\Disk\Volume\Fragment $fragment */
			$fragment = $this->getFragmentResult($row);
			...
			if (
				($fragment->getIndicatorType() == \Thurly\Disk\Volume\Folder::className()) &&
				($storage instanceof \Thurly\Disk\Storage) 
			)
			{
				if ($this->isAllowClearStorage($storage))
				{
						...
						if ($fragment->getUnnecessaryVersionCount() <= self::SETUP_CLEANER_FILE_THRESHOLD_COUNT)
						{
							$actions[] = array(
								...
								'onclick' => 'BX.Disk.measureManager.openConfirm('.\CUtil::PhpToJSObject(array(
										...
										'indicatorId' => \Thurly\Disk\Volume\Folder::getIndicatorId(),
										'storageId' => (int)$row['STORAGE_ID'],
										'filterId' => (int)$row['ID'],
										'before' => 'BX.Disk.measureManager.showAlertSetupProcess',
									)).');'
	...
	public function getFileActionMenu(&$row, &$storage)
	{
		...
		try
		{
			...
			if (
				($fragment->getIndicatorType() == \Thurly\Disk\Volume\File::className()) &&
				($storage instanceof \Thurly\Disk\Storage)
			)
			{
				if ($this->isAllowClearStorage($storage))
	...
	public function getDeletedFileActionMenu(&$row)
	{
		...
		try
		{
			...
			if ($fragment->getIndicatorType() == \Thurly\Disk\Volume\FileDeleted::className())
			{
				$actions[] = array(
					"text" => Loc::getMessage('DISK_VOLUME_ACTION_DELETE_FILE'),
					'onclick' => 'BX.Disk.measureManager.openConfirm('. \CUtil::PhpToJSObject(array(
