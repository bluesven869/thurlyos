...
use Thurly\Lists\Field;

class ListsElementAttachedCrmComponent extends CThurlyComponent
{
	...
	protected function performGridActionDelete()
	{
		...
		foreach($newPropertyValues as $elementId => $listPropertyData)
		{
			...
			{
				$propertyObject = Thurly\Iblock\PropertyTable::getList(array(
					'select' => array('USER_TYPE'),
					'filter' => array('IBLOCK_ID'=> $this->iblockId, 'ID' => $propertyId)
				));
				if($property = $propertyObject->fetch())
				{
					if($property['USER_TYPE'] == Thurly\Crm\Integration\IBlockElementProperty::USER_TYPE)
	...
	{
		$iblockObject = Thurly\Iblock\IblockTable::getList(array(
	...
	protected function getElementIdByEntityId()
	{
		$filter = array(
			'=ACTIVE' => 'Y',
			'=USER_TYPE' => Thurly\Crm\Integration\IBlockElementProperty::USER_TYPE,
		);
		if($this->iblockId)
	...
	protected function getGridOptions()
	{
		...
		{
			$gridOptions = new Thurly\Main\Grid\Options($gridId);
	...
	protected function setRowGrid($iblockId, $elementId)
	{
		...
		$downloadFileUrl = '/thurly/components/thurly/lists.element.attached.crm/lazyload.ajax.php?&site='.SITE_ID.'&'.
			thurly_sessid_get().'&list_id=#list_id#&element_id=#element_id#&field_id=#field_id#&file_id=#file_id#';
