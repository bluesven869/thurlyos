...
else
{
	...
	else
	{
		...
		if ($_SERVER["REQUEST_METHOD"]=="POST" && (strlen($_POST["do_read"]) > 0 || strlen($_POST["do_delete"]) > 0 || (array_key_exists("do_delete_all_flag", $_POST) && $_POST["do_delete_all_flag"] == "Y")) && check_thurly_sessid())
		{
			$errorMessage = "";
		...
		$arResult["Urls"]["BanLink"] = htmlspecialcharsbx($APPLICATION->GetCurUri("userID=".$arResult["User"]["ID"]."&action=ban&".thurly_sessid_get().""));
		...
		$arImage = CSocNetTools::InitImage($arResult["User"]["PERSONAL_PHOTO"], 150, "/thurly/images/socialnetwork/nopic_user_150.gif", 150, $arResult["Urls"]["User"], $arResult["CanViewProfile"]);
