		...
		d.prototype = {
			init : function(params) {
				this.agent = BX.Uploader.getInstance({
					...
					uploadFileUrl : params["urlUpload"], //'/thurly/tools/upload.php?lang=' + BX.message("LANGUAGE_ID"),
			...
			onUploadDone : function(item, result) {
				...
				if (BX(node))
				{
					...
					file["ext"] = item.ext;
					file["preview_url"] = (item.canvas ? item.canvas.toDataURL("image/png") : "/thurly/images/1.gif");
					item.canvas = null;
					delete item.canvas;
					for (var ii in file)
					{
