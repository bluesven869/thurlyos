...
BX.Mobile.Crm.ProductEditor = {
	...
	generateProductHtml: function(product)
	{
		...
		childrenNodes.push(
			BX.create("span", {
				...
				children: [
					BX.create("img", {
						attrs: {src : "/thurly/components/thurly/crm.product_row.list/templates/mobile/images/icon-product.png"}
					})
				]
			}),
			BX.create("span", {
		...
		if (this.isEditMode) // quantity
		{
			...
			childrenNodesEdit.push(
				...
				BX.create("span", {
					...
					attrs: {"data-role": "productQuantity", className: "crm-mobile-product-view-block-item-col"}
				}),
				BX.create("span", {
	...
	changeProductQuantity : function(dataRole, type)
	{
		...
		var quantityNodes = productNode.querySelectorAll("[data-role='productQuantity']");
		if (typeof quantityNodes === "object")
		{
			var curQuantity = Number(quantityNodes[0].innerHTML);
			...
			{
				quantityNodes[i].innerHTML = curQuantity;
			}
...
if (typeof(BX.CrmProductEditor) === "undefined")
{
	...
	BX.CrmProductEditor.prototype =
	{
		...
		handleFormSubmit: function(e)
		{
			...
			var fieldSettings = BX.findChild(form, {'tag':'input', 'attr':{'type':'hidden', 'name': this.getSetting('dataFieldName', 'PRODUCT_ROW_DATA') + '_SETTINGS'}}, true, false);
			...
			if (fieldSettings)
			{
				fieldSettings.value = '{"ENABLE_DISCOUNT": "' + (this.isDiscountEnabled() ? 'Y' : 'N') + '",' +
		...
		handleChoiceProductButtonClick: function(e)
		{
			...
			var dlg = BX.CrmProductSearchDialogWindow.create(
				{
					content_url: "/thurly/components/thurly/crm.product_row.list/product_choice_dialog.php" +
		...
		_deleteProduct: function(product)
		{
			...
			if (this._products.length === 0)
			{
				BX.onCustomEvent(this, 'sumTotalChange', ["0.00"]);
		...
		_handleEntitySelectorChangeValue: function (id, type, value, entityEditor)
		{
			if (type !== "COMPANY" && type !== "CONTACT")
				return;
...
if (typeof(BX.CrmProduct) === "undefined")
{
	...
	BX.CrmProduct.prototype =
	{
				...
				case "TAX_RATE":
					if(cEdit)
					{
						...
						for(i = 0; i < cEdit.options.length; i++)
						{
							...
							else if(cEdit.options[i].value > value)
							{
								break;
							}
						}
		...
		saveSettings: function()
		{
			...
			this._settings['PRODUCT_NAME'] = BX.util.trim(this._fieldValue['PRODUCT_NAME']);
			...
			this._settings['MEASURE_NAME'] = this._fieldValue['MEASURE_NAME'];
		},
		...
		{
			var discount, discountSum, discountSubTotal, exclusivePrice;
			var discountType = this.getDiscountTypeId();
			...
			else if(fieldId === 'TAX_RATE')
			{
				...
				discount = discountType === BX.CrmDiscountType.percentage ? this._settings['DISCOUNT_RATE'] : this._settings['DISCOUNT_SUM'];
				exclusivePrice = this._round(this._calculatePrice(this._settings['PRICE_NETTO'], discount, discountType), 2);
			...
			else if(fieldId === 'TAX_INCLUDED')
			{
				...
				{
					var isTaxIncluded = this._settings['TAX_INCLUDED'] = !!this._fieldValue[fieldId];
					if(!isTaxIncluded)
					{
	...
	BX.CrmProductSearch.prototype =
	{
		initialize: function(config, product)
		{
			...
			if (this._settings["MIN_QUERY_LEN"] <= 0)
				this._settings["MIN_QUERY_LEN"] = 1;
			...
			if(BX.browser.IsSafari() || BX.browser.IsIE())
				...
				this.INPUT.onkeypress = BX.delegate(this.onKeyDown, this);
