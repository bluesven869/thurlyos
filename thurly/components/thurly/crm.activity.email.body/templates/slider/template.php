			...
			<div class="crm-task-list-mail-file-inner">
				<? foreach ($activity['__files'] as $item): ?>
					...
					<div class="crm-task-list-mail-file-item diskuf-files-entity">
						<span class="feed-com-file-icon feed-file-icon-<?=htmlspecialcharsbx(\Thurly\Main\IO\Path::getExtension($item['fileName'])) ?>"></span>
...
<form action="/thurly/components/thurly/crm.activity.editor/ajax.php?action=save_email" method="POST"
	...
	$quote = preg_replace_callback(
		'#/thurly/tools/crm_show_file\.php\?fileId=(\d+)#i',
	...
	$selectorParams = array(
		'pathToAjax'               => '/thurly/components/thurly/crm.activity.editor/ajax.php?soc_net_log_dest=search_email_comms',
		'extranetUser'             => false,
		'isCrmFeed'                => true,
...
BX.ready(function()
{
	var instance = new BXCrmActivityEmail({
		...
		rcptSelected: <?=\Thurly\Main\Web\Json::encode($rcptSelected) ?>,
		rcptAllSelected: <?=\Thurly\Main\Web\Json::encode($rcptAllSelected) ?>,
		rcptCcSelected: <?=\Thurly\Main\Web\Json::encode($rcptCcSelected) ?>,
		templates: <?=\Thurly\Main\Web\Json::encode($arParams['TEMPLATES']) ?>
	});
