...
if(!function_exists('__CrmActivityViewPrepareNameForJson'))
{
	...
	{
		if(!\Thurly\Main\Application::getInstance()->isUtfMode())
		{
			return \Thurly\Main\Text\Encoding::convertEncodingArray($string, SITE_CHARSET, 'UTF-8');
		}
		return $string;
	}
}
...
?><div id="<?=htmlspecialcharsbx($UID)?>" class="crm_wrapper">
	...
	<div class="crm_block_container">
		<div class="crm_contact_info">
			...
			<?endif;?>
			<?if($storageTypeID === \Thurly\Crm\Integration\StorageType::Disk && !empty($diskFiles)):?>
				<hr/>
				<div class="crm_files">
					<ul>
			...
			<?elseif($storageTypeID === \Thurly\Crm\Integration\StorageType::File && !empty($files)):?>
				<hr/>
				<div class="crm_files">
					<ul>
			...
			<?elseif($storageTypeID === \Thurly\Crm\Integration\StorageType::WebDav && !empty($webdavElements)):?>
				<hr/>
				<div class="crm_files">
					<ul>
