	...
	function CrmEntityTreeDrawItem($item, $params, $result)
	{
		...
		if (empty($lang))
		{
			$lang = array(
				...
				'SIP_NO_SUPPORTED' => Loc::getMessage('CRM_SIP_NO_SUPPORTED'),
			);
		}
					...
					<div class="crm-doc-info">
						<?if ($item['STATUS_ID']):
							...
							$color = $statuses['STATUS'][$item['STATUS_ID']]['COLOR'] != ''
									...
									: \Thurly\Crm\Color\LeadStatusColorScheme::getDefaultColorByStatus($item['STATUS_ID']);
							?>
						...
						<div class="crm-doc-info-progress">
							<?if ($item['STAGE_ID']):
								...
								$color = $statusGroup[$item['STAGE_ID']]['COLOR'] != ''
										...
										: \Thurly\Crm\Color\DealStageColorScheme::getDefaultColorByStage($item['STAGE_ID']);
								?>
							<div class="crm-doc-info-progressbar">
								<div class="crm-doc-info-progressbar-indikator" style="background-color: <?= $color?>; width: <?= $width?>%"></div>
							</div>
						...
						<div class="crm-doc-info-progress">
							<?if ($item['STATUS_ID']):
								...
								$color = $statuses['QUOTE_STATUS'][$item['STATUS_ID']]['COLOR'] != ''
										...
										: \Thurly\Crm\Color\QuoteStatusColorScheme::getDefaultColorByStatus($item['STATUS_ID']);
								?>
							<div class="crm-doc-info-progressbar">
								<div class="crm-doc-info-progressbar-indikator" style="background-color: <?= $color?>; width: <?= $width?>%"></div>
							</div>
				...
				<div class="crm-doc-cart<?= $selected ? ' crm-tree-active' : ''?><?= $counter == 1 ? ' crm-doc-cart-top' : ''?>">
					<div class="crm-doc-cart-info">
						<a href="<?= $item['URL']?>" target="_top" class="crm-doc-cart-title crm-doc-cart-title-invoice crm-tree-link" data-id="<?= $item['ID']?>" data-type="<?= $item['TREE_TYPE']?>"><?
							?><span class="crm-doc-gray"><?= $lang['INVOICE']?><?= $item['ACCOUNT_NUMBER']?>:</span> <?
							?><?= $item['ORDER_TOPIC']?><?
						?></a>
					...
					<div class="crm-doc-cart-param">
						<div class="crm-doc-info-progress">
							<?if ($item['STATUS_ID']):
								$name = $statuses['INVOICE_STATUS'][$item['STATUS_ID']]['NAME'];
								$width = isset($statuses['INVOICE_STATUS'][$item['STATUS_ID']]['CHUNK'])
										? round($statuses['INVOICE_STATUS'][$item['STATUS_ID']]['CHUNK'] * 100 / $statuses['INVOICE_STATUS']['__COUNT'], 2)
										: 100;
								$color = $statuses['INVOICE_STATUS'][$item['STATUS_ID']]['COLOR'] != ''
										? $statuses['INVOICE_STATUS'][$item['STATUS_ID']]['COLOR']
										: \Thurly\Crm\Color\PhaseColorScheme::getDefaultColorBySemantics(\CCrmInvoice::GetSemanticID($item['STATUS_ID']));
								?>
							<div class="crm-doc-info-progressbar">
								<div class="crm-doc-info-progressbar-indikator" style="background-color: <?= $color?>; width: <?= $width?>%"></div>
							</div>
