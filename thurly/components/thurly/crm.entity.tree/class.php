...
class CrmEntityTreeComponent extends \CThurlyComponent
{
	...
	protected function getStatuses()
	{
		...
		$colors = array(
			'QUOTE_STATUS' => (array)unserialize(\Thurly\Main\Config\Option::get('crm', 'CONFIG_STATUS_QUOTE_STATUS')),
			'INVOICE_STATUS' => (array)unserialize(\Thurly\Main\Config\Option::get('crm', 'CONFIG_STATUS_INVOICE_STATUS')),
			'STATUS' => (array)unserialize(\Thurly\Main\Config\Option::get('crm', 'CONFIG_STATUS_STATUS')),
			'DEAL_STAGE' => (array)unserialize(\Thurly\Main\Config\Option::get('crm', 'CONFIG_STATUS_DEAL_STAGE')),
		);
		foreach (\Thurly\Crm\Category\DealCategory::getList(array())->fetchAll() as $cat)
		{
			$colors['DEAL_STAGE_' . $cat['ID']] = (array)unserialize(\Thurly\Main\Config\Option::get('crm', 'CONFIG_STATUS_DEAL_STAGE_' . $cat['ID']));
		}
		...
		foreach (\CCrmStatusInvoice::GetList(array('SORT' => 'ASC'), array()) as $row)
		{
	...
	protected function getSubEntity(array $filter, $entityCode)
	{
		...
		$provider = '\CCrm'.$entityCode;
		if (isset($this->selectPresets[$entityCode]))
		{
			$select = $this->selectPresets[$entityCode];
		}
