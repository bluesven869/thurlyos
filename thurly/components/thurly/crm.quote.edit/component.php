...
$bPostChecked = ($_SERVER['REQUEST_METHOD'] === 'POST' && check_thurly_sessid());
...
if ($bPostChecked)
{
	...
	elseif(isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']) || isset($_POST['continue']))
	{
		...
		elseif(!$bEdit && !$bCopy)
		{
			$arFields['OPENED'] = \Thurly\Crm\Settings\QuoteSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N';
		}
		...
		if (empty($arResult['ERROR_MESSAGE']))
		{
			...
			if ($bSuccess)
			{
				...
				{
					\Thurly\Crm\Requisite\EntityLink::unregister(CCrmOwnerType::Quote, $arResult['ELEMENT']['ID']);
				}
			}
...
$arResult['FIELDS']['tab_1'][] = array(
	...
	'value' => isset($arResult['ELEMENT']['OPENED'])
		? $arResult['ELEMENT']['OPENED'] : (\Thurly\Crm\Settings\QuoteSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N'),
...
$arResult['FIELDS']['tab_1'][] = array(
	...
	'componentParams' => array(
		...
		'SERVICE_URL' => '/thurly/components/thurly/crm.quote.edit/ajax.php?'.thurly_sessid_get(),
...
else
{
	...
	$fileViewer = new \Thurly\Crm\UserField\FileViewer(CCrmOwnerType::Quote, $arResult['ELEMENT']['ID']);
}
