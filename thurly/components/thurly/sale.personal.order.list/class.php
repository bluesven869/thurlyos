...
use Thurly\Main,
	...
	Thurly\Main\Data,
	Thurly\Sale,
	Thurly\Sale\OrderStatus,
	Thurly\Sale\Cashbox\CheckManager;

if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die();

class CThurlyPersonalOrderListComponent extends CThurlyComponent
{
	...
	protected function processRequest()
	{
		...
		$tableFieldNameList = Thurly\Sale\Order::getAllFields();
		...
		if ($this->sortBy == 'STATUS')
		{
			$getListParams['runtime'] = array(
				new \Thurly\Main\Entity\ReferenceField(
					'STATUS',
					'\Thurly\Sale\Internals\StatusTable',
					array(
		...
		else
		{
			...
			$countQuery = Sale\Order::getList(
				array(
					...
					"select"=>array(new \Thurly\Main\Entity\ExpressionField('CNT', 'COUNT(1)'))
				)
			);
