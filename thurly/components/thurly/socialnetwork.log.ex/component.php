...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/components/thurly/socialnetwork.log.ex/include.php");
...
$pathToSmile = ($pathToSmile ? $pathToSmile : "/thurly/images/socialnetwork/smile/");
...
if (is_object($parent) && strlen($parent->__name) > 0)
{
		...
		&& CModule::IncludeModule("iblock"))
	{
		...
		$cache_dir = '/thurly/sonet_group_photo_iblock';
...
)
{
	...
	{
		$filtered = false;
		$filterOption = new \Thurly\Main\UI\Filter\Options($arResult["FILTER_ID"]);
		...
		if (!empty($filterContent))
		{
			...
			$operation = \Thurly\Socialnetwork\LogIndexTable::getEntity()->fullTextIndexEnabled("CONTENT") ? '*' : '*%';
			$arFilter[$operation."CONTENT"] = \Thurly\Socialnetwork\Item\LogIndex::prepareToken($filterContent);
		}
	...
	elseif (SITE_TEMPLATE_ID === 'thurlyos')
	{
		$filterOption = new \Thurly\Main\UI\Filter\Options($arResult["FILTER_ID"]);
		$filterOption->reset();
	}
	...
	if ($arParams["SET_LOG_PAGE_CACHE"] == "Y")
	{
		...
		$rsLogPages = \Thurly\Socialnetwork\LogPageTable::getList(array(
			'order' => array(),
			'filter' => array(
	...
	{
		if ($contentId = \Thurly\Socialnetwork\Livefeed\Provider::getContentId($eventFields))
		{
	...
	$arResult["ContentViewData"] = (!empty($contentIdList)
		? \Thurly\Socialnetwork\Item\UserContentView::getViewData(array(
	...
	)
	{
		...
		$rs = \Thurly\Socialnetwork\LogViewTable::getList(array(
			'order' => array(),
			'filter' => array(
				"USER_ID" => $arResult["currentUserId"],
				"EVENT_ID" => 'tasks'
	...
	)
	{
		...
		{
			$arResult["EXPERT_MODE_SET"] = \Thurly\Socialnetwork\LogViewTable::checkExpertModeAuto($arResult["currentUserId"], $tasksNum, $arParams["PAGE_SIZE"]);
	...
	else
	{
		...
		else
		{
			...
			while ($arSmile = $rsSmile->Fetch())
			{
				...
				$arSmile["IMAGE"] = "/thurly/images/socialnetwork/smile/".$arSmile["IMAGE"];
