...
<form name="configPostForm" id="configPostForm" method="POST" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
	...
	<table id="content-edit-form-config" class="content-edit-form" cellspacing="0" cellpadding="0">
		...
		<tr>
			...
			<td class="content-edit-form-field-input"><input type="text" name="site_title" value="<?=htmlspecialcharsbx(COption::GetOptionString("thurlyos", "site_title", ""));?>"  class="content-edit-form-field-input-text"/></td>
			<td class="content-edit-form-field-error"></td>
		</tr>
		...
		<?if (
			...
			$logo24show = COption::GetOptionString("thurlyos", "logo24show", "Y");
		?>
		<tr>
	...
	<!-- webdav/disk-->
		...
		<tr>
			<td class="content-edit-form-field-name content-edit-form-field-name-left">
				...
				<?if ($arResult["IS_THURLY24"] && !in_array($arResult["LICENSE_TYPE"], array("team", "company", "nfr", "demo", "edu"))):?>
					<?
					CThurlyOS::initLicenseInfoPopupJS("disk_version_limit_per_file");
					?>
			...
			<td class="content-edit-form-field-input">
				<select name="disk_version_limit_per_file" <?if ($arResult["IS_THURLY24"] && !in_array($arResult["LICENSE_TYPE"], array("team", "company", "nfr", "demo", "edu"))) echo "disabled";?>>
		...
		<tr>
			<td class="content-edit-form-field-name content-edit-form-field-name-left">
				...
				<?if ($arResult["IS_THURLY24"] && !in_array($arResult["LICENSE_TYPE"], array("team", "company", "nfr", "demo", "edu"))):?>
					<?
					CThurlyOS::initLicenseInfoPopupJS("disk_allow_use_external_link");
					?>
		...
		<tr>
			<td class="content-edit-form-field-name content-edit-form-field-name-left">
				...
				<?if ($arResult["IS_THURLY24"] && !in_array($arResult["LICENSE_TYPE"], array("team", "company", "nfr", "demo", "edu"))):?>
					<?
					CThurlyOS::initLicenseInfoPopupJS("disk_object_lock_enabled");
					?>
			...
			<td class="content-edit-form-field-input">
				<input type="checkbox" <?if ($arResult["IS_THURLY24"] && !in_array($arResult["LICENSE_TYPE"], array("team", "company", "nfr", "demo", "edu"))) echo "disabled";?> id="disk_object_lock_enabled" name="disk_object_lock_enabled" <?if (COption::GetOptionString("disk", "disk_object_lock_enabled", "N") == "Y"):?>checked<?endif?> class="content-edit-form-field-input-selector"/>
			</td>
			<td class="content-edit-form-field-error"></td>
		</tr>
	...
	<!-- im general chat right-->
		...
		<tr>
			<td class="content-edit-form-field-name content-edit-form-field-name-left">
				...
				if($arResult["IS_THURLY24"] && !$arResult['CREATOR_CONFIRMED']):
				?>
				...
				elseif ($arResult['ALLOW_NETWORK_CHANGE'] === 'N'):
					...
					CThurlyOS::initLicenseInfoPopupJS("network_available");
				?>
	...
	?>
		...
		<tr>
			...
			<td class="content-edit-form-field-input" colspan="2">
				<a href="javascript:void(0)" onclick="BX.ThurlyOS.renamePortal()"><?=GetMessage('CONFIG_NAME_CHANGE_ACTION')?></a>
			</td>
		</tr>
		<tr>
			<td colspan="3">
	...
	?>
		...
		<tr>
			...
			<td class="content-edit-form-field-input" colspan="2">
				...
				<input class="content-edit-form-field-input-text" name="google_api_key" value="<?=\Thurly\Main\Text\HtmlFilter::encode($arResult['GOOGLE_API_KEY'])?>">
			</td>
		</tr>
<?
				...
				<td colspan="3">
					<div class="config_notify_message" style="margin: 10px 20px 10px 20px">
						<?=GetMessage("CONFIG_NAME_GOOGLE_API_HOST_HINT", array(
							'#domain#' => \Thurly\Main\Text\HtmlFilter::encode($arResult['GOOGLE_API_KEY_HOST'])
						))?>
					</div>
				</td>
			</tr>
...
<table class="content-edit-form" cellspacing="0" cellpadding="0">
	<tr>
		<td class="content-edit-form-buttons" style="border-top: 1px #eaeae1 solid; text-align:center">
			<span onclick="BX.ThurlyOS.Configs.Functions.submitForm(this)" class="webform-button webform-button-create">
...
<!-- logo -->
<?
if ($arResult["IS_THURLY24"] && in_array($arResult["LICENSE_TYPE"], array("team", "company", "nfr", "edu", "demo")) || !$arResult["IS_THURLY24"])
{
	$clientLogoID = COption::GetOptionInt("thurlyos", "client_logo", "");
?>
<table id="content-edit-form-config" class="content-edit-form" cellspacing="0" cellpadding="0">
	<tr>
		<td class="content-edit-form-header" colspan="3" >
...
{
?>
	<form id="thurlyos-otp-tell-about-form" style="display: none" action="/thurly/urlrewrite.php?SEF_APPLICATION_CUR_PAGE_URL=<?=str_replace("%23", "#", urlencode($arParams["CONFIG_PATH_TO_POST"]))?>" method="POST">
	...
	<script>
		BX.ready(function(){
			var popup = BX.PopupWindowManager.create("thurlyosOtpImportant", null, {
				autoHide: true,
				offsetLeft: 0,
				offsetTop: 0,
				overlay : true,
				...
				content: BX("thurlyos-otp-tell-about-form"),
				buttons: [
					new BX.PopupWindowButton({
						...
						events: {
							click: function() {
								BX.submit(BX("thurlyos-otp-tell-about-form"), 'dummy')
							}
						}
					}),
...
<script>
	...
	var B24ConfigsLogo = new BX.ThurlyOS.Configs.LogoClass("<?=CUtil::JSEscape(POST_FORM_ACTION_URI)?>");

	BX.ready(function(){
		BX.ThurlyOS.Configs.Functions.init();
	});

	<?if (in_array($arResult["LICENSE_TYPE"], array("team", "company", "nfr", "edu"))):?>
	var B24ConfigsIpObj = new BX.ThurlyOS.Configs.IpSettingsClass(<?=CUtil::PhpToJSObject(array_keys($arCurIpRights))?>);
	<?endif?>

	var B24ConfigsLiveFeedObj = new BX.ThurlyOS.Configs.LiveFeedRightClass(<?=CUtil::PhpToJSObject($arToAllRights)?>);

	var B24ImChatObj = new BX.ThurlyOS.Configs.ImGeneralChatClass(<?=CUtil::PhpToJSObject($arChatToAllRights)?>);
</script>