...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/components/thurly/socialnetwork.log.entry/include.php");
...
if(CModule::IncludeModule("socialnetwork"))
{
	...
	elseif ($action == "add_comment")
	{
		...
		)
		{
			...
			if ($arCommentEvent)
			{
				$bCanAddComments = \Thurly\Socialnetwork\ComponentHelper::canAddComment($arLog, $arCommentEvent);
...
//					$parser = new logTextParser(LANGUAGE_ID, $arParams["PATH_TO_SMILE"]);
					...
					if (strlen($comment_text) > 0)
					{
						...
						$inlineTagList = \Thurly\Socialnetwork\Util::detectTags($arFields, array("MESSAGE"));
						...
						)
						{
							...
							{
								$res = \Thurly\Socialnetwork\ComponentHelper::addLiveComment(
									$arComment,
									$arLog,
									$arCommentEvent,
									array(
		...
		)
		{
			...
			$res = $APPLICATION->IncludeComponent(
				...
				array(
					...
					"ENTITY_XML_ID" => $entity_xml_id,
					"RECORDS" => $records,
					"NAV_STRING" => '/thurly/components/thurly/socialnetwork.log.entry/ajax.php?'.http_build_query(array(
							"action" => 'get_comments',
							"logid" => $arLog["ID"],
					...
					"MODERATE_URL" => "",
					"DELETE_URL" => '/thurly/components/thurly/socialnetwork.log.entry/ajax.php?lang='.LANGUAGE_ID.'&action=delete_comment&delete_comment_id=#ID#&post_id='.$arLog["ID"].'&site='.SITE_ID,
	...
	?><?=\Thurly\Main\Web\Json::encode($arResult)?><?
