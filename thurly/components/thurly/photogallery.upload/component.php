...
$test_str = '/thurly/urlrewrite.php?SEF_APPLICATION_CUR_PAGE_URL=';
if (strncmp(POST_FORM_ACTION_URI, $test_str, 52) === 0)
{
	...
	$arParams["ACTION_URL"] = htmlspecialcharsbx("/thurly/urlrewrite.php?SEF_APPLICATION_CUR_PAGE_URL=".urlencode($sUrlPath));
}
else
{
...
{
	if (isset($_REQUEST['watermark_iframe']) && $_REQUEST['watermark_iframe'] == 'Y' && check_thurly_sessid())
	{
		$UploadError = false;
		$pathto = '';
...
require_once($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/main/classes/general/uploader.php");
...
/***************** URL *********************************************/
	...
	$arParams["REDIRECT_URL"] = CHTTP::urlDeleteParams($arParams["REDIRECT_URL"], array("clear_cache", "thurly_include_areas", "thurly_show_mode", "back_url_admin", "bx_photo_ajax", "change_view_mode_data", "sessid", "uploader_redirect"));
	$arParams["REDIRECT_URL"] .= (strpos($arParams["REDIRECT_URL"], "?") === false ? "?" : "&")."uploader_redirect=Y&sessid=".thurly_sessid();

	$arParams["SIMPLE_FORM_URL"] = $APPLICATION->GetCurPageParam("view_mode=form&".thurly_sessid_get(), array("view_mode", "sessid", "uploader_redirect"));
	$arParams["MULTIPLE_FORM_URL"] = $APPLICATION->GetCurPageParam("view_mode=applet&".thurly_sessid_get(), array("view_mode", "sessid", "uploader_redirect"));
...
if ($_REQUEST["uploader_redirect"] == "Y" && check_thurly_sessid())
{
