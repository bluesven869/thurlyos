	...
	switch ($_POST["action"])
	{
		case "set_certificate":
			...
			if ($_POST["type"] == "le")
			{
				...
				$action = \Thurly\Scale\ActionsData::getActionObject("CERTIFICATE_LETS_ENCRYPT_CONF", "", $certificateParams);
				$result = $action->start();
			}
			elseif ($_POST["type"] == "self")
			{
				...
				$action = \Thurly\Scale\ActionsData::getActionObject("CERTIFICATE_SELF_CONF", "", $certificateParams);
				$result = $action->start();
			}
		...
		case "check_state":
			$result = \Thurly\Scale\ActionsData::checkRunningAction();
			if (empty($result))
			{
