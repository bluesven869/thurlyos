...
<?if(!empty($arResult["TOP_CATEGORIES_LIST"]) && is_array($arResult["TOP_CATEGORIES_LIST"])):?>
	...
	<div id="<?=$arResult["VARIATIONS_BLOCK_ID"]?>" style="display:<?=(strlen($arResult["EBAY_CATEGORY_ID"]) <= 0 || empty($arResult["EBAY_CATEGORY_VARIATIONS"]) ? 'none' : 'block')?>;">
		<?foreach($arResult["VARIATIONS_VALUES"] as $ebayVariationId => $thurlyPropId):?>
			<div>
				...
				<select name="<?=$arParams["THURLY_CATEGORY_PROPS_SN"]?>[]">
					<option value=""></option>
					<option value="">------------------</option>
					<option value=""><?=Loc::getMessage("SALE_EBAY_SEC_CATEGORY_PROPS")?></option>
					<option value="">------------------</option>
					<?foreach ($arResult["CATEGORY_PROPS"] as $propId => $prop):?>
						<option value="<?=htmlspecialcharsbx($propId)?>"<?=($thurlyPropId == $propId ? ' selected' : '')?>><?=htmlspecialcharsbx($prop["NAME"])?></option>
					<?endforeach;?>
	...
	<script type="text/javascript">
		...
		BX.ready(function() {
			BX.Sale.EbayCategories.init({
				...
				thurlyPropsSelectName: "<?=$arParams["THURLY_CATEGORY_PROPS_SN"]?>",
				thurlyCategoryId: "<?=$arResult["THURLY_CATEGORY_ID"]?>",
				iBlockId: "<?=$arResult["IBLOCK_ID"]?>",
				siteId: "<?=$arResult["SITE_ID"]?>"
			});

			BX.addCustomEvent('onIblockPropertyAdded', function(params) {
				BX.Sale.EbayCategories.linkPropertyToCategory(<?=$arResult["THURLY_CATEGORY_ID"]?>, params.propertyId);
			});
		});
	</script>
<?else:?>
	<?=Loc::getMessage(
		...
		array(
			'#A1#' => '<a href="/thurly/admin/sale_ebay_exchange.php?lang='.LANGUAGE_ID.'&tabControl_active_tab=ebay_meta">',
			'#A2#' => '</a>'
		))?>
<?endif;?>