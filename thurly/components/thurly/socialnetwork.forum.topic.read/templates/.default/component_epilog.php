...
if ($arParams['AJAX_POST'] == 'Y' && $arParams['ACTION'] == 'REPLY')
{
	...
	if ($_POST["MESSAGE_MODE"] != "VIEW")
	{
		...
		else 
		{
			...
			{
				$fname = $_SERVER["DOCUMENT_ROOT"]."/thurly/components/thurly/forum.interface/templates/spoiler/script.js";
				if (file_exists($fname))
					$JSResult['message'] =
						'<script src="/thurly/components/thurly/forum.interface/templates/spoiler/script.js?'.filemtime($fname).'" type="text/javascript"></script>'.
							"\n".$JSResult['message'];
			}
	...
	if ($request->getPost("dataType") == "json")
	{
		...
		echo \Thurly\Main\Web\Json::encode($JSResult);
