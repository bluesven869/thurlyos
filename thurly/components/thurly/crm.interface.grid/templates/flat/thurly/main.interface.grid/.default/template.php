...
$APPLICATION->AddHeadScript('/thurly/js/crm/interface_grid.js');
...
<table cellspacing="0" class="bx-crm-interface-grid<?if($arResult["OPTIONS"]["theme"] <> '') echo " bx-crm-interface-grid-theme-".$arResult["OPTIONS"]["theme"]?>" id="<?=$arParams["GRID_ID"]?>">
	<thead>
...
<?if($arResult["ALLOW_EDIT"]):?>
		<td class="bx-crm-checkbox-col" width="1%"><input type="checkbox" name="" id="<?=$arParams["GRID_ID"]?>_check_all" value="" title="<?echo GetMessage("interface_grid_check_all")?>" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.SelectAllRows(this);"></td>
<?endif?>
		<td class="bx-crm-actions-col" width="1%"><a draggable="false" href="javascript:void(0);"
			...
			title="<?echo GetMessage("interface_grid_settings")?>" class="bx-crm-action"><div class="empty"></div></a></td>
<?
...
?>
	<tr class="bx-crm-table-body" oncontextmenu="return bxGrid_<?=$arParams["GRID_ID"]?>.oActions[<?=$index?>]"<?if($sDefAction <> ''):?> ondblclick="<?=htmlspecialcharsbx($sDefAction)?>"<?endif?>>
<?if($arResult["ALLOW_EDIT"]):?>
	<?
	if($aRow["editable"] !== false):
		$data_id = ($aRow["id"] <> ''? $aRow["id"] : $aRow["data"]["ID"]);
	?>
		<td class="bx-crm-checkbox-col"><input type="checkbox" name="ID[]" id="ID_<?=$data_id?>" value="<?=$data_id?>" title="<?echo GetMessage("interface_grid_check")?>"></td>
	<?else:?>
		<td class="bx-crm-checkbox-col">&nbsp;</td>
	<?endif?>
<?endif?>
	<?if(is_array($aRow["actions"]) && count($aRow["actions"]) > 0):?>
		<td class="bx-crm-actions-col"><a draggable="false" href="javascript:void(0);"
			onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ShowActionMenu(this, <?=$index?>);"
			title="<?echo GetMessage("interface_grid_act")?>" class="bx-crm-action"><div class="empty"></div></a></td>
	<?else:?>
		<td>&nbsp;</td>
	<?endif?>
<?
...
?>
	<tr><td class="bx-crm-not-result" colspan="<?=$colspan?>"><?echo GetMessage("interface_grid_no_data")?></td></tr>
<?endif?>
	</tbody>
<?if($arResult["ALLOW_EDIT"] || is_array($arParams["FOOTER"]) && count($arParams["FOOTER"]) > 0 || $arResult["NAV_STRING"] <> ''):?>
	<tfoot>
	<tr class="bx-crm-grid-footer">
		<td colspan="<?=$colspan?>">
			<table cellpadding="0" cellspacing="0" border="0" class="bx-crm-table-footer bx-crm-grid-footer">
				<tbody><tr>
			<?if($arResult["ALLOW_EDIT"]):?>
			...
			<?endforeach?>
					...
					<td class="bx-crm-right"><?=($arResult["NAV_STRING"] <> ''? $arResult["NAV_STRING"] : '&nbsp;')?></td>
				</tr></tbody>
			</table>
		</td>
	</tr>
...
<?if($arResult["ALLOW_EDIT"]):?>
<div class="bx-crm-footer-interface-toolbar-container">
<input type="hidden" name="action_button_<?=$arParams["GRID_ID"]?>" value="">
<table cellpadding="0" cellspacing="0" border="0" class="">
<!--	<tr class="bx-top"><td class="bx-crm-left"><div class="empty"></div></td><td><div class="empty"></div></td><td class="bx-crm-right"><div class="empty"></div></td></tr>-->
	<tr>
<!--		<td class="bx-crm-left"><div class="empty"></div></td>-->
<!--		<td class="bx-content">-->
...
<?if($arResult["ALLOW_INLINE_EDIT"]):?>
	<?if($bNeedSep):?>
<!--		<td><div class="bx-crm-separator"></div></td>-->
	<?endif;?>
		<td class="vam"><a href="javascript:void(0);" onclick="bxGrid_<?=$arParams["GRID_ID"]?>.ActionEdit(this);" title="<?echo GetMessage("interface_grid_edit_selected")?>" class="bx-crm-bd-icon-edit" id="edit_button_<?=$arParams["GRID_ID"]?>"></a></td>
<?
	$bNeedSep = true;
endif;
?>
...
<?endif?>
<?if($bShowApply):?>
		<td class="vam" style="padding-left:2px;"><input class="bx-crm-btn bx-crm-btn-medium bx-crm-btn-gray mb0" type="submit" name="apply" value="<?echo GetMessage("interface_grid_apply")?>" disabled></td>
<?endif?>
<!--				</tr>-->
<!--			</table>-->
<!--		</td>-->
		<td class="bx-crm-right"><div class="empty"></div></td>
	</tr>
	<tr class="bx-crm-bottom"><td class="bx-crm-left"><div class="empty"></div></td><td><div class="empty"></div></td><td class="bx-crm-right"><div class="empty"></div></td></tr>
</table>
</div>
</div>
<?endif?>
...
//Can not call Reset() - there are another controls on page can use jsDD
jsDD.Enable();
...
$managerData = isset($arParams["MANAGER"]) ? $arParams["MANAGER"] : null;
$managerConfig = is_array($managerData) && isset($managerData['CONFIG']) ? $managerData['CONFIG'] : null;
if(is_array($managerConfig)):
...
?><script type="text/javascript">
	BX.ready(
		function()
		{
			var managerId = "<?=CUtil::JSEscape($managerID) ?>";
			var manager = BX.CrmInterfaceGridManager.items[managerId];
			if(manager)
			{
			...
			{
				BX.CrmInterfaceGridManager.create(managerId, <?=CUtil::PhpToJSObject($managerConfig) ?>);
			}
		}
	);
</script>
