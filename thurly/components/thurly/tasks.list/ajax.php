...
if (check_thurly_sessid())
{
	...
	try
	{
		if (intval($_POST["id"]) > 0)
		{
			...
			if ($_POST["mode"] == "load")
			{
				...
				list($tasks, $rsTask) = CTaskItem::fetchList(
					...
					$arOrder,
					$arFilter,
					array('LOAD_PARAMETERS' => true),
					array('*', \Thurly\Tasks\Integration\CRM\UserField::getMainSysUFCode())
				);
				...
				if($bGannt)
				{
					$res = \Thurly\Tasks\Task\DependenceTable::getListByLegacyTaskFilter($arFilter);
				...
				foreach ($arTasks as $task)
				{
					...
					else
					{
						...
						$APPLICATION->IncludeComponent(
							'thurly:tasks.list.items', '.default',
			...
			{
				$oTask = CTaskItem::getInstanceFromPool($_POST['id'], \Thurly\Tasks\Util\User::getId());
				...
				else
				{
					...
					if ($_POST["mode"] == "mark" && in_array($_POST["mark"], array("NULL", "P", "N")))
					{
						...
						if ($arTask["SUBORDINATE"] == "Y" && $arTask["RESPONSIBLE_ID"] != \Thurly\Tasks\Util\User::getId() && isset($_POST["report"]))
							$arFields["ADD_IN_REPORT"] = $_POST["report"] == "true" ? "Y" : "N";
					}
					elseif ($_POST["mode"] == "report" && isset($_POST["report"]))
					{
						if ($arTask["SUBORDINATE"] == "Y" && $arTask["RESPONSIBLE_ID"] != \Thurly\Tasks\Util\User::getId())
							$arFields["ADD_IN_REPORT"] = $_POST["report"] == "true" ? "Y" : "N";
					}
		...
		elseif ($_POST["mode"] == "add" && strlen(trim($_POST["title"])) > 0 && intval($_POST["responsible"]) > 0 && in_array($_POST["priority"], array(0, 1, 2)) && \Thurly\Tasks\Util\User::isAuthorized())
		{
			$columnsOrder = null;
			...
			CMain::FinalActions(); // to make events work on thurlyos
			exit();
		}
		elseif (isset($_POST['mode']))
		{
			if ($_POST['mode'] === 'resizeColumn')
			{
				...
				$loggedInUserId  = \Thurly\Tasks\Util\User::getId();
