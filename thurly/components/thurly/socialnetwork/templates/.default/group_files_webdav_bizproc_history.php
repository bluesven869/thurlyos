...
if ($arResult["ELEMENT"]["WF_PARENT_ELEMENT_ID"] > 0)
{
	$this->__component->arResult["arButtons"]["versions"] = array(
		...
		"LINK" => CComponentEngine::MakePathFromTemplate($arResult["~PATH_TO_GROUP_FILES_ELEMENT_VERSIONS"], 
...
$this->__component->arResult["arButtons"]["delete"] = array(
	...
	"LINK" => CComponentEngine::MakePathFromTemplate($arResult["~PATH_TO_GROUP_FILES_ELEMENT_EDIT"], 
...
if ($arParams["SET_NAV_CHAIN"] != "N")
{
	...
	foreach ($arResult["NAV_CHAIN"] as $res)
	{
		...
		if (count($arNavChain) >= count($arResult["NAV_CHAIN"]))
			break; 
		$url = CComponentEngine::MakePathFromTemplate($arResult["~PATH_TO_GROUP_FILES"], 
...
?><?$APPLICATION->IncludeComponent("thurly:bizproc.document.history", "", Array(
