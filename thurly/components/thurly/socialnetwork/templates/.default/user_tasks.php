...
{
	\Thurly\Tasks\Ui\Filter\Task::setUserId($arResult[ "VARIABLES" ][ "user_id" ]);
	$state = \Thurly\Tasks\Ui\Filter\Task::listStateInit()->getState();

	switch ($state[ 'VIEW_SELECTED' ][ 'CODENAME' ])
	{
		case 'VIEW_MODE_GANTT':
			$componentName = 'thurly:tasks.task.gantt';
			break;
		case 'VIEW_MODE_PLAN':
//			case 'VIEW_MODE_KANBAN':
			$componentName = 'thurly:tasks.kanban';
			break;
		//		case 'VIEW_MODE_TIMELINE':
		//			$componentName = 'thurly:tasks.timeline';
		//			break;
		default:
			\Thurly\Tasks\Ui\Filter\Task::listStateInit()->setViewMode(\CTaskListState::VIEW_MODE_LIST);
			$componentName = 'thurly:tasks.task.list';
			break;
	}
