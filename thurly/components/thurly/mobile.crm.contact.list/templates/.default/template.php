...
$APPLICATION->IncludeComponent("thurly:mobile.interface.grid", "", array(
...
$isBizCardScanSupported = (IsModuleInstalled("bizcard") && Thurly\MobileApp\Mobile::$apiVersion>=18);
	...
	BX.ready(function(){
						...
						callback: function(buttonIndex)
						{
							...
							if(buttonIndex == 1)
							{
								BXMPager.loadPageBlank({
									url: "https://www.thurlyos.ru/prices/"
								});
							}
						}
					});
		...
		var slidingPanelParams = {
			...
			buttons:
			{
				addContact: {
					...
					iconHeight:20,
					image: "/thurly/components/thurly/mobile.crm.contact.list/images/icon-contact-plus.png",
				...
				scanCard: {
					...
					iconHeight:20,
					image: "/thurly/components/thurly/mobile.crm.contact.list/images/icon_bizcard_scan.png",
					callback: BX.proxy(function () {
		...
		var customItems = [
			<?if ($arResult["IS_CREATE_PERMITTED"]):?>
				{
					...
					image: "/thurly/js/mobile/images/plus.png",
					action: function()
					{
						if(isBizCardScanSupported)
						{
			...
			<?foreach($arResult['FILTER_PRESETS'] as $code => $preset):
				$imagePath = "/thurly/js/mobile/images/filter.png";
				if ($code == $arResult["CURRENT_FILTER"])
					$imagePath = "/thurly/js/mobile/images/select.png";
				elseif ($code == "all")
					$imagePath = "/thurly/js/mobile/images/all.png";
			?>
			{
				name: '<?=$preset["name"]?>',
				image: "<?=$imagePath?>",
