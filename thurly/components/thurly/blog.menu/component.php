	...
	if(!empty($arBlog))
	{
		...
		if(empty($arOwnBlog) || $arOwnBlog["URL"] != $arBlog["URL"])
		{
			if(IntVal($user_id)>0)
			{
				...
				{
					$arResult["urlToBecomeFriend"] = $arResult["urlToCurrentBlog"].(strpos($arResult["urlToCurrentBlog"], "?") === false ? "?" : "&")."become_friend=Y&".thurly_sessid_get();
					$arResult["SecondLine"] = "Y";
				}
				...
				if(!empty($arOwnBlog))
				{
					if (!CBlog::IsFriend($arOwnBlog["ID"], $arBlog["OWNER_ID"]))
					{
						...
						$arResult["urlToAddFriend"] = $tmpUrl.(strpos($tmpUrl, "?") === false ? "?" : "&")."add_friend[]=".UrlEncode($arBlog["URL"])."&".thurly_sessid_get();
						$arResult["SecondLine"] = "Y";
					}
				}
			}
