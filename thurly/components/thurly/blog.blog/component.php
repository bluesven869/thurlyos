...
if(strlen($arParams["BLOG_URL"]) > 0)
{
	...
	if (IntVal($_GET["del_id"]) > 0)
	{
		if($arResult["BLOG"] = CBlog::GetByUrl($arParams["BLOG_URL"], $arParams["GROUP_ID"]))
		{
			...
			{
				if (check_thurly_sessid() && CBlogPost::CanUserDeletePost(IntVal($_GET["del_id"]), $user_id))
				{
	...
	if(!empty($arResult["BLOG"]))
	{
		...
		if($arBlog["ACTIVE"] == "Y")
		{
			...
			{
				$APPLICATION->AddHeadScript("/thurly/components/thurly/player/wmvplayer/wmvscript.js");
				$APPLICATION->AddHeadScript("/thurly/components/thurly/player/wmvplayer/silverlight.js");
				$APPLICATION->AddHeadScript("/thurly/components/thurly/player/wmvplayer/wmvplayer.js");
				$APPLICATION->AddHeadScript("/thurly/components/thurly/player/mediaplayer/flvscript.js");
			...
			else
			{
				...
				if($arGroup["SITE_ID"] == SITE_ID)
				{
					...
					if($arResult["PostPerm"] >= BLOG_PERMS_READ)
					{
						...
						$blogUser = new \Thurly\Blog\BlogUser($CACHE_TIME);
						$blogUser->setBlogId($arBlog["ID"]);
						$postsUsers = $blogUser->getUsers(\Thurly\Blog\BlogUser::getPostAuthorsIdsByBlogId($arBlog["ID"]));
						...
						while($curPost = $dbPost->GetNext())
						{
							...
							while ($arImage = $res->Fetch())
							{
								...
								$curPost["arImages"][$arImage['ID']] = Array(
									...
									"full" => "/thurly/components/thurly/blog/show_file.php?fid=".$arImage['ID']
...
//								get only size for post
	...
	if($_GET["become_friend"]!="Y" && !empty($arResult["BLOG"]) && $arResult["PostPerm"] < BLOG_PERMS_READ)
	{
		...
		if($USER->IsAuthorized())
			$arResult["MESSAGE"][] = GetMessage("BLOG_BLOG_BLOG_U_CAN").' <a href="'.htmlspecialcharsbx($APPLICATION->GetCurPageParam('become_friend=Y&'.thurly_sessid_get(), Array("become_friend", "sessid"))).'">'.GetMessage("BLOG_BLOG_BLOG_U_CAN1").'</a> '.GetMessage("BLOG_BLOG_BLOG_U_CAN2");
		else
