...
//$APPLICATION->SetAdditionalCSS("/thurly/themes/.default/adminstyles.css");
$APPLICATION->SetAdditionalCSS("/thurly/themes/.default/pubstyles.css");
$APPLICATION->SetAdditionalCSS("/thurly/themes/.default/jspopup.css");
$APPLICATION->SetAdditionalCSS("/thurly/themes/.default/calendar.css");
...
$releaseDate = IsModuleInstalled('thurlyos')? GetMessage('BIZPROC_IMPORT_FILE_OLD_TEMPLATE_B24') : GetMessage('BIZPROC_IMPORT_FILE_OLD_TEMPLATE_CP');
...
function BCPProcessExport()
{
	...
	window.open('<?=CUtil::JSEscape($v)?><?if(strpos($v, "?")):?>&<?else:?>?<?endif?>action=exportTemplate&<?=thurly_sessid_get()?>');
}
...
function BCPSaveUserParams()
{
	...
	jsExtLoader.startPost('<?= CUtil::JSEscape($v) ?><?if(strpos($v, "?")):?>&<?else:?>?<?endif?><?=thurly_sessid_get()?>&action=saveAjax&saveuserparams=Y', data);
}
...
function BCPSaveTemplate(save)
{
	...
	jsExtLoader.onajaxfinish = BCPSaveTemplateComplete;
	// TODO: add sessid
	jsExtLoader.startPost('<?=CUtil::JSEscape($v)?><?if(strpos($v, "?")):?>&<?else:?>?<?endif?><?=thurly_sessid_get()?>&action=saveAjax'+
		(save ? '': '&apply=Y'),
		data);
}
...
{
	(new BX.CAdminDialog({
	'content_url': "/thurly/admin/<?=$arResult['MODULE_ID']?>_bizproc_wf_settings.php?mode=public&bxpublic=Y&lang=<?=LANGUAGE_ID?>&entity=<?=$arResult['ENTITY']?>",
...
.btn_settings {background-image:url(/thurly/images/bizproc/settings.gif);}
.btn_list {background-image:url(/thurly/images/bizproc/list.gif);}
.btn_new {background-image:url(/thurly/images/bizproc/new.gif);}
