...
$arResult['DOCUMENT_TYPE'] = \Thurly\Disk\BizProcDocument::generateDocumentType($arParams["STORAGE_ID"]);
...
if($ID > 0)
{
	...
	if($arTemplate["ENTITY"] == 'Thurly\\Disk\\BizProcDocument')
	{
		$documentType = \Thurly\Disk\BizProcDocument::generateDocumentComplexType($arParams['STORAGE_ID']);
		$arResult['ENTITY'] = $arTemplate["ENTITY"];
	}
	else
	{
		$documentType = \Thurly\Disk\BizProcDocumentCompatible::generateDocumentComplexType($arParams['STORAGE_ID']);
		$arResult['ENTITY'] = $arTemplate["ENTITY"];
	}
}
if($ID <= 0)
