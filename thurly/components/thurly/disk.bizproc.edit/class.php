...
use Thurly\Main\Web\PostDecodeFilter;

if (!defined('B_PROLOG_INCLUDED') || B_PROLOG_INCLUDED !== true) die();

Loc::loadMessages(__FILE__);

class CDiskBizprocEditComponent extends BaseComponent
{
	...
	protected function prepareParams()
	{
		...
		$this->arResult['DOCUMENT_TYPE'] = \Thurly\Disk\BizProcDocument::generateDocumentType($this->arParams['STORAGE_ID']);
	...
	protected function processBeforeAction($actionName)
	{
		...
		if($this->arResult['ID'] > 0)
		{
			...
			if($template["ENTITY"] == Thurly\Disk\BizProcDocument::className())
			{
				$this->arResult['DOCUMENT_COMPLEX_TYPE'] = \Thurly\Disk\BizProcDocument::generateDocumentComplexType($this->arParams['STORAGE_ID']);
				$this->arResult['ENTITY'] = $template['ENTITY'];
			}
			else
			{
				$this->arResult['DOCUMENT_COMPLEX_TYPE'] = \Thurly\Disk\BizProcDocumentCompatible::generateDocumentComplexType($this->arParams['STORAGE_ID']);
				$this->arResult['ENTITY'] = $template['ENTITY'];
			}
		}
		else
		{
			$this->arResult['ENTITY'] = Thurly\Disk\BizProcDocument::className();
			$this->arResult['DOCUMENT_COMPLEX_TYPE'] = \Thurly\Disk\BizProcDocument::generateDocumentComplexType($this->arParams['STORAGE_ID']);
			if(!CBPDocument::canUserOperateDocumentType(
				CBPCanUserOperateOperation::CreateWorkflow,
				$this->getUser()->getID(),
	...
	protected function processActionDefault()
	{
		...
		$documentType = \Thurly\Disk\BizProcDocumentCompatible::generateDocumentComplexType($this->arParams['STORAGE_ID']);
	...
	protected function processActionSaveAjax()
	{
		...
		if($this->request->getQuery('saveuserparams')=='Y')
		{
			...
			if (\Thurly\Main\Text\BinaryString::getLength($serializeValue) > 64000)
			{
				$response = "
					<script>
		...
		$response = "
			<script>
				...
				str_replace('#ID#', $this->arResult['ID'], $this->arResult['EDIT_PAGE_TEMPLATE']) : CUtil::JSEscape($this->arResult['LIST_PAGE_URL']))."';
			</script>
		";
		$this->sendResponse($response);
	}
