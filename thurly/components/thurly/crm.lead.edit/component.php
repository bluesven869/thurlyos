...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_thurly_sessid())
{
	...
	elseif(isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']))
	{
		...
		elseif(!$bEdit)
		{
			$arFields['OPENED'] = \Thurly\Crm\Settings\LeadSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N';
		}
		...
		if (!$bEdit)
		{
			\Thurly\Crm\Automation\Factory::runOnAdd(\CCrmOwnerType::Lead, $arResult['ELEMENT']['ID']);
		}
		elseif (isset($arFields['STATUS_ID']) && is_array($arResult['ELEMENT']) && $arFields['STATUS_ID'] != $arResult['ELEMENT']['STATUS_ID'])
		{
			\Thurly\Crm\Automation\Factory::runOnStatusChanged(\CCrmOwnerType::Lead, $arResult['ELEMENT']['ID']);
		}
		//end automation
...
$enableDupControl = $arResult['DUPLICATE_CONTROL']['ENABLED'] =
	!$bEdit && \Thurly\Crm\Integrity\DuplicateControl::isControlEnabledFor(CCrmOwnerType::Lead);
...
$arResult['FIELDS']['tab_1'][] = array(
	...
	'value' => isset($arResult['ELEMENT']['OPENED'])
		? $arResult['ELEMENT']['OPENED'] : (\Thurly\Crm\Settings\LeadSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N'),
...
if ($arResult['ELEMENT']['IS_RETURN_CUSTOMER'] != 'Y')
{
	...
	ob_start();
	$APPLICATION->IncludeComponent('thurly:crm.field_multi.edit', 'new',
		array(
			'FM_MNEMONIC' => 'LFM',
			'ENTITY_ID' => 'LEAD',
	...
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'componentParams' => array(
			'SERVICE_URL' => '/thurly/components/thurly/crm.lead.edit/ajax.php?siteID='.SITE_ID.'&'.thurly_sessid_get(),
			'DATA' => array(
