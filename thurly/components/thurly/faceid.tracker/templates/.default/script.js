...
function BXFaceIdStart(settings)
{
	...
	{
		faceTrackerSettings[i] = settings[i];
	}
...
function _BXFaceIdStart ()
{
	...
	function buildCameraList()
	{
		...
		{
			startupFailed(BX.message('FACEID_TRACKER_CMP_JS_CAMERA_ERROR'));
			return;
		}
			...
			.then(function(devices) {
				...
				devices.forEach(function(device) {
					...
					if (device.kind == "videoinput")
					{
						...
						if (!label.length)
						{
							label = BX.message('FACEID_TRACKER_CMP_JS_CAMERA_DEFAULT');
						}
	...
	function initStream()
	{
		...
		navigator.getMedia(
			...
			function(err) {
				...
				{
					msg = BX.message('FACEID_TRACKER_CMP_JS_CAMERA_NOT_FOUND');
				}
				else
				{
					msg = BX.message('FACEID_TRACKER_CMP_JS_CAMERA_NO_SUPPORT');
				}
	...
	function capture()
	{
		...
		if ($("#faceid-auto-identify").prop('checked') == false)
		{
			return;
		}
	...
	function vlGetFaces(canvas, modeAuto)
	{
		...
		if (ids.length)//photoMaker.haveFaceDetection())
		{
			for (var i = 0; i < ids.length; i++)
			{
				...
				// do not return faces that are being recognized now
				if (currentAjaxConnections.indexOf(id) == -1)
				{
					faces.push({
						frameId: id,
		...
		if (faces.length > 0)
		{
			for (i = 0; i < faces.length; i++)
			{
				//if (faces[i].width / 2 > 24)
				{
					...
					// correct detection X and Y to stay in frame
					if (sourceX + sourceWidth > sizes.snapshotWidth)
					{
						sourceX = sizes.snapshotWidth - sourceWidth;
					}

					if (sourceY + sourceHeight > sizes.snapshotHeight)
					{
						sourceY = sizes.snapshotHeight - sourceHeight;
					}
	...
	function handleNewVisitorFace(imageData, trackingFrameId, faceNotFound)
	{
		...
		if (faceNotFound)
		{
			...
			loaderCont.innerHTML = '<div class="faceid-tracker-main-user-info-name"> \
				<div class="faceid-tracker-main-user-info-name-warning">'+BX.message('FACEID_TRACKER_CMP_JS_FACE_NOT_FOUND')+'</div> \
			</div>';
		}
		else
		{
			...
			// go ajax
			BX.ajax({
				url: '/thurly/components/thurly/faceid.tracker/ajax.php',
				method: 'POST',
				data: {action: 'identify', image: imageData},
				dataType: 'json',
				processData: false,
				start: true,
				onsuccess: function (json) {
					...
					var ok = false;
					var errorMessage = BX.message('FACEID_TRACKER_CMP_JS_FACE_NOT_FOUND');
					...
					if (json.length)
					{
						...
						if (result.visitor && !isEmpty(result.visitor))
						{
							...
							visitor.image_src = visitor.shot_src;
							visitor.shot_src = imageData;

							var updateCounters = renderVisitor(newVisitorDiv, visitor);

							// remember visitor name
							if (trackingFrameId >= 0)
							{
								trackingVisitorsName[trackingFrameId] = {name: visitor.name};
							}

							// increment visitors count
							if (updateCounters)
							{
								updateVisitorsCount(visitor);
							}
						}
						...
						// update balance
						if (result.status && result.status.balance >= 0)
						{
							BX('faceid-credits-balance').innerText = result.status.balance;
						}
					}
					...
					if (!ok)
					{
						...
						var loaderCont = BX.findChild(newVisitorDiv, {class: 'faceid-tracker-main-user-info'}, true);
						loaderCont.innerHTML = '<div class="faceid-tracker-main-user-info-name"> \
							<div class="faceid-tracker-main-user-info-name-warning">'+errorMessage+'</div> \
						</div>';
					}
				},
				onfailure: function () {
	...
	function renderVisitor(newVisitorDiv, json)
	{
		...
		var html = '<div class="faceid-tracker-main-user-photo"> \
						...
						<div class="faceid-tracker-main-user-photo-count"> \
							...
							<div class="faceid-tracker-main-user-photo-count-info"> \
								<div class="faceid-tracker-main-user-photo-count-info-desc">'+BX.message('FACEID_TRACKER_CMP_JS_FACE_ORIGINAL')+'</div> \
					...
					<div class="faceid-tracker-main-user-info"> \
						...
						if (result.crm_url.length)
							html += '<a href="'+result.crm_url+'" class="faceid-tracker-main-user-info-name-item" target="_blank">'+result.name+'</a>';
						else
		...
		html += '\
						...
						if (!result.crm_url.length)
							...
							<div class="faceid-tracker-main-user-info-control-block'+(result.crm_url.length?" faceid-tracker-button-state":"")+'"> \
								<div class="webform-small-button webform-small-button-blue faceid-tracker-main-user-info-control-button">'+BX.message('FACEID_TRACKER_CMP_JS_SAVE_CRM')+' \
									<div class="faceid-tracker-user-loader"> \
										<div class="faceid-tracker-user-loader-item"> \
											<div class="faceid-tracker-loader"> \
												<svg class="faceid-tracker-circular" viewBox="25 25 50 50"> \
								...
								<div class="webform-small-button webform-small-button-transparent faceid-tracker-main-user-info-control-button">'+BX.message('FACEID_TRACKER_CMP_JS_SAVE_CRM_DONE')+'</div> \
							</div>';

		html += '\
							<div class="faceid-tracker-main-user-info-control-social"> \
								<span class="faceid-tracker-main-user-info-control-social-name">'+BX.message('FACEID_TRACKER_CMP_JS_VK_LINK')+'</span>';
								...
								else if (faceTrackerSettings.socnet_enabled)
									html += '\
									<span class="faceid-tracker-main-user-info-control-social-item"><span>'+BX.message('FACEID_TRACKER_CMP_JS_VK_LINK_ACTION')+'</span></span>';
		...
		if (!result.vk_id.length)
		{
			...
			BX.bind(searchVkButton, 'click', function () {
				...
				BX.ajax({
					url: '/thurly/components/thurly/faceid.tracker/ajax_vk.php',
					method: 'POST',
					...
					onsuccess: function (json)
					{
						...
						if (vkResult.length)
						{
							...
							countCont.innerHTML = vkResult.length + " "+BX.message('FACEID_TRACKER_CMP_JS_VK_FOUND_PEOPLE');
		...
		BX.bind(crmButton, 'click', function() {
			...
			if (!BX.hasClass(buttonParent, 'faceid-tracker-button-state') && !BX.hasClass(buttonParent, 'faceid-control-loader-state'))
			{
				...
				BX.ajax({
					url: '/thurly/components/thurly/faceid.tracker/ajax_crm.php',
					method: 'POST',
					...
					onsuccess: function (json)
					{
						...
						var nameCont = BX.findChild(newVisitorDiv, {class: 'faceid-tracker-main-user-info-name'}, true);
						nameCont.innerHTML = '<a href="'+crmResult.url+'" class="faceid-tracker-main-user-info-name-item" target="_blank">'+result.name+'</a>';
	...
	function renderVkItem(item, featured)
	{
		var html = '<div class="faceid-tracker-main-user-item faceid-tracker-profile-search-found faceid-tracker-photo'+(featured?' faceid-tracker-found-user':'')+'"> \
					...
					<div class="faceid-tracker-main-user-info"> \
						...
						<div class="faceid-tracker-main-user-info-control"> \
							<div class="webform-small-button webform-small-button-blue faceid-tracker-main-user-info-control-button '+(featured?'':'faceid-tracker-search-button')+'">'+BX.message('FACEID_TRACKER_CMP_JS_VK_SELECT')+'</div> \
						</div> \
					</div> \
				</div>';
	...
	BX.ready(function ()
	{
		BX.bind(BX('faceid-tracker-main-user-more'), 'click', function(){
			...
			BX.ajax({
				url: '/thurly/components/thurly/faceid.tracker/ajax_more.php',
				method: 'POST',
	...
	BX.bind(BX('faceid-fullscreen-button'), 'click', function ()
	{
		...
		if (BX.getFullScreenElement())
		{
			BX.cancelFullScreen();
		}
		else
		{
			BX.requestFullScreen(fsContainer);
		}
	});

	BX.bind(document, 'fullscreenchange', function ()
	{
		...
		BX.toggleClass(button, 'faceid-tracker-sidebar-photo-full-mode');
		BX.toggleClass(button, 'faceid-tracker-sidebar-photo-fullback-mode');
