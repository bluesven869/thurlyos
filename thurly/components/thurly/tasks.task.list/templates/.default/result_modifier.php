...
{
	$arParams['HEADERS']['UF_CRM_TASK'] = array(
		'id' => 'UF_CRM_TASK',
		'name' => GetMessage('TASKS_COLUMN_UF_CRM_TASK'),
		'sort' => false,
		'first_order' => 'desc',
		'editable' => false
	);
...
if (!function_exists('tasksCheckRightEditPlan'))
{
	function tasksCheckRightEditPlan($row)
	{
		...
		$can = 'N';
		if ((($row["RESPONSIBLE_ID"] == Util\User::getId()) || (in_array(Util\User::getId(), $row['ACCOMPLICES']))) &&
...
if (!function_exists('prepareTaskRow'))
{
	...
	{
		$group = \Thurly\Tasks\Integration\SocialNetwork\Group::getData(array($row['GROUP_ID']));
		...
		$resultRow = array(
			...
			'ORIGINATOR_NAME' => prepareTaskRowUserBaloonHtml(
				array(
					...
					'USER_NAME' => (\Thurly\Tasks\Util\User::getUserName($row['CREATED_BY'])),
			...
			'RESPONSIBLE_NAME' => prepareTaskRowUserBaloonHtml(
				array(
					...
					'USER_NAME' => (\Thurly\Tasks\Util\User::getUserName($row['RESPONSIBLE_ID'])),
		...
		$actionList[] = array(
			...
			'ONCHANGE' => array(
				array(
					...
					'DATA' => array(
						array(
							'TYPE' => Thurly\Main\Grid\Panel\Types::DATE,
...
if (!function_exists('prepareTaskRowUserBaloonHtml'))
{
	function prepareTaskRowUserBaloonHtml($arParams)
	{
		...
		$arParams['USER_PROFILE_URL'] = $user['IS_EXTERNAL']
			? \Thurly\Tasks\Integration\Socialnetwork\Task::addContextToURL(
...
if (!empty($arResult['LIST']))
{
	...
	{
		if ($arParams['NEED_GROUP_BY_GROUPS'] === 'Y' && $prevGroupId != $row['GROUP_ID'])
		{
		...
		$rowItem = array(
			...
			'parent_id' => \Thurly\Main\Grid\Context::isInternalRequest() ? $row['PARENT_ID'] : 0,
