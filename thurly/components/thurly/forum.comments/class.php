...
use Thurly\Forum\Internals\Error\Error;
use Thurly\Forum\Internals\Error\ErrorCollection;
use Thurly\Main\Application;
use Thurly\Main\Localization\Loc;
use Thurly\Main\UI\Uploader\ErrorCatcher;
use Thurly\Main\Web\Json;
use Thurly\Main\Config;

Loc::loadMessages(__FILE__);

final class ForumCommentsComponent extends CThurlyComponent
{
	...
	/** @var \Thurly\Forum\Comments\Feed */
	protected $feed;
	/** @var  CCaptcha */
	public $captcha;
	/** @var  array */
	...
	public function __construct($component = null)
	{
		...
		if (is_callable(array('\Thurly\MobileApp\Mobile', 'getApiVersion')) && \Thurly\MobileApp\Mobile::getApiVersion() >= 1 &&
		...
		if ((empty($templateName) || $templateName == ".default" || $templateName == "thurlyos"))
		{
			if ($this->isWeb())
	...
	protected function end($terminate = true)
	{
		...
		{
			\Thurly\Disk\Internals\Diag::getInstance()->logDebugInfo($this->getName());
		}
		if($terminate)
		{
...
/*
			if (IsModuleInstalled("disk"))
			{
				\Thurly\Disk\Internals\Diag::getInstance()
//					->setExclusiveUserId(45)
					->setEnableTimeTracker(true)
					->setMemoryBehavior(\Thurly\Disk\Internals\Diag::MEMORY_PRINT_DIFF)
					->setSqlBehavior(\Thurly\Disk\Internals\Diag::SQL_COUNT)
					->collectDebugInfo($this->componentId, $this->getName());
			}
*/
			$this->checkRequiredParams();

			$this->feed = new \Thurly\Forum\Comments\Feed(
			...
			if (IsModuleInstalled("disk"))
			{
				\Thurly\Disk\Internals\Diag::getInstance()->logDebugInfo($this->componentId, $this->getName());
			}
		}
		catch(Exception $e)
		{
	...
	public function subscribeAuthor($type, $id, $tid)
	{
		if ($this->feed->getEntity()->getType() == $type && $this->feed->getEntity()->getId() == $id)
		{
			...
			BXClearCache(true, "/thurly/forum/user/".$this->getUser()->getId()."/subscribe/");
		}
	}
