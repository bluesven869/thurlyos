	...
	ForumCommentsComponent $component)
{
	...
	if (!empty($res["FILES"]))
	{
		...
		{
			$res["FILES"][$key]["URL"] = "/thurly/components/thurly/forum.interface/show_file.php?fid=".$file["ID"];
			if (CFile::IsImage($file["SRC"], $file["CONTENT_TYPE"]))
			{
				$res["FILES"][$key]["THUMBNAIL"] = "/thurly/components/thurly/forum.interface/show_file.php?fid=".$file["ID"]."&width=90&height=90";
				$res["FILES"][$key]["SRC"] = "/thurly/components/thurly/forum.interface/show_file.php?fid=".$file["ID"];
			}
		}
	}
