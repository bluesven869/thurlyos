				...
				foreach ($arResult["RESULT"][$FIELD_SID]["ANSWER_VALUE"] as $key => $arrA)
				{
					...
					if (intval($arrA["USER_FILE_ID"])>0)
					{
							...
							if (is_array($arrResultAnswer))
							{
								foreach ($arrResultAnswer as $key => $arrAns)
								{
									...
									if (intval($arrAns["USER_FILE_ID"])>0)
									{
										...
										{
											$file_link = "/thurly/tools/form_show_file.php?rid=".$arParams["RESULT_ID"]."&hash=".$arrAns["USER_FILE_HASH"]."&lang=".LANGUAGE_ID;
						...
						else
						{
							...
							$arResultAnswer["ANSWER_FILE"]["URL"] = "/thurly/tools/form_show_file.php?rid=".$arParams["RESULT_ID"]."&hash=".$arrA["USER_FILE_HASH"]."&lang=".LANGUAGE_ID;
