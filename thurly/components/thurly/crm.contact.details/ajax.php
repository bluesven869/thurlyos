...
elseif($action === 'SAVE')
{
	...
	if(!empty($fields) || !empty($entityRequisites) || !empty($entityBankDetails))
	{
		...
		if($isCopyMode)
		{
			...
			$merger = new \Thurly\Crm\Merger\ContactMerger($currentUserID, false);
		...
		if($isNew)
		{
			...
			{
				$fields['OPENED'] = \Thurly\Crm\Settings\ContactSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N';
			}
	...
	CThurlyComponent::includeComponentClass('thurly:crm.contact.details');
