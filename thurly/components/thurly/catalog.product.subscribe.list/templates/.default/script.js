...
var unsubscribeButton = function(params)
{
	unsubscribeButton.superclass.constructor.apply(this, arguments);
	this.nameNode = BX.create('span', {
		props : { id : this.id },
...
BX.extend(unsubscribeButton, BX.PopupWindowButton);
...
window.JCCatalogProductSubscribeList = function (arParams)
{
	...
	this.ajaxUrl = '/thurly/components/thurly/catalog.product.subscribe.list/ajax.php';
...
window.JCCatalogProductSubscribeList.prototype.showPopupNotifyingUser = function()
{
	...
	this.obPopupWin.setTitleBar(this.notifyPopupTitle);
	var className;
	if(this.notifySuccess) className = 'bx-cpsl-notify-content-success';
	else className = 'bx-cpsl-notify-content-error';
	...
	this.obPopupWin.setButtons([
		new unsubscribeButton({
			text : BX.message('CPSL_BTN_MESSAGE_CLOSE'),
...
window.JCCatalogProductSubscribeList.prototype.QuantityUp = function()
{
	var curValue = 0,
		boolSet = true;
...
window.JCCatalogProductSubscribeList.prototype.QuantityDown = function()
{
	var curValue = 0,
		boolSet = true;
...
window.JCCatalogProductSubscribeList.prototype.QuantityChange = function()
{
	var curValue = 0,
		boolSet = true;
...
window.JCCatalogProductSubscribeList.prototype.QuantitySet = function(index)
{
	if (0 === this.errorCode)
	{
		if (this.showQuantity)
...
window.JCCatalogProductSubscribeList.prototype.RowLeft = function()
{
	var i = 0,
		strTreeValue = '',
		index = -1,
...
window.JCCatalogProductSubscribeList.prototype.RowRight = function()
{
	var i = 0,
		strTreeValue = '',
		index = -1,
...
window.JCCatalogProductSubscribeList.prototype.InitBasketUrl = function()
{
	switch (this.productType)
	{
		case 1://product
...
window.JCCatalogProductSubscribeList.prototype.FillBasketProps = function()
{
...
window.JCCatalogProductSubscribeList.prototype.SendToBasket = function()
{
	if (!this.canBuy)
	{
		return;
...
window.JCCatalogProductSubscribeList.prototype.Basket = function()
{
	...
	case 2://set
		...
		{
			this.InitPopupWindow();
			this.obPopupWin.setTitleBar(BX.message('CPSL_TITLE_BASKET_PROPS'));
			...
			this.obPopupWin.setButtons([
				new BasketButton({
					...
					text: BX.message('CPSL_BTN_MESSAGE_SEND_PROPS'),
					events: {
...
window.JCCatalogProductSubscribeList.prototype.BasketResult = function(arResult)
{
	var strContent = '',
		strName = '',
		strPict = '',
	...
	if (successful)
	{
		...
		buttons = [
			new BasketButton({
				ownerClass: this.obProduct.parentNode.parentNode.parentNode.className,
				text: BX.message("CPSL_BTN_MESSAGE_BASKET_REDIRECT"),
				events: {
					click: BX.delegate(function(){
						location.href = (!!this.basketData.basketUrl ? this.basketData.basketUrl : BX.message('CPSL_BASKET_URL'));
					}, this)
				}
			})
		];
	...
	{
		strContent = (!!arResult.MESSAGE ? arResult.MESSAGE : BX.message('CPSL_BASKET_UNKNOWN_ERROR'));
		buttons = [
			new BasketButton({
				ownerClass: this.obProduct.parentNode.parentNode.parentNode.className,
				text: BX.message('CPSL_BTN_MESSAGE_CLOSE'),
	...
	this.InitPopupWindow();
	this.obPopupWin.setTitleBar(BX.message('CPSL_TITLE_ERROR'));
	this.obPopupWin.setContent(strContent);
	this.obPopupWin.setButtons(buttons);
	this.obPopupWin.show();
};

window.JCCatalogProductSubscribeList.prototype.InitPopupWindow = function()
{
	if (!!this.obPopupWin)
		return;
