...
if($_SERVER['REQUEST_METHOD'] == 'GET')
{
	...
	if(isset($_GET['tag']) && isset($_GET['sender_subscription']) && $_GET['sender_subscription']=='confirm')
	{
		...
		try
		{
			$arTag = \Thurly\Main\Mail\Tracking::parseSignedTag($_REQUEST['tag']);
			if($arTag['MODULE_ID'] == 'sender' && check_email($arTag['FIELDS']['EMAIL']) && is_array($arTag['FIELDS']['MAILING_LIST']) && isset($arTag['FIELDS']['SITE_ID']))
			{
				\Thurly\Sender\Subscription::add($arTag['FIELDS']['EMAIL'], $arTag['FIELDS']['MAILING_LIST'], $arTag['FIELDS']['SITE_ID']);
		...
		catch (\Thurly\Main\Security\Sign\BadSignatureException $exception)
		{
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_thurly_sessid() && isset($_POST['sender_subscription']) && $_POST['sender_subscription']=='add')
{
	if(check_email($_POST["SENDER_SUBSCRIBE_EMAIL"], true))
	{
		...
		$mailingList = \Thurly\Sender\Subscription::getMailingList($arFilter);
		$mailingIdList = array();
		...
		if($arParams["CONFIRMATION"])
		{
			...
			if(count($mailingIdList) > 0)
			{
				...
				$subscriptionDb = \Thurly\Sender\MailingSubscriptionTable::getSubscriptionList(array(
			...
			else
			{
				...
				$contactDb = \Thurly\Sender\ContactTable::getList(array('filter' => array('=EMAIL' => strtolower($_POST["SENDER_SUBSCRIBE_EMAIL"]))));
			...
			{
				\Thurly\Sender\Subscription::sendEventConfirm($_POST["SENDER_SUBSCRIBE_EMAIL"], $mailingIdList, SITE_ID);
		...
		{
			\Thurly\Sender\Subscription::add($_POST["SENDER_SUBSCRIBE_EMAIL"], $mailingIdList, SITE_ID);
			$APPLICATION->set_cookie("SENDER_SUBSCR_EMAIL", $_POST["SENDER_SUBSCRIBE_EMAIL"], $cookieLifeTime);
			$arResult['MESSAGE'] = array('TYPE' => 'NOTE', 'CODE' => 'message_success');
			$subscr_EMAIL = $_POST["SENDER_SUBSCRIBE_EMAIL"];
			unset($_SESSION['SENDER_SUBSCRIBE_LIST']);
