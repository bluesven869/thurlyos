...
if (!$USER->IsAuthorized() || CModule::IncludeModule('extranet') && !CExtranet::IsIntranetUser($SITE_ID) && !$USER->IsAdmin() || !check_thurly_sessid())
	die();
...
$win_id = htmlspecialcharsbx($win_id);
...
<table cellspacing="0" width="100%">
	<tr>
		<td width="100%" class="title-text" onmousedown="jsFloatDiv.StartDrag(arguments[0], document.getElementById('<?echo $win_id?>'));"><?echo GetMessage('INTR_EMP_WINDOW_TITLE')?></td>
		<td width="0%"><a class="close" href="javascript:document.getElementById('<?echo $win_id?>').__object.CloseDialog();" title="<?=GetMessage("INTR_EMP_WINDOW_CLOSE")?>"></a></td>
	</tr>
</table>
</div>
<script>
...
function BXLoadEmployees(SECTION_ID, bShowOnly, bScrollToSection)
{
	...
	{
		var url = '/thurly/components/thurly/intranet.user.search/ajax.php?lang=<?echo LANGUAGE_ID?>&MODE=EMPLOYEES&SECTION_ID=' + SECTION_ID + '&SITE_ID=<?=$SITE_ID?>' + '&IS_EXTRANET=<?=(($bExtranet) ? "Y" : "N")?>&sessid=<?=thurly_sessid();?>&nt=<?=urlencode($nameTemplate);?>';
...
var jsEmpSearch = {
	...
	Run: function()
	{
		...
		if (jsEmpSearch._control.value && jsEmpSearch._control.value != jsEmpSearch._control.value_tmp)
		{
			...
			jsUtils.loadJSFile('/thurly/components/thurly/intranet.user.search/ajax.php?lang=<?echo LANGUAGE_ID?>&MODE=SEARCH&search=' + encodeURIComponent(jsEmpSearch._value) + '&SITE_ID=<?=$SITE_ID?>' + '&IS_EXTRANET=<?=(($bExtranet) ? "Y" : "N")?>&sessid=<?=thurly_sessid();?>&nt=<?=urlencode($nameTemplate);?>');
		}
	},
...
<div class="buttons">
	...
	<input type="button" value="<?echo GetMessage('INTR_EMP_CANCEL')?>" onclick="document.getElementById('<?echo $win_id?>').__object.CloseDialog();" title="<?echo GetMessage('INTR_EMP_CANCEL_TITLE')?>" />
</div>