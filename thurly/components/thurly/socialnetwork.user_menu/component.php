...
else
{
	...
	if (!CSocNetUser::CanProfileView($USER->GetID(), $arResult["User"], SITE_ID, \Thurly\Socialnetwork\ComponentHelper::getUrlContext()))
	{
		$arResult["User"] = false;
	}
	...
	else
	{
		...
		$arResult["Urls"]["Edit"] = \Thurly\Socialnetwork\ComponentHelper::addContextToUrl(CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_USER_EDIT"], array("user_id" => $arResult["User"]["ID"])), $arContext);
		...
		$arResult["Urls"]["Main"] = \Thurly\Socialnetwork\ComponentHelper::addContextToUrl(CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_USER"], array("user_id" => $arResult["User"]["ID"])), $arContext);
		...
		$arExternalAuthId = \Thurly\Socialnetwork\ComponentHelper::checkPredefinedAuthIdList(array('replica', 'bot', 'email', 'imconnector'));
		...
		{
			$diskEnabled = CModule::includeModule('disk') && \Thurly\Disk\Driver::isSuccessfullyConverted();
			if($diskEnabled)
			{
		...
		$externalAuthIdPerms = \Thurly\Socialnetwork\Util::getPermissionsByExternalAuthId($arResult["User"]["EXTERNAL_AUTH_ID"]);
