...
if(!function_exists('__CrmContactStExportEndResonse'))
{
	function __CrmContactStExportEndResonse($result)
	{
...
{
	__CrmContactStExportEndResonse(array('ERROR' => 'Site ID is not specified.'));
}


if (!CModule::IncludeModule('crm'))
{
	__CrmContactStExportEndResonse(array('ERROR' => 'Could not include crm module.'));
}
...
if ($action === 'STEXPORT')
{
	...
	if($entityTypeName === '')
	{
		__CrmContactStExportEndResonse(array('ERROR' => 'Entity type is not specified.'));
	}

	$exportType = isset($params['EXPORT_TYPE']) ? $params['EXPORT_TYPE'] : '';
	if(!in_array($exportType, array('csv', 'excel', true)))
	{
		__CrmContactStExportEndResonse(
	...
	{
		__CrmContactStExportEndResonse(array('ERROR' => 'Undefined entity type is specified.'));
	}
	...
	{
		__CrmContactStExportEndResonse(array('ERROR' => 'Access denied.'));
	}
	...
	if($processToken === '')
	{
		__CrmContactStExportEndResonse(array('ERROR' => 'Process token is not specified.'));
	}
	...
	do
	{
		...
		{
			__CrmContactStExportEndResonse(array('ERROR' => 'Progress data is incorrect.'));
		}
		...
		if (is_array($cResult))
		{
			...
			{
				__CrmContactStExportEndResonse(array('ERROR' => $cResult['ERROR']));
			}
			else
			{
	...
	{
		$fileUrl = SITE_DIR.'thurly/components/thurly/crm.contact.list/stexport.php?type='.$exportType;
		CUserOptions::DeleteOption('crm', 'crm_stexport_contact');
		__CrmContactStExportEndResonse(
			array(
				'STATUS' => 'COMPLETED',
