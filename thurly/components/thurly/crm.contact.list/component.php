...
if (!$isErrorOccured && !empty($sExportType) && $CCrmContact->cPerms->HavePerm('CONTACT', BX_CRM_PERM_NONE, 'EXPORT'))
{
	$errorMessage = GetMessage('CRM_PERMISSION_DENIED');
	$isErrorOccured = true;
}
...
{
	AddEventHandler('crm', 'onCrmContactListItemBuildMenu', array('\Thurly\Crm\CallList\CallList', 'handleOnCrmContactListItemBuildMenu'));
}
...
if (!$bInternal)
{
	...
	$arResult['FILTER'] = array(
		...
		array('id' => 'TYPE_ID', 'params' => array('multiple' => 'Y'), 'name' => GetMessage('CRM_COLUMN_TYPE'), 'type' => 'list', 'items' => CCrmStatus::GetStatusList('CONTACT_TYPE')),
	...
	{
		$arResult['HEADERS']['SKIP_HEADERS'] = array('ADDRESS', 'ADDRESS_2', 'ADDRESS_CITY', 'ADDRESS_REGION', 'ADDRESS_PROVINCE');
	}
...
Thurly\Crm\Search\SearchEnvironment::convertEntityFilterValues(CCrmOwnerType::Contact, $arFilter);
...
{
	$pageSize = $arParams['CONTACT_COUNT'];
}
else
{
...
else
{
	...
	if(!empty($addressSort))
	{
		...
		$navDbResult = \Thurly\Crm\ContactAddress::getEntityList(
...
if (!$isInExportMode)
{
	...
	if(!$bInternal)
	{
		...
		$arResult['NEED_FOR_BUILD_TIMELINE'] = \Thurly\Crm\Agent\Timeline\ContactTimelineBuildAgent::getInstance()->isEnabled();

		$agent = Thurly\Crm\Agent\Duplicate\ContactDuplicateIndexRebuildAgent::getInstance();
