				...
				<span class="idea-rating-block-right">
					<span class="idea-rating-block-content idea-rating-block-content-ext-<?=$arParams['RATING_TEMPLATE']?>">
						...
						<?$APPLICATION->IncludeComponent(
							"thurly:rating.vote", $arParams['RATING_TEMPLATE'],
							Array(
			...
			<div class="blog-qtr">
				<div class="blog-idea-body">
					<div class="idea-owner">
						...
						?><noindex><?$APPLICATION->IncludeComponent("thurly:main.user.link",
								'',
								array(
						...
						<div class="idea-post-meta">
							<div class="idea-post-meta-util">
								<?if(strLen($CurPost["urlToHide"])>0):?>
									<a href="<?=$CurPost["urlToHide"]?>" onclick="if(confirm('<?=GetMessageJS("BLOG_MES_HIDE_POST_CONFIRM")?>')){this.href+='&sessid='+BX.thurly_sessid(); return true;}return false;"><span class="idea-post-link-caption"><?=GetMessage("BLOG_MES_HIDE")?></span></a>
								<?elseif(strLen($CurPost["urlToShow"])>0):?>
									<a href="<?=$CurPost["urlToShow"]?>" onclick="if(confirm('<?=GetMessageJS("IDEA_MES_SHOW_POST_CONFIRM")?>')){this.href+='&sessid='+BX.thurly_sessid(); return true;}return false;"><span class="idea-post-link-caption"><?=GetMessage("IDEA_MES_SHOW")?></span></a>
								<?endif;?>
								...
								<?if(strLen($CurPost["urlToDelete"])>0):?>
									<a href="<?=$CurPost["urlToDelete"]?>" onclick="if(confirm('<?=GetMessageJS("BLOG_MES_DELETE_POST_CONFIRM")?>')){this.href+='&sessid='+BX.thurly_sessid(); return true;}return false;"><span class="idea-post-link-caption"><?=GetMessage("BLOG_MES_DELETE")?></span></a>
								<?endif;?>
							</div>
							<br clear="both"/>
						</div>
