...
class CDiskBizprocListComponent extends BaseComponent
{
	...
	protected function prepareParams()
	{
		...
		$this->arParams['DOCUMENT_TYPE'] = \Thurly\Disk\BizProcDocument::generateDocumentType($this->arParams['STORAGE_ID']);
	...
	protected function getTemplateData()
	{
		...
		foreach($this->arParams['DOCUMENT_DATA'] as $nameModule => $data)
		{
			...
			if ($result)
			{
				...
				$adminPage = $this->application->getCurPageParam('&action=delete&'.thurly_sessid_get(),
				...
				while ($res = $result->getNext())
				{
					...
					$url = $this->application->getCurPageParam('ID='.$res['ID'].'&action=create'.($presentCreate ? 'present' : '').'&'.thurly_sessid_get(),
						array('back_url', 'action', 'ID', 'sessid'));
					$presentEdit = (($res["~AUTO_EXECUTE"] & CBPDocumentEventType::Edit) != 0);
					$url1 = $this->application->getCurPageParam('ID='.$res["ID"].'&action=edit'.($presentEdit ? 'present' : '').'&'.thurly_sessid_get(),
						array('back_url', 'action', 'ID', 'sessid'));
