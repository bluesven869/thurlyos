...
$documentData = array(
	'DISK' => array(
		'DOCUMENT_TYPE' => \Thurly\Disk\BizProcDocument::generateDocumentComplexType($arParams["STORAGE_ID"]),
	),
	'WEBDAV' => array(
		'DOCUMENT_TYPE' => \Thurly\Disk\BizProcDocumentCompatible::generateDocumentComplexType($arParams["STORAGE_ID"]),
	)
);
$arResult["STORAGE_ID"] = \Thurly\Disk\BizProcDocument::generateDocumentType($arParams["STORAGE_ID"]);
...
foreach($documentData as $nameModule => $data)
{
	...
	if ($dbRes)
	{
		...
		$adminPage = $APPLICATION->GetCurPageParam(
			'back_url='.urlencode($back_url).'&action=delete&'.thurly_sessid_get(),
		...
		while ($res = $dbRes->GetNext())
		{
			...
			$url = $APPLICATION->GetCurPageParam('ID='.$res["ID"].'&action=autoload_create'.($b ? '_n' : '').'&'.thurly_sessid_get(),
				array('back_url', 'action', 'ID', 'sessid'));
			$b1 = (($res["~AUTO_EXECUTE"] & CBPDocumentEventType::Edit) != 0);
			$url1 = $APPLICATION->GetCurPageParam('ID='.$res["ID"].'&action=autoload_edit'.($b1 ? '_n' : '').'&'.thurly_sessid_get(),
				array('back_url', 'action', 'ID', 'sessid'));
