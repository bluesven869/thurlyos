...
if ($USER->IsAuthorized() && check_thurly_sessid())
{
	...
	if($actionSaveSettings || $actionEnable)
	{
			...
			&& \Thurly\Main\Loader::includeModule('crm'))
		{
			...
			$canEditPreset = \Thurly\Crm\EntityPreset::checkUpdatePermission();
	...
	else if($_REQUEST["action"] == "savelastselectedpreset")
	{
		...
		if ($entityTypeId > 0 && \Thurly\Main\Loader::includeModule('crm'))
		{
			$requisite = new \Thurly\Crm\EntityRequisite();
	...
	else if($_REQUEST["action"] == "savelastselectedrequisite")
	{
		...
		if ($entityTypeId > 0 && $entityId > 0 && $requisiteId > 0 && \Thurly\Main\Loader::includeModule('crm'))
		{
			$requisite = new \Thurly\Crm\EntityRequisite();
