...
if ($_POST['mfi_mode'])
{
	...
	if (!$cid || !preg_match('/^[a-f01-9]{32}$/', $cid) || !check_thurly_sessid())
		die();
			...
			if (strlen($res) <= 0)
			{
				...
				if ($fileID)
				{
					...
					if ($file)
					{
						...
						$tmp["fileURL"] = CHTTP::URN2URI($APPLICATION->GetCurPageParam("mfi_mode=down&fileID=".$fileID."&cid=".$cid."&".thurly_sessid_get(), array("mfi_mode", "fileID", "cid")));
