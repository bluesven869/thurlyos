...
if (isset($_REQUEST['import']) && isset($_SESSION['CRM_IMPORT_FILE']))
{
	...
	$import = new Thurly\Crm\Import\VCardImport();
	...
	$reader = new Thurly\Crm\VCard\VCardReader($_SESSION['CRM_IMPORT_FILE'], $_SESSION['CRM_IMPORT_FILE_POS']);
	$reader->open();
	while($reader->readElement())
	{
		$element = $reader->getElement();
		if(!$import->mapContact($element, $mappedFields))
		{
			$processedQty++;
		...
		$mappedFields = $import->getMappedFields();
			...
			&& ($enableDupCtrlByPerson || $enableDupCtrlByPhone || $enableDupCtrlByEmail))
		{
			...
			$adapter = \Thurly\Crm\EntityAdapterFactory::create($fields, CCrmOwnerType::Contact);
		...
		else
		{
			if (isset($fields['COMPANY_TITLE']) && $fields['COMPANY_TITLE'] !== '')
			{
				...
				$dbResult = CCrmCompany::GetListEx(array(), array('TITLE' => $companyTitle), false, false, array('ID'));
	...
	if($arResult['error'] > 0)
	{
		$arResult['errata_url'] = SITE_DIR.'thurly/components/thurly/crm.contact.import.vcard/show_file.php?name=errata';
	}

	if($enableDupFile && $arResult['duplicate'] > 0)
	{
		$arResult['duplicate_url'] = SITE_DIR.'thurly/components/thurly/crm.contact.import.vcard/show_file.php?name=duplicate';
	}
...
$paramValue = Thurly\Crm\Settings\ContactSettings::getCurrent()->areOutmodedRequisitesEnabled() ? 'N' : 'Y';
