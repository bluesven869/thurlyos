	...
	implements IBPEventActivity, IBPActivityExternalEventListener
{
	...
	public function Execute()
	{
		...
		foreach (array_keys($arFields) as $fieldName)
		{
			if (
				...
				||
				\Thurly\Tasks\Util\Userfield::isUFKey($fieldName)
			)
			{
				if('UF_TASK_WEBDAV_FILES' == $fieldName && is_array($arFields[$fieldName]))
				{
					foreach($arFields[$fieldName] as $key => $fileId)
					{
						...
						{
							if(CModule::IncludeModule("disk") && \Thurly\Disk\Configuration::isSuccessfullyConverted())
							{
								$item = \Thurly\Disk\Internals\FileTable::getList(array(
									'select' => array('ID'),
									'filter' => array('=XML_ID' => $fileId, 'TYPE' => \Thurly\Disk\Internals\FileTable::TYPE_FILE)
								))->fetch();
		...
		\Thurly\Tasks\Util\User::setOccurAsId($arFieldsChecked['CREATED_BY']);
		...
		try
		{
			// todo: use \Thurly\Tasks\Item\Task here
			$task = CTaskItem::add($arFieldsChecked, \Thurly\Tasks\Util\User::getAdminId());
			$result = $task->getId();
		}
		...
		if (!is_array($arCurrentValues))
		{
				...
				&& is_array($arCurrentActivity["Properties"]["Fields"]))
			{
				foreach ($arCurrentActivity["Properties"]["Fields"] as $k => $v)
				{
					...
					if('UF_TASK_WEBDAV_FILES' == $k && is_array($arCurrentValues[$k]) && CModule::IncludeModule("disk") && \Thurly\Disk\Configuration::isSuccessfullyConverted())
					{
	...
	public static function GetPropertiesDialogValues($documentType, $activityName, &$arWorkflowTemplate, &$arWorkflowParameters, &$arWorkflowVariables, $arCurrentValues, &$arErrors)
	{
		...
		$arUserFields = \Thurly\Tasks\Util\Userfield\Task::getScheme();
