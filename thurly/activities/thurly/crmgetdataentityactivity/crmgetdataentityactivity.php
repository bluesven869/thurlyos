...
class CBPCrmGetDataEntityActivity extends CBPActivity
{
	...
	protected function getEntityData()
	{
		...
		switch($this->EntityType)
		{
			...
			case 'CONTACT':
			{
				...
				$entityData['COMPANY_IDS'] = \Thurly\Crm\Binding\ContactCompanyTable::getContactCompanyIDs($entityId);
				...
				if($printableVersion)
				{
					...
					if($entityData['COMPANY_ID'])
					{
						$objectResult = CCrmCompany::getList(
							array('ID' => 'DESC'),
							array('=ID' => $entityData['COMPANY_ID'], 'CHECK_PERMISSIONS' => 'N'),
							array('TITLE')
						);
					...
					if($entityData['COMPANY_IDS'])
					{
						$objectResult = CCrmCompany::getList(
							array('ID' => 'DESC'),
							array('=ID' => $entityData['COMPANY_IDS'], 'CHECK_PERMISSIONS' => 'N'),
							array('TITLE')
						);
						$listCompanyName = array();
					...
					if(!empty($entityData['PHOTO']))
					{
						...
						if (is_array($entityData['PHOTO']))
						{
							foreach ($entityData['PHOTO'] as $r)
							{
								...
								{
									$photo[] = "[url=/thurly/tools/bizproc_show_file.php?f=".
						...
						else
						{
							...
							if ($arImg = $dbImg->fetch())
							{
								$photo = "[url=/thurly/tools/bizproc_show_file.php?f=".urlencode($arImg["FILE_NAME"]).
			...
			case 'DEAL':
			{
				...
				$entityData['CONTACT_IDS'] = \Thurly\Crm\Binding\DealContactTable::getDealContactIDs($entityId);
				$entityData['CATEGORY_ID'] = isset($entityData['CATEGORY_ID'])
					? $entityData['CATEGORY_ID'] : CCrmDeal::getCategoryID($entityId);
				if($printableVersion)
				{
					$stageList = Thurly\Crm\Category\DealCategory::getStageList(
					...
					if($entityData['CONTACT_ID'])
					{
						$objectResult = CCrmContact::getList(
							array('ID' => 'DESC'),
							array('=ID' => $entityData['CONTACT_ID'], 'CHECK_PERMISSIONS' => 'N'),
							array('FULL_NAME')
						);
					...
					if($entityData['CONTACT_IDS'])
					{
						$objectResult = CCrmContact::getList(
							array('ID' => 'DESC'),
							array('=ID' => $entityData['CONTACT_IDS'], 'CHECK_PERMISSIONS' => 'N'),
							array('FULL_NAME')
						);
						$listContactName = array();
	...
	protected static function renderEntityFields($entityType, $currentValues = array())
	{
		$html = '';
	...
	protected static function getEntityFields($entityType)
	{
		...
		switch($entityType)
		{
			case 'LEAD':
				...
				break;
			case 'CONTACT':
				$listIgnoreFieldId = array('LEAD_ID', 'ORIGIN_ID', 'ORIGINATOR_ID');
				$listIgnoreAddressFieldId = array();
				$preparedFields = CCrmDocumentContact::getEntityFields($entityType);
				if(Thurly\Crm\Settings\ContactSettings::getCurrent()->areOutmodedRequisitesEnabled())
				{
					$listIgnoreAddressFieldId = array('ADDRESS', 'ADDRESS_2', 'ADDRESS_CITY', 'ADDRESS_POSTAL_CODE',
				...
				break;
			case 'COMPANY':
				$listIgnoreFieldId = array('LEAD_ID', 'ORIGIN_ID', 'ORIGINATOR_ID', 'CONTACT_ID');
				$listIgnoreAddressFieldId = array();
				$preparedFields = CCrmDocumentCompany::getEntityFields($entityType);
				if(Thurly\Crm\Settings\CompanySettings::getCurrent()->areOutmodedRequisitesEnabled())
				{
					$listIgnoreAddressFieldId = array('ADDRESS', 'ADDRESS_LEGAL');
				}
				$listIgnoreFieldId = array_merge($listIgnoreFieldId, $listIgnoreAddressFieldId);
				...
				break;
			case 'DEAL':
				$listIgnoreFieldId = array('LEAD_ID', 'ORIGIN_ID', 'ORIGINATOR_ID', 'EVENT_DATE',
		...
		$userFields = array('CREATED_BY', 'CREATED_BY_ID', 'MODIFY_BY', 'MODIFY_BY_ID', 'ASSIGNED_BY', 'ASSIGNED_BY_ID');
	...
	protected function getMultiValue($entityType, &$entityData)
	{
		...
		$objectResult = CCrmFieldMulti::getList(
			array('ID' => 'asc'),
			array('ENTITY_ID' => $entityType, 'ELEMENT_ID' => $entityData['ID'])
		);
		$entityData['FM'] = array();
		while($multiFields = $objectResult->fetch())
		{
			$entityData['FM'][$multiFields['TYPE_ID']][$multiFields['ID']] = array(
	...
	protected function getUfValue(CCrmUserType $CCrmUserType, $entityType, &$entityData, $selectedUfFields, $printableVersion)
	{
		...
		{
			if(!in_array($ufField['FIELD_NAME'], $selectedUfFields)) continue;
			if($printableVersion)
			{
				switch($ufField['USER_TYPE_ID'])
				{
					...
					case 'enumeration':
					{
						...
						$enumQuery = $enumObject->getList(array(), array('USER_FIELD_ID' => $ufField['ID']));
						$realListValue = array();
			...
			else
			{
				if(is_array($ufField['VALUE']))
				{
					$entityData[$ufField['FIELD_NAME']] = implode(', ', $ufField['VALUE']);
				}
				else
				{
