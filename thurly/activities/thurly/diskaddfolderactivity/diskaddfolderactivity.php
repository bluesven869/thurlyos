	...
	extends CBPActivity
{
	...
	private function getTargetFolder($entityType, $entityId)
	{
		...
		{
			return \Thurly\Disk\Folder::loadById($entityId);
		}
		...
		{
			case 'user':
				$entityType = \Thurly\Disk\ProxyType\User::className();
				$entityId = CBPHelper::ExtractUsers($entityId, $documentId, true);
				break;
			case 'sg':
				$entityType = \Thurly\Disk\ProxyType\Group::className();
				break;
			case 'common':
				$entityType = \Thurly\Disk\ProxyType\Common::className();
				break;
			default:
				$entityType = null;
		}
	...
	public function Execute()
	{
		...
		$folderName = \Thurly\Disk\Ui\Text::correctFolderName($this->FolderName);
		...
		if (!$createdBy)
			$createdBy = \Thurly\Disk\SystemUser::SYSTEM_USER_ID;

		$newFolder = $folder->addSubFolder(array('NAME' => $folderName, 'CREATED_BY' => $createdBy));
		if(!$newFolder && $folder->getErrorByCode(\Thurly\Disk\BaseObject::ERROR_NON_UNIQUE_NAME))
		{
			$newFolder = \Thurly\Disk\Folder::load(array(
				'=NAME' => $folderName,
