...
<? if ($selectedRecipientType === CBPCrmSendSmsActivity::RECIPIENT_TYPE_USER):
	$config = array(
		'valueInputName' => $recipientUser['FieldName'],
		'selected' => \Thurly\Crm\Automation\Helper::prepareUserSelectorEntities(
			$dialog->getDocumentType(),
			$dialog->getCurrentValue($recipientUser['FieldName'], $recipientUser['Default'])
		),
		'multiple' => (bool)$recipientUser['Multiple'],
		'required' => (bool)$recipientUser['Required'],
	);
...
<script>
	BX.ready(function()
	{
		...
		var providers = <?=\Thurly\Main\Web\Json::encode($data['providers'])?>;
