		...
		while($obElement = $dbIBlockElement->GetNextElement())
		{
			...
			?><div class="bx-gadgets-text" style="clear: both; padding: 0 0 10px 0;"><?
				...
				if (strlen($arGadgetParams["TITLE_FIELD"]) > 0)
				{
					...
					{
						?><a href="/thurly/admin/iblock_element_edit.php?ID=<?=$arIBlockElement["ID"]?>&type=<?=$arGadgetParams["IBLOCK_TYPE"]?>&IBLOCK_ID=<?=$arGadgetParams["IBLOCK_ID"]?>&lang=<?=LANGUAGE_ID?>"><?=$strTitle?></a><br><?
					}
				}
