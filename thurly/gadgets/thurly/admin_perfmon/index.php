...
$APPLICATION->SetAdditionalCSS('/thurly/gadgets/thurly/admin_perfmon/styles.css');
...
if($bPerfmonModuleInstalled):
	...
	if($mark_value > 0):
		...
		$text2 = str_replace(array("#STARTLINK#", "#ENDLINK#"), ($GLOBALS["APPLICATION"]->GetGroupRight("perfmon") >= "W" ? array('<a href="/thurly/admin/perfmon_panel.php?lang='.LANGUAGE_ID.'">', '</a>') : array('', '')), GetMessage("GD_PERFMON_NO_RES"));
	endif;
else:
...
?><div class="bx-gadget-bottom-cont<?=(
	...
	if (!$bPerfmonModuleInstalled)
	{
		...
		{
			?><a class="bx-gadget-button bx-gadget-button-clickable" href="/thurly/admin/module_admin.php?id=perfmon&install=Y&lang=<?=LANGUAGE_ID?>&<?=thurly_sessid_get()?>">
	...
	else
	{
		...
		{
			?><a class="bx-gadget-button bx-gadget-button-clickable<?=($mark_value > 0 ? " bx-gadget-button-active" : "")?>" href="/thurly/admin/perfmon_panel.php?lang=<?=LANGUAGE_ID?>">
				<div class="bx-gadget-button-lamp"></div>
