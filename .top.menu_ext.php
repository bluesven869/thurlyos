...
IncludeModuleLangFile($_SERVER["DOCUMENT_ROOT"]."/thurly/modules/intranet/public/.top.menu_ext.php");
...
if (CModule::IncludeModule("intranet") && CIntranetUtils::IsExternalMailAvailable())
{
	$arMenuB24[] = array(
		...
		array(
			...
			"warning_link" => SITE_DIR."company/personal/mail/?config",
...
foreach ($aMenuLinks as $arItem)
{
	...
	if (preg_match("~/(workgroups|crm|marketplace|docs|timeman|bizproc|company)/$~i", $menuLink))
	{
		continue;
	}
...
$arMenuB24[] = array(
	...
	'CModule::IncludeModule("imopenlines") && \Thurly\ImOpenlines\Security\Helper::isMainMenuEnabled()'
);
...
$arMenuB24[] = array(
	...
	'CModule::IncludeModule("voximplant") && Thurly\Voximplant\Security\Helper::isMainMenuEnabled()'
);
