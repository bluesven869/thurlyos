...
if ((isset($_REQUEST['paySystemId']) || isset($_REQUEST['invoice_id'])) && (!isset($_REQUEST['initiate_pay']) || $_REQUEST['initiate_pay'] != 'Y'))
{
	...
	{
		include ($_SERVER['DOCUMENT_ROOT'].'/thurly/components/thurly/crm.invoice.payment.client/ajax.php');
	}
	else
	{
...
else
{
	...
	global $APPLICATION;
	$APPLICATION->IncludeComponent("thurly:crm.invoice.payment.client", ".default", array(
